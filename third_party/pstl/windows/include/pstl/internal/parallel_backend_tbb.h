...
void downsweep(_Index __i, _Index __m, _Index __tilesize, _Tp* __r, _Index __lastsize, _Tp __initial, _Cp __combine, _Sp __scan) {
    if (__m == 1)
        ...
        tbb::parallel_invoke(
                ...
                [=, &__combine] { par_backend::downsweep(__i + __k, __m - __k, __tilesize, __r + __k, __lastsize,
...
tbb::task* stable_sort_task<_RandomAccessIterator1, _RandomAccessIterator2, _Compare, _LeafSort>::execute() {
    ...
    if (__n <= __sort_cut_off) {
        ...
        auto __move_sequences = [](_RandomAccessIterator2 __first1, _RandomAccessIterator2 __last1, _RandomAccessIterator1 __first2) {return std::move(__first1, __last1, __first2); };
        if (_M_inplace == 2)
            __m = new (allocate_continuation()) merge_task<_RandomAccessIterator2, _RandomAccessIterator2, _RandomAccessIterator1, _Compare, serial_destroy, serial_move_merge<decltype(__move_values), decltype(__move_sequences)>>(_M_zs, __zm, __zm, __ze, _M_xs, _M_comp, serial_destroy(), serial_move_merge<decltype(__move_values), decltype(__move_sequences)>(__nmerge, __move_values, __move_sequences));
        else if (_M_inplace)
            __m = new (allocate_continuation()) merge_task<_RandomAccessIterator2, _RandomAccessIterator2, _RandomAccessIterator1, _Compare, binary_no_op, serial_move_merge<decltype(__move_values), decltype(__move_sequences)>>(_M_zs, __zm, __zm, __ze, _M_xs, _M_comp, binary_no_op(), serial_move_merge<decltype(__move_values), decltype(__move_sequences)>(__nmerge, __move_values, __move_sequences));
        else {
            auto __move_values = [](_RandomAccessIterator1 __x, _RandomAccessIterator2 __z) {*__z = std::move(*__x); };
            auto __move_sequences = [](_RandomAccessIterator1 __first1, _RandomAccessIterator1 __last1, _RandomAccessIterator2 __first2) {return std::move(__first1, __last1, __first2); };
            __m = new (allocate_continuation()) merge_task<_RandomAccessIterator1, _RandomAccessIterator1, _RandomAccessIterator2, _Compare, binary_no_op, serial_move_merge<decltype(__move_values), decltype(__move_sequences)>>(_M_xs, __xm, __xm, _M_xe, _M_zs, _M_comp, binary_no_op(), serial_move_merge<decltype(__move_values), decltype(__move_sequences)>(__nmerge, __move_values, __move_sequences));
        }
        __m->set_ref_count(2);
        task* __right = new(__m->allocate_child()) stable_sort_task(__xm, _M_xe, __zm, !_M_inplace, _M_comp, _M_leaf_sort, __nmerge);
        spawn(*__right);
...
void parallel_stable_sort(_ExecutionPolicy&&, _RandomAccessIterator __xs, _RandomAccessIterator __xe, _Compare __comp, _LeafSort __leaf_sort, std::size_t __nsort = 0) {
    tbb::this_task_arena::isolate([=, &__nsort]() {
        ...
        if (__n > __sort_cut_off) {
            ...
            using tbb::task;
            task::spawn_root_and_wait(*new(task::allocate_root()) stable_sort_task<_RandomAccessIterator, _ValueType*, _Compare, _LeafSort>(__xs, __xe, (_ValueType*)__buf.get(), 2, __comp, __leaf_sort, __nsort));
            return;
        }
        //serial sort
