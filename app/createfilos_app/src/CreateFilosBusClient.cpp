...
void CreateFilosBusClient::onReceivedMessage(const ModGMBusMessage &message) {
  ...
  Printer::instance()->info("[CreateFilos]BusClient > onReceivedMessage [from:'%s' id='%d']",
                            from.toStdString().c_str(), id);
...
void CreateFilosBusClient::get_shared_memory() {
  ...
  Printer::instance()->info("[CreateFilos]BusClient > get_shared_memory [size:%d, path:%s]", size,
