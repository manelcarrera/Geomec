...
const int CDependencies::PRINT_ALL =
    CDependencies::PRINT_PROJECT | CDependencies::PRINT_LIB | CDependencies::PRINT_NOT_FOUND;
...
static const QString MSSDK_IncludePath =
...
void CDependencies::accept_connection()
{
  ...
  //FIXME:
  //connect( m_tcpServerConnection, SIGNAL( error(QAbstractSocket::SocketError)),	this, SLOT( displayError(
...
void CDependencies::clean() { m_projects.clear(); }
...
void CDependencies::new_print_results(CDependencies::CProject::ProjectsType _type) {
  ...
  for (int j = 0; j < number_of_projects.size(); j++) // header
  {
    ...
    line += project_name.left(14).leftJustified(14 + 1 /*DefaultPaddingLength*/,
                                                ' '); // 16: estimated max project name length
    ...
    for (int j = 0; j < number_of_projects.size(); j++) // for each project : x coordenate
    {
      ...
      if (i < _number_of_projects)
        project = _type == CDependencies::CProject::ProjectsType::Dependent ? p->dependent_projects[i]
                  : _type == CDependencies::CProject::ProjectsType::LikedBy
                      ? p->linked_by[i]
...
void CDependencies::print_results(const int what_to_print) {
  ...
  if ((what_to_print & PRINT_PROJECT) == PRINT_PROJECT) {
    qDebug().noquote() << QString("   Name").leftJustified(33, ' ') << QString("h").rightJustified(3, ' ')
                       << QString("d").rightJustified(3, ' ') << QString("dp").rightJustified(3, ' ')
                       << QString("nf").rightJustified(3, ' ') << QString("lb").rightJustified(3, ' ');
    qDebug() << "";
  }
        ...
        if (project->dependencies_not_found.size()) {
          if ((what_to_print & PRINT_PROJECT) == PRINT_PROJECT) // FIXME: not working this filter
          {
            ...
            qDebug().noquote() << "->" << project->name.leftJustified(30, ' ')
                               ...
                               << QString::number(project->linked_by.size())
                                      .rightJustified(
                                          3, ' '); // print numer of headers and dependencies (headers) of the project
          } else {
            qDebug().noquote() << "->" << project->name.leftJustified(30, ' ')
                               ...
                               << "nf:"
                               << QString::number(project->dependencies_not_found.size())
                                      .rightJustified(
                                          3, ' '); // print numer of headers and dependencies (headers) of the project*/
          }
        }
...
QString CDependencies::find_dependent_project(QString &dependency) {
  ...
  while (i.hasNext()) {
    ...
    for (int j = 0; j < p->headers.size(); j++)
      ...
      if (!QString::compare(dependency, p->headers[j].fileName(),
...
QString
CDependencies::get_included_file(QString &_line) // parser of the lines that contain the reserved word "#include"
{
  QString line = _line;
