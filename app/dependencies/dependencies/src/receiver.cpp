...
CReceiver::CReceiver(CThread &_thread) : m_thread(_thread) {
  connect(&m_tcp_server, SIGNAL(newConnection()), this, SLOT(accept_connection()));
  // connect( &m_tcp_server, SIGNAL( acceptError(QAbstractSocket::SocketError socketError) ), this, SLOT( accept_error(
...
void CReceiver::accept_error(QAbstractSocket::SocketError socketError) { qDebug() << socketError; }
