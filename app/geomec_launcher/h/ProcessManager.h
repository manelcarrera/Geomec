...
public slots:
  ...
  void start(const QString &name, const QStringList &args, const QProcessEnvironment *env = nullptr);

  void start(const QString &name, const QProcessEnvironment *env = nullptr);

  void stop_(
      const QString &
          name); // FIXME: why can't be named just 'stop'
