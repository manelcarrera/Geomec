...
QStringList getArgs(quint64 pid)
{
  return QStringList{ QString("pid=%1").arg(pid) };
}


ProcessManager::ProcessManager(quint64 pid, QObject *parent)
  : IConnectionManager(parent)
  , m_pid(pid)
{
}
...
void ProcessManager::start()
{
  ...
  {
    Printer::instance()->info("ProcessManager > start > process: '%s' pid:'%d'", it.key().toStdString().c_str(), m_pid );
...
void ProcessManager::start( const QString& name, 
