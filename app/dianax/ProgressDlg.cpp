...
CProgressDlg::CProgressDlg(CWnd *pParent /*=NULL*/) : CDialog(CProgressDlg::IDD, pParent) {
...
void CProgressDlg::AddMessage(CString Message) {
  ...
  if (!pCtrl->m_bProgressIsLifesign)
    pCtrl->StepProgressBar();
  CallBack();
}
...
CDianaXCtrl *CProgressDlg::GetControl() { return CDianaXCtrl::GetMe(); }
