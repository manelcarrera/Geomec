...
CDianaXCtrl *GetControl() { return CDianaXCtrl::GetMe(); }
...
int RunDiana(CString Exe, CString Dat, CString Com, CString Filos, CString Base, BOOL DisplayDefMessages,
  ...
  InstallDCMessageHandler(ASK_DC_HANDLE_ERRMSG, ASK_DC_TERMINATE_ERRMSG, DisplayDianaErrorMessage);

  InstallDCMessageHandler(ASK_DC_APPSPC_MSG, NULL, UserMessageHandler);

  if (!UserLeader.IsEmpty() && !UserTrailer.IsEmpty()) {
    InstallDCMessageHandler(UserLeader.GetBuffer(UserLeader.GetLength()),
                            UserTrailer.GetBuffer(UserTrailer.GetLength()), UserMessageHandler);
...
static int UserMessageHandler(char *txt) {
  CProgressDlg *pDlg = GetControl()->m_pDlg;
  CArray<CString, CString> Lines;
  GetSeparateStrings(txt, Lines);
  for (int i = 0; i < Lines.GetSize(); ++i) {
    if (pDlg)
      pDlg->AddMessage(Lines[i]);
    GetControl()->DoFireOnDiaMessage(Lines[i]);
    if (Lines[i].Find(GetControl()->m_strSucceedMessage) != -1)
      GetControl()->m_bCalculationSucceeded = TRUE;
  }
...
static int ApplicationSpecLogging(char *txt) { return 0; }
...
static int UserFunc() {
  ...
  if (!rv)
    GetControl()->DoFireOnCallback(&rv);
...
static void DefMsgHandler(char *buf, int size) {
  if (size <= 0)
    return;
