...
DISP_PROPERTY_EX_ID(CDianaXCtrl, "SuccessMessageString", dispidSuccessMessageString, GetSuccessMessageString,
                    SetSuccessMessageString, VT_BSTR)
DISP_PROPERTY_EX_ID(CDianaXCtrl, "NumberOfMessages", dispidNumberOfMessages, GetNumberOfMessages, SetNumberOfMessages,
                    VT_I4)
DISP_PROPERTY_EX_ID(CDianaXCtrl, "DisplayStopMessage", dispidDisplayStopMessage, GetDisplayStopMessage,
                    SetDisplayStopMessage, VT_BOOL)
DISP_PROPERTY_EX_ID(CDianaXCtrl, "ShowDefaultMessages", dispidShowDefaultMessages, GetShowDefaultMessages,
                    SetShowDefaultMessages, VT_BOOL)
DISP_FUNCTION_ID(CDianaXCtrl, "SetEnvironmentVar", dispidSetEnvironmentVar, SetEnvironmentVar, VT_BOOL,
                 VTS_BSTR VTS_BSTR)
DISP_FUNCTION_ID(CDianaXCtrl, "ExecuteDiana", dispidExecuteDiana, ExecuteDiana, VT_I2,
                 VTS_BSTR VTS_BSTR VTS_BSTR VTS_BSTR VTS_BSTR VTS_BSTR)
DISP_FUNCTION_ID(CDianaXCtrl, "GetCalculationResult", dispidGetCalculationResult, GetCalculationResult, VT_BOOL,
                 VTS_NONE)
...
DISP_PROPERTY_EX_ID(CDianaXCtrl, "AutoCloseDialog", dispidAutoCloseDialog, GetAutoCloseDialog, SetAutoCloseDialog,
                    VT_BOOL)

// version 2.1
DISP_PROPERTY_EX_ID(CDianaXCtrl, "ProgressIsLifesign", dispidProgressIsLifesign, GetProgressIsLifesign,
                    SetProgressIsLifesign, VT_BOOL)

// version 2.2
DISP_PROPERTY_EX_ID(CDianaXCtrl, "NumberOfAppSpcMessages", dispidNumberOfAppSpcMessages, GetNumberOfAppSpcMessages,
                    SetNumberOfAppSpcMessages, VT_I4)

// vesrsion 2.3
DISP_PROPERTY_EX_ID(CDianaXCtrl, "AlwaysAutoCloseDialog", dispidAlwaysAutoCloseDialog, GetAlwaysAutoCloseDialog,
                    SetAlwaysAutoCloseDialog, VT_BOOL)
END_DISPATCH_MAP()
...
IMPLEMENT_OLECREATE_EX(CDianaXCtrl, "DIANAX.DianaXCtrl.1", 0x3a01bc17, 0xba9f, 0x11d2, 0xb3, 0xb4, 0, 0x10, 0x4b, 0x25,
                       0x56, 0xd9)
...
const IID BASED_CODE IID_DDianaX = {0x3a01bc15, 0xba9f, 0x11d2, {0xb3, 0xb4, 0, 0x10, 0x4b, 0x25, 0x56, 0xd9}};
const IID BASED_CODE IID_DDianaXEvents = {0x3a01bc16, 0xba9f, 0x11d2, {0xb3, 0xb4, 0, 0x10, 0x4b, 0x25, 0x56, 0xd9}};

/////////////////////////////////////////////////////////////////////////////
// Control type information

static const DWORD BASED_CODE _dwDianaXOleMisc = OLEMISC_INVISIBLEATRUNTIME | OLEMISC_ACTIVATEWHENVISIBLE |
                                                 OLEMISC_SETCLIENTSITEFIRST | OLEMISC_INSIDEOUT |
...
BOOL CDianaXCtrl::CDianaXCtrlFactory::UpdateRegistry(BOOL bRegister) {
  ...
  if (bRegister)
    return AfxOleRegisterControlClass(AfxGetInstanceHandle(), m_clsid, m_lpszProgID, IDS_DIANAX, IDB_DIANAX,
                                      afxRegApartmentThreading, _dwDianaXOleMisc, _tlid, _wVerMajor, _wVerMinor);
  else
...
CDianaXCtrl *CDianaXCtrl::GetMe() {
  if (s_lstControlStack.GetCount() > 0)
    return s_lstControlStack.GetHead();
  assert(FALSE);
  return NULL;
}
...
void CDianaXCtrl::OnDraw(CDC *pdc, const CRect &rcBounds, const CRect &rcInvalid) {
...
BOOL CDianaXCtrl::GetShowDialog() { return m_bShowDialog; }
...
short CDianaXCtrl::ExecuteDiana(LPCTSTR ExeName, LPCTSTR WorkingDir, LPCTSTR ComFile, LPCTSTR DatFile,
                                LPCTSTR FilosFile, LPCTSTR BaseName) {
  ...
  if (m_bShowDialog) {
    ...
    if (m_nTotalMessages < 1 && m_nTotalAppSpcMessages < 1 && !m_bProgressIsLifesign) {
      ...
      if (!(ProgressBar = m_pDlg->GetDlgItem(IDC_PROGRESS)))
        return -4;
      if (!(ProgressBarText = m_pDlg->GetDlgItem(IDC_TXT_PROGRESS)))
        return -4;
  ...
  int iret = pStart->RunDiana(ExeName, WorkingDir, ComFile, DatFile, FilosFile, BaseName, m_bDisplayDefaultMessages,
                              (LPCSTR)m_strUserLeader, (LPCSTR)m_strUserTrailer);
  m_bChildIsRunning = FALSE;
...
BOOL CDianaXCtrl::GetCalculationResult() { return m_bCalculationSucceeded; }
...
long CDianaXCtrl::GetNumberOfMessages() { return m_nTotalMessages; }
...
long CDianaXCtrl::GetNumberOfAppSpcMessages() { return m_nTotalAppSpcMessages; }
...
BOOL CDianaXCtrl::GetDisplayStopMessage() { return m_bDisplayStopMessage; }
...
BOOL CDianaXCtrl::GetShowDefaultMessages() { return m_bDisplayDefaultMessages; }
...
void CDianaXCtrl::SetDialogCaption(LPCTSTR lpszNewValue) { m_strDialogCaption = lpszNewValue; }
...
void CDianaXCtrl::SetAlwaysAutoCloseDialog(BOOL bNewValue) { m_bAlwaysAutoCloseDialog = bNewValue; }

BOOL CDianaXCtrl::GetAlwaysAutoCloseDialog() { return m_bAlwaysAutoCloseDialog; }

void CDianaXCtrl::SetAutoCloseDialog(BOOL bNewValue) { m_bAutoCloseDialog = bNewValue; }

BOOL CDianaXCtrl::GetAutoCloseDialog() { return m_bAutoCloseDialog; }
...
BOOL CDianaXCtrl::GetProgressIsLifesign() { return m_bProgressIsLifesign; }
