...
const QString APP_VERSION = "RockMech Processor " xstr(GM_MAJOR) "." xstr(GM_MINOR) "." xstr(GM_BUILD) "." xstr(
...
static int Usage_DEPRECATED(const char *argv0) {
  fprintf(stderr,
          "Usage: %s /ROCKMECHFILE:<rockmechfile> <modelfile> [/OUTPUTDIR:<dir>] [/LOG:<filename>] [/TEMPDIR:<dir>]\n",
...
static bool RunRockMech_DEPRECATED(CModelBase &model, const QString &sRockMechFile, const QString &sModelFile,
                                   const QString &sOutputDir, const QString &sTempDir, bool validate,
  ...
  QString destination =
      QDir::toNativeSeparators((sOutputDir.isEmpty() ? sModelPath : sOutputDir) + "/" + sModelFileName);

  copyFile_DEPRECATED(destination, source);

  rmp.executeRockMechCommands_DEPRECATED(nStage, sModelName.toStdString(), &diaexec, destination, bRet, smc, retr,
                                         bSaveModel, validate);
...
bool GetParameter_DEPRECATED(const char *arg, const char *magic, const char **value) {
  if (strlen(arg) > strlen(magic) &&
      (QString::compare(QString(arg).left(int(strlen(magic))), magic, Qt::CaseInsensitive) == 0)) {
...
int main_DEPRECATED(int argc, char *argv[]) {
  ...
  TParameter params[] = {
      {rockmagic, &sRockMechFile}, {outputmagic, &sOutputDir}, {logmagic, &sLog}, {tempmagic, &sTempDir}, {0, 0}};
  ...
  if (sModelFile != 0) {
    ...
    CTetraSubHorizon *top = new CTetraSubHorizon("Top Horizon", false, *pModel);
    CTetraSubHorizon *bottom = new CTetraSubHorizon("Bottom Horizon", false, *pModel);
    CTetraSubBoundary &boundary = dynamic_cast<CTetraModel *>(pModel)->SubBoundary();
  ...
  try {
    ...
    if (!RunRockMech_DEPRECATED(*pModel, strRockMechFile, strModelFile, strOutputDir, strTempDir, validate,
                                sModelFile != 0)) {
