...
DR::DR(IListenerIpc *gi) : IListener(CEvents::DR_EH, {}), m_th(nullptr), m_gi(gi), m_done(false) {
  m_printer->info("new");
}
...
void DR::run_(const QStringList &params) {
  ...
  auto prepare_run = [=]() {
    ...
    //
    int i = 0;
    for (const auto &x : {CDianaStartUp::FFDIR, CDianaStartUp::FF}) {
    ...
    //
    for (const auto &x :
         {CDianaStartUp::BASE, CDianaStartUp::DISPLAY, CDianaStartUp::USER_LEADER, CDianaStartUp::USER_TRAILER}) {
      dsu->var(x, params[i++]);
    }
  ...
  try {
    ...
    if (dsu->quit()) {
      ...
      _e->notify(Cmd(eCmd::RunDiana_Done_, new std::string(std::to_string(res == 0 ? 1 : 0))),
                 //
