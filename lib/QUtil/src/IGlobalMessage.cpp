...
void IGlobalMessage::AttachObserver(IGlobalMessageObserver *observer) { m_observers.push_back(observer); }
...
std::string IGlobalMessage::constructMessage(const std::vector<std::string> &message) {
  std::string newMessage;

  for (std::vector<std::string>::const_iterator begin = message.begin(), end = message.end(); begin != end;) {
    newMessage += *begin;
    ++begin;
...
std::string IGlobalMessage::constructMessage(unsigned int message) { return std::string(); }
