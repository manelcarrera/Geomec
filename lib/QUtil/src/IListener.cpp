...
IListener::IListener(CEvents::eEventsHandler eh_id, std::vector<eCmd> events_v_)

    : m_thread(nullptr), m_eh_id(eh_id) {
  Printer::eModule module = eh_id == CEvents::GuiApp_EH      ? Printer::DocT
                            : eh_id == CEvents::NonGuiApp_EH ? Printer::RunWF
                            : eh_id == CEvents::RA_EH        ? Printer::RunA
                                                             :
                                                             //
                                eh_id == CEvents::DI_EH ? Printer::DiaI
                            : eh_id == CEvents::HB_EH   ? Printer::DiaH
                                                        : // Heartbeat
                                //
                                eh_id == CEvents::GI_EH || eh_id == CEvents::DR_EH ? Printer::DiaE
                                                                                   :
                                                                                   //
                                eh_id == CEvents::Question_EH ? Printer::Doc
                                                              : // FIXME
...
void IListener::push(Cmd cmd) {
  if (cmd.first != Progress && cmd.first != Log && cmd.first != DianaMsg)
...
// for convinience
void IListener::push_(eCmd cmd_id) { push(make_cmd(cmd_id)); }
...
void IListener::clear() { m_queue.clear(); }

int IListener::size() { return m_queue.size(); }
