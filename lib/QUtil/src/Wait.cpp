...
CWait::CWait(CEvents::eEventsHandler eh_id) : m_eh_id(eh_id), m_waiting(false) {
  Printer::eModule module = eh_id == CEvents::NonGuiApp_EH  ? Printer::RunWF
...
bool CWait::wait(int timeout) {
  ...
  if (timeout) {
    ...
    //
    res = m_cond_var.wait_for(mlock, std::chrono::milliseconds(timeout),
                              [=] { return !m_waiting; }); // predicate == false -> continue waiting
