...
QString CEnvironment::get(const QString key) const {
  ...
  QStringList DIANA_VARS = QStringList() << CEnvironment::DIAPATH << CEnvironment::DIASHARE << CEnvironment::DIALIB
                                         << CEnvironment::DIASLIB << CEnvironment::DIAERRPATH << CEnvironment::STP0;
  ...
  if (!b_diana_var || !b_diana_local) {
    ...
    Printer::instance(Printer::Gen)
        ->debug("env : get : getenv : %s -> %s", key.toStdString().c_str(), value.toStdString().c_str());
    //
  ...
  Printer::instance(Printer::Gen)
      ->debug("env : get : %s -> %s ... diana_var: %s ... local: %s", key.toStdString().c_str(),
              value.toStdString().c_str(), b_diana_var ? "yes" : "no", b_diana_local ? "yes" : "no");
  ...
  if (value.isEmpty()) {
    if (key == DIAPATH) {
      ...
      if (key == DianaExeName || key == InputExeName || key == FgvExeName) {
        value = key == DianaExeName   ? defaultDianaExeName
                : key == InputExeName ? defaultInputExeName
                                      : defaultFgvExeName;
