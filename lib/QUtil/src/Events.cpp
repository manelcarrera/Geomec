...
const char *CEvents::m_eh_cp[] = {"GuiApp_EH", "NonGuiApp_EH", "RA_EH",       "DI_EH",       "GI_EH",
                                  "DR_EH",     "HB_EH",        "HB2_EH",      "Question_EH", "Controller_EH",
                                  "Tests_EH",  "TI_EH",        "Undefined_EH"};
...
void CEvents::notify(eCmd cmd, eEventsHandler id) { notify(make_cmd(cmd), id); }
...
void CEvents::broadcast(Cmd cmd) {
  ...
  for (auto const &item : m_eh_map) // (eh id, eh struct)
  {
    ...
    for (auto const &evt_id : eh.events_v) // cmd_v
    {
      ...
      {
        if (cmd_id != Progress && cmd_id != Log && cmd_id != DianaMsg)
...
void CEvents::broadcast(eCmd cmd_id) { broadcast(make_cmd(cmd_id)); }