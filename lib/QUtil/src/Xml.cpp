...
std::string CXml::value( std::string key ){ toUpper( key ); return exist( key ) ? m_map[ key ] : NOT_FOUND; }
...
bool CXml::exist( std::string key ){ toUpper( key ); return m_map.find( key ) != m_map.end(); }
