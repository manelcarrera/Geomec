...
IListenerIpc::IListenerIpc(eType type, const std::string &token_)
  ...
  m_printer_ipc = Printer::instance(type == Server ? Printer::DiaI : Printer::DiaE);
  if (type == Server) {
...
void IListenerIpc::listen() {
  ...
  try {
    ...
    trace_queue *data = m_type == Server ? new (addr) trace_queue : static_cast<trace_queue *>(addr);

    m_data_listen = data;

    m_printer_ipc->debug("listen_ipc : shm : Ok");

    auto timeout = [=](int delay) { return second_clock::universal_time() + seconds(delay); };
...
// for convinience
void IListenerIpc::push_ipc(eCmd cmd_id) { push_ipc(make_cmd(cmd_id)); }
