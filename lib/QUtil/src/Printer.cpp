...
Printer::Config::Config() {
  ...
  std::map<eModule, std::string> key_m = {// general
                                          {Sel, "SEL"},
                                          {Gen, "GEN"},
                                          {App, "APP"},
                                          {Doc, "DOC"},
  ...
  CXml cfg(cfg_file, {},
           {// types
            ...
            // modules
            // general
            key_m[Sel], key_m[Gen], key_m[App], key_m[Doc], key_m[DocT], key_m[Shell], key_m[Tests], key_m[Evt],
  ...
  for (auto const &item : key_m) {
    ...
    m_module_DIE[item.first] = val.empty()
                                   ? DIE{false, false, false, false}
                                   : DIE{true, val.find('D') != std::string::npos, val.find('I') != std::string::npos,
...
Printer::Printer() : m_b_multi_process(false) {
...
std::string datetime_() {
  ...
  char tmp[100];
  std::snprintf(tmp, 100, "[%04u-%02u-%02u %02u:%02u:%02u.%03u]", t.tm_year + 1900, t.tm_mon + 1, t.tm_mday, t.tm_hour,
                t.tm_min, t.tm_sec, static_cast<unsigned>(tp / milliseconds(1)));
  return tmp;
}
...
void Printer::print(const char *cp, eType type) {
  ...
  myfile << datetime_() << m_multi_process_s
         << (type == Debug  ? "[D  ]"
             : type == Info ? "[ I ]"
                            : "[  E]")
         << (m_module_s + " ") << cp << std::endl;
