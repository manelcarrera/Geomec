...
bool CRegistryBase::create( std::string key, std::string value )
{ 
	return modify( key, value ); 
}

std::string CRegistryBase::path()
{ 
	return m_path; 
}
...
CRegistry::CRegistry(	std::string release, 
						std::string build ) 
	...
	:	m_release( release ), 
		...
		CRegistryBase( QString::asprintf( 
			...
			SETTINGS_PATH.c_str(), 
			release.c_str() ).toStdString() )
{
}

CRegistry::eRelease CRegistry::check()
{
	std::string build_reg = m_settings->value( 
		QString::fromStdString( BUILD_KEY ) ).toString().toStdString();

	return build_reg.empty() ? NewRelease : 
		build_reg == m_build ?	Same : 
								NewBuild;
}
