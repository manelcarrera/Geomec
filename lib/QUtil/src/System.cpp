...
std::string CSystem::get_unzip() {
  return QDir::toNativeSeparators(QString::fromStdString(util::GetAppPath()) + "/7za.exe").toStdString();
} // FIXME
...
bool CSystem::unpack(const std::string &package, const std::string &dst_dir, const std::string &files) {
  QString unzip = QString::asprintf("\"%s\" x -y -o\"%s\" \"%s\" %s", get_unzip().c_str(), dst_dir.c_str(),
                                    package.c_str(), files.c_str());
