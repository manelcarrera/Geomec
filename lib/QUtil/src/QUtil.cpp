...
QString QUtil::url( QString path, QString filename )
{
  return QDir::toNativeSeparators( path + "/" + filename );
}
...
std::string QUtil::usr_dir( eUsrDir type_ )
{
  ...
  switch( type_ )
  {
    ...
    case Base:		//dir += "/";						
            break;
    case Goemec:	dir += "/Geomec";			break;
    case Plugins:	dir += "/Geomec/plugins";	break;
    //case Changelog:	dir += "/Geomec/changelog";	break;
    case Temp:		dir += "/Geomec/temp";		break;
  }
  return QDir::toNativeSeparators( dir ).toStdString();
}

std::string QUtil::native_sepatators( std::string s ){ return QDir::toNativeSeparators( QString::fromStdString( s ) ).toStdString(); }
...
taskkill /F /IM %s && \
(tasklist | find \"%s\") && \
>> %s\\kill_processes.txt 2>&1", process_.c_str(), process_.c_str(), DEFAULT_FOLDER.c_str() );
...
int QUtil::token_from_file( const std::string& file_ )
{
  ...
  //Printer::instance()->info( "CDianaStartUp > RunDiana > run_diana_task > pid [file:%s]", file.fileName().toStdString().c_str() );
  ...
  if(file.open(QIODevice::ReadOnly)) 
  {
    ...
    //Printer::instance()->info( "CDianaStartUp > RunDiana > run_diana_task > pid [file:%s pid=%d]", file.fileName().toStdString().c_str(), pid );
  }
  else
  {
    //Printer::instance()->info( "CDianaStartUp > RunDiana > run_diana_task > pid [file:%s NOT EXIST]", file.fileName().toStdString().c_str() );
  }
  return pid;
}
