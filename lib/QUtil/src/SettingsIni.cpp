...
std::vector<QString> url_v(eConfigFileType type_) {
  ...
  return std::vector<QString>{file_DEF, file_VER, file_INST, file_MERGED};
}
...
//
CSettingsIni::CSettingsIni() { m_env_DIAPATH = getenv("DIAPATH"); }
...
QString CSettingsIni::version_str() {
  return QString::asprintf("%d.%d.%d.%d_%s", GM_MAJOR, GM_MINOR, GM_BUILD, GM_REVISION,
