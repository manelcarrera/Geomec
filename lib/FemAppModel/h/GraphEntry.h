...
public:
  ...
  QString createUniqueName(const QString& defaultName, size_t initialNumber)
    const;
  std::pair <QString, QString> createUniqueNames(const QString& firstName,
    const QString& secondName, size_t initialNumber) const;
...
template<class T>
class CGraphEntryTemp : public CGraphEntry
{
public:
...
public:
	...
	virtual TEntryNodeSet GraphEntryNodes() const
	{
		...
		for(typename TNodeSet::const_iterator it = m_stNode.begin();
			it != m_stNode.end(); it++)
			stRet.insert(*it);
  ...
  bool LinkNodeToIndex(CGraphNode& node, int nIndex)
  {
	...
	const TNodeSet& EntryNodes() const
	{
		return m_stNode;
	}
	...
	virtual bool Destroy()
	{
		...
		for(typename TNodeSet::iterator it = stNodes.begin();
			it != stNodes.end(); it++)
			if( (*it)->CanDestroy() )
				(*it)->Destroy();
