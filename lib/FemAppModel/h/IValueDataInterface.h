...
public:
  enum TValueDataType : int { SCALAR = 0, VECTOR, TENSOR };
...
template <int DATA_TYPE, class VALUE_DOMAIN> class IValueDataInterfaceTempl : public IValueDataInterface {
public:
  ...
  TMinMax MinMax(IProgressBase &progressBase, const geo::IObject &object,
  ...
  TValue Average(IProgressBase &progressBase, const geo::IObject &object,
                 const UNIT unit = IQuantityDouble::SI_UNIT) const;
  virtual TValue Average(IProgressBase &progressBase, const UNIT unit = IQuantityDouble::SI_UNIT) const = 0;

  virtual TValue ValuePoint(const geo::IPoint &pt, const UNIT unit = IQuantityDouble::SI_UNIT,
                            geo::IParallelInitializationCallback *cb = 0) const = 0;
  virtual TValueVec ValueElement(const geo::IElement &elm, const UNIT unit = IQuantityDouble::SI_UNIT,
                                 geo::IParallelInitializationCallback *cb = 0) const = 0;

  virtual IValueDataInterface::TValueDataType Type() const {
    return static_cast<IValueDataInterface::TValueDataType>(DATA_TYPE);
  }
  virtual IValueDataInterface::TValueDataType Type() {
    return static_cast<IValueDataInterface::TValueDataType>(DATA_TYPE);
  }
};

template <int DATA_TYPE, class VALUE_DOMAIN>
typename VALUE_DOMAIN::TMinMax IValueDataInterfaceTempl<DATA_TYPE, VALUE_DOMAIN>::MinMax(IProgressBase &progressBase,
                                                                                         const geo::IObject &object,
...
template <int DATA_TYPE, class VALUE_DOMAIN>
typename VALUE_DOMAIN::TValue IValueDataInterfaceTempl<DATA_TYPE, VALUE_DOMAIN>::Average(IProgressBase &progressBase,
                                                                                         const geo::IObject &object,
                                                                                         const UNIT unit) const {
  TValue ret;
  const geo::IArray *ptArr = dynamic_cast<const geo::IArray *>(&object);
  ...
  } else if (pElementSet && pElementSet->ElementSize() > 0) {
    double sumArr = 0;
    for (int i = 0; i < pElementSet->ElementSize(); i++) {
...
template <class PARENT_COMPONENT> class IValueDataInterfaceScalarTempl : public IValueDataInterfaceScalar {
protected:
