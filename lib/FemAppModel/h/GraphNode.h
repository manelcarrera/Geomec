...
public: // Public classes and typedefs ...
  ...
  class CPtrLess {
  public:
    template <class T> bool operator()(const T *pItem1, const T *pItem2) const { return pItem1 < pItem2; }
  };

  // Less operator uses the customised less function of the graphnode it selves.
  class CLess {
  public:
    template <class T> bool operator()(const T *pItem1, const T *pItem2) const { return pItem1->Less(*pItem2); }
  };
  ...
  enum eHandleStatus { NO_ACTION = 0, TO_BE_DELETED = 1, FORCE_DELETE, TO_BE_ADDED };
...
public:
  ...
  template <class TYPE> std::set<TYPE *> Links() const;
...
public:
  static std::string Register(const std::type_info &tinfo, CDelegateFactory::TFactory createMethod);
...
template <class TYPE> std::set<TYPE *> CGraphNode::Links() const {
  std::set<TYPE *> stRet;
