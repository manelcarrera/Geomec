...
CArchiveQDataStream::CArchiveQDataStream(QIODevice* device, bool bIsHDF5File)
  ...
  , m_bIsHDF5(bIsHDF5File || dynamic_cast<CHDF5File *>(device))
{
}
...
bool CArchiveQDataStream::IsStoring() const
{
  return archive.device()->isWritable();
}

bool CArchiveQDataStream::Exists() const
{
  return IsHDF5() || static_cast<QFile *>(archive.device())->exists();
}

bool CArchiveQDataStream::IsHDF5() const
{
  return m_bIsHDF5;
}
...
quint64 CArchiveQDataStream::GetPosition()
{
...
void CArchiveQDataStream::Close()
{
  archive.device()->close();
}

void CArchiveQDataStream::Abort()
{
  archive.device()->close();
}
...
CArchiveQDataStream& CArchiveQDataStream::operator << (
  const QDateTime& dt)
{
  archive << dt;
...
bool CArchiveQDataStream::IsCompressed() const
{
  return (dynamic_cast<QtIOCompressor*>(archive.device()) != 0);
}
