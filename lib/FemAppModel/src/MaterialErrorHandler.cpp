...
CMaterialErrorHandler::CMaterialErrorHandler(CAnalysisLogger &analysisLogger)
    : m_analysisLogger(analysisLogger), m_errorMessages(), m_firstMessage(true) {}

CMaterialErrorHandler::~CMaterialErrorHandler() {
  if (!m_errorMessages.empty()) {
    m_analysisLogger.AddLine(REPEAT_HEADER);

    for (std::map<QString, size_t>::const_iterator errorMessage = m_errorMessages.begin();
         errorMessage != m_errorMessages.end(); ++errorMessage) {
      QString logMessage = QString(REPEAT_MESSAGE).arg((*errorMessage).first).arg((*errorMessage).second);
...
void CMaterialErrorHandler::handleError(const QString &material, const QString &parameter, const QString &message) {
  if (!message.isEmpty()) {
    // if (m_firstMessage)
    //{
    ...
    QString errorMessage = QString(LOG_MESSAGE).arg(parameter).arg(material).arg(message);
    std::pair<std::map<QString, size_t>::iterator, bool> result =
        m_errorMessages.insert(std::map<QString, size_t>::value_type(errorMessage, 1));
