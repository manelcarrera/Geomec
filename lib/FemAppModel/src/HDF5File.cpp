...
CHDF5File::CHDF5Stream::CHDF5Stream(H5::H5File *file, const QString &name, size_t chunkSize)
    : m_file(file), m_mode(0), m_name(name), m_chunkSize(chunkSize), m_index(0), m_buffer(0), m_processed(0), m_read(0),
      m_dataBuffer(0), m_dataSpace(0), m_dataSet(0), m_prop(0), m_type(0), m_flush(false) {}

CHDF5File::CHDF5Stream::~CHDF5Stream() { close(); }
...
bool CHDF5File::CHDF5Stream::open(unsigned int mode) {
  ...
  try {
    ...
    if (mode == H5F_ACC_TRUNC) {
      ...
      unsigned int
          cd_values[7]; // 0-3 reserved, 4 compression level, 5 shuffle active flag (0/1), 6 compression library
...
qint64 CHDF5File::CHDF5Stream::bytesRead() const { return m_read; }

void CHDF5File::CHDF5Stream::bytesRead(qint64 addBytes) { m_read += addBytes; }
...
CHDF5File::CHDF5File(QIODevice *file, QObject *parent)
    : QIODevice(parent), m_file(file), m_h5file(0), m_mode(0), m_stream(0) {
  try {
...
QString CHDF5File::FileName() const { return static_cast<QFile *>(m_file)->fileName(); }

// For now we only support either read-only or write-only
bool CHDF5File::CanRead() const { return m_mode == H5F_ACC_RDONLY; }

bool CHDF5File::CanWrite() const { return m_mode == H5F_ACC_TRUNC; }

QString CHDF5File::ModelGroup(int modelIndex) { return QString("/Models/%1/").arg(modelIndex); }

QString CHDF5File::ResultGroup(int modelIndex) { return QString("/Results/%1/").arg(modelIndex); }
