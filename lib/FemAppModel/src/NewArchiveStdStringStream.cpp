...
template <class S> void writeStringLength(S &archive, long length, bool unicode) {
...
template <class S> long readStringLength(S &archive, int &charSize) {
  UInt64 uint64Length;
  UInt32 uint32Length;
  UInt16 uint16Length;
  Byte byteLength;
...
CNewArchiveStdStringStream::CNewArchiveStdStringStream(std::ios_base::openmode mode) : archive(mode), mode(mode) {
  assert(mode & std::ios_base::binary);
}

CNewArchiveStdStringStream::~CNewArchiveStdStringStream() {}

bool CNewArchiveStdStringStream::IsLoading() const { return (mode & std::ios_base::in); }

bool CNewArchiveStdStringStream::IsStoring() const { return (mode & std::ios_base::out); }

bool CNewArchiveStdStringStream::Exists() const { return true; }

quint64 CNewArchiveStdStringStream::GetPosition() { return archive.tellg(); }

void CNewArchiveStdStringStream::SetPosition(quint64 position) { archive.seekg(position, std::stringstream::beg); }

void CNewArchiveStdStringStream::Flush() { archive.flush(); }
...
CNewArchiveStdStringStream &CNewArchiveStdStringStream::operator<<(const QDateTime &dt) {
...
CNewArchiveStdStringStream &CNewArchiveStdStringStream::operator<<(bool /*b*/) {
  assert(false);

  return *this;
}

CNewArchiveStdStringStream &CNewArchiveStdStringStream::operator<<(quint8 /*b*/) {
  assert(false);
...
#ifndef WIN32
CNewArchiveStdStringStream &CNewArchiveStdStringStream::operator<<(wchar_t /*wc*/) {
  assert(false);
...
CNewArchiveStdStringStream &CNewArchiveStdStringStream::operator<<(quint16 /*w*/) {
  assert(false);

  return *this;
}

CNewArchiveStdStringStream &CNewArchiveStdStringStream::operator<<(short /*w*/) {
  assert(false);

  return *this;
}

CNewArchiveStdStringStream &CNewArchiveStdStringStream::operator<<(qint64 /*ll*/) {
  assert(false);

  return *this;
}

CNewArchiveStdStringStream &CNewArchiveStdStringStream::operator<<(quint64 /*ull*/) {
  assert(false);

  return *this;
}

CNewArchiveStdStringStream &CNewArchiveStdStringStream::operator<<(float /*f*/) {
  assert(false);
