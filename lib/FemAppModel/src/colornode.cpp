...
//////////////////////////////////////////////////////////////////////
CColorNode::CColorNode(CFemAppModel &model) : COpenGLNode(model), m_nColor(0) {}
...
CColorNode::CColorNode(const QString &strName, CFemAppModel &model) : COpenGLNode(strName, model) {
  CColorEntry *pEntry = (CColorEntry *)(Model().GraphEntry(MD_BASE_COLOR_NODE));
  m_nColor = pEntry->GetColor();
  LinkTo(*pEntry);
}

CColorNode::CColorNode(const unsigned int uName, CFemAppModel &model) : COpenGLNode(uName, model) {
  CColorEntry *pEntry = (CColorEntry *)(Model().GraphEntry(MD_BASE_COLOR_NODE));
  m_nColor = pEntry->GetColor();
  LinkTo(*pEntry);
}

CColorNode::CColorNode(const CColorNode &rhs) : COpenGLNode(rhs), m_nColor(rhs.m_nColor) {}

CColorNode::~CColorNode() {
  // TODO : Release color factory if any ...
}

CColorNode::TColor CColorNode::Color() const { return m_nColor; }
