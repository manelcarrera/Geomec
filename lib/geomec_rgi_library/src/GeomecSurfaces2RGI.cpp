...
CGeomecSurfaces2RGI::CGeomecSurfaces2RGI(RGInterface &rgi, const CRockMechProcessor &rmp, const CModelBase &modelBase)
    : m_rgi(rgi), m_rmp(rmp), m_modelBase(modelBase) {}
...
template <typename S> void createSurface(RGSurface &rgSurface, RGInterface &rgi, S &surface) {
  try {
...
CSurfaceWrapper::CSurfaceWrapper(const geo::CElementGroup *elementGroup, Side side)
    : m_elementGroup(elementGroup), m_side(side) {}

int CSurfaceWrapper::ElementSize() const { return m_elementGroup->ElementSize(); }

const geo::IElement &CSurfaceWrapper::Element(int index) const {
  const geo::IInterfaceElement &interfaceElement =
      dynamic_cast<const geo::IInterfaceElement &>(m_elementGroup->Element(index));
...
std::vector<int> CGeomecSurfaces2RGI::retrieveSurfaceNodeIndices(const geo::ISurface &surface) const {
...
void CGeomecSurfaces2RGI::saveFaults() {
  ...
  if (dynamic_cast<const CTetraBoundary *>(&m_modelBase.Boundary())) {
    ...
    const THorizonBaseEntry *horizonBaseEntry =
        dynamic_cast<const THorizonBaseEntry *>(m_modelBase.GraphEntry(MD_BASE_HORIZON));
    THorizonBaseEntry::TSortedNodeSet stNodes = horizonBaseEntry->SortedEntryNodes();

    for (THorizonBaseEntry::TSortedNodeSet::const_iterator horizon = stNodes.begin(); horizon != stNodes.end();
         ++horizon) {
      if ((*horizon)->Slip()) {
        const geo::CElementGroup *elementGroup = (*horizon)->InterfaceElementGroup();

        {
          RGSurface rgSurface((*horizon)->Name().toStdString() + SPACE + SURFACE_FRONT, RGSurface::FaultFront,
                              RGSurface::Regular);
        ...
        {
          RGSurface rgSurface((*horizon)->Name().toStdString() + SPACE + SURFACE_BACK, RGSurface::FaultBack,
                              RGSurface::Regular);
    ...
    const CHexaFormationEntry *hexaEntry =
        dynamic_cast<const CHexaFormationEntry *>(m_modelBase.GraphEntry(MD_HEXA_FORMATION));
    if (hexaEntry) {
...
void CGeomecSurfaces2RGI::saveHorizon(const C3DHorizon *horizon, RGSurface::Type type, RGSurface::Attribute attribute) {
  ...
  for (int s = 0; s < horizon->MeshedSurfaceSize(); ++s) {
    ...
    std::vector<int> meshedSurfaceNodeIndices = retrieveSurfaceNodeIndices(surface);

    if (std::find(savedMeshedSurfaces.begin(), savedMeshedSurfaces.end(), meshedSurfaceNodeIndices) ==
        savedMeshedSurfaces.end()) {
...
void CGeomecSurfaces2RGI::saveHorizon(const CHexaHorizon *horizon, const CFormationPlane *plane, RGSurface::Type type,
...
void CGeomecSurfaces2RGI::saveHorizons() {
  if (dynamic_cast<const CTetraBoundary *>(&m_modelBase.Boundary())) {
    const CTetraBoundary &tetraBoundary = dynamic_cast<const CTetraBoundary &>(m_modelBase.Boundary());
    ...
    for (THorizonBaseEntry::TNodeSet::const_iterator horizon = stNodes.begin(); horizon != stNodes.end(); ++horizon) {
      if ((*horizon != topHorizon) && (*horizon != bottomHorizon) && !(*horizon)->Slip()) {
        saveHorizon(dynamic_cast<const C3DHorizon *>(*horizon), RGSurface::Horizon, RGSurface::Regular);
      }
    }
  } else {
    assert(dynamic_cast<const CHexaModel *>(&m_modelBase));
    ...
    if (hexaEntry) {
      ...
      saveHorizon(&topFormation->UpperHorizon(), topFormation->FormationPlane(CFormationPlane::FP_TOP).data(),
                  RGSurface::Horizon, RGSurface::Top);
      saveHorizon(&botFormation->LowerHorizon(), botFormation->FormationPlane(CFormationPlane::FP_BOTTOM).data(),
                  RGSurface::Horizon, RGSurface::Bottom);

      const CHexaFormation *formation = topFormation->LowerFormation();

      while (formation) {
        if (!formation->UpperHorizon().Slip()) {
          saveHorizon(&formation->UpperHorizon(), formation->FormationPlane(CFormationPlane::FP_TOP).data(),
