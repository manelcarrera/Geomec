...
void orderPolygon(const GeomecRGI::CLoadGridFromRGI::TIndices &input, GeomecRGI::CLoadGridFromRGI::TIndices &output) {
  size_t pivot = 0;
...
void CLoadGridFromRGI::PolygonInfo::setPolygon(TIndices &polygon) { orderPolygon(polygon, nodes); }

CLoadGridFromRGI::CLoadGridFromRGI(CModelBase &model, RGInterface &rgi, const CRockMechProcessor &rmp)
    : m_model(model), m_mesh(static_cast<geo::CTetMeshBase &>(m_model.Mesh().Mesh())), m_rgi(rgi), m_rmp(rmp),
      m_geomecFormations(), m_dump(getenv("RGIDDUMP") != 0) {}
...
bool CLoadGridFromRGI::operator()() {
  ...
  constructFormations(formationNames, formationVolume2VolumeMapping, elementIndices);

  std::vector<TSharedBodyTriangleMap> trianglesPerVolume = getTrianglesPerVolume();
  TNodes2Triangles nodes2Triangles = createNodes2TrianglesMapping(trianglesPerVolume);
  ...
  constructFaults(linkFace2Horizon, linkFace2SurfaceDescriptor, nodes2Triangles, nodeIndices, polygonInfo);

  // Horizons

  constructHorizons(linkFace2Horizon, linkFace2SurfaceDescriptor, nodes2Triangles, nodeIndices);

  // Side Surfaces

  constructSideSurfaces(linkFace2SurfaceDescriptor, nodes2Triangles, nodeIndices);

  // Volumes

  constructFormationVolumes(linkFace2Horizon, linkFace2SurfaceDescriptor, formationVolume2VolumeMapping,
                            trianglesPerVolume, formationNames);
...
std::vector<CFormationBase *> CLoadGridFromRGI::getGeomecFormations() const { return m_geomecFormations; }
...
const QString INVALID_FORMATION_COUNT = "The number of loaded formation ID's "
                                        ...
                                        " elements (%2)!";
const QString INVALID_FORMATION_VOLUME_COUNT =
    "The number of loaded formation "
...
CLoadGridFromRGI::TIndices insertElementTE12L(const RGInterface &rgi, const CLoadGridFromRGI::TIndices &nodeIndices,
                                              const RGElement &rgElement, size_t element) {
  ...
  for (int i = 0; i < 4; ++i) {
    if (!nodeIds.insert(rgElement.getNodeN(i)).second) {
      QString message = NO_DEGENERATES.arg(element).arg(rgElement.getNodeN(i));
  ...
  if (!geo::ITetrahedron::CheckOrientation(point[0], point[1], point[2], point[3])) {
...
void CLoadGridFromRGI::insertElements(TIndices &elementIndices, TIndices &nodeIndices, TPolygonInfo &polygonInfo,
                                      TPolygonMap &polygonMap) {
  // we're going to fill the polygonInfo, so that we have a mapping from RGPolygon to body faces and where appropriate a
  // reference to an interface element
...
std::map<int, int> retrieveCounts(const CLoadGridFromRGI::TIndices &elementIndices,
...
void getFormationNames(std::vector<QString> &formationNames, const std::map<int, int> &formationSize,
                       const CLoadGridFromRGI::TIndices &elementIndices, const CLoadGridFromRGI::TIndices &formationID,
                       const CLoadGridFromRGI::TIndices &formationIndex, const RGInterface &rgi) {
  formationNames.resize(formationSize.size());

  for (std::map<int, int>::const_iterator iterator = formationSize.begin(); iterator != formationSize.end();
       ++iterator) {
    ...
    for (size_t element = 0; element < elementIndices.size(); ++element) {
      if (formationID[element] == iterator->first) {
        if (!formationNameFilled) {
          formationNames[formationIndex[formationID[element]]] = rgi.getFormationName(formationID[element]).c_str();
...
CLoadGridFromRGI::TIndices loadFormationID(RGInterface &rgi, const CLoadGridFromRGI::TIndices &elementIndices) {
  CLoadGridFromRGI::TIndices formationID;

  rgi.loadProperty(RGPropertyType::propElementFormation, formationID);
  propertyValuesMayNotContainNull(RGPropertyType::propElementFormation, formationID);
  ...
  if (formationID.size() != elementIndices.size()) {
    QString message = INVALID_FORMATION_COUNT.arg(formationID.size()).arg(elementIndices.size());
...
CLoadGridFromRGI::TIndices loadFormationVolumeID(RGInterface &rgi, CLoadGridFromRGI::TIndices &formationID,
  ...
  propertyValuesMayNotContainNull(RGPropertyType::propBodyIndex, formationVolumeID);
  ...
  if (formationVolumeID.size() > 0) {
    if (formationVolumeID.size() != elementIndices.size()) {
      QString message = INVALID_FORMATION_VOLUME_COUNT.arg(formationID.size()).arg(elementIndices.size());
...
void CLoadGridFromRGI::constructFormations(std::vector<QString> &formationNames,
                                           std::map<int, int> &formationVolume2VolumeMapping,
                                           const TIndices &elementIndices) {
  TIndices formationID = loadFormationID(m_rgi, elementIndices);
  std::map<int, int> formationSize = retrieveCounts(elementIndices, formationID);
  TIndices formationVolumeID = loadFormationVolumeID(m_rgi, formationID, m_rmp, elementIndices);
  std::map<int, int> formationVolumeSize = retrieveCounts(elementIndices, formationVolumeID);
  ...
  for (std::map<int, int>::const_iterator iterator = formationVolumeSize.begin(); iterator != formationVolumeSize.end();
       ++iterator) {
    ...
    if (iterator->first >= 0)
      formationVolume2VolumeMapping.insert(
          std::make_pair(m_mesh.RegisterTetVolume(formationVolumeIndices), volume - minimumID));
  }

  getFormationNames(formationNames, formationSize, elementIndices, formationID, formationIndex, m_rgi);
}

std::vector<TSharedBodyTriangleMap> CLoadGridFromRGI::getTrianglesPerVolume() {
  ...
  for (int volume = 0; volume < m_mesh.NrOfVolumes(); ++volume) {
    ...
    trianglesPerVolume[volume] = calculateBoundingSurfaces.getSharedBodyTriangleMap();
  }
...
TSharedBodyTriangle findMatchingTriangle(std::vector<TSharedBodyTriangleMap> &trianglesPerVolume,
  ...
  for (int v = (volume + 1); v < mesh.NrOfVolumes(); ++v) {
    TSharedBodyTriangleMap::iterator opposite = trianglesPerVolume[v].find(key);
...
CLoadGridFromRGI::createNodes2TrianglesMapping(std::vector<TSharedBodyTriangleMap> trianglesPerVolume) const {
  TNodes2Triangles nodes2Triangles;

  for (int volume = 0; volume < m_mesh.NrOfVolumes(); ++volume) {
    for (TSharedBodyTriangleMap::const_iterator triangle = trianglesPerVolume[volume].begin();
         triangle != trianglesPerVolume[volume].end(); ++triangle) {
      TSharedBodyTriangle match = findMatchingTriangle(trianglesPerVolume, m_mesh, (*triangle).first, volume);
      std::pair<TNodes2Triangles::iterator, bool> result =
          nodes2Triangles.insert(std::make_pair((*triangle).first, std::make_pair((*triangle).second, match)));
...
void CLoadGridFromRGI::constructFaults(TFaceDescriptorMap &linkFace2Horizon,
  ...
  for (size_t f = 0; f < m_rgi.getNumFaults(); ++f) {
    ...
    constructFault(linkFace2Horizon, linkFace2SurfaceDescriptor, nodes2Triangles, nodeIndices, polygonInfo, back,
                   front);
  }
}
...
void CLoadGridFromRGI::constructHorizons(TFaceDescriptorMap &linkFace2Horizon,
  ...
  for (size_t h = 0; h < m_rgi.getNumHorizons(); ++h) {
    ...
    if (horizon.getAttribute() == RGSurface::Regular) {
      constructRegularHorizon(linkFace2Horizon, linkFace2SurfaceDescriptor, nodes2Triangles, nodeIndices, horizon);
    } else if ((horizon.getAttribute() == RGSurface::Top) || (horizon.getAttribute() == RGSurface::Bottom)) {
      constructTopBottomHorizon(linkFace2Horizon, linkFace2SurfaceDescriptor, nodes2Triangles, nodeIndices, horizon);
    }
  }
}

void CLoadGridFromRGI::constructSideSurfaces(TFaceDescriptorMap &linkFace2SurfaceDescriptor,
                                             const TNodes2Triangles &nodes2Triangles, const TIndices &nodeIndices) {
  for (size_t s = 0; s < m_rgi.getNumHorizons(); ++s) {
    ...
    if (sideSurface.getAttribute() == RGSurface::Side) {
      CTetraSubBoundary &subBoundary = static_cast<CTetraModel &>(m_model).SubBoundary();
      CMeshSurface *meshSurface = constructMeshSurface(sideSurface, nodes2Triangles, nodeIndices);

      constructFaceDescriptorMap(linkFace2SurfaceDescriptor, meshSurface);

      m_mesh.AddSurface(*meshSurface, sideSurface.getName(), false);

      CTetraSurface *tetraSurface = new CTetraSurface(sideSurface.getName().c_str(), *meshSurface, m_model);

      subBoundary.LinkTo(*tetraSurface);
      static_cast<CTetraMesh &>(m_model.Mesh()).m_vcSurfaceSource.push_back(&subBoundary);
    }
  }
}
...
std::vector<QString> constructFormationVolumeNames(std::map<int, int> &formationVolume2VolumeMapping,
  ...
  for (size_t volume = 0; volume < volumes; ++volume) {
    if (formationNames[formationVolume2VolumeMapping[volume]].isEmpty()) {
      ...
      formationVolumeNames[volume] = FORMAT.arg(formationNames[formationVolume2VolumeMapping[volume]]).arg(volume);
    }
  }
...
void validateFormationNames(std::vector<QString> &formationNames) {
  ...
  for (std::vector<QString>::iterator formationName = formationNames.begin(); formationName != formationNames.end();
       ++formationName) {
    if ((*formationName).isEmpty()) {
      ...
      } while (std::find(formationNames.begin(), formationNames.end(), uniqueName) != formationNames.end());
...
void CLoadGridFromRGI::constructFormationVolumes(TFaceDescriptorMap & /*linkFace2Horizon*/,
                                                 TFaceDescriptorMap &linkFace2SurfaceDescriptor,
                                                 std::map<int, int> &formationVolume2VolumeMapping,
  ...
  std::vector<QString> formationVolumeNames =
      constructFormationVolumeNames(formationVolume2VolumeMapping, formationNames, m_mesh.NrOfVolumes());
  ...
  for (int volume = 0; volume < m_mesh.NrOfVolumes(); ++volume) {
    ...
    if (formations[formationVolume2VolumeMapping[volume]] == 0) {
      formations[formationVolume2VolumeMapping[volume]] =
          new CTetraFormation(formationNames[formationVolume2VolumeMapping[volume]], m_model);
      m_geomecFormations[formationVolume2VolumeMapping[volume]] = formations[formationVolume2VolumeMapping[volume]];
    }

    CTetraFormation *tetraFormation = formations[formationVolume2VolumeMapping[volume]];
    CTetraFormationVolume *tetraFormationVolume =
        new CTetraFormationVolume(*tetraFormation, const_cast<geo::CBodyGroup &>(bodyGroup));
    std::vector<CMeshSurface *> meshSurface =
        extrapolateMeshSurfaces(trianglesPerVolume[volume], linkFace2SurfaceDescriptor);
    int surfaceIndex = 0;
    ...
    for (size_t surface = 0; surface < meshSurface.size(); ++surface) {
      TFaceDescriptorMap::const_iterator iterator = linkFace2SurfaceDescriptor.find(&meshSurface[surface]->Face(0));
      ...
      /*geo::CTetSurface* tetSurface =*/new geo::CTetSurface(
          const_cast<geo::CSurfaceDesc &>(m_mesh.SurfaceDesc(surfaceIndex)), const_cast<geo::CBodyGroup &>(bodyGroup));
      geo::CBodyGroup::CSideSurface &sideSurface =
          const_cast<geo::CBodyGroup &>(bodyGroup).SideSurface(bodyGroup.SideSurfaceSize() - 1);
...
void CLoadGridFromRGI::constructFaceDescriptorMap(TFaceDescriptorMap &linkFace2SurfaceDescriptor,
  ...
  for (int face = 0; face < meshSurface->FaceSize(); ++face) {
    std::pair<TFaceDescriptorMap::const_iterator, bool> result = linkFace2SurfaceDescriptor.insert(
        std::make_pair<const geo::IFace *, int>(&meshSurface->Face(face), m_mesh.NrOfSurfaces()));
...
const TSharedBodyTriangle getSharedBodyTriangle(const RGPolygonId &polygonId, const RGInterface &rgi,
                                                const CLoadGridFromRGI::TIndices &nodeIndices,
  ...
  QString key =
      QString("%1.%2.%3").arg(nodeIndices[polygon[0]]).arg(nodeIndices[polygon[1]]).arg(nodeIndices[polygon[2]]);
  const CLoadGridFromRGI::TNodes2Triangles::const_iterator result = nodes2Triangles.find(key);

  if (result == nodes2Triangles.end()) {
    QString s = QString("Cannot find polygon %1 (nodes %2, %3, %4) in our collection of body faces.")
                    .arg(polygonId)
                    .arg(polygon[0])
                    .arg(polygon[1])
                    .arg(polygon[2]);
...
const geo::CBodyTriangle *getBodyTriangle(const RGPolygonId &polygonId, const RGInterface &rgi,
                                          const CLoadGridFromRGI::TIndices &nodeIndices,
                                          const CLoadGridFromRGI::TNodes2Triangles &nodes2Triangles) {
  return getSharedBodyTriangle(polygonId, rgi, nodeIndices, nodes2Triangles)->getBodyTriangle();
}

const QString NO_INTERFACE_ELEMENT = "Encountered a non-interface element while constructing a fault";
...
void CLoadGridFromRGI::constructFault(TFaceDescriptorMap & /*linkFace2Horizon*/,
                                      TFaceDescriptorMap & /*linkFace2SurfaceDescriptor*/,
                                      const TNodes2Triangles & /*nodes2Triangles*/, const TIndices & /*nodeIndices*/,
                                      const TPolygonInfo &polygonInfo, const RGSurface &back,
                                      const RGSurface &front) const {
  CSurfaceEntry &surfaceEntry = dynamic_cast<CSurfaceEntry &>(*(m_model.GraphEntry(MD_BASE_SURFACE)));
  ...
  CSurfaceBase *surfaceBase = surfaceEntry.InsertSurface(back.getName().c_str(), *geoSurface);
  CTetraSubHorizon *tetraSubHorizon = new CTetraSubHorizon(back.getName().c_str(), TRUE, m_model);
  geo::CSurfaceDesc *surfaceDesc = new geo::CSurfaceDesc(m_mesh, surfaceBase->Surface(), std::string(), true);

  static_cast<CTetraMesh &>(m_model.Mesh()).m_vcSurfaceSource.push_back(surfaceBase);
...
CMeshSurface *CLoadGridFromRGI::constructMeshSurface(const RGSurface &surface, const TNodes2Triangles &nodes2Triangles,
  ...
  for (size_t p = 0; p < surface.getSize(); ++p) {
    meshSurface->AddFace(*getBodyTriangle(surface.getPolygonN(p), m_rgi, nodeIndices, nodes2Triangles));
  }

  return meshSurface;
}

void CLoadGridFromRGI::constructRegularHorizon(TFaceDescriptorMap &linkFace2Horizon,
                                               TFaceDescriptorMap &linkFace2SurfaceDescriptor,
                                               const TNodes2Triangles &nodes2Triangles, const TIndices &nodeIndices,
                                               const RGSurface &horizon) const {
  CMeshSurface *meshSurface = constructMeshSurface(horizon, nodes2Triangles, nodeIndices);
  ...
  CTetraSurface *tetraSurface = new CTetraSurface(horizon.getName().c_str(), *meshSurface, m_model);
  CTetraSubHorizon *tetraSubHorizon = new CTetraSubHorizon(horizon.getName().c_str(), FALSE, m_model);

  tetraSubHorizon->LinkTo(*tetraSurface);
  static_cast<CTetraMesh &>(m_model.Mesh()).m_vcSurfaceSource.push_back(tetraSurface);
}

void CLoadGridFromRGI::constructTopBottomHorizon(TFaceDescriptorMap & /*linkFace2Horizon*/,
                                                 TFaceDescriptorMap &linkFace2SurfaceDescriptor,
                                                 const TNodes2Triangles &nodes2Triangles, const TIndices &nodeIndices,
                                                 const RGSurface &horizon) const {
  CMeshSurface *meshSurface = constructMeshSurface(horizon, nodes2Triangles, nodeIndices);
...
collectSurfaces(const TSharedBodyTriangleMap &triangles,
  ...
  for (TSharedBodyTriangleMap::const_iterator triangle = triangles.begin(); triangle != triangles.end(); ++triangle) {
...
std::vector<CMeshSurface *>
CLoadGridFromRGI::extrapolateMeshSurfaces(const TSharedBodyTriangleMap &triangles,
                                          const TFaceDescriptorMap &linkFace2SurfaceDescriptor) const {
  TSurfaceCollection surfaces = collectSurfaces(triangles, linkFace2SurfaceDescriptor);
  std::vector<CMeshSurface *> meshSurfaces;

  for (TSurfaceCollection::const_iterator surface = surfaces.begin(); surface != surfaces.end(); ++surface) {
    CMeshSurface *meshSurface = new CMeshSurface();

    for (std::set<const geo::IFace *>::const_iterator face = (*surface).second.begin(); face != (*surface).second.end();
         ++face) {
