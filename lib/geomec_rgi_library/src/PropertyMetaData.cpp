...
void PropertyMetaData::Setup(const std::map<std::string, const IValueComponentBase *> &properties,
  ...
  for (std::map<std::string, const IValueComponentBase *>::const_iterator it = properties.begin();
    ...
    if (meta.propertyName == "Dlt_PStress1L" || meta.propertyName == "Dlt_PStress2L" ||
        meta.propertyName == "Dlt_PStress3L") {
    ...
    if (processed) {
      ...
      if (processed) {
        ...
        if (typeName.startsWith("WPStress"))
          ...
          typeName.remove(0, 2);
        else if (typeName.startsWith("EStress") || typeName.startsWith("PStress") || typeName.startsWith("PStrain") ||
                 typeName.startsWith("TStress"))
          typeName.remove(0, 1);
