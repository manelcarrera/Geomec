...
: CLoadPropertyTemplate<IDT_VALUETYPE_SHEARMODULUS>(rgProperty, rgi, modelBase, rmp)
, m_NormalYoungsModulus(0)
, m_TransverseYoungsModulus(0)
, m_NormalPoissonsRatio(0)
...
std::vector <std::vector <geo::CValue> >
  CLoadPropertyShearModulus::calculatePropertyValues() const
{
...
bool CLoadPropertyShearModulus::enforceUniqueness(unsigned int uValueType) const
{
  return true;
}
...
void CLoadPropertyShearModulus::retrieveValueTypes(
  CValueType** normalYoungsModulus, CValueType** transverseYoungsModulus,
  CValueType** normalPoissonsRatio, CValueType** transversePoissonsRatio) const
{
