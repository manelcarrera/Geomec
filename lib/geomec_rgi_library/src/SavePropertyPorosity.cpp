...
CSavePropertyPorosity::CSavePropertyPorosity(const RGProperty &rgProperty) : CSavePropertyBase(rgProperty) {}

CSavePropertyPorosity::~CSavePropertyPorosity() {}

bool CSavePropertyPorosity::saveProperty(RGInterface &rgi, CModelBase &modelBase, const CRockMechProcessor &rmp) {
  CDepletionStage &stage =
      modelBase.DepletionStageEntry().StageByIndex(rgi.getCurrentDepletionStage().getDepletionStage());
  const IResultComponent *pComp = modelBase.ResultTree().Porosity().ResultComponent(stage, CAnalysisType::AT_NONLIN, 0);

  if ((pComp == 0) && !stage.IsMarkedAsInitial()) {
    QString l = QString("No porosity component found for depletion stage %1").arg(stage.Index());
    rmp.AddLogLine(l, &rgi, false, true);
    return false;
  }

  std::vector<double> vcValues(modelBase.Mesh().Mesh().ElementSize(), RGUtils::nullReal());
  ...
  for (int i = 0; i < modelBase.Mesh().Mesh().ElementSize(); ++i) {
    ...
    if (pComp == 0) {
      ...
      if (formationBase) {
        const CFFMaterial &cffMaterial = formationBase->Material(stage).Material(elm);
    ...
    } else // (pComp != 0)
    {
      ...
      if (!val.Valid()) {
        if (!logLineAdded) {
          QString l = QString("Found an invalid porosity for depletion stage %1").arg(stage.Index());
          rmp.AddLogLine(l, &rgi, false, true);
          logLineAdded = true;
        }
      } else {
