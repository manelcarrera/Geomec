...
CLoadPropertyPressureBase::CLoadPropertyPressureBase(const RGProperty &rgProperty, RGInterface &rgi,
...
void CLoadPropertyPressureBase::linkValueTypes(CFormationBase &formation, CValueType *pVT, const CDepletionStage &stage,
  ...
  if (isFractureApertureModel(formation, stage)) {
    ...
    if (((m_RGProperty.getType() == RGPropertyType::propElementDeltaFluidPressure) ||
        ...
        (static_cast<CElementValueSet &>(static_cast<CValueComponent &>(pVT->Component()).ValueSet()).ValueSize() >
         0)) {
