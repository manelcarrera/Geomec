...
CSavePropertyIsReservoir::CSavePropertyIsReservoir(const RGProperty &rgProperty) : CSavePropertyBase(rgProperty) {}

CSavePropertyIsReservoir::~CSavePropertyIsReservoir() {}

bool CSavePropertyIsReservoir::saveProperty(RGInterface &rgi, CModelBase &modelBase, const CRockMechProcessor &rmp) {
  ...
  if (exportResultData) {
    ...
    for (COpenGLNodeSelection::iterator node = exportResultData->m_selection.begin();
         node != exportResultData->m_selection.end(); ++node) {
