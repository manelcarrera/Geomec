...
std::vector<int> loadRGIFormationIDs(RGInterface &rgi) {
  ...
  GeomecRGI::propertyValuesMayNotContainNull(RGPropertyType::propElementFormation, rgiFormationIDs);

  return rgiFormationIDs;
}

std::map<int, std::size_t> loadRGIFormation2RGIElementMap(const std::set<int> &distinctRGIFormationIDs,
                                                          const std::vector<int> &rgiFormationIDs) {
  std::map<int, std::size_t> rgiFormation2RGIElementMap;

  for (std::set<int>::const_iterator id = distinctRGIFormationIDs.begin(); id != distinctRGIFormationIDs.end(); ++id) {
    const std::vector<int>::const_iterator element = std::find(rgiFormationIDs.begin(), rgiFormationIDs.end(), *id);

    assert(element != rgiFormationIDs.end());

    size_t offset = std::distance(rgiFormationIDs.begin(), element);

    rgiFormation2RGIElementMap.insert(std::map<int, std::size_t>::value_type(*id, offset));
  }
...
CRGIFormationIDs::CRGIFormationIDs(RGInterface &rgi)
    : m_RGIFormationIDs(loadRGIFormationIDs(rgi)),
      ...
      m_RGIFormation2RGIElementMap(loadRGIFormation2RGIElementMap(m_DistinctRGIFormationIDs, m_RGIFormationIDs)) {}

CRGIFormationIDs::~CRGIFormationIDs() {}

std::vector<int> CRGIFormationIDs::getRGIFormationIDs() const { return m_RGIFormationIDs; }

std::set<int> CRGIFormationIDs::getDistinctRGIFormationIDs() const { return m_DistinctRGIFormationIDs; }

std::size_t CRGIFormationIDs::mapRGIFormation2RGIElement(int rgiFormation) const {
  if (m_RGIFormation2RGIElementMap.find(rgiFormation) != m_RGIFormation2RGIElementMap.end()) {
...
std::set<int> CRGIFormationIDs::getDistinctFormationIDs(const std::vector<int> &formationIDs) {
