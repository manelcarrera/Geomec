...
GeomecRGI::TBoundingBox extrapolateBoundingBox(const GeomecRGI::TSharedBodyTriangleMap &triangleMap) {
  GeomecRGI::TBoundingBox boundingBox;

  boundingBox.front = (*triangleMap.begin()).second->getBodyTriangle()->Point(0).X();
  boundingBox.back = (*triangleMap.begin()).second->getBodyTriangle()->Point(0).X();
  boundingBox.right = (*triangleMap.begin()).second->getBodyTriangle()->Point(0).Y();
  boundingBox.left = (*triangleMap.begin()).second->getBodyTriangle()->Point(0).Y();
  boundingBox.top = (*triangleMap.begin()).second->getBodyTriangle()->Point(0).Z();
  boundingBox.bottom = (*triangleMap.begin()).second->getBodyTriangle()->Point(0).Z();

  for (GeomecRGI::TSharedBodyTriangleMap::const_iterator triangle = triangleMap.begin(); triangle != triangleMap.end();
       ++triangle) {
    for (int point = 0; point < (*triangle).second->getBodyTriangle()->NrOfPoints(); ++point) {
      adjustBoundaries(boundingBox.front, boundingBox.back, (*triangle).second->getBodyTriangle()->Point(point).X());
      adjustBoundaries(boundingBox.right, boundingBox.left, (*triangle).second->getBodyTriangle()->Point(point).Y());
      adjustBoundaries(boundingBox.top, boundingBox.bottom, (*triangle).second->getBodyTriangle()->Point(point).Z());
    }
  }
...
CCalculateBoundingBox::CCalculateBoundingBox(geo::IElementSet &elementSet)
    : CCalculateBoundingSurfaces(elementSet), m_BoundingBox(extrapolateBoundingBox(m_SharedBodyTriangleMap)) {}
...
TTriangleSurfaces CCalculateBoundingBox::collectTriangleSurfaces() {
  ...
  TTriangleSurfaces triangleSurfaces(NUMBER_OF_SURFACES);

  for (TTriangleMap::const_iterator triangle = triangleMap.begin(); triangle != triangleMap.end(); ++triangle) {
    if ((*triangle).second == 0) {
      ...
      if ((middleX <= x) && (x <= m_BoundingBox.front)) {
        ...
        double distanceZ = ERROR_MARGIN * (((m_BoundingBox.top - m_BoundingBox.bottom) / scale) / 2);
        double distanceY = ERROR_MARGIN * (((m_BoundingBox.right - m_BoundingBox.left) / scale) / 2);

        if (((middleZ - distanceZ) <= z) && (z <= (middleZ + distanceZ)) && ((middleY - distanceY) <= y) &&
            (y <= (middleY + distanceY))) {
      ...
      if ((middleY <= y) && (y <= m_BoundingBox.right)) {
        ...
        double scale = std::sqrt(scaleSquared);
        double distanceZ = ERROR_MARGIN * (((m_BoundingBox.top - m_BoundingBox.bottom) / scale) / 2);
        double distanceX = ERROR_MARGIN * (((m_BoundingBox.front - m_BoundingBox.back) / scale) / 2);

        if (((middleZ - distanceZ) <= z) && (z <= (middleZ + distanceZ)) && ((middleX - distanceX) <= x) &&
            (x <= (middleX + distanceX))) {
      ...
      if ((middleZ <= z) && (z <= m_BoundingBox.top)) {
        ...
        if (((middleX - distanceX) <= x) && (x <= (middleX + distanceX)) && ((middleY - distanceY) <= y) &&
            (y <= (middleY + distanceY))) {
...
TTriangleMap CCalculateBoundingBox::getTriangleMap() const {
  ...
  for (TSharedBodyTriangleMap::const_iterator triangle = m_SharedBodyTriangleMap.begin();
  ...
  for (std::set<const geo::CBodyTriangle *>::const_iterator bodyTriangle = bodyTriangleSet.begin();
       bodyTriangle != bodyTriangleSet.end(); ++bodyTriangle) {
    std::pair<TTriangleMap::iterator, bool> result = triangleMap.insert(std::make_pair(*bodyTriangle, 0));
