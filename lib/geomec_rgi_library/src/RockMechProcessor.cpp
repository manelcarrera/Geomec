...
CRockMechProcessor::CRockMechProcessor(const QString &strFileName, const QString &strOutputDir, QString strTempDir)
    : m_model(nullptr), m_strFileName(strFileName), m_strOutputDir(strOutputDir), m_bQuit(false), m_drc(0),
...
const QString TENSOR_IS_NOT_COMPLETE = "Not all components of the residual strain tensor have been loaded, "
...
*/
void CRockMechProcessor::executeRockMechCommands_DEPRECATED(int &nStage, const std::string &title,
                                                            IDianaXWrapper *dianaXWrapper, const QString &getPathName,
                                                            bool &bRet, ISaveModel &saveModel,
                                                            IRetrieveDianaFileNames &retrieveDianaFileNames,
                                                            bool &bSaveModel, bool validate) {
  bSaveModel = false;
  ...
  try {
    ...
    while (!m_bQuit) {
        ...
        for (i = 0; i < nCommands && !bQuit; ++i) {
          ...
          case typeCommandCalculate:
            if (bRet) {
              ...
              if (bHibernating) {
                if (!bHaveCalculation)
                  bRet = CalculateFirst(getPathName, dianaXWrapper, title, rgi, saveModel, retrieveDianaFileNames);
                else
                  bRet = CalculateNext(getPathName, dianaXWrapper, title, rgi, saveModel, retrieveDianaFileNames);

                bHaveCalculation = true;
                bSaveModel = true;
              } else {
                bRet = Calculate(getPathName, dianaXWrapper, title, rgi, saveModel, retrieveDianaFileNames);
                bSaveModel = true;
              }
            }
            break;
      ...
      // in hibernation mode we need to do saving on quit here, as it seems that dynamo kills our thread after the lock
      // is released
...
bool CRockMechProcessor::QuitCommandReceived() const { return m_bQuit; }

bool CRockMechProcessor::isExistingModel() const { return m_isExistingModel; }

// static

CMaterialEntry *CRockMechProcessor::createMaterial(CModelBase &modelBase, const QString &materialName,
                                                   int materialModel) {
...
void CRockMechProcessor::connectMaterials2Formations(CModelBase &modelBase, const std::vector<IMaterial *> materials) {
...
void CRockMechProcessor::connectMaterial2Formation(CModelBase &modelBase, IMaterial *material,
    ...
    if (materialServer != 0) {
      if (materialServer->LibraryMaterial() != 0) {
        if (materialServer->MaterialModel() != material->MaterialModel()) {
          ...
          copyMaterialLibraryValue(oldMatLib, newMatLib, IDT_VALUETYPE_YOUNGS_MODULUS, IDT_VALUETYPE_YOUNGMODULUS_NORM,
          ...
          copyMaterialLibraryValue(oldMatLib, newMatLib, IDT_VALUETYPE_POISSONS_RATIO, IDT_VALUETYPE_POISSONRATIO_NORM,
...
bool CRockMechProcessor::LoadGrid(RGInterface &rgi) {
  ...
  CMaterialEntry *materialEntry = createMaterial(*m_model, "linear rgi", MM_LINEAR);
  ...
  CLoadPropertyMaterialType loadPropertyMaterialType(rgProperty, rgi, *m_model, *this);
...
bool CRockMechProcessor::LoadProperty(RGInterface &rgi, const RGProperty &rgProperty) {
  ...
  try {
    ...
    bool ok = EnsureDepletionStageAvailable(rgi) && loadPropertyBridge.loadProperty();

    if (!ok) {
      l = QString("A mismatch has occurred loading property %1").arg(PropertyName(rgProperty));
...
bool CRockMechProcessor::SaveProperty(RGInterface &rgi, const RGProperty &rgProperty) {
  ...
  try {
    ...
    bool ok = EnsureDepletionStageAvailable(rgi) && savePropertyBridge.saveProperty(rgi, *m_model, *this);

    if (!ok) {
      l = QString("A mismatch has occurred saving property %1").arg(PropertyName(rgProperty));
      AddLogLine(l, &rgi, true, true);
    }
...
bool CRockMechProcessor::CalculateFirst(const QString &getPathName, IDianaXWrapper *dianaXWrapper,
  ...
  try {
    ...
    dia::IDianaRunner::RunParams p = {getPathName,   title,      m_drc->tmp().toStdString(),
...
bool CRockMechProcessor::CalculateNext(const QString &getPathName, IDianaXWrapper *dianaXWrapper,
                                       const std::string &title, RGInterface &rgi, ISaveModel &saveModel,
                                       IRetrieveDianaFileNames &retrieveDianaFileNames) {
...
bool CRockMechProcessor::Calculate(const QString &getPathName, IDianaXWrapper *dianaXWrapper, const std::string &title,
                                   RGInterface &rgi, ISaveModel &saveModel,
                                   IRetrieveDianaFileNames &retrieveDianaFileNames) {
  if (rgi.getCurrentDepletionStage().getDepletionStage() == 0) {
    AddLogLine("Cannot calculate the initial stage", &rgi, false, true);
    return false;
  ...
  bool bRet;
  try {
    bRet = m_model->WriteFilosModel(getPathName, dianaXWrapper, title, CAnalysisType::AT_NONLIN, false, false,
                                    saveModel, retrieveDianaFileNames);
  } catch (CProgressCancel *p) {
    delete p;
    bRet = false;
    AddLogLine("GEOMEC analysis cancelled", &rgi, false, true);
...
bool CRockMechProcessor::DefineInitialDate(RGInterface &rgi, const RGDate &date) {
  QString l;
  l = QString("Define Initial Date, year = %1, month = %2, day = %3")
          .arg(date.getYear())
          .arg(date.getMonth())
          .arg(date.getDay());
  AddLogLine(l, &rgi, true);
...
bool CRockMechProcessor::EnsureDepletionStageAvailable(RGInterface &rgi) {
  ...
  if (idx > nStagesInModel) {
    QString l;
    l = QString(
            "Number of stages in model = %1, current depletion stage = %2. It seems at least one stage was skipped...")
            .arg(nStagesInModel)
            .arg(idx);
    AddLogLine(l, &rgi, false, true);
    return false;
  ...
  if (pLastStage) {
    if (!pLastStage->Initial()) {
      ...
      pStage->SetNonLinParams(pLastStage->AutomaticStepSizes(), pLastStage->MaxAutomaticStepSize(),
                              pLastStage->NLSteps(), pLastStage->NLStepScalingFactor(), pLastStage->NLConvNormVal(),
                              pLastStage->NLIterationSteps(), pLastStage->NLConvergenceType(),
  ...
  for (size_t i = 0; i < m_monitorPointSets.size(); ++i) {
    ...
    for (size_t j = 0; j < numProps; ++j) {
      ...
      if (prop == properties.end()) {
        AddLogLine(QString("PointSet '%1': Property '%2' not found for depletion stage %3")
                       .arg(ps->Name(), RGProps[j].c_str(),
                            QString::number(rgi.getCurrentDepletionStage().getDepletionStage())));
        continue;
      }
      ...
      if (meta == metaData.end() || meta->second.typeName.empty() || meta->second.componentIndex < 0) {
        AddLogLine(QString("PointSet '%1': Cannot associate property '%2' with a Geomec type")
                       .arg(ps->Name(), RGProps[j].c_str()));
        continue;
      }
      ...
      std::pair<std::map<std::string, int>::iterator, bool> res =
          composites.insert(std::make_pair(meta->second.propertyName, 0));

      int idx = -1;

      if (res.second) {
        CValueType *composite = factory->BuildValueType(*ps, meta->second.typeName.c_str(),
                                                        QString("%1_D%2")
                                                            .arg(meta->second.propertyName.c_str())
                                                            .arg(rgi.getCurrentDepletionStage().getDepletionStage()));

        if (!composite) {
          AddLogLine(QString("PointSet '%1': Cannot create Geomec type '%2' for property '%3'")
                         .arg(ps->Name(), meta->second.typeName.c_str(), RGProps[j].c_str()));
          continue;
        }
...
struct CollectResults : IGeomecModelVisitor {
  ...
  CollectResults(std::map<std::string, const IValueComponentBase *> &properties, const CDepletionStage *depStage)
      : properties(properties), depStage(depStage) {}
  ...
  virtual bool VisitIResultComponent(IResultComponent &component) {
    if (component.Type() == IValueComponentBase::SCALAR &&
        component.AnalysisType().AnalysisType() == CAnalysisType::AT_NONLIN && component.CanComputeOnPoints() &&
        (depStage == nullptr || depStage->Index() == component.Stage().Index())) {
      ...
      if (!exportLabel.empty()) {
        std::transform(exportLabel.begin(), exportLabel.end(), exportLabel.begin(),
                       [](char c) { return c == ' ' ? '_' : c; });
...
void CRockMechProcessor::CollectMonitorablePropertiesFromModel(
    std::map<std::string, const IValueComponentBase *> &properties, bool withDepletionStages) const {
...
void CRockMechProcessor::TransformMonitorableProperties(
    const std::map<std::string, const IValueComponentBase *> &properties,
    std::vector<RGGeneralProperty> &rgProps) const {
  rgProps.reserve(100);

  for (std::map<std::string, const IValueComponentBase *>::const_iterator it = properties.begin();
...
CAnalysisLogger &CRockMechProcessor::Logger() const { return const_cast<CAnalysisLogger &>(m_model->Logger()); }
...
QString CRockMechProcessor::getSaveModelName(const QString &pathName, int nStage, bool bSuccess,
                                             bool bAppendStageNumber) {
  QString sPath;
  QString sFileName;
...
bool CRockMechProcessor::CheckModel(RGInterface &rgi) const {
  ...
  if (pHomoEntry && !pHomoEntry->GraphEntryNodes().empty()) {
    AddLogLine("This model contains at least one scaling box. Scaling boxes are not allowed with RockMech processing",
               &rgi, false, true);
    return false;
  }
  ...
  for (TFormationBaseEntry::TNodeSet::iterator it = stNode.begin(); it != stNode.end(); it++) {
    ...
    if (formation.PressureChange() &&
        formation.Material(m_model->InitialDepletionStage()).MaterialModel() == MM_UNDRAINED)
