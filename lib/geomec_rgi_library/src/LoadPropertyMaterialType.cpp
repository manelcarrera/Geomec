...
, m_materialTypes(loadMaterialTypes(rgi))
{
}
...
const QString MODIFYING_PROPERTY =
  QObject::tr("Modifying property '%1' for an existing model");
const QString UNSUPPORTED_MATERIAL_TYPE =
  QObject::tr("Unsupported material type '%1'");
...
bool CLoadPropertyMaterialType::mapMaterialType2MaterialModel(
  std::vector <int>& materialModels,
...
std::map <int, int> createFormation2MaterialMap(
...
const QString MATERIAL_MODEL_NOT_CREATED =
  QObject::tr("The material model could not be created");

} // anonymous namespace

bool CLoadPropertyMaterialType::assignMaterialModels(
  const std::vector <int>& materialModels) const
{
...
IMaterial* findMaterial(const CGraphEntry::TEntryNodeSet& entryNodeSet,
  int materialModel)
{
