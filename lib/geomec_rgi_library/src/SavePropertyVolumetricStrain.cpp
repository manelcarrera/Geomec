...
bool CSavePropertyVolumetricStrain::saveProperty(RGInterface& rgi,
  CModelBase& modelBase, const CRockMechProcessor& rmp)
{
  ...
  const IValueComponentBase* pComp =
    modelBase.ResultTree().TotalStrain().Invariants().ResultComponent(stage, CAnalysisType::AT_NONLIN, 0, IStrainTensorGroup::CInvariantComposite::INV_I1);
  const IValueComponentBase* pIsoStressComp =
    modelBase.ResultTree().TotalStress().InvariantComposite()->ResultComponent(stage, CAnalysisType::AT_NONLIN, 0, IStressTensorGroup::CInvariantComposite::INV_I1);
  const IValueComponentBase* pIsoStressCompIni =
    modelBase.ResultTree().TotalStress().InvariantComposite()->ResultComponent(iniStage, CAnalysisType::AT_NONLIN, 0, IStressTensorGroup::CInvariantComposite::INV_I1);
  if(!pComp)
  {
    QString l;
    ...
    if(bValid && !vcElementValues.empty())
    {
      ...
      if(pFormation)
      {
        ...
        if(ffmat.IsParameter(IDT_VALUETYPE_POROSITY) && ffmat.IsParameter(IDT_VALUETYPE_THERMAL_EXPANSION))
        {
          ...
          IValueDomainScalar::TValueVec vcT = pFormation->EffectiveTemperature(stage).Component().ScalarData().ValueElement(elm);
          IValueDomainScalar::TValueVec vcT0 = pFormation->EffectiveTemperature(iniStage).Component().ScalarData().ValueElement(elm);
          ...
          if(ffmat.IsParameter(IDT_VALUETYPE_GRAINSTIFFNESS))
          {
            ...
            if(Kg != 0)
            {
              geo::CValue P = 0;
              geo::CValue P0 = 0;
              IValueDomainScalar::TValueVec vcP = pFormation->Pressure(stage).Component().ScalarData().ValueElement(elm);
              for(size_t j = 0; j < vcP.size(); ++j)
                P += vcP[j];
              P /= vcP.size();
              IValueDomainScalar::TValueVec vcP0 = pFormation->Pressure(iniStage).Component().ScalarData().ValueElement(elm);
