...
CElementValueSet &getResidualStrainSet(int &residualStrainSet, CModelBase &modelBase) {
...
void loadValues(CElementValueSet &elementValueSet, RGInterface &rgi, const RGProperty &rgProperty,
                CModelBase &modelBase, int /*residualStrainSet*/) {
...
const QString TENSOR_ALREADY_CONNECTED_2_STRAIN_LOAD = "A tensor is already connected to the strain load, "
                                                       "not attempting to connect again";

void connectStrainLoad2Tensor(CModelBase &modelBase, RGInterface &rgi, GeomecRGI::CRockMechProcessor &rmp) {
  ...
  for (TFormationBaseEntry::TNodeSet::iterator formation = formationBaseEntry->EntryNodes().begin();
...
bool CBuildTensor::CBuildTensorImpl::loadProperty(
    RGInterface &rgi, CModelBase &modelBase, CRockMechProcessor &rmp, const RGProperty &rgProperty,
    const ITensorGroup::CComponentComposite::TENSOR_COMPONENT &tensorComponent) {
  CElementValueSet &elementValueSet = getResidualStrainSet(g_residualStrainSet[tensorComponent], modelBase);

  loadValues(elementValueSet, rgi, rgProperty, modelBase, g_residualStrainSet[tensorComponent]);
  ...
  IValueComponentBase &valueComponentBase = g_tensor->Component(tensorComponent);
...
bool CBuildTensor::CBuildTensorImpl::isTensorComplete() const {
  return (std::find(g_residualStrainSet.begin(), g_residualStrainSet.end(), NULL_TENSOR_COMPONENT) ==
          g_residualStrainSet.end());
}
...
QSharedPointer<CBuildTensor::CBuildTensorImpl> constructTensor() {
  if (g_tensorImpl.isNull()) {
    g_tensorImpl = QSharedPointer<CBuildTensor::CBuildTensorImpl>(new CBuildTensor::CBuildTensorImpl());
  }
...
CBuildTensor::CBuildTensor() : m_Tensor(constructTensor()) {}

CBuildTensor::~CBuildTensor() { destructTensor(); }

bool CBuildTensor::loadProperty(RGInterface &rgi, CModelBase &modelBase, CRockMechProcessor &rmp,
                                const RGProperty &rgProperty,
                                const ITensorGroup::CComponentComposite::TENSOR_COMPONENT &tensorComponent) {
  return m_Tensor->loadProperty(rgi, modelBase, rmp, rgProperty, tensorComponent);
}

bool CBuildTensor::isTensorComplete() const { return m_Tensor->isTensorComplete(); }
