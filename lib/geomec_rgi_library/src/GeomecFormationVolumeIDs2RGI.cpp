...
std::set<int> getDistinctFormationVolumeIDs(const std::vector<int> &formationVolumeIDs) {
...
std::vector<int> getGeomecFormationVolumeIDs(const CModelBase &modelBase,
  ...
  std::vector<int> geomecFormationVolumeIDs(modelBase.Mesh().Mesh().ElementSize());
  ...
  for (int e = 0; e < modelBase.Mesh().Mesh().ElementSize(); ++e) {
    ...
    const IFormationElementSet *formationElementSet = modelBase.Mesh().FormationElementSet(element);

    assert(formationElementSet != 0);

    std::vector<CGraphNode *>::const_iterator formationVolume =
        std::find(formationVolumes.begin(), formationVolumes.end(), formationElementSet);

    assert(formationVolume != formationVolumes.end());

    geomecFormationVolumeIDs[e] = std::distance(formationVolumes.begin(), formationVolume);
  }
...
CGeomecFormationVolumeIDs2RGI::CGeomecFormationVolumeIDs2RGI(RGInterface & /*rgi*/, const CModelBase &modelBase)
    : m_modelBase(modelBase), m_formationVolumes(getFormationVolumes(m_modelBase)),
      m_GeomecFormationVolumeIDs(getGeomecFormationVolumeIDs(m_modelBase, m_formationVolumes)),
      m_distinctGeomecFormationVolumeIDs(getDistinctFormationVolumeIDs(m_GeomecFormationVolumeIDs)) {}

CGeomecFormationVolumeIDs2RGI::~CGeomecFormationVolumeIDs2RGI() {}

std::vector<int> CGeomecFormationVolumeIDs2RGI::getFormationVolumeIDs() const { return m_GeomecFormationVolumeIDs; }
