...
public:
  CLoadPropertyFluidPressure(const RGProperty &rgProperty, RGInterface &rgi, CModelBase &modelBase,
                             CRockMechProcessor &rmp);
  virtual ~CLoadPropertyFluidPressure();

private:
  CLoadPropertyFluidPressure(const CLoadPropertyFluidPressure &rhs);
  CLoadPropertyFluidPressure &operator=(const CLoadPropertyFluidPressure &rhs);
};
