...
class CLoadPropertyShearModulus : public CLoadPropertyTemplate<IDT_VALUETYPE_SHEARMODULUS> {
public:
  using CLoadPropertyBase::loadProperty;

  CLoadPropertyShearModulus(const RGProperty &rgProperty, RGInterface &rgi, CModelBase &modelBase,
...
private:
  ...
  CLoadPropertyShearModulus &operator=(const CLoadPropertyShearModulus &rhs);

  virtual void convertValue(geo::CValue &dValue) const;

  void retrieveValueTypes(CValueType **normalYoungsModulus, CValueType **transverseYoungsModulus,
