...
public:
  ...
  void executeRockMechCommands_DEPRECATED(int &nStage, const std::string &title, IDianaXWrapper *dianaXWrapper,
                                          const QString &getPathName, bool &bRet, ISaveModel &saveModel,
                                          IRetrieveDianaFileNames &retrieveDianaFileNames, bool &bSaveModel,
                                          bool validate = true);
  bool QuitCommandReceived() const;
  bool isExistingModel() const;

  static CMaterialEntry *createMaterial(CModelBase &modelBase, const QString &materialName, int materialModel);
  static void connectMaterials2Formations(CModelBase &modelBase, const std::vector<IMaterial *> materials);
  static void connectMaterial2Formation(CModelBase &modelBase, IMaterial *material, CFormationBase *formationBase);
...
private:
  ...
  bool CalculateFirst(const QString &getPathName, IDianaXWrapper *dianaXWrapper, const std::string &title,
                      RGInterface &rgi, ISaveModel &saveModel, IRetrieveDianaFileNames &retrieveDianaFileNames);
  bool CalculateNext(const QString &getPathName, IDianaXWrapper *dianaXWrapper, const std::string &title,
                     RGInterface &rgi, ISaveModel &saveModel, IRetrieveDianaFileNames &retrieveDianaFileNames);
  bool Calculate(const QString &getPathName, IDianaXWrapper *dianaXWrapper, const std::string &title, RGInterface &rgi,
  ...
  void CollectMonitorablePropertiesFromModel(std::map<std::string, const IValueComponentBase *> &properties,
                                             bool withDepletionStages) const;
  void TransformMonitorableProperties(const std::map<std::string, const IValueComponentBase *> &properties,
...
private:
  ...
  struct Quantity {
    ...
    Quantity(const std::string &name, bool doConvert, double conversionFactor)
        : name(name), doConvert(doConvert), conversionFactor(conversionFactor) {}
