...
{
  public:
  CLoadPropertyBase(const RGProperty& rgProperty, RGInterface& rgi,
      CModelBase& modelBase, CRockMechProcessor& rmp);
  virtual ~CLoadPropertyBase() = 0;

  bool loadProperty(unsigned int uValueType);
  bool loadProperty(const std::vector <geo::CValue>& vcValues,
      const QString& strPropertyName, unsigned int uValueType);
  bool loadProperty(const std::vector <std::vector <geo::CValue> >& vcValues,
      const QString& strPropertyName, unsigned int uValueType);
  ...
  bool isFractureApertureModel(const CFormationBase& formationBase,
      const CDepletionStage& depletionStage) const;
  bool isFractureApertureModel(const CFormationBase& formationBase,
      const CDepletionStage& depletionStage,
      const geo::IElement& element) const;
  bool addDeltaBase(std::vector <geo::CValue>& vcNodalValues,
      const QString& strPropertyName, const CFormationBase* pFormation,
      const geo::IElement& elm, int nNod) const;
  ...
  bool loadValues(std::vector <geo::CValue>& vcValues,
      const QString& strPropertyName) const;
  bool assignValues2Elements(CElementValueSet& elementValueSet,
  ...
  void buildValueTypes(CValueType** pVT, CElementValueSet& elementValueSet,
      unsigned int uValueType) const;
  bool hasFractureApertureFormations() const;
  void linkValueTypes(CValueType* pVT, const CDepletionStage& stage,
  ...
  static void setAnisotropic(bool& isAnisotropic, bool& anisotropicIsSet,
      RGInterface& rgi, const CModelBase& modelBase, CRockMechProcessor& rmp);
