...
template <typename T> bool propertyValuesContainNull(const std::vector<T> &propertyValues) {
...
template <typename T>
void propertyValuesMayNotContainNull(const RGPropertyType::Type &propertyType, const std::vector<T> &propertyValues) {
  if ((propertyValues.size() > 0) && propertyValuesContainNull(propertyValues)) {
    QString message = NULL_VALUES.arg(RGProperty::toString(propertyType).c_str());

    throw message;
  }
}

template <typename T> void convert2geoCValue(std::vector<geo::CValue> &geoCValues, const std::vector<T> &values) {
