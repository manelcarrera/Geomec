...
public:
  CCalculateShearModulus(const RGProperty &rgProperty, RGInterface &rgi, CModelBase &modelBase,
                         CRockMechProcessor &rmp);
  virtual ~CCalculateShearModulus();

  template <typename P, unsigned int VALUE_TYPE, unsigned int NORMAL, unsigned int TRANSVERSE> bool loadProperty();
...
template <typename P, unsigned int VALUE_TYPE, unsigned int NORMAL, unsigned int TRANSVERSE>
bool CCalculateShearModulus::loadProperty() {
  ...
  if (returnValue && isAnisotropic()) {
    ...
    returnValue = returnValue && normal.loadProperty(NORMAL) && transverse.loadProperty(TRANSVERSE);

    RGProperty rgProperty(RGPropertyType::propElementShearModulus);
    CLoadPropertyShearModulus loadPropertyShearModulus(rgProperty, m_RGI, m_ModelBase, m_RMP);

    if (loadPropertyShearModulus.canCalculatePropertyValues()) {
      std::vector<std::vector<geo::CValue>> vcValues = loadPropertyShearModulus.calculatePropertyValues();

      returnValue = returnValue && loadPropertyShearModulus.loadProperty(vcValues, m_RMP.PropertyName(rgProperty),
