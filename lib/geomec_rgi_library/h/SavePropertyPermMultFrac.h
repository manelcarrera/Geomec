...
public:
  ...
  CSavePropertyPermMultFrac(const RGProperty &rgProperty, TDensityDirection densityDirection);
...
private:
  ...
  CSavePropertyPermMultFrac &operator=(const CSavePropertyPermMultFrac &rhs);

  std::pair<double, bool> getLength(const IScalarResultComponent *scalarResultComponent, const CModelBase &modelBase,
                                    const geo::IElement &element) const;
  const IScalarResultComponent *getScalarResultComponent(CModelBase &modelBase,
