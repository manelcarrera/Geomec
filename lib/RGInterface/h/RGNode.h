...
{
public:
   RGNode( const RGNode & tcp ) { m_cors[0] = tcp.m_cors[0]; m_cors[1] = tcp.m_cors[1]; m_cors[2] = tcp.m_cors[2]; }
   RGNode( const double & northing, const double & easting, const double & depth ) { m_cors[0] = northing; m_cors[1] = easting; m_cors[2] = depth; }
   ~RGNode() {}
   ...
   const double & operator[]( size_t n ) const { assert( n < 3 ); return m_cors[n]; }
   double       & operator[]( size_t n ) { assert( n < 3 ); return m_cors[n]; }
