...
#if defined(WIN32) || defined(WIN64) || defined(_WIN32) || defined(_WIN64)
  if (somePath.find("\\\\") == 0)
    return true; // win network path
  if (somePath.find(":") == 1 && isalpha(somePath[0]))
    return true; // path starts with letter drive
#else
  if (somePath.find("/") == 0)
    return true;
#endif
...
std::string FilesystemHelper::RGGetFileName(std::string modelName, bool isTxtFormat) {
...
#if defined(WIN32) || defined(WIN64) || defined(_WIN32) || defined(_WIN64)
  if (!(modelName.find("\\\\") == 0) &&
      modelName.find(osPathDelimit) == 0) // \some\thing\like\this - just add current disk prefix
  {
  ...
  size_t len = modelName.length();
  if (len > 4 && (modelName.substr(len - 4) == ".bin" ||
                  modelName.substr(len - 4) == ".txt")) { // full file name is given just return what was given
    ...
    if (pos == std::string::npos) // no path is given construct simple file name
    {
      rescueFileName += modelName + osPathDelimit + modelName + fileExt;
               ...
               len - 1) // else split last part of given path as model name and create file name using this model name
    {
