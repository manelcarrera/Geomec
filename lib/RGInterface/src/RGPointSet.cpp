...
RGPointSet::RGPointSet(const std::string &name, const std::vector<double> &northing, const std::vector<double> &easting,
                       const std::vector<double> &depth) {
  int nSize = northing.size();
  int eSize = easting.size();
  int dSize = depth.size();

  if ((nSize != eSize && eSize != dSize) || name == "" || name.find(' ') != std::string::npos || nSize == 0 ||
      eSize == 0 || dSize == 0) {
...
std::string RGPointSet::GetName() const { return m_name; }
