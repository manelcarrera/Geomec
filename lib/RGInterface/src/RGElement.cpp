...
static const std::string s_elementName[ RGElementType::typeElementLast ] =
{
   ...
   "3-noded-tri-interface",
   "6-noded-tri-interface",
   "4-noded-quad-interface",
   "8-noded-quad-interface"
};


std::string RGElementType::toString( RGElementType::Type e )
{
   return s_elementName[e];
}

/// @brief Copy constructor
/// @parentGeometry tcp object to be copied
RGElement::RGElement( const RGElement & tcp ) :
   m_type( tcp.m_type )
{
...
RGElement::RGElement( const RGElementType::Type & type, const std::vector< RGNodeId > & nodes ) :
   m_type( type )
{
...
RGElementType::Type RGElement::getType() const
{
   return m_type;
}
