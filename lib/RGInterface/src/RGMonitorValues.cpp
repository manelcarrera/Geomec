...
const RGMonitorPointSets &RGMonitorValues::GetPointSets() const { return m_pointSets; }

RGDepletionStage RGMonitorValues::GetDepletionStage() const { return m_depletionStage; }
...
void RGMonitorValues::Set(const int &pointSetIndex, const int &pointIndex, const int &propertyIndex, double value) {
  ...
  if (pointSetIndex > m_pointSets.GetNumPointSets() ||
      pointIndex > m_pointSets.GetPointSet(pointSetIndex).GetNumPoints() ||
      propertyIndex > m_pointSets.GetProperties(pointSetIndex).size()) {
...
void RGMonitorValues::clear() { m_monitorValues.clear(); }
