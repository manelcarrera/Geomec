...
public:
  ...
  public:
    ...
    CRpnOperatorType(const std::string &o, int p, Assoc_t a, int fA = 0, bool fS = false, bool u = false)
        : m_op(o), m_prec(p), m_assoc(a), m_functionArgs(fA), m_functionSep(fS), m_unary(u) {
      ;
    }
...
public:
  CRpnOperator(const std::string &op, bool isUnary = false) : m_ifthenelse_ok(false) {
    m_pOperator = CRpnOperators::instance()->getOperatorType(op, isUnary);
  }
  CRpnOperator(const CRpnOperator &other) : m_ifthenelse_ok(other.m_ifthenelse_ok), m_pOperator(other.m_pOperator) { ; }
  CRpnOperators::CRpnOperatorType *getOperator() { return m_pOperator; }
  void setIfThenElseOk() { m_ifthenelse_ok = true; }
  bool getIfThenElseOk() {
    if (m_pOperator == NULL)
      return false;
    if (m_pOperator->getName() == "IF")
      return m_ifthenelse_ok;
    if (m_pOperator->getName() == "THEN")
      return m_ifthenelse_ok;
    if (m_pOperator->getName() == "ELSE")
      return m_ifthenelse_ok;
    return true;
  }
