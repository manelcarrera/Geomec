...
CRpnOperators *CRpnOperators::instance() {
  if (m_instance == NULL) {
    ...
    m_vOperatorTypes.push_back(new CRpnOperatorType("*", 7, CRpnOperators::CRpnOperatorType::ASSOC_LEFT));
    m_vOperatorTypes.push_back(
        new CRpnOperatorType("-", 9, CRpnOperators::CRpnOperatorType::ASSOC_RIGHT, 0, false, true));
  }
  return m_instance;
}

/// \brief Remove spaces from the beginning and the end of the input string
void CRpnOperators::removeSpaces(std::string &s ///< The string from which the spaces are removed.
) {
...
size_t CRpnOperators::HasPositiveFloat(const std::string &myString ///< the string to be tested for the float
) {
...
/// \return true
bool CRpnOperators::createTokens(std::string input ///< the input string
                                 ,
  ...
  while (pos < input.size()) {
    ...
    for (size_t ii = 0; ii < getOperatorTypeCount(); ++ii) {
      if (getOperatorType(ii)->isFunction())
        continue;
      ...
      //
      if (input.substr(pos, size) == getOperatorType(ii)->getName()) {
...
void CRpnOperators::fixDivide(std::list<CRpnToken *> &tokens) {
  ...
  for (it1 = tokens.begin(); it1 != tokens.end(); ++it1) {
    ...
    if (opand1 == NULL)
      continue;
    if (opand1->getValue() != "SHtot" && opand1->getValue() != "Shtot")
      continue;
    ...
    //
    if (opand3->getValue().find("Svtot") != 0)
      continue;

    // combine 'SHtot' '/' 'Svtot' into a single operand 'SHtot/Svtot'
    CRpnOperand *opand = new CRpnOperand(opand1->getValue() + optor->getOperator()->getName() + opand3->getValue());
...
CRpnOperators::CRpnOperatorType *CRpnOperators::getOperatorType(const std::string &op, bool isUnary) const {
  for (size_t ii = 0; ii < m_vOperatorTypes.size(); ++ii) {
    if (m_vOperatorTypes[ii]->getName() == op && m_vOperatorTypes[ii]->isUnary() == isUnary)
...
CRpnOperators::CRpnOperatorType *CRpnOperators::getOperatorType(size_t index) const {
  if (index >= m_vOperatorTypes.size())
    return NULL;
  return m_vOperatorTypes[index];
}

size_t CRpnOperators::getOperatorTypeCount() const { return m_vOperatorTypes.size(); }
