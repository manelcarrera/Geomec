...
RescueColorTable::RescueColorTable(RescueModel *parentModelIn, RESCUEINT64 probableRows, Role roleIn)
    : RescueObject(parentModelIn->Context()), parentModel(parentModelIn) {
...
RescueColorTable::~RescueColorTable() { DropMemory(); }
...
void RescueColorTable::Archive(FILE *archiveFile) {
  ...
  if (context->FileVersion() > 9) {
    if (IsLoaded()) {
      if (context->delayedPropertyLoad == FALSE || hasChanged == TRUE || parentModel->propertyActionImmediate == TRUE) {
        WriteData();
      }
...
void RescueColorTable::Relink(RescueObject *parentModelIn) { parentModel = (RescueModel *)parentModelIn; }
...
void RescueColorTable::GetColorRow(RESCUEINT64 zeroBasedOrdinal, RescueColor **lowColorBuf, RescueColor **highColorBuf,
                                   RESCUEBOOL &haveLowHintBuf, RESCUEFLOAT &lowHintBuf, RESCUEBOOL &haveHighHintBuf,
                                   RESCUEFLOAT &highHintBuf) {
...
void RescueColorTable::AddColorRow(RESCUEFLOAT lowValue, RESCUEFLOAT highValue, RescueColor *colorLow,
                                   RescueColor *colorHigh) {
  InsureCapacity();
...
void RescueColorTable::AddColorRow(RESCUEBOOL lowToInfinite, RESCUEFLOAT lowValue, RESCUEBOOL highToInfinite,
                                   RESCUEFLOAT highValue, RescueColor *color) {
  InsureCapacity();
  lowColor[rowCount] = new RescueColor(*color);
  highColor[rowCount] = 0;
...
void RescueColorTable::AddColorRow(RESCUEBOOL lowToInfinite, RESCUEFLOAT lowValue, RESCUEBOOL highToInfinite,
                                   RESCUEFLOAT highValue, RescueColor *colorLow, RescueColor *colorHigh) {
  InsureCapacity();
  lowColor[rowCount] = new RescueColor(*colorLow);
  highColor[rowCount] = new RescueColor(*colorHigh);
  haveLowHint[rowCount] = (lowToInfinite == TRUE) ? FALSE : TRUE;
