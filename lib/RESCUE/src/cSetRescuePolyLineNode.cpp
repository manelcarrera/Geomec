...
RESCUEBOOL cSetRescuePolyLineNode::operator+=(RescuePolyLineNode *newObject) {
  ...
  if (newObject->IsA() != R_RescuePolyLineNode && newObject->IsA() != R_RescuePolyLineNodeAvatar) {
    myReturn = FALSE;
  } else {
    if (allocated == count) {
      allocated += 10;
...
RESCUEINT64 cSetRescuePolyLineNode::Count64(void) { return count; }

RESCUEINT32 cSetRescuePolyLineNode::Count(void) { return (RESCUEINT32)count; }
