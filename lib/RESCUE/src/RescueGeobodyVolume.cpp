...
void RescueGeobodyVolume::AddKLayerEdge(RescueEdgeSet *existingEdgeSet) {
  (*kLayerEdges) += new RescueEdgeSetStub(this->ParentPart()->Body()->ParentModel()->Context(), existingEdgeSet);
...
void RescueGeobodyVolume::AddGeobodySurface(RescueSurface *existingSurface, RescueGeobodySurface::Role role) {
...
RescueGeobodyVolume::RescueGeobodyVolume(RescueGeobodyPart *part)
    : RescueHistoryObject(part->Body()->ParentModel()->Context()), parentPart(part) {
...
RescueGeobodyVolume::RescueGeobodyVolume(RescueContext *context, FILE *archiveFile)
    : RescueHistoryObject(context), parentPart(0) {
  isA = R_RescueGeobodyVolume;
  surfaces = new cSetRescueGeobodySurface();
  kLayerEdges = new cSetRescueEdgeSetStub();
...
void RescueGeobodyVolume::RelinkWireframeData(RescueObject *parentModel) { surfaces->RelinkWireframeData(parentModel); }
