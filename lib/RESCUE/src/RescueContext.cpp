...
RESCUEINT32 RescueContext::LibraryVersion() { return FILE_VERSION; }

RESCUEINT32 RescueContext::LibrarySubVersion() { return FILE_SUBVERSION; }
    ...
    if (existsAndIsDir == FALSE) {
      ...
      while (pleaseContinue) {
        ...
        if (ext != 0 && (sep == 0 || sep < ext)) {
          if (strcmp(ext, ".wireframe") == 0 || strcmp(ext, ".lock") == 0) {
            *ext = 0;
          } else {
  ...
  */
  ...
  while (pleaseContinue == TRUE) {
    ...
    if (strncmp(thisName.String(), (*lockFileName).String(), (int)lockNameLen) == 0) {
      if (needWrite || thisName.length64() == lockNameLen) {
...
void RescueContext::CurrentISOTime(RCHString &buffer) {
  ...
  sprintf(buffer.String(), "%4.4d-%2.2d-%2.2dT%2.2d:%2.2d:%2.2d Z", (tStruct->tm_year + 1900), (tStruct->tm_mon + 1),
...
RESCUEBOOL RescueContext::SamePoint(RESCUEFLOAT x1, RESCUEFLOAT y1, RESCUEFLOAT z1, RESCUEFLOAT x2, RESCUEFLOAT y2,
                                    RESCUEFLOAT z2) {
...
RESCUEINT64 *RescueContext::Allocate64For32(RESCUEINT32 *inputArray, RESCUEINT32 cellToCellCount,
                                            RESCUEBOOL freeInput) {
...
RESCUEINT32 *RescueContext::Allocate32For64(RESCUEINT64 *inputArray, RESCUEINT32 cellToCellCount, RESCUEBOOL freeInput,
