...
RescueQuadrupletArray::RescueQuadrupletArray(RescueGrid *existingGrid, RESCUEFLOAT missingValueIn,
    ...
    : RescueTripletArray(parentModelIn->Context(), existingGrid, missingValueIn, parentModelIn), mdValue(0),
      mdValueLength(0) {
...
RescueQuadrupletArray::RescueQuadrupletArray(RescueContext *context, FILE *archiveFile)
    : RescueTripletArray(context, archiveFile), mdValue(0), mdValueLength(0) {
  isA = R_RescueQuadrupletArray;
}
    ...
    */
    ...
    for (loop = 0; loop < howMany; loop++) {
      ...
      fprintf(file, "      <dispNs uom=\"%s\">%.2f</dispNs>\n", uom->NonNullString(),
              (loop == 0) ? 0.0 : northing[loop] - northing[loop - 1]);
      fprintf(file, "      <dispEw uom=\"%s\">%.2f</dispEw>\n", uom->NonNullString(),
              (loop == 0) ? 0.0 : easting[loop] - easting[loop - 1]);
      fprintf(file, "      <mdDelta uom=\"%s\">%.2f</mdDelta>\n", uom->NonNullString(),
              (loop == 0) ? 0.0 : mds[loop] - mds[loop - 1]);
      fprintf(file, "      <tvdDelta uom=\"%s\">%.2f</tvdDelta>\n", uom->NonNullString(),
              (loop == 0) ? 0.0 : tvds[loop] - tvds[loop - 1]);
