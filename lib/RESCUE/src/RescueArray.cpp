...
RescueArray::RescueArray(const RESCUECHAR *propertyNameIn, const RESCUECHAR *propertyTypeIn,
                         const RESCUECHAR *unitOfMeasureIn, RescueGrid *existingGrid, RescueModel *parentModelIn,
                         RESCUEBOOL cellCenteredIn)
    : RescueObject(parentModelIn->Context()), propertyName(0), propertyType(0), unitOfMeasure(0), grid(existingGrid),
      cellCentered(cellCenteredIn), hasChanged(TRUE), dirtyMinMax(TRUE), parentModel(parentModelIn), arrayVersion(0),
      fragments(0) {
...
RescueArray::RescueArray(RescueContext *context, FILE *archiveFile)
    : RescueObject(context), grid(0), hasChanged(FALSE), parentModel(0), arrayVersion(-1), fragments(0) {
  RESCUECHAR myString[255];
...
void RescueArray::Archive(FILE *archiveFile) {
  ...
  if (parentModel->Context()->fileVersion > 9) {
    ...
    if (IsLoaded()) {
      if (parentModel->Context()->delayedPropertyLoad == FALSE || hasChanged == TRUE ||
