...
RESCUEBOOL RescuePillar::IsSplit() const
{
  return dirCount > 1;
}

RESCUEBOOL RescuePillar::IsTruncated() const
{
  return (top_truncation != 0 || base_truncation != 0);
}
...
RescueSplineCoef *RescuePillar::getXSplineCoefAt(RESCUEINT64 ndx) const
{
  return &splineCoefs[0 + ndx];
}

RescueSplineCoef *RescuePillar::getYSplineCoefAt(RESCUEINT64 ndx) const
{
  return &splineCoefs[splineCoefsCount + ndx];
}

RescuePoint *RescuePillar::getCtrlPointAt(RESCUEINT64 ndx) const
{
  return ctrlPoints->NthObject(ndx);
}

RescuePoint *RescuePillar::getMinCtrlPoint() const
{
  return ctrlPoints->NthObject(0);
}

RescuePoint *RescuePillar::getMaxCtrlPoint() const
{
  return ctrlPoints->NthObject(ctrlPoints->Count() - 1);
}

const RescuePoint RescuePillar::getMinCtrlPointRef() const
{
  return RescuePoint(ctrlPoints->NthObject(0));
}
...
RescuePoint *RescuePillar::getMaxTangent()
{
  return new RescuePoint(this->getMaxTangentRef());
}

RescuePoint *RescuePillar::getByZ(RESCUEFLOAT z) const
{
  return new RescuePoint(this->getRefByZ(z));
}
...
void RescuePillar::Values(RescueGeometry *geometry, 
     ...
     // Handle truncations
      ...
      if (top_truncation != 0 || base_truncation != 0) {
        ...
        if (top_pillar != 0 || base_pillar != 0) {
          ...
          bool z_is_elevation = (orientation == RescueCoordinateSystem::LDF || orientation == RescueCoordinateSystem::LDB ||
...
void RescuePillar::SwapIJAxis(bool swapI, RESCUEINT64 iLowBound, RESCUEINT64 iCount, 
...
RescuePillar::RescuePillar(RescueContext *context, FILE *archiveFile, RESCUEBOOL compress)
                                                                     :ctrlPoints(new cSetRescuePoint())
                                                                     ,top_truncation(0)
                                                                     ,base_truncation(0)
                                                                     ,splineCoefs(0)
                                                                     ,splineCoefsCount(0)
