...
RescueTripletArray::RescueTripletArray(RescueGrid *existingGrid, RESCUEFLOAT missingValueIn, RESCUEFLOAT *valueArray,
                                       RescueModel *parentModelIn)
    : RescueObject(parentModelIn->Context()), grid(existingGrid), xValue(0), yValue(0), zValue(0), xValueLength(0),
      yValueLength(0), zValueLength(0), hasChanged(TRUE), loaded(TRUE), parentModel(parentModelIn), arrayVersion(0),
      iNodes(0), jNodes(0), vertices(0) {
...
RescueTripletArray::RescueTripletArray(RescueContext *context, FILE *archiveFile)
    : RescueObject(context), grid(0), xValue(0), yValue(0), zValue(0), xValueLength(0), yValueLength(0),
      zValueLength(0), hasChanged(FALSE), loaded(FALSE), parentModel(0), arrayVersion(0), iNodes(0), jNodes(0),
      vertices(0) {
  isA = R_RescueTripletArray;
...
RESCUEBOOL RescueTripletArray::IsRegular(RESCUEINT64 which) { return grid->IsRegular(which); }
...
void RescueTripletArray::SendBuffer(RESCUEFLOAT *value, RESCUEFLOAT *buffer, RESCUEINT64 offset,
      ...
      if (axis->relatedAxis->parentCoordinateSystem->NdxOf(axis->relatedAxis) == axisNdxOfArray) {
        ...
        case 1: {
          ...
          for (iLoop = axis->LowBound64(); iLoop < axis->LowBound64() + axis->Count64(); iLoop++) {
            (*arrayToMake)[iLoop - axis->LowBound64()] = axis->Origin() + (axis->Step() * (iLoop - axis->LowBound64()));
          }
        } break;
        case 2: {
          RescueGridAxis *iAxis = grid->Axis(0);
          RescueGridAxis *jAxis = grid->Axis(1);
          RCH2DArray controller = RCH2DArray(iAxis->Count64(), jAxis->Count64(), (*arrayToMake));
          ...
          for (iLoop = iAxis->LowBound64(); iLoop < iAxis->LowBound64() + iAxis->Count64(); iLoop++) {
            for (jLoop = jAxis->LowBound64(); jLoop < jAxis->LowBound64() + jAxis->Count64(); jLoop++) {
              if (axis == iAxis) {
                controller.Ndx(iLoop - iAxis->LowBound64(), jLoop - jAxis->LowBound64()) =
                    axis->Origin() + (axis->Step() * (iLoop - axis->LowBound64()));
              } else {
                controller.Ndx(iLoop - iAxis->LowBound64(), jLoop - jAxis->LowBound64()) =
                    axis->Origin() + (axis->Step() * (jLoop - axis->LowBound64()));
              }
            }
          }
        } break;
        case 3: {
          ...
          RCH3DArray controller = RCH3DArray(iAxis->Count64(), jAxis->Count64(), kAxis->Count64(), (*arrayToMake));
            ...
            for (jLoop = jAxis->LowBound64(); jLoop < jAxis->LowBound64() + jAxis->Count64(); jLoop++) {
              for (kLoop = kAxis->LowBound64(); kLoop < kAxis->LowBound64() + kAxis->Count64(); kLoop++) {
                if (axis == iAxis) {
                  ...
                  controller.Ndx(iLoop - iAxis->LowBound64(), jLoop - jAxis->LowBound64(),
                                 kLoop - kAxis->LowBound64()) =
                      axis->Origin() + (axis->Step() * (kLoop - axis->LowBound64()));
                }
              }
            }
          }
...
void RescueTripletArray::XYZAt(RESCUEINT64 i, RESCUEINT64 j, RESCUEINT64 k, RESCUEFLOAT &x, RESCUEFLOAT &y,
                               RESCUEFLOAT &z) {
  EnsureLoaded();
  RescueGridAxis *iAxis = grid->Axis(0);
  RescueGridAxis *jAxis = grid->Axis(1);
  ...
  RESCUEINT64 ndx = ((k - kAxis->LowBound64()) * (kCount * jCount)) + ((j - jAxis->LowBound64()) * jCount) +
                    (i - iAxis->LowBound64());
...
void RescueTripletArray::XYZCornerAt(RESCUEINT64 i, RESCUEINT64 j, RESCUEINT64 faceNumber, RESCUEFLOAT &x,
...
void RescueTripletArray::SetXYZCorner(RESCUEINT64 i, RESCUEINT64 j, RESCUEINT64 faceNumber, RESCUEFLOAT x,
                                      RESCUEFLOAT y, RESCUEFLOAT z) {
  EnsureLoaded();
  if (vertices == 0) {
    RescueGridAxis *iAxis = grid->Axis(0);
