...
RescueArray2dVector::~RescueArray2dVector() {
  if (value != 0) {
    delete[] value;
  }
}

Rescue2dVector *RescueArray2dVector::DemandValue() {
  if (IsLoaded() == FALSE) {
    RescueArray::Load();
  }
  return value;
...
void RescueArray2dVector::AllValues(Rescue2dVector *buffer, RESCUEINT64 offset, RESCUEINT64 bufferLength) {
  if (IsLoaded() == FALSE) {
    RescueArray::Load();
  }
  if (value != 0) {
...
void RescueArray2dVector::Archive(FILE *archiveFile) {
...
void RescueArray2dVector::CalculateMinMaxData(FILE *fragmentFile, RESCUEINT64 fileVersion) {
  if (value == 0) {
      ...
      if (count != 0) {
        ...
        for (loop = 0; loop < count; loop++) {
          ...
          for (frag = 0; frag < fragCount; frag++) {
            data = ((RescueArrayFragment2dVector *)fragments->NthObject(frag))
                       ->Replace(origILowBound + iFollower, origJLowBound + jFollower, origKLowBound + kFollower, data);
          }

          if (data.coord1 != nullValue.coord1 || data.coord2 != nullValue.coord2) {
            if (firstIteration) {
              minValue = data;
              maxValue = data;
    ...
    bool firstIteration = true;
    while (items > 0) {
      if (ptr->coord1 != nullValue.coord1 || ptr->coord2 != nullValue.coord2) {
        if (firstIteration) {
          minValue = *ptr;
          maxValue = *ptr;
          firstIteration = false;
...
void RescueArray2dVector::ReadMinMax(RescueContext *context, FILE *archiveFile) {
  RescueBuffer buf(context, archiveFile);
  buf >> minValue.coord1;
  buf >> minValue.coord2;
  buf >> maxValue.coord1;
...
void RescueArray2dVector::WriteMinMax(RescueContext *context, FILE *archiveFile) {
...
void RescueArray2dVector::InitMinMax(void) {
  minValue.coord1 = 0;
  minValue.coord2 = 0;
  maxValue.coord1 = 0;
  maxValue.coord2 = 0;
...
void RescueArray2dVector::ArchiveData(FILE *archiveFile, FILE *fragmentFile, RESCUEINT64 fileVersion) {
  if (value == 0) {
    if (HasFragments()) {
      RESCUEINT64 count = 0;
      if (fragmentFile != 0) {
...
void RescueArray2dVector::UnArchiveData(FILE *archiveFile, RESCUEINT64 fileVersion) {
...
void RescueArray2dVector::SetValue(Rescue2dVector nullValueIn, Rescue2dVector *valueArray) {
  DropFragments();
...
void RescueArray2dVector::AcceptValue(Rescue2dVector nullValueIn, Rescue2dVector *valueArray) {
  DropFragments();
  if (value != 0) {
    delete[] value;
  }
...
RescueArrayFragment *RescueArray2dVector::Create(RESCUEINT32 iLowBound, RESCUEINT32 iCount, RESCUEINT32 jLowBound,
                                                 RESCUEINT32 jCount) {
  ...
  ((RescueArrayFragment2dVector *)myReturn)->InitializeArray();
  return myReturn;
}

RescueArrayFragment *RescueArray2dVector::Create(RESCUEINT64 iLowBound, RESCUEINT64 iCount, RESCUEINT64 jLowBound,
                                                 RESCUEINT64 jCount) {
...
RescueArrayFragment *RescueArray2dVector::CreatePrimitive(RESCUEINT64 iLowBound, RESCUEINT64 iCount,
                                                          RESCUEINT64 jLowBound, RESCUEINT64 jCount) {
  RescueArrayFragment *myReturn = 0;
  if (grid->Dimensions() == 3) {
    RescueGridAxis *kAxis = grid->Axis(2);
    myReturn = new RescueArrayFragment2dVector(this, 3, iLowBound, iCount, jLowBound, jCount, kAxis->LowBound64(),
                                               kAxis->Count64());
  } else {
    myReturn = new RescueArrayFragment2dVector(this, 2, iLowBound, iCount, jLowBound, jCount, -1, -1);
  }
  DemandFragments();
  (*fragments) += myReturn;
  return myReturn;
...
RescueArrayFragment *RescueArray2dVector::CreatePrimitive(RESCUEINT64 iLowBound, RESCUEINT64 iCount,
  ...
  RescueArrayFragment *myReturn =
      new RescueArrayFragment2dVector(this, 3, iLowBound, iCount, jLowBound, jCount, kLowBound, kCount);
  DemandFragments();
  (*fragments) += myReturn;
  return myReturn;
}

RescueArrayFragment *RescueArray2dVector::Load(RESCUEINT32 iLowBound, RESCUEINT32 iCount, RESCUEINT32 jLowBound,
                                               RESCUEINT32 jCount, RESCUEINT32 kLowBound, RESCUEINT32 kCount) {
  RescueArrayFragment *myReturn = CreatePrimitive((RESCUEINT64)iLowBound, (RESCUEINT64)iCount, (RESCUEINT64)jLowBound,
                                                  (RESCUEINT64)jCount, (RESCUEINT64)kLowBound, (RESCUEINT64)kCount);
...
RescueArrayFragment *RescueArray2dVector::Load(RESCUEINT64 iLowBound, RESCUEINT64 iCount, RESCUEINT64 jLowBound,
                                               RESCUEINT64 jCount, RESCUEINT64 kLowBound, RESCUEINT64 kCount) {
  RescueArrayFragment *myReturn = CreatePrimitive(iLowBound, iCount, jLowBound, jCount, kLowBound, kCount);
  LoadFragment(myReturn);
  return myReturn;
...
void RescueArray2dVector::LoadAndSwapArray() {
  if (IsLoaded() == FALSE) {
    RescueArray::Load();
  }
  RESCUEINT64 fNdx = 0;
...
void RescueArray2dVector::SwapAxes(bool swapI, RESCUEINT64 iNodes, bool swapJ, RESCUEINT64 jNodes, bool swapK,
                                   RESCUEINT64 kNodes) {
  if (IsLoaded() == FALSE) {
    RescueArray::Load();
  }
...
void RescueArray2dVector::SwapAxes(bool swapI, RESCUEINT64 iNodes, bool swapJ, RESCUEINT64 jNodes) {
  if (IsLoaded() == FALSE) {
    RescueArray::Load();
  }
  if (swapI) {
...
RESCUEBOOL RescueArray2dVector::IsOfType(_RescueObjectType thisType) {
