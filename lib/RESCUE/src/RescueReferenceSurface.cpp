...
RescueReferenceSurface::RescueReferenceSurface(RescueModel *model, RescueCoordinateSystem::Orientation orientation,
    ...
    : RescueSurface(model, orientation, model->CoordinateSystem()->Axis(0), i_origin, i_step, i_lowbound, i_count,
                    model->CoordinateSystem()->Axis(1), j_origin, j_step, j_lowbound, j_count, missingValue, typeIn) {
...
RescueReferenceSurface::RescueReferenceSurface(RescueModel *model, RescueCoordinateSystem::Orientation orientation,
                                               ...
                                               RESCUEINT64 i_count, RESCUEFLOAT j_origin, RESCUEFLOAT j_step,
                                               RESCUEINT64 j_lowbound, RESCUEINT64 j_count, RESCUEFLOAT missingValue,
                                               RESCUEFLOAT *values, RescueSurface::SurfaceType typeIn)
    : RescueSurface(model, orientation, model->CoordinateSystem()->Axis(0), i_origin, i_step, i_lowbound, i_count,
                    model->CoordinateSystem()->Axis(1), j_origin, j_step, j_lowbound, j_count, missingValue, values,
                    typeIn) {
  CommonInitialization(model);
}
