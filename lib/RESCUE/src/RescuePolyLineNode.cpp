...
RescuePolyLineNode::RescuePolyLineNode(RescueContext *context, RESCUEDOUBLE x, RESCUEDOUBLE y, RESCUEDOUBLE z)
    : RescueObject(context), xValue(x), yValue(y), zValue(z), surfaceUVs(0), layerParams(0), tSurfUVs(0) {
...
RescuePolyLineNode::RescuePolyLineNode(RescueContext *context, RESCUEFLOAT x, RESCUEFLOAT y, RESCUEFLOAT z)
    : RescueObject(context), xValue((RESCUEDOUBLE)x), yValue((RESCUEDOUBLE)y), zValue((RESCUEDOUBLE)z), surfaceUVs(0),
      layerParams(0), tSurfUVs(0) {
  isA = R_RescuePolyLineNode;
}
...
void RescuePolyLineNode::SetUVValue(RescueTriangulatedSurface *surface, RESCUEINT64 faceNdx, RESCUEFLOAT u,
                                    RESCUEFLOAT v) {
  RescuePolyLineNodeUVT *surfaceIntersect = UVForFace(surface);
  if (surfaceIntersect == 0) {
    surfaceIntersect =
        new RescuePolyLineNodeUVT(surface->ParentSurface()->ParentModel()->Context(), surface, faceNdx, u, v);
    if (tSurfUVs == 0) {
...
RescuePolyLineNodeBUG *RescuePolyLineNode::ParamOn(RescueGeometry *geometry, RescueLayerType layerType) {
  ...
  if (layerParams != 0) {
    ...
    while (myReturn == 0 && instance != 0) {
      if (instance->Geometry() == geometry && instance->Type() == layerType) {
        myReturn = instance;
      } else {
...
void RescuePolyLineNode::SetLayerValue(RescueGeometry *geometry, RescueLayerType layerType, RESCUEFLOAT layerNdxParam) {
  RescuePolyLineNodeBUG *parameterization = ParamOn(geometry, layerType);
  if (parameterization == 0) {
    parameterization =
        new RescuePolyLineNodeBUG(geometry->ParentModel()->Context(), geometry, layerType, layerNdxParam);
    if (layerParams == 0) {
...
RESCUEBOOL RescuePolyLineNode::SurfaceOnNode(RescueIJSurface &surf) { // If the node has UV coords on the
  RESCUEBOOL onSurf = FALSE;                                          // given surface return TRUE
  if (surfaceUVs != 0) {
