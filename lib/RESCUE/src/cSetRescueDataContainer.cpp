...
cSetRescueDataContainer::cSetRescueDataContainer(RescueModel *modelIn, RESCUEINT64 identifierIn) {
  ...
  if (identifier > context->uniqueIDCounter)
    context->uniqueIDCounter = identifier + 1;
  allocated = 10;
  count = 0;
...
RESCUEINT64 cSetRescueDataContainer::Count64(void) { return count; }

RESCUEINT32 cSetRescueDataContainer::Count(void) { return (RESCUEINT32)count; }
