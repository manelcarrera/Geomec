...
RescueMacroVolume::RescueMacroVolume(RescueBlockUnit *parentBUin)
    : RescueHistoryObject(parentBUin->ParentModel()->Context()), oldEdgeSets(0), oldTopEdge(0), oldBottomEdge(0) {
...
RescueMacroVolume::RescueMacroVolume(RescueContext *context, FILE *archiveFile)
    : RescueHistoryObject(context), parentBU(0), blockUnitSides(0), interiorSections(0), topEdges(0), bottomEdges(0),
      kLayerEdges(0), iLayerEdges(0), jLayerEdges(0), interiorSectionIDs(0), oldEdgeSets(0), oldTopEdge(0),
      oldBottomEdge(0) {
  isA = R_RescueMacroVolume;
...
void RescueMacroVolume::Relink(RescueObject *parent) {
  ...
  if (interiorSectionIDs != 0) {
    for (loop = 0; loop < interiorSectionIDs->Count64(); loop++) {
      (*interiorSections) += parentModel->SectionIdentifiedBy(interiorSectionIDs->NthObject(loop));
    }
