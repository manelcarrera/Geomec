...
RescueStairSteppedFault::RescueStairSteppedFault(RescueGeometry *geometryIn, RescueIJSurface *surfaceIn,
                                                 RescueIJKCellPair *arrayIn, RESCUEINT64 arrayLengthIn)
    : RescueObject(geometryIn->ParentModel()->Context()), geometry(geometryIn), surface(surfaceIn), pairs(arrayIn),
      count(arrayLengthIn), properties(0), propertyContainerId(0), surfaceID(0) {
...
RescueStairSteppedFault::RescueStairSteppedFault(RescueContext *context, FILE *archiveFile)
    : RescueObject(context), geometry(0), surface(0), pairs(0), count(0), properties(0), propertyContainerId(0),
      surfaceID(0) {
  isA = R_RescueStairSteppedFault;
