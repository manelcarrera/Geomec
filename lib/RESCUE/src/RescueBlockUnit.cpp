...
RescueBlockUnit::RescueBlockUnit(RescueBlock *parentBlock, RescueUnit *parentUnit)
    : RescueWireframeOwner(parentBlock->ParentModel()->Context()), block(parentBlock), unit(parentUnit),
      surfaceAboveMe(0), surfaceBelowMe(0), macroVolumes(0), propertyGroups(0), propertyGroupsID(0), color(0) {
...
RescueBlockUnit::RescueBlockUnit(RescueCoordinateSystem::Orientation orientation, RescueBlock *parentBlock,
                                 RescueUnit *parentUnit, RESCUEFLOAT i_origin, RESCUEFLOAT i_step,
                                 RESCUEINT64 i_lowbound, RESCUEINT64 i_count, RESCUEFLOAT j_origin, RESCUEFLOAT j_step,
                                 RESCUEINT64 j_lowbound, RESCUEINT64 j_count, RESCUEFLOAT k_origin, RESCUEFLOAT k_step,
                                 RESCUEINT64 k_lowbound, RESCUEINT64 k_count, RESCUEFLOAT missingValue)
    : RescueWireframeOwner(parentBlock->ParentModel()->Context()), block(parentBlock), unit(parentUnit),
      surfaceAboveMe(0), surfaceBelowMe(0), macroVolumes(0), propertyGroups(0), propertyGroupsID(0), color(0) {
  geobodyParts = new cBagRescueGeobodyPart();
  macroVolumes = new cSetRescueMacroVolume();
  ...
  RescueGrid *blockUnitGrid = new RescueGrid(
      parentBlock->ParentModel()->Context(), orientation, parentBlock->ParentModel()->CoordinateSystem()->Axis(0),
      i_origin, i_step, i_lowbound, i_count, parentBlock->ParentModel()->CoordinateSystem()->Axis(1), j_origin, j_step,
...
RescueBlockUnit::RescueBlockUnit(RescueCoordinateSystem::Orientation orientation, RescueBlock *parentBlock,
                                 RescueUnit *parentUnit, RESCUEFLOAT i_origin, RESCUEFLOAT i_step,
                                 RESCUEINT32 i_lowbound, RESCUEINT32 i_count, RESCUEFLOAT j_origin, RESCUEFLOAT j_step,
                                 RESCUEINT32 j_lowbound, RESCUEINT32 j_count, RESCUEFLOAT k_origin, RESCUEFLOAT k_step,
                                 RESCUEINT32 k_lowbound, RESCUEINT32 k_count, RESCUEFLOAT missingValue)
    : RescueWireframeOwner(parentBlock->ParentModel()->Context()), block(parentBlock), unit(parentUnit),
      surfaceAboveMe(0), surfaceBelowMe(0), macroVolumes(0), propertyGroups(0), propertyGroupsID(0), color(0) {
  geobodyParts = new cBagRescueGeobodyPart();
  macroVolumes = new cSetRescueMacroVolume();
...
RescueBlockUnit::RescueBlockUnit(RescueCoordinateSystem::Orientation orientation, RescueBlock *parentBlock,
                                 RescueUnit *parentUnit, RESCUEFLOAT i_origin, RESCUEFLOAT i_step,
                                 RESCUEINT32 i_lowbound, RESCUEINT32 i_count, RESCUEFLOAT j_origin, RESCUEFLOAT j_step,
                                 RESCUEINT32 j_lowbound, RESCUEINT32 j_count, RESCUEINT32 k_lowbound,
                                 RESCUEINT32 k_count, RESCUEFLOAT missingValue)
    : RescueWireframeOwner(parentBlock->ParentModel()->Context()), block(parentBlock), unit(parentUnit),
      surfaceAboveMe(0), surfaceBelowMe(0), macroVolumes(0), propertyGroups(0), propertyGroupsID(0), color(0) {
  geobodyParts = new cBagRescueGeobodyPart();
  macroVolumes = new cSetRescueMacroVolume();
...
RescueBlockUnit::RescueBlockUnit(RescueCoordinateSystem::Orientation orientation, RescueBlock *parentBlock,
  ...
  (*gridGeometries) += new RescueGeometry(parentBlock->ParentModel(), blockUnitGrid, missingValue, referenceSurfaceIn,
...
RescueBlockUnit::RescueBlockUnit(RescueCoordinateSystem::Orientation orientation, RescueBlock *parentBlock,
                                 ...
                                 RESCUEINT64 i_lowbound, RESCUEINT64 i_count, RESCUEFLOAT j_origin, RESCUEFLOAT j_step,
                                 RESCUEINT64 j_lowbound, RESCUEINT64 j_count, RESCUEINT64 k_lowbound,
                                 RESCUEINT64 k_count, RESCUEFLOAT missingValue, RescueReferenceSurface *topSurfaceIn,
                                 RESCUEFLOAT topOffsetIn, RescueReferenceSurface *bottomSurfaceIn,
  ...
  (*gridGeometries) += new RescueGeometry(parentBlock->ParentModel(), blockUnitGrid, missingValue, topSurfaceIn,
...
RescueBlockUnit::RescueBlockUnit(RescueCoordinateSystem::Orientation orientation, RescueBlock *parentBlock,
                                 ...
                                 RESCUEINT32 i_lowbound, RESCUEINT32 i_count, RESCUEFLOAT j_origin, RESCUEFLOAT j_step,
                                 RESCUEINT32 j_lowbound, RESCUEINT32 j_count, RESCUEINT32 k_lowbound,
                                 RESCUEINT32 k_count, RESCUEFLOAT missingValue, RescueReferenceSurface *topSurfaceIn,
                                 RESCUEFLOAT topOffsetIn, RescueReferenceSurface *bottomSurfaceIn,
                                 RESCUEFLOAT bottomOffsetIn)
    : RescueWireframeOwner(parentBlock->ParentModel()->Context()), block(parentBlock), unit(parentUnit),
      surfaceAboveMe(0), surfaceBelowMe(0), macroVolumes(0), propertyGroups(0), propertyGroupsID(0), color(0) {
...
RescueBlockUnit::RescueBlockUnit(RescueContext *context, FILE *archiveFile)
    : RescueWireframeOwner(context), block(0), unit(0), surfaceAboveMe(0), surfaceBelowMe(0), macroVolumes(0),
      propertyGroups(0), gridGeometries(0), propertyGroupsID(0), color(0) {
  RESCUEINT64 count;
  ...
  if (context->ReadFileVersion() >= 11 && context->ReadFileVersion() < 28) {
    ...
    while (pOrdinal < howMany) {
      ...
      for (subloop = 0; subloop < geoCount && found == FALSE; subloop++) {
        ...
        if (context->ReadFileVersion() < 18 || geometry->Identifier() == property->geometryId) {
...
void RescueBlockUnit::Relink(RescueObject *parent) {
  ...
  if (propertyGroupsID != 0) {
    ...
    for (loop = 0; loop < propertyGroupsID->Count64(); loop++) {
      ...
      RescuePropertyGroup *group =
          (RescuePropertyGroup *)RescueHistory::FindObject(block->ParentModel(), R_RescuePropertyGroup, groupID);
      if (group != 0) {
  ...
  if (block->ParentModel()->Context()->ReadFileVersion() >= 15 &&
      block->ParentModel()->Context()->ReadFileVersion() <= 21 && GridGeometry() != 0) {
...
void RescueBlockUnit::Archive(FILE *archiveFile) {
  ...
  if (GridGeometry() == 0 && context->FileVersion() < 23) {
    ...
    RescueGeometry *fakeGeometry = new RescueGeometry(block->ParentModel(), fakeGrid, (RESCUEFLOAT)-9999.0);
...
RescueModel *RescueBlockUnit::ParentModel() { return block->ParentModel(); }
...
RESCUEINT32 RescueBlockUnit::PropertyCount() { return (RESCUEINT32)PropertyCount64(); }
...
void RescueBlockUnit::SetOrientation(RescueOrientationLedger *ledger, RescueCoordinateSystem::Orientation orientation) {
  RESCUEINT64 howMany = gridGeometries->Count64();
  RESCUEINT64 loop;
  for (loop = 0; loop < howMany; loop++) {
    RescueGeometry *bug = gridGeometries->NthObject(loop);
