...
void RescueWellbore::SetWellboreName(RESCUECHAR *newName) { (*wellboreName) = newName; }

RescueWellbore::RescueWellbore(RescueCoordinateSystem::Orientation orientation, RescueModel *parentModelIn,
                               const RESCUECHAR *wellboreNameIn, RESCUEFLOAT missingValue, RESCUEINT64 i_lowbound,
                               RESCUEINT64 i_count)
    : RescueHistoryObject(parentModelIn->Context()), wellboreName(0), parentModel(parentModelIn), sampling(0),
      surfaceIntersections(0), cellIntersections(0), wellboreGrid(0) {
  ...
  wellboreGeometry = new RescueQuadrupletArray(parentModelIn->Context(), WellboreGrid(), missingValue, parentModel);
}

RescueWellbore::RescueWellbore(RescueCoordinateSystem::Orientation orientation, RescueModel *parentModelIn,
                               const RESCUECHAR *wellboreNameIn, RESCUEFLOAT missingValue, RESCUEFLOAT *quadrupletArray,
  ...
  wellboreGeometry = new RescueQuadrupletArray(WellboreGrid(), missingValue, quadrupletArray, parentModel);
}

RescueWellbore::RescueWellbore(RescueContext *context, FILE *archiveFile)
    : RescueHistoryObject(context), wellboreName(0), parentModel(0), sampling(0), surfaceIntersections(0),
      cellIntersections(0), wellboreGrid(0) {
...
void RescueWellbore::SetOrientation(RescueOrientationLedger *ledger, RescueCoordinateSystem::Orientation orientation) {
  ...
  } else if (orientation == RescueCoordinateSystem::LDF || orientation == RescueCoordinateSystem::LDB ||
             orientation == RescueCoordinateSystem::RDF || orientation == RescueCoordinateSystem::RDB) {
    SwapAxis();
  }
