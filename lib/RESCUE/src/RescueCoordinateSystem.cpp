...
RescueCoordinateSystem::RescueCoordinateSystem(
    RescueContext *contextIn, RESCUECHAR *name, RescueCoordinateSystem::Orientation displayOrientationIn,
    RescueVertex *vertexIn, const RESCUECHAR *Xproperty, const RESCUECHAR *Xuom, const RESCUECHAR *Yproperty,
    const RESCUECHAR *Yuom, const RESCUECHAR *Zproperty, const RESCUECHAR *Zuom, RESCUEINT64 insysIn,
    RESCUEINT64 inzoneIn, RESCUEINT64 inUnitIn, RESCUEINT64 insphIn, RESCUEDOUBLE semiMajorAxisIn,
    RESCUEDOUBLE eccentricityIn, RESCUEDOUBLE standardLatIn, RESCUEDOUBLE secondLatIn, RESCUEDOUBLE centralLongIn,
    RESCUEDOUBLE latOriginIn, RESCUEDOUBLE FALSEEastingIn, RESCUEDOUBLE FALSENorthingIn)
    : RescueHistoryObject(contextIn), context(contextIn), coordinateSystemName(0),
      displayOrientation(displayOrientationIn), vertex(vertexIn), insys(insysIn), inzone(inzoneIn), inUnit(inUnitIn),
      insph(insphIn), semiMajorAxis(semiMajorAxisIn), eccentricity(eccentricityIn), standardLat(standardLatIn),
      secondLat(secondLatIn), centralLong(centralLongIn), latOrigin(latOriginIn), FALSEEasting(FALSEEastingIn),
      FALSENorthing(FALSENorthingIn), epsgId(0), parentModel(0) {
...
RescueCoordinateSystem::RescueCoordinateSystem(RescueContext *contextIn, FILE *archiveFile)
    : RescueHistoryObject(contextIn), context(contextIn), coordinateSystemName(0), vertex(0), epsgId(0),
      parentModel(0) {
  RESCUECHAR name[255];
