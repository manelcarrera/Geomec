...
RescueBlock::RescueBlock(const RESCUECHAR *newBlockName, RescueModel *newBlockParent)
    : RescueHistoryObject(newBlockParent->Context()), blockName(0), parentModel(newBlockParent), blockUnits(0) {
...
RESCUEBOOL RescueBlock::IsNamed(const RESCUECHAR *possibleName) { return ((*blockName) == possibleName); }
...
RescueBlock::RescueBlock(RescueContext *context, FILE *archiveFile)
    : RescueHistoryObject(context), blockName(0), parentModel(0), blockUnits(0) {
...
void RescueBlock::SetOrientation(RescueOrientationLedger *ledger, RescueCoordinateSystem::Orientation orientation) {
  RESCUEINT64 ndx = 0;
  RescueBlockUnit *bu = (*blockUnits).NthObject(ndx++);
  while (bu != 0) {
