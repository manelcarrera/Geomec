...
RescueWellboreProperty::RescueWellboreProperty(RescueWellboreSampling *parentSamplingIn, const RESCUECHAR *propertyName,
                                               const RESCUECHAR *propertyType, const RESCUECHAR *unitOfMeasure,
                                               RESCUEUCHAR nullValue, RescueLookup *lookupIn, RESCUEUCHAR *values)
  ...
  data = new RescueArrayByte(propertyName, propertyType, unitOfMeasure, parentSampling->SampleGrid(), nullValue,
                             lookupIn, parentSampling->ParentWellbore()->ParentModel(), FALSE, values);
}

RescueWellboreProperty::RescueWellboreProperty(RescueWellboreSampling *parentSamplingIn, const RESCUECHAR *propertyName,
                                               const RESCUECHAR *propertyType, const RESCUECHAR *unitOfMeasure,
                                               RESCUEUSHORT nullValue, RescueLookup *lookupIn, RESCUEUSHORT *values)
    : RescueHistoryObject(parentSamplingIn->ParentWellbore()->ParentModel()->Context()),
      parentSampling(parentSamplingIn) {
  isA = R_RescueWellboreProperty;
  (*parentSampling->properties) += this;
  data = new RescueArrayShort(propertyName, propertyType, unitOfMeasure, parentSampling->SampleGrid(), nullValue,
                              lookupIn, parentSampling->ParentWellbore()->ParentModel(), FALSE, values);
}

RescueWellboreProperty::RescueWellboreProperty(RescueWellboreSampling *parentSamplingIn, const RESCUECHAR *propertyName,
                                               const RESCUECHAR *propertyType, const RESCUECHAR *unitOfMeasure,
                                               RESCUEFLOAT nullValue, RESCUEFLOAT *values)
    : RescueHistoryObject(parentSamplingIn->ParentWellbore()->ParentModel()->Context()),
      parentSampling(parentSamplingIn) {
  isA = R_RescueWellboreProperty;
  (*parentSampling->properties) += this;
  data = new RescueArrayFloat(propertyName, propertyType, unitOfMeasure, parentSamplingIn->SampleGrid(), nullValue,
                              parentSampling->ParentWellbore()->ParentModel(), FALSE, values);
}

RescueWellboreProperty::RescueWellboreProperty(RescueWellboreSampling *parentSamplingIn, const RESCUECHAR *propertyName,
                                               const RESCUECHAR *propertyType, const RESCUECHAR *unitOfMeasure,
                                               RESCUEINT32 nullValue, RESCUEINT32 *values)
    : RescueHistoryObject(parentSamplingIn->ParentWellbore()->ParentModel()->Context()),
      parentSampling(parentSamplingIn) {
  isA = R_RescueWellboreProperty;
  (*parentSampling->properties) += this;
  data = new RescueArrayInt(propertyName, propertyType, unitOfMeasure, parentSamplingIn->SampleGrid(), nullValue,
                            parentSampling->ParentWellbore()->ParentModel(), FALSE, values);
}

RescueWellboreProperty::RescueWellboreProperty(RescueContext *context, FILE *archiveFile)
    : RescueHistoryObject(context), parentSampling(0) {
