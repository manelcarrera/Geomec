...
RescueTrimEdge::RescueTrimEdge(RescuePolyLine *polyLineIn, _RescueLineDirection directionIn)
    : RescueObject(polyLineIn->ParentModel()->Context()), polyLine(0), ownerLoop(0), direction(directionIn) {
...
RescueTrimVertex *RescueTrimEdge::StartingPoint(RescueModel *model) {
  ...
  if (polyLineObj != 0) {
    myReturn = (direction == R_LEFT_TO_RIGHT) ? polyLineObj->LeftVertex() : polyLineObj->RightVertex();
...
RescueTrimVertexStub *RescueTrimEdge::EndingPointObj() {
  RescueTrimVertexStub *myReturn = (direction == R_LEFT_TO_RIGHT) ? rightVertex : leftVertex;
  return myReturn;
}

RescueTrimVertexStub *RescueTrimEdge::StartingPointObj() {
  RescueTrimVertexStub *myReturn = (direction == R_LEFT_TO_RIGHT) ? leftVertex : rightVertex;
  return myReturn;
}
...
RescueTrimVertex *RescueTrimEdge::EndingPoint(RescueModel *model) {
  ...
  RescuePolyLine *polyLineObj = polyLine->PolyLine(model);
  if (polyLineObj != 0) {
    myReturn = (direction == R_LEFT_TO_RIGHT) ? polyLineObj->RightVertex() : polyLineObj->LeftVertex();
    FixupStubs(polyLineObj);
  }
  return myReturn;
}
...
RescuePolyLineNode *RescueTrimEdge::NthNode(RESCUEINT64 zeroBasedOrdinal) {
  ...
  RescuePolyLine *polyLineObj = polyLine->PolyLine(0, FALSE);
  if (polyLineObj != 0) {
    myReturn = polyLineObj->PolyLineNodes()->NthObject(
        (direction == R_LEFT_TO_RIGHT) ? zeroBasedOrdinal
...
RescueTrimEdge::RescueTrimEdge(RescueContext *context, FILE *archiveFile)
    : RescueObject(context), polyLine(0), leftVertex(0), rightVertex(0), ownerLoop(0) {
...
RESCUEBOOL RescueTrimEdge::SurfaceOnEdge(RescueIJSurface &surf) { // If this edge lies on the given
