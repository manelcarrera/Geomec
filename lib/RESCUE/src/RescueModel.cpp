...
void RescueModel::RegisterObject(RescueObject *toRegister) { registeredObjects->Add(toRegister); }
...
RescueModel::RescueModel(RescueContext *contextIn, RESCUECHAR *modelNameIn)
    : RescueHistoryObject(contextIn), modelMoniker(0), coordinateSystem(0), units(0), blocks(0), horizons(0),
      wellbores(0), sections(0), referenceSurfaces(0), lookups(0), nameValuePairs(0), logicalOrder(0),
      wireframeCallBackObject(0), rotation((RESCUEFLOAT)0), properties(0), propertyContainerId(0), documents(0),
      documentListId(0) {
...
RescueModel::RescueModel(RESCUECHAR *modelNameIn, RescueCoordinateSystem *coordinateSystemIn)
    : RescueHistoryObject(coordinateSystemIn->Context()), modelMoniker(0), coordinateSystem(coordinateSystemIn),
      units(0), blocks(0), horizons(0), wellbores(0), sections(0), referenceSurfaces(0), lookups(0), nameValuePairs(0),
      logicalOrder(0), wireframeCallBackObject(0), rotation((RESCUEFLOAT)0), properties(0), propertyContainerId(0),
      documents(0), documentListId(0) {
...
RESCUEBOOL RescueModel::ArchiveModel(RESCUECHAR *pathName, RESCUEBOOL binary, RESCUEINT32 fileVersionIn,
                                     RESCUEBOOL write32in) {
  if (fileVersionIn != 9 && fileVersionIn < 19) {
    return FALSE;
  } else {
    ...
    geometryActionImmediate = FALSE;
    if (currentPathName != 0) {
      if (strcmp(vettedFileName.String(), currentPathName) != 0 || context->fileVersion != currentFileVersion) {
        propertyActionImmediate = TRUE;
        geometryActionImmediate = TRUE;
        if (oldPathName != 0) {
          free(oldPathName);
...
RescueEventDescr *RescueModel::MakeUnconformity(RESCUECHAR *name) {
  RescueEventDescr *myReturn = new RescueEventDescr(Context(), name, RescueEventDescr::UNCONFORMITY);
  (*eventDescr) += myReturn;
  return myReturn;
}
...
void RescueModel::Archive(FILE *archiveFile) {
  ...
  if (context->fileVersion > 9) {
    ...
    if (IsWireframeLoaded()) {
      if (context->delayedGeometryLoad == FALSE || WireframeHasChanged() == TRUE || geometryActionImmediate == TRUE) {
  ...
  if (context->FileVersion() >= 37) {
    if (properties == 0 && propertyContainerId != 0 && this->propertyActionImmediate == TRUE) {
    ...
    if (documents == 0 && documentListId != 0 && this->propertyActionImmediate == TRUE) {
...
RescueModel *RescueModel::UnarchiveModel(RescueContext *context, RESCUECHAR *pathName) {
  ...
  */
  if (version >= 4 && version <= FILE_VERSION) {
    context->SoftwareVersion(vettedFileName.String(), context->readFileMainSoftwareVersion,
                             context->readFileSubSoftwareVersion);
    if (context->binaryFlag) {
...
RescueModel::RescueModel(RescueContext *contextIn, FILE *archiveFile)
    : RescueHistoryObject(contextIn), modelMoniker(0), coordinateSystem(0), units(0), blocks(0), horizons(0),
      wellbores(0), sections(0), referenceSurfaces(0), lookups(0), logicalOrder(0), colorTableList(0),
      wireframeCallBackObject(0), rotation((RESCUEFLOAT)0), properties(0), propertyContainerId(0), documents(0),
      documentListId(0) {
...
RESCUECHAR *RescueModel::IJKPreference() { return nameValuePairs->GetNameValuePair("Preferred User Orientation"); }
...
void RescueModel::SetWriter(const RESCUECHAR *vendorName, const RESCUECHAR *vendorVersion, const RESCUECHAR *modelDate,
                            const RESCUECHAR *userName) {
  RCHString readable(100);
  readable << "Rescue Creator Record:Vendor=" << vendorName << " Version=" << vendorVersion << " Date=" << modelDate
           << " User=" << userName;
  RCHString stamp(200);
  stamp << "<stamp>";
  stamp << "<vendor>";
...
RCHString *RescueModel::VendorName() { return VendorSegment("<vendor>", "</vendor>"); }

RCHString *RescueModel::VendorVersion() { return VendorSegment("<version>", "</version>"); }

RCHString *RescueModel::ModelDate() { return VendorSegment("<date>", "</date>"); }

RCHString *RescueModel::UserName() { return VendorSegment("<user>", "</user>"); }

int RescueModel::SetOrientation(RescueContext *context, char *modelFileName,
    ...
    while (token != 0) {
      if (tokens->TokenIsTag("trajectory")) {
        ...
        if (dCount > 0 && wellboreName.length64() > 0) {
          ...
          if (logWellbore == 0) {
            logWellbore =
                new RescueWellbore(RescueCoordinateSystem::LUB, this, wellboreName.String(), -999.25F, 0, dCount);
            if (loggingObject != 0) {
              RCHString message;
                ...
                if (tokens->TokenIsTag("/logData")) {
                  ...
                  if (tokens->TokenIsTag("data")) {
                    ...
                    if (dCount == dAlloc) {
                      dAlloc *= 2;
                      for (pLoop = 0; pLoop < pCount; pLoop++) {
                        pValue[pSpot[bNdx][pLoop]] =
                            (RESCUEFLOAT *)realloc(pValue[pSpot[bNdx][pLoop]], sizeof(RESCUEFLOAT *) * dAlloc);
                      }
                    }
        ...
        if (wellbore != 0 && skipLog == FALSE && dCount > 0) {
          ...
          for (pLoop = 0; pLoop < pCount; pLoop++) {
            if (pLoop != pDepthNdx) {
              RescueWellboreProperty *incumbent =
                  sampling->PropertyDescribedBy(pName[pLoop], pType[pLoop], pUom[pLoop]);
              if (incumbent != 0) {
                RescueArray *data = incumbent->Data();
                if (data->IsA() != R_RescueArrayFloat) {
                  if (loggingObject != 0) {
                    RCHString message;
                    message << "Did not update property " << pName[pLoop] << "," << pType[pLoop] << "," << pUom[pLoop]
                            << " incumbent was not RescueArrayFloat";
                    loggingObject->ReportProgress(message.String());
                  }
                } else {
                  ...
                  if (loggingObject != 0) {
                    RCHString message;
                    message << "Updated existing property " << pName[pLoop] << "," << pType[pLoop] << ","
                            << pUom[pLoop];
                    loggingObject->ReportProgress(message.String());
                  }
                }
              } else {
                new RescueWellboreProperty(sampling, pName[pLoop], pType[pLoop], pUom[pLoop], pNull[pLoop],
                                           pValue[pLoop]);
                pValue[pLoop] = 0;
...
bool RescueModel::WriteWITSML(const RESCUECHAR *pathName) {
  ...
  if (file != 0) {
    ...
    fprintf(file, "%s\n",
            "<WITSMLComposite xmlns=\"http://www.witsml.org/schemas/131\" "
            "xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" "
            "xsi:schemaLocation=\"http://www.witsml.org/schemas/131 "
            "http://www.witsml.org/schemas/131/obj_trajectory.xsd\" version=\"1.3.1.0\">");
    fprintf(file, "%s\n", "  <allObjects>");
    fprintf(file, "%s\n", "    <wellDatum>");
