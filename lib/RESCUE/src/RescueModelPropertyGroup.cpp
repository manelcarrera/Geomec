...
RescueModelPropertyGroup::RescueModelPropertyGroup(RESCUECHAR *groupNameIn, RescueModel *modelIn)
    : RescueHistoryObject(modelIn->Context()), parentModel(modelIn), groupName(0), groups(0), isLocked(FALSE),
      propertyGroupsID(0) {
...
void RescueModelPropertyGroup::SetLock() {
  ...
  for (loop = 0; loop < (*groups).Count64(); loop++) {
    RescuePropertyGroup *group = (RescuePropertyGroup *)(*groups).NthObject(loop);
    group->SetLock();
  }
}
...
RescueModelPropertyGroup::RescueModelPropertyGroup(RescueContext *context, FILE *archiveFile)
    : RescueHistoryObject(context), parentModel(0), groupName(0), groups(0), isLocked(FALSE), propertyGroupsID(0) {
  ...
  if (propertyGroupsID != 0) {
    ...
    for (loop = 0; loop < propertyGroupsID->Count64(); loop++) {
      ...
      RescuePropertyGroup *group =
          (RescuePropertyGroup *)RescueHistory::FindObject(parentModel, R_RescuePropertyGroup, groupID);
      if (group != 0) {
        (*groups) += group;
      }
    }
