...
RescueTriangulatedSurface::RescueTriangulatedSurface(RescueContext *context, FILE *archiveFile)
    : RescueObject(context), verticesLookups(0), vertices(new cSetRescueTriangleVertex),
      faces(new cSetRescueTriangleFace), octTrees(new cSetRescueSOctTreeNode), faceGrid(0), nodeGrid(0),
      arrayVersion(0), hasChanged(FALSE), loaded(FALSE), faceStatus(0) {
...
void RescueTriangulatedSurface::Archive(FILE *archiveFile) {
  ...
  if (IsLoaded()) {
    if (context->delayedGeometryLoad == FALSE || hasChanged == TRUE ||
...
void RescueTriangulatedSurface::SwapAxes(bool swapI, RescueGridAxis *iAxis, bool swapJ, RescueGridAxis *jAxis) {
  if (IsLoaded() == FALSE) {
    Load();
  }
