...
RescueGeometryPropertyList::RescueGeometryPropertyList(RescueGeometry *geometry, RESCUEBOOL byTimeStepIn,
                                                       RESCUEBOOL includeLGRsIn, RescuePropertyGroup *nullOrGroup,
                                                       RESCUECHAR *nullOrTimeStepName)
    : includeLGRs(includeLGRsIn), byTimeStep(byTimeStepIn) {
  count = 0;
  allocated = 100;
...
void RescueGeometryPropertyList::AddGroup(RescuePropertyGroup *toAdd, RescueGeometry *geometry,
                                          RESCUECHAR *nullOrTimeStepName) {
  RESCUEINT32 tsNdx = 0;
...
void RescueGeometryPropertyList::AddRow(RescueProperty *property, RescueGeometry *geom, RescueTimeStepGroup *ts,
...
RESCUEINT64 RescueGeometryPropertyList::NdxOf(RescueProperty *property, RescueGeometry *geom, RescueTimeStepGroup *ts,
                                              RescuePropertyGroup *group) {
  RESCUEINT64 ndx = 0;
...
RESCUEINT64 RescueGeometryPropertyList::CompareRow(RESCUEINT64 ndx, RescueProperty *property, RescueGeometry *geom,
                                                   RescueTimeStepGroup *ts, RescuePropertyGroup *group) {
  RESCUEINT64 myReturn = 0;
  if (byTimeStep) {
    myReturn = strcmp(ts->TimeStepName()->NonNullString(), timeSteps[ndx]->TimeStepName()->NonNullString());
    if (myReturn == 0) {
