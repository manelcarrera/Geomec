...
RescueTriangulatedSurface *RescueSurface::CreateTriangulatedSurface(RESCUEINT32 vertexCount, RESCUEFLOAT **coordinates,
                                                                    RESCUEINT32 faceCount, RESCUEINT32 **faceNdxs) {
...
RescueTriangulatedSurface *RescueSurface::CreateTriangulatedSurface(RESCUEINT64 vertexCount, RESCUEFLOAT **coordinates,
                                                                    RESCUEINT64 faceCount, RESCUEINT64 **faceNdxs) {
  RescueContext *context = ParentModel()->Context();
  cSetRescueTriangleVertex *vertices = new cSetRescueTriangleVertex();
  cSetRescueTriangleFace *faces = new cSetRescueTriangleFace();
  RESCUEINT64 loop;
...
RescueSurface::RescueSurface(RescueModel *modelIn, RescueCoordinateSystem::Orientation orientation,
                             RescueCoordinateSystemAxis *i_axis, RESCUEFLOAT i_origin, RESCUEFLOAT i_step,
                             RESCUEINT64 i_lowbound, RESCUEINT64 i_count, RescueCoordinateSystemAxis *j_axis,
                             RESCUEFLOAT j_origin, RESCUEFLOAT j_step, RESCUEINT64 j_lowbound, RESCUEINT64 j_count,
                             RESCUEFLOAT missingValue, SurfaceType typeIn)
    : RescueWireframeOwner(modelIn->Context()), edges(0), type(typeIn), color(0), properties(0), groups(0),
      triangulatedSurface(0), ijSurfaces(0), propertyContainers(0), propertyContainerId(0) {
  parentModel = modelIn;
  ijSurfaces = new cSetRescueIJSurface();
  groups = new cSetRescuePropertyGroup();
  properties = new cSetRescueProperty();
  isA = R_RescueSurface;
  new RescueIJSurface(this, orientation, i_axis, i_origin, i_step, i_lowbound, i_count, j_axis, j_origin, j_step,
...
RescueSurface::RescueSurface(RescueModel *modelIn, RescueCoordinateSystem::Orientation orientation,
                             ...
                             RESCUEINT64 i_lowbound, RESCUEINT64 i_count, RescueCoordinateSystemAxis *j_axis,
                             RESCUEFLOAT j_origin, RESCUEFLOAT j_step, RESCUEINT64 j_lowbound, RESCUEINT64 j_count,
                             RESCUEFLOAT missingValue, RESCUEFLOAT *values, SurfaceType typeIn)
    : RescueWireframeOwner(modelIn->Context()), edges(0), type(typeIn), color(0), properties(0), groups(0),
      triangulatedSurface(0), ijSurfaces(0), propertyContainers(0), propertyContainerId(0) {
  parentModel = modelIn;
  ijSurfaces = new cSetRescueIJSurface();
  ...
  isA = R_RescueSurface;
  new RescueIJSurface(this, orientation, i_axis, i_origin, i_step, i_lowbound, i_count, j_axis, j_origin, j_step,
                      j_lowbound, j_count, missingValue, values);
}
...
RescueSurface::RescueSurface(RescueModel *modelIn, RescueCoordinateSystem::Orientation orientation,
  ...
  properties = new cSetRescueProperty();
  isA = R_RescueSurface;
  new RescueIJSurface(this, orientation, i_lowbound, i_count, j_lowbound, j_count, missingValue);
}

RescueSurface::RescueSurface(RescueModel *modelIn, RescueCoordinateSystem::Orientation orientation,
                             RESCUEINT64 i_lowbound, RESCUEINT64 i_count, RESCUEINT64 j_lowbound, RESCUEINT64 j_count,
                             RESCUEFLOAT missingValue, RESCUEFLOAT *tripletArray, SurfaceType typeIn)
    : RescueWireframeOwner(modelIn->Context()), edges(0), type(typeIn), color(0), properties(0), groups(0),
      triangulatedSurface(0), ijSurfaces(0), propertyContainers(0), propertyContainerId(0) {
  parentModel = modelIn;
  ijSurfaces = new cSetRescueIJSurface();
  ...
  properties = new cSetRescueProperty();
  isA = R_RescueSurface;
  new RescueIJSurface(this, orientation, i_lowbound, i_count, j_lowbound, j_count, missingValue, tripletArray);
}

RescueSurface::RescueSurface(RescueContext *context, FILE *archiveFile)
    : RescueWireframeOwner(context), edges(0), color(0), properties(0), groups(0), triangulatedSurface(0),
      ijSurfaces(0), propertyContainers(0), propertyContainerId(0) {
  ijSurfaces = new cSetRescueIJSurface();
...
void RescueSurface::SetOrientation(RescueOrientationLedger *ledger, RescueCoordinateSystem::Orientation orientation) {
  RESCUEINT64 sOrd = 0;
  ...
  while (ijSurf != 0) {
    if (sOrd == 1 && triangulatedSurface != 0) {
      ...
      if (swapI == true || swapJ == true) {
        triangulatedSurface->SwapAxes(swapI, ijSurf->Grid()->Axis(0), swapJ, ijSurf->Grid()->Axis(1));
      }
    }
