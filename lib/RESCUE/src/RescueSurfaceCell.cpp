...
RescueSurfaceCell::RescueSurfaceCell(RescueGeometry *geometryIn, RescueIJSurface *surfaceIn, RESCUEINT64 cellNumberIn,
                                     RESCUEINT64 cellFaceNumberIn, SurfaceSide sideIn, RESCUEFLOAT *uvsIn)
    : RescueObject(geometryIn->ParentModel()->Context()), geometry(geometryIn), surface(surfaceIn),
      cellNumber(cellNumberIn), cellFaceNumber(cellFaceNumberIn), side(sideIn), uvs(uvsIn) {
...
RescueSurfaceCell::RescueSurfaceCell(RescueContext *context, FILE *archiveFile)
    : RescueObject(context), surface(0), uvs(0) {
