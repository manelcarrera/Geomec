...
RescueEdgeSet::RescueEdgeSet(RescueWireframeOwner *ownerIn)
    : RescueObject(ownerIn->ParentModel()->Context()), boundaryLoops(0), interiorLoops(0), owner(ownerIn->Wireframe()),
      ownerIsA((_RescueObjectType)0), ownerId(0) {
  isA = R_RescueEdgeSet;
...
RescueEdgeSet::RescueEdgeSet(RescueContext *context, FILE *archiveFile)
    : RescueObject(context), boundaryLoops(0), interiorLoops(0), owner(0), ownerIsA((_RescueObjectType)0), ownerId(0) {
  isA = R_TOMBSTONE;
