...
RESCUEBOOL RCHString::operator!=(RCHString &other)
{
...
void RCHString::tokenize(const RESCUECHAR *delimitersIn, RESCUEBOOL eachIn, 
                                             RESCUEBOOL respectQuotesIn)
{
  if (delimiters != 0)
  {
    free(delimiters);
...
RESCUEBOOL RCHString::StartsWith(RCHString &other)
{
  return StartsWith(other.String());
}
...
RESCUEBOOL RCHString::EndsWith(RCHString &other)
{
  return EndsWith(other.String());
}
