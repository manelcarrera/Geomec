...
RescueHorizon::RescueHorizon(RESCUECHAR *horizonNameIn, RescueModel *parentModelIn)
    : RescueLogicalOrderEntry(parentModelIn->Context()), horizonName(0), parentModel(parentModelIn), surfaces(0),
      unitAboveID(0), unitBelowID(0), eventDescrAboveID(0), eventDescrBelowID(0) {
  isA = R_RescueHorizon;
...
RescueHorizon::RescueHorizon(RescueContext *context, FILE *archiveFile)
    : RescueLogicalOrderEntry(context), horizonName(0), parentModel(0), surfaces(0), unitAboveID(0), unitBelowID(0),
      eventDescrAboveID(0), eventDescrBelowID(0) {
...
RESCUEBOOL RescueHorizon::IsNamed(const RESCUECHAR *possibleName) { return ((*horizonName) == possibleName); }
...
void RescueHorizon::SetOrientation(RescueOrientationLedger *ledger, RescueCoordinateSystem::Orientation orientation) {
  RESCUEINT64 ndx = 0;
...
void RescueHorizon::FindUniquePropertyNames(cSetString *container) { surfaces->FindUniquePropertyNames(container); }
