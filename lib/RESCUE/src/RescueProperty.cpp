...
RescueProperty::RescueProperty(RescueDataContainer *parentContainer, RescueModel *model,
                               const RESCUECHAR *propertyNameIn, const RESCUECHAR *propertyTypeIn,
                               const RESCUECHAR *unitOfMeasureIn, AppliesToSurface applies, RESCUEFLOAT nullValue,
                               RESCUEFLOAT *valueArray, AttachmentPoint attachedToIn)
    : RescueHistoryObject(model->Context()), parentBlockUnit(0), parentGeobodyPart(0), geometry(0),
      container(parentContainer), surface(0), loops(0), howApplies(applies), attachedTo(attachedToIn),
      triangulatedSurface(FALSE), ijSurface(0), isLocked(FALSE), context(model->Context()), colorTable(0) {
  ...
  data = new RescueArrayFloat(propertyNameIn, propertyTypeIn, unitOfMeasureIn, container->Grid(), nullValue, model,
                              cellCentered, valueArray);
}

RescueProperty::RescueProperty(RescueDataContainer *containerIn, RescueModel *modelIn, const RESCUECHAR *propertyNameIn,
                               const RESCUECHAR *propertyTypeIn, const RESCUECHAR *unitOfMeasureIn,
                               AppliesToSurface applies, RESCUEINT32 nullValue, RESCUEINT32 *valueArray,
                               AttachmentPoint attachedToIn)
    : RescueHistoryObject(modelIn->Context()), parentBlockUnit(0), parentGeobodyPart(0), geometry(0),
      container(containerIn), surface(0), loops(0), howApplies(applies), attachedTo(attachedToIn),
      triangulatedSurface(FALSE), ijSurface(0), isLocked(FALSE), context(modelIn->Context()), colorTable(0) {
  RESCUEBOOL cellCentered = TRUE;
  if (attachedTo != CELL_CENTERED && attachedTo != FACE_CENTERED) {
    cellCentered = FALSE;
  }
  ...
  (*container->properties) += this;
  loops = new cSetRescueTrimLoopStub();
  data = new RescueArrayInt(propertyNameIn, propertyTypeIn, unitOfMeasureIn, containerIn->Grid(), nullValue, modelIn,
                            cellCentered, valueArray);
}

RescueProperty::RescueProperty(RescueDataContainer *containerIn, RescueModel *modelIn, const RESCUECHAR *propertyNameIn,
                               const RESCUECHAR *propertyTypeIn, const RESCUECHAR *unitOfMeasureIn,
                               AppliesToSurface applies, RescueLookup *lookupTableIn, RESCUEUCHAR nullValue,
                               RESCUEUCHAR *valueArray, AttachmentPoint attachedToIn)
    : RescueHistoryObject(modelIn->Context()), parentBlockUnit(0), parentGeobodyPart(0), geometry(0),
      container(containerIn), surface(0), loops(0), howApplies(applies), attachedTo(attachedToIn),
      triangulatedSurface(FALSE), ijSurface(0), isLocked(FALSE), context(modelIn->Context()), colorTable(0) {
  RESCUEBOOL cellCentered = TRUE;
  ...
  (*container->properties) += this;
  loops = new cSetRescueTrimLoopStub();
  data = new RescueArrayByte(propertyNameIn, propertyTypeIn, unitOfMeasureIn, container->Grid(), nullValue,
                             lookupTableIn, modelIn, cellCentered, valueArray);
}

RescueProperty::RescueProperty(RescueDataContainer *containerIn, RescueModel *modelIn, const RESCUECHAR *propertyNameIn,
                               const RESCUECHAR *propertyTypeIn, const RESCUECHAR *unitOfMeasureIn,
                               AppliesToSurface applies, RescueLookup *lookupTableIn, RESCUEUSHORT nullValue,
                               RESCUEUSHORT *valueArray, AttachmentPoint attachedToIn)
    : RescueHistoryObject(modelIn->Context()), parentBlockUnit(0), parentGeobodyPart(0), geometry(0),
      container(containerIn), surface(0), loops(0), howApplies(applies), attachedTo(attachedToIn),
      triangulatedSurface(FALSE), ijSurface(0), isLocked(FALSE), context(modelIn->Context()), colorTable(0) {
  RESCUEBOOL cellCentered = TRUE;
  ...
  (*container->properties) += this;
  loops = new cSetRescueTrimLoopStub();
  data = new RescueArrayShort(propertyNameIn, propertyTypeIn, unitOfMeasureIn, container->Grid(), nullValue,
                              lookupTableIn, modelIn, cellCentered, valueArray);
}

RescueProperty::RescueProperty(RescueDataContainer *containerIn, RescueModel *modelIn, const RESCUECHAR *propertyNameIn,
                               const RESCUECHAR *propertyTypeIn, const RESCUECHAR *unitOfMeasureIn,
                               AppliesToSurface applies, Rescue2dVector nullValue, Rescue2dVector *valueArray,
                               AttachmentPoint attachedToIn)
    : RescueHistoryObject(modelIn->Context()), parentBlockUnit(0), parentGeobodyPart(0), geometry(0),
      container(containerIn), surface(0), loops(0), howApplies(applies), attachedTo(attachedToIn),
      triangulatedSurface(FALSE), ijSurface(0), isLocked(FALSE), context(modelIn->Context()), colorTable(0) {
  ...
  (*container->properties) += this;
  loops = new cSetRescueTrimLoopStub();
  data = new RescueArray2dVector(propertyNameIn, propertyTypeIn, unitOfMeasureIn, container->Grid(), nullValue, modelIn,
                                 cellCentered, valueArray);
}

RescueProperty::RescueProperty(RescueDataContainer *containerIn, RescueModel *modelIn, const RESCUECHAR *propertyNameIn,
                               const RESCUECHAR *propertyTypeIn, const RESCUECHAR *unitOfMeasureIn,
                               AppliesToSurface applies, Rescue3dVector nullValue, Rescue3dVector *valueArray,
                               AttachmentPoint attachedToIn)
    : RescueHistoryObject(modelIn->Context()), parentBlockUnit(0), parentGeobodyPart(0), geometry(0),
      container(containerIn), surface(0), loops(0), howApplies(applies), attachedTo(attachedToIn),
      triangulatedSurface(FALSE), ijSurface(0), isLocked(FALSE), context(modelIn->Context()), colorTable(0) {
  ...
  (*container->properties) += this;
  loops = new cSetRescueTrimLoopStub();
  data = new RescueArray3dVector(propertyNameIn, propertyTypeIn, unitOfMeasureIn, container->Grid(), nullValue, modelIn,
                                 cellCentered, valueArray);
}

RescueProperty::RescueProperty(RescueTriangulatedSurface *parentSurface, const RESCUECHAR *propertyNameIn,
                               const RESCUECHAR *propertyTypeIn, const RESCUECHAR *unitOfMeasureIn,
                               AppliesToSurface applies, RESCUEFLOAT nullValue, RESCUEFLOAT *valueArray,
                               AttachmentPoint attachedToIn)
    : RescueHistoryObject(parentSurface->ParentSurface()->ParentModel()->Context()), parentBlockUnit(0),
      parentGeobodyPart(0), geometry(0), container(0), surface(parentSurface->ParentSurface()), loops(0),
      howApplies(applies), attachedTo(attachedToIn), triangulatedSurface(TRUE), ijSurface(0), isLocked(FALSE),
      context(parentSurface->ParentSurface()->ParentModel()->Context()), colorTable(0) {
  isA = R_RescueProperty;
  (*parentSurface->ParentSurface()->properties) += this;
  VetIJSurface();
  loops = new cSetRescueTrimLoopStub();
  data = new RescueArrayFloat(propertyNameIn, propertyTypeIn, unitOfMeasureIn, parentSurface->Grid(attachedTo),
                              nullValue, parentSurface->ParentSurface()->ParentModel(), TRUE, valueArray);
}
...
RescueProperty::RescueProperty(RescueTriangulatedSurface *parentSurface, const RESCUECHAR *propertyNameIn,
  ...
  VetIJSurface();
  loops = new cSetRescueTrimLoopStub();
  data = new RescueArrayInt(propertyNameIn, propertyTypeIn, unitOfMeasureIn, parentSurface->Grid(attachedTo), nullValue,
                            parentSurface->ParentSurface()->ParentModel(), TRUE, valueArray);
}
...
RescueProperty::RescueProperty(RescueTriangulatedSurface *parentSurface, const RESCUECHAR *propertyNameIn,
  ...
  VetIJSurface();
  loops = new cSetRescueTrimLoopStub();
  data = new RescueArrayByte(propertyNameIn, propertyTypeIn, unitOfMeasureIn, parentSurface->Grid(attachedTo),
                             nullValue, lookupTableIn, parentSurface->ParentSurface()->ParentModel(), TRUE, valueArray);
}
...
RescueProperty::RescueProperty(RescueTriangulatedSurface *parentSurface, const RESCUECHAR *propertyNameIn,
  ...
  loops = new cSetRescueTrimLoopStub();
  data =
      new RescueArrayShort(propertyNameIn, propertyTypeIn, unitOfMeasureIn, parentSurface->Grid(attachedTo), nullValue,
                           lookupTableIn, parentSurface->ParentSurface()->ParentModel(), TRUE, valueArray);
}
...
RescueProperty::RescueProperty(RescueTriangulatedSurface *parentSurface, const RESCUECHAR *propertyNameIn,
  ...
  VetIJSurface();
  loops = new cSetRescueTrimLoopStub();
  data = new RescueArray2dVector(propertyNameIn, propertyTypeIn, unitOfMeasureIn, parentSurface->Grid(attachedTo),
                                 nullValue, parentSurface->ParentSurface()->ParentModel(), TRUE, valueArray);
}
...
RescueProperty::RescueProperty(RescueTriangulatedSurface *parentSurface, const RESCUECHAR *propertyNameIn,
  ...
  VetIJSurface();
  loops = new cSetRescueTrimLoopStub();
  data = new RescueArray3dVector(propertyNameIn, propertyTypeIn, unitOfMeasureIn, parentSurface->Grid(attachedTo),
                                 nullValue, parentSurface->ParentSurface()->ParentModel(), TRUE, valueArray);
}

RescueProperty::RescueProperty(RescueSurface *parentSurface, const RESCUECHAR *propertyNameIn,
                               const RESCUECHAR *propertyTypeIn, const RESCUECHAR *unitOfMeasureIn,
                               AppliesToSurface applies, RESCUEFLOAT nullValue, RESCUEFLOAT *valueArray,
                               AttachmentPoint attachedToIn, RescueIJSurface *surfaceRepresentation)
    : RescueHistoryObject(parentSurface->ParentModel()->Context()), parentBlockUnit(0), parentGeobodyPart(0),
      geometry(0), container(0), surface(parentSurface), loops(0), howApplies(applies), attachedTo(attachedToIn),
      triangulatedSurface(FALSE), ijSurface(surfaceRepresentation), isLocked(FALSE),
      context(parentSurface->ParentModel()->Context()), colorTable(0) {
  isA = R_RescueProperty;
  ...
  data = new RescueArrayFloat(propertyNameIn, propertyTypeIn, unitOfMeasureIn, ijSurface->Grid(), nullValue,
                              parentSurface->ParentModel(), cellCentered, valueArray);
}
...
RescueProperty::RescueProperty(RescueSurface *parentSurface, const RESCUECHAR *propertyNameIn,
  ...
  data = new RescueArrayInt(propertyNameIn, propertyTypeIn, unitOfMeasureIn, ijSurface->Grid(), nullValue,
                            parentSurface->ParentModel(), cellCentered, valueArray);
}
...
RescueProperty::RescueProperty(RescueSurface *parentSurface, const RESCUECHAR *propertyNameIn,
  ...
  data = new RescueArrayByte(propertyNameIn, propertyTypeIn, unitOfMeasureIn, ijSurface->Grid(), nullValue,
                             lookupTableIn, surface->ParentModel(), cellCentered, valueArray);
}
...
RescueProperty::RescueProperty(RescueSurface *parentSurface, const RESCUECHAR *propertyNameIn,
  ...
  data = new RescueArrayShort(propertyNameIn, propertyTypeIn, unitOfMeasureIn, ijSurface->Grid(), nullValue,
                              lookupTableIn, surface->ParentModel(), cellCentered, valueArray);
}
...
RescueProperty::RescueProperty(RescueSurface *parentSurface, const RESCUECHAR *propertyNameIn,
  ...
  data = new RescueArray2dVector(propertyNameIn, propertyTypeIn, unitOfMeasureIn, ijSurface->Grid(), nullValue,
                                 parentSurface->ParentModel(), cellCentered, valueArray);
}
...
RescueProperty::RescueProperty(RescueSurface *parentSurface, const RESCUECHAR *propertyNameIn,
  ...
  data = new RescueArray3dVector(propertyNameIn, propertyTypeIn, unitOfMeasureIn, ijSurface->Grid(), nullValue,
                                 surface->ParentModel(), cellCentered, valueArray);
}

RescueProperty::RescueProperty(RescueGeometry *geometryIn, const RESCUECHAR *propertyNameIn,
                               const RESCUECHAR *propertyTypeIn, const RESCUECHAR *unitOfMeasureIn,
                               RESCUEFLOAT nullValue, RESCUEFLOAT *valueArray, AttachmentPoint attachedToIn)
    : RescueHistoryObject(geometryIn->ParentModel()->Context()), parentBlockUnit(0), parentGeobodyPart(0),
      geometry(geometryIn), container(0), surface(0), loops(0), howApplies(RescueProperty::NA),
      attachedTo(attachedToIn), triangulatedSurface(FALSE), ijSurface(0), isLocked(FALSE),
      context(geometryIn->ParentModel()->Context()), colorTable(0) {
  isA = R_RescueProperty;
  (*geometry->properties) += this;
  parentBlockUnit = geometry->ParentBlockUnit();
  parentGeobodyPart = geometry->ParentGeobodyPart();
  data = new RescueArrayFloat(propertyNameIn, propertyTypeIn, unitOfMeasureIn, geometry->Grid(), nullValue,
                              geometry->ParentModel(), (attachedTo != NODE_CENTERED), valueArray);
}

RescueProperty::RescueProperty(RescueGeometry *geometryIn, const RESCUECHAR *propertyNameIn,
                               const RESCUECHAR *propertyTypeIn, const RESCUECHAR *unitOfMeasureIn,
                               RESCUEINT32 nullValue, RESCUEINT32 *valueArray, AttachmentPoint attachedToIn)
    : RescueHistoryObject(geometryIn->ParentModel()->Context()), parentBlockUnit(0), parentGeobodyPart(0),
      geometry(geometryIn), container(0), surface(0), loops(0), howApplies(RescueProperty::NA),
      attachedTo(attachedToIn), triangulatedSurface(FALSE), isLocked(FALSE),
      context(geometryIn->ParentModel()->Context()), colorTable(0) {
  isA = R_RescueProperty;
  (*geometry->properties) += this;
  parentBlockUnit = geometry->ParentBlockUnit();
  parentGeobodyPart = geometry->ParentGeobodyPart();
  data = new RescueArrayInt(propertyNameIn, propertyTypeIn, unitOfMeasureIn, geometry->Grid(), nullValue,
                            geometry->ParentModel(), (attachedTo != NODE_CENTERED), valueArray);
}

RescueProperty::RescueProperty(RescueGeometry *geometryIn, const RESCUECHAR *propertyNameIn,
                               const RESCUECHAR *propertyTypeIn, const RESCUECHAR *unitOfMeasureIn,
                               Rescue2dVector nullValue, Rescue2dVector *valueArray, AttachmentPoint attachedToIn)
    : RescueHistoryObject(geometryIn->ParentModel()->Context()), parentBlockUnit(0), parentGeobodyPart(0),
      geometry(geometryIn), container(0), surface(0), loops(0), howApplies(RescueProperty::NA),
      attachedTo(attachedToIn), triangulatedSurface(FALSE), isLocked(FALSE),
      context(geometryIn->ParentModel()->Context()), colorTable(0) {
  ...
  parentBlockUnit = geometry->ParentBlockUnit();
  parentGeobodyPart = geometry->ParentGeobodyPart();
  data = new RescueArray2dVector(propertyNameIn, propertyTypeIn, unitOfMeasureIn, geometry->Grid(), nullValue,
                                 geometry->ParentModel(), (attachedTo != NODE_CENTERED), valueArray);
}

RescueProperty::RescueProperty(RescueGeometry *geometryIn, const RESCUECHAR *propertyNameIn,
                               const RESCUECHAR *propertyTypeIn, const RESCUECHAR *unitOfMeasureIn,
                               Rescue3dVector nullValue, Rescue3dVector *valueArray, AttachmentPoint attachedToIn)
    : RescueHistoryObject(geometryIn->ParentModel()->Context()), parentBlockUnit(0), parentGeobodyPart(0),
      geometry(geometryIn), container(0), surface(0), loops(0), howApplies(RescueProperty::NA),
      attachedTo(attachedToIn), triangulatedSurface(FALSE), isLocked(FALSE),
      context(geometryIn->ParentModel()->Context()), colorTable(0) {
  ...
  parentBlockUnit = geometry->ParentBlockUnit();
  parentGeobodyPart = geometry->ParentGeobodyPart();
  data = new RescueArray3dVector(propertyNameIn, propertyTypeIn, unitOfMeasureIn, geometry->Grid(), nullValue,
                                 geometry->ParentModel(), (attachedTo != NODE_CENTERED), valueArray);
}

RescueProperty::RescueProperty(RescueGeometry *geometryIn, const RESCUECHAR *propertyNameIn,
                               const RESCUECHAR *propertyTypeIn, const RESCUECHAR *unitOfMeasureIn,
                               RescueLookup *lookupTableIn, RESCUEUCHAR nullValue, RESCUEUCHAR *valueArray,
                               AttachmentPoint attachedToIn)
    : RescueHistoryObject(geometryIn->ParentModel()->Context()), parentBlockUnit(0), parentGeobodyPart(0),
      geometry(geometryIn), container(0), surface(0), loops(0), howApplies(RescueProperty::NA),
      attachedTo(attachedToIn), triangulatedSurface(FALSE), isLocked(FALSE),
  ...
  parentBlockUnit = geometry->ParentBlockUnit();
  parentGeobodyPart = geometry->ParentGeobodyPart();
  data = new RescueArrayByte(propertyNameIn, propertyTypeIn, unitOfMeasureIn, geometry->Grid(), nullValue,
                             lookupTableIn, geometry->ParentModel(), (attachedTo != NODE_CENTERED), valueArray);
}

RescueProperty::RescueProperty(RescueGeometry *geometryIn, const RESCUECHAR *propertyNameIn,
                               const RESCUECHAR *propertyTypeIn, const RESCUECHAR *unitOfMeasureIn,
                               RescueLookup *lookupTableIn, RESCUEUSHORT nullValue, RESCUEUSHORT *valueArray,
                               AttachmentPoint attachedToIn)
    : RescueHistoryObject(geometryIn->ParentModel()->Context()), parentBlockUnit(0), parentGeobodyPart(0),
      geometry(geometryIn), container(0), surface(0), loops(0), howApplies(RescueProperty::NA),
      attachedTo(attachedToIn), triangulatedSurface(FALSE), isLocked(FALSE),
  ...
  parentBlockUnit = geometry->ParentBlockUnit();
  parentGeobodyPart = geometry->ParentGeobodyPart();
  data = new RescueArrayShort(propertyNameIn, propertyTypeIn, unitOfMeasureIn, geometry->Grid(), nullValue,
                              lookupTableIn, geometry->ParentModel(), (attachedTo != NODE_CENTERED), valueArray);
}
...
RescueProperty::RescueProperty(RescueContext *contextIn, FILE *archiveFile)
    : RescueHistoryObject(contextIn), parentBlockUnit(0), parentGeobodyPart(0), geometry(0), container(0), surface(0),
      loops(0), triangulatedSurface(FALSE), ijSurface(0), isLocked(FALSE), context(contextIn), colorTable(0) {
...
void RescueProperty::Archive(FILE *archiveFile) {
  ...
  if (parentBlockUnit != 0) {
    ...
    */
    if (context->FileVersion() < 20) {
      if (data->IsA() == R_RescueArray2dVector || data->IsA() == R_RescueArray3dVector) {
        archiveGeometry = FALSE;
      }
    }
    /*
  ...
  if (context->FileVersion() >= 20) {
                                                     ...
                                                     // now test is if geobodyPart or global to model.
    {
      ...
      if (triangulatedSurface && data->ParentModel()->propertyActionImmediate) {
...
RESCUEBOOL RescueProperty::IsNamed(const RESCUECHAR *possibleName) { return ((*data->propertyName) == possibleName); }
...
void RescueProperty::SwapAxes(bool swapI, RESCUEINT64 iNodes, bool swapJ, RESCUEINT64 jNodes, bool swapK,
                              RESCUEINT64 kNodes) {
  if (data->CellCentered() == TRUE) {
    iNodes--;
    jNodes--;
