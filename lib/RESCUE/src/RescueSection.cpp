...
RescueSection::RescueSection(RescueCoordinateSystem::Orientation orientation, RESCUECHAR *newSectionName,
                             RescueModel *parentModelIn, RescueSurface::SurfaceType typeIn, RESCUEINT64 i_lowbound,
                             RESCUEINT64 i_count, RESCUEINT64 j_lowbound, RESCUEINT64 j_count, RESCUEFLOAT missingValue)
    : RescueSurface(parentModelIn, orientation, i_lowbound, i_count, j_lowbound, j_count, missingValue, typeIn),
      sectionName(0) {
  isA = R_RescueSection;
...
RescueSection::RescueSection(RescueCoordinateSystem::Orientation orientation, RESCUECHAR *newSectionName,
                             RescueModel *parentModelIn, RescueSurface::SurfaceType typeIn, RESCUEFLOAT missingValue)
    : RescueSurface(parentModelIn, orientation, 0, 0, 0, 0, missingValue, typeIn), sectionName(0) {
  isA = R_RescueSection;
  sectionName = new RCHString(newSectionName);
  (*parentModel->sections) += this;
}
...
RescueSection::RescueSection(RescueCoordinateSystem::Orientation orientation, RESCUECHAR *newSectionName,
    ...
    : RescueSurface(parentModelIn, orientation, i_lowbound, i_count, j_lowbound, j_count, missingValue, tripletArray,
                    typeIn),
      sectionName(0) {
  isA = R_RescueSection;
  sectionName = new RCHString(newSectionName);
...
RescueSection::RescueSection(RescueCoordinateSystem::Orientation orientation, RESCUECHAR *newSectionName,
                             RescueModel *modelIn, SurfaceType typeIn, RescueCoordinateSystemAxis *i_axis,
                             RESCUEFLOAT i_origin, RESCUEFLOAT i_step, RESCUEINT64 i_lowbound, RESCUEINT64 i_count,
                             RescueCoordinateSystemAxis *j_axis, RESCUEFLOAT j_origin, RESCUEFLOAT j_step,
...
RESCUEBOOL RescueSection::IsNamed(const RESCUECHAR *possibleName) { return ((*sectionName) == possibleName); }
