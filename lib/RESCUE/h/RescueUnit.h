...
public:
  ...
  RESCUEINT32 PropertyGroupCount(RESCUEBOOL throwIfTooBig) {
    return RescueContext::Return32For64(groups->Count(), throwIfTooBig);
  }
  ...
  RESCUEBOOL DropRescuePropertyGroup(RescuePropertyGroup *unitToDrop) { return ((*groups) -= unitToDrop); }

  void AddBlockUnit(RescueBlockUnit *existingBlockUnit) { (*blockUnits) += existingBlockUnit; }
