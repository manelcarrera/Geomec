...
private:
  RescueArrayShort(const RESCUECHAR *propertyNameIn, const RESCUECHAR *propertyTypeIn,
                   const RESCUECHAR *unitOfMeasureIn, RescueGrid *existingGrid, RESCUEUSHORT nullValueIn,
                   RescueLookup *lookupIn, RescueModel *parentModelIn, RESCUEBOOL cellCenteredIn = FALSE,
                   RESCUEUSHORT *valueArray = 0)
      : RescueArray(propertyNameIn, propertyTypeIn, unitOfMeasureIn, existingGrid, parentModelIn, cellCenteredIn),
        nullValue(nullValueIn), value(valueArray), lookupTable(lookupIn) {
  ...
  virtual void SwapAxes(bool swapI, RESCUEINT64 iNodes, bool swapJ, RESCUEINT64 jNodes, bool swapK, RESCUEINT64 kNodes);
