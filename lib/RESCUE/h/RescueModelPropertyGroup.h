...
public:
  RescueModelPropertyGroup(RESCUECHAR *groupNameIn, RescueModel *modelIn);
  virtual ~RescueModelPropertyGroup();
  RCHString *GroupName() { return groupName; }
  // Do NOT delete the returned object.
  void SetGroupName(const RESCUECHAR *newGroupName) { (*groupName) = newGroupName; }
  RESCUEBOOL IsNamed(const RESCUECHAR *possibleName) { return ((*groupName) == possibleName); }
  ...
  RescuePropertyGroup *NthGroup(RESCUEINT64 zeroBasedOrdinal) { return groups->NthObject(zeroBasedOrdinal); }
  // Do NOT delete the object returned.
  RESCUEBOOL RemoveGroup(RescuePropertyGroup *groupToDrop) { return ((*groups) -= groupToDrop); }
