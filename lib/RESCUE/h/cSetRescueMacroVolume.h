...
#ifndef cSetRescueMacroVolume_H
#define cSetRescueMacroVolume_H
...
public:
  cSetRescueMacroVolume();
  ~cSetRescueMacroVolume();
  void operator+=(RescueMacroVolume *newObject);
  RESCUEBOOL operator-=(RescueMacroVolume *existingObject);
  RESCUEBOOL operator-=(RESCUEINT64 ndx);
  RescueMacroVolume *NthObject(RESCUEINT64 ordinal);
  RescueMacroVolume *ObjectNamed(const RESCUECHAR *nameIn);
