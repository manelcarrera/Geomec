...
public:
  ...
  void AddSection(RescueSection *existingSection) { (*sections) += existingSection; }
  void DropSection(RescueSection *existingSection) { (*sections) -= existingSection; }
  RESCUEINT64 SectionCount64() { return sections->Count64(); }
  RESCUEINT32 SectionCount() { return sections->Count(); }
  RescueSection *NthSection(RESCUEINT64 zeroBasedOrdinal) { return sections->NthObject(zeroBasedOrdinal); }
...
private:
  RescueEventDescr(RescueContext *context, RESCUECHAR *nameIn, EventType flagIn);
