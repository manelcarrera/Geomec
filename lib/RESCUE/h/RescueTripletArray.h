...
public:
  RescueTripletArray(RescueContext *context, RescueGrid *existingGrid, RESCUEFLOAT missingValueIn,
                     RescueModel *parentModelIn)
      : RescueObject(context), grid(existingGrid), missingValue(missingValueIn), xValue(0), yValue(0), zValue(0),
        hasChanged(TRUE), loaded(TRUE), parentModel(parentModelIn), arrayVersion(0), iNodes(0), jNodes(0), vertices(0) {
    isA = R_RescueTripletArray;
  }
  RescueTripletArray(RescueGrid *existingGrid, RESCUEFLOAT missingValueIn, RESCUEFLOAT *valueArray,
                     RescueModel *parentModelIn);
  ...
  ***********************************************************************************/
  void XYZCornerAt(RESCUEINT64 i, RESCUEINT64 j, RESCUEINT64 faceNumber, RESCUEFLOAT &x, RESCUEFLOAT &y,
                   RESCUEFLOAT &z);
