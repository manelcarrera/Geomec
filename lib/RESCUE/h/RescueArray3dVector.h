...
private:
  RescueArray3dVector(const RESCUECHAR *propertyNameIn, const RESCUECHAR *propertyTypeIn,
                      const RESCUECHAR *unitOfMeasureIn, RescueGrid *existingGrid, Rescue3dVector nullValueIn,
                      RescueModel *parentModelIn, RESCUEBOOL cellCenteredIn = FALSE, Rescue3dVector *valueArray = 0)
      : RescueArray(propertyNameIn, propertyTypeIn, unitOfMeasureIn, existingGrid, parentModelIn, cellCenteredIn),
        nullValue(nullValueIn), value(valueArray) {
