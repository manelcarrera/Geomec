...
public:
  ...
  RescueWellbore(RescueCoordinateSystem::Orientation orientation, RescueModel *parentModel,
                 const RESCUECHAR *wellboreName, RESCUEFLOAT missingValue, RESCUEINT64 i_lowbound, RESCUEINT64 i_count);
  RescueWellbore(RescueCoordinateSystem::Orientation orientation, RescueModel *parentModel,
                 const RESCUECHAR *wellboreName, RESCUEFLOAT missingValue, RESCUEFLOAT *quadrupletArray,
  ...
  RESCUEBOOL DropRescueWellboreSampling(RescueWellboreSampling *unitToDrop) { return ((*sampling) -= unitToDrop); }
