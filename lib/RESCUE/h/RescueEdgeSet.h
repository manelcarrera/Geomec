...
public:
  ...
  RescueTrimLoop *NthBoundaryLoop(RESCUEINT64 zeroBasedOrdinal) { return (*boundaryLoops).NthObject(zeroBasedOrdinal); }
  void AddInteriorLoop(RescueTrimLoop *existingLoop);
  void DropInteriorLoop(RescueTrimLoop *existingLoop);
  RescueTrimLoop *NthInteriorLoop(RESCUEINT64 zeroBasedOrdinal) { return (*interiorLoops).NthObject(zeroBasedOrdinal); }
