...
public:
  ...
  RescueCoordinateSystemAxis *RelatedCoordinateSystemAxis() { return relatedAxis; }
  ...
  RescueGridAxis(RescueContext *context, RESCUEINT64 lowBoundIn, RESCUEINT64 countIn, RESCUEFLOAT originIn,
                 RESCUEFLOAT stepIn)
      : RescueObject(context) // This public constructor is for use with abstract RescueDataContainer grids.
        ...
        ,
        lowBound(lowBoundIn), count(countIn), origin(originIn), step(stepIn) {
...
private:
  ...
  RescueGridAxis(RescueContext *context, RescueGrid *parentIn, RESCUEINT64 lowBoundIn, RESCUEINT64 countIn)
      : RescueObject(context), parentGrid(parentIn), relatedAxis(0), lowBound(lowBoundIn), count(countIn),
        origin((RESCUEFLOAT)0.0), step((RESCUEFLOAT)0.0) {
    isA = R_RescueGridAxis;
  }
  RescueGridAxis(RescueContext *context, RescueGrid *parentIn, RescueCoordinateSystemAxis *axis, RESCUEFLOAT originIn,
                 RESCUEFLOAT stepIn, RESCUEINT64 lowBoundIn, RESCUEINT64 countIn)
      : RescueObject(context), parentGrid(parentIn), relatedAxis(axis), lowBound(lowBoundIn), count(countIn),
        origin(originIn), step(stepIn) {
    isA = R_RescueGridAxis;
  }
  RescueGridAxis(RescueContext *context, RescueGrid *parentIn, RESCUEFLOAT originIn, RESCUEFLOAT stepIn,
