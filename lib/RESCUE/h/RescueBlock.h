...
public:
  ...
  RescueBlockUnit *NthRescueBlockUnit(RESCUEINT64 zeroBasedOrdinal) { return blockUnits->NthObject(zeroBasedOrdinal); }
  RescueBlockUnit *NthRescueBlockUnit(RESCUEINT32 zeroBasedOrdinal) { return blockUnits->NthObject(zeroBasedOrdinal); }
  RescueBlockUnit *BlockUnitIdentifiedBy(RESCUEINT64 identifier) { return blockUnits->ObjectIdentifiedBy(identifier); }
  RescueBlockUnit *BlockUnitIdentifiedBy(RESCUEINT32 identifier) { return blockUnits->ObjectIdentifiedBy(identifier); }
  ...
  RESCUEBOOL DropBlockUnit(RescueBlockUnit *blockUnitToDrop) { return ((*blockUnits) -= blockUnitToDrop); }
  ...
  RescueGeometry *GeometryIdentifiedBy(RESCUEINT32 identifier) {
    return blockUnits->GeometryIdentifiedBy((RESCUEINT64)identifier);
  }
...
private:
  ...
  void UnArchiveWireframeData(RescueModel *model, FILE *archiveFile) {
    blockUnits->UnArchiveWireframeData(model, archiveFile);
  }
