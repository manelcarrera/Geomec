...
public:
  typedef enum {
    R_FACE_ACTIVE,   // Face is completely inside the surface
    R_FACE_INACTIVE, // Face is completely outside the surface
    R_FACE_TRUNCATED
  } // Cell is inactivated by a Y Fault.
  RescueFaceStatus;
  RescueIJSurface(RescueSurface *surfaceIn, RescueCoordinateSystem::Orientation orientation,
                  RescueCoordinateSystemAxis *i_axis, RESCUEFLOAT i_origin, RESCUEFLOAT i_step, RESCUEINT64 i_lowbound,
                  RESCUEINT64 i_count, RescueCoordinateSystemAxis *j_axis, RESCUEFLOAT j_origin, RESCUEFLOAT j_step,
                  RESCUEINT64 j_lowbound, RESCUEINT64 j_count, RESCUEFLOAT missingValue);
  RescueIJSurface(RescueSurface *surfaceIn, RescueCoordinateSystem::Orientation orientation,
                  RescueCoordinateSystemAxis *i_axis, RESCUEFLOAT i_origin, RESCUEFLOAT i_step, RESCUEINT64 i_lowbound,
                  RESCUEINT64 i_count, RescueCoordinateSystemAxis *j_axis, RESCUEFLOAT j_origin, RESCUEFLOAT j_step,
                  RESCUEINT64 j_lowbound, RESCUEINT64 j_count, RESCUEFLOAT missingValue, RESCUEFLOAT *values);
  // Creates a new surface with regular x/y geometry.
  // If passed, the values are applied to the Z axis.
  RescueIJSurface(RescueSurface *surfaceIn, RescueCoordinateSystem::Orientation orientation, RESCUEINT64 i_lowbound,
                  RESCUEINT64 i_count, RESCUEINT64 j_lowbound, RESCUEINT64 j_count, RESCUEFLOAT missingValue);
  RescueIJSurface(RescueSurface *surfaceIn, RescueCoordinateSystem::Orientation orientation, RESCUEINT64 i_lowbound,
                  RESCUEINT64 i_count, RESCUEINT64 j_lowbound, RESCUEINT64 j_count, RESCUEFLOAT missingValue,
...
private:
  RESCUEINT64 *AddVertexToArray(RESCUEINT64 *arrayIn, RESCUEINT64 i, RESCUEINT64 j, RESCUEINT64 &allocated,
                                RESCUEINT64 &arrayLength);
  void AddWellboreSurface(RescueWellboreSurface *existingIntersect) { (*surfaceWellbore) += existingIntersect; }
  void DropWellboreSurface(RescueWellboreSurface *existingIntersect) { (*surfaceWellbore) -= existingIntersect; }
  // Constructor for RescueWellboreSurface will connect
  // both ends.
  void AddSurfaceCell(RescueSurfaceCell *existingCell) { (*surfaceCell) += existingCell; }
  void DropSurfaceCell(RescueSurfaceCell *existingCell) { (*surfaceCell) -= existingCell; }
  // Surface cell intersections are managed from
  // the geometry side. See RescueGeometry.
  void AddSurfaceToGrid(RescueSurfaceToGrid *existingToGrid) { (*surfaceToGrid) += existingToGrid; }
  void DropSurfaceToGrid(RescueSurfaceToGrid *existingToGrid) { (*surfaceToGrid) -= existingToGrid; }
  // Surface to grid intersections are managed from
  // the geometry side. See RescueGeometry.
  void AddStairSteps(RescueStairSteppedFault *existingFault) { (*stairSteps) += existingFault; }
  void DropSurfaceToGrid(RescueStairSteppedFault *existingFault) { (*stairSteps) -= existingFault; }
