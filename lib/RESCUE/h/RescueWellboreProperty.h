...
public:
  RescueWellboreProperty(RescueWellboreSampling *parentSampling, const RESCUECHAR *propertyName,
                         const RESCUECHAR *propertyType, const RESCUECHAR *unitOfMeasure, RESCUEFLOAT nullValue,
                         RESCUEFLOAT *values = 0);
  // Create a Wellbore property for an array of floats.
  RescueWellboreProperty(RescueWellboreSampling *parentSampling, const RESCUECHAR *propertyName,
                         const RESCUECHAR *propertyType, const RESCUECHAR *unitOfMeasure, RESCUEINT32 nullValue,
                         RESCUEINT32 *values = 0);
  // Create a Wellbore property for an array of ints.
  RescueWellboreProperty(RescueWellboreSampling *parentSampling, const RESCUECHAR *propertyName,
                         const RESCUECHAR *propertyType, const RESCUECHAR *unitOfMeasure, RESCUEUCHAR nullValue,
                         RescueLookup *lookupIn, RESCUEUCHAR *values = 0);
  // Create a Wellbore property using a lookup table where there
  // are 255 or fewer entries in the table.
  RescueWellboreProperty(RescueWellboreSampling *parentSampling, const RESCUECHAR *propertyName,
                         const RESCUECHAR *propertyType, const RESCUECHAR *unitOfMeasure, RESCUEUSHORT nullValue,
                         RescueLookup *lookupIn, RESCUEUSHORT *values = 0);
  // Create a Wellbore property using a lookup table where there
