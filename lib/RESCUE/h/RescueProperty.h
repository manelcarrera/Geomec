...
public:
  ...
  **********************************************************************************************/
  ...
  RescueProperty(RescueTriangulatedSurface *parentSurface, const RESCUECHAR *propertyNameIn,
                 const RESCUECHAR *propertyTypeIn, const RESCUECHAR *unitOfMeasureIn, AppliesToSurface applies,
                 RESCUEFLOAT nullValue, RESCUEFLOAT *valueArray = 0, AttachmentPoint attachedToIn = FACE_CENTERED);
  // Constructs a property array for floats,
  RescueProperty(RescueTriangulatedSurface *parentSurface, const RESCUECHAR *propertyNameIn,
                 const RESCUECHAR *propertyTypeIn, const RESCUECHAR *unitOfMeasureIn, AppliesToSurface applies,
                 RESCUEINT32 nullValue, RESCUEINT32 *valueArray = 0, AttachmentPoint attachedToIn = FACE_CENTERED);
  // Constructs a property array for ints,
  RescueProperty(RescueTriangulatedSurface *parentSurface, const RESCUECHAR *propertyNameIn,
                 const RESCUECHAR *propertyTypeIn, const RESCUECHAR *unitOfMeasureIn, AppliesToSurface applies,
                 RescueLookup *lookupTableIn, RESCUEUCHAR nullValue, RESCUEUCHAR *valueArray = 0,
                 AttachmentPoint attachedToIn = FACE_CENTERED);
  // Constructs a property array for a lookup table
  // with 255 or fewer entries,
  RescueProperty(RescueTriangulatedSurface *parentSurface, const RESCUECHAR *propertyNameIn,
                 const RESCUECHAR *propertyTypeIn, const RESCUECHAR *unitOfMeasureIn, AppliesToSurface applies,
                 RescueLookup *lookupTableIn, RESCUEUSHORT nullValue, RESCUEUSHORT *valueArray = 0,
                 AttachmentPoint attachedToIn = FACE_CENTERED);
  // Constructs a property array for a lookup table
  // with 2^16-1 or fewer entries,
  RescueProperty(RescueTriangulatedSurface *parentSurface, const RESCUECHAR *propertyNameIn,
                 const RESCUECHAR *propertyTypeIn, const RESCUECHAR *unitOfMeasureIn, AppliesToSurface applies,
                 Rescue2dVector nullValue, Rescue2dVector *valueArray = 0,
                 AttachmentPoint attachedToIn = FACE_CENTERED);
  // Constructs a property array for 2d vectors,
  RescueProperty(RescueTriangulatedSurface *parentSurface, const RESCUECHAR *propertyNameIn,
                 const RESCUECHAR *propertyTypeIn, const RESCUECHAR *unitOfMeasureIn, AppliesToSurface applies,
                 Rescue3dVector nullValue, Rescue3dVector *valueArray = 0,
                 AttachmentPoint attachedToIn = FACE_CENTERED);
  ...
  :
  **********************************************************************************************/
  RescueProperty(RescueSurface *parentSurface, const RESCUECHAR *propertyNameIn, const RESCUECHAR *propertyTypeIn,
                 const RESCUECHAR *unitOfMeasureIn, AppliesToSurface applies, RESCUEFLOAT nullValue,
                 RESCUEFLOAT *valueArray = 0, AttachmentPoint attachedToIn = FACE_CENTERED,
                 RescueIJSurface *parentIJSurface = 0);
  // Constructs a property array for floats,
  RescueProperty(RescueSurface *parentSurface, const RESCUECHAR *propertyNameIn, const RESCUECHAR *propertyTypeIn,
                 const RESCUECHAR *unitOfMeasureIn, AppliesToSurface applies, RESCUEINT32 nullValue,
                 RESCUEINT32 *valueArray = 0, AttachmentPoint attachedToIn = FACE_CENTERED,
                 RescueIJSurface *parentIJSurface = 0);
  // Constructs a property array for ints,
  RescueProperty(RescueSurface *parentSurface, const RESCUECHAR *propertyNameIn, const RESCUECHAR *propertyTypeIn,
                 const RESCUECHAR *unitOfMeasureIn, AppliesToSurface applies, RescueLookup *lookupTableIn,
                 RESCUEUCHAR nullValue, RESCUEUCHAR *valueArray = 0, AttachmentPoint attachedToIn = FACE_CENTERED,
                 RescueIJSurface *parentIJSurface = 0);
  // Constructs a property array for a lookup table
  // with 255 or fewer entries,
  RescueProperty(RescueSurface *parentSurface, const RESCUECHAR *propertyNameIn, const RESCUECHAR *propertyTypeIn,
                 const RESCUECHAR *unitOfMeasureIn, AppliesToSurface applies, RescueLookup *lookupTableIn,
                 RESCUEUSHORT nullValue, RESCUEUSHORT *valueArray = 0, AttachmentPoint attachedToIn = FACE_CENTERED,
                 RescueIJSurface *parentIJSurface = 0);
  // Constructs a property array for a lookup table
  // with 2^16-1 or fewer entries,
  RescueProperty(RescueSurface *parentSurface, const RESCUECHAR *propertyNameIn, const RESCUECHAR *propertyTypeIn,
                 const RESCUECHAR *unitOfMeasureIn, AppliesToSurface applies, Rescue2dVector nullValue,
                 Rescue2dVector *valueArray = 0, AttachmentPoint attachedToIn = FACE_CENTERED,
                 RescueIJSurface *parentIJSurface = 0);
  // Constructs a property array for 2d vectors,
  RescueProperty(RescueSurface *parentSurface, const RESCUECHAR *propertyNameIn, const RESCUECHAR *propertyTypeIn,
                 const RESCUECHAR *unitOfMeasureIn, AppliesToSurface applies, Rescue3dVector nullValue,
                 Rescue3dVector *valueArray = 0, AttachmentPoint attachedToIn = FACE_CENTERED,
                 RescueIJSurface *parentIJSurface = 0);
  // Constructs a property array for 3d vectors,
  /**********************************************************************************************
  ...
  **********************************************************************************************/
  RescueProperty(RescueGeometry *geometryIn, const RESCUECHAR *propertyNameIn, const RESCUECHAR *propertyTypeIn,
                 const RESCUECHAR *unitOfMeasureIn, RESCUEFLOAT nullValue, RESCUEFLOAT *valueArray = 0,
                 AttachmentPoint attachedToIn = CELL_CENTERED);
  // Constructs a property array for floats,
  RescueProperty(RescueGeometry *geometryIn, const RESCUECHAR *propertyNameIn, const RESCUECHAR *propertyTypeIn,
                 const RESCUECHAR *unitOfMeasureIn, RESCUEINT32 nullValue, RESCUEINT32 *valueArray = 0,
                 AttachmentPoint attachedToIn = CELL_CENTERED);
  // Constructs a property array for ints,
  RescueProperty(RescueGeometry *geometryIn, const RESCUECHAR *propertyNameIn, const RESCUECHAR *propertyTypeIn,
                 const RESCUECHAR *unitOfMeasureIn, RescueLookup *lookupTableIn, RESCUEUCHAR nullValue,
                 RESCUEUCHAR *valueArray = 0, AttachmentPoint attachedToIn = CELL_CENTERED);
  // Constructs a property array for a lookup table
  // with 255 or fewer entries,
  RescueProperty(RescueGeometry *geometryIn, const RESCUECHAR *propertyNameIn, const RESCUECHAR *propertyTypeIn,
                 const RESCUECHAR *unitOfMeasureIn, RescueLookup *lookupTableIn, RESCUEUSHORT nullValue,
                 RESCUEUSHORT *valueArray = 0, AttachmentPoint attachedToIn = CELL_CENTERED);
  // Constructs a property array for a lookup table
  // with 2^16-1 or fewer entries,
  RescueProperty(RescueGeometry *geometryIn, const RESCUECHAR *propertyNameIn, const RESCUECHAR *propertyTypeIn,
                 const RESCUECHAR *unitOfMeasureIn, Rescue2dVector nullValue, Rescue2dVector *valueArray = 0,
                 AttachmentPoint attachedToIn = CELL_CENTERED);
  // Constructs a property array for 2d vectors,
  RescueProperty(RescueGeometry *geometryIn, const RESCUECHAR *propertyNameIn, const RESCUECHAR *propertyTypeIn,
                 const RESCUECHAR *unitOfMeasureIn, Rescue3dVector nullValue, Rescue3dVector *valueArray = 0,
                 AttachmentPoint attachedToIn = CELL_CENTERED);
  // Constructs a property array for 3d vectors,
  /**********************************************************************************************
  ...
  **********************************************************************************************/
  ...
  RescueProperty(RescueDataContainer *containerIn, RescueModel *modelIn, const RESCUECHAR *propertyNameIn,
                 const RESCUECHAR *propertyTypeIn, const RESCUECHAR *unitOfMeasureIn, AppliesToSurface applies,
                 RESCUEFLOAT nullValue, RESCUEFLOAT *valueArray = 0, AttachmentPoint attachedToIn = FACE_CENTERED);
  // Constructs a property array for floats,
  RescueProperty(RescueDataContainer *containerIn, RescueModel *modelIn, const RESCUECHAR *propertyNameIn,
  ...
  RescueTrimLoopStub *NthTrimLoop(RESCUEINT64 zeroBasedOrdinal) {
    return (loops == 0) ? 0 : loops->NthObject(zeroBasedOrdinal);
  }
  ...
  RESCUEBOOL IsNamed(const RESCUECHAR *mayBeName);
...
private:
  ...
  void SwapAxes(bool swapI, RESCUEINT64 iNodes, bool swapJ, RESCUEINT64 jNodes, bool swapK, RESCUEINT64 kNodes);
