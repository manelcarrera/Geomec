...
public:
  ...
  virtual RescueTriangleVertex *NthVertex(RESCUEINT64 zeroBasedOrdinal) {
    return vertices->NthObject(zeroBasedOrdinal);
  }
  ...
  ***********************************************************************************/
  ...
  virtual RESCUEINT32 Version(RESCUEBOOL reload = FALSE); // These methods return a version number for the file which
                                                          ...
                                                          // number AFTER a write operation). If reload is TRUE the
                                                          // library always goes back to the disk to read the version
                                                          // number, in case it has been recently changed.  Therefore,
                                                          // the version number returned is the version which would be
                                                          // available if the data were loaded now, not the version
                                                          // which IS loaded now (if any). If false, the version number
                                                          // returned is the version of the last data loaded (if any).
protected:
  RescueTriangulatedSurface(RescueContext *context, RescueSurface *parentIn, cSetRescueTriangleVertex *verticesIn,
                            cSetRescueTriangleFace *facesIn)
      : RescueObject(context), verticesLookups(0), parentSurface(parentIn), vertices(verticesIn), faces(facesIn),
        octTrees(new cSetRescueSOctTreeNode()), faceGrid(0), nodeGrid(0), arrayVersion(0), hasChanged(TRUE),
        loaded(TRUE), faceStatus(0) {};
