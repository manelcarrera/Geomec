...
#ifndef cSetRescueHorizon_H
#define cSetRescueHorizon_H
...
class cSetRescueHorizon
{
protected:
  RescueHorizon **objects;
...
public:
  cSetRescueHorizon();
  ~cSetRescueHorizon();
  void operator+=(RescueHorizon *newObject);
  RESCUEBOOL operator-=(RescueHorizon *existingObject);
  RESCUEBOOL operator-=(RESCUEINT64 ndx);
  RescueHorizon *NthObject(RESCUEINT64 ordinal);
  RescueHorizon *ObjectNamed(const RESCUECHAR *nameIn);
  RescueHorizon *ObjectIdentifiedBy(RESCUEINT64 identifier);
  RESCUEINT64 Count64(void);
  RESCUEINT32 Count(void);
  RESCUEBOOL operator-=(RESCUEINT32 ndx) {return (*this) -=((RESCUEINT64) ndx);}
  RescueHorizon *NthObject(RESCUEINT32 ordinal) {return NthObject((RESCUEINT64) ordinal);}
