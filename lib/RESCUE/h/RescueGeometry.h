...
public:
  /******************************** Applies to CPG Only ********************************************/
  typedef enum {
    R_EQUAL_AXIS,          // x, y, z derived from grid origin and step or from reference surfaces.
    ...
    R_SPLINE_LINE
  } // splined line via RescuePillar.
  RescueVertexType; // It makes the most sense to put R_SPLINE_LINE before
                    // R_SPLIT_LINE, but that would invalidate older models.

  typedef enum {
    R_ONLAP, // Reference surface is ABOVE the grid.
    R_OFFLAP
  } // Reference surface is BELOW the grid.
  RescueLapType;

  typedef enum {
    R_XY_ORTHOGONAL,       // If VertexIs() == R_UNEQUAL_AXIS  or VertexIs() == R_EQUAL_AXIS
  ...
  typedef enum {
    R_STRUCTURED_CPG, // The type of LGR grid.
    R_STRUCTURED_RADIAL,
  ...
  /***************************************************************************************************/
  RescueGeometry(RescueModel *parentModelIn, RescueGrid *existingGrid, RESCUEFLOAT missingValueIn);
  // Normal constructor for most block unit grids.
  // x,y,z of vertexes of type R_EQUAL_AXIS are
  // determined by the grid.
  RescueGeometry(RescueModel *parentModelIn, RescueGrid *existingGrid, RESCUEFLOAT missingValueIn,
                 RescueReferenceSurface *topSurfaceIn, RESCUEFLOAT topOffsetIn, RescueReferenceSurface *bottomSurfaceIn,
  ...
  RescueGeometry(RescueModel *parentModelIn, RescueGrid *existingGrid, RESCUEFLOAT missingValueIn,
                 RescueReferenceSurface *referenceSurfaceIn, RESCUEFLOAT referenceOffsetIn, RESCUEFLOAT thicknessIn,
  ...
  ******************************************************************************************************/
  RescueGeometry(RescueGeometry *parentGrid,   // Constructor for a rule based (implicitly mapped) LGR.
                 ...
                 RESCUEINT64 parentKCount, RESCUEINT64 refinedKLowBound, RESCUEINT64 refinedKCount);
  RescueGeometry(RescueGeometry *parentGrid,   // Constructor for a rule based (implicitly mapped) LGR.
                 ...
                 RESCUEINT32 parentKCount, RESCUEINT32 refinedKLowBound, RESCUEINT32 refinedKCount);
  ...
  RescueGeometry(RescueGeometry *parentGrid,      // Constructor for a rule based (implicitly mapped) LGR.
                 ...
                 RESCUEINT64 parentKLowBound, RESCUEINT64 parentKCount, RESCUEINT64 *refinedKCountArray);
  RescueGeometry(RescueGeometry *parentGrid,      // Constructor for a rule based (implicitly mapped) LGR.
                 RESCUEINT32 referenceID,         // For each axis, LowBound and Count define the
                 RESCUEINT32 parentILowBound,     // section of the axis that is being refined,
                 ...
                 RESCUEINT32 parentKLowBound, RESCUEINT32 parentKCount, RESCUEINT32 *refinedKCountArray);
  ...
  RescueGeometry(RescueGeometry *parentGrid, // Constructor for a weight based (implicitly mapped) LGR.
                 ...
                 RESCUEINT64 zLowBoundin, RESCUEDOUBLE *zWeights, RESCUEINT64 zWeightLength);
  RescueGeometry(RescueGeometry *parentGrid, // Constructor for a weight based (implicitly mapped) LGR.
                 ...
                 RESCUEINT32 zLowBoundin, RESCUEDOUBLE *zWeights, RESCUEINT32 zWeightLength);
  ...
  RescueGeometry(RescueGeometry *parentGrid,  // Constructor for a weight based (implicitly mapped) LGR.
                 ...
                 RESCUEINT64 parentKLowBound, RESCUEINT64 parentKCount, RESCUEINT64 *zWeightLength,
  ...
  RescueGeometry(RescueGeometry *parentGrid,  // Constructor for a weight based (implicitly mapped) LGR.
                 ...
                 RESCUEDOUBLE **yWeights,     // (R_WEIGHT_BASED_NOSPAN).
                 RESCUEINT32 parentKLowBound, RESCUEINT32 parentKCount, RESCUEINT32 *zWeightLength,
                 RESCUEDOUBLE **zWeights);
  ...
  RescueGeometry(RescueGeometry *parentGrid, // Constructor for an unconstrained (explicitly mapped) LGR.
                 ...
                 RescueGrid *existingGrid, RESCUEFLOAT missingValueIn);
  RescueGeometry(RescueGeometry *parentGrid, // Constructor for an unconstrained (explicitly mapped) LGR.
  ...
  ******************************************************************************************************/
  ...
  RescueGeometry(RescueGeometry *parentGrid,
                 ...
                 RESCUEINT64 parentJCount, RESCUEINT64 parentKLowBound, RESCUEINT64 parentKCount,
                 RESCUEINT64 referenceID, RescueLGRGridType gridType, RESCUEINT64 expectedVertexCount,
                 RESCUEINT64 expectedFaceCount, RESCUEINT64 expectedFaceVertexTuples, RESCUEINT64 expectedCellCount,
                 RESCUEINT64 expectedCellFaceTuples, RESCUEINT64 expectedVertexRealloc = 100,
                 RESCUEINT64 expectedFaceRealloc = 100, RESCUEINT64 expectedFaceVertexRealloc = 100,
                 RESCUEINT64 expectedCellRealloc = 100, RESCUEINT64 expectedCellFaceRealloc = 100);
  /****************************************************************************************************
  ...
  ******************************************************************************************************/
  ...
  RESCUEINT64 ReferenceID64() {
    return referenceID;
  } // Application-defined id used for mapping   areas
  ...
  RESCUEINT64 ParentKCount64() {
    return parentKCount;
  } // For rule-based implicit LGR's returns
    // the section of the parent axis involved.  The count of
    // the axes is available from the grid, as usual.
  RESCUEINT32 ParentLowBound(RescueGridAxis::GridNdx ndx) {
    return RescueContext::Return32For64(ParentLowBound64(ndx), false);
  }
  RESCUEINT32 ParentCount(RescueGridAxis::GridNdx ndx) {
    return RescueContext::Return32For64(ParentCount64(ndx), false);
  }
  ...
  RESCUEINT32 ParentLowBound(RescueGridAxis::GridNdx ndx, RESCUEBOOL throwIfTooBig) {
    return RescueContext::Return32For64(ParentLowBound64(ndx), throwIfTooBig);
  }
  RESCUEINT32 ParentCount(RescueGridAxis::GridNdx ndx, RESCUEBOOL throwIfTooBig) {
    return RescueContext::Return32For64(ParentCount64(ndx), throwIfTooBig);
  }
  RESCUEINT32 ParentILowBound(RESCUEBOOL throwIfTooBig) {
    return RescueContext::Return32For64(parentILowBound, throwIfTooBig);
  }
  RESCUEINT32 ParentICount(RESCUEBOOL throwIfTooBig) {
    return RescueContext::Return32For64(parentICount, throwIfTooBig);
  }
  RESCUEINT32 ParentJLowBound(RESCUEBOOL throwIfTooBig) {
    return RescueContext::Return32For64(parentJLowBound, throwIfTooBig);
  }
  RESCUEINT32 ParentJCount(RESCUEBOOL throwIfTooBig) {
    return RescueContext::Return32For64(parentJCount, throwIfTooBig);
  }
  RESCUEINT32 ParentKLowBound(RESCUEBOOL throwIfTooBig) {
    return RescueContext::Return32For64(parentKLowBound, throwIfTooBig);
  }
  RESCUEINT32 ParentKCount(RESCUEBOOL throwIfTooBig) {
    return RescueContext::Return32For64(parentKCount, throwIfTooBig);
  }

  RESCUEINT64 WeightLength64(RescueGridAxis::GridNdx ndx);
  RESCUEINT32 WeightLength(RescueGridAxis::GridNdx ndx) {
    return RescueContext::Return32For64(WeightLength64(ndx), false);
  }
  ...
  RESCUEDOUBLE *ZWeight() {
    return zWeight;
  } // DO NOT delete the returned array.
  ...
  void WeightFor(RescueGridAxis::GridNdx ndx, RESCUEINT64 iNdx, RESCUEDOUBLE *buffer, RESCUEINT64 offset,
  ...
  void SetCoordinateLine(RESCUEINT64 i, RESCUEINT64 j, RESCUEFLOAT xTop, RESCUEFLOAT yTop, RESCUEFLOAT zTop,
                         RESCUEFLOAT xBottom, RESCUEFLOAT yBottom, RESCUEFLOAT zBottom, RESCUEFLOAT *middleZvalues = 0);
  ...
  void SetCornerNode(RESCUEINT64 i, RESCUEINT64 j, RESCUEINT64 k, RESCUEINT64 corner, RESCUEFLOAT x, RESCUEFLOAT y,
                     RESCUEFLOAT z);
  ...
  void Values(RESCUEINT64 i, RESCUEINT64 j, RESCUEINT64 k, RESCUEINT64 corner, RESCUEFLOAT &x, RESCUEFLOAT &y,
              RESCUEFLOAT &z);
  void Values(RESCUEINT32 i, RESCUEINT32 j, RESCUEINT32 k, RESCUEINT32 corner, RESCUEFLOAT &x, RESCUEFLOAT &y,
              RESCUEFLOAT &z);
  ...
  void CellToNode(RESCUEINT64 i, RESCUEINT64 j, RESCUEINT64 k, RESCUEINT64 corner, RESCUEINT64 &iPrime,
                  RESCUEINT64 &jPrime, RESCUEINT64 &kPrime);
  void CellToNode(RESCUEINT32 i, RESCUEINT32 j, RESCUEINT32 k, RESCUEINT32 corner, RESCUEINT32 &iPrime,
                  RESCUEINT32 &jPrime, RESCUEINT32 &kPrime);
  ...
  void UVWValues(RESCUEINT64 i, RESCUEINT64 j, RESCUEINT64 k, RESCUEDOUBLE &uTerm, RESCUEDOUBLE &vTerm,
                 RESCUEDOUBLE &wTerm);
  void UVWValues(RESCUEINT32 i, RESCUEINT32 j, RESCUEINT32 k, RESCUEDOUBLE &uTerm, RESCUEDOUBLE &vTerm,
                 RESCUEDOUBLE &wTerm);
  void UVWValues(RESCUEDOUBLE i, RESCUEDOUBLE j, RESCUEDOUBLE k, RESCUEDOUBLE &uTerm, RESCUEDOUBLE &vTerm,
                 RESCUEDOUBLE &wTerm);
  ...
  // Corresponds to
  // void Values(RESCUEINT64 i, RESCUEINT64 j, RESCUEINT64 k, RESCUEINT64 corner, RESCUEFLOAT &x, RESCUEFLOAT &y,
  // RESCUEFLOAT &z); DO delete the returned array.
  ...
  // Corresponds to
  // RESCUEBOOL UVWValues(RESCUEDOUBLE i, RESCUEDOUBLE j, RESCUEDOUBLE k, RESCUEFLOAT &x, RESCUEFLOAT &y, RESCUEFLOAT
  // &z); Returns 0 if the value would have been FALSE. DO delete the returned array.
  RESCUEFLOAT ZValue(RESCUEINT64 i, RESCUEINT64 j,
                     RESCUEINT64 k); // Corresponds to
                                     // RESCUEBOOL Values(RESCUEINT64 i, RESCUEINT64 j, RESCUEINT64 k, RESCUEFLOAT &z);
  ...
  /****************************************************************************************************/
  void SetStairSteppedSurface(RescueIJSurface *surface, RescueIJKCellPair *cellPairs, RESCUEINT64 arrayLength);
  ...
  void SetSurfaceIntersection(RescueIJSurface *surface, RESCUEINT64 *iList, RESCUEINT64 *jList, RESCUEINT64 listLength);
  void SetSurfaceIntersection(RescueIJSurface *surface, RESCUEINT32 *iList, RESCUEINT32 *jList, RESCUEINT32 listLength);
  ...
  void SetSurfaceIntersection(RescueIJSurface *surface, RescueSurfaceCell::SurfaceSide side, RESCUEINT64 i,
                              RESCUEINT64 j, RESCUEINT64 k, RESCUEINT64 cellFaceNumber, RESCUEFLOAT *uvs = 0);
  ...
  // and becomes the property of the grid.
  RESCUEBOOL DropRescueSurfaceCell(RescueSurfaceCell *cellToDrop) { return ((*surfaceCell) -= cellToDrop); }
  ...
  RescueGeometryUnit *NthGeometryUnit(RESCUEINT64 zeroBasedOrdinal) {
    return geometryUnit->NthObject(zeroBasedOrdinal);
  }
  ...
  void SetWellboreIntersection(RescueWellbore *wellbore, RESCUEINT64 i, RESCUEINT64 j, RESCUEINT64 k);
  // Declare a new relationship between a cell
  // in this grid and a wellbore.
  RESCUEBOOL DropRescueWellboreCell(RescueWellboreCell *cellToDrop) { return ((*wellboreCell) -= cellToDrop); }
  // Destructor will drop it from the surface side.
  RescueWellboreCell *NthWellboreCell(RESCUEINT64 zeroBasedOrdinal) {
    return wellboreCell->NthObject(zeroBasedOrdinal);
  }
...
private:
  ...
  RESCUEBOOL ProportionalValueAt(RESCUEINT64 i, RESCUEINT64 j, RESCUEINT64 k,
                                 RESCUEFLOAT &z); // These use the first RescueIJSurface
  RESCUEBOOL ReferencedValueAt(RESCUEINT64 i, RESCUEINT64 j, RESCUEINT64 k,
                               RESCUEFLOAT &z); // defined for the reference surfaces.
  void PrepareModelDelete() {
    if (surfaceCell != 0)
      surfaceCell->PrepareModelDelete();
  }
  ...
  void CornerValuesPrimitive(RESCUEINT64 kLayer, RESCUEBOOL layerValid, RESCUEFLOAT *buffer, RESCUEINT64 offset,
