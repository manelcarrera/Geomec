...
protected:
  RescuePolyLineNodeBUG(RescueContext *context, RescueGeometry *geometryIn, RescuePolyLineNode::RescueLayerType typeIn,
                        RESCUEFLOAT ndxParamIn)
      : RescueObject(context), layerType(typeIn), layerNdxParam(ndxParamIn), geometry(geometryIn) {
    isA = R_RescuePolyLineNodeBUG;
  }
  virtual void Archive(RescueContext *context, FILE *archiveFile);
  RescuePolyLineNodeBUG(RescueContext *context, FILE *archiveFile);
