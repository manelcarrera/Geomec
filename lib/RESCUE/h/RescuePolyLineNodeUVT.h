...
public:
  RescuePolyLineNodeUVT(RescueContext *context, RescueTriangulatedSurface *faceIn, RESCUEINT64 faceNdxIn, RESCUEFLOAT u,
                        RESCUEFLOAT v)
      : RescueObject(context), uValue(u), vValue(v), faceNdx(faceNdxIn), face(faceIn), faceID(0) {
    isA = R_RescuePolyLineNodeUVT;
  }
  void SetUVValue(RESCUEINT64 faceNdxIn, RESCUEFLOAT u, RESCUEFLOAT v) {
    faceNdx = faceNdxIn;
    uValue = u;
    vValue = v;
  }
