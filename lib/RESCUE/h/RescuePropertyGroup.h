...
public:
  ...
  static RescuePropertyGroup *MakeRescuePropertyGroup(const RESCUECHAR *groupNameIn, RescueBlockUnit *blockUnitIn);
  // Use this to group properties on block unit grids.
  static RescuePropertyGroup *MakeRescuePropertyGroup(const RESCUECHAR *groupNameIn, RescueGeobodyPart *partIn);
  // Use this to group properties on Geobodies.
  static RescuePropertyGroup *MakeRescuePropertyGroup(const RESCUECHAR *groupNameIn, RescueSurface *surfaceIn);
  // Use this to group properties on surfaces.
  static RescuePropertyGroup *MakeRescuePropertyGroup(const RESCUECHAR *groupNameIn, RescueModel *modelIn);
  ...
  void SetGroupName(const RESCUECHAR *newGroupName);
  ...
  RescueTimeStepGroup *NthTimeStepGroup(RESCUEINT32 zeroBasedOrdinal) { return timeSteps->NthObject(zeroBasedOrdinal); }
  ...
  // doesn't already exist.
  RESCUEBOOL DropRescueTimeStepGroup(RescueTimeStepGroup *timeStep) { return ((*timeSteps) -= timeStep); }
...
private:
  RescuePropertyGroup(const RESCUECHAR *groupNameIn, RescueBlockUnit *blockUnitIn);
  // Use this to group properties on block unit grids.
  RescuePropertyGroup(const RESCUECHAR *groupNameIn, RescueGeobodyPart *partIn);
  // Use this to group properties on Geobodies.
  RescuePropertyGroup(const RESCUECHAR *groupNameIn, RescueSurface *surfaceIn);
  // Use this to group properties on surfaces.
  RescuePropertyGroup(const RESCUECHAR *groupNameIn, RescueModel *modelIn);
  // Use this one to group properties on global grids.
  RescuePropertyGroup(const RESCUECHAR *groupNameIn, RescueModel *modelIn, RescueDataContainer *containerIn);
  // Use this one to group properties of data containers.
