...
static bool InterpolateInclination_newton(	const double& tmd1,
                				const double& tmd2,
                      const double& tvd1,
                      const double& tvd2,
                      const double& azi1,
...
static bool InterpolateTmd_newton(	const double& tmd1,
                				const double& tmd2,
                      const double& tvd1,
                      const double& tvd2,
                      const double& azi1,
...
static double GetDeltaTMD(const double& a1,
           const double& a2,
           const double& i1,
           const double& i2,
           const double& tmd1,
...
CWellDateList::Iterator CWellDateList::begin() const
{
  return This()->m_lstDates.begin();
}

CWellDateList::Iterator CWellDateList::end() const
{
...
QDate* CWellDateList::At(int index)
{
  return m_lstDates.at(index);
}

const QDate* CWellDateList::At(int index) const
{
  return This()->m_lstDates.at(index);
}

CWellDateList* CWellDateList::This() const
{
  return const_cast<CWellDateList*>(this);
}

int CWellDateList::Size() const
{
  return m_lstDates.count();
}
...
void CWellDateList::AutoDelete(bool bAutoDelete)
{
...
void CWellPathBase::Init()
{
  ...
  connect(&m_MeanGroundOrSeabedLevel,SIGNAL(OnChanged(const CDoubleQuantity&)),this,SLOT(OnQuantityChanged(const CDoubleQuantity&)));
  connect(&m_MeanSeaLevel,SIGNAL(OnChanged(const CDoubleQuantity&)),this,SLOT(OnQuantityChanged(const CDoubleQuantity&)));
}
...
bool CWellPathBase::InitFromPointArray(const QVector<geo::CPoint>& vcPoint)
{
  ...
  for(int i = 0; i < tmd.count() - 1;i++) 
  {
    ...
    if(azi_rad <0) azi_rad = 2.0*PI+azi_rad;
  ...
  InitFromArray_tmd_azi_inc( vcPoint[0].X(),
                 vcPoint[0].Y(),
                 vcPoint[0].Z(),
                 0,0,0,
                tmd,
...
CWellPathBase::CWellPathBase(const CWellPathBase& right)
:CCommonWellPath(right.text()), m_pDefPointList(0)
{
  create();
...
CWellPathBase::CWellPathBase(const QString &text)
:CCommonWellPath(text),
 m_pDefPointList(0)
{
...
bool CWellPathBase::AutoDeletePointLists() const
{
  return m_AutoDeletePointLists;
}

void CWellPathBase::AutoDeletePointLists(bool autodel)
{
  m_AutoDeletePointLists=autodel;
}


bool CWellPathBase::AutoDeleteSectionLists() const
{
  return m_AutoDeleteSectionLists;
}

void CWellPathBase::AutoDeleteSectionLists(bool autodel)
{
  m_AutoDeleteSectionLists=autodel;
}


//##ModelId=3F7AA157015A
const CWellDefinitionPointList& CWellPathBase::DefPointList() const
{
  return *m_pDefPointList;
}


//##ModelId=3F7AA1570160
CWellDefinitionPointList& CWellPathBase::DefPointList()
{
  return *m_pDefPointList;
}
...
void CWellPathBase::GetBoundingBox(geo::IPoint& max,geo::IPoint& min) const
{
  ...
  while(p1 && p2)
  {
    ...
    if((p1->Inclination().Value() < 90.0 && p2->Inclination().Value() > 90.0) || (p1->Inclination().Value() > 90.0 && p2->Inclination().Value() < 90.0))
    {
...
namespace
{
  ...
  {
  public:
  bool operator()(const CWellPoint* p1, const CWellPoint* p2)
  {
      return (p1->TMD().Value() < p2->TMD().Value());
  }
  };
...
QList<int> CWellPathBase::GetAllPointListID() const
{
  return m_PointListMap.keys();
}
...
int CWellPathBase::AddDate(QDate *pDate)
{
  return m_DateList.AddDate(pDate); 
}

QDate *CWellPathBase::GetDate(int nIndex)
{
  return m_DateList.At(nIndex);
}
...
CWellPathBase* CWellPathBase::This() const
{ 
  return const_cast<CWellPathBase*>(this);
}
...
bool CWellPathBase::IsVertical() const
{
  return !IsDeviated();
}

void CWellPathBase::InitVertical(double start_northing,
...
bool CWellPathBase::InitWellPoint(CWellPoint& Point) const
{
  ...
  {
    //above fisrt point
    GetNewPosition(Upper->Azimuth().Value(),Upper->Azimuth().Value(),
            Upper->TMD().Value(),TMD,TMD,
            Upper->Inclination().Value(),Upper->Inclination().Value(),
            Upper->Easting().Value(),Upper->Northing().Value(),Upper->TVD().Value(),
            eas,
            nor,
            tvd,
            azi,
  ...
  {
    //below last point
    GetNewPosition(Lower->Azimuth().Value(),Lower->Azimuth().Value(),
            Lower->TMD().Value(),TMD,TMD,
            Lower->Inclination().Value(),Lower->Inclination().Value(),
            Lower->Easting().Value(),Lower->Northing().Value(),Lower->TVD().Value(),
            eas,
            nor,
            tvd,
            azi,
...
void CWellPathBase::GetUpLowDefPoints(double tmd, CWellPoint **pUpper, CWellPoint **pLower) const
{
  ...
  if(tmd - pFirst->TMD().Value() < 0.0)
  {// tmd above all definition points
    *pUpper = 0;
    *pLower = const_cast<CWellPoint*>(pFirst);
    return;
  }

  if(tmd - pLast->TMD().Value() > 0.0)
  {// tmd below all definition points
...
//              
//*******************************************************************
bool CWellPathBase::InitFromArray_tmd_tvd_azi(
                        ...
                        
{
  ...
  for(int i =0; i < tmd.count() -1 ;i++)
  {
    ...
    GetNewPosition(azi[i],azi[i+1],tmd[i],tmd[i+1],tmd[i+1],inc1,inc2,eas1,nor1,tvd[i],eas2,nor2,tvd[i+1],dum_azi,dum_inc,dum_dls);
...
bool CWellPathBase::InitFromArray_tmd_azi_inc(
                      double global_northing,
                     	double global_easting,
                      double global_depth,
                      double start_northing,
...
QString CWellPathBase::GetWellName() const 
{
  return m_WellName;
}
...
double CWellPathBase::GetDogLegSeverity(double tmd) const
{
  ...
  if(Upper && Lower)
  {
    GetNewPosition(Upper->Azimuth().Value(),
             Lower->Azimuth().Value(),
             Upper->TMD().Value(),
             Lower->TMD().Value(),
             tmd,
             Upper->Inclination().Value(),
             Lower->Inclination().Value(),
             Upper->Easting().Value(),
             Upper->Northing().Value(),
             Upper->TVD().Value(),
             dum_eas,
             dum_nor,
             dum_tvd,
  ...
  if((def_list.First() == Upper) || (def_list.First() == Lower))
  {// tmd is exactly on the first definition point, set to zero
    return 0.0;
  }
  ...
  GetNewPosition(p1->Azimuth().Value(),
       p2->Azimuth().Value(),
       p1->TMD().Value(),
       p2->TMD().Value(),
       (p1->TMD().Value() + p2->TMD().Value()) / 2.0,
       p1->Inclination().Value(),
       p2->Inclination().Value(),
       p1->Easting().Value(),
       p1->Northing().Value(),
       p1->TVD().Value(),
       dum_eas,
...
QString CWellPathBase::GetCustomerName() const
{
  return m_CustName; 
}

QString CWellPathBase::GetJobNumber() const
{
  return m_JobNumber;
}

QString CWellPathBase::GetCustomerContact() const
{
  return m_CustContact;
}

QString CWellPathBase::GetDescription() const
{
  return m_Description;
}

QString CWellPathBase::GetWellType() const
{
  return m_WellType;
}

QString CWellPathBase::GetLocationName() const
{
  return m_LocationName;
}

QString CWellPathBase::GetCountry() const
{
  return m_Country; 
}

const QDate& CWellPathBase::GetSpudDate() const
{
  return m_SpudDate;
}

const QDate& CWellPathBase::GetBirthDate() const
{
  return m_BirthDate;	 
}

void CWellPathBase::SetSpudDate(const QDate& date)
{
  m_SpudDate=date;
}

void CWellPathBase::SetBirthDate(const QDate& date)
{
  m_BirthDate = date;
}


void CWellPathBase::SetLocationName(QString str)
{
  m_LocationName=str;
}


void CWellPathBase::SetCountry(QString str)
{
  m_Country=str;
}

void CWellPathBase::SetWellType(QString str)
{
  m_WellType=str;
}

void CWellPathBase::SetDescription(QString str)
{
  m_Description=str;
}


QString CWellPathBase::GetFieldName() const
{
  return m_FieldName;
}

void CWellPathBase::SetFieldName(QString str) 
{
  m_FieldName=str;
}

void CWellPathBase::SetCustomerName(QString str)
{
  m_CustName = str; 
}

void CWellPathBase::SetCustomerContact(QString str)
{
  m_CustContact = str; 
}

void CWellPathBase::SetJobNumber(QString str)
{
  m_JobNumber = str;
}

CLengthQuantity& CWellPathBase::MeanGroundOrSeabedLevel()
{
  return m_MeanGroundOrSeabedLevel;
}

CLengthQuantity& CWellPathBase::MeanSeaLevel()
{
  return m_MeanSeaLevel;
}

CWellPathBase::LOCATION CWellPathBase::GetLocation() const
{
  return m_Location;
}
...
double CWellPathBase::TVD2TMD(double tvd) const
{
  ...
  if(tvd - pFirst->TVD().Value() < 0.0)
  {// tvd above all definition points

    double delta_tmd = GetDeltaTMD(	pFirst->Azimuth().Value(),
                    pFirst->Azimuth().Value(),
                    pFirst->Inclination().Value(),	
                    pFirst->Inclination().Value(),	
                    pFirst->TMD().Value(),	
                    pFirst->TMD().Value(),	
                    pFirst->TVD().Value(),	
  ...
  else if(tvd - pLast->TVD().Value() > 0.0)
  {// tvd below all definition points

    double delta_tmd = GetDeltaTMD(	pLast->Azimuth().Value(),
                    pLast->Azimuth().Value(),
                    pLast->Inclination().Value(),	
                    pLast->Inclination().Value(),
                    pLast->TMD().Value(),	
                    pLast->TMD().Value(),	
                    pLast->TVD().Value(),	
  ...
  if(it != DefPointList().end())
    ...
    double delta_tmd = GetDeltaTMD(	p1->Azimuth().Value(),
                    p2->Azimuth().Value(),
                    p1->Inclination().Value(),	
                    p2->Inclination().Value(),	
                    p1->TMD().Value(),
...
const CLengthQuantity& CWellPathBase::GlobalNorthing() const
{
  return m_GlobalNorthing ;
}

const CLengthQuantity& CWellPathBase::GlobalEasting() const
{
  return m_GlobalEasting;
}

const CLengthQuantity& CWellPathBase::GlobalTVD() const
{
  return m_GlobalTVD;
}
...
void CWellPathBase::GetBetweenPosition( const CWellPoint& p1,
                  const CWellPoint& p2,
                  const double& tmd,
                  double& eas,
                  double& nor,
                      ...
                      double& tmd)
{
  ...
  for(int i=0 ; i<MAXIT ; i++)
  {
    ...
    CCommonWellPath::GetNewPosition(azi1,azi2,tmd1,tmd2,x    ,inc1,inc2,eas1,nor1,tvd1,eas2,nor2,f,dum_azi,dum_inc,dum_dls);
    CCommonWellPath::GetNewPosition(azi1,azi2,tmd1,tmd2,x+delta ,inc1,inc2,eas1,nor1,tvd1,eas2,nor2,df,dum_azi,dum_inc,dum_dls);
                      ...
                      double& inc2)
{
  ...
  for(int i=0 ; i<MAXIT ; i++)
  {
    ...
    CCommonWellPath::GetNewPosition(azi1,azi2,tmd1,tmd2,tmd2,inc1,rtn,eas1,nor1,tvd1,eas2,nor2,f,dum_azi,dum_inc,dum_dls);
    CCommonWellPath::GetNewPosition(azi1,azi2,tmd1,tmd2,tmd2,inc1,rtn+delta,eas1,nor1,tvd1,eas2,nor2,df,dum_azi,dum_inc,dum_dls);
