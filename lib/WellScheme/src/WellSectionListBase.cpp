...
CWellSectionListBase::CWellSectionListBase(const CWellPathBase &WellPath, bool bAutoDelete) : m_pWellPath(&WellPath) {
  AutoDelete(bAutoDelete);
}

// ##ModelId=3F7AA15602DD
CWellSectionListBase::~CWellSectionListBase() { clear(); }
...
CWellSectionListBase::CWellSectionListBase(const CWellSectionListBase &rhs)
    : QObject(), // QObject(const QObject&) is private,
      ...
      m_pWellPath(rhs.m_pWellPath), m_Description(rhs.m_Description), m_lstSections(rhs.m_lstSections),
      m_bAutoDelete(false) {}

// ##ModelId=3F7AA15602EA
bool CWellSectionListBase::AutoDelete() const { return m_bAutoDelete; }

void CWellSectionListBase::AutoDelete(bool autodelete) { m_bAutoDelete = autodelete; }

int CWellSectionListBase::NrOfSections() const { return m_lstSections.size(); }
...
int CWellSectionListBase::GetIndex(IWellSection *section) const { return m_lstSections.indexOf(section); }

bool CWellSectionListBase::Exist(IWellSection *section) const { return !(GetIndex(section) == -1); }
...
CWellPathBase &CWellSectionListBase::WellPath() { return *const_cast<CWellPathBase *>(m_pWellPath); }

const CWellPathBase &CWellSectionListBase::WellPath() const { return *m_pWellPath; }

QString CWellSectionListBase::GetDescription() const { return m_Description; }

void CWellSectionListBase::SetDescription(QString description) { m_Description = description; }
...
CWellSectionList::Iterator CWellSectionListBase::begin() const { return m_lstSections.begin(); }

CWellSectionList::Iterator CWellSectionListBase::end() const { return m_lstSections.end(); }
