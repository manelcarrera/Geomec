...
CWellLog::CWellLog(CWellPathBase *pWellPath, IWellSection *pSection)
    : m_pWellPath(pWellPath), m_pSection(pSection), m_dVc(0), m_point_log_tmd_list(*pWellPath, true, false),
      m_point_avax_rat_list(*pWellPath, true, true), m_compress_map("Compressibility", m_point_log_tmd_list, 0),
      m_modul_map("Modulation", m_point_log_tmd_list, 0), m_avax_map("Average axial strain", m_point_avax_rat_list, 0) {
}

CWellLog::~CWellLog() {}

CWellLog::CWellLog()
    : m_pWellPath(0), m_pSection(0), m_dVc(0), m_compress_map("Compressibility", m_point_log_tmd_list, 0),
      m_modul_map("Modulation", m_point_log_tmd_list, 0), m_avax_map("Average axial strain", m_point_avax_rat_list, 0) {
  m_point_avax_rat_list.AllowDuplicateTMD(true);
}

CWellLog::CWellLog(const CWellLog &welllog)
    : m_compress_map("Compressibility", m_point_log_tmd_list, 0), m_modul_map("Modulation", m_point_log_tmd_list, 0),
...
size_t CWellLog::NonSandLayerSize() const { return m_vcNonSandLayers.size(); }
...
double CWellLog::SandDelta(const double &dStartTMD, const double &dEndTMD) const {
  ...
  if (prStart.first != prEnd.first && prStart.second &&
      prEnd.second) { // start and end are both in a sand, not the same.
    dTotal = 0.0;
  ...
  if (prStart.first != prEnd.first && prStart.second &&
      !prEnd.second) { // start in a sand, end below a sand (not the same)
    dTotal = 0.0;
    for (i = prStart.first; i <= prEnd.first; i++) {
      if (i == prStart.first)
        dTotal += BottomSand(i, STRETCHED_TMD) - dStartTMD;
  ...
  if (prStart.first != prEnd.first && !prStart.second && prEnd.second) { // start in a non-sand, end in a sand
  ...
  if (prStart.first == -1 && prEnd.second) { // start above all sandlayers, end in a sandlayer.
    dTotal = 0.0;
  ...
  if (prStart.first == -1 && !prEnd.second) { // start above all sandlayers, end in non-sandlayer
    dTotal = 0.0;
...
int CWellLog::RTagSize() const { return m_vcRTTMD.size(); }

int CWellLog::SandLayerSize() const { return m_vcSandLayers.size(); }
...
double CWellLog::AverageCompressibility(const double &dStartTMD, const double &dEndTMD) const {
  ...
  if ((prIndices1.first == -1 && prIndices2.first == -1) || (prIndices1.second == -1 && prIndices2.second == -1))
  ...
  if (prIndices1.first == -1) {
    ...
    if (NearestSandLayer(dEndTMD).second) // Is the end in a sand?
    {
      ...
      dEndValue =
          dComp1 * ((dDepth2 - dEndTMD) / (dDepth2 - dDepth1)) + dComp2 * ((dEndTMD - dDepth1) / (dDepth2 - dDepth1));
...
const CWellPointList &CWellLog::LogPoints() const { return m_point_log_tmd_list; }

const CWellPointList &CWellLog::RatAvAxPoints() const { return m_point_avax_rat_list; }

const CWellPointValueMap &CWellLog::AvAxMap() const { return m_avax_map; }

const CWellPointValueMap &CWellLog::ModulMap() const { return m_modul_map; }

const CWellPointValueMap &CWellLog::CompressMap() const { return m_compress_map; }
...
void CWellLog::WellPointModul(CWellPointList &point_list, CWellPointValueMap &vlmap) const
{
  ...
  //std::pair<CWellPointList, CWellPointValueMap> retpr(point_list, CWellPointValueMap("Compressibility", point_list,
0));
...
std::pair<double, double> CWellLog::AvgCompComponents(int nStartIndex, int nEndIndex, eDepthType type) const {
  ...
  for (int i = prStart.first; i <= prEnd.first; i++) {
    if (prStart.first == prEnd.first) { // 1) first and last sandlayer are the same
    ...
    } else if (i == prStart.first) { // 2) current sandlayer is the first sandlayer
      if (prStart.second) {
        first = nStartIndex;
        second = m_vcSandLayers[i].second;
      } else // !prStart.second
    ...
    } else if (i == prEnd.first) { // 3) current sandlayer is the last sandlayer
      if (prEnd.second) {
        first = m_vcSandLayers[i].first;
        second = nEndIndex;
      } else // !prEnd.second
    ...
    } else { // 4) current sandlayer is a sandlayer in between the first and last sandlayer
      first = m_vcSandLayers[i].first;
      second = m_vcSandLayers[i].second;
    }
    for (int j = first; j <= second; j++) {
...
int CWellLog::LogPointSize() const { return m_vcOriginalTMD.size(); }
...
void CWellLog::OriginalTMD(const double &TMD) { m_vcOriginalTMD.push_back(TMD); }

const CWellPathBase *CWellLog::WellPath() const { return m_pWellPath; }
...
const IWellSection *CWellLog::WellSection() const { return m_pSection; }
...
double CWellLog::SandCutOff() const { return m_dVc; }

void CWellLog::SandCutOff(const double &dVc) { m_dVc = dVc; }

void CWellLog::addShalyness(const double &dVsh) { m_vcShalyness.push_back(dVsh); }
...
void CWellLog::addCompressibility(const double &dComp) { m_vcCompressibility.push_back(dComp); }
