...
IWellSection::IWellSection(const CWellPathBase& WellPath)
...
IWellSection::~IWellSection()
{
	emit OnDestroy(*this);
}


const CWellPathBase& IWellSection::WellPath() const
{
	return *m_pWellPath;
}

CWellPathBase& IWellSection::WellPath()
{
	return *const_cast<CWellPathBase*>(m_pWellPath);
}

//##ModelId=3F7AA1560354
bool IWellSection::Defined() const
{
	return (Top().Defined() && Bottom().Defined());
}
...
void IWellSection::AddIntervalPoints(CWellPointList& lst , int NrOfIntervals,const double& distance,const IWellPointFactory* pFactory /*=NULL*/ ) const
{
...
double IWellSection::DeltaTVD() const
{
	return (Bottom().TVD().Value() - Top().TVD().Value());
}

double IWellSection::DeltaTMD() const
{
	return (Bottom().TMD().Value() - Top().TMD().Value());
}
