...
void CWellPoint::Connect() {
  ...
  connect(&m_TMD, SIGNAL(OnChanged(const CDoubleQuantity &)), this, SLOT(OnQuantityChanged(const CDoubleQuantity &)));

  connect(m_pWellPath, SIGNAL(onGeometryChanged()), this, SLOT(OnWellDefPointsChanged()));
}
...
CWellPoint::~CWellPoint() { emit OnDestroy(*this); }
...
const CLengthQuantity &CWellPoint::Northing() const { return m_Northing; }

const CLengthQuantity &CWellPoint::Easting() const { return m_Easting; }

const CAngleQuantity &CWellPoint::Azimuth() const { return m_Azimuth; }

const CAngleQuantity &CWellPoint::Inclination() const { return m_Inclination; }

CLengthQuantity &CWellPoint::TMD() { return m_TMD; }

const CLengthQuantity &CWellPoint::TMD() const { return m_TMD; }

const CLengthQuantity &CWellPoint::TVD() const { return m_TVD; }
...
bool CWellPoint::operator!=(const CWellPoint &rhs) const { return !(*this == rhs); }
...
void CWellPoint::setElements(const std::set<const geo::IElement *> &stElements) { m_stElements = stElements; }

const std::set<const geo::IElement *> &CWellPoint::getElements() const { return m_stElements; }
...
/*virtual*/
void CWellPoint::AssertValid() const { assert(m_pWellPath); }
...
const CWellPathBase &CWellPoint::WellPath() const { return *m_pWellPath; }

CWellPathBase &CWellPoint::WellPath() { return const_cast<CWellPathBase &>(*m_pWellPath); }
