...
public:
  ...
  ConnectivityHelper(bool owner) : m_owner(owner) { point_insert_count = 0; }
  ...
  class IPointLess {
  public:
    bool operator()(const TConstPoint &p1, const TConstPoint &p2) const { return *p1 < *p2; }
  };
...
const std::vector<SIDE_ID> &CTriaIntersecBox::GetSides() const { return m_Sides; }
...
/*static*/ void CTriaIntersecBox::GetPointsOnRib(const geo::IBox &box, std::vector<const geo::IPoint *> &vec,
                                                 const TRib &rib, int nrOfPoints) {
  assert(nrOfPoints > 1);
...
const std::map<SIDE_ID, geo::CTriSurface *> &CTriaIntersecBox::getSideSurfaces() const { return m_SideSurfaces; }
...
class Tria {
public:
  Tria(const geo::IPoint *p1, const geo::IPoint *p2, const geo::IPoint *p3, int i1, int i2, int i3) {
    assert(i1 != i2);
    assert(i2 != i3);
    assert(i3 != i1);
    ...
    for (size_t i = 0; i < intersec.m_Sides.size(); i++) {
      ...
      while (triit != triaset.end()) {
        ...
        if (on1 && on2) {
          ...
          //					intersec.AddToTestArray("Segments on Side",new geo::CLine(*new geo::CPoint(*on1),*new
          // geo::CPoint(*on2)));
          ...
          if (on3) {
            //						intersec.AddToTestArray("Segments on Side and triangle",new geo::CLine(*new
            // geo::CPoint(*on1),*new geo::CPoint(*on3)));
            assert(false); // testing
            CSegmentOnSide seg2(new geo::CPoint(*on1), new geo::CPoint(*on3));
            vec.push_back(seg2);

            //						intersec.AddToTestArray("Segments on Side and triangle",new geo::CLine(*new
            // geo::CPoint(*on2),*new geo::CPoint(*on3)));
...
class Segment {
public:
  Segment(const geo::IPoint *p1, const geo::IPoint *p2, int i1, int i2) {
    assert(i1 != i2);
    assert(*p1 != *p2);
...
class SegmentLess {
public:
  SegmentLess(bool use_pointer_compare) : m_use_pointer_compare(use_pointer_compare) {}
...
public:
  SegmentHelper(bool owner, bool use_pointer_compare = true)
      : ConnectivityHelper(owner), segmentset(SegmentLess(use_pointer_compare)) {}
...
int CTriaIntersecBox::CutSurfacesCount() const { return m_CutSurfaces.size(); }

const STNamedSurface &CTriaIntersecBox::GetCutSurfaces(int index) const { return m_CutSurfaces[index]; }

const std::vector<STNamedSurface> &CTriaIntersecBox::getCutSurfaces() const { return m_CutSurfaces; }
...
public:
  ...
  PointLessOnLine(const PointLessOnLine &rhs) { m_line = rhs.m_line; }
...
static void InsertIsolatedEdgeInSet(const geo::ISurface *surf, std::set<Segment *> &once_inserted_segments,
                                    SegmentHelper &segments) {
...
const geo::CBox &CTriaIntersecBox::Box() const { return m_box; }
