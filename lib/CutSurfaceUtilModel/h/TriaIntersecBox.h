...
struct STNamedSurface
{
  ...
  STNamedSurface(	const std::string& name,
...
private:
  ...
  void FilterSurfaceElements(const geo::ISurface& surf,
...
public:
  ...
  CTriaIntersecBox(const geo::CBox& box)
    : m_box(box)
  {
  }
  ...
  static void	GetPointsOnRib(	const geo::IBox& box,
