...
CDianaSignals::CDianaSignals(const QObject* sender)
: QObject()
, m_sender(sender)
{
  assert(m_sender != 0);
...
void CDianaSignals::slotAppSpcMessage(QString appSpcMessage)
{
  onAppSpcMessage(appSpcMessage);
}

void CDianaSignals::slotError(QString error)
{
  onError(error);
}

void CDianaSignals::slotWarning(QString warning)
{
  onWarning(warning);
}

void CDianaSignals::slotStopExecuting(bool &stop)
{
  onStopExecuting(stop);
}

void CDianaSignals::slotFinished()
{
  onFinished();
}
