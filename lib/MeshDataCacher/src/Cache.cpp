...
CCache::CCache(const CMeshDataCacher &cacher, int length)
    : m_meshdatacacher(cacher), m_nLength(0), m_nMaxLength(length), m_nMaxLengthDefined(length),
      m_nSuccessiveFailedWrites(0), m_first(0), m_last(0), m_nCriticalSectionSize(0) {}
...
int CCache::MaxLength() const { return m_nMaxLength; }

int CCache::MaxLengthDefined() const { return m_nMaxLengthDefined; }
...
#define IF_VALID_RETURN(data)                                                                                          \
  if (nColumn < m_vcCriticalSection.size())                                                                            \
    ...
    if (m_nLength == m_nMaxLength && m_nMaxLength > 0 && m_last) {
      ...
      if (bValid) {
        ...
        if (m_nSuccessiveFailedWrites == 1)
          _m()->msg(
              "Failure to write to the result cache; keeping blocks in memory. Please check whether the disk is full.");
...
CCache::CCellList::CCellList(CCellList *prev, CCellList *next, CDataCell &cell)
    : m_cell(cell), m_prev(prev), m_next(next) {
  if (prev)
    prev->m_next = this;
  if (next)
    next->m_prev = this;
...
CCache::CCellList::~CCellList() { delete &m_cell; }

CCache::CCellList *CCache::CCellList::Next() { return m_next; }

CCache::CCellList *CCache::CCellList::Prev() { return m_prev; }

CDataCell &CCache::CCellList::Cell() { return m_cell; }
