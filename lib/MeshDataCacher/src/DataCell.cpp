...
CDataCell::CDataCell(const CMeshDataCacher &cacher, int nColumn)
    : m_cacher(cacher), m_pData(0), m_nColumn(nColumn), m_bDirty(false),
...
bool CDataCell::Contains(int nColumn) const { return m_nColumn == nColumn; }
...
bool CDataCell::ReadData(int /*nElement*/) { return Cacher().StorageInterface()->ReadBlock(*this); }

bool CDataCell::WriteData() { return Cacher().StorageInterface()->WriteBlock(*this); }
...
bool CDataCell::Dirty() const { return m_bDirty; }

const CMeshDataCacher &CDataCell::Cacher() const { return m_cacher; }
