  ...
  , m_maxIfNodes(0)
{
  ...
  if (bodies.size() == 1 && faults.size() == 1 && bodies[0].first == 0 && bodies[0].second == faults[0].first && faults[0].second == m_size)
  {
...
int CMeshDataCacher::Size() const
{
  return m_pivot * m_maxEltNodes + (m_size - m_pivot) * m_maxIfNodes;
}

int CMeshDataCacher::MaxNodeSize(int nElement) const
{
  return nElement < m_pivot ? m_maxEltNodes : m_maxIfNodes;
}
...
int CMeshDataCacher::NrCells(int cacheSizeMb)
{
  ...
  if(numcells > INT_MAX) numcells = INT_MAX; // cap for integer usage
  if(numcells < 64) numcells = 64;
...
void CMeshDataCacher::GetChecksum(int nColumn, unsigned char *buffer)
{
  m_pCache->GetChecksum(nColumn, buffer);
}

void CMeshDataCacher::SetChecksum(int nColumn, unsigned char *buffer)
{
  m_pCache->SetChecksum(nColumn, buffer);
}

bool CMeshDataCacher::ChecksumOK(int nColumn, int *nStatus)
{
  return m_pCache->ChecksumOK(nColumn, nStatus);
}

void CMeshDataCacher::Flush()
{
  m_pCache->Flush();
}
...
IStorageInterface *CMeshDataCacher::StorageInterface() const
{
  return m_storage;
}
...
CMeshDataCacherTestAccessor::CMeshDataCacherTestAccessor(CMeshDataCacher& mdc)
  : m_MeshDataCacher(mdc)
{
}
...
void CMeshDataCacherTestAccessor::DisablePivot()
{
  ...
  m_MeshDataCacher.m_maxEltNodes = m_MeshDataCacher.m_maxEltNodes = std::max(m_MeshDataCacher.m_maxEltNodes, m_MeshDataCacher.m_maxEltNodes);
}

int CMeshDataCacherTestAccessor::Length() const
{
  return m_MeshDataCacher.m_pCache->m_nLength;
}

int CMeshDataCacherTestAccessor::MaxLength() const
{
  return m_MeshDataCacher.m_pCache->m_nMaxLength;
}
