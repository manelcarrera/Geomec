...
template <class T> class CNewWellPointMapper : public INewWellPointMapper {
public:
  ...
  CNewWellPointMapper(const CNewWellPointMapper &rhs)
      : m_Map(rhs.m_Map), m_Description(rhs.m_Description), m_pList(rhs.m_pList) {}
...
CNewWellPointMapper<T>::CNewWellPointMapper(QString description, const std::list<CNewWellPoint> &List)
    : m_Description(description), m_pList(&List) {}

template <class T> CNewWellPointMapper<T>::~CNewWellPointMapper() { ClearMap(); }

template <class T> const T &CNewWellPointMapper<T>::GetItem(const CNewWellPoint *point) const {
  typename std::map<const CNewWellPoint *, T>::const_iterator it = m_Map.find(point);
  assert(it != m_Map.end());

  return (*it).second;
}

template <class T> T &CNewWellPointMapper<T>::GetItem(const CNewWellPoint *point) {
...
template <class T> const std::list<CNewWellPoint> &CNewWellPointMapper<T>::List() const { return *m_pList; }

template <class T> const CNewWellPoint *CNewWellPointMapper<T>::At(size_t index) const {
...
template <class T> void CNewWellPointMapper<T>::InsertItem(CNewWellPoint *point, const T &item) {
  typename std::map<const CNewWellPoint *, T>::iterator it = m_Map.find(point);
  assert(it == m_Map.end());
  m_Map.insert(std::make_pair(point, item));
}

template <class T> void CNewWellPointMapper<T>::ClearMap() { m_Map.clear(); }

template <class T> const QString CNewWellPointMapper<T>::Description() const { return m_Description; }

template <class T> void CNewWellPointMapper<T>::Description(QString description) { m_Description = description; }
