...
CNewWellDefinitionPointList::CNewWellDefinitionPointList(CNewWellPathBase &WellPath)
...
long CNewWellDefinitionPointList::RemoveRedundantPoints() {
  ...
  while (p1 && p2 && p3) {
    ...
    CNewWellPathBase::GetBetweenPosition(*p1, *p3, p2->TMD(), eas, nor, tvd, azi, inc);

    if (fabs(eas - p2->Easting()) < eps && fabs(nor - p2->Northing()) < eps && fabs(tvd - p2->TVD()) < eps)
