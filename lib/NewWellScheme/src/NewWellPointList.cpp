...
CNewWellPointList::CNewWellPointList(const CNewWellPathBase& WellPath,
  bool bAutoDelete, bool bAllowDuplicateTMD)
: QObject()
, m_MaxPoint()
, m_MinPoint()
...
bool CNewWellPointList::AutoDelete() const
{
  return m_bAutoDelete;
}

void CNewWellPointList::AutoDelete(bool autodelete)
{
  m_bAutoDelete = autodelete;
}

bool CNewWellPointList::AllowDuplicateTMD() const
{
  return m_bAllowDuplicateTMD;
}

void CNewWellPointList::AllowDuplicateTMD(bool allowduplicate)
{
  m_bAllowDuplicateTMD = allowduplicate;
}

int CNewWellPointList::NrOfPoints() const
{
...
bool CNewWellPointList::EqualPointExist(const CNewWellPoint& point) const
{
  return (GetIndex(&point) != -1);
}

bool CNewWellPointList::Exist(const double& tmd) const
{
  return EqualPointExist(CNewWellPoint(WellPath(),tmd)); 
}

bool CNewWellPointList::Exist(const CNewWellPoint* point) const
{
  return !(GetIndex(point)==-1);
}


CNewWellPointList::Iterator CNewWellPointList::GetIteratorAt(
  const CNewWellPoint* point) const
{
  for(Iterator ret = This()->m_lstPoints.begin();
  ret != This()->m_lstPoints.end(); ++ret)
  {
  if(*ret == point)
      return ret;
  }
...
CNewWellPointList::Iterator CNewWellPointList::begin() const
{
  return This()->m_lstPoints.begin();
}

CNewWellPointList::Iterator CNewWellPointList::end() const
{
  return This()->m_lstPoints.end();
}
...
{
  if(m_lstPoints.indexOf(const_cast<CNewWellPoint*>(point)) != -1 &&
  !m_bAllowDuplicateTMD)
  {
    return -1;
  }
...
const CNewWellPathBase& CNewWellPointList::WellPath() const
{
  return *m_pWellPath;
}

void CNewWellPointList::WellPath(CNewWellPathBase *pWellPath) 
{
...
QString CNewWellPointList::GetDescription() const
{
  return m_Description;
}

void CNewWellPointList::SetDescription(QString description) 
{
...
CNewWellPointList* CNewWellPointList::This() const
{
  return const_cast<CNewWellPointList*>(this);
}
