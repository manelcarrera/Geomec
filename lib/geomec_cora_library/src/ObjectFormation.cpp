...
CObjectFormation::CObjectFormation(CFormationBase* formationBase,
  CModelBase* modelBase)
: CObjectBase()
, m_formationBase(formationBase)
, m_parameters(getParameters(m_formationBase, modelBase))
, m_owner(false)
, m_elementSet(createElementSet(m_owner, modelBase, formationBase))
...
const QString& CObjectFormation::name() const
{
  return m_formationBase->Name();
}

const TParameters& CObjectFormation::getParameters() const
{
  return m_parameters;
}
...
const QString& CObjectFormation::prefix() const
{
  return PREFIX;
}

const COpenGLNode* CObjectFormation::object() const
{
  return m_formationBase;
}
...
const QString& CObjectFormation::typeName() const
{
  return TYPE_NAME;
}
...
CElementSet* CObjectFormation::getElementSet() const
{
  return m_elementSet;
}
...
void getMaterialParameters(TParameters& parameters,
  CFormationBase* formationBase, CModelBase* /*modelBase*/,
...
TParameters CObjectFormation::getParameters(CFormationBase* formationBase,
  CModelBase* modelBase)
{
    ...
    if ((*depletionStage).IsPhaseStartStage())
    {
      ...
      getPressureParameters <CFormationBase, geo::IElementSet,
        CParameter::TFormationParameter4Pressure, CPressure> (parameters,
          ...
          IDS_VALUENAME_PRESSURE, PRESSURE_TYPE);
...
const QString NUMBER_OF_NODES =
  "Only 4 or 8 nodes can be handled, detected %1 nodes";
...
CElementSet* CObjectFormation::createElementSet(bool& owner,
  CModelBase* modelBase, CFormationBase* formationBase)
{
...
CElementSet* CObjectFormation::elementSetExists(CModelBase* modelBase,
