...
CXMLModelInfo::CXMLModelInfo(CModelData &modelData, const char *modelInfoFileName)
    : m_modelData(modelData), m_modelInfoFileName(modelInfoFileName), m_components(modelData) {}

bool CXMLModelInfo::operator()() const { return (((m_modelData() != 0) && createModelInfo()) ? true : false); }
...
bool CXMLModelInfo::createModelInfo() const {
  std::ofstream modelInfoFile((QString(m_modelInfoFileName) + ".xml").toStdString().c_str());
