...
CParameterModifier::CParameterModifier(CSummaryResultFile &summaryResultFile, const QString &option,
                                       std::vector<double> &value)
    : m_parameterModifierBase(selectParameterModifier(summaryResultFile, option, value)) {}
...
const QString ERROR_MESSAGE = "Unknown option '%1', known options are: '%2', '%3', '%4'";
...
bool verifyArgumentCount(CSummaryResultFile &summaryResultFile, const QString &option, std::vector<double> &value) {
  bool invalidCount = false;
  size_t requiredArguments = ZERO_ARGUMENTS;

  if ((option == OPTION_FACTOR) || (option == OPTION_SHIFT) || (option == OPTION_ABSOLUTE)) {
    requiredArguments = ONE_ARGUMENT;
    invalidCount = (value.size() != ONE_ARGUMENT);
  } else if ((option == OPTION_FACTOR_SHIFT) || (option == OPTION_SHIFT_FACTOR)) {
    requiredArguments = TWO_ARGUMENTS;
    invalidCount = (value.size() != TWO_ARGUMENTS);
  }

  if (invalidCount) {
    QString additionalInformation = QString(ERROR_COUNT).arg(option).arg(requiredArguments);
...
TParameterModifierBase CParameterModifier::selectParameterModifier(CSummaryResultFile &summaryResultFile,
  ...
  if (verifyArgumentCount(summaryResultFile, option, value)) {
    if (option == OPTION_FACTOR) {
      parameterModifierBase = TParameterModifierBase(new CParameterModifierFactor(value));
    } else if (option == OPTION_SHIFT) {
      parameterModifierBase = TParameterModifierBase(new CParameterModifierShift(value));
    } else if (option == OPTION_ABSOLUTE) {
      parameterModifierBase = TParameterModifierBase(new CParameterModifierAbsolute(value));
    } else if (option == OPTION_FACTOR_SHIFT) {
      parameterModifierBase = TParameterModifierBase(new CParameterModifierFactorShift(value));
    } else if (option == OPTION_SHIFT_FACTOR) {
      parameterModifierBase = TParameterModifierBase(new CParameterModifierShiftFactor(value));
    } else {
      QString additionalInformation =
          QString(ERROR_MESSAGE).arg(option).arg(OPTION_FACTOR).arg(OPTION_SHIFT).arg(OPTION_ABSOLUTE);
