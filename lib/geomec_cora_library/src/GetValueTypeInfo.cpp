...
std::pair<geo::CValue, geo::CValue> CGetValueTypeInfo::getRange(unsigned int valueTypeID,
                                                                const QString &valueTypeName) const {
  CPointSet pointSet(*m_modelBase);
  QSharedPointer<CValueType> valueType =
      QSharedPointer<CValueType>(CValueTypeFactory::instance()->BuildValueType(pointSet, valueTypeID, valueTypeName));
  IValueComponentBase &valueComponent = valueType->Component();
  CValueComponent &component = dynamic_cast<CValueComponent &>(valueComponent);
...
CGetValueTypeInfo::getRange(const CLibraryMaterialParameter &libraryMaterialParameter) const {
  return getRange(libraryMaterialParameter.ValueTypeID(), libraryMaterialParameter.Name());
}

// private

CGetValueTypeInfo::CGetValueTypeInfo(CModelBase *modelBase) : m_modelBase(modelBase) { atexit(&cleanup); }
