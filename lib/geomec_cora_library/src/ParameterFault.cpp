...
CParameterFault::CParameterFault(const QString& name,
  const geo::CValue& minimum, const geo::CValue& maximum, double mean,
  unsigned int valueTypeID, int depletionStage, CHorizonBase* horizonBase)
: CParameterLimit()
, m_name(name)
, m_minimum(minimum)
, m_maximum(maximum)
...
int CParameterFault::depletionStage() const
{
  return m_depletionStage;
}

unsigned int CParameterFault::valueTypeID() const
{
  return m_valueTypeID;
}

const QString& CParameterFault::name() const
{
  return m_name;
}

CGraphNode* CParameterFault::object() const
{
...
double CParameterFault::mean() const
{
  return m_mean;
}
