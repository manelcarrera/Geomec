...
CFailureTypeParameterFormation4PressureChange::CFailureTypeParameterFormation4PressureChange(
...
void CFailureTypeParameterFormation4PressureChange::modify(CModelBase *modelBase) {
  if (m_actualParameter) {
    ...
    const CPressure &initialPressure = formationBase->Pressure(modelBase->InitialDepletionStage());
    CPressure &pressure =
    ...
    for (int s = 0; s < formationBase->ElementSetSize(); ++s) {
      for (int e = 0; e < formationBase->ElementSet(s).ElementSet().ElementSize(); ++e) {
        ...
        for (size_t n = 0; n < newValues.size(); ++n) {
          newValues[n] = m_parameterModifier->modify(pressureValues[n] - initialPressureValues[n]).Value();
        }
