...
CParameterFile::CParameterFile(CSummaryResultFile &summaryResultFile, const CGetModelInfo &modelInfo,
                               const QString &fileName)
    : m_fileName(fileName), m_parameterFile(m_fileName.toStdString().c_str()), m_summaryResultFile(summaryResultFile),
      m_parameters(readParameterFile(m_summaryResultFile, modelInfo, m_parameterFile, m_fileName)) {
  validateParameterFile();
}

std::vector<TFailureTypeParameter> CParameterFile::getParameters() const { return m_parameters; }
...
const QString PRESSURE_ORDER = "Pressure_D0 not defined before Dlt_Pressure_D..., re-ordering";
const QString DEPLETION_STAGE_DESIGNATOR = "Pressure_Dx and Dlt_Pressure_Dx have the same depletion stage designator";
...
void CParameterFile::validatePressureOrdering() {
  ...
  for (std::vector<TFailureTypeParameter>::const_iterator parameter = m_parameters.begin();
       parameter != m_parameters.end(); ++parameter, ++position) {
    ...
    if (actualParameter->isParameterType<CParameterFormation4PressureChange>()) {
      pressureChangeSeen = true;
      pressureChangePosition = position;
    }
  }

  if (pressureSeen && pressureChangeSeen && (pressurePosition > pressureChangePosition)) {
...
void CParameterFile::validateDepletionStageDesignator() {
  ...
  for (std::vector<TFailureTypeParameter>::const_iterator parameter = m_parameters.begin();
       parameter != m_parameters.end(); ++parameter) {
    TParameter actualParameter = (*parameter)->getActualParameter();
  ...
  for (std::vector<TFailureTypeParameter>::const_iterator parameter = m_parameters.begin();
    ...
    if (actualParameter->isParameterType<CParameterFormation4PressureChange>()) {
      if (!pressureDepletionStages.insert(actualParameter->depletionStage()).second) {
...
const QString PARAMETER_FILE_DOES_NOT_EXIST = "parameter file '%1' does not exist";

} // anonymous namespace

std::vector<TFailureTypeParameter> CParameterFile::readParameterFile(CSummaryResultFile &summaryResultFile,
  ...
  if (!parameterFile.good()) {
    ...
    summaryResultFile.addAdditionalInformation(QString(PARAMETER_FILE_DOES_NOT_EXIST).arg(fileName));
  }
  ...
  while (!parameterFile.eof() && !std::getline(parameterFile, line).fail()) {
    if (!line.empty()) {
      ...
      try {
        failureType = TFailureTypeParameter(new CFailureTypeParameter(summaryResultFile, stream, modelInfo));
      }
