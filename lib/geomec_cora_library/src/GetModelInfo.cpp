...
CGetModelInfo::CGetModelInfo(CModelData& modelData,
  const char* modelInfoFileName)
: m_modelData(modelData)
, m_modelInfoFileName(modelInfoFileName)
, m_elementSetInfo(m_modelData())
, m_estimatedMemoryUsage()
, m_faultInfo(CGetFaultInfo::instance(m_modelData()))
, m_formationInfo(CGetFormationInfo::instance(m_modelData()))
, m_pointSetInfo(m_modelData())
, m_failureModeInfo(m_modelData())
, m_surfaceInfo(CGetSurfaceInfo::instance(m_modelData()))
...
bool CGetModelInfo::operator () () const
{
  return (((m_modelData() != 0) && createModelInfo()) ? true : false);
}

const CGetFaultInfo& CGetModelInfo::getFaultInfo() const
{
  return m_faultInfo;
}

const CGetFormationInfo& CGetModelInfo::getFormationInfo() const
{
  return m_formationInfo;
}

CGetFailureModeInfo& CGetModelInfo::getFailureModeInfo()
{
  return m_failureModeInfo;
}

const CGetSurfaceInfo& CGetModelInfo::getSurfaceInfo() const
{
  return m_surfaceInfo;
}

const CGetHorizonInfo& CGetModelInfo::getHorizonInfo() const
{
  return m_horizonInfo;
}

const CGetWellPathInfo& CGetModelInfo::getWellPathInfo() const
{
  return m_wellPathInfo;
}
