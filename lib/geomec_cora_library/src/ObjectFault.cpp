...
, m_parameters(getParameters(modelBase, horizonBase))
, m_owner(false)
, m_elementSet(createElementSet(m_owner, modelBase, horizonBase))
, m_sequenceState(0)
{
}
...
const QString& CObjectFault::name() const
{
  return m_horizonBase->Name();
}

const TParameters& CObjectFault::getParameters() const
{
  return m_parameters;
}
...
const QString& CObjectFault::prefix() const
{
  return PREFIX;
}

const COpenGLNode* CObjectFault::object() const
{
  return m_horizonBase;
}
...
const QString& CObjectFault::typeName() const
{
  return TYPE_NAME;
}
...
CElementSet* CObjectFault::getElementSet() const
{
  return m_elementSet;
}
...
TParameters CObjectFault::getParameters(CModelBase* modelBase,
...
const QString NUMBER_OF_NODES =
  "Only 3 or 4 nodes can be handled, detected %1 nodes";

} // anonymous namespace

CElementSet* CObjectFault::createElementSet(bool& owner, CModelBase* modelBase,
  CHorizonBase* horizonBase)
{
...
CElementSet* CObjectFault::elementSetExists(CModelBase* modelBase,
  CHorizonBase* horizonBase)
{
