...
bool CFilterOutputProperty::isOutputPropertyAllowed(const CObject& object,
  const QString& result) const
{
  ...
  return (result == FORMATION_SHEAR_FAILURE) ||
      ...
      (result == FORMATION_EXTENSION_FAILURE) ||
      (result == FORMATION_VOLUMETRIC_COMPACTION) ||
      (result == FORMATION_COMPRESSIVE_PLASTIC_STRAIN) ||
      (result == FORMATION_EXTENSIONAL_PLASTIC_STRAIN);
  }
