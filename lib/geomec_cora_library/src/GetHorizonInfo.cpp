...
const TObjects CGetHorizonInfo::getObjects() const { return m_objects; }
...
TObjects CGetHorizonInfo::getObjects(const CModelBase *modelBase) {
  ...
  if (modelBase != 0) {
    ...
    const THorizonBaseEntry::TNodeSet entryNodes = horizonBaseEntry->EntryNodes();

    for (THorizonBaseEntry::TNodeSet::const_iterator entryNode = entryNodes.begin(); entryNode != entryNodes.end();
         ++entryNode) {
      // TODO SURFACE-HORIZON
      // for now only add the top-horizon

      if ((dynamic_cast<C3DHorizon *>(*entryNode))->IsTopHorizon()) {
        objects.push_back(TObject(new CObject(CObject::horizonObject, *entryNode)));
      }
    }
  }
...
std::ostream &operator<<(std::ostream &os, const cora::CGetHorizonInfo &i) { return i(os); }
