...
bool CResponseTypeBaseValidate::validateFunctionSize(CSummaryResultFile &summaryResultFile,
...
struct TAccumulate {
  QString operator()(const QString &lhs, const QString &rhs) { return lhs + (lhs.length() > 0 ? COMMA : EMPTY) + rhs; }
};

} // anonymous namespace

QString CResponseTypeBaseValidate::concatenateFunction(const std::vector<QString> &function) {
  /*
  ...
  */
  ...
  QString concatenation = std::accumulate(function.begin(), function.end(), QString(), TAccumulate());
...
double CResponseTypeBaseValidate::extractValue(CSummaryResultFile &summaryResultFile,
                                               const std::vector<QString> &function, size_t validFunctionSize,
  ...
  if (!ok) {
    ...
    summaryResultFile.setResultValue(CSummaryResultFile::RESULT_VALUE_INCONSISTENT);
    message = QString(LSF_CANNOT_CONVERT_2_DOUBLE).arg(message).arg(function[valuePosition]);
    summaryResultFile.addAdditionalInformation(message);
