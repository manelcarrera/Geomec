...
CRunModel::CRunModel(CRunModelData &runModelData, const QString &applicationVersion)
    : m_runModelData(runModelData), m_applicationVersion(applicationVersion),
      m_analysisTypes(selectAnalysisTypes(m_runModelData.summaryResultFile(), m_runModelData.selectedLSFs())) {
  processModelData();
...
bool CRunModel::operator()() {
  ...
  if ((m_runModelData.modelData())() != 0) {
    ...
    SplitPathAndFileName(m_runModelData.modelData().fileName(), modelPath, modelFileName);

    QString modelName = RemoveExtension(modelFileName);
    CRunModelEngine runModelEngine(*(m_analysisTypes.begin()));

    m_runModelData.summaryResultFile().addAdditionalInformation(addFullPath(m_runModelData.modelData().fileName()));

    ok = ok && runModelEngine.run(m_runModelData, modelName, m_applicationVersion);

    if (ok) {
      if (!m_runModelData.minimumOutput()) {
        ok = IModelLifetimeFacade::SaveMain(CModelBase::CModelLoadSaveDefault(m_applicationVersion),
                                            *m_runModelData.modelData()(), modelFileName, true);
      }
    } else {
  ...
  return (ok && (m_runModelData.summaryResultFile().getResultValue() == CSummaryResultFile::RESULT_VALUE_OK));
}
...
void CRunModel::validateVersionNumber() {
  if (VERSION_NUMBER < m_runModelData.versionNumber()) {
    QString additionalInformation = QString(INVALID_VERSION).arg(m_runModelData.versionNumber()).arg(VERSION_NUMBER);

    m_runModelData.summaryResultFile().setResultValue(CSummaryResultFile::RESULT_VALUE_INCONSISTENT);
    m_runModelData.summaryResultFile().addAdditionalInformation(additionalInformation);
  }
}
...
void CRunModel::processModelData() {
  if (m_runModelData.modelData()() == 0) {
    QString additionalInformation = QString(MODEL_DOES_NOT_EXIST).arg(m_runModelData.modelData().fileName());
...
CRunModel::TAnalysisTypes CRunModel::selectAnalysisTypes(CSummaryResultFile &summaryResultFile,
                                                         const CSelectedLSFs &selectedLSFs) {
  TAnalysisTypes analysisTypes;
  std::vector<TLimitStateFunction> functions = selectedLSFs.getLimitStateFunctions();
  ...
  if (analysisTypes.size() != ONE_ANALYSIS_TYPE) {
    ...
    summaryResultFile.addAdditionalInformation(
        QString(FIRST_ANALYSIS_TYPE_IS_PROCESSED).arg(analysisType.Label().toLower()));
  }
