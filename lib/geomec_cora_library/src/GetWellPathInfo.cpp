...
const TObjects CGetWellPathInfo::getObjects() const { return m_objects; }
...
TObjects CGetWellPathInfo::getObjects(const CModelBase *modelBase) {
  ...
  if (modelBase != 0) {
    const CNewWellPathEntry *newWellPathEntry =
        dynamic_cast<const CNewWellPathEntry *>(modelBase->GraphEntry(MD_NEW_WELLPATH));
    const CNewWellPathEntry::TSortedNodeSet entryNodes = newWellPathEntry->SortedEntryNodes();

    for (CNewWellPathEntry::TSortedNodeSet::const_iterator entryNode = entryNodes.begin();
...
std::ostream &operator<<(std::ostream &os, const cora::CGetWellPathInfo &i) { return i(os); }
