...
CGetFormationParameterInfo::CGetFormationParameterInfo(
  const CFormationBase* formationBase)
: m_formationBase(formationBase)
, m_totalVolume(calculateVolume(m_formationBase))
{
}

double CGetFormationParameterInfo::getMean(
  const CLibraryMaterialParameter& libraryMaterialParameter,
  const std::set <CValueType*>& valueTypes) const
{
  for (std::set <CValueType*> ::const_iterator
    valueType = valueTypes.begin(); valueType != valueTypes.end(); ++valueType)
  {
...
std::pair <geo::CValue, geo::CValue> CGetFormationParameterInfo::getRange(
  const CLibraryMaterialParameter& libraryMaterialParameter,
  const std::set <CValueType*>& valueTypes) const
{
  CGetValueTypeInfo& getValueTypeInfo(CGetValueTypeInfo::instance(
    const_cast <CModelBase*> (dynamic_cast <const CModelBase*> (
      &m_formationBase->Model()))));
  std::pair <geo::CValue, geo::CValue>
    range = getValueTypeInfo.getRange(libraryMaterialParameter);
...
double CGetFormationParameterInfo::calculateVolume(
  const CFormationBase* formationBase)
{
  ...
  {
    const IFormationElementSet&
      formationElementSet = formationBase->ElementSet(set);
    const geo::IElementSet& elementSet = formationElementSet.ElementSet();

    for (int element = 0; element < elementSet.ElementSize(); ++element)
    {
      const geo::IBody& body =
        dynamic_cast <const geo::IBody&> (elementSet.Element(element));
  ...
  const CValueType* valueType) const
{
  ...
  {
    const IFormationElementSet&
      formationElementSet = m_formationBase->ElementSet(set);
    const geo::IElementSet& elementSet = formationElementSet.ElementSet();
    ...
    for (int element = 0; element < elementSet.ElementSize(); ++element)
    {
      ...
      for (IValueDomainScalar::TValueVec::const_iterator
        value = valueVec.begin(); value != valueVec.end(); ++value)
      {
        if ((*value).Valid())
        {
          ++count;
      ...
      mean += (elementSet.Element(element).Size() / m_totalVolume) *
        (totalValue / (count == 0 ? 1 : count));
    }
  }
...
void validateValueVector(
  const IValueDomainScalar::TValueVec& valueVec, const CValueType* valueType)
{
...
std::pair <geo::CValue, geo::CValue> CGetFormationParameterInfo::getRange(
  ...
  const CValueType* valueType) const
{
  std::pair <geo::CValue, geo::CValue> range = std::make_pair(
  ...
  for (int set = 0; set < m_formationBase->ElementSetSize(); ++set)
  {
    ...
    for (int element = 0; element < elementSet.ElementSize(); ++element)
    {
      ...
      range.first = std::min(range.first.Value(),
        CUtilities4ValueVector::calculateAverage(valueVec));
