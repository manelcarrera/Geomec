  ...
  if (m_actualParameter) {
    ...
    for (int e = 0; e < modelBase->Mesh().Mesh().ElementSize(); ++e) {
      ...
      const CFFMaterial &cffMaterial =
          formationBase->Material(modelBase->DepletionStageEntry().StageByIndex(m_actualParameter->depletionStage()))
              .Material(element);
      ...
      if (cffMaterial.IsParameter(valueTypeID)) {
        double value = cffMaterial.ParameterValue(valueTypeID);

        newValue = m_parameterModifier->modify(value).Value();
      } else {
        m_summaryResultFile.addAdditionalInformation(
            QString(VALUE_TYPE_NOT_PRESENT).arg(valueTypeID).arg(FORMATION).arg(m_object));
      }
    ...
    createAndLinkValueType(modelBase, elementValueSet, formationBase->ActiveMaterial(), modelBase->Mesh(), valueTypeID);
  }
}
