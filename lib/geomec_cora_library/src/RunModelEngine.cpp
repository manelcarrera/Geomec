...
bool CRunModelEngine::run(CRunModelData &runModelData, const QString &modelName,
                          const QString &applicationVersion) const {
  CDianaExecuter dianaExecuter;
  CSaveModelConsole saveModelConsole(*runModelData.modelData()(), applicationVersion);
  CRetrieveDianaFileNamesConsole retrieveDianaFileNamesConsole;

  return m_runModelEngineBase->run(runModelData, dianaExecuter, modelName, saveModelConsole,
...
TRunModelEngineBase CRunModelEngine::selectEngine(CAnalysisType::TAnalysisType analysisType) {
  TRunModelEngineBase runModelEngineBase;

  if (analysisType == CAnalysisType::AT_NONLIN) {
    runModelEngineBase = TRunModelEngineBase(new CRunModelStagedRunEngine(analysisType));
  } else {
    runModelEngineBase = TRunModelEngineBase(new CRunModelSingleRunEngine(analysisType));
  }
