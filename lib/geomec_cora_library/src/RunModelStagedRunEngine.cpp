...
CRunModelStagedRunEngine::CRunModelStagedRunEngine(
...
void setOutputTypes(CModelBase* modelBase,
...
bool CRunModelStagedRunEngine::run(CRunModelData& runModelData,
  ...
  CRetrieveDianaFileNamesConsole& retrieveDianaFileNamesConsole) const
{
  ...
  dia::IDianaRunner::RunParams p = { runModelData.modelData().fileName(), modelName.toStdString(), geomecTempPath.toStdString(), &dianaExecuter, &saveModelConsole, &retrieveDianaFileNamesConsole, nullptr, false, _g->dsa() };
