...
CXMLResponseParameter::CXMLResponseParameter(TFailureMode &failureMode)
    : m_failureMode(failureMode), m_responseValueTypes() {}
...
std::ostream &CXMLResponseParameter::operator()(std::ostream &stream) const {
  stream << RESPONSE_PARAMETER_OPEN.arg(m_failureMode->getFailureModeLabel())
                .arg(m_failureMode->getResultComponent()->UnitName())
                .toStdString()
         << std::endl;
...
std::ostream &operator<<(std::ostream &stream, const cora::CXMLResponseParameter &rhs) { return rhs(stream); }
...
std::ostream &operator<<(std::ostream &stream, const cora::TXMLResponseParameters &rhs) {
  for (cora::TXMLResponseParameters::const_iterator responseParameter = rhs.begin(); responseParameter != rhs.end();
       ++responseParameter) {
