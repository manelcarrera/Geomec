...
CResponseTypeBaseWithValue::CResponseTypeBaseWithValue(CSummaryResultFile &summaryResultFile,
                                                       const std::vector<QString> &function)
    : CResponseTypeBaseValidate(summaryResultFile, function),
      m_value(extractValue(summaryResultFile, function, LSF_LENGTH_WITH_VALUE, VALUE_POSITION)),
...
const QString CResponseTypeBaseWithValue::getResponseType() const {
  QString responseType =
      QString("%1,%2,%3").arg(CResponseTypeBase::getResponseType()).arg(m_value).arg(m_tooHigh ? TOO_HIGH : TOO_LOW);
...
bool CResponseTypeBaseWithValue::extractInternalFailsWhen(CSummaryResultFile &summaryResultFile,
                                                          const std::vector<QString> &function,
                                                          const QString &internalFailsWhen) {
  validateFunctionSize(summaryResultFile, function, LSF_LENGTH_WITH_VALUE);

  if ((function[INTERNAL_FAILS_WHEN_POSITION] != TOO_LOW) && (function[INTERNAL_FAILS_WHEN_POSITION] != TOO_HIGH)) {
