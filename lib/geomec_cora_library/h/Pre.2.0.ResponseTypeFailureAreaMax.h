...
public:
  CPre20ResponseTypeFailureAreaMax(CSummaryResultFile &summaryResultFile, const std::vector<QString> &function);
  virtual ~CPre20ResponseTypeFailureAreaMax();

  virtual double calculate(const TObject &object, const TFailureMode &failureMode);

private:
  CPre20ResponseTypeFailureAreaMax(const CPre20ResponseTypeFailureAreaMax &rhs);
  CPre20ResponseTypeFailureAreaMax &operator=(CPre20ResponseTypeFailureAreaMax rhs);
};
