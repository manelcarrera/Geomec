...
public:
  CFailureTypeParameterBase(CSummaryResultFile &summaryResultFile, const std::string &object, const std::string &option,
                            const std::string &parameter, std::vector<double> &value, const TObjects &objects,
  ...
  static TObject findActualObject(CSummaryResultFile &summaryResultFile, const std::string &object,
                                  const TObjects &objects, const QString &objectType);
  static TParameter findActualParameter(CSummaryResultFile &summaryResultFile, const std::string &object,
                                        const std::string &parameter, const TParameters &parameters);

protected:
  template <typename V> void removeValueType(CModelBase *modelBase, const V &valueNode, unsigned int valueTypeID) const;
  template <typename V>
  void createAndLinkValueType(CModelBase *modelBase, CElementValueSet &elementValueSet, V &valueNode,
                              IPointSet &pointSet, unsigned int valueTypeID) const;
...
private:
  ...
  CFailureTypeParameterBase &operator=(const CFailureTypeParameterBase &rhs);
};

typedef QSharedPointer<CFailureTypeParameterBase> TFailureTypeParameterBase;

template <typename V>
void CFailureTypeParameterBase::removeValueType(CModelBase *modelBase, const V &valueNode,
                                                unsigned int valueTypeID) const {
  TValueCompositeEntry *valueCompositeEntry =
      dynamic_cast<TValueCompositeEntry *>(modelBase->GraphEntry(MD_BASE_VALUE_COMPOSITE));
  ...
  TValueCompositeEntry::TNodeSet entryNodes = valueCompositeEntry->EntryNodes();
  ...
  while (node != entryNodes.end()) {
    ...
    if (valueComposite->TypeId() == valueTypeID) {
      if (valueNode.IsLinkedTo(*valueComposite) && valueComposite->IsLinkedTo(valueNode)) {
...
template <typename V>
void CFailureTypeParameterBase::createAndLinkValueType(CModelBase *modelBase, CElementValueSet &elementValueSet,
                                                       V &valueNode, IPointSet &pointSet,
                                                       unsigned int valueTypeID) const {
  ...
  if (valueTypeFactory->ValueTypeAvailable(valueTypeID)) {
    ...
    CValueType *valueType = valueTypeFactory->BuildValueType(pointSet, valueTypeID, valueTypeName);
