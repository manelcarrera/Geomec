...
template <typename ParametersNode> class CCohesionValueTypeWrapper : public IValueTypeWrapper {
public:
...
private:
  ...
  CCohesionValueTypeWrapper &operator=(const CCohesionValueTypeWrapper &rhs);

  const ParametersNode &m_parametersNode;
};

template <typename ParametersNode>
CCohesionValueTypeWrapper<ParametersNode>::CCohesionValueTypeWrapper(const ParametersNode &parametersNode)
    : IValueTypeWrapper(), m_parametersNode(parametersNode) {}

template <typename ParametersNode> CCohesionValueTypeWrapper<ParametersNode>::~CCohesionValueTypeWrapper() {}

template <typename ParametersNode> bool CCohesionValueTypeWrapper<ParametersNode>::hasDistributedValueType() const {
  return m_parametersNode.DistributedCohesion();
}

template <typename ParametersNode>
const std::vector<double>
CCohesionValueTypeWrapper<ParametersNode>::getDistributedValueType(const geo::IElement &element,
                                                                   const CQuantity::UNIT unit) const {
  return m_parametersNode.Cohesion(element, unit);
}

template <typename ParametersNode>
const CDoubleQuantity &CCohesionValueTypeWrapper<ParametersNode>::getDistributedValueTypeQuantity() const {
