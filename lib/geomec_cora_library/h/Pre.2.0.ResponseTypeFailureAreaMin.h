...
public:
  CPre20ResponseTypeFailureAreaMin(CSummaryResultFile &summaryResultFile, const std::vector<QString> &function);
  virtual ~CPre20ResponseTypeFailureAreaMin();

  virtual double calculate(const TObject &object, const TFailureMode &failureMode);

private:
  CPre20ResponseTypeFailureAreaMin(const CPre20ResponseTypeFailureAreaMin &rhs);
  CPre20ResponseTypeFailureAreaMin &operator=(CPre20ResponseTypeFailureAreaMin rhs);
};
