...
//
CDianaExecuter::CDianaExecuter() : m_bShowDefaultMessages(true) {
  m_handler = new CDianaExecuterSignalHandler();

  m_dsu = CDianaStartUp::instance();
}

CDianaExecuter::~CDianaExecuter() { delete m_handler; }
...
bool CDianaExecuter::GetCalculationResult() {
  ...
  //
    ...
    dia::IDianaRunner::GetResultInfo(FFDIR.toStdString().c_str(), FF.toStdString().c_str(), b_HaveResults,
                                     b_CalculationResult);
