...
// const QString VERSION = xstr(GM_MAJOR) "." xstr(GM_MINOR) "." xstr(GM_BUILD) "." xstr(GM_REVISION) " "
// GM_BUILD_PREFIX " " xstr(GM_BUILD_STRING);
...
void CRunGm::init(IDianaRunController *drc) {
  ...
  // FIXME: MCR
  // m_pDianaRunController = new CDianaRunController(*m_model, CAnalysisType::AT_NONLIN, false, false, false,
  // m_temp_path);
...
bool CRunGm::step() {
  ...
  if (!m_bHasRun) {
    ...
    CDepletionStage *pStage =
        m_model->DepletionStageEntry()->LastStage(); //&m_model->DepletionStageEntry().LastStage();
    ...
    if (pStage) {
      ...
      try {
        ...
        bRet = m_drc->RunFirst(m_model_full_path, m_pDianaExecuter, m_model_name.toStdString(), *m_pSaveModel,
    ...
    try {
      bRet = m_drc->RunNext(m_model_full_path, m_pDianaExecuter, m_model_name.toStdString(), *m_pSaveModel,
                            *m_pRetrieveDianaFileNames);
    } catch (CProgressCancel *p) {
      delete p;
      bRet = false;
