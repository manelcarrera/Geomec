...
CMatParam::CMatParam(const QString &strName, double dValue, CMaterial &parentMat, const CUnitType &unitConversion,
                     const CCheckStrategy *pCheckStrategy, const CSetStrategy *pSetStrategy)
    : m_strName(strName), m_dValue(dValue), m_dPreviousValue(dValue), m_parentMat(parentMat),
      m_pUnitType(&unitConversion), m_pCheckStrategy(pCheckStrategy), m_pSetStrategy(pSetStrategy) {}

CMatParam::CMatParam(CMaterial &parentMat)
    : m_parentMat(parentMat), m_pUnitType(0), m_pCheckStrategy(0), m_pSetStrategy(0) {}

CMatParam::CMatParam(const CMatParam &rhs, CMaterial &parentMat) : m_parentMat(parentMat) { *this = rhs; }
...
CMatParam *CMatParam::Clone(CMaterial &parentMat) const { return new CMatParam(*this, parentMat); }
...
bool CMatParam::operator==(const CMatParam &rhs) const {
  return (m_strName == rhs.m_strName && fabs(m_dValue - rhs.m_dValue) < std::max(1e-20, fabs(m_dValue * 1e-4)));
}

bool CMatParam::operator!=(const CMatParam &rhs) const { return !operator==(rhs); }

const QString &CMatParam::Name() const { return m_strName; }

const char *CMatParam::UnitName(int nUnitDef) const { return m_pUnitType->UnitName(nUnitDef); }

double CMatParam::Value() const { return m_dValue; }
...
double CMatParam::PreviousValue() const { return m_dPreviousValue; }
...
const CMaterial &CMatParam::ParentMaterial() const { return m_parentMat; }

CMaterial &CMatParam::ParentMaterial() { return m_parentMat; }
...
void CMatParam::LoadValue(double dValue) { InternalSetValue(dValue); }
