...
CDaoWellInterface::CDaoWellInterface(well::CWellPathBase& wellpath)
{
...
bool CDaoWellInterface::LoadFromDatabase(const CString & filename)
{
	...
	table_global.Open();
		...
		CString description, well_name, type_name, field_name, location_name, country, customer,
				cust_contact, job_number;
		long day = 0;
		long month = 0;
		long year = 0;
	...
	if(count >1)
	{
		...
		if(!m_pWellPath->InitFromArray_tmd_azi_inc(glb_northing,
												glb_easting,
												glb_depth,
												start_northing,start_easting,start_depth,
												tmd,azi,inc))
		{
			return false;
		}
