...
public:
  ...
  CSingleQuantityTemp(const double &value, const UNIT unit)
  : CDoubleQuantity()
  {
    MaxValue(DOUBLE_MAX_,  SI_UNIT);
    MinValue(DOUBLE_MIN_,  SI_UNIT);
  ...
  CSingleQuantityTemp(const double& value,const double &min ,const double &max, const UNIT unit)
  :CDoubleQuantity()
  {
    MaxValue(max,  unit);
    MinValue(min,  unit);
  ...
  CSingleQuantityTemp(const CCohesionQuantity& rhs)
  : CDoubleQuantity(rhs)
  {
  }

  CSingleQuantityTemp(const double& value,
            const double &min ,
            const double &max,
            bool bIncludeMin,
            bool bIncludeMax,
