...
CGridDefinition::CGridDefinition(const double &trackspacing,
                                 ...
                                 int firstbin,
                                 int firsttrack)
:m_TrackSpacing(trackspacing),
 m_BinSpacing(binspacing),
 m_Azimuth(azimuth),
 m_CornerNorthing(cornernorthing),
 m_CornerEasting(cornereasting),
 m_NumBin(numbin),
 m_NumTrack(numtrack),
 m_DeltaBin(deltabin),
 m_DeltaTrack(deltatrack),
 m_FirstBin(firstbin),
 m_FirstTrack(firsttrack)
{
}
/*
...
double CGridDefinition::GetLength() const
{
  return NumBin() * BinSpacing();
}

double CGridDefinition::GetWidth() const
{
  return NumTrack() * TrackSpacing();
}
...
int CGridDefinition::PointsEasting() const 
{
  return m_NumTrack;
}

int CGridDefinition::PointsNorthing() const
{
  return m_NumBin;
}
