...
const double *ITargetPointsetSource::MaximumDepth() const { return 0; }

// generate from corners and nr of points
CGridPointsetSource::CGridPointsetSource(const geo::IPoint &ptCorner1, const geo::IPoint &ptCorner2,
                                         int nPointsNorthing, int nPointsEasting)
...
int CGridPointsetSource::PointSize() const { return m_vcPoints.size(); }
...
CPointsetSource::CPointsetSource(const IPointSet &pointset) : m_pointset(pointset), m_dMaxDepth(0), m_pMaxDepth(0) {}

CPointsetSource::CPointsetSource(const IPointSet &pointset, double dMaxDepth)
    : m_pointset(pointset), m_dMaxDepth(dMaxDepth), m_pMaxDepth(&m_dMaxDepth) {}

int CPointsetSource::PointSize() const { return m_pointset.PointSize(); }

const geo::IPoint &CPointsetSource::Point(int nIndex) const { return m_pointset.PointAt(nIndex); }

const double *CPointsetSource::MaximumDepth() const { return m_pMaxDepth; }

double CPillarPos::m_dZ = 0;

CPillarPos::CPillarPos(double dX, double dY) : m_dX(dX), m_dY(dY) {}

CPillarPos::CPillarPos(const CPillarPos &rhs) : m_dX(rhs.m_dX), m_dY(rhs.m_dY) {}
...
const double &CPillarPos::X() const { return m_dX; }

const double &CPillarPos::Y() const { return m_dY; }

const double &CPillarPos::Z() const { return m_dZ; }

void CPillarPos::X(const double & /*dX*/) { assert(false); }

void CPillarPos::Y(const double & /*dY*/) { assert(false); }

void CPillarPos::Z(const double & /*dZ*/) { assert(false); }

void CPillarPos::AssertValid() const {}

bool CPillarPos::Empty() const { return false; }

CPillar::CPillarPoint::CPillarPoint(const CConstIterator &iter)
    : m_dX(iter.m_pPillar->Position().X()), m_dY(iter.m_pPillar->Position().Y()), m_dZ(iter.m_it->first) {}

const double &CPillar::CPillarPoint::X() const { return m_dX; }

const double &CPillar::CPillarPoint::Y() const { return m_dY; }

const double &CPillar::CPillarPoint::Z() const { return m_dZ; }

void CPillar::CPillarPoint::X(const double & /*dX*/) { assert(false); }

void CPillar::CPillarPoint::Y(const double & /*dY*/) { assert(false); }

void CPillar::CPillarPoint::Z(const double & /*dZ*/) { assert(false); }

void CPillar::CPillarPoint::AssertValid() const {}

bool CPillar::CPillarPoint::Empty() const { return false; }

CPillar::CConstIterator::CConstIterator() : m_pPillar(0) {}

CPillar::CConstIterator::CConstIterator(const CPillar &pillar, std::map<double, const geo::IBody *>::const_iterator it)
    : m_pPillar(&pillar), m_it(it) {}

CPillar::CConstIterator::CConstIterator(const CConstIterator &rhs) : m_pPillar(rhs.m_pPillar), m_it(rhs.m_it) {}
...
bool CPillar::CConstIterator::operator!=(const CConstIterator &rhs) { return !operator==(rhs); }
...
CPillar::CPillar(const CPillarPos &pos) : m_dX(pos.X()), m_dY(pos.Y()) {}

CPillarPos CPillar::Position() const { return CPillarPos(m_dX, m_dY); }

CPillar::const_iterator CPillar::begin() const { return CConstIterator(*this, m_mpInterval.begin()); }

CPillar::const_iterator CPillar::end() const { return CConstIterator(*this, m_mpInterval.end()); }

CPillar::size_type CPillar::size() const { return m_mpInterval.size(); }

std::pair<CPillar::const_iterator, bool> CPillar::insert(double dZ) {
  std::pair<TIntervalMap::iterator, bool> prInsert =
      m_mpInterval.insert(TIntervalMap::value_type(dZ, (const geo::IBody *)0));
...
long CPillar::SavedItems() const { return m_mpInterval.size(); }
...
void CPillar::LoadStream(CStorageNode::TSTREAM &stream, CStorageNode::TSTREAMVERSION & /*version*/,
                         CStorageNode::TPROGRESS &progress, const geo::IMesh &mesh) {
  stream >> m_dX;
  stream >> m_dY;
...
CPillarMap::CPillarMap(CFemAppModel &model) : CColorNode(model) {}

CPillarMap::CPillarMap(const QString &strName, CModelBase &model) : CColorNode(strName, model) {}

CPillarMap::~CPillarMap() { InvalidateDrawCache(); }
...
int CPillarMap::DisplayListSize() const { return 2; }
...
bool CPillarMap::Empty() const { return false; }
...
unsigned int CPillarMap::IconId() const { return IDI_POINTSET; }

unsigned int CPillarMap::TypeId() const { return 0; }
...
CPillarMap::const_iterator CPillarMap::begin() const { return m_mpPillars.begin(); }

CPillarMap::const_iterator CPillarMap::end() const { return m_mpPillars.end(); }

size_t CPillarMap::size() const { return m_mpPillars.size(); }
  ...
  if (m_arDrawPoints.Empty() && m_arDrawLines.Empty()) {
    ...
    for (it = m_mpPillars.begin(); it != m_mpPillars.end(); ++it) {
      ...
      if (m_arDrawPoints.Size() > 1)
        m_arDrawLines.PushBack(
            *new geo::CLine(m_arDrawPoints.Object(startidx), m_arDrawPoints.Object(m_arDrawPoints.Size() - 1)));
...
CMeshSampler::CMeshSampler(const ITargetPointsetSource &source, const CMeshBase &mesh)
    : m_source(source), m_mesh(mesh) {}

CPointSet *CMeshSampler::GeneratePointset(const QString &name, CModelBase &model, IProgressBase *pProgress,
                                          bool bStorePointsOutsideMesh) {
...
CPillarMap *CMeshSampler::GeneratePillarMap(const QString &name, CModelBase &model, IProgressBase *pProgress,
                                            bool bInclude) {
...
void CMeshSampler::Generate(IGenerator &generator, IProgressBase *pProgress) {
  ...
  for (int i = 0; i < m_source.PointSize(); ++i) {
    ...
    if (samplePoint.X() > m_mesh.Max().X() || samplePoint.Y() > m_mesh.Max().Y() ||
        samplePoint.X() < m_mesh.Min().X() || samplePoint.Y() < m_mesh.Min().Y()) {
      ...
      for (it = elements.begin(); it != elements.end(); ++it) {
        ...
        if (el) {
          ...
          if (ptMin.X() <= samplePoint.X() && ptMax.X() >= samplePoint.X() && ptMin.Y() <= samplePoint.Y() &&
              ptMax.Y() >= samplePoint.Y()) {
            int j;
            for (j = 0; j < el->NrOfFaces(); ++j) {
              ...
              if (face.NrOfPoints() == 3) {
                ...
                if (fabs(nz) > EPS) {
                  ...
                  if ((bOn12 && (bOn23 || bOn31 || sign(A23) == sign(A31))) ||
                ...
                if (!pt.Empty()) {
                  geo::CCoordinateSet<geo::CElementPoint, CEps>::iterator its =
                      intersects.insert(geo::CElementPoint(pt)).first;
                  its->AddElement(*el);
                }
              }
            }
...
bool CMeshSampler::IGenerator::AddPointsOutsideMesh() const { return false; }

///// CPointsetGenerator
CMeshSampler::CPointsetGenerator::CPointsetGenerator(CPointSet &pointset, bool bStorePointsOutsideMesh)
    : m_pointset(pointset), m_bStorePointsOutsideMesh(bStorePointsOutsideMesh), m_pCurrentPosition(0) {}

void CMeshSampler::CPointsetGenerator::OnNewSourcePoint(const geo::IPoint &point) { m_pCurrentPosition = &point; }
...
void CMeshSampler::CPointsetGenerator::AddPoint(double dZ, const std::set<const geo::IElement *> &stElements) {
  ...
  m_pointset.PushBack(geo::CElementPoint(stElements, m_pCurrentPosition->X(), m_pCurrentPosition->Y(), dZ),
                      std::vector<double>());
}

bool CMeshSampler::CPointsetGenerator::AddPointsOutsideMesh() const { return m_bStorePointsOutsideMesh; }

///// CPillarMapGenerator
CMeshSampler::CPillarMapGenerator::CPillarMapGenerator(CPillarMap &pillarmap)
    : m_pillarmap(pillarmap), m_pCurrentPosition(0) {}
