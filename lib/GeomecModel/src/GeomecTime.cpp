...
CGeomecTime::CGeomecTime(int nYear, int nMonth)
: m_datetime(QDate(nYear, nMonth, 1))
{
...
CGeomecTime::CGeomecTime(const CGeomecTime& rhs)
...
CGeomecTime::operator QDateTime() const
{
  return m_datetime;
}
...
int CGeomecTime::DaysTo(const CGeomecTime& rhs) const
{
  return m_datetime.daysTo(rhs.m_datetime);
}
...
int CGeomecTime::GetHour() const
{
  return m_datetime.time().hour();
}

int CGeomecTime::GetMinute() const
{
  return m_datetime.time().minute();
}

int CGeomecTime::GetSecond() const
{
  return m_datetime.time().second();
}

void CGeomecTime::Set(int nYear, int nMonth)
{
  m_datetime.setDate(QDate(nYear, nMonth, 1));
}
...
void CGeomecTime::SetOneMonthEarlier()
{
  m_datetime = m_datetime.addMonths(-1);
}

void CGeomecTime::SetOneMonthLater()
{
  m_datetime = m_datetime.addMonths(1);
}

QString CGeomecTime::Month() const
{
  return QDate::longMonthName(m_datetime.date().month());
}
