...
CWellPath::CWellPath(CFemAppModel &model) : CColorNode(model) {
  IModelObject::create();
  Init();
}

CWellPath::CWellPath(const QString &sName, CFemAppModel &model) : CColorNode(sName, model) {
  IModelObject::create();
  Init();
...
CWellPath::CWellPath(const QString &sName, CFemAppModel &model, const geo::CPoint &BasePoint)
    : CColorNode(sName, model), m_VerticalPointDef(BasePoint) {
  IModelObject::create();
  Init();
...
CWellPath::CWellPath(const CWellPath &well_path, CFemAppModel &model)
    : CColorNode(well_path.Name().toStdString().c_str(), model), CWellPathBase(well_path),
...
CWellPath::CWellPath(const CWellPath &WellPath)
    : CColorNode(WellPath), CWellPathBase(WellPath), m_pIntermediate(WellPath.Intermediate()) {
...
const well::CWellPointList &CWellPath::DStaborPoints() const { return GetWellPointList(LST_PT_DSTABOR); }
...
void CWellPath::UpdateFormationSections() {
  ...
  if (DefPointList().First() && DefPointList().Last()) {
    ...
    well::CWellSection *pSection =
        new well::CWellSection(*this, DefPointList().First()->TMD().Value(), DefPointList().Last()->TMD().Value());
  ...
  */
  ...
  while (top && bottom) {
    ...
    if (!mid.Formation()) { // this only happens when there is a void in the model caused by a fault made up from two
                            // surfaces
      top = bottom;
      ++it;
      if (it != lst.end())
...
CWellPath::TColor CWellPath::Color() const { return CColorNode::Color(); }
...
void CWellPath::InitVerticalWell(bool bResetGlobalTVD) {
  ...
  InitVertical(first->X(), first->Y(), bResetGlobalTVD ? first->Z() : GlobalTVD().Value(), 0, last->Z());
...
bool CWellPath::ShowDsbAnalysisPointsInScene() { return m_bShowDsbPointsInScene; }

geo::CPoint CWellPath::MaxPoint() const { return DefPointList().MaxPoint(); }

geo::CPoint CWellPath::MinPoint() const { return DefPointList().MinPoint(); }
...
int CWellPath::DisplayListSize() const { return 1; }

const geo::IObject &CWellPath::DisplayList(int /*nIndex*/) const { return m_display_list; }

geo::CPtrArray<geo::IObject> &CWellPath::DisplayList() { return m_display_list; }

unsigned int CWellPath::IconId() const { return IDI_WELLPATH; }

unsigned int CWellPath::TypeId() const { return IDT_TREE_WELL_PATHS; }

QString CWellPath::TypeName() const { return getStringTableEntry(IDS_TREE_WELL_PATHS); }
...
void CWellPath::LoadStream(TSTREAM &stream, CStreamVersion &version, TPROGRESS &progress) {
  ...
  if (bVertical)
    m_GlobalTVD.SetValue(global_depth); // wjrx mantis 2555
  ...
  if (nSize > 0 && !bVertical) {
    assert(nSize > 1);
    InitFromArray_tmd_azi_inc(global_northing, global_easting, global_depth, 0, 0, 0, tmd, azi, inc);
  }
  ...
  for (int i = 0; i < nDStaborSize; i++) {
    ...
    while (bLoadRes) {
      ...
      stream >> bLinear;
      CDsbStatusValue &rs =
          DsbResults().GetResult(*pPoint, *pDepl, (bLinear ? CAnalysisType::AT_LINEAR : CAnalysisType::AT_NONLIN));
      stream >> status;
      stream >> value;
...
void CWellPath::LoadWellLogData(TSTREAM &stream, CStreamVersion &version, TPROGRESS &progress) {
  ...
  for (int k = 0; k < nMapSize; k++) {
    ...
    stream >> correction;
    m_mpCorrectAllStages.insert(
        std::map<CFormationSection *, bool>::value_type((CFormationSection *)(*form_it), correction != 0));
    progress.Step();
  }
}
...
void CWellPath::SaveStream(TSTREAM &stream, TPROGRESS &progress) {
  ...
  stream << CWellPathBase::DefPointList()
                .NrOfPoints(); // from base class, does not trigger intersection calculation for vertical wellpaths
  // Save all wellpoints
  ...
  for (int i = 0; i < fist_list.NrOfPoints(); i++)
    stream << fist_list.At(i)->TMD().Value();
...
bool CWellPath::Empty() const { return DefPointList().NrOfPoints() == 0; }

/*virtual*/ bool CWellPath::IsVertical() const { return !m_VerticalPointDef.Empty(); }
...
void CWellPath::GetIntersectionPointsWithPlane(geo::CPtrArray<CGeoWellPoint> & /*points*/,
                                               const geo::CPlane & /*plane*/) const {
  // TODO htg//
}

CDstaborResultMap &CWellPath::DsbResults() { return *m_pDsbResults; }

const CDstaborResultMap &CWellPath::DsbResults() const { return *m_pDsbResults; }

well::CWellDateList &CWellPath::FistDates() { return m_FistDates; }

const well::CWellDateList &CWellPath::FistDates() const { return m_FistDates; }

const well::CWellPointList &CWellPath::FistExportPoints() const { return GetWellPointList(LST_PT_FIST_EXP); }

well::CWellPointList &CWellPath::FistExportPoints() { return GetWellPointList(LST_PT_FIST_EXP); }

void CWellPath::MudGradRefDepth(double newval) { m_dMudGradRefDepth = newval; }

double CWellPath::MudGradRefDepth() const { return m_dMudGradRefDepth; }
...
double CWellPath::DsbLiquidFluid() const { return m_DsbLiquidFluid; }

void CWellPath::DsbLiquidFluid(double newval) { m_DsbLiquidFluid = newval; }

double CWellPath::DsbFluidLossCoefficient() const { return m_DsbFluidLossCoefficient; }

void CWellPath::DsbFluidLossCoefficient(double newval) { m_DsbFluidLossCoefficient = newval; }
...
bool CWellPath::hasZoomInModel() const { return (m_pZoomInModel != 0); }
...
void CWellPath::setNewModelFunction(bool (CNewModel::*newModel)(bool)) { m_newModelFunction = newModel; }

CNewModel &CWellPath::getNewModel() { return m_newModel; }

void CWellPath::DestroyZoomInModel() {
  if (m_pZoomInModel) {
    m_pZoomInModel->DestroyZoomInModel();
    if (m_pZoomInModel) // this one is 0, but I leave it in, so that we can look at this later. Ideally, we use the
                        // facade
...
void CWellPath::GetGeomecStrainValues(well::CWellPointList &list, well::CWellPointValueMap &map,
                                      const CDepletionStage &stage, const CAnalysisType &antype) const {
  map.ClearMap();
  const CModelBase &model = (const CModelBase &)(Model());
  ...
  while (it_list != list.end()) {
    CStrainTensor geom_eps(result_tree.TotalStrain()
                               .Components()
                               .Component(ITensorGroup::CComponentComposite::TC_XX, nMode)
                               .ScalarData()
                               .ValuePoint(**it_list)
                               .Value(),
                           result_tree.TotalStrain()
                               .Components()
                               .Component(ITensorGroup::CComponentComposite::TC_YY, nMode)
                               .ScalarData()
                               .ValuePoint(**it_list)
                               .Value(),
                           result_tree.TotalStrain()
                               .Components()
                               .Component(ITensorGroup::CComponentComposite::TC_ZZ, nMode)
                               .ScalarData()
                               .ValuePoint(**it_list)
                               .Value(),
                           result_tree.TotalStrain()
                               .Components()
                               .Component(ITensorGroup::CComponentComposite::TC_XY, nMode)
                               .ScalarData()
                               .ValuePoint(**it_list)
                               .Value(),
                           result_tree.TotalStrain()
                               .Components()
                               .Component(ITensorGroup::CComponentComposite::TC_YZ, nMode)
                               .ScalarData()
                               .ValuePoint(**it_list)
                               .Value(),
                           result_tree.TotalStrain()
                               .Components()
                               .Component(ITensorGroup::CComponentComposite::TC_ZX, nMode)
                               .ScalarData()
                               .ValuePoint(**it_list)
                               .Value());
    ...
    rot_mat.Value(0, 0, perp1.X());
    rot_mat.Value(0, 1, perp2.X());
    rot_mat.Value(0, 2, direction.X());
    rot_mat.Value(1, 0, perp1.Y());
    rot_mat.Value(1, 1, perp2.Y());
    rot_mat.Value(1, 2, direction.Y());
    rot_mat.Value(2, 0, perp1.Z());
    rot_mat.Value(2, 1, perp2.Z());
    rot_mat.Value(2, 2, direction.Z());
...
const CFormationSection *CWellPath::FormationSection(const CGeoWellPoint &pt) const {
  ...
  if (formation_list.NrOfSections() == 0)
    return 0;
...
void CWellPath::ToggleDrawWellInfo() { DrawWellInfo(!DrawWellInfo()); }
...
CWellPath::IIntermediate *CWellPath::Intermediate() const { return m_pIntermediate; }
...
void CWellPathEntry::ShowAllWellPathInfo() { AllWellPathInfo(true); }

void CWellPathEntry::HideAllWellPathInfo() { AllWellPathInfo(false); }
...
bool CWellPathEntry::CanExport() const { return !EntryNodes().empty(); }
...
bool CWellPathEntry::ImportFile(const QString &strFileName, IProgressBase &dlg, CModelBase &model,
...
CDstaborWellPoint::~CDstaborWellPoint() { ((CWellPath *)(&WellPath()))->DsbResults().RemoveResult(*this); }

CDstaborWellPoint::CDstaborWellPoint(const CWellPath &wellpath, const double &tmd) : CGeoWellPoint(wellpath, tmd) {}
//***********************************************************************************************************

CGeoWellPoint::CGeoWellPoint(const CWellPath &wellpath, const double &tmd)
    : well::CWellPoint(wellpath, tmd), m_pFormation(0), m_bInit(false),
...
CGeoWellPoint::CGeoWellPoint(CModelBase &model, const well::CWellPoint &wellpoint)
    : well::CWellPoint(wellpoint), m_pFormation(0), m_bInit(false), m_model(&model) {}
...
//*********************************************  CResultMap ******************************************
CWellPointResultKey::CWellPointResultKey(const well::CWellPoint &point, const CDepletionStage &stage,
