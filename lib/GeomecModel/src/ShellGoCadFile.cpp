...
CShellGoCadFile::CShellSurfaceDesc::CShellSurfaceDesc(geo::CTetMeshBase& mesh, const geo::ISurface &surface, const std::string& sName, bool bSlip)
...
*/
void CShellGoCadFile::CShellSurfaceDesc::AddTetIndexPair(const geo::CTetrahedron *Tet, int nIndex, eTetOrientation orientation)
{
...
bool CShellGoCadFile::OnRead(TInputStream& stream)
{
  ...
  int i;
    ...
    {
      if(vcSideSurface[i].second->TetSurface(0).Max().Z() < vcSideSurface[nUpperHorizon].second->TetSurface(0).Max().Z())
        nUpperHorizon = i;
      if(vcSideSurface[i].second->TetSurface(0).Min().Z() > vcSideSurface[nLowerHorizon].second->TetSurface(0).Min().Z())
        nLowerHorizon = i;
    }
    ...
    CTetraSubHorizon *pUpperHorizon = CreateSubHorizon(vcSideSurface[nUpperHorizon].first, *vcSideSurface[nUpperHorizon].second, vcSideSurface[nUpperHorizon].second->Slip());
    ...
    CTetraSubHorizon *pLowerHorizon = CreateSubHorizon(vcSideSurface[nLowerHorizon].first, *vcSideSurface[nLowerHorizon].second, vcSideSurface[nUpperHorizon].second->Slip());
...
bool CShellGoCadFile::ReadTetrahedron(TInputStream& stream, const QString& /*sToken*/)
{
  ...
  if( stream.peek() == '#' ) {
    ...
    QString sKeyword;
    stream >> sKeyword;
    if( sKeyword.toUpper() != "CTETRA" ) throw CReadException("Keyword CTETRA expected.");
    stream >> sKeyword;
    ...
    for(int i = 0; i < 4; i++) {
      ...
      if( sSideName.toUpper() == "NONE" ) {
        ...
        int nFaceIndex = i;
        if(m_bOrderSwitched && i == 2) nFaceIndex = 3;
        if(m_bOrderSwitched && i == 3) nFaceIndex = 2;
...
geo::CBodyGroup& CShellGoCadFile::AddBodyToRegion(const std::string &sName)
{
  ...
  if(it == m_mpRegions.end())
  {	// entry does not exist yet. Create a tetvolume
...
//		pVolume->LinkTo(model.Mesh());
  ...
  { // entry is present, just add.
    pGroup = it->second;	
  }
  
  const geo::CTetrahedron& tet = dynamic_cast<const geo::CTetrahedron&> (m_tetmesh.m_pMesh->Element(m_nCurrentTetIndex));
  pGroup->AddBody(tet);
  return *pGroup;
}
...
void CShellGoCadFile::AddFaceToSurface(const std::string &sName, geo::CBodyGroup& group, int nFaceIndex, CShellSurfaceDesc::eTetOrientation orientation)
{
