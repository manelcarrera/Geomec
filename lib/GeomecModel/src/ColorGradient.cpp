...
CColorGradient::CColorGradient(CFemAppModel &model)
    : IColorScaleNode(model), m_bGlobal(true), m_bSystemColorGradient(false), m_bAutoExtremes(true) {
  // Used for loading
}
...
CColorGradient::CColorGradient(CFemAppModel &model, const QString &name)
    : IColorScaleNode(model), m_bGlobal(true), m_bSystemColorGradient(true), m_bAutoExtremes(true) {
  Name(name);
...
CColorGradient::CColorGradient(unsigned int uName, CFemAppModel &model, const double &firstValue,
                               const double &secondValue, TColor min_color, TColor max_color, bool bGlobal)
    : IColorScaleNode(uName, model), m_bGlobal(bGlobal), m_bSystemColorGradient(false), m_bAutoExtremes(true) {
...
CColorGradient::CColorGradient(const CColorGradient &rhs)
    : IColorScaleNode(rhs), m_mpColor(rhs.m_mpColor), m_bGlobal(rhs.m_bGlobal), m_MinMaxValue(rhs.m_MinMaxValue),
...
bool CColorGradient::operator==(const CColorGradient &rhs) const {
  ...
  return (m_mpColor == rhs.m_mpColor && m_MinMaxValue.first == rhs.MinMaxValue().first &&
          m_MinMaxValue.second == rhs.MinMaxValue().second && m_bGlobal == rhs.IsGlobal() &&
          m_bSystemColorGradient == rhs.m_bSystemColorGradient && /// wjrx mantis 2560
          m_bAutoExtremes == rhs.m_bAutoExtremes);
}
...
bool CColorGradient::AddValue(int iNumerator, int iDenominator, TColor iColor) {
  ...
  double dValue =
      (m_MinMaxValue.second - m_MinMaxValue.first) * (double(iNumerator) / double(iDenominator)) + m_MinMaxValue.first;
...
#pragma optimize("", off)
static bool Indeterminate(const double &dValue) { return dValue != dValue; }
#pragma optimize("", on)
...
CColorGradient::color_iterator CColorGradient::Begin() const { return m_mpColor.begin(); }

CColorGradient::color_iterator CColorGradient::End() const { return m_mpColor.end(); }

size_t CColorGradient::NumColors() const { return m_mpColor.size(); }
...
CColorGradient::TColor CColorGradient::Color(const TValue &value, const TMinMax &min_max) const {
  if (value.Valid() && min_max.first.Valid() && min_max.second.Valid()) {
    ...
    // Calculate factor ....
    double percentage =
        ((value.Value() - min_max.first.Value()) / (min_max.second.Value() - min_max.first.Value())) * 100.0;
...
bool CColorGradient::AutoExtremes() const { return m_bAutoExtremes; }

void CColorGradient::AutoExtremes(bool bAuto) { m_bAutoExtremes = bAuto; }

CColorGradient::TColorMap CColorGradient::colorMap() const { return m_mpColor; }
...
unsigned int CColorGradient::IconId() const { return IDI_COLOR_GRADIENT; }

// ##ModelId=3BC55D64013F
unsigned int CColorGradient::TypeId() const { return 0; }

const std::pair<double, double> &CColorGradient::MinMaxValue() const { return m_MinMaxValue; }
...
long CColorGradient::SavedItems() const { return IColorScaleNode::SavedItems() + m_mpColor.size(); }
...
void CColorGradient::LoadRegistry(const QString &strSection, const QString &strEntry) {
  ...
  // wjrx mantis 2574
  QString sDefault =
      "7 0  255   0   0 17 255 127   0 33 255 255   0 50   0 255   0 67  30 144 255 83  76   0 130 100 246  0 197";
