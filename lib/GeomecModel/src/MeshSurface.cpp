...
int CMeshSurface::FaceSize() const { return m_vcFaces.size(); }
...
int CMeshSurface::PointSize() const { return m_vcPoints.size(); }
...
void CMeshSurface::AddFace(const geo::IFace &face) {
  ...
  for (i = 0; i < face.NrOfPoints(); ++i) {
    std::pair<TPointMap::iterator, bool> prInsert =
        m_mpPoints.insert(TPointMap::value_type(&face.Point(i), std::vector<const geo::IFace *>()));
