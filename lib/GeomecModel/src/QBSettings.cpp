...
CQBSettings::CQBSettings(CModelBase &model)
: CStorageNode(model),
  m_fpLog(0),
  m_Model(model),
  m_QBPointSet(0)
...
QString CQBSettings::ListObjectName() const
{
  return "dP";
}
...
// ***************************************************************
CQBInputWriter::CQBInputWriter(CQBSettings &settings)
: m_QBSettings(settings),
  m_fp(0),
  m_strError("")
{
}
...
{
  return 2 + m_QBSettings.PointSet()->PointSize();
}
