...
CInterfaceElementMaterial::CInterfaceElementMaterial(const ftn_double_t &cohesion, const ftn_double_t &friction,
                                                     const ftn_double_t &dstiffnormal, const ftn_double_t &dstiffshear,
                                                     CHorizonBase::TSlipType slip_type)
    : m_cohesion(cohesion), m_friction(friction), m_dstiffnormal(dstiffnormal), m_dstiffshear(dstiffshear),
...
bool CInterfaceElementMaterial::operator<(const dia::IMaterial &rhs) const {
  ...
  if (mat) {
    if (m_slip_type < mat->m_slip_type)
      return true;
    if (m_slip_type > mat->m_slip_type)
      return false;

    if (m_slip_type == CHorizonBase::USER) {
      if (m_cohesion < mat->m_cohesion)
        return true;
      else if (m_cohesion > mat->m_cohesion)
        return false;

      if (m_friction < mat->m_friction)
        return true;
      else if (m_friction > mat->m_friction)
        return false;

      if (m_dstiffnormal < mat->m_dstiffnormal)
        return true;
      else if (m_dstiffnormal > mat->m_dstiffnormal)
        return false;

      if (m_dstiffshear < mat->m_dstiffshear)
        return true;
    }
...
int CInterfaceElementMaterial::Type() const { return 103; }

bool CInterfaceElementMaterial::isInterfaceElementMaterial() const { return true; }
