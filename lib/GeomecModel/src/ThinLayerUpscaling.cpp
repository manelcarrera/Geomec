...
CThinLayerUpscalingNode::CThinLayerUpscalingNode(CFemAppModel &model) : IUpscalingNodeBase(model) {}
...
CThinLayerUpscalingNode::CThinLayerUpscalingNode(const CThinLayerUpscalingNode &rhs) : IUpscalingNodeBase(rhs) {}

CThinLayerUpscalingNode::~CThinLayerUpscalingNode() {}

int CThinLayerUpscalingNode::FactoryType() const { return FACTORYTYPE_THINLAYERUPSCALING; }
...
void CThinLayerUpscalingNode::Execute(IProgressBase &prog) {
  ...
  try {
              ...
              if (IsTarget(*pBody)) {
                ...
                if (di > 0) {
                  ...
                  if (stage.Initial()) {
                    ...
                    const CFFMaterial &ffMaterial = formation.Material(stage).Material(*pBody);
                    ...
                    if (ffMaterial.IsParameter(IDT_VALUETYPE_BULKSTIFFNESS) &&
                      ...
                      double bulkModulus = ffMaterial.ParameterValue(IDT_VALUETYPE_BULKSTIFFNESS);
                      double grainModulus = ffMaterial.ParameterValue(IDT_VALUETYPE_GRAINSTIFFNESS);
    ...
    int nValueSetSize = nStartStages * sizeof(TUpscaledMatParams) / sizeof(double) +
                        nStages * sizeof(TUpscaledLoadParams) / sizeof(double) - 2;
    ...
    for (itds = deplentry.begin(); itds != deplentry.end(); ++itds) {
      ...
      if (stage.IsPhaseStartStage()) {
        ...
        t = QString("%1_D%2").arg(s).arg(idxStage);
        pPointset->NodalValueSet(++p).LinkTo(
            factory->BuildValueType(*pPointset, IDT_VALUETYPE_THERM_LIN_EXP_NORM, t)->Component());
        s = getStringTableEntry(IDS_VALUENAME_THERM_LIN_EXP_LAT);
        t = QString("%1_D%2").arg(s).arg(idxStage);
        pPointset->NodalValueSet(++p).LinkTo(
            factory->BuildValueType(*pPointset, IDT_VALUETYPE_THERM_LIN_EXP_LAT, t)->Component());
      }
      ...
      if (!stage.Initial()) {
        ...
        t = QString("%1_D%2").arg(s).arg(idxStage);
        pPointset->NodalValueSet(++p).LinkTo(
            factory->BuildValueType(*pPointset, IDT_VALUETYPE_NRMSTRAIN, t)->Component());

        s = getStringTableEntry(IDS_VALUENAME_LATSTRAIN);
        t = QString("%1_D%2").arg(s).arg(idxStage);
        pPointset->NodalValueSet(++p).LinkTo(
            factory->BuildValueType(*pPointset, IDT_VALUETYPE_LATSTRAIN, t)->Component());
      }
    }
...
geo::CValue CThinLayerUpscalingNode::GetLoadValue(const geo::IBody &body, const geo::IPoint &pt,
...
bool CThinLayerUpscalingNode::CanExecute() const {
  const TFormationBaseEntry *formationBaseEntry =
      dynamic_cast<const TFormationBaseEntry *>(Model().GraphEntry(MD_BASE_FORMATION));
  const TFormationBaseEntry::TNodeSet formations = formationBaseEntry->EntryNodes();
  ...
  for (CDepletionStageEntry::const_iterator depletionStage = depletionStageEntry.begin();
       (depletionStage != depletionStageEntry.end()) && materialAssigned; ++depletionStage) {
    for (TFormationBaseEntry::TNodeSet::const_iterator formation = formations.begin();
         (formation != formations.end()) && materialAssigned; ++formation) {
      const CMaterialServer &materialServer = (*formation)->Material(*depletionStage);
  ...
  return !fractureApertureModel && materialAssigned && FormationsBranch().HasFormations() &&
         TargetPointsetBranch().HasPointset();
}
