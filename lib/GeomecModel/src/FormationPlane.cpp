...
CFormationPlane::plane_face::plane_face(const CPlaneSurface &surface, int p1, int p2, int p3, int p4)
    : m_surface(surface) {
  m_pointindices[0] = p1;
  m_pointindices[1] = p2;
  m_pointindices[2] = p3;
...
void CFormationPlane::plane_face::Point(const int /*nIndex*/, const geo::IPoint & /*pt*/) { assert(false); }

int CFormationPlane::plane_face::NrOfPoints() const { return 4; }
...
geo::CValue CFormationPlane::plane_face::InterpolateValue(const geo::IPoint & /*point*/,
...
void CFormationPlane::CPlaneSurface::InsertFace(const std::vector<int> &pointindices, const geo::IFace *pFace) {
  ...
  for (int i = 0; i < 4; i++) {
    std::pair<TPointToFaceMap::iterator, bool> prInsert =
        m_mpPointToFace.insert(std::make_pair(pointindices[i], std::vector<const geo::IFace *>()));
...
int CFormationPlane::CPlaneSurface::FaceSize() const { return m_vcFaces.size(); }
...
int CFormationPlane::CPlaneSurface::PointSize() const { return m_vcPoints.size(); }
  ...
  for (int i = nStart; i < m_formation.Volume(0).Volume().ElementSize(); i += m_formation.Elements()) {
    ...
    case FP_MIDDLE:
      if (div(m_formation.Elements(), 2).rem != 0) {
        ...
        points[0] =
            geo::CPoint(face1->Point(0).X(), face1->Point(0).Y(), (face1->Point(0).Z() + face2->Point(1).Z()) / 2);
        points[1] =
            geo::CPoint(face1->Point(1).X(), face1->Point(1).Y(), (face1->Point(1).Z() + face2->Point(0).Z()) / 2),
        points[2] =
            geo::CPoint(face1->Point(2).X(), face1->Point(2).Y(), (face1->Point(2).Z() + face2->Point(3).Z()) / 2),
        points[3] =
            geo::CPoint(face1->Point(3).X(), face1->Point(3).Y(), (face1->Point(3).Z() + face2->Point(2).Z()) / 2);
...
unsigned int CFormationPlane::IconId() const { return IDI_FORMATIONPLANE; }

unsigned int CFormationPlane::TypeId() const { return IDT_FORMATIONPLANE; }

QString CFormationPlane::TypeName() const { return getStringTableEntry(IDS_FORMATIONPLANE); }
...
int CFormationPlane::NrOfFaces() const { return m_pSurface ? m_pSurface->FaceSize() : 0; }

bool CFormationPlane::Dirty() const { return NrOfFaces() == 0; }
...
CHexaFormation &CFormationPlane::Formation() { return m_formation; }

const CHexaFormation &CFormationPlane::Formation() const { return m_formation; }

CFormationPlane::ePlaneType CFormationPlane::PlaneType() const { return m_type; }

bool CFormationPlane::CanDestroy() const { return false; }
