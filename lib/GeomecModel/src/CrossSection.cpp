...
CCrossSection::CCrossSection(CFemAppModel &model, const geo::IPoint &basePoint, const geo::IVector &normal)
    : CStorageNode("", model), m_pWellPathEntry(0), m_pModifier(0), m_pExportSurface(0), m_bDirty(true),
...
CCrossSection::CCrossSection(const QString &sName, CFemAppModel &model, const geo::IPoint &basePoint,
                             const geo::IVector &normal)
    : CStorageNode(sName, model), m_pWellPathEntry(0), m_pModifier(0), m_pExportSurface(0), m_bDirty(true),
      m_crossSectionType(VERTICAL) {
  m_pWellPathEntry = (CNewWellPathEntry *)(Model().GraphEntry(MD_NEW_WELLPATH));
  if (m_pWellPathEntry)
    LinkTo(*m_pWellPathEntry);
...
CCrossSection::CCrossSection(CFemAppModel &model, const geo::IPoint &first, const geo::IPoint &second)
    : CStorageNode("", model), m_pWellPathEntry(0), m_orientationBookmarked(ORIENTATION_NE), m_pModifier(0),
      m_pExportSurface(0), m_bDirty(true), m_crossSectionType(VERTICAL) {
  // We create a default name
  QString sName;
  sName = QString("New Cross section %1").arg(model.GraphEntry(MD_BASE_XSECTION)->GraphEntryNodes().size() + 1);
  Name(sName);
...
CCrossSection::CCrossSection(const QString &sName, CFemAppModel &model, const geo::IPoint &first,
                             const geo::IPoint &second)
    : CStorageNode(sName, model), m_pWellPathEntry(0), m_orientationBookmarked(ORIENTATION_NE), m_pModifier(0),
      m_pExportSurface(0), m_bDirty(true), m_crossSectionType(VERTICAL) {
  m_pWellPathEntry = (CNewWellPathEntry *)(Model().GraphEntry(MD_NEW_WELLPATH));
  if (m_pWellPathEntry)
    LinkTo(*m_pWellPathEntry);
...
CCrossSection::CCrossSection(CFemAppModel &model, const double &dDepth)
    : CStorageNode("", model), m_pWellPathEntry(0), m_orientationBookmarked(ORIENTATION_DEPTH), m_pModifier(0),
      m_pExportSurface(0), m_bDirty(true), m_crossSectionType(VERTICAL) {
  reParent(model.GraphEntry(MD_BASE_XSECTION));
...
CCrossSection::CCrossSection(const QString &sName, CFemAppModel &model, const double &dDepth)
    : CStorageNode(sName, model), m_pWellPathEntry(0), m_orientationBookmarked(ORIENTATION_DEPTH), m_pModifier(0),
      m_pExportSurface(0), m_bDirty(true), m_crossSectionType(VERTICAL) {
  reParent(model.GraphEntry(MD_BASE_XSECTION));
  LinkTo((static_cast<CModelBase &>(model)).Mesh());
  m_pWellPathEntry = (CNewWellPathEntry *)(Model().GraphEntry(MD_NEW_WELLPATH));
...
CCrossSection::CCrossSection(const QString &sName, CFemAppModel &model)
    : CStorageNode(sName, model), m_pWellPathEntry(0), m_orientationBookmarked(ORIENTATION_ANY), m_pModifier(0),
      m_pExportSurface(0), m_bDirty(true), m_crossSectionType(VERTICAL) {
  reParent(model.GraphEntry(MD_BASE_XSECTION));
  CModelBase *pModel = dynamic_cast<CModelBase *>(&model);
  LinkTo(pModel->Mesh());
...
CCrossSection::CCrossSection(CFemAppModel &model)
    : CStorageNode("", model),
      // m_bXsecValid(false),
      m_pWellPathEntry(0), m_orientationBookmarked(ORIENTATION_ANY), m_pModifier(0), m_pExportSurface(0),
      m_bDirty(true), m_crossSectionType(VERTICAL) {
#if 0
  reParent(model.GraphEntry(MD_BASE_XSECTION));
  CModelBase *pModel = dynamic_cast<CModelBase*>(&model);
  LinkTo(pModel->Mesh());
...
CCrossSection::CCrossSection(const CCrossSection &rhs)
    : CStorageNode(rhs),
      // m_bXsecValid(false),
      m_pWellPathEntry(rhs.m_pWellPathEntry), m_NormalBookmarked(rhs.m_NormalBookmarked),
      m_IntersectPlaneBookmarked(rhs.m_IntersectPlaneBookmarked), m_orientationBookmarked(rhs.m_orientationBookmarked),
      m_pModifier(0), m_pExportSurface(rhs.m_pExportSurface), m_bDirty(true),
...
void CCrossSection::Recalculate() {
  ...
  {
    TFormationBaseEntry::TNodeSet formations = ((TFormationBaseEntry*)
  model.GraphEntry(MD_BASE_FORMATION))->EntryNodes(); for(TFormationBaseEntry::TNodeSet::iterator iter =
  formations.begin(); iter != formations.end(); iter++)
    {
      ...
      else //cement and steel
      {
        ...
        Casing3DFormation* f3d = new Casing3DFormation( formation,
  (geo::CBodyGroup&)model_casing->Mesh().CementElements() ); //FIXME: it nedds to be tracked and deleted
        //SteelElements()
...
void CCrossSection::SetBookmarked(const geo::IPoint &basePoint, const geo::IVector &normal, OrientationType orientation,
                                  bool bSendModified) {
...
void CCrossSection::SetScreen(const geo::IPoint &basePoint, const geo::IVector &normal, OrientationType orientation,
                              bool bSendModified) {
  m_NormalScreen = normal;
  m_IntersectPlaneScreen = geo::CPlane(basePoint, normal);
...
void CCrossSection::LoadStream(TSTREAM &stream, CStreamVersion &version, TPROGRESS &progress) {
  ...
  if (CStreamVersion(4, 1, 18) < version) {
    ...
    orientation = (iHorizontal != 0) ? ORIENTATION_DEPTH : ORIENTATION_NE;
...
void CCrossSection::SaveStream(TSTREAM &stream, TPROGRESS &progress) {
  ...
  if (!calculateTwoArbitraryPointsInPlane(first, second, m_IntersectPlaneBookmarked,
                                          dynamic_cast<CModelBase &>(Model()))) {
...
bool CCrossSection::CanFlip() const { return (!m_NormalScreen.Empty()); }
...
CCrossSection::OrientationType CCrossSection::Orientation() const { return m_orientationBookmarked; }

bool CCrossSection::Horizontal() const { return m_orientationBookmarked == ORIENTATION_DEPTH; }

long CCrossSection::SavedItems() const { return CStorageNode::SavedItems() + 1; }
...
unsigned int CCrossSection::TypeId() const { return 0; }
...
geo::CPoint CCrossSection::First() const { throw std::logic_error("The method or operation is not implemented."); }

geo::CPoint CCrossSection::Second() const { throw std::logic_error("The method or operation is not implemented."); }

//***************************************************************************************
//************	CCSObject implementation	*****************************************
CCrossSection::CCSObject::CCSObject(CCrossSection &cross_section, COpenGLNode &base_node)
    : COpenGLNode(cross_section.Name().toStdString().c_str(), cross_section.Model()), m_cross_section(cross_section),
      m_base_node(base_node) {}

const CCrossSection &CCrossSection::CCSObject::CrossSection() const { return m_cross_section; }

CCrossSection &CCrossSection::CCSObject::CrossSection() { return m_cross_section; }

unsigned int CCrossSection::CCSObject::IconId() const { return m_base_node.IconId(); }

unsigned int CCrossSection::CCSObject::TypeId() const { return m_base_node.TypeId(); }

QString CCrossSection::CCSObject::TypeName() const { return m_base_node.TypeName(); }

const QString &CCrossSection::CCSObject::Name() const { return m_base_node.Name(); }

CCrossSection::CCSObject::TColor CCrossSection::CCSObject::Color() const { return m_base_node.Color(); }

bool CCrossSection::CCSObject::Less(const CGraphNode &node) const { return m_base_node.Less(node); }
...
const C3DFormation &CCrossSection::CCSFormation::BaseObject() const { return m_BaseFormation; }

C3DFormation &CCrossSection::CCSFormation::BaseObject() { return m_BaseFormation; }
...
CCrossSection::CCSWellPath::~CCSWellPath() { m_DisplayList.ClearAndDelete(); }
...
int CCrossSection::CCSWellPath::DisplayListSize() const { return 1; }
...
const CNewWellPath &CCrossSection::CCSWellPath::WellPath() const { return m_WellPath; }

CNewWellPath &CCrossSection::CCSWellPath::WellPath() { return m_WellPath; }

const geo::IPoint &CCrossSection::basePoint() const { return m_IntersectPlaneBookmarked.BasePoint(); }

const geo::IVector &CCrossSection::normal() const { return m_NormalBookmarked; }

CCrossSection::TCrossSectionType CCrossSection::crossSectionType() const { return m_crossSectionType; }

void CCrossSection::crossSectionType(TCrossSectionType crossSectionType) { m_crossSectionType = crossSectionType; }

// static

bool CCrossSection::calculateTwoArbitraryPointsInPlane(geo::IPoint &first, geo::IPoint &second,
                                                       const geo::IPlane &plane, const CModelBase &modelBase) {
