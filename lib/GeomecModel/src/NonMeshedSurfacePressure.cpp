...
CNonMeshedSurfacePressureComponent::CNonMeshedSurfacePressureComponent(const QString &sName,
                                                                       CNonMeshedSurfacePressure &pressure)
    : IValueComponentBase(sName, pressure), m_Data(*this), m_pressure(pressure) {}

CNonMeshedSurfacePressureComponent::CNonMeshedSurfacePressureComponent(const CNonMeshedSurfacePressureComponent &rhs)
    : IValueComponentBase(rhs), m_Data(*this), m_pressure(rhs.m_pressure) {}
...
const IValueDataInterfaceScalar &CNonMeshedSurfacePressureComponent::ScalarData() const { return m_Data; }

IValueDataInterfaceScalar &CNonMeshedSurfacePressureComponent::ScalarData() { return m_Data; }

const CNonMeshedSurfacePressure &CNonMeshedSurfacePressureComponent::Pressure() const { return m_pressure; }

CNonMeshedSurfacePressure &CNonMeshedSurfacePressureComponent::Pressure() { return m_pressure; }
...
QString CNonMeshedSurfacePressureComponent::ExportLabel() const { return getStringTableEntry(IDS_ET_PRESSURE); }

bool CNonMeshedSurfacePressureComponent::Empty() const { return false; }

long CNonMeshedSurfacePressureComponent::SavedItems() const { // TODO
  return 0;
}

geo::CValue CNonMeshedSurfacePressureComponent::ValuePoint(const geo::IPoint &pt, const CQuantity::UNIT unit,
...
CNonMeshedSurfacePressureComponent::ValueElement(const geo::IElement &elm, UNIT unit,
                                                 geo::IParallelInitializationCallback *cb) const {
  ...
  for (size_t i = 0; i < m_pressure.DistributedSize(); ++i) {
    IValueDomainScalar::TValueVec vcPressureValues =
        m_pressure.DistributedValue(i).Component().ScalarData().ValueElement(elm, unit, cb);
  ...
  for (int i = 0; i < elm.NrOfPoints(); ++i) {
    if (!values[i].Valid()) {
      if (m_pressure.ManualValues()) {
        values[i] =
            (elm.Point(i).Z() - m_pressure.ReferenceDepth()) * m_pressure.Gradient() + m_pressure.ReferencePressure();
        if (unit == CQuantity::FIELD_UNIT)
...
unsigned int CNonMeshedSurfacePressureComponent::IconId() const { return 0; }

unsigned int CNonMeshedSurfacePressureComponent::TypeId() const { return 0; }
...
IValueDomainScalar::TMinMax CNonMeshedSurfacePressureComponent::MinMax(IProgressBase &progressBase,
                                                                       const UNIT unit) const {
  geo::CBox box(m_pressure.Surface().Min(), m_pressure.Surface().Max());
  return ScalarData().MinMax(progressBase, box, unit);
}

IValueDomainScalar::TValue CNonMeshedSurfacePressureComponent::Average(IProgressBase &progressBase,
                                                                       const UNIT unit) const {
  geo::CBox box(m_pressure.Surface().Min(), m_pressure.Surface().Max());
...
CNonMeshedSurfacePressure::CNonMeshedSurfacePressure(CNonMeshedSurface &surface)
    : IValueComposite("Pressure", surface.Model()), m_surface(surface), m_bDistributedOnly(false), m_bManual(false),
      m_dRefPressure(0), m_dRefDepth(0), m_dGradient(0) {
...
CNonMeshedSurfacePressure::CNonMeshedSurfacePressure(const CNonMeshedSurfacePressure &rhs)
    : IValueComposite(rhs), m_surface(rhs.m_surface), m_vcDistributed(rhs.m_vcDistributed),
      m_bDistributedOnly(rhs.m_bDistributedOnly), m_bManual(rhs.m_bManual), m_dRefPressure(rhs.m_dRefPressure),
      m_dRefDepth(rhs.m_dRefDepth), m_dGradient(rhs.m_dGradient) {}
...
bool CNonMeshedSurfacePressure::operator==(const CNonMeshedSurfacePressure &rhs) const {
  return (m_vcDistributed == rhs.m_vcDistributed && m_bDistributedOnly == rhs.m_bDistributedOnly &&
          m_bManual == rhs.m_bManual && m_dRefPressure == rhs.m_dRefPressure && m_dRefDepth == rhs.m_dRefDepth &&
          m_dGradient == rhs.m_dGradient);
}

size_t CNonMeshedSurfacePressure::DistributedSize() const { return m_vcDistributed.size(); }

const TPressure &CNonMeshedSurfacePressure::DistributedValue(size_t nIndex) const { return *m_vcDistributed[nIndex]; }

bool CNonMeshedSurfacePressure::DistributedOnly() const { return m_bDistributedOnly; }

void CNonMeshedSurfacePressure::DistributedOnly(bool bDistributedOnly) { m_bDistributedOnly = bDistributedOnly; }

bool CNonMeshedSurfacePressure::ManualValues() const { return m_bManual; }

void CNonMeshedSurfacePressure::ManualValues(bool bManual) { m_bManual = bManual; }

double CNonMeshedSurfacePressure::ReferencePressure() const { return m_dRefPressure; }

double CNonMeshedSurfacePressure::ReferenceDepth() const { return m_dRefDepth; }

double CNonMeshedSurfacePressure::Gradient() const { return m_dGradient; }

void CNonMeshedSurfacePressure::ReferencePressure(double dPressure) { m_dRefPressure = dPressure; }

void CNonMeshedSurfacePressure::ReferenceDepth(double dDepth) { m_dRefDepth = dDepth; }

void CNonMeshedSurfacePressure::Gradient(double dGradient) { m_dGradient = dGradient; }
...
long CNonMeshedSurfacePressure::SavedItems() const { return 0; }
...
const CNonMeshedSurface &CNonMeshedSurfacePressure::Surface() const { return m_surface; }

CNonMeshedSurface &CNonMeshedSurfacePressure::Surface() { return m_surface; }

QString CNonMeshedSurfacePressure::ExportLabel(int /*nComponent*/) const { // TOTO
  return QString();
}
...
unsigned int CNonMeshedSurfacePressure::IconId() const { return IDI_PRESSURES; }

unsigned int CNonMeshedSurfacePressure::TypeId() const { return 0; }
