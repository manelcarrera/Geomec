...
CCamClayUpscalingNode::CCamClayUpscalingNode(CFemAppModel &model) : IUpscalingNodeBase(model) {}
...
CCamClayUpscalingNode::CCamClayUpscalingNode(const CCamClayUpscalingNode &rhs) : IUpscalingNodeBase(rhs) {
  m_formationFilter = new CCamClayFormationFilter();
  FormationsBranch().FormationFilter(m_formationFilter);
}

CCamClayUpscalingNode::~CCamClayUpscalingNode() { delete m_formationFilter; }

int CCamClayUpscalingNode::FactoryType() const { return FACTORYTYPE_CAMCLAYUPSCALING; }
...
void CCamClayUpscalingNode::Execute(IProgressBase &prog) {
  ...
  try {
    ...
    for (dsit = deplentry.begin(); dsit != deplentry.end(); ++dsit) {
      ...
      if (stage.IsPhaseStartStage()) {
        ...
        for (it = TargetPointsetBranch().begin(); it != TargetPointsetBranch().end(); ++it) {
            ...
            if (!ptPrev.Empty()) {
              ...
              if (IsTarget(*pBody)) {
                ...
                if (dThickness > 0) {
                  ...
                  calculatedValues.Cei = ElasticCompressibility(
                      materialModelParameters.Nui,
                      materialModelParameters.Ei); //( (1 + Nui)*(1 - 2 * Nui) ) / ( Ei*(1 - Nui) );
                  ...
                  v15 += ((1 + materialModelParameters.Nui) / (1 - materialModelParameters.Nui)) *
                         materialModelParameters.Betai * dThickness;
              ...
              if (IsTarget(*pBody) && IsCCBody(*pBody, stage)) {
                ...
                if (dThickness > 0) {
                  ...
                  calculatedValues.Cp1i = PlasticCompressibility1(
                      materialModelParameters.n0i, materialModelParameters.Lam1i, materialModelParameters.Pc1i,
                      calculatedValues.Cei, materialModelParameters.alphai,
                      materialModelParameters.Phii); // ( ( (1 - n0i) * Lam1i * psii ) / Pc1i ) + Cei; // {45}
                  calculatedValues.Cp2i = PlasticCompressibility2(
                      materialModelParameters.n0i, materialModelParameters.Lam1i, materialModelParameters.Lam2i,
                      materialModelParameters.Pc2i, calculatedValues.Cei, materialModelParameters.alphai,
                      materialModelParameters
                          .Phii); //( ( ((1 - n0i) * (Lam1i + Lam2i)) * psii ) / Pc2i ) + Cei; // {47}
                  ...
                  v14 += ((1 - 1.3 * sqrt(materialModelParameters.alphai) *
                                   pow(sin(DEG2RAD(materialModelParameters.Phii)), 2))) *
                         dThickness;
          ...
          matParams.Lam1 =
              ((matParams.Cp1 - matParams.Ce) * matParams.Pc1) / ((1 - matParams.n0) * calculatedValues.psib); // {52}
          matParams.Lam2 =
              ((matParams.Cp2 - matParams.Cp1) * matParams.Pc2) / ((1 - matParams.n0) * calculatedValues.psib); // {52}
            ...
            if (IsTarget(*pBody)) {
              ...
              if (di > 0) {
                ...
                if (IsCCBody(*pBody, stage)) {
                  ...
                  calculatedValues.Cp1i = PlasticCompressibility1(
                      materialModelParameters.n0i, materialModelParameters.Lam1i, materialModelParameters.Pc1i,
                      calculatedValues.Cei, materialModelParameters.alphai, materialModelParameters.Phii);
                  calculatedValues.Cp2i = PlasticCompressibility2(
                      materialModelParameters.n0i, materialModelParameters.Lam1i, materialModelParameters.Lam2i,
                      materialModelParameters.Pc2i, calculatedValues.Cei, materialModelParameters.alphai,
                      materialModelParameters.Phii);
                ...
                if (stage.Initial()) {
                  ...
                  geo::CValue prevtemperature =
                      GetLoadValue(*pBody, ptMid, formation.EffectiveTemperature(stage.Previous()));
                  ...
                  t1 += ((1 + materialModelParameters.Nui) / (1 - materialModelParameters.Nui)) *
                        materialModelParameters.Betai * deltaTi;
    ...
    std::pair<QString, QString> uniqueNames = psentry.createUniqueNames(
        ...
        DEFAULT_3D_POINT_SET_NAME.arg(Name()) + DEFAULT_3D_POINT_SET_NUMBER, DEFAULT_INITIAL_NUMBER);
    ...
    int nValueSetSize = nStartStages * sizeof(TUpscaledParams) / sizeof(double) +
                        nStages * sizeof(TUpscaledLoadParams) / sizeof(double);
    ...
    for (itds = deplentry.begin(); itds != deplentry.end(); ++itds) {
      ...
      if (stage.IsPhaseStartStage()) {
        ...
        pPointset->NodalValueSet(++p).LinkTo(
            factory->BuildValueType(*pPointset, IDT_VALUETYPE_ELASCOMPRES, t)->Component());
        s = getStringTableEntry(IDS_VALUENAME_POISSONS_RATIO);
        t = QString("%1_D%2").arg(s).arg(idxStage);
        pPointset->NodalValueSet(++p).LinkTo(
            factory->BuildValueType(*pPointset, IDT_VALUETYPE_POISSONS_RATIO, t)->Component());
        s = getStringTableEntry(IDS_VALUENAME_YOUNGS_MODULUS);
        t = QString("%1_D%2").arg(s).arg(idxStage);
        pPointset->NodalValueSet(++p).LinkTo(
            factory->BuildValueType(*pPointset, IDT_VALUETYPE_YOUNGS_MODULUS, t)->Component());
        s = getStringTableEntry(IDS_VALUENAME_POROSITY);
        t = QString("%1_D%2").arg(s).arg(idxStage);
        pPointset->NodalValueSet(++p).LinkTo(
            factory->BuildValueType(*pPointset, IDT_VALUETYPE_POROSITY, t)->Component());
        s = getStringTableEntry(IDS_VALUENAME_PLASCOMPRES);
        t = QString("%1_D%2").arg(s).arg(idxStage);
        pPointset->NodalValueSet(++p).LinkTo(
            factory->BuildValueType(*pPointset, IDT_VALUETYPE_PLASCOMPRES, t)->Component());
        s = getStringTableEntry(IDS_VALUENAME_PLASCOMPRES);
        ...
        t = QString("%1_D%2").arg(s).arg(idxStage);
        pPointset->NodalValueSet(++p).LinkTo(
            factory->BuildValueType(*pPointset, IDT_VALUETYPE_PRECONSOLIDATION, t)->Component());
        s = getStringTableEntry(IDS_VALUENAME_SEC_PRECON);
        t = QString("%1_D%2").arg(s).arg(idxStage);
        pPointset->NodalValueSet(++p).LinkTo(
            factory->BuildValueType(*pPointset, IDT_VALUETYPE_SEC_PRECON, t)->Component());
        s = getStringTableEntry(IDS_VALUENAME_CAPSHAPE);
        t = QString("%1_D%2").arg(s).arg(idxStage);
        pPointset->NodalValueSet(++p).LinkTo(
            factory->BuildValueType(*pPointset, IDT_VALUETYPE_CAPSHAPE, t)->Component());
        s = getStringTableEntry(IDS_VALUENAME_TENSILE_STRETCH);
        t = QString("%1_D%2").arg(s).arg(idxStage);
        pPointset->NodalValueSet(++p).LinkTo(
            factory->BuildValueType(*pPointset, IDT_VALUETYPE_TENSILE_STRETCH, t)->Component());
        s = getStringTableEntry(IDS_VALUENAME_LATRATIO_MAX);
        t = QString("%1_D%2").arg(s).arg(idxStage);
        pPointset->NodalValueSet(++p).LinkTo(
            factory->BuildValueType(*pPointset, IDT_VALUETYPE_LATRATIO_MAX, t)->Component());
        s = getStringTableEntry(IDS_VALUENAME_LATRATIO_MIN);
        t = QString("%1_D%2").arg(s).arg(idxStage);
        pPointset->NodalValueSet(++p).LinkTo(
            factory->BuildValueType(*pPointset, IDT_VALUETYPE_LATRATIO_MIN, t)->Component());
        s = getStringTableEntry(IDS_VALUENAME_AZIMUTH);
        t = QString("%1_D%2").arg(s).arg(idxStage);
        pPointset->NodalValueSet(++p).LinkTo(
            factory->BuildValueType(*pPointset, IDT_VALUETYPE_AZIMUTH, t)->Component());
        s = getStringTableEntry(IDS_VALUENAME_INITFRICTION);
        t = QString("%1_D%2").arg(s).arg(idxStage);
        pPointset->NodalValueSet(++p).LinkTo(
            factory->BuildValueType(*pPointset, IDT_VALUETYPE_INITFRICTION, t)->Component());
        s = getStringTableEntry(IDS_VALUENAME_HARDENING);
        t = QString("%1_D%2").arg(s).arg(idxStage);
        pPointset->NodalValueSet(++p).LinkTo(
            factory->BuildValueType(*pPointset, IDT_VALUETYPE_HARDENING, t)->Component());
        s = getStringTableEntry(IDS_VALUENAME_SEC_HARDENING);
        t = QString("%1_D%2").arg(s).arg(idxStage);
        pPointset->NodalValueSet(++p).LinkTo(
            factory->BuildValueType(*pPointset, IDT_VALUETYPE_SEC_HARDENING, t)->Component());
        ...
        t = QString("%1_D%2").arg(s).arg(idxStage);
        pPointset->NodalValueSet(++p).LinkTo(
            factory->BuildValueType(*pPointset, IDT_VALUETYPE_THERMAL_EXPANSION, t)->Component());
        s = getStringTableEntry(IDS_VALUENAME_YOUNGMODULUS_TRANS);
        t = QString("%1_D%2").arg(s).arg(idxStage);
        pPointset->NodalValueSet(++p).LinkTo(
            factory->BuildValueType(*pPointset, IDT_VALUETYPE_YOUNGMODULUS_TRANS, t)->Component());
        s = getStringTableEntry(IDS_VALUENAME_YOUNGMODULUS_NORM);
        t = QString("%1_D%2").arg(s).arg(idxStage);
        pPointset->NodalValueSet(++p).LinkTo(
            factory->BuildValueType(*pPointset, IDT_VALUETYPE_YOUNGMODULUS_NORM, t)->Component());
        s = getStringTableEntry(IDS_VALUENAME_POISSONRATIO_TRANS);
        t = QString("%1_D%2").arg(s).arg(idxStage);
        pPointset->NodalValueSet(++p).LinkTo(
            factory->BuildValueType(*pPointset, IDT_VALUETYPE_POISSONRATIO_TRANS, t)->Component());
        s = getStringTableEntry(IDS_VALUENAME_POISSONRATIO_NORM);
        t = QString("%1_D%2").arg(s).arg(idxStage);
        pPointset->NodalValueSet(++p).LinkTo(
            factory->BuildValueType(*pPointset, IDT_VALUETYPE_POISSONRATIO_NORM, t)->Component());
        s = getStringTableEntry(IDS_VALUENAME_ANISOTROPIC_SHEARMODULUS);
        t = QString("%1_D%2").arg(s).arg(idxStage);
        pPointset->NodalValueSet(++p).LinkTo(
            factory->BuildValueType(*pPointset, IDT_VALUETYPE_ANISOTROPIC_SHEARMODULUS, t)->Component());
      }
      ...
      t = QString("%1_D%2").arg(s).arg(idxStage);
      pPointset->NodalValueSet(++p).LinkTo(
          factory->BuildValueType(*pPointset, IDT_VALUETYPE_TEMPERATURE, t)->Component());
    }
...
geo::CValue CCamClayUpscalingNode::GetLoadValue(const geo::IBody &body, const geo::IPoint &pt,
                                                const IValueComposite &loadcomposite) const {
...
CCamClayUpscalingNode::TUpscaledParams &CCamClayUpscalingNode::UpscaledParameters(TMatParamsMap &mpParams,
                                                                                  const CDepletionStage &stage,
                                                                                  const CPillarPos &pos) {
...
double CCamClayUpscalingNode::PlasticCompressibility1(const double &n0, const double &Lam1, const double &Pc1,
                                                      const double &Ce, const double &alpha, const double &phi) {
  double psi = 1 - (0.5 * sqrt(alpha) * pow(sin(DEG2RAD(phi)), 2)); // {40}
  return (((1 - n0) * Lam1 * psi) / Pc1) + Ce;                      // {39}
}
double CCamClayUpscalingNode::PlasticCompressibility2(const double &n0, const double &Lam1, const double &Lam2,
                                                      const double &Pc2, const double &Ce, const double &alpha,
                                                      const double &phi) {
  double psi = 1 - (0.5 * sqrt(alpha) * pow(sin(DEG2RAD(phi)), 2)); // {40}
...
double CCamClayUpscalingNode::MaterialValue(const geo::IBody &body, const CDepletionStage &stage,
...
bool CCamClayUpscalingNode::ParameterExists(const geo::IBody &body, const CDepletionStage &stage,
                                            unsigned int parameterID) {
  const CModelBase &model = static_cast<const CModelBase &>(Model());
  const CFormationBase &formation = *model.Mesh().Formation(body);
  const CFFMaterial &mat = formation.Material(stage).Material(body);
