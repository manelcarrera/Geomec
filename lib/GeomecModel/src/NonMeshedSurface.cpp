...
CNonMeshedSurface::CGeoSurface::CGeoSurface(const geo::CTriSurface &surface, const CNonMeshedSurface &parent)
    : m_vcTriangle(surface.FaceSize()), m_vcPoint(surface.PointSize()), m_parent(parent) {
  ...
  for (int i = 0; i < surface.FaceSize(); ++i) {
    ...
    for (int j = 0; j < 3; ++j) {
      TPointToFacesMap::iterator it =
          m_mpPointToFaces.insert(TPointToFacesMap::value_type(&m_vcPoint[vcNodes[j]], geo::CPtrArray<geo::IFace>()))
              .first;
...
const geo::CTriangle &CNonMeshedSurface::CGeoSurface::Face(int nIndex) const { return *m_vcTriangle[nIndex]; }

int CNonMeshedSurface::CGeoSurface::FaceSize() const { return m_vcTriangle.size(); }

const geo::CPoint &CNonMeshedSurface::CGeoSurface::Point(int nIndex) const { return m_vcPoint[nIndex]; }
...
int CNonMeshedSurface::CGeoSurface::PointSize() const { return m_vcPoint.size(); }
...
const CNonMeshedSurface &CNonMeshedSurface::CGeoSurface::Parent() const { return m_parent; }

geo::CPoint &CNonMeshedSurface::CGeoSurface::PointAt(int nIndex) { return m_vcPoint[nIndex]; }

///////

CNonMeshedSurface::CNonMeshedSurface(CFemAppModel &model)
    : CColorNode(model), m_pSurface(0), m_pGeoSurface(0), m_pParametersNode(0), m_pPressure(0) {}

CNonMeshedSurface::CNonMeshedSurface(CSurfaceBase &surface)
    : CColorNode(surface.Name(), surface.Model(), surface.Color()), m_pSurface(0), m_pGeoSurface(0),
      m_pParametersNode(0), m_pPressure(0) {
  LinkTo(surface);
...
QString CNonMeshedSurface::TypeName() const { return "Non-meshed surfaces"; }
...
long CNonMeshedSurface::SavedItems() const { return 1; }
...
CNonMeshedSurfaceEntry::CNonMeshedSurfaceEntry(CFemAppModel &model)
    : CStorageNodeEntry<CNonMeshedSurface>::CStorageNodeEntry(
          MD_BASE_NONMESHEDSURFACE, model.GraphEntry(MD_BASE_SURFACE)->IconId(), "Non-meshed surfaces", model) {}
