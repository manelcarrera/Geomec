...
: m_pNodalExportFormat(0),
  ...
  m_bCoorUnitSet(false),
  m_pElementSet(0), 
  m_pPointSet(0), 
  m_pNodalFormat(),
  m_pElementFormat(),
  m_pResultFormat(0),
  m_pWellPathInputFormat(0),
  m_bWriteHeader(true),
  m_model(model), 
  m_strExportDelimiter("\t"),
  m_created_objects(0)
{
  // Define comment
  DefineComment("*");
...
: m_pNodalExportFormat(0),
  ...
  m_model(model), 
  m_strExportDelimiter("\t"),
  m_created_objects(&created_objects)
{
  // Define comment
  DefineComment("*");
  DefineComment("#");
...
: m_pNodalExportFormat(0),
  ...
  m_pNodalFormat(),
  m_pElementFormat(),
  m_pResultFormat(&result_format),
  m_pWellPathInputFormat(0),
  m_bWriteHeader(true),
  m_model(result_format.Model()),
  m_strExportDelimiter("\t"),
  m_created_objects(0)
{
  // Define comment
...
: m_pNodalExportFormat(&format.NodalExportFormat()),
  ...
  m_created_objects(0)
{
  dynamic_cast <CElementSetExportFormat*> (
  const_cast <IElementSetExportFormat*> (
      m_pElementSetExportFormat))->sortNodeVector();
}
...
: m_pNodalExportFormat(0),
  ...
  m_pElementFormat(),
  m_pResultFormat(0),
  m_pWellPathInputFormat(&well_format),
  m_bWriteHeader(true),
  m_model((CModelBase&)well_format.NewWellPathInput()->Model()), 
  m_strExportDelimiter("\t"),
  m_created_objects(0)
{
  // Define comment
...
bool CElementSetFile::IsElementSet() const
{
  return m_pElementSet != 0;
}
...
bool CElementSetFile::IsPointSet() const
{
  return m_pPointSet != 0;
}
...
CElementSet* CElementSetFile::OnCreateElementSet(const QString& sName, CFemAppModel& model, CQuantity::UNIT coordinate_unit, CQuantity::UNIT value_unit) const
{
  return new CElementSet(sName, model, coordinate_unit, value_unit);
}

CPointSet* CElementSetFile::OnCreatePointSet(const QString& sName, CFemAppModel& model, int nValueSetSize, IPointSet::DIMENSION dim, bool bCreateValueSetForCoordinates) const
{
...
QString CElementSetFile::ReadName(TInputStream& stream)
{
  ...
  if(stream.peek() == '"') {
    ...
    if( ch != '"' ) throw CReadException("String is not terminated with \" on the same line.");
  } else {
    stream >> sRet;
  }
  return sRet;
...
bool CElementSetFile::OnElementSet(TInputStream& /*stream*/, const QString& /*sToken*/)
{
  ...
  m_pNodalFormat =
    QSharedPointer <CNodalImportFormat> (new CNodalImportFormat(*pElementSet));
...
bool CElementSetFile::OnSurface(TInputStream& /*stream*/, const QString& /*sToken*/)
{
  ...
  m_pNodalFormat = QSharedPointer <CNodalImportFormat> (
    new CNodalImportFormat(*m_pElementSet));
...
{
  if((m_pNodalFormat || m_pElementFormat) &&
    dynamic_cast<CSurfaceBase*>(m_pElementSet) == 0 )
  {
...
{
  if( m_pWellPathInputFormat ) return true;
  ...
  m_pNodalFormat =
    QSharedPointer <CNodalImportFormat> (new CNodalImportFormat(*pPointSet));
  assert(!pPointSet->Coordinates().Component(0).Defined() && 
       !pPointSet->Coordinates().Component(1).Defined() &&
...
bool CElementSetFile::OnElementTrailer(TInputStream& /*stream*/, const QString& /*sToken*/)
{
  ...
  {
    m_pElementFormat = QSharedPointer <CElementImportFormat> (
      new CElementImportFormat(*m_pNodalFormat));
...
bool CElementSetFile::OnEnd(TInputStream& /*stream*/, const QString& /*sToken*/)
{
  return false;
}
...
const CModelBase& CElementSetFile::Model() const
{
  return m_model;
}

CModelBase& CElementSetFile::Model()
{
  return m_model;
}
...
bool CElementSetFile::WriteDepStages(TOutputStream& stream)
{
  ...
  {
    dep = QString("D%1%2%3%4%5\n").arg(i).arg(m_strExportDelimiter).arg(it->Time().GetMonth()).arg(m_strExportDelimiter).arg(it->Time().GetYear());
    stream << dep;
  }
...
bool CElementSetFile::WriteTable(const IExportFormat& format, TOutputStream& stream)
{
  for(size_t i = 0; i < format.ValueTypeSize(); i++) {
    ...
    stream << '\n';
    if(format.ValueTypeSize() == i + 1) stream << '\n';
  }
  ...
  if (!format.isGVT() &&
  (format.ValueTypeSize() == 0) && (format.DataValueSize() > 0))
  {
  ...
  for (size_t s = 0; s < format.DataValueSize(); ++s)
  {
    ...
    try
    {
      ...
      stage = depletionStage.Index();
          ...
          try
          {
      const CMaterialResult& materialResult =
              dynamic_cast <const CMaterialResult&> (valueComposite);
      ...
      {
              if (((s + t) < format.DataValueSize()) && dynamic_cast<const IResultComponent *>(&format.DataValue(s + t)) &&
        (stage == static_cast <const IResultComponent&> (
                  format.DataValue(s + t)).Stage().Index()) &&
        (&valueComposite == &format.DataValue(s + t).Parent()))
              {
        stream << m_strExportDelimiter;
        WriteName(format.StringAt(0, format.getDataOffset() + s + t),
                  stream);
        processed[s + t] = true;
              }
      }
  ...
  {
  QString curr; QTextStream currStream(&curr);
...
bool CElementSetFile::OnPoints(TInputStream& /*stream*/, const QString& /*sToken*/)
{
  return true;
}
...
bool CElementSetFile::OnParseFail(TInputStream& stream, const QString& sToken)
{
  ...
  {
  throw CReadException(QString("Could not validate the column headers:\n(%1)\n\nFound '%2'.").arg(e.ErrorMessage()).arg(sToken));
  }
...
: CTokenTemplate<CElementSetFile>(instance, sToken, token),
  m_instance(instance)
{
}
