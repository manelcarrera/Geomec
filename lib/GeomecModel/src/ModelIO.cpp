...
CModelBase* LoadModel(const CModelBase::IModelLoadSave& modelLoadSave, const QString& strFileName, CAnalysisLogger& logger, bool bMain, bool bSilent)
{
  QFile *qFile = 0;
  QIODevice *pFile = 0;
  CHDF5File *h5File = 0;
  ...
  {
    QString message = QString((!archive || !archive->Exists()) ? MODEL_DOES_NOT_EXIST : MODEL_CANNOT_BE_OPENED).arg(strFileName);
      ...
      *archive >> nMajor >> nMinor >> nRevision;
      ...
      if (m_versionManager.CheckVersion(m_versionManager.FileVersion()))
      {
        ...
        *archive >> nModelType;
        ...
        {
        case HEXA_MODEL:
          pModel = bMain ? IModelLifetimeFacade::NewMain(HEXA_MODEL, logger, m_versionManager) : IModelLifetimeFacade::NewChild(HEXA_MODEL, logger, m_versionManager);
          break;
        case GOCAD_MODEL:
#ifdef TETRA_KEYFILE
          if (keyFile.Unlocked())
          {
#endif
            pModel = bMain ? IModelLifetimeFacade::NewMain(GOCAD_MODEL, logger, m_versionManager) : IModelLifetimeFacade::NewChild(GOCAD_MODEL, logger, m_versionManager);
#ifdef TETRA_KEYFILE
          }
          else
          {
        ...
        case ECLIPSE_MODEL:
          ...
          {
#endif
            pModel = bMain ? IModelLifetimeFacade::NewMain(ECLIPSE_MODEL, logger, m_versionManager) : IModelLifetimeFacade::NewChild(ECLIPSE_MODEL, logger, m_versionManager);
#ifdef ECLIPSE_KEYFILE
          }
          else
          {
...
bool SaveModel(const CModelBase::IModelLoadSave& modelLoadSave, CModelBase& model, const QString& strFileName, bool bSwitch)
{
  class CBackup
  {
  public:
  ...
  if (bNormalSave)
    guard->AttachHDF5File(0); // if we have an HDF5 file open as "database", this MAY close it, so that we can create a backup
  ...
  try
  {
    ...
    // If we are saving an HDF5 file, and we got to this point, we need to open the backup to gain access to the database
    if (bNormalSave)
    {
