...
CRotatedSystem::CRotatedSystem(double dAzimuth, const geo::IPoint &ptRotation) // azimuth in deg
    : m_dAzimuth(DEG2RAD(dAzimuth)), m_pptRotation(new geo::CPoint(ptRotation)) {}

CRotatedSystem::~CRotatedSystem() { delete m_pptRotation; }

// convert from global (x, y) to local (xi, eta) system
geo::CPoint CRotatedSystem::ToLocal(const geo::IPoint &pt) const {
  return geo::CPoint((pt.X() - m_pptRotation->X()) * cos(m_dAzimuth) + (pt.Y() - m_pptRotation->Y()) * sin(m_dAzimuth),
...
geo::CVector CRotatedSystem::ToLocal(const geo::IVector &v) const {
  return geo::CVector(v.X() * cos(m_dAzimuth) + v.Y() * sin(m_dAzimuth),
                      -v.X() * sin(m_dAzimuth) + v.Y() * cos(m_dAzimuth), v.Z());
}
...
void CRotatedSystem::ToLocalSet(geo::IPoint &pt) const {
  pt.Set((pt.X() - m_pptRotation->X()) * cos(m_dAzimuth) + (pt.Y() - m_pptRotation->Y()) * sin(m_dAzimuth),
         -(pt.X() - m_pptRotation->X()) * sin(m_dAzimuth) + (pt.Y() - m_pptRotation->Y()) * cos(m_dAzimuth), pt.Z());
}

void CRotatedSystem::ToLocalSet(geo::IVector &v) const {
  v.Set(v.X() * cos(m_dAzimuth) + v.Y() * sin(m_dAzimuth), -v.X() * sin(m_dAzimuth) + v.Y() * cos(m_dAzimuth), v.Z());
}

// convert from local (xi, eta) to global (x, y) system
geo::CPoint CRotatedSystem::ToGlobal(const geo::IPoint &pt) const {
  return geo::CPoint(pt.X() * cos(m_dAzimuth) - pt.Y() * sin(m_dAzimuth) + m_pptRotation->X(),
                     pt.X() * sin(m_dAzimuth) + pt.Y() * cos(m_dAzimuth) + m_pptRotation->Y(), pt.Z());
}
