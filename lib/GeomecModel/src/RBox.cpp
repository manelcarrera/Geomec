...
// size
T3da rw_to_oiv_SIZE(T3da size) { return T3da{size[0] / 2, size[1] / 2, size[2] / 2}; }

T3da oiv_to_rw_SIZE(T3da size) { return T3da{2 * size[0], 2 * size[1], 2 * size[2]}; }
...
const TBox RefinementBox::box() const { return TBox{min_(), max_()}; }

/////////////////////////////////////// RelativeSize /////////////////////////////////////////

bool RelativeSize::operator<(const RelativeSize &other) const {
  return m_distance < other.m_distance && m_size < other.m_size;
};
bool RelativeSize::operator<=(const RelativeSize &other) const {
  return m_distance <= other.m_distance && m_size <= other.m_size;
};
...
void RelativeSize::print() { Printer::instance()->debug("Gradient > [d:%d,s:%.2f]", m_distance, m_size); }
...
RefinementBox::RefinementBox(
    ...
    const T3da &center, const T3da &size, const T3da &rotation, GradientType_ direction, const TGradientV &profile_v,
    const QString &name, const bool &visible)

    : m_center(center), m_size(size), m_rotation(rotation), m_gradientType(direction), m_profile_v(profile_v),
...
void RefinementBox::print() {
  ...
  Printer::instance()->debug(
      "RefinementBox > n:[%s] v:[%d] c:[%.2f,%.2f,%.2f] s:[%.2f,%.2f,%.2f] r:[%.2f,%.2f,%.2f] g:%d steps:%d",
      this->Name().toStdString().c_str(), this->IsVisible(), c[0], c[0], c[0], s[0], s[0], s[0], r[0], r[0], r[0], g,
      this->Profile().size());
...
bool RefinementBox::operator==(const RefinementBox &other) const {
  bool res1 = Name() == other.Name() && IsVisible() == other.IsVisible() && Center() == other.Center() &&
              Size() == other.Size() && Rotation() == other.Rotation() && GradientType() == other.GradientType();
...
bool RefinementBox::operator!=(const RefinementBox &other) const { return !(*this == other); }
