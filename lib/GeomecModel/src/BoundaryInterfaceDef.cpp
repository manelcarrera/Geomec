...
CBoundaryInterfaceDef::CBoundaryInterfaceDef(CBoundaryBase& boundary)
  : CStorageNode(boundary.Model()),
  m_boundary(boundary),
  m_ShearQuantity(1.E7, 0.0, DOUBLE_MAX_, true, true),
  m_lambda(100),
  m_KradTop(0.001, 0.0, DOUBLE_MAX_, true, true),
  m_KradBottom(100., 0.0, DOUBLE_MAX_, true, true),
  m_KradSides(100., 0.0, DOUBLE_MAX_, true, true),
  m_KtanTop(0.001, 0.0, DOUBLE_MAX_, true, true),
  m_KtanBottom(0.001, 0.0, DOUBLE_MAX_, true, true),
  m_KtanSides(0.001, 0.0, DOUBLE_MAX_, true, true),
  m_eIntMatType(K),
  m_pBoundIntMatTop(0),
...
CBoundaryInterfaceDef::~CBoundaryInterfaceDef()
{
  ClearInterfaceMaterials();
}
...
CBoundaryInterfaceMaterial& CBoundaryInterfaceDef::FindOrCreateInterfaceMaterial(const geo::CInterfaceElement& iface, bool isBottom) const
{
...
const CBoundaryInterfaceMaterial& CBoundaryInterfaceDef::InterfaceMaterialTop(const geo::CInterfaceElement& iface) const
{
...
const CBoundaryInterfaceMaterial& CBoundaryInterfaceDef::InterfaceMaterialBottom(const geo::CInterfaceElement& iface) const
{
  if (IntMatType() != L)
  {
...
const CBoundaryInterfaceMaterial& CBoundaryInterfaceDef::InterfaceMaterialSides(const geo::CInterfaceElement& iface) const
{
...
void CBoundaryInterfaceDef::ClearInterfaceMaterials()
{
  ...
  for (std::map<const geo::CInterfaceElement *, CBoundaryInterfaceMaterial *>::iterator it = m_materials.begin(); it != m_materials.end(); ++it)
    delete it->second;
  m_materials.clear();
...
unsigned int CBoundaryInterfaceDef::IconId() const
{
  return 0;
}

unsigned int CBoundaryInterfaceDef::TypeId() const
{
  return 0;
}

bool CBoundaryInterfaceDef::Empty() const
{
  return false;
}

long CBoundaryInterfaceDef::SavedItems() const
{
  return 0;
}
