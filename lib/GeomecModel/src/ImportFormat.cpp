...
//////////////////////////////////////////////////////////////////////
IImportFormat::IImportFormat(CModelBase &model) : m_pValueType(0), m_nNextComponent(-1), m_model(model) {
  CreateAliases();
}

const CModelBase &IImportFormat::Model() const { return m_model; }
...
CValueType *IImportFormat::CreateValueType(const TStringVec &vcType) {
  ...
  if (m_ValueTypeAliases.find(vcType[0].toStdString()) != m_ValueTypeAliases.end()) {
    std::map<std::string, std::string>::iterator it = m_ValueTypeAliases.find(vcType[0].toStdString());
  ...
  if (vcType.size() != pRet->ComponentSize() + 1 + conversionArg &&
    ...
    throw CReadException(
        QString("%1 type requires at least %2 name labels.").arg(vcType[0]).arg(pRet->ComponentSize()));
  }
  ...
  if (m_mpComponent.size() == 0) {
    if (m_pValueType) {
      ...
      if (m_pValueType->Component(m_nNextComponent).ExportLabel().compare(sName, Qt::CaseInsensitive) == 0) {
        ...
        throw CReadException(
            QString("Expecting component %1 of %2 not %3").arg(m_nNextComponent).arg(m_pValueType->Name()).arg(sName));
      }
    }
...
CNodalImportFormat::CNodalImportFormat(IElementSet &element_set)
    : IImportFormat((CModelBase &)element_set.Model()), m_nNodeIndex(-1), m_pElementSet(&element_set), m_pPointSet(0) {}

CNodalImportFormat::CNodalImportFormat(CPointSet &point_set)
    : IImportFormat((CModelBase &)point_set.Model()), m_nNodeIndex(-1), m_pElementSet(0), m_pPointSet(&point_set) {}
...
void CNodalImportFormat::SetCoordinate(COORDINATE_TYPE coordinate_type) {
  ...
  if (m_pPointSet) {
    // Create column
    QString sName;
    if (coordinate_type == CT_DEPTH)
      sName = getStringTableEntry(IDS_RC_DEPTH);
    if (coordinate_type == CT_NORTHING)
      sName = getStringTableEntry(IDS_RC_NORTHING);
    if (coordinate_type == CT_EASTING)
    ...
    for (int i = 0; i < m_pPointSet->NodalValueSetSize(); i++)
      if (&m_pPointSet->NodalValueSet(i) == pValueSet1)
        nIndex = i;
  ...
  if (m_pPointSet) {
    ...
    for (size_t s = 0; s < m_pPointSet->ValueSetSize(); ++s) {
      if (m_pPointSet->ValueSet(s).Component() &&
        ...
        l.append(
            QString("Component \"%1\" has values out of range.").arg(m_pPointSet->ValueSet(s).Component()->Name()));
    }
    if (!l.empty())
...
void CNodalImportFormat::AppendRow(const TValueVec &vcRow) {
  if (vcRow.size() != ColumnSize()) {
    throw CReadException(
        QString("Expected %1 value; read %2 values in nodal section").arg(ColumnSize()).arg(vcRow.size()));
  }

  // Are there NANs in the coordinate section?
  for (TCoordinateMap::const_iterator it = m_mpCoordinate.begin(); it != m_mpCoordinate.end(); it++)
    if (!vcRow[it->first].Valid())
      throw CReadException("NAN values in coordinate definition");
  ...
  if (m_pElementSet) {
    ...
    for (size_t i = 0; i < m_pElementSet->ValueSetSize(); ++i)
      if (!m_pElementSet->ValueSet(i).Component()->IsInRange(m_pElementSet->ValueSet(i)))
        l.append(
            QString("Component \"%1\" has values out of range.").arg(m_pPointSet->ValueSet(i).Component()->Name()));
  } else {
    assert(m_pPointSet);
...
CResultImportFormat::CResultImportFormat(CModelBase &model)
    : IImportFormat(model), m_nElementNodeColumn(0), m_Unit(CQuantity::SI_UNIT),
      m_vcElementRead(model.Mesh().Mesh().ElementSize(), false), m_nCurrentNode(0), m_pCurrentElement(0) {}

CResultImportFormat::~CResultImportFormat() {
  for (size_t i = 0; i < m_vcResultDescriptions.size(); i++)
    delete m_vcResultDescriptions[i];
}
...
int CResultImportFormat::ColumnSize() const { return m_vcResultDescriptions.size() + m_nElementNodeColumn; }
...
void CResultImportFormat::AppendRow(const TValueVec &vcRow) {
  if (vcRow.size() != ColumnSize()) {
    throw CReadException(
        QString("Expected %1 values, read %2 values in results section.").arg(ColumnSize()).arg(vcRow.size() - 2));
  }
  ...
  if (vcOptions.size() == 0) {
    throw CReadException(
        QString("Node %1 in element %2 not found in mesh.").arg((int)vcRow[1].Value()).arg((int)vcRow[0].Value()));
  }
...
int CResultImportFormat::ResultTypeIndex(const QString &typeName) const {
  if (typeName == "displacementn")
    return CI_DISPLA_X;
  if (typeName == "displacemente")
    return CI_DISPLA_Y;
  if (typeName == "displacementv")
    return CI_DISPLA_Z;

  if (typeName == "pressure")
    return CI_POREPRES;

  if (typeName == "estressnn")
    return CI_STRESS_XX;
  if (typeName == "estressee")
    return CI_STRESS_YY;
  if (typeName == "estressvv")
    return CI_STRESS_ZZ;
  if (typeName == "estressne")
    return CI_STRESS_XY;
  if (typeName == "estressev")
    return CI_STRESS_YZ;
  if (typeName == "estressnv")
    return CI_STRESS_XZ;

  if (typeName == "strainnn")
    return CI_STRAIN_XX;
  if (typeName == "strainee")
    return CI_STRAIN_YY;
  if (typeName == "strainvv")
    return CI_STRAIN_ZZ;
  if (typeName == "strainne")
    return CI_STRAIN_XY;
  if (typeName == "strainev")
    return CI_STRAIN_YZ;
  if (typeName == "strainnv")
    return CI_STRAIN_XZ;

  if (typeName == "strainplasticnn")
    return CI_PSTRAIN_XX;
  if (typeName == "strainplasticee")
    return CI_PSTRAIN_YY;
  if (typeName == "strainplasticvv")
    return CI_PSTRAIN_ZZ;
  if (typeName == "strainplasticne")
    return CI_PSTRAIN_XY;
  if (typeName == "strainplasticev")
    return CI_PSTRAIN_YZ;
  if (typeName == "strainplasticnv")
    return CI_PSTRAIN_XZ;

  if (typeName == "straincreepnn")
    return CI_CSTRAIN_XX;
  if (typeName == "straincreepee")
    return CI_CSTRAIN_YY;
  if (typeName == "straincreepvv")
    return CI_CSTRAIN_ZZ;
  if (typeName == "straincreepne")
    return CI_CSTRAIN_XY;
  if (typeName == "straincreepev")
    return CI_CSTRAIN_YZ;
  if (typeName == "straincreepnv")
    return CI_CSTRAIN_XZ;

  if (typeName == "faultdisplacementn")
    return CI_STRAIN_XX;
  if (typeName == "faultdisplacemente")
    return CI_STRAIN_YY;
  if (typeName == "faultdisplacementv")
    return CI_STRAIN_ZZ;

  if (typeName == "faultestressnormal")
    return CI_PSTRAIN_XX;

  if (typeName == "faultestressn")
    return CI_STRESS_XX;
  if (typeName == "faultestresse")
    return CI_STRESS_YY;
  if (typeName == "faultestressv")
    return CI_STRESS_ZZ;

  if (typeName == "faultpressure")
    return CI_POREPRES;

  if (typeName == "temperature")
    return CI_TEMPERATURE;

  if (typeName == "preconpressure")
    return CI_PRECON;
...
CResultImportFormat::CResultDescription::CResultDescription(CResultImportFormat &parent)
    : m_parent(parent), m_bIgnore(true) {}

CResultImportFormat::CResultDescription::CResultDescription(CResultImportFormat &parent, int nResultType, bool bFault,
                                                            int nDepStage, const CAnalysisType &antype)
    : m_nResultType(nResultType), m_parent(parent), m_nDepStage(nDepStage), m_antype(antype), m_bFault(bFault),
      m_bIgnore(false) {}

/*!
  Returns true when the result is a pressure type
*/
bool CResultImportFormat::CResultDescription::IsPressure() const { return m_nResultType == CI_POREPRES; }

bool CResultImportFormat::CResultDescription::IsPreconPressure() const { return m_nResultType == CI_PRECON; }
...
bool CResultImportFormat::CResultDescription::IsStress() const {
  if ((m_nResultType >= CI_STRESS_XX) && (m_nResultType <= CI_STRESS_XZ))
    return true;
...
bool CResultImportFormat::CResultDescription::IsStrain() const {
  if ((m_nResultType >= CI_STRAIN_XX) && (m_nResultType <= CI_STRAIN_XZ))
    return true;

  if ((m_nResultType >= CI_PSTRAIN_XX) && (m_nResultType <= CI_PSTRAIN_XZ))
    return true;

  if ((m_nResultType >= CI_CSTRAIN_XX) && (m_nResultType <= CI_CSTRAIN_XZ))
    return true;
...
bool CResultImportFormat::CResultDescription::IsFault() const { return m_bFault; }
...
double CResultImportFormat::CResultDescription::FieldFactor() const {
  ...
  if (m_nResultType == CI_POREPRES)
    return FF_FACTOR_STRESS;

  if ((m_nResultType >= CI_DISPLA_X) && (m_nResultType <= CI_DISPLA_Z))
    return FF_FACTOR_LENGTH;

  if ((m_nResultType >= CI_STRAIN_XX) && (m_nResultType <= CI_STRAIN_ZZ) && IsFault())
    return FF_FACTOR_LENGTH;
...
bool CResultImportFormat::CResultDescription::AddResultValue(int nLocalNodeIndex, const geo::CValue &val) {
  ...
  if (bRet) {
    ...
    if (m_parent.Unit() == CQuantity::FIELD_UNIT) {
      m_parent.Model().ResultRegister().AddResultFromImport(m_nResultType, m_parent.Element().Index(), nLocalNodeIndex,
                                                            this->m_nDepStage, m_antype, value.Value() / FieldFactor());
    } else {
      assert(m_parent.Unit() == CQuantity::SI_UNIT);
      if (IsStress()) {
        m_parent.Model().ResultRegister().AddResultFromImport(
            m_nResultType, m_parent.Element().Index(), nLocalNodeIndex, this->m_nDepStage, m_antype,
        ...
        m_parent.Model().ResultRegister().AddResultFromImport(
            m_nResultType, m_parent.Element().Index(), nLocalNodeIndex, this->m_nDepStage, m_antype, value.Value());
      }
    }
  }
...
CElementImportFormat::CElementImportFormat(CNodalImportFormat &nodal_format)
    : IImportFormat((CModelBase &)nodal_format.ElementSet().Model()), m_nodal_format(nodal_format), m_nNodeIndex(-1),
      m_nElementIndex(-1), m_nNodeNr(-1) {}
...
void CElementImportFormat::AppendColumn(const QString &sName) {
  ...
  } else if (sName.compare("ElementIndex", Qt::CaseInsensitive) == 0 ||
             sName.compare("ElementNr", Qt::CaseInsensitive) == 0) {
...
void CElementImportFormat::AppendRow(const TValueVec &vcRow) {
  ...
  if (vcRow.size() != ColumnSize()) {
    throw CReadException(
        QString("Expected %1 values; read %2 values in element section").arg(ColumnSize()).arg(vcRow.size()));
  }
  ...
  if (pElementSet)
    nPointSize = pElementSet->ElementPointSize();
  if (m_vcElementData[0].size() == nPointSize) {
    ...
    if (nElement == -1)
      throw CReadException("Wrong number of element nodes supplied.");
      ...
      if (pConversion) {
        for (size_t jj = 0; jj < m_vcElementData[m_vcValueSet[i]].size(); ++jj) {
          m_vcElementData[m_vcValueSet[i]][jj].Value(
              pConversion->ConvertToSi(m_vcElementData[m_vcValueSet[i]][jj].Value()));
        }
...
IPointSet &CElementImportFormat::PointSet() { return m_nodal_format.ElementSet(); }
...
CWellPathInputImportFormat::CWellPathInputImportFormat(CNewWellPathInput *pNewWellPathInput)
    : IImportFormat((CModelBase &)pNewWellPathInput->Model()), m_Unit(UNIT_SI), m_column(5, -1),
      m_pNewWellPathInput(pNewWellPathInput), m_lastColumnIndex(4) {
...
void CWellPathInputImportFormat::AppendColumn(const QString &sName) {
  if (sName.compare("Easting", Qt::CaseInsensitive) == 0 || sName.compare("Y", Qt::CaseInsensitive) == 0)
    ...
    setColumn(1, sName);
  else if (sName.compare("Depth", Qt::CaseInsensitive) == 0 || sName.compare("Z", Qt::CaseInsensitive) == 0 ||
           sName.compare("TVD", Qt::CaseInsensitive) == 0)
    setColumn(2, sName);
  else if (sName.compare("AHD", Qt::CaseInsensitive) == 0 || sName.compare("TMD", Qt::CaseInsensitive) == 0 ||
...
void CWellPathInputImportFormat::ValidateColumns() {
  if (m_column[0] == -1)
    throw CReadException("Easting not specified in the header.");
  if (m_column[1] == -1)
    throw CReadException("Northing not specified in the header.");
  if (m_column[2] == -1)
    throw CReadException("Depth not specified in the header.");
}
...
void CWellPathInputImportFormat::AppendRow(const TValueVec &vcRow) {
  ...
  geo::CPoint pt(vcRow[m_column[1]].Value(), vcRow[m_column[0]].Value(), vcRow[m_column[2]].Value());
