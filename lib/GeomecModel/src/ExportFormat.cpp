...
IExportFormat::IExportFormat(const TValueTypeVec& vcData, const CExportArg& arg)
: m_arg(arg), m_vcValueType( vcData )
{
...
QString IExportFormat::LoadFromString(unsigned int uString) const
{
	return getStringTableEntry(uString);
}

int IExportFormat::ColumnSize() const
{
	return m_vcData.size();
}

IExportFormat::TOutputType IExportFormat::OutputType(int /*nRow*/, int /*nColumn*/) const
{
	return OT_STRING;
}
...
size_t IExportFormat::DataValueSize() const
{
	return m_vcData.size();
}
		...
		if(HotSpot())
		{
			...
			else
			{
				if(value.Value() < HotSpot()->MinMaxValue().first)
					...
					ret.second.sString = QString("%1%").arg((((value.Value() - HotSpot()->MinMaxValue().first)/(HotSpot()->MinMaxValue().second - HotSpot()->MinMaxValue().first))*100.0));
			}
		}
		else
		{
...
size_t IExportFormat::ValueTypeSize() const
{
	return m_vcValueType.size();
}

const CValueType& IExportFormat::ValueType(size_t nIndex) const
{ 
	return *m_vcValueType[nIndex];
}
...
IExportFormat::TDataVec IExportFormat::eliminateDuplicates(const TDataVec& dataVector)
{
  ...
  {
    if (((*data)->ExportLabel() ==
        getStringTableEntry(IDS_MESH_RESULT_DEPTH)) ||
      ((*data)->ExportLabel() ==
        getStringTableEntry(IDS_MESH_RESULT_NORTHING)) ||
...
CNodalExportFormat::CNodalExportFormat(const CPointSet& point_set,// PointSet to export
  ...
  bool export2LAS)
: IExportFormat(vcData, arg), 
  m_pPointSet(&point_set), 
  m_pElementSetFormat(0),
  m_pNewWellPath(0),
  m_row_cache_index(-1),
  m_bAddFormationNames(arg.m_bFormationNames)
, m_export2LAS(export2LAS)
, m_iteratorWellPathPosition(-1)
, m_data_offset(0)
{
...
: IExportFormat(vcData, arg), 
  m_pPointSet(0), 
  m_pElementSetFormat(&element_set_format), 
  m_pNewWellPath(0),
  m_row_cache_index(-1),
...
QString CNodalExportFormat::ExportType() const
{
	...
	if(m_pElementSetFormat) return m_pElementSetFormat->ExportType();
	return "POINTSET";
}

bool CNodalExportFormat::HasExportName() const
{
	return (m_pNewWellPath != 0 );
}

QString CNodalExportFormat::ExportName() const
{
	if ( m_pNewWellPath ) return QString(m_pNewWellPath->Name());

	assert( false );
	return 0;
}

bool CNodalExportFormat::isGVT() const
{
  return m_arg.m_bGVT;
}

const CHotSpot* CNodalExportFormat::HotSpot() const
{
	return m_arg.m_pHotSpot;
}
...
void CNodalExportFormat::CreateCache(int nRow) const
{
  ...
  else // wellpath
  {
    ...
    else // row != 0
    {
        ...
        if (m_arg.m_wellpathPoints == MESH_INTERSECTION_POINTS)
        {
          ...
          {
			  const std::list< CNewWellPoint >& points_l = 
				  ( const_cast< CNewWellPath* >(m_pNewWellPath) )->MeshIntersecPoints();
  ...
  for(int i = 0; i < IExportFormat::ColumnSize(); i++)
  {
    ...
    else
    {
      ...
      if ( pNewWellPoint ) pPoint= pNewWellPoint;
      else if(pElementPoint) pPoint = pElementPoint;
  ...
  if (pNewWellPoint && m_arg.m_wellpathPoints == INPUT_POINTS) delete pNewWellPoint;
}
...
{
	if(nRow != m_row_cache_index) CreateCache(nRow);
	assert(nColumn >= 0);
	...
	assert(m_pNewWellPath);
        {
          return (m_arg.m_wellpathPoints == INPUT_POINTS ? m_pNewWellPath->WellPointSize()
                                                         : m_pNewWellPath->MapValuesSize()) + 1;
        }
}
...
int CNodalExportFormat::ColumnSize() const
{
	...
	if(m_pElementSetFormat)
	{
		...
		for(unsigned int i = 0; i < m_pPointSet->Coordinates().ComponentSize(); i++)
			if(m_pPointSet->Coordinates().Component(i).Defined()) nRet++;
	} else {
		assert(m_pNewWellPath);
...
CNodalExportFormat::UNIT CNodalExportFormat::Unit() const
{
	return m_arg.m_unit;
}

bool CNodalExportFormat::HasRowComment(int /*nRow*/) const
{
	return false;
}
...
int CNodalExportFormat::getDataOffset() const
{
...
//////////////////////////////////////////////////////////////////////
IElementSetExportFormat::IElementSetExportFormat(const TDataVec& vcNodalData,		
												 const TDataVec& vcElementData,
												 const CExportArg& arg)			// Arguments for the export
: IExportFormat(vcElementData, arg),
  m_nodal_format(*this, vcNodalData, arg),
...
const CNodalExportFormat& IElementSetExportFormat::NodalExportFormat() const
{
	return m_nodal_format;
}
...
IElementSetExportFormat::TElementIndex IElementSetExportFormat::ElementIndex(int nRow) const
{
	...
	if(m_mpElementNodeSize.size() == 0) InitElementNodeSize();
...
int IElementSetExportFormat::ColumnSize() const
{
	return IExportFormat::ColumnSize() + 2;
}
...
IElementSetExportFormat::TOutputType IElementSetExportFormat::OutputType(int nRow, int nColumn) const
{
	...
	if(index.first != m_element_cache_index) CreateCache(index.first);
	assert(nColumn >= 0);
...
QString IElementSetExportFormat::StringAt(int nRow, int nColumn) const
{
	...
	if((nRow == 0) && (nColumn == 1)) return "NodeIndex"; 
	if((nRow == 0) && (nColumn > 1))  return IExportFormat::StringAt(nRow, nColumn - 2);
...
QString IElementSetExportFormat::ElementType() const
{
	...
	if(sType.toUpper() == "HX24L") return "HEXA8";
	if(sType.toUpper() == "TE12L") return "TETRA4";
	if(sType.toUpper() == "Q24IF") return "IQUAD8";
	if(sType.toUpper() == "T18IF") return "ITRI6";
	if(sType.toUpper() == "T6EPS") return "TRI3";	   // Triangle in 2D
	if(sType.toUpper() == "L8IF" ) return "ILINE4";  // 2D interface element
...
bool IElementSetExportFormat::HasRowComment(int /*nRow*/) const
{
	return false;
}
...
const CHotSpot* IElementSetExportFormat::HotSpot() const
{
	return m_nodal_format.HotSpot();
}

bool IElementSetExportFormat::isGVT() const
{
  return m_nodal_format.isGVT();
}
...
*/
void CElementSetExportFormat::AddElementSet( TNodeMap& mpNode, const geo::IElementSet& element_set, const QString& sName )
{
...
bool CElementSetExportFormat::sortNodeOnIndex(const TNode& lhs,
  const TNode& rhs)
{
  return lhs.first < rhs.first;
}

CElementSetExportFormat::CElementSetExportFormat(const IElementSet& element_set,
												 const TDataVec& vcElementData,
												 const CExportArg& arg)			// Arguments for the export
: IElementSetExportFormat(TDataVec(), vcElementData, arg),
  m_model(dynamic_cast<const CModelBase&>( element_set.Model() )),
  m_bTwoDimensional( element_set.Dimension() == IPointSet::DIM_2D ),
  m_bUseMesh(true),
  m_bAddFormationNames(false)
{
	TNodeMap mpNode;
...
CElementSetExportFormat::CElementSetExportFormat(const IElementSet& element_set,
												 const TValueTypeVec& vcElementData,
												 const CExportArg& arg)			// Arguments for the export
...
CElementSetExportFormat::CElementSetExportFormat(const TFormationVec& vcFormation, 
											     const TDataVec& vcNodalData,		
											     const TDataVec& vcElementData,
												 const CExportArg& arg)			// Arguments for the export
: IElementSetExportFormat(vcNodalData, vcElementData, arg),
  m_model(dynamic_cast<const CModelBase&>( (*vcFormation.begin())->Model() )),
  m_bUseMesh(true),
  m_bAddFormationNames(arg.m_bFormationNames)
{
...
CElementSetExportFormat::CElementSetExportFormat(const CFormationBase& formation,
												 const TDataVec& vcNodalData,		
												 const TDataVec& vcElementData,
												 const CExportArg& arg)			// Arguments for the export
: IElementSetExportFormat(vcNodalData, vcElementData, arg),
  m_model(dynamic_cast<const CModelBase&>( formation.Model() )),
  m_bUseMesh(true),
  m_bAddFormationNames(arg.m_bFormationNames)
{
	m_bTwoDimensional = false;
...
CElementSetExportFormat::CElementSetExportFormat
( const CFormationPlane &formation_plane,
  const TDataVec& vcElementData,
  const CExportArg& arg )	// Arguments for the export
: IElementSetExportFormat(TDataVec(), vcElementData, arg),
  m_model(dynamic_cast<const CModelBase&>( formation_plane.Model() )),
  m_bUseMesh(true),
  m_bAddFormationNames(false)
{
	m_bTwoDimensional = false;
...
( const TFormationPlaneVec &formation_plane_vec,
  ...
  const CExportArg& arg )	// Arguments for the export
: IElementSetExportFormat(TDataVec(), vcElementData, arg),
  m_model(dynamic_cast<const CModelBase&>( formation_plane_vec[0]->Model() )),
  m_bUseMesh(true),
  m_bAddFormationNames(false)
{
	m_bTwoDimensional = false;
	...
	{
		TNodeMap mpNode;
		AddElementSet(mpNode, formation_plane_vec[ii]->PlaneSurface(), formation_plane_vec[ii]->Name().toStdString().c_str());
		AddNodes( mpNode );
...
*/
CElementSetExportFormat::CElementSetExportFormat(const THorizonVec &vcHorizon,
						 const TDataVec& vcNodalData,
						 const TDataVec& vcElementData,
						 const CExportArg& arg)			// Arguments for the export
: IElementSetExportFormat(vcNodalData, vcElementData, arg),
  m_model(dynamic_cast<const CModelBase&>( vcHorizon[0]->Model() )),
  m_bAddFormationNames(false)
{
	m_bTwoDimensional = false;
...
: IElementSetExportFormat(TDataVec(), vcElementData, arg),
  m_model(dynamic_cast<const CModelBase&>(vcRTCI[0]->Model())),
  m_bTwoDimensional(false),
  m_bUseMesh(false),
  m_bAddFormationNames(false)
{
...
*/
CElementSetExportFormat::CElementSetExportFormat(const CCrossSection &xsec,
												 const TDataVec& vcNodalData,		
												 const TDataVec& vcElementData,
												 const CExportArg& arg)			// Arguments for the export
: IElementSetExportFormat(vcNodalData, vcElementData, arg),
  m_model(dynamic_cast<const CModelBase&>( xsec.Model() )),
  m_bUseMesh(true),
  m_bAddFormationNames(false)
{
	m_bTwoDimensional = false;
...
: IElementSetExportFormat(TDataVec(), vcElementData, arg),
  m_model(dynamic_cast<const CModelBase&>(vcSurface[0]->Model())),
  m_bTwoDimensional(false),
  m_bUseMesh(false),
  m_bAddFormationNames(false)
{
  ...
  {
    TNodeMap mpNode;
    AddElementSet(mpNode, static_cast<const geo::IElementSet&>(vcSurface[i]->DisplayList(0)), vcSurface[i]->Name().toStdString().c_str());
    AddNodes(mpNode);
    assert(m_vcElementSet.size() > 0);
  }
}
...
*/
void CElementSetExportFormat::sortNodeVector()
{
  std::sort(m_vcNode.begin(), m_vcNode.end(), sortNodeOnIndex);
}
...
QString CElementSetExportFormat::ExportType() const
{
	...
	if( dynamic_cast<const geo::ISurface*>(m_vcElementSet[0].first) && DataValueSize() == 0 )
		return "SURFACE";
	if( NodeSize() ) return "ELEMENTSET";
	return "RESULTS";
}

int CElementSetExportFormat::NodeSize() const
{
	return m_vcNode.size();
}

int CElementSetExportFormat::NodeIndex(int nIndex) const
{
	return m_vcNode[nIndex].first + 1;
}

bool CElementSetExportFormat::TwoDimensional() const
{
	return m_bTwoDimensional;
}
...
const CModelBase& CElementSetExportFormat::Model() const
{
  return m_model;
}
...
int CElementSetExportFormat::getDataOffset() const
{
  return 2;
}
