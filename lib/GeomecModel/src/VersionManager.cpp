...
: m_file_version(VERSION_MAJOR, VERSION_MINOR, VERSION_REVISION)
...
QString& CVersionManager::AppVersion()
{
  return m_applicationVersion;
}

const QString& CVersionManager::AppVersion() const
{
  return m_applicationVersion;
}

void CVersionManager::AppVersion(const QString& applicationVersion)
{
  m_applicationVersion = applicationVersion;
}

CStreamVersion& CVersionManager::FileVersion()
{
  return m_file_version;
}

const CStreamVersion& CVersionManager::FileVersion() const
{
  return m_file_version;
}

void CVersionManager::FileVersion(const CStreamVersion& file_version)
{
...
bool CVersionManager::CheckVersion(const CStreamVersion& version) const
{
  ...
  {
    if (CStreamVersion(VERSION_MAJOR, VERSION_MINOR, VERSION_REVISION) <
      version)
    {
    ...
    if(version.majorNr() == 3 && version.minorNr() == 7 && version >
         CStreamVersion(VERSION_MAJOR_3, VERSION_MINOR_3, VERSION_REVISION_3))
    {
