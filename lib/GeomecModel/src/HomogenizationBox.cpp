...
CHomogenizationBox::CHomogenizationBox(CFemAppModel &model) : IScalingBox(model), m_mesh_ptset(0) {
  m_mesh = new geo::CBlockMesh();
  m_display_mesh = new geo::CBlockMesh();
}

CHomogenizationBox::CHomogenizationBox(const CHomogenizationBox &rhs) : IScalingBox(rhs), m_mesh_ptset(0) {
  m_mesh = new geo::CBlockMesh();
  m_display_mesh = new geo::CBlockMesh();
  *m_mesh = *rhs.m_mesh;
...
long CHomogenizationBox::SavedItems() const { return 3 + CColorNode::SavedItems(); }

unsigned int CHomogenizationBox::IconId() const { return IDI_SCALING_BOX; }

unsigned int CHomogenizationBox::TypeId() const { return 0; }

bool CHomogenizationBox::CanConnectItem(const CGraphNode & /*item*/) const { return false; }

bool CHomogenizationBox::ConnectItem(const CGraphNode &item) { return CColorNode::ConnectItem(item); }
...
CElementSet *CHomogenizationBox::CaptureElementSet(CModelBase &target_model) const {
  ...
  TFormationBaseEntry::TNodeSet formations =
      ((TFormationBaseEntry *)source_model.GraphEntry(MD_BASE_FORMATION))->EntryNodes();
  TFormationBaseEntry::TNodeSet::iterator it;

  // make 6 planes describing the faces
  geo::CVector x_plane(
      geo::CPoint(DisplayMesh().XBase(), DisplayMesh().YBase(), DisplayMesh().ZBase()),
  ...
  QString name =
      target_model.GraphEntry(MD_BASE_POINTSET)
          ->createUniqueName(DEFAULT_CAPTURED_NAME.arg(Name()) + DEFAULT_CAPTURED_NUMBER, DEFAULT_INITIAL_NUMBER);
  CElementSet *pElSet = new CElementSet(elms, name.toStdString().c_str(), type, target_model);
  return pElSet;
}

bool CHomogenizationBox::Destroy() { return CColorNode::Destroy(); }

void CHomogenizationBox::SetMesh(const geo::IPoint &center, const double &sizeX, const double &sizeY,
                                 const double &sizeZ, int divX, int divY, int divZ, const double &azimuth) {
  Center(center);
  SideX(sizeX);
  SideY(sizeY);
  SideZ(sizeZ);
  ...
  geo::CPoint ptMin(center.X() - sizeX / 2.0, center.Y() - sizeY / 2.0, center.Z() - sizeZ / 2.0);
...
const QString &CHomogenizationBox::Name() const { return CGraphNode::Name(); }

void CHomogenizationBox::assignMesh(geo::CBlockMesh &mesh, const CHomogenizationBox &hbox) const {
  geo::CPoint ptMin(hbox.Center().X() - hbox.SideX() / 2.0, hbox.Center().Y() - hbox.SideY() / 2.0,
                    hbox.Center().Z() - hbox.SideZ() / 2.0);

  mesh.SetMesh(ptMin, hbox.SideX(), hbox.SideY(), hbox.SideZ(), hbox.Mesh().LengthDivision(),
               hbox.Mesh().WidthDivision(), hbox.Mesh().HeightDivision());
...
int CHomogenizationBox::DisplayListSize() const { return 1; }

const geo::IObject &CHomogenizationBox::DisplayList(int /*nIndex*/) const { return *m_display_mesh; }
...
void CHomogenizationBox::SortOutTyings(std::map<int, std::set<int>> &connections) const {
  ...
  */
  for (it = connections.begin(); it != connections.end(); it++) {
    ...
    while (
        connections.find(first) !=
        connections.end()) { // as soon as this conditions is not satisfied we found a node which is only a mater node!
