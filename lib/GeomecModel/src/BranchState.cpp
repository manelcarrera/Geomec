...
CBranchState::COverride::COverride(CBranchState& state, const CDepletionStage& stage)
: m_state(state),
  m_pOldStage(0)
{
  if(state.IsBranch())
...
CBranchState::COverride::~COverride()
{
  m_state.m_pBranchStage = m_pOldStage;
}


//// CBranchState

CBranchState::CBranchState(CModelBase& model)
: m_model(model),
  m_pBranchStage(0)
{
}

void CBranchState::SetBranchStage(const CDepletionStage& stage)
{
  m_pBranchStage = &stage;
}
...
bool CBranchState::IsBranch() const
{
  return (m_pBranchStage != 0);
}
...
const CModelBase& CBranchState::Model() const
{
  return m_model;
}
