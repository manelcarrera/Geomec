...
IUpscalingNodeBase::IUpscalingNodeBase(CFemAppModel &model) : CStorageNode(model) {}

IUpscalingNodeBase::IUpscalingNodeBase(const QString &strName, C3DModel &model) : CStorageNode(strName, model) {
...
IUpscalingNodeBase::IUpscalingNodeBase(const unsigned int uName, C3DModel &model) : CStorageNode(uName, model) {}

IUpscalingNodeBase::IUpscalingNodeBase(const IUpscalingNodeBase &rhs) : CStorageNode(rhs) {}

bool IUpscalingNodeBase::Empty() const { return false; }

long IUpscalingNodeBase::SavedItems() const {
  return m_pFormationsBranch->SavedItems() + m_pPointsetRangesBranch->SavedItems() +
         m_pTargetPointsetBranch->SavedItems() + CStorageNode::SavedItems();
}
...
unsigned int IUpscalingNodeBase::IconId() const { return IDI_UPSCALING; }

unsigned int IUpscalingNodeBase::TypeId() const { return 0; }

const CUpscalingFormationsBranch &IUpscalingNodeBase::FormationsBranch() const { return *m_pFormationsBranch; }

CUpscalingFormationsBranch &IUpscalingNodeBase::FormationsBranch() { return *m_pFormationsBranch; }

const CUpscalingPointsetRangesBranch &IUpscalingNodeBase::PointsetRangesBranch() const {
  return *m_pPointsetRangesBranch;
}

CUpscalingPointsetRangesBranch &IUpscalingNodeBase::PointsetRangesBranch() { return *m_pPointsetRangesBranch; }

const CUpscalingTargetPointsetBranch &IUpscalingNodeBase::TargetPointsetBranch() const {
  return *m_pTargetPointsetBranch;
}

CUpscalingTargetPointsetBranch &IUpscalingNodeBase::TargetPointsetBranch() { return *m_pTargetPointsetBranch; }

// protected

const QString IUpscalingNodeBase::DEFAULT_2D_POINT_SET_NAME = QObject::tr("%1 2D Pointset");
const QString IUpscalingNodeBase::DEFAULT_2D_POINT_SET_NUMBER = QObject::tr(" %1");
const QString IUpscalingNodeBase::DEFAULT_3D_POINT_SET_NAME = QObject::tr("%1 3D Pointset");
const QString IUpscalingNodeBase::DEFAULT_3D_POINT_SET_NUMBER = QObject::tr(" %1");
const size_t IUpscalingNodeBase::DEFAULT_INITIAL_NUMBER = 1;

/////

CUpscalingNodeFactory::CUpscalingNodeFactory(C3DModel &model) : m_model(model) {}
...
void CUpscalingEntry::LoadStream(CStorageNode::TSTREAM &stream, CStorageNode::TSTREAMVERSION &version,
...
const QString DEFAULT_THIN_LAYER_UPSCALING_NAME = QObject::tr("New Thin Layer Upscaling %1");
const size_t DEFAULT_INITIAL_NUMBER = 1;

} // anonymous namespace

void CUpscalingEntry::CreateThinLayerUpscaling() {
  QString strName = createUniqueName(DEFAULT_THIN_LAYER_UPSCALING_NAME, DEFAULT_INITIAL_NUMBER);
  new CThinLayerUpscalingNode(strName.toStdString().c_str(), static_cast<C3DModel &>(Model()));
}

namespace {

const QString DEFAULT_CAMCLAY_UPSCALING_NAME = QObject::tr("New CamClay Upscaling %1");

} // anonymous namespace

void CUpscalingEntry::CreateCamClayUpscaling() {
  QString strName = createUniqueName(DEFAULT_CAMCLAY_UPSCALING_NAME, DEFAULT_INITIAL_NUMBER);
...
CUpscalingFormationsBranch::CUpscalingFormationsBranch(CFemAppModel &model)
    : COpenGLNode(model), m_formationFilter(0) {}

CUpscalingFormationsBranch::CUpscalingFormationsBranch(IUpscalingNodeBase &parent)
    : COpenGLNode(FORMATIONS_BRANCH_NAME, parent.Model()), m_formationFilter(0) {
  reParent(&parent);
}

CUpscalingFormationsBranch::CUpscalingFormationsBranch(const CUpscalingFormationsBranch &rhs)
    : COpenGLNode(rhs), m_formationFilter(0) {}

bool CUpscalingFormationsBranch::HasFormations() const { return !m_stFormations.empty(); }
...
bool CUpscalingFormationsBranch::CanConnectItem(const CGraphNode &item) const {
  ...
  const CDepletionStageEntry &depletionStageEntry = static_cast<const CModelBase &>(Model()).DepletionStageEntry();
  bool fractureApertureModel = false;

  for (CDepletionStageEntry::const_iterator depletionStage = depletionStageEntry.begin();
       (pForm != 0) && (depletionStage != depletionStageEntry.end()) && !fractureApertureModel; ++depletionStage) {
    ...
    int materialModel = (libraryMaterial == 0 ? MM_LINEAR : materialServer.MaterialModel());

    fractureApertureModel =
        fractureApertureModel || (materialModel == MM_FRACTURE_APERTURE) || (materialModel == MM_FRACTURE_APERTURE2);
  }

  return !fractureApertureModel && (pForm != 0) && (m_stFormations.find(pForm) == m_stFormations.end());
}

bool CUpscalingFormationsBranch::Empty() const { return false; }

long CUpscalingFormationsBranch::SavedItems() const { return 1 + COpenGLNode::SavedItems(); }
...
unsigned int CUpscalingFormationsBranch::IconId() const { return IDI_FORMATIONS; }

unsigned int CUpscalingFormationsBranch::TypeId() const { return 0; }
...
CUpscalingFormationsBranch::CFormationDrawDef::Color(const geo::IObject & /*object*/) const {
...
IUpscalingPointsetBranchBase::IUpscalingPointsetBranchBase(CFemAppModel &model) : COpenGLNode(model) {}
...
unsigned int IUpscalingPointsetBranchBase::TypeId() const { return 0; }

int IUpscalingPointsetBranchBase::DisplayListSize() const { return 0; }
...
CUpscalingPointsetRangesBranch::CUpscalingPointsetRangesBranch(CFemAppModel &model)
    : IUpscalingPointsetBranchBase(model), m_bAllPointsets(false) {}

CUpscalingPointsetRangesBranch::CUpscalingPointsetRangesBranch(IUpscalingNodeBase &parent)
    : IUpscalingPointsetBranchBase(POINTSET_RANGES_BRANCH_NAME, parent), m_bAllPointsets(false) {}

CUpscalingPointsetRangesBranch::CUpscalingPointsetRangesBranch(const CUpscalingPointsetRangesBranch &rhs)
    : IUpscalingPointsetBranchBase(rhs), m_bAllPointsets(rhs.m_bAllPointsets) {}

bool CUpscalingPointsetRangesBranch::Empty() const { return false; }

long CUpscalingPointsetRangesBranch::SavedItems() const { return 1 + IUpscalingPointsetBranchBase::SavedItems(); }
...
unsigned int CUpscalingPointsetRangesBranch::IconId() const { return IDI_POINTSETS; }
...
bool CUpscalingPointsetRangesBranch::UseAllPointsets() const { return m_bAllPointsets; }

void CUpscalingPointsetRangesBranch::UseAllPointsets(bool b) { m_bAllPointsets = b; }
...
COpenGLNode::CDrawDef::DrawDecisionVector
CUpscalingPointsetRangesBranch::CPointsetDrawDef::Color(const geo::IObject & /*object*/) const {
...
CUpscalingTargetPointsetBranch::CUpscalingTargetPointsetBranch(CFemAppModel &model)
    : IUpscalingPointsetBranchBase(model), m_pPillarMap(0) {}

CUpscalingTargetPointsetBranch::CUpscalingTargetPointsetBranch(IUpscalingNodeBase &parent)
    : IUpscalingPointsetBranchBase(TARGET_POINTSET_BRANCH_NAME, parent), m_pPillarMap(0) {
...
CUpscalingTargetPointsetBranch::CUpscalingTargetPointsetBranch(const CUpscalingTargetPointsetBranch &rhs)
    : IUpscalingPointsetBranchBase(rhs), m_pPillarMap(rhs.m_pPillarMap) {}
...
bool CUpscalingTargetPointsetBranch::Empty() const { return false; }

long CUpscalingTargetPointsetBranch::SavedItems() const {
  return (m_pPillarMap ? m_pPillarMap->SavedItems() : 0) + IUpscalingPointsetBranchBase::SavedItems();
}
...
unsigned int CUpscalingTargetPointsetBranch::IconId() const { return IDI_POINTSETS; }
...
bool CUpscalingTargetPointsetBranch::ConnectItem(const CGraphNode &item) {
  ...
  if (pPointSet) {
    ...
    QString msg =
        QString("Do you want to generate a target pointset from the 2D pointset '%1'?").arg(pPointSet->Name());
...
bool CUpscalingTargetPointsetBranch::HasPointset() const { return (m_pPillarMap != 0); }
...
void CUpscalingTargetPointsetBranch::CopyPillarMapAsPointset(const CPointSet &set2d, const QString &sName) {
  ...
  for (int n = 0; n < nValueSetSize; ++n) {
    ...
    assert(pVT);
    pPointset->NodalValueSet(n + 3).LinkTo(
        factory->BuildValueType(*pPointset, pVT->TypeId(), pVT->Name())->Component());
  }
...
void CUpscalingTargetPointsetBranch::CreateFromRegular2DGrid(const geo::IPoint &corner1, const geo::IPoint &corner2,
                                                             int pointsNorthing, int pointsEasting) {
  CGridPointsetSource source(corner1, corner2, pointsNorthing, pointsEasting);
  CreateTargetPointset(source);
}

bool CUpscalingTargetPointsetBranch::CanCreateFromRegular2DGrid() const { return !HasPointset(); }
