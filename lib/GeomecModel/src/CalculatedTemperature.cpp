...
: IValueComposite(stage.Name().toStdString().c_str(), formation.Model()),
...
CDepletionStage& CCalculatedTemperature::DepletionStage()
{
  return m_stage;
}

const CFormationBase* CCalculatedTemperature::Formation() const
{
  return &m_formation;
}

CFormationBase* CCalculatedTemperature::Formation()
{
  return &m_formation;
}

unsigned int CCalculatedTemperature::IconId() const
{
  return IDI_CALC_TEMPER;
}

unsigned int CCalculatedTemperature::TypeId() const
{
  return IDT_COMPONENT_CALC_TEMPER;
}
...
long CCalculatedTemperature::SavedItems() const
{
  return 0;
}
...
: IValueComponentBase(parent),
  m_parent(parent),
  m_ResultSource(static_cast<CModelBase&>(parent.Model())),
  m_Data(*this)
{
  LinkTo(m_ResultSource.ResultRegister());
}

CCalculatedTemperature::CCalculatedTemperatureComponent::CCalculatedTemperatureComponent(CCalculatedTemperature& parent, CModelBase& ResultSource)
: IValueComponentBase(parent),
  m_parent(parent),
...
IValueDataInterfaceScalar& CCalculatedTemperature::CCalculatedTemperatureComponent::ScalarData()
{
  return m_Data;
}

QString CCalculatedTemperature::CCalculatedTemperatureComponent::ExportLabel() const
{
  return QString();
}
...
IValueDomainScalar::TMinMax CCalculatedTemperature::CCalculatedTemperatureComponent::MinMax(IProgressBase& progressBase, const UNIT unit) const
{
  assert(Defined());
  return ResultComponent().MinMax(progressBase, unit);
}

IValueDomainScalar::TValue CCalculatedTemperature::CCalculatedTemperatureComponent::Average(IProgressBase& progressBase, const UNIT unit) const
{
  assert(Defined());
  return ResultComponent().Average(progressBase, unit);
}

geo::CValue CCalculatedTemperature::CCalculatedTemperatureComponent::ValuePoint(const geo::IPoint& pt, const UNIT unit, geo::IParallelInitializationCallback *cb) const
{
  assert(Defined());
  return ResultComponent().ValuePoint(pt, unit, cb);
}

IValueDomainScalar::TValueVec CCalculatedTemperature::CCalculatedTemperatureComponent::ValueElement(const geo::IElement& elm, const UNIT unit, geo::IParallelInitializationCallback *cb) const
{
  assert(Defined());
  return ResultComponent().ValueElement(elm, unit, cb);
}

unsigned int CCalculatedTemperature::CCalculatedTemperatureComponent::IconId() const
{
  return m_parent.IconId();
}

unsigned int CCalculatedTemperature::CCalculatedTemperatureComponent::TypeId() const
{
  return m_parent.TypeId();
}

bool CCalculatedTemperature::CCalculatedTemperatureComponent::Empty() const
{
  return false;
}

long CCalculatedTemperature::CCalculatedTemperatureComponent::SavedItems() const
{
  return 0;
}
...
{
  const IResultComponent* pComp = m_ResultSource.ResultTree().Temperature().ResultComponent(m_parent.DepletionStage(), CAnalysisType::AT_HEAT);
  assert(pComp);
  return *pComp;
}
