...
:	CColorNode(strName, model),
	m_box(geo::CPoint(0, 0), geo::CPoint(100, 100)),
	m_dSideX(0),
	m_dSideY(0),
	m_dSideZ(0),
	m_dAzimuth(0),
 ...
 */
IScalingBox::IScalingBox(CFemAppModel& model, const geo::IPoint& center, const double& sideX,
...
void IScalingBox::UpdateBox()
{
	...
	ptMin.Set(-m_dSideX/2.0,
			  -m_dSideY/2.0,
			  -m_dSideZ/2.0);

	ptMax.Set(m_dSideX/2.0,
			  m_dSideY/2.0,
			  m_dSideZ/2.0);
...
int IScalingBox::DisplayListSize() const
{
	return 1; 
}

const geo::IObject& IScalingBox::DisplayList(int /*nIndex*/) const
{
	return m_box; 
}
...
{
	if(fabs(m_dSideX - dSide) < EPS &&
	   fabs(m_dSideY - dSide) < EPS &&
	   fabs(m_dSideZ - dSide) < EPS) return;
...
const double& IScalingBox::SideX() const
{
	return m_dSideX;
}

const double& IScalingBox::SideY() const
{
	return m_dSideY;
}

const double& IScalingBox::SideZ() const
{
	return m_dSideZ;
}
...
const double& IScalingBox::Azimuth() const
{
	return m_dAzimuth;
}
...
const geo::IPoint &IScalingBox::Center() const
{
	return m_ptCenter; 
}
