...
CWellZoomInModel::CWellZoomInModel(CFemAppModel& /*model*/, CWellPath& wellpath, CAnalysisLogger& logger, const CVersionManager& versionManager)
...
CWellZoomInModel::CWellZoomInModel(CFemAppModel& /*model*/, CNewWellPath& wellpath, CAnalysisLogger& logger, const CVersionManager& versionManager)
: IWellModel(logger, versionManager),
  m_pWellPath(0),
...
: IWellModel(rhs),
  ...
  m_pNewWellPath(rhs.m_pNewWellPath),
...
bool CWellZoomInModel::operator==(const CWellZoomInModel& rhs) const
{
  ...
  return ( m_pWellPath == rhs.m_pWellPath) &&
         ( m_pNewWellPath == rhs.m_pNewWellPath);
}
...
void CWellZoomInModel::createDefaults()
{
  ...
  m_pCasingModel = static_cast<CWellCasingModel *>(IModelLifetimeFacade::NewChild(WELL_CASING_MODEL, *this, Logger(), getVersionManager()));

  // there is a new child
  OnNewChildModel(*m_pCasingModel, false);
}

unsigned int CWellZoomInModel::ModelIconId() const
{
  return IDI_WELLZOOMIN_MODEL;
}

unsigned int CWellZoomInModel::TypeId() const
{
  return 0;
}
...
int CWellZoomInModel::Dimension() const
{
  return 3;
}
...
int CWellZoomInModel::childModelSize() const
{
  return (m_pCasingModel != 0 ? 1 : 0);
}
...
{
  return eWellZoomInModel;
}

void CWellZoomInModel::setNewModelFunction(bool (CNewModel::*newModel)(bool))
{
  m_newModelFunction = newModel;
}

CNewModel& CWellZoomInModel::getNewModel()
{
  return m_newModel;
}

QString CWellZoomInModel::AttributesDialogCaption() const
{
  return "Wellpath Zoomin Model Attributes";
}
...
const CWellZoomInMesh& CWellZoomInModel::Mesh() const
{
  return *m_pMesh;
}

CWellZoomInMesh& CWellZoomInModel::Mesh()
{
  return *m_pMesh;
}
