...
static TStressStrainInit Default_StressStrain[] =
{
  { 360, 0 },
  { 370, 0.0001 },
  { 380, 0.0002 },
...
: IMaterialBase(strName, entry.Model()),
  m_dYoung(DEF_YOUNG),
  m_dPoisson(DEF_POISSON),
  m_dDensity(DEF_DENSITY),
  m_dThermalExpansion(DEF_THERMALEXPANSION),
  m_dThermalConductivity(DEF_THERMALCONDUC),
  m_dThermalCapacity(DEF_THERMALCAPACI),
  m_bPerforated(false)
{
  size_t i;
...
bool CWellCasingSteelMaterial::Write(const CFFMaterial &ffmat, dia::IDianaRunner& diarunner) const
{
  ...
  ftn_double_t dThermX = ffmat.ParameterValue(IDT_VALUETYPE_THERMAL_EXPANSION) / 3.0; // go from volumetric to linear thermal expansion coefficient
...
int CWellCasingSteelMaterial::Type() const
{
  return 101;
}
...
bool CWellCasingSteelMaterial::WriteFilosParamName(const CFFMaterial &/*ffmat*/, dia::IDianaRunner& diarunner, int i, char *name) const
{
  assert(dynamic_cast<CGeomecDianaRunnerBase*>(&diarunner));
  CGeomecDianaRunnerBase& runner = static_cast<CGeomecDianaRunnerBase&>(diarunner);
...
void CWellCasingSteelMaterial::WriteFilosParamValues(const CFFMaterial &ffmat, dia::IDianaRunner& diarunner, double *values, int stride) const
{
  assert(dynamic_cast<CGeomecDianaRunnerBase*>(&diarunner));
  CGeomecDianaRunnerBase& runner = static_cast<CGeomecDianaRunnerBase&>(diarunner);
...
double CWellCasingSteelMaterial::YoungsModulus() const
{
  return m_dYoung;
}

double CWellCasingSteelMaterial::PoissonsRatio() const
{
  return m_dPoisson;
}

double CWellCasingSteelMaterial::Density() const
{
  return m_dDensity;
}

double CWellCasingSteelMaterial::ThermalExpansion() const
{
  return m_dThermalExpansion;
}

double CWellCasingSteelMaterial::ThermalConductivity() const
{
  return m_dThermalConductivity;
}

double CWellCasingSteelMaterial::ThermalCapacity() const
{
  return m_dThermalCapacity;
}

bool CWellCasingSteelMaterial::Perforated() const
{
  return m_bPerforated;
}
...
void CWellCasingSteelMaterial::SetPerforated(bool bValue)
{
  m_bPerforated = bValue;
}

CWellCasingSteelMaterial::CStressStrain& CWellCasingSteelMaterial::StressStrain()
{
  return m_StressStrain;
}

const CWellCasingSteelMaterial::CStressStrain& CWellCasingSteelMaterial::StressStrain() const
{
  return m_StressStrain;
}
...
bool CWellCasingSteelMaterial::Empty() const
{
  return false;
}
...
bool CWellCasingSteelMaterial::CStressStrain::empty() const
{
  return m_lstStressStrain.empty();
}
...
void CWellCasingSteelMaterial::CStressStrain::clear()
{
  m_lstStressStrain.clear();
}
...
CWellCasingSteelMaterial::CStressStrain::const_reverse_iterator CWellCasingSteelMaterial::CStressStrain::rbegin() const
{
  return m_lstStressStrain.rbegin();
}
...
void CWellCasingSteelMaterial::CStressStrain::pop_back()
{
  m_lstStressStrain.pop_back();
}

void CWellCasingSteelMaterial::CStressStrain::pop_front()
{
  m_lstStressStrain.pop_front();
}

CWellCasingSteelMaterial::CStressStrain::iterator CWellCasingSteelMaterial::CStressStrain::insert(iterator _Where, const TStressStrainPair& _Val)
{
...
CWellCasingSteelMaterial::CStressStrain::iterator CWellCasingSteelMaterial::CStressStrain::erase(iterator _First, iterator _Last)
{
  return m_lstStressStrain.erase(_First, _Last);
}

void CWellCasingSteelMaterial::CStressStrain::sort()
{
  m_lstStressStrain.sort(CLess());
}

void CWellCasingSteelMaterial::CStressStrain::LoadStream(CStorageNode::TSTREAM& stream, CStorageNode::TSTREAMVERSION& /*version*/, CStorageNode::TPROGRESS& progress)
{
  clear();
...
void CWellCasingSteelMaterial::CStressStrain::SaveStream(CStorageNode::TSTREAM& stream, CStorageNode::TPROGRESS& progress)
{
...
long CWellCasingSteelMaterial::CStressStrain::SavedItems() const
{
  return m_lstStressStrain.size();
}
...
void CWellCasingSteelMaterialEntry::LoadStream(CFemAppModel& /*model*/, TSTREAM& stream, CStreamVersion& version, TPROGRESS& progress)
{
  int nMat;
  stream >> nMat;
...
const QString DEFAULT_STEEL_MATERIAL_NAME =
  QObject::tr("New Steel Material %1");
