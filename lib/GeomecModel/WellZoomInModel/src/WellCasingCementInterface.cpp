...
CWellCasingCementInterface::CWellCasingCementInterface(const QString &strName, CWellCasingModel &model)
    : TBase(strName, model), m_bShowMidpoints(false), m_color(LIGHT_BROWN), m_sliptype(USER) {
  LinkTo(model.Mesh());
}

CWellCasingCementInterface::CWellCasingCementInterface(CWellCasingModel &model)
    : TBase(model), m_bShowMidpoints(false), m_color(LIGHT_BROWN), m_sliptype(USER) {
  LinkTo(model.Mesh());
}
...
unsigned int CWellCasingCementInterface::IconId() const { return IDI_WELLCASINGMODEL; }

unsigned int CWellCasingCementInterface::TypeId() const { return 0; }
...
bool CWellCasingCementInterface::Empty() const { return false; }

long CWellCasingCementInterface::SavedItems() const { return 0; }
...
CWellCasingCementInterface::TColor CWellCasingCementInterface::Color() const { return m_color; }

void CWellCasingCementInterface::Color(TColor nColor) { m_color = nColor; }
...
CWellCasingCementInterface::TSlipType CWellCasingCementInterface::SlipType() const { return m_sliptype; }
...
CWellCasingCementInterfaceMaterialServer::CWellCasingCementInterfaceMaterialServer(
    TWellCasingCementInterfaceMaterialServerParent &parent, const CDepletionStage &stage)
    : TBase(parent, stage) {
  reParent(&parent);
...
CInterfaceMaterial *CWellCasingCementInterfaceMaterialServer::FindIndexInEntry(int nIndex) const {
  CInterfaceMaterialEntry &material_entry =
      (CInterfaceMaterialEntry &)*((CModelBase &)Model()).GraphEntry(MD_BASE_INTERFACEMATERIAL);
