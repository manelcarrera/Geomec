...
CWellCasingNode::CWellCasingNode(CWellCasingModel &model)
    : COpenGLNode("Casing", model), m_pSteel(0), m_pCementInterface(0) {
  if (!model.Loading()) {
    ...
    CWellCasingSteelMaterialEntry &steel_entry =
        (CWellCasingSteelMaterialEntry &)(*model.GraphEntry(MD_WELLCASING_STEELMATERIAL_ENTRY));
    ...
    CWellCasingCementInterfaceMaterialServer &ifacematserver =
        m_pCementInterface->Material(model.InitialDepletionStage());
...
unsigned int CWellCasingNode::IconId() const { return IDI_WELLCASINGMODEL; }

unsigned int CWellCasingNode::TypeId() const { return 0; }

bool CWellCasingNode::Empty() const { return false; }
...
void CWellCasingNode::OnNewNeighbour(const CGraphNode &node) {
  ...
  if (pPressure)
    m_mpPressures.insert(
        TPressureMap::value_type(&pPressure->Stage(), const_cast<CWellCasingInternalPressure *>(pPressure)));

  const CWellCasingInternalTemperature *pTemperature = dynamic_cast<const CWellCasingInternalTemperature *>(&node);
  if (pTemperature)
    m_mpTemperatures.insert(TTemperatureMap::value_type(&pTemperature->Stage(),
                                                        const_cast<CWellCasingInternalTemperature *>(pTemperature)));
...
CWellCasingSteel &CWellCasingNode::Steel() { return *m_pSteel; }

const CWellCasingSteel &CWellCasingNode::Steel() const { return *m_pSteel; }

CWellCasingCementInterface &CWellCasingNode::CementInterface() { return *m_pCementInterface; }

const CWellCasingCementInterface &CWellCasingNode::CementInterface() const { return *m_pCementInterface; }
