...
: IWellMesh(sName, casingmodel),
  m_dCasingDiameter(0.1778), // 7"
  m_dCasingThickness(0.01),
  m_dHoleDiameter(0.25),
  m_nRadialElements(5),
  m_nTangentialElements(8),
  m_dTargetElementSize(2),
  m_pCementGroup(0),
  m_pSteelGroup(0),
  m_pCementInterfaceGroup(0)
, m_numberOfSteelElements(DEFAULT_NUMBER_OF_STEEL_ELEMENTS)
, m_numberOfCementElements(DEFAULT_NUMBER_OF_CEMENT_ELEMENTS)
, m_outerDiameterRock(2 * casingmodel.Radius())
{
}
...
bool CWellCasingMesh::CanCreateMesh() const
{
  return !IsMesh();
}

bool CWellCasingMesh::IsMesh() const
{
...
const QString CREATE_WELL_CASING_MESH =
  QObject::tr("Creating casing model mesh");
...
double CWellCasingMesh::CasingDiameter() const
{
  return m_dCasingDiameter;
}
...
double CWellCasingMesh::CasingThickness() const
{
  return m_dCasingThickness;
}
...
double CWellCasingMesh::HoleDiameter() const
{
  return m_dHoleDiameter;
}
...
int CWellCasingMesh::RadialElements() const
{
...
int CWellCasingMesh::TangentialElements() const
{
  return m_nTangentialElements;
}
...
double CWellCasingMesh::TargetElementSize() const
{
  return m_dTargetElementSize;
}
...
int CWellCasingMesh::NodesPerSpoke() const
{
  return 5 + m_nRadialElements;
}

int CWellCasingMesh::NodesPerGrid() const
{
  return NodesPerSpoke() * m_nTangentialElements;
}
...
int CWellCasingMesh::ElementsPerSegment() const
{
  return NodesPerSpoke() - 1;
}

int CWellCasingMesh::ElementsPerGrid() const
{
  return ElementsPerSegment() * m_nTangentialElements;
}

int CWellCasingMesh::NumberOfElementGrids() const
{
  return NumberOfGrids() - 1;
}

size_t CWellCasingMesh::numberOfSteelElements() const
{
  return m_numberOfSteelElements;
}
...
size_t CWellCasingMesh::numberOfCementElements() const
{
  return m_numberOfCementElements;
}
...
double CWellCasingMesh::outerDiameterRock() const
{
  return m_outerDiameterRock;
}
...
void CWellCasingMesh::CollectBoundaryNodes(CModelBase::TNodeElementsMap& mpNodeElements) const
{
  ...
  //int nNode = 2 * m_vcTopGrid.size() + nNodesPerSpoke - 2; // move to the one-but-last node in the spoke (not the outer one connected to the interface)
...
bool CWellCasingMesh::getInvalidateAfterDlg() const
{
  return m_bInvalidateAfterDlg;
}

void CWellCasingMesh::setInvalidateAfterDlg(bool invalidateAfterDlg)
{
...
bool addPoint(std::vector <geo::CPoint*>& vc2DGrid, IProgressBase& progress,
  double radius, double cosineAngle, double sineAngle)
{
...
void CWellCasingMesh::RegisterGridNodes(const T3DGrid& vc3DGrid, std::vector<int>& vcGridNodeIndices, bool bModelStart, bool bModelEnd)
{
...
geo::CHexahedron* CWellCasingMesh::createHexaElement(
  IWellModel::CBoundary& boundary, geo::CBodyGroup* bodyGroup,
  std::set <const geo::IElement*>* elements, const std::vector <int>& grid1,
  const std::vector <int>& grid2, const SegmentInfo& segmentinfo,
  const CFormationBase* formationBase, int firstSpoke, int secondSpoke,
...
void CWellCasingMesh::CreateHexaElements(const std::vector<int>& vcGrid1, const std::vector<int>& vcGrid2, double uppertmd, double lowertmd, TBoundaryElementVec& /*vcBoundaryElements*/, const SegmentInfo& segmentinfo)
{
  ...
  if(pVol)
  {
    ...
    for(i = 0; i < m_nTangentialElements; ++i)
    {
      ...
      // outer interface
      geo::CInterfaceElement& iface = CreateSideInterface(vcGrid1,
                                                          vcGrid2,
...
void CWellCasingMesh::LoadBodyGroup(geo::CBodyGroup& bodygroup, TSTREAM& stream, CStreamVersion& /*version*/, TPROGRESS& progress)
{
  int nBodySize;
  stream >> nBodySize;
  int i;
...
geo::CBodyGroup* CWellCasingMesh::findBodyGroupContainingMidPointOfHexahedron(
...
void CElementCenterPointCache::Clear()
{
  m_array.Clear();
}

bool CElementCenterPointCache::Empty() const
{
  return m_array.Empty();
}

const geo::CArray<geo::CElementPoint>& CElementCenterPointCache::Get() const
{
  return m_array;
}
