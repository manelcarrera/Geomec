...
CWellTetraMesher::CWellTetraMesher(const CModelBase &model, const std::vector<geo::CPoint> &well, double radius,
                                   double idealElementLength)
    : m_model(model), m_radius(radius), m_idealElementLength(idealElementLength) {
  CreatePolyLine(well);
}

CWellTetraMesher::~CWellTetraMesher() {}

const CModelBase &CWellTetraMesher::Model() const { return m_model; }

int CWellTetraMesher::NrOfJobs() const { return 4; }
...
double CWellTetraMesher::Radius() const { return m_radius; }

double CWellTetraMesher::IdealElementLength() const { return m_idealElementLength; }

int CWellTetraMesher::PolyLineSize() const {
  return static_cast<int>(std::round(2 * M_PI * Radius() / IdealElementLength()));
}

int CWellTetraMesher::PolyLineLength() const { return static_cast<int>(m_well.size()); }

int CWellTetraMesher::OriginalPolyLineLength() const { return static_cast<int>(m_originalWell.size()); }
...
geo::CMatrix CWellTetraMesher::Create3DGridTransformationMatrix(const geo::IPoint &point,
    ...
    for (size_t w = 0; w < surfPointSize; ++w) {
      ...
      for (size_t p = 0; p < m_surfaces[s].points[w].size(); ++p) {
        ...
        if (w == 0) {
          ...
          geo::CVector dir(geo::CPoint((w + 1) * factor, CalculateAHD(w + 1, m_surfaces[s].points[w + 1][p])),
                           newPoint);
...
void CWellTetraMesher::MapAngleAHDTo3DPoint(const geo::IPoint &inputPoint, geo::CPoint &outputPoint) const {
  ...
  double x_factor = (inputPoint.X() - m_2DSideSurface[size_index_0][length_index_0].point.X()) /
                    (m_2DSideSurface[size_index_1][length_index_1].point.X() -
                     m_2DSideSurface[size_index_0][length_index_0].point.X());
  double z_factor = 0;

  if (x_factor < 0.5)
    z_factor = (inputPoint.Y() - m_AHDGrid[size_index_0][length_index_0]) /
               (m_AHDGrid[size_index_0][length_index_0 + 1] - m_AHDGrid[size_index_0][length_index_0]);
  else
    z_factor = (inputPoint.Y() - m_AHDGrid[size_index_1][length_index_1]) /
               (m_AHDGrid[size_index_1][length_index_1 + 1] - m_AHDGrid[size_index_1][length_index_1]);

  size_index_1 = size_index_1 % wellSize;
  geo::CPoint x_top =
      m_3DGrid[size_index_0][length_index_0] +
      geo::CVector(m_3DGrid[size_index_0][length_index_0], m_3DGrid[size_index_1][length_index_1]) * x_factor;
  geo::CPoint x_bot =
      m_3DGrid[size_index_0][length_index_0 + 1] +
      geo::CVector(m_3DGrid[size_index_0][length_index_0 + 1], m_3DGrid[size_index_1][length_index_1 + 1]) * x_factor;

  outputPoint = x_top + geo::CVector(x_top, x_bot) * z_factor;
}

void CWellTetraMesher::Convert2DSurfaceToVectors(const CTriangleSurface &inputSurface, std::vector<geo::CPoint> &points,
...
void CWellTetraMesher::Check2DSideSurfaceArea(std::vector<geo::CPoint> &points,
                                              std::vector<std::vector<int>> &triangles) {
...
void CWellTetraMesher::Map2DSideSurfaceTo3DSideSurface(const std::vector<geo::CPoint> &points,
                                                       const std::vector<std::vector<int>> &triangles,
                                                       geo::CTriSurface &outputSurface) {
  int wellSize = PolyLineSize();
  int wellLength = PolyLineLength();
...
void CWellTetraMesher::Map2DGridSurfaceToWellPoint(int wellPointIndex, const CTriangleSurface &inputSurface,
                                                   geo::CTriSurface &outputSurface) {
  int wellLength = PolyLineLength();
...
void CWellTetraMesher::Map2DIntersectionSurfaceBoundaryTo3DSurface(const SurfaceInfo &surfInfo,
                                                                   TPolyline &outputBoundary) {
...
void CWellTetraMesher::Map3DIntersectionSurfaceBoundaryTo2DSurface(const SurfaceInfo &surfInfo,
                                                                   const TPolyline &inputBoundary,
                                                                   CTriangleSurface &outputSurface) {
...
void CWellTetraMesher::Map2DIntersectionSurfaceTo3DData(const CTriangleSurface &inputSurface,
                                                        const TPolyline &inputBoundary,
                                                        std::vector<geo::CPoint> &points,
                                                        std::vector<std::vector<int>> &triangles) {
...
bool CWellTetraMesher::TriangleInPointMap(const TTrianglePointMap &pointMap, const geo::IPoint &point1,
                                          const geo::IPoint &point2, const geo::IPoint &point3) {
  ...
  if (it != pointMap.end()) {
    ...
    if (it != pointMap.end()) {
      ...
      std::set_intersection(found.begin(), found.end(), it->second.begin(), it->second.end(),
...
void CWellTetraMesher::FilterCoincidingTriangles(const TTrianglePointMap &pointMap,
                                                 const std::vector<geo::CPoint> &inputPoints,
                                                 const std::vector<std::vector<int>> &inputTriangles,
                                                 std::vector<geo::CPoint> &points,
                                                 std::vector<std::vector<int>> &triangles) {
...
void CWellTetraMesher::CreateSurface(const std::vector<geo::CPoint> &points,
                                     const std::vector<std::vector<int>> &triangles, geo::CTriSurface &outputSurface) {
...
void CWellTetraMesher::FindBoundary(const geo::ISurface &surface, TPolyline &boundary) {
  ...
  do {
    ...
    if (found) {
      if ((item.first == first.first && item.second == first.second) ||
          (item.first == first.second && item.second == first.first))
        found = false;
      else
...
geo::CTetMeshBase *CWellTetraMesher::MeshVolume(const std::vector<geo::CTriSurface *> &surfaces, IProgressBase &prog) {
  ...
  geo::CTetMeshBase::TRet ret =
      mesh->CreateMesh(env->get(CEnvironment::DIAPATH).toStdString(), env->get(CEnvironment::DIASHARE).toStdString(),
...
void CWellTetraMesher::Debug_WritePointset(const std::string &fileName,
                                           const std::vector<std::vector<geo::CPoint>> &points) {
  std::vector<geo::CPoint> combined;
...
void CWellTetraMesher::Debug_WritePointset(const std::string &fileName,
                                           const std::vector<std::vector<GridPoint>> &points) {
  std::vector<geo::CPoint> combined;
  combined.reserve(1000);
...
void CWellTetraMesher::Debug_WriteSurface(const std::string &fileName, const std::vector<geo::CPoint> &points,
                                          const std::vector<std::vector<int>> &triangles) {
  FILE *fp = fopen(fileName.c_str(), "w");
  if (fp) {
    fprintf(fp, "GOCAD TSurf\nHEADER {\nname:surf\n}\nAXIS_UNIT\t\"m\"\t\"m\"\t\"m\"\nTFACE\n");
...
CWellTetraMesher::GridPoint::GridPoint() : surface(-1), index(-1), pointIndex(-1) {}

CWellTetraMesher::GridPoint::GridPoint(const geo::CPoint &point, int surface)
    : point(point), surface(surface), index(-1), pointIndex(-1) {}

CWellTetraMesher::SurfaceInfo::SurfaceInfo(const CHorizonBase *horizon, const CSurfaceBase *surface,
                                           const geo::CPoint &restrict_min, const geo::CPoint &restrict_max,
                                           int wellSize)
    : surfaceIndex(0), horizon(horizon), surface(surface), min(DBL_MAX, DBL_MAX, DBL_MAX),
      max(-DBL_MAX, -DBL_MAX, -DBL_MAX), empty(true) {
...
bool CWellTetraMesher::PointComparer::operator()(const CWellTetraMesher::GridPoint &p1,
                                                 const CWellTetraMesher::GridPoint &p2) const {
