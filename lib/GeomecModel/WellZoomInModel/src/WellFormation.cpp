...
CWellFormation::CWellFormation(CFemAppModel& model)
: C3DFormation(model)
{
}
...
{
  const CWellFormationEntry& entry = static_cast<const CWellFormationEntry&>(*Model().GraphEntry(MD_WELLMODEL_FORMATION));
...
long CWellFormation::SavedItems() const
{
  return C3DFormation::SavedItems() + 1;
}
...
void CWellFormation::LoadMaterial(CDepletionStage& stage, TSTREAM& stream, CStreamVersion& version, TPROGRESS& progress)
{
...
void CWellFormation::DuplicateLoads()
{
  ...
  myPressure.GWC().Set(parentPressure.GWC().ContactPressure().Value(),
                         ...
                         parentPressure.GWC().UpperGradient().Value(),
