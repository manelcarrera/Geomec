...
CWellCasingCement::CWellCasingCement(const QString &strName, CWellCasingModel &model)
    : TBase(strName, model), m_bShowMidpoints(false) {
  Color(DARK_GRAY);
  LinkTo(model.Mesh());
}

CWellCasingCement::CWellCasingCement(CWellCasingModel &model) : TBase(model), m_bShowMidpoints(false) {
  Color(DARK_GRAY);
  LinkTo(model.Mesh());
}

CWellCasingCement::~CWellCasingCement() { InvalidateCache(); }

unsigned int CWellCasingCement::IconId() const { return IDI_WELLCASINGMODEL; }

unsigned int CWellCasingCement::TypeId() const { return 0; }
...
bool CWellCasingCement::Less(const CGraphNode & /*node*/) const { return false; }
...
bool CWellCasingCement::Empty() const { return false; }
...
bool CWellCasingCement::CMaterialLess::operator()(const CWellCasingCementInitialMaterial *m1,
                                                  const CWellCasingCementInitialMaterial *m2) const {
  return (*m1) < (*m2);
}
