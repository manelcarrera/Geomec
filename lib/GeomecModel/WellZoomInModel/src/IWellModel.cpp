...
IWellModel::IWellModel(CAnalysisLogger &logger, const CVersionManager &versionManager)
    : CModelBase(logger, versionManager), m_dRadius(0), m_StartPosDef(*this, true), m_EndPosDef(*this, false),
      m_modelProxy(0) {
  setModelProxy(new CModelProxyConsole(*this));
}

IWellModel::IWellModel(const IWellModel &rhs)
    : CModelBase(const_cast<CAnalysisLogger &>(rhs.Logger()), rhs.getVersionManager()), m_dRadius(rhs.m_dRadius),
      m_StartPosDef(rhs.m_StartPosDef), m_EndPosDef(rhs.m_EndPosDef), m_modelProxy(0) {
  setModelProxy(new CModelProxyConsole(*this));
}
...
bool IWellModel::operator==(const IWellModel &rhs) const {
  return (fabs(m_dRadius - rhs.m_dRadius) < EPS && m_StartPosDef == rhs.m_StartPosDef &&
          m_EndPosDef == rhs.m_EndPosDef);
}
...
const double &IWellModel::Depth(const geo::ICoordinate &coord) const { return coord.Z(); }

double IWellModel::Northing(const geo::ICoordinate &coord) const { return coord.X(); }

double IWellModel::Easting(const geo::ICoordinate &coord) const { return coord.Y(); }
...
CModelBase &IWellModel::RootModel() { return const_cast<CModelBase &>(((const IWellModel *)this)->RootModel()); }

bool IWellModel::CanCalculateHeatFlow() const { return false; }

bool IWellModel::CanCalculateMixture() const { return false; }

bool IWellModel::CanUseCalculatedTemperatures() const { return false; }
...
bool IWellModel::OnLoad(CStorageNode::TSTREAM &stream, CStreamVersion &version, CStorageNode::TPROGRESS &prog) {
  ...
  // load formation map
  TFormationBaseEntry &parent_formation_entry =
      static_cast<TFormationBaseEntry &>(*ParentModel().GraphEntry(MD_BASE_FORMATION));
  int nSize;
  stream >> nSize;
  int i;
  for (i = 0; i < nSize; ++i) {
...
double IWellModel::Radius() const { return m_dRadius; }
...
bool IWellModel::Empty() const { return false; }
...
const IWellModel::CPositionDef &IWellModel::StartPosition() const { return m_StartPosDef; }

IWellModel::CPositionDef &IWellModel::StartPosition() { return m_StartPosDef; }

const IWellModel::CPositionDef &IWellModel::EndPosition() const { return m_EndPosDef; }

IWellModel::CPositionDef &IWellModel::EndPosition() { return m_EndPosDef; }
...
double IWellModel::FirstValidAHD() const {
  ...
  if (WellPath()) {
    const well::CWellPointList &lstFormationPoints = const_cast<CWellPath *>(WellPath())->FormIntersecPoints();
...
double IWellModel::LastValidAHD() const {
  ...
  if (WellPath()) {
    const well::CWellPointList &lstFormationPoints = WellPath()->FormIntersecPoints();
...
const CFormationBase *IWellModel::FormationContaining(double dAHD) const {
  ...
  {
    const std::list<CNewFormationSection> &sectionList = const_cast<CNewWellPath *>(NewWellPath())->FormationSections();
...
// wjrx mantis 3401
const CNewFormationSection *IWellModel::NewFormationSection(const CFormationBase &formation) const {
  assert(WellPath() == 0);
  assert(NewWellPath() != 0);
  if (NewWellPath() == 0)
    return 0;
    ...
    while (it != sectionlist.end()) {
      ...
      if (pFormation && stProcessed.insert(pFormation).second) {
        ...
        if (valStartAHD.Valid() && valEndAHD.Valid()) {
          ...
          bInParentModel =
              (!(dSectionEndAHD < valStartAHD.Value() + EPS) && !(dSectionStartAHD > valEndAHD.Value() - EPS));
        }
...
IWellModel::ModelType IWellModel::modelType() const { return eWellModel; }
...
void IWellModel::CreateBoundary() { m_pBoundary = new CBoundary(*this); }
...
void IWellModel::DetectFormations() {
  ...
  if (WellPath()) {
    ...
    while (its != lstFormationSections.end()) {
      ...
      if (it == m_mpFormations.end())
        m_mpFormations.insert(
            TFormationMap::value_type(pForm, new CWellFormation(*pForm, MD_WELLMODEL_FORMATION, *this)));

      ++its;
    }
  } else // wjrx mantis 3401
  {
    const std::list<CNewFormationSection> &sectionList = NewWellPath()->FormationSections();
...
void IWellModel::addFormation(const CFormationBase *formation) {
  m_mpFormations.insert(
      TFormationMap::value_type(formation, new CWellFormation(*formation, MD_WELLMODEL_FORMATION, *this)));
}

/////

IWellModel::CPositionDef::CPositionDef(IWellModel &model, bool bStart)
    : CGraphNode(""), m_model(model), m_nMethod(PDM_DEFAULT), m_dAHD(0), m_pFormation(0), m_bStart(bStart) {
  //  SetDefault();
  LinkTo(model);
}

IWellModel::CPositionDef::CPositionDef(const CPositionDef &rhs)
    : CGraphNode(""), m_model(rhs.m_model), m_nMethod(rhs.m_nMethod), m_dAHD(rhs.m_dAHD),
      m_pFormation(rhs.m_pFormation), m_bStart(rhs.m_bStart) {}
...
bool IWellModel::CPositionDef::operator==(const CPositionDef &rhs) const {
  return (&m_model == &rhs.m_model && m_nMethod == rhs.m_nMethod && m_dAHD == rhs.m_dAHD && m_bStart == rhs.m_bStart &&
          m_pFormation == rhs.m_pFormation);
}
...
IWellModel::CPositionDef::TMethod IWellModel::CPositionDef::Method() const { return m_nMethod; }

double IWellModel::CPositionDef::AHD() const { return m_dAHD; }

bool IWellModel::CPositionDef::InsideFormation() const { return m_pFormation != 0; }
...
void IWellModel::CPositionDef::LoadStream(CStorageNode::TSTREAM &stream, CStreamVersion & /*version*/,
...
IWellModel::CBoundary::~CBoundary() { delete m_pBoundaryInterfaceDef; }

IWellModel::CBoundary::TMinMax IWellModel::CBoundary::SnapToGrid(const TMinMax &minmax) const { return minmax; }
...
int IWellModel::CBoundary::DisplayListSize() const { return 1; }
...
void IWellModel::CBoundary::AddBoundaryElement(geo::CInterfaceElement &iface,
...
const CBoundaryInterfaceDef &IWellModel::CBoundary::InterfaceDefinition() const { return *m_pBoundaryInterfaceDef; }

int IWellModel::CBoundary::BoundaryElementSize() const { return m_arBoundaryElements.Size(); }
...
CBoundaryInterfaceDef::TBoundarySurface
IWellModel::CBoundary::BoundarySurfaceDef(const geo::CInterfaceElement &iface) const {
...
bool IWellModel::CBoundary::HasInterfaces() const { return true; }
...
IWellModel::CBoundary::IWellSupportNode::IWellSupportNode(unsigned int uName, IWellModel::CBoundary &boundary)
    : CBaseSupportNode(uName, boundary), m_boundary(boundary) {}

unsigned int IWellModel::CBoundary::IWellSupportNode::TypeId() const { return 0; }

bool IWellModel::CBoundary::IWellSupportNode::Empty() const { return false; }

int IWellModel::CBoundary::IWellSupportNode::DisplayListSize() const { return m_boundary.DisplayListSize(); }
