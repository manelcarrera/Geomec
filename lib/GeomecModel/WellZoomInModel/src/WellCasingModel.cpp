...
CWellCasingModel::CWellCasingModel(CWellZoomInModel &parent, CAnalysisLogger &logger,
                                   const CVersionManager &versionManager)
    : IWellModel(logger, versionManager), m_parent(parent), m_pMesh(0), m_pCasingNode(0), m_pCasingCement(0),
      m_deformation(0) {
  Radius(10);
...
CWellCasingModel::CWellCasingModel(const CWellCasingModel &rhs)
    : IWellModel(rhs), m_parent(rhs.m_parent), m_pMesh(rhs.m_pMesh), // shallow copy of the mesh
      m_deformation(0) {}
...
bool CWellCasingModel::operator==(const CWellCasingModel & /*rhs*/) const { return (true); }
...
unsigned int CWellCasingModel::ModelIconId() const { return IDI_WELLCASINGMODEL; }

unsigned int CWellCasingModel::TypeId() const { return 0; }

QString CWellCasingModel::documentType() const { return "Casing Model"; }
...
CGeomecDianaRunnerBase *CWellCasingModel::OnCreateDianaRunner(CDianaRunController &controller) {
  ...
  case CAnalysisType::AT_MIXTURE:
    return new CWellCasingMixtureDianaRunner(*this, controller);
  case CAnalysisType::AT_MIXTURE_CONTAINMENT:
    return new CWellCasingMixtureContainmentDianaRunner(*this, controller);
  case CAnalysisType::AT_HEAT:
    return new CWellCasingHeatFlowDianaRunner(*this, controller);
  default:
    return new CWellCasingDianaRunner(*this, controller);
  }
}

const double &CWellCasingModel::Depth(const geo::ICoordinate &coord) const { return coord.Z(); }

double CWellCasingModel::Northing(const geo::ICoordinate &coord) const { return coord.X(); }

double CWellCasingModel::Easting(const geo::ICoordinate &coord) const { return coord.Y(); }

bool CWellCasingModel::IsResult() const { return ResultRegister().Linear() || ResultRegister().NonLinear(); }

void CWellCasingModel::Calculate() {}

int CWellCasingModel::Dimension() const { return 3; }

const CWellZoomInModel &CWellCasingModel::ParentModel() const { return m_parent; }

CWellZoomInModel &CWellCasingModel::ParentModel() { return m_parent; }

CWellCasingMesh &CWellCasingModel::Mesh() { return *m_pMesh; }

const CWellCasingMesh &CWellCasingModel::Mesh() const { return *m_pMesh; }
...
bool CWellCasingModel::CanCalculateMixture() const { return CanCalculate(); }

bool CWellCasingModel::CanCalculateHeatFlow() const { return true; }
...
bool CWellCasingModel::LoadWellCasing(CStorageNode::TSTREAM &stream, CStreamVersion &version,
...
CWellPath *CWellCasingModel::WellPath() { return m_parent.WellPath(); }

const CWellPath *CWellCasingModel::WellPath() const { return m_parent.WellPath(); }

CNewWellPath *CWellCasingModel::NewWellPath() { return m_parent.NewWellPath(); }

const CNewWellPath *CWellCasingModel::NewWellPath() const { return m_parent.NewWellPath(); }

CWellCasingNode &CWellCasingModel::CasingNode() { return *m_pCasingNode; }

const CWellCasingNode &CWellCasingModel::CasingNode() const { return *m_pCasingNode; }

CWellCasingCement &CWellCasingModel::CasingCement() { return *m_pCasingCement; }

const CWellCasingCement &CWellCasingModel::CasingCement() const { return *m_pCasingCement; }
...
int CWellCasingModel::childModelSize() const { return 0; }
...
CModelBase *CWellCasingModel::parentModel() { return &ParentModel(); }

CWellCasingModel::ModelType CWellCasingModel::modelType() const { return eWellCasingModel; }

QString CWellCasingModel::AttributesDialogCaption() const { return "Wellpath Casing Model Attributes"; }
