...
CWellZoomInMesh::CWellZoomInMesh(const QString &sName, CWellZoomInModel &zoominmodel)
    : IWellMesh(sName, zoominmodel), m_zoominmodel(zoominmodel), m_nRadialElements(nDefaultRadialElements),
      m_dLongElementSize(zoominmodel.Radius()), m_isHexaMesh(true), m_tetraRadius(zoominmodel.Radius()),
      m_tetraIdealElementSize(10) {
  LinkTo(zoominmodel);
}

CWellZoomInMesh::CWellZoomInMesh(const CWellZoomInMesh &rhs)
    : IWellMesh(rhs), m_zoominmodel(rhs.m_zoominmodel), m_nRadialElements(rhs.m_nRadialElements),
      m_dLongElementSize(rhs.m_dLongElementSize), m_isHexaMesh(rhs.m_isHexaMesh), m_tetraRadius(rhs.m_tetraRadius),
      m_tetraIdealElementSize(rhs.m_tetraIdealElementSize) {}

CWellZoomInMesh::~CWellZoomInMesh() { ClearDefinitionPointList(); }
...
bool CWellZoomInMesh::operator==(const CWellZoomInMesh &rhs) const {
  return (&m_zoominmodel == &rhs.m_zoominmodel && m_nRadialElements == rhs.m_nRadialElements &&
          m_dLongElementSize == rhs.m_dLongElementSize && m_isHexaMesh == rhs.m_isHexaMesh &&
          m_tetraRadius == rhs.m_tetraRadius && m_tetraIdealElementSize == rhs.m_tetraIdealElementSize);
}

bool CWellZoomInMesh::CanCreateMesh() const { return !IsMesh(); }

bool CWellZoomInMesh::IsMesh() const { return !m_bMeshing && Mesh().ElementSize() > 0; }
...
bool CWellZoomInMesh::Empty() const { return false; }
...
const CWellZoomInModel &CWellZoomInMesh::ZoomInModel() const { return m_zoominmodel; }

CWellZoomInModel &CWellZoomInMesh::ZoomInModel() { return m_zoominmodel; }

int CWellZoomInMesh::NrOfRadialElements() const { return m_nRadialElements; }
...
double CWellZoomInMesh::TargetLongitudinalElementSize() const { return m_dLongElementSize; }
...
bool CWellZoomInMesh::isHexaMesh() const { return m_isHexaMesh; }

bool CWellZoomInMesh::isTetraMesh() const { return !m_isHexaMesh; }

double CWellZoomInMesh::TetraRadius() const { return m_tetraRadius; }
...
double CWellZoomInMesh::TetraIdealElementSize() const { return m_tetraIdealElementSize; }
...
void CWellZoomInMesh::LoadStream(TSTREAM &stream, CStreamVersion &version, TPROGRESS &progress) {
  ...
  if (nIsMesh != 0) {
    ...
    for (i = 0; i < nIface; ++i) {
      ...
      boundary.AddBoundaryElement(
          *new geo::CInterfaceElement(Mesh(), &parent.Face(nFaceIndex), &parent.Face(nFaceIndex), vcNode),
    ...
    for (i = 0; i < nSize; ++i) {
      ...
      m_mpBody2MidPoint.insert(TBody2MidPointMap::value_type(static_cast<const geo::IBody *>(&Mesh().Element(nIndex)),
                                                             geo::CPoint(x, y, z)));
      progress.Step();
    }
...
void CWellZoomInMesh::SaveStream(TSTREAM &stream, TPROGRESS &progress) {
  ...
  if (IsMesh()) {
    ...
    for (i = 0; i < nHexaTetra; ++i) {
      ...
      assert((isHexaMesh() && dynamic_cast<const geo::CHexahedron *>(&el)) ||
             (isTetraMesh() && dynamic_cast<const geo::CTetrahedron *>(&el)));
...
void CWellZoomInMesh::CreateHexaElements(const std::vector<int> &vcGrid1, const std::vector<int> &vcGrid2,
  ...
  if (pVol) {
    ...
    for (nXIndex = 0; nXIndex < sz; ++nXIndex) {
      for (nYIndex = 0; nYIndex < sz; ++nYIndex) {
        ...
        geo::CBodyGroup *pActualVol =
            findBodyGroupContainingMidPointOfHexahedron(m_zoominmodel, pVol, pFormation, pHexa);
        ...
        if (segmentinfo.bFormationTop) {
          ...
          if (segmentinfo.bModelStart) {
            ...
            vcBoundaryElements.push_back(
                CBoundaryElementDef(pHexa->Face(0), vcInterfaceNode, CBoundaryInterfaceDef::BSURF_TOP));
          }
        }
        ...
        if (segmentinfo.bFormationBottom) {
          ...
          if (segmentinfo.bModelEnd) {
            ...
            assert(vcInterfaceNode[4] != -1);
            vcBoundaryElements.push_back(
                CBoundaryElementDef(pHexa->Face(5), vcInterfaceNode, CBoundaryInterfaceDef::BSURF_BOTTOM));
          }
        }
        ...
        if (nXIndex == 0) // left
        {
          ...
          vcBoundaryElements.push_back(
              CBoundaryElementDef(pHexa->Face(2), vcInterfaceNode, CBoundaryInterfaceDef::BSURF_SIDE));
        } else if (nXIndex == sz - 1) // right
        {
          ...
          vcBoundaryElements.push_back(
              CBoundaryElementDef(pHexa->Face(4), vcInterfaceNode, CBoundaryInterfaceDef::BSURF_SIDE));
        }
        ...
        if (nYIndex == 0) // top
        {
          ...
          vcBoundaryElements.push_back(
              CBoundaryElementDef(pHexa->Face(1), vcInterfaceNode, CBoundaryInterfaceDef::BSURF_SIDE));
        } else if (nYIndex == sz - 1) // bottom
        {
          ...
          vcBoundaryElements.push_back(
              CBoundaryElementDef(pHexa->Face(3), vcInterfaceNode, CBoundaryInterfaceDef::BSURF_SIDE));
        }
      }
    }
  }
...
bool CWellZoomInMesh::getInvalidateAfterDlg() const { return m_bInvalidateAfterDlg; }

void CWellZoomInMesh::setInvalidateAfterDlg(bool invalidateAfterDlg) { m_bInvalidateAfterDlg = invalidateAfterDlg; }

void CWellZoomInMesh::RegisterGridNodes(const T3DGrid &vc3DGrid, std::vector<int> &vcGridNodeIndices, bool bModelStart,
                                        bool bModelEnd) {
  ...
  for (i = 0; i < vc3DGrid.second.size(); ++i) {
    ...
    bool bBoundary = (i < rowlength ||                    // left edge
                      ...
                      bModelStart || bModelEnd);

    if (bBoundary) {
      int iBoundaryNode =
          Mesh().RegisterNode(vc3DGrid.second[i], false); // duplicate node at that position (bUnique = false)
...
bool CWellZoomInMesh::CheckCurvature() {
  ...
  if (pWellPath) {
    ...
    if (dMaxSquaredRadius + EPS < (dRadius * dRadius)) {
      ...
      msg = QString("Due to the curvature of the well the maximum allowable radius is %1 %2.\nThe current radius is %3 "
                    "%4.\n\nWould you like the model's radius to be adapted?")
                .arg(lqmax.Value(unittype), 0, 'f', 2)
                .arg(lqmax.UnitName(unittype).c_str())
                .arg(lqcur.Value(unittype), 0, 'f', 2)
                .arg(lqcur.UnitName(unittype).c_str());
