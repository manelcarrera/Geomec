...
: CMeshBase(sName, model.DepletionStageEntry())
, m_bMeshing(false)
, m_mesh(new geo::CMesh3D)
{
}

IWellMesh::IWellMesh(const IWellMesh& rhs)
  : CMeshBase(rhs)
  , m_bMeshing(false)
  , m_mesh(new geo::CMesh3D)
{
...
bool IWellMesh::operator==(const IWellMesh& /*rhs*/) const
{
  return true;
}

unsigned int IWellMesh::IconId() const
{
  return IDI_MESH;
}

unsigned int IWellMesh::TypeId() const
{
  return 0;
}
...
const CHorizonBase* IWellMesh::SlipHorizon(const geo::IElement& /*element*/) const
{
  return 0;
}

const geo::IMesh& IWellMesh::Mesh() const
{
  return *m_mesh;
}

geo::IMesh& IWellMesh::Mesh()
{
  return *m_mesh;
}
...
int IWellMesh::DefinitionPointSize() const
{
  return m_vcDefPoints.size();
}

int IWellMesh::NewDefinitionPointSize() const
{
  return m_vcNewDefPoints.size();
}
...
const QString FORMATION_WITHOUT_HORIZONS =
  QObject::tr("encountered a formation without horizons");

CNoHorizonFound::CNoHorizonFound()
: std::runtime_error(FORMATION_WITHOUT_HORIZONS.toStdString())
{
}
...
IWellMesh::TFormations collectAllFormations(const CFemAppModel& femAppModel,
  std::vector <IWellMesh::T3DGrid>& grid)
{
...
void IWellMesh::CreateTetraMesh(CModelBase& model, const std::vector<geo::CPoint>& points, double radius, double eltSize)
{
  m_bMeshing = true;

  CWellTetraMesher mesher(model, points, radius, eltSize);

  std::auto_ptr <IProgressBase> prog(_g->prog()->create(eProgress::Dual, "Create well zoom-in tetra mesh", true, 3 + mesher.NrOfJobs()));
...
geo::CMatrix IWellMesh::Create3DGridTransformationMatrix(const geo::IPoint& point, const geo::IVector& vecNormal) const
{
...
geo::CBodyGroup* IWellMesh::getBodyGroup4Formation(IWellModel& wellModel,
...
geo::CBodyGroup* IWellMesh::findBodyGroupContainingMidPointOfHexahedron(
