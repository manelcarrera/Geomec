...
public:
  ...
  public:
    typedef enum { PDM_DEFAULT = 0, PDM_AHD, PDM_FORMATION } TMethod;
...
public:
  ...
  CWellFormation *AssociatedFormation(const CFormationBase &formation); // returns 0 if no formation is associated
                                                                        // (zoom-in model doesn't run through it)
...
private:
  ...
  const CNewFormationSection *NewFormationSection(const CFormationBase &formation) const;
