...
public:
  ...
  virtual const CFFMaterial *getCFFMaterial(const geo::IElement &element, const CDepletionStage &depletionStage) const;
...
private:
  geo::CHexahedron *createHexaElement(IWellModel::CBoundary &boundary, geo::CBodyGroup *bodyGroup,
                                      std::set<const geo::IElement *> *elements, const std::vector<int> &grid1,
                                      const std::vector<int> &grid2, const SegmentInfo &segmentinfo,
                                      const CFormationBase *formationBase, int firstSpoke, int secondSpoke, bool first,
                                      bool last, bool top, bool bottom, bool useActualBodyGroup);
  ...
  THexahedronBodyGroupPair CreateHexahedron(const std::vector<int> &vcGrid1, const std::vector<int> &vcGrid2,
                                            int idxStartFirstSpoke, int idxStartSecondSpoke,
                                            std::set<const geo::IElement *> *pstElements, geo::CBodyGroup *BodyGroup,
                                            const CFormationBase *formationBase);
  geo::CInterfaceElement &CreateCapInterface(const std::vector<int> &vcGrid1, const std::vector<int> &vcGrid2,
                                             int idxStartFirstSpoke, int idxStartSecondSpoke, const geo::IFace *pFront,
                                             const geo::IFace *pBack, std::set<const geo::IElement *> &stElements);
  geo::CInterfaceElement &CreateSideInterface(const std::vector<int> &vcGrid1, const std::vector<int> &vcGrid2,
                                              int idxStartFirstSpoke, int idxStartSecondSpoke, const geo::IFace *pFront,
                                              const geo::IFace *pBack, std::set<const geo::IElement *> &stElements);
...
public:
  typedef enum { SS_TOP = 0, SS_BOTTOM = 1, SS_INNER = 2, SS_OUTER = 3 } TSideSurface;
