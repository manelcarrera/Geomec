...
class CWellTetraMesher {
  ...
  struct SurfaceInfo {
    ...
    SurfaceInfo(const CHorizonBase *horizon, const CSurfaceBase *surface, const geo::CPoint &restrict_min,
                const geo::CPoint &restrict_max, int wellSize);
...
public:
  CWellTetraMesher(const CModelBase &model, const std::vector<geo::CPoint> &well, double radius,
...
private:
  ...
  void Convert2DSurfaceToVectors(const CTriangleSurface &inputSurface, std::vector<geo::CPoint> &points,
                                 std::vector<std::vector<int>> &triangles);
  void Check2DSideSurfaceArea(std::vector<geo::CPoint> &points, std::vector<std::vector<int>> &triangles);
  void Map2DSideSurfaceTo3DSideSurface(const std::vector<geo::CPoint> &points,
  ...
  void Map2DGridSurfaceToWellPoint(int wellPointIndex, const CTriangleSurface &inputSurface,
  ...
  void Map3DIntersectionSurfaceBoundaryTo2DSurface(const SurfaceInfo &surfInfo, const TPolyline &inputBoundary,
                                                   CTriangleSurface &outputSurface);
  void Map2DIntersectionSurfaceTo3DData(const CTriangleSurface &inputSurface, const TPolyline &inputBoundary,
                                        std::vector<geo::CPoint> &points, std::vector<std::vector<int>> &triangles);
  ...
  bool TriangleInPointMap(const TTrianglePointMap &pointMap, const geo::IPoint &point1, const geo::IPoint &point2,
                          const geo::IPoint &point3);
  void FilterCoincidingTriangles(const TTrianglePointMap &pointMap, const std::vector<geo::CPoint> &inputPoints,
                                 const std::vector<std::vector<int>> &inputTriangles, std::vector<geo::CPoint> &points,
                                 std::vector<std::vector<int>> &triangles);
  void CreateSurface(const std::vector<geo::CPoint> &points, const std::vector<std::vector<int>> &triangles,
                     geo::CTriSurface &outputSurface);
...
private:
  ...
  void Debug_WriteSurface(const std::string &fileName, const std::vector<geo::CPoint> &points,
