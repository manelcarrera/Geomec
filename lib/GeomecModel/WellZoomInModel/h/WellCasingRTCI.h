...
public:
  ...
  typedef enum { RD_CLOCKWISE, RD_ANTICLOCKWISE } TRtciDir;

  class CSegmentInfo : public geo::ILine {
  public:
    CSegmentInfo(int nIndex, const geo::CHexahedron &hexa, double dStart, double dEnd, const geo::IPoint &ptStart,
                 const geo::IPoint &ptEnd, double dStartAHD, double dEndAHD);
    ~CSegmentInfo();
  ...
  private:
    void AxialStrain(const CStrainTensorValueSet &strain, const geo::CPoint &pt,
                     const geo::IElement::TDoubleVec &vcIsoCoords, geo::CValue &val) const;
