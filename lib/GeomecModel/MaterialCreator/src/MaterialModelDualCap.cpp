...
void CMaterialDualCapCreator::CPQPlotter::GetSecondaryYieldCurve(const CLibraryMaterial& mat, double dPMin, double dPMax, CStressStrainArray& values) const
{
  const ml::CMatParam *pPrecon = mat.Parameter(MLD_SEC_PRECON);
  GetYieldCurve(mat, pPrecon->Value(), dPMin, dPMax, values);
}

bool CMaterialDualCapPreconsolidationCheckStrategy::operator()(double dValue, const ml::CMatParam& param, QString& strErrorMsg, int nUnitDef) const
{
...
bool CMaterialDualCapSecondaryPreconCheckStrategy::operator()(double dValue, const ml::CMatParam& param, QString& strErrorMsg, int nUnitDef) const
{
...
static bool ElasticValue(const double& dPoison, const double& dPrecon, const double& dPorosity, const double& dOther, double& dValue)
{
