...
void CMaterialCreator::OnCreateParameters(ml::CMaterial& mat)
{
  ...
  AddNumber(*pLibMat, 1);
  AddLibParam(MLD_DUMMY, 0, *pLibMat, f->getMPUnitType(CMaterialHelperFactory::UT_NONE), IDT_VALUETYPE_DUMMY, 0, 0, QString(), false);
}
...
void CMaterialCreator::AddNumber(CLibraryMaterial& /*parentMat*/,
  size_t /*number*/)
{
}

void CMaterialCreator::AddLibParam(const QString& strName,
                   double dValue,
                   CLibraryMaterial& parentMat,
                   const ml::CMatParam::CUnitType& unitConversion,
                   unsigned int uiValueTypeID,
                   const ml::CMatParam::CCheckStrategy* pCheckStrategy,
                   const ml::CMatParam::CSetStrategy* pSetStrategy,
