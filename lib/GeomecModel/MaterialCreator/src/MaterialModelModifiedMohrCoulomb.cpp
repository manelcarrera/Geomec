...
void CMaterialModifiedMohrCoulombCreatorV2::OnCreateLibraryParameters(CLibraryMaterial &m) {
  ...
  AddLibParam(MLD_FRICTIONANGLE, DEF_FRICTION, m, f->getMPUnitType(CMaterialHelperFactory::UT_ANGLE),
              IDT_VALUETYPE_FRICTION_ANGLE, f->getMPCheckStrategy(CMaterialHelperFactory::MP_GE_0_LT_70), 0,
  ...
  AddLibParam(MLD_PRECONSOLIDATION, DEF_PRECONSO, m, f->getMPUnitType(CMaterialHelperFactory::UT_STRESS),
              IDT_VALUETYPE_PRECONSOLIDATION, f->getMPCheckStrategy(CMaterialHelperFactory::MP_GT_0), 0,
              PRECON_FILOSPATH, false);
  AddLibParam(MLD_HARDENING, DEF_HARDENING, m, f->getMPUnitType(CMaterialHelperFactory::UT_NONE),
              IDT_VALUETYPE_HARDENING, f->getMPCheckStrategy(CMaterialHelperFactory::MP_GT_0_LE_1), 0, LAMBDA_FILOSPATH,
              false);
  AddLibParam(MLD_POROSITY, DEF_PORO, m, f->getMPUnitType(CMaterialHelperFactory::UT_NONE), IDT_VALUETYPE_POROSITY,
              f->getMPCheckStrategy(CMaterialHelperFactory::MP_GE_0_LT_1), 0, POROSI_FILOSPATH, true);
  ...
  AddLibParam(MLD_CAPSHAPEPARAM, DEF_CAPSHAPEPARAM, m, f->getMPUnitType(CMaterialHelperFactory::UT_NONE),
              IDT_VALUETYPE_CAPSHAPE, f->getMPCheckStrategy(CMaterialHelperFactory::MP_GE_1Emin1_LE_2), 0,
              ALPHA_FILOSPATH, false);
...
int CMaterialModifiedMohrCoulombCreatorV2::MaterialModel() { return MM_MODIFIEDMOHRCOULOMB; }

QString CMaterialModifiedMohrCoulombCreatorV2::MaterialModelName() { return MLD_MODIFIEDMOHRCOULOMB; }

QString CMaterialModifiedMohrCoulombCreatorV2::CalibrationPath() const { return "MMOHRC"; }
...
void CMaterialModifiedMohrCoulombCreatorV2::CPQPlotter::GetPrimaryYieldCurve(const CLibraryMaterial &mat,
  ...
  double dPInt =
      ((-dDeltaP / dCapShape) - k * m +
       sqrt(pow((dDeltaP / dCapShape) + k * m, 2) - (m * m + 1 / dCapShape) * (k * k + (dDeltaP * dDeltaP) / dCapShape -
                                                                               (dPreConso * dPreConso) / dCapShape))) /
      (m * m + 1 / dCapShape);
...
void CMaterialModifiedMohrCoulombCreatorDEPRECATED::OnCreateLibraryParameters(CLibraryMaterial &m) {
  ...
  Add(new CLibraryMaterialParameter(MLD_FRICTIONANGLE, DEF_FRICTION, m, *new CUnitTypeAngle,
                                    IDT_VALUETYPE_FRICTION_ANGLE, new MP_GE_LT(0, 70), 0, FRICTI_FILOSPATH, false));
  ...
  Add(new CLibraryMaterialParameter(MLD_VISCOSITY, DEF_VISCOSITY, m, *new CUnitTypeViscosity, IDT_VALUETYPE_VISCOSITY,
                                    new MP_GT(0), 0));
  Add(new CLibraryMaterialParameter(MLD_CAPSHAPEPARAM, DEF_CAPSHAPEPARAM, m, *new CUnitTypeNone, IDT_VALUETYPE_CAPSHAPE,
                                    new MP_GE_LE(0.1, 2), 0, ALPHA_FILOSPATH, false));
...
int CMaterialModifiedMohrCoulombCreatorDEPRECATED::MaterialModel() { return MM_MODIFIEDMOHRCOULOMB; }

QString CMaterialModifiedMohrCoulombCreatorDEPRECATED::MaterialModelName() { return MLD_MODIFIEDMOHRCOULOMB; }

QString CMaterialModifiedMohrCoulombCreatorDEPRECATED::CalibrationPath() const { return "MMOHRC"; }

IPQPlotter *CMaterialModifiedMohrCoulombCreatorDEPRECATED::PQPlotter() const { return new CPQPlotter; }

IPQPlotter *CMaterialModifiedMohrCoulombCreatorDEPRECATED::CPQPlotter::Clone() const { return new CPQPlotter; }

#ifdef min
#undef min // To give std::min precedence over #define min(...)
#endif     // min

void CMaterialModifiedMohrCoulombCreatorDEPRECATED::CPQPlotter::GetPrimaryYieldCurve(const CLibraryMaterial &mat,
                                                                                     double /*dPMin*/, double dPMax,
                                                                                     CStressStrainArray &values) const {
  const ml::CMatParam *pPhi = mat.Parameter(MLD_FRICTIONANGLE);
  const ml::CMatParam *pCohesion = mat.Parameter(MLD_COHESION);
