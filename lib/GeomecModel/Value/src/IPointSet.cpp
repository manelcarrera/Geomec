...
static TVTPair prVt[] =
{
  { IDT_VALUETYPE_ANISOTROPIC_VP, IDT_VALUETYPE_VELOCITYP },
...
IPointSet::IPointSet(const QString& sName, CFemAppModel& model, CQuantity::UNIT coordinate_unit, CQuantity::UNIT value_unit)
...
: CColorNode(rhs)
, m_mpUnitConversion(rhs.m_mpUnitConversion)
...
: CColorNode(rhs.Name().toStdString().c_str(), model), 
  m_value_unit(rhs.m_value_unit), 
  m_zaxis(rhs.m_zaxis), 
  m_coordinate_unit(rhs.m_coordinate_unit)
{
...
int IPointSet::NodalValueSetSize() const
{
  return m_vcNodalValueSet.size();
}

bool IPointSet::Overlap(const IPointSet& point_set) const
{
  return (Min().X() < point_set.Max().X()) &&
       (Min().Y() < point_set.Max().Y()) &&
       (Min().Z() < point_set.Max().Z()) &&
       (point_set.Min().X() < Max().X()) &&
       (point_set.Min().Y() < Max().Y()) &&
       (point_set.Min().Z() < Max().Z());
}
...
int IPointSet::RpnValueSetSize() const
{
  return m_vcRpnValueSet.size();
}

size_t IPointSet::UnitConversionSize() const
{
  return m_mpUnitConversion.size();
}

bool IPointSet::AnyUnitConversionDefined() const
{
  return UnitConversionSize() != 0;
}
...
void IPointSet::RemoveUnitConversion( const std::string &name)
{
  ...
  if ( it == m_mpUnitConversion.end() ) return;
...
CQuantity::UNIT IPointSet::CoordinateUnit() const
{
  return m_coordinate_unit;
}
...
void IPointSet::ValueUnit(CQuantity::UNIT unit)
{
  m_value_unit = unit;
}
...
int IPointSet::ValueSetSize() const
{
  return NodalValueSetSize() + RpnValueSetSize();
}
...
IPointSet::Z_AXIS IPointSet::ZAxis() const
{
  return m_zaxis;
}
...
{
  return !((Links <CValueType> ().size() > 0) && (NodalValueSetSize() > 0) &&
  (NodalValueSet(0).ValueSize() == 2));
}
...
void CPointSetEntry::DestroyCaches()
{
  m_cache_manager.DestroyCaches();
}
...
void CPointSetEntry::SetCreateHullCallback(CreateHullCallback cb)
{
  m_hullCB = cb;
}
