...
void loadBoolean(bool& boolean, CStorageNode::TSTREAM& stream,
...
void saveBoolean(CStorageNode::TSTREAM& stream,
  CStorageNode::TPROGRESS& /*progress*/, bool boolean)
{
  stream << (int) (boolean ? 1 : 0);
}

void loadHotSpot(const CHotSpot* hotSpot, CFemAppModel& model,
  ...
  CStorageNode::TPROGRESS& /*progress*/)
{
  ...
  IColorScaleEntry* colorScaleEntry = dynamic_cast <IColorScaleEntry*> (
      model.GraphEntry(MD_BASE_COLOR_SCALE));

  hotSpot = (CHotSpot*) colorScaleEntry->FindIndex(index);
  }
}

void saveHotSpot(CStorageNode::TSTREAM& stream,
  CStorageNode::TPROGRESS& /*progress*/, const CHotSpot* hotSpot)
{
...
void loadTimeStep(const CDepletionStage** timeStep, CFemAppModel& model,
  CStorageNode::TSTREAM& stream, CStreamVersion& /*version*/,
  CStorageNode::TPROGRESS& /*progress*/)
{
  int boolean = 0;
  ...
  CDepletionStageEntry* depletionStageEntry =
    ...
    model.GraphEntry(MD_BASE_DEPLETION_STAGE));

  *timeStep = (CDepletionStage*) depletionStageEntry->FindIndex(index);
  }
}

void saveTimeStep(CStorageNode::TSTREAM& stream,
  CStorageNode::TPROGRESS& /*progress*/, const CDepletionStage* timeStep)
{
...
void loadTimeStep(CExportResultData::TTimeStepSet& timeStepSet,
  CFemAppModel& model, CStorageNode::TSTREAM& stream,
  CStreamVersion& /*version*/, CStorageNode::TPROGRESS& /*progress*/)
{
  CDepletionStageEntry* depletionStageEntry =
  dynamic_cast <CDepletionStageEntry*> (
  ...
  const CDepletionStage* depletionStage =
      (CDepletionStage*) depletionStageEntry->FindIndex(index);
  ...
  const CExportResultData::TTimeStepSet& timeStepSet)
{
  ...
  for (CExportResultData::TTimeStepSet::const_iterator iterator =
  timeStepSet.begin(); iterator != timeStepSet.end(); ++iterator)
  {
...
void loadResultComponentSet(TResultComponentSet& resultComponentSet,
  CFemAppModel& model, CStorageNode::TSTREAM& stream,
  CStreamVersion& /*version*/, CStorageNode::TPROGRESS& /*progress*/)
{
  ...
  const IValueComposite* valueComposite =
      valueCompositeEntry->FindIndex(valueCompositeIndex);

  resultComponentSet.insert(std::make_pair(valueComposite, index));
  }
}

void saveResultComponentSet(CStorageNode::TSTREAM& stream,
  CStorageNode::TPROGRESS& /*progress*/,
  const TResultComponentSet& resultComponentSet)
{
  stream << (unsigned long) resultComponentSet.size();

  for (TResultComponentSet::const_iterator iterator =
  resultComponentSet.begin(); iterator != resultComponentSet.end();
  ++iterator)
  {
...
void fillResultComponentSet(TResultComponentSet& resultComponentSet,
  const CResultGroup& tree, unsigned int valueCompositeIndex, int index)
{
  for (int child = 0; child < tree.ChildSize(); ++child)
  {
  if (dynamic_cast <const CMaterialResult*> (&tree.Child(child)) == 0)
  {
      fillResultComponentSet(resultComponentSet,
    dynamic_cast <const CResultGroup&> (tree.Child(child)),
  ...
  else
  {
      ...
      {
    resultComponentSet.insert(std::make_pair(
          &(dynamic_cast <const CMaterialResult&> (tree.Child(child))), index));
    break;
      }
  }
  }
...
void loadMaterialResultComponentSet(TResultComponentSet& resultComponentSet,
  CFemAppModel& model, CStorageNode::TSTREAM& stream,
  CStreamVersion& /*version*/, CStorageNode::TPROGRESS& /*progress*/)
{
  ...
  fillResultComponentSet(resultComponentSet, modelBase.MaterialResultTree(),
      valueCompositeIndex, index);
  }
}

void saveMaterialResultComponentSet(CStorageNode::TSTREAM& stream,
  CStorageNode::TPROGRESS& /*progress*/,
  const TResultComponentSet& resultComponentSet)
{
  stream << (unsigned long) resultComponentSet.size();
...
void CExportResultDataSettings::LoadStream(CStorageNode::TSTREAM& stream, CStreamVersion& version, CStorageNode::TPROGRESS& progress)
{
  stream >> m_folder;
  stream >> m_file;
...
CExportResultDataDepletionStageFilter::CExportResultDataDepletionStageFilter()
  : m_nType(NONE)
  , m_nSetValue(INITIAL)
{
}
...
{
  CDepletionStage *initStage = &static_cast<CModelBase&>(exportResultData.Model()).DepletionStageEntry().InitialStage();
...
void CExportResultDataDepletionStageFilter::LoadStream(CStorageNode::TSTREAM& stream, CStreamVersion& /*version*/, CStorageNode::TPROGRESS& /*progress*/)
{
  int temp;
  stream >> temp; m_nType = (TType)temp;

  switch (m_nType)
  {
  case SET:
  stream >> temp; m_nSetValue = (TSetValue)temp;
  break;
  case BYTYPE:
  stream >> temp; m_nTypeValue = (CDepletionStage::eOutputType)temp;
  break;
  case BYNAME:
  stream >> m_sName;
  break;
...
void CExportResultDataDepletionStageFilter::SaveStream(CStorageNode::TSTREAM& stream, CStorageNode::TPROGRESS& /*progress*/)
{
  stream << (int)m_nType;
...
void CExportResultDataDepletionStageFilterVector::unbind(CExportResultData& exportResultData, bool bStore)
{
  ...
  if (!exportResultData.m_stTimeStep.empty())
  {
    ...
    for (CExportResultData::TTimeStepSet::iterator it = exportResultData.m_stTimeStep.begin(); it != exportResultData.m_stTimeStep.end(); ++it)
    {
...
void CExportResultDataDepletionStageFilterVector::LoadStream(CStorageNode::TSTREAM& stream, CStreamVersion& version, CStorageNode::TPROGRESS& progress)
{
  int size;
  stream >> size;
...
void CExportResultDataDepletionStageFilterVector::SaveStream(CStorageNode::TSTREAM& stream, CStorageNode::TPROGRESS& progress)
{
  int size = (int)m_depletionStageFilters.size();
  stream << size;
  for (int i = 0; i < size; ++i)
...
void CExportResultDataModelFilter::bind(CExportResultData& exportResultData)
{
  ...
  case SET:
      if (m_nSetValue == ALL)
      {
    THorizonBaseEntry::TNodeSet stNodes = static_cast<THorizonBaseEntry *>(static_cast<CModelBase&>(exportResultData.Model()).GraphEntry(MD_BASE_HORIZON))->EntryNodes();
  ...
  case SET:
      if (m_nSetValue == ALL)
      {
    TFormationBaseEntry::TNodeSet stNodes = static_cast<TFormationBaseEntry *>(static_cast<CModelBase&>(exportResultData.Model()).GraphEntry(MD_BASE_FORMATION))->EntryNodes();
  ...
  case BYNAME:
      {
    CHexaFormationEntry* hexaEntry = dynamic_cast <CHexaFormationEntry*>(static_cast<CModelBase&>(exportResultData.Model()).GraphEntry(MD_HEXA_FORMATION));
    if (hexaEntry)
    {
  ...
  case SET:
      if (m_nSetValue == ALL)
      {
    CNewWellPathEntry::TNodeSet stNodes = static_cast<CNewWellPathEntry *>(static_cast<CModelBase&>(exportResultData.Model()).GraphEntry(MD_NEW_WELLPATH))->EntryNodes();
  ...
  case SET:
      if (m_nSetValue == ALL)
      {
    TPointSetEntry::TNodeSet stNodes = static_cast<TPointSetEntry *>(static_cast<CModelBase&>(exportResultData.Model()).GraphEntry(MD_BASE_POINTSET))->EntryNodes();
  ...
  case SET:
      if (m_nSetValue == ALL)
      {
    CNonMeshedSurfaceEntry::TNodeSet stNodes = static_cast<CNonMeshedSurfaceEntry *>(static_cast<CModelBase&>(exportResultData.Model()).GraphEntry(MD_BASE_NONMESHEDSURFACE))->EntryNodes();
...
void CExportResultDataModelFilter::LoadStream(CStorageNode::TSTREAM& stream, CStreamVersion& /*version*/, CStorageNode::TPROGRESS& /*progress*/)
{
  int temp;
  stream >> temp; m_nType = (TType)temp;
  ...
  {
  stream >> temp; m_nFilter = (TFilter)temp;
  switch (m_nType)
  {
  case SET:
      stream >> temp; m_nSetValue = (TSetValue)temp;
...
void CExportResultDataModelFilterVector::unbind(CExportResultData& exportResultData, bool bStore)
{
  ...
  THorizonBaseEntry::TNodeSet stTotalHorizons = static_cast<THorizonBaseEntry *>(static_cast<CModelBase&>(exportResultData.Model()).GraphEntry(MD_BASE_HORIZON))->EntryNodes();
  TFormationBaseEntry::TNodeSet stTotalFormations = static_cast<TFormationBaseEntry *>(static_cast<CModelBase&>(exportResultData.Model()).GraphEntry(MD_BASE_FORMATION))->EntryNodes();
  CNewWellPathEntry::TNodeSet stTotalWellpaths = static_cast<CNewWellPathEntry *>(static_cast<CModelBase&>(exportResultData.Model()).GraphEntry(MD_NEW_WELLPATH))->EntryNodes();
  TPointSetEntry::TNodeSet stTotalPointsets = static_cast<TPointSetEntry *>(static_cast<CModelBase&>(exportResultData.Model()).GraphEntry(MD_BASE_POINTSET))->EntryNodes();
  CNonMeshedSurfaceEntry::TNodeSet stTotalNonMeshed = static_cast<CNonMeshedSurfaceEntry *>(static_cast<CModelBase&>(exportResultData.Model()).GraphEntry(MD_BASE_NONMESHEDSURFACE))->EntryNodes();
  ...
  for (COpenGLNodeSelection::iterator it = exportResultData.m_selection.begin(); it != exportResultData.m_selection.end(); ++it)
  {
  ...
  if (nSelectedFormationPlanes > 0)
  {
    ...
    if (dynamic_cast<const CFormationPlane *>(*it))
    {
          ...
          std::pair<TFormationPlaneTypeSet::iterator, bool> retval = planes.insert(std::make_pair(&plane->Formation(), TFormationPlaneTypes()));
...
void CExportResultDataModelFilterVector::LoadStream(CStorageNode::TSTREAM& stream, CStreamVersion& version, CStorageNode::TPROGRESS& progress)
{
  int size;
  stream >> size;
...
void CExportResultDataResultFilter::bind(CExportResultData& exportResultData)
{
  ...
  std::vector<const IValueComponentBase *>& flatDerivedResults = *exportResultData.m_resultFilterVector.m_flatDerivedResults;
  std::vector<const IValueComponentBase *>& flatGVTResults = *exportResultData.m_resultFilterVector.m_flatGVTResults;
  std::vector<const IValueComponentBase *>& flatGVTDerivedResults = *exportResultData.m_resultFilterVector.m_flatGVTDerivedResults;
  ...
  case SET:
      ...
      {
    for (std::vector<IValueComponentBase *>::const_iterator it = matResults.groups[0].begin(); it != matResults.groups[0].end(); ++it)
    {
      ...
      {
    for (std::vector<IValueComponentBase *>::const_iterator it = matResults.groups[1].begin(); it != matResults.groups[1].end(); ++it)
    {
          exportResultData.m_MatRGObserverData.insert(TResultComponent(&(*it)->Parent(), (*it)->ComponentIndex()));
    }
      }
      break;
  case BYNAME:
      for (std::vector<IValueComponentBase *>::const_iterator it = flatMatResults.begin(); it != flatMatResults.end(); ++it)
      {
    if (static_cast<CMaterialResult&>(static_cast<CMaterialResult::CMaterialResultComponent *>(*it)->Parent()).StoredExportLabel() == m_sName)
    {
          exportResultData.m_MatRGObserverData.insert(TResultComponent(&(*it)->Parent(), (*it)->ComponentIndex()));
          break;
    }
  ...
  case SET:
      if (m_nSetValue == ALL)
      {
    for (std::vector<IValueComponentBase *>::const_iterator it = meshResults.groups[0].begin(); it != meshResults.groups[0].end(); ++it)
    {
  ...
  case SET:
      if (m_nSetValue == ALL)
      {
    for (std::vector<const IValueComponentBase *>::const_iterator it = flatResults.begin(); it != flatResults.end(); ++it)
    {
          exportResultData.m_RGObserverData.insert(TResultComponent(&(*it)->Parent(), (*it)->ComponentIndex()));
    }
      }
  ...
  case BYNAME:
    ...
    if (static_cast<const IResult *>(&(*it)->Parent())->ExportLabel((*it)->ComponentIndex()) == m_sName)
    {
      ...
      for (std::vector<const IValueComponentBase *>::const_iterator it = flatDerivedResults.begin(); it != flatDerivedResults.end(); ++it)
      {
    if (static_cast<const IResult *>(&(*it)->Parent())->ExportLabel((*it)->ComponentIndex()) == m_sName)
    {
          exportResultData.m_RGObserverData.insert(TResultComponent(&(*it)->Parent(), (*it)->ComponentIndex()));
  ...
  case SET:
      if (m_nSetValue == ALL)
      {
    for (std::vector<const IValueComponentBase *>::const_iterator it = flatGVTResults.begin(); it != flatGVTResults.end(); ++it)
    {
  ...
  case BYNAME:
    ...
    if (pValueType && pValueType->ExportLabel() == m_sName)
    {
      ...
      for (std::vector<const IValueComponentBase *>::const_iterator it = flatGVTDerivedResults.begin(); it != flatGVTDerivedResults.end(); ++it)
      {
    const IResult *pResult = dynamic_cast<const IResult *>(&(*it)->Parent());
    const CValueType *pValueType = dynamic_cast<const CValueType *>(&(*it)->Parent());
...
void CExportResultDataResultFilter::LoadStream(CStorageNode::TSTREAM& stream, CStreamVersion& /*version*/, CStorageNode::TPROGRESS& /*progress*/)
{
  int temp;
  stream >> temp; m_nType = (TType)temp;
...
void CExportResultDataResultFilterVector::bind(CExportResultData& exportResultData)
{
  ...
  std::vector<IValueComponentBase *> flatMatResults; flatMatResults.reserve(matResults.nTotal);
  for (std::vector<IValueComponentBase *>::const_iterator it = matResults.groups[0].begin(); it != matResults.groups[0].end(); ++it)
  ...
  std::vector<const IValueComponentBase *> flatResults; flatResults.reserve(results.nTotal);
  for (size_t i = 0; i < results.groups.size(); ++i)
  for (size_t j = 0; j < results.groups[i].size(); ++j)
      flatResults.push_back(results.groups[i][j]);

  std::vector<const IValueComponentBase *> flatDerivedResults; flatDerivedResults.reserve(results.nTotal);
  ...
  std::vector<const IValueComponentBase *> flatGVTResults; flatGVTResults.reserve(gvtResults.nTotal);
  ...
  std::vector<const IValueComponentBase *> flatGVTDerivedResults; flatGVTDerivedResults.reserve(gvtResults.nTotal);
...
void CExportResultDataResultFilterVector::unbindMaterialResults(CExportResultData& exportResultData)
{
  ...
  for (TResultComponentSet::const_iterator it = exportResultData.m_MatRGObserverData.begin(); it != exportResultData.m_MatRGObserverData.end(); ++it)
  {
      for (size_t i = 0; i < it->first->ComponentSize(); ++i)
      {
    if (std::find(matResults.groups[1].begin(), matResults.groups[1].end(), &it->first->Component(i)) != matResults.groups[1].end())
          ++nFractureSelected;
      }
  }
...
void CExportResultDataResultFilterVector::unbindResults(CExportResultData& exportResultData) // TODO: split mesh and normal results
{
  CollectResults meshResults;
  static_cast<CModelBase&>(exportResultData.Model()).MeshResultTree().Accept(meshResults);
  ...
  for (TResultComponentSet::const_iterator it = exportResultData.m_RGObserverData.begin(); it != exportResultData.m_RGObserverData.end(); ++it)
  {
  ...
  // Note: we only check for nodes that can be mapped to the current model, but we don't differentiate between unselected and unmappable;
  ...
  {
  std::vector<const IValueComponentBase *> flat; flat.reserve(results.nTotal);
  for (size_t i = 0; i < results.groups.size(); ++i)
      for (size_t j = 0; j < results.groups[i].size(); ++j)
  ...
  std::vector<const IValueComponentBase *> flatDerived; flatDerived.reserve(10);
  for (size_t i = 0; i < results.derivedGroups.size(); ++i)
  for (size_t j = 0; j < results.derivedGroups[i].size(); ++j)
  ...
  if (dynamic_cast<const IResult *>(it->first))
  {
      bool inTree = std::find(flatDerived.begin(), flatDerived.end(), &it->first->Component(it->second)) != flatDerived.end();
...
void CExportResultDataResultFilterVector::unbindGVTResults(CExportResultData& exportResultData)
{
  ...
  {
  std::vector<const IValueComponentBase *> flat; flat.reserve(gvtResults.nTotal);
  for (size_t i = 0; i < gvtResults.groups.size(); ++i)
      for (size_t j = 0; j < gvtResults.groups[i].size(); ++j)
    flat.push_back(gvtResults.groups[i][j]);

  for (TResultComponentSet::const_iterator it = exportResultData.m_GVTRGObserverData.begin(); it != exportResultData.m_GVTRGObserverData.end(); ++it)
  {
      bool inTree = std::find(flat.begin(), flat.end(), &it->first->Component(it->second)) != flat.end();
...
void CExportResultDataResultFilterVector::LoadStream(CStorageNode::TSTREAM& stream, CStreamVersion& version, CStorageNode::TPROGRESS& progress)
{
  int size;
  stream >> size;
...
void CExportResultDataDeformation::bind(CExportResultData& exportResultData)
{
  ...
  {
  case INITIAL:
      exportResultData.m_pTimeStep = &static_cast<CModelBase&>(exportResultData.Model()).DepletionStageEntry().InitialStage();
      break;
  case LAST:
      exportResultData.m_pTimeStep = &static_cast<CModelBase&>(exportResultData.Model()).DepletionStageEntry().LastStage();
      break;
  }
  break;
  case BYNAME:
  {
      CDepletionStage *current = &static_cast<CModelBase&>(exportResultData.Model()).DepletionStageEntry().InitialStage();
      while (current)
      {
    if (current->Name() == m_sName)
    {
...
void CExportResultDataDeformation::LoadStream(CStorageNode::TSTREAM& stream, CStreamVersion& /*version*/, CStorageNode::TPROGRESS& /*progress*/)
{
  int temp;
  stream >> temp; m_nType = (TType)temp;
  switch (m_nType)
  ...
  stream >> temp; m_nCalculation = (TCalculation)temp;
}
...
void CExportResultDataHotSpot::bind(CExportResultData& exportResultData)
{
  ...
  IColorScaleEntry *colorScaleEntry = dynamic_cast<IColorScaleEntry *>(static_cast<CModelBase&>(exportResultData.Model()).GraphEntry(MD_BASE_COLOR_SCALE));
  if (colorScaleEntry)
  {
  ...
  for (IColorScaleEntry::TNodeSet::iterator it = stNodes.begin(); it != stNodes.end(); ++it)
  {
    ...
    if (m_bCenterIsTransparent == tryCenterIsTransparent && m_nMinColor == tryMinColor && m_nMaxColor == tryMaxColor && std::abs(m_dMinMaxValue.first - tryMMValue.first) < 1E-6 &&  std::abs(m_dMinMaxValue.second - tryMMValue.second) < 1E-6)
    {
...
void CExportResultDataHotSpot::LoadStream(CStorageNode::TSTREAM& stream, CStreamVersion& version, CStorageNode::TPROGRESS& progress)
{
  int temp;
  stream >> temp; m_nType = (TType)temp;
  if (m_nType != NONE)
...
: CColorNode(model)
, m_settings()
, m_depletionStageFilterVector()
, m_modelFilterVector()
, m_resultFilterVector()
, m_deformation()
, m_hotspot()
, m_folder()
, m_file()
...
CExportResultData::CExportResultData(const QString& exportResultName,
  CFemAppModel& model)
: CColorNode(exportResultName, model)
, m_settings()
, m_depletionStageFilterVector()
, m_modelFilterVector()
, m_resultFilterVector()
...
unsigned int CExportResultData::IconId() const
{
  return IDI_EXPORT_MACROS;
}

unsigned int CExportResultData::TypeId() const
{
  return IDT_TREE_EXPORT_MACROS;
}

int CExportResultData::DisplayListSize() const
{
  return 0;
}
...
bool CExportResultData::ReservoirMacro() const
{
  return m_settings.m_bReservoirMacro;
}

void CExportResultData::LoadStream(TSTREAM& stream, CStreamVersion& version,
  TPROGRESS& progress)
{
...
void CExportResultData::unbind(bool bStore)
{
  ...
  m_resultFilterVector.unbind(*this, bStore); // results before model, as we still need the latter to check for allowed results (upon saving)
