...
//////////////////////////////////////////////////////////////////////
CDerivedResult::CResultCoordinateProxy::CResultCoordinateProxy(unsigned int uName, rpn::CRpnStack &stack,
                                                               COORDINATE coor, const QString &sProxyId)
...
CDerivedResult::CResultCoordinateProxy::CResultCoordinateProxy(const QString &sName, rpn::CRpnStack &stack,
                                                               COORDINATE coor, const QString &sProxyId)
...
unsigned int CDerivedResult::CResultCoordinateProxy::IconId() const { return IDI_VALUETYPE_COORDINATE; }

int CDerivedResult::CResultCoordinateProxy::TypeId() const { return COORDINATE_ID; }

bool CDerivedResult::CResultCoordinateProxy::Recursive(TParentSet /*stParent*/) const { return false; }
...
CDerivedResult::CDerivedResultComponent::CValueComponentProxy::CValueComponentObserver::CValueComponentObserver(
    IValueComponentBase &value_component, CValueComponentProxy &proxy)
...
CDerivedResult::CDerivedResultComponent::CValueComponentProxy::CValueComponentObserver::~CValueComponentObserver() {
  if (m_proxy)
    m_proxy->m_pObserver = 0;
}

const IValueComponentBase &
CDerivedResult::CDerivedResultComponent::CValueComponentProxy::CValueComponentObserver::ValueComponent() const {
  assert(!m_value_component.IsCopy());
  return m_value_component;
}

IValueComponentBase &
CDerivedResult::CDerivedResultComponent::CValueComponentProxy::CValueComponentObserver::ValueComponent() {
  assert(!m_value_component.IsCopy());
  return m_value_component;
}

void CDerivedResult::CDerivedResultComponent::CValueComponentProxy::CValueComponentObserver::OnNeighbourDeleted(
    const CGraphNode &item) {
...
rpn::CRpnObject *
CDerivedResult::CDerivedResultComponent::CValueComponentProxy::CValueComponentObserver::GetExpandedRpnObject(
    rpn::CRpnStack &targetstack, std::list<std::string> &lstMessages) const {
...
CDerivedResult::CDerivedResultComponent::CValueComponentProxy::CValueComponentProxy(
    const QString &sName, IValueComponentBase &value_component, rpn::CRpnStack &NewStack, bool bFixedLinStage)
    : IValueProxy(NewStack), m_sName(sName), m_nIndex(-1), m_nComponent(-1), m_nTimeStep(-1),
      m_antype(CAnalysisType::AT_NONLIN), m_bStoreOnFile(false), m_bFixedLinStage(bFixedLinStage) {
...
CDerivedResult::CDerivedResultComponent::CValueComponentProxy::CValueComponentProxy(const CValueComponentProxy &rhs,
                                                                                    rpn::CRpnStack &NewStack)
    : IValueProxy(NewStack, rhs.ProxyId().toStdString().c_str()), m_pObserver(0), m_sName(rhs.m_sName),
      m_nIndex(rhs.m_nIndex), m_nComponent(rhs.m_nComponent), m_nTimeStep(rhs.m_nTimeStep), m_antype(rhs.m_antype),
      m_pModel(rhs.m_pModel), m_bStoreOnFile(rhs.m_bStoreOnFile), m_bFixedLinStage(rhs.m_bFixedLinStage) {
  if (rhs.m_pObserver)
    m_pObserver = new CValueComponentObserver(rhs.m_pObserver->ValueComponent(), *this);
}

CDerivedResult::CDerivedResultComponent::CValueComponentProxy::CValueComponentProxy(rpn::CRpnStack &NewStack,
                                                                                    const QString &sProxyId)
    : IValueProxy(NewStack, sProxyId.toStdString().c_str()), m_pObserver(0), m_nIndex(-1), m_nComponent(-1),
      m_nTimeStep(-1), m_antype(CAnalysisType::AT_NONLIN), m_pModel(0), m_bFixedLinStage(false) {}
...
rpn::CRpnOperand::IValueProxy *
CDerivedResult::CDerivedResultComponent::CValueComponentProxy::Clone(rpn::CRpnStack &NewStack) const {
...
void CDerivedResult::CDerivedResultComponent::CValueComponentProxy::LoadStream(CStorageNode::TSTREAM &stream,
...
bool CDerivedResult::CDerivedResultComponent::CValueComponentProxy::Recursive(TParentSet stParent) const {
  if (m_pObserver) {
    const CDerivedResultComponent *pComponent =
        dynamic_cast<const CDerivedResultComponent *>(&m_pObserver->ValueComponent());
    if (pComponent) {
...
void CDerivedResult::CDerivedResultComponent::CValueComponentProxy::LoadStream(std::stringstream &stream,
                                                                               CStreamVersion &version) {
  assert(m_pObserver == 0);
...
bool CDerivedResult::CDerivedResultComponent::CValueComponentProxy::exists() const { return (m_pObserver != 0); }

IValueComposite *
CDerivedResult::CDerivedResultComponent::CValueComponentProxy::FindResultByIndex(CResultGroup &rg) const {
  assert(m_nIndex >= 0);
...
QString CDerivedResult::CDerivedResultComponent::CValueComponentProxy::TextTag() const { return m_sName; }
...
CDerivedResult::TValue CDerivedResult::CDerivedResultComponent::CValueComponentProxy::Value(const geo::IPoint &pt,
                                                                                            UNIT unit) const {
  assert(m_pObserver || IsResult());
...
CDerivedResult::TValueVec CDerivedResult::CDerivedResultComponent::CValueComponentProxy::Value(const geo::IElement &el,
                                                                                               UNIT unit) const {
  assert(m_pObserver || IsResult());
  // Try to connect
  if (m_pObserver == 0) {
...
bool CDerivedResult::CDerivedResultComponent::CValueComponentProxy::CanDelete() const { return true; }
...
bool CDerivedResult::CDerivedResultComponent::CValueComponentProxy::IsResult() const { return m_nTimeStep >= 0; }

int CDerivedResult::CDerivedResultComponent::CValueComponentProxy::TypeId() const { return VALUE_ID; }

bool CDerivedResult::CDerivedResultComponent::CValueComponentProxy::Properties(bool fixedLinStage,
                                                                               const IValueComponentBase *selection,
                                                                               const QString &name) {
  assert(m_pObserver);
  assert(m_pModel);
...
void CDerivedResult::CDerivedResultComponent::CValueComponentProxy::SetAnalysisTypeAndStage(const CAnalysisType &antype,
                                                                                            int nStage) {
...
bool CDerivedResult::CDerivedResultComponent::CValueComponentProxy::StoreOnFile() const { return m_bStoreOnFile; }

rpn::CRpnObject *CDerivedResult::CDerivedResultComponent::CValueComponentProxy::GetExpandedRpnObject(
    rpn::CRpnStack &targetstack, std::list<std::string> &lstMessages) const {
  if (!m_pObserver) {
    lstMessages.push_back(std::string("The operand named '") + m_sName.toStdString() +
                          "' is referring to a non-existing result. The value 1 is inserted at this location");
...
CDerivedResult::CDerivedResultComponent::CDerivedResultComponent(CDerivedResult &parent, const CDepletionStage &stage,
...
void CDerivedResult::CDerivedResultComponent::MapValueElement             // NOT CASCADING PARALLELINITIALIZATION
    (const geo::IElement &elm, TValueVec &values, IVectorResult::TMapType /*map_type*/
     ,
...
void CDerivedResult::CDerivedResultComponent::SaveStream(std::stringstream &stream) {
  ...
  if (Parent().Model().parentModel() == 0 &&
      static_cast<CDerivedResult &>(Parent()).StoreOnFile()) // only in top model, only on file
  {
...
void CDerivedResult::CDerivedResultComponent::LoadStream(std::stringstream &stream, rpn::CRpnVersion &version) {
  ...
  if (version >= rpn::CRpnVersion(1, 0, 10)) {
    if (version < rpn::CRpnVersion(1, 0, 11) || (bIsTopModel && !bIsRegResult)) {
      ...
      if (!bIsTopModel || bIsRegResult) // 1.0.10 erroneously loads top displacements for zoom-in and/or registry
                                        // results, so we remove them here
      {
        rpn::CProxyPersistant<CRpnTopDisplacementProxy<0, IDS_MEASURED_TOP_DISPL_N>>::TProxyVec proxiesTDN(
            psTDN.Proxies(*Stack()));
        for (size_t i = 0; i < proxiesTDN.size(); ++i) {
          Stack()->DetachValueProxy(*proxiesTDN[i]);
        }

        rpn::CProxyPersistant<CRpnTopDisplacementProxy<1, IDS_MEASURED_TOP_DISPL_E>>::TProxyVec proxiesTDE(
            psTDE.Proxies(*Stack()));
        for (size_t i = 0; i < proxiesTDE.size(); ++i) {
          Stack()->DetachValueProxy(*proxiesTDE[i]);
        }

        rpn::CProxyPersistant<CRpnTopDisplacementProxy<2, IDS_MEASURED_TOP_DISPL_D>>::TProxyVec proxiesTDD(
            psTDD.Proxies(*Stack()));
...
geo::CValue CDerivedResult::CDerivedResultComponent::ValuePoint(
    const geo::IPoint &pt, const UNIT unit,
    geo::IParallelInitializationCallback * /*cb*/) const // NOT CASCADING PARALLELINITIALIZATION
{
  (const_cast<CDerivedResultComponent *>(this))->MakeCurrent(false);
  return RpnStack().Value(pt, unit);
}

IValueDomainScalar::TValueVec CDerivedResult::CDerivedResultComponent::ValueElement(
    const geo::IElement &elm, const UNIT unit,
    geo::IParallelInitializationCallback * /*cb*/) const // NOT CASCADING PARALLELINITIALIZATION
{
  (const_cast<CDerivedResultComponent *>(this))->MakeCurrent(false);
...
unsigned int CDerivedResult::CDerivedResultComponent::TypeId() const { return 0; }

bool CDerivedResult::CDerivedResultComponent::Empty() const { return Stack() == 0; }

const QString &CDerivedResult::CDerivedResultComponent::Name() const { return Parent().Name(); }

void CDerivedResult::CDerivedResultComponent::Name(const QString &strName) { Parent().Name(strName); }
...
void CDerivedResult::CDerivedResultComponent::RpnVersionFromFileVersion(
    TSTREAM & /*stream*/, const CStreamVersion &version, std::vector<rpn::CRpnVersion> &vcRpnVersion) const {
  vcRpnVersion.clear();
...
void CDerivedResult::CDerivedResultComponent::LoadStream(TSTREAM &stream, CStreamVersion &version,
                                                         TPROGRESS &progress) {
  // loading base
      ...
      if (pEnv && vcRpnVersion.size() == 2) {
        if (!strcmp(pEnv, "NO")) {
          ...
          progress.StatusMessage(
              QString("Loading derived results (forcing version (%1))").arg(vcRpnVersion[1].GetVersionString()));
          LoadStream(string_stream, vcRpnVersion[1]);
        } else {
          progress.StatusMessage(
              QString("Loading derived results (forcing version (%1))").arg(vcRpnVersion[0].GetVersionString()));
...
QString CDerivedResult::CDerivedResultComponent::ExportLabel() const {
  ...
  if (bAppend) {
    if (nm.indexOf(' ') > -1)
      nm = QString("[") + nm + "]";
    nm = QString("%1_D%2_%3").arg(nm).arg(Stage().Index()).arg(AnalysisType().ExportCharacter());
  }
...
IValueDomainScalar::TMinMax CDerivedResult::CDerivedResultComponent::MinMax(IProgressBase & /*progressBase*/,
...
void CDerivedResult::CDerivedResultComponent::MakeCurrentValueComponentProxies(bool &bModified) {
  ...
  for (size_t i = 0; i < vcProxies.size(); ++i) {
    ...
    if (pVCProxy->HasValueComponent()) {
      CDerivedResultComponent *pDerivedResultComp =
          dynamic_cast<CDerivedResultComponent *>(&pVCProxy->ValueComponent());
...
CDerivedResult::CDerivedResult(CDerivedResultGroup &group) : IResult("", group), m_pGroup(&group), m_pStack(0) {
  BuildComponents();
  assert(Index() == -1);
}

CDerivedResult::~CDerivedResult() { delete m_pStack; }

QString CDerivedResult::ExportLabel(int nComponent) const { return Component(nComponent).Name(); }

IResultComponent *CDerivedResult::OnGetResultComponent(int nTimeStep, const CAnalysisType &antype, int nRegister,
...
unsigned int CDerivedResult::IconId() const { return IDI_RESULT; }

unsigned int CDerivedResult::TypeId() const { return 0; }
...
bool CDerivedResult::StoreOnFile() const { return m_pGroup->StoreOnFile(); }

bool CDerivedResult::Empty() const { return Component(0, 0).Empty(); }
...
const rpn::CRpnStack *CDerivedResult::RpnStack() const { return m_pStack; }

rpn::CRpnStack *CDerivedResult::RpnStack() { return m_pStack; }
...
bool CDerivedResult::ValidName(const std::string &name, CResultInfo &resultInfo) const {
...
bool CDerivedResult::PrepareMapping(const geo::IElementSet * /* pElementSet */,
...
CDerivedResultGroup::CDerivedResultGroup(unsigned int uGroupName, CResultGroup &parent, bool bStoreOnFile)
    : CResultGroup(uGroupName, parent), m_bStoreOnFile(bStoreOnFile) {}
...
bool CDerivedResultGroup::StoreOnFile() const { return m_bStoreOnFile; }
...
void CDerivedResultGroup::LoadStream(CStorageNode::TSTREAM &stream, CStreamVersion &version,
