...
#ifdef SKUA_NEW
CGocadElementSet::CGocadElementSet(const QString& sName, eElementType type,
  const TPointVec& vcPoint, const TElementVec& vcElement, CFemAppModel& model,
  std::vector<const gm_skua::SKUAParseData *>& solids)
: CElementSet(sName, type, vcPoint, vcElement, model)
, m_Solids(solids)
{
}
#else
...
CGocadElementSet::CGocadElementSet(CFemAppModel& model)
: CElementSet(model)
, m_Solids()
{
}
...
{
  const CGoCadModel& model = dynamic_cast <const CGoCadModel&> (Model());
  CTetraMesh& mesh = const_cast <CTetraMesh&> (
    dynamic_cast <const CTetraMesh&> (model.Mesh()));
...
void CGocadElementSet::LoadStream(TSTREAM& stream, CStreamVersion& version, TPROGRESS& progress)
{
  ...
  {
    QSharedPointer <CGocadData::CTSolid> solid =
      QSharedPointer <CGocadData::CTSolid>(new CGocadData::CTSolid());
...
#else
  std::vector <QSharedPointer <CGocadData::CTSolid> >::const_iterator
    iterator = m_Solids.begin();
