...
IStrainTensorGroup::IStrainTensorGroup(unsigned int uResultName, TValueSetFunction function, CResultGroup &group)
    : ITensorGroupTemplate<CStrainTensor>(uResultName, function, group), m_pInVariant(0), m_pAxialStrainComposite(0) {}

IStrainTensorGroup::IStrainTensorGroup(const QString &sResultName, TValueSetFunction function, CResultGroup &group)
    : ITensorGroupTemplate<CStrainTensor>(sResultName, function, group), m_pInVariant(0), m_pAxialStrainComposite(0) {}
...
bool IStrainTensorGroup::OnBuildComponent(const CDepletionStage &stage, const CAnalysisType & /*antype*/,
                                          int /*nRegister*/) const {
  return !stage.Initial();
}

double IStrainTensorGroup::ConvertToField(const double &dValue) const { return dValue; }
...
void IStrainTensorGroup::CInvariantComposite::BuildComponent(const CDepletionStage &stage, const CAnalysisType &antype,
                                                             int nRegister) {
...
unsigned int IStrainTensorGroup::CInvariantComposite::IconId() const { return IDI_RESULT_MAP; }
...
//////////////////////////////////////////////////////////////////////
IStrainTensorGroup::CInvariantComposite::CInvariantComponent::CInvariantComponent(
...
unsigned int IStrainTensorGroup::CInvariantComposite::CInvariantComponent::IconId() const { return IDI_RESULT; }

unsigned int IStrainTensorGroup::CInvariantComposite::CInvariantComponent::TypeId() const { return 0; }
...
geo::CValue IStrainTensorGroup::CInvariantComposite::CInvariantComponent::ValuePoint(
  ...
  if (pParent->Change()) {
    ...
    geo::CValue iniValue =
        Value(pParent->TensorAtPoint(pt, *this, dep_entry.MarkedAsInitialStage(), AnalysisType(), false, cb), unit);
...
void IStrainTensorGroup::CInvariantComposite::CInvariantComponent::MapValueElement(
  ...
  const IStrainTensorGroup::TTensorValueVec &vcTensor =
  ...
  if (pParent->Change()) {
    CDepletionStageEntry &dep_entry = (CDepletionStageEntry &)*Model().GraphEntry(MD_BASE_DEPLETION_STAGE);
    const IStrainTensorGroup::TTensorValueVec &vcIniTensor =
...
geo::CValue IStrainTensorGroup::CInvariantComposite::CInvariantComponent::Value(const CStrainTensor &tensor,
...
bool IStrainTensorGroup::CInvariantComposite::CInvariantComponent::PrepareMapping(
    const geo::IElementSet * /*pElementSet*/) {
  bool retval = true;

  int c = ResultRegister().ColumnNumber(
      AnalysisType(), ResultRegister().DepletionStageEntry().MarkedAsInitialStage().Index(), CI_STRAIN_XX);
...
bool IStrainTensorGroup::CInvariantComposite::ValidName(const std::string &name, CResultInfo &resultInfo) const {
  // Get the type name of the current tensor
  const IStrainTensorGroup *pParent = dynamic_cast<const IStrainTensorGroup *>(Parent());
  assert(pParent);
  ...
  return validateName(resultInfo, name, sCombinedName, StrainInvariantResultComponentList(), "_L|_M|_C|_N|_H", pParent);
}

///// IStrainTensorGroup::CWellPathAxialStrainComposite

IStrainTensorGroup::CWellPathAxialStrainComposite::CWellPathAxialStrainComposite(unsigned int uName,
                                                                                 CResultGroup &group)
    : IResult(uName, group) {
...
IStrainTensorGroup::CWellPathAxialStrainComposite::CWellPathAxialStrainComposite(const QString &sName,
                                                                                 CResultGroup &group)
    : IResult(sName, group) {
  m_pTensorGroup = static_cast<IStrainTensorGroup *>(&group);
...
unsigned int IStrainTensorGroup::CWellPathAxialStrainComposite::IconId() const { return IDI_RESULT; }
...
bool IStrainTensorGroup::CWellPathAxialStrainComposite::ValidName(const std::string &name,
...
bool IStrainTensorGroup::CWellPathAxialStrainComposite::CanMap(const COpenGLNode &node, int /*nRegister*/) const {
  return ((dynamic_cast<const CWellFormation *>(&node) || dynamic_cast<const CWellZoomInMesh *>(&node) ||
           dynamic_cast<const CNewWellPath *>(&node) || (dynamic_cast<const CWellCasingModel *>(&Model()) != 0)) &&
          dynamic_cast<const CWellCasingRTCI *>(&node) == 0);
}

bool IStrainTensorGroup::CWellPathAxialStrainComposite::OnBuildComponent(const CDepletionStage &stage,
                                                                         const CAnalysisType &antype,
                                                                         int nRegister) const {
  return m_pTensorGroup->OnBuildComponent(stage, antype, nRegister);
}

void IStrainTensorGroup::CWellPathAxialStrainComposite::BuildComponent(const CDepletionStage &stage,
                                                                       const CAnalysisType &antype, int nRegister) {
...
IStrainTensorGroup::CWellPathAxialStrainComposite::GetParallelInitializationCallback() {
  ITensorGroup *pGroup = dynamic_cast<ITensorGroup *>(Parent());
  if (pGroup)
    return pGroup->GetParallelInitializationCallback();
...
bool IStrainTensorGroup::CWellPathAxialStrainComposite::PrepareMapping(const geo::IElementSet *pElementSet,
...
IStrainTensorGroup::CWellPathAxialStrainComposite::CWellPathAxialStrainComponent::CWellPathAxialStrainComponent(
    unsigned int uName, CWellPathAxialStrainComposite &parent, const CDepletionStage &stage,
    const CAnalysisType &antype, int nRegister)
...
QString
IStrainTensorGroup::CWellPathAxialStrainComposite::CWellPathAxialStrainComponent::UnitName(const UNIT /*unit*/) const {
  return QString("-");
}

geo::CValue IStrainTensorGroup::CWellPathAxialStrainComposite::CWellPathAxialStrainComponent::ValuePoint(
...
void IStrainTensorGroup::CWellPathAxialStrainComposite::CWellPathAxialStrainComponent::MapValueElement(
    const geo::IElement &elm, IValueDomainScalar::TValueVec &values, TMapType map_type, UNIT /*unit*/,
    geo::IParallelInitializationCallback * /*cb*/) const // NOT CASCADING PARALLELINITIALIZATION
{
...
bool IStrainTensorGroup::CWellPathAxialStrainComposite::CWellPathAxialStrainComponent::
