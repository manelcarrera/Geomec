...
CMatParamsTensor::CMatTensorComponent::CMatTensorComponent(CValueType &value_type, const Units::CUnitType &unittype,
                                                           ...
                                                           unsigned int uComponentName, unsigned int uComponentIndex,
                                                           unsigned int uExportLabel,
                                                           unsigned int typeID /*, unsigned int iconID*/)
    : CValueComponent(uComponentName, value_type, uComponentIndex, 0), m_UnitType(unittype), m_RangeType(rangetype),
      m_ExportLabel(uExportLabel), m_TypeID(typeID) {}

CMatParamsTensor::CMatTensorComponent::CMatTensorComponent(const CMatTensorComponent &rhs)
    : CValueComponent(rhs), m_UnitType(rhs.m_UnitType), m_RangeType(rhs.m_RangeType), m_ExportLabel(rhs.m_ExportLabel),
...
unsigned int CMatParamsTensor::CMatTensorComponent::TypeId() const { return m_TypeID; }

QString CMatParamsTensor::CMatTensorComponent::ExportLabel() const { return getStringTableEntry(m_ExportLabel); }

double CMatParamsTensor::CMatTensorComponent::FieldFactor() const { return m_UnitType.field_factor(); }
...
CMatParamsTensor::CMatParamsTensor(unsigned int uExportID, unsigned int uTypeID, unsigned int uIconID,
    ...
    : CValueType(point_set, strName), m_CurrentIndex(0), m_ExportID(uExportID), m_TypeID(uTypeID), m_IconID(uIconID),
      m_TypeName(strName) {}

QString CMatParamsTensor::ExportType() const { return getStringTableEntry(m_ExportID); }

unsigned int CMatParamsTensor::TypeId() const { return m_TypeID; }

unsigned int CMatParamsTensor::IconId() const { return m_IconID; }
...
QString CMatParamsTensor::TypeName() const { return m_TypeName; }
