...
CMeshResult::CMeshResultComponent::CResultValueSet::CResultValueSet(const CMeshResultComponent& component, const geo::IMesh& mesh, CQuantity::UNIT unit)
: m_component(component), m_mesh(mesh), m_unit(unit)
{
}

const geo::IElementSet &CMeshResult::CMeshResultComponent::CResultValueSet::ElementSet() const
{
  return m_mesh;
}

CQuantity::UNIT CMeshResult::CMeshResultComponent::CResultValueSet::Unit() const
{
  return m_unit;
}

std::pair<double, double> CMeshResult::CMeshResultComponent::CResultValueSet::MinMaxEdge( const geo::IElement& element ) const
{
...
void CMeshResult::CMeshResultComponent::CResultValueSet::ElementValues(TValueVec& values, int iElementIndex, geo::IParallelInitializationCallback * /*cb*/) const
{
...
void CMeshResult::CMeshResultComponent::CResultValueSet::Empty()
{
  assert(false);
}

bool CMeshResult::CMeshResultComponent::CResultValueSet::IsEmpty() const
{
  return false;
}
...
bool CMeshResult::Empty() const
{
  return false;
}

long CMeshResult::SavedItems() const
{
  return 0;
}

unsigned int CMeshResult::IconId() const
{
  return IDI_RESULT;
}

unsigned int CMeshResult::TypeId() const
{
  return Index();
}
...
bool CMeshResult::isMeshResult() const
{
  return true;
}
...
: IValueComponentBase(uName, parent),
  m_pSIValueSet(0), 
  m_pFieldValueSet(0),
  m_Data(*this)
{
...
const IValueDataInterfaceScalar& CMeshResult::CMeshResultComponent::ScalarData() const
{
  return m_Data;
}

IValueDataInterfaceScalar& CMeshResult::CMeshResultComponent::ScalarData()
{
  return m_Data;
}

unsigned int CMeshResult::CMeshResultComponent::IconId() const
{
  return IDI_RESULT;
}

unsigned int CMeshResult::CMeshResultComponent::TypeId() const
{
   return 0;
}
...
bool CMeshResult::CMeshResultComponent::Defined() const
{
  ...
  return (Parent().Index() == RC_MR_DEPTH) || (Parent().Index() == RC_MR_NORTHING) || (Parent().Index() == RC_MR_EASTING) || (Parent().Index() == RC_MR_SURFACE) || (pModel->IsMesh());
}

IValueDomainScalar::TMinMax CMeshResult::CMeshResultComponent::MinMax(IProgressBase& /*progressBase*/, const CQuantity::UNIT /*unit*/) const
{
...
bool CMeshResult::CMeshResultComponent::Empty() const
{
  return false;
}
...
IValueDomainScalar::TValue CMeshResult::CMeshResultComponent::ValuePoint(const geo::IPoint& pt, const UNIT unit, geo::IParallelInitializationCallback *cb) const
{
...
{
  if(!(Parent().Index() == RC_MR_DEPTH || Parent().Index() == RC_MR_NORTHING || Parent().Index() == RC_MR_EASTING || Parent().Index() == RC_MR_SURFACE))
  {
...
IValueDomainScalar::TValueVec CMeshResult::CMeshResultComponent::ValueElement(const geo::IElement& elm, const UNIT unit, geo::IParallelInitializationCallback *cb) const
{
  assert(Defined());
  IValueDomainScalar::TValueVec vcRet(elm.NrOfPoints());

  bool bWellpathResult = Parent().Index() == RC_MR_AHD || Parent().Index() == RC_MR_AZIMUTH || Parent().Index() == RC_MR_INCLINATION;

  if(Parent().Index() == RC_MR_DEPTH || Parent().Index() == RC_MR_NORTHING || Parent().Index() == RC_MR_EASTING || bWellpathResult)
  {
    for(int i = 0; i < vcRet.size(); i++) vcRet[i] = ValuePoint( elm.Point(i), unit );
  }
  ...
  for(int i = 0; i < vcRet.size(); i++) vcRet[i] = size;
  } else {
...
bool CMeshResultTree::ValidName
( const std::string &name
, CResultInfo &resultInfo
) const
{
  ...
  {
  const CMeshResult *pMeshResult= 
      dynamic_cast<const CMeshResult *>(&childAt(ii));
