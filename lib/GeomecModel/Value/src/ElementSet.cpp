...
IElementSet::IElementSet(const QString &sName, CFemAppModel &model, CQuantity::UNIT coordinate_unit,
                         CQuantity::UNIT value_unit)
    : IPointSet(sName, model, coordinate_unit, value_unit) {}

IElementSet::IElementSet(CFemAppModel &model) : IPointSet(model) {}

IElementSet::IElementSet(const IElementSet &rhs) : IPointSet(rhs) {
...
bool IElementSet::PointInConvexHull(const geo::IPoint &pt) const { return ElementSet().ElementsAt(pt).size() > 0; }

int IElementSet::DisplayListSize() const { return 1; }
...
int IElementSet::ValueSetSize() const { return IPointSet::ValueSetSize() + ElementValueSetSize(); }
...
int IElementSet::ElementValueSetSize() const { return m_vcElementValueSet.size(); }
...
IElementSet::TWeightMap IElementSet::WeightFactors(int /*nNode*/, TWeightingType /*nWeightingType*/,
                                                   const CFormationBase * /*pFormation*/) const {
  return TWeightMap();
}
...
bool IElementSet::PrepareMapping(const geo::IElementSet *) { return false; }

//////////////////////////////////////////////////////////////////////
// Construction/Destruction
//////////////////////////////////////////////////////////////////////

CElementSet::CLineElement::CLineElement(geo::IMesh &mesh, int nPoint1, int nPoint2) : m_mesh(mesh) {
  m_nodes[0] = nPoint1;
  m_nodes[1] = nPoint2;
...
size_t CElementSet::CLineElement::NrOfNodes() const { return 2; }
...
const geo::IElementSet *CElementSet::CLineElement::IndexingElementSet() const { return &m_mesh; }

int CElementSet::CLineElement::Index() const { return m_nIndex; }
...
size_t CElementSet::CRectangleElement::NrOfNodes() const { return 4; }
...
const geo::IElementSet *CElementSet::CRectangleElement::IndexingElementSet() const { return &m_mesh; }

int CElementSet::CRectangleElement::Index() const { return m_nIndex; }

CElementSet::CElementSet(const QString &sName, eElementType type, const TPointVec &vcPoint,
...
CElementSet::CElementSet(const CGoCadVolume &volume, CFemAppModel &model, CQuantity::UNIT coordinate_unit,
                         CQuantity::UNIT value_unit)
  ...
  for (int nProperty = 0; nProperty < volume.Header().PropertySize(); nProperty++) {
    ...
    CElementValueSet *pValueSet =
        new CElementValueSet(volume.Header().Property(nProperty).Name(), CQuantity::SI_UNIT, *this);
...
CElementSet::CElementSet(const QString &sName, CFemAppModel &model, CQuantity::UNIT coordinate_unit,
                         CQuantity::UNIT value_unit)
    : IElementSet(sName, model, coordinate_unit, value_unit) {
  reParent(model.GraphEntry(MD_BASE_POINTSET));
}

CElementSet::CElementSet(const CElementSet &rhs) : IElementSet(rhs), m_eElType(rhs.m_eElType) {
...
int CElementSet::RegisterPoint(const geo::IPoint &point) { return m_mesh.RegisterNode(point, false); }

CElementSet::CElementSet(CFemAppModel &model) : IElementSet(model), m_eElType(UNKNOWN) {}

CElementSet::CElementSet(const CElementSet & /*volume*/, CModelBase &model, TPROGRESS & /*progress*/,
                         std::map<const CValueType *, CValueType *> & /*mpValueType*/)
    : IElementSet(model) {}

CElementSet::CElementSet(const CEclipseFile &file, CModelBase &model) : IElementSet(model), m_eElType(ECLIPSE) {
  ...
  for (size_t nProperty = 0; nProperty < file.PropertySize(); nProperty++) {
    ...
    for (int X = 0; X < file.SizeX(); X++) {
      for (int Y = 0; Y < file.SizeY(); Y++) {
        for (int Z = 0; Z < file.SizeZ(); Z++) {
          std::set<geo::CPoint>::iterator it = ptset.find(
              geo::CPoint(X, Y, Z)); // check to see if the current cell is one of the cells that pinches out....
          if (it == ptset.end())
...
CElementSet::~CElementSet() { m_mesh.Clear(); }

void CElementSet::ElementType(eElementType type) { m_eElType = type; }

bool CElementSet::operator==(const CElementSet &rhs) const { return IElementSet::operator==(rhs); }
...
void CElementSet::OnCoordinateUnitChanged(CQuantity::UNIT old_unit, CQuantity::UNIT new_unit) {
  ...
  for (i = 0; i < m_mesh.NodeSize(); i++) {
    ...
    m_mesh.Node(i, geo::CPoint(lq.Convert(pt.X(), new_unit, old_unit), lq.Convert(pt.Y(), new_unit, old_unit),
                               lq.Convert(pt.Z(), new_unit, old_unit)));
  }
}

CElementSet::CElementSet(const std::set<const geo::IElement *> &stElements, const QString &sName, eElementType type,
                         CFemAppModel &model)
    : IElementSet(sName, model, CQuantity::SI_UNIT, CQuantity::SI_UNIT), m_eElType(type) {
...
CElementSet::CElementSet(const std::vector<const geo::IElement *> &vcElements, const QString &sName, eElementType type,
                         CFemAppModel &model)
    : IElementSet(sName, model, CQuantity::SI_UNIT, CQuantity::SI_UNIT), m_eElType(type) {
  int j;
...
const geo::IElementSet &CElementSet::ElementSet() const { return m_mesh; }

geo::IElementSet &CElementSet::ElementSet() { return m_mesh; }

unsigned int CElementSet::IconId() const { return IDI_ELEMENT_SET; }

unsigned int CElementSet::TypeId() const { return IDT_ELEMENTSET; }

QString CElementSet::TypeName() const { return getStringTableEntry(IDS_ELEMENTSET); }
...
IPointSet::DIMENSION CElementSet::Dimension() const {
  ...
  if (m_eElType == TRIANGLE_2D || m_eElType == QUAD_2D)
    return DIM_2D;
  return DIM_3D;
}

bool CElementSet::Empty() const { return !(m_mesh.ElementSize() > 0); }
      ...
      if (_m()->msg(sMessage, MB_YESNO) == IDYES) {
        ...
        for (int nComponent = 0; nComponent < pCopy->ComponentSize(); nComponent++) {
          ...
          for (nElement = 0; nElement < ElementSet().ElementSize(); nElement++) {
            pValueSet->PushBack(
                pValueType->Component(nComponent).ScalarData().ValueElement(ElementSet().Element(nElement)));
            progress->Step();
          }
        }
