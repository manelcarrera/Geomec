...
CRpnValueSet::CValueSetProxy::CValueSetObserver::~CValueSetObserver() { assert(m_stProxy.size() == 0); }

CRpnValueSet::CValueSetProxy::CValueSetObserver &
CRpnValueSet::CValueSetProxy::CValueSetObserver::operator=(const CValueSetObserver &rhs) {
...
unsigned int CRpnValueSet::CValueSetProxy::CValueSetObserver::IconId() const { return 0; }

unsigned int CRpnValueSet::CValueSetProxy::CValueSetObserver::TypeId() const { return 0; }
...
//////////////////////////////////////////////////////////////////////
CRpnValueSet::CValueSetCoordinateProxy::CValueSetCoordinateProxy(CRpnValueSet &value_set, unsigned int uName,
                                                                 rpn::CRpnStack &stack, COORDINATE coor,
                                                                 const QString &sProxyId)
    : CCoordinateProxy(uName, stack, coor, sProxyId), m_pValueSet(&value_set) {}

CRpnValueSet::CValueSetCoordinateProxy::CValueSetCoordinateProxy(CRpnValueSet &value_set, const QString &sName,
                                                                 rpn::CRpnStack &stack, COORDINATE coor,
                                                                 const QString &sProxyId)
...
unsigned int CRpnValueSet::CValueSetCoordinateProxy::IconId() const { return IDI_VALUETYPE_COORDINATE; }

int CRpnValueSet::CValueSetCoordinateProxy::TypeId() const { return COORDINATE_ID; }

bool CRpnValueSet::CValueSetCoordinateProxy::Recursive(TParentSet /*stParent*/) const { return false; }
...
void CRpnValueSet::CValueSetCoordinateProxy::ValueSet(CRpnValueSet &value_set) { m_pValueSet = &value_set; }
...
CRpnValueSet::CValueSetProxy::CValueSetObserver &CRpnValueSet::CValueSetProxy::Observer() { return *m_pObserver; }

const CRpnValueSet::CValueSetProxy::CValueSetObserver &CRpnValueSet::CValueSetProxy::Observer() const {
  return *m_pObserver;
}

void CRpnValueSet::CValueSetProxy::SetObserver(CValueSetObserver *pObserver) { m_pObserver = pObserver; }
...
int CRpnValueSet::CValueSetProxy::TypeId() const { return VALUE_ID; }
...
CRpnValueSet::TValueVec CRpnValueSet::CValueSetProxy::Value(const geo::IElement &el, UNIT unit) const {
  ...
  IValueSet::TValueVec vcValues =
      m_pObserver->ValueSet().ValueElement(el, geo::IParallelInitializationCallback::Sequential);
...
CRpnValueSet::CRpnValueSet(const CRpnValueSet &rhs)
    : IValueSet(rhs), m_pStack(0), m_stUsers(rhs.m_stUsers), m_model(-1) {
...
CRpnValueSet::CRpnValueSet(IPointSet &point_set, const CRpnValueSet &value_set)
    : IValueSet(value_set.Name().toStdString().c_str(), CQuantity::SI_UNIT, point_set), m_pStack(0),
      m_stUsers(value_set.m_stUsers), m_model(-1) {
...
CRpnValueSet &CRpnValueSet::operator=(const CRpnValueSet &rhs) {
  ...
  for (size_t i = 0; i < vcProxy1.size(); i++) {
    ...
    CValueSetProxy::CValueSetObserver &source =
        dynamic_cast<CValueSetProxy::CValueSetObserver &>(vcProxy1[i]->Observer().Source());
...
unsigned int CRpnValueSet::TypeId() const { return 0; }

// Saving and loading
bool CRpnValueSet::Empty() const { return m_pStack == 0; }
...
void CRpnValueSet::UpdateProxy() {
  ...
  for (int nValueSet = 0; nValueSet < PointSet().ValueSetSize(); nValueSet++) {
    ...
    if ((&value_set != this) && bCreateProxy) {
      ...
      if (PointSet().ValueSet(nValueSet).IsCopy())
        new CValueSetProxy((IValueSet &)PointSet().ValueSet(nValueSet).Source(), *m_pStack, IsCopy() == true);
      else
        new CValueSetProxy(PointSet().ValueSet(nValueSet), *m_pStack, IsCopy() == true);
    }
  }
  ...
  for (TFormationBaseEntry::TNodeSet::iterator it = stNode.begin(); it != stNode.end(); it++) {
    ...
    rpn::CProxyPersistant<CRpnFormationProxy>::TProxyVec vcProxy = psProxy.Proxies(*m_pStack);
    for (size_t i = 0; i < vcProxy.size(); i++) {
      if (&vcProxy[i]->Formation() == *it)
        bCreateProxy = false;
    }

    if (bCreateProxy)
      new CRpnFormationProxy(**it, *m_pStack);
  }
}
...
const rpn::CRpnStack &CRpnValueSet::RpnStack() const { return *m_pStack; }
...
long CRpnValueSet::SavedItems() const { return PointSet().ValueSetSize(); }

int CRpnValueSet::ValueSize() const { return 0; }
...
CRpnValueSet::TValue CRpnValueSet::ValuePoint(const geo::IPoint &pt,
...
CRpnValueSet::TValueVec CRpnValueSet::ValueElement(const geo::IElement &element,
                                                   geo::IParallelInitializationCallback * /*cb*/) const {
  if (m_pStack)
...
bool CRpnValueSet::Used() const { return !m_stUsers.empty(); }

bool CRpnValueSet::CanDestroy() const { return !Used(); }

rpn::CRpnStack *CRpnValueSet::getStack() const { return m_pStack; }

void CRpnValueSet::setStack(rpn::CRpnStack *stack) { m_pStack = stack; }
