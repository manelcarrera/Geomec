...
CFillMaterialParameterProxies::CFillMaterialParameterProxies(CMeshBase &mesh, rpn::CRpnStack &rpnStack)
...
const std::set<std::string> &CFillMaterialParameterProxies::GetNames(const CModelBase &model) {
  if (m_stProxyNames.size() == 0) {
    rpn::CRpnStack rpnStack;
    CFillMaterialParameterProxies proxies(const_cast<CMeshBase &>(model.Mesh()), rpnStack);
    std::vector<IRpnMaterialParameterProxy *> vcProxy = proxies.getMaterialParameterProxies();
...
// waij TFS 92432
//
IRpnMaterialParameterProxy *CFillMaterialParameterProxies::CreateMaterialParameterProxy(const CModelBase &model,
  ...
  if (CFillMaterialParameterProxies::GetNames(model).find(name) !=
    ...
    proxy = CFillMaterialParameterProxies::CreateMaterialParameterProxy(model, name, 0, rpnStack);
  } else if (SimpleSplitTextTag(name, baseName, timeStep) && timeStep != -1) // aap_D1 -> aap,1
  {
    if (CFillMaterialParameterProxies::GetNames(model).find(baseName) !=
        CFillMaterialParameterProxies::GetNames(model).end()) {
      const CDepletionStage *pStage = model.FindDepletionStage(timeStep);

      assert(pStage); // we checked the user input, it should be there
      proxy = CFillMaterialParameterProxies::CreateMaterialParameterProxy(model, baseName, pStage, rpnStack);
    }
  }
...
IRpnMaterialParameterProxy *CFillMaterialParameterProxies::CreateMaterialParameterProxy(const CModelBase &model,
  ...
  CFillMaterialParameterProxies myProxies(const_cast<CMeshBase &>(model.Mesh()), newStack);

  IRpnMaterialParameterProxy *proxy = 0;
  std::vector<IRpnMaterialParameterProxy *> vcProxies = myProxies.getMaterialParameterProxies();

  for (std::vector<IRpnMaterialParameterProxy *>::iterator it = vcProxies.begin(); it < vcProxies.end(); ++it) {
    IRpnMaterialParameterProxy *pr = *it; // assignment for debug

    if (pr->TextTag().toStdString() == name) {
      proxy = dynamic_cast<IRpnMaterialParameterProxy *>(pr->Clone(rpnStack));
...
std::vector<IRpnMaterialParameterProxy *> CFillMaterialParameterProxies::getMaterialParameterProxies() const {
...
CFillMaterialParameterProxies::fillMaterialParameterProxies(CMeshBase &mesh, rpn::CRpnStack &rpnStack) {
  ...
  materialParameterProxies.push_back(
      new CRpnMaterialParameterProxy<TInitialFractureApertureHigh>(mesh, rpnStack, true));
  materialParameterProxies.push_back(
      new CRpnMaterialParameterProxy<TInitialFractureApertureInter>(mesh, rpnStack, true));
