...
CMaterialResult::CMaterialResultComponent::CValueSet::CValueSet(const CMaterialResultComponent &component)
    : m_component(component), m_unit(IQuantityDouble::SI_UNIT) {}
...
void CMaterialResult::CMaterialResultComponent::CValueSet::ElementValues(
...
bool CMaterialResult::CMaterialResultComponent::CValueSet::IsEmpty() const { return false; }

bool CMaterialResult::CMaterialResultComponent::CValueSet::IsValid(const geo::CValue &value) const {
  return value.Valid();
}

void CMaterialResult::CMaterialResultComponent::CValueSet::Unit(IQuantityDouble::UNIT unit) { m_unit = unit; }

//// CMaterialResult::CMaterialResultComponent

CMaterialResult::CMaterialResultComponent::CMaterialResultComponent(const QString &strName,
                                                                    const CDepletionStage &stage,
                                                                    CMaterialResult &parent,
                                                                    const CAnalysisType &antype, int nRegister)
    : IResultComponent(strName, parent, stage, antype, nRegister, 0), m_valueset(*this) {}

CMaterialResult::CMaterialResultComponent::~CMaterialResultComponent() {}

unsigned int CMaterialResult::CMaterialResultComponent::IconId() const { return Parent().IconId(); }

unsigned int CMaterialResult::CMaterialResultComponent::TypeId() const { return Parent().TypeId(); }

QString CMaterialResult::CMaterialResultComponent::UnitName(const UNIT unit) const {
  return MaterialResult().UnitName(unit);
}

long CMaterialResult::CMaterialResultComponent::SavedItems() const { return 0; }

geo::CValue CMaterialResult::CMaterialResultComponent::SingleValueElement(const geo::IElement &elm,
  ...
  if (pHor) {
    ...
    if ((material != 0) && material->IsParameter(MaterialResult().TypeId())) {
      return material->ParameterValue(MaterialResult().TypeId()) * MaterialResult().UnitFactor(unit);
    }
  }
...
geo::CValue CMaterialResult::CMaterialResultComponent::ValuePoint(const geo::IPoint &pt, const UNIT unit,
...
IValueDomainScalar::TValueVec CMaterialResult::CMaterialResultComponent::ElementValues(const geo::IElement &elm,
                                                                                       const UNIT unit) const {
  geo::CValue val = SingleValueElement(elm, unit);
...
void CMaterialResult::CMaterialResultComponent::MapValueElement(const geo::IElement &elm,
...
QString CMaterialResult::CMaterialResultComponent::ExportLabel() const {
  QString s;
  s = QString("%1_D%2").arg(MaterialResult().StoredExportLabel()).arg(Stage().Index());
  return s;
}
...
CMaterialResult::CBoundaryMaterialResultComponent::CBoundaryMaterialResultComponent(const QString &strName,
                                                                                    const CDepletionStage &stage,
                                                                                    CMaterialResult &parent,
                                                                                    const CAnalysisType &antype,
                                                                                    int nRegister, bool showRad)
    : CMaterialResultComponent(strName, stage, parent, antype, nRegister), m_showRad(showRad) {}

CMaterialResult::CBoundaryMaterialResultComponent::~CBoundaryMaterialResultComponent() {}

geo::CValue CMaterialResult::CBoundaryMaterialResultComponent::SingleValueElement(const geo::IElement &elm,
                                                                                  const UNIT unit) const {
  const geo::CInterfaceElement *ifElt = dynamic_cast<const geo::CInterfaceElement *>(&elm);

  if (ifElt && &ifElt->Front() == &ifElt->Back()) {
    const IWellModel::CBoundary *pWellBoundary =
        dynamic_cast<const IWellModel::CBoundary *>(&static_cast<const CModelBase &>(Model()).Boundary());
    ...
    const CInterfaceBoundary *pBoundary =
        dynamic_cast<const CInterfaceBoundary *>(&static_cast<const CModelBase &>(Model()).Boundary());
...
CMaterialResult::CMaterialResult(unsigned int uName, unsigned int uiValueTypeID, unsigned int uiIconID,
                                 CResultGroup &group)
    : IResult(uName, group), m_uiValueTypeID(uiValueTypeID), m_uiIconID(uiIconID) {
  CPointSet ps(Model());
...
bool CMaterialResult::Empty() const { return false; }

long CMaterialResult::SavedItems() const { return 0; }

unsigned int CMaterialResult::IconId() const { return m_uiIconID; }

unsigned int CMaterialResult::TypeId() const { return m_uiValueTypeID; }
...
bool CMaterialResult::isMaterialResult() const { return true; }

bool CMaterialResult::OnDefined(const IResultComponent & /*component*/) const { return true; }

bool CMaterialResult::CanMap(const COpenGLNode & /*node*/, int /*nRegister*/) const { return true; }
...
const QString &CMaterialResult::StoredExportLabel() const { return m_strExportLabel; }
...
void CMaterialResult::BuildComponent(const CDepletionStage &stage, const CAnalysisType &antype, int nRegister) {
  ...
  if (it == m_mpComponents.end())
    m_mpComponents.insert(
        TComponentMap::value_type(idx, new CMaterialResultComponent(Name(), stage, *this, antype, nRegister)));
  else {
    RegisterMode(stage, antype, nRegister);
  }
}

IResultComponent *CMaterialResult::OnGetResultComponent(int nTimeStep, const CAnalysisType & /*antype*/,
                                                        int /*nRegister*/, unsigned int /*nComponent*/) const {
...
bool CMaterialResult::ValidName(const std::string & /*name*/
                                ,
...
CBoundaryMaterialResult::CBoundaryMaterialResult(unsigned int uName, unsigned int uiValueTypeID, unsigned int uiIconID,
                                                 CResultGroup &group)
...
void CBoundaryMaterialResult::BuildComponent(const CDepletionStage &stage, const CAnalysisType &antype, int nRegister) {
  ...
  if (it == m_mpComponents.end())
    m_mpComponents.insert(
        TComponentMap::value_type(idx, new CBoundaryMaterialResultComponent(Name(), stage, *this, antype, nRegister,
                                                                            TypeId() == IDT_VALUETYPE_KRAD)));
  else {
    RegisterMode(stage, antype, nRegister);
  }
}
...
CMaterialResultTree::CMaterialResultTree(CModelBase &model) : CResultGroup("Material Parameters", model) {
  ...
  new CMaterialResult(IDS_VALUENAME_PRECONSOLIDATION, IDT_VALUETYPE_PRECONSOLIDATION, IDI_VALUETYPE_PRECONSOLIDATION,
                      *this);
  ...
  //  new CMaterialResult(IDS_VALUENAME_EQUIV_PLAST_STRAIN1, IDT_VALUETYPE_EQUIV_PLAST_STRAIN1,
  //  IDI_VALUETYPE_EQUIV_PLAST_STRAIN1, *this); new CMaterialResult(IDS_VALUENAME_EQUIV_PLAST_STRAIN2,
  //  IDT_VALUETYPE_EQUIV_PLAST_STRAIN2, IDI_VALUETYPE_EQUIV_PLAST_STRAIN2, *this); new
  //  CMaterialResult(IDS_VALUENAME_EQUIV_PLAST_STRAIN3, IDT_VALUETYPE_EQUIV_PLAST_STRAIN3,
  //  IDI_VALUETYPE_EQUIV_PLAST_STRAIN3, *this);
  ...
  new CMaterialResult(IDS_VALUENAME_TENSILE_STRETCH, IDT_VALUETYPE_TENSILE_STRETCH, IDI_VALUETYPE_TENSILE_STRETCH,
                      *this);
  new CMaterialResult(IDS_VALUENAME_THERMAL_EXPANSION, IDT_VALUETYPE_THERMAL_EXPANSION, IDI_VALUETYPE_THERMAL_EXPANSION,
                      *this);
  new CMaterialResult(IDS_VALUENAME_ELASTIC_HARDENING, IDT_VALUETYPE_ELASTIC_HARDENING, IDI_VALUETYPE_ELASTIC_HARDENING,
                      *this);
  ...
  new CMaterialResult(IDS_VALUENAME_YOUNGMODULUS_NORM, IDT_VALUETYPE_YOUNGMODULUS_NORM, IDI_VALUETYPE_YOUNGMODULUS_NORM,
                      *this);
  new CMaterialResult(IDS_VALUENAME_YOUNGMODULUS_TRANS, IDT_VALUETYPE_YOUNGMODULUS_TRANS,
                      IDI_VALUETYPE_YOUNGMODULUS_TRANS, *this);
  new CMaterialResult(IDS_VALUENAME_POISSONRATIO_NORM, IDT_VALUETYPE_POISSONRATIO_NORM, IDI_VALUETYPE_POISSONRATIO_NORM,
                      *this);
  new CMaterialResult(IDS_VALUENAME_POISSONRATIO_TRANS, IDT_VALUETYPE_POISSONRATIO_TRANS,
                      IDI_VALUETYPE_POISSONRATIO_TRANS, *this);
  new CMaterialResult(IDS_VALUENAME_LAYER_NORMAL_INCL, IDT_VALUETYPE_LAYER_NORMAL_INCL, IDI_VALUETYPE_LAYER_NORMAL_INCL,
                      *this);
  new CMaterialResult(IDS_VALUENAME_LAYER_NORMAL_AZI, IDT_VALUETYPE_LAYER_NORMAL_AZI, IDI_VALUETYPE_LAYER_NORMAL_AZI,
                      *this);
  new CMaterialResult(IDS_VALUENAME_FRACT_STIFF_NORMAL, IDT_VALUETYPE_FRACT_STIFF_NORMAL,
                      IDI_VALUETYPE_FRACT_STIFF_NORMAL, *this);
  new CMaterialResult(IDS_VALUENAME_FRACT_STIFF_SHEAR, IDT_VALUETYPE_FRACT_STIFF_SHEAR, IDI_VALUETYPE_FRACT_STIFF_SHEAR,
                      *this);
  //  new CMaterialResult(IDS_VALUENAME_HIGH_FRACT_DENS, IDT_VALUETYPE_HIGH_FRACT_DENS, IDI_VALUETYPE_HIGH_FRACT_DENS,
  //  *this); new CMaterialResult(IDS_VALUENAME_HIGH_FRACT_DENS_INCL, IDT_VALUETYPE_HIGH_FRACT_DENS_INCL,
  //  IDI_VALUETYPE_HIGH_FRACT_DENS_INCL, *this); new CMaterialResult(IDS_VALUENAME_HIGH_FRACT_DENS_AZI,
  //  IDT_VALUETYPE_HIGH_FRACT_DENS_AZI, IDI_VALUETYPE_HIGH_FRACT_DENS_AZI, *this); new
  //  CMaterialResult(IDS_VALUENAME_LOW_FRACT_DENS, IDT_VALUETYPE_LOW_FRACT_DENS, IDI_VALUETYPE_LOW_FRACT_DENS, *this);
  //  new CMaterialResult(IDS_VALUENAME_LOW_FRACT_DENS_INCL, IDT_VALUETYPE_LOW_FRACT_DENS_INCL,
  //  IDI_VALUETYPE_LOW_FRACT_DENS_INCL, *this); new CMaterialResult(IDS_VALUENAME_LOW_FRACT_DENS_AZI,
  //  IDT_VALUETYPE_LOW_FRACT_DENS_AZI, IDI_VALUETYPE_LOW_FRACT_DENS_AZI, *this); new
  //  CMaterialResult(IDS_VALUENAME_INTER_FRACT_DENS, IDT_VALUETYPE_INTER_FRACT_DENS, IDI_VALUETYPE_INT_FRACT_DENS,
  //  *this);
  new CMaterialResult(IDS_VALUENAME_THERM_LIN_EXP_NORM, IDT_VALUETYPE_THERM_LIN_EXP_NORM,
                      IDI_VALUETYPE_THERM_LIN_EXP_NORM, *this);
  new CMaterialResult(IDS_VALUENAME_THERM_LIN_EXP_LAT, IDT_VALUETYPE_THERM_LIN_EXP_LAT, IDI_VALUETYPE_THERM_LIN_EXP_LAT,
                      *this);
  ...
  new CMaterialResult(IDS_VALUENAME_REFERENCESTRESS, IDT_VALUETYPE_REFERENCESTRESS, IDI_VALUETYPE_REFERENCESTRESS,
                      *this);
  ...
  new CMaterialResult(IDS_VALUENAME_YOUNGMODULUS_NORM_DECOMP, IDT_VALUETYPE_YOUNGMODULUS_NORM_DECOMP,
                      IDI_VALUETYPE_YOUNGMODULUS_NORM_DECOMP, *this);
  new CMaterialResult(IDS_VALUENAME_YOUNGMODULUS_TRANS_DECOMP, IDT_VALUETYPE_YOUNGMODULUS_TRANS_DECOMP,
                      IDI_VALUETYPE_YOUNGMODULUS_TRANS_DECOMP, *this);
  new CMaterialResult(IDS_VALUENAME_POISSONRATIO_NORM_DECOMP, IDT_VALUETYPE_POISSONRATIO_NORM_DECOMP,
                      IDI_VALUETYPE_POISSONRATIO_NORM_DECOMP, *this);
  new CMaterialResult(IDS_VALUENAME_POISSONRATIO_TRANS_DECOMP, IDT_VALUETYPE_POISSONRATIO_TRANS_DECOMP,
                      IDI_VALUETYPE_POISSONRATIO_TRANS_DECOMP, *this);
  new CMaterialResult(IDS_VALUENAME_ANISOTROPIC_SHEARMODULUS, IDT_VALUETYPE_ANISOTROPIC_SHEARMODULUS,
                      IDI_VALUETYPE_ANISOTROPIC_SHEARMODULUS, *this);
  new CMaterialResult(IDS_VALUENAME_ANISOTROPIC_SHEARMODULUS_DECOMP, IDT_VALUETYPE_ANISOTROPIC_SHEARMODULUS_DECOMP,
                      IDI_VALUETYPE_ANISOTROPIC_SHEARMODULUS_DECOMP, *this);
  ...
  new CMaterialResult(IDS_VALUENAME_THOMSEN_EPSILON, IDT_VALUETYPE_THOMSEN_EPSILON, IDI_VALUETYPE_THOMSEN_EPSILON,
                      *this);
  ...
  new CMaterialResult(IDS_VALUENAME_REFERENCE_APERTURE, IDT_VALUETYPE_REFERENCE_APERTURE,
                      IDI_VALUETYPE_REFERENCE_APERTURE, *this);

  CResultGroup *pFractGroup = new CResultGroup(IDS_VALUENAME_FRACT_TENSOR, *this);
  new CMaterialResult(IDS_VALUENAME_HIGH_FRACT_DENS, IDT_VALUETYPE_HIGH_FRACT_DENS, IDI_VALUETYPE_HIGH_FRACT_DENS,
                      *pFractGroup);
  new CMaterialResult(IDS_VALUENAME_HIGH_FRACT_DENS_INCL, IDT_VALUETYPE_HIGH_FRACT_DENS_INCL,
                      IDI_VALUETYPE_HIGH_FRACT_DENS_INCL, *pFractGroup);
  new CMaterialResult(IDS_VALUENAME_HIGH_FRACT_DENS_AZI, IDT_VALUETYPE_HIGH_FRACT_DENS_AZI,
                      IDI_VALUETYPE_HIGH_FRACT_DENS_AZI, *pFractGroup);
  new CMaterialResult(IDS_VALUENAME_LOW_FRACT_DENS, IDT_VALUETYPE_LOW_FRACT_DENS, IDI_VALUETYPE_LOW_FRACT_DENS,
                      *pFractGroup);
  new CMaterialResult(IDS_VALUENAME_LOW_FRACT_DENS_INCL, IDT_VALUETYPE_LOW_FRACT_DENS_INCL,
                      IDI_VALUETYPE_LOW_FRACT_DENS_INCL, *pFractGroup);
  new CMaterialResult(IDS_VALUENAME_LOW_FRACT_DENS_AZI, IDT_VALUETYPE_LOW_FRACT_DENS_AZI,
                      IDI_VALUETYPE_LOW_FRACT_DENS_AZI, *pFractGroup);
  new CMaterialResult(IDS_VALUENAME_INTER_FRACT_DENS, IDT_VALUETYPE_INTER_FRACT_DENS, IDI_VALUETYPE_INT_FRACT_DENS,
                      *pFractGroup);
...
unsigned int CMaterialResultTree::IconId() const { return IDI_ROCK_MATERIAL; }
