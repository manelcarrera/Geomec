...
ITensorGroup::ITensorGroup(const QString &sName, CResultGroup &parent)
    : CResultGroup(sName, parent), m_pComponent(0), m_pCylindric(0) {
...
ITensorGroup::ITensorGroup(unsigned int uName, CResultGroup &parent)
    : CResultGroup(uName, parent), m_pComponent(0), m_pCylindric(0) {
  m_pPrincipal[CVectorComposite::PD_MAX] = 0;
  m_pPrincipal[CVectorComposite::PD_MED] = 0;
  m_pPrincipal[CVectorComposite::PD_MIN] = 0;
}
...
bool ITensorGroup::OnBuildComponent(const CDepletionStage & /*stage*/, const CAnalysisType & /*antype*/,
                                    int /*nRegister*/) const {
  return true;
}
...
void ITensorGroup::CreatePrincipalDirections() {
  ...
  m_pPrincipal[CVectorComposite::PD_MAX] =
      new CVectorComposite(IDS_RG_PRINC_MAX, *pPrincipalDirections, CVectorComposite::PD_MAX);
  m_pPrincipal[CVectorComposite::PD_MED] =
      new CVectorComposite(IDS_RG_PRINC_MED, *pPrincipalDirections, CVectorComposite::PD_MED);
  m_pPrincipal[CVectorComposite::PD_MIN] =
      new CVectorComposite(IDS_RG_PRINC_MIN, *pPrincipalDirections, CVectorComposite::PD_MIN);
}
...
ITensorGroup::CTensorData::CTensorData(const IResultComponent &parent)
  ...
  if (pTensorGroup) {
    ...
    const ITensorGroup::CVectorComposite *pResult =
        static_cast<const ITensorGroup::CVectorComposite *>(&parent.Parent());
...
IValueDataInterfaceTensor::TMinMax ITensorGroup::CTensorData::MinMax(IProgressBase &,
                                                                     const IValueDataInterface::UNIT) const {
  return TMinMax(m_Min, m_Max);
}

IValueDataInterfaceTensor::TValue
ITensorGroup::CTensorData::ValuePoint(const geo::IPoint &pt, const IValueDataInterface::UNIT /*unit*/,
                                      geo::IParallelInitializationCallback *cb) const {
  return m_Group->TensorPoint(pt, m_Parent, cb);
}

IValueDataInterfaceTensor::TValue ITensorGroup::CTensorData::Average(IProgressBase &,
                                                                     const IValueDataInterface::UNIT) const {
...
IValueDataInterfaceTensor::TValueVec
ITensorGroup::CTensorData::ValueElement(const geo::IElement &elt, const IValueDataInterface::UNIT unit,
                                        geo::IParallelInitializationCallback *cb) const {
  IValueDataInterfaceTensor::TValueVec tensors;
  tensors.reserve(elt.NrOfNodes());

  for (size_t i = 0; i < elt.NrOfNodes();
       ++i) // TODO: for now we do it like this, but ITensorGroup needs to return a TValueVec, with the right values
  {
...
void ITensorGroup::CComponentComposite::BuildComponent(const CDepletionStage &stage, const CAnalysisType &antype,
                                                       int nRegister) {
...
unsigned int ITensorGroup::CComponentComposite::IconId() const { return IDI_RESULT_MAP; }
...
bool ITensorGroup::CComponentComposite::ValidName(const std::string &name, CResultInfo &resultInfo) const {
  // Get the type name of the current tensor
  const ITensorGroup *pParent = dynamic_cast<const ITensorGroup *>(Parent());
  assert(pParent);
  ...
  return validateName(resultInfo, name, sCombinedName, TensorResultComponentList(), "_L|_M|_C|_N", pParent);
}
...
ITensorGroup::CComponentComposite::CComponent::CComponent(unsigned int uComponentName, CComponentComposite &parent,
                                                          enum TENSOR_COMPONENT component, const CDepletionStage &stage,
...
unsigned int ITensorGroup::CComponentComposite::CComponent::IconId() const { return IDI_RESULT; }

unsigned int ITensorGroup::CComponentComposite::CComponent::TypeId() const { return 0; }
...
geo::CValue ITensorGroup::CComponentComposite::CComponent::ValuePoint(const geo::IPoint &pt, const UNIT unit,
                                                                      geo::IParallelInitializationCallback *cb) const {
  const IResult &result = (const IResult &)(Parent());
  const ITensorGroup *pParent = dynamic_cast<const ITensorGroup *>(result.Parent());
  assert(pParent);
...
void ITensorGroup::CComponentComposite::CComponent::MapValueElement(const geo::IElement &elm,
...
bool ITensorGroup::CComponentComposite::CComponent::NeedParallelInitializationCallback() const { return true; }

geo::IParallelInitializationCallback *
ITensorGroup::CComponentComposite::CComponent::GetParallelInitializationCallback() {
...
unsigned int ITensorGroup::CComponentComposite::CFullTensorComponent::IconId() const { return IDI_RESULT; }

unsigned int ITensorGroup::CComponentComposite::CFullTensorComponent::TypeId() const { return 0; }
...
geo::CValue ITensorGroup::CComponentComposite::CFullTensorComponent::ValuePoint(
    const geo::IPoint & /*pt*/, const UNIT /*unit*/, geo::IParallelInitializationCallback * /*cb*/) const {
  return geo::CValue();
}

void ITensorGroup::CComponentComposite::CFullTensorComponent::MapValueElement(
...
IValueDataInterfaceTensor *ITensorGroup::CComponentComposite::CFullTensorComponent::TensorData() { return &m_Data; }

void ITensorGroup::CComponentComposite::CFullTensorComponent::ResetMinMax() const { m_Data.ResetMinMax(); }
...
geo::IParallelInitializationCallback *
ITensorGroup::CComponentComposite::CFullTensorComponent::GetParallelInitializationCallback() {
  ITensorGroup *pTensorGroup = dynamic_cast<ITensorGroup *>(Parent().parent());
  assert(pTensorGroup);
  return pTensorGroup->GetParallelInitializationCallback();
}
...
bool ITensorGroup::CVectorComposite::ValidName(const std::string &name, CResultInfo &resultInfo) const {
  // Get direction
  ...
  std::string sCombinedName = sPreName + std::string("P") + sName + sDirection + sPostName;

  return validateName(resultInfo, name, sCombinedName, VectorResultComponentList(), "_L|_M|_C|_N");
}

bool ITensorGroup::CVectorComposite::OnBuildComponent(const CDepletionStage &stage, const CAnalysisType &antype,
                                                      int nRegister) const {
...
const ITensorGroup *ITensorGroup::CVectorComposite::TensorGroup() { return m_pTensorGroup; }

const ITensorGroup *ITensorGroup::CVectorComposite::TensorGroup() const { return m_pTensorGroup; }
...
ITensorGroup::CVectorComposite::CVectorComposite(const QString &sName, CResultGroup &group,
...
const geo::IVector &ITensorGroup::CVectorComposite::VectorPoint(const geo::IPoint &point,
                                                                const IResultComponent &component,
                                                                geo::IParallelInitializationCallback *cb) const {
  return m_pTensorGroup->VectorPoint(point, m_direction, component, cb).first;
}

const geo::IVector &ITensorGroup::CVectorComposite::VectorElement(const geo::IElement &element, int nNodeIndex,
...
const ITensor &ITensorGroup::CVectorComposite::TensorElement(const geo::IElement &element, int nNodeIndex,
                                                             TMapType map_type, const IResultComponent &component,
                                                             geo::IParallelInitializationCallback *cb) const {
  return m_pTensorGroup->TensorElement(element, nNodeIndex, map_type, component, cb);
}

void ITensorGroup::CVectorComposite::BuildComponent(const CDepletionStage &stage, const CAnalysisType &antype,
                                                    int nRegister) {
...
ITensorGroup::CVectorComposite::CVectorComponent::CVectorComponent(
    unsigned int uComponentName,  // Name from resource
    ...
    : IVectorResult::CVectorComponent(uComponentName, parent, uComponent, stage, antype, nRegister),
      m_TensorData(*this) {}
...
IValueDataInterfaceTensor *ITensorGroup::CVectorComposite::CVectorComponent::TensorData() { return &m_TensorData; }

void ITensorGroup::CVectorComposite::CVectorComponent::ResetMinMax() const { m_TensorData.ResetMinMax(); }

bool ITensorGroup::CVectorComposite::CVectorComponent::NeedParallelInitializationCallback() const { return true; }

geo::IParallelInitializationCallback *
ITensorGroup::CVectorComposite::CVectorComponent::GetParallelInitializationCallback() {
  ITensorGroup *pTensorGroup = dynamic_cast<ITensorGroup *>(Parent().parent());
  if (!pTensorGroup)
...
ITensorGroup::CVectorComposite::CLengthComponent::CLengthComponent(
    IVectorResult &parent,        // The vector composite parent
    const CDepletionStage &stage, // Depletion stage of the component
    const CAnalysisType &antype,  // Analysis type
    int nRegister,
...
geo::CValue
ITensorGroup::CVectorComposite::CLengthComponent::ValuePoint(const geo::IPoint &pt, const UNIT unit,
                                                             geo::IParallelInitializationCallback *cb) const {
  ...
  if (pTensorGroup->VectorPoint(pt, pResult->Direction(), *this, cb).first.Empty())
    return geo::CValue();
...
void ITensorGroup::CVectorComposite::CLengthComponent::MapValueElement(const geo::IElement &elm,
                                                                       IValueDomainScalar::TValueVec &values,
                                                                       TMapType map_type, UNIT unit,
                                                                       geo::IParallelInitializationCallback *cb) const {
...
/*
geo::CVector ITensorGroup::CVectorComposite::VectorAtComponent(const geo::IElement& mesh_element, int nNodeIndex, const
IResultComponent& component, UNIT unit) const
{
  const ITensorGroup* pParent = dynamic_cast<const ITensorGroup*>(Parent()->Parent());
  assert(pParent);
...
ITensorGroup::CVectorComposite::CFullVectorComponent::CFullVectorComponent(
    IVectorResult &parent,        // The vector composite parent
    const CDepletionStage &stage, // Depletion stage of the component
    const CAnalysisType &antype,  // Analysis type
    int nRegister)
...
IValueDataInterfaceTensor *ITensorGroup::CVectorComposite::CFullVectorComponent::TensorData() { return &m_TensorData; }
...
bool ITensorGroup::CVectorComposite::CFullVectorComponent::NeedParallelInitializationCallback() const { return true; }

geo::IParallelInitializationCallback *
ITensorGroup::CVectorComposite::CFullVectorComponent::GetParallelInitializationCallback() {
  const CVectorComposite *pResult = dynamic_cast<const CVectorComposite *>(&Parent());
  assert(pResult);
  const ITensorGroup *pTensorGroup = pResult->TensorGroup();
  assert(pTensorGroup);
...
void ITensorGroup::PrincipalDirections(geo::CVector &vcmax, double &max_length, geo::CVector &vcmed, double &med_length,
                                       geo::CVector &vcmin, double &min_length, const ITensor &tensor) const {
  if (tensor.Empty())
    vcmax = vcmed = vcmin = geo::CVector();
  ...
  if ((fabs(max_length - med_length) < EPS) &&
      !(fabs(min_length - med_length) < EPS)) { // Max and Med principal values are equal.
    TVectorSet stVector;
  ...
  if ((fabs(min_length - med_length) < EPS) &&
      !(fabs(max_length - med_length) < EPS)) { // Min and Med principal values are equal.
    TVectorSet stVector;
    stVector.insert(vcmed);
...
QString ITensorGroup::CVectorComposite::UnitName(UNIT unit) const { return m_pTensorGroup->UnitName(unit); }

unsigned int ITensorGroup::CVectorComposite::IconId() const { return IDI_RESULT_MAP; }
