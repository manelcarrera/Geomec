...
unsigned int CFormationObserver::IconId() const { return 0; }

unsigned int CFormationObserver::TypeId() const { return 0; }
...
IRpnFormationProxyBase::IRpnFormationProxyBase(const IRpnFormationProxyBase &proxy, rpn::CRpnStack &NewStack)
    : IValueProxy(NewStack, proxy.ProxyId().toStdString().c_str()), m_nIndex(-1) {
...
int IRpnFormationProxyBase::Index() const { return m_nIndex; }
...
CFemAppModel *IRpnFormationProxyBase::FindModel(CFemAppModel &model) {
  ...
  const CZoominModelEntry *pEntry1 =
      dynamic_cast<const CZoominModelEntry *>(models[0]->GraphEntry(MD_BASE_ZOOMIN_MODEL));
  if (pEntry1) {
...
void IRpnFormationProxyBase::AttachToModel(CFemAppModel &model) {
  ...
  if (pFormation) // a pointset rpn formula may be explicitly linked to a zoom-in model, and not find the formations in
                  // the main model (Bug 115938)
...
bool CRpnFormationProxy::Recursive(TParentSet /*stParent*/) const { return false; }
...
unsigned int CRpnFormationProxy::IconId() const { return Formation().IconId(); }

int CRpnFormationProxy::TypeId() const { return FORMATION_ID; }

QString CRpnFormationProxy::TextTag() const { return Formation().Name(); }
...
CRpnFormationThicknessProxy::CRpnFormationThicknessProxy(const CRpnFormationThicknessProxy &proxy,
...
QString CRpnFormationThicknessProxy::TextTag() const { return Formation().Name() + " thickness"; }
...
unsigned int CRpnFormationThicknessProxy::IconId() const { return IDI_FORMATION_THICKNESS; }

bool CRpnFormationThicknessProxy::Recursive(TParentSet /*stParent*/) const { return false; }

bool CRpnFormationThicknessProxy::Defined() const { return (Formation().ElementSetSize() > 0); }

bool CRpnFormationThicknessProxy::exists() const { return Defined(); }

int CRpnFormationThicknessProxy::TypeId() const { return FORMATION_THICKNESS_ID; }
...
bool CRpnReservoirProxy::IsElementDepleting(const geo::IElement &element) const {
  ...
  IValueDomainScalar::TValueVec vcInitialValues =
      pFormation->Pressure(*pStage).Component().ScalarData().ValueElement(element);

  while (pStage) {
    if (!pStage->Initial()) {
      IValueDomainScalar::TValueVec vcCurrent =
          pFormation->Pressure(*pStage).Component().ScalarData().ValueElement(element);
...
bool CRpnReservoirProxy::Recursive(TParentSet /*stParent*/) const { return false; }

QString CRpnReservoirProxy::TextTag() const { return "IsReservoir"; }

void CRpnReservoirProxy::Mesh(CMeshBase &mesh) { m_pMesh = &mesh; }
...
unsigned int CRpnReservoirProxy::IconId() const { return IDI_ISRESERVOIR; }

int CRpnReservoirProxy::TypeId() const { return RESERVOIR_ID; }
