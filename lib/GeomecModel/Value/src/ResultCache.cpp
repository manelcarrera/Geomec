...
template <typename Type> class blocked_range {
  Type m_begin;
  Type m_end;
...
public:
  ...
  virtual size_t size() const { return m_pHorizon->InterfaceElementGroup()->ElementSize(); }
};
...
public:
  ...
  virtual size_t size() const { return m_pElementSet->ElementSize(); }
};
...
public:
  ElementPtrArrayAccessor(const geo::CPtrArray<geo::IElement> &elementPtrArray)
      : m_pElementPtrArray(&elementPtrArray) {}
  virtual const geo::IElement &element(AccessorIterator &accessorIterator) const {
    return m_pElementPtrArray->Object(accessorIterator.index);
  }
  virtual size_t size() const { return m_pElementPtrArray->Size(); }
};
...
struct ConnectionInfo {
  ...
  bool operator<(const ConnectionInfo &rhs) const { return *node < *rhs.node; }
};
...
public:
#ifndef FAKE_TBB
  TaskConnectionInfo(const AccessorBase *object, tbb::concurrent_vector<ConnectionInfo> &data)
      : m_object(object), m_data(data), m_dispatcher(0) {}

  tbb::concurrent_vector<ConnectionInfo> &data() const { return m_data; }
#else
  TaskConnectionInfo(const AccessorBase *object, std::vector<ConnectionInfo> &data)
      : m_object(object), m_data(data), m_dispatcher(0) {}
...
public:
#ifndef FAKE_TBB
  TaskAveraging(tbb::concurrent_vector<ConnectionInfo> &data, tbb::concurrent_vector<ConnectionInfoIndex> &index,
                CResultCache &cache)
...
CResultAverager::CResultAverager(const AccessorBase &object, CResultCache &cache) : m_object(&object), m_cache(cache) {}
...
#endif
{
  ...
  if (objectSize < 4000) // guess work
  {
    ...
    mp::IDispatchedTask *disTaskInit1 =
        NEW_DISPATCH_TASK(mp::CKernelSequential, TaskConnectionInfo)(ciKernel, taskConnectionInfo);
    ...
    // unfortunately, tbb slows down a lot if we try to adjust for "non-aligned" indices in-situ, so we gather the index
    // info here and feed that to tbb
    infoIndex.reserve(info.size() / 4);
    ...
    for (size_t c = 0; c < m_cache.NumColumns(); ++c) {
      ...
      mp::IDispatchedTask *disTaskInit2 =
          NEW_DISPATCH_TASK(mp::CKernelSequential, TaskAveraging)(aKernel, taskAveraging);
    ...
    mp::IDispatchedTask *disTaskInit1 =
        NEW_DISPATCH_TASK(mp::CKernelParallel, TaskConnectionInfo)(ciKernel, taskConnectionInfo);
    dispatcher.launch(progdlg, disTaskInit1);
...
CResultCache::CResultCache(CModelBase &model, const geo::IMesh &mesh, int nColumns, int nCacheSizeMb)
    : m_model(model), m_mesh(mesh), m_nColumns(nColumns), m_nCacheSizeMb(validateCacheSize(nCacheSizeMb)),
      m_pAveragedCache(0) {
  QString tmp = CreateTempDirExt(CTempPath::TEMP_GENERAL);
  mdc::IStorageInterface *pStorageInterface =
      model.GetConsistencyGuard()->HDF5StorageInterface(model.Index(), nColumns / 150, 30);
...
CResultCache::CResultCache(CModelBase &model, const geo::IMesh &mesh, int nColumns)
    : m_model(model), m_mesh(mesh), m_nColumns(nColumns), m_nCacheSizeMb(0), m_pAveragedCache(0) {
  mdc::IStorageInterface *pStorageInterface =
      model.GetConsistencyGuard()->HDF5StorageInterface(model.Index(), nColumns / 150, 30);
  if (!pStorageInterface) {
...
mdc::CMeshDataCacher &CResultCache::RawCacher() { return *m_pRawCache; }
...
int CResultCache::NumColumns() const { return m_nColumns; }
...
bool CResultCache::HasAveragedCache() const { return m_pAveragedCache != 0; }
...
void CResultCache::switchRawCacher(bool enableResultCache, int resultCacheSize, bool bForce) {
  if (enableResultCache) {
    ...
    QString tmpDir = m_pRawCache->StorageInterface()->Directory()
                         ? QString(m_pRawCache->StorageInterface()->Directory())
                         : CreateTempDirExt(CTempPath::TEMP_GENERAL);

    m_pRawCache->Resize(tmpDir.toStdString().c_str(), m_nCacheSizeMb);

    if (m_pAveragedCache) {
      tmpDir = m_pAveragedCache->StorageInterface()->Directory()
                   ? QString(m_pAveragedCache->StorageInterface()->Directory())
                   : CreateTempDirExt(CTempPath::TEMP_GENERAL);
...
void CResultCache::resizeRawCacher(int /*oldResultCacheSize*/, int newResultCacheSize) {
...
void CResultCache::FinalizeStorage() {
  ...
  if (m_pRawCache->StorageInterface()->Directory()) // swapping to disk?
  {
    QString tmp = CreateTempDirExt(CTempPath::TEMP_GENERAL);
    m_pAveragedCache =
        new mdc::CMeshDataCacher(m_mesh, new mdc::CStorageInterfaceDefault(tmp.toStdString().c_str()), m_nCacheSizeMb);
  } else {
  ...
  const TFormationBaseEntry &formation_entry =
      static_cast<const TFormationBaseEntry &>(*m_model.GraphEntry(MD_BASE_FORMATION));
  ...
  // there will be two passes over the elements, the first is over the elements once, the second is over the elements
  ...
  public:
    CScopeGuard(mdc::CMeshDataCacher **ppAveragedCacher, IProgressBase &progdlg)
        : m_ppAveragedCacher(ppAveragedCacher), m_dlg(progdlg) {}
    ...
    void ReleaseCacher() { m_ppAveragedCacher = 0; }
