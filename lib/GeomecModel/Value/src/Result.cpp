...
const CResultGroup *CResultGroup::Parent() const { return m_pParent; }

CResultGroup *CResultGroup::Parent() { return m_pParent; }

int CResultGroup::ChildSize() const { return m_vcChild.size(); }
...
void CResultGroup::PushBack(CGraphNode &child) { m_vcChild.push_back(&child); }
...
unsigned int CResultGroup::IconId() const { return IDI_RESULT_MAP; }

unsigned int CResultGroup::TypeId() const { return 0; }
...
bool CResultGroup::ValidName(const std::string &name, CResultInfo &resultInfo) const {
  for (int ii = 0; ii < ChildSize(); ++ii) {
    const CResultGroup *pResultGroup = dynamic_cast<const CResultGroup *>(&Child(ii));

    if (pResultGroup) {
      if (pResultGroup->ValidName(name, resultInfo))
        return true;
    } else {
      const IResult *pIResult = dynamic_cast<const IResult *>(&Child(ii));
      if (pIResult) {
...
IResult::CResultKey::CResultKey(int nStage, const CAnalysisType &antype, int nRegister)
    : m_antype(antype), m_nStage(nStage), m_nRegister(nRegister) {}
...
QString IResult::ExportLabel() const { return ExportLabel(Component().ComponentIndex()); }

const CResultGroup *IResult::Parent() const { return &m_group; }

CResultGroup *IResult::Parent() { return &m_group; }
...
unsigned int IResult::IconId() const { return IDI_RESULT; }

unsigned int IResult::TypeId() const { return 0; }
...
std::vector<geo::CValue> IResult::ResultAtTime(const CAnalysisType &antype, const CGeomecTime &time,
  ...
  if (pStage1 == pStage2) {
    ...
    std::vector<geo::CValue> vals1 =
        Component(nComponent, Mode(antype, *pStage1)).ScalarData().ValueElement(element, unit);
    std::vector<geo::CValue> vals2 =
        Component(nComponent, Mode(antype, *pStage2)).ScalarData().ValueElement(element, unit);
...
geo::CValue IResult::ResultAtTime(const CAnalysisType &antype, const CGeomecTime &time, const geo::IPoint &pt,
                                  const IQuantityDouble::UNIT unit /*= IQuantityDouble::SI_UNIT*/,
                                  int nComponent /* = 0 */) const {
  std::pair<const CDepletionStage *, const CDepletionStage *> pr = NeighboringStages(time);
  const CDepletionStage *pStage1 = pr.first;
...
std::pair<const CDepletionStage *, const CDepletionStage *> IResult::NeighboringStages(const CGeomecTime &time) const {
  ...
  while (pStage) {
    if (pStage == &model.InitialDepletionStage()) {
      if (pStage->Time() > time) { // time is before initial depletion pStage
        pStage1 = pStage;
        pStage2 = pStage;
        break;
      }
    ...
    if (pStage->Time() < time) {
      if (pStage->Last()) { // time is after last depletion pStage, take the value at the last pStage
        pStage1 = pStage;
        pStage2 = pStage;
        break;
      }
...
const IResultComponent *IResult::ResultComponent(const CDepletionStage &stage, const CAnalysisType &antype,
                                                 int nRegister, unsigned int nComponent) const {
  return ResultComponent(stage.Index(), antype, nRegister, nComponent);
}

IResultComponent *IResult::ResultComponent(const CDepletionStage &stage, const CAnalysisType &antype, int nRegister,
                                           unsigned int nComponent) {
  return ResultComponent(stage.Index(), antype, nRegister, nComponent);
}

const IResultComponent *IResult::ResultComponent(int nTimeStep, const CAnalysisType &antype, int nRegister,
...
IResultComponent *IResult::ResultComponent(int nTimeStep, const CAnalysisType &antype, int nRegister,
                                           unsigned int nComponent) {
  return OnGetResultComponent(nTimeStep, antype, nRegister, nComponent);
  /*
...
IResultComponent *IResult::OnGetResultComponent(int nTimeStep, const CAnalysisType &antype, int nRegister,
                                                unsigned int nComponent) const {
  const IResultComponent *pRes = 0;
  TResultMap::const_iterator it = m_mpResult.find(CResultKey(nTimeStep, antype, nRegister));
  if (it != m_mpResult.end())
...
bool IResult::Empty() const { return false; }

long IResult::SavedItems() const { return 0; }
...
bool IResult::isResult() const { return !isMaterialResult(); }
...
bool IResult::OnBuildComponent(const CDepletionStage & /*stage*/, const CAnalysisType & /*antype*/,
                               int /*nRegister*/) const {
  return true;
}

bool IResult::validateName(CResultInfo &resultInfo, const std::string &name, const std::string &label,
  ...
  valid = SplitTextTag(name, componentList, analysisTypeList, baseTag, component, timeStep, analysisType);
...
bool IResult::validateDerivedName(CResultInfo &resultInfo, const std::string &name, const std::string &label,
                                  const std::string &componentList, const std::string &analysisTypeList) const {
  bool valid = false;
...
IResultComponent::IResultComponent(unsigned int uName, IResult &result, const CDepletionStage &stage,
                                   const CAnalysisType &antype, int nRegister, unsigned int uComponent)
    : IValueComponentBase(uName, result, uComponent, result.RegisterMode(stage, antype, nRegister)), m_stage(stage),
      m_antype(antype), m_nRegister(nRegister), m_Data(*this) {
...
IResultComponent::IResultComponent(const QString &sName, IResult &result, const CDepletionStage &stage,
                                   const CAnalysisType &antype, int nRegister, unsigned int uComponent)
    : IValueComponentBase(sName, result, uComponent, result.RegisterMode(stage, antype, nRegister)), m_stage(stage),
      m_antype(antype), m_nRegister(nRegister), m_Data(*this) {
  assert(IsLinkedTo(result)); // Assert that we linked to our parent
  assert(result.Mode(antype, stage, nRegister) != -1);
  assert(&ResultRegister().DepletionStageEntry() == &stage.Entry());
...
const IValueDataInterfaceScalar &IResultComponent::ScalarData() const { return m_Data; }

IValueDataInterfaceScalar &IResultComponent::ScalarData() { return m_Data; }
...
const CAnalysisType &IResultComponent::AnalysisType() const { return m_antype; }

const CDepletionStage &IResultComponent::Stage() const { return m_stage; }

int IResultComponent::RegisterIndex() const { return m_nRegister; }
...
const CMeshBase &IResultComponent::Mesh() const { return ResultRegister().Mesh(); }

CMeshBase &IResultComponent::Mesh() { return ResultRegister().Mesh(); }

bool IResultComponent::Empty() const { return false; }
...
QString IResultComponent::ExportLabel() const {
  ...
  QString sRet;
  sRet = QString("%1_D%2_").arg(pResult->ExportLabel(ComponentIndex())).arg(Stage().Index());
...
IValueDomainScalar::TValue IResultComponent::ValuePoint(const geo::IPoint & /*pt*/, const CQuantity::UNIT /*unit*/,
                                                        geo::IParallelInitializationCallback * /*cb*/) const {
  return IValueDomainScalar::TValue();
}

IValueDomainScalar::TValue IResultComponent::GradientValuePoint(const geo::IPoint &pt, const UNIT unit,
                                                                geo::IParallelInitializationCallback *cb) const {
  assert(Gradient());
...
IValueDomainScalar::TValueVec IResultComponent::ValueElement(const geo::IElement &element, const CQuantity::UNIT unit,
                                                             geo::IParallelInitializationCallback *cb) const {
  assert(Defined());
    ...
    if (!pBody) {
      ...
      if (pBodyQuad1)
        ...
        if (pBodyTri1) {
          ...
          if (pInterface) {
            const geo::CBodyQuadrilateral *pBodyQuad2 =
                dynamic_cast<const geo::CBodyQuadrilateral *>(&pInterface->Front());
            if (pBodyQuad2)
...
std::vector<CTensor> IResultComponent::TensorElement(const geo::IElement &element, const CQuantity::UNIT /*unit*/,
                                                     geo::IParallelInitializationCallback *cb) const {
  assert(Defined());
...
bool IResultComponent::Gradient() const { return false; }
...
CQuantity::UNIT IScalarResultComponent::CResultValueSet::Unit() const { return m_unit; }

void IScalarResultComponent::CResultValueSet::ElementValues(TValueVec &values, int iElementIndex,
                                                            geo::IParallelInitializationCallback *cb) const {
...
void IScalarResultComponent::CResultValueSet::Empty() { assert(false); }

bool IScalarResultComponent::CResultValueSet::IsEmpty() const { return false; }
...
IScalarResultComponent::IScalarResultComponent(unsigned int uName, IResult &result, const CDepletionStage &stage,
                                               const CAnalysisType &antype, int nRegister, unsigned int uComponent)
    : IResultComponent(uName, result, stage, antype, nRegister, uComponent), m_si(*this, CQuantity::SI_UNIT),
      m_field(*this, CQuantity::FIELD_UNIT) {}

IScalarResultComponent::IScalarResultComponent(const QString &sName, IResult &result, const CDepletionStage &stage,
                                               const CAnalysisType &antype, int nRegister, unsigned int uComponent)
    : IResultComponent(sName, result, stage, antype, nRegister, uComponent), m_si(*this, CQuantity::SI_UNIT),
      m_field(*this, CQuantity::FIELD_UNIT) {}
...
geo::CValue IScalarResultComponent::ValuePoint(const geo::IPoint &pt, const CQuantity::UNIT unit,
                                               geo::IParallelInitializationCallback *cb) const {
...
void IScalarResultComponent::MapValueElement(const geo::IElement &elm, IValueDomainScalar::TValueVec &values,
