...
CLasPointSet::CLasPointSet(CFemAppModel& model)
: CPointSet(model)
, m_pNewWellPath(0)
{
...
CLasPointSet::CLasPointSet(const CNewWellPath& well,
                           const QString &sName, 
                           int nValueSetSize, 
                           DIMENSION dim, 
                           bool bCreateValueSetForCoordinates)
: CPointSet
  ( sName, const_cast<CFemAppModel&>(well.Model())
  , nValueSetSize
  , dim
  , bCreateValueSetForCoordinates
  , false
  )
{
  pointSetType(IPointSet::WELLPATH);
