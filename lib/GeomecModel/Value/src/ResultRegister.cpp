...
namespace{ Printer* printer = Printer::instance(Printer::RunRes); }
...
static const int nonlin_plain_indices [] =
{
  CI_DISPLA_X,
  CI_DISPLA_Y,
  CI_DISPLA_Z,
...
static const int heat_plain_indices [] =
{
  CI_TEMPERATURE,
  -1
};

static const int mixture_plain_indices [] =
{
  CI_DISPLA_X,
  CI_DISPLA_Y,
  CI_DISPLA_Z,
...
static const int heat_fault_indices [] =
{
  CI_TEMPERATURE,
  -1
};
...
{
  CI_DISPLA_X, PLAIN | FAULT,
  CI_DISPLA_Y, PLAIN | FAULT,
  CI_DISPLA_Z, PLAIN | FAULT,
  CI_POREPRES, PLAIN | FAULT,
  CI_STRESS_XX, PLAIN | FAULT,
  CI_STRESS_YY, PLAIN | FAULT,
  CI_STRESS_ZZ, PLAIN | FAULT,
  CI_STRESS_XY, PLAIN,
  CI_STRESS_YZ, PLAIN,
  CI_STRESS_XZ, PLAIN,
  CI_STRAIN_XX, PLAIN | FAULT,
  CI_STRAIN_YY, PLAIN | FAULT,
  CI_STRAIN_ZZ, PLAIN | FAULT,
  CI_STRAIN_XY, PLAIN,
  CI_STRAIN_YZ, PLAIN,
  CI_STRAIN_XZ, PLAIN,
  CI_TEMPERATURE, PLAIN,
  -1, NONE
};
...
static const int heat_indices [] =
{
  CI_TEMPERATURE, PLAIN | FAULT,
  -1, NONE
};
...
void CResultRegister::showChecksumErrors(std::set<int> stages, std::set<int> resultTypes) const
{
  ...
  {
  if (*it < minStage) minStage = *it;
  if (*it > maxStage) maxStage = *it;
  }
...
void CResultRegister::CreateCache()
{
  if(!m_pCache && Mesh().IsMesh())
  {
    ...
    if(cachesize < MIN_CACHE_SIZE) cachesize = MIN_CACHE_SIZE; // use minimum of MIN_CACHE_SIZE Mb

    m_pCache = new CResultCache(static_cast<CModelBase&>(Model()), Mesh().Mesh(), 5 * NUM_COLUMNS * TimeStepSize(), cachesize);
...
bool CResultRegister::cacheExists() const
{
  return (m_pCache != 0);
}
...
bool CResultRegister::Heat() const
{
  return !m_stAvailableHeatResults.empty();
}

bool CResultRegister::Mixture() const
{
  return !m_stAvailableMixtureResults.empty();
}

bool CResultRegister::MixtureContainment() const
{
  return !m_stAvailableMixtureContainmentResults.empty();
}
...
const CDepletionStageEntry& CResultRegister::DepletionStageEntry() const
{
  return m_dp_entry;
}

CDepletionStageEntry& CResultRegister::DepletionStageEntry()
{
  return m_dp_entry;
}
...
bool CResultRegister::IsFaultElement(const geo::IElement& element) const
{
  return !IsPlainElement(element);
}

void CResultRegister::ReadFilosElementNodeResult(const geo::IElement& element,
                         int nNodeNr,
                         ...
                         const CAnalysisType& antype)
{
  ...
  {
  Cache().RawCacher().Value(element.Index(), ColumnNumber(antype, stage.Index(), CI_PSTRAIN_XX), nNodeNr, plastic_strain.XX() + creep_strain.XX());
  Cache().RawCacher().Value(element.Index(), ColumnNumber(antype, stage.Index(), CI_PSTRAIN_YY), nNodeNr, plastic_strain.YY() + creep_strain.YY());
  Cache().RawCacher().Value(element.Index(), ColumnNumber(antype, stage.Index(), CI_PSTRAIN_ZZ), nNodeNr, plastic_strain.ZZ() + creep_strain.ZZ());
  Cache().RawCacher().Value(element.Index(), ColumnNumber(antype, stage.Index(), CI_PSTRAIN_XY), nNodeNr, plastic_strain.XY() + creep_strain.XY());
  Cache().RawCacher().Value(element.Index(), ColumnNumber(antype, stage.Index(), CI_PSTRAIN_YZ), nNodeNr, plastic_strain.YZ() + creep_strain.YZ());
  Cache().RawCacher().Value(element.Index(), ColumnNumber(antype, stage.Index(), CI_PSTRAIN_XZ), nNodeNr, plastic_strain.XZ() + creep_strain.XZ());
  }
  ...
  if(XistIndexed("SIG", 0))
  {
    ...
    valeps = (ftn_double_t *) DiMalloc(sz * sizeof (ftn_double_t),
      "CResultRegister::ReadFilosElementNodeResult");
    valsig = (ftn_double_t *) DiMalloc(sz * sizeof (ftn_double_t),
      "CResultRegister::ReadFilosElementNodeResult");
  ...
  for(int i = 0; i < sz; i++) valsig[i] *= 1e-6;
  ...
  else
  {
    ...
    Cache().RawCacher().Value(element.Index(), ColumnNumber(antype, stage.Index(), CI_STRAIN_YY), nNodeNr, -valeps[1]);
    Cache().RawCacher().Value(element.Index(), ColumnNumber(antype, stage.Index(), CI_STRAIN_ZZ), nNodeNr, -valeps[2]);
    Cache().RawCacher().Value(element.Index(), ColumnNumber(antype, stage.Index(), CI_STRAIN_XY), nNodeNr, -valeps[3] * 0.5);
    Cache().RawCacher().Value(element.Index(), ColumnNumber(antype, stage.Index(), CI_STRAIN_YZ), nNodeNr, -valeps[4] * 0.5);
    Cache().RawCacher().Value(element.Index(), ColumnNumber(antype, stage.Index(), CI_STRAIN_XZ), nNodeNr, -valeps[5] * 0.5);
  ...
  if (IsPlainElement(element))
  {
  Cache().RawCacher().Value(element.Index(), ColumnNumber(antype, stage.Index(), CI_TEMPERATURE), nNodeNr, dTemperature);
...
void CResultRegister::DefaultElementNodeResult(const geo::IElement &element,
                        int nNodeNr,
                        const CDepletionStage &stage,
                        const CAnalysisType& antype)
{
...
void CResultRegister::ReadFilosElementResults(const CDepletionStage &Stage,
                        const CAnalysisType& antype,
...
void CResultRegister::ReadFilosNodeResults(const geo::IElement &element,
                       const CDepletionStage& stage,
                       const CAnalysisType& antype)
{
    ...
    if(XistIndexed("NODES", &idx))
  {
    ...
    Cache().RawCacher().Value(element.Index(), ColumnNumber(antype, stage.Index(), CI_TEMPERATURE), i, temper - 273.15); // convert to celsius
      }
...
void CResultRegister::ReadFilosTimeSteps(const CAnalysisType& antype,
                     const CDepletionStage& startStage,
                     const CDepletionStage& endStage,
                     IProgressBase& progress)
{
  printer->info("read filos steps : %s -> %s", startStage.Name().toStdString().c_str(), endStage.Name().toStdString().c_str() );
...
unsigned int CResultRegister::TypeId() const
{
  return 0;
}

unsigned int CResultRegister::IconId() const
{
  return 0;
}

int CResultRegister::TimeStepSize() const
{
  return DepletionStageEntry().EntryNodes().size();
}


/* Result Import ******************************************************************************* */
void CResultRegister::AddResultFromImport(int nResultType,
                      int nElementIndex,
                      int nNodeIndex,
                      int nDepletionStage,
                      const CAnalysisType& antype,
                      double value)
{
  ...
  if(nResultType >= CI_CSTRAIN_XX && nResultType <= CI_CSTRAIN_XZ)
    IncrementCacheValue(nElementIndex, antype, nDepletionStage, CI_PSTRAIN_XX + (nResultType - CI_CSTRAIN_XX), nNodeIndex, value);
  else
    Cache().RawCacher().Value(nElementIndex, 
                 ColumnNumber(antype, nDepletionStage, nResultType),
             nNodeIndex,
             value);
...
bool CResultRegister::ReadFilosResults(const QString& filosFile)
{
  ...
  if (_m()->msg("WARNING: experimental feature, please make sure your model is saved. For expert use only.", MB_OKCANCEL) == IDCANCEL)
  return false;
  ...
  {
      if (_m()->msg("Mixture results found; press Ok for Mixture Containment or cancel for normal Mixture", MB_OKCANCEL) == IDCANCEL)
  ...
  if (XistIndexed("STEP", 0))
  {
      ...
      for (TDepletionStageEntry::TNodeSet::const_iterator stage = stDepletionStage.begin(); stage != stDepletionStage.end(); ++stage)
      {
...
bool CResultRegister::ReadFilosResults(	const CDepletionStage& startStage, 
                    const CDepletionStage& endStage, 
                    const CAnalysisType& antype )
{
  printer->info("read filos results : %s -> %s", startStage.Name().toStdString().c_str(), endStage.Name().toStdString().c_str() );
...
//	int nJobs = TimeStepSize();
  ...
  std::auto_ptr <IProgressBase> progress(
  _g->prog()->create(eProgress::Dual, primaryTitle, false, nJobs ));
...
void CResultRegister::LoadBodyTensorToCache(TSTREAM &stream, TPROGRESS &progress, const CAnalysisType& antype, int iBegin)
{
  int nVals;
  int iStage;
  int iElement;
...
void CResultRegister::LoadBodyDoubleToCache(TSTREAM &stream, TPROGRESS &progress, const CAnalysisType& antype, int iBegin)
{
  int nVals;
  int iStage;
  int iElement;
      ...
      for(iElement = 0; iElement < Mesh().Mesh().ElementSize(); iElement++)
      {
        ...
        if(IsPlainElement(elm))
        {
          for(int iNode = 0; iNode < elm.NrOfPoints(); iNode++)
          {
            double dVal;
            stream >> dVal;
            if(m_pCache) Cache().RawCacher().Value(iElement, ColumnNumber(antype, iStage, iBegin), iNode, dVal);
            progress.Step();
            nVals--;
          }
        }
...
bool CResultRegister::LoadDisplacementsToCache(TSTREAM &stream, TPROGRESS &progress, const CAnalysisType& antype)
{
  ...
  if((vcNode.size() == 0) || !Mesh().IsMesh()) return false;
...
void CResultRegister::IncrementCacheValue(int iElement, const CAnalysisType& antype, int iStage, int iCacheEntry, int iNode, const double& dAdd)
{
...
void CResultRegister::LoadFaultDoubleToCache(TSTREAM &stream, TPROGRESS &progress, const CAnalysisType& antype, int iBegin)
{
  int nVals;
  int iStage;
  int iElement;
...
void CResultRegister::LoadFaultVectorToCache(TSTREAM &stream, CStreamVersion &version, TPROGRESS &progress, const CAnalysisType& antype, int iBegin)
{
  int nVals;
  int iStage;
  int iElement;
...
void CResultRegister::LoadCacheResults(TSTREAM &stream, CStreamVersion &version, TPROGRESS &progress, const CAnalysisType& antype)
{
  bool bIgnore = false;
  ...
  {
  bIgnore = pDiagnostics->Setting(gm_diagnostics::Setting_LoadModelNoResults)
      || (!pModel->parentModel() && pDiagnostics->Setting(gm_diagnostics::Setting_LoadMainModelNoResults))
      || (pModel->parentModel() &&
                 (dynamic_cast<CWellZoomInModel *>(pModel) ? pDiagnostics->Setting(gm_diagnostics::Setting_LoadWellZoominModelNoResults)
  ...
  for (int iStage = 0; iStage < TimeStepSize(); iStage++)
  {
    ...
    {
          if (antype == CAnalysisType::AT_LINEAR) indices = linear_plain_indices;
          else if (antype == CAnalysisType::AT_NONLIN) indices = nonlin_plain_indices;
          else if (antype == CAnalysisType::AT_HEAT) indices = heat_plain_indices;
          else if (antype == CAnalysisType::AT_MIXTURE) indices = mixture_plain_indices;
          else if (antype == CAnalysisType::AT_MIXTURE_CONTAINMENT) indices = mixture_plain_indices;
    }
    else
    {
          if (antype == CAnalysisType::AT_LINEAR) indices = linear_fault_indices;
          else if (antype == CAnalysisType::AT_NONLIN) indices = nonlin_fault_indices;
          else if (antype == CAnalysisType::AT_HEAT) indices = heat_fault_indices;
          else if (antype == CAnalysisType::AT_MIXTURE) indices = mixture_fault_indices;
          else if (antype == CAnalysisType::AT_MIXTURE_CONTAINMENT) indices = mixture_fault_indices;
    }
    ...
    for (i = 0; indices[i] != -1; i++)
    {
      ...
      if ((CStreamVersion(3, 0, 74) > version) &&
              dynamic_cast<const geo::CInterfaceElement*>(&elm)) nNrOfNodes = 4;


      mdc::CMeshDataCacher & rawCacher = Cache().RawCacher();

      bool doIncrementCacheValue = (elmIsPlainElement && antype == CAnalysisType::AT_NONLIN && indices[i] >= CI_CSTRAIN_XX && indices[i] < CI_CSTRAIN_XZ);
      ...
      for (iNode = 0; iNode < nNrOfNodes; iNode++)
      {
        ...
        if (doIncrementCacheValue)
                  IncrementCacheValue(iElement, CAnalysisType::AT_NONLIN, iStage, CI_PSTRAIN_XX + (indices[i] - CI_CSTRAIN_XX), iNode, dVal); // add to plastic
        else
...
void CResultRegister::LoadStream(TSTREAM& stream, CStreamVersion& version, TPROGRESS& progress)
{
  ...
  else
  {
    if(version < CStreamVersion(3, 0, 41))
    {
      ...
      if(!bLinear) ClearLinear(false);
      if(!bNonlinear) ClearNonLinear(false);
    }
    else
    {
      // load cache
...
//				m_bNonlinear = (iNonlinear != 0);
  ...
  if(!m_pCache) CreateCache(); //wjrx mantis 2594
...
void CResultRegister::load_old_result(TSTREAM& stream, const CStreamVersion& version, TPROGRESS& progress, const geo::IElement& element, bool bLinear)
{
...
void CResultRegister::load_old_geo_result(TSTREAM& stream, const CStreamVersion& version, const geo::IElement& element, int /*nNodeNr*/, int /*nTimeStep*/, bool /*bLinear*/)
{
...
void CResultRegister::SaveCacheResults(TSTREAM &stream, TPROGRESS &progress, const CAnalysisType& antype)
{
  ...
  if (stream.IsHDF5())
  {
  HDF5StorageInterface = new CHDF5StorageInterface(*stream.GetHDF5File(), static_cast<CModelBase&>(Model()).Index(), TimeStepSize(), NUM_COLUMNS);
  }

  for(int iStage = 0; iStage < TimeStepSize(); ++iStage)
  {
  int iResultsAvailable = ResultsAvailable(antype, static_cast<CModelBase&>(Model()).DepletionStageEntry().StageByIndex(iStage)) ? 1 : 0;
...
//	stream << (Linear() ? 1 : 0);
  ...
  for(it = m_stAvailableLinearResults.begin(); it != m_stAvailableLinearResults.end(); ++it)
    stream << *it;
  // linear
  if(Linear()) SaveCacheResults(stream, progress, CAnalysisType::AT_LINEAR);
...
//	stream << (NonLinear() ? 1 : 0);
  ...
  for(it = m_stAvailableNonlinResults.begin(); it != m_stAvailableNonlinResults.end(); ++it)
    stream << *it;
  // non-linear
  if(NonLinear()) SaveCacheResults(stream, progress, CAnalysisType::AT_NONLIN);

  stream << (int)m_stAvailableHeatResults.size();
  for(it = m_stAvailableHeatResults.begin(); it != m_stAvailableHeatResults.end(); ++it)
  stream << *it;
  if(Heat()) SaveCacheResults(stream, progress, CAnalysisType::AT_HEAT);
  ...
  stream << *it;
  if(Mixture()) SaveCacheResults(stream, progress, CAnalysisType::AT_MIXTURE);
  ...
  stream << *it;
  if(MixtureContainment()) SaveCacheResults(stream, progress, CAnalysisType::AT_MIXTURE_CONTAINMENT);

  // Results are imported or not. We don't make difference right now.
  stream << 0;
}

void CResultRegister::SaveStressTensorVec(TSTREAM& stream, TPROGRESS& progress, const std::vector<CStressTensor>& vcTensor)
{
  // Save nr of tensors
...
void CResultRegister::SaveStrainTensorVec(TSTREAM& stream, TPROGRESS& progress, const std::vector<CStrainTensor>& vcTensor)
{
  // Save nr of tensors
  int nTensorSize = vcTensor.size();
  stream << nTensorSize;
...
void CResultRegister::LoadVectorVec(TSTREAM& stream, const CStreamVersion& version, TPROGRESS& progress, std::vector<geo::CVector>& vcVector)
{
  int nSize;
  stream >> nSize;
...
void CResultRegister::LoadDoubleVec(TSTREAM& stream, const CStreamVersion& /*version*/, TPROGRESS& progress, std::vector<double>& vcDouble)
{
  int nDoubleSize;
  stream >> nDoubleSize;
...
{
  int factor = 0;
  if (Linear()) factor += sizeof(linear_indices) / (2 * sizeof(int)) - 1;
  if (NonLinear()) factor += sizeof(nonlin_indices) / (2 * sizeof(int)) - 1;
  if (Heat()) factor += sizeof(heat_indices) / (2 * sizeof(int)) - 1;
  if (Mixture()) factor += sizeof(mixture_indices) / (2 * sizeof(int)) - 1;
  if (MixtureContainment()) factor += sizeof(mixture_indices) / (2 * sizeof(int)) - 1;

  return factor * TimeStepSize() * Mesh().Mesh().ElementSize();
}

void CResultRegister::Node(geo::IPoint &target, const CDepletionStage &stage, const CAnalysisType& antype, int nNodeIndex) const
{
...
CScalarValueSet CResultRegister::PorePressure(const CDepletionStage& stage, const CAnalysisType& antype, bool bChange) const
{
  return CScalarValueSet(*this, stage, antype, CI_POREPRES, false, bChange);
}

CScalarValueSet CResultRegister::Temperature(const CDepletionStage& stage, const CAnalysisType& antype, bool bChange) const
{
  return CScalarValueSet(*this, stage, antype, CI_TEMPERATURE, false, bChange);
}

CScalarValueSet CResultRegister::FaultPorePressure(const CDepletionStage& stage, const CAnalysisType& antype, bool bChange) const
{
  return CScalarValueSet(*this, stage, antype, CI_POREPRES, true, bChange);
}

CScalarValueSet CResultRegister::Consolidation(const CDepletionStage& stage, const CAnalysisType& antype, bool bChange) const
{
...
void CResultRegister::TensorFromCache(ITensor &target, const CDepletionStage &stage, const CAnalysisType& antype, int nElementIndex, int nNodeIndex, int nStart) const
{
...
void CResultRegister::VectorFromCache(geo::IVector &target, const CDepletionStage &stage, const CAnalysisType& antype, int nElementIndex, int nNodeIndex, int nStart) const
{
...
CStressTensorValueSet CResultRegister::TotalStress(const CDepletionStage& stage, const CAnalysisType& antype, bool bChange) const
{
  return CStressTensorValueSet(*this, stage, antype, CI_STRESS_XX, true, bChange); 
}

CStressTensorValueSet CResultRegister::EffectiveStress(const CDepletionStage& stage, const CAnalysisType& antype, bool bChange) const
{
  return CStressTensorValueSet(*this, stage, antype, CI_STRESS_XX, false, bChange); 
}

CStrainTensorValueSet CResultRegister::TotalStrain(const CDepletionStage& stage, const CAnalysisType& antype, bool bChange) const
{
  assert(!bChange); // No change possible for this result
  return CStrainTensorValueSet(*this, stage, antype, CI_STRAIN_XX, false, true); 
}

CStrainTensorValueSet CResultRegister::PlasticStrain(const CDepletionStage& stage, const CAnalysisType& antype, bool bChange) const
{
  assert(!bChange); // No change possible for this result
...
/*
CStrainTensorValueSet CResultRegister::CreepStrain(const CDepletionStage& stage, const CAnalysisType& antype, bool bChange) const
{
  assert(!bChange); // No change possible for this result
...
CVectorValueSet CResultRegister::FaultShearStress(const CDepletionStage& stage, const CAnalysisType& antype, bool bChange) const
{
  return	CVectorValueSet(*this, stage, antype, CI_STRESS_XX, true, true, bChange);
}

CVectorValueSet CResultRegister::FaultEffectiveStress(const CDepletionStage& stage, const CAnalysisType& antype, bool bChange) const
{
  return	CVectorValueSet(*this, stage, antype, CI_STRESS_XX, false, true, bChange);
}

CVectorValueSet CResultRegister::Displacement(const CDepletionStage& stage, const CAnalysisType& antype, bool bChange) const
{
  assert(!bChange);
  return	CVectorValueSet(*this, stage, antype, CI_DISPLA_X, false, false, true);// CI_STRAIN_XX
}

CVectorValueSet CResultRegister::FaultDisplacement(const CDepletionStage& stage, const CAnalysisType& antype, bool /*bChange*/) const
{
...
double CResultRegister::MaterialParameterValue(int nElementIndex, unsigned int ValueTypeID, const CDepletionStage& stage) const
{
  ...
  if(pWellCasingModel && pWellCasingModel->Mesh().IsCementInterfaceElement(element))
      return pWellCasingModel->CasingNode().CementInterface().Material(stage).Material(element).ParameterValue(ValueTypeID);
...
bool CResultRegister::ResultAvailable(const CDepletionStage &stage, const CAnalysisType& antype, int nElementIndex, int nResultType) const
{
...
bool CResultRegister::ResultAvailable(int nTimeStep, const CAnalysisType& antype, int nElementIndex, int nResultType) const
{
  const CModelBase& model = static_cast<const CModelBase&>(Model());
...
bool CResultRegister::HasSmoothedResults() const
{
  return Cache().HasAveragedCache();
}
