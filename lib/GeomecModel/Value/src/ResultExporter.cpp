...
///// IResultExporter
IResultExporter::IResultExporter(CModelBase &model) : m_model(model) {}

IResultExporter::~IResultExporter() {}

void IResultExporter::Export2LAS(const QString & /*sFileName*/, const IExportFormat::TDataVec & /*vcData*/,
                                 const IExportFormat::CExportArg /*arg*/) const {}

CModelBase &IResultExporter::Model() const { return m_model; }

// protected

template <typename TExportFormat>
bool IResultExporter::exportExcel(TExportFormat &exportFormat, const QString &sFileName, bool bAppend) const {
...
template <typename TExportFormat>
bool IResultExporter::exportASCII(TExportFormat &exportFormat, const QString &sFileName, bool bAppend) const {
...
CElementSetExporter::CElementSetExporter(CModelBase &model, const std::vector<const IElementSet *> &vcElementset)
    : IResultExporter(model), m_vcElementset(vcElementset) {}

void CElementSetExporter::ExportExcel(const QString &sFileName, const IExportFormat::TDataVec &vcData,
...
void CElementSetExporter::ExportASCII(const QString &sFileName, const IExportFormat::TDataVec &vcData,
                                      const IExportFormat::CExportArg arg, bool bAppend) const {
  for (size_t i = 0; i < m_vcElementset.size(); ++i) {
    CElementSetExportFormat format(*m_vcElementset[i], vcData, arg);
...
CFormationExporter::CFormationExporter(CModelBase &model, const std::vector<const CFormationBase *> &vcFormation)
    : IResultExporter(model), m_vcFormation(vcFormation) {}

void CFormationExporter::ExportExcel(const QString &sFileName, const IExportFormat::TDataVec &vcData,
                                     const IExportFormat::CExportArg arg, bool bAppend) const {
  if (arg.m_elementCenterPoints == IExportFormat::CENTER_WELL) {
    WellPath::CWellPathCenterPointSet wellPathCenterPointSet(m_vcFormation, Model());
    ...
    QSharedPointer<CPointSet> pps = QSharedPointer<CPointSet>(CreateFormationCenterPointSet(vcData));
...
void CFormationExporter::ExportASCII(const QString &sFileName, const IExportFormat::TDataVec &vcData,
                                     const IExportFormat::CExportArg arg, bool bAppend) const {
  if (arg.m_elementCenterPoints == IExportFormat::CENTER_WELL) {
    WellPath::CWellPathCenterPointSet wellPathCenterPointSet(m_vcFormation, Model());
    CNodalExportFormat format(wellPathCenterPointSet, vcData, arg);
...
const COpenGLNode *CFormationExporter::GetFirstNode() const { return (m_vcFormation.empty() ? 0 : m_vcFormation[0]); }
...
CPointSet *CFormationExporter::CreateFormationCenterPointSet(const IExportFormat::TDataVec & /*vcData*/) const {
  ...
  CPointSet *pps = new CPointSet("", Model(), 0, (IPointSet::DIMENSION)dim);
...
CFormationPlaneExporter::CFormationPlaneExporter(CModelBase &model,
                                                 const std::vector<const CFormationPlane *> &vcFormplane)
    : IResultExporter(model), m_vcFormplane(sortFormationPlanes(vcFormplane)) {}

void CFormationPlaneExporter::ExportExcel(const QString &sFileName, const IExportFormat::TDataVec &vcData,
                                          const IExportFormat::CExportArg arg, bool bAppend) const {
  if (arg.m_elementCenterPoints == IExportFormat::CENTER_ELEMENT) {
    QSharedPointer<CPointSet> pps = QSharedPointer<CPointSet>(CreateFormationPlaneCenterPointSet(vcData));
    CNodalExportFormat format(*pps, vcData, arg);
...
void CFormationPlaneExporter::ExportASCII(const QString &sFileName, const IExportFormat::TDataVec &vcData,
                                          const IExportFormat::CExportArg arg, bool bAppend) const {
  if (arg.m_elementCenterPoints == IExportFormat::CENTER_ELEMENT) {
    QSharedPointer<CPointSet> pps = QSharedPointer<CPointSet>(CreateFormationPlaneCenterPointSet(vcData));
    CNodalExportFormat format(*pps, vcData, arg);
  ...
  for (size_t jj = 0; jj < m_vcFormplane.size(); ++jj) {
    ...
    for (int ii = 0; ii < pFormPlane->PlaneSurface().FaceSize(); ++ii) {
      ...
      for (size_t iData = 0; iData < vcData.size(); ++iData) {
        vcPointData[iData + dim] = quad.InterpolateValue(midPt, vcData[iData]->ScalarData().ValueElement(quad));
      }
...
bool sortFormationsComparison(const CFormationPlane *lhs, const CFormationPlane *rhs) {
  return lhs->Formation().Less(rhs->Formation());
}

bool sortFormationPlanesComparison(const CFormationPlane *lhs, const CFormationPlane *rhs) { return lhs->Less(*rhs); }

} // anonymous namespace

std::vector<const CFormationPlane *>
CFormationPlaneExporter::sortFormationPlanes(const std::vector<const CFormationPlane *> &formationPlanes) {
  std::vector<const CFormationPlane *> sortedFormationPlanes(formationPlanes);

  std::sort(sortedFormationPlanes.begin(), sortedFormationPlanes.end(), sortFormationsComparison);

  std::vector<const CFormationPlane *>::iterator begin = sortedFormationPlanes.begin(),
                                                 end = sortedFormationPlanes.begin();
  std::vector<const CFormationPlane *>::const_iterator start = sortedFormationPlanes.begin(),
                                                       finish = sortedFormationPlanes.end();

  while ((start != finish) || ((start == finish) && (begin != end))) {
    if ((end == finish) || ((*begin)->Formation().Index() != (*end)->Formation().Index())) {
...
CHorizonExporter::CHorizonExporter(CModelBase &model, const std::vector<const CHorizonBase *> &vcHorizon)
    : IResultExporter(model), m_vcHorizon(vcHorizon) {}

void CHorizonExporter::ExportExcel(const QString &sFileName, const IExportFormat::TDataVec &vcData,
                                   const IExportFormat::CExportArg arg, bool bAppend) const {
  if (arg.m_elementCenterPoints == IExportFormat::CENTER_ELEMENT) {
    QSharedPointer<CPointSet> pps = QSharedPointer<CPointSet>(CreateHorizonsPointSet(vcData));
    CNodalExportFormat format(*pps, vcData, arg);
...
void CHorizonExporter::ExportASCII(const QString &sFileName, const IExportFormat::TDataVec &vcData,
                                   const IExportFormat::CExportArg arg, bool bAppend) const {
  if (arg.m_elementCenterPoints == IExportFormat::CENTER_ELEMENT) {
    QSharedPointer<CPointSet> pps = QSharedPointer<CPointSet>(CreateHorizonsPointSet(vcData));
    CNodalExportFormat format(*pps, vcData, arg);
...
const COpenGLNode *CHorizonExporter::GetFirstNode() const { return (m_vcHorizon.empty() ? 0 : m_vcHorizon[0]); }
  ...
  for (size_t i = 0; i < m_vcHorizon.size(); ++i) {
    ...
    for (int e = 0; e < hor3D.AsElementSet().ElementSize(); ++e) {
      ...
      for (int n = 0; n < element.NrOfPoints(); n++) {
        std::pair<TNodalValuesMap::iterator, bool> pr_insert = mNodalValues.insert(
            TNodalValuesMap::value_type(element.PointIndex(n), std::vector<geo::CValue>(vcNodalData.size())));
...
void CSlipHorizonExporter::ExportExcel(const QString &sFileName, const IExportFormat::TDataVec &vcData,
                                       const IExportFormat::CExportArg arg, bool bAppend) const {
  CElementSetExportFormat format(m_vcHorizon, IExportFormat::TDataVec(), vcData, arg);
...
void CSlipHorizonExporter::ExportASCII(const QString &sFileName, const IExportFormat::TDataVec &vcData,
                                       const IExportFormat::CExportArg arg, bool bAppend) const {
  CElementSetExportFormat format(m_vcHorizon, IExportFormat::TDataVec(), vcData, arg);
...
const COpenGLNode *CSlipHorizonExporter::GetFirstNode() const { return (m_vcHorizon.empty() ? 0 : m_vcHorizon[0]); }

///// CPointSetExporter
CPointSetExporter::CPointSetExporter(CModelBase &model, const std::vector<const CPointSet *> &vcPointset)
    : IResultExporter(model), m_vcPointset(vcPointset) {}

void CPointSetExporter::ExportExcel(const QString &sFileName, const IExportFormat::TDataVec &vcData,
                                    const IExportFormat::CExportArg arg, bool bAppend) const {
  QSharedPointer<CPointSet> pps = QSharedPointer<CPointSet>(CreateCombinedPointSet(vcData));
...
void CPointSetExporter::ExportASCII(const QString &sFileName, const IExportFormat::TDataVec &vcData,
                                    const IExportFormat::CExportArg arg, bool bAppend) const {
  QSharedPointer<CPointSet> pps = QSharedPointer<CPointSet>(CreateCombinedPointSet(vcData));
...
const COpenGLNode *CPointSetExporter::GetFirstNode() const { return (m_vcPointset.empty() ? 0 : m_vcPointset[0]); }
...
CPointSet *CPointSetExporter::CreateCombinedPointSet(const IExportFormat::TDataVec & /*vcNodalData*/) const {
  ...
  std::auto_ptr<IProgressBase> progdlg(
      _g->prog()->create(eProgress::Dual, "Preparing pointset for export", true, nJobs));
...
CRTCIExporter::CRTCIExporter(CModelBase &model, const std::vector<const CWellCasingRTCI *> &vcRTCI)
    : IResultExporter(model), m_vcRTCI(vcRTCI) {}

void CRTCIExporter::ExportExcel(const QString &sFileName, const IExportFormat::TDataVec &vcData,
                                const IExportFormat::CExportArg arg, bool bAppend) const {
...
void CRTCIExporter::ExportASCII(const QString &sFileName, const IExportFormat::TDataVec &vcData,
                                const IExportFormat::CExportArg arg, bool bAppend) const {
  CElementSetExportFormat format(m_vcRTCI, IExportFormat::TDataVec(), vcData, arg);
...
const COpenGLNode *CRTCIExporter::GetFirstNode() const { return (m_vcRTCI.empty() ? 0 : m_vcRTCI[0]); }

///// CNonMeshedSurfaceExporter
CNonMeshedSurfaceExporter::CNonMeshedSurfaceExporter(CModelBase &model,
                                                     const std::vector<const CNonMeshedSurface *> &vcSurface)
    : IResultExporter(model), m_vcSurface(vcSurface) {}

void CNonMeshedSurfaceExporter::ExportExcel(const QString &sFileName, const IExportFormat::TDataVec &vcData,
                                            const IExportFormat::CExportArg arg, bool bAppend) const {
...
void CNonMeshedSurfaceExporter::ExportASCII(const QString &sFileName, const IExportFormat::TDataVec &vcData,
                                            const IExportFormat::CExportArg arg, bool bAppend) const {
  CElementSetExportFormat format(m_vcSurface, IExportFormat::TDataVec(), vcData, arg);
...
///// CExporterFactory
CExporterFactory::CExporterFactory(CModelBase &model) : m_model(model) {}
...
template <>
const IResultExporter *
CExporterFactory::CreateTypedExporter(const std::vector<const CNonMeshedSurface *> &vcSelection) {
