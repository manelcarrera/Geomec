...
std::string SurfaceStressComponentList() { return "Normal|ShearPlane|ShearPlaneDipAngle"; }

std::string WellPathStressComponentList() { return "Axial|RadialMax|RadialMin"; }
...
///
bool SimpleSplitTextTag(const std::string &textTag //!< tag to be split 'aap_D1'
                        ,
  ...
  return SplitTextTag(textTag, "", "", baseTag, dummy, timeStepIndex, dummy);
}
...
/// \returns true if a valid split could be made
///
bool SplitTextTag(const std::string &textTag //!< tag to be split 'aapN_D1_L'
                  ,
  ...
  if (!componentList.empty()) {
    QRegExp compRegExp(QString("(") + QString(componentList.c_str()) + QString(")($|_)"));
  ...
  if (!analysisTypeList.empty()) {
    const QRegExp atRegExp(QString("(") + QString(analysisTypeList.c_str()) + QString(")$"));
  ...
  if (bOk)
    bOk = textTag.size() == (baseTag.size() + component.size() + timeStepIndexSize + analysisType.size());
