...
CValueMapper::CValueMapper()
    : m_nLevelOfParallization(PARALLEL_FULL), m_nDianaDistributedMapping(DIA_DM_YES),
      m_bEncounteredStrainOrStress(false), m_pResultRegister(0) {}
...
bool CValueMapper::UseLegacyCode() const { return m_nLevelOfParallization == PARALLEL_NONE; }

void CValueMapper::UseLegacyCode(bool bUseLegacyCode) {
  m_nLevelOfParallization = bUseLegacyCode ? PARALLEL_NONE : PARALLEL_PARTIAL;
}

bool CValueMapper::AllowParallelMapping() const { return m_nLevelOfParallization == PARALLEL_FULL; }
...
bool CValueMapper::DianaDistributedMapping() const { return m_nDianaDistributedMapping == DIA_DM_YES; }
...
void CValueMapper::NextSection() {
  ...
  for (TStageInfoMap::iterator stageIt = m_stageInfo.begin(); stageIt != m_stageInfo.end(); ++stageIt) {
    for (TFormationElementMap::iterator it = stageIt->second.mpFormationElement.begin();
         it != stageIt->second.mpFormationElement.end(); ++it)
      it->second.second = false;
    for (TCasingModelElementMap::iterator it = stageIt->second.mpCasingModelElement.begin();
         it != stageIt->second.mpCasingModelElement.end(); ++it)
      it->second.second = false;
    for (TElementSetMap::iterator it = stageIt->second.mpHandledSets.begin(); it != stageIt->second.mpHandledSets.end();
         ++it)
      it->second = false;
  }
...
void CValueMapper::Add(CResultTree::CScalarResult::CScalarResultComponent *pComponent,
...
void CValueMapper::Add(CResultTree::CScalarResult::CScalarResultComponent *pComponent,
                       CResultTree::CScalarResult::CScalarResultComponent::TFormAvgVolMap *pCache) {
  m_vcResultAvgVolumeCaches.push_back(std::make_pair(pComponent, pCache));
}

void CValueMapper::collectFormationModelElementMaps(const CModelBase &model, const geo::IElementSet &elementSet,
                                                    const CDepletionStage &stage, bool bDoWellCasing) {
  ...
  {
    std::pair<TElementSetMap::iterator, bool> retval =
        stageInfo->mpHandledSets.insert(TElementSetMap::value_type(&elementSet, false));
    if (retval.second)
  ...
  for (int i = 0; i < elementSet.ElementSize(); ++i) {
    if (!elementSet.Element(i).IsInterfaceElement()) {
      ...
      if (pFormation) {
        std::pair<TFormationElementMap::iterator, bool> retval = stageInfo->mpFormationElement.insert(
            TFormationElementMap::value_type(pFormation, TElementPair(pElement, false)));
        if (retval.second)
        ...
        if (pWellCasingModel) {
          std::pair<TCasingModelElementMap::iterator, bool> retval = stageInfo->mpCasingModelElement.insert(
              TCasingModelElementMap::value_type(pWellCasingModel, TElementPair(pElement, false)));
          if (retval.second)
            retval.first->second.second = true;
        }
      }
...
bool CValueMapper::PrepareMappingMaterial(const CModelBase &model, const geo::IElementSet *pElementSet,
                                          const CDepletionStage &stage, int flag) {
  if (!pElementSet)
  ...
  for (TFormationElementMap::const_iterator it = stageInfo->mpFormationElement.begin();
       it != stageInfo->mpFormationElement.end(); ++it) {
    if (!it->second.second)
      continue;
  ...
  for (TCasingModelElementMap::const_iterator it = stageInfo->mpCasingModelElement.begin();
       it != stageInfo->mpCasingModelElement.end(); ++it) {
    if (!it->second.second)
      continue;
...
bool CValueMapper::HasUndrainedMaterial(const CModelBase &model, const geo::IElementSet *pElementSet,
...
bool CValueMapper::PrepareMappingComponent(const CModelBase &model, const geo::IElementSet *pElementSet,
                                           const CDepletionStage &stage, TPrepareMappingComponentType type) {
  if (!pElementSet)
    return true;
  ...
  for (TFormationElementMap::const_iterator it = stageInfo->mpFormationElement.begin();
    ...
    case MAP_TEMPERATURE:
      if (!const_cast<IValueComponentBase &>(it->first->EffectiveTemperature(stage).Component())
               .PrepareMapping(pElementSet))
        retval = false;
      break;
...
bool CValueMapper::PrepareMappingSurfaces(const CModelBase &model, const geo::IElementSet *pElementSet,
                                          const CDepletionStage &stage) {
  if (!pElementSet)
    return true;
...
template <bool CheckForBody> class CTaskFillCache {
...
template <> void CTaskFillCache<false>::operator()(const tbb::blocked_range<size_t> &r) const {
...
template <> void CTaskFillCache<true>::operator()(const tbb::blocked_range<size_t> &r) const {
  for (size_t i = r.begin(); i != r.end(); ++i) {
...
bool CValueMapper::PrepareMappingStress(const CModelBase &model, IResultComponent &component,
                                        CResultRegister &resultRegister, bool change) {
  bool retval = true;
  ...
  if (change) {
    int c = resultRegister.ColumnNumber(
        component.AnalysisType(), resultRegister.DepletionStageEntry().MarkedAsInitialStage().Index(), CI_STRESS_XX);
...
bool CValueMapper::PrepareMappingStrain(const CModelBase &model, IResultComponent &component,
  ...
  int c = resultRegister.ColumnNumber(
      component.AnalysisType(), resultRegister.DepletionStageEntry().MarkedAsInitialStage().Index(), CI_STRAIN_XX);
...
bool CValueMapper::PrepareMappingPlasticEnergy(const CModelBase &model, IResultComponent &component,
                                               CResultRegister &resultRegister) {
  bool retval = true;
  ...
  int c = resultRegister.ColumnNumber(
      component.AnalysisType(), resultRegister.DepletionStageEntry().MarkedAsInitialStage().Index(), CI_PSTRAIN_XX);
  if (!resultRegister.Cache().ActiveCacher().StartCriticalSection(c, 6))
    retval = false;
...
bool CValueMapper::PrepareMappingPorePressure(const CModelBase &model, IResultComponent &component,
                                              CResultRegister &resultRegister, const geo::IElementSet *pElementSet,
                                              bool change) {
  bool retval = true;
...
bool CValueMapper::PrepareMappingGammaValues(const CModelBase &model, IResultComponent &component,
                                             CResultRegister &resultRegister, const geo::IElementSet *pElementSet) {
  bool retval = true;
...
bool CValueMapper::PrepareMappingIndex(int index, const CModelBase & /*model*/, IResultComponent &component,
                                       CResultRegister &resultRegister, bool change) {
  bool retval = true;

  if (change) {
    int c = resultRegister.ColumnNumber(component.AnalysisType(),
                                        resultRegister.DepletionStageEntry().MarkedAsInitialStage().Index(), index);
...
bool CValueMapper::EncounteredStrainOrStress() const { return m_bEncounteredStrainOrStress; }