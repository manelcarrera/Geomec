...
*/
CScalarValueSet::CScalarValueSet(const CResultRegister &parent, const CDepletionStage &stage,
                                 const CAnalysisType &antype, int nStart, bool bInterface, bool bChange)
    : m_nStart(nStart), m_antype(antype), m_bChange(bChange), m_bInterface(bInterface), m_pStage(&stage),
      m_pParent(&parent) {}
...
*/
const double *CScalarValueSet::ValuesFromCache(int nElementIndex, const CAnalysisType &antype,
...
*/
void CScalarValueSet::ElementValues(TValueVec &values, int iElementIndex,
...
*/
bool CScalarValueSet::IsEmpty() const { return true; }

bool CScalarValueSet::ElementValuesAvailable(int nElementIndex) const {
  if (!m_pParent->ResultAvailable(*m_pStage, m_antype, nElementIndex, m_nStart))
    return false;

  if (m_bChange) {
    const CDepletionStage &init_stage = m_pParent->DepletionStageEntry().MarkedAsInitialStage();
    if (!m_pParent->ResultAvailable(init_stage, m_antype, nElementIndex, m_nStart))
      return false;
  }
