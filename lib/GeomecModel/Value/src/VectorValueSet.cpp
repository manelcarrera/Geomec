...
*/
CVectorValueSet::CVectorValueSet(const CResultRegister& parent, 
...
*/
const double* CVectorValueSet::ValuesFromCache( int nElementIndex,
                                           const CAnalysisType& antype,
                               const CDepletionStage& stage,
                               int nStart) const 
{
...
*/
void CVectorValueSet::ElementValues(TValueVec& values, int iElementIndex, geo::IParallelInitializationCallback * /*cb*/) const
{
  ...
  if( pInterface && m_bInterface && m_bShear ) {
    ...
    if(pFace) normal = pFace->Normal().UnitVector();
    const geo::ILine* pLine = dynamic_cast<const geo::ILine*> (&pInterface->Front());
    if(pLine) normal = pLine->Dir().CrossProduct(geo::CVector::Zaxis).UnitVector();
  }

  if(!ElementValuesAvailable(iElementIndex) &&
    (pInterface != 0) == m_bInterface ) {
    // Result does not exist
    for(size_t i = 0; i < values.size(); i++) values[i] = geo::CVector();
    return;
  }
  ...
  if( m_bChange ) {
    ...
    for(size_t i = 0; i < values.size(); i++) {
      values[i] = geo::CVector(pVectorVal[0][i] - pInitVectorVal[0][i],
                             pVectorVal[1][i] - pInitVectorVal[1][i],
         				     pVectorVal[2][i] - pInitVectorVal[2][i]);
      if( m_bShear ) values[i] = values[i] - normal * values[i].DotProduct( normal );
    }
  } else {
    for(size_t i = 0; i < values.size(); i++) {
      values[i] = geo::CVector(pVectorVal[0][i],
                             pVectorVal[1][i],
         				     pVectorVal[2][i]);
      if( m_bShear ) values[i] = values[i] - normal * values[i].DotProduct( normal );
    }
...
*/
bool CVectorValueSet::IsEmpty() const
{
  return true;
}
...
void CVectorValueSet::MapValueElement(const geo::IElement& elm, TValueVec& values, TMapType map_type, geo::IParallelInitializationCallback *cb) const
{
  ...
  for(size_t i = 0; i < values.size(); i++) {
    if( scalars[0][i].Valid() &&
    	  scalars[1][i].Valid() &&
      scalars[2][i].Valid() ) {
    values[i] = geo::CVector( scalars[0][i].Value(),
                              scalars[1][i].Value(),
      					      scalars[2][i].Value() );
    } else {
