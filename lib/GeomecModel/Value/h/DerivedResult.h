...
public:
    ...
    void MapValueElement(const geo::IElement& elm, TValueVec& values, IVectorResult::TMapType map_type, UNIT unit = CQuantity::SI_UNIT, geo::IParallelInitializationCallback *cb = 0) const;
      ...
      bool m_bFixedLinStage; // Are the linearity and stage fixed?
		...
		public:
			...
			CValueComponentProxy(const QString& sName, IValueComponentBase& value_component, rpn::CRpnStack& NewStack, bool bFixedLinStage);
...
//			void SaveStream(CStorageNode::TSTREAM& stream);
			...
			virtual bool Properties(bool fixedLinStage,
				const IValueComponentBase* selection, const QString& name);
      ...
      virtual rpn::CRpnObject *GetExpandedRpnObject(rpn::CRpnStack& targetstack, std::list<std::string>& lstMessages) const;
    ...
    void RpnVersionFromFileVersion(TSTREAM& stream, const CStreamVersion& version, std::vector<rpn::CRpnVersion>& vcRpnVersion) const;

	public:
		// Construction ...
		CDerivedResultComponent(CDerivedResult& parent, const CDepletionStage& stage,
      const CAnalysisType& m_antype, int m_nRegister);
    ...
    virtual TValueVec ValueElement(const geo::IElement& elm, const UNIT unit = CQuantity::SI_UNIT, geo::IParallelInitializationCallback *cb = 0) const;
  ...
  private:
    template <class T>
    void CollectProxies(std::vector<T*>& vcProxies) const
    {
