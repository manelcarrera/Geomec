...
#ifdef SKUA_NEW
  CGocadElementSet(const QString &sName, eElementType type, const TPointVec &vcPoint, const TElementVec &vcElement,
                   CFemAppModel &model, std::vector<const gm_skua::SKUAParseData *> &solids);
#else
  CGocadElementSet(const QString &sName, eElementType type, const TPointVec &vcPoint, const TElementVec &vcElement,
                   CFemAppModel &model, std::vector<QSharedPointer<CGocadData::CTSolid>> &solids);
#endif
...
#ifdef SKUA_NEW
  bool importInGocadModel(bool (*selectGocadSolidDlg)(const std::vector<const gm_skua::SKUAParseData *> &solids,
                                                      std::vector<int> &vcSolids));
#else
  bool importInGocadModel(bool (*selectGocadSolidDlg)(const std::vector<QSharedPointer<CGocadData::CTSolid>> &solids,
                                                      std::vector<int> &vcSolids));
#endif
