...
public:
  ...
  // The given formation is only used in the (derived) mesh elementset, it is ignored in the implementation of this
  // class
  virtual TWeightMap WeightFactors(int nNode, TWeightingType nWeightingType,
                                   const CFormationBase *pFormation = 0) const;
...
public:
  ...
  CElementSet(const QString &sName, eElementType type, const TPointVec &vcPoint, const TElementVec &vcElement,
              CFemAppModel &model);
  CElementSet(const CGoCadVolume &volume, CFemAppModel &model, CQuantity::UNIT coordinate_unit,
              CQuantity::UNIT value_unit);
  CElementSet(const QString &sName, CFemAppModel &model, CQuantity::UNIT coordinate_unit, CQuantity::UNIT value_unit);
  CElementSet(const CElementSet &volume, CModelBase &model, TPROGRESS &progress,
  ...
  CElementSet(const std::set<const geo::IElement *> &stElements, const QString &sName, eElementType type,
              CFemAppModel &model);
  CElementSet(const std::vector<const geo::IElement *> &vcElements, const QString &sName, eElementType type,
              CFemAppModel &model);
