...
// At some point we may want to have these settings per-model, but for development and evaluation a global instance is
// easier
class CValueMapper {
...
public:
  ...
  bool PrepareMappingMaterial(const CModelBase &model, const geo::IElementSet *pElementSet,
                              const CDepletionStage &stage, int flag = NONE);
  ...
  bool PrepareMappingComponent(const CModelBase &model, const geo::IElementSet *pElementSet,
                               const CDepletionStage &stage, TPrepareMappingComponentType type);

  bool PrepareMappingSurfaces(const CModelBase &model, const geo::IElementSet *pElementSet,
                              const CDepletionStage &stage);

  bool PrepareMappingElementSet(const geo::IElementSet *pElementSet, geo::IMesh *pMesh = 0);

  bool PrepareMappingStress(const CModelBase &model, IResultComponent &component, CResultRegister &resultRegister,
                            bool change);
  bool PrepareMappingStrain(const CModelBase &model, IResultComponent &component, CResultRegister &resultRegister);
  bool PrepareMappingPlasticEnergy(const CModelBase &model, IResultComponent &component,
                                   CResultRegister &resultRegister);
  bool PrepareMappingPorePressure(const CModelBase &model, IResultComponent &component, CResultRegister &resultRegister,
                                  const geo::IElementSet *pElementSet, bool change);
  bool PrepareMappingGammaValues(const CModelBase &model, IResultComponent &component, CResultRegister &resultRegister,
                                 const geo::IElementSet *pElementSet);
  bool PrepareMappingIndex(int index, const CModelBase &model, IResultComponent &component,
  ...
  void Add(CResultTree::CScalarResult::CScalarResultComponent *pComponent,
...
private:
  ...
  typedef std::pair<CResultTree::CScalarResult::CScalarResultComponent *,
                    CResultTree::CScalarResult::CScalarResultComponent::TDisplacementMap *>
      TResultDisplacementCachePair;
  typedef std::pair<CResultTree::CScalarResult::CScalarResultComponent *,
                    CResultTree::CScalarResult::CScalarResultComponent::TFormAvgVolMap *>
      TResultAvgVolumeCachePair;
  ...
  void collectFormationModelElementMaps(const CModelBase &model, const geo::IElementSet &elementSet,
                                        const CDepletionStage &stage, bool bDoWellCasing = false);
};
