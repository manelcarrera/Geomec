...
private:
  IValueDomainScalar::TValue InterpolateValuePoint(const geo::IPoint& pt, const CQuantity::UNIT unit, geo::IParallelInitializationCallback *cb) const;
...
public:
  ...
  // Construction ...
  CValueComponent(unsigned int uName, CValueType &value_type, IValueSet &value_set, unsigned int uComponent, unsigned int uMode);
  CValueComponent(const QString& sName, CValueType &value_type, IValueSet &value_set, unsigned int uComponent, unsigned int uMode);
...
public:
  ...
  : m_bMin(bMin), m_dMin(dMin), m_bMinInclusive(bMinInclusive), m_bMax(bMax), m_dMax(dMax), m_bMaxInclusive(bMaxInclusive)
  {
  }

  CRangeType(const CRangeType& rhs)
  : m_bMin(rhs.m_bMin), m_dMin(rhs.m_dMin), m_bMinInclusive(rhs.m_bMinInclusive), m_bMax(rhs.m_bMax), m_dMax(rhs.m_dMax), m_bMaxInclusive(rhs.m_bMaxInclusive)
  {
  }
...
public:
  CComponentTemp(CValueType &value_type, IValueSet &value_set, const Units::CUnitType& unittype, const Ranges::CRangeType& rangetype)
