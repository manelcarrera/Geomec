...
public:
  ...
  const IResultComponent* ResultComponent(const CDepletionStage& stage, const CAnalysisType& antype, int nRegister = 0, unsigned int nComponent = 0) const;
  IResultComponent* ResultComponent(const CDepletionStage& stage, const CAnalysisType& antype, int nRegister = 0, unsigned int nComponent = 0);
  const IResultComponent* ResultComponent(int nTimeStep, const CAnalysisType& antype, int nRegister = 0, unsigned int nComponent = 0) const;
  virtual IResultComponent* ResultComponent(int nTimeStep, const CAnalysisType& antype, int nRegister = 0, unsigned int nComponent = 0);

  bool Available(const CAnalysisType& antype, const CDepletionStage& stage, int nRegister = 0) const;

  geo::CValue ResultAtTime(const CAnalysisType& antype, const CGeomecTime &time, const geo::IPoint &pt, const IQuantityDouble::UNIT unit = IQuantityDouble::SI_UNIT, int nComponent = 0) const;
  std::vector<geo::CValue> ResultAtTime(const CAnalysisType& antype, const CGeomecTime &time, const geo::IElement &element, const IQuantityDouble::UNIT unit = IQuantityDouble::SI_UNIT, int nComponent = 0) const;
...
protected:
  virtual bool OnBuildComponent(const CDepletionStage& stage,
  const CAnalysisType& antype, int nRegister) const;

  bool validateName(CResultInfo& resultInfo, const std::string& name,
  ...
  bool validateDerivedName(CResultInfo& resultInfo, const std::string& name,
  const std::string& label, const std::string& componentList,
  const std::string& analysisTypeList) const;
};
...
public:
  ...
  // Construction
  IResultComponent(unsigned int uName, IResult &result, const CDepletionStage& stage, const CAnalysisType& antype, int nRegister, unsigned int uComponent);
  IResultComponent(const QString& sName, IResult& result, const CDepletionStage& stage, const CAnalysisType& antype, int nRegister, unsigned int uComponent);
  ...
  virtual void MapValueElement(const geo::IElement& elm, IValueDomainScalar::TValueVec& values, TMapType map_type, UNIT unit = CQuantity::SI_UNIT, geo::IParallelInitializationCallback *cb = 0) const = 0;
  ...
  virtual IValueDomainScalar::TValue GradientValuePoint(const geo::IPoint& pt, const UNIT unit = CQuantity::SI_UNIT, geo::IParallelInitializationCallback *cb = 0) const;
  ...
  virtual void MapTensorElement(const geo::IElement& /*elm*/, std::vector<CTensor> & /*values*/, geo::IParallelInitializationCallback* /*cb*/ = 0) const
  {
    assert(false);
  }

  virtual std::vector<CTensor>  TensorElement(const geo::IElement& element, const CQuantity::UNIT unit = CQuantity::SI_UNIT, geo::IParallelInitializationCallback *cb = 0) const;
...
public:
  IScalarResultComponent(unsigned int uName, IResult &result, const CDepletionStage& stage, const CAnalysisType& antype, int nRegister, unsigned int uComponent);
  IScalarResultComponent(const QString& sName, IResult& result, const CDepletionStage& stage, const CAnalysisType& antype, int nRegister, unsigned int uComponent);
  ...
  virtual void ElementValues(IValueDomainScalar::TValueVec& values, const geo::IElement& element, UNIT unit, geo::IParallelInitializationCallback *cb) const = 0;
