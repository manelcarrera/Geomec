...
class CTensorValueSet : public geo::IValueSetTemplate<TENSOR_TYPE>
{
public:
    ...
    CTensorValueSet(const CResultRegister& parent, 
  ...
  virtual void ElementValues(TTensorValueVec& values, int iElementIndex, geo::IParallelInitializationCallback *cb) const;
	typedef CValueType::TMapEnum TMapType;
  void MapValueElement(const geo::IElement& elm, TTensorValueVec& values, TMapType map_type, geo::IParallelInitializationCallback *cb) const;
...
*/
template<class TENSOR_TYPE>
CTensorValueSet<TENSOR_TYPE>::CTensorValueSet(const CResultRegister& parent, 
											  const CDepletionStage& stage, 
											  const CAnalysisType& antype, 
											  int nStart,
											  bool bTotal,
...
*/
template<class TENSOR_TYPE>
const geo::IElementSet &CTensorValueSet<TENSOR_TYPE>::ElementSet() const
{
	assert(m_pParent);
...
*/
template<class TENSOR_TYPE>
const double* CTensorValueSet<TENSOR_TYPE>::ValuesFromCache( int nElementIndex,
		                                       const CAnalysisType& antype,
								               const CDepletionStage& stage,
								               int nStart) const 
{
...
template<class TENSOR_TYPE>
void CTensorValueSet<TENSOR_TYPE>::FilterElements(std::vector<int> &indices) const
{
...
*/
template<class TENSOR_TYPE>
void CTensorValueSet<TENSOR_TYPE>::ElementValues(TTensorValueVec& values, int iElementIndex, geo::IParallelInitializationCallback * /*cb*/) const
{
	...
	if(!ElementValuesAvailable(iElementIndex) ||
		...
		for(size_t i = 0; i < values.size(); i++) values[i] = TENSOR_TYPE();
		return;
	}
		...
		for(size_t i = 0; i < values.size(); i++) {
			...
			values[i] = TENSOR_TYPE( pTensorVal[0][i] - pInitTensorVal[0][i] + dPressure - dInitPressure,
				                     pTensorVal[1][i] - pInitTensorVal[1][i] + dPressure - dInitPressure,
				 				     ...
				 				     pTensorVal[3][i] - pInitTensorVal[3][i],
				 				     pTensorVal[4][i] - pInitTensorVal[4][i],
				 				     pTensorVal[5][i] - pInitTensorVal[5][i]);
		}
	} else {
		for(size_t i = 0; i < values.size(); i++) {
			double dPressure = 0;
			if(m_bTotal) dPressure = ValuesFromCache(iElementIndex, m_antype, *m_pStage, CI_POREPRES)[i];
			values[i] = TENSOR_TYPE( pTensorVal[0][i] + dPressure,
				                     pTensorVal[1][i] + dPressure,
				 				     pTensorVal[2][i] + dPressure,
				 				     pTensorVal[3][i],
								     pTensorVal[4][i],
								     pTensorVal[5][i]);
		}
...
*/
template<class TENSOR_TYPE>
bool CTensorValueSet<TENSOR_TYPE>::IsEmpty() const
{
	return true;
}

template<class TENSOR_TYPE>
bool CTensorValueSet<TENSOR_TYPE>::ElementValuesAvailable(int nElementIndex ) const
{
	int i;
	for(i = 0; i < 6; i++) {
	  if(!m_pParent->ResultAvailable(*m_pStage, m_antype, nElementIndex, m_nStart + i)) return false;
	}

	if(m_bTotal && !m_pParent->ResultAvailable(*m_pStage, m_antype, nElementIndex, CI_POREPRES)) return false;

	if(m_bChange) {
		const CDepletionStage* pIniStage = &m_pParent->DepletionStageEntry().MarkedAsInitialStage();

		for(i = 0; i < 6; i++) {
		  if(!m_pParent->ResultAvailable(*pIniStage, m_antype, nElementIndex, m_nStart + i)) return false;
		}

		if(m_bTotal && !m_pParent->ResultAvailable(*pIniStage, m_antype, nElementIndex, CI_POREPRES)) return false;
	}
...
template<class TENSOR_TYPE>
void CTensorValueSet<TENSOR_TYPE>::MapValueElement(const geo::IElement& elm, TTensorValueVec& values, TMapType map_type, geo::IParallelInitializationCallback *cb) const
{
  ...
  if(map_type == CValueType::MT_POINT || !pBody) {
	...
	// Make tensor
	for(size_t i = 0; i < values.size(); i++) {
	  if( scalars[0][i].Valid() &&
	  	  scalars[1][i].Valid() &&
		  scalars[2][i].Valid() &&
		  scalars[3][i].Valid() &&
		  scalars[4][i].Valid() &&
		  scalars[5][i].Valid() ) {
		values[i] = TENSOR_TYPE( scalars[0][i].Value(),
		                         scalars[1][i].Value(),
		  					     scalars[2][i].Value(),
								 scalars[3][i].Value(),
								 scalars[4][i].Value(),
