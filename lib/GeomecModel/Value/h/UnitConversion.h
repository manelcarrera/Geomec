...
public:
  ...
  static IUnitConversion *Create(const std::string &sJustAName // identify by name
                                 ,
...
{
public:
  static CLinearRationalUnitConversion *Create(const std::string &sJustAName, const std::string &sUnitName);
  virtual double ConvertToSi(const double &value) const;

private:
  CLinearRationalUnitConversion(const std::string &name, const double &divisor_factor = 1,
                                const double &divisor_addition = 0, const double &denominator_factor = 0,
