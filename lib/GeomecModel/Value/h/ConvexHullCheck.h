...
public:
  ...
  virtual void DistributedValue(size_t index, const geo::IElement &elm, const CDoubleQuantity::UNIT unit,
                                IValueDomainScalar::TValueVec &vcValues) = 0;
  virtual void DistributedValue(size_t index, const geo::IElement &elm, const CDoubleQuantity::UNIT unit,
                                IValueDomainVector::TValueVec &vcValues) = 0;
  virtual void DistributedValue(size_t index, const geo::IElement &elm, const CDoubleQuantity::UNIT unit,
                                IValueDomainTensor::TValueVec &vcValues) = 0;

  virtual bool DistributedOnly() = 0;
  virtual bool DoAveraging() = 0;
};

template <class VALUE_DOMAIN_TYPE> class CConvexHullCheck {
public:
  static void ValueElement(IDistributedSetWrapper &distriSet, const geo::IElement &elm, const CQuantity::UNIT unit,
                           VALUE_DOMAIN_TYPE &vcRet);
};

template <class VALUE_DOMAIN_TYPE>
void CConvexHullCheck<VALUE_DOMAIN_TYPE>::ValueElement(IDistributedSetWrapper &distriSet, const geo::IElement &elm,
                                                       const CQuantity::UNIT unit, VALUE_DOMAIN_TYPE &vcRet) {
