...
public:
  IRpnMaterialParameterProxy(CMeshBase &mesh, rpn::CRpnStack &NewStack, bool bObsolete = false)
      : rpn::CRpnOperand::IValueProxy(NewStack), m_pMesh(&mesh), m_DeplStage(0), m_bFixedStage(false),
        m_bObsolete(bObsolete) {}

  IRpnMaterialParameterProxy(CMeshBase &mesh, const CDepletionStage *pStage, rpn::CRpnStack &NewStack,
                             const QString &sProxyId, bool bFixedStage, bool bObsolete = false)
      : rpn::CRpnOperand::IValueProxy(NewStack, sProxyId), m_pMesh(&mesh), m_DeplStage(pStage),
        m_bFixedStage(bFixedStage), m_bObsolete(bObsolete) {}

  IRpnMaterialParameterProxy(rpn::CRpnStack &stack, const QString &sProxyId)
      : rpn::CRpnOperand::IValueProxy(stack, sProxyId), m_pMesh(0), m_DeplStage(0), m_bFixedStage(false),
        m_bObsolete(false) {}
...
public:
  ...
  CRpnMaterialParameterProxy(CMeshBase &mesh, rpn::CRpnStack &NewStack, bool bObsolete = false)
      : IRpnMaterialParameterProxy(mesh, NewStack, bObsolete) {}

  CRpnMaterialParameterProxy(CMeshBase &mesh, const CDepletionStage *pStage, rpn::CRpnStack &NewStack,
                             const QString &sProxyId, bool bFixedStage)
      : IRpnMaterialParameterProxy(mesh, pStage, NewStack, sProxyId.toStdString().c_str(), bFixedStage) {}

  CRpnMaterialParameterProxy(rpn::CRpnStack &stack, const QString &sProxyId)
      : IRpnMaterialParameterProxy(stack, sProxyId.toStdString().c_str()) {}
  ...
  virtual rpn::CRpnOperand::IValueProxy *Clone(rpn::CRpnStack &NewStack) const {
    ...
    return new CRpnMaterialParameterProxy(mesh, pTimeStep, NewStack, sProxyId, bFixedStage);
  }
  ...
  virtual QString DisplayName() const { return getStringTableEntry(T::name_id()); }
  ...
  if (Mesh()->IsMesh()) {
    ...
    if (pElement) {
      ...
      if (pInterface) {
        ...
        const CInterfaceElementMaterial &elMat = dynamic_cast<const CInterfaceElementMaterial &>(mat);
      ...
      // end wjrx mantis 2545
      {
        ...
        if (Mesh()->Formation(*pElement)->Material(model.InitialDepletionStage()).LibraryMaterial() != 0) {
          const CFFMaterial &material = (TimeStep())
                                            ? Mesh()->Formation(*pElement)->Material(*TimeStep()).Material(*pElement)
