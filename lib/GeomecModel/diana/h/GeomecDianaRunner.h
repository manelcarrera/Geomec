...
public:
  CGeomecDianaRunnerBase(CMeshBase &meshbase,
               CModelBase &model,
...
protected:
  typedef enum eSolver {
  ITERATIVE = 0,
  PARALLEL_DIRECT_SPARSE = 1
  } TSolver;
  ...
  public:
    ...
    CEdgeLine &operator=(const CEdgeLine &rhs) { m_pt1 = rhs.m_pt1; m_pt2 = rhs.m_pt2; return *this; }
  ...
  CDistributedElementSetInfo(const CFormationBase *pFormation, ftn_int_t eltidx, const dia::IElementProperty *pMaterial, const dia::IElementProperty *pGeometry);
  ...
  virtual void _CreateVolumetricStrainLoad(const geo::IElement& elm, const CDepletionStage& stage, dia::CLoadCase& lcase);
  ...
  bool WriteZoomInInitialStresses(const geo::IElement &element, dia::CLoadCase &lcase, const IValueDomainScalar::TValueVec &vcInitialPressures);
  ...
  geo::CVector GetDisplacementVector(const CVectorValueSet& displa, const geo::IElement& elm, const geo::IPoint& ptSample) const;
...
private:
  bool calculateVolumetricStrain(std::vector <double>& vcElementValues,
  ...
  bool WriteHexaHorizonBoundaryLoads(
    const C3DHorizon& hor,
    TStageMap& mpStages,
    const CPressureSupportNode& pnode);

  bool WriteInterfaceBoundaryLoads(
    TStageMap& mpStages,
    const CInterfaceBoundary& boundary,
    const CMeshBase& mesh);

  bool WriteBoundaryStressLoadsFromParentModel(	
    TStageMap &mpStages,
    const CInterfaceBoundary &boundary,
    const CMeshBase &mesh);

  bool WriteBoundaryDisplacementLoadsFromParentModel(	
    TStageMap &mpStages,
    const CInterfaceBoundary &boundary,
    const CMeshBase &mesh);

  bool WriteBoundaryLoadsUsingDistributions(
    TStageMap &mpStages,
    const CInterfaceBoundary &boundary,
    const CMeshBase &mesh);

  bool WriteBoundaryLoadsForSurfaceDesc(
    TStageMap &mpStages,
    const geo::CSurfaceDesc& surfdesc,
    const CPressureSupportNode &pnode);

  bool WriteBoundaryLoadsForSurfaces(
    TStageMap &mpStages,
    const std::vector<const geo::ISurface*>& vcSurface,
    const CPressureSupportNode &pnode);

  void CreatePrescDisplLoad(
    const CDisplacementSupportNode &node,
    int nNode,
  ...
  void CreateBodyFaceLoad(
    const CPressureSupportNode &node,
  ...
  bool WritePrescribedDisplacements(
    TStageMap &mpStages,
    const std::vector<const geo::ISurface*>& vcSurface,
    CPressureSupportNode::CFaceSupportDef::TSupportType suptype);

  geo::CMatrix CreateGlobalTensorPreMultMatrix(
    const CDisplacementSupportNode &dnode,
    const CDepletionStage &stage);

  void WritePrescribedDisplacementForSupport(
    dia::CLoadCase& lcase,
    const geo::IVector& vecDispla,
    const geo::CTranslationSupport& support);

  bool WritePrescribedDisplacementsFromInterpolation(
    const geo::IMesh &mesh,
  ...
  CDisplacementSupportNode::CNodalSupportDef::CSupportDef InterpolateSupportDef(
    const geo::IPoint &pt,
  ...
  void WritePrescribedDisplacementsFromGlobalTensor(
    const geo::IMesh &mesh,
    //const geo::CTetSurface &surface,
    const geo::ISurface &surface,
    const CDisplacementSupportNode &dnode,
  ...
  geo::CVector PressureVector(
    const geo::IFace &face,
    const CDepletionStage &stage,
    const geo::ISurface &surface);

  void WriteBoundaryPressureEntry(
    const CDepletionStage &stage,
    dia::CLoadCase &lcase,
    const geo::ISurface &surface);

  void WriteBoundaryPressure(
    TStageMap &mpStages,
    const geo::ISurface &surfacedesc,
    const CPressureSupportNode::CFaceSupportDef &facedef);

  std::vector <double> recalculateElementConcentrations(
  ...
  virtual void ValueAt(const geo::IElement &element, const dia::IValueTable::CTimePoint &tpoint, std::vector<double>& vcValues) const;
  };
