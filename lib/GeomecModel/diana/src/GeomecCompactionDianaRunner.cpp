...
CGeomecCompactionDianaRunner::CGeomecCompactionDianaRunner(CMeshBase &meshbase, CModelBase &model,
...
bool CGeomecCompactionDianaRunner::OnlyNewLoads(const CDepletionStage &startstage) const {
  ...
  for (THorizonBaseEntry::TNodeSet::const_iterator it = stHorNodes.begin(); it != stHorNodes.end(); ++it) {
    ...
    if (hor.Slip() && hor.SlipType() == CHorizonBase::USER) {
      ...
      if (!params.CohesionSameAsPrevious() || !params.FrictionSameAsPrevious() || !params.DStiffSameAsPrevious() ||
          params.DistributedCohesion() || params.DistributedFriction()) {
...
void CGeomecCompactionDianaRunner::WriteNonlinCommands(const CDepletionStage &startstage,
                                                       const CDepletionStage &endstage, int iStartS) {
  const CModelBase *pModel = dynamic_cast<const CModelBase *>(&MeshBase().Model());

  if (pModel->LargeDeformations())
  ...
  if (!pStage->Initial()) {
    ftn_double_t ttime = (ftn_double_t)((pModel->InitialDepletionStage().Time().SecondsTo(pStage->Previous().Time())) /
                                        SECONDS_PER_MONTH);
    PutItem("TTIME", &ttime);
  }
...
int CGeomecCompactionDianaRunner::CommandsProgressSize() const { return 1; }
...
bool CGeomecCompactionDianaRunner::OnEndPostprocessWriteInputFiles(const std::string &sComfileName,
                                                                   const std::string &sDatfileName) {
  if (!CGeomecDianaRunnerBase::OnEndPostprocessWriteInputFiles(sComfileName, sDatfileName))
    return false;

  if (Controller().AnalysisType().AnalysisType() == CAnalysisType::AT_NONLIN &&
      (Controller().WriteInputFiles() || Controller().WriteQuadDat())) {
    ...
    if (fpin) {
      char *tmpcomfile =
          vDiStrsave(GetGeomecTempPathExt(CTempPath::TEMP_CALCULATION).toStdString().c_str(), "\\geomeccomXXXXXX", 0);
      mktemp(tmpcomfile);
...
bool CGeomecCompactionDianaRunner::OnWriteCommands() {
  ...
  if (Controller().AnalysisType().AnalysisType() == CAnalysisType::AT_NONLIN &&
    ...
    bRet = DoWriteCommands(Controller().StartStage().InitialStage(), Controller().EndStage(),
...
bool CGeomecCompactionDianaRunner::DoWriteCommands(const CDepletionStage &startstage, const CDepletionStage &endstage,
                                                   int iStartS) {
  ...
  if (Controller().WriteQuadDat()) {
    ...
    case CAnalysisType::AT_LINEAR: {
      ...
      ftn_double_t *pTimes =
          (ftn_double_t *)DiMalloc(nStages * sizeof(ftn_double_t), "CGeomecCompactionDianaRunner::DoWriteCommands");
...
bool CGeomecCompactionDianaRunner::FetchInitialTemperatures(const geo::IElement &element,
                                                            const CDepletionStage & /*stage*/,
                                                            IValueDomainScalar::TValueVec &vcInitialTemperatures) {
  if (dynamic_cast<const geo::CInterfaceElement *>(&element) == 0 &&
      dynamic_cast<const geo::CLineInterface *>(&element) == 0)
    FetchElementInitialTemperatures(Model().InitialDepletionStage(), element, vcInitialTemperatures);

  return true;
}

void CGeomecCompactionDianaRunner::FetchElementInitialTemperatures(
    const CDepletionStage &stage, const geo::IElement &element, IValueDomainScalar::TValueVec &vcInitialTemperatures) {
  // lookup formation ...
...
bool CGeomecCompactionDianaRunner::FetchElementInitialPressures(const CDepletionStage &stage,
...
void CGeomecCompactionDianaRunner::WriteElementInitialLoads(
    const CDepletionStage & /*stage*/, const geo::IElement &element, dia::CLoadCase &lcase,
...
void CGeomecCompactionDianaRunner::FetchStagePressures(const geo::IElement &element, const CDepletionStage &stage,
...
void CGeomecCompactionDianaRunner::FetchStageTemperatures(const geo::IElement &element, const CDepletionStage &stage,
...
void CGeomecCompactionDianaRunner::WriteElementStageLoads(const CDepletionStage &stage, const geo::IElement &element,
                                                          dia::CLoadCase &lcase,
                                                          const IValueDomainScalar::TValueVec &vcInitialPressures,
                                                          const IValueDomainScalar::TValueVec &vcInitialTemperatures) {
  // depletion stage pressures
