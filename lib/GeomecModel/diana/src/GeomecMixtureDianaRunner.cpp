...
CGeomecMixtureDianaRunner::CGeomecMixtureDianaRunner(CMeshBase &meshbase,
                                                     CModelBase &model,
...
bool CGeomecMixtureDianaRunner::AllowLinearCalc() const
{
  return false;
}
...
bool CGeomecMixtureDianaRunner::OnApplyLoads(dia::CLoadManager &manager)
{
  	...
  	if(!pIface)
  {
    ...
    vcInitialTemperatures = formation.EffectiveTemperature(Model().InitialDepletionStage()).Component().ScalarData().ValueElement(elm);
  }
  ...
  for(TStageMap::iterator it = mpStages.begin(); it != mpStages.end(); ++it)
  {
    ...
    if(stage.Initial())
    {
      if(!pIface)
      {
        ...
        {
        const CFormationBase &formation = *MeshBase().Formation(elm);
        IValueDomainScalar::TValueVec vcInitialPressures = formation.Pressure(stage).Component().ScalarData().ValueElement(elm);
...
bool CGeomecMixtureDianaRunner::OnWriteCommands()
{
  ...
  if(!pStage->Initial())
  {
    ftn_double_t ttime = (ftn_double_t)((Model().InitialDepletionStage().Time().SecondsTo(pStage->Previous().Time())) / SECONDS_PER_MONTH);
    PutItem("TTIME", &ttime);
  }
...
int CGeomecMixtureDianaRunner::LoadsProgressSize() const
{
  return Mesh().NodeSize() + Mesh().ElementSize();
}

int CGeomecMixtureDianaRunner::CommandsProgressSize() const
{
...
void CGeomecMixtureDianaRunner::ElementPressures(const geo::IElement& elm, const CDepletionStage& stage, IValueDomainScalar::TValueVec& vcValues) const
{
...
void CGeomecMixtureDianaRunner::ModifyMaterialForContainment(const dia::IMaterial& mat, const geo::IElement& element) const
{
...
void CGeomecMixtureDianaRunner::FetchFixedPotentialNodes() const
{
  ...
  if(horizon.Slip())
  {
    ...
    for(i = 0; i < pGroup->ElementSize(); ++i)
    {
          ...
          {
      CModelBase::TNodeElementsMap::iterator ite = m_mpDepletingFaultNodes.insert(CModelBase::TNodeElementsMap::value_type(elm.Node(n).Index(), std::set<int>())).first;
...
geo::CValue CGeomecMixtureDianaRunner::Pressure(const geo::INode& node, const std::set<int>& stAttachedElements, const CDepletionStage& stage) const
{
...
{
  return (dynamic_cast<const geo::CInterfaceElement*>(&elm) != 0 || dynamic_cast<const geo::CLineInterface*>(&elm) != 0);
}
...
void CGeomecMixtureDianaRunner::FetchStageTemperatures(const geo::IElement& element, const CDepletionStage& stage, IValueDomainScalar::TValueVec& vcTemperatures)
{
...
void CGeomecMixtureDianaRunner::WriteElementStageLoads(const CDepletionStage &stage, const geo::IElement &element, dia::CLoadCase &lcase, const IValueDomainScalar::TValueVec& vcInitialTemperatures)
{
  // depletion stage pressures
  IValueDomainScalar::TValueVec vcValues;
