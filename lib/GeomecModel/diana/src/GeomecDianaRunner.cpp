...
CGeomecDianaRunnerBase::CElementSetInfo::CElementSetInfo(ftn_int_t matidx, ftn_int_t geoidx,
                                                         const CFormationBase *pFormation, ftn_int_t eltidx)
    : matidx(matidx), geoidx(geoidx), pFormation(pFormation), eltidx(eltidx) {
  assert(eltidx > 0);
}
...
CGeomecDianaRunnerBase::CGroupInfo::CGroupInfo(const CFormationBase *pFormation, ftn_int_t eltsetidx)
    : pFormation(pFormation), eltsetidx(eltsetidx) {
  assert(eltsetidx > 0);
}
...
CGeomecDianaRunnerBase::CDistributedElementSetInfo::CDistributedElementSetInfo(const CFormationBase *pFormation,
                                                                               ftn_int_t eltidx,
                                                                               const dia::IElementProperty *pMaterial,
                                                                               const dia::IElementProperty *pGeometry)
    : pFormation(pFormation), eltidx(eltidx) {
  assert(eltidx > 0);
  assert(pMaterial);
...
CGeomecDianaRunnerBase::CGeomecDianaRunnerBase(CMeshBase &meshbase, CModelBase &model, CDianaRunController &controller)
    : CDianaRunner(meshbase.Mesh()), m_MeshBase(meshbase), m_Model(model), m_controller(controller),
      m_bAnalysisSuccess(false), m_AllowCleanUpAfterRun(true), m_bPerformInitialStressCalculation(true),
      m_bBehaveLinearly(false), m_bBehaveLinearlyDirty(true) {
...
const CDianaRunController &CGeomecDianaRunnerBase::Controller() const { return m_controller; }

void CGeomecDianaRunnerBase::SetLocation(const std::string &strLocation) { m_strLocation = strLocation; }

void CGeomecDianaRunnerBase::SetFileName(const std::string &strFileName) { m_strFileName = strFileName; }

bool CGeomecDianaRunnerBase::HasTemperatureValue(const geo::IElement &element) const {
  // not for interface elements
  if (dynamic_cast<const geo::CInterfaceElement *>(&element))
    return false;
...
bool CGeomecDianaRunnerBase::WriteBoundaryLoadsForSurfaceDesc(TStageMap &mpStages, const geo::CSurfaceDesc &surfdesc,
...
bool CGeomecDianaRunnerBase::WriteHexaHorizonBoundaryLoads(const C3DHorizon &hor, TStageMap &mpStages,
                                                           const CPressureSupportNode &pnode) {
...
IValueDomainScalar::TValueVec CGeomecDianaRunnerBase::ElementTemperatures(const geo::IElement &element,
...
IValueDomainScalar::TValueVec CGeomecDianaRunnerBase::ElementConcentrations(const geo::IElement &element,
                                                                            const CDepletionStage &stage) const {
  // get the formation
  const CFormationBase *pFormation = Model().Mesh().Formation(element);
  assert(pFormation);
...
CGeomecDianaRunnerBase::TSolver CGeomecDianaRunnerBase::Solver() const { return ITERATIVE; }

bool CGeomecDianaRunnerBase::WriteInterfaceBoundaryLoads(TStageMap &mpStages, const CInterfaceBoundary &boundary,
  ...
  if (!bUserLoads) {
    for (TStageMap::const_iterator it = mpStages.begin(); it != mpStages.end(); ++it) {
      if (pnode.Distributed(*it->first) || dnode.Distributed(*it->first)) {
        bUserLoads = true;
        break;
      }
    }
...
bool CGeomecDianaRunnerBase::WriteBoundaryStressLoadsFromParentModel(TStageMap &mpStages,
  ...
  CStressTensorValueSet initialStress = Model().parentModel()->ResultRegister().TotalStress(
      parentInitialStage,
      Model().ParentLinearResults(Controller().AnalysisType()) ? CAnalysisType::AT_LINEAR : CAnalysisType::AT_NONLIN,
      false);
  ...
  for (TStageMap::iterator it = mpStages.begin(); it != mpStages.end(); ++it) {
    ...
    CStressTensorValueSet stress = Model().parentModel()->ResultRegister().TotalStress(
        parentStage,
        Model().ParentLinearResults(Controller().AnalysisType()) ? CAnalysisType::AT_LINEAR : CAnalysisType::AT_NONLIN,
        false);
    ...
    for (int i = 0; i < boundary.InterfaceElements()->ElementSize(); ++i) {
      ...
      for (int n = 0; n < bodyface.NrOfNodes(); ++n) {
        ...
        {
          stresstensor =
              stresstensor - initialStress.ValuePoint(node, geo::IParallelInitializationCallback::Sequential);
        }
...
bool CGeomecDianaRunnerBase::WriteBoundaryDisplacementLoadsFromParentModel(TStageMap &mpStages,
                                                                           const CInterfaceBoundary &boundary,
                                                                           const CMeshBase &mesh) {
  ...
  // so for now we leave this as it is, and if we get a bug-report about it: solution analogous to
  // WriteBoundaryStressLoadsFromParentModel
  ...
  for (TStageMap::iterator it = mpStages.begin(); it != mpStages.end(); ++it) {
    ...
    CVectorValueSet displa = Model().parentModel()->ResultRegister().Displacement(
        parentStage,
        Model().ParentLinearResults(Controller().AnalysisType()) ? CAnalysisType::AT_LINEAR : CAnalysisType::AT_NONLIN,
        false);
...
bool CGeomecDianaRunnerBase::WriteBoundaryLoadsUsingDistributions(TStageMap &mpStages,
                                                                  const CInterfaceBoundary &boundary,
                                                                  const CMeshBase &mesh) {
  const CPressureSupportNode &pnode = boundary.PressureSupportNode();
  const CDisplacementSupportNode &dnode = boundary.DisplacementSupportNode();
  ...
  for (i = 0; i < boundary.InterfaceNodeSize(); i++) {
    ...
    for (it = mpStages.begin(); it != mpStages.end(); it++) {
      ...
      if (dnode.Distributed(*(*it).first)) { // a distribution was dropped on the node for this stage
...
bool CGeomecDianaRunnerBase::WriteBoundaryLoadsForSurfaces(TStageMap &mpStages,
...
void CGeomecDianaRunnerBase::CreatePrescDisplLoad(const CDisplacementSupportNode &node, int nNode,
                                                  const geo::IMesh &mesh, const CDepletionStage &stage,
...
void CGeomecDianaRunnerBase::CreateBodyFaceLoad(const CPressureSupportNode &node, const geo::IElement &element,
...
void CGeomecDianaRunnerBase::StressVectors(const CPressureSupportNode &node, const geo::IElement & /*element*/,
                                           const CDepletionStage &stage, const geo::IFace &face, bool bDistrAvailable,
                                           std::vector<geo::CVector> &vcPressure) {
...
void CGeomecDianaRunnerBase::DisplacementVector(const CDisplacementSupportNode &node, int nNode, const geo::IMesh &mesh,
                                                const CDepletionStage &stage, bool bDistrAvailable,
...
bool CGeomecDianaRunnerBase::WritePrescribedDisplacements(
    TStageMap &mpStages, const std::vector<const geo::ISurface *> &vcSurface,
  ...
  do {
    ...
    for (int j = 0; j < surface.PointSize(); j++) {
      ...
      if (itSupDef != mpSupportDef.end()) {
        ...
        bMustInterpolate = bMustInterpolate || (pSupDef->SupportDef(*it->first).SupportType() !=
                                                CDisplacementSupportNode::CNodalSupportDef::CSupportDef::ST_GLOBAL);
      }
    }
    ...
    if (bMustInterpolate || MatPreMult.Empty()) {
      if (!WritePrescribedDisplacementsFromInterpolation(Mesh(), surface, dnode, *it->first, *it->second, MatPreMult,
                                                         mpSupportDef))
        return false;
    } else {
...
geo::CMatrix CGeomecDianaRunnerBase::CreateGlobalTensorPreMultMatrix(const CDisplacementSupportNode &dnode,
                                                                     const CDepletionStage &stage) {
  return dnode.CreateGlobalTensorPreMultMatrix(stage);
}

void CGeomecDianaRunnerBase::WritePrescribedDisplacementForSupport(dia::CLoadCase &lcase, const geo::IVector &vecDispla,
...
void CGeomecDianaRunnerBase::WritePrescribedDisplacementsFromGlobalTensor(
    const geo::IMesh &mesh, const geo::ISurface &surface, const CDisplacementSupportNode &dnode,
    const CDepletionStage &stage, dia::CLoadCase &lcase, const geo::IMatrix &MatPreMult) {
  for (int i = 0; i < surface.PointSize(); i++) {
    ...
    if (mesh.HasTranslationSupport(pt.Index())) {
      if (m_mpProcessedNodes.find(CStagePointCombi(stage, pt)) == m_mpProcessedNodes.end()) {
        ...
        m_mpProcessedNodes.insert(
            std::make_pair(CStagePointCombi(stage, pt), CDisplacementSupportNode::CNodalSupportDef::CSupportDef()));
...
bool CGeomecDianaRunnerBase::WritePrescribedDisplacementsFromInterpolation(
    const geo::IMesh &mesh, const geo::ISurface &surface, const CDisplacementSupportNode &dnode,
    const CDepletionStage &stage, dia::CLoadCase &lcase, const geo::IMatrix &MatPreMult, TSupportDefMap &mpSupportDef) {
  ...
  if (vcAllEdges.size() == 0) {
    if (dynamic_cast<CHexaModel *>(&Model())) {
      // we found no edge, possibly due to tyings; we try different epsilons (if need be) until we get one edge (we
      // assume that we don't go from NO edge to several rings)
        ...
        if (it == mpSupportDef.end()) {
          if (mpSupportDef.size() > 0) {
            ...
            for (;;) {
              ...
              if (mpSupportDef.find((geo::INode *)vcEdge[iPrev]) != mpSupportDef.end() || iPrev == i) {
            ...
            for (;;) {
              iLast = iNext;
              if (++iNext >= vcEdge.size())
                iNext = 0;
              dDistNext += vcEdge[iNext]->Distance(*vcEdge[iLast]);
              if (mpSupportDef.find((geo::INode *)vcEdge[iNext]) != mpSupportDef.end())
                break;
            }
  ...
  if (vecAverageNormal == geo::CVector::NullVector)
    vecAverageNormal =
        geo::CVector::Xaxis; // probably a side surface that goes around the entire model (single side surface)
      ...
      if (m_mpProcessedNodes.find(CStagePointCombi(stage, pt)) == m_mpProcessedNodes.end()) {
        ...
        for (j = vcEdge.size() - 1, k = 0; k < vcEdge.size(); j = k++) {
          ...
          if (!ptIntersect.Empty())
            mpIntersectionPoints.insert(std::make_pair(ptIntersect, l));
        }
        ...
        if (mpIntersectionPoints.size() > 2) {
          ...
          for (geo::CCoordinateMap<geo::CPoint, CEdgeLine>::iterator it = mpIntersectionPoints.begin();
               it != mpIntersectionPoints.end(); it++) {
            std::map<geo::IVector *, const geo::IPoint *>::iterator itm =
                mpVectors
                    .insert(std::map<geo::IVector *, const geo::IPoint *>::value_type(new geo::CVector(pt, it->first),
                                                                                      &(it->first)))
                    .first;
          ...
          for (std::map<geo::IVector *, const geo::IPoint *>::iterator itm = mpVectors.begin(); itm != mpVectors.end();
               itm++) {
          ...
          for (itmap2 = mpIntersectionPoints.begin(); itmap2 != mpIntersectionPoints.end(); itmap2++) {
            if (*pLeave1 != itmap2->first && *pLeave2 != itmap2->first)
              vcDelete.push_back(&(itmap2->first));
          }
...
CGeomecDianaRunnerBase::InterpolateSupportDef(const geo::IPoint &pt, const geo::IPoint &pt1, const geo::IPoint &pt2,
...
geo::CVector CGeomecDianaRunnerBase::PressureVector(const geo::IFace &face, const CDepletionStage &stage,
...
void CGeomecDianaRunnerBase::WriteBoundaryPressureEntry(const CDepletionStage &stage, dia::CLoadCase &lcase,
                                                        const geo::ISurface &surface) {
...
void CGeomecDianaRunnerBase::WriteBoundaryPressure(TStageMap &mpStages, const geo::ISurface &surface,
                                                   const CPressureSupportNode::CFaceSupportDef &facedef) {
  assert(facedef.SupportType() == CPressureSupportNode::CFaceSupportDef::ST_STRESS);
...
std::vector<double> CGeomecDianaRunnerBase::recalculateElementConcentrations(bool &bValid, bool &bWrite, bool &bEqual,
                                                                             IValueDomainScalar::TValueVec &vcValues,
...
bool CGeomecDianaRunnerBase::OnInitialize(const std::string &title, IRetrieveDianaFileNames &retrieveDianaFileNames) {
  ...
  if (!WriteFullModel()) {
    ...
    if (UseLegacyCodeForMaterialProperties()) {
      ...
      for (int i = 0; i < Mesh().ElementSize(); i++) {
        ...
        distributed_info.push_back(CDistributedElementSetInfo(MeshBase().Formation(elm), elm.Index() + 1,
                                                              &Material(elm), ElementGeometry(elm)));
...
bool CGeomecDianaRunnerBase::OnWriteElementGroups(TGroupInfoVector &group_info) {
  m_mpFormationElmsetNames.clear(); // !! if this routine is ever overridden, make sure to handle this properly !! for
                                    // now we assume we build it from scratch here

  if ((m_controller.WriteInputFiles() || m_controller.WriteQuadDat()) && WriteFullModel()) {
    // get the dimension of /GROUPS, override may have written groups already...
    int offset = 0;
    if (XistIndexed("/GROUPS", 0))
      offset = Inquire("/GROUPS", "DIM");
...
bool CGeomecDianaRunnerBase::OnWriteElementSets(TElementSetInfoVector &elementset_info) {
  ...
  int offset = 0;
  if (XistIndexed("/ELMSET", 0))
    offset = Inquire("/ELMSET", "DIM");
  ...
  while (idx_cur < idx_max) {
    ...
    while (idx_cur < idx_max && matidx == elementset_info[idx_cur].matidx &&
           geoidx == elementset_info[idx_cur].geoidx && pFormation == elementset_info[idx_cur].pFormation)
      ++idx_cur;
...
bool compareMaterialProperties(const dia::IElementProperty *lhs, const dia::IElementProperty *rhs) {
  ...
  if (lhs->isInterfaceElementMaterial()) {
    ...
    return static_cast<const CInterfaceElementMaterial *>(lhs)->sameSlip(
        *static_cast<const CInterfaceElementMaterial *>(rhs));
  }
...
bool CGeomecDianaRunnerBase::OnWriteElementInformation(TDistributedElementSetInfoVector &distributed_info) {
  ...
  int elmset_offset = 0;
  if (XistIndexed("/ELMSET", 0))
    elmset_offset = Inquire("/ELMSET", "DIM");
  ...
  */
  ...
  while (idx_cur < idx_max) {
    ...
    const dia::IElementProperty *pProperty[2] = {distributed_info[idx_start].pProperty[M],
                                                 distributed_info[idx_start].pProperty[G]};

    ++idx_cur;

    while (idx_cur < idx_max && pFormation == distributed_info[idx_cur].pFormation &&
           compareMaterialProperties(pProperty[M], distributed_info[idx_cur].pProperty[M]) &&
           ((pProperty[G] == 0 && distributed_info[idx_cur].pProperty[G] == 0) ||
            (pProperty[G] != 0 && distributed_info[idx_cur].pProperty[G] != 0)))
      ++idx_cur;
    ...
    ftn_int_t *Elements =
        (ftn_int_t *)DiMalloc(NrOfElements * sizeof(ftn_int_t), "IDianaRunner::OnWriteElementInformation");
    ...
    int propidx[2] = {0, 0};
    int nSize[2] = {pProperty[M]->WriteFilosParamSize(*this),
                    pProperty[G] ? pProperty[G]->WriteFilosParamSize(*this) : 0};
    ...
    for (int i = 0; i < 2; ++i) {
      ...
      for (size_t j = 0; j < NrOfElements; ++j)
        distributed_info[idx_start + j].pProperty[i]->WriteFilosParamValues(*this,
                                                                            valueCache[i] + nSize[i] * j); // STRIDE 1
      // distributed_info[idx_start + j].pProperty[i]->WriteFilosParamValues(*this, valueCache[i] + j, NrOfElements); //
      // STRIDE #Elts
      ...
      //  update: change to stride 1 due to different order in Diana precluding us from using memcpy; now we go for
      //  locality
    ...
    for (int i = propStart; i < propSize; ++i) {
      ...
      ftn_char_t *parameptr =
          (ftn_char_t *)DiMalloc(paramSize[i] * sizeof(ftn_char_t), "IDianaRunner::OnWriteElementInformation");
      ftn_char_t parameval =
          (ftn_char_t)DiCalloc(paramSize[i], sizeof(char) * 10, "IDianaRunner::OnWriteElementInformation");
      for (int j = 0; j < paramSize[i]; ++j)
        parameptr[j] = parameval + j * 10;

      ftn_double_t *values = (ftn_double_t *)DiMalloc(paramSize[i] * NrOfElements * sizeof(ftn_double_t),
                                                      "IDianaRunner::OnWriteElementInformation");
      ...
      for (int j = 0; j < nSize[i]; ++j) {
        if (bDistributed[i][j]) {
          ...
          // for (int k = 0; k < NrOfElements; ++k) // CAUTION: this needs to be checked with final Diana
          // implementation, but order should be param0 for all elts, param1 for all elts, etc
          // values[dis_idx * NrOfElements + k] = *(valueCache[i] + k * nSize[i] + j); // STRIDE 1 (including the
          // for-loop, of course) memcpy(values + dis_idx * NrOfElements, valueCache[i] + j * NrOfElements, NrOfElements
          // * sizeof(double)); // STRIDE #Elts
...
bool CGeomecDianaRunnerBase::WriteElements() {
  ...
  if (UseLegacyCodeForMaterialProperties()) {
    ...
    for (int i = 0; i < Mesh().ElementSize(); i++) {
      ...
      int j;
      for (j = 0; j < nnod; j++)
        nodenrs[j] = (ftn_int_t)(elm.PointIndex(j) + 1);
      ...
      elementset_info.push_back(
          CElementSetInfo(property_idx[0], property_idx[1], MeshBase().Formation(elm), elm.Index() + 1));
  ...
  } else if (Mesh().ElementSize() >
             0) // just to be correct; we shouldn't be here without elements, but if it were to happen, we'd crash
  {
    std::vector<CDistributedElementSetInfo> distributed_info;
    ...
    for (int i = 0; i < Mesh().ElementSize(); i++) {
      ...
      distributed_info.push_back(
          CDistributedElementSetInfo(MeshBase().Formation(elm), elm.Index() + 1, &Material(elm), ElementGeometry(elm)));
...
bool CGeomecDianaRunnerBase::AllowLinearCalc() const { return Model().AllowLinearCalc(); }
...
void CGeomecDianaRunnerBase::WriteDepletionStageStepSizesAndTime(const CDepletionStage &stage) {
  ...
  if (BehavesLinearly(stage)) {
    ...
    ftn_double_t *dStepSize =
        (ftn_double_t *)DiMalloc(1 * sizeof(double), "CGeomecDianaRunnerBase::WriteDepletionStageStepSizesAndTime");
    dStepSize[0] = 1;
    ...
    ftn_double_t *dStepSize =
        (ftn_double_t *)DiMalloc(n * sizeof(double), "CGeomecDianaRunnerBase::WriteDepletionStageStepSizesAndTime");
    dStepSize[0] = x / n;
...
double CGeomecDianaRunnerBase::calculateBulkModulus(const CFFMaterial *cffMaterial) const {
  double youngModulusN = cffMaterial->ParameterValue(IDT_VALUETYPE_YOUNGMODULUS_NORM);
  double youngModulusT = cffMaterial->ParameterValue(IDT_VALUETYPE_YOUNGMODULUS_TRANS);
  double bulkModulus = 0;

  if ((youngModulusN != 0) && (youngModulusT != 0)) {
    double poissonRatioN = cffMaterial->ParameterValue(IDT_VALUETYPE_POISSONRATIO_NORM);
    double poissonRatioT = cffMaterial->ParameterValue(IDT_VALUETYPE_POISSONRATIO_TRANS);

    bulkModulus = (2 - (2 * poissonRatioT) - (2 * poissonRatioN)) / youngModulusT;
...
double CGeomecDianaRunnerBase::calculateBulkModulus(const geo::IElement &elm) const {
  const dia::IMaterial &material = Material(elm);
  const CFFMaterial *cffMaterial = dynamic_cast<const CFFMaterial *>(&material);
  double bulkModulus = 0;

  if (cffMaterial) {
    if (cffMaterial->IsParameter(IDT_VALUETYPE_BULKSTIFFNESS)) {
      bulkModulus = cffMaterial->ParameterValue(IDT_VALUETYPE_BULKSTIFFNESS);
...
bool CGeomecDianaRunnerBase::calculateVolumetricStrain(std::vector<double> &vcElementValues, const geo::IElement &elm,
                                                       const CDepletionStage &stage) const {
  const CFormationBase &formationBase = *MeshBase().Formation(elm);
  const CPressure &initialPressure = formationBase.Pressure(stage.InitialStage());
  ...
  if (!stage.IsMarkedAsInitial() && initialPressure.Component().Defined() &&
      initialFractureMatrixPressure.Component().Defined() && pressure.Component().Defined() &&
      fractureMatrixPressure.Component().Defined()) {
    ...
    if (bCalculate) {
      IValueDomainScalar::TValueVec initialPressureValues = initialPressure.Component().ScalarData().ValueElement(elm);
      ...
      IValueDomainScalar::TValueVec pressureValues = pressure.Component().ScalarData().ValueElement(elm);
      ...
      for (size_t s = 0; s < vcElementValues.size(); ++s) {
        if (initialFractureMatrixPressureValues[s].Valid() && initialPressureValues[s].Valid() &&
            fractureMatrixPressureValues[s].Valid() && pressureValues[s].Valid()) {
          vcElementValues[s] += ((fractureMatrixPressureValues[s].Value() - pressureValues[s].Value()) -
                                 (initialFractureMatrixPressureValues[s].Value() - initialPressureValues[s].Value())) /
                                bulkModulus;
        } else {
          valid = false;
          break;
...
void CGeomecDianaRunnerBase::_CreateVolumetricStrainLoad(const geo::IElement &elm, const CDepletionStage &stage,
                                                         dia::CLoadCase &lcase) {
  ...
  if (!pInterface) {
    ...
    if (strain.IsVolumetric() && strain.Component().Defined()) {
      ...
      if (vcValues.size() == elm.NrOfNodes()) {
        ...
        vcElementValues = recalculateElementConcentrations(bValid, bWrite, bEqual, vcValues, elm, stage);
...
void CGeomecDianaRunnerBase::_CreatePlanarStrainLoad(const geo::IElement &elm, const CDepletionStage &stage,
                                                     dia::CLoadCase &lcase) {
  const geo::IInterfaceElement *pInterface = dynamic_cast<const geo::IInterfaceElement *>(&elm);
  if (!pInterface) {
    ...
    if (vcTensors.size() == elm.NrOfNodes()) {
      ...
      bool bWrite = fabs(vcTensors[0].XX()) > 1e-15 || fabs(vcTensors[0].YY()) > 1e-15 ||
                    fabs(vcTensors[0].ZZ()) > 1e-15 || fabs(vcTensors[0].XY()) > 1e-15 ||
                    fabs(vcTensors[0].YZ()) > 1e-15 || fabs(vcTensors[0].XZ()) > 1e-15;
      ...
      for (size_t i = 1; i < vcTensors.size(); ++i) {
        ...
        bWrite = bWrite || fabs(vcTensors[i].XX()) > 1e-15 || fabs(vcTensors[i].YY()) > 1e-15 ||
                 fabs(vcTensors[i].ZZ()) > 1e-15 || fabs(vcTensors[i].XY()) > 1e-15 ||
                 fabs(vcTensors[i].YZ()) > 1e-15 || fabs(vcTensors[i].XZ()) > 1e-15;
...
void CGeomecDianaRunnerBase::CreateStrainLoad(const geo::IElement &elm, const CDepletionStage &stage,
                                              dia::CLoadCase &lcase) {
...
const CStrainLoad &CGeomecDianaRunnerBase::StrainLoad(const geo::IElement &element,
                                                      const CDepletionStage &stage) const {
...
const dia::IElementGeometry *CGeomecDianaRunnerBase::ElementGeometry(const geo::IElement &element) const {
  ...
  if (pffmat) {
    ...
    if (pMat) {
      ...
      if (pMat->MaterialModel() == MM_UPSCALED_ANISOTROPY || pMat->MaterialModel() == MM_ANISOTROPIC_CAMCLAY ||
          pMat->MaterialModel() == MM_FRACTURE_APERTURE || pMat->MaterialModel() == MM_FRACTURE_APERTURE2) {
...
bool CGeomecDianaRunnerBase::CreateTopLoad3D(const CHorizonBase &top, dia::CLoadCase &lcase) {
...
bool CGeomecDianaRunnerBase::IsZoomInModel() const { return (Model().parentModel() != 0); }

bool CGeomecDianaRunnerBase::WriteZoomInInitialStresses(const geo::IElement &element, dia::CLoadCase &lcase,
  ...
  if (pBound) {
    ...
    if (pIniStresses) {
      ...
      for (iComp = 0; iComp < 6 && bAllValid; ++iComp) {
        ...
        for (iNode = 0; iNode < element.NrOfNodes(); ++iNode) {
          ...
          if (val.Valid()) {
            ...
            if (iComp < 3) // normal
              components[6 * iNode + iComp] =
                  -(val.Value() - vcInitialPressures[iNode].Value()) * 1e6; // total -> effective && MPa -> Pa
            else                                                            // shear
      ...
      if (bSomeValid && !bAllValid) {
        _m()->error("Not all initial stress tensors on the boundary are valid. Please check your input.",
                    MB_ICONEXCLAMATION);
        return false;
      }
      ...
      for (iNode = 0; iNode < element.NrOfNodes(); ++iNode) {
        ...
        if (bWriteThem && iNode) {
          if ((components[6 * iNode].Valid() &&
               fabs(components[6 * iNode].Value() - vcTensors[iNode - 1].XX()) > MIN_PRESTR_LOAD_VAL) ||
              (components[6 * iNode + 1].Valid() &&
               fabs(components[6 * iNode + 1].Value() - vcTensors[iNode - 1].YY()) > MIN_PRESTR_LOAD_VAL) ||
              (components[6 * iNode + 2].Valid() &&
               fabs(components[6 * iNode + 2].Value() - vcTensors[iNode - 1].ZZ()) > MIN_PRESTR_LOAD_VAL) ||
              (components[6 * iNode + 3].Valid() &&
               fabs(components[6 * iNode + 3].Value() - vcTensors[iNode - 1].XY()) > MIN_PRESTR_LOAD_VAL) ||
              (components[6 * iNode + 4].Valid() &&
               fabs(components[6 * iNode + 4].Value() - vcTensors[iNode - 1].YZ()) > MIN_PRESTR_LOAD_VAL) ||
              (components[6 * iNode + 5].Valid() &&
               fabs(components[6 * iNode + 5].Value() - vcTensors[iNode - 1].XZ()) > MIN_PRESTR_LOAD_VAL)) {
            bWriteMultiple = true;
          }
        }

        if (bWriteThem) {
          vcTensors.push_back(CStressTensor(components[6 * iNode].Value(), components[6 * iNode + 1].Value(),
                                            components[6 * iNode + 2].Value(), components[6 * iNode + 3].Value(),
                                            components[6 * iNode + 4].Value(), components[6 * iNode + 5].Value()));
        } else {
          break;
        }
      }
      ...
      CStressTensorValueSet stress = Model().parentModel()->ResultRegister().EffectiveStress(
          parentStage,
          Model().ParentLinearResults(Controller().AnalysisType()) ? CAnalysisType::AT_LINEAR
      ...
      for (int i = 0; i < element.NrOfNodes(); ++i) {
        ...
        if (vcTensors[i].Empty()) {
          _m()->msg(
              QObject::tr("Node %1 (%2,%3,%4) seems to reside outside the parent model, analysis cannot be performed")
                  .arg(node.Index() + 1)
                  .arg(node.X())
                  .arg(node.Y())
                  .arg(node.Z()));
      ...
      for (size_t i = 0; i < vcTensors.size(); ++i) {
        vcTensors[i] *= -1e6;
        if (fabs(vcTensors[i].XX()) > MIN_PRESTR_LOAD_VAL || fabs(vcTensors[i].YY()) > MIN_PRESTR_LOAD_VAL ||
            fabs(vcTensors[i].ZZ()) > MIN_PRESTR_LOAD_VAL || fabs(vcTensors[i].XY()) > MIN_PRESTR_LOAD_VAL ||
            fabs(vcTensors[i].YZ()) > MIN_PRESTR_LOAD_VAL || fabs(vcTensors[i].XZ()) > MIN_PRESTR_LOAD_VAL) {
          bWriteThem = true;
        }

        if (i && (fabs(vcTensors[i].XX() - vcTensors[i - 1].XX()) > MIN_PRESTR_LOAD_VAL ||
...
void CGeomecDianaRunnerBase::PerformInitialStressCalculation(bool b) { m_bPerformInitialStressCalculation = b; }

bool CGeomecDianaRunnerBase::PerformInitialStressCalculation() const { return m_bPerformInitialStressCalculation; }
...
bool CGeomecDianaRunnerBase::ExportCommandFileWithDefaults() const { return Model().ExportCommandFilesWithDefaults(); }

geo::CVector CGeomecDianaRunnerBase::GetDisplacementVector(const CVectorValueSet &displa, const geo::IElement &elm,
...
void CGeomecDianaRunnerBase::WriteConvergenceCommands() {
  ...
  if (Model().ConvergenceChecks()) {
    ...
    if (CheckConvergenceMinimumVolume())
      ...
      val = 1E-5; // in zoom-in we do have small elements, and Diana will list all smaller than the volume given
                  // (typically 1m^3), even though it will ignore the check -- we give it a smaller volume to get rid of
                  // (most of) those messages
    PutItem("MINVOL", &val);
    SetActive("LINELM",
              CheckConvergenceMinimumVolume() && Model().ConvergenceMinimumVolumeCheck() ? FTN_TRUE : FTN_FALSE);
    PopDir();
...
QString CGeomecDianaRunnerBase::DianaExecutable() {
  return CEnvironment::instance()->get(m_controller.WriteInputFiles() ? CEnvironment::InputExeName
...
void CGeomecDianaRunnerBase::AnalysisSuccess(bool bAnalysisSuccess) { m_bAnalysisSuccess = bAnalysisSuccess; }

bool CGeomecDianaRunnerBase::LoadPartialResults() { return Model().ResultRegister().ReadFilosResults(FF().c_str()); }

void CGeomecDianaRunnerBase::OnError(eError id, const std::string &msg2) {
  if (id == Running || id == RunAnalysis) {
    QString msg = getStringTableEntry(id == Running ? IDP_ERRORRUNNINGANALYSIS : // error starting kernel
...
bool CGeomecDianaRunnerBase::OnEndAnalysis(const QString &getPathName, ISaveModel &saveModel) {
  ...
  if (!m_controller.WriteInputFiles() && !m_controller.WriteQuadDat()) {
    Model().ResultRegister().ReadFilosResults(m_controller.StartStage(), m_controller.EndStage(),
                                              m_controller.AnalysisType());

    bRet = !m_AllowCleanUpAfterRun || m_controller.OnWriteBranchFiles(getPathName, m_bAnalysisSuccess, saveModel);
  }
...
void CGeomecDianaRunnerBase::WriteTableCommands() {
  ...
  if (stage.Initial()) {
    ...
    const TFormationBaseEntry &formation_entry =
        static_cast<const TFormationBaseEntry &>(*Model().GraphEntry(MD_BASE_FORMATION));
    ...
    for (TFormationBaseEntry::TNodeSet::const_iterator it = stFormations.begin(); it != stFormations.end(); ++it) {
      ...
      if (!stage.Previous().Initial() || formation.Strain(stage).DistributedSize() > 0 ||
    ...
    const THorizonBaseEntry &horizon_entry =
        static_cast<const THorizonBaseEntry &>(*Model().GraphEntry(MD_BASE_HORIZON));
    ...
    for (THorizonBaseEntry::TNodeSet::const_iterator it = stHorizons.begin(); it != stHorizons.end(); ++it) {
      ...
      if (horizon.Slip()) {
        ...
        if (!fpn.CohesionSameAsPrevious() || !fpn.FrictionSameAsPrevious() || !fpn.DStiffSameAsPrevious()) {
          bFaultMaterial = true;
        }
      }
    }
    ...
    if (bFormationPressure || bFormationTemperature || bFormationVolStrain || bFormationMaterial || bFaultPressure ||
        bFaultMaterial) {
...
bool CGeomecDianaRunnerBase::OnEndPostprocessWriteInputFiles(const std::string & /*sComfileName*/,
                                                             const std::string &sDatfileName) {
  ...
  if (pTetMesh) {
    ...
    if (fpin) {
      char *tmpdatfile =
          vDiStrsave(GetGeomecTempPathExt(CTempPath::TEMP_CALCULATION).toStdString().c_str(), "\\geomecdatXXXXXX", 0);
      mktemp(tmpdatfile);
...
bool CGeomecDianaRunnerBase::OnWriteElementOrder(bool forceQuadratic) {
  if (forceQuadratic) {
    ...
    const TFormationBaseEntry *pFormationEntry =
        static_cast<const TFormationBaseEntry *>(pModel->GraphEntry(MD_BASE_FORMATION));
    ...
    if (dynamic_cast<const CHexaModel *>(pModel)) {
      const CHexaMeshRegionEntry *pEntry =
          dynamic_cast<const CHexaMeshRegionEntry *>(pModel->GraphEntry(MD_HEXA_MESH_ZONE));
      if (pEntry) {
    ...
    if (stNodes.size() != vcGlobal.size()) // we may have conversions
    {
      ...
      if (stNodes.size() == vcLinear.size())
        ...
        if (stNodes.size() != vcQuadra.size()) {
          _m()->error("Hexamodel with submeshregions does not support quadratic elements per formation; please adjust "
                      "your input");
          return false;
        }
      }
    ...
    if (vcQuadra.size() > 0 && vcQuadra.size() < stNodes.size()) {
      ...
      ftn_char_t *elmsetptr =
          (ftn_char_t *)DiMalloc(l.size() * sizeof(ftn_char_t), "IDianaRunner::OnWriteElementOrder");
      ftn_char_t elmsetval =
          (ftn_char_t)DiMalloc(l.size() * 25 * sizeof(ftn_char_t), "IDianaRunner::OnWriteElementOrder");
...
dia::ITemperatureTable *CGeomecDianaRunnerBase::OnCreateTemperatureTable() const {
  ...
  /*
    ...
    int i;
    CDepletionStageEntry::iterator it;
    for(i = 0, it = Model().DepletionStageEntry().begin(); i < pTable->Size() && it !=
    Model().DepletionStageEntry().end(); ++it, ++i) pTable->RegisterTemperaturePoint(pTable->Time(i), *it);
...
void CGeomecDianaRunnerBase::OnExtendTemperatureTable(dia::ITemperatureTable * /*pTable*/) const {
  /*
    ...
    CDepletionStageEntry::iterator it;
    for(i = 0, it = Model().DepletionStageEntry().begin(); i < pTable->Size() && it !=
    Model().DepletionStageEntry().end(); ++it, ++i) if (i >= oldSize)
...
dia::IConcentrationTable *CGeomecDianaRunnerBase::OnCreateConcentrationTable() const {
  ...
  int i;
  CDepletionStageEntry::iterator it;
  for (i = 0, it = Model().DepletionStageEntry().begin();
       i < pTable->Size() && it != Model().DepletionStageEntry().end(); ++it, ++i)
...
const dia::IMaterial &CGeomecDianaRunnerBase::Material(const geo::IElement &element) const {
  ...
  if (pInterfaceMaterial)
    return *pInterfaceMaterial;
...
bool CGeomecDianaRunnerBase::CanCleanupAfterRun() const {
  bool res = m_AllowCleanUpAfterRun &&
             (&m_controller.EndStage() == &m_controller.EndStage().LastStage() || !m_bAnalysisSuccess);
...
void CGeomecDianaRunnerBase::AllowCleanUpAfterRun(bool allow) { m_AllowCleanUpAfterRun = allow; }

void CGeomecDianaRunnerBase::ForceCleanUp(const std::string &subdir) { Cleanup(subdir); }

// return an existing path if you want to use that instead of a generated one
std::string CGeomecDianaRunnerBase::UserDefinedLocation() const { return m_strLocation; }
...
bool CGeomecDianaRunnerBase::UserDefinedFilosFile(const QString &getPathName, std::string &strFilosFile,
                                                  bool &bInitFilos) const {
  ...
  if (!bInitFilos) {
    if (m_strFileName.empty()) {
      ...
      CFileCopier fcopier(strFilosPath.toStdString().c_str(),
                          (QString(FFDIR().c_str()) + QDir::separator() + strFilosFileName).toStdString().c_str(),
...
CMeshBase &CGeomecDianaRunnerBase::MeshBase() const { return m_MeshBase; }

CModelBase &CGeomecDianaRunnerBase::Model() const { return m_Model; }
...
bool CGeomecDianaRunnerBase::isAnisotropic(const geo::IElement &element) const {
  ...
  if (formation) {
    ...
    return library && library->IsParameter(IDT_VALUETYPE_YOUNGMODULUS_NORM) &&
...
CGeomecDianaRunnerBase::CTemperatureTable::CTemperatureTable(const CGeomecDianaRunnerBase &runner,
                                                             const CModelBase &model)
    : dia::ITemperatureTable(runner), m_runner(const_cast<CGeomecDianaRunnerBase *>(&runner)), m_model(model) {}

void CGeomecDianaRunnerBase::CTemperatureTable::RegisterTemperaturePoint(const dia::IValueTable::CTimePoint &tpoint,
                                                                         const CDepletionStage &dstage) {
...
void CGeomecDianaRunnerBase::CTemperatureTable::ValueAt(const geo::IElement &element,
                                                        const dia::IValueTable::CTimePoint &tpoint,
...
CGeomecDianaRunnerBase::CConcentrationTable::CConcentrationTable(const CGeomecDianaRunnerBase &runner,
                                                                 const CModelBase &model)
    : dia::IConcentrationTable(runner), m_runner(const_cast<CGeomecDianaRunnerBase *>(&runner)), m_model(model) {}

void CGeomecDianaRunnerBase::CConcentrationTable::RegisterConcentrationPoint(const dia::IValueTable::CTimePoint &tpoint,
                                                                             const CDepletionStage &dstage) {
  VERIFY(m_mpTimePoint2DeplStage.insert(TTimePoint2DeplStageMap::value_type(&tpoint, &dstage)).second);
}
...
void CGeomecDianaRunnerBase::CConcentrationTable::ValueAt(const geo::IElement &element,
                                                          const dia::IValueTable::CTimePoint &tpoint,
                                                          std::vector<double> &vcValues) const {
  assert(!dynamic_cast<const geo::CInterfaceElement *>(&element));
  // get the depletion stage
  ...
  if (dstage.Initial()) {
    ...
    std::vector<double> vcElementValues =
        m_runner->recalculateElementConcentrations(bValid, bWrite, bEqual, vcConcenValues, element, dstage);
...
CGeomecDianaRunnerBase::CUpscalingAnisotropyElementGeometry::CUpscalingAnisotropyElementGeometry(double dInclination,
                                                                                                 double dAzimuth)
    : m_dInclination(dInclination), m_dAzimuth(dAzimuth) {}
...
int CGeomecDianaRunnerBase::CUpscalingAnisotropyElementGeometry::Type() const { return 201; }

int CGeomecDianaRunnerBase::CUpscalingAnisotropyElementGeometry::WriteFilosParamSize(
    IDianaRunner & /*diarunner*/) const {
  return 6;
}

bool CGeomecDianaRunnerBase::CUpscalingAnisotropyElementGeometry::WriteFilosParamName(IDianaRunner & /*diarunner*/,
                                                                                      int i, char *name) const {
  if (i < 3) {
...
void CGeomecDianaRunnerBase::CUpscalingAnisotropyElementGeometry::WriteFilosParamValues(IDianaRunner & /*diarunner*/,
                                                                                        double *values,
                                                                                        int stride) const {
  double azi = m_dAzimuth * PI / 180;
  double inc = m_dInclination * PI / 180;
...
CGeomecDianaRunnerBase::CFractureApertureElementGeometry::CFractureApertureElementGeometry(
    const geo::IVector &vecHighDensity, const geo::IVector &vecLowDensity)
    : m_vecHighDensity(vecHighDensity), m_vecLowDensity(vecLowDensity) {}
...
int CGeomecDianaRunnerBase::CFractureApertureElementGeometry::Type() const { return 200; }

int CGeomecDianaRunnerBase::CFractureApertureElementGeometry::WriteFilosParamSize(IDianaRunner & /*diarunner*/) const {
  return 6;
}

bool CGeomecDianaRunnerBase::CFractureApertureElementGeometry::WriteFilosParamName(IDianaRunner & /*diarunner*/, int i,
                                                                                   char *name) const {
  if (i < 3) {
    QString xaxis = QString("XAXIS(%1)").arg(i + 1);
    strncpy(name, xaxis.toStdString().c_str(), 10);
...
void CGeomecDianaRunnerBase::CFractureApertureElementGeometry::WriteFilosParamValues(IDianaRunner & /*diarunner*/,
...
/*virtual*/ const geo::CPtrArray<geo::IFace>
CGeomecDianaRunnerBase::CComboSurface::FacesAt(const geo::IPoint & /*p*/) const {
  assert(false);
...
/*virtual*/ const geo::CPtrArray<geo::IFace>
CGeomecDianaRunnerBase::CComboSurface::FacesAtNode(const geo::IPoint & /*p*/) const {
  assert(false);
  geo::CPtrArray<geo::IFace> ret;
  return ret;
}
...
/*virtual*/ int CGeomecDianaRunnerBase::CComboSurface::FaceSize() const { return m_vcFaces.size(); }
...
int CGeomecDianaRunnerBase::CComboSurface::PointSize() const { return m_vcPoints.size(); }
...
/*virtual*/ int CGeomecDianaRunnerBase::CComboSurface::ElementSize() const { return m_vcFaces.size(); }
