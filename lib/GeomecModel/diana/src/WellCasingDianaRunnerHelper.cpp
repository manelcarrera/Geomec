...
CWellCasingDianaRunnerHelper::CWellCasingDianaRunnerHelper(const IWellModel &model,
                                                           const CDianaRunController &controller,
                                                           bool bParentLinearResults)
    : m_model(model), m_controller(controller), m_bParentLinearResults(bParentLinearResults),
...
const dia::IMaterial *CWellCasingDianaRunnerHelper::InterfaceMaterial(const geo::IElement &element) const {
  ...
  if (pIface) {
    if (CasingModel().Mesh().IsOuterInterfaceElement(*pIface)) {
      ...
      const CWellCasingModel::CBoundary &boundary =
          static_cast<const CWellCasingModel::CBoundary &>(CasingModel().Boundary());
      ...
      if (Controller().AnalysisType() == CAnalysisType::AT_HEAT) {
        ...
        if (CasingModel().Mesh().SteelElements().ElementSize() > 0) {
          ...
          double dConduc = CasingModel()
                               .CasingNode()
                               .Steel()
                               .Material(Controller().StartStage())
                               .Material(body)
                               .ParameterValue(IDT_VALUETYPE_THERM_CONDUCT);
...
const CDianaRunController &CWellCasingDianaRunnerHelper::Controller() const { return m_controller; }
    ...
    for (it = mpStages.begin(); it != mpStages.end(); ++it) {
      ...
      for (n = 0; n < bodyface.NrOfNodes(); ++n) {
        ...
        if (pVTP) {
          ...
          if (bAllValid) {
            CTensor t(vals[0].Value(), vals[1].Value(), vals[2].Value(), vals[3].Value(), vals[4].Value(),
                      vals[5].Value());
        ...
        if (bNotProcessed) {
          ...
          if (pVTD) {
            ...
            if (bAllValid) {
              geo::CVector v(vals[0].Value(), vals[1].Value(), vals[2].Value());
...
bool CWellCasingDianaRunnerHelper::WriteBoundaryLoadsFromParentModel(const TStageMap &mpStages) {
  ...
  for (int i = 0; i < nBound; ++i) {
    ...
    if (!bNodeOutsideParentMesh) {
      ...
      CStressTensorValueSet initialStress = parentModel.ResultRegister().TotalStress(
          parentInitialStage, m_bParentLinearResults ? CAnalysisType::AT_LINEAR : CAnalysisType::AT_NONLIN, false);
      ...
      for (it = mpStages.begin(); it != mpStages.end(); ++it) {
        ...
        CStressTensorValueSet stress = parentModel.ResultRegister().TotalStress(
            parentStage, m_bParentLinearResults ? CAnalysisType::AT_LINEAR : CAnalysisType::AT_NONLIN, false);
...
geo::CVector CWellCasingDianaRunnerHelper::GetDisplacementVector(const CVectorValueSet &displa,
