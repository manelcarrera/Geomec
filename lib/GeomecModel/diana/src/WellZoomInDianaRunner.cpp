...
bool CWellZoomInDianaRunner::FetchInitialPressures(const geo::IElement &element, const CDepletionStage &stage,
...
bool CWellZoomInDianaRunner::CreateInitialLoads(const geo::IElement &element, const CDepletionStage &stage,
                                                dia::CLoadCase &lcase,
...
bool CWellZoomInDianaRunner::CreateStageLoads(const geo::IElement &element, const CDepletionStage &stage,
                                              dia::CLoadCase &lcase,
                                              const IValueDomainScalar::TValueVec &vcInitialPressures,
                                              const IValueDomainScalar::TValueVec &vcInitialTemperatures) {
  const geo::CInterfaceElement *pIface = dynamic_cast<const geo::CInterfaceElement *>(&element);
...
CWellZoomInModel &CWellZoomInDianaRunner::ZoomInModel() { return static_cast<CWellZoomInModel &>(Model()); }
  ...
  for (i = 0; i < n; ++i) {
    ...
    if (!vcElementIndices.empty()) {
      ...
      CStressTensorValueSet initialStress = parentModel.ResultRegister().TotalStress(
          parentInitialStage, ParentLinearResults() ? CAnalysisType::AT_LINEAR : CAnalysisType::AT_NONLIN, false);
      ...
      for (it = mpStages.begin(); it != mpStages.end(); ++it) {
        ...
        {
          stresstensor =
              stresstensor - initialStress.ValuePoint(ptSample1, geo::IParallelInitializationCallback::Sequential);
        }
        ...
        // create the load
        new dia::CBodyFaceLoad(lcase, vecPressure.Length(), vecPressure.UnitVector(), *bodyface.Parent(),
                               bodyface.Index());
