...
CGeomecHeatFlowDianaRunner::CGeomecHeatFlowDianaRunner(CMeshBase &meshbase, CModelBase &model,
                                                       CDianaRunController &controller)
    : CGeomecDianaRunnerBase(meshbase, model, controller) {}

bool CGeomecHeatFlowDianaRunner::AllowLinearCalc() const { return false; }
...
int CGeomecHeatFlowDianaRunner::CommandsProgressSize() const { return Model().NrOfDepletionStages(); }
...
void CGeomecHeatFlowDianaRunner::CreateNodalTemperatures(const TStageMap &mpStages,
...
geo::CValue CGeomecHeatFlowDianaRunner::GetAverageTemperatureInNode(int iNode, const std::set<int> &stElements,
  ...
  for (it = stElements.begin(); it != stElements.end(); ++it) {
    ...
    if (pFormation) {
      IValueDomainScalar::TValueVec vcTemp =
          Model().Mesh().Formation(elm)->UserTemperature(stage).Component().ScalarData().ValueElement(elm);
      int localidx = -1;
      int j;
