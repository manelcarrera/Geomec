...
namespace
{
	...
	//
	const char* g_run_res_cp[] = { "Success", "RunError", "EndDepletionError", "Cancelled", "StartCheckError", "GuardCheckError", "RunFirstError", "RunNextError", "UndefinedError", "EndStageError", "StartRunnerError" };
...
namespace controller
{
	namespace print
	{
		void depletions( const CModelBase& model )
		{
			...
			while( d )
			{
				p->info( QString::asprintf( "%d: %s (%s)", 
					i++, 
					d->Name().toStdString().c_str(),
					CDepletionStage::m_output_type_cp[ d->OutputType() ] ).toStdString().c_str() );
		...
		void depletion( const CDepletionStage* d, int i )
		{
			...
			p->info( QString::asprintf( "--stage-- %d: %s (%s)", 
				i++, 
				d->Name().toStdString().c_str(), 
				CDepletionStage::m_output_type_cp[ d->OutputType() ] ).toStdString().c_str() );
		}
...
CDianaRunController::CDianaRunController( const controller::Params& p ) 
	: CDianaRunController( p.model, p.antype, p.bWriteInputFiles, p.bWriteQuadDat, p.bWriteOutputStreamFile, p.strTempPath )
{}
...
: 
  IListener( CEvents::Controller_EH, { CancelRun, RunSuccess, RunEnd, RunController } ),
  m_model(model),
  m_antype(antype),
  m_bWriteInputFiles(bWriteInputFiles),
  m_bWriteQuadDat(bWriteQuadDat),
  m_bWriteOutputStreamFile(bWriteOutputStreamFile),
  m_pStartStage(0),
  m_pEndStage(0),
...
bool CDianaRunController::run_step(eRunStep step)
{
    ...
    // JH: because they are owned by the m_pRunner and would be deleted as well. This way we can just copy the pointers to the new instance,
    // instead of copying or recreating the entire data structures.
...
{
 	m_printer->info("run stage ... end stage: %s", m_antype == CAnalysisType::AT_NONLIN ? "next branch stage" : "last stage");
	...
	m_pEndStage =
		m_antype == CAnalysisType::AT_NONLIN ?
			&m_pStartStage->GetNextBranchStage(*m_pStartStage) :
	...
	m_pRunner->progress(
		p.bSilent ?
			_g->prog()->create(eProgress::Silent) :
...
CDianaRunController::eRunRes CDianaRunController::run_all()
{
	...
	{
		QString msg = QString("Please define at least one depletion stage after the branch stage '%1'").arg(m_model.BranchState().BranchStage().Name());
		_m()->error(msg);
	...
	m_pStartStage =
		(m_model.BranchState().IsBranch() && m_antype == CAnalysisType::AT_NONLIN) ?
...
void CDianaRunController::SetLicenseRetry(int nLicenseRetry)
{
  m_nLicenseRetry = nLicenseRetry;
}

QString CDianaRunController::LicenseError() const
{
  return m_sLicenseError;
}

bool CDianaRunController::HaveResults() const
{
  return m_bHaveResults;
}

bool CDianaRunController::GetCalculationResult() const
{
  return m_bCalculationResult;
}

void CDianaRunController::CleanUpAfterRun(
...
CModelBase& CDianaRunController::Model()
{
	return m_model;
}

const CModelBase& CDianaRunController::Model() const
{
	return m_model;
}

const CAnalysisType& CDianaRunController::AnalysisType() const
{
	return m_antype;
}

bool CDianaRunController::WriteInputFiles() const
{
	return m_bWriteInputFiles;
}

bool CDianaRunController::WriteQuadDat() const
{
	return m_bWriteQuadDat;
}
...
bool CDianaRunController::CleanupOldResults(int iStartStage) const
{
  ...
  for(it = m_model.DepletionStageEntry().begin(), iStage = 0; it != m_model.DepletionStageEntry().end() && iStage < iStartStage; ++it, ++iStage)
  {
...
bool CDianaRunController::OnWriteBranchFiles(
	const QString& getPathName,
	bool bCalcResult, 
	ISaveModel& saveModel) const
{
  ...
  // removing the next superfluous assert: we simply save as Untitled_resultsX or require the user to save first in case of branches
  ...
  strDocBase = CreateBranchFileBaseName(strDocBase, EndStage().Index());
    ...
    TTimeInterval timespan = calculateTimeInterval(timeCloseStart, timeCloseEnd);
	  QString strTime = QString("*** Close analysis database took %1 days, %2 hours, %3 minutes, %4 seconds ***").arg(timespan.days).arg(timespan.hours).arg(timespan.minutes).arg(timespan.seconds);
    logger.AddLine(strTime.toStdString(), true);
	  ...
	  strTime = QString("*** Copy branch analysis database took %1 days, %2 hours, %3 minutes, %4 seconds ***").arg(timespan.days).arg(timespan.hours).arg(timespan.minutes).arg(timespan.seconds);
    logger.AddLine(strTime.toStdString(), true);
...
#define DOCOMPRESS 0
    ...
    timespan = calculateTimeInterval(timeOpenStart, timeOpenEnd);
	  strTime = QString("*** Reopen analysis database took %1 days, %2 hours, %3 minutes, %4 seconds ***").arg(timespan.days).arg(timespan.hours).arg(timespan.minutes).arg(timespan.seconds);
    logger.AddLine(strTime.toStdString(), true);
    ...
    if(bRet)
      ...
      timespan = calculateTimeInterval(timeSaveStart, timeSaveEnd);
	    strTime = QString("*** Save branch model file took %1 days, %2 hours, %3 minutes, %4 seconds ***").arg(timespan.days).arg(timespan.hours).arg(timespan.minutes).arg(timespan.seconds);
      logger.AddLine(strTime.toStdString(), true);
...
bool CDianaRunController::OnQueryInputFileNames(const std::string& title,
...
void CDianaRunController::RemoveRedundantLoads() const
{
  if(!m_pStartStage->Initial())
  {
    int iStage;
    CDepletionStageEntry::const_iterator it;
    for(iStage = 0, it = m_model.DepletionStageEntry().begin(); it != m_model.DepletionStageEntry().end() && &(*it) != m_pStartStage; ++it, ++iStage)
    {
...
{ 
	m_params = p; 
	m_printer->info("--params-- path:%s, title:%s, tmp:%s, silent:%s dsa:%s", p.path.toStdString().c_str(), p.title.c_str(), p.rootdir.c_str(), p.bSilent?"yes":"no", p.dsa?"yes":"no");
};
