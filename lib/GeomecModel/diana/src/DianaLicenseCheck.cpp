...
CDianaLicenseCheck::CDianaLicenseCheck()
    : m_active(false), m_modelName("LicenseCheck.gm4"), m_lastCheck(tbb::tick_count::now()), m_lastResult(false) {
...
void CDianaLicenseCheck::Activate() { m_active = true; }
...
bool CDianaLicenseCheck::FoundLicense() {
  ...
  if (m_pModel) {
    if (!m_lastResult || (tbb::tick_count::now() - m_lastCheck).seconds() > 0) {
      ...
      CDianaRunController drc(*m_pModel, CAnalysisType::AT_LINEAR, false, false, false,
                              GetGeomecTempPathExt(CTempPath::TEMP_CALCULATION));

      dia::IDianaRunner::RunParams p = {
          m_modelName, "License Check", drc.tmp().toStdString(), &diaexec, &smc, &retr, nullptr, true, _g->dsa()};
      drc.params(p);
