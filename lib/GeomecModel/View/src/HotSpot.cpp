...
CHotSpot::CHotSpot(unsigned int uName, CFemAppModel &model, TColor min_color, TColor max_color)
    : IColorScaleNode(uName, model), m_MinMaxValue(0, 0), m_nMaxColor(max_color), m_nMinColor(min_color),
...
CHotSpot::CHotSpot(CFemAppModel &model)
    : IColorScaleNode(model), m_MinMaxValue(0, 0), m_nMaxColor(qRgb(255, 0, 0)), m_nMinColor(qRgb(0, 0, 255)),
      m_bCenterIsTransparent(true) {}

CHotSpot::CHotSpot(const CHotSpot &rhs)
    : IColorScaleNode(rhs), m_MinMaxValue(rhs.m_MinMaxValue), m_nMaxColor(rhs.MaxColor()), m_nMinColor(rhs.MinColor()),
...
bool CHotSpot::operator==(const CHotSpot &rhs) const {
  ...
  return (m_bCenterIsTransparent == rhs.CenterIsTransparent() && m_MinMaxValue == rhs.m_MinMaxValue &&
          m_nMaxColor == rhs.MaxColor() && m_nMinColor == rhs.MinColor());
}
  ...
  if (value.Valid() && min_max.first.Valid() && min_max.second.Valid()) {
    if (m_bCenterIsTransparent) {
      ...
      if (value.Value() >= m_MinMaxValue.second)
        ...
        return qRgba(qRed(m_nMinColor), qGreen(m_nMinColor), qBlue(m_nMinColor),
                     0); // dVal less than the minimum value, return transparent
...
unsigned int CHotSpot::IconId() const { return IDI_COLOR_GRADIENT; }

unsigned int CHotSpot::TypeId() const { return 0; }

bool CHotSpot::Empty() const { return !IsLinkedTo(*((CFemAppModel &)Model()).GraphEntry(MD_BASE_COLOR_SCALE)); }
...
long CHotSpot::SavedItems() const { return CStorageNode::SavedItems() + 4; }

const std::pair<double, double> &CHotSpot::MinMaxValue() const { return m_MinMaxValue; }
