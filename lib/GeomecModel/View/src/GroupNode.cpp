...
CGroupNode::CGroupNode(CFemAppModel &model) : COpenGLNode(model), m_entry(nullptr), m_readOnly(false) {
...
CGroupNode::CGroupNode(const QString &strName, CFemAppModel &model, CGraphEntry *entry)
    : COpenGLNode(strName, model), m_entry(entry), m_readOnly(false) {
  CGroupNodeEntry *groupEntry = static_cast<CGroupNodeEntry *>(model.GraphEntry(MD_BASE_GROUPNODE));
  reParent(groupEntry);
...
CGroupNode::CGroupNode(const CGroupNode &rhs)
    : COpenGLNode(rhs), m_entry(rhs.m_entry), m_readOnly(rhs.m_readOnly), m_description(rhs.m_description) {}
...
bool CGroupNode::operator==(const CGroupNode &rhs) const {
  return COpenGLNode::operator==(rhs) && m_entry == rhs.m_entry && m_readOnly == rhs.m_readOnly &&
         m_description == rhs.m_description;
}

const QString &CGroupNode::Description() const { return m_description; }

void CGroupNode::Description(const QString &description) { m_description = description; }

bool CGroupNode::ReadOnly() const { return m_readOnly; }

void CGroupNode::ReadOnly(bool readOnly) { m_readOnly = readOnly; }

bool CGroupNode::Empty() const { return true; }

long CGroupNode::SavedItems() const { return 1; }
...
void CGroupNode::LoadStream(TSTREAM &stream, CStreamVersion &version, TPROGRESS &progress) {
  ...
  if (m_entry) {
    ...
    if (groupSize > 0) {
      // technical debt: methods to find nodes by id are template-typed, and can't be used here; TODO: refactor storage
      // node entry
...
int CGroupNode::DisplayListSize() const { return 0; }
...
COpenGLNodeBase::TColor CGroupNode::Color() const { return TColor(); }

bool CGroupNode::CanDestroy() const { return !ReadOnly(); }
...
bool CGroupNode::CanConnectItem(const CGraphNode &item) const { return !ReadOnly() && item.IsLinkedTo(*m_entry); }

bool CGroupNode::CanDisconnectItem(const CGraphNode &item) const { return !ReadOnly() && item.IsLinkedTo(*this); }

void CGroupNode::Modified(enum ModifiedHint uHint) {
  COpenGLNode::Modified(uHint);
  if (m_entry)
    m_entry->Modified(uHint);
}

const CGraphEntry *CGroupNode::GraphEntry() const { return m_entry; }
...
CGroupNodeEntry::CGroupNodeEntry(C3DModel &model) : BASE_CLASS(MD_BASE_GROUPNODE, IDI_GROUPNODE, "GroupNodes", model) {}
