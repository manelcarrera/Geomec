...
: CStorageNode(rhs),
  ...
  m_bdVp(rhs.m_bdVp),
  m_bdt(rhs.m_bdt),
  m_bResultsChanged(rhs.m_bResultsChanged),
...
: CStorageNode(model),
  ...
  m_bImportBack(true),
  m_data(model),
  m_bForceNewPointSets(false),
  m_GVTVelocityModelFile(GVT::CGVTVelocityModelFile::velocityFromFile, m_Model)
{
...
CGVTSettings::~CGVTSettings(void)
{
  ClearMemory();
}
...
int CGVTSettings::NoOfGrids() const
{
  return m_SeismicGrids.size();
}
...
QString  CGVTSettings::ListObjectName() const
{
  return "Scenario ";
}
...
bool CGVTSettings::TimeLapseReady() const
{
  return m_vcTimeLapses.size() > 0;
}

bool CGVTSettings::VelocityModelFileReady() const
{
  return m_GVTVelocityModelFile.isVelocityModelFileReady();
}
...
int CGVTSettings::velocitySource() const
{
  return m_GVTVelocityModelFile.velocityModelSourceNumerator();
}
