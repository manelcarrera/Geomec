...
bool CGVTVtFileCheck::RowsIsTracks() { return track_direction == 'H'; }

int CGVTVtFileCheck::NumRows() { return RowsIsTracks() ? num_tracks : num_bins; }

int CGVTVtFileCheck::FirstRow() { return RowsIsTracks() ? first_track : first_bin; }

int CGVTVtFileCheck::DeltaRow() { return RowsIsTracks() ? delta_track : delta_bin; }

int CGVTVtFileCheck::NumCols() { return RowsIsTracks() ? num_bins : num_tracks; }

int CGVTVtFileCheck::FirstCol() { return RowsIsTracks() ? first_bin : first_track; }

int CGVTVtFileCheck::DeltaCol() { return RowsIsTracks() ? delta_bin : delta_track; }

double CGVTVtFileCheck::CornerNorthing() { return geo_y; }

double CGVTVtFileCheck::DeltaNorthing() { return delta_y; }

double CGVTVtFileCheck::CornerEasting() { return geo_x; }

double CGVTVtFileCheck::DeltaEasting() { return delta_x; }
...
CGVTVtFile::CGVTVtFile(CModelBase &model) : m_model(model), m_pPointSet(0), m_emptyTraces(0) {}

CGVTVtFile::~CGVTVtFile() { delete m_pPointSet; }

CGVTVtFileHeader &CGVTVtFile::getHeaderInfo() { return m_header; }

CGVTVtFileCheck &CGVTVtFile::getCheckInfo() { return m_check; }
...
bool CGVTVtFile::Import(const QString &fileName, IProgressBase &prog, bool bNoData) {
  ...
  if (m_ifs.is_open()) {
    ...
    m_pPointSet->NodalValueSet(3).LinkTo(
        factory->BuildValueType(*m_pPointSet, IDT_VALUETYPE_DELTAT, "Timeshifts")->Component());
  ...
  if (m_emptyTraces || !m_incompleteTraces.empty()) {
    ...
    if (!m_incompleteTraces.empty()) {
      msg += del + QString("%1 incomplete trace(s) (extending past the end of the file) at address")
                       .arg(m_incompleteTraces.size());
...
bool CGVTVtFile::ReadArraySize(int section, unsigned int expect) {
  ...
  if (m_uiArraySize != expect) {
    _m()->msg(QString("Error while reading section %1: expected size %2, found %3")
                  .arg(section)
                  .arg(expect)
                  .arg(m_uiArraySize));
    return false;
...
bool CGVTVtFile::ReadSection1() {
  ...
  if (m_uiFileSize * VT_WORD_SIZE != m_uiRealFileSize) {
    _m()->msg(QString("Warning while reading section 1: real file size is %1, but in file it says %2 (* 4 = %3)")
                  .arg(m_uiRealFileSize)
                  .arg(m_uiFileSize)
                  .arg(m_uiFileSize * VT_WORD_SIZE));
  }
...
bool CGVTVtFile::ReadSection3() {
  ...
  if (m_header.bits_per_sample != 8 && m_header.bits_per_sample != 16 && m_header.bits_per_sample != 32) {
    _m()->msg(QString("Error while reading section 3: bits per sample is %1, but only 8, 16, and 32 are supported")
                  .arg(m_header.bits_per_sample));
    return false;
  }
...
bool CGVTVtFile::ReadTrace(int address, geo::CPoint &point, double min_z, double delta_z,
                           double conversion) // address is in 4 byte int offsets (from beginning of file)
{
  ...
  if (size <= 0) // This shouldn't happen, but I know these people!
  {
    ...
    size_t maxPos = m_uiFileSize * VT_WORD_SIZE -
                    m_header.bits_per_sample / 8; // apparently we have 'empty' traces that aren't completely written
