...
template <class VECTOR> void SetupVectors(const CPillarMap &pillarMap, VECTOR &vector) {
...
public:
  CTaskGatherInput(std::vector<const CPillar *> &pillars, CGVTDataTypes::TStageMap &input)
      : m_pillars(pillars), m_input(input), m_dispatcher(0), m_bSequential(false) {}
  ...
  void operator()(const tbb::blocked_range<size_t> &r) const {
    ...
    for (CGVTDataTypes::TStageMap::const_iterator stageIt = m_input.begin(); stageIt != m_input.end(); ++stageIt) {
      for (CGVTDataTypes::TValueMap::const_iterator valuesIt = stageIt->second.begin();
           valuesIt != stageIt->second.end(); ++valuesIt) {
        ...
        for (size_t i = r.begin(); i != r.end(); ++i) {
          ...
          CGVTDataTypes::TPillarPointValueVector &values =
              const_cast<CGVTDataTypes::TPillarPointValueVector &>(valuesIt->second.second[i]);
...
void CGVTDataInput::Build(IProgressBase &progress, const CModelBase &model, const CPillarMap &pillarMap,
                          const CGVTDataTypes::TFormationVelocityModels &formVelocityModels,
                          const CGVTDataTypes::TTimeLapses &timelapses, bool bRebuild, bool bReinterpolate,
                          CPointSet *input, bool &requireNewInput) {
    ...
    if (input) {
      ...
      if (bInitDone) // let's first check that we only have the properties we need
      {
        for (size_t v = 3; v < input->NodalValueSetSize(); ++v) {
          ...
          CGVTDataTypes::TPillarValueVector *pvv =
              CGVTDataTypes::FindValueVector(m_stageMap, input->NodalValueSet(v).Component()->Parent().Name());
      ...
      if (bInitDone) // and that we have all the properties (technically, we may not need to, as time-lapse set could be
                     // subset of input, but we don't check for that yet
      {
...
void CGVTDataInput::Setup(const CModelBase &model, const CPillarMap &pillarMap,
                          const CGVTDataTypes::TFormationVelocityModels &formVelocityModels) {
  ...
  while (pStage) {
    ...
    if (stageIt != m_stageMap.end()) {
      ...
      if (model.ResultTree().EffectiveStress().InvariantComposite()->Available(anType, *pStage)) {
        std::pair<CGVTDataTypes::TValueMap::iterator, bool> retval = valueMap.insert(
            std::make_pair(CGVTDataTypes::MeanStress,
                           std::make_pair(model.ResultTree().EffectiveStress().InvariantComposite()->ResultComponent(
                                              *pStage, anType, 0, IStressTensorGroup::CInvariantComposite::INV_I1),
                                          CGVTDataTypes::TPillarValueVector())));
        SetupVectors(pillarMap, retval.first->second.second);
        // m_TypeVec.push_back(IDT_VALUETYPE_PRESSURE);
        // m_DataVec.push_back(Model().ResultTree().EffectiveStress().InvariantComposite()->ResultComponent(*pStage,
        // anType, 0, IStressTensorGroup::CInvariantComposite::INV_I1));
      }

      if (model.ResultTree().EffectiveStress().Components().Available(anType, *pStage)) {
        std::pair<CGVTDataTypes::TValueMap::iterator, bool> retval = valueMap.insert(std::make_pair(
            CGVTDataTypes::StressZZ, std::make_pair(model.ResultTree().EffectiveStress().Components().ResultComponent(
                                                        *pStage, anType, 0, ITensorGroup::CComponentComposite::TC_ZZ),
                                                    CGVTDataTypes::TPillarValueVector())));
        SetupVectors(pillarMap, retval.first->second.second);
        // m_TypeVec.push_back(IDT_VALUETYPE_PRESSURE);
        // m_DataVec.push_back(Model().ResultTree().EffectiveStress().Components().ResultComponent(*pStage, anType, 0,
        // ITensorGroup::CComponentComposite::TC_ZZ));
      }

      if (model.ResultTree().TotalStrain().Components().Available(anType, *pStage)) {
        std::pair<CGVTDataTypes::TValueMap::iterator, bool> retval = valueMap.insert(std::make_pair(
            CGVTDataTypes::StrainZZ, std::make_pair(model.ResultTree().TotalStrain().Components().ResultComponent(
                                                        *pStage, anType, 0, ITensorGroup::CComponentComposite::TC_ZZ),
                                                    CGVTDataTypes::TPillarValueVector())));
        SetupVectors(pillarMap, retval.first->second.second);
        // m_TypeVec.push_back(IDT_VALUETYPE_STRAIN);
        // m_DataVec.push_back(Model().ResultTree().TotalStrain().Components().ResultComponent(*pStage, anType, 0,
        // ITensorGroup::CComponentComposite::TC_ZZ));
      }

      {
        std::pair<CGVTDataTypes::TValueMap::iterator, bool> retval = valueMap.insert(std::make_pair(
            CGVTDataTypes::Velocity,
            std::make_pair(static_cast<const CMaterialResult *>(&model.MaterialResultTree().Child(VpIndex(model)))
                               ->ResultComponent(*pStage, anType),
                           CGVTDataTypes::TPillarValueVector())));
        SetupVectors(pillarMap, retval.first->second.second);
      }
      ...
      if (model.ResultTree().Displacement().Available(anType, *pStage)) {
        std::pair<CGVTDataTypes::TValueMap::iterator, bool> retval = valueMap.insert(std::make_pair(
            CGVTDataTypes::Displacement,
            std::make_pair(model.ResultTree().Displacement().ResultComponent(*pStage, anType, 0, IVectorResult::VC_Z),
                           CGVTDataTypes::TPillarValueVector())));
        SetupVectors(pillarMap, retval.first->second.second);
        // m_TypeVec.push_back(IDT_VALUETYPE_STRAIN);
        // m_DataVec.push_back(Model().ResultTree().TotalStrain().Components().ResultComponent(*pStage, anType, 0,
      ...
      // m_DataVec.push_back(static_cast<CMaterialResult
      // *>(&Model().MaterialResultTree().Child(63))->ResultComponent(*pStage, anType));
    }
...
void CGVTDataInput::FillPointSet(IProgressBase &progress, const CPillarMap &pillarMap, CPointSet *pointSet) {
  ...
  int i = 0;
  for (CGVTDataTypes::TStageMap::const_iterator stageIt = m_stageMap.begin(); stageIt != m_stageMap.end(); ++stageIt) {
    for (CGVTDataTypes::TValueMap::const_iterator valuesIt = stageIt->second.begin(); valuesIt != stageIt->second.end();
         ++valuesIt) {
      int type = -1;
      ...
      if (type >= 0)
        pointSet->NodalValueSet(3 + i++).LinkTo(
            factory->BuildValueType(*pointSet, type, valuesIt->second.first->ExportLabel())->Component());
    }
  }
  ...
  for (CPillarMap::const_iterator mapIt = pillarMap.begin(); mapIt != pillarMap.end(); ++mapIt, ++i) {
    ...
    for (CPillar::const_iterator pillarIt = mapIt->second.begin(); pillarIt != mapIt->second.end(); ++pillarIt, ++j) {
      ...
      int k = 0;
      for (CGVTDataTypes::TStageMap::const_iterator stageIt = m_stageMap.begin(); stageIt != m_stageMap.end();
           ++stageIt) {
        for (CGVTDataTypes::TValueMap::const_iterator valuesIt = stageIt->second.begin();
             valuesIt != stageIt->second.end(); ++valuesIt) {
...
int CGVTDataInput::CalculationSize() const { return m_stageMap.begin()->second.begin()->second.second.size(); }

CGVTDataTypes::TValueMap &CGVTDataInput::ValueMap(unsigned int key) { return m_stageMap[key]; }

const CGVTDataTypes::TPillarVelocityModelVector &CGVTDataInput::VelocityModels() const { return m_velocityVector; }

/////////////////////////////////////////////////////////////////////////////
// CGVTDataOutput
/////////////////////////////////////////////////////////////////////////////

void CGVTDataOutput::Build(const CPillarMap &pillarMap, const CGVTDataTypes::TTimeLapses &timelapses, bool deltaVp,
                           bool deltaT) {
  m_stageMap.clear();
...
void CGVTDataOutput::Setup(const CPillarMap &pillarMap, bool /*deltaVP*/, bool /*deltaT*/) {
  for (CGVTDataTypes::TStageMap::iterator stageIt = m_stageMap.begin(); stageIt != m_stageMap.end(); ++stageIt) {
    ...
    std::pair<CGVTDataTypes::TValueMap::iterator, bool> retval = valueMap.insert(std::make_pair(
        CGVTDataTypes::LENGTH, std::make_pair((const IResultComponent *)0, CGVTDataTypes::TPillarValueVector())));
    SetupVectors(pillarMap, retval.first->second.second);

    int i = 0;
    for (CPillarMap::const_iterator pillarMapIt = pillarMap.begin(); pillarMapIt != pillarMap.end();
         ++pillarMapIt, ++i) {
      int j = 0;
      for (CPillar::const_iterator pillarIt = pillarMapIt->second.begin(); pillarIt != pillarMapIt->second.end();
           ++pillarIt, ++j) {
        retval.first->second.second[i][j].Value((*pillarIt).first.Z());
      }
    }

    retval = valueMap.insert(std::make_pair(
        CGVTDataTypes::VSTRAIN, std::make_pair((const IResultComponent *)0, CGVTDataTypes::TPillarValueVector())));
    SetupVectors(pillarMap, retval.first->second.second);

    // retval = valueMap.insert(std::make_pair(CGVTDataTypes::TSTRAIN, std::make_pair((const IResultComponent *)0,
    // CGVTDataTypes::TPillarValueVector()))); SetupVectors(pillarMap, retval.first->second.second);
  ...
  if (deltaVp)
    ...
    {
      retval = valueMap.insert(std::make_pair(
          CGVTDataTypes::TIMESHIFT, std::make_pair((const IResultComponent *)0, CGVTDataTypes::TPillarValueVector())));
      SetupVectors(pillarMap, retval.first->second.second);
    }
#if 0
  if (deltaT)
...
#endif
    retval = valueMap.insert(std::make_pair(
        CGVTDataTypes::DELTAV, std::make_pair((const IResultComponent *)0, CGVTDataTypes::TPillarValueVector())));
    SetupVectors(pillarMap, retval.first->second.second);
#if 0
...
bool CGVTDataOutput::Empty() const { return m_stageMap.empty() || m_stageMap.begin()->second.empty(); }

int CGVTDataOutput::CalculationSize() const { return m_stageMap.begin()->second.begin()->second.second.size(); }
...
CGVTDataTypes::TValueMap &CGVTDataOutput::ValueMap(unsigned int key) { return m_stageMap[key]; }
...
CGVTData::CGVTData(CModelBase &model)
    : m_model(&model), m_SamplePointSet(0), m_AnalysisPointSet(0), m_PillarMap(0), m_PillarMapPointSet(0),
      m_InputPointSet(0) {}

CModelBase &CGVTData::Model() { return *m_model; }

void CGVTData::Build(IProgressBase &progress, CGVTSeismicGridDefinition &gridDef,
                     const CGVTDataTypes::TFormationVelocityModels &formVelocityModels,
                     const CGVTDataTypes::TTimeLapses &timelapses, bool deltaVp, bool deltaT, bool bRebuild,
                     bool bReinterpolate, bool bBuildInputPointSet) {
  ...
  */
  ...
  m_Input.Build(progress, Model(), *m_PillarMap, formVelocityModels, timelapses, bRebuild, bReinterpolate, input,
                requireNewInput);
...
void CGVTData::BuildAnalysisPointSet(IProgressBase &progress, CGVTSeismicGridDefinition &gridDef, bool bRebuild,
                                     bool bReinterpolate) {
  assert(m_SamplePointSet);
    ...
    for (it = stPsNode.begin(); it != stPsNode.end(); ++it) {
      ...
      if (pPointSet && (pPointSet->Name() == pointSetName)) {
        if (bReinterpolate) {
          ...
          delete pPointSet;
        } else { // just use this pointset, it should still be good.
...
void CGVTData::BuildVelocityPointSet(IProgressBase &progress, CGVTSeismicGridDefinition &gridDef) {
  ...
  for (TFormationBaseEntry::TNodeSet::const_iterator formationIt = formationBaseEntry->EntryNodes().begin();
       formationIt != formationBaseEntry->EntryNodes().end(); ++formationIt) {
  ...
  const CValueTypeFactory *factory = CValueTypeFactory::instance();
  pPointSet->NodalValueSet(nValueSize + 3 - 1)
      .LinkTo(factory->BuildValueType(*pPointSet, IDT_VALUETYPE_VELOCITYP, "Vp_D0")->Component());
...
void CGVTData::ConstructNewPointSet(const QString &name, const CGVTSeismicGridDefinition &gridDef,
...
CGVTDataInput &CGVTData::Input() { return m_Input; }

CGVTDataOutput &CGVTData::Output() { return m_Output; }
