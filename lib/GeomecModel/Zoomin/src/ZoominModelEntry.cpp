...
CZoominModelEntry::CZoominModelEntry(CModelBase &model)
    : CStorageNodeEntry<CZoominModelPlaceHolder>(MD_BASE_ZOOMIN_MODEL, IDI_ZOOMIN_MODELS, QObject::tr("Zoom-in Models"),
                                                 model) {}
...
bool CZoominModelEntry::AddChildModel(CModelBase &childModel) {
  ...
  if (!dynamic_cast<CHexaModel *>(&childModel) && !dynamic_cast<CGoCadModel *>(&childModel)) {
  ...
  QString sName = createUniqueName(DEFAULT_ZOOM_IN_MODEL_NAME, DEFAULT_INITIAL_NUMBER);
