...
public:
  ProgressWrapper(IProgressBase &progress) : gm_skua::IProgressHandler(), progress(progress) {}
...
CGocadImport::CGocadImport(bool bRequireSurfaceDefinitions, bool bAllowLightTSolid)
    : m_bRequireSurfaceDefinitions(bRequireSurfaceDefinitions), m_bAllowLightTSolid(bAllowLightTSolid),
      m_importer(nullptr) {}

CGocadImport::~CGocadImport() { delete m_importer; }
...
CElementSet *CGocadImport::CreateElementSet(CFemAppModel &model, int solidIndex) const {
  ...
  for (size_t i = 0; i < tetra_size; ++i) {
    ...
    vcElements.emplace_back(std::vector<int>{ptIndex[tetra.node[0]], ptIndex[tetra.node[1]], ptIndex[tetra.node[2]],
                                             ptIndex[tetra.node[3]]});
  }

#ifdef SKUA_ALLOW_MERGE
  // Meshing multiple solids should lead to a selection of solids and some merge, but that doesn't seem to work: no
  // dialog Let's rethink and possibly add this later Also, it looks like each gocad elementset would save ALL gocad
  // data (although using shared pointers on import, that is lost on save/load
...
#endif
  CGocadElementSet *pElementSet =
      new CGocadElementSet(solid.name.data, CElementSet::TETRA, vcPoints, vcElements, model, solid_copies);
...
int CGocadImport::SolidSize() const { return static_cast<int>(m_solids.size()); }

int CGocadImport::VsetSize() const { return static_cast<int>(m_vsets.size()); }

int CGocadImport::SurfSize() const { return static_cast<int>(m_surfaces.size()); }

std::vector<const gm_skua::SKUAParseData *> &CGocadImport::getSolids() { return m_solids; }

const std::string &CGocadImport::Error() const { return m_strError; }
...
const gm_skua::SKUAParseData *CGocadImport::Solid(int i) const { return m_solids[i]; }

const gm_skua::SKUAParseData *CGocadImport::Vset(int i) const { return m_vsets[i]; }

const gm_skua::SKUAParseData *CGocadImport::Surf(int i) const { return m_surfaces[i]; }
...
#define ENSURE_HEADER_SEEN(gocadType)                                                                                  \
  {                                                                                                                    \
    if (!m_status.HeaderSeen()) {                                                                                      \
      QString strError;                                                                                                \
      strError = QString("'%1' keyword before HEADER definition").arg(strKeyword);                                     \
...
CGocadImport::CGocadImport(bool bRequireSurfaceDefinitions, bool bAllowLightTSolid)
    : m_bRequireSurfaceDefinitions(bRequireSurfaceDefinitions), m_bAllowLightTSolid(bAllowLightTSolid), m_fp(0),
      m_nLineNr(0), m_pSolid(0), m_pVset(0), m_pTSurf(0), m_pVolume(0), m_pTetra(0), m_pSurface(0), m_pTFace(0),
      m_currentTri(0) {}
...
std::vector<QSharedPointer<CGocadData::CTSolid>> &CGocadImport::getSolids() { return m_vcSolids; }

int CGocadImport::SolidSize() const { return m_vcSolids.size(); }
...
int CGocadImport::VsetSize() const { return m_vcVsets.size(); }
...
int CGocadImport::SurfSize() const { return m_vcTSurfs.size(); }
...
std::string CGocadImport::Error() const { return m_strError.toStdString(); }
      ...
      if (itArg != itArgEnd) {
        ...
        if (*itArg == "TSolid" || *itArg == "LightTSolid") {
          ...
          if (!m_bAllowLightTSolid && bLightTSolid) {
            PARSE_ERROR(
                "Experimental feature 'GOCAD LightTSolid' encountered outside mesh import; please import as mesh. ");
          }
          ...
          if (m_pSolid) {
            ...
            QString message = QString("'GOCAD %1' encountered while reading a %2 (missing END marker?)")
                                  .arg(encountered)
                                  .arg(reading);
            PARSE_ERROR(message);
          }
...
bool CGocadImport::CheckSolid() {
  ...
  if (!CheckSolidProperties(m_pSolid->m_vcTetraPropertyNames, m_pSolid->m_vcTetraESizes,
                            m_pSolid->m_vcTetraNoDataValues, "TETRA_"))
    return false;
...
bool CGocadImport::CheckSolidProperties(const std::vector<QString> &vcPropertyNames, std::vector<int> &vcESizes,
                                        const std::vector<double> &vcNoDataValues, const QString &strPrefix) {
  ...
  if (vcPropertyNames.size() != vcESizes.size()) {
    QString s;
    s = QString("The Solid has %1 %2PROPERTIES defined, and %3 %4ESIZES; these numbers should match")
            .arg(vcPropertyNames.size())
            .arg(strPrefix)
            .arg(vcESizes.size())
            .arg(strPrefix);
    VALIDATION_ERROR(s);
  ...
  if (vcNoDataValues.size() != vcPropertyNames.size()) {
    QString s;
    s = QString("The Solid has %1 %2PROPERTIES defined, and %3 %4NO_DATA_VALUES; these numbers should match")
            .arg(vcPropertyNames.size())
            .arg(strPrefix)
            .arg(vcNoDataValues.size())
            .arg(strPrefix);
    VALIDATION_ERROR(s);
  }
  ...
  for (size_t i = 0; i < 4; ++i) {
    ...
    if (QString::compare(surf, "none", Qt::CaseInsensitive)) {
      ...
      if (bQuoted) {
        ...
        while (itArg != itArgEnd && QString::compare(*itArg, "none", Qt::CaseInsensitive) && (*itArg)[0] != '+' &&
               (*itArg)[0] != '-')
          surf += " " + *itArg++;
      }
      if (surf[1] == '(') // check for names like "+(boundary)name"
      {
        ...
        if (len > 0) {
          if (surf.mid(1, len) == "(boundary)") {
            ...
            // we replace the shared vertex with a new vertex, so that we can have a proper fault with different faces
            // on each side
            if (surf.endsWith("_fault") && m_pTetra->m_Nodes[i]->Type() == CGocadData::IS_SHAREDVERTEX) {
              const CGocadData::CSharedVertex *pSharedVertex =
                  static_cast<const CGocadData::CSharedVertex *>(m_pTetra->m_Nodes[i]);
...
bool CGocadImport::ParseProperties(TArgIterator itArg, TArgIterator itArgEnd, const std::vector<int> &vcESizes,
                                   std::vector<std::vector<double>> &vcProperties) {
  int nProperties = 0;
  int nFound = 0;
...
CGocadImport::CReadStatusException::CReadStatusException(const QString &strError) : m_strError(strError) {}

CGocadImport::CReadStatusException::CReadStatusException(const CReadStatusException &rhs)
    : m_strError(rhs.m_strError) {}

const QString &CGocadImport::CReadStatusException::Error() const { return m_strError; }

///// CReadStatus
CGocadImport::CReadStatus::CReadStatus()
    : m_bSectionOpen(false), m_bReadingHeader(false), m_bHeaderSeen(false), m_bZPositiveSeen(false),
      m_bAxisUnitSeen(false), m_bPropertiesSeen(false), m_bTetraPropertiesSeen(false) {}

CGocadImport::CReadStatus::CReadStatus(const CReadStatus &rhs)
    : m_bSectionOpen(rhs.m_bSectionOpen), m_bReadingHeader(rhs.m_bReadingHeader), m_bHeaderSeen(rhs.m_bHeaderSeen),
      m_bZPositiveSeen(rhs.m_bZPositiveSeen), m_bAxisUnitSeen(rhs.m_bAxisUnitSeen),
      m_bPropertiesSeen(rhs.m_bPropertiesSeen), m_bTetraPropertiesSeen(rhs.m_bTetraPropertiesSeen) {}
...
bool CGocadImport::CReadStatus::SectionOpen() const { return m_bSectionOpen; }

bool CGocadImport::CReadStatus::ReadingHeader() const { return m_bReadingHeader; }

bool CGocadImport::CReadStatus::HeaderSeen() const { return m_bHeaderSeen; }

bool CGocadImport::CReadStatus::ZPositiveSeen() const { return m_bZPositiveSeen; }

bool CGocadImport::CReadStatus::AxisUnitSeen() const { return m_bAxisUnitSeen; }

bool CGocadImport::CReadStatus::PropertiesSeen() const { return m_bPropertiesSeen; }

bool CGocadImport::CReadStatus::TetraPropertiesSeen() const { return m_bTetraPropertiesSeen; }

void CGocadImport::CReadStatus::OpenSection() { m_bSectionOpen = true; }
...
CElementSet *CGocadImport::CreateElementSet(CFemAppModel &model, int solidIndex) const {
  ...
  std::vector<std::vector<std::vector<std::vector<double>>>>
      vcElementNodeProperties; // nr of elements * nr of properties * ESIZE * nr of nodes
  ...
  for (iVolume = 0; iVolume < solid.VolumeSize(); ++iVolume) {
    ...
    for (iTetra = 0; iTetra < volume.TetraSize(); ++iTetra) {
      ...
      for (iProp = 0; iProp < nNodalProperties; ++iProp) {
        ...
        bool bOrientOk = geo::ITetrahedron::CheckOrientation(Node2Point(tetra.Node(0)), Node2Point(tetra.Node(1)),
  ...
  CGocadElementSet *pElementSet = new CGocadElementSet(solid.Name(), CElementSet::TETRA, vcPoints, vcElements, model,
                                                       const_cast<CGocadImport *>(this)->getSolids());
...
void CGocadImport::CreateSurface(CFemAppModel &model, int surfIndex) const {
  ...
  for (int i = 0; i < surf.m_pSurface->TFaceSize(); ++i) {
    ...
    for (size_t j = 0; j < face.TriangleSize(); ++j) {
      ...
      if (!triangle.Node(0).IsEqualTo(triangle.Node(1)) && !triangle.Node(1).IsEqualTo(triangle.Node(2)) &&
          !triangle.Node(2).IsEqualTo(triangle.Node(0)))
