...
geo::CPoint Node2Point(const CGocadData::CNode &node) { return geo::CPoint(node.X(), node.Y(), node.Z()); }

CGocadMeshImporter::CGocadMeshImporter(const gm_skua::SKUAParseData *solid, CTetraMesh &tetramesh)
    : m_inputSolids(1, solid), m_solid(solid), m_tetramesh(tetramesh) {}

CGocadMeshImporter::CGocadMeshImporter(const std::vector<const gm_skua::SKUAParseData *> &solids, CTetraMesh &tetramesh)
    : m_inputSolids(solids), m_solid(solids.empty() ? nullptr : solids.front()), m_tetramesh(tetramesh) {
...
CTetraModel &CGocadMeshImporter::Model() { return static_cast<CTetraModel &>(m_tetramesh.Model()); }

const CTetraModel &CGocadMeshImporter::Model() const { return static_cast<const CTetraModel &>(m_tetramesh.Model()); }

const geo::CTetMeshBase &CGocadMeshImporter::TetMesh() const {
  return static_cast<geo::CTetMeshBase &>(m_tetramesh.Mesh());
}

geo::CTetMeshBase &CGocadMeshImporter::TetMesh() { return static_cast<geo::CTetMeshBase &>(m_tetramesh.Mesh()); }
...
void CGocadMeshImporter::SortFormations(IProgressBase &progress) {
  CTetraFormationEntry *tetraFormationEntry =
      static_cast<CTetraFormationEntry *>(Model().GraphEntry(MD_TETRA_FORMATION));
  ...
  // Min() and Max() of CTetraFormation use DisplayList, and that is not setup yet, so we do our own gathering of min_z
  // and max_z If this is too slow, we may do something clever by keeping track of these values during import/parsing
  // the file
  struct TetraFormationWrapper {
    ...
    TetraFormationWrapper(CTetraFormation *formation) : formation(formation), min_z(DBL_MAX), max_z(-DBL_MAX) {
  ...
  std::sort(
      formations.begin(), formations.end(),
      [](const TetraFormationWrapper *lhs, const TetraFormationWrapper *rhs) -> bool {
        if (lhs->min_z == rhs->min_z) {
          if (lhs->max_z == rhs->max_z)
            ...
            return lhs->max_z >
                   rhs->max_z; // Careful, if same min_z but different max_z we sort with largest formation height first
        }
...
bool CGocadMeshImporter::CreateSurfaces(IProgressBase &progress, std::vector<geo::CSurfaceDesc *> &surface_descriptions,
                                        std::vector<CSurfaceBase *> &mesh_surfaces,
  ...
  if (!_createDataSurfaces(progress, surface_descriptions, re_index_tetras, mesh_surfaces, mesh_surface_names,
                           surface2bodies))
    return false;
...
bool CGocadMeshImporter::CreateInterfaceElements(IProgressBase &progress,
                                                 std::vector<geo::CSurfaceDesc *> &surface_descriptions,
                                                 const std::vector<int> &re_index_tetras) {
  ...
  for (size_t i = 0; i < surfaces.size(); ++i) {
    if (surface_types[i] == gm_skua::SKUAParseData::SurfaceType::SurfFault) {
      for (size_t j = 0; j < surfaces[i].size(); ++j) {
        ...
        const geo::IFace *frontFace =
            &static_cast<const geo::CTetrahedron &>(TetMesh().Element(tetra_minus)).Face(minus.opposite);
        const geo::IFace *backFace =
            &static_cast<const geo::CTetrahedron &>(TetMesh().Element(tetra_plus)).Face(plus.opposite);
...
bool CGocadMeshImporter::CreateIntermediateSurfaces(IProgressBase &progress,
                                                    const std::vector<geo::CSurfaceDesc *> surface_descriptions) {
  ...
  for (int i = 0; i < surface_types.size(); ++i) {
    if (surface_types[i] == gm_skua::SKUAParseData::SurfaceType::SurfDSFFault) {
      if (surface_names[i].ends_with("_minus")) // remove last 11 characters
      {
        ...
        TDSFFaultPairMap::iterator result =
            dsf_fault_pair_map.insert(std::make_pair(fault_base, std::pair<int, int>(-1, -1))).first;
  ...
  for (TDSFFaultPairMap::const_iterator it = dsf_fault_pair_map.begin(); it != dsf_fault_pair_map.end(); ++it) {
    ...
    std::vector<const geo::IFace *> surf_0;
    surf_0.reserve(100);
    std::vector<const geo::IFace *> surf_1;
    surf_1.reserve(100);
...
bool CGocadMeshImporter::_createFormationsFromTetraRegions(IProgressBase &progress) {
  ...
  for (size_t i = 0; i < volume_names.size(); ++i) {
    if (canHandleVolume(volume_types[i])) {
      ...
      CTetraFormationVolume *formation_volume =
          new CTetraFormationVolume(*formation, const_cast<geo::CBodyGroup &>(volume));
...
bool CGocadMeshImporter::_createDataSurfaces(
    IProgressBase &progress, std::vector<geo::CSurfaceDesc *> &surface_descriptions,
  ...
  for (size_t i = 0; i < surfaces.size(); ++i) {
    ...
    for (size_t j = 0; j < surfaces[i].size(); ++j) {
      ...
      if (triangle.minus >= 0) {
        ...
        if (tetra_minus >= 0) {
          ...
          std::map<size_t, std::vector<const geo::IFace *>>::iterator faces =
              surface2bodies.back().insert(std::make_pair(volume_minus, std::vector<const geo::IFace *>())).first;
      ...
      if (triangle.plus >= 0) {
        ...
        if (tetra_plus >= 0) {
          ...
          std::map<size_t, std::vector<const geo::IFace *>>::iterator faces =
              surface2bodies.back().insert(std::make_pair(volume_plus, std::vector<const geo::IFace *>())).first;
          if (!face && backface) {
    ...
    if (geo_surface.FaceSize() > 0) {
      ...
      lg(QString("CSurfaceBase %1 with %2 triangles")
             .arg(surface_names[i].data)
             .arg(mesh_surface->Surface().FaceSize()));
      mesh_surfaces.emplace_back(mesh_surface);
      mesh_surface_names.emplace_back(surface_names[i].data);
    } else
      surface2bodies.resize(surface2bodies.size() - 1);
...
bool CGocadMeshImporter::_createModelSurfaces(
    IProgressBase &progress, std::vector<geo::CSurfaceDesc *> &surface_descriptions,
  ...
  for (size_t i = 0; i < surface_types.size(); ++i) {
    ...
    geo::CSurfaceDesc *surface_desc =
        new geo::CSurfaceDesc(TetMesh(), mesh_surfaces[i]->Surface(), mesh_surface_names[i], is_fault);
    surface_descriptions.push_back(surface_desc);

    if (is_dsf) {
      for (std::map<size_t, std::vector<const geo::IFace *>>::iterator it = surface2bodies[i].begin();
           it != surface2bodies[i].end(); ++it) {
        ...
        for (size_t j = 0; j < it->second.size(); ++j) {
          ...
          tetsurf->AddFace(*new geo::CTiedPointBodyTriangle(const_cast<geo::IBody &>(*body_triangle->Parent()),
                                                            body_triangle->Index()));
        }

        lg(QString("CTetSurface %1 with %2 triangles")
               .arg(tetsurf->SurfaceDesc().Name().c_str())
               .arg(tetsurf->FaceSize()));
      }
    } else {
      for (std::map<size_t, std::vector<const geo::IFace *>>::iterator it = surface2bodies[i].begin();
           it != surface2bodies[i].end(); ++it) {
...
bool CGocadMeshImporter::canHandleVolume(int type) { return type == gm_skua::SKUAParseData::VolumeType::VolNormal; }
...
CGocadMeshImporter::CGocadSurface::CGocadSurface(const CGocadData::CSurface &gocadsurface)
    : m_surface(gocadsurface), m_bFault(false) {
  int i;
...
const CGocadData::CSurface &CGocadMeshImporter::CGocadSurface::Surface() const { return m_surface; }

int CGocadMeshImporter::CGocadSurface::TFaceSize() const { return m_vcTFaces.size(); }
...
bool CGocadMeshImporter::CGocadSurface::IsFault() const { return m_bFault; }

void CGocadMeshImporter::CGocadSurface::MarkAsFault() { m_bFault = true; }

///// CGocadMeshImporter::CGocadTFace

CGocadMeshImporter::CGocadTFace::CGocadTFace(const CGocadData::CTFace &tface)
    : m_tface(tface), m_pFrontGroup(0), m_pBackGroup(0), m_pSurface(0) {}

const geo::CBodyGroup *CGocadMeshImporter::CGocadTFace::FrontGroup() const { return m_pFrontGroup; }

const geo::CBodyGroup *CGocadMeshImporter::CGocadTFace::BackGroup() const { return m_pBackGroup; }

int CGocadMeshImporter::CGocadTFace::FacePairSize() const { return m_vcFacePairs.size(); }
...
bool CGocadMeshImporter::CGocadTFace::CollectFacesSimple(const TPositionFacesMap &mpPositionFaces,
                                                         bool bTwoSeparateBodies) {
...
bool CGocadMeshImporter::CGocadTFace::CollectFaces(const TPositionFacesMap &mpPositionFaces) {
  ...
  // 3.c Horizons (can) have two PointIndex-identical triangles with different BodyGroups attached; process with the old
  // method
...
CSurfaceBase *CGocadMeshImporter::CGocadTFace::AttachedSurface() { return m_pSurface; }
...
CGocadMeshImporter::CGocadMeshImporter(const CGocadData::CTSolid &tsolid, CTetraMesh &tetramesh)
    : m_vcTSolids(1, &tsolid), m_tetramesh(tetramesh) {}

CGocadMeshImporter::CGocadMeshImporter(const std::vector<const CGocadData::CTSolid *> &vcTSolids, CTetraMesh &tetramesh)
    : m_vcTSolids(vcTSolids), m_tetramesh(tetramesh) {}

CGocadMeshImporter::~CGocadMeshImporter() { DestroyGocadSurfaces(); }
  ...
  for (int iVolume = 0; iVolume < TetMesh().NrOfVolumes(); ++iVolume) {
    ...
    if (m_vcTSolids.size() == 1) {
      if (vcFormations.empty()) {
        ...
        const std::map<CGocadData::CTVolume *, int>::const_iterator it =
            mp.find(const_cast<CGocadData::CTVolume *>(&m_vcTSolids[iSolid]->Volume(iSolidVol)));
...
void CGocadMeshImporter::set_tetras(int iSolid, int iVolume, TVertex2NodeIndexMap &mpVertex2NodeIndex,
                                    std::vector<int> &vol_elems_v, CProgressBase &progdlg) {
  ...
  for (int iTetra = 0; iTetra < num_tetras; iTetra++) {
    ...
    for (int iNode = 0; iNode < 4; ++iNode) {
      ...
      switch (node.Type()) {
      case CGocadData::IS_VERTEX:
        pVertex = static_cast<const CGocadData::CVertex *>(&node);
        break;
      case CGocadData::IS_ATOM:
        pVertex = &static_cast<const CGocadData::CAtom &>(node).Vertex();
        break;
      case CGocadData::IS_SHAREDVERTEX:
        pVertex = &static_cast<const CGocadData::CSharedVertex &>(node).Vertex();
        break;
      }
    ...
    bool res = geo::ITetrahedron::CheckOrientation(Node2Point(tetra.Node(0)), Node2Point(tetra.Node(1)),
                                                   Node2Point(tetra.Node(2)), Node2Point(tetra.Node(3)));
    if (!res) {
      // swap nodes 3 and 4
      int tmp = vcTetra[3];
      vcTetra[3] = vcTetra[2];
  ...
  // int volume_idx = TetMesh().RegisterTetVolume( vcVolumeElements );// ATENTTION !!! : all tetras in one volume
  // (bodygroup) IT_Printer::numbers( vcVolumeElements ); //FIXME: singleton, etc.
}

int CGocadMeshImporter::set_points(int iSolid, int iVolume, TVertex2NodeIndexMap &mpVertex2NodeIndex,
    ...
    for (int iSolid = 0; iSolid < num_solids; iSolid++) {
      ...
      for (int iVolume = 0; iVolume < num_vols; ++iVolume) {
        ...
        if (!solid->Volume(iVolume).DoubleSidedFaultVolume()) // TODO: 'volume_idx' ????
          int volume_idx =
              TetMesh().RegisterTetVolume(vol_elems_v); // ATENTTION !!! : all tetras in one volume (bodygroup)
      }
    }
...
CGocadMeshImporter::TPositionFacesMap
CGocadMeshImporter::map_position_faces(TPositionTriangleSet &stGocadSurfaceTriangles, CProgressBase &progdlg) {
    ...
    for (int j = 0; j < num_bodies; ++j) {
      ...
      for (int k = 0; k < num_faces; ++k) {
        ...
        if (stGocadSurfaceTriangles.find(CPositionTriangle(face)) != stGocadSurfaceTriangles.end()) {
          TPositionFacesMap::iterator it =
              mpPositionFaces.insert(TPositionFacesMap::value_type(CPositionTriangle(face), TFaceBodyPairSet())).first;
...
// For '.ts' file
void CGocadMeshImporter::add_to_gocad_sur_tri_set(const CGocadData::CSurface &sur,
                                                  TPositionTriangleSet &stGocadSurfaceTriangles) {
  ...
  for (int i = 0; i < num_tfaces; i++) {
    ...
    for (int j = 0; j < num_tri; j++) {
      ...
      if (!&tri.Node(0) || !&tri.Node(1) || !&tri.Node(2)) {
        // Printer::instance()->error("add_to_gocad_sur_tri_set > tface: %d, tri: %d, node: %d", i, j, !&tri.Node( 0 ) ?
        // 0 : !&tri.Node( 1 ) ? 1 : 2 );
      } else
...
bool CGocadMeshImporter::add_surface(const CGocadData::CSurface &gocadsurface, TPositionFacesMap &mpPositionFaces,
                                     CProgressBase &progdlg) {
...
bool CGocadMeshImporter::create_gm_objs_01(bool &bHaveDoubleSidedFaults, TAverageDepthMap &mpAverageDepth,
                                           CProgressBase &progdlg) {
  ...
  for (int i = 0; i < m_vcGocadSurfaces.size(); ++i) {
    ...
    if (gocadsurface.IsFault() && (gocadsurface.TFaceSize() > ONE_FACE)) {
      ...
      for (int face = 0; face < gocadsurface.TFaceSize(); ++face) {
        faces.push_back(STNamedSurface(gocadsurface.TFace(face).AttachedSurface()->Name().toStdString(),
...
void CGocadMeshImporter::create_gm_objs_02(int iTopSurface, TSurfaceSourceMap &mpSurfaceSource,
                                           ...
                                           CProgressBase &progdlg) // top horizon
{
  ...
  for (int i = 0; i < m_vcGocadSurfaces[iTopSurface]->TFaceSize(); ++i) {
    ...
    TBodyGroup2SurfaceMap::iterator it = mpBodyGroup2Surface
                                             .insert(TBodyGroup2SurfaceMap::value_type(
                                                 m_vcGocadSurfaces[iTopSurface]->TFace(i).FrontGroup(), TSurfaceVec()))
                                             .first;
    it->second.push_back(TSurfaceDef(m_vcGocadSurfaces[iTopSurface]->TFace(i), true, false));
  }
}

void CGocadMeshImporter::create_gm_objs_03(int iBotSurface, TSurfaceSourceMap &mpSurfaceSource,
                                           TBodyGroup2SurfaceMap &mpBodyGroup2Surface,
                                           CProgressBase &progdlg) // bottom horizon
{
  ...
  for (int i = 0; i < m_vcGocadSurfaces[iBotSurface]->TFaceSize(); ++i) {
    ...
    TBodyGroup2SurfaceMap::iterator it = mpBodyGroup2Surface
                                             .insert(TBodyGroup2SurfaceMap::value_type(
                                                 m_vcGocadSurfaces[iBotSurface]->TFace(i).FrontGroup(), TSurfaceVec()))
                                             .first;
...
void CGocadMeshImporter::create_gm_objs_04(TSurfaceSourceMap &mpSurfaceSource,
                                           TBodyGroup2SurfaceMap &mpBodyGroup2Surface, int iTopSurface, int iBotSurface,
                                           CProgressBase &progdlg) {
  int iFault = 0;
  int iHorizon = 0;
  ...
  for (int i = 0; i < m_vcGocadSurfaces.size(); ++i) {
    ...
    if (gocadsurface.IsFault()) {
      ...
      for (int j = 0; j < gocadsurface.TFaceSize(); ++j) {
        ...
        it = mpBodyGroup2Surface
                 .insert(TBodyGroup2SurfaceMap::value_type(gocadsurface.TFace(j).FrontGroup(), TSurfaceVec()))
                 .first;
        it->second.push_back(TSurfaceDef(gocadsurface.TFace(j), true, true));

        if (!gocadsurface.Surface().DoubleSidedFaultSurface()) {
          it = mpBodyGroup2Surface
                   .insert(TBodyGroup2SurfaceMap::value_type(gocadsurface.TFace(j).BackGroup(), TSurfaceVec()))
                   .first;
    ...
    //------------------------------------------------------ OTHERS THAN FAULTS
      ...
      if (i != iTopSurface && i != iBotSurface) // these two have already been processed
      {
        for (int j = 0; j < gocadsurface.TFaceSize(); ++j) {
          ...
          CSurfaceBase *pSurfaceBase = gocadsurface.TFace(j).AttachedSurface();
          TBodyGroup2SurfaceMap::iterator it1 =
              mpBodyGroup2Surface
                  .insert(TBodyGroup2SurfaceMap::value_type(gocadsurface.TFace(j).FrontGroup(), TSurfaceVec()))
                  .first;
          ...
          if (gocadsurface.TFace(j).BackGroup() != 0) {
            ...
            TBodyGroup2SurfaceMap::iterator it2 =
                mpBodyGroup2Surface
                    .insert(TBodyGroup2SurfaceMap::value_type(gocadsurface.TFace(j).BackGroup(), TSurfaceVec()))
                    .first;
...
void CGocadMeshImporter::create_gm_objs_05(TSurfaceSourceMap &mpSurfaceSource,
                                           TBodyGroup2SurfaceMap &mpBodyGroup2Surface, int bHaveDoubleSidedFaults,
                                           CProgressBase &progdlg) {
  ...
  for (it = mpBodyGroup2Surface.begin(); it != mpBodyGroup2Surface.end(); ++it) {
    ...
    for (size_t i = 0; i < it->second.size(); ++i) {
      ...
      geo::CSurfaceDesc &surfacedesc =
          GetSurfaceDescriptor(surfacebase, mpSurface2SurfaceDesc, bFault, *mpSurfaceSource[&tface]);
...
void CGocadMeshImporter::CreateInterfaceElements(IProgressBase &progdlg,
                                                 TSurface2SurfaceDescMap &mpSurface2SurfaceDesc) {
  // create the fault interface elements
  for (size_t i = 0; i < m_vcGocadSurfaces.size(); ++i) {
    CGocadSurface &gocadsurface = *m_vcGocadSurfaces[i];
...
void CGocadMeshImporter::CreateIntermediateSurfaces(IProgressBase &progdlg,
                                                    TSurface2SurfaceDescMap &mpSurface2SurfaceDesc) {
  for (size_t i = 0; i < m_vcGocadSurfaces.size(); ++i) {
    CGocadSurface &minusGocadSurface = *m_vcGocadSurfaces[i];
    if (minusGocadSurface.IsFault()) {
...
geo::CSurfaceDesc &CGocadMeshImporter::GetSurfaceDescriptor(const CSurfaceBase &surfacebase,
