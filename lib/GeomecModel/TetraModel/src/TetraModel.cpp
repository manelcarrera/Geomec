...
CTetraModel::~CTetraModel() { delete m_mesh_refinement_data; }

// ##ModelId=3B6905BF01C5
unsigned int CTetraModel::ModelIconId() const { return staticModelIconId(); }

// ##ModelId=3B6905BF01D5
QString CTetraModel::documentType() const { return staticDocumentType(); }

unsigned int CTetraModel::staticModelIconId() { return IDI_TETRA_MODEL; }

QString CTetraModel::staticDocumentType() { return "Tetrahedron"; }
...
bool CTetraModel::IsResult() const { return ResultRegister().ResultsAvailable(); }

void CTetraModel::Calculate() {}

CMeshBase &CTetraModel::Mesh() { return *m_pMesh; }

const CMeshBase &CTetraModel::Mesh() const { return *m_pMesh; }
...
bool CTetraModel::LoadTetra1(CStorageNode::TSTREAM &stream, CStreamVersion &version, CStorageNode::TPROGRESS &prog) {
  ...
  */
  ...
  CTetraSuperHorizonEntry &s_horizon_entry =
      dynamic_cast<CTetraSuperHorizonEntry &>(*GraphEntry(MD_TETRA_SUPER_HORIZON));
...
bool CTetraModel::OnLoad(CStorageNode::TSTREAM &stream, CStreamVersion &version, CStorageNode::TPROGRESS &prog) {
  return (LoadTetra1(stream, version, prog) && LoadTetra2(stream, version, prog));
}
...
long CTetraModel::SavedItems() const {
  ...
  */
  const CTetraSubHorizonEntry &fault_entry =
      dynamic_cast<const CTetraSubHorizonEntry &>(*GraphEntry(MD_TETRA_SUB_FAULT));
  lRet += fault_entry.SavedItems();
  const CTetraSubHorizonEntry &horizon_entry =
      dynamic_cast<const CTetraSubHorizonEntry &>(*GraphEntry(MD_TETRA_SUB_HORIZON));
  lRet += horizon_entry.SavedItems();
  lRet += SubBoundary().SavedItems();
  const CTetraSuperHorizonEntry &s_horizon_entry =
      dynamic_cast<const CTetraSuperHorizonEntry &>(*GraphEntry(MD_TETRA_SUPER_HORIZON));
  ...
  */
  const CTetraFormationEntry &formation_entry =
      dynamic_cast<const CTetraFormationEntry &>(*GraphEntry(MD_TETRA_FORMATION));
...
CTetraModel::ModelType CTetraModel::modelType() const { return eTetraModel; }

const CTetraSubBoundary &CTetraModel::SubBoundary() const { return *m_pSubBoundary; }

CTetraSubBoundary &CTetraModel::SubBoundary() { return *m_pSubBoundary; }

bool CTetraModel::OnModelSpecificAnalysisCheck(const CAnalysisType & /*antype*/, bool /*bWriteInputFiles*/,
                                               bool /*bWriteQuadDat*/) {
  ...
  if (boundary.CreateInterfaces() && boundary.InterfaceElements() &&
      (!CheckZoomInBoundaryLoads(boundary.DisplacementSupportNode()) ||
       !CheckZoomInBoundaryLoads(boundary.PressureSupportNode()))) {
