...
DSFSurfaceCombiner::DSFSurfaceCombiner(const IntermediateSurfaceInfo &surf_src_0,
                                       const IntermediateSurfaceInfo &surf_src_1)
    : m_surf_src_0(surf_src_0), m_surf_src_1(surf_src_1) {}
...
void DSFSurfaceCombiner::Generate(IntermediateSurfaceInfo &surf_dst, std::vector<IntermediateSurfaceInfo> *helpers) {
  ...
  double optimum_size =
      (((plane_info_0.min_area + plane_info_0.max_area + plane_info_1.min_area + plane_info_1.max_area) / 4) +
       plane_info_0.avg_area + plane_info_1.avg_area) /
      3;
...
void DSFSurfaceCombiner::getSurfaceEdges(const IntermediateSurfaceInfo &surf,
...
void DSFSurfaceCombiner::getOrderedSurfaceEdges(const IntermediateSurfaceInfo &surf, std::vector<geo::CPoint> &points,
                                                std::vector<std::pair<size_t, size_t>> &ordered_edges) const {
  ...
  for (std::set<std::pair<int, int>>::iterator it = surf_edges.begin(); it != surf_edges.end(); ++it) {
    std::pair<std::map<int, std::pair<int, int>>::iterator, bool> retval =
        node2neighbors.insert(std::make_pair(it->first, std::pair<int, int>()));
    if (retval.second)
...
// want that.)
bool DSFSurfaceCombiner::createStitchedCopy(const IntermediateSurfaceInfo &surf,
                                            IntermediateSurfaceInfo &surf_cpy) const {
  typedef std::pair<int, int> TEdge;
  typedef std::set<int> TOppositeNodes;
  typedef std::map<TEdge, TOppositeNodes> TEdgeMap;
  TEdgeMap edge_map;
  ...
  for (TPoint2Nodes::const_iterator it = point2nodes.begin(); it != point2nodes.end(); ++it) {
    if (it->second.size() > 1) // we expect 2, but who knows?
    {
      ...
      if (processed > 1)
        new_points[keep].Set(
            point.X() / processed, point.Y() / processed,
            point.Z() / processed); // not really necessary, but this way the non-duplicate node lies in the middle
    }
  }
...
void DSFSurfaceCombiner::createRotatedCopy(const IntermediateSurfaceInfo &surf, const geo::CPoint &plane_mid,
                                           const geo::CVector &plane_normal, IntermediateSurfaceInfo &surf_cpy,
                                           PlaneInfo &plane_info) const {
  ...
  for (int i = 0; i < surf.triangles.size(); ++i) {
    ...
    if (area < plane_info.min_area)
      plane_info.min_area = area;
    if (area > plane_info.max_area)
      plane_info.max_area = area;
  }
...
void DSFSurfaceCombiner::getRotationInfo(const IntermediateSurfaceInfo &surf_0, const IntermediateSurfaceInfo &surf_1,
                                         geo::CPoint &rotation_mid, geo::CVector &rotation_normal) const {
...
void DSFSurfaceCombiner::getCorrectionSurface(const IntermediateSurfaceInfo &surf_rot_0,
                                              const IntermediateSurfaceInfo &surf_flat_0,
                                              const IntermediateSurfaceInfo &surf_rot_1,
                                              const IntermediateSurfaceInfo &surf_flat_1,
...
DSFSurfaceCombiner::TriangleInfo::TriangleInfo(const IntermediateSurfaceInfo &surf, int triangle)
    : surf(surf), triangle(triangle), x(0), y(0), z(), r2(0) {
  const std::vector<geo::CPoint> &pts = surf.points;
  const std::vector<int> &tri = surf.triangles[triangle];
...
void DSFSurfaceCombiner::getTyingInformation(TyingInfoMap &mpNodeInfo, const IntermediateSurfaceInfo &surf_src_input,
                                             const IntermediateSurfaceInfo &surf_dst_input, double eps) const {
  double min_eps = -eps;
