...
CTetraHorizonBase::CTetraHorizonBase(CFemAppModel &model) : C3DHorizon(model), m_pInterfaceElements(0) {}

CTetraHorizonBase::CTetraHorizonBase(CSurfaceBase &surface, CFemAppModel &model, bool bSlip, bool bAttachToEntry)
    : C3DHorizon(surface, model, bSlip, bAttachToEntry), m_pInterfaceElements(0) {}

CTetraHorizonBase::CTetraHorizonBase(const QString &strInstanceName, const double dDepth, CFemAppModel &model,
                                     bool bAttachToEntry)
    : C3DHorizon(strInstanceName, dDepth, model, bAttachToEntry), m_pInterfaceElements(0) {}

CTetraHorizonBase::CTetraHorizonBase(const QString &strInstanceName, CFemAppModel &model, bool bAttachToModel)
    : C3DHorizon(strInstanceName, model, bAttachToModel), m_pInterfaceElements(0) {}

CTetraHorizonBase::CTetraHorizonBase(const C3DHorizon &rhs) : C3DHorizon(rhs), m_pInterfaceElements(0) {}
...
bool CTetraHorizonBase::Flip() const { return m_bFlip; }

void CTetraHorizonBase::Flip(bool bFlip) { m_bFlip = bFlip; }
...
const geo::CElementGroup *CTetraHorizonBase::InterfaceElementGroup() const {
  ...
  for (int i = 0; (i < OutputSurfaceSize()) && !hasInterfaceElements; ++i) {
    hasInterfaceElements = (OutputSurface(i).Slip() && (OutputSurface(i).interfaceElements().ElementSize() > 0));
  }
  ...
  if (hasInterfaceElements) {
    ...
    for (i = 0; i < OutputSurfaceSize(); ++i) {
      ...
      for (j = 0; j < OutputSurface(i).interfaceElements().ElementSize(); ++j)
        m_pInterfaceElements->AddMeshElement(
            const_cast<geo::IElement &>(OutputSurface(i).interfaceElements().Element(j)));
    }
  }
