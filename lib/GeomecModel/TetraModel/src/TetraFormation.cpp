...
CTetraFormationVolume::CTetraFormationVolume(CTetraFormation &formation) : CFormationVolume(formation) {}
...
int CTetraFormationVolume::DisplayListSize() const {
  ...
  if (nRet) {
    ...
    const CTetraFormationEntry *pEntry =
        dynamic_cast<const CTetraFormationEntry *>(Model().GraphEntry(MD_TETRA_FORMATION));
    if (pEntry) {
      if (pEntry->ShowSkin())
...
TGraphNodeSet CTetraFormationVolume::Identifier() const { return m_identifier; }
...
CTetraFormation::CTetraFormation(const QString &strName, CModelBase &model)
    : C3DFormation(strName, model), m_order(initialOrdering++) {
  create(Model().GraphEntry(MD_TETRA_FORMATION));
}

CTetraFormation::CTetraFormation(CFemAppModel &model) : C3DFormation(model), m_order(initialOrdering++) {}

CTetraFormation::~CTetraFormation() {}

CTetraFormation::CTetraFormation(const CTetraFormation &rhs) : C3DFormation(rhs), m_order(rhs.m_order) {}

bool CTetraFormation::operator==(const CTetraFormation &rhs) const { return C3DFormation::operator==(rhs); }
...
bool CTetraFormation::canMoveDown() const {
  const CTetraFormationEntry *tetraFormationEntry =
      dynamic_cast<const CTetraFormationEntry *>(Model().GraphEntry(MD_TETRA_FORMATION));
  const CTetraFormationEntry::TSortedNodeSet sortedNodeSet = tetraFormationEntry->SortedEntryNodes();
  CTetraFormation *tetraFormation = const_cast<CTetraFormation *>(this);

  return ((sortedNodeSet.size() > 1) && (sortedNodeSet.find(tetraFormation) != sortedNodeSet.end()) &&
...
bool CTetraFormation::canMoveUp() const {
  ...
  return ((sortedNodeSet.size() > 1) && (sortedNodeSet.find(tetraFormation) != sortedNodeSet.begin()));
}
...
void CTetraFormation::moveDown() {
  const CTetraFormationEntry *tetraFormationEntry =
      dynamic_cast<const CTetraFormationEntry *>(Model().GraphEntry(MD_TETRA_FORMATION));
  const CTetraFormationEntry::TSortedNodeSet sortedNodeSet = tetraFormationEntry->SortedEntryNodes();
  const CTetraFormationEntry::TSortedNodeSet::const_iterator nextFormation = ++(sortedNodeSet.find(this));
...
void CTetraFormation::moveUp() {
  const CTetraFormationEntry *tetraFormationEntry =
      dynamic_cast<const CTetraFormationEntry *>(Model().GraphEntry(MD_TETRA_FORMATION));
  const CTetraFormationEntry::TSortedNodeSet sortedNodeSet = tetraFormationEntry->SortedEntryNodes();
  const CTetraFormationEntry::TSortedNodeSet::const_iterator previousFormation = --(sortedNodeSet.find(this));
...
unsigned int CTetraFormation::Order() const { return m_order; }

void CTetraFormation::Order(unsigned int order) { m_order = order; }

bool CTetraFormation::Less(const CGraphNode &node) const {

  const CTetraFormation *tetraFormation = dynamic_cast<const CTetraFormation *>(&node);
...
void CTetraFormation::createInitialOrdering() {
  const CTetraFormationEntry *tetraFormationEntry =
      dynamic_cast<const CTetraFormationEntry *>(Model().GraphEntry(MD_TETRA_FORMATION));
  const CTetraFormationEntry::TNodeSet nodeSet = tetraFormationEntry->EntryNodes();

  if (nodeSet.size() > 1) {
    std::vector<CTetraFormation *> formations;

    for (CTetraFormationEntry::TNodeSet::const_iterator node = nodeSet.begin(); node != nodeSet.end(); ++node) {
    ...
    for (std::vector<CTetraFormation *>::iterator formation = formations.begin(); formation != formations.end();
         ++formation) {
