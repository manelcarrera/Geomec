...
CTetraSuperHorizon::CTetraSuperHorizon(CFemAppModel &model) : CTetraHorizonBase(model) {}

CTetraSuperHorizon::CTetraSuperHorizon(const QString &sName, const double &dDepth, CFemAppModel &model,
                                       bool bAttachToEntry)
...
bool CTetraSuperHorizon::CanConnectItem(const CGraphNode &item) const {
  ...
  if (pSurf)
    return !IsLinkedTo(item) && pSurf->Used() == 0 &&
...
bool CTetraSuperHorizonEntry::SurfaceIntersection(const geo::ISurface &surface1, const geo::ISurface &surface2) const {
  // Do surfaces overlap?
  if (surface1.Min().X() > surface2.Max().X() || surface1.Min().Y() > surface2.Max().Y() ||
      surface1.Min().Z() > surface2.Max().Z() || surface1.Max().X() < surface2.Min().X() ||
      surface1.Max().Y() < surface2.Min().Y() || surface1.Max().Z() < surface2.Min().Z())
    return false;
  ...
  for (int nFace1 = 0; nFace1 < surface1.FaceSize(); nFace1++) {
    ...
    for (int nFace2 = 0; nFace2 < surface2.FaceSize(); nFace2++) {
      ...
      if (!(face1.Min().X() > face2.Max().X() || face1.Min().Y() > face2.Max().Y() ||
            face1.Min().Z() > face2.Max().Z() || face1.Max().X() < face2.Min().X() ||
            face1.Max().Y() < face2.Min().Y() || face1.Max().Z() < face2.Min().Z()))
        return true;
    }
  }
