...
DSFIntermediateSurfaceGenerator::DSFIntermediateSurfaceGenerator(CTetraMesh &tetramesh) : m_mesh(tetramesh) {}

CTetraModel &DSFIntermediateSurfaceGenerator::Model() { return static_cast<CTetraModel &>(m_mesh.Model()); }
...
bool DSFIntermediateSurfaceGenerator::GenerateIntermediateSurface(const QString &name, const TSurface &surf_0,
                                                                  const TSurface &surf_1, bool precise) {
  // Minus fault
  ...
  // here we create the actual tying info from our new surface back to the original ones (same routine, different
  // direction)
  mpMinus.clear();
  mpPlus.clear();
...
double DSFIntermediateSurfaceGenerator::TriangleInfo::Contains(const geo::IPoint &p) const {
  if (SquaredDistance(p) < r2) {
    ...
    geo::CPoint trans(p.X() - signedDist * normal.X(), p.Y() - signedDist * normal.Y(),
                      p.Z() - signedDist * normal.Z());
...
DSFIntermediateSurfaceGenerator::TriangleInfo::TriangleInfo(const geo::IFace *triangle)
    : triangle(triangle), x(triangle->MidPoint().X()), y(triangle->MidPoint().Y()), z(triangle->MidPoint().Z()), r2(0) {
  for (int i = 0; i < 3; ++i) {
...
void DSFIntermediateSurfaceGenerator::TNodeInfo::process() {
  if (!tmp_tying_infos.empty()) {
    ...
    tying_info.point_dst.Set(tying_info.point_dst.X() / tmp_tying_infos.size(),
                             tying_info.point_dst.Y() / tmp_tying_infos.size(),
                             tying_info.point_dst.Z() / tmp_tying_infos.size());
...
void DSFIntermediateSurfaceGenerator::getTyingInformation(TNodeInfoMap &mpNodeInfo, const TSurface &surf_src,
                                                          const TSurface &surf_dst, bool precise, double eps) {
  double min_eps = -eps;
  ...
  if (precise) {
    ...
    for (int i = 0; i < surf_src.size(); ++i) {
      ...
      for (int j = 0; j < 3; ++j) {
        std::pair<TNode2Faces::iterator, bool> retval =
            node2Faces.insert(std::make_pair(face->PointIndex(j), std::vector<int>()));
        if (retval.second)
  ...
  for (int i = 0; i < surf_src.size(); ++i) {
    ...
    for (int m = 0; m < face1->NrOfPoints(); ++m) {
      ...
      std::pair<TNodeInfoMap::iterator, bool> retval =
          mpNodeInfo.insert(std::make_pair(face1->PointIndex(m), TNodeInfo()));
      if (retval.second) {
        // find triangle
...
// The map mpNodeInfo_dst contains links for each point of surf_src to the other surface (surf_dst, which itself is not
// needed here). We create a new point half-way these two points (or by changing distance, somewhere on this vector from
// point_src to point_dst). No duplicates. Triangles are the same topology as in surf_src.
void DSFIntermediateSurfaceGenerator::getIntermediateSurfaceInfo(IntermediateSurfaceInfo &surf_new,
                                                                 const TNodeInfoMap &mpNodeInfo_dst,
                                                                 const TSurface &surf_src, double distance) const {
...
void DSFIntermediateSurfaceGenerator::selectIntermediateSurfaceInfo(IntermediateSurfaceInfo &surf_minus,
                                                                    IntermediateSurfaceInfo &surf_plus,
                                                                    IntermediateSurfaceInfo &surf_select,
                                                                    bool precise) {
  if (!precise) {
    // Select plus or minus based on size: the larger one is the outersurface in a curved fault in our examples; THIS IS
    // NOT A GOOD TEST!
...
// Select one of the two intermediate surfaces, or create a new one... [DEBUG version: shows helper surfaces created and
// used during creation of the final intermediate surface]
void DSFIntermediateSurfaceGenerator::selectIntermediateSurfaceInfo(const QString &name,
                                                                    IntermediateSurfaceInfo &surf_minus,
                                                                    IntermediateSurfaceInfo &surf_plus,
                                                                    IntermediateSurfaceInfo &surf_select,
                                                                    bool precise) {
  if (!precise) {
...
geo::CSurfaceDesc *DSFIntermediateSurfaceGenerator::createSurface(const QString &name,
                                                                  const IntermediateSurfaceInfo &surf,
                                                                  const CTSSurface &tempSurf) {
...
void DSFIntermediateSurfaceGenerator::createIFElements(TNodeInfoMap &mpMinus, TNodeInfoMap &mpPlus,
                                                       const TSurface &surface, geo::CSurfaceDesc &surf_desc,
                                                       InterfaceInfo &if_info) {
...
// This uses a variant of the "generateTyingInfo" routine, and this needs to be refactored a bit to remove duplicate
// code (TODO).
void DSFIntermediateSurfaceGenerator::createResultTyings(const InterfaceInfo &if_info, const TSurface &surf_front_src,
                                                         const TSurface &surf_back_src) {
  double min_eps = -0.1;
  double max_eps = 1.1;
  ...
  for (int i = 0; i < surf_front_src.size(); ++i) {
    ...
    for (int m = 0; m < face1->NrOfPoints(); ++m) {
      ...
      std::pair<TDone::iterator, bool> result =
          done.insert(std::make_pair(face1->PointIndex(m), std::make_pair(0, geo::CPoint())));
      ...
      if (result.second) {
        for (int n = 0; n < if_info.front.size(); ++n) {
          if (if_info.front[n].Contains(point)) {
            ...
            if (isoPt[0] < max_eps && isoPt[0] > min_eps && isoPt[1] < max_eps && isoPt[1] > min_eps) {
              ...
              static_cast<geo::CTiedPointBodyTriangle *>(const_cast<geo::IFace *>(face1))
                  ->AddTying(m, *if_info.if_elts[n], plane.Project(point), 0);
        ...
        static_cast<geo::CTiedPointBodyTriangle *>(const_cast<geo::IFace *>(face1))
            ->AddTying(m, *if_info.if_elts[result.first->second.first], result.first->second.second, 0);
      }
    }
  }
...
void DSFIntermediateSurfaceGenerator::createTyings(const TNodeInfoMap &mpNodeInfo) {
  ...
  for (TNodeInfoMap::const_iterator it = mpNodeInfo.begin(); it != mpNodeInfo.end(); ++it) {
    ...
    geo::CTriangle localTriangle(rs.ToLocal(triangle->Point(0)), rs.ToLocal(triangle->Point(1)),
                                 rs.ToLocal(triangle->Point(2)));
