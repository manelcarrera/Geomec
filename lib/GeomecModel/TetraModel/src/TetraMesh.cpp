...
CTetraMesh::CTetraMesh(CTetraModel &model)
    : CMeshBase("Mesh", model.DepletionStageEntry()), m_bCanInvalidateMesh(true), m_bBuildingFromGocad(false),
      m_dMaxGocadSurfaceAngle(50 * PI / 180), m_bImportedMesh(false),
#ifdef HAVE_CM2_MESHER
      m_pMesh(geo::CTetMeshBase::GetTetMesher("CM2"))
#else
      m_mesherType(FGV), m_pMesh(geo::CTetMeshBase::GetTetMesher())
#endif
{
...
bool CTetraMesh::IsMesh() const { return m_pMesh->ElementSize() > 0 && !m_bBuildingFromGocad; }

bool CTetraMesh::CanInvalidateMesh() const {
  return m_bCanInvalidateMesh != false /*&& !m_bImportedMesh*/ &&
         !(static_cast<const CModelBase &>(Model())).BranchState().IsBranch();
}
...
void print(const gocadmesh::RefinementBox &box) {
  ...
  Printer::instance()->debug(
      "[gocadmesh]RefinementBox > c:[%.2f,%.2f,%.2f] s:[%.2f,%.2f,%.2f] r:[%.2f,%.2f,%.2f] g:%d steps:%d", c[0], c[0],
      c[0], s[0], s[0], s[0], r[0], r[0], r[0], g_t, box.Profile().size());

  for (auto &gradient_step : box.Profile())
    Printer::instance()->debug("[gocadmesh]Gradient > [d:%.2F,s:%.2F]", gradient_step.Distance(), gradient_step.Size());
}
...
void CTetraMesh::CreateMesh() {
  ...
  if (cm2_mesher) {
    ...
    if (t == T3da{0, 0, 0}) {
      ...
      t = T3da{(m1.X() + M1.X()) / 2, (m1.Y() + M1.Y()) / 2, (m1.Z() + M1.Z()) / 2};
    }

    std::vector<gocadmesh::RefinementBox> diana_boxes_v =
        convert(boxes_v, t); // from Geomec types scope to diana types scope
  ...
  if (pBoundary->State() != CTetraBoundary::DEFAULT_DEFINED) {
    for (int i = 0; i < pModel->SubBoundary().SideSurfaceSize(); i++) {
      ...
      // the source vector.
      m_pMesh->AddSurface(pModel->SubBoundary().SideSurface(i).Surface(),
                          pModel->SubBoundary().SideSurface(i).Name().toStdString(), false);
  ...
  geo::CTetMeshBase::TRet ret =
      m_pMesh->CreateMesh(env->get(CEnvironment::DIAPATH).toStdString(), env->get(CEnvironment::DIASHARE).toStdString(),
                          env->get(CEnvironment::DIALIB).toStdString(), env->get(CEnvironment::DIASLIB).toStdString(),
                          sFgvExeName, progress);
  ...
  for (int i = 0; i < m_pMesh->NrOfVolumes(); i++) {
    if (stVolume.size() > 0) {
      ...
      QString sFormationName = pFormationEntry->createUniqueName(DEFAULT_FORMATION_NAME, DEFAULT_INITIAL_NUMBER);
      ...
      if (version < CStreamVersion(3, 0, 10)) // In version 3.0.10 are boundary surfaces also saved
      {
        ...
        if (pBoundary->State() != CTetraBoundary::DEFAULT_DEFINED) {
          for (int i = 0; i < pModel->SubBoundary().SideSurfaceSize(); i++) {
            // Register the side surfaces in the mesh and add the pointer of the sub boundary to
            // the source vector.
            AddSurface(pModel->SubBoundary().SideSurface(i).Surface(), pModel->SubBoundary().SideSurface(i).Name(),
                       false);
            m_vcSurfaceSource.push_back(&pModel->SubBoundary());
            assert(m_pMesh->NrOfSurfaces() == m_vcSurfaceSource.size());
          }
      ...
      for (int i = 0; i < nIndexSize; i++) {
        ...
        } else if (nIndex == -1) // Super model side surface
        {
          ...
          if (nSuperSideSurface < pBoundary->SideSurfaceSize())
            m_pMesh->AddSurface(pBoundary->SideSurface(nSuperSideSurface),
                                pBoundary->SideSurfaceName(nSuperSideSurface).toStdString(), false);
          m_vcSurfaceSource.push_back(pBoundary);
          nSuperSideSurface++;
          assert(m_pMesh->NrOfSurfaces() == m_vcSurfaceSource.size());
        } else if (nIndex == -2) // Sub model side sidesurface
        {
          ...
          if (nSubSideSurface < pModel->SubBoundary().SideSurfaceSize())
            m_pMesh->AddSurface(pModel->SubBoundary().SideSurface(nSubSideSurface).Surface(),
                                pModel->SubBoundary().SideSurface(nSubSideSurface).Name().toStdString(), false);
          m_vcSurfaceSource.push_back(&pModel->SubBoundary());
          nSubSideSurface++;
          assert(m_pMesh->NrOfSurfaces() == m_vcSurfaceSource.size());
        }
  ...
  if (CStreamVersion(3, 0, 67) < version) {
    ...
    for (nElement = 0; nElement < TotalElements; nElement++) {
      ...
      if (nElementType == 0) { // tetrahedron
        ...
        progress.Step();
      } else if (nElementType == 1) { // contact element
      ...
      for (int nSurface = 0; nSurface < nSurfaceSize; nSurface++) {
        ...
        if (version < CStreamVersion(3, 0, 105)) {
          ...
          if (nSurfaceDescriptor < m_pMesh->NrOfSurfaces()) // Bug #1145
          {
            m_pMesh->RegisterTetSurface((geo::CSurfaceDesc &)m_pMesh->SurfaceDesc(nSurfaceDescriptor),
                                        (geo::CBodyGroup &)m_pMesh->Volume(nVolume), vcIndex2);
          }
        } else // version >= 3.0.105
        {
          ...
          if (nSurfaceDescriptor < m_pMesh->NrOfSurfaces()) {
            if (nIsTiedSurface) {
              geo::CTiedBodyTetSurface *pSurface = new geo::CTiedBodyTetSurface(
                  const_cast<geo::CSurfaceDesc &>(m_pMesh->SurfaceDesc(nSurfaceDescriptor)),
              ...
              geo::CTetSurface *pSurface =
                  new geo::CTetSurface(const_cast<geo::CSurfaceDesc &>(m_pMesh->SurfaceDesc(nSurfaceDescriptor)),
                                       const_cast<geo::CBodyGroup &>(m_pMesh->Volume(nVolume)));
    ...
    for (int nVolume = 0; nVolume < nVolumeSize; nVolume++) {
      if (CStreamVersion(3, 0, 3) < version) {
        ...
        if (nIndex > -1) {
          ...
          if (pVolume->Identifier().size() == 0)
            pVolume->Identifier(VolumeIdentity(m_pMesh->Volume(nIndex)));
        }
      } else {
        // Creating a volume
  ...
  for (int nVolume = 0; nVolume < m_pMesh->NrOfVolumes(); nVolume++) {
    ...
    for (int nSurface = 0; nSurface < volume.SideSurfaceSize(); nSurface++) {
      ...
      if (surface.isTiedSurface()) {
        ...
        // each face is defined by its parent index and its local face index, and by 3 tyings to points in interface
        // elements
...
const geo::IFace *CTetraMesh::LoadTiedPointBodyTriangle(TSTREAM &stream, CStreamVersion &version) {
  ...
  geo::CTiedPointBodyTriangle *tiedFace =
      new geo::CTiedPointBodyTriangle(const_cast<geo::IBody &>(*pBody), pTriangle->Index());
...
const geo::IElementSet &CTetraMesh::ElementSet() const { return *m_pMesh; }

geo::IElementSet &CTetraMesh::ElementSet() { return *m_pMesh; }

CTetraMesh::DIMENSION CTetraMesh::Dimension() const { return DIM_3D; }

unsigned int CTetraMesh::IconId() const { return IDI_MESH; }

unsigned int CTetraMesh::TypeId() const { return 0; }

int CTetraMesh::DisplayListSize() const { return 1; }

const geo::IObject &CTetraMesh::DisplayList(int /*nIndex*/) const { return *m_pMesh; }

const geo::IMesh &CTetraMesh::Mesh() const { return *m_pMesh; }
...
geo::IMesh &CTetraMesh::Mesh() { return *m_pMesh; }

#ifdef SKUA_NEW
// const std::vector<const gm_skua::SKUAParseData *>

bool CTetraMesh::ImportMeshFromGoCadFile(
    const QString &sPath, bool (*selectGocadSolidDlg)(const std::vector<const gm_skua::SKUAParseData *> &solids,
...
bool CTetraMesh::CreateMeshFromSolids(const QString &sPath, const std::vector<const gm_skua::SKUAParseData *> &solids,
...
bool CTetraMesh::CreateMeshFromSolids(
    const QString &sPath, const std::vector<const gm_skua::SKUAParseData *> &solids,
...
#else
bool CTetraMesh::ImportMeshFromGoCadFile(
    const QString &sPath, bool (*selectGocadSolidDlg)(const std::vector<QSharedPointer<CGocadData::CTSolid>> &solids,
                                                      std::vector<int> &vcSolids)) {
  CGocadImport gi(true, true);
...
bool CTetraMesh::CreateMeshFromSolids(const QString &sPath, std::vector<QSharedPointer<CGocadData::CTSolid>> &solids,
                                      const std::vector<int> &vcSolidIndices) {
...
bool CTetraMesh::CreateMeshFromSolids(
    const QString &sPath, std::vector<QSharedPointer<CGocadData::CTSolid>> &solids,
...
void CTetraMesh::GenerateSupportsForSurfaceDesc(const geo::CSurfaceDesc &surfacedesc, const CPressureSupportNode &pnode,
                                                int /*iRefNode*/, const geo::IVector &vecNormal,
...
const CHorizonBase *CTetraMesh::SlipHorizon(const geo::IElement &interface_element) const {
  ...
  CTetraSubHorizonEntry::TNodeSet stFaults =
      ((CTetraSubHorizonEntry *)(Model().GraphEntry(MD_TETRA_SUB_FAULT)))->EntryNodes();
  ...
  for (CTetraSubHorizonEntry::TNodeSet::iterator it = stFaults.begin(); it != stFaults.end(); it++) {
    ...
    for (int i = 0; i < (*it)->OutputSurfaceSize(); i++) {
      ...
      for (int j = 0; j < surfdesc.interfaceElements().ElementSize(); j++) {
        if (&interface_element == &surfdesc.interfaceElements().Element(j))
          return *it;
      }
    }
  }
...
const double &CTetraMesh::MaxGocadSurfaceAngle() const { return m_dMaxGocadSurfaceAngle; }

void CTetraMesh::MaxGocadSurfaceAngle(const double &dValue) { m_dMaxGocadSurfaceAngle = dValue; }

bool CTetraMesh::ImportedMesh() const { return m_bImportedMesh; }
  ...
  for (its = stFrontSurfaces.begin(); its != stFrontSurfaces.end(); ++its) {
    ...
    for (i = 0; i < (*its)->FaceSize(); ++i) {
      ...
      TPositionFacesMap::iterator it =
          mpPositionFaces.insert(TPositionFacesMap::value_type(TPositionTriangle(face), TFaceSet())).first;
        ...
        for (n = 0; n < sidesurface.PointSize(); ++n) {
          ...
          for (j = 0; j < vcBodies.size(); ++j) {
            ...
            geo::CTetrahedron &tet =
                const_cast<geo::CTetrahedron &>(static_cast<const geo::CTetrahedron &>(*vcBodies[j]));
...
bool CTetraMesh::CombineSurfaceNodes(geo::CSurfaceDesc &surfdesc) {
  ...
  for (its = stFrontSurfaces.begin(); its != stFrontSurfaces.end(); ++its) {
    for (int i = 0; i < (*its)->FaceSize(); ++i) {
      const geo::IFace &face = (*its)->Face(i);
      TPositionFacesMap::iterator it =
          mpPositionFaces.insert(TPositionFacesMap::value_type(TPositionTriangle(face), TFaceBodyPairSet())).first;
  ...
  for (itg = stBackGroups.begin(); itg != stBackGroups.end(); ++itg) {
    ...
    for (i = 0; i < bodygroup.SideSurfaceSize(); ++i) {
      ...
      if (stBackSurfaces.find(&sidesurface) != stBackSurfaces.end()) {
        ...
        std::pair<TSideSurfaceMap::iterator, bool> prInsert =
            mpRecreateSurfaces.insert(TSideSurfaceMap::value_type(&sidesurface, std::vector<int>()));
...
bool CTetraMesh::GetFrontAndBack(const geo::CSurfaceDesc &surfdesc, std::set<const geo::CBodyGroup *> &stFrontGroups,
                                 std::set<const geo::CBodyGroup *> &stBackGroups,
                                 std::set<const geo::CTetSurface *> &stFrontSurfaces,
...
orderSurfaces(const geo::CBodyGroup &bodyGroup, const std::set<const geo::CBodyGroup::CSideSurface *> &faultSurfaces) {
  ...
  for (int i = 0; i < bodyGroup.SideSurfaceSize(); ++i) {
    ...
    if ((sideSurfaces.size() > 1) && (faultSurfaces.find(sideSurfaces.back()) != faultSurfaces.end())) {
...
void CTetraMesh::SeparateBodyGroups(const geo::CBodyGroup &currentgroup,
  ...
  assert(stFrontGroups.find(&currentgroup) != stFrontGroups.end() ||
         stBackGroups.find(&currentgroup) != stBackGroups.end());
  ...
  std::vector<const geo::CBodyGroup::CSideSurface *> sideSurfaces = orderSurfaces(currentgroup, stFaultSurfaces);
