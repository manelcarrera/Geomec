...
CPressureSupportNode::CPressureSupportNode(const CPressureSupportNode &rhs) : C3DSupportNode(rhs) {}

CPressureSupportNode::~CPressureSupportNode() {}

bool CPressureSupportNode::Less(const CGraphNode &rhs) const {
  if (dynamic_cast<const CDisplacementSupportNode *>(&rhs) != 0)
    return true;
...
unsigned int CPressureSupportNode::IconId() const {
  if (Mesh().IsMesh()) {
    ...
    return IDI_FACESUPPORT_DISABLED;
}

bool CPressureSupportNode::Empty() const { return false; }

long CPressureSupportNode::SavedItems() const { return m_Supports.size() + C3DSupportNode::SavedItems(); }
...
void CPressureSupportNode::LoadStream(TSTREAM &stream, CStreamVersion &version, TPROGRESS &progress) {
  if (CStreamVersion(3, 0, 5) < version) // new in version 3.0.6
  {
    ...
    for (int i = 0; i < sz; i++) {
      ...
      std::pair<TStageSupportMap::iterator, bool> prInsert =
          m_Supports.insert(std::make_pair(sd, CFaceSupportDef(pModel->Mesh())));
...
void CPressureSupportNode::BuildStageSupportMap() const {
  ...
  for (size_t i = 0; i < m_vcSurfaces.size(); i++) {
    m_Supports.insert(
        std::make_pair(CSurfaceDef(*m_vcSurfaces[i].first), CFaceSupportDef(m_vcSurfaces[i].second, pModel->Mesh())));
  }
...
geo::CVector CPressureSupportNode::StressFromDistribution(const geo::IPoint &pt, const CDepletionStage &stage,
  ...
  if (pVT) {
    ...
    CTensor t(interpolated[0].Value(), interpolated[1].Value(), interpolated[2].Value(), interpolated[3].Value(),
              interpolated[4].Value(), interpolated[5].Value());
...
CPressureSupportNode::TSurfaceVec CPressureSupportNode::getSurfaces() const { return m_vcSurfaces; }

void CPressureSupportNode::setSelectedSurface(const geo::ISurface *selectedSurface) {
  m_pSelectedSurface = selectedSurface;
}

const geo::ISurface *CPressureSupportNode::getSelectedSurface() const { return m_pSelectedSurface; }

CPressureSupportNode::TStageSupportMap &CPressureSupportNode::getSupports() const { return m_Supports; }
...
CPressureSupportNode::CDefaultDrawDef::CDefaultDrawDef(const COpenGLNode &node) : CDrawDef(node) {}

CPressureSupportNode::CDefaultDrawDef::DrawDecisionFloat CPressureSupportNode::CDefaultDrawDef::LineWidth() const {
...
CPressureSupportNode::CFilledSurfaceDrawDef::CFilledSurfaceDrawDef(const COpenGLNode &node) : CDrawDef(node) {}
...
CPressureSupportNode::CFaceSupportDef::CSupportDef::CSupportDef() : m_nPressureType(PT_UNLOADED), m_vecLocal(0, 0, 0) {}
...
bool CPressureSupportNode::CFaceSupportDef::CSupportDef::operator==(const CSupportDef &rhs) const {
  if (m_nPressureType != rhs.m_nPressureType)
    return false;

  if (m_nPressureType == PT_LOCAL && m_vecLocal != rhs.m_vecLocal)
    return false;

  return true;
}

CPressureSupportNode::CFaceSupportDef::CSupportDef::TPressureType
CPressureSupportNode::CFaceSupportDef::CSupportDef::PressureType() const {
  return m_nPressureType;
}

void CPressureSupportNode::CFaceSupportDef::CSupportDef::PressureType(TPressureType nType) { m_nPressureType = nType; }

const geo::IVector &CPressureSupportNode::CFaceSupportDef::CSupportDef::LocalVector() const { return m_vecLocal; }

geo::IVector &CPressureSupportNode::CFaceSupportDef::CSupportDef::LocalVector() { return m_vecLocal; }

void CPressureSupportNode::CFaceSupportDef::CSupportDef::LoadStream(TSTREAM &stream, CStreamVersion & /*version*/,
                                                                    TPROGRESS & /*progress*/) {
  int dum;
  double x, y, z;
...
CPressureSupportNode::CFaceSupportDef::CFaceSupportDef(const CFaceSupportDef &rhs)
    : m_SupportMap(rhs.m_SupportMap), m_nSupportType(rhs.m_nSupportType),
      //  m_bPreScribedDisplacement(rhs.m_bPreScribedDisplacement),
      m_SurfaceLocation(rhs.m_SurfaceLocation), m_mesh(rhs.m_mesh) {}
...
CPressureSupportNode::CFaceSupportDef &CPressureSupportNode::CFaceSupportDef::operator=(const CFaceSupportDef &rhs) {
  ...
  if ((m_SurfaceLocation == SL_BOTTOM && rhs.m_nSupportType != ST_HORIZONTAL && rhs.m_nSupportType != ST_UNSUPPORTED &&
       rhs.m_nSupportType != ST_VERTICAL) ||
      (m_SurfaceLocation == SL_SIDE && rhs.m_nSupportType != ST_UNSUPPORTED && rhs.m_nSupportType != ST_VERTICAL) ||
      (m_SurfaceLocation == SL_TOP &&
       (rhs.m_nSupportType == ST_UNSUPPORTED || rhs.m_nSupportType == ST_VERTICAL || rhs.m_nSupportType == ST_FULL))) {
...
bool CPressureSupportNode::CFaceSupportDef::operator==(const CFaceSupportDef &rhs) const {
  ...
  if (m_SurfaceLocation != rhs.m_SurfaceLocation)
    return false;
  ...
  for (TSupportMap::const_iterator it = m_SupportMap.begin(); it != m_SupportMap.end(); it++) {
    ...
    if (itrhs == rhs.m_SupportMap.end())
      return false; // not even the same depletion stages
    if (!(it->second == itrhs->second))
      return false; // not the same supportdef
  }
...
const CPressureSupportNode::CFaceSupportDef::CSupportDef &
CPressureSupportNode::CFaceSupportDef::SupportDef(const CDepletionStage &stage) const {
...
CPressureSupportNode::CFaceSupportDef::CSupportDef &
CPressureSupportNode::CFaceSupportDef::SupportDef(const CDepletionStage &stage) {
...
void CPressureSupportNode::CFaceSupportDef::LoadStream(TSTREAM &stream, CStreamVersion &version, TPROGRESS &progress,
...
CPressureSupportNode::CSurfaceDef::CSurfaceDef(const geo::ISurface &s) : m_ptMin(s.Min()), m_ptMax(s.Max()) {}

bool CPressureSupportNode::CSurfaceDef::operator<(const CSurfaceDef &rhs) const {
  if (m_ptMin < rhs.m_ptMin)
    return true;
  if (rhs.m_ptMin < m_ptMin)
    return false;
...
void CPressureSupportNode::CSurfaceDef::LoadStream(TSTREAM &stream, CStreamVersion & /*version*/,
                                                   TPROGRESS & /*progress*/) {
  double x, y, z;
  stream >> x;
  stream >> y;
