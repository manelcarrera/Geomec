...
private:
  ...
  void GenerateSupportsForSurfaceDesc(const geo::CSurfaceDesc &surfacedesc, const CPressureSupportNode &pnode,
                                      int iRefNode, const geo::IVector &vecNormal, const geo::IVector &vecTangent);
  ...
  */
  ...
  bool GetFrontAndBack(const geo::CSurfaceDesc &surfdesc, std::set<const geo::CBodyGroup *> &stFrontGroups,
...
public:
  ...
  bool CreateMeshFromSolids(const QString &sPath, const std::vector<const gm_skua::SKUAParseData *> &solids,
...
#else
  bool
  ImportMeshFromGoCadFile(const QString &string,
                          bool (*selectGocadSolidDlg)(const std::vector<QSharedPointer<CGocadData::CTSolid>> &solids,
                                                      std::vector<int> &vcSolids));
  bool CreateMeshFromSolids(const QString &sPath, std::vector<QSharedPointer<CGocadData::CTSolid>> &solids,
                            const std::vector<int> &vcSolidIndices);
  bool CreateMeshFromSolids(const QString &sPath, std::vector<QSharedPointer<CGocadData::CTSolid>> &solids,
                            bool (*selectGocadSolidDlg)(const std::vector<QSharedPointer<CGocadData::CTSolid>> &solids,
                                                        std::vector<int> &vcSolids));
