...
public:
  ...
  bool GenerateIntermediateSurface(const QString &name, const TSurface &surf_0, const TSurface &surf_1,
                                   bool precise = false);
...
private:
  ...
  struct TNodeInfo {
    ...
    TNodeInfo() : duplicate_node(-1) { tmp_tying_infos.reserve(6); }
  ...
  void getTyingInformation(TNodeInfoMap &mpNodeInfo, const TSurface &surf_src, const TSurface &surf_dst,
                           bool precise = false, double eps = .1);
  void getIntermediateSurfaceInfo(IntermediateSurfaceInfo &surf_new, const TNodeInfoMap &mpNodeInfo_dst,
                                  const TSurface &surf_src, double distance = .5) const;

  void selectIntermediateSurfaceInfo(IntermediateSurfaceInfo &surf_minus, IntermediateSurfaceInfo &surf_plus,
                                     IntermediateSurfaceInfo &surf_selected, bool precise);
  void selectIntermediateSurfaceInfo(const QString &name, IntermediateSurfaceInfo &surf_minus,
                                     IntermediateSurfaceInfo &surf_plus, IntermediateSurfaceInfo &surf_selected,
                                     bool precise);

  geo::CSurfaceDesc *createSurface(const QString &name, const IntermediateSurfaceInfo &surf,
                                   const CTSSurface &mesh_surf);
  geo::CSurfaceDesc *createSurface(const QString &name, const IntermediateSurfaceInfo &surf);

  void duplicateNodes(TNodeInfoMap &mpNodeInfo, const geo::IElementSet &set);
  void createIFElements(TNodeInfoMap &mpMinus, TNodeInfoMap &mpPlus, const TSurface &surface,
                        geo::CSurfaceDesc &surf_desc, InterfaceInfo &if_info);
