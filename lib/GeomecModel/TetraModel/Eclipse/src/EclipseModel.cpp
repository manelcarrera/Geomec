...
CEclipseModel::CEclipseModel(CAnalysisLogger& logger, const CVersionManager& versionManager)
 : CTetraModel(logger, versionManager), m_nUpperHorizon(0), m_nLowerHorizon(0), m_pFrontSurface(0), m_pBackSurface(0),
   m_pLeftSurface(0), m_pRightSurface(0)
{
}
...
void CEclipseModel::InitFile(const CEclipseFile& file)
{
  ...
  m_vcHorizon.resize(file.HorizonSize(), THorizon(TEclipseHorizon(file.SizeX() + 1, TRow(file.SizeY() + 1)), (CTetraSubHorizon*)0));
  ...
  for(int i = 0; i < HorizonSize(); i++) HorizonEnable(i, true);
...
int CEclipseModel::ReservoirSize() const
{
  return m_vcReservoir.size();
}
...
int CEclipseModel::HorizonSize() const
{
  return m_vcHorizon.size();
}
...
void CEclipseModel::ExtractCells(const CEclipseReservoir *pReservoir)
{
  ...
  int i, j, k;
  for(i = pReservoir->LowerBoundary(); i < pReservoir->UpperBoundary(); i++)
  { // Loop over layers in rervoir

    for(j = 0; j < pReservoir->SizeX(); j++)
    { // Loop over columns in X direction in layer i
      
      for(k = 0; k < pReservoir->SizeY(); k++)
      { // Loop over cells in column j
...
unsigned int CEclipseModel::ModelIconId() const
{
  return staticModelIconId();
}

QString CEclipseModel::documentType() const
{
  return staticDocumentType();
}

CEclipseModel::ModelType CEclipseModel::modelType() const
{
  return eEclipseModel;
}


unsigned int CEclipseModel::staticModelIconId()
{
  return IDI_ECLIPSE_MODEL;
}

QString CEclipseModel::staticDocumentType()
{
  return "Eclipse";
}


void CEclipseModel::setFileName(const QString& fileName)
{
  m_fileName = fileName;
}
...
bool CEclipseModel::LoadEclipse(CStorageNode::TSTREAM &stream, CStreamVersion &/*version*/, CStorageNode::TPROGRESS &prog)
{
  int nHorizonSize;
  stream >> nHorizonSize;
