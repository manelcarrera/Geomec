...
template <typename V, typename G>
  class CGlobalValueObserver : public CGraphNode
{
public:
...
template <typename V, typename G>
  unsigned int CGlobalValueObserver <V, G> ::IconId() const
{
  return 0;
}

template <typename V, typename G>
  unsigned int CGlobalValueObserver <V, G> ::TypeId() const
{
  return 0;
}

template <typename V, typename G>
  void CGlobalValueObserver <V, G> ::OnNewNeighbour(const CGraphNode& /*node*/)
{
}
...
template <typename V, typename G>
  void CGlobalValueObserver <V, G> ::OnNeighbourDeleted(const CGraphNode& /*node*/)
{
}
