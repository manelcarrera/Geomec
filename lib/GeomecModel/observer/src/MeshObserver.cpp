...
unsigned int CMeshObserver::IconId() const
{
  return 0;
}

unsigned int CMeshObserver::TypeId() const
{
  return 0;
}

void CMeshObserver::OnNewNeighbour(const CGraphNode& node)
{
...
void CMeshObserver::OnNeighbourDeleted(const CGraphNode& node)
{
  CGraphNode::OnNeighbourDeleted(node);
}

CMeshBase* CMeshObserver::getMesh() const
{
  return m_Mesh;
}
