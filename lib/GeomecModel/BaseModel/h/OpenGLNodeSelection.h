...
template <class T> class CTypedSelection : public IOpenGLNodeSubSelection {
public:
  template <class TT> class Less {
  public:
  ...
  public:
    CConstIterator();
    CConstIterator(const IOpenGLNodeSubSelection &selection, const TContainer &container,
  ...
  public:
    CIterator();
    CIterator(const IOpenGLNodeSubSelection &selection, const TContainer &container,
              typename TContainer::const_iterator pos);
  ...
  template <class TT> class CFilter {
  public:
    virtual ~CFilter() {};
...
public:
  ...
  const COpenGLNodeSelection &getSelectionSource() const { return m_selectionSource; }
...
CTypedSelection<T>::CTypedSelection(COpenGLNodeSelection &selection, const CFilter<T> *pFilter)
    : m_selectionSource(selection), m_pFilter(pFilter) {
  if (!m_pFilter)
    m_pFilter = new CFilter<T>;
}

template <class T> CTypedSelection<T>::~CTypedSelection() { delete m_pFilter; }

template <class T> bool CTypedSelection<T>::IsSelected(const T &object) const { return find(object) != end(); }

template <class T> void CTypedSelection<T>::SetSelected(const T &object, bool bSelected) {
  if (bSelected)
    insert(object);
  else
    erase(object);
...
template <class T> bool CTypedSelection<T>::CanSelect(const COpenGLNode &object) const {
  return (dynamic_cast<const T *>(&object) != 0 && (*m_pFilter)(static_cast<const T &>(object)));
}

template <class T> bool CTypedSelection<T>::IsSelected(const COpenGLNode &object) const {
  return IsSelected(static_cast<const T &>(object));
}

template <class T> void CTypedSelection<T>::SetSelected(const COpenGLNode &object, bool bSelected) {
  assert(CanSelect(object));
  SetSelected(static_cast<const T &>(object), bSelected);
}

template <class T> typename CTypedSelection<T>::const_iterator CTypedSelection<T>::begin() const {
...
template <class T> typename CTypedSelection<T>::const_iterator CTypedSelection<T>::end() const {
  return CConstIterator(*this, m_selectionSource, m_selectionSource.end());
}

template <class T> typename CTypedSelection<T>::iterator CTypedSelection<T>::begin() {
  for (COpenGLNodeSelection::const_iterator it = m_selectionSource.begin(); it != m_selectionSource.end(); ++it) {
    if (CanSelect(**it))
...
template <class T> typename CTypedSelection<T>::iterator CTypedSelection<T>::end() {
  return CIterator(*this, m_selectionSource, m_selectionSource.end());
}

template <class T> std::pair<typename CTypedSelection<T>::iterator, bool> CTypedSelection<T>::insert(const T &object) {
...
template <class T> void CTypedSelection<T>::erase(iterator pos) {
  const T &object = *pos;
  m_selectionSource.erase(object);
}

template <class T> void CTypedSelection<T>::erase(const T &object) { m_selectionSource.erase(object); }

template <class T> typename CTypedSelection<T>::iterator CTypedSelection<T>::find(const T &object) {
  return CIterator(m_selectionSource, m_selectionSource.find(&object));
}

template <class T> typename CTypedSelection<T>::const_iterator CTypedSelection<T>::find(const T &object) const {
  return CConstIterator(*this, m_selectionSource, m_selectionSource.find(object));
}

template <class T> void CTypedSelection<T>::clear() {
  while (!empty())
    erase(begin());
}

template <class T> bool CTypedSelection<T>::empty() const { return begin() == end(); }

///// CTypedSelection::CConstIterator

template <class T> CTypedSelection<T>::CConstIterator::CConstIterator() : m_pSelection(0), m_pContainer(0) {}

template <class T>
CTypedSelection<T>::CConstIterator::CConstIterator(const IOpenGLNodeSubSelection &selection,
                                                   const TContainer &container, typename TContainer::const_iterator pos)
    : m_pSelection(&selection), m_pContainer(&container), m_pos(pos) {}

template <class T>
CTypedSelection<T>::CConstIterator::CConstIterator(const CConstIterator &rhs)
    : m_pSelection(rhs.m_pSelection), m_pContainer(rhs.m_pContainer), m_pos(rhs.m_pos) {}
...
template <class T> bool CTypedSelection<T>::CConstIterator::operator==(const CConstIterator &rhs) const {
  ...
  return (m_pSelection == rhs.m_pSelection && m_pContainer == rhs.m_pContainer && m_pos == rhs.m_pos);
}
...
template <class T> const T &CTypedSelection<T>::CConstIterator::operator*() {
  assert(dynamic_cast<const T *>(*m_pos));
  return static_cast<const T &>(**m_pos);
}

template <class T> const T *CTypedSelection<T>::CConstIterator::operator->() { return *m_pos; }

///// CTypedSelection::CIterator

template <class T> CTypedSelection<T>::CIterator::CIterator() {}

template <class T>
CTypedSelection<T>::CIterator::CIterator(const IOpenGLNodeSubSelection &selection, const TContainer &container,
                                         typename TContainer::const_iterator pos)
    : CTypedSelection<T>::CConstIterator(selection, container, pos) {}

template <class T>
CTypedSelection<T>::CIterator::CIterator(const CIterator &rhs) : CTypedSelection<T>::CConstIterator(rhs) {}
