...
public:
  ...
  public:
    CBoundaryDrawDef(const COpenGLNode &node) : CColorNode::CDrawDef(node) {}
  ...
  typedef struct _Rotated {
    ...
    _Rotated(const _Rotated &rhs)
        : ptMid(rhs.ptMid), dAzimuth(rhs.dAzimuth), dLength(rhs.dLength), dWidth(rhs.dWidth), dMinDepth(rhs.dMinDepth),
    ...
    _Rotated(const geo::IPoint &mid, double azimuth, double length, double width, double minDepth, double maxDepth)
        : ptMid(mid), dAzimuth(azimuth), dLength(length), dWidth(width), dMinDepth(minDepth), dMaxDepth(maxDepth) {}
...
public:
  ...
  CBoundaryBase(const geo::IPoint &ptMin, const geo::IPoint &ptMax, CFemAppModel &model,
  ...
  bool Set(const geo::IPoint &ptMid, double dAzimuth, double dLength, double dWidth, double dMinDepth,
           double dMaxDepth);
  ...
  virtual const CHorizonBase &GetTopHorizon() const {
    assert(false);
    CHorizonBase *pBogus = 0;
    return *pBogus;
  }
  virtual const CHorizonBase &GetBottomHorizon() const {
    assert(false);
    CHorizonBase *pBogus = 0;
    return *pBogus;
  }
  ...
  virtual std::vector<const geo::ISurface *> GetSideMeshSurfaces() const {
    assert(false);
    return std::vector<const geo::ISurface *>();
  };
