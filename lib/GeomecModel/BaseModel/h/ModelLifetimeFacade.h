...
//
// This makes it difficult to add decorators in a single place (for example, resetting singletons for a new(-ly loaded) project
//
...
public:
  ...
  static CModelBase *NewChild(int type, CWellZoomInModel& parent, CAnalysisLogger& logger, const CVersionManager& versionManager);
  static CModelBase *NewChild(int type, CModelBase& parent, CNewWellPath& wp, CAnalysisLogger& logger, const CVersionManager& versionManager);
  static CModelBase *NewChild(int type, CModelBase& parent, CWellPath& wp, CAnalysisLogger& logger, const CVersionManager& versionManager);

  static CModelBase* LoadMain(const CModelBase::IModelLoadSave& modelLoadSave, const QString& strFileName, CAnalysisLogger& logger, bool bSilent = false);
  static CModelBase* LoadChild(const CModelBase::IModelLoadSave& modelLoadSave, const QString& strFileName, CAnalysisLogger& logger);
  static bool SaveMain(const CModelBase::IModelLoadSave& modelLoadSave, CModelBase& model, const QString& strFileName, bool bSwitch);
