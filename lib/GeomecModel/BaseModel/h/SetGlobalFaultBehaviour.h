...
public:
  ...
  enum { DEPTH = 0, PRESSURE = 1, GRADIENT = 2 };

  std::vector<QString> getUnitNames(const CUnitNode &unitNode) const;
  bool getModeType(CFaultPressure::TModeType &modeType) const;
  bool getValues(std::vector<double> &values, const CUnitNode &unitNode) const;

  void setGlobalFaultBehaviour(const std::vector<double> &values, const CUnitNode &unitNode,
