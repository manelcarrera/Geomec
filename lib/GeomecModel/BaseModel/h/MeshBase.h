...
public:
  ...
  double operator()(const double &x) const { return m_Multiplier * x; }
};
...
class CMeshBase : public IElementSet {
  ...
  mutable std::vector<const IFormationElementSet *>
      m_vcElementToFormationSet; // indexed map from element index to formation
