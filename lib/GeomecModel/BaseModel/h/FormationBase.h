...
//	void InitialPressure(bool bInitialPressure);
  ...
  // must be overridden
  virtual geo::CValue ThicknessAt(const geo::IPoint& /*pt*/, geo::IPoint& /*ptTop*/, geo::IPoint& /*ptBottom*/, geo::IParallelInitializationCallback * /*cb*/) const { assert(false); return geo::CValue(); }
...
*/
  ...
  geo::CValue ThicknessAt(const geo::IPoint& pt, geo::IPoint& ptTop, geo::IPoint& ptBottom, geo::IParallelInitializationCallback *cb) const;
...
class CFormationEntryTempl : public CStorageNodeEntry<FORMATION>
{
public:
...
: CStorageNodeEntry<FORMATION>(nEntryId, IDI_FORMATIONS, IDS_TREE_FORMATIONS, model),
  m_ShowState(FULL)
{
}

template <class FORMATION>
bool CFormationEntryTempl<FORMATION>::CanExportGocad() const
{
  return ((static_cast<const CModelBase&>(this->Model())).Mesh().IsMesh());
}

template <class FORMATION>
bool CFormationEntryTempl<FORMATION>::ShowSkin() const
{
	return m_ShowState == SKIN;
}

template <class FORMATION>
void CFormationEntryTempl<FORMATION>::OnShowSkin()
{
	if(m_ShowState != SKIN)
	{
		m_ShowState = SKIN;
...
template <class FORMATION>
bool CFormationEntryTempl<FORMATION>::ShowMidpoints() const
{
	return m_ShowState == MIDPOINTS;
}

template <class FORMATION>
void CFormationEntryTempl<FORMATION>::OnShowMidpoints()
{
...
template <class FORMATION>
bool CFormationEntryTempl<FORMATION>::ShowFullMesh() const
{
	return m_ShowState == FULL;
}

template <class FORMATION>
void CFormationEntryTempl<FORMATION>::OnShowFullMesh()
{
	if(m_ShowState != FULL)
	{
		m_ShowState = FULL;
...
template <class FORMATION>
void CFormationEntryTempl<FORMATION>::UpdateFormationElementSets()
{
	// Update all formation volumes
  COperation oper(CStorageNodeEntry <FORMATION> ::Model());

	typename CStorageNodeEntry <FORMATION> ::TNodeSet stFormation =
		CStorageNodeEntry <FORMATION> ::EntryNodes();
	for(typename CStorageNodeEntry <FORMATION> ::TNodeSet::iterator it =
		stFormation.begin(); it != stFormation.end(); it++)
	{
...
//typedef COpenGLNodeObserver_Delegate<CFormationBase, CFormationBase_Delegate, CDummyNode, CDummyObserver, false, FIXED_ITEM> TFormationObserver;
