...
CMeasuredTopDisplacementsNode::CMeasuredTopDisplacementsNode(const QString &sName, C3DModel &model)
    : CStorageNode(sName, model), m_pDisplaVector(0) {}
...
unsigned int CMeasuredTopDisplacementsNode::IconId() const { return IDI_MEASURED_DISPLACEMENT; }

unsigned int CMeasuredTopDisplacementsNode::TypeId() const { return 0; }

bool CMeasuredTopDisplacementsNode::Empty() const { return false; }

long CMeasuredTopDisplacementsNode::SavedItems() const { return 1; }
...
void CMeasuredTopDisplacementsNode::LoadStream(TSTREAM &stream, CStreamVersion & /*version*/, TPROGRESS &progress) {
  ...
  if (iVectorIndex >= 0) {
    ...
    TValueCompositeEntry &composite_entry =
        static_cast<TValueCompositeEntry &>(*model.RootModel().GraphEntry(MD_BASE_VALUE_COMPOSITE));
...
const TDisplacementVector *CMeasuredTopDisplacementsNode::DisplacementVector() const { return m_pDisplaVector; }
