...
CDepletionStage::CDepletionStage(CDepletionStageEntry &entry)
    : CStorageNode(entry.Model()), m_pPrevious(0), m_pNext(0),
      m_Time(QDateTime::currentDateTime().date().year(), QDateTime::currentDateTime().date().month()), m_entry(entry),
      m_bMarkedAsInitial(false), m_output_type(DefaultOutputType()) {
  SetDefaultNonLinParams();
  Index(-1);
}

// ##ModelId=3BC55D630297
CDepletionStage::CDepletionStage(const QString &strName, CDepletionStageEntry &entry, int nSteps,
                                 const double &dStepScalingFactor, const double &dConvNormVal, int nIterationSteps,
                                 eConvergenceType convergence_type, eIterationScheme iteration_scheme,
                                 eOutputType output_type, bool bAutoStepSizes, double dMaxAutoStepSize)
    : CStorageNode(strName, entry.Model()), m_pPrevious(0), m_pNext(0),
      m_Time(QDateTime::currentDateTime().date().year(), QDateTime::currentDateTime().date().month()), m_entry(entry),
      m_bMarkedAsInitial(false), m_nSteps(nSteps), m_dStepScalingFactor(dStepScalingFactor),
      m_dConvNormVal(dConvNormVal), m_nIterationSteps(nIterationSteps), m_convergence_type(convergence_type),
      m_iteration_scheme(iteration_scheme), m_output_type(output_type), m_bAutoStepSizes(bAutoStepSizes),
...
CDepletionStage::CDepletionStage(const QString &strName, CDepletionStage &previous, int nSteps,
                                 const double &dStepScalingFactor, const double &dConvNormVal, int nIterationSteps,
                                 eConvergenceType convergence_type, eIterationScheme iteration_scheme,
                                 eOutputType output_type, bool bAutoStepSizes, double dMaxAutoStepSize)
    : CStorageNode(strName, previous.Model()), m_pPrevious(&previous), m_pNext(0),
      m_Time(previous.Time().GetYear() + 1, previous.Time().GetMonth()), m_entry(previous.Entry()),
      m_bMarkedAsInitial(false), m_nSteps(nSteps), m_dStepScalingFactor(dStepScalingFactor),
      m_dConvNormVal(dConvNormVal), m_nIterationSteps(nIterationSteps), m_convergence_type(convergence_type),
      m_iteration_scheme(iteration_scheme), m_output_type(output_type), m_bAutoStepSizes(bAutoStepSizes),
      m_dMaxAutoStepSize(dMaxAutoStepSize) {
...
CDepletionStage::CDepletionStage(const QString &strName, CDepletionStage *previous, CDepletionStage *next,
                                 const CGeomecTime &time, int nSteps, const double &dStepScalingFactor,
                                 const double &dConvNormVal, int nIterationSteps, eConvergenceType convergence_type,
                                 eIterationScheme iteration_scheme, eOutputType output_type, bool bAutoStepSizes,
                                 double dMaxAutoStepSize)
    : CStorageNode(strName, next->Model()), m_pPrevious(previous), m_pNext(next), m_Time(time), m_entry(next->Entry()),
      m_bMarkedAsInitial(false), m_nSteps(nSteps), m_dStepScalingFactor(dStepScalingFactor),
      m_dConvNormVal(dConvNormVal), m_nIterationSteps(nIterationSteps), m_convergence_type(convergence_type),
      m_iteration_scheme(iteration_scheme), m_output_type(output_type), m_bAutoStepSizes(bAutoStepSizes),
      m_dMaxAutoStepSize(dMaxAutoStepSize) {
...
CDepletionStage::CDepletionStage(const CDepletionStage &rhs)
    : CStorageNode(rhs), m_pPrevious(rhs.m_pPrevious), m_pNext(rhs.m_pNext), m_Time(rhs.m_Time), m_entry(rhs.m_entry),
      m_bMarkedAsInitial(rhs.m_bMarkedAsInitial), m_nSteps(rhs.m_nSteps),
      m_dStepScalingFactor(rhs.m_dStepScalingFactor), m_dConvNormVal(rhs.m_dConvNormVal),
      m_nIterationSteps(rhs.m_nIterationSteps), m_convergence_type(rhs.m_convergence_type),
      m_iteration_scheme(rhs.m_iteration_scheme), m_output_type(rhs.m_output_type),
      m_bAutoStepSizes(rhs.m_bAutoStepSizes), m_dMaxAutoStepSize(rhs.m_dMaxAutoStepSize) {}

const CDepletionStageEntry &CDepletionStage::Entry() const { return m_entry; }

CDepletionStageEntry &CDepletionStage::Entry() { return m_entry; }
...
CDepletionStage &CDepletionStage::operator=(const CDepletionStage &rhs) {
  ...
  if ((m_Time != rhs.m_Time) || (m_nSteps != rhs.m_nSteps) || (m_dStepScalingFactor != rhs.m_dStepScalingFactor) ||
      (m_dConvNormVal != rhs.m_dConvNormVal) || (m_nIterationSteps != rhs.m_nIterationSteps) ||
      (m_convergence_type != rhs.m_convergence_type) || (m_iteration_scheme != rhs.m_iteration_scheme) ||
      (m_bAutoStepSizes != rhs.m_bAutoStepSizes) || (fabs(m_dMaxAutoStepSize - rhs.m_dMaxAutoStepSize) >= 1e-6)) {
...
bool CDepletionStage::operator==(const CDepletionStage &rhs) const {
  ...
  return ((m_Time == rhs.m_Time) && (m_nSteps == rhs.m_nSteps) && (m_dStepScalingFactor == rhs.m_dStepScalingFactor) &&
          (m_dConvNormVal == rhs.m_dConvNormVal) && (m_nIterationSteps == rhs.m_nIterationSteps) &&
          (m_convergence_type == rhs.m_convergence_type) && (m_iteration_scheme == rhs.m_iteration_scheme) &&
          (m_bAutoStepSizes == rhs.m_bAutoStepSizes) && (fabs(m_dMaxAutoStepSize - rhs.m_dMaxAutoStepSize) < 1e-6));
}

void CDepletionStage::SetNonLinParams(bool bAutoStepSizes, double dMaxAutoStepSize, int nSteps,
                                      const double &dStepScalingFactor, const double &dConvNormVal, int nIterationSteps,
                                      eConvergenceType convergence_type, eIterationScheme iteration_scheme) {
...
int CDepletionStage::NLSteps() const { return m_nSteps; }

const double &CDepletionStage::NLStepScalingFactor() const { return m_dStepScalingFactor; }

const double &CDepletionStage::NLConvNormVal() const { return m_dConvNormVal; }

int CDepletionStage::NLIterationSteps() const { return m_nIterationSteps; }

CDepletionStage::eConvergenceType CDepletionStage::NLConvergenceType() const { return m_convergence_type; }

CDepletionStage::eIterationScheme CDepletionStage::NLIterationScheme() const { return m_iteration_scheme; }

void CDepletionStage::setNLIterationScheme(eIterationScheme itScheme) { m_iteration_scheme = itScheme; }

bool CDepletionStage::AutomaticStepSizes() const { return (/*Initial() ? false : */ m_bAutoStepSizes); }

double CDepletionStage::MaxAutomaticStepSize() const { return m_dMaxAutoStepSize; }
...
CDepletionStage::eOutputType CDepletionStage::OutputType() const { return m_output_type; }
...
bool CDepletionStage::Initial() const { return !m_pPrevious; }

// ##ModelId=3BC55D6302C5
bool CDepletionStage::Last() const { return !m_pNext; }
...
unsigned int CDepletionStage::TypeId() const { return 0; }

// ##ModelId=3BE7AEC5011E
const CGeomecTime &CDepletionStage::Time() const { return m_Time; }
...
bool CDepletionStage::Empty() const { return ((m_pPrevious == 0) && (m_pNext == 0)); }
...
void CDepletionStage::LoadStream(TSTREAM &stream, CStreamVersion &version, TPROGRESS &progress) {
  ...
  if (version < CStreamVersion(3, 0, 33)) {
    ...
    if (nYear < 1900)
      nYear += 2000;
    if (!Initial() && nYear <= Previous().Time().GetYear())
      nYear = Previous().Time().GetYear() + 1;
    m_Time.Set(nYear, 1);
  } else if (version < CStreamVersion(3, 0, 58)) {
    QDateTime temp;
    stream >> temp;
    m_Time.Set(temp.date().year(), temp.date().month());
  } else if (version < CStreamVersion(3, 7, 26) ||
             (version >= CStreamVersion(3, 8, 0) && version < CStreamVersion(3, 8, 3))) {
    int nYear, nMonth;
    stream >> nYear;
    stream >> nMonth;
...
long CDepletionStage::SavedItems() const { return 1; }
...
CDepletionStage::eOutputType CDepletionStage::DefaultOutputType() { return RESULTS; }

const CDepletionStage &CDepletionStage::InitialStage() const { return m_entry.InitialStage(); }

CDepletionStage &CDepletionStage::InitialStage() { return m_entry.InitialStage(); }

const CDepletionStage &CDepletionStage::LastStage() const { return m_entry.LastStage(); }

CDepletionStage &CDepletionStage::LastStage() { return m_entry.LastStage(); }
...
bool CDepletionStage::IsMarkedAsInitial() const { return m_bMarkedAsInitial; }

bool CDepletionStage::CanMarkAsInitial() const { return !Last() && !m_bMarkedAsInitial; }
...
const CDepletionStage &CDepletionStage::GetNextBranchStage(const CDepletionStage &stage) const {
  const CDepletionStage *pStage = &stage;

  while (pStage) {
    CDepletionStage::eOutputType outputtype = pStage->OutputType();

    if (outputtype == CDepletionStage::BRANCH || outputtype == CDepletionStage::PHASE) {
      break;
    }
...
std::string CDepletionStage::ModifiedFileName(const std::string &fileName) const {
  ...
  if (iDot >= 0) {
    return (strFile.left(iDot) + strPostfix + strFile.right(strFile.length() - iDot)).toStdString();
  }
...
CConstDepletionStageIterator::CConstDepletionStageIterator() : m_pStage(0) {}

CConstDepletionStageIterator::CConstDepletionStageIterator(const CDepletionStage &stage) : m_pStage(&stage) {}
...
CDepletionStageIterator::CDepletionStageIterator(CDepletionStage &stage) : CConstDepletionStageIterator(stage) {}
...
void CDepletionStageEntry::LoadStream(CStorageNode::TSTREAM &stream, CStreamVersion &version,
...
CDepletionStageEntry::const_iterator CDepletionStageEntry::end() const { return CConstDepletionStageIterator(); }

CDepletionStageEntry::iterator CDepletionStageEntry::begin() { return CDepletionStageIterator(*m_pFirstStage); }

CDepletionStageEntry::iterator CDepletionStageEntry::end() { return CDepletionStageIterator(); }
...
int CDepletionStageEntry::StageIndexFromName(QString strName) const {
  ...
  if (idxUnderscore > 0) {
    ...
    if (idxUnderscore == strName.length() - 2 &&
        (strName[strName.length() - 1] == 'L' || strName[strName.length() - 1] == 'N' ||
         strName[strName.length() - 1] == 'M' || strName[strName.length() - 1] == 'H' ||
