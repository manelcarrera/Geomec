...
C3DHorizon::CElementSet::CTriangle::CTriangle(const CElementSet &elementset, const geo::IPoint &p1,
                                              const geo::IPoint &p2, const geo::IPoint &p3, int nIndex)
    : m_elementset(elementset), m_vcPoint(3), m_nIndex(nIndex) {
  m_vcPoint[0] = &p1;
  m_vcPoint[1] = &p2;
  m_vcPoint[2] = &p3;
}

const geo::IElementSet *C3DHorizon::CElementSet::CTriangle::IndexingElementSet() const { return &m_elementset; }
...
void C3DHorizon::CElementSet::CTriangle::Point(int /*nIndex*/, const geo::IPoint & /*pt*/) { assert(false); }

int C3DHorizon::CElementSet::CTriangle::NrOfPoints() const { return m_vcPoint.size(); }

size_t C3DHorizon::CElementSet::CTriangle::Order() const { return 1; }
...
int C3DHorizon::CElementSet::CTriangle::Index() const { return m_nIndex; }

/////

C3DHorizon::CElementSet::CElementSet(const C3DHorizon &horizon) : m_horizon(horizon) {
...
int C3DHorizon::CElementSet::PointSize() const { return m_vcPoint.size(); }
...
int C3DHorizon::CElementSet::ElementSize() const { return m_vcElement.size(); }
  ...
  if (element.NrOfPoints() == 4) {
    ...
    for (i = 0; i < 4; ++i) {
      TPointMap::iterator it =
          m_mpOwnPoints.insert(TPointMap::value_type(&element.Point(i), geo::CPoint(element.Point(i)))).first;
      points[i] = &it->second;
    }
...
C3DHorizon::C3DHorizon(CFemAppModel &model)
    : CHorizonBase(model), m_pConstDepthSurface(0), m_pDepth(0), m_nDivisionNorthing(4), m_nDivisionEasting(4),
...
C3DHorizon::C3DHorizon(const QString &strInstanceName, const double &dDepth, CFemAppModel &model, bool bAttachToEntry)
    : CHorizonBase(strInstanceName, model, bAttachToEntry), m_pConstDepthSurface(0), m_pDepth(0),
      m_nDivisionNorthing(4), m_nDivisionEasting(4), m_pElementSetCache(0) {
...
C3DHorizon::C3DHorizon(CSurfaceBase &surface, CFemAppModel &model, bool bSlip, bool bAttachToEntry)
    : CHorizonBase(surface.Name().toStdString().c_str(), model, bAttachToEntry), m_pConstDepthSurface(0), m_pDepth(0),
      m_nDivisionNorthing(4), m_nDivisionEasting(4), m_pElementSetCache(0) {
  LinkTo(surface);
...
C3DHorizon::C3DHorizon(const C3DHorizon &rhs)
    : CHorizonBase(rhs), m_vcSurfaces(rhs.m_vcSurfaces), m_pConstDepthSurface(0), m_pDepth(0),
      m_nDivisionNorthing(rhs.m_nDivisionNorthing), m_nDivisionEasting(rhs.m_nDivisionEasting), m_pElementSetCache(0) {
...
int C3DHorizon::SurfaceSize() const { return m_vcSurfaces.size(); }
...
bool C3DHorizon::ConstantDepth() const { return m_pDepth != 0; }
/*
...
bool C3DHorizon::operator==(const C3DHorizon &rhs) const {
  ...
  if (ConstantDepth()) {
    if ((Depth().Value() != rhs.Depth().Value()) || (m_nDivisionNorthing != rhs.m_nDivisionNorthing) ||
...
C3DHorizon &C3DHorizon::operator=(const C3DHorizon &rhs) {
  ...
  bool bInvalidateMesh(
      (m_vcSurfaces != rhs.m_vcSurfaces) || (Slip() != rhs.Slip()) || (ConstantDepth() != rhs.ConstantDepth()) ||
      (m_pDepth && rhs.m_pDepth &&
       ((m_pDepth->Value() != rhs.m_pDepth->Value()) || (m_nDivisionNorthing != rhs.m_nDivisionNorthing) ||
        (m_nDivisionEasting != rhs.m_nDivisionEasting))));
...
int C3DHorizon::DivisionNorthing() const { return m_nDivisionNorthing; }

int C3DHorizon::DivisionEasting() const { return m_nDivisionEasting; }
...
void C3DHorizon::OnCreateConstantDepthSurface() const {
  ...
  for (int x = 0; x < (m_nDivisionNorthing + 1); x++) {
    for (int y = 0; y < (m_nDivisionEasting + 1); y++) {
      vcPoint.PushBack(
          *(new geo::CPoint(ptMin.X() + ((ptMax.X() - ptMin.X()) * x) / m_nDivisionNorthing,
                            ptMin.Y() + ((ptMax.Y() - ptMin.Y()) * y) / m_nDivisionEasting, Depth().Value())));
    }
  }
...
long C3DHorizon::SavedItems() const { return CColorNode::SavedItems() + 1; }
...
const dia::IMaterial &C3DHorizon::InterfaceMaterial(const geo::IElement &element, const CDepletionStage &stage) const {
  ...
  for (size_t i = 0; i < vcCohesion.size(); i++)
    dAvCohesion += vcCohesion[i];
  dAvCohesion /= vcCohesion.size();

  // use average value inside the element
  double dAvFriction = 0;
  for (size_t i = 0; i < vcFriction.size(); i++)
    dAvFriction += vcFriction[i];
  ...
  CInterfaceElementMaterial *pMat =
      new CInterfaceElementMaterial(dAvCohesion, dAvFriction, dDStiffNormal, dDStiffShear, SlipType());
...
double C3DHorizon::CalculateAverageDStif() const {
  ...
  for (i = 0; i < pInterfaces->ElementSize(); ++i) {
    ...
    if (pIface->FrontFace()->Parent()) {
      ...
      dYoung1 =
          For1.ActiveMaterial().Material(*pIface->FrontFace()->Parent()).ParameterValue(IDT_VALUETYPE_YOUNGS_MODULUS);
    } else // TiedBodyTriangle
    {
      assert(dynamic_cast<const geo::CTiedBodyTriangle *>(pIface->FrontFace()));
      const geo::CTiedBodyTriangle *pTiedBodyTriangle =
          static_cast<const geo::CTiedBodyTriangle *>(pIface->FrontFace());

      for (int j = 0; j < 3; ++j) {
        const CFormationBase &For1 = *model.Mesh().Formation(*pTiedBodyTriangle->BodyTriangle(j)->Parent());
        dYoung1 += For1.ActiveMaterial()
                       .Material(*pTiedBodyTriangle->BodyTriangle(j)->Parent())
                       .ParameterValue(IDT_VALUETYPE_YOUNGS_MODULUS);
      }
    ...
    if (pIface->BackFace()->Parent()) {
      ...
      dYoung2 =
          For2.ActiveMaterial().Material(*pIface->BackFace()->Parent()).ParameterValue(IDT_VALUETYPE_YOUNGS_MODULUS);
    } else // TiedBodyTriangle
    {
