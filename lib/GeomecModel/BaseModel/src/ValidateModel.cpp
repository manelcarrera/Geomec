...
CValidateModel::CValidateModel(CModelBase* modelBase)
...
bool CValidateModel::checkMesh() const
{
...
{
  TFormationBaseEntry* pEntry = static_cast <TFormationBaseEntry*> (
    m_ModelBase->GraphEntry(MD_BASE_FORMATION));
  TFormationBaseEntry::TNodeSet stFormation = pEntry->EntryNodes();
  int nDepleting = 0;
  for (TFormationBaseEntry::TNodeSet::iterator it = stFormation.begin();
    it != stFormation.end(); it++)
  {
    if((*it)->Depleting())
    {
      nDepleting++;
...
bool CValidateModel::checkExistenceDepletionStage() const
{
  return (m_ModelBase->NrOfDepletionStages() - 1 > 0);
}
...
bool CValidateModel::checkBoundaryConditions() const
{
  ...
  {
    if (collectNormalVectors(pTetHorBas->OutputSurface(i), vcNormals,
      pTetBound))
    {
      return true;
    }
  ...
  {
    if (collectNormalVectors(pTetBound->SideSurfaceDesc(i), vcNormals,
      pTetBound))
    {
      return true;
    }
...
bool CValidateModel::checkNoEmptyFormations() const
{
  ...
  // Get the formations
  CTetraFormationEntry* pEntry = static_cast <CTetraFormationEntry*> (
    m_ModelBase->GraphEntry(MD_TETRA_FORMATION));
  assert(pEntry);
  CTetraFormationEntry::TNodeSet stNode = pEntry->EntryNodes();
  for(CTetraFormationEntry::TNodeSet::const_iterator it = stNode.begin();
    it != stNode.end(); it++)
  {
...
bool CValidateModel::collectNormalVectors(const geo::CSurfaceDesc& Desc,
...
void CValidateModel::addNorm(const geo::ISurface& surf,
  std::vector <geo::CVector>& normvec) const
{
  ...
  if(vecsize > 1)
  { // two vectors present and the reference point lies in the same plane as the two vectors
    assert(vecsize == 2);
