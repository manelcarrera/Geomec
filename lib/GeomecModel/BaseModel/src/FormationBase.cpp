...
IFormationElementSet::IFormationElementSet(CFormationBase &formation, bool bDoLink)
    : IElementSet(formation.Name().toStdString().c_str(), formation.Model(), CQuantity::SI_UNIT, CQuantity::SI_UNIT),
      m_pFormation(&formation), m_pElementCenters(0) {
...
IFormationElementSet::IFormationElementSet(const IFormationElementSet &rhs)
    : IElementSet(rhs), m_pFormation(rhs.m_pFormation), m_pElementCenters(0) {}

IFormationElementSet::~IFormationElementSet() { ClearElementCenterCache(); }
...
const CFormationBase &IFormationElementSet::Formation() const { return *m_pFormation; }

CFormationBase &IFormationElementSet::Formation() { return *m_pFormation; }
...
int IFormationElementSet::Resemblance(const TGraphNodeSet &identifier) const {
  ...
  std::set_intersection(m_identifier.begin(), m_identifier.end(), identifier.begin(), identifier.end(),
...
CFormationBase::CFormationBase(CFemAppModel &model)
    : TBase(model), m_MinThickness(1, 0.01, 100000, CQuantity::SI_UNIT), m_pParent(0), m_FormationType(FT_NOTSPECIFIED),
...
CFormationBase::CFormationBase(const CFormationBase &rhs)
    : TBase(rhs), m_vcElementSet(rhs.m_vcElementSet), m_mpPressure(rhs.m_mpPressure),
      m_mpFractureMatrixPressure(rhs.m_mpFractureMatrixPressure), m_mpTemperature(rhs.m_mpTemperature),
      m_mpCalcTemper(rhs.m_mpCalcTemper), m_MinThickness(rhs.m_MinThickness.Value(), 0.01, 100000, CQuantity::SI_UNIT),
      m_pParent(rhs.m_pParent), m_FormationType(rhs.m_FormationType), m_pThickness(rhs.m_pThickness),
      m_formationElementOrder(rhs.m_formationElementOrder) {}

CFormationBase::CFormationBase(const QString &strName, CModelBase &model)
    : TBase(strName, model), m_MinThickness(1, 0.01, 100000, CQuantity::SI_UNIT), m_pParent(0),
      m_FormationType(FT_NOTSPECIFIED), m_formationElementOrder(model, true) {
...
template <typename LOAD, typename LOAD_MAP> void unLinkLoads(CFormationBase &formation, LOAD_MAP &loadMap) {
  std::vector<LOAD *> loads;

  for (typename LOAD_MAP::iterator iterator = loadMap.begin(); iterator != loadMap.end(); ++iterator) {
    loads.push_back(iterator->second);
  }

  for (typename std::vector<LOAD *>::iterator load = loads.begin(), end = loads.end(); load != end; ++load) {
...
CFormationBase::~CFormationBase() {
  if (!IsCopy()) {
    ...
    unLinkLoads<CFractureMatrixPressure, TFractureMatrixPressureMap>(*this, m_mpFractureMatrixPressure);
    unLinkLoads<CTemperature, TTemperatureMap>(*this, m_mpTemperature);
    unLinkLoads<CCalculatedTemperature, TCalcTemperMap>(*this, m_mpCalcTemper);
...
const CFormationBase *CFormationBase::ParentFormation() const { return m_pParent; }
  ...
  if (Model().parentModel() && !m_pParent) {
    TFormationBaseEntry::TNodeSet stNodes =
        static_cast<TFormationBaseEntry *>(
            static_cast<CModelBase *>(Model().parentModel())->GraphEntry(MD_BASE_FORMATION))
            ->EntryNodes();
...
CFormationBase::TFormationType CFormationBase::FormationType() const { return m_FormationType; }

void CFormationBase::FormationType(CFormationBase::TFormationType newtype) { m_FormationType = newtype; }

bool CFormationBase::operator==(const CFormationBase &rhs) const {
  if (!TBase::operator==(rhs))
    return false;

  return (m_FormationType == rhs.m_FormationType && m_MinThickness.Value() == rhs.m_MinThickness.Value());
}

void CFormationBase::Export(const QString &fileName, const IExportFormat::CExportArg &arg) {
  if (fileName.right(4).compare(".xls", Qt::CaseInsensitive) == 0 ||
      fileName.right(5).left(4).compare(".xls", Qt::CaseInsensitive) == 0) {
    CElementSetExportFormat format(*this, CElementSetExportFormat::TDataVec(), CElementSetExportFormat::TDataVec(),
                                   arg);
...
void CFormationBase::OnNeighbourDeleted(const CGraphNode &node) {
  ...
  for (TFractureMatrixPressureMap::iterator itd = m_mpFractureMatrixPressure.begin();
       itd != m_mpFractureMatrixPressure.end(); itd++) {
    if (itd->second == &node) {
...
void CFormationBase::OnNewNeighbour(const CGraphNode &node) {
  ...
  if (pPressure) {
    VERIFY(
        m_mpPressure.insert(TPressureMap::value_type(&pPressure->DepletionStage(), (CPressure *)(pPressure))).second);
    assert(pPressure->Formation() == this);
  }

  if (pFractureMatrixPressure) {
    VERIFY(m_mpFractureMatrixPressure
               .insert(TFractureMatrixPressureMap::value_type(&pFractureMatrixPressure->DepletionStage(),
                                                              (CFractureMatrixPressure *)(pFractureMatrixPressure)))
               .second);
    assert(pFractureMatrixPressure->Formation() == this);
  }

  if (pTemperature) {
    VERIFY(m_mpTemperature
               .insert(TTemperatureMap::value_type(&pTemperature->DepletionStage(), (CTemperature *)(pTemperature)))
               .second);
  ...
  if (pCalcTemper) {
    VERIFY(
        m_mpCalcTemper
            .insert(TCalcTemperMap::value_type(&pCalcTemper->DepletionStage(), (CCalculatedTemperature *)(pCalcTemper)))
            .second);
  ...
  /*
    if(pMatServer)
    {
    VERIFY(m_mpMaterial.insert(TMaterialMap::value_type(&pMatServer->Stage(),
    const_cast<CMaterialServer*>(pMatServer))).second);
    }
  */
    ...
    for (CDepletionStageEntry::iterator itd = entry.begin(); itd != entry.end(); ++itd) {
      ...
      if (m_mpPressure.find(&*itd) == m_mpPressure.end()) {
        ...
        if (m_mpPressure.find(&*previous) != m_mpPressure.end()) {
          pPressure->DistributedOnly(m_mpPressure.find(&*previous)->second->DistributedOnly());
        }
        bModified = true;
      }
      ...
      if (m_mpFractureMatrixPressure.find(&*itd) == m_mpFractureMatrixPressure.end()) {
        ...
        assert(pPressure != 0);
        CFractureMatrixPressure *pFractureMatrixPressure =
            new CFractureMatrixPressure(pPressure, *this, *itd); // Default "Same as previous"
        ...
        if (m_mpFractureMatrixPressure.find(&*previous) != m_mpFractureMatrixPressure.end()) {
          pFractureMatrixPressure->DistributedOnly(
              m_mpFractureMatrixPressure.find(&*previous)->second->DistributedOnly());
        }
        bModified = true;
      }
      ...
      if (m_mpTemperature.find(&*itd) == m_mpTemperature.end()) {
        ...
        if (m_mpTemperature.find(&*previous) != m_mpTemperature.end()) {
          pTemperature->DistributedOnly(m_mpTemperature.find(&*previous)->second->DistributedOnly());
        }
        bModified = true;
      }
      ...
      if (m_mpStrains.find(&*itd) == m_mpStrains.end() && !itd->Initial()) {
        ...
        if (m_mpStrains.find(&*previous) != m_mpStrains.end()) {
          pStrain->DistributedOnly(m_mpStrains.find(&*previous)->second->DistributedOnly());
        }
        bModified = true;
      }
...
unsigned int CFormationBase::TypeId() const { return IDT_TREE_FORMATIONS; }

QString CFormationBase::TypeName() const { return getStringTableEntry(IDS_TREE_FORMATIONS); }
...
CFractureMatrixPressure &CFormationBase::FractureMatrixPressure(const CDepletionStage &dep) {
  return const_cast<CFractureMatrixPressure &>(static_cast<const CFormationBase &>(*this).FractureMatrixPressure(dep));
}
...
CCalculatedTemperature &CFormationBase::CalculatedTemperature(const CDepletionStage &stage) {
  return const_cast<CCalculatedTemperature &>(static_cast<const CFormationBase &>(*this).CalculatedTemperature(stage));
}
...
IValueComposite &CFormationBase::EffectiveTemperature(const CDepletionStage &stage) {
  return const_cast<IValueComposite &>(static_cast<const CFormationBase &>(*this).EffectiveTemperature(stage));
}
...
bool CFormationBase::Depleting() const { return PressureChange() || TemperatureChange() || StrainChange(); }
...
bool CFormationBase::PressureChange() const {
  for (TPressureMap::const_iterator itp = m_mpPressure.begin(); itp != m_mpPressure.end(); ++itp) {
    ...
    assert(pPressure);
    if (((pPressure->Type() != CPressure::REPEAT_MODE) || (pPressure->DistributedSize() != 0)) &&
        (!pPressure->DepletionStage().Initial())) {
      return true;
    }
  }
...
bool CFormationBase::TemperatureChange() const {
  for (TTemperatureMap::const_iterator itt = m_mpTemperature.begin(); itt != m_mpTemperature.end(); ++itt) {
    ...
    assert(pTemperature);
    if (((pTemperature->Type() != CTemperature::REPEAT_MODE) || (pTemperature->DistributedSize() != 0)) &&
        (!pTemperature->DepletionStage().Initial())) {
      return true;
    }
  }
...
int CFormationBase::ElementSetSize() const { return m_vcElementSet.size(); }
...
const IFormationElementSet &CFormationBase::ElementSet(int nIndex) const { return *m_vcElementSet[nIndex]; }
...
{
  return (stage.Initial() || stage.Previous().OutputType() == CDepletionStage::BRANCH || stage.Previous().OutputType()
== CDepletionStage::PHASE);
}
...
void CFormationBase::LoadStream(TSTREAM &stream, CStreamVersion &version, TPROGRESS &progress) {
  ...
  while (pStage) {
    ...
    if (!pStage->Initial()) {
      CStrainLoad *pStrainLoad =
          new CStrainLoad(*(new CNormalStrainType()), *this, *pStage, true /*0, CDoubleQuantity::SI_UNIT*/);
  ...
  if (bShowVolStrainWarning) {
    _m()->msg("At least one 'old style' volumetric strain definition has been ignored because a 'new style' volumetric "
              "strain definition is also present for that stage");
  }
  ...
  if (pPointSet) {
    ...
    for (int i = 0; i < pPointSet->ValueSetSize(); i++) {
      if (pPointSet->ValueSet(i).Component()) {
        ...
        if (dynamic_cast<const TPressure *>(&parent) || dynamic_cast<const TTemperature *>(&parent) ||
            dynamic_cast<const TVolumetricStrain *>(&parent) || dynamic_cast<const TLateralStrain *>(&parent) ||
        ...
        /*
            ...
            if(idxParentStage > 0)
            {
                  const CMaterialServer* pMatServer =
           ConnectedMaterial(model.DepletionStageEntry().StageByIndex(idxParentStage)); if(pMatServer &&
           pMatServer->CanConnectItem(parent)) return true;
            }
            else
            {
        ...
        if (idxParentStage >= 0) {
          ...
          if (pVolStrain) {
            ...
            for (size_t j = 0; j < strain.DistributedSize(); ++j) {
              const TVolumetricStrain *pDistriStrain =
                  dynamic_cast<const TVolumetricStrain *>(&strain.DistributedValue(j));
              if (pDistriStrain)
...
geo::CValue CFormationBase::ThicknessAt(const geo::IPoint &pt, geo::IPoint &ptTop, geo::IPoint &ptBottom,
...
bool CFormationBase::Less(const CGraphNode &node) const {
  ...
  if (min_this.Valid() && min_rhs.Valid())
    return min_this.Value() < min_rhs.Value();
  if (min_this.Valid() && !min_rhs.Valid())
    return false;
  if (!min_this.Valid() && min_rhs.Valid())
    return true;
...
const CLengthQuantity &CFormationBase::MinThickness() const { return m_MinThickness; }

void CFormationBase::MinThickness(const double &value, CQuantity::UNIT unit) { m_MinThickness.Value(value, unit); }

CElementOrder::TElementOrder CFormationBase::ElementOrder() const { return m_formationElementOrder.elementOrder(); }
...
bool CFormationBase::inheritElementOrderFromModel() const { return m_formationElementOrder.inheritFromModel(); }

void CFormationBase::inheritElementOrderFromModel(bool inheritElementOrderFromModel) {
...
unsigned int CFormationThickness::IconId() const { return IDI_FORMATION_THICKNESS; }

unsigned int CFormationThickness::TypeId() const { return 0; }

long CFormationThickness::SavedItems() const { return 0; }

/////////////////////////////////////////////////////////////
// CFormationThickness::CFormationThicknessComponent
/////////////////////////////////////////////////////////////

CFormationThickness::CFormationThicknessComponent::CFormationThicknessComponent(CFormationThickness &parent,
                                                                                const CFormationBase &formation)
    : IValueComponentBase(formation.Name(), parent), m_Data(*this), m_formation(formation) {}
...
IValueDataInterfaceScalar &CFormationThickness::CFormationThicknessComponent::ScalarData() { return m_Data; }

unsigned int CFormationThickness::CFormationThicknessComponent::IconId() const { return IDI_RESULT; }

unsigned int CFormationThickness::CFormationThicknessComponent::TypeId() const { return 0; }

bool CFormationThickness::CFormationThicknessComponent::Empty() const { return false; }
...
IValueDomainScalar::TMinMax
CFormationThickness::CFormationThicknessComponent::MinMax(IProgressBase & /*progressBase*/,
                                                          const CQuantity::UNIT /*unit*/) const {
  return IValueDomainScalar::TMinMax();
}

IValueDomainScalar::TValue CFormationThickness::CFormationThicknessComponent::Average(IProgressBase &,
                                                                                      const CQuantity::UNIT) const {
  return IValueDomainScalar::TValue();
}

IValueDomainScalar::TValue
CFormationThickness::CFormationThicknessComponent::ValuePoint(const geo::IPoint &pt, const UNIT unit,
                                                              geo::IParallelInitializationCallback *cb) const {
...
CFormationThickness::CFormationThicknessComponent::ValueElement(const geo::IElement &elm, const UNIT unit,
                                                                geo::IParallelInitializationCallback *cb) const {
  int i;
...
bool CFormationThickness::CFormationThicknessComponent::Defined() const { return true; }

const CFormationBase &CFormationThickness::CFormationThicknessComponent::Formation() const { return m_formation; }

////////////////////////////////////////
// TFormationBaseEntry
////////////////////////////////////////

TFormationBaseEntry::TFormationBaseEntry(int nEntryId, unsigned int uIconId, const QString &strName,
                                         CFemAppModel &model)
