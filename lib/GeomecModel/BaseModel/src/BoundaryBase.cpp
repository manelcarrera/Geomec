...
CBoundaryBase::CBoundaryBase(const geo::IPoint &ptMin, const geo::IPoint &ptMax, CFemAppModel &model,
                             BOUNDARY_STATE state)
    : CColorNode(IDS_BOUNDARY, model, qRgb(126, 126, 126)), m_bsState(state), m_pBox(0), m_ptMin(ptMin), m_ptMax(ptMax),
      m_ptRotation(0, 0, 0), m_dAzimuth(0), m_dLength(fabs(ptMin.X() - ptMax.X())),
...
CBoundaryBase::CBoundaryBase(const CBoundaryBase &rhs)
    : CColorNode(rhs), m_bsState(rhs.State()), m_pBox(0), m_ptMin(rhs.m_ptMin), m_ptMax(rhs.m_ptMax),
      m_ptRotation(rhs.m_ptRotation), m_dAzimuth(rhs.m_dAzimuth), m_dLength(rhs.m_dLength), m_dWidth(rhs.m_dWidth) {
  assert(rhs.IsCopied());
  assert(!rhs.IsCopy());
...
CBoundaryBase::~CBoundaryBase() { delete m_pBox; }
...
const geo::CBox &CBoundaryBase::Box() const { return *m_pBox; }
...
bool CBoundaryBase::operator==(const CBoundaryBase &rhs) const {
  ...
  return (m_bsState == rhs.m_bsState && m_ptMin == rhs.m_ptMin && m_ptMax == rhs.m_ptMax &&
          m_ptRotation == rhs.m_ptRotation && fabs(m_dAzimuth - rhs.m_dAzimuth) < EPS &&
          fabs(m_dLength - rhs.m_dLength) < EPS && fabs(m_dWidth - rhs.m_dWidth) < EPS);
}
...
bool CBoundaryBase::Set(const geo::IPoint &ptMin, const geo::IPoint &ptMax) { return OnSet(TMinMax(ptMin, ptMax)); }

bool CBoundaryBase::Set(const geo::IPoint &ptMid, double dAzimuth, double dLength, double dWidth, double dMinDepth,
                        double dMaxDepth) {
  assert(CanRotate());
...
geo::CBox *CBoundaryBase::CreateRotatedBox(const TRotated &rotated) const {
  ...
  geo::CBox *pBox = new geo::CBox(
      geo::CPoint(rotated.ptMid.X() + rotated.dWidth / 2. * sin(a) - rotated.dLength / 2. * cos(a),
                  rotated.ptMid.Y() - rotated.dWidth / 2. * cos(a) - rotated.dLength / 2. * sin(a), rotated.dMinDepth),
...
CLengthQuantity CBoundaryBase::R() const {
  return CLengthQuantity((fabs(Max().X() - Min().X()) + fabs(Max().Y() - Min().Y()) + fabs(Max().Z() - Min().Z())) /
                         6.0);
}
...
void CBoundaryBase::OnGridModified() {
  ...
  case ROTATED: {
    TRotated rotated =
        SnapToGrid(TRotated(m_ptRotation, m_dAzimuth, m_dLength, m_dWidth, bestfit.first.Z(), bestfit.second.Z()));
...
unsigned int CBoundaryBase::IconId() const { return IDI_BOUNDARY; }

// ##ModelId=3BC55D6402C5
unsigned int CBoundaryBase::TypeId() const { return IDT_BOUNDARY; }

QString CBoundaryBase::TypeName() const { return getStringTableEntry(IDS_BOUNDARY); }

CBoundaryBase::CDrawDef *CBoundaryBase::OnCreateDrawDef(const geo::IObject & /*object*/) const {
  return new CBoundaryDrawDef(*this);
}

int CBoundaryBase::DisplayListSize() const { return 1; }
...
bool CBoundaryBase::Empty() const { return m_pBox == 0; }
...
long CBoundaryBase::SavedItems() const { return CColorNode::SavedItems() + 4; }

CBoundaryBase::BOUNDARY_STATE CBoundaryBase::State() const { return m_bsState; }
...
const geo::CPoint &CBoundaryBase::MaxPoint() const { return m_ptMax; }

const geo::CPoint &CBoundaryBase::MinPoint() const { return m_ptMin; }

const geo::CPoint &CBoundaryBase::Mid() const { return m_ptRotation; }

double CBoundaryBase::Azimuth() const { return m_dAzimuth; }

double CBoundaryBase::Length() const { return m_dLength; }

double CBoundaryBase::Width() const { return m_dWidth; }
...
std::vector<CGraphNode *> CBoundaryBase::ItemsOutsideBoundary(const geo::CRectangle &rectangle) const {
  ...
  CAnalysisPointEntry::TNodeSet an_points =
      ((CAnalysisPointEntry *)model.GraphEntry(MD_BASE_ANALYSIS_POINT))->EntryNodes();
...
bool CBoundaryBase::HasInterfaces() const { return false; }

int CBoundaryBase::InterfaceNodeSize() const { return 0; }
...
bool CBoundaryBase::CanAssignElementSet(const IElementSet & /*elset*/) const { return HasInterfaces(); }
  ...
  for (size_t ii = 0; ii < childSize(); ++ii) {
    ...
    while (pStage) {
      ...
      if (pStage->Name() == "Initial") {
        ...
        stageId = pStage->Name().right(pStage->Name().length() - prefixLen);
      }
