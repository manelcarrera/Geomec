...
CGeneralDataBlock::CGeneralDataBlock(const QString& name, int type)
  : m_name(name)
  , m_type(type)
{
}
...
CGeneralDataBlock::CGeneralDataBlock(const CGeneralDataBlock& rhs)
  : m_name(rhs.m_name)
  , m_type(rhs.m_type)
  , m_intValues(rhs.m_intValues)
  , m_dblValues(rhs.m_dblValues)
{
}
...
bool CGeneralDataBlock::operator==(const CGeneralDataBlock& rhs) const
{
...
QString CGeneralDataBlock::Name() const
{
  return m_name;
}

void CGeneralDataBlock::Name(const QString& name)
{
  m_name = name;
}
...
int CGeneralDataBlock::Size() const
{
  switch (m_type)
  {
  case 0: return int(m_intValues.size());
  case 1: return int(m_dblValues.size());
  case 2: return int(m_strValues.size());
  }

  return 0;
}

int CGeneralDataBlock::Type() const
{
  return m_type;
}
