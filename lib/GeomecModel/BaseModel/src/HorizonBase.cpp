...
//////////////////////////////////////////////////////////////////////
//Default constructor...
CHorizonBase::CHorizonBase(CFemAppModel &model)
: CColorNode(model),
  m_bSlip(false),
  m_SlipType(SLIP)
{
...
: CColorNode(strInstanceName, model),
  m_bSlip(false),
  m_SlipType(SLIP)
{
	CreatePressures();
...
{
  return Slip() && (stage.Initial() || stage.Previous().OutputType() == CDepletionStage::BRANCH || stage.Previous().OutputType() == CDepletionStage::PHASE);
}
...
unsigned int CHorizonBase::TypeId() const
{
	return IDT_TREE_HORIZONS;
}

QString CHorizonBase::TypeName() const
{
	return getStringTableEntry(IDS_TREE_HORIZONS);
}
...
CHorizonBase::TSlipType CHorizonBase::SlipType() const
{
	return m_SlipType;
}

void CHorizonBase::SlipType(TSlipType slip_type)
{
	m_SlipType = slip_type;
}
...
bool CHorizonBase::DoubleSidedFault() const
{
  return false;
}

bool CHorizonBase::IgnoreFault() const
{
  return false;
}
...
long CHorizonBase::SavedItems() const
{
	return CColorNode::SavedItems() + 1;
}

const dia::IMaterial &CHorizonBase::InterfaceMaterial(const geo::IElement &/*element*/, const CDepletionStage& /*stage*/) const
{
	assert(false);
...
{
	for(TMaterialSet::iterator it = m_stMaterials.begin(); it != m_stMaterials.end(); it++) delete *it;
	m_stMaterials.clear();
}
...
const double& CHorizonBase::AverageDStif() const
{
  return m_dAverageDStif;
}
