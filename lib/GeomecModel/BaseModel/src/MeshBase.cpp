...
: IElementSet(sName, dep_entry.Model(), CQuantity::SI_UNIT, CQuantity::SI_UNIT), 
  m_bLocked(false), 
...
/*!
Invalidating the mesh means that the mesh between surfaces of the horizons, faults and boundary of the sub- and super model is deleted. This happens whenever there is a change in the model’s geometry and / or in the mesh parameters. As a result all the bodies generated by the mesh procedure are deleted automatically from the model. For example the following actions result in invalidation of the mesh:
-	Inserting  a horizon
-	Deleting a horizon
-	Refining or coarsening surfaces of a horizon, a fault or the boundary
-	Changing the state of the boundary
Invalidating the mesh automatically results in invalidation of any results.  
*/
...
bool CMeshBase::getGenerateSupports() const
{
  return m_bGenerateSupports;
}

void CMeshBase::setGenerateSupports(bool bGenerateSupports)
{
  m_bGenerateSupports = bGenerateSupports;
}
