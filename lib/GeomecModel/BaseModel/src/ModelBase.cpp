...
CModelBase::CModelLoadSaveDefault::CModelLoadSaveDefault(CModelBase *pParentModel, IModelDiagnostics *pModelDiagnostics)
    : m_pVersionManager(0), m_pParentModel(pParentModel), m_pModelDiagnostics(pModelDiagnostics) {}

CModelBase::CModelLoadSaveDefault::CModelLoadSaveDefault(const QString &appVersion, CModelBase *pParentModel,
                                                         IModelDiagnostics *pModelDiagnostics)
    : m_pVersionManager(0), m_pParentModel(pParentModel), m_pModelDiagnostics(pModelDiagnostics) {
...
bool CModelBase::CModelLoadSaveDefault::LoadModel(CStorageNode::TSTREAM &stream, CStreamVersion & /*version*/,
...
bool CModelBase::CModelLoadSaveDefault::SaveModel(CStorageNode::TSTREAM &stream, CStorageNode::TPROGRESS &prog,
                                                  CModelBase &model) {
...
long CModelBase::CModelLoadSaveDefault::SavedItems(CModelBase &model) const { return model.SavedItems(); }
...
CModelBase::CModelBase(CAnalysisLogger &logger, const CVersionManager &versionManager)
    : CFemAppModel(logger), m_pDepletionStageEntry(0), m_pPressure(0), m_pGlobalTemperature(0),
      m_gravity(9.8, 0, 15, CQuantity::SI_UNIT), m_pBranchState(0), m_bNoCSHE(true), m_nElementOrder(DEF_ELEMENT_ORDER),
      m_nFaultInit(DEF_FAULT_INIT), m_bLargeDeform(false), m_bLoading(false), m_bInvalidateResultsAfterLoad(false),
      m_bAllowLinearCalc(true), m_bSaltStressInitRelax(true), m_nIniStressMaxIterations(3),
      m_dIniStressConvCriterion(0.01), m_bLinstaOnlyInitialStress(false), m_bPressureSupportNodeLoadWarningDone(false),
      m_bAverageResults(false), m_pGVTSettings(0), m_pQBSettings(0), m_bUseParallelSolver(true), m_nNumSolverThreads(8),
      m_bUseStressSmootheningOnDSF(true), m_nDSFInterSurfType(1), m_bUseDecompactionParams(false),
      m_bAllowRedrawing(true), m_bForcingRedraw(false), m_bAutomaticallyDisableDrawing(false),
      m_bExportCommandFilesWithDefaults(false), m_bWriteOutputStreamFile(false), m_bCleanupOldBranchResults(false),
      m_bShowConvexHulls(false), m_pChildModelLoadSave(0), m_versionManager(versionManager), m_nIndex(-1),
      m_pParentModel(0), m_bSourceDepletionStagesChanged(false), m_bSourceGlobalPressureChanged(false),
      m_bSourceGlobalTemperatureChanged(false), m_bSourceGravityChanged(false), m_ParentResultsDef(PRD_EQUAL),
      m_EnableResultCache(false), m_ResultCacheSize(0), m_bConvergenceChecks(false), m_bConvergenceMinimumAngle(true),
      m_dConvergenceMinimumAngle(3), m_bConvergenceMinimumRatio(true), m_dConvergenceMinimumRatio(0.01),
      m_bConvergenceMinimumVolume(true), m_dConvergenceMinimumVolume(1), m_bDefaultDianaZoominSetting(false),
      m_pModelDiagnostics(0), m_pConsistencyGuard(0), m_pBoundary(0), m_materialErrorHandler(logger),
      m_show_cm2_dlg(true), m_mesher_dlg(nullptr)

{
  ...
  m_pConsistencyGuard = new CConsistencyGuard(
      *this); // we always create one, but will delete it for child models; they will access the root guard
}
...
CModelBase::~CModelBase() {
  ...
  for (std::map<QString, CGeneralDataBlock *>::iterator it = m_generalDataBlocks.begin();
       it != m_generalDataBlocks.end(); ++it)
    delete it->second;
}

void CModelBase::destroy() { IModelObject::destroy(); }

void CModelBase::CloseModel() { CFemAppModel::CloseModel(); }
...
bool CModelBase::CanUseCalculatedTemperatures() const { return true; }

const CGlobalInitialStressNode &CModelBase::GlobalInitialStress() const { return *m_pGlobalStressNode; }

CGlobalInitialStressNode &CModelBase::GlobalInitialStress() { return *m_pGlobalStressNode; }

const CGlobalPressure &CModelBase::GlobalPressure() const { return *m_pPressure; }

CGlobalPressure &CModelBase::GlobalPressure() { return *m_pPressure; }

const CGlobalTemperature &CModelBase::InitialTemperature() const { return *m_pGlobalTemperature; }

CGlobalTemperature &CModelBase::InitialTemperature() { return *m_pGlobalTemperature; }

// ##ModelId=3BC55D6102A7
const QString &CModelBase::ProjectInfo() const { return m_strProjectInfo; }

// ##ModelId=3BC55D6102A5
void CModelBase::ProjectInfo(const QString &strProjectInfo) { m_strProjectInfo = strProjectInfo; }

const CAcceleration &CModelBase::Gravity() const { return m_gravity; }

bool CModelBase::HasFormations() const {
  CModelBase *pThis = const_cast<CModelBase *>(this);
  TFormationBaseEntry::TNodeSet formations =
      ((TFormationBaseEntry *)pThis->GraphEntry(MD_BASE_FORMATION))->EntryNodes();
...
bool CModelBase::CanCreateMesh() { return Mesh().CanCreateMesh(); }

bool CModelBase::IsMesh() const { return Mesh().IsMesh(); }
...
void CModelBase::InvalidateMesh() { Mesh().InvalidateMesh(); }
...
CGraphNode &CModelBase::GravityNotifier() { return m_GravityNotifier; }

const CGVTSettings &CModelBase::GVTSettings() const { return *m_pGVTSettings; }

CGVTSettings &CModelBase::GVTSettings() { return *m_pGVTSettings; }

const CQBSettings &CModelBase::QBSettings() const { return *m_pQBSettings; }

CQBSettings &CModelBase::QBSettings() { return *m_pQBSettings; }
...
const CDepletionStage &CModelBase::LastDepletionStage() const { return InitialDepletionStage().LastStage(); }

CDepletionStage &CModelBase::LastDepletionStage() { return InitialDepletionStage().LastStage(); }

/// \brief determine the depl.stage, based on timeStepIndex
//
const CDepletionStage *CModelBase::FindDepletionStage(int timeStepIndex) const {
...
const CDepletionStageEntry &CModelBase::DepletionStageEntry() const { return *m_pDepletionStageEntry; }

CDepletionStageEntry &CModelBase::DepletionStageEntry() { return *m_pDepletionStageEntry; }
...
void CModelBase::createDefaults() {
  ...
  if (parentModel()) {
    // link to the source model's depletion stage entry, so we can notify the user when the source depletion stages have
    // changed
...
void CModelBase::DuplicateGlobalTemperature() {
  ...
  myTemp.Constant().Set(parentTemp.Constant().ReferenceValue().Value(), parentTemp.Constant().ReferenceDepth().Value(),
...
void CModelBase::NumberModels(int &nIndex) {
  ...
  if (wpEntry != 0) {
    ...
    for (CNewWellPathEntry::TNodeSet::iterator it = nodes.begin(); it != nodes.end(); ++it) {
      if ((*it)->hasZoomInModel()) {
        // if the well zoomin hierarchy gets more complicated, we may make this method virtual instead and handle it
        // there
...
bool CModelBase::LoadPre(CStorageNode::TSTREAM &stream, CStorageNode::TSTREAM **ppcstream, CStreamVersion &version,
                         TPROGRESS &prg, bool bLoadHeader) {
  assert(!Loading());
  ...
  if (CStreamVersion(3, 0, 22) < version) {
    ...
    assert(pDoc != 0);
    if (!pDoc->ExperimentalUnlocked() && (ElementOrder() != CElementOrder::EO_LINEAR)) {
      _m()->msg("Quadratic elements are selected in this model. This feature is locked in this version, the option is "
                "switched back to linear elements");
  ...
  if (version >= CStreamVersion(3, 7, 9) &&
    ...
    ((CWellCasingSteelMaterialEntry *)GraphEntry(MD_WELLCASING_STEELMATERIAL_ENTRY))
        ->LoadStream(*this, cstream, version, prg);
...
bool CModelBase::Load(CStorageNode::TSTREAM &stream, CStreamVersion &version, TPROGRESS &prg, bool bLoadHeader) {
  ...
  bool bRet = (LoadPre(stream, &pCompressedStream, version, prg, bLoadHeader) &&
               OnLoad(*pCompressedStream, version, prg) && LoadPost(*pCompressedStream, version, prg));
...
bool CModelBase::LoadPost(CStorageNode::TSTREAM &stream, CStreamVersion &version, TPROGRESS &prg) {
  ...
  if ((version < CStreamVersion(3, 8, 0) && version >= CStreamVersion(3, 7, 25)) ||
      version >= CStreamVersion(3, 8, 4)) {
...
int CModelBase::LoadSceneMarker(CStorageNode::TSTREAM &stream, CStreamVersion &version) {
  int marker = 0;
...
// The result server
int CModelBase::ResultRegisterSize() const { return 1; }

QString CModelBase::ResultRegisterName(int /*nRegister*/) const { return "Results"; }
...
CMaterialResultTree &CModelBase::MaterialResultTree() { return *m_pMatResultTree; }

const CMaterialResultTree &CModelBase::MaterialResultTree() const { return *m_pMatResultTree; }
...
bool CModelBase::LoadChildModel(CStorageNode::TSTREAM &stream, CStreamVersion &version, CStorageNode::TPROGRESS &prog,
...
void CModelBase::SaveHeader(CStorageNode::TSTREAM &stream, TPROGRESS &prg, int &nMaxNrOfSteps, quint64 &dwStepPos) {
  ...
  int bSuccess = false;
  unsigned long steps =
      10000; // We now stream a dummy value and keep the filepointer. In the end we write the real nr of steps
...
void CModelBase::FinalizeSave(CStorageNode::TSTREAM &stream, TPROGRESS & /*prg*/, int nMaxNrOfSteps,
                              quint64 dwStepPos) {
  assert(!stream.IsCompressed());
  stream.Flush();
...
bool CModelBase::Save(CStorageNode::TSTREAM &stream, TPROGRESS &prg, bool bSaveHeader) {
  ...
  stream << m_nIndex; // save it here, as we will want it outside the compression in HDF5 [ that is no longer relevant,
                      // as HDF5 uses internal compression, and we're oblivious to it ]
  ...
  // we save in each model, as we play around with the singleton-ness depending on parent; need to check what happens on
  // import of models
...
void CModelBase::SaveFileHeader(CStorageNode::TSTREAM &stream, const QString &currentAppVersion) {
  ...
  if (!parentModel()) {
    ...
    for (TPointSetEntry::TNodeSet::iterator it = stNode.begin(); it != stNode.end(); it++) {
      if (!dynamic_cast<CMeshBase *>(*it)) {
        const CGocadElementSet *gocadElementSet = dynamic_cast<const CGocadElementSet *>(*it);
        ...
        if (!isGocadElementSet) {
          ...
          if (!bVolume) {
            ...
            stream << bLasPointSet;
            if (bLasPointSet == 0) {
              int isNewWellPathInput = (dynamic_cast<const CNewWellPathInput *>(*it) != 0 ? 1 : 0);
...
void CModelBase::LoadSurfaces(CStorageNode::TSTREAM &stream, CStreamVersion &version,
...
void CModelBase::LoadRockMaterials(CStorageNode::TSTREAM &stream, CStreamVersion &version,
                                   CStorageNode::TPROGRESS &progress) {
  if (version < CStreamVersion(4, 1, 0) || !parentModel() || !parentModel()->Loading()) {
    CMaterialEntry &rock_material_entry = static_cast<CMaterialEntry &>(*GraphEntry(MD_ROCK_MATERIAL));
...
CStreamVersion CModelBase::currentVersion() const { return m_versionManager.CurrentVersion(); }

CStreamVersion CModelBase::documentVersion() const { return m_versionManager.FileVersion(); }
...
bool CModelBase::CanCalculateHeatFlow() const { return HeatFlowCalculationCriteriaMet(); }

bool CModelBase::CanCalculateMixture() const { return CanCalculate(); }

bool CModelBase::WriteFilosModel(const QString &getPathName, IDianaXWrapper *dianaXWrapper, const std::string &title,
                                 const CAnalysisType &antype, bool bWriteInputFiles, bool bWriteQuadDat,
  ...
  CDianaRunController drc(*this, antype, bWriteInputFiles, bWriteQuadDat, m_bWriteOutputStreamFile, strTempPath);

  dia::IDianaRunner::RunParams p = {getPathName,   title,      strTempPath.toStdString(),
                                    dianaXWrapper, &saveModel, &retrieveDianaFileNames,
                                    nullptr,       false,      _g->dsa()};
  drc.params(p);
...
bool CModelBase::CheckForAnalysis(const CAnalysisType &antype, bool bWriteInputFiles, bool bWriteQuadDat) {
  // check whether the model has branches or phases
  // if so, warn user when analysis type is not non-linear
  if (antype.AnalysisType() != CAnalysisType::AT_NONLIN && (HasBranches() || HasPhases()) && !bWriteInputFiles &&
      !bWriteQuadDat) {
    ...
    if (_m()->msg(strMsg, MB_ICONEXCLAMATION | MB_YESNO) == IDNO)
      return false;
  }
...
bool CModelBase::LargeDeformations() const { return m_bLargeDeform; }
...
bool CModelBase::AllowLinearCalc() const { return m_bAllowLinearCalc; }
...
bool CModelBase::UseParallelSolver() const { return m_bUseParallelSolver; }
...
int CModelBase::NumSolverThreads() const { return m_nNumSolverThreads; }
...
int CModelBase::MaxNumSolverThreads() const { return MAX_NUM_SOLVER_THREADS; }

bool CModelBase::UseStressSmootheningOnDSF() const { return m_bUseStressSmootheningOnDSF; }
...
int CModelBase::DSFIntermediateSurfaceType() const { return m_nDSFInterSurfType; }
...
CElementOrder::TElementOrder CModelBase::ElementOrder() const { return m_nElementOrder.elementOrder(); }
...
void CModelBase::InvalidateResultsAfterLoad() { m_bInvalidateResultsAfterLoad = true; }
...
void CModelBase::CopyDepletionStages(const CDepletionStageEntry &source, CDepletionStageEntry &target,
                                     TPROGRESS &progress) {
...
void CModelBase::CopyFormationElementSet(const IFormationElementSet &source, IFormationElementSet &target) {
  assert(&source.Model() != &target.Model());
  assert(this == &target.Model());

  QString strName = source.Name();
  if (strName.isEmpty())
    strName = source.Formation().Name();
  target.Name(strName);
...
void CModelBase::LoadZoominProperties(CStorageNode::TSTREAM &stream, CStreamVersion &version,
                                      CStorageNode::TPROGRESS & /*progress*/) {
...
void CModelBase::DoDensityCorrection() {
  ...
  std::auto_ptr<IProgressBase> dlg(
      _g->prog()->create(eProgress::Dual, getStringTableEntry(IDS_CALC_DENSITIES), false, 1));
  ...
  // Apply density
  for (it = stFormation.begin(); it != stFormation.end(); it++) {
    if ((*it)->Material(InitialDepletionStage()).Valid())
      (*it)->Material(InitialDepletionStage()).LinkTo(*pType);
  }
}

bool CModelBase::OnModelSpecificAnalysisCheck(const CAnalysisType & /*antype*/, bool /*bWriteInputFiles*/,
                                              bool /*bWriteQuadDat*/) {
  return true;
}
...
int CModelBase::IniStressMaxIterations() const { return m_nIniStressMaxIterations; }
...
double CModelBase::IniStressConvCriterion() const { return m_dIniStressConvCriterion; }
...
bool CModelBase::LinstaOnlyInitialStress() const { return m_bLinstaOnlyInitialStress; }
...
bool CModelBase::isPressureSupportNodeLoadWarningDone() const { return m_bPressureSupportNodeLoadWarningDone; }

void CModelBase::setPressureSupportNodeLoadWarningDone() { m_bPressureSupportNodeLoadWarningDone = true; }

const CBranchState &CModelBase::BranchState() const { return *m_pBranchState; }

CBranchState &CModelBase::BranchState() { return *m_pBranchState; }
...
bool CModelBase::AverageResults() const { return m_bAverageResults; }
...
void CModelBase::CollectDepletingFormationNodes(TNodeElementsMap &mpNodeElements,
      ...
      for (i = 0; i < formation.ElementSetSize(); ++i) {
        ...
        for (el = 0; el < elementset.ElementSet().ElementSize(); ++el) {
          ...
          for (n = 0; n < elm.NrOfNodes(); ++n) {
            TNodeElementsMap::iterator itInsert =
                mpNodeElements.insert(TNodeElementsMap::value_type(elm.Node(n).Index(), std::set<int>())).first;
...
bool CModelBase::CanImportResults() const {
  ...
  TFormationBaseEntry &entry = (TFormationBaseEntry &)*GraphEntry(MD_BASE_FORMATION);
...
// wjrx mantis 2976
bool CModelBase::CanInsertNewWellPath() const { return GraphEntry(MD_BASE_HORIZON)->GraphEntryNodes().size() > 1; }

bool CModelBase::UseDecompactionParameters() const { return m_bUseDecompactionParams; }
...
bool CModelBase::WriteOutputStreamFile() const { return m_bWriteOutputStreamFile; }
...
bool CModelBase::CleanupOldBranchResults() const { return m_bCleanupOldBranchResults; }
...
void CModelBase::AutomaticallyDisableDrawing(bool bDisable) { m_bAutomaticallyDisableDrawing = bDisable; }

bool CModelBase::AutomaticallyDisableDrawing() const { return m_bAutomaticallyDisableDrawing; }
...
void CModelBase::setAllowRedrawing(bool allowRedrawing) { m_bAllowRedrawing = allowRedrawing; }

void CModelBase::setForcingRedraw(bool forcingRedraw) { m_bForcingRedraw = forcingRedraw; }

bool CModelBase::RedrawingDisabled() const { return !m_bAllowRedrawing; }

bool CModelBase::SceneCanDraw() const { return m_bAllowRedrawing; }

bool CModelBase::ShowConvexHulls() const { return m_bShowConvexHulls; }

void CModelBase::ShowConvexHulls(bool bShowConvexHulls) { m_bShowConvexHulls = bShowConvexHulls; }

const CModelBase &CModelBase::RootModel() const { return (const_cast<CModelBase *>(this))->RootModel(); }
...
bool CModelBase::ExportCommandFilesWithDefaults() const { return m_bExportCommandFilesWithDefaults; }

void CModelBase::ExportCommandFilesWithDefaults(bool b) { m_bExportCommandFilesWithDefaults = b; }

int CModelBase::childModelSize() const {
  CWellPathEntry *entry = dynamic_cast<CWellPathEntry *>(const_cast<CGraphEntry *>(GraphEntry(MD_BASE_WELLPATH)));
  int count = 0;

  if (entry != 0) {
    CWellPathEntry::TNodeSet nodes = entry->EntryNodes();

    for (CWellPathEntry::TNodeSet::const_iterator iterator = nodes.begin(); iterator != nodes.end(); ++iterator) {
      count += (dynamic_cast<CWellPath *>(*iterator)->hasZoomInModel()) ? 1 : 0;
    }
  }

  // wjrx mantis 3687
  CNewWellPathEntry *newEntry =
      dynamic_cast<CNewWellPathEntry *>(const_cast<CGraphEntry *>(GraphEntry(MD_NEW_WELLPATH)));

  if (newEntry != 0) {
    CNewWellPathEntry::TNodeSet nodes = newEntry->EntryNodes();

    for (CNewWellPathEntry::TNodeSet::const_iterator iterator = nodes.begin(); iterator != nodes.end(); ++iterator) {
...
CModelBase *CModelBase::parentModel() { return m_pParentModel; }

const CModelBase *CModelBase::parentModel() const { return m_pParentModel; }
...
CVersionManager CModelBase::getVersionManager() const { return m_versionManager; }

int CModelBase::Index() const { return m_nIndex; }

void CModelBase::Index(int nIndex) { m_nIndex = nIndex; }

void CModelBase::SetAppVersion(const QString &applicationVersion) { m_versionManager.AppVersion(applicationVersion); }
...
CModelBase *CModelBase::Model(int nIndex) {
  ...
  if (nIndex == m_nIndex) {
    ...
    const CZoominModelEntry *pEntry =
        dynamic_cast<const CZoominModelEntry *>(RootModel().GraphEntry(MD_BASE_ZOOMIN_MODEL));
    if (pEntry) {
...
CNewModel &CModelBase::getNewModelObject() { return m_NewModel; }
...
void CModelBase::SwitchTo(CFemAppModel *currentModel) {
  ...
  if (m_bSourceDepletionStagesChanged) {
    // should we update?
    if (_m()->msg("The parent model's depletion stages have been modified.\n\nWould you like to reinitialize this "
                  "wellpath zoom-in model's depletion stage list?",
                  MB_YESNO) == IDYES) {
      BuildDepletionStages();
      ResultRegister().ClearAll();
    }
...
void CModelBase::FindParentFormations() {
  if (Mesh().ElementSet().ElementSize() > 0) {
    TFormationBaseEntry::TNodeSet stNodes =
        static_cast<TFormationBaseEntry *>(GraphEntry(MD_BASE_FORMATION))->EntryNodes();
    for (TFormationBaseEntry::TNodeSet::iterator formationIt = stNodes.begin(); formationIt != stNodes.end();
         ++formationIt)
      (*formationIt)->FindParentFormation();
  }
}

void CModelBase::SwitchToParent(CModelBase &parent) { GetConsistencyGuard()->SwitchToModel(parent); }
...
void CModelBase::MenuInvokedBuildDepletionStages() {
  if (_m()->msg("Are you sure you want to reinitialize all depletion stages from the parent model's depletion stages?",
                MB_YESNO) == IDYES)
    BuildDepletionStages();
}
...
void CModelBase::BuildDepletionStages() {
  ...
  while (pStage && pSrcStage) {
    ...
    if (pStage->NLSteps() != pSrcStage->NLSteps() ||
      ...
      pStage->SetNonLinParams(pSrcStage->AutomaticStepSizes(), pSrcStage->MaxAutomaticStepSize(), pSrcStage->NLSteps(),
                              pSrcStage->NLStepScalingFactor(), pSrcStage->NLConvNormVal(),
                              pSrcStage->NLIterationSteps(), pSrcStage->NLConvergenceType(),
                              DefaultIterationScheme(*pSrcStage));
      bClearNonlin = true;
    }
...
CModelBase::TParentResultsDef CModelBase::ParentResultsDefinition() const { return m_ParentResultsDef; }
...
bool CModelBase::ParentLinearResults(const CAnalysisType &antype) const {
  ...
  switch (resultdef) {
  case PRD_EQUAL:
    bRet = (antype == CAnalysisType::AT_LINEAR); // only use linear results for a linear analysis, otherwise nonlin
                                                 // (also for mixture)
    break;
  case PRD_LINEAR:
    bRet = true;
...
bool CModelBase::CheckParentResults(const CAnalysisType &antype) const {
  ...
  QString sParentRes;
  sParentRes = QString("\n\nRight-click '%1' in the tree and select 'Set parent model results type' to specify the "
                       "result type from the parent model.")
                   .arg(Name());
  const QString sLinearMsg = "No linear results are available in the parent model, please run the linear analysis for "
                             "the parent model first." +
                             sParentRes;
  const QString sNonlinMsg = "No nonlinear results are available in the parent model, please run the nonlinear "
                             "analysis for the parent model first." +
                             sParentRes;
  const QString sMixtureMsg =
      "You have chosen to use equal results in the parent model.\nFor a mixture analysis nonlinear results are "
      "selected in that case, but no nonlinear "
      "results are available in the parent model.\n\nPlease run the nonlinear analysis for the parent model first." +
      sParentRes;
  ...
  case PRD_EQUAL:
    ...
    if ((antype == CAnalysisType::AT_MIXTURE || antype == CAnalysisType::AT_MIXTURE_CONTAINMENT) &&
        !parentModel()->ResultRegister().NonLinear()) {
      _m()->msg(sMixtureMsg);
      return false;
    }
    ...
    for (int i = 0; i < size; ++i) {
      ...
      for (int j = 0; j < elt.NrOfNodes(); ++j)
        if (!wellMesh.Contains(elt.Node(j), false)) {
          QString msg = "Some of the mesh points are not contained in the well mesh, and not all parent stresses will "
                        "be assigned. "
                        "This may lead to inaccuracies in the results. You can check the parent stresses under "
                        "Results->Input, or adjust your "
...
bool CModelBase::getEnableResultCache() const { return m_EnableResultCache; }

int CModelBase::getResultCacheSize() const { return m_ResultCacheSize; }
...
CMaterialErrorHandler &CModelBase::getMaterialErrorHandler() { return m_materialErrorHandler; }
...
bool CModelBase::ConvergenceChecks() const { return m_bConvergenceChecks; }
...
bool CModelBase::ConvergenceMinimumAngleCheck() const { return m_bConvergenceMinimumAngle; }
...
double CModelBase::ConvergenceMinimumAngleValue() const { return m_dConvergenceMinimumAngle; }
...
bool CModelBase::ConvergenceMinimumRatioCheck() const { return m_bConvergenceMinimumRatio; }
...
double CModelBase::ConvergenceMinimumRatioValue() const { return m_dConvergenceMinimumRatio; }
...
bool CModelBase::ConvergenceMinimumVolumeCheck() const { return m_bConvergenceMinimumVolume; }
...
double CModelBase::ConvergenceMinimumVolumeValue() const { return m_dConvergenceMinimumVolume; }
...
bool CModelBase::DefaultDianaZoominSetting() const { return m_bDefaultDianaZoominSetting; }
