...
C3DModel::C3DModel(CAnalysisLogger &logger, const CVersionManager &versionManager)
    : CModelBase(logger, versionManager), m_pTopDisplacementsNode(0) {}
...
long C3DModel::SavedItems() const {
  ...
  const CNonMeshedSurfaceEntry &nmsEntry =
      static_cast<const CNonMeshedSurfaceEntry &>(*GraphEntry(MD_BASE_NONMESHEDSURFACE));
  return m_pTopDisplacementsNode->SavedItems() + upscalingentry.SavedItems() + nmsEntry.SavedItems() +
         CModelBase::SavedItems();
}
...
CGeomecDianaRunnerBase *C3DModel::OnCreateDianaRunner(CDianaRunController &controller) {
  ...
  case CAnalysisType::AT_HEAT:
    return new CGeomecHeatFlowDianaRunner(Mesh(), *this, controller);
  case CAnalysisType::AT_MIXTURE:
    return new CGeomecMixtureDianaRunner(Mesh(), *this, controller);
  case CAnalysisType::AT_MIXTURE_CONTAINMENT:
    return new CGeomecContainmentMixtureDianaRunner(Mesh(), *this, controller);

  case CAnalysisType::AT_LINEAR:
  case CAnalysisType::AT_NONLIN:
  default:
    return new CGeomec3DDianaRunner(Mesh(), *this, controller);
  }
}
...
void C3DModel::CollectNodesFromBodyFace(TNodeElementsMap &mpNodeElements, const geo::IFace &face) const {
  ...
  for (j = 0; j < face.NrOfNodes(); ++j) {
    TNodeElementsMap::iterator it =
        mpNodeElements.insert(TNodeElementsMap::value_type(face.Node(j).Index(), std::set<int>())).first;
...
C3DModel::ModelType C3DModel::modelType() const { return e3DModel; }
