...
CSupportDepletion::CSupportDepletion(const CDepletionStage &stage, CBaseSupportNode &snode)
    : CStorageNode(stage.Name(), snode.Model()), m_pValueType(0), m_stage(stage) {
...
unsigned int CSupportDepletion::IconId() const { return IDI_DEPLETION_STAGE; }

unsigned int CSupportDepletion::TypeId() const { return 0; }
...
bool CSupportDepletion::CanDisconnectItem(const CGraphNode & /*item*/) const { return true; }

bool CSupportDepletion::Empty() const { return false; }
...
long CSupportDepletion::SavedItems() const { return 1; }

///// CBaseSupportNode

CBaseSupportNode::CBaseSupportNode(unsigned int uName, CBoundaryBase &boundary) : COpenGLNode(uName, boundary.Model()) {
  reParent(&boundary);
  LinkTo(*Model().GraphEntry(MD_BASE_DEPLETION_STAGE));
}

CBaseSupportNode::CBaseSupportNode(const CBaseSupportNode &rhs) : COpenGLNode(rhs) {}
...
bool CBaseSupportNode::Distributed(const CDepletionStage &stage) const { return DistriValues(stage) != 0; }

bool CBaseSupportNode::IsValueTypeAllowed(const CDepletionStage & /*stage*/) const {
  return Boundary().HasInterfaces();
}

const CBoundaryBase &CBaseSupportNode::Boundary() const { return static_cast<const CBoundaryBase &>(*parent()); }

CBoundaryBase &CBaseSupportNode::Boundary() { return static_cast<CBoundaryBase &>(*parent()); }
