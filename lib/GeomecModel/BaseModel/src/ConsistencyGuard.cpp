...
CConsistencyGuardOptions::CConsistencyGuardOptions()
    : consistencyGuardUsage(CGU_ALWAYS), propagateResultClearing(PRC_ALWAYS), propagateWellpaths(PWP_ALWAYS),
      movePointsets(MPS_ALWAYS), autoResultExport(ARE_ALWAYS), bLoaded(false) {}

bool CConsistencyGuardOptions::operator==(const CConsistencyGuardOptions &rhs) const { return !operator!=(rhs); }

bool CConsistencyGuardOptions::operator!=(const CConsistencyGuardOptions &rhs) const {
  return consistencyGuardUsage != rhs.consistencyGuardUsage || propagateResultClearing != rhs.propagateResultClearing ||
         propagateWellpaths != rhs.propagateWellpaths || movePointsets != rhs.movePointsets ||
         autoResultExport != rhs.autoResultExport;
}

// we maintain our own versioning
void CConsistencyGuardOptions::LoadStream(CStorageNode::TSTREAM &stream, CStorageNode::TSTREAMVERSION & /*version*/,
                                          CStorageNode::TPROGRESS & /*prg*/) {
  int optionsVersion;
  ...
  stream >> temp;
  if (!bLoaded)
    consistencyGuardUsage = (TConsistencyGuardUsageType)temp;
  stream >> temp;
  if (!bLoaded)
    propagateResultClearing = (TPropagateResultClearingType)temp;
  stream >> temp;
  if (!bLoaded)
    propagateWellpaths = (TPropagateWellpathType)temp;
  stream >> temp;
  if (!bLoaded)
    movePointsets = (TMovePointsetType)temp;
  stream >> temp;
  if (!bLoaded)
    autoResultExport = (TAutoResultExportType)temp;
...
CConsistencyGuard::CConsistencyGuard(CModelBase &rootModel)
    : m_rootModel(&rootModel), m_currentModel(&rootModel), m_storageFile(0), m_nAttaching(0), m_nChildLoading(0),
      m_nDeleting(0), m_status(UNKNOWN) {
  m_models.reserve(8);
  m_models.push_back(&rootModel);
}

CConsistencyGuard::~CConsistencyGuard() { ClearHDF5Interfaces(); }
...
{
  static const std::string GEOMEC_CONFIG_FILE = QUtil::native_sepatators( std::string( getenv("APPDATA") ) +
"/Geomec/geomec.xml");
  ...
  CXml gm_cfg( GEOMEC_CONFIG_FILE, {} , {"deformation","xsec_improvement"} ); //FIXME: already defined at 'ChangelogDlg'
(main app)
  ...
  for (TModelErrors::iterator it = m_modelErrors.begin(); it != m_modelErrors.end(); ++it) {
    ...
    for (TModelErrorEvents::iterator event = it->second.begin(); event != it->second.end(); ++event) {
      ...
      case WRONG_CLEARANCE_ATTEMPT:
        errors.append(
            QString("Model %1: a child model tried to delete results (%2 attempts)").arg(index).arg(event->second));
        break;
      }
    }
  }
...
CConsistencyGuardOptions CConsistencyGuard::Options() const { return m_options; }

void CConsistencyGuard::Options(CConsistencyGuardOptions &options) { m_options = options; }

void CConsistencyGuard::LoadStream(CStorageNode::TSTREAM &stream, CStorageNode::TSTREAMVERSION &version,
                                   CStorageNode::TPROGRESS &prg) {
...
void CConsistencyGuard::getChildren(CModelBase &model, std::vector<CModelBase *> &children) {
  ...
  for (std::vector<CModelBase *>::iterator descendant = m_models.begin() + 1; descendant != m_models.end();
       ++descendant)
...
bool CConsistencyGuard::AllowResultClearing(CModelBase &model) {
  ...
  // we should only be clearing results from the current model as root, unless we're creating new models or loading them
  // (because we don't switch there), or closing them
  return (m_rootModel->getDeleting() || m_nDeleting > 0 || m_nChildLoading > 0 || m_nAttaching > 0 ||
          m_currentModel == m_resultClearingOriginators.front() || isDescendant(model, *m_currentModel)) &&
         isDescendant(model, *m_resultClearingOriginators.front());
}
...
void CConsistencyGuard::AutoResultExport(CModelBase &model, CAnalysisType::TAnalysisType antype) {
  ...
  CExportResultDataEntry *pExportResultDataEntry =
      dynamic_cast<CExportResultDataEntry *>(model.GraphEntry(MD_BASE_EXPORT_MACROS));
