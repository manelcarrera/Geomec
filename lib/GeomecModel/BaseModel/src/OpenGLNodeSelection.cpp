...
COpenGLNodeSelection::COpenGLNodeSelection(const COpenGLNodeSelection &rhs) : m_stSelection(rhs.m_stSelection) {}
...
COpenGLNodeSelection::const_iterator COpenGLNodeSelection::begin() const { return m_stSelection.begin(); }

COpenGLNodeSelection::const_iterator COpenGLNodeSelection::end() const { return m_stSelection.end(); }

COpenGLNodeSelection::iterator COpenGLNodeSelection::begin() { return m_stSelection.begin(); }

COpenGLNodeSelection::iterator COpenGLNodeSelection::end() { return m_stSelection.end(); }
...
COpenGLNodeSelection::iterator COpenGLNodeSelection::find(const COpenGLNode &node) { return m_stSelection.find(&node); }
...
bool COpenGLNodeSelection::empty() const { return m_stSelection.empty(); }
...
void COpenGLNodeSelection::UnregisterObserver(CObserver &observer) { m_stObservers.erase(&observer); }
...
void COpenGLNodeSelection::LoadStream(CStorageNode::TSTREAM &stream, CStreamVersion & /*version*/,
                                      CStorageNode::TPROGRESS & /*progress*/, CFemAppModel &model) {
  THorizonBaseEntry *horizonBaseEntry = dynamic_cast<THorizonBaseEntry *>(model.GraphEntry(MD_BASE_HORIZON));
  TFormationBaseEntry *formationBaseEntry = dynamic_cast<TFormationBaseEntry *>(model.GraphEntry(MD_BASE_FORMATION));
  CHexaFormationEntry *hexaFormationEntry = dynamic_cast<CHexaFormationEntry *>(model.GraphEntry(MD_HEXA_FORMATION));
  CNewWellPathEntry *newWellPathEntry = dynamic_cast<CNewWellPathEntry *>(model.GraphEntry(MD_NEW_WELLPATH));
  TPointSetEntry *pointSetEntry = dynamic_cast<TPointSetEntry *>(model.GraphEntry(MD_BASE_POINTSET));
  ...
  for (unsigned long s = 0; s < size; ++s) {
    ...
    if ((horizonBaseEntry != 0) && (horizonBaseEntry->FindIndex(index) != 0)) {
    ...
    } else if ((formationBaseEntry != 0) && (formationBaseEntry->FindIndex(index) != 0)) {
      m_stSelection.insert(formationBaseEntry->FindIndex(index));
    } else if ((newWellPathEntry != 0) && (newWellPathEntry->FindIndex(index) != 0)) {
      ...
      if ((hexaFormationEntry != 0) && (hexaFormationEntry->FindIndex(parent) != 0)) {
        CHexaFormation *hexaFormation = hexaFormationEntry->FindIndex(parent);

        m_stSelection.insert(hexaFormation->FormationPlane(CFormationPlane::ePlaneType(planeType)).data());
      } else {
        assert(false);
      }
    }
...
void COpenGLNodeSelection::SaveStream(CStorageNode::TSTREAM &stream, CStorageNode::TPROGRESS & /*progress*/) {
  stream << (unsigned long)m_stSelection.size();

  for (TSelectionSet::const_iterator iterator = m_stSelection.begin(); iterator != m_stSelection.end(); ++iterator) {
    stream << (*iterator)->Index();

    if (dynamic_cast<const CFormationPlane *>(*iterator) != 0) {
      const CFormationPlane *formationPlane = dynamic_cast<const CFormationPlane *>(*iterator);

      stream << formationPlane->PlaneType();
      stream << dynamic_cast<CStorageNode *>(formationPlane->parent())->Index();
    }
  }
}
