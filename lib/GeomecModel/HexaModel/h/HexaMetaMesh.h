...
private:
  ...
  static void CreateLayerDefinition(IProgressBase &progress, CHexaMetaMesh &metaMesh, int index,
  ...
  void LoopOverFormations(IProgressBase &progress,
                          void (*func)(IProgressBase &progress, CHexaMetaMesh &, int, CHexaFormation &));

  void FillMainRegionLayerDepths(IProgressBase &progress, DefinitionLayer &def, CHexaHorizon &horizon,
  ...
  void local_Tyings_FindMasterRegion(const geo::IPoint &point, std::set<int> &regions, int itMeshRegion,
                                     int &nDanglingType, int &itMasterRegion, bool check_region);
  void Tyings_CreateTying(const geo::IVector &Xaxis, const geo::IVector &Yaxis, int index, int master1, int master2,
                          bool force_z, double calcdist);
  void Tyings_HandleX(const geo::IVector &Xaxis, const geo::IVector &Yaxis, DefinitionLayer &def, Layer &layer,
                      CRotatedSystem &rs, geo::CPoint &pp_local, int i_local, int nMasterRegion, bool do_z,
                      bool only_main);
  void Tyings_HandleY(const geo::IVector &Xaxis, const geo::IVector &Yaxis, DefinitionLayer &def, Layer &layer,
                      CRotatedSystem &rs, geo::CPoint &pp_local, int i_local, int nMasterRegion, bool do_z,
                      bool only_main);
  void Tyings_HandleZ(const geo::IVector &Xaxis, const geo::IVector &Yaxis, DefinitionLayer &def, Layer &layer,
                      Layer &top, Layer &bot, CRotatedSystem &rs, geo::CPoint &pp_local, int i_local, int nDanglingType,
                      int nMasterRegion);
  void Tyings_HandleXY(DefinitionLayer &def, Layer &layer, DefinitionLayer &topDef, Layer &topLayer, CRotatedSystem &rs,
                       geo::CPoint &pp_local, int i_local);
  Layer *find_top_layer(int layer, geo::CPoint &point, int i, std::set<int> &dregions, std::set<int> &lregions,
                        int &nDanglingType, int &nMasterRegion);
