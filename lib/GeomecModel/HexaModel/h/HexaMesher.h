...
class CHexaMesher : public IHexaMesher
{
 ...
 //typedef std::map<const geo::CInterfaceElement*, CHexaHorizon*> TInterfaceMap;
  ...
  void _Tyings_HandleVertical(_TyingsInfo& info, CRotatedSystem& rs, geo::CPoint& pp_local, CHexaMeshRegionBase* pMasterRegion);

  void _Tyings_Create(_TyingsInfo& info, int index, int nMaster1, int nMaster2, double calcdist, bool do_z);
  void _Tyings_CreateForPoints(_TyingsInfo& info, CRotatedSystem& rs, geo::CPoint& pp_local, bool force_z, std::vector<int>& points, TPointMap& mpPoint, int k0, int k1, double calcdist);
