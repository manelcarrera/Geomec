...
CHexaMesherNG::CHexaMesherNG(CHexaMesh &mesh) : IHexaMesher(mesh), m_metaMesh(mesh), m_bCanMesh(true) {}

CHexaMesherNG::~CHexaMesherNG() {}

int CHexaMesherNG::MesherType() const { return NEXT_GEN; }
...
bool CHexaMesherNG::CreateMesh(IProgressBase &progress) {
  ...
  if (ProblematicRegions()) {
    _m()->msg("Mesh regions with special refinements extend to the border of the main mesh and the tyings will clash "
              "with supports. Check your input.");
    return false;
  }
...
bool CHexaMesherNG::ProblematicRegions() {
  ...
  for (CHexaMeshRegionEntry::TNodeSet::iterator itMeshRegion = stNode.begin(); itMeshRegion != stNode.end();
    ...
    else if (dynamic_cast<CHexaSubMeshRegion *>(*itMeshRegion) &&
             !static_cast<CHexaSubMeshRegion *>(*itMeshRegion)->FullModel())
  ...
  for (std::set<CHexaSubMeshRegion *>::iterator itSubMeshRegion = stSubRegions.begin();
       itSubMeshRegion != stSubRegions.end(); ++itSubMeshRegion) {
    geo::CPoint ptRegionMin = rs.LocalMin((*itSubMeshRegion)->Hull());
    geo::CPoint ptRegionMax = rs.LocalMax((*itSubMeshRegion)->Hull());
    bool bInside = (ptRegionMin.X() > ptMeshMin.X() && ptRegionMin.Y() > ptMeshMin.Y() &&
                    ptRegionMax.X() < ptMeshMax.X() && ptRegionMax.Y() < ptMeshMax.Y());
...
void CHexaMesherNG::CreateTyings(IProgressBase &progress) {
  ...
  for (hmm::TTyingMap::iterator it = m_metaMesh.CreatedTyings().begin(); it != m_metaMesh.CreatedTyings().end(); ++it) {
    ...
    switch (v.size()) {
    case 6:
      Mesh().m_mesh.CreateTying(it->first, rsNormal[v[i].axis], geo::CTying::TR, v[i].master, rsNormal[v[i].axis],
                                geo::CTying::TR, v[i].distance, v[i + 1].master, rsNormal[v[i + 1].axis],
                                geo::CTying::TR, v[i + 1].distance);
      i += 2;
      /* Fall-through */
    case 4:
      Mesh().m_mesh.CreateTying(it->first, rsNormal[v[i].axis], geo::CTying::TR, v[i].master, rsNormal[v[i].axis],
...
bool CHexaMesherNG::CanCreateMesh() { return m_bCanMesh; }
...
long CHexaMesherNG::SavedItems() const { return const_cast<hmm::CHexaMetaMesh &>(m_metaMesh).CreatedTyings().size(); }
