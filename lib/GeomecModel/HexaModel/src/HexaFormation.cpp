...
CHexaFormationVolume::CHexaFormationVolume(C3DFormation &formation) : CFormationVolume(formation) {}
...
int CHexaFormationVolume::DisplayListSize() const {
  ...
  if (nRet) {
    ...
    const CHexaFormationEntry *pEntry =
        dynamic_cast<const CHexaFormationEntry *>(Model().GraphEntry(MD_HEXA_FORMATION));
    if (pEntry) {
      if (pEntry->ShowSkin())
        return 3;
...
void CHexaFormationVolume::buildSkin() const {
  ...
  // unfortunately the top and bottom planes are destroyed and recreated and can therefore not be added to the array as
  // well,
  ...
  for (i = 0; i < ElementSet().ElementSize(); ++i) {
    ...
    if (pBody) {
      ...
      for (j = 0; j < pBody->NrOfFaces(); ++j) {
        ...
        TNodeToFaceMap::iterator itInsert = mpNodeToFace.insert(TNodeToFaceMap::value_type(stNodes, TFaceVec())).first;
...
CHexaFormation::CHexaFormation(CFemAppModel &model) : C3DFormation(model), m_pUpper(0), m_pLower(0), m_nElements(1) {
...
CHexaFormation::CHexaFormation(const QString &strName, CHexaHorizon &upper, CHexaHorizon &lower, int nElements,
                               CModelBase &model)
...
bool CHexaFormation::Less(const CGraphNode &node) const {
  ...
  if (!pFor && pCSFor)
    pFor = (CHexaFormation *)(&(pCSFor->BaseObject()));

  if (pFor) {
    if ((&UpperHorizon() == &pFor->UpperHorizon()) && (&LowerHorizon() == &pFor->LowerHorizon()))
...
void CHexaFormation::SplitFormation(CHexaHorizon &horizon) {
  ...
  /*CHexaFormation *pFormation =*/new CHexaFormation(horizon.Name().toStdString().c_str(), horizon, *pLowerHorizon,
                                                     ELEMENTS_HEIGHT, (CModelBase &)Model());
...
int CHexaFormation::Elements() const { return m_nElements; }

void CHexaFormation::Elements(int nElements) { m_nElements = nElements; }

//=============================================================================
IValueDomainScalar::TValueVec CHexaFormation::CalculatePorePressure(const geo::IBody &body,
