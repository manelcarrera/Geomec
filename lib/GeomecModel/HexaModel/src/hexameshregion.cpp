...
//////////////////////////////////////////////////////////////////////
CHexaMeshRegionBase::CHexaMeshRegionBase(CFemAppModel &model) : CMeshRegionBase(model) {}

CHexaMeshRegionBase::CHexaMeshRegionBase(const QString &sName, CFemAppModel &model) : CMeshRegionBase(sName, model) {}
...
bool CHexaMeshRegionBase::operator==(const CHexaMeshRegionBase &rhs) const { return CMeshRegionBase::operator==(rhs); }
...
geo::CPoint CHexaMeshRegionBase::Min() const { return Hull().Min(); }

geo::CPoint CHexaMeshRegionBase::Max() const { return Hull().Max(); }
...
bool CHexaMeshRegionBase::IsInvalid() const { return ((m_vcGridPoints.Size() == 0) || (m_hull.NrOfPoints() == 0)); }
...
bool CHexaMeshRegionBase::Empty() const { return !IsLinkedTo(*((CModelBase &)Model()).GraphEntry(MD_HEXA_MESH_ZONE)); }

int CHexaMeshRegionBase::DisplayListSize() const { return 1; }
...
double CHexaMainMeshRegion::GridSizeX() const { return m_size_x; }

double CHexaMainMeshRegion::GridSizeY() const { return m_size_y; }
...
unsigned int CHexaMainMeshRegion::IconId() const { return IDI_MESH_MASTER; }

int CHexaMainMeshRegion::NrOfElemX() const { return int((m_pBoundary->Max().X() - m_pBoundary->Min().X()) / m_size_x); }

int CHexaMainMeshRegion::NrOfElemY() const { return int((m_pBoundary->Max().Y() - m_pBoundary->Min().Y()) / m_size_y); }
...
CHexaMainMeshRegion &CHexaMainMeshRegion::operator=(const CHexaMainMeshRegion &rhs) {
  ...
  if ((m_size_x != rhs.m_size_x) || (m_size_y != rhs.m_size_y)) {
    ...
    // Correct boundary
    //		CBoundaryBase::TMinMax minmax = m_pBoundary->SnapToGrid(CBoundaryBase::TMinMax(m_pBoundary->Min(),
    // m_pBoundary->Max())); 		m_pBoundary->Set(minmax.first, minmax.second);
...
void CHexaMainMeshRegion::OnNeighbourModified(const CGraphNode &node, enum ModifiedHint uHint) {
  ...
  if (pBoundary && (!IsInvalid())) {
    ...
    if ((Hull().Min() != pBoundary->Min()) || (Hull().Max() != pBoundary->Max())) {
...
bool CHexaMainMeshRegion::CanDestroy() const { return false; }
...
long CHexaMainMeshRegion::SavedItems() const { return 1 + CHexaMeshRegionBase::SavedItems(); }
...
CHexaMainMeshRegion *CHexaMainMeshRegion::getHexaMainMeshRegion(const CModelBase &modelBase) {
  CHexaMainMeshRegion *hexaMainMeshRegion = 0;
  const CHexaMeshRegionEntry *hexaMeshRegionEntry =
      dynamic_cast<const CHexaMeshRegionEntry *>(modelBase.GraphEntry(MD_HEXA_MESH_ZONE));

  assert(hexaMeshRegionEntry != 0);

  const CHexaMeshRegionEntry::TNodeSet &entryNodes = hexaMeshRegionEntry->EntryNodes();

  for (CHexaMeshRegionEntry::TNodeSet::const_iterator entryNode = entryNodes.begin();
       (entryNode != entryNodes.end()) && (hexaMainMeshRegion == 0); ++entryNode) {
...
CHexaSubMeshRegion::CHexaSubMeshRegion(const CHexaSubMeshRegion &rhs)
    : CHexaMeshRegionBase(rhs), m_exp_x(rhs.m_exp_x), m_exp_y(rhs.m_exp_y), m_pMain(rhs.m_pMain),
      m_formations(rhs.m_formations) {}

int CHexaSubMeshRegion::ExpX() const { return m_exp_x; }

int CHexaSubMeshRegion::ExpY() const { return m_exp_y; }

unsigned int CHexaSubMeshRegion::IconId() const { return IDI_MESH_REGION; }

double CHexaSubMeshRegion::GridSizeX() const { return Main().GridSizeX() / pow(double(2), m_exp_x); }

double CHexaSubMeshRegion::GridSizeY() const { return Main().GridSizeY() / pow(double(2), m_exp_y); }
...
const CHexaMainMeshRegion &CHexaSubMeshRegion::Main() const { return *m_pMain; }

CHexaMainMeshRegion &CHexaSubMeshRegion::Main() { return *m_pMain; }

void CHexaSubMeshRegion::ClearFormations() { m_formations.clear(); }
...
long CHexaSubMeshRegion::SavedItems() const { return m_polygon.NrOfPoints() + CHexaMeshRegionBase::SavedItems(); }
...
CHexaBoundary *CHexaMainMeshRegion::getBoundary() const { return m_pBoundary; }
...
geo::CBox GetBoundingMeshBox(geo::CArray<geo::CPoint> polygon) {
  ...
  for (size_t i = 0; i < polygon.Size(); i++) {
    ...
    if (polygon.Object(i).X() < minx)
      minx = x;
    else if (polygon.Object(i).X() > maxx)
      maxx = x;
    //		if (polygon.Object(i).X() < minx) minx = polygon.Object(i).X();
    //		else if (polygon.Object(i).X() > maxx) maxx = polygon.Object(i).X();
    if (polygon.Object(i).Y() < miny)
      miny = polygon.Object(i).Y();
    else if (polygon.Object(i).Y() > maxy)
      maxy = polygon.Object(i).Y();
  }

  return geo::CBox(geo::CPoint(minx, miny), geo::CPoint(maxx, maxy));
}

const geo::CArray<geo::CPoint> &CHexaMeshRegionBase::GetMeshRegion() { return MeshGrid(); }
...
int CHexaMeshRegionBase::IsDangling(const CHexaMeshRegionBase &meshzone, const geo::IPoint &pt,
...
const geo::CPolygon &CHexaSubMeshRegion::Polygon() const { return m_polygon; }
...
void CHexaSubMeshRegion::RecalcMeshRegion() {
  ...
  do {
    ...
    if (m_hull.NrOfPoints() > 2) {
      // Now check for lines where the next-next point is the same as the current point. If so the next point and the
      // next-next point can be deleted
      int i = 0;
      int j = 1;
      int k = 2;
      ...
      while (i < m_hull.NrOfPoints()) {
        ...
        i++;
        j++;
        if (j == m_hull.NrOfPoints())
          j = 0;
        k++;
        if (k == m_hull.NrOfPoints())
          k = 0;
      }
    }
  } while (bRemovedLines);
...
const CHexaMainMeshRegion &CHexaMeshRegionEntry::Main() const {
  CHexaMainMeshRegion *hexaMainMeshRegion =
      CHexaMainMeshRegion::getHexaMainMeshRegion(dynamic_cast<const CModelBase &>(Model()));
