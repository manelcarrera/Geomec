...
void CHexaMesh::CreateStandardSupports(const geo::ISurface& surface, TSupportMap& mpSupportMap, IProgressBase& progress)
{
  CHexaModel& model = static_cast<CHexaModel&>(Model());
  CHexaBoundary& boundary = static_cast<CHexaBoundary&>(model.Boundary());
  ...
  for(int n = 0; n < surface.Face(j).NrOfNodes(); ++n)
  {
    ...
    mpSupportMap.insert(TSupportMap::value_type(surface.Face(j).Node(n).Index(), geo::CCoordinateSet<geo::CVector>())).first;
...
void CHexaMesh::CreateBoundaryInterfaceElements(const geo::ISurface& surface, TSupportMap& mpSupportMap, IProgressBase& progress)
{
  for(int j = 0; j < surface.FaceSize(); ++j)
  {
...
void CHexaMesh::CreateBoundaryInterfaceElement(const geo::IFace& face, TSupportMap& mpSupportMap, IProgressBase& progress)
{
...
void CHexaMesh::InvalidateSupports()
{
  m_mesh.ClearSupports();
}
...
int CHexaMesh::DisplayListSize() const
{
  return 1;
}

const geo::IObject& CHexaMesh::DisplayList(int /*nIndex*/) const
{
  return m_mesh;
}

CHexaMesh::DIMENSION CHexaMesh::Dimension() const
{
  return CHexaMesh::DIM_3D;
}

unsigned int CHexaMesh::IconId() const
{
   return IDI_MESH;
}

unsigned int CHexaMesh::TypeId() const
{
  return 0;
}
...
void CHexaMesh::LoadStream(TSTREAM& stream, CStreamVersion& version, TPROGRESS& progress)
{
  ...
  if (m_pMesher->MesherType() == IHexaMesher::OLD_MESHER)
  {
    ...
    if(version >= CStreamVersion(4, 1, 2))
    {
        ...
        new geo::CInterfaceElement(m_mesh,
                                   &front.Face(nFrontFaceIndex),
                                   &back.Face(nBackFaceIndex),
                                   vcIndex);
      }
      else
      {
...
void CHexaMesh::old_load_stream(TSTREAM& stream, CStreamVersion& /*version*/, TPROGRESS& progress)
{
  ...
  for (lCount = 0; lCount < lBodySize; lCount++)
  {
    ...
    geo::CInterfaceElement *ptr = new geo::CInterfaceElement(m_mesh, &pUpper->Face(HEXA_LOWERFACE), &pLower->Face(HEXA_UPPERFACE), vcHexahedron);
...
void CHexaMesh::LoadSideSurface(geo::CBodyGroup::CSideSurface& side_surface, bool bTop, TSTREAM& stream, TPROGRESS& progress)
{
...
void CHexaMesh::LoadBodyGroup(geo::CBodyGroup& body_group, TSTREAM& stream, CStreamVersion &version, TPROGRESS& progress)
{
...
void CHexaMesh::SaveBodyGroup(const geo::CBodyGroup& body_group, TSTREAM& stream, TPROGRESS& progress)
{
  ...
  for(int j = 0; j < body_group.SideSurfaceSize(); j++)
  {
    ...
    {
      const geo::CBodyQuadrilateral& body_face = dynamic_cast<const geo::CBodyQuadrilateral&>(body_group.SideSurface(j).Face(i));
