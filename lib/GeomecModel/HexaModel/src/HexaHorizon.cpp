...
// default constructor...
CHexaHorizon::CHexaHorizon(CFemAppModel &model) : C3DHorizon(model), m_pInterfaceElementGroup(0) {}

CHexaHorizon::CHexaHorizon(const QString &strInstanceName, const double &dDepth, CFemAppModel &model,
                           bool bAttachToModel)
    : C3DHorizon(strInstanceName, dDepth, model, bAttachToModel), m_pInterfaceElementGroup(0) {
  if (bAttachToModel)
    AttachToEntry();
...
CHexaHorizon::CHexaHorizon(CSurfaceBase &surface, CFemAppModel &model, bool bSlip, bool bAttachToModel)
    : C3DHorizon(surface, model, bSlip, bAttachToModel), m_pInterfaceElementGroup(0) {
  if (bAttachToModel) {
    // Link to hexa horizon entry
    assert(model.GraphEntry(MD_HEXA_HORIZON));
    reParent(model.GraphEntry(MD_HEXA_HORIZON));
  ...
  if (pSurf) {
    if (!IsLinkedTo(item)) {
      if (SurfaceSize() == 0) {
        ...
        QString sMessage;
        sMessage =
            QString("Surface %1 is dropped on horizon %2. Do you want to merge them?").arg(item.Name()).arg(Name());
...
bool CHexaHorizon::LocalDestroy(CHexaFormation *pUpper, CHexaFormation *pLower, const CHexaFormation *selection) {
...
bool CHexaHorizon::operator==(const CHexaHorizon &rhs) const { return C3DHorizon::operator==(rhs); }
...
bool CHexaHorizon::CanMove() const { return !(static_cast<const CModelBase &>(Model())).BranchState().IsBranch(); }
...
void CHexaHorizonEntry::InsertHorizon(CHexaHorizon &horizon) {
    ...
    for (TNodeSet::iterator it = stHorizon.begin(); it != stHorizon.end(); it++) {
      if ((*it) != &horizon) {
        ...
        if (pOther->Min().Z() > horizon.Min().Z())
          new CHexaFormation(horizon.Name().toStdString().c_str(), horizon, *pOther, ELEMENTS_HEIGHT,
                             (CModelBase &)horizon.Model());
        else
          new CHexaFormation(pOther->Name().toStdString().c_str(), *pOther, horizon, ELEMENTS_HEIGHT,
                             (CModelBase &)horizon.Model());
      }
    }
    // Reorder tree
  ...
  if (horizon.Min().Z() < pFormation->UpperHorizon().Min().Z()) {
    new CHexaFormation(horizon.Name().toStdString().c_str(), horizon, (CHexaHorizon &)(pFormation->UpperHorizon()),
                       ELEMENTS_HEIGHT, (CModelBase &)horizon.Model());
    pModel->InvalidateMesh();
    horizon.Modified();
    return; // Exit
  }
  ...
  while (pFormation) {
    ...
    if (!pFormation->LowerFormation()) {
      new CHexaFormation(pFormation->LowerHorizon().Name().toStdString().c_str(),
                         (CHexaHorizon &)(pFormation->LowerHorizon()), horizon, ELEMENTS_HEIGHT,
                         (CModelBase &)horizon.Model());
      pModel->InvalidateMesh();
      horizon.Modified();
      return;
...
void CHexaHorizon::SaveStream(TSTREAM &stream, TPROGRESS &progress) { C3DHorizon::SaveStream(stream, progress); }

long CHexaHorizon::SavedItems() const { return C3DHorizon::SavedItems(); }
...
#ifdef _DEBUG
void CHexaHorizon::AssertValid() const { C3DHorizon::AssertValid(); }
#endif //_DEBUG
...
int CHexaHorizon::MeshedSurfaceSize() const { return 1; }
...
int CHexaHorizon::MeshFaceSize() const { return InterfaceElementGroup()->ElementSize(); }
...
const geo::IFace &CHexaHorizon::BodyFace(int nIndex) const {
  ...
  if (UpperFormation()) {
    if (nIndex < UpperFormation()->Volume(0).Volume().SideSurface(1).FaceSize())
      ...
      pFace = &LowerFormation()->Volume(0).Volume().SideSurface(0).Face(
          nIndex - UpperFormation()->Volume(0).Volume().SideSurface(1).FaceSize());
    }
  }
  if (!pFace) {
    assert(LowerFormation());
...
bool CHexaHorizon::CanSlip() const { return (UpperFormation() != 0 && LowerFormation() != 0); }

bool CHexaHorizon::IsTopHorizon() const { return UpperFormation() == 0; }
...
/*
CHexaHorizonEntryObserver::CHexaHorizonEntryObserver(CHexaHorizonEntry& entry, CTreeCtrl& ctrl, HTREEITEM hParent,
HTREEITEM hInsertAfter) : THexaHorizonEntryObserver(entry, ctrl, false, DELETE_ITEM, hParent, hInsertAfter, true, false)
{
  Update();
}
