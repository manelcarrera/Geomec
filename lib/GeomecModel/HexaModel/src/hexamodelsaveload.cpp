...
{
  return (
  LoadHexa1(stream, version, prog) &&
  LoadHexa2(stream, version, prog) &&
  LoadHexa3(stream, version, prog));
}
...
long CHexaModel::SavedItems() const
{
  ...
  */
  const CHexaFormationEntry& formation_entry = dynamic_cast<const CHexaFormationEntry&>(*GraphEntry(MD_HEXA_FORMATION));
