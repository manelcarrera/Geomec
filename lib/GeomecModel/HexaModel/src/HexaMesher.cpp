...
CHexaMesher::CHexaMesher(CHexaMesh &mesh) : IHexaMesher(mesh) {}

int CHexaMesher::MesherType() const { return OLD_MESHER; }
      ...
      for (itRegion = stNode.begin(); itRegion != stNode.end(); itRegion++) {
        ...
        if (bInside) {
          ...
          if ((*itRegion)->GridSizeX() > (*itMeshRegion)->GridSizeX() ||
            ...
            if (pErrMsg) {
              pErrMsg->append(
                  QString("The grid of region '%1' is courser than the grid of region '%2', "
                          "but region '%3' is completely inside region '%4'.\nThis is not allowed.\n\nMeshing aborted.")
                      .arg((*itRegion)->Name())
                      .arg((*itMeshRegion)->Name())
                      .arg((*itRegion)->Name())
                      .arg((*itMeshRegion)->Name()));
            }
            return false;
...
void CHexaMesher::ModifyMeshNodes(IProgressBase &progress) {
  ...
  for (int i = 0; i < iNumOfSets - 1; i++) {
    ...
    for (int j = 0; j < meshsize; j++) {
      if (Mesh().m_mesh.Point(m_pPoints[(i + 1) * meshsize + j]).Z() -
              Mesh().m_mesh.Point(m_pPoints[i * meshsize + j]).Z() <
          dMinThickness) {
        const geo::IPoint &p_old = Mesh().m_mesh.Point(m_pPoints[(i + 1) * meshsize + j]);
        Mesh().m_mesh.Node(
            m_pPoints[(i + 1) * meshsize + j],
            geo::CPoint(p_old.X(), p_old.Y(), Mesh().m_mesh.Point(m_pPoints[i * meshsize + j]).Z() + dMinThickness));
      }
    }
    // Next Formation.
    CHexaHorizon *pHor = &pFormation->LowerHorizon();
  ...
  for (int mesh = 0; mesh < iNumOfSets - 1; mesh++) {
    ...
    for (int bl = 1; bl < meshblock; bl++) {
      for (int it = 0; it < meshsize; it++) {
        ...
        int nIndex = Mesh().m_mesh.RegisterNode(
            geo::CPoint(point.X(), point.Y(), point.Z() + bl * ((point_ref.Z() - point.Z()) / meshblock)));
...
void CHexaMesher::CreateHexahedronMesh(IProgressBase &progress) {
  ...
  geo::CLine
      CheckLine; // This line is the line from p to n in the hexahedron that was just created. If the p of the next
  ...
  for (int mesh = 0; mesh < iNumOfSets - 1; mesh++) {
    ...
    for (it = iCount; it < (mesh * meshsize) + meshsize; it++) {
      progress.Step();
      found = false;
      if (++iCount > (mesh * meshsize) + meshsize)
        break;
      p = m_pPoints[it];

      if (it != 0) { // Do not perform the check when this is the first point
      ...
      for (next = it + 1; next < (mesh * meshsize) + meshsize; next++) {
        if (++iNextCount > (mesh * meshsize) + meshsize)
          break;
        ...
        if (fabs(rs.ToLocal(Mesh().m_mesh.Point(n)).Y() - rs.ToLocal(Mesh().m_mesh.Point(p)).Y()) < EPS) {
          ...
          for (up = next + 1; up < (mesh * meshsize) + meshsize; up++) {
            if (++iUpCount > (mesh * meshsize) + meshsize)
              break;
            ...
            if (fabs(rs.ToLocal(Mesh().m_mesh.Point(u)).X() - rs.ToLocal(Mesh().m_mesh.Point(p)).X()) < EPS) {
              ...
              for (upnext = up + 1; upnext < (mesh * meshsize) + meshsize; upnext++) {
                if (++iUpNextCount > (mesh * meshsize) + meshsize)
                  break;
                ...
                if ((fabs(rs.ToLocal(Mesh().m_mesh.Point(v)).X() - rs.ToLocal(Mesh().m_mesh.Point(n)).X()) < EPS) &&
                  ...
                  geo::CPoint ptCenter(local_p.X() + (dXOff / 2), local_p.Y() + (dYOff / 2), 0);
                  ...
                  for (size_t l = 0; l < m_vcRegions.size(); l++) {
                    ...
                    if (pReg->InsideRegion(rs.ToGlobal(ptCenter), false) && fabs(dXOff - dGridsizeX) < EPS &&
                        fabs(dYOff - dGridsizeY) < EPS && dRest < EPS)
                      break;
                  ...
                  for (int bl = 0; bl < meshblock; bl++) {
                    ...
                    if (bl == 0) {
                      ...
                      if ((pPrevHorizon != 0) && pPrevHorizon->Slip()) {
                        ...
                        hexpoints.push_back(
                            m_pSlipPoints[(iCount - 1) - (mesh * meshsize) + (iSlipCounter - 1) * meshsize]);
                        hexpoints.push_back(
                            m_pSlipPoints[(iNextCount - 1) - (mesh * meshsize) + (iSlipCounter - 1) * meshsize]);
                        hexpoints.push_back(
                            m_pSlipPoints[(iUpNextCount - 1) - (mesh * meshsize) + (iSlipCounter - 1) * meshsize]);
                        hexpoints.push_back(
                            m_pSlipPoints[(iUpCount - 1) - (mesh * meshsize) + (iSlipCounter - 1) * meshsize]);
                      } else {
                      ...
                      hexpoints.push_back(
                          m_pPointsInternal[(iInternalCount + bl - 1) * meshsize + (iCount - mesh * meshsize) - 1]);
                      hexpoints.push_back(
                          m_pPointsInternal[(iInternalCount + bl - 1) * meshsize + (iNextCount - mesh * meshsize) - 1]);
                      hexpoints.push_back(m_pPointsInternal[(iInternalCount + bl - 1) * meshsize +
                                                            (iUpNextCount - mesh * meshsize) - 1]);
                      hexpoints.push_back(
                          m_pPointsInternal[(iInternalCount + bl - 1) * meshsize + (iUpCount - mesh * meshsize) - 1]);
                    }
                    ...
                    if (bl == meshblock - 1) {
                      ...
                      hexpoints.push_back(
                          m_pPointsInternal[(iInternalCount + bl) * meshsize + (iCount - mesh * meshsize) - 1]);
                      hexpoints.push_back(
                          m_pPointsInternal[(iInternalCount + bl) * meshsize + (iNextCount - mesh * meshsize) - 1]);
                      hexpoints.push_back(
                          m_pPointsInternal[(iInternalCount + bl) * meshsize + (iUpNextCount - mesh * meshsize) - 1]);
                      hexpoints.push_back(
                          m_pPointsInternal[(iInternalCount + bl) * meshsize + (iUpCount - mesh * meshsize) - 1]);
                    }
...
void CHexaMesher::_Tyings_SetupPoints(_TyingsInfo &info) {
  ...
  // add the slip-points first, so the test below for the lowest points (which should not be tied in Z-direction) still
  // works
...
void _local_Tyings_FindMasterRegion(const geo::IPoint &point, CHexaMeshRegionEntry::TNodeSet &stNode,
                                    CHexaMeshRegionEntry::TNodeSet::iterator &itMeshRegion, int &nDanglingType,
                                    CHexaMeshRegionBase **pMasterRegion) {
  std::vector<CHexaMeshRegionBase *> vcRegions;
  vcRegions.reserve(20);

  for (CHexaMeshRegionEntry::TNodeSet::iterator itTempMeshReg = stNode.begin(); itTempMeshReg != stNode.end();
  ...
  if (*pMasterRegion)
    nDanglingType = (*pMasterRegion)->IsDangling(*(*itMeshRegion), point, true);
}
...
void CHexaMesher::_Tyings_Create(_TyingsInfo &info, int index, int nMaster1, int nMaster2, double calcdist, bool do_z) {
  ...
  Mesh().m_mesh.CreateTying(index, info.Xaxis, geo::CTying::TR, nMaster1, info.Xaxis, geo::CTying::TR, 1 - calcdist,
                            nMaster2, info.Xaxis, geo::CTying::TR, calcdist);
  Mesh().m_mesh.CreateTying(index, info.Yaxis, geo::CTying::TR, nMaster1, info.Yaxis, geo::CTying::TR, 1 - calcdist,
  ...
  if (do_z) {
    Mesh().m_mesh.CreateTying(index, geo::CVector::Zaxis, geo::CTying::TR, nMaster1, geo::CVector::Zaxis,
                              geo::CTying::TR, 1 - calcdist, nMaster2, geo::CVector::Zaxis, geo::CTying::TR, calcdist);
  }

  info.mpCreatedTyings.insert(std::make_pair(index, std::make_pair(nMaster1, nMaster2)));
}

void CHexaMesher::_Tyings_CreateForPoints(_TyingsInfo &info, CRotatedSystem &rs, geo::CPoint &pp_local, bool force_z,
                                          std::vector<int> &points, TPointMap &mpPoint, int k0, int k1,
                                          double calcdist) {
  int cnt = 0;
  ...
  for (size_t i = 0; i < points.size(); i++) {
    ...
    if ((fabs(local.X() - pp_local.X()) < EPS) && (fabs(local.Y() - pp_local.Y()) < EPS)) {
      ...
      if (itTying == info.mpCreatedTyings.end()) {
        ...
        _Tyings_Create(info, points[i], nMaster1, nMaster2, calcdist,
                       force_z || (points.size() - i > m_aMeshGrid.Size()));
      } else {
        // make sure it's the same
        assert((mpPoint[&info.basicpoints->Object(k0)][cnt] == itTying->second.first &&
                mpPoint[&info.basicpoints->Object(k1)][cnt] == itTying->second.second) ||
               (mpPoint[&info.basicpoints->Object(k1)][cnt] == itTying->second.first &&
                mpPoint[&info.basicpoints->Object(k0)][cnt] == itTying->second.second));
      }
      cnt++;
    }
  }
...
void CHexaMesher::_Tyings_HandleVertical(_TyingsInfo &info, CRotatedSystem &rs, geo::CPoint &pp_local,
                                         CHexaMeshRegionBase *pMasterRegion) {
              ...
              if (fabs(local_u.X() - local_p.X()) < EPS) {
                ...
                for (upnext = up + 1; upnext < (mesh * meshsize) + meshsize; upnext++) {
                  ...
                  if ((fabs(local_v.X() - local_n.X()) < EPS) && (fabs(local_u.Y() - local_v.Y()) < EPS)) {
                    ...
                    hexpoints.push_back(
                        m_pSlipPoints[(iUpNextCount - 1) - (mesh * meshsize) + iSlipCounter * meshsize]);
                    ...
                    const geo::IBody &upper =
                        pFormation->Volume(0).Volume().Body((iNrHexPerLayer - 1) * nrElements + nrElements - 1);

                    CHexaHorizon *pHor = &pFormation->LowerHorizon();
                    nrElements = pHor->LowerFormation()->Elements();
                    const geo::IBody &lower =
                        pHor->LowerFormation()->Volume(0).Volume().Body((iNrHexPerLayer - 1) * nrElements);

                    const geo::CBodyQuadrilateral &upperface =
                        dynamic_cast<const geo::CBodyQuadrilateral &>(upper.Face(HEXA_LOWERFACE));
                    const geo::CBodyQuadrilateral &lowerface =
                        dynamic_cast<const geo::CBodyQuadrilateral &>(lower.Face(HEXA_UPPERFACE));

                    // Add the interface element to the list.
                    geo::CInterfaceElement *ptr =
                        new geo::CInterfaceElement(Mesh().m_mesh, &upperface, &lowerface, hexpoints);
...
bool CHexaMesher::OnGridLine(const geo::IPoint &pt1, const geo::IPoint &pt2) const {
  if (fabs(pt1.X() - pt2.X()) < EPS ||
      fabs(pt1.Y() - pt2.Y()) < EPS) { // the line is horizontal or vertical. could be exactly on a grid line

    CHexaMeshRegionEntry *pEntry =
        (CHexaMeshRegionEntry *)(const_cast<CHexaMesher &>(*this).Model().GraphEntry(MD_HEXA_MESH_ZONE));
    CHexaMeshRegionEntry::TNodeSet stNode = pEntry->EntryNodes();
...
void CHexaMesher::SavePointIndices(const std::vector<int> &vcIndex, CArchiveInterface &stream,
                                   IProgressBase &progress) {
...
long CHexaMesher::SavedItems() const { return m_pPoints.size() + m_pPointsInternal.size() + m_pSlipPoints.size(); }
