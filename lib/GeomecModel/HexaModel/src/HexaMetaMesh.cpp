...
public:
  ...
  bool find(CRotatedSystem &rs, const DefinitionLayer &def, int &start, const geo::CPoint &local_point_0,
            geo::CPoint &point_1, int axis, int mask);
  bool findr(CRotatedSystem &rs, const DefinitionLayer &def, int &start, const geo::CPoint &local_point_0,
             geo::CPoint &point_1, int axis, int region, bool only_main = false);

  bool marked(int index) const;
  void mark(int index);
};

Layer::Layer(int def) : definition(def) {}

Layer::Layer(int def, std::vector<double> &depths) : definition(def), depths(depths) {}

bool Layer::main_layer() const { return regions.find(0) != regions.end(); }
...
bool Layer::find(CRotatedSystem &rs, const DefinitionLayer &def, int &start, const geo::CPoint &local_point_0,
                 geo::CPoint &point_1, int axis, int mask) {
...
bool Layer::findr(CRotatedSystem &rs, const DefinitionLayer &def, int &start, const geo::CPoint &local_point_0,
                  geo::CPoint &point_1, int axis, int region, bool only_main) {
  bool found = false;

  while (!found) {
    while (start < depths.size() &&
           (only_main ? def.mask[start].find(0) == def.mask[start].end() : !active(def, start, region)))
      ++start;
...
bool Layer::marked(int index) const { return marks[index] > 0; }

void Layer::mark(int index) { ++marks[index]; }
...
public:
  int i[4];
  double iso[4];
  bool find_horizontal_rectangle(CRotatedSystem &rs, DefinitionLayer &def, Layer &layer, int index,
                                 bool only_main = false);
  bool find_vertical_square_x(CRotatedSystem &rs, DefinitionLayer &def, Layer &layer, Layer &top, Layer &bot,
                              geo::CPoint &pp_local, int index, int nMasterRegion);
  bool find_vertical_square_y(CRotatedSystem &rs, DefinitionLayer &def, Layer &layer, Layer &top, Layer &bot,
                              geo::CPoint &pp_local, int index, int nMasterRegion, double gridSizeY);
...
bool Quadrilateral::find_horizontal_rectangle(CRotatedSystem &rs, DefinitionLayer &def, Layer &layer, int index,
                                              bool only_main) {
...
bool Quadrilateral::find_vertical_square_x(CRotatedSystem &rs, DefinitionLayer &def, Layer &layer, Layer &top,
                                           Layer &bot, geo::CPoint &pp_local, int index, int nMasterRegion) {
...
bool Quadrilateral::find_vertical_square_y(CRotatedSystem &rs, DefinitionLayer &def, Layer &layer, Layer &top,
                                           Layer &bot, geo::CPoint &pp_local, int index, int nMasterRegion,
                                           double gridSizeY) {
  for (int k = index - 1; k >= 0; --k) {
    if (!top.active(def, k, nMasterRegion))
...
CHexaMetaMesh::CHexaMetaMesh(CHexaMesh &mesh) : m_mesh(mesh) {}

CHexaMetaMesh::~CHexaMetaMesh() { clear(); }

CModelBase &CHexaMetaMesh::Model() { return static_cast<CModelBase &>(m_mesh.Model()); }

CHexaMesh &CHexaMetaMesh::Mesh() { return m_mesh; }
...
void CHexaMetaMesh::CreateHexaMetaMesh(IProgressBase &progress) {
  ...
  // disable for checkin; this whole BuildMetaMesh is only gathering data yet, so could be called without impact (other
...
void CHexaMetaMesh::CreateLayerDefinition(IProgressBase &progress, CHexaMetaMesh &metaMesh, int /*index*/,
                                          CHexaFormation &formation) {
  ...
  for (std::vector<CHexaMeshRegionBase *>::iterator itMeshRegion = metaMesh.m_regions.begin();
       itMeshRegion != metaMesh.m_regions.end(); ++itMeshRegion) {
    ...
    if (dynamic_cast<CHexaSubMeshRegion *>(*itMeshRegion)) {
      ...
      for (std::vector<CHexaMeshRegionBase *>::iterator itRegion = metaMesh.m_regions.begin();
           itRegion != metaMesh.m_regions.end(); ++itRegion) {
        ...
        if ((*itRegion)->GridSizeX() > (*itMeshRegion)->GridSizeX() ||
            (*itRegion)->GridSizeY() > (*itMeshRegion)->GridSizeY()) {
          if (bInside) {
    ...
    if (vcRegsInCurrentReg.size() == 0) {
      for (geo::CArray<geo::CPoint>::iterator PtIt = pointszone.begin(); PtIt != pointszone.end(); ++PtIt) {
        TPointMap::iterator it =
            pointMap.insert(TPointMap::value_type(rs.ToLocal(*PtIt), std::make_pair(*PtIt, std::set<int>()))).first;
...
void CHexaMetaMesh::CreateLayer(IProgressBase &progress, CHexaMetaMesh &metaMesh, int index,
                                CHexaFormation &formation) {
...
void CHexaMetaMesh::FillMainRegionLayerDepths(IProgressBase &progress, DefinitionLayer &def, CHexaHorizon &horizon,
                                              std::vector<double> &depths) {
...
void CHexaMetaMesh::LoopOverFormations(IProgressBase &progress,
                                       void (*func)(IProgressBase &progress, CHexaMetaMesh &, int, CHexaFormation &)) {
...
void CHexaMetaMesh::RegisterNodes(IProgressBase &progress, CHexaMetaMesh &metaMesh, int index) {
  ...
  if (index > 0 && layer.definition != metaMesh.layers[index - 1]->definition && !def.formation->UpperHorizon().Slip())
    std::copy(metaMesh.layers[index - 1]->points.begin(), metaMesh.layers[index - 1]->points.end(),
              layer.points.begin());
...
void CHexaMetaMesh::CreateHexahedrons(IProgressBase &progress, CHexaMetaMesh &metaMesh, int index) {
  ...
  for (i0 = 0; i0 < size; ++i0) {
    if (square.find_horizontal_rectangle(rs, def, top, i0)) {
      ...
      std::set_intersection(def.mask[i0].begin(), def.mask[i0].end(), def.mask[i1].begin(), def.mask[i1].end(),
                            std::inserter(one, one.end()));
      std::set_intersection(def.mask[i2].begin(), def.mask[i2].end(), def.mask[i3].begin(), def.mask[i3].end(),
                            std::inserter(two, two.end()));
      ...
      for (std::set<int>::iterator it = three.begin(); it != three.end(); ++it) {
        if (*it < 0)
          continue;
        double tmpMin = metaMesh.m_regions[*it]->GridSizeX() < metaMesh.m_regions[*it]->GridSizeY()
                            ? metaMesh.m_regions[*it]->GridSizeX()
                            : metaMesh.m_regions[*it]->GridSizeY();
        if (tmpMin < minGrid) {
          minGrid = tmpMin;
          n = *it;
        }
...
void CHexaMetaMesh::CreateInterfaceElements(IProgressBase &progress, CHexaMetaMesh &metaMesh, int index) {
  ...
  for (i0 = 0; i0 < size; ++i0) {
    if (square.find_horizontal_rectangle(rs, topDef, top, i0)) {
      ...
      const geo::CBodyQuadrilateral &topFace =
          static_cast<const geo::CBodyQuadrilateral &>(topHex.Face(HEXA_LOWERFACE));
      const geo::CBodyQuadrilateral &botFace =
          static_cast<const geo::CBodyQuadrilateral &>(botHex.Face(HEXA_UPPERFACE));
...
void CHexaMetaMesh::local_Tyings_FindMasterRegion(const geo::IPoint &point, std::set<int> &regions, int itMeshRegion,
                                                  int &nDanglingType, int &itMasterRegion, bool check_region) {
  std::vector<int> vcRegions;
  vcRegions.reserve(20);

  itMasterRegion = -1;

  for (std::set<int>::iterator itTempMeshReg = regions.begin(); itTempMeshReg != regions.end();
       itTempMeshReg++) { // find the regions that contain the point (excluding the edge)
    if (*itTempMeshReg == itMeshRegion)
      continue;
  ...
  for (size_t k = 0; k < vcRegions.size();
       k++) { // pMasterRegion should be the region with the smallest grid size (either in X or in Y direction)
  ...
  if (itMasterRegion >= 0)
    nDanglingType =
        m_regions[check_region ? itMasterRegion : 0]->IsDangling(*m_regions[itMeshRegion], point, check_region);
}

void CHexaMetaMesh::Tyings_CreateTying(const geo::IVector &Xaxis, const geo::IVector &Yaxis, int index, int nMaster1,
                                       int nMaster2, bool do_z, double calcdist) {
  if (mpCreatedTyings.find(index) == mpCreatedTyings.end()) {
    ...
    Mesh().m_mesh.CreateTying(index, Xaxis, geo::CTying::TR, nMaster1, Xaxis, geo::CTying::TR, 1 - calcdist, nMaster2,
                              Xaxis, geo::CTying::TR, calcdist);
    Mesh().m_mesh.CreateTying(index, Yaxis, geo::CTying::TR, nMaster1, Yaxis, geo::CTying::TR, 1 - calcdist, nMaster2,
                              Yaxis, geo::CTying::TR, calcdist);
    if (do_z) {
      v[4].axis = 2;
      v[4].master = nMaster1;
      ...
      Mesh().m_mesh.CreateTying(index, geo::CVector::Zaxis, geo::CTying::TR, nMaster1, geo::CVector::Zaxis,
                                geo::CTying::TR, 1 - calcdist, nMaster2, geo::CVector::Zaxis, geo::CTying::TR,
                                calcdist);
    }
...
void CHexaMetaMesh::Tyings_HandleX(const geo::IVector &Xaxis, const geo::IVector &Yaxis, DefinitionLayer &def,
                                   Layer &layer, CRotatedSystem &rs, geo::CPoint &pp_local, int i_local,
                                   int nMasterRegion, bool do_z, bool only_main) {
  ...
  for (int k = i_local - 1; k >= 0; --k) {
    ...
    if (fabs(pp_local.Y() - local_0.Y()) < EPS) {
      for (int l = i_local + 1; l < layer.depths.size(); ++l) {
        ...
        if ((pp_local.X() + EPS > local_0.X()) && (pp_local.X() < local_1.X() + EPS)) {
          ...
          //_Tyings_CreateForPoints(info, rs, pp_local, true, m_pPointsInternal, info.mpInternalPoint, k, k + 1,
          // calcdist);
    ...
    // We have past the search point so exit the loop.
    if (local_0.Y() > pp_local.Y() + EPS)
      break;
  }
}

void CHexaMetaMesh::Tyings_HandleY(const geo::IVector &Xaxis, const geo::IVector &Yaxis, DefinitionLayer &def,
                                   Layer &layer, CRotatedSystem &rs, geo::CPoint &pp_local, int i_local,
                                   int nMasterRegion, bool do_z, bool only_main) {
  geo::CPoint local_0, local_1;
  ...
  for (int k = i_local - 1; k >= 0; --k) {
    ...
    if ((fabs(pp_local.X() - local_0.X()) < EPS) &&
        ((pp_local.Y() - local_0.Y()) < m_regions[nMasterRegion]->GridSizeY())) {
      for (int l = i_local + 1; l < layer.depths.size(); ++l) {
        if (!layer.active(def, l, nMasterRegion))
          continue;
        ...
        if ((fabs(pp_local.X() - local_1.X()) < EPS) &&
            ((local_1.Y() - pp_local.Y()) < m_regions[nMasterRegion]->GridSizeY())) {
...
void CHexaMetaMesh::Tyings_HandleZ(const geo::IVector &Xaxis, const geo::IVector &Yaxis, DefinitionLayer &def,
                                   Layer &layer, Layer &top, Layer &bot, CRotatedSystem &rs, geo::CPoint &pp_local,
                                   int i_local, int nDanglingType, int nMasterRegion) {
  ...
  if (nDanglingType == MESH_NO_DANG) {
    ...
    Quadrilateral square;
    if (square.find_vertical_square_y(rs, def, layer, top, bot, pp_local, i_local, nMasterRegion,
                                      m_regions[nMasterRegion]->GridSizeY())) {
      int nodes[4];
      double isoPt[4];
...
void CHexaMetaMesh::Tyings_HandleXY(DefinitionLayer &def, Layer &layer, DefinitionLayer &topDef, Layer &topLayer,
                                    CRotatedSystem &rs, geo::CPoint &pp_local, int i_local) {
  if (mpCreatedTyings.find(layer.points[i_local]) != mpCreatedTyings.end())
    return;
...
Layer *CHexaMetaMesh::find_top_layer(int layer, geo::CPoint &point, int i, std::set<int> &dregions,
                                     std::set<int> &lregions, int &nDanglingType, int &nMasterRegion) {
  nMasterRegion = -1;
      ...
      ++count;
          ...
          if (layer.active(def, i, *reg) && metaMesh.m_regions[*reg]->InsideRegion(point, true)) {
            if (!metaMesh.m_regions[*reg]->InsideRegion(point, false)) {
              double min = metaMesh.m_regions[*reg]->GridSizeX() < metaMesh.m_regions[*reg]->GridSizeY()
                               ? metaMesh.m_regions[*reg]->GridSizeX()
                               : metaMesh.m_regions[*reg]->GridSizeY();
        ...
        if (/*count == 1 && */ !skip && region >= 0) {
          Layer *top =
              metaMesh.find_top_layer(index, point, region, def.mask[i], layer.regions, nDanglingType, nMasterRegion);
          Layer *bot = metaMesh.find_bot_layer(index, nMasterRegion);

          geo::CPoint pp_local = rs.ToLocal(point);

          if (layer.main_layer() && nMasterRegion > 0 && nMasterRegion != region &&
              metaMesh.m_regions[nMasterRegion]->InsideRegion(point, false)) {
            if (nDanglingType != MESH_NO_DANG && nDanglingType != MESH_NOT_ON_GRID) {
              layer.mark(i);
            }
          } else {
        ...
        for (std::set<int>::iterator it = layer.regions.begin(); it != layer.regions.end(); ++it) {
          int nDanglingType = MESH_NO_DANG, nMasterRegion;
          metaMesh.local_Tyings_FindMasterRegion(point, layer.regions, *it, nDanglingType, nMasterRegion,
                                                 !switch_region);
          ...
          if (nDanglingType != MESH_NO_DANG && nDanglingType != MESH_NOT_ON_GRID) {
            ...
            if (topLayer) // && (!topLayer->active(*topDef, i) /* || !topLayer->main_node(*topDef, i) || topDef->mask[i]
                          // != def.mask[i] */))
            {
              bool t0 = !topLayer->active(*topDef, i);
              bool t1 = false; // !topLayer->main_node(*topDef, i);
              ...
              if (t0 || t1 || t2) {
                nDanglingType = MESH_NO_DANG;
                metaMesh.local_Tyings_FindMasterRegion(point, topLayer->regions, *it, nDanglingType, nMasterRegion,
                                                       false);
            ...
            if (botLayer) //&& (!botLayer->active(*botDef, i) /* || !botLayer->main_node(*botDef, i) || botDef->mask[i]
                          //!= def.mask[i] */))
            {
              bool t0 = !botLayer->active(*botDef, i);
              bool t1 = false; // !botLayer->main_node(*botDef, i);
              ...
              if (t0 || t1 || t2) {
                nDanglingType = MESH_NO_DANG;
                metaMesh.local_Tyings_FindMasterRegion(point, botLayer->regions, *it, nDanglingType, nMasterRegion,
                                                       false);
      ...
      ++count;
        ...
        if (/*count == 1 && */ !skip && region >= 0) {
          ...
          if (layer.main_layer() && nMasterRegion > 0 && nMasterRegion != region &&
            ...
            metaMesh.Tyings_HandleZ(Xaxis, Yaxis, def, layer, *top, *bot, rs, pp_local, i, nDanglingType,
                                    nMasterRegion);
          }
        }
      } else {
        for (std::set<int>::iterator it = layer.regions.begin(); it != layer.regions.end(); ++it) {
          ...
          if (nDanglingType != MESH_NO_DANG && nDanglingType != MESH_NOT_ON_GRID) {
            ...
            if (nDanglingType == MESH_HOR_DANG) {
              metaMesh.Tyings_HandleX(Xaxis, Yaxis, def, layer, rs, pp_local, i, nMasterRegion,
                                      switch_region || not_last, false);
            } else // nDanglingType == MESH_VER_DANG
            {
              metaMesh.Tyings_HandleY(Xaxis, Yaxis, def, layer, rs, pp_local, i, nMasterRegion,
                                      switch_region || not_last, false);
            }
          } else {
            if (topLayer) // && (!topLayer->active(*topDef, i) /* || !topLayer->main_node(*topDef, i) || topDef->mask[i]
...
void CHexaMetaMesh::LoopOverLayers(IProgressBase &progress,
                                   void (*func)(IProgressBase &progress, CHexaMetaMesh &, int)) {
