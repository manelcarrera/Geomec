...
CHexaBoundary::CHexaBoundary(const geo::IPoint& ptMin,
...
void CHexaBoundary::OnUpdateBoundary()
{
...

  {
    ...
    geo::CPoint ptMax(MaxPoint());
      Set(geo::CPoint(ptMin.X(), ptMin.Y(), bestfit.first.Z()), 
        geo::CPoint(ptMax.X(), ptMax.Y(), bestfit.second.Z()));
  }
...
{
  return CHexaMainMeshRegion::getHexaMainMeshRegion(
...
const CHexaHorizon &CHexaBoundary::GetTopHorizon() const
{
  ...
  while(pFor->UpperFormation()) pFor = pFor->UpperFormation();
...
const CHexaHorizon &CHexaBoundary::GetBottomHorizon() const
{
  ...
  assert(!stNode.empty());
  const CHexaFormation *pFor = dynamic_cast<const CHexaFormation *> (*stNode.begin());
  while(pFor->LowerFormation()) pFor = pFor->LowerFormation();
...
std::vector<const geo::ISurface*> CHexaBoundary::GetSideMeshSurfaces() const
{
  ...
  for(std::set<const geo::IObject*, CMinMaxLess>::const_iterator it2 = stObjects.begin(); it2 != stObjects.end(); ++it2)
...
bool CHexaBoundary::CanRotate() const
{
  return true;
}
