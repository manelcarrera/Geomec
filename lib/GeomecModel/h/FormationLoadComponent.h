...
template <class VALUE_TYPE> class CFormationLoadComponent : public IValueComponentBase {
  bool m_bEmpty;
...
CFormationLoadComponent<VALUE_TYPE>::CFormationLoadComponent(CFormationLoadBase<VALUE_TYPE> &formationload,
                                                             unsigned int uMode, bool /*bEmpty*/)
...
template <class VALUE_TYPE> IValueComponentBase::TValueComponentType CFormationLoadComponent<VALUE_TYPE>::Type() const {
  return static_cast<IValueComponentBase::TValueComponentType>(ScalarData().Type());
}

template <class VALUE_TYPE> const IValueDataInterfaceScalar &CFormationLoadComponent<VALUE_TYPE>::ScalarData() const {
  return m_Data;
}

template <class VALUE_TYPE> IValueDataInterfaceScalar &CFormationLoadComponent<VALUE_TYPE>::ScalarData() {
  return m_Data;
}

template <class VALUE_TYPE>
const CFormationLoadBase<VALUE_TYPE> &CFormationLoadComponent<VALUE_TYPE>::FormationLoadBase() const {
  const CFormationLoadBase<VALUE_TYPE> *pFormationLoad =
      dynamic_cast<const CFormationLoadBase<VALUE_TYPE> *>(&Parent());
  assert(pFormationLoad);
  return *pFormationLoad;
}

template <class VALUE_TYPE> CFormationLoadBase<VALUE_TYPE> &CFormationLoadComponent<VALUE_TYPE>::FormationLoadBase() {
...
template <class VALUE_TYPE>
IValueDomainScalar::TMinMax CFormationLoadComponent<VALUE_TYPE>::MinMax(IProgressBase &progressBase,
...
template <class VALUE_TYPE>
IValueDomainScalar::TValue CFormationLoadComponent<VALUE_TYPE>::Average(IProgressBase &progressBase,
                                                                        const CQuantity::UNIT unit) const {
  geo::CBox box(FormationLoadBase().Min(), FormationLoadBase().Max());
...
CFormationLoadComponent<VALUE_TYPE>::ValueElement(const geo::IElement &elm, const CQuantity::UNIT unit,
  ...
  if (FormationLoadBase().DistributedSize() > 0) {
    ...
    for (size_t i = 0; i < FormationLoadBase().DistributedSize(); i++) {
      ...
      aPointInConvexHull = aPointInConvexHull || formationload.PointSet().PointInConvexHull(pElement->MidPoint());
    }

    for (size_t i = 0; i < FormationLoadBase().DistributedSize(); i++) {
      const VALUE_TYPE &formationload = FormationLoadBase().DistributedValue(i);
      IValueDomainScalar::TValueVec vcValue = formationload.Component().ScalarData().ValueElement(elm, unit, cb);
      bool anElementInConvexHull = formationload.PointSet().PointInConvexHull(pElement->MidPoint());
      ...
      for (int n = 0; n < elm.NrOfPoints(); n++) {
        ...
        } else if (FormationLoadBase().DistributedOnly() && vcValue[n].Valid()) // validity necessary?
        {
          ...
          if (representativeLength < 0) // not CPointSet or no convex hull
          {
            ...
            //
            representativeLength = formationload.PointSet().Min().Distance(formationload.PointSet().Max()) / 2;
          }
  ...
  // test whether new routine is same as old; if satisfactory, replace above with this (but without creating the wrapper
  // all the time)
...
CFormationLoadComponent<VALUE_TYPE>::ValuePoint(const geo::IPoint &pt, const CQuantity::UNIT unit,
                                                geo::IParallelInitializationCallback *cb) const {
  if (FormationLoadBase().DistributedSize() > 0) {
    int n = 0;
    double dResult = 0;
...
template <class VALUE_TYPE>
CFormationLoadComponent<VALUE_TYPE> &
CFormationLoadComponent<VALUE_TYPE>::operator=(const CFormationLoadComponent &rhs) {
...
template <class VALUE_TYPE> bool CFormationLoadComponent<VALUE_TYPE>::Empty() const { return m_bEmpty; }

template <class VALUE_TYPE>
void CFormationLoadComponent<VALUE_TYPE>::LoadStream(TSTREAM & /*stream*/, CStreamVersion & /*version*/,
                                                     TPROGRESS & /*progress*/) {
  m_bEmpty = false;
}
...
template <class VALUE_TYPE> bool CFormationLoadComponent<VALUE_TYPE>::CanMap(const COpenGLNode &node) const {
