...
public:
  ...
  private:
    ...
    typedef std::pair<const geo::CTetrahedron*, TIndexOrientPr> TTetIndexPr; // int is an index refering to one of the faces of the tetrahedron (either 0, 1, 2)
...
private:
  ...
  void AddFaceToSurface(const std::string &sName, geo::CBodyGroup& group, int nFaceIndex, CShellSurfaceDesc::eTetOrientation orientation);
