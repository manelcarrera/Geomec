...
private:
  ...
  private:
    ...
    CResultDescription(CResultImportFormat &parent, int ResultType, bool bFault, int nDepStage,
  ...
  public:
    bool AddResultValue(int nLocalNodeIndex, const geo::CValue &value);
    bool IsStress() const;
...
class CWellPathInputImportFormat : public IImportFormat {
  typedef enum { UNIT_SI, UNIT_FIELD } TUnit;
...
public:
  ...
  virtual const CNewWellPathInput *NewWellPathInput() const { return m_pNewWellPathInput; }
};
