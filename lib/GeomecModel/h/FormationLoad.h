...
private:
  ...
  virtual CFormationLoad<VALUE_TYPE, CONSTANT_TYPE, REPEATER_TYPE, GWC_TYPE> &
  FormationStageLoad(CFormationBase &formation, CDepletionStage &next_stage) = 0;
...
template <class VALUE_TYPE, class CONSTANT_TYPE, class REPEATER_TYPE, class GWC_TYPE>
CFormationLoad<VALUE_TYPE, CONSTANT_TYPE, REPEATER_TYPE, GWC_TYPE>::CFormationLoad(CFormationBase &f,
                                                                                   CDepletionStage &d)
    : CFormationLoadBase<VALUE_TYPE>(d.Name().toStdString().c_str(), f.Model()), m_formation(f), m_depletion_stage(d),
      m_bAutoDelete(d.Initial() == true), m_eType(CFormationLoadBase<VALUE_TYPE>::REPEAT_MODE) {}
...
CFormationLoad<VALUE_TYPE, CONSTANT_TYPE, REPEATER_TYPE, GWC_TYPE>::CFormationLoad(const CFormationLoad &rhs)
    : CFormationLoadBase<VALUE_TYPE>(rhs), m_formation(rhs.m_formation), m_depletion_stage(rhs.m_depletion_stage),
      m_bAutoDelete(rhs.m_bAutoDelete), m_eType(rhs.m_eType) {
...
template <class VALUE_TYPE, class CONSTANT_TYPE, class REPEATER_TYPE, class GWC_TYPE>
unsigned int
CFormationLoad<VALUE_TYPE, CONSTANT_TYPE, REPEATER_TYPE, GWC_TYPE>::ComponentSize(unsigned int /*uMode*/) const {
  return 1;
}

template <class VALUE_TYPE, class CONSTANT_TYPE, class REPEATER_TYPE, class GWC_TYPE>
IValueComponentBase &
CFormationLoad<VALUE_TYPE, CONSTANT_TYPE, REPEATER_TYPE, GWC_TYPE>::Component(unsigned int /*uComponent*/,
                                                                              unsigned int /*uMode*/) {
...
template <class VALUE_TYPE, class CONSTANT_TYPE, class REPEATER_TYPE, class GWC_TYPE>
typename CFormationLoadBase<VALUE_TYPE>::eModeType
CFormationLoad<VALUE_TYPE, CONSTANT_TYPE, REPEATER_TYPE, GWC_TYPE>::Type() const {
  return m_eType;
}
...
template <class VALUE_TYPE, class CONSTANT_TYPE, class REPEATER_TYPE, class GWC_TYPE>
void CFormationLoad<VALUE_TYPE, CONSTANT_TYPE, REPEATER_TYPE, GWC_TYPE>::LoadStream(TSTREAM &stream,
...
template <class VALUE_TYPE, class CONSTANT_TYPE, class REPEATER_TYPE, class GWC_TYPE>
void CFormationLoad<VALUE_TYPE, CONSTANT_TYPE, REPEATER_TYPE, GWC_TYPE>::SaveStream(TSTREAM &stream,
                                                                                    TPROGRESS &progress) {
  // Save base class
...
template <class VALUE_TYPE, class CONSTANT_TYPE, class REPEATER_TYPE, class GWC_TYPE>
bool CFormationLoad<VALUE_TYPE, CONSTANT_TYPE, REPEATER_TYPE, GWC_TYPE>::IsSimilar(const CFormationLoad &rhs,
                                                                                   bool bCheckDistribution) const {
  if (Type() != rhs.Type())
...
CFormationLoad<VALUE_TYPE, CONSTANT_TYPE, REPEATER_TYPE, GWC_TYPE>::operator=(const CFormationLoad &rhs) {
  ...
  if (CFormationLoadBase<VALUE_TYPE>::parent() && !stage.Last()) {
    ...
    CFormationLoad<VALUE_TYPE, CONSTANT_TYPE, REPEATER_TYPE, GWC_TYPE> &next_load =
        FormationStageLoad(formation, next_stage);
    next_load.Modified();
  }
...
template <class VALUE_TYPE, class CONSTANT_TYPE, class REPEATER_TYPE, class GWC_TYPE>
void CFormationLoad<VALUE_TYPE, CONSTANT_TYPE, REPEATER_TYPE, GWC_TYPE>::OnNeighbourModified(const CGraphNode &node,
  ...
  if (pFormationLoad) {
    ...
    case CFormationLoadBase<VALUE_TYPE>::CONSTANT_MODE:
      ...
      Constant().Set(pFormationLoad->Constant().ReferenceValue().Value(),
                     pFormationLoad->Constant().ReferenceDepth().Value(), pFormationLoad->Constant().Gradient().Value(),
                     CQuantity::SI_UNIT);
      break;
    case CFormationLoadBase<VALUE_TYPE>::REPEAT_MODE:
    case CFormationLoadBase<VALUE_TYPE>::GWC_MODE:
...
bool CFormationLoad<VALUE_TYPE, CONSTANT_TYPE, REPEATER_TYPE, GWC_TYPE>::CanConnectItem(const CGraphNode &item) const {
  ...
  if (pFormationLoad)
    return !((pFormationLoad->Type() == CFormationLoadBase<VALUE_TYPE>::REPEAT_MODE) ||
             (pFormationLoad->Type() == CFormationLoadBase<VALUE_TYPE>::GWC_MODE));

  return false;
}

template <class VALUE_TYPE, class CONSTANT_TYPE, class REPEATER_TYPE, class GWC_TYPE>
bool CFormationLoad<VALUE_TYPE, CONSTANT_TYPE, REPEATER_TYPE, GWC_TYPE>::CanDisconnectItem(
    const CGraphNode &item) const {
  const CModelBase &model = static_cast<const CModelBase &>(CFormationLoadBase<VALUE_TYPE>::Model());
