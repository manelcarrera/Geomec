...
public:
  CStrainLoadGradientComponent(CStrainType &type,
                               CFormationLoad<TStrain, CStrainLoadGradientComponent, CStrainLoadRepeaterComponent,
                                              CStrainLoadGWCDummyComponent> &parent,
                               bool bEmpty = false);
...
class CStrainLoadRepeaterComponent
    : public CFormationLoadRepeaterComponent<TStrain, CStrainLoadGradientComponent, CStrainLoadRepeaterComponent,
                                             CStrainLoadGWCDummyComponent> {
...
private:
  ...
  public:
    ...
    virtual IValueDomainScalar::TValue ValuePoint(const geo::IPoint &pt, const UNIT unit = IQuantityDouble::SI_UNIT,
                                                  geo::IParallelInitializationCallback *cb = 0) const;
    virtual IValueDomainScalar::TValueVec ValueElement(const geo::IElement &elm,
                                                       const UNIT unit = IQuantityDouble::SI_UNIT,
                                                       geo::IParallelInitializationCallback *cb = 0) const;
...
class CStrainLoad : public CFormationLoad<TStrain, CStrainLoadGradientComponent, CStrainLoadRepeaterComponent,
                                          CStrainLoadGWCDummyComponent> {
private:
  typedef CFormationLoad<TStrain, CStrainLoadGradientComponent, CStrainLoadRepeaterComponent,
                         CStrainLoadGWCDummyComponent>
      TBase;
  CStrainType *m_Type;

  // Temporary solution; this should use m_vcDistributed, but that one is based on the derived VALUE_TYPE, whereas we
  // want to use a base class of that
...
public:
  ...
  CStrainLoad(CStrainType &t, CFormationBase &f, CDepletionStage &d, const double &dVolStrain,
  ...
  template <typename TYPE> const TYPE *StrainComponent() const;
  ...
  virtual size_t DistributedSize() const { return m_vcDistributedNodes.size(); }
