...
public:
  CGridPointsetSource(const geo::IPoint &ptCorner1, const geo::IPoint &ptCorner2, int nPointsNorthing,
                      int nPointsEasting);
...
public:
  ...
  void LoadStream(CStorageNode::TSTREAM &stream, CStorageNode::TSTREAMVERSION &version,
...
public:
  ...
  CPointSet *GeneratePointset(const QString &name, CModelBase &model, IProgressBase *pProgress = 0,
                              bool bStorePointsOutsideMesh = false);

  // ownership of the resulting pillarmap is transferred to the receiver
  CPillarMap *GeneratePillarMap(const QString &name, CModelBase &model, IProgressBase *pProgress = 0,
                                bool bInclude = false);
