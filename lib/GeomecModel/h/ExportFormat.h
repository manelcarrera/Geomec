...
public:
  ...
  */
  class CExportArg {
  public:
    CExportArg(UNIT unit = CQuantity::SI_UNIT, TAxisSystem axis_system = AXIS3D,
               CValueType::TMapEnum map_type = CValueType::MT_NONE, const CHotSpot *pHotSpot = 0,
               const CDepletionStage *pDeformation = 0, const CAnalysisType &antype = CAnalysisType::AT_NONLIN,
               const CResultRegister *pResultRegister = 0, TElementExportType elementCenterPoints = FULL_ELEMENT,
               TWellPathExportType wellpathPoints = INPUT_POINTS, bool bOldFormat = false, bool bFormationNames = false,
               bool bGVT = false)
        : m_unit(unit), m_axis_system(axis_system), m_map_type(map_type), m_pHotSpot(pHotSpot),
          m_pDeformation(pDeformation), m_antype(antype), m_pResultRegister(pResultRegister), m_bOldFormat(bOldFormat),
          m_elementCenterPoints(elementCenterPoints), m_wellpathPoints(wellpathPoints),
          m_bFormationNames(bFormationNames), m_bGVT(bGVT) {}
...
class CNodalExportFormat : public IExportFormat {
  ...
  typedef std::pair<CNewWellPoint, geo::CValue> TPointValue; // same definition as in wellpath, cannot include that
                                                             // header though (TODO: extract def to separate file)
...
public:
  CElementSetExportFormat(const IElementSet &element_set, const TValueTypeVec &vcElementSetData, const CExportArg &arg);

  CElementSetExportFormat(const IElementSet &element_set, const TDataVec &vcElementData,
                          const CExportArg &arg); // Export options

  typedef std::vector<const CFormationBase *> TFormationVec;
  CElementSetExportFormat(const TFormationVec &vcFormation, const TDataVec &vcNodalData, const TDataVec &vcElementData,
                          const CExportArg &arg); // Export options

  CElementSetExportFormat(const CFormationBase &formation, const TDataVec &vcNodalData, const TDataVec &vcElementData,
                          const CExportArg &arg); // Export options

  CElementSetExportFormat(const CFormationPlane &formation_plane, const TDataVec &vcElementData,
                          const CExportArg &arg); // Export options

  // wjrx mantis 2950
  typedef std::vector<const CFormationPlane *> TFormationPlaneVec;
  CElementSetExportFormat(const TFormationPlaneVec &formation_plane_vec, const TDataVec &vcElementData,
                          const CExportArg &arg); // Arguments for the export

  typedef std::vector<const CHorizonBase *> THorizonVec;
  CElementSetExportFormat(const THorizonVec &vcHorizon, const TDataVec &vcNodalData, const TDataVec &vcElementData,
                          const CExportArg &arg); // Export options

  typedef std::vector<const CWellCasingRTCI *> TRTCIVec;
  CElementSetExportFormat(const TRTCIVec &vcRTCI, const TDataVec &vcNodalData, const TDataVec &vcElementData,
                          const CExportArg &arg); // Export options

  CElementSetExportFormat(const CCrossSection &xsec, const TDataVec &vcNodalData, const TDataVec &vcElementData,
                          const CExportArg &arg); // Export options

  typedef std::vector<const CNonMeshedSurface *> TNonMeshedSurfaceVec;
  CElementSetExportFormat(const TNonMeshedSurfaceVec &vcSurface, const TDataVec &vcNodalData,
                          const TDataVec &vcElementData, const CExportArg &arg);
