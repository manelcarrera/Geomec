...
template <typename T> class CValueCache : public CValueCacheBase {
public:
...
template <typename T> typename CValueCache<T>::TValuePair CValueCache<T>::emptyValuePair;

template <typename T>
CValueCache<T>::CValueCache(CMeshBase *mesh)
    : CValueCacheBase(), m_NodeSize(0), m_ValueCache(m_NodeSize, emptyValuePair), m_MeshObserver(mesh, *this),
      m_Linked(false) {}

template <typename T>
CValueCache<T>::CValueCache(const CValueCache &rhs)
    : CValueCacheBase(rhs), m_NodeSize(rhs.m_NodeSize), m_ValueCache(rhs.m_ValueCache),
      m_MeshObserver(rhs.m_MeshObserver), m_Linked(rhs.m_Linked) {}

template <typename T> CValueCache<T> &CValueCache<T>::operator=(const CValueCache<T> &rhs) {
...
template <typename T> void CValueCache<T>::clearCache() {
  m_ValueCache.clear();
...
typename
#endif // (_MSC_VER == 1500)
    const typename CValueCache<T>::TValuePair &CValueCache<T>::find(const geo::INode *node) const {
...
template <typename T> void CValueCache<T>::insert(const geo::INode *node, const T &value) {
  size_t nodeIndex = node->Index();

  if (!m_Linked && (m_MeshObserver.getMesh() != 0) && m_MeshObserver.getMesh()->IsMesh()) {
...
typename
#endif // (_MSC_VER == 1500)
    const typename CValueCache<T>::TValuePair &CValueCache<T>::find(const geo::IElement &element) const {
...
template <typename T> void CValueCache<T>::insert(const geo::IElement &element, const T &value) {
  size_t nodeIndex = element.Index();
...
template <typename T> void CValueCache<T>::swap(CValueCache<T> aValueCache) {
