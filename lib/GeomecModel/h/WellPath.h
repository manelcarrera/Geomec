...
template <class T> class CResultMap;
...
public:
  // ISection interface
  CFormationSection(well::CWellPathBase &WellPath, const CFormationBase *Formation, well::CWellPoint *Top,
...
public:
  ...
  public:
    ...
    virtual void AddSection(well::IWellSection &section, SECTION_TYPE type, QRgb color, int iLineWidth, bool bVisible,
                            const QString &strDescription) = 0;
    virtual void AddPoint(well::CWellPoint &point, QRgb color, int iPointSize, bool bVisible) = 0;
...
public:
  void GetGeomecStrainValues(well::CWellPointList &list, well::CWellPointValueMap &map, const CDepletionStage &stage,
...
template <class T> CResultMap<T>::CResultMap(CWellPath &wellpath) { m_pWellPath = &wellpath; }

template <class T> void CResultMap<T>::RemoveResult(const well::CWellPoint &wellpoint) {
  // point found
...
template <class T>
bool CResultMap<T>::ResultExist(const well::CWellPoint &Wellpoint, const CDepletionStage &stage,
                                const CAnalysisType &antype) const {
  // point found
  typename TResultMap::iterator it = m_Result.find(CWellPointResultKey(Wellpoint, stage, antype));
  return !(it == m_Result.end());
}

template <class T>
T &CResultMap<T>::GetResult(const well::CWellPoint &Wellpoint, const CDepletionStage &stage,
                            const CAnalysisType &antype) const {
...
template <class T> void CResultMap<T>::ClearAllResults() { m_Result.clear(); }
