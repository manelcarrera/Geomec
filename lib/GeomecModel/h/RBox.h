...
struct TBox {
private:
  static bool overlapping_1D(const TLine &l1, const TLine &l2) { return (l1.M >= l2.m && l2.M >= l1.m); }
...
public:
  ...
  bool contains(TBox b2) {
    return (m[0] <= b2.m[0] && M[0] >= b2.M[0]) && (m[1] <= b2.m[1] && M[1] >= b2.M[1]) &&
           (m[2] <= b2.m[2] && M[2] >= b2.M[2]);
  }

  static bool overlapping_3D(TBox b1, TBox b2) { return b1.overlaps(b2); }
};
...
class RefinementBox {
public:
  enum eProperty { Position, Size_ };

  enum GradientType_ : unsigned int { PlanarZ, CenterPoint };
...
public:
  ...
  RefinementBox(const T3da &center, const T3da &size, const T3da &rotation, GradientType_ direction,
                const TGradientV &profile, const QString &name, const bool &visible);
