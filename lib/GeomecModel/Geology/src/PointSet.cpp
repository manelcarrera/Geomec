...
// CCoordinateType
//////////////////////////////////////////////////////////////////////
CPointSet::CCoordinateType::CCoordinateType(CPointSet &point_set, IPointSet::DIMENSION dim, bool bCreateValueSet)
...
CPointSet::CCoordinateType::~CCoordinateType() { Invalidate(); }
...
bool CPointSet::CCoordinateType::CanDestroy() const { return m_bDestroy; }

void CPointSet::CCoordinateType::DestroyEnable(bool bCanDestroy) { m_bDestroy = bCanDestroy; }
...
unsigned int CPointSet::CCoordinateType::IconId() const { return IDI_VALUETYPE_COORDINATE; }

unsigned int CPointSet::CCoordinateType::TypeId() const { return IDT_VALUETYPE_COORDINATE; }

QString CPointSet::CCoordinateType::TypeName() const { return getStringTableEntry(IDS_VALUENAME_COORDINATE); }
...
//////////////////////////////////////////////////////////////////////
CPointSet::CCoordinateType::CCoordinateComponent::CCoordinateComponent(const QString &sName,
                                                                       CCoordinateType &value_type, int uComponentIndex)
    : CValueComponent(sName, value_type, uComponentIndex, 0) {}

CPointSet::CCoordinateType::CCoordinateComponent::CCoordinateComponent(const QString &sName,
                                                                       CCoordinateType &value_type,
                                                                       CNodalValueSet &value_set, int uComponentIndex)
...
double CPointSet::CCoordinateType::CCoordinateComponent::FieldFactor() const { return FF_FACTOR_LENGTH; }
...
CPointSet::CPointSet(CFemAppModel &model)
    : IPointSet(model), m_pValueType(0), m_pHull_3D(0), m_pHull_2D(0), m_pHull_Cylindric(0), m_display_hull(0),
      m_display_cylindrichull(0), m_pointSetType(POINTSET), m_ResizeHull(0.0), m_bTriedCreatingHull(false) {}

CPointSet::CPointSet(const QString &strName, CFemAppModel &model, const std::vector<CPointSet::TLineVec> vcData,
                     const CPointSet::DIMENSION dim, bool bCreateValueSetForCoordinates)
    : IPointSet(strName, model, CQuantity::SI_UNIT, CQuantity::SI_UNIT), m_pValueType(0), m_pHull_3D(0), m_pHull_2D(0),
      m_pHull_Cylindric(0), m_display_hull(0), m_display_cylindrichull(0), m_pointSetType(POINTSET), m_ResizeHull(0.0),
...
CPointSet::CPointSet(const QString &sName, CFemAppModel &model, int nValueSetSize, const CPointSet::DIMENSION dim,
                     bool bCreateValueSetForCoordinates, bool bLinkToEntry)
    : IPointSet(sName, model, CQuantity::SI_UNIT, CQuantity::SI_UNIT), m_pValueType(0), m_pHull_3D(0), m_pHull_2D(0),
      m_pHull_Cylindric(0), m_display_hull(0), m_display_cylindrichull(0), m_pointSetType(POINTSET), m_ResizeHull(0.0),
      m_bTriedCreatingHull(false) {
...
unsigned int CPointSet::IconId() const { return IDI_POINTSET; }

// ##ModelId=3C74E3EB014E
unsigned int CPointSet::TypeId() const { return 0; }

// ##ModelId=3C74E3EB00FC
CPointSet::CPointSet(const CPointSet &rhs)
    : IPointSet(rhs), m_pValueType(rhs.m_pValueType), m_pHull_3D(0), m_pHull_2D(0), m_pHull_Cylindric(0),
      m_display_hull(0), m_display_cylindrichull(0), m_pointSetType(rhs.m_pointSetType), m_ResizeHull(rhs.m_ResizeHull),
...
CPointSet::DIMENSION CPointSet::Dimension() const { return m_pValueType->Dimension(); }
...
geo::CConvexHull *CPointSet::Create3DHull(bool &bCanceled) const {
  ...
  try {
    ...
    if (!bCreated)
      throw((const char *)("Convex Hull failed its consistency checks"));
...
int CPointSet::DisplayListSize() const { return static_cast<const CModelBase &>(Model()).ShowConvexHulls() ? 2 : 1; }
  ...
  if (nIndex == 1) {
    ...
    if (m_pHull_Cylindric) {
      if (m_display_cylindrichull.Size() == 0) {
        ...
        for (geo::CPtrArray<geo::IPoint>::const_iterator it = m_pValueType->NearestNeighbour().begin();
             it != m_pValueType->NearestNeighbour().end(); ++it) {
          const geo::IPoint *curPoint = (*it);

          if (prevPoint) {
            geo::CVector v(*curPoint - *prevPoint);
            m_display_cylindrichull.PushBack(
                *new geo::CConus(*prevPoint, v.UnitVector(), m_pHull_Cylindric->Radius(), v.Length(), 1));
          }
      ...
      if (m_display_hull.Size() == 0) {
        // Using lines, as they get picked up by OIV right away. For 3D it may not be ideal in the viewing, but it
        // should give a decent idea of whether the convex hull is correct, which is the goal
        if (m_pHull_2D) {
          ...
          for (int i = 0; i < m_pHull_3D->NrOfFaces(); ++i) {
            for (int j = 0; j < m_pHull_3D->Face(i).NrOfLines(); ++j) {
              ...
              for (int k = 0; k < 2; ++k)
                const_cast<geo::IPoint &>(line->Point(k))
                    .Set(line->Point(k).X() + centerPoint.X(), line->Point(k).Y() + centerPoint.Y(),
                         line->Point(k).Z() + centerPoint.Z());
...
void CPointSet::ResizeHullValue(double d) {
  ...
  m_ResizeHull = d;
  // we don't have to call DestroyHull() as the resize distance isn't (and never was) used during creation; but we do
  // need to notify the cylindric hull, if we have it actually, we need to notify any of them
  if (m_pHull_3D)
...
int CPointSet::PushBack(const std::vector<double> &vcRow, bool bConvertValue) {
  ...
  for (int i = 0; i < NodalValueSetSize(); i++) {
    // wjrx mantis 3340
    //
    const IUnitConversion *pConversion = UnitConversion(ValueSet(i).Name().toStdString());

    if (pConversion)
      NodalValueSet(i).PushBack(geo::CValue(pConversion->ConvertToSi(vcRow[i])));
...
int CPointSet::PushBack(const std::vector<geo::CValue> &vcRow, bool bConvertValue) {
  ...
  for (int i = 0; i < NodalValueSetSize(); i++) {
    ...
    if (pConversion && vcRow[i].Valid())
      NodalValueSet(i).PushBack(geo::CValue(pConversion->ConvertToSi(vcRow[i].Value())));
    else {
...
bool CPointSet::Empty() const { return m_pValueType == 0; }

CPointSet::CCoordinateType &CPointSet::Coordinates() { return *m_pValueType; }

const CPointSet::CCoordinateType &CPointSet::Coordinates() const { return *m_pValueType; }
...
void CPointSet::LoadPointSetData(TSTREAM &stream, CStreamVersion &version, TPROGRESS &progress) {
  ...
  stream >> nDimension;
  m_pValueType =
      new CCoordinateType(*this,                               // PointSet
...
void CPointSet::GetValueTypesNoAveraging(CPointSet &target, const CPointSet &ptset,
                                         std::vector<CValueType *> vcTypes) const {
  CValueType *pVT;
...
// sampledPointSet.
//
bool CPointSet::DoSample(CPointSet &sampledPointSet, double blockSize_north, double blockSize_east,
                         double blockSize_depth, IProgressBase &progress) {
  ...
  if (blockSize_north > 0)
    blocksXcount = size.X() / blockSize_north + 1;
  if (blockSize_east > 0)
    blocksYcount = size.Y() / blockSize_east + 1;
  if (blockSize_depth > 0)
    blocksZcount = size.Z() / blockSize_depth + 1;
  ...
  QString message = QString("Initialise conversion of %1 points to %2 blocks")
                        .arg(PointSize())
                        .arg(blocksXcount * blocksYcount * blocksZcount);
  ...
  TVec3DValues values_per_block(
      blocksXcount, TVec2DValues(blocksYcount, TVec1DValues(blocksZcount, TVecValues(nodalValueSetSize,
                                                                                     std::vector<geo::CValue>()))));

  progress.Step();
  message =
      QString("Sampling  %1 points to %2 blocks").arg(PointSize()).arg(blocksXcount * blocksYcount * blocksZcount);
  progress.StatusMessage(message);
  ...
  for (int jj = 0; jj < ValueSetSize(); ++jj) {
    ...
    if (p_valueTypeOrig && dynamic_cast<CCoordinateType *>(p_valueTypeOrig) == 0) {
      p_valueTypeSampled = CValueTypeFactory::instance()->BuildValueType(sampledPointSet, p_valueTypeOrig->TypeId(),
                                                                         p_valueTypeOrig->Name());
    }
        ...
        for (size_t z = 0; z < values_per_block[x][y].size(); ++z) {
          ...
          for (size_t v = 1; v < values_per_block[x][y][z].size(); ++v)
            if (values_per_block[x][y][z][v].size() != x_coord_size) {
              message =
                  QString("Found illegal number of values in block[%1][%2][%3], aborting...").arg(x).arg(y).arg(z);
              progress.StatusMessage(message);
              return false;
            }
    ...
    for (size_t y = 0; y < values_per_block[x].size(); ++y) {
      for (size_t z = 0; z < values_per_block[x][y].size(); ++z) {
        ...
        if (x_coord_size > 0) {
          ...
          size_t candidate = x_coord_size >> 1; // we're going to select a point in the middle of the gathered values
                                                // (heuristically ballpark ok as mean)
...
// wjrx mantis 3186
void CPointSet::Sample(const QString &name, const double blockSizeNorth, const double blockSizeEast,
                       const double blockSizeDepth) {
  CPointSet *ps = new CPointSet(name, Model(), 0, Dimension());
  ...
  if (DoSample(*ps, blockSizeNorth, blockSizeEast, blockSizeDepth, *progress) == false)
    delete ps;
}

void CPointSet::Export(const IExportFormat::CExportArg &arg, const QString &fileName) {
