...
unsigned int CFaultPressure::CComponentForwarder::TypeId() const { return ActiveComponent().TypeId(); }

unsigned int CFaultPressure::CComponentForwarder::IconId() const { return ActiveComponent().IconId(); }

QString CFaultPressure::CComponentForwarder::TypeName() const { return ActiveComponent().TypeName(); }

geo::CValue CFaultPressure::CComponentForwarder::ValuePoint(const geo::IPoint &pt, const CQuantity::UNIT unit,
...
CFaultPressure::CComponentForwarder::ValueElement(const geo::IElement &elm, const CQuantity::UNIT unit,
                                                  geo::IParallelInitializationCallback *cb) const {
...
CFaultPressure::CFaultPressure(CHorizonBase &fault, CDepletionStage &dstage, TModeType mode)
    : IValueComposite("Fault Pressure", fault.Model()), m_fault(fault), m_stage(dstage), m_mode(mode),
...
CFaultPressure::CFaultPressure(const CFaultPressure &rhs)
    : IValueComposite(rhs), m_fault(rhs.m_fault), m_stage(rhs.m_stage), m_mode(rhs.m_mode),
      m_vcDistributed(rhs.m_vcDistributed), m_bDistributedOnly(rhs.m_bDistributedOnly) {
...
CFaultPressure::~CFaultPressure() { assert(!IsCopied()); }
...
bool CFaultPressure::operator==(const CFaultPressure &rhs) const {
  return (IValueComposite::operator==(rhs) && &m_fault == &rhs.m_fault && &m_stage == &rhs.m_stage &&
          m_mode == rhs.m_mode && m_bDistributedOnly == rhs.m_bDistributedOnly &&
          m_vcDistributed == rhs.m_vcDistributed && Gradient() == rhs.Gradient() &&
          MinComponent() == rhs.MinComponent() && MaxComponent() == rhs.MaxComponent() &&
          MaxGradientComponent() == rhs.MaxGradientComponent());
}

CFaultPressure::TModeType CFaultPressure::Mode() const { return m_mode; }
...
size_t CFaultPressure::DistributedSize() const { return m_vcDistributed.size(); }
...
bool CFaultPressure::DistributedOnly() const { return m_bDistributedOnly; }
...
bool CFaultPressure::CanConnectItem(const CGraphNode &item) const {
  ...
  return (!model.BranchState().IsFixedStage(m_stage) && pDistriPressure != 0 &&
          pDistriPressure->Component().Defined() && !IsLinkedTo(*pDistriPressure));
}
...
unsigned int CFaultPressure::ModeSize() const { return 1; }

QString CFaultPressure::ModeName(unsigned int /*uMode*/) const { return QString(); }

unsigned int CFaultPressure::ComponentSize(unsigned int /*uMode*/) const { return 1; }
...
unsigned int CFaultPressure::TypeId() const { return IDT_FAULT_PRESSURE; }

QString CFaultPressure::TypeName() const { return Component().TypeName(); }

unsigned int CFaultPressure::IconId() const { return Component().IconId(); }

const QString &CFaultPressure::Name() const { return m_stage.Name(); }
...
const CHorizonBase &CFaultPressure::Fault() const { return m_fault; }

CHorizonBase &CFaultPressure::Fault() { return m_fault; }

const CDepletionStage &CFaultPressure::DepletionStage() const { return m_stage; }

CDepletionStage &CFaultPressure::DepletionStage() { return m_stage; }
...
const IValueDataInterfaceScalar &CFaultPressureComponent::ScalarData() const { return m_Data; }

IValueDataInterfaceScalar &CFaultPressureComponent::ScalarData() { return m_Data; }

const CFaultPressure &CFaultPressureComponent::FaultPressure() const { return (const CFaultPressure &)(Parent()); }

CFaultPressure &CFaultPressureComponent::FaultPressure() { return (CFaultPressure &)(Parent()); }
...
QString CFaultPressureComponent::ExportLabel() const { return getStringTableEntry(IDS_ET_PRESSURE); }

IValueDomainScalar::TMinMax CFaultPressureComponent::MinMax(IProgressBase &progressBase,
...
IValueDomainScalar::TValue CFaultPressureComponent::Average(IProgressBase &progressBase,
                                                            const CQuantity::UNIT unit) const {
  geo::CBox box(FaultPressure().Fault().Min(), FaultPressure().Fault().Max());
  return ScalarData().Average(progressBase, box, unit);
}

bool CFaultPressureComponent::Empty() const { return false; }

long CFaultPressureComponent::SavedItems() const { return 0; }

bool CFaultPressureComponent::Defined() const { return true; }

geo::CValue CFaultPressureComponent::ValuePoint(const geo::IPoint &pt, const CQuantity::UNIT unit,
                                                geo::IParallelInitializationCallback *cb) const {
...
IValueDomainScalar::TValueVec CFaultPressureComponent::ValueElement(const geo::IElement &elm,
  ...
  if (FaultPressure().DistributedSize() > 0) {
    for (size_t i = 0; i < FaultPressure().DistributedSize(); i++) {
      ...
      for (int n = 0; n < elm.NrOfPoints(); ++n) {
        if ((FaultPressure().DistributedOnly() || pressure.PointSet().PointInConvexHull(elm.Point(n))) &&
            vcValue[n].Valid()) {
          if (vcNrOfValue[n] == 0)
            vcRet[n] = vcValue[n];
          else
...
const CFormationBase &CFaultPressureComponent::Formation(const geo::IElement &elm) const {
  ...
  if (pTiedBodyTriangle) {
    assert(false); // it's probably wrong to find the formation for the tied body triangle directly, but since all body
                   // triangles *should* lie in the same formation, we can return it
...
IValueDomainScalar::TValueVec CFaultPressureComponent::InterfaceValues3D(const geo::CInterfaceElement &iface, bool bMax,
                                                                         const CQuantity::UNIT unit) const {
  ...
  IValueDomainScalar::TValueVec vcFrontValues =
      Formation(front).Pressure(FaultPressure().DepletionStage()).Component().ScalarData().ValueElement(front, unit);
  ...
  if (dynamic_cast<const geo::CTiedBodyTriangle *>(iface.BackFace())) {
    ...
    for (int i = 0; i < 3; ++i) {
      IValueDomainScalar::TValueVec vcTmpValues = Formation(*pTiedBodyTriangle->BodyTriangle(i))
                                                      .Pressure(FaultPressure().DepletionStage())
                                                      .Component()
                                                      .ScalarData()
                                                      .ValueElement(*pTiedBodyTriangle->BodyTriangle(i), unit);
    ...
    vcBackValues =
        Formation(back).Pressure(FaultPressure().DepletionStage()).Component().ScalarData().ValueElement(back, unit);
  }
...
std::pair<const geo::IElement *, const geo::IElement *>
CFaultPressureComponent::GetFrontAndBack(const geo::IInterfaceElement &iface) const {
...
IValueDomainScalar::TValueVec CFaultPressureComponent::InterfaceValues(const geo::IInterfaceElement &iface, bool bMax,
                                                                       const CQuantity::UNIT unit) const {
  assert(dynamic_cast<const C3DModel *>(&Model()));
  const geo::CInterfaceElement *p3DInterface = dynamic_cast<const geo::CInterfaceElement *>(&iface);
  assert(p3DInterface);
...
CFaultPressureGradientComponent::CFaultPressureGradientComponent(CFaultPressure &faultpressure)
    : CFaultPressureComponent(faultpressure, CFaultPressure::MT_GRADIENT), m_depth(0), m_refpressure(0), m_gradient(0) {
}

CFaultPressureGradientComponent::CFaultPressureGradientComponent(const CFaultPressureGradientComponent &rhs)
    : CFaultPressureComponent(rhs), m_depth(rhs.m_depth), m_refpressure(rhs.m_refpressure), m_gradient(rhs.m_gradient) {
}
...
bool CFaultPressureGradientComponent::operator==(const CFaultPressureGradientComponent &rhs) const {
  return (m_depth == rhs.m_depth && m_refpressure == rhs.m_refpressure && m_gradient == rhs.m_gradient);
}

const CLengthQuantity &CFaultPressureGradientComponent::ReferenceDepth() const { return m_depth; }

const CSinglePressure &CFaultPressureGradientComponent::ReferencePressure() const { return m_refpressure; }

const CPressureGradientQuantity &CFaultPressureGradientComponent::Gradient() const { return m_gradient; }
...
unsigned int CFaultPressureGradientComponent::TypeId() const { return IDT_COMPONENT_PRESSURE_CONSTANT; }

unsigned int CFaultPressureGradientComponent::IconId() const { return IDI_COMPONENT_PRESSURE_CONSTANT; }

QString CFaultPressureGradientComponent::TypeName() const {
  return getStringTableEntry(IDS_COMPONENT_PRESSURE_CONSTANT);
}

long CFaultPressureGradientComponent::SavedItems() const { return 1; }
...
geo::CValue CFaultPressureGradientComponent::ValuePoint(const geo::IPoint &pt, const CQuantity::UNIT unit,
                                                        geo::IParallelInitializationCallback *cb) const {
  const CModelBase &model = static_cast<const CModelBase &>(Model());
  ...
  return geo::CValue(
      (ReferenceDepth().Convert(model.Depth(pt), unit, CQuantity::SI_UNIT) - ReferenceDepth().Value(unit)) *
          Gradient().Value(unit) +
      ReferencePressure().Value(unit));
}

IValueDomainScalar::TValueVec
CFaultPressureGradientComponent::ValueElement(const geo::IElement &elm, const CQuantity::UNIT unit,
                                              geo::IParallelInitializationCallback *cb) const {
  const CModelBase &model = static_cast<const CModelBase &>(Model());
  ...
  for (int i = 0; i < vcRet.size(); i++) {
    if (!vcRet[i].Valid())
      vcRet[i] = geo::CValue((ReferenceDepth().Convert(model.Depth(elm.Point(i)), unit, CQuantity::SI_UNIT) -
                              ReferenceDepth().Value(unit)) *
                                 Gradient().Value(unit) +
                             ReferencePressure().Value(unit));
  }
...
unsigned int CFaultPressureMinComponent::TypeId() const { return IDT_COMPONENT_PRESSURE_MINIMUM; }

unsigned int CFaultPressureMinComponent::IconId() const { return IDI_COMPONENT_PRESSURE_MINIMUM; }

QString CFaultPressureMinComponent::TypeName() const { return getStringTableEntry(IDS_COMPONENT_PRESSURE_MINIMUM); }

geo::CValue CFaultPressureMinComponent::ValuePoint(const geo::IPoint & /*pt*/, const CQuantity::UNIT /*unit*/,
                                                   geo::IParallelInitializationCallback * /*cb*/) const {
...
IValueDomainScalar::TValueVec CFaultPressureMinComponent::ValueElement(const geo::IElement &elm,
                                                                       const CQuantity::UNIT unit,
                                                                       geo::IParallelInitializationCallback *cb) const {
  IValueDomainScalar::TValueVec vcRet = CFaultPressureComponent::ValueElement(elm, unit, cb);
...
unsigned int CFaultPressureMaxComponent::TypeId() const { return IDT_COMPONENT_PRESSURE_MAXIMUM; }

unsigned int CFaultPressureMaxComponent::IconId() const { return IDI_COMPONENT_PRESSURE_MAXIMUM; }

QString CFaultPressureMaxComponent::TypeName() const { return getStringTableEntry(IDS_COMPONENT_PRESSURE_MAXIMUM); }

geo::CValue CFaultPressureMaxComponent::ValuePoint(const geo::IPoint & /*pt*/, const CQuantity::UNIT /*unit*/,
                                                   geo::IParallelInitializationCallback * /*cb*/) const {
  // this component needs an element, not possible
  assert(false);
  return geo::CValue();
...
IValueDomainScalar::TValueVec CFaultPressureMaxComponent::ValueElement(const geo::IElement &elm,
                                                                       const CQuantity::UNIT unit,
                                                                       geo::IParallelInitializationCallback *cb) const {
  IValueDomainScalar::TValueVec vcRet = CFaultPressureComponent::ValueElement(elm, unit, cb);
...
unsigned int CFaultPressureMaxGradientComponent::TypeId() const { return IDT_COMPONENT_PRESSURE_MAXGRADIENT; }

unsigned int CFaultPressureMaxGradientComponent::IconId() const { return IDI_COMPONENT_PRESSURE_MAXGRADIENT; }

QString CFaultPressureMaxGradientComponent::TypeName() const {
  return getStringTableEntry(IDS_COMPONENT_PRESSURE_MAXGRADIENT);
}

geo::CValue CFaultPressureMaxGradientComponent::ValuePoint(const geo::IPoint & /*pt*/, const CQuantity::UNIT /*unit*/,
                                                           geo::IParallelInitializationCallback * /*cb*/) const {
  // this component needs an element, not possible
  assert(false);
  return geo::CValue();
...
IValueDomainScalar::TValueVec
CFaultPressureMaxGradientComponent::ValueElement(const geo::IElement &elm, const CQuantity::UNIT unit,
                                                 geo::IParallelInitializationCallback *cb) const {
...
void CFaultPressureMaxGradientComponent::DetermineValues() const {
  ...
  for (int i = 0; i < pInterfaces->ElementSize(); ++i) {
    ...
    const CFFMaterial &front_mat =
        Formation(*prAdjacent.first).Material(FaultPressure().DepletionStage()).Material(*prAdjacent.first);
    ...
    if (dynamic_cast<const geo::CTiedBodyTriangle *>(prAdjacent.second)) {
      ...
      for (int j = 0; j < 3; ++j) {
        const CFFMaterial &back_mat = Formation(*pTiedBodyTriangle->BodyTriangle(j))
                                          .Material(FaultPressure().DepletionStage())
                                          .Material(*pTiedBodyTriangle->BodyTriangle(j));
      ...
      const CFFMaterial &back_mat =
          Formation(*prAdjacent.second).Material(FaultPressure().DepletionStage()).Material(*prAdjacent.second);
  ...
  for (int i = 0; i < pInterfaces->ElementSize(); ++i) {
    ...
    // get pressure values
    IValueDomainScalar::TValueVec vcFrontValues = Formation(*prAdjacent.first)
                                                      .Pressure(FaultPressure().DepletionStage())
                                                      .Component()
                                                      .ScalarData()
                                                      .ValueElement(*prAdjacent.first);
    IValueDomainScalar::TValueVec vcBackValues(vcFrontValues.size(), 0);

    if (dynamic_cast<const geo::CTiedBodyTriangle *>(prAdjacent.second)) {
      const geo::CTiedBodyTriangle *pTiedBodyTriangle = static_cast<const geo::CTiedBodyTriangle *>(prAdjacent.second);
      for (int j = 0; j < 3; ++j) {
        IValueDomainScalar::TValueVec vcTmpValues = Formation(*pTiedBodyTriangle->BodyTriangle(j))
                                                        .Pressure(FaultPressure().DepletionStage())
                                                        .Component()
                                                        .ScalarData()
                                                        .ValueElement(*pTiedBodyTriangle->BodyTriangle(j));
        assert(vcTmpValues.size() == vcBackValues.size());
      ...
      vcBackValues = Formation(*prAdjacent.second)
                         .Pressure(FaultPressure().DepletionStage())
                         .Component()
                         .ScalarData()
                         .ValueElement(*prAdjacent.second);
    }
