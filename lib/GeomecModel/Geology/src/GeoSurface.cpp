...
: IElementSet(surface), 
  m_pSurface(surface.m_pSurface), 
  m_pValueType(surface.m_pValueType),
  m_unit(surface.m_unit)
{
  AssertValid();
}
...
: IElementSet(value_type.PointSet().Name(), 
  value_type.Model(), 
  CQuantity::SI_UNIT, CQuantity::SI_UNIT), 
  m_pSurface(0), 
  m_pValueType(&value_type), 
...
: IElementSet(strName, model, CQuantity::SI_UNIT, CQuantity::SI_UNIT), m_pValueType(0), m_unit(CDoubleQuantity::SI_UNIT)
{
...
CSurfaceBase::CSurfaceBase(const QString &strName, 
...
CSurfaceBase::CSurfaceBase(const QString &strName, 
               const std::vector<const geo::IFace*> &vcFace,
               CFemAppModel& model)
: IElementSet(strName, model, CQuantity::SI_UNIT, CQuantity::SI_UNIT), m_pValueType(0), m_unit(CDoubleQuantity::SI_UNIT)
{
  CTSSurfaceProgress SurfaceProgress;
...
void CSurfaceBase::DeleteAllMeshregions()
{
  MeshRegions().ClearAndDelete();
}
...
bool CSurfaceBase::Build() const
{
  ...
  if(pComponent->Defined() && (m_pValueType->PointSet().PointSize() > 0))
  {
    ...
    {
      geo::IPoint* pPoint = new geo::CPoint(pPointSet->PointAt(i).X(),
                          pPointSet->PointAt(i).Y(),
...
unsigned int CSurfaceBase::TypeId() const
{ 
  return IDT_TREE_SURFACES;
}

QString CSurfaceBase::TypeName() const
{
  return getStringTableEntry(IDS_TREE_SURFACES);
}
...
int CSurfaceBase::DisplayListSize() const
{
   return 2;
}
...
public:
  ...
  CTriFace(const TPointMap& mpPoint, int p1, int p2, int p3)
  : m_mpPoint(mpPoint)
  {
    m_node[0] = p1;
    m_node[1] = p2;
  ...
  { assert(false); return geo::CValue(); }
};
...
*/
bool CSurfaceBase::Increase(double dXNOffset, double dYNOffset, double dXMOffset, double dYMOffset, double dOffset /* = 0 */)
{
  // Do we need to build the surface
  if(m_pSurface == 0)
    VERIFY(Build());	// Build should succeed
...
bool CSurfaceBase::Increase(geo::CBox box, double dOffset /* = 0 */)
{
  ...
  if ((dOffset == 0) ||
    (minpoint.X() - box.Min().X() > dHalfEps) ||
    (minpoint.Y() - box.Min().Y() > dHalfEps) || 
    (maxpoint.X() - box.Max().X() < -dHalfEps) || 
    (maxpoint.Y() - box.Max().Y() < -dHalfEps))
  {
...
CSurfaceBase* CSurfaceBase::OnCreateSurface(const QString& /*sName*/, const geo::CPtrArray<geo::IPoint>& /*vcPoint*/)
{
  assert(false);	// Should not be called...
  return 0;
}

CSurfaceBase* CSurfaceBase::OnCreateSurface(const QString& /*sName*/,const std::vector<const geo::IFace*>& /*vcFaces*/)
{
  assert(false);
  return 0;
}
...
CSurfaceBase* CSurfaceBase::MergeSurface(const CSurfaceBase& surface, bool bReTriangulate)
{
  ...
  if(!bReTriangulate)
  { // create the new surface using the old connectivity
...
void CSurfaceBase::ExportSurface(const QString& sExt, const CUnitNode& unitNode,
  const QString& fileName)
{
    ...
    else if(sExt.compare(".xls", Qt::CaseInsensitive) == 0 || fileName.right(5).left(4).compare(".xls", Qt::CaseInsensitive) == 0)			
    {
      IExportFormat::CExportArg arg( unitNode.Unit() );
      CElementSetExportFormat format(*this, ValueTypes(), arg);
...
void CSurfaceBase::Unit(CDoubleQuantity::UNIT unit)
{
  ...
  for(i = 0; i < surf.PointSize(); i++)
  {
    pt = &(surf.Point(i));
                          // out    in
    surf.Point(i, geo::CPoint(lq.Convert(pt->X(), m_unit, unit),
                  lq.Convert(pt->Y(), m_unit, unit),
...
bool CSurfaceEntry::CanSelectAndDeleteSurfaces() const
{
  return NonUsedSurface();
}
...
void CSurfaceEntry::DoCutSurfaces(CModelBase& model, int cutSurfacesCount,
  ...
  const std::map <SIDE_ID, geo::CTriSurface*>& sideSurfaces,
  const QString& baseName, const std::vector <SIDE_ID>& sides,
  const geo::CBox* /*pbox*/)
{
  int iCount=1;
  ...
  while(its != sides.end())
  {
    ...
    if (side_name == "Top")
    {
      pTetra->SubBoundary().TopHorizon().ConnectItem(
      *new CTetraSubHorizon(*pSurfaceBase, model, false) );

    }
    else if (side_name == "Bottom")
    {
      pTetra->SubBoundary().BottomHorizon().ConnectItem(
      *new CTetraSubHorizon(*pSurfaceBase, model, false) );
    }
    else
...
void CSurfaceEntry::ExportSurfaces(CNamedSurfaces& slected_surfaces, const QString& fileName, const CUnitNode& unitNode)
{
    // Create file object
    geo::CMesh mesh;
    CGoCadFile file(mesh);
