...
: CPointSet(rootModel(model))
, m_bIsVertical(false)
, m_dApproximateRadius(1)
, m_dGlobalNorthing(0)
, m_dGlobalEasting(0)
, m_dGlobalTVD(0)
{
...
: CPointSet(sName, rootModel(model), 0, DIM_3D)
, m_bIsVertical(false)
, m_dApproximateRadius(1)
, m_dGlobalNorthing(0)
, m_dGlobalEasting(0)
...
, m_bIsVertical(true)
, m_dApproximateRadius(1)
, m_dGlobalNorthing(BasePoint.X())
, m_dGlobalEasting(BasePoint.Y())
, m_dGlobalTVD(0)
{
  Init();
...
CNewWellPathInput::CNewWellPathInput(const QString &file, const QString &name, CFemAppModel &model, CQuantity::UNIT unit, IProgressBase &dlg)
: CPointSet(name, rootModel(model), 2, DIM_3D)
, m_bIsVertical(false)
, m_dApproximateRadius(1)
, m_dGlobalNorthing(0)
, m_dGlobalEasting(0)
...
, m_bIsVertical(wellpath.IsVertical())
, m_dApproximateRadius(1)
, m_dGlobalNorthing(wellpath.GlobalNorthing())
, m_dGlobalEasting(wellpath.GlobalEasting())
, m_dGlobalTVD(wellpath.GlobalTVD())
{
  Init();
...
{
  const TPointSetEntry* pointSetEntry = dynamic_cast<const TPointSetEntry*>(input->Model().GraphEntry(MD_BASE_POINTSET));
...
{
  const TPointSetEntry* pointSetEntry = dynamic_cast<const TPointSetEntry*>(model.RootModel().GraphEntry(MD_BASE_POINTSET));
  const TPointSetEntry::TNodeSet nodeSet = pointSetEntry->EntryNodes();
...
const CNewWellPath *CNewWellPathInput::CreateWellPath(CNewWellPathInput *input, const CModelBase& model, std::vector<std::string> *messages)
{
...
void CNewWellPathInput::GetInterSectionPointsWithFormationSkin(const geo::ILine& line , std::set<geo::CPoint>& PointSet) const
{
...
void CNewWellPathInput::InitVertical(double start_northing, double start_easting, double start_depth, double start_tmd, double end_depth)
{
...
void CNewWellPathInput::InitFromPointArray(QVector<geo::CPoint>& vcPoint, bool setGlobals, QVector<double> *azimuth, QVector<double> *inclination)
{
  assert(PointSize() == 0);
...
void CNewWellPathInput::Recalculate(const geo::CPoint &point)
{
...
bool CNewWellPathInput::CanConnectItem(const CGraphNode& /*item*/) const
{
  return false;
}

bool CNewWellPathInput::ConnectItem(const CGraphNode& /*item*/)
{
  return false;
}
