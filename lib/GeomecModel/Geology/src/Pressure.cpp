...
CComponentRepeater::CComponentRepeater(CPressure &pressure)
    : CFormationLoadRepeaterComponent<TPressure, CComponentConstant<TPressure>, CComponentRepeater, CComponentGWC>(
          pressure) {}
...
QString CComponentRepeater::ExportLabel() const { return getStringTableEntry(IDS_ET_PRESSURE); }
...
CComponentGWC::CGWCPressure::CGWCPressure(const CGWCPressure &rhs) : CSinglePressure(rhs), m_current(rhs.m_current) {}
...
bool CComponentGWC::CGWCPressure::operator==(const CGWCPressure &rhs) const { return CSinglePressure::operator==(rhs); }
...
CComponentGWC::CGWCDepth::CGWCDepth(const CGWCDepth &rhs) : CLengthQuantity(rhs), m_current(rhs.m_current) {}
...
bool CComponentGWC::CGWCDepth::operator==(const CGWCDepth &rhs) const { return CLengthQuantity::operator==(rhs); }

CComponentGWC::CComponentGWC(CPressure &pressure, bool bEmpty)
    : CCommonPressureComponent<TPressure>(pressure, CPressure::GWC_MODE, bEmpty), m_contact_pressure(pressure, 0),
      m_trans_pressure(pressure, 0), m_contact_depth(pressure, 0), m_trans_depth(pressure, 0),
...
CComponentGWC::CComponentGWC(const CComponentGWC &rhs)
    : CCommonPressureComponent<TPressure>(rhs), m_contact_pressure(rhs.m_contact_pressure),
...
const CComponentGWC &CComponentGWC::FirstGWC() const {
  ...
  if (!pFirstGWC)
    pFirstGWC = &pres.Formation()->Pressure(pStageThis->InitialStage()).GWC();
...
const CComponentGWC &CComponentGWC::PreviousGWC() const {
  ...
  if (!pPrevGWC)
    pPrevGWC = &pres.Formation()->Pressure(pStageThis->InitialStage()).GWC();
...
QString CComponentGWC::TypeName() const { return getStringTableEntry(IDS_COMPONENT_PRESSURE_GWC); }

const CSinglePressure &CComponentGWC::ContactPressure() const { return m_contact_pressure; }
...
const CLengthQuantity &CComponentGWC::ContactDepth() const { return m_contact_depth; }
...
bool CComponentGWC::TransitionFromPrevious() const { return m_bTransitionFromPrevious; }
...
void CComponentGWC::Set(const double &dContactPressure, const double &dTransitionPressure, const double &dContactDepth,
...
IValueDomainScalar::TValueVec CComponentGWC::ValueElement(const geo::IElement &elm, const CQuantity::UNIT unit,
...
geo::CValue CComponentGWC::ValuePoint(const geo::IPoint &pt, const CQuantity::UNIT unit,
                                      geo::IParallelInitializationCallback *cb) const {
...
CPressure::CPressure(CFormationBase &f, CDepletionStage &d, bool bEmpty) // Undefined initialisation
    : TPressureBase(f, d), m_GlobalPressureObserver(this, &(static_cast<CModelBase &>(f.Model()).GlobalPressure())) {
  init(f, d, bEmpty);
}

CPressure::CPressure(CFormationBase &f, CDepletionStage &d, const double &dReferencePressure,
                     const double &dReferenceDepth, const double &dGradient,
...
CPressure::CPressure(CFormationBase &f, CDepletionStage &d, const double &dContactPressure,
                     const double &dTransitionPressure, const double &dContactDepth, const double &dTransitionDepth,
                     const double &dUpperGradient, const double &dLowerGradient, bool bTransitionFromPrevious,
...
CPressure::CPressure(CFormationBase &f, CDepletionStage &d, TPressure & /*p*/)
    : TPressureBase(f, d), m_GlobalPressureObserver(this, &(static_cast<CModelBase &>(f.Model()).GlobalPressure())) {
  init(f, d);
}

CPressure::CPressure(const CPressure &rhs)
    : TPressureBase(rhs), m_GlobalPressureObserver(rhs.m_GlobalPressureObserver) {
...
unsigned int CPressure::TypeId() const { return IDT_VALUETYPE_PRESSURE; }

QString CPressure::TypeName() const { return getStringTableEntry(IDS_VALUENAME_PRESSURE); }
...
bool CPressure::IsFirstValidDepletionStage() const { return (DepletionStage().Initial() != false); }
...
const CPressure *CPressure::ParentPressure() const {
  ...
  if (pFormation && pFormation->ParentFormation())
    pParentPressure = &pFormation->ParentFormation()->Pressure(
