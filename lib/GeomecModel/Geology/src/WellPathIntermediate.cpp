...
CCachingIntermediate::CCachingIntermediate() : m_bHasColor(false), m_bHasDrawInfo(false), m_pIntermediate(0) {}
...
void CCachingIntermediate::AddSection(well::IWellSection &section, SECTION_TYPE type, QRgb color, int iLineWidth,
                                      bool bVisible, const QString &strDescription) {
  if (m_pIntermediate)
  ...
  VERIFY(connect(&section, SIGNAL(OnDestroy(const well::IWellSection &)), this,
                 SLOT(OnSectionDestroyed(const well::IWellSection &))));
}

void CCachingIntermediate::AddPoint(well::CWellPoint &point, QRgb color, int iPointSize, bool bVisible) {
  if (m_pIntermediate)
  ...
  VERIFY(connect(&point, SIGNAL(OnDestroy(const well::CWellPoint &)), this,
                 SLOT(OnPointDestroyed(const well::CWellPoint &))));
}
