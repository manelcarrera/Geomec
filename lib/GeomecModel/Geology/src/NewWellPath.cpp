...
CNewWellPath::CNewWellPath(CFemAppModel &model)
    : CColorNode(model), m_input(0), m_DsbResults(this), m_bShowDsbPointsInScene(false), m_dGlobalNorthing(0),
      m_dGlobalEasting(0), m_dGlobalTVD(0), m_bDrawWellInfo(true), m_pZoomInModel(0), m_dMudGradRefDepth(0) {
  Init();
}

CNewWellPath::CNewWellPath(const CNewWellPathInput &input, CFemAppModel &model)
    : CColorNode(input.Name(), model), m_input(0), m_DsbResults(this), m_bShowDsbPointsInScene(false),
      m_dGlobalNorthing(0), m_dGlobalEasting(0), m_dGlobalTVD(0), m_bDrawWellInfo(true), m_pZoomInModel(0),
...
const CNewWellPathInput *CNewWellPath::WellPathInput() const { return m_input; }
...
void CNewWellPath::InitFromWellPathInput(const CNewWellPathInput &input) {
  if (input.IsVertical()) {
    ...
    if (input.PointSize() >= 2)
      InitVertical(GlobalNorthing(), GlobalEasting(), input.PointAt(0).Z(), 0,
                   input.PointAt(input.PointSize() - 1).Z());
  } else {
...
double CNewWellPath::CalcInclination(const geo::IPoint &pt1, const geo::IPoint &pt2) {
  geo::CVector v(pt1, pt2);
  return RAD2DEG(acos(v.Z() / v.Length()));
}

double CNewWellPath::CalcAzimuth(const geo::IPoint &pt1, const geo::IPoint &pt2) {
  geo::CVector v(pt1, pt2);
...
CAngleQuantity CNewWellPath::Azimuth(size_t index) const { return m_vcAzimuthInclination.Azimuth(index); }

CAngleQuantity CNewWellPath::Inclination(size_t index) const { return m_vcAzimuthInclination.Inclination(index); }
...
void CNewWellPath::ClearAngles() { m_vcAzimuthInclination.Clear(); }
...
// wjrx mantis 3549
//
void CNewWellPath::GetInterSectionPointsWithFormationSkin(const geo::ILine &line,
...
// wjrx mantis 3549
//
void CNewWellPath::InitVertical(double start_northing, double start_easting, double start_depth, double start_tmd,
                                double end_depth) {
...
unsigned int CNewWellPath::IconId() const { return IDI_WELLPATH; }

unsigned int CNewWellPath::TypeId() const { return IDT_TREE_WELL_PATHS; }

QString CNewWellPath::TypeName() const { return getStringTableEntry(IDS_TREE_WELL_PATHS); }

void CNewWellPath::ToggleDrawWellInfo() { DrawWellInfo(!DrawWellInfo()); }
...
bool CNewWellPath::DrawWellInfo() const { return m_bDrawWellInfo; }
...
int CNewWellPath::DisplayListSize() const {
  ...
  if (m_bDrawWellInfo) // wjrx mantis 3322
    retval += m_FormationLines.size() + m_FormationLabels.size() + m_FormationArrows.size();
...
unsigned int CNewWellPath::WellPointSize() const { return m_WellPathPolyLine.PointSize(); }
...
CNewWellPoint CNewWellPath::WellPoint(unsigned int index) const {
  ...
  return CNewWellPoint(*this, m_WellPathPolyLine.Point(index), m_WellPathTMD[index]);
}

double CNewWellPath::TMD(unsigned int index, CDoubleQuantity::UNIT unit) const {
  assert(index < WellPointSize());
...
CNewWellPath::TColor CNewWellPath::Color() const {
  // during loading (conversion) we may get asked for a color while we have no m_input yet, in which case we return our
  // own color
...
void CNewWellPath::Sample(QVector<geo::CPoint> &vcPoint) {
  ...
  if (vcSampled.size() < 2 || vcSampled[vcSampled.size() - 1].SquareDistance(vcPoint[lastPt]) >= 1) {
...
void CNewWellPath::SaveStream(TSTREAM &stream, TPROGRESS &progress) {
  ...
  for (std::list<CNewWellPoint>::const_iterator it = m_FistExportPoints.begin(); it != m_FistExportPoints.end(); ++it) {
    stream << (*it).TMD();
  }
  ...
  for (std::list<CNewWellPoint>::const_iterator it = DStaborPoints().begin(); it != DStaborPoints().end(); ++it) {
...
//
void CNewWellPath::LoadStreamOldToNew(TSTREAM &stream, CStreamVersion &version, TPROGRESS &progress) {
  ...
  if (bVertical)
    m_dGlobalTVD = global_depth; // wjrx mantis 2555
  ...
  for (int i = 0; i < nDStaborSize; i++) {
    ...
    while (bLoadRes) {
      ...
      stream >> bLinear;
      CDsbStatusValue &rs =
          DsbResults().GetResult(point, *pDepl, (bLinear ? CAnalysisType::AT_LINEAR : CAnalysisType::AT_NONLIN));
      stream >> status;
      stream >> value;
...
// lose this data
//
void CNewWellPath::LoadMeshIntersectionNew_Skip(TSTREAM &stream, CStreamVersion & /*version*/
                                                ,
                                                TPROGRESS & /*progress*/) {
...
// lose this data
//
void CNewWellPath::LoadWellLogData_Skip(TSTREAM &stream, CStreamVersion &version, TPROGRESS &progress) {
...
// lose this data
//
void CNewWellPath::LoadWellLog_Skip(/*well::CWellLog *log*/
                                    TSTREAM &stream, TPROGRESS &progress) {
...
// lose this data
//
void CNewWellPath::LoadDCasint_Skip(TSTREAM &stream, CStreamVersion &version, TPROGRESS &progress) {
  long nSize;
  stream >> nSize;
...
void CNewWellPath::UpdateFormationSections() {
  ...
  while (pFormationTopPoint && pFormationBottomPoint) {
    ...
    if (!mid.Formation()) { // this only happens when there is a void in the model
    ...
    CNewFormationSection section(*this, mid.Formation(), pFormationTopPoint->TMD(), pFormationBottomPoint->TMD());
    ...
    //
    const double offset = (m_WellPathTMD[m_WellPathTMD.size() - 1] - m_WellPathTMD[0]) / 20.0;
    ...
    */
      ...
      if (m_WellPathTMD[ii] > pFormationBottomPoint->TMD()) {
        if (!bArrowBaseFound) {
          ...
          if (geo::CLine(pt, topArrowPoint).SquareLength() > EPS * EPS) {
            ...
            m_FormationArrows.push_back(new CVectorSymbol(topArrowPoint, dir, 20, false));
          }
        }
    ...
    for (int ii = arrowLine.PointSize() - 1; ii >= 0 && pArrowBase == 0; --ii) {
      if (arrowLine.Point(ii) != bottomArrowPoint) {
        ...
        if (geo::CLine(*pArrowBase, bottomArrowPoint).SquareLength() > EPS * EPS) {
          ...
          m_FormationArrows.push_back(new CVectorSymbol(bottomArrowPoint, dir2, 20, false));
        }
      }
    }
    ...
    if (arrowLine.PointSize() >= 2) {
      ...
      m_FormationLabels.push_back(CLabelPoint(textPos, QString(mid.Formation()->Name())));
    }
...
const CNewFormationSection *CNewWellPath::FormationSection(const CNewWellPoint &pt) const {
  const std::list<CNewFormationSection> &list = FormationSections();
  CNewFormationSectionList newFormationSectionList(list);
  std::list<const CNewFormationSection *> formation_list = newFormationSectionList.getSections(pt);
...
void CNewWellPath::CalculateIntersections(const geo::IMesh &mesh) {
  ...
  while (++lineIndex < m_WellPathPolyLine.PointSize()) {
    ...
    for (std::set<int>::iterator it_eltIndex = stElementIndices.begin(); it_eltIndex != stElementIndices.end();
         it_eltIndex++) {
      const geo::IBody *pInspectedBodyElement = dynamic_cast<const geo::IBody *>(&mesh.Element(*it_eltIndex));

      if (!pInspectedBodyElement)
        continue;

      const geo::IBody *pIntersectedBodyElement = 0;
      const geo::IFace *pIntersectedFace = 0;

      if (geo::BBox::Intersects(*pWellPoint, *pNext, pInspectedBodyElement->Max(), pInspectedBodyElement->Min(),
                                true)) {
        for (int inspectedBodyElementFaceIndex = 0; inspectedBodyElementFaceIndex < pInspectedBodyElement->NrOfFaces();
             inspectedBodyElementFaceIndex++) {
          const geo::IFace &inspectedFace = pInspectedBodyElement->Face(inspectedBodyElementFaceIndex);
          if (geo::BBox::Intersects(*pWellPoint, *pNext, inspectedFace.Max(), inspectedFace.Min(), true)) {
            CNewWellPoint intersectionPoint(*this, 0, 0, 0, 0);
            if (CalculateInterSectionWithFace(inspectedFace, pt1, pt2, intersectionPoint)) {
            ...
            if (pIntersectedFace && pIntersectedFace->NrOfNodes() > 0) {
              ...
              //
              const geo::INode &IntersectedFaceNode0 = pIntersectedFace->Node(0);
              ...
              if (IntersectedFaceNode0.AttachedElementSize() != 1)
                for (int attachedElementIndex = 0; attachedElementIndex < IntersectedFaceNode0.AttachedElementSize();
                     ++attachedElementIndex) {
                  const geo::IElement &AttachedElement = IntersectedFaceNode0.AttachedElement(attachedElementIndex);

                  // One of the elements attached to the face-node is the
                  // element that owns the face. We want to know about the
                  // OTHER elements.
                  //
                  if (&AttachedElement == pIntersectedBodyElement)
                    continue;

                  const geo::CInterfaceElement *pAttachedInterfaceElement =
                      dynamic_cast<const geo::CInterfaceElement *>(&AttachedElement);
                  const geo::IBody *pAttachedBody = dynamic_cast<const geo::IBody *>(&AttachedElement);
                  ...
                  if (pAttachedInterfaceElement &&
                      (pAttachedInterfaceElement->FrontFace() == 0 || pAttachedInterfaceElement->BackFace() == 0)) {
                    ...
                    for (int attachedFaceNodeIndex = 0; attachedFaceNodeIndex < pFace->NrOfNodes();
                         ++attachedFaceNodeIndex) {
                      for (int intersectedFaceNodeIndex = 0; intersectedFaceNodeIndex < pIntersectedFace->NrOfNodes();
                           ++intersectedFaceNodeIndex) {
                        if (&pFace->Node(attachedFaceNodeIndex) == &pIntersectedFace->Node(intersectedFaceNodeIndex)) {
                  ...
                  } else if (pAttachedBody) // IBody
                  {
                    ...
                    //
                    for (int attachedBodyFaceIndex = 0; attachedBodyFaceIndex < pAttachedBody->NrOfFaces();
                         ++attachedBodyFaceIndex) {
                      unsigned int identicalNodeCounter = 0;
                      const geo::IFace &AttachedFace = pAttachedBody->Face(attachedBodyFaceIndex);

                      for (int attachedFaceNodeIndex = 0; attachedFaceNodeIndex < AttachedFace.NrOfNodes();
                           ++attachedFaceNodeIndex) {
  ...
  if (firstPoint.Formation() &&
      (m_MeshIntersectionPoints.empty() || m_WellPathTMD.front() < m_MeshIntersectionPoints.front().TMD())) {
    m_MeshIntersectionPoints.push_front(firstPoint);
  }

  CNewGeoWellPoint lastPoint(*this, m_WellPathTMD.back());
  if (lastPoint.Formation() &&
      (m_MeshIntersectionPoints.empty() || m_WellPathTMD.back() > m_MeshIntersectionPoints.back().TMD())) {
...
const std::list<CNewWellPoint> &CNewWellPath::DStaborPoints() const { return m_DStaborPoints; }
...
void CNewWellPath::Recalculate(const geo::CPoint &point) { Recalculate(point.X(), point.Y(), point.Z()); }

void CNewWellPath::Recalculate(const double &Global_Northing, const double &Global_Easting, const double &Global_TVD) {
...
// wjrx mantis 3822
//
geo::CPoint CNewWellPath::QuadLineIntersect(const geo::IQuadrilateral &quad, const geo::ILine &line) {
  geo::CPoint ret;
  /*
...
bool CNewWellPath::CalculateInterSectionWithFace(const geo::IFace &face, const CNewWellPoint &pt1,
  ...
  const geo::IQuadrilateral *pQuad = dynamic_cast<const geo::IQuadrilateral *>(&face);
  if (pQuad != 0)
...
bool CNewWellPath::hasZoomInModel() const { return m_pZoomInModel != 0; }
...
void CNewWellPath::CreateZoomInModel() {
  if (!m_pZoomInModel) {
    if (1) // FormIntersecPoints().size() >= 2 )
    {
      ...
      m_pZoomInModel = static_cast<CWellZoomInModel *>(IModelLifetimeFacade::NewChild(
          WELL_ZOOMIN_MODEL, modelBase, *this, Model().Logger(), modelBase.getVersionManager()));
...
public:
  ZLess_FormationBase(const geo::IPoint &pt) { m_pt = pt; }
...
void CNewWellPath::CalcGraphPoints() {
  ...
  for (std::set<const geo::IElement *>::const_iterator it_elt = (*intersect.begin()).getElements().begin();
       it_elt != (*intersect.begin()).getElements().end() && bCurrentlyOutsideMesh == false; ++it_elt) {
    if (*it_elt == 0)
  ...
  while (it_pt != intersect.end()) {
    ...
    // formation.
    //
    typedef std::map<const CFormationBase *, const geo::IElement *, ZLess_FormationBase> TFormationMap;
    ZLess_FormationBase zLess_FormationBase(*it_pt);
    TFormationMap map_formation_element(zLess_FormationBase);

    for (std::set<const geo::IElement *>::const_iterator it_elt = (*it_pt).getElements().begin();
         it_elt != (*it_pt).getElements().end(); ++it_elt) {
      // wjrx mantis 3756
    ...
    for (it_map = map_formation_element.begin(); it_map != map_formation_element.end(); ++it_map) {
      CNewWellPoint p = *it_pt; // copy mesh intersection point
      std::set<const geo::IElement *> emptySet;
...
int CNewWellPath::MapValuesSize() const {
  ...
  for (std::list<CNewWellPoint>::const_iterator itPoint = ptList.begin(); itPoint != ptList.end(); ++itPoint) {
    ...
    if (p.getElements().empty()) {
      ...
      for (std::set<const geo::IElement *>::iterator itElt = p.getElements().begin(); itElt != p.getElements().end();
           ++itElt) {
        ...
        TFormationElementsMap::iterator retIt =
            mpFormationElements.insert(std::make_pair(pFormation, TElements())).first;
...
void CNewWellPath::MapValues(const IValueComponentBase &component, CUnitNode::TUnitType unit,
                             TPointValueVector &vcPointValues) {
  const CModelBase &model = static_cast<const CModelBase &>(Model());
  ...
  bool bDoPointCalculation =
      dynamic_cast<const IStressTensorGroup::CWellPathStressComposite::CWellPathStressComponent *>(&component) ||
      dynamic_cast<const IStrainTensorGroup::CWellPathAxialStrainComposite::CWellPathAxialStrainComponent *>(
          &component);
  ...
  for (std::list<CNewWellPoint>::const_iterator itPoint = ptList.begin(); itPoint != ptList.end(); ++itPoint) {
    ...
    if (p.getElements().empty()) {
      TPointValueVector vc;
      vc.push_back(std::make_pair(p, value));
      ...
      for (TFormationElementsMap::iterator itForm = mpFormationElements.begin(); itForm != mpFormationElements.end();
           ++itForm) {
        ...
        if (!added) {
          TPointValueVector vc;
          vc.push_back(std::make_pair(newP, value));
...
std::list<const QDate *> &CNewWellPath::FistDates() { return m_FistDates; }

const std::list<const QDate *> &CNewWellPath::FistDates() const { return m_FistDates; }
...
std::list<CNewWellPoint> &CNewWellPath::FistExportPoints() { return m_FistExportPoints; }

const std::list<CNewWellPoint> &CNewWellPath::FistExportPoints() const {
  return const_cast<CNewWellPath *>(this)->FistExportPoints();
}

void CNewWellPath::MudGradRefDepth(double newval) { m_dMudGradRefDepth = newval; }

double CNewWellPath::MudGradRefDepth() const { return m_dMudGradRefDepth; }
...
bool CNewWellPathEntry::CanExport() const { return !EntryNodes().empty(); }

// wjrx mantis 3322
void CNewWellPathEntry::ToggleDrawWellInfo() { DrawWellInfo(!DrawWellInfo()); }
...
// wjrx mantis 3322
bool CNewWellPathEntry::DrawWellInfo() const { return m_bDrawWellInfo; }
...
bool CNewWellPathEntry::ImportFile(const QString &strFileName, IProgressBase &dlg, CModelBase &model,
  ...
  } else if (sFile.right(4).compare(".XLS", Qt::CaseInsensitive) == 0 ||
...
void CNewWellPathEntry::addPointSet2DataStorage(const CGraphNode::TNodeVec &nodeVec, CModelBase &model) {
  ...
  for (CGraphNode::TNodeVec::const_iterator node = nodeVec.begin(); node != nodeVec.end(); ++node) {
    if (!*node)
      continue;
    ...
    // Create pointset
    CPointSet *ps = new CPointSet(pWellPath->Name(), model, 0, CPointSet::DIM_3D);
...
bool CNewWellPathEntry::ImportFile(const QString &strFileName, IProgressBase &dlg, CModelBase &model) {
  ...
  try {
    ...
    delete p;
    for (CGraphNode::TNodeVec::iterator node = nodeVec.begin(); node != nodeVec.end(); ++node) {
      delete *node;
    }
    return false;
  }
...
bool CNewWellPathEntry::ImportPetrelFile(const QString &strFileName, IProgressBase &dlg, CModelBase &model,
...
const QString CNewWellPathEntry::DEFAULT_NEWWELL_NAME = QObject::tr("New Wellpath (ps) %1");
...
void CNewWellPath::AddDefaultFistPoints(std::list<CNewWellPoint> &FistPoints) const {
  const CModelBase *pModel = dynamic_cast<const CModelBase *>(&Model());
  if (!pModel->IsMesh()) {
    return;
  }
...
void CNewWellPath::AddDefaultDstaborPoints(std::list<CNewWellPoint> &DstaborPoints) const {
  const CModelBase *pModel = dynamic_cast<const CModelBase *>(&Model());
...
bool CNewWellPath::CanImportLAS() const { return true; }
...
bool CNewWellPath::ShowDsbAnalysisPointsInScene() const { return m_bShowDsbPointsInScene; }

CNewDstaborResultMap &CNewWellPath::DsbResults() { return m_DsbResults; }

const CNewDstaborResultMap &CNewWellPath::DsbResults() const { return m_DsbResults; }
...
const geo::CPolyLine &CNewWellPath::PolyLine() const { return m_WellPathPolyLine; }

void CNewWellPath::GetIntersectionPointsWithPlane(geo::CPtrArray<CNewGeoWellPoint> & /*points*/,
                                                  const geo::CPlane & /*plane*/) const {
  // TODO htg//
}
...
// ---------------------------------------------------------------------------
//
CNewGeoWellPoint::CNewGeoWellPoint(const CNewWellPath &wellpath, const double &tmd)
    : CNewWellPoint(wellpath, tmd), m_bInit(false), m_model(dynamic_cast<const CModelBase *>(&wellpath.Model())) {}

CNewGeoWellPoint::CNewGeoWellPoint(const CNewGeoWellPoint &rhs)
    : CNewWellPoint(rhs), m_bInit(rhs.m_bInit), m_model(rhs.m_model) {}
...
CNewGeoWellPoint::CNewGeoWellPoint(CModelBase &model, const CNewWellPoint &wellpoint)
    : CNewWellPoint(wellpoint), m_bInit(false), m_model(&model) {}
...
// ----------------------------------------------------------------------------
//
CNewFormationSection::CNewFormationSection(CNewWellPathBase &wellPath, const CFormationBase *pFormation,
                                           const double &topTMD, const double &bottomTMD)
    : INewWellSection(wellPath), m_pFormation(pFormation), m_Top(wellPath, topTMD), m_Bottom(wellPath, bottomTMD) {}

//
// ----------------------------------------------------------------------------
//
size_t CAngleInfoVector::Size() const { return m_AziInc.size(); }
...
void CAngleInfoVector::PushBack(const double &azi, const double &inc) { m_AziInc.push_back(std::make_pair(azi, inc)); }
void CAngleInfoVector::Clear() { m_AziInc.clear(); }

//******************************  CResultMap **********************************
CNewWellPointResultKey::CNewWellPointResultKey(const CNewWellPoint &point, const CDepletionStage &stage,
