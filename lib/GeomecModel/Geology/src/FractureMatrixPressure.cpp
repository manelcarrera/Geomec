...
CFractureMatrixComponentRepeater::CFractureMatrixComponentRepeater(const CPressure *pressure,
    ...
    : TFractureMatrixComponentRepeaterBase(fractureMatrixPressure), m_Pressure(pressure),
...
CFractureMatrixComponentRepeater::CFractureMatrixComponentRepeater(const CFractureMatrixComponentRepeater &rhs)
    : TFractureMatrixComponentRepeaterBase(rhs), m_Pressure(rhs.m_Pressure),
...
const CFractureMatrixComponentRepeater::TFormationLoad &
CFractureMatrixComponentRepeater::PreviousFormationLoad() const {
  const CFractureMatrixPressure *pPressure = dynamic_cast<const CFractureMatrixPressure *>(&FormationLoadBase());
  assert(pPressure);
...
geo::CValue CFractureMatrixComponentRepeater::ValuePoint(const geo::IPoint &pt, const CQuantity::UNIT unit,
...
CFractureMatrixComponentRepeater::ValueElement(const geo::IElement &elm, const CQuantity::UNIT unit,
                                               geo::IParallelInitializationCallback *cb) const {
  if ((m_FractureMatrixPressure.Type() == CFractureMatrixPressure::REPEAT_MODE)) {
    // skip baseclass version, we only want distributed values
    IValueDomainScalar::TValueVec vcValuesDist =
        CFormationLoadComponent<TFractureMatrixPressure>::ValueElement(elm, unit, cb);
...
CFractureMatrixComponentGWC::CGWCPressure::CGWCPressure(CFractureMatrixPressure &current, const double &value,
                                                        const UNIT unit)
...
CFractureMatrixComponentGWC::CGWCPressure &
CFractureMatrixComponentGWC::CGWCPressure::operator=(const CSinglePressure &rhs) {
  CSinglePressure::operator=(rhs);
  return *this;
}

CFractureMatrixComponentGWC::CGWCPressure &
CFractureMatrixComponentGWC::CGWCPressure::operator=(const CGWCPressure &rhs) {
  if (this != &rhs) {
    CSinglePressure::operator=(rhs);
...
CFractureMatrixComponentGWC::CGWCDepth::CGWCDepth(CFractureMatrixPressure &current, const double &value,
                                                  const UNIT unit)
...
CFractureMatrixComponentGWC::CFractureMatrixComponentGWC(CFractureMatrixPressure &pressure, bool bEmpty)
    : CCommonPressureComponent<TFractureMatrixPressure>(pressure, CFractureMatrixPressure::GWC_MODE, bEmpty),
      m_contact_pressure(pressure, 0), m_trans_pressure(pressure, 0), m_contact_depth(pressure, 0),
      m_trans_depth(pressure, 0), m_bTransitionFromPrevious(true) {
...
CFractureMatrixComponentGWC::CFractureMatrixComponentGWC(const CFractureMatrixComponentGWC &rhs)
    : CCommonPressureComponent<TFractureMatrixPressure>(rhs), m_contact_pressure(rhs.m_contact_pressure),
      m_trans_pressure(rhs.m_trans_pressure), m_contact_depth(rhs.m_contact_depth), m_trans_depth(rhs.m_trans_depth),
...
const CFractureMatrixComponentGWC &CFractureMatrixComponentGWC::FirstGWC() const {
  ...
  do {
    ...
    if (pStage->Last() || pStage == pStageThis)
      pStage = 0;
    else
      pStage = &pStage->Next();
  } while (pStage);
  ...
  if (!pFirstGWC)
    pFirstGWC = &pres.Formation()->FractureMatrixPressure(pStageThis->InitialStage()).GWC();
...
const CFractureMatrixComponentGWC &CFractureMatrixComponentGWC::PreviousGWC() const {
  ...
  if (!pPrevGWC)
    pPrevGWC = &pres.Formation()->FractureMatrixPressure(pStageThis->InitialStage()).GWC();
...
QString CFractureMatrixComponentGWC::TypeName() const { return getStringTableEntry(IDS_COMPONENT_PRESSURE_GWC); }

const CSinglePressure &CFractureMatrixComponentGWC::ContactPressure() const { return m_contact_pressure; }
...
bool CFractureMatrixComponentGWC::IsLastGWC() const {
  ...
  while (!pStage->Last()) {
    pStage = &pStage->Next();
    if (pres.Formation()->FractureMatrixPressure(*pStage).Type() == CFractureMatrixPressure::GWC_MODE)
      return false; // a later one was found
  }
...
const CSinglePressure &CFractureMatrixComponentGWC::TransitionPressure() const {
  ...
  if (TransitionFromPrevious())
    return PreviousGWC().TransitionPressure();

  return m_trans_pressure;
}

const CLengthQuantity &CFractureMatrixComponentGWC::ContactDepth() const { return m_contact_depth; }
...
const CLengthQuantity &CFractureMatrixComponentGWC::TransitionDepth() const {
  ...
  if (TransitionFromPrevious())
    return PreviousGWC().TransitionDepth();
...
bool CFractureMatrixComponentGWC::TransitionFromPrevious() const { return m_bTransitionFromPrevious; }
...
bool CFractureMatrixComponentGWC::CheckContactDepth(const double &dContactDepth) const {
  if (!IsFirstGWC()) {
    ...
    const CFractureMatrixComponentGWC &prev = PreviousGWC();
    if (dContactDepth > prev.ContactDepth().Value())
      return false;
  }
  ...
  if (!IsLastGWC()) {
    ...
    if (dContactDepth < next.ContactDepth().Value())
      return false;
  }
...
void CFractureMatrixComponentGWC::Set(const double &dContactPressure, const double &dTransitionPressure,
...
IValueDomainScalar::TValueVec
CFractureMatrixComponentGWC::ValueElement(const geo::IElement &elm, const CQuantity::UNIT unit,
                                          geo::IParallelInitializationCallback *cb) const {
...
double CFractureMatrixComponentGWC::GWCValue(const geo::IPoint &pt, const CQuantity::UNIT unit) const {
  ...
  if (model.Depth(pt) > TransitionDepth().Value())
    return TransitionPressure().Value(unit) +
           (Z.Value(unit) - TransitionDepth().Value(unit)) * LowerGradient().Value(unit);

  if (fabs(TransitionDepth().Value(unit) - ContactDepth().Value(unit)) < EPS)
    return ContactPressure().Value(unit);

  // inside the transition zone
  double grad = (TransitionPressure().Value(unit) - ContactPressure().Value(unit)) /
                (TransitionDepth().Value(unit) - ContactDepth().Value(unit));
  return (Z.Value(unit) - ContactDepth().Value(unit)) * grad + ContactPressure().Value(unit);
}

geo::CValue CFractureMatrixComponentGWC::ValuePoint(const geo::IPoint &pt, const CQuantity::UNIT unit,
                                                    geo::IParallelInitializationCallback *cb) const {
...
CFractureMatrixPressure::CFractureMatrixPressure(const CPressure *pressure, CFormationBase &f, CDepletionStage &d,
                                                 bool bEmpty) // Undefined initialisation
    : TFractureMatrixPressureBase(f, d),
      m_GlobalPressureObserver(this, &(static_cast<CModelBase &>(f.Model()).GlobalPressure())), m_Pressure(pressure) {
  init(f, d, bEmpty);
}

CFractureMatrixPressure::CFractureMatrixPressure(const CPressure *pressure, CFormationBase &f, CDepletionStage &d,
                                                 const double &dReferencePressure, const double &dReferenceDepth,
  ...
  Constant().Set(dReferencePressure, dReferenceDepth, dGradient, unit);

  assert(Type() == CONSTANT_MODE);
}

CFractureMatrixPressure::CFractureMatrixPressure(const CPressure *pressure, CFormationBase &f, CDepletionStage &d,
                                                 const double &dContactPressure, const double &dTransitionPressure,
                                                 const double &dContactDepth, const double &dTransitionDepth,
                                                 const double &dUpperGradient, const double &dLowerGradient,
                                                 bool bTransitionFromPrevious, const CQuantity::UNIT unit)
    : TFractureMatrixPressureBase(f, d),
  ...
  GWC().Set(dContactPressure, dTransitionPressure, dContactDepth, dTransitionDepth, dUpperGradient, dLowerGradient,
            bTransitionFromPrevious, unit);
...
CFractureMatrixPressure::CFractureMatrixPressure(const CFractureMatrixPressure &rhs)
    : TFractureMatrixPressureBase(rhs), m_GlobalPressureObserver(rhs.m_GlobalPressureObserver),
...
unsigned int CFractureMatrixPressure::TypeId() const { return IDT_VALUETYPE_FRACTURE_MATRIX_PRESSURE; }
...
bool CFractureMatrixPressure::IsFirstValidDepletionStage() const { return (DepletionStage().Initial() != false); }

void CFractureMatrixPressure::LoadStream(TSTREAM &stream, CStreamVersion &version, TPROGRESS &progress) {
  ...
  if (version < CStreamVersion(4, 1, 24)) {
    if (!pressures.empty()) {
      ...
      std::set<IValueSet *> valueSets = pressure->Component().Links<IValueSet>();
...
void CFractureMatrixPressure::LoadComponents(TSTREAM &stream, CStreamVersion &version, TPROGRESS &progress) {
  ...
  case REPEAT_MODE:
    ...
    break;
  case CONSTANT_MODE: {
    CComponentConstant<TFractureMatrixPressure> *pConstant =
        new CComponentConstant<TFractureMatrixPressure>(*this, true);
...
TFractureMatrixPressureBase &CFractureMatrixPressure::FormationStageLoad(CFormationBase &formation,
