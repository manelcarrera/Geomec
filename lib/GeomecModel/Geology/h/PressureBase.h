...
template <typename Pressure>
  class CPressureComponent : public IValueComponentBase
{
	bool m_bEmpty;
...
template <typename Pressure>
  CPressureComponent <Pressure>::CPressureComponent(CFormationLoadBase<Pressure> &pressure, unsigned int uMode, bool /*bEmpty*/)
...
template <typename Pressure>
IValueComponentBase::TValueComponentType CPressureComponent<Pressure>::Type() const
{
  return static_cast<IValueComponentBase::TValueComponentType>(ScalarData().Type());
}

template <typename Pressure>
const IValueDataInterfaceScalar& CPressureComponent<Pressure>::ScalarData() const
{
  return m_Data;
}

template <typename Pressure>
IValueDataInterfaceScalar& CPressureComponent<Pressure>::ScalarData()
{
  return m_Data;
}

template <typename Pressure>
  const CFormationLoadBase<Pressure>& CPressureComponent <Pressure>::PressureBase() const
{
...
template <typename Pressure>
  CFormationLoadBase<Pressure>& CPressureComponent <Pressure>::PressureBase()
{
...
template <typename Pressure>
  IValueDomainScalar::TMinMax CPressureComponent <Pressure>::MinMax(IProgressBase& progressBase, const CQuantity::UNIT unit) const
{
...
template <typename Pressure>
  IValueDomainScalar::TValue CPressureComponent <Pressure>::Average(IProgressBase& progressBase, const CQuantity::UNIT unit) const
{
	geo::CBox box(PressureBase().Min(), PressureBase().Max());
...
template <typename Pressure>
  QString CPressureComponent <Pressure>::UnitName(const CQuantity::UNIT unit) const
{
	QString sRet;
...
template <typename Pressure>
  QString CPressureComponent <Pressure>::ExportLabel() const
{
	 return getStringTableEntry(IDS_ET_PRESSURE);
}

template <typename Pressure>
IValueDomainScalar::TValueVec CPressureComponent <Pressure>::ValueElement(const geo::IElement& elm, const CQuantity::UNIT unit, geo::IParallelInitializationCallback *cb) const
{
	...
	if(PressureBase().DistributedSize() > 0)
	{
		for(size_t i = 0; i < PressureBase().DistributedSize(); i++)
		{
			...
			{
				if((PressureBase().DistributedOnly() || pressure.PointSet().PointInConvexHull(elm.Point(n))) && vcValue[n].Valid())
				{
					if(vcNrOfValue[n] == 0)
						vcRet[n] = vcValue[n];
					else
...
template <typename Pressure>
IValueDomainScalar::TValue CPressureComponent <Pressure>::ValuePoint(const geo::IPoint& pt, const CQuantity::UNIT unit, geo::IParallelInitializationCallback *cb) const
{
	if(PressureBase().DistributedSize() > 0)
	{
		int n = 0;
...
template <typename Pressure>
  bool CPressureComponent <Pressure>::operator==(const CPressureComponent &rhs) const
{
	return IValueComponentBase::operator ==(rhs);
}

template <typename Pressure>
  bool CPressureComponent <Pressure>::Empty() const
{
	return m_bEmpty;
}

template <typename Pressure>
  void CPressureComponent <Pressure>::LoadStream(TSTREAM &/*stream*/, CStreamVersion &/*version*/, TPROGRESS &/*progress*/)
{
	m_bEmpty = false;
}
...
template <typename Pressure>
  bool CPressureComponent <Pressure>::CanMap(const COpenGLNode& node) const
{
