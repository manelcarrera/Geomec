...
public:
  ...
  {return m_pFormation->Name().toStdString();}
...
public:
    ...
    //CNewWellPath(const QString &file, const QString &name, CFemAppModel &model, CQuantity::UNIT unit, IProgressBase &dlg);
...
public:
    ...
    virtual void
        SaveStream(TSTREAM& stream, TPROGRESS& progress);
    virtual void
        LoadStream(TSTREAM& stream, CStreamVersion& version,TPROGRESS& progress);
...
private:
    ...
    void Recalculate
        ( const double &GlobalNorthing
        , const double &GlobalEasting
        , const double &GlobalTVD
        );
...
public:
    ...
    std::list<INewWellSection *>& FormationSectionPointers()
    { return m_FormationSectionPointers; }
...
private:
    void LoadStreamOldToNew
        ( TSTREAM& stream, CStreamVersion& version,TPROGRESS& progress);
    void LoadMeshIntersectionNew_Skip
        ( TSTREAM& stream, CStreamVersion& version, TPROGRESS& progress);
    void LoadMeshIntersection_Skip
        ( TSTREAM& stream, CStreamVersion& version, TPROGRESS& progress);
    void LoadWellLogData_Skip
        ( TSTREAM& stream, CStreamVersion& version, TPROGRESS& progress);
    void LoadWellLog_Skip
        ( /*well::CWellLog *log,*/ TSTREAM& stream, TPROGRESS& progress);
    void LoadDCasint_Skip
        ( TSTREAM& stream, CStreamVersion& version, TPROGRESS& progress);
    ...
    static geo::CPoint QuadLineIntersect
        ( const geo::IQuadrilateral &quad
        , const geo::ILine &line
        );
    bool CalculateInterSectionWithFace
...
public:
  ...
  static bool ImportPetrelFile(const QString &strFileName, IProgressBase &dlg,
    CModelBase &model, const CQuantity::UNIT& lateralUnit,
...
template<class T>
CNewResultMap<T>::CNewResultMap(CNewWellPath* pWellPath)
...
template<class T>
void CNewResultMap<T>::RemoveResult(const CNewWellPoint& wellpoint)
{
  //point found
...
template<class T>
bool CNewResultMap<T>::ResultExist(const CNewWellPoint& Wellpoint,const CDepletionStage& stage,const CAnalysisType& antype) const
{
  //point found
  typename TResultMap::iterator it =
    m_Result.find(CNewWellPointResultKey(Wellpoint, stage,antype));
  return !(it == m_Result.end());
}


template<class T>
T& CNewResultMap<T>::GetResult(const CNewWellPoint& Wellpoint,const CDepletionStage& stage,const CAnalysisType& antype) const
{
...
template<class T>
void CNewResultMap<T>::ClearAllResults()
{
  m_Result.clear();
}
