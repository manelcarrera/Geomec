...
public:
	...
	public:
		...
		public:
			...
			CCoordinateComponent(const QString& sName, CCoordinateType& value_type, CNodalValueSet& value_set, int uComponentIndex);
	...
	private:
		...
		void Update() const;
	public:
		CCoordinateType(CPointSet& point_set, 
...
private:
  ...
  bool DoSample(CPointSet &sampledPointSet, double boxSize_north, double boxSize_east, double boxSize_depth, IProgressBase &progress);
...
public:
  ...
  int PushBack(const geo::IPoint &point, const std::vector<double> &vcRow, bool bConvertValue = false);
	...
	CPointSet(const QString &sName, 
			  CFemAppModel& model, 
			  int nValueSetSize, 
			  DIMENSION dim, 
			   bool bCreateValueSetForCoordinates = true,
         bool bLinkToEntry = true);
	CPointSet(const QString &strName, 
			   CFemAppModel& model, 
			   const std::vector<TLineVec> vcData, 
			   const DIMENSION dim,
			   bool bCreateValueSetForCoordinates = true);
	...
	virtual void Sample(const QString& name, const double blockSizeNorth,
		const double blockSizeEast, const double blockSizeDepth);
	virtual void Export(const IExportFormat::CExportArg& arg,
