...
CMaterialCamClay::CMaterialCamClay(CMaterialEntry &entry, CLibraryMaterial &libmat) : IMaterialRock(entry, libmat) {}
...
bool CMaterialCamClay::WriteFilosParamName(const CFFMaterial &ffmat, dia::IDianaRunner &diarunner, int i,
                                           char *name) const {
  if (i < 8) {
    switch (i) {
    case 0:
...
void CMaterialCamClay::WriteFilosParamValues(const CFFMaterial &ffmat, dia::IDianaRunner &diarunner, double *values,
                                             int stride) const {
...
bool CMaterialCamClay::WriteDefaultPorosity() const { return false; }