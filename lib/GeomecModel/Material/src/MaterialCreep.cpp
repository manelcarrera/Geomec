...
CMaterialCreep::CMaterialCreep(CMaterialEntry &entry, CLibraryMaterial& libmat) 
...
bool CMaterialCreep::WriteFilosParamName(const CFFMaterial &ffmat, dia::IDianaRunner& diarunner, int i, char *name) const
{
  if (i == 0)
  {
...
void CMaterialCreep::WriteFilosParamValues(const CFFMaterial &ffmat, dia::IDianaRunner& diarunner, double *values, int stride) const
{
  double dCohesi = ffmat.ParameterValue(IDT_VALUETYPE_COHESION) * 1e6;
  double dFricti = ffmat.ParameterValue(IDT_VALUETYPE_FRICTION_ANGLE) * PI / 180;
  double dA1 = ffmat.ParameterValue(IDT_VALUETYPE_CREEP_A1);
