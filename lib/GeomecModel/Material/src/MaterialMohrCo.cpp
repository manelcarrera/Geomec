...
CMaterialMohrCo::CMaterialMohrCo(CMaterialEntry &entry, CLibraryMaterial &libmat) : IMaterialRock(entry, libmat) {}
...
bool CMaterialMohrCo::WriteFilosParamName(const CFFMaterial &ffmat, dia::IDianaRunner &diarunner, int i,
                                          char *name) const {
  if (i == 0) {
...
void CMaterialMohrCo::WriteFilosParamValues(const CFFMaterial &ffmat, dia::IDianaRunner &diarunner, double *values,
                                            int stride) const {
  double dCohesi = ffmat.ParameterValue(IDT_VALUETYPE_COHESION) * 1e6;
  double dFricti = ffmat.ParameterValue(IDT_VALUETYPE_FRICTION_ANGLE) * PI / 180;
  double dDilatation = ffmat.ParameterValue(IDT_VALUETYPE_DILATATION) * PI / 180;
