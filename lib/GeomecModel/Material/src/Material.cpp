  ...
  CMaterialErrorHandler& materialErrorHandler)
{
  ...
  for(it = lstParamValue.begin(); it != lstParamValue.end(); ++it)
  {
    ...
    materialErrorHandler.handleError(mat.Name(),
      mat.ParameterByValueTypeID(it->first)->Name(), strErrorMsg);
    ...
    if(pMat1)
    {
      ...
      if(libmat.GroupSize() > 0)
      {
        ...
        const CModelBase& modelBase =
          dynamic_cast <const CModelBase& > (Material().Model());
        CMaterialErrorHandler& materialErrorHandler =
          const_cast <CModelBase& > (modelBase).getMaterialErrorHandler();

        QString errorMessage;
        CLibraryMaterialParameter* libraryMaterialParameter = 0;

        // Assign only if the domain is valid
        for (TValueMap::const_iterator value = m_mpValue.begin();
          value != m_mpValue.end(); ++value)
        {
        ...
        for(int i = 1; i < pMat2->GroupSize(); ++i)
          AssignParameterValuesToGroup(pMat2->Group(i), m_mpValue,
            materialErrorHandler);

        // now process group 0, this will overwrite any values that were set in
        // the other groups
        AssignParameterValuesToGroup(pMat2->Group(0), m_mpValue,
          materialErrorHandler);
...
void CFFMaterial::SetParameterValue(unsigned int uValueTypeID, double dValue)
{
  m_mpValue[uValueTypeID] = dValue;
}

const IMaterialBase& CFFMaterial::Material() const
{
  return m_Material;
}

bool CFFMaterial::IsParameter(unsigned int ValueTypeID) const
{
	return m_Material.IsParameter(ValueTypeID);
}

double CFFMaterial::ParameterValue(unsigned int ValueTypeID) const
{
	TValueMap::const_iterator it = m_mpValue.find(ValueTypeID);
		if(it != m_mpValue.end()) return it->second;
...
bool CFFMaterial::operator<(const dia::IMaterial &rhs) const
{
  ...
  if(pRef)
  {
	  ...
	  if((!bRet) && (!pRef->m_Material.Less(m_Material)))
	  {
		  ...
		  return std::lexicographical_compare(m_mpValue.begin(),
											  m_mpValue.end(),
											  pRef->m_mpValue.begin(),
											  pRef->m_mpValue.end());
	  }
...
bool CFFMaterial::WriteFilos(dia::IDianaRunner& diarunner) const
{
	return m_Material.Write(*this, diarunner);
}

int CFFMaterial::Type() const
{
