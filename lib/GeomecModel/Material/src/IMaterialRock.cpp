...
IMaterialRock::IMaterialRock(CMaterialEntry &entry, CLibraryMaterial &libmat) : IMaterial(entry, libmat) {}
...
int IMaterialRock::WriteFilosTransverseIsotropicParamSize(const CFFMaterial & /*ffmat*/,
                                                          dia::IDianaRunner &diarunner) const {
  assert(dynamic_cast<CGeomecDianaRunnerBase *>(&diarunner));
  CGeomecDianaRunnerBase &runner = static_cast<CGeomecDianaRunnerBase &>(diarunner);
  if (runner.Model().UseDecompactionParameters()) {
    return 10;
...
bool IMaterialRock::WriteFilosTransverseIsotropicParamName(const CFFMaterial & /*ffmat*/, dia::IDianaRunner &diarunner,
...
void IMaterialRock::WriteFilosTransverseIsotropicParamValues(const CFFMaterial &ffmat, dia::IDianaRunner &diarunner,
