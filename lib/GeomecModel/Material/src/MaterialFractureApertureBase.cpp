...
geo::CVector CMaterialFractureApertureBase::VectorFromAngles(double dInclination, double dAzimuth) const
{
  ...
  geo::CVector vec(sin(inc) * cos(azi),
                   sin(inc) * sin(azi),
                   cos(inc));
...
bool CMaterialFractureApertureBase::WriteFilosParamName(const CFFMaterial &ffmat, dia::IDianaRunner& diarunner, int i, char *name) const
{
  if (i < 12)
  {
...
void CMaterialFractureApertureBase::WriteFilosParamValues(const CFFMaterial &ffmat, dia::IDianaRunner& diarunner, double *values, int stride) const
{
  double dStiffNormal;
  double dStiffShear;
