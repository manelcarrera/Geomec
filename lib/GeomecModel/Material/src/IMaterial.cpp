...
IMaterial::IMaterial(CMaterialEntry &entry, CLibraryMaterial &libmat)
    : IMaterialBase("", entry.Model()), m_entry(entry), m_LibraryMaterial(libmat), m_observer(*this, libmat) {
  CreateParameters();
}

IMaterial::~IMaterial() { DestroyParameters(); }
...
IMaterial& IMaterial::operator =(const IMaterial& rhs)
{
  ...
  m_entry.Library().SetMaxNumIterations(Name().toStdString().c_str(),
rhs.m_entry.Library().GetMaxNumIterations(Name().toStdString().c_str()));
  m_entry.Library().SetConvCriterion(Name().toStdString().c_str(),
rhs.m_entry.Library().GetConvCriterion(Name().toStdString().c_str()));
  ...
  for(int i = 0; i < m_entry.Library().GetMatParamCount(Name().toStdString().c_str()); i++)
  {
    ...
    m_entry.Library().GetParameter(Name().toStdString().c_str(),
sParamName)->SetValue(rhs.m_entry.Library().GetParameter(Name().toStdString().c_str(), sParamName)->GetValue());
    m_entry.Library().GetParameter(Name().toStdString().c_str(),
sParamName)->SetFixed(rhs.m_entry.Library().GetParameter(Name().toStdString().c_str(), sParamName)->GetFixed());
...
void IMaterial::CreateParameters() {
  for (size_t i = 0; i < m_LibraryMaterial.ParameterSize(); ++i) {
    ...
    assert(succeeded);
    succeeded =
        m_mpParameters.insert(TParameterMap::value_type(&param, new CMaterialParameter(uiValueTypeID, param))).second;
    assert(succeeded);
  }
  /*
    ...
    for(int i = 0; i < nParams; i++)
    {
      ...
      m_mpParameters.insert(TParameterMap::value_type(uiValueTypeID, CMaterialParameter(dValue, uiValueTypeID,
    ParamName)));
    }
  */
}
...
unsigned int IMaterial::TypeId() const { return 0; }
...
bool IMaterial::CanDestroy() const { return !IsLocked(); }

bool IMaterial::IsLocked() const { return false; }

bool IMaterial::Valid() const { return true; }
                   ...
                   int nMaxIterations, double dConvCriterion)
{
  ...
  for(i = 0; i < m_entry.Library().GetMatParamCount(Name().toStdString().c_str()); i++)
  {
    ...
    std::map<QString, std::pair<double, bool>, TnoCaseStringCompare>::iterator it =
mpParam.find(m_entry.Library().GetMatParamName(Name().toStdString().c_str(), i)); if(it != mpParam.end())
    {
...
long IMaterial::SavedItems() const { return CStorageNode::SavedItems() + m_mpParameters.size() + 2; }

bool IMaterial::Empty() const { return !IsLinkedTo(m_entry); }
...
void IMaterial::ConfigureReadOnlyParametersInLibrary() TODO wedx
{
  ...
  if(CMaterialServer::FixedParameters[i].model == MaterialModel() || CMaterialServer::FixedParameters[i].model == -1)
      lib.SetParameterReadOnlyForMaterial(Name().toStdString().c_str(),
CMaterialServer::FixedParameters[i].param.c_str());
  }
}
...
void IMaterial::ConfigurePermanentReadOnlyParametersInLibrary() TODO wedx
{
  ...
  if(MaterialModel() != MM_FRACTURE_ANISOTROPY && MaterialModel() != MM_UPSCALED_ANISOTROPY && MaterialModel() !=
MM_FRACTURE_APERTURE) lib.SetParameterReadOnly(MLD_SHEARMODULUS);
}
...
bool IMaterial::Less(const CGraphNode &node) const {
  ...
  if (pMat)
    return std::lexicographical_compare(m_mpParameters.begin(), m_mpParameters.end(), pMat->m_mpParameters.begin(),
                                        pMat->m_mpParameters.end());
...
bool IMaterial::Write(const CFFMaterial &ffmat, dia::IDianaRunner &diarunner) const {
  ...
  if (runner.Model().UseDecompactionParameters() && ffmat.IsParameter(IDT_VALUETYPE_YOUNG_DECOMP) &&
...
int IMaterial::Type() const { return MaterialModel(); }
...
void IMaterial::WriteFilosParamValues(const CFFMaterial &ffmat, dia::IDianaRunner &diarunner, double *values,
                                      int stride) const {
  assert(!Empty());
  assert(dynamic_cast<CGeomecDianaRunnerBase *>(&diarunner));
  CGeomecDianaRunnerBase &runner = static_cast<CGeomecDianaRunnerBase &>(diarunner);
...
bool IMaterial::WriteFilosK0ParamName(const CFFMaterial & /*ffmat*/, dia::IDianaRunner & /*diarunner*/, int i,
                                      char *name) const {
  if (i < 2) {
...
void IMaterial::WriteFilosK0ParamValues(const CFFMaterial &ffmat, dia::IDianaRunner & /*diarunner*/, double *values,
                                        int stride) const {
...
bool IMaterial::CanExportToMatLib() const { return m_entry.HasValidMatLib(); }

void IMaterial::ExportToMatLib() { m_entry.ExportToMatLib(LibraryMaterial()); }
...
const CLibraryMaterial &IMaterial::LibraryMaterial() const { return m_LibraryMaterial; }

CLibraryMaterial &IMaterial::LibraryMaterial() { return m_LibraryMaterial; }
...
IMaterial::CObserver::CObserver(IMaterial &parent, ml::CMaterial &notifier) : m_parent(parent), m_pNotifier(&notifier) {
...
void IMaterial::CObserver::Modified() { m_parent.LibraryMaterialModified(); }
