...
CInterfaceBoundary::CInterfaceBoundary(const geo::IPoint &ptMin, const geo::IPoint &ptMax, CFemAppModel &model,
                                       BOUNDARY_STATE state)
    : CBoundaryBase(ptMin, ptMax, model, state), m_bCreateInterfaces(model.parentModel() != 0), m_pInterfaceElements(0),
      m_pBoundaryInterfaceDef(new CBoundaryInterfaceDef(*this)) {
  CreateChildren();
}

CInterfaceBoundary::CInterfaceBoundary(const CInterfaceBoundary &rhs)
    : CBoundaryBase(rhs), m_bCreateInterfaces(rhs.m_bCreateInterfaces), m_pInterfaceElements(0),
      m_pBoundaryInterfaceDef(new CBoundaryInterfaceDef(*rhs.m_pBoundaryInterfaceDef)),
      m_pDisplacementSupportNode(rhs.m_pDisplacementSupportNode), m_pPressureSupportNode(rhs.m_pPressureSupportNode) {}

CInterfaceBoundary::~CInterfaceBoundary() { delete m_pBoundaryInterfaceDef; }
...
const CBoundaryInterfaceDef &CInterfaceBoundary::InterfaceDefinition() const { return *m_pBoundaryInterfaceDef; }

CBoundaryInterfaceDef &CInterfaceBoundary::InterfaceDefinition() { return *m_pBoundaryInterfaceDef; }

void CInterfaceBoundary::ToggleInterfaces() { CreateInterfaces(!CreateInterfaces()); }

bool CInterfaceBoundary::CreateInterfaces() const { return m_bCreateInterfaces; }

bool CInterfaceBoundary::HasInterfaces() const { return CreateInterfaces(); }
  ...
  /*
    ...
    if(is.second == this) // side surface
    {
      ...
      for(k = 0; k < tetsurf.FaceSize(); ++k)
      {
            ...
            mpFace2BoundarySurface.insert(TFace2BoundarySurfaceMap::value_type(&face,
...
const geo::CElementGroup *CInterfaceBoundary::InterfaceElements() const { return m_pInterfaceElements; }
