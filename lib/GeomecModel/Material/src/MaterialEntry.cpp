...
CMaterialEntry::CMaterialEntry(int nEntryId, unsigned int uIconId, const QString &strName, CModelBase &model)
    : CGraphEntryTemp<IMaterial>(nEntryId, uIconId, strName, model), m_pMaterialLibrary(new ml::CMaterialLibrary),
      m_pLibraryObserver(new CMaterialLibraryObserver(*this)), m_pExternalLibrary(0) {}

CMaterialEntry::CMaterialEntry(int nEntryId, unsigned int uIconId, unsigned int uName, CModelBase &model)
    : CGraphEntryTemp<IMaterial>(nEntryId, uIconId, uName, model), m_pMaterialLibrary(new ml::CMaterialLibrary),
      m_pLibraryObserver(new CMaterialLibraryObserver(*this)), m_pExternalLibrary(0) {}
...
const QString &CMaterialEntry::GetExternalMatLibFileName() const { return m_strExternalLibraryFileName; }

bool CMaterialEntry::HasValidMatLib() const { return !m_strExternalLibraryFileName.isEmpty(); }
  ...
  for (int i = 0; i < nMaterialSize; i++) {
    ...
    if (nMaterialModel == MM_FRACTURE_APERTURE) {
      ...
      if (!bLockedFractureApertureMessageShown) {
        _m()->msg("At least one formation makes use of the obsolete 'Fracture Aperture' material model, these "
                  "materials have been changed into the new 'Fracture Aperture 2' material model");
...
int CMaterialEntry::MaterialSize() const { return m_pMaterialLibrary->MaterialSize(); }

const IMaterial &CMaterialEntry::Material(int i) const { return (const_cast<CMaterialEntry *>(this))->Material(i); }
...
int CMaterialEntry::MaterialModelFilter() const {
  ...
  int nFilter = MLFO_LINEAR | MLFO_CAMCLAY | MLFO_CREEP | MLFO_MOHRCOULOMB | MLFO_MODIFIEDMOHRCOULOMB |
                MLFO_MC_COHESION_HARD1 | MLFO_MC_COHESION_HARD2 | MLFO_MC_COHESION_HARD3 | MLFO_MC_FRICTION_HARD1 |
                MLFO_MC_FRICTION_HARD2 | MLFO_MC_FRICTION_HARD3 | MLFO_UNDRAINED | MLFO_RIGIDITY | MLFO_DUALCAP_LINELA |
                MLFO_FRACTURE_ANISOTROPY | MLFO_UPSCALED_ANISOTROPY | MLFO_ANISOTROPIC_CAMCLAY |
                MLFO_FRACTURE_APERTURE2 | MLFO_CAMCLAY_CREEP;
...
CMaterialEntry::CMaterialLibraryObserver::CMaterialLibraryObserver(CMaterialEntry &entry) : m_entry(entry) {
