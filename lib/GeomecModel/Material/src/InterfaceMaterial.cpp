...
CInterfaceMaterial::CInterfaceMaterial(CInterfaceMaterialEntry &entry, const QString &strName)
    : IMaterialBase(strName, entry.Model()), m_dCohesion(DEF_COHESION), m_dFriction(DEF_FRICTION),
      m_dNormalStiffness(DEF_NORMALSTIFF), m_dShearStiffness(DEF_SHEARSTIFF), m_dDFlux(DEF_DFLUX) {
  reParent(&entry);
}

CInterfaceMaterial::CInterfaceMaterial(const CInterfaceMaterial &rhs)
    : IMaterialBase(rhs), m_dCohesion(rhs.m_dCohesion), m_dFriction(rhs.m_dFriction),
      m_dNormalStiffness(rhs.m_dNormalStiffness), m_dShearStiffness(rhs.m_dShearStiffness), m_dDFlux(rhs.m_dDFlux) {}
...
bool CInterfaceMaterial::operator==(const CInterfaceMaterial &rhs) const {
  ...
  return (fabs(m_dCohesion - rhs.m_dCohesion) < 1e-6 && fabs(m_dFriction - rhs.m_dFriction) < 1e-6 &&
          fabs(m_dNormalStiffness - rhs.m_dNormalStiffness) < 1e-6 &&
          fabs(m_dShearStiffness - rhs.m_dShearStiffness) < 1e-6 && fabs(m_dDFlux - rhs.m_dDFlux) < 1e-6);
}
...
unsigned int CInterfaceMaterial::TypeId() const { return 0; }
...
bool CInterfaceMaterial::IsParameter(unsigned int ValueTypeID) const {
  return (ValueTypeID == IDT_VALUETYPE_COHESION || ValueTypeID == IDT_VALUETYPE_FRICTION_ANGLE ||
          ValueTypeID == IDT_VALUETYPE_DFLUX || ValueTypeID == IDT_VALUETYPE_NORMALSTIFFNESS ||
...
int CInterfaceMaterial::Type() const { return 100; }
...
bool CInterfaceMaterial::WriteFilosParamName(const CFFMaterial &ffmat, dia::IDianaRunner &diarunner, int i,
                                             char *name) const {
  assert(dynamic_cast<CGeomecDianaRunnerBase *>(&diarunner));
  CGeomecDianaRunnerBase &runner = static_cast<CGeomecDianaRunnerBase &>(diarunner);
...
void CInterfaceMaterial::WriteFilosParamValues(const CFFMaterial &ffmat, dia::IDianaRunner &diarunner, double *values,
                                               int stride) const {
  assert(dynamic_cast<CGeomecDianaRunnerBase *>(&diarunner));
  CGeomecDianaRunnerBase &runner = static_cast<CGeomecDianaRunnerBase &>(diarunner);
...
double CInterfaceMaterial::Cohesion() const { return m_dCohesion; }

double CInterfaceMaterial::Friction() const { return m_dFriction; }

double CInterfaceMaterial::NormalStiffness() const { return m_dNormalStiffness; }

double CInterfaceMaterial::ShearStiffness() const { return m_dShearStiffness; }

double CInterfaceMaterial::DFlux() const { return m_dDFlux; }
...
bool CInterfaceMaterial::Empty() const { return false; }
...
bool CInterfaceMaterial::Less(const CGraphNode &node) const {
  ...
  if (pMat) {
    if (m_dCohesion < pMat->m_dCohesion)
      return true;
    if (m_dCohesion > pMat->m_dCohesion)
      return false;

    if (m_dFriction < pMat->m_dFriction)
      return true;
    if (m_dFriction > pMat->m_dFriction)
      return false;

    if (m_dNormalStiffness < pMat->m_dNormalStiffness)
      return true;
    if (m_dNormalStiffness > pMat->m_dNormalStiffness)
      return false;

    if (m_dShearStiffness < pMat->m_dShearStiffness)
      return true;
    if (m_dShearStiffness > pMat->m_dShearStiffness)
      return false;
...
void CInterfaceMaterialEntry::LoadStream(CFemAppModel & /*model*/, TSTREAM &stream, CStreamVersion &version,
                                         TPROGRESS &progress) {
  int nMat;
  stream >> nMat;
...
const QString DEFAULT_INTERFACE_MATERIAL_NAME = QObject::tr("New Interface Material %1");
const size_t DEFAULT_INITIAL_NUMBER = 1;

} // anonymous namespace

CInterfaceMaterial &CInterfaceMaterialEntry::InsertNew() {
  QString sName = createUniqueName(DEFAULT_INTERFACE_MATERIAL_NAME, DEFAULT_INITIAL_NUMBER);
