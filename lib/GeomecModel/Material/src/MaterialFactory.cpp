...
{
  bool b = m_mpFactory.insert(TFactoryMap::value_type(iMaterialModel, std::make_pair(createMethod, getCreatorMethod))).second;
...
void CMaterialFactory::RegisterMaterial()
{
  Register(MATERIAL_MODEL, &IMaterial::Create<MATERIAL_TYPE>, &MATERIAL_TYPE::GetLibraryMaterialCreator);
}

CMaterialFactory::TFactoryMap::const_iterator
  CMaterialFactory::findMaterialModel(int materialModel) const
{
