...
template <class PARENT> class IRockMaterialServerTempl : public IMaterialServerTempl<IMaterialRock, PARENT> {
...
typename IRockMaterialServerTempl<PARENT>::TMatParamPair IRockMaterialServerTempl<PARENT>::InitialOnlyParameters[] = {
    {-1, MLD_LATRATIO_MAX}, {-1, MLD_LATRATIO_MIN}, {-1, MLD_AZIMUTH}, {-1, QString()} // terminator (param.isEmpty())
};
...
template <class PARENT> IRockMaterialServerTempl<PARENT>::~IRockMaterialServerTempl() {}

template <class PARENT> bool IRockMaterialServerTempl<PARENT>::ConnectItem(const CGraphNode &item) {
  ...
  if (pValueType) {
    ...
    if (!TBase::Loading()) {
      ...
      for (i = 0; !FixedParameters[i].param.isEmpty(); ++i) {
        if ((FixedParameters[i].model == -1 || FixedParameters[i].model == TBase::LibraryMaterial()->MaterialModel()) &&
    ...
    if (bIsFixed) {
      ...
      while (pStage) {
        IRockMaterialServerTempl *pServer = TBase::Parent().ConnectedMaterial(*pStage);
...
template <class PARENT> bool IRockMaterialServerTempl<PARENT>::CanConnectItem(const CGraphNode &item) const {
  if (!TBase::ReadOnly()) {
      ...
      if (!TBase::IsLinkedTo(item) && IsValueTypeValid(*pType)) {
        if (!TBase::Stage().Initial()) {
          if (IsInitialOnlyParameter(pType->TypeId())) {
            TBase::StatusMessage(
                "This material parameter can only be attached to the initial depletion stage's material");
            return false;
          }
        }
            ...
            for (i = 0; i < pMat->ParameterSize(); ++i) {
              ...
              if (uiValueTypeID == pType->TypeId()) {
                ...
                if (pGroup) {
                  ...
                  std::set<CValueType *> stVT =
                      (const_cast<IRockMaterialServerTempl<PARENT> *>(this))->template Links<CValueType>();
                    ...
                    if (CMaterialParameter::MatLibXIDFromValueTypeID((*it)->TypeId(), strParamName)) {
                      ...
                      if (pParam) {
                        ...
                        if ((parameterGroup != 0) && (parameterGroup != pGroup)) {
                          TBase::StatusMessage(
                              "Material parameters of different parameter groups are not allowed simultaneously");
                          return false;
                        }
                      }
                    }
        ...
        const IRockMaterialServerTempl *pIniServer = TBase::Parent().ConnectedMaterial(TBase::Stage().InitialStage());
        assert(pIniServer != 0);
...
template <class PARENT> bool IRockMaterialServerTempl<PARENT>::IsFixedParameter(unsigned int typeID) const {
  QString strMatLibXID;
...
template <class PARENT> bool IRockMaterialServerTempl<PARENT>::IsValueTypeValid(const CValueType &value_type) const {
    ...
    if (pMPTensor) {
      for (unsigned int i = 0; i < pMPTensor->ComponentSize(); ++i) {
        if (!CMaterialParameter::MatLibXIDFromValueTypeID(pMPTensor->ComponentTypeID(i), strMatLibXID))
          return false;
        if (!TBase::LibraryMaterial()->IsParameter(pMPTensor->ComponentTypeID(i)))
          return false;
      }
      return true;
    }
...
template <class PARENT>
bool IRockMaterialServerTempl<PARENT>::MatchingParameterValues(const IMaterialRock &mat1, const IMaterialRock &mat2,
                                                               const QString &sParamName) const {
...
template <class PARENT> void IRockMaterialServerTempl<PARENT>::OnNeighbourDeleted(const CGraphNode &node) {
  for (size_t i = 0; i < TBase::ValueTypePairVec().size(); i++) {
    if (TBase::ValueTypePairVec()[i].first == &node) {
      const CMatParamsTensor *pMPTensor = dynamic_cast<const CMatParamsTensor *>(TBase::ValueTypePairVec()[i].first);
      if (pMPTensor) {
        ...
        UnlinkFixedParameterFromStages(typeId, *TBase::ValueTypePairVec()[i].first);
      }
    }
  }
...
template <class PARENT>
bool IRockMaterialServerTempl<PARENT>::UnlinkFixedParameterFromStages(unsigned int typeId,
                                                                      const CValueType &valueType) {
  QString strMatLibXID;
  VERIFY(CMaterialParameter::MatLibXIDFromValueTypeID(typeId, strMatLibXID));
  int j;
  for (j = 0; !FixedParameters[j].param.isEmpty(); ++j) {
    if (TBase::LibraryMaterial() &&
        (FixedParameters[j].model == -1 || FixedParameters[j].model == TBase::LibraryMaterial()->MaterialModel()) &&
...
template <class PARENT> bool IRockMaterialServerTempl<PARENT>::IsInitialOnlyParameter(unsigned int typeID) const {
  QString strMatLibXID;
  VERIFY(CMaterialParameter::MatLibXIDFromValueTypeID(typeID, strMatLibXID));
  int i;
  for (i = 0; !InitialOnlyParameters[i].param.isEmpty(); ++i) {
    if ((InitialOnlyParameters[i].model == -1 ||
         InitialOnlyParameters[i].model == TBase::LibraryMaterial()->MaterialModel()) &&
...
template <class PARENT> void IRockMaterialServerTempl<PARENT>::OnNewNeighbour(const CGraphNode &node) {
  ...
  if (pMaterial && !TBase::Stage().Initial() && !TBase::Loading()) {
    ...
    IRockMaterialServerTempl *pIniServer = TBase::Parent().ConnectedMaterial(TBase::Stage().InitialStage());
    if (pIniServer) {
      const typename TBase::TValueTypePairVec &vcValueTypes = pIniServer->ValueTypePairVec();
...
template <class PARENT>
const CFFMaterial &IRockMaterialServerTempl<PARENT>::Material(const geo::IElement &element,
                                                              const CFFMaterial::TValueMap &mpValueInit) const {
  assert(TBase::LibraryMaterial() != 0);

  for (size_t i = 0; i < TBase::ValueTypePairVec().size(); i++) {
    const CValueType *pValueType = TBase::ValueTypePairVec()[i].first;
    assert(pValueType);
...
template <class PARENT>
const CFFMaterial &IRockMaterialServerTempl<PARENT>::Material(const geo::IPoint &point,
                                                              const CFFMaterial::TValueMap &mpValueInit) const {
  assert(TBase::LibraryMaterial() != 0);
  ...
  for (size_t i = 0; i < TBase::ValueTypePairVec().size(); i++) {
    ...
    if (pMPTensor) {
      ...
      for (unsigned int j = 0; j < pMPTensor->ComponentSize(); ++j) {
        ...
        if (value.Valid() &&
            (TBase::IsExtrapolatingValueType(*pMPTensor) || pMPTensor->PointSet().PointInConvexHull(point))) {
...
template <class PARENT> IMaterialRock *IRockMaterialServerTempl<PARENT>::FindIndexInEntry(int nIndex) const {
  CMaterialEntry &material_entry = (CMaterialEntry &)*((CModelBase &)TBase::Model()).GraphEntry(MD_ROCK_MATERIAL);
...
template <class PARENT> int IRockMaterialServerTempl<PARENT>::MaterialModel() const {
  return IMaterialServerTempl<IMaterialRock, PARENT>::LibraryMaterial()->MaterialModel();
}

template <class PARENT> bool IRockMaterialServerTempl<PARENT>::FixedMaterialModel() const {
  ...
  while (pStage) {
    const IRockMaterialServerTempl *pServer =
        IMaterialServerTempl<IMaterialRock, PARENT>::Parent().ConnectedMaterial(*pStage);
    if (pServer && pServer != this && pServer->LibraryMaterial() != 0 &&
        pServer->LibraryMaterial() != IMaterialServerTempl<IMaterialRock, PARENT>::LibraryMaterial())
      return true;
