...
template <class MATERIAL, class PARENT> class IMaterialServerTempl : public IMaterialServerBase {
public:
  // wjrx mantis 2583
...
public:
  virtual void AddToCache(CMaterialKey &key, CFFMaterial *material) const { m_mpMaterials[key] = material; }
...
private:
  TValueTypeSet m_stExtrapolatingValueTypes; // any valuetype (distributed property) in here is extrapolated outside the
                                             // convex hull
...
public:
  ...
  virtual const CFFMaterial &Material(const geo::IElement &element,
                                      const CFFMaterial::TValueMap &mpValueInit = CFFMaterial::TValueMap()) const;
  virtual const CFFMaterial &Material(const geo::IPoint &point,
                                      const CFFMaterial::TValueMap &mpValueInit = CFFMaterial::TValueMap()) const;
...
template <class MATERIAL, class PARENT> void IMaterialServerTempl<MATERIAL, PARENT>::UpdateLinks() {
...
template <class MATERIAL, class PARENT> bool IMaterialServerTempl<MATERIAL, PARENT>::CanConfigureDistributions() const {
  return m_pMaterial != 0 && !ReadOnly() && !m_vcValueType.empty();
}

template <class MATERIAL, class PARENT> const QString &IMaterialServerTempl<MATERIAL, PARENT>::MaterialName() const {
  if (m_pMaterial)
...
bool IMaterialServerTempl<MATERIAL, PARENT>::MatchingMaterials(const MATERIAL & /*mat1*/,
                                                               const MATERIAL & /*mat2*/) const {
  return true;
}
...
template <class MATERIAL, class PARENT> void IMaterialServerTempl<MATERIAL, PARENT>::DefaultStatusMessage() const {
...
typedef struct _Mapping {
  ...
  _Mapping() : contained(false), count(0), valid(false), value(0) {}
} Mapping;

Mapping processValueVector(Mapping &mapping, const IValueDomainScalar::TValueVec &vcValues) {
...
template <class H>
void calculateDistance(double &distance, int &count, double &value, unsigned int &valueType,
                       const CValueType *pValueType, H *hull, const geo::IElement &element) {
  double element2Center = hull->MidPoint().Distance(element.MidPoint());
  double representativeLength = hull->RepresentativeLength();

  if ((distance == 0) || ((element2Center / representativeLength) < distance)) {
    Mapping mapping;

    processValueVector(mapping, pValueType->Component().ScalarData().ValueElement(element));
...
class CElSetWrapper {
public:
  CElSetWrapper(const IElementSet &elset) : m_elset(elset) {}

  geo::CPoint MidPoint() const {
    return geo::CPoint((Min().X() + Max().X()) / 2., (Min().Y() + Max().Y()) / 2., (Min().Z() + Max().Z()) / 2.);
  }
...
template <class MATERIAL, class PARENT>
void IMaterialServerTempl<MATERIAL, PARENT>::MapValueTypes(const geo::IElement &element,
                                                           CFFMaterial::TValueMap &mpValue) const {
  ...
  for (size_t i = 0; i < m_vcValueType.size(); i++) {
    ...
    if (materialMappingIterator == materialMapping.end()) {
      materialMappingIterator = (materialMapping.insert(std::make_pair(pValueType->TypeId(), Mapping()))).first;
    }
    ...
    if (pValueType->PointSet().PointInConvexHull(element.MidPoint())) {
      ...
      processValueVector(materialMappingIterator->second, pValueType->Component().ScalarData().ValueElement(element));
    }
  }

  for (materialMappingIterator = materialMapping.begin(); materialMappingIterator != materialMapping.end();
       ++materialMappingIterator) {
    if (materialMappingIterator->second.contained) {
      if (materialMappingIterator->second.valid && (materialMappingIterator->second.count != 0)) {
      ...
      if (IsExtrapolatingParameter(materialMappingIterator->first)) {
        ...
        for (size_t i = 0; i < m_vcValueType.size(); i++) {
          ...
          if (pValueType->TypeId() == materialMappingIterator->first) {
            ...
            if (pPointSet) {
              if ((pPointSet->m_pHull_2D == 0) && (pPointSet->m_pHull_3D == 0)) {
                pPointSet->CreateHull();
              }

              if (pPointSet->m_pHull_3D != 0) {
                calculateDistance(distance, count, value, valueType, pValueType, pPointSet->m_pHull_3D, element);
              }

              if (pPointSet->m_pHull_2D != 0) {
                calculateDistance(distance, count, value, valueType, pValueType, pPointSet->m_pHull_2D, element);
              }
            } else {
...
IMaterialServerTempl<MATERIAL, PARENT>::IMaterialServerTempl(PARENT &parent, const CDepletionStage &stage)
    : IMaterialServerBase("NONE", parent.Model()), m_stage(stage), m_parent(parent), m_pMaterial(0), m_bLoading(false) {
}

template <class MATERIAL, class PARENT> IMaterialServerTempl<MATERIAL, PARENT>::~IMaterialServerTempl() {
  Clear();
...
template <class MATERIAL, class PARENT> void IMaterialServerTempl<MATERIAL, PARENT>::Clear() const {
  m_mpMaterials.clear();
}

template <class MATERIAL, class PARENT> const QString &IMaterialServerTempl<MATERIAL, PARENT>::Name() const {
  m_strName = m_stage.Name() + " (" + MaterialName() + ")";
  return m_strName;
}

template <class MATERIAL, class PARENT> unsigned int IMaterialServerTempl<MATERIAL, PARENT>::IconId() const {
  if (m_pMaterial)
...
template <class MATERIAL, class PARENT> bool IMaterialServerTempl<MATERIAL, PARENT>::Destroy() {
  assert(CanDestroy());
  UnLink(*m_pMaterial);
...
template <class MATERIAL, class PARENT> bool IMaterialServerTempl<MATERIAL, PARENT>::CanDestroy() const {
...
void IMaterialServerTempl<MATERIAL, PARENT>::OnNewNeighbour(const CGraphNode &node) {
  ...
  if (dynamic_cast<const CValueType *>(&node)) {
    ...
    if (pValueType) {
      ...
      for (CDepletionStageEntry::const_iterator iterator = entry.begin(); iterator != entry.end(); ++iterator) {
        const IMaterialServerTempl *server = this->Parent().ConnectedMaterial(*iterator);

        if (server && (server != this)) {
          isExtrapolatingValueType = isExtrapolatingValueType || server->IsExtrapolatingValueType(*pValueType);
        }
      }
...
template <class MATERIAL, class PARENT> unsigned int IMaterialServerTempl<MATERIAL, PARENT>::TypeId() const {
  return 0;
}

template <class MATERIAL, class PARENT>
const CFFMaterial &IMaterialServerTempl<MATERIAL, PARENT>::Material(const geo::IElement &element,
                                                                    const CFFMaterial::TValueMap &mpValueInit) const {
  assert(m_pMaterial != 0);
...
template <class MATERIAL, class PARENT>
const CFFMaterial &IMaterialServerTempl<MATERIAL, PARENT>::Material(const geo::IPoint &point,
                                                                    const CFFMaterial::TValueMap &mpValueInit) const {
  assert(m_pMaterial != 0);
  ...
  for (size_t i = 0; i < m_vcValueType.size(); i++) {
    ...
    if (value.Valid() && (m_stExtrapolatingValueTypes.find(pValueType) != m_stExtrapolatingValueTypes.end() ||
...
template <class MATERIAL, class PARENT> const PARENT &IMaterialServerTempl<MATERIAL, PARENT>::Parent() const {
  return m_parent;
}

template <class MATERIAL, class PARENT> PARENT &IMaterialServerTempl<MATERIAL, PARENT>::Parent() { return m_parent; }

template <class MATERIAL, class PARENT> const CDepletionStage &IMaterialServerTempl<MATERIAL, PARENT>::Stage() const {
  return m_stage;
}
...
template <class MATERIAL, class PARENT> MATERIAL *IMaterialServerTempl<MATERIAL, PARENT>::LibraryMaterial() {
  return m_pMaterial;
}

template <class MATERIAL, class PARENT> bool IMaterialServerTempl<MATERIAL, PARENT>::Valid() const {
  return m_pMaterial != 0;
}

template <class MATERIAL, class PARENT> bool IMaterialServerTempl<MATERIAL, PARENT>::ValidValues() const {
...
template <class MATERIAL, class PARENT> const IMaterialBase *IMaterialServerTempl<MATERIAL, PARENT>::Material() const {
  return m_pMaterial;
}
...
template <class MATERIAL, class PARENT> long IMaterialServerTempl<MATERIAL, PARENT>::SavedItems() const {
  long lRet = 1;
...
template <class MATERIAL, class PARENT> bool IMaterialServerTempl<MATERIAL, PARENT>::Empty() const { return false; }

template <class MATERIAL, class PARENT>
const typename IMaterialServerTempl<MATERIAL, PARENT>::TValueTypePairVec &
IMaterialServerTempl<MATERIAL, PARENT>::ValueTypePairVec() const {
...
template <class MATERIAL, class PARENT> bool IMaterialServerTempl<MATERIAL, PARENT>::ReadOnly() const {
...
template <class MATERIAL, class PARENT> bool IMaterialServerTempl<MATERIAL, PARENT>::Less(const CGraphNode &rhs) const {
...
template <class MATERIAL, class PARENT> bool IMaterialServerTempl<MATERIAL, PARENT>::Loading() const {
  return m_bLoading;
}
