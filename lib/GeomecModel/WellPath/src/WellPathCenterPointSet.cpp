...
CWellPathCenterPointSet::CWellPathCenterPointSet(const std::vector<const CFormationBase *> &formations,
                                                 CModelBase &modelBase)
    : CPointSet("", modelBase, 0, static_cast<IPointSet::DIMENSION>(modelBase.Dimension())) {
...
TElementVertex retrieveCenterVertex(TWellPathSectionVertices &wellPathSectionVertices) {
  while (wellPathSectionVertices.size() > 1) {
    std::vector<TElementVertex> elementVertices2Clean;

    for (TWellPathSectionVertices::const_iterator elementVertex = wellPathSectionVertices.begin();
    ...
    for (std::vector<TElementVertex>::const_iterator elementVertex = elementVertices2Clean.begin();
         elementVertex != elementVertices2Clean.end(); ++elementVertex) {
...
void CWellPathCenterPointSet::populatePointSet(const std::vector<const CFormationBase *> &formations,
                                               CModelBase & /*modelBase*/) {
  ...
  for (TWellPathSectionsVertices::iterator wellPathSectionVertices =
       ...
       wellPathSectionVertices != wellPathSectionsVertices.wellPathSectionsVertices().end();
       ++wellPathSectionVertices) {
    centerVertices.push_back(retrieveCenterVertex(*wellPathSectionVertices));
  }

  std::vector<TElementVertex>::const_iterator centerVertex = centerVertices.begin();

  {
    geo::CElementPoint elementPoint =
        (*centerVertex)->element().IndexingElementSet()->Point((*centerVertex)->vertexIndices()[0]);
  ...
  for (; centerVertex != centerVertices.end(); ++centerVertex) {
    geo::CElementPoint elementPoint =
        (*centerVertex)->element().IndexingElementSet()->Point((*centerVertex)->vertexIndices()[1]);
