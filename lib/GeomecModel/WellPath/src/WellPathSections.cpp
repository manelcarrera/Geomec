...
const TWellPathSections& CWellPathSections::wellPathSections() const
{
  return m_wellPathSections;
}
...
TWellPathSections CWellPathSections::createWellPathSections(
  const CElementFaces& elementFaces)
{
  ...
  for (TElementFaces::const_iterator
      elementFace = elementFaces.elementFaces().begin();
      elementFace != elementFaces.elementFaces().end(); ++elementFace)
  {
      if (((*elementFace)->referenceCount() == INITIAL_REFERENCE_COUNT) &&
    ((*elementFace)->otherElementFace()->referenceCount() >=
          INITIAL_REFERENCE_COUNT) && (*elementFace)->top())
      {
  ...
  if (!wellPathSection.empty())
  {
      ...
      for (TElementFaces::const_iterator
    elementFace = elementFacesInSection.begin();
