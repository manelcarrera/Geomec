...
public:
  ...
  CElementFace(int index0, int index1, int index2, int index3, const geo::IElement &element, size_t referenceCount,
               bool top);
  ...
  void otherElementFace(const QSharedPointer<CElementFace> &otherElementFace);

private:
  CElementFace(const CElementFace &rhs);
  CElementFace &operator=(CElementFace rhs);

  static TIndices createFaceIndices(int index0, int index1, int index2, int index3);
