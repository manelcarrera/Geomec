...
CDCasing::CDCasing() : m_Version(DCASING_VERSION), m_Revision(DCASING_REVISION) {}

//////////////////////////////////////////////////////////////////////

CDCasing::~CDCasing() { RemoveAll(); }
...
void CDCasing::RemoveAll() { m_points.clear(); }

//////////////////////////////////////////////////////////////////////

unsigned long CDCasing::NumberfPoints() const { return m_points.count(); }
...
void CDCasing::HandleMessage(QString str) { emit OnMessage(str); }

void CDCasing::HandleError(QString str) { emit OnError(str); }

void CDCasing::HandleWarning(QString str) { emit OnWarning(str); }
...
void CDCasing::StartAnalysis() {
  ...
  dsu->SetDianaEnv(CEnvironment::STP0, dsu->GetDianaEnv(CEnvironment::DIAPATH) + "/binseg/ap/" + DCASING_EXE);
...
void CDCasing::WriteInputFile(const QString &FileName) {
  ...
  PipeLength.SetValue(m_DCasingGeneralParameters.OuterDiameter().Value() *
                      m_DCasingGeneralParameters.PipeLengthOD().Value());

  TextStream << ConstructItem(DCASING_INPUT_PIPE_LENGTH, PipeLength.Value(), PipeLength.UnitName()) << '\n';

  TextStream << ConstructItem(DCASING_INPUT_PIPE_STEEL_OUTER, m_DCasingGeneralParameters.OuterDiameter().Value(),
                              m_DCasingGeneralParameters.OuterDiameter().UnitName())
             << '\n';
  TextStream << ConstructItem(DCASING_INPUT_PIPE_STEEL_THICKNESS, m_DCasingGeneralParameters.WallThickness().Value(),
                              m_DCasingGeneralParameters.WallThickness().UnitName())
             << '\n';
  TextStream << ConstructItem(DCASING_INPUT_PIPE_STEEL_JOINT_SPACING, m_DCasingGeneralParameters.JointSpacing().Value(),
                              m_DCasingGeneralParameters.JointSpacing().UnitName())
             << '\n';
  TextStream << ConstructItem(DCASING_INPUT_MIN_REQ_DRIFT_DIAM, m_DCasingGeneralParameters.RequiredDrifting().Value(),
                              m_DCasingGeneralParameters.RequiredDrifting().UnitName())
             << '\n';
  TextStream << ConstructItem(DCASING_INPUT_DRIFTING_DIAMETER, m_DCasingGeneralParameters.DriftingDiameter().Value(),
                              m_DCasingGeneralParameters.DriftingDiameter().UnitName())
             << '\n';
  TextStream << ConstructItem(DCASING_INPUT_OUTER_INNER_RADIUS_RATIO,
                              m_DCasingGeneralParameters.ModelOutDiamBoreDiamRatio().Value())
             << "\n\n";

  // write the MATERIAL Block
  TextStream << DCASING_INPUT_MATERIAL_HEADING << '\n';
  TextStream << ConstructItem(DCASING_INPUT_YIELD_STRAIN_PIPE_STEEL,
                              m_DCasingGeneralParameters.SteelYieldStrain().Value())
             << '\n';
  TextStream << ConstructItem(DCASING_INPUT_POISSON_RATIO, m_DCasingGeneralParameters.SteelPoissonsRatio().Value())
             << '\n';
  TextStream << ConstructItem(DCASING_INPUT_LINEAR_COEFF_OF_PIPE,
                              m_DCasingGeneralParameters.LinearCoeffStressStrain().Value())
             << '\n';
  TextStream << ConstructItem(DCASING_INPUT_POWER_COEFF_OF_PIPE,
                              m_DCasingGeneralParameters.PowerCoeffStressStrain().Value())
             << '\n';
  TextStream << ConstructItem(DCASING_INPUT_HARDENING_COEFF_OF_PIPE,
                              m_DCasingGeneralParameters.HardeningCoeffStressStrain().Value())
             << '\n';
  TextStream << ConstructItem(DCASING_INPUT_STEEL_PIPE_YOUNGS_MODULUS,
                              m_DCasingGeneralParameters.SteelPipeYoungsModulus().Value(),
                              m_DCasingGeneralParameters.SteelPipeYoungsModulus().UnitName(CDoubleQuantity::SI_UNIT))
             << '\n';
  TextStream << ConstructItem(DCASING_INPUT_STEEL_PIPE_FRICTION_COEF,
                              m_DCasingGeneralParameters.PipeFrictionCoeff().Value())
             << '\n';

  TextStream << ConstructItem(DCASING_INPUT_DEFORMATION_THEORY, m_DCasingGeneralParameters.DeformationTheory()) << '\n';
  TextStream << ConstructItem(DCASING_INPUT_CEMENT_YOUNGS_MODULUS,
                              m_DCasingGeneralParameters.CementYoungsModulus().Value(),
                              m_DCasingGeneralParameters.CementYoungsModulus().UnitName())
             << '\n';
  TextStream << ConstructItem(DCASING_INPUT_POISSON_RATIO_OF_CEMENT,
                              m_DCasingGeneralParameters.CementPoissonRatio().Value())
             << "\n\n";

  // write the LOADING Block
  TextStream << DCASING_INPUT_LOADING_HEADING << '\n';
  TextStream << ConstructItem(DCASING_INPUT_MAX_AXIAL_COMPR_STRAIN,
                              m_DCasingGeneralParameters.MaxAxialCompStrain().Value())
             << '\n';
  ...
  TextStream << QString("%1").arg(DCASING_INPUT_WELL_POINT) << QString("%1").arg(DCASING_INPUT_SUP_MOD, 13 - OFFSET)
             << QString("%1").arg(DCASING_INPUT_SUP_POIS, 11) << QString("%1").arg(DCASING_INPUT_PRESSURE_IN, 14)
             << QString("%1").arg(DCASING_INPUT_PRESSURE_EX, 15) << QString("%1").arg(DCASING_INPUT_EPS_COM, 11)
             << '\n';
  ...
  for (int counter = 0; counter < m_points.size(); ++counter) {
    ...
    //		TextStream<<QString("%1").arg(QString("%1").arg(m_points[counter]->ElasticSupportYoungsMod().Value(),0,'E',5)
    //+ QString(" %1").arg(m_points[counter]->ElasticSupportYoungsMod().UnitName().begin()),-20);
    //		TextStream<<QString("%1").arg(QString("%1").arg( m_points[counter] ->ElasticSupportPoissonRatio().Value(), 0,
    //'g', 5 ),-20);
    //		TextStream<<QString("%1").arg(QString("%1").arg(m_points[counter]->InternalPressure().Value(),0,'E',5) +
    // QString(" %1").arg(m_points[counter]->InternalPressure().UnitName().begin()),-20);
    //		TextStream<<QString("%1").arg(QString("%1").arg(m_points[counter]->ExternalPressure().Value(),0,'E',5) +
    // QString(" %1").arg(m_points[counter]->ExternalPressure().UnitName().begin()),-20)<<'\n';
...
CDCasingGeneralParameters &CDCasing::GetGeneralParameters() { return m_DCasingGeneralParameters; }
...
void CDCasing::ReadSupportedValues(QFile &varFile) {
  ...
  while (true) {
    while (ReadString.indexOf(RESULTHEADERDESCRIPTION, 0, Qt::CaseSensitive) < 0 && !TextStream.atEnd() &&
           ReadString.indexOf(HEADERDESCRIPTION, 0, Qt::CaseSensitive) < 0)
      ReadString = TextStream.readLine();
    ...
    for (int counter = 0; counter < m_points.size(); ++counter) {
      ...
      m_points[counter]->GetCDCasingPointResult().GetDCasingPointSupportedResultMap()[ResultType] =
          varDCasingPointSupportedResult;
...
void CDCasing::ReadUnsupportedValues(QFile &varFile) {
  ...
  while (true) {
    ...
    for (int counter = 0; counter < m_points.size(); ++counter) {
      ...
      m_points[counter]->GetCDCasingPointResult().GetDCasingPointUnsupportedResultMap().insert(
          ResultType, varDCasingPointUnsupportedResult);
