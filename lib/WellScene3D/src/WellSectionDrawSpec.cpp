...
CWellSectionDrawSpec::CWellSectionDrawSpec(CWellSceneInterMed& WellSceneInterMed, well::IWellSection& WellSection,CWellSectionDrawSpec::TYPE  Type)
:CWellDrawSpecBase(WellSceneInterMed),m_pWellSection(&WellSection)
{
  ...
  connect(m_pWellSection,SIGNAL(OnDestroy(const well::IWellSection&)),this,SLOT(OnSectionDestroyed(const well::IWellSection&)));
  m_Type=Type;
  m_GradientType = TMD;
  CreateScene();
...
{
public:
  CWellLine(well::CWellPoint* p1, well::CWellPoint* p2,CWellLine* prev =0)
    :m_p1(p1),m_p2(p2),m_prev(prev){}
  ~CWellLine() { delete m_p1,delete m_p2;}

  double GetTotalLength() const
  {
    if(m_prev)	return m_prev->GetTotalLength() + Length();
  ...
  double GetLength(const geo::IPoint& point) const
  {
    ...
    if(m_prev)	return m_prev->GetTotalLength() + l;
...
CWellSectionDrawSpec::TYPE CWellSectionDrawSpec::GetType() const
{
  return m_Type;
}
...
void CWellSectionDrawSpec::SetArrowDistanceLevel(int level)
{
  m_ArrowDistanceLevel=level;
}

int CWellSectionDrawSpec::GetArrowDistanceLevel()
{
  return m_ArrowDistanceLevel;
}

void CWellSectionDrawSpec::SetArrowLineAngle(double lineAngle)
{
  m_ArrowLineAngle=lineAngle;
}
...
well::CWellPoint* CWellSectionDrawSpec::ProcessNewPointSelection(std::vector<const geo::IObject*>& vec, const geo::CLine& selection_line )
{
