...
CWellPointDrawSpec::CWellPointDrawSpec(CWellSceneInterMed &WellSceneInterMed, well::CWellPoint &WellPoint,
                                       CWellPointDrawSpec::TYPE type)
    : CWellDrawSpecBase(WellSceneInterMed), m_pWellPoint(&WellPoint) {

  assert(&m_pWellPoint->WellPath() == &InterMed().WellPath());

  connect(m_pWellPoint, SIGNAL(OnDestroy(const well::CWellPoint &)), this,
...
CWellPointDrawSpec::~CWellPointDrawSpec() { DeleteAllDisplayList(); }
...
void CWellPointDrawSpec::OnTMDChanged() { CreateScene(); }

void CWellPointDrawSpec::SetArrowDirection(const geo::CVector &dir) { m_ArrowDirection = dir; }
