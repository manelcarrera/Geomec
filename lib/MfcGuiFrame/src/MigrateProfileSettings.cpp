...
const CString SUCCESSFUL_MIGRATION =
  "Succesfully migrated the user settings from %s to %s";

} // anonymous namespace

void CMigrateProfileSettings::migrateProfileSettings(
	const LPCTSTR& pszRegistryKey, 
...
bool CMigrateProfileSettings::migrateProfileSettings(
	const CString& strOldProductName, 
	const LPCTSTR& pszRegistryKey,
	const LPCTSTR& pszProfileName)
{
  ...
  if( res == ERROR_SUCCESS )
  {
	...
	res = RegCreateKeyEx( hSoftKey, pszRegistryKey, 0, REG_NONE, REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, NULL,	&hCompanyKey, &dw ); 
	if( res == ERROR_SUCCESS )
    {
...
bool CMigrateProfileSettings::CopyRegistrySubTree(
	HKEY hSourceKey, 
	const CString& strSourceSubKey,
	HKEY hTargetKey,
	const CString& strTargetSubKey)
{
  ...
  if( res == ERROR_SUCCESS )
  {
	...
	res = RegCreateKeyEx(hTargetKey, strTargetSubKey, 0, REG_NONE, REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, NULL, &hTargetSubKey, &dw );
    if( res == ERROR_SUCCESS )
    {
