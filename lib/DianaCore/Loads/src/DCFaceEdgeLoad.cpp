...
CFaceEdgeLoad::CFaceEdgeLoad(CLoadCase &loadcase, double size, const geo::IVector &vecDirection, const geo::IFace &face,
                             int iEdge)
...
CFaceEdgeLoad::CFaceEdgeLoad(CLoadCase &loadcase, const std::vector<double> &vcValues, const geo::IVector &vecDirection,
                             const geo::IFace &face, int iEdge)
  ...
  for (size_t i = 0; i < vcValues.size(); i++)
    m_vcVectors[i] = geo::CVector(vecDirection * vcValues[i]);
}

// different vectors for each node
CFaceEdgeLoad::CFaceEdgeLoad(CLoadCase &loadcase, const std::vector<geo::CVector> &vcVectors, const geo::IFace &face,
                             int iEdge)
    : IDirectedLoad(loadcase, 0, geo::CVector(0, 0, 0)), m_Face(face), m_iEdgeIndex(iEdge),
      m_vcVectors(vcVectors.size()) {
  // only works for triangles and quadrilaterals
  assert(face.NrOfPoints() == 3 || face.NrOfPoints() == 4);
  assert(vcVectors.size() == 2);
  assert(iEdge >= 0 && iEdge < face.NrOfLines());
  for (size_t i = 0; i < vcVectors.size(); i++)
    m_vcVectors[i] = vcVectors[i];
}
...
bool CFaceEdgeLoad::WriteFilos() const {
  ...
  for (size_t i = 0; i < m_vcVectors.size(); i++) {
    ...
    if (fabs(vcValues[i]) > 0)
      nonzero = true;
  }
  if (nonzero)
    WriteFilos(vcValues, geo::CVector::Xaxis);
  ...
  if (nonzero)
    WriteFilos(vcValues, geo::CVector::Yaxis);
  ...
  if (nonzero)
    WriteFilos(vcValues, geo::CVector::Zaxis);

  return true;
}

const geo::IFace &CFaceEdgeLoad::Face() const { return m_Face; }

int CFaceEdgeLoad::EdgeIndex() const { return m_iEdgeIndex; }
