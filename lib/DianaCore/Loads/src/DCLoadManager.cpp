...
CLoadManager::CLoadManager(const geo::IMesh &mesh, IDianaRunner &runner) : m_Mesh(mesh), m_Runner(runner) {}

CLoadManager::~CLoadManager() { Clear(); }

void CLoadManager::Clear() {
  for (std::set<ILoad *>::iterator it = m_stLoads.begin(); it != m_stLoads.end(); it++)
    delete *it;
  m_stLoads.clear();
...
int CLoadManager::LoadCaseSize() const { return m_vcLoadCases.size(); }

const CLoadCase &CLoadManager::LoadCase(int nCase) const { return *m_vcLoadCases[nCase]; }

CLoadCase &CLoadManager::LoadCase(int nCase) { return *m_vcLoadCases[nCase]; }
...
int CLoadManager::BoundaCaseSize() const { return m_vcBoundaCases.size(); }

const CBoundaCase &CLoadManager::BoundaCase(int nCase) const { return *m_vcBoundaCases[nCase]; }

CBoundaCase &CLoadManager::BoundaCase(int nCase) { return *m_vcBoundaCases[nCase]; }
...
int CLoadManager::CaseSize() const { return LoadCaseSize() + BoundaCaseSize(); }
...
int CLoadManager::LoadCombinationSize() const { return m_vcLoadCombinations.size(); }

const CLoadCombination &CLoadManager::LoadCombination(int nCombination) const {
  return *m_vcLoadCombinations[nCombination];
}

CLoadCombination &CLoadManager::LoadCombination(int nCombination) { return *m_vcLoadCombinations[nCombination]; }
...
bool CLoadManager::InsertLoad(ILoad &load) { return m_stLoads.insert(&load).second; }
...
bool CLoadManager::WriteFilos() const {
  ...
  for (i = 0; i < CaseSize(); i++)
    if (!Case(i).WriteFilos())
      return false;

  for (i = 0; i < LoadCombinationSize(); i++)
    if (!LoadCombination(i).WriteFilos())
      return false;

  return true;
}

const geo::IMesh &CLoadManager::Mesh() const { return m_Mesh; }

const IDianaRunner &CLoadManager::Runner() const { return m_Runner; }

IDianaRunner &CLoadManager::Runner() { return m_Runner; }
