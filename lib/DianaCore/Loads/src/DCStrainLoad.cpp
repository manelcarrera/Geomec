...
CStrainLoad::CStrainLoad(CLoadCase &loadcase, const CTensor &values, const geo::IElement &element, bool writePrinciples)
    : IStructuralLoad(loadcase, 0), m_Element(element), m_WritePrinciples(writePrinciples) {
  m_vcTensors.push_back(values);
}

CStrainLoad::CStrainLoad(CLoadCase &loadcase, const std::vector<CTensor> &vcValues, const geo::IElement &element,
...
bool CStrainLoad::WriteFilos() const {
  ...
  if (m_WritePrinciples) // Principle values are in global coordinate system
  {
    ...
    ftn_double_t *pStrain =
        (ftn_double_t *)DiMalloc(3 * m_vcTensors.size() * sizeof(ftn_double_t), "CStrainLoad::WriteFilos");
  ...
  } else // Diana's full tensor is in local coordinate system
  {
    ...
    ftn_double_t *pTensor =
        (ftn_double_t *)DiMalloc(6 * m_vcTensors.size() * sizeof(ftn_double_t), "CStrainLoad::WriteFilos");
...
const geo::IElement &CStrainLoad::Element() const { return m_Element; }
