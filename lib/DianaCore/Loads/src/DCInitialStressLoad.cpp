...
CInitialStressLoad::CInitialStressLoad(CLoadCase &loadcase, const std::vector<CTensor> &vcValues, const geo::IElement &element)
:	IStructuralLoad(loadcase, 0), m_element(element), m_pvcTensors(0), m_pvcVectors(0)
{
	assert(vcValues.size() == element.NrOfPoints());
	m_pvcTensors = new std::vector<CTensor>(vcValues.size());
	for(size_t i = 0; i < vcValues.size(); i++) (*m_pvcTensors)[i] = vcValues[i];
}

// vector of vector values for each node (vcValues.size() must equal to body.NrOfPoints())
CInitialStressLoad::CInitialStressLoad(CLoadCase &loadcase, const std::vector<geo::CVector> &vcValues, const geo::IElement &element)
:	IStructuralLoad(loadcase, 0), m_element(element), m_pvcTensors(0), m_pvcVectors(0)
{
	m_pvcVectors = new std::vector<geo::CVector>(vcValues.size());
	for(size_t i = 0; i < vcValues.size(); i++) (*m_pvcVectors)[i] = vcValues[i];
}
...
bool CInitialStressLoad::WriteFilos() const
{
    ...
    int i;
		...
		assert((index == 3 * ValueSize() && (!dynamic_cast<const geo::CLineInterface *> (&Element()))) || (index == 2 * ValueSize()));
		PutItemLength("PRESTR", pPrestr, index);
	}
...
{
	if(m_pvcTensors != 0) return m_pvcTensors->size();
	else return m_pvcVectors->size();
}
...
const geo::IElement &CInitialStressLoad::Element() const
{
	return m_element;
}
