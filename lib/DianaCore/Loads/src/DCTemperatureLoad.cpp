...
CTemperatureLoad::CTemperatureLoad(CLoadCase &loadcase, const std::vector<double> &vcValues,
                                   const geo::IElement &element)
    : IStructuralLoad(loadcase, vcValues), m_Element(element) {}

CTemperatureLoad::~CTemperatureLoad() {}

const geo::IElement &CTemperatureLoad::Element() const { return m_Element; }
