...
ITimeTable::ITimeTable(ICase &icase) : m_case(icase) { icase.AddTimeTable(*this); }

ITimeTable::~ITimeTable() {}

void ITimeTable::Insert(double dTime, double dFactor) { m_mpTable.insert(TTimeFactorMap::value_type(dTime, dFactor)); }

const ICase &ITimeTable::Case() const { return m_case; }
