...
IDianaRunner::IDianaRunner(const geo::IMesh &mesh)
    : m_pTempTable(0), m_pConcenTable(0), m_pFixedPotential(0), m_bCalculationResultExpectOKMessage(false),
      m_bCanceled(false), m_nLicenseRetry(0), m_bHaveResults(false), m_bCalculationResult(false),
      m_LoadManager(mesh, *this), m_pAnalysisStatus(0), m_FF_(nullptr), m_progress(nullptr) {
  printer->info("new >>");
...
const geo::IMesh &IDianaRunner::Mesh() const { return m_LoadManager.Mesh(); }

const std::string &IDianaRunner::FFDIR() const { return m_FFDIR; }
...
bool IDianaRunner::CreateSubdir(const std::string &rootdir) {
  ...
  if (mkdir(copy, S_IFDIR | S_IRWXU) != 0)
    return false;
#endif // defined(WIN32) || defined(WIN64)
...
void IDianaRunner::InitializeFile() { InitializeFilos(); }
...
IDianaRunner::TDirectionMap *IDianaRunner::DirectionMap() { return m_pmpDirections; }

void IDianaRunner::DirectionMap(TDirectionMap *pMap) { m_pmpDirections = pMap; }

IAnalysisStatus *IDianaRunner::AnalysisStatus() { return m_pAnalysisStatus; }

void IDianaRunner::AnalysisStatus(IAnalysisStatus *pAnalysisStatus) { m_pAnalysisStatus = pAnalysisStatus; }

int IDianaRunner::MeshProgressSize() const { return Mesh().ElementSize() + Mesh().PointSize(); }

int IDianaRunner::GroupsProgressSize() const {
  int sz = 0;

  int i;
  for (i = 0; i < Mesh().ElementGroupSize(); i++)
    sz += Mesh().ElementGroup(i).ElementSize();
  for (i = 0; i < Mesh().NodeGroupSize(); i++)
    sz += Mesh().NodeGroup(i).NodeSize();

  return sz;
}

int IDianaRunner::SupportProgressSize() const { return Mesh().SupportSize(); }

int IDianaRunner::TyingsProgressSize() const { return Mesh().TyingSize(); }

int IDianaRunner::LoadsProgressSize() const { return 0; }

int IDianaRunner::CommandsProgressSize() const { return 1; }
...
bool IDianaRunner::WriteInputFiles(std::string & /*comfilename*/, std::string & /*datfilename*/) const { return false; }
...
bool IDianaRunner::WriteDatabase(const std::string &title, IRetrieveDianaFileNames &retrieveDianaFileNames) {
...
void IDianaRunner::OnError(eError id, const std::string &msg2) {
  ...
  case Running:
    msg = "running";
    break;
  case PreProcessWriteInputFiles:
    msg = "pre process write unput files";
    break;
  case PostProcessWriteInputFiles:
    msg = "post process write unput files";
    break;
  case EndAnalysis:
    msg = "end analysis";
    break;
  case CreateSubDir:
    msg = "create subdir";
    break; // called when no subdirectory could be created as working directory (temp dir read-only?)

  default:
    msg = "error type not defined";
    break;
  }
...
bool IDianaRunner::create_FF() {
  ...
  // Below we give a final status message that should be visible just long enough for the user to read, until we hide
  // the window
...
bool IDianaRunner::OnApplyLoads(CLoadManager & /*manager*/) { return true; }
...
bool IDianaRunner::OnWriteLoads() { return m_LoadManager.WriteFilos(); }
...
bool IDianaRunner::OnEndWriteCommands(const std::string & /*title*/) { return EndWriteCommands(); }

bool IDianaRunner::OnRunAnalysis(IDianaXWrapper * /*dianaXWrapper*/, bool /* bSilent */) { return true; }

bool IDianaRunner::OnEndAnalysis(const QString & /*getPathName*/, ISaveModel & /*saveModel*/) { return true; }
...
int IDianaRunner::AddDirection(const geo::IVector &dir) {
  ...
  std::pair<TDirectionMap::iterator, bool> prInsert = m_pmpDirections->insert(TDirectionMap::value_type(unitdir, 0));
...
const std::string &IDianaRunner::FF() const { return m_FF; }
...
bool IDianaRunner::CalculationResultExpectOKMessage() const { return m_bCalculationResultExpectOKMessage; }

void IDianaRunner::CalculationResultExpectOKMessage(bool flag) { m_bCalculationResultExpectOKMessage = flag; }
...
void IDianaRunner::GetResultInfo(bool &bHaveResults, bool &bCalculationResult, bool bSave) {
  ...
  if (bSave) {
    GetResultInfo(FFDIR().c_str(), FF().c_str(), bHaveResults, bCalculationResult, CalculationResultExpectOKMessage());
...
void IDianaRunner::GetResultInfo(const char *workingdir, const char *filosfile, bool &bHaveResults,
                                 bool &bCalculationResult, bool expectOkMessage) {
  ...
  printer->info("get results info : FFDIR:%s : FF:%s -> have_results:%d : calculation_results:%d", workingdir,
                filosfile, bHaveResults, bCalculationResult);
  // printer->info("get results info : have_results:%d : calculation_results:%d", bHaveResults, bCalculationResult);
}

void IDianaRunner::Cancel() { m_bCanceled = true; }

bool IDianaRunner::Canceled() const { return m_bCanceled; }

void IDianaRunner::LicenseRetry(int nLicenseRetry) { m_nLicenseRetry = nLicenseRetry; }

int IDianaRunner::LicenseRetry() const { return m_nLicenseRetry; }
...
bool IDianaRunner::PreprocessWriteInputFiles(std::string &sComfileName, std::string &sDatfileName) {
  ...
  if (!SaveComFile(sComfileName.c_str(), sInitialComFileText.empty() ? 0 : sInitialComFileText.c_str(),
                   bExportComFileWithDefaults ? 1 : 0))
    return false;
...
bool IDianaRunner::OnEndPreprocessWriteInputFiles() { return true; }
...
bool IDianaRunner::PostprocessWriteInputFiles(const std::string &sComfileName, const std::string &sDatfileName) {
  ...
  if (strcmp(sDatfileName.c_str(), m_sDatFileName.c_str()) == 0)
    return true;
...
bool IDianaRunner::OnEndPostprocessWriteInputFiles(const std::string & /*sComFileName*/,
...
*/
const IElementData *IDianaRunner::ElementData(const geo::IElement & /*element*/) const { return 0; }

/*!
 return element geometry for the given element
 return 0 if no geometry attached
*/
const IElementData *IDianaRunner::ElementGeometry(const geo::IElement & /*element*/) const { return 0; }
...
// For a temper table
ITemperatureTable *IDianaRunner::OnCreateTemperatureTable() const { return 0; }
...
ITemperatureTable *IDianaRunner::TemperatureTable() { return m_pTempTable; }
...
IConcentrationTable *IDianaRunner::OnCreateConcentrationTable() const { return 0; }
...
IConcentrationTable *IDianaRunner::ConcentrationTable() { return m_pConcenTable; }
...
std::string IDianaRunner::UserDefinedLocation() const { return std::string(); }

std::string IDianaRunner::InitialComFileText() const { return "*FILOS\n INITIA\n*INPUT\n"; }

bool IDianaRunner::ExportCommandFileWithDefaults() const { return false; }

void IDianaRunner::executeCommandInGeomec()
    const // overrides are for casing (GEOTYP=UPDATE) which should not be combined with the HX24L1 setting
{
...
IDianaRunner::CCurDirSaver::CCurDirSaver() { m_strOldCwd = GetCurrentDir(); }
