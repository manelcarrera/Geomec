...
void IAnalysisLogger::Empty()
{
	m_vcStrings.clear();
}

void IAnalysisLogger::Silent(bool flag)
{
  m_bSilent = flag;
}
...
bool IAnalysisLogger::IsEmpty() const
{
	return m_vcStrings.empty();
}

long IAnalysisLogger::LineSize() const
{
	return (long)m_vcStrings.size();
}

const std::string &IAnalysisLogger::Line(int nIndex) const
{
	return m_vcStrings[nIndex].first;
}

bool IAnalysisLogger::Verbose(int nIndex) const
{
  return m_vcStrings[nIndex].second;
}
