...
IAnalysisStatus::IAnalysisStatus()
  : m_nPhase(0)
  , m_bInitializationConvergence(true) // we're optimistic
{
...
int IAnalysisStatus::PhaseNumber() const
{
  return m_nPhase;
}

bool IAnalysisStatus::InitializationConvergence() const
{
  return m_bInitializationConvergence;
}

size_t IAnalysisStatus::TimeSteps() const
{
  return m_vcTimeSteps.size();
}
...
bool IAnalysisStatus::Converged() const
{
  return InitializationConvergence() && TimeSteps() == TimeStepsConverged();
}
...
size_t IAnalysisStatusContainer::Phases() const
{
  return m_vcAnalysisStatus.size();
}
...
void IAnalysisStatusContainer::NewPhase()
{
  m_vcAnalysisStatus.push_back(new IAnalysisStatus());
}
