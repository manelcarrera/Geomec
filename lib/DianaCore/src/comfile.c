...
static void FreeSubDirInfoStruct(DiSubDirInfo_t *pStruct) {
  ...
  if (pStruct->sName)
    DiFree(pStruct->sName, "FreeSubDirInfoStruct");
  if (pStruct->sValue) {
...
static DiSubDirInfo_t *AddChild(DiSubDirInfo_t *pRoot, const char *Name) {
  ...
  pRoot->nChildren++;
  pRoot->pChildren = DiRealloc(pRoot->pChildren, 0, pRoot->nChildren * sizeof(DiSubDirInfo_t *), "GetSubDirInfo");
  pRoot->pChildren[pRoot->nChildren - 1] = DiMalloc(sizeof(DiSubDirInfo_t), "GetSubDirInfo");
    ...
    if (strncmpcb(ui->Uitype, "gui", 3)) {
      ...
      case FL_INQ_TYPE_DIR:
        ...
        if (InActive) {
          ...
          ChangeDir(Name);
          if (iRange)
            GetRangeItems(pChild, ui->Range);
          if (GetSubDirInfo(pChild, iWriteDefaultValues))
            pRoot->iBlockFormat = 1;
          PopDir();
        }
        break;
      ...
      case FL_INQ_TYPE_DIR + FL_INQ_TYPE_DIMENSIONED:
        ...
        ChangeDir(Name);
        {
          ...
          idx = InquireIndex(".");
          do {
            if (IsActive(".") == FTN_FALSE)
              continue;
            ActiveFound = 1;
            pChild = AddChild(pRoot, iValueOnly ? NULL : Name);
            if (pRoot->nChildren > 1)
              pRoot->iBlockFormat = 1;
            pChild->iIsDir = 1;
      ...
      case FL_INQ_TYPE_STRING + FL_INQ_TYPE_DIMENSIONED: {
        ...
        pChild->nValues = nDim;
        pChild->sValue = DiMalloc(nDim * sizeof(char *), tb);
        for (j = 0; j < nDim; j++)
          pChild->sValue[j] = pData[j];
        DiFree(pData, tb);
      } break;
      ...
      case FL_INQ_TYPE_REAL + FL_INQ_TYPE_DIMENSIONED: {
        ...
        pChild->sValue = DiMalloc(length * sizeof(char *), tb);
        fValues = DiMalloc(length * sizeof(ftn_real_t), tb);
      ...
      case FL_INQ_TYPE_LOG:
        ...
        if ((!iDefault && !iValue) || (iDefault && iValue == ui->Default.u.idef[0]))
          pChild->iIsDefault = 1;
        if (!pChild->iIsDefault)
          SetAllParentsDefault(pChild, 0);
        if (!iValue) {
      ...
      if (NumberOfNonDefaultChildren(pRoot) > 1)
        pRoot->iBlockFormat = 1;
    }
  }
...
static void WriteCommandsToFile(FILE *fp, DiSubDirInfo_t *pRoot, int iIndent) {
  ...
  if (!NumberOfNonDefaultChildren(pRoot))
    return;
...
extern int SaveComFile(const char *PathName, const char *InitialText, int iWriteDefaultValues) {
  ...
  if (!XistIndexed("/CMNDS/SEGINF/", NULL))
    return 0;
  PushDir();
