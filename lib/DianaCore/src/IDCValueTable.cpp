...
int IValueTable::Size() const
{
  return (int)m_vcTimes.size();
}
...
void IValueTable::CleanUp(bool cleanup)
{
  m_bCleanUp = cleanup;
}

bool IValueTable::CompressTable() const
{
  return true;
}
...
IValueTable::CTimePoint::CTimePoint(double time)
: m_time(time)
{
}

const double& IValueTable::CTimePoint::Time() const
{
  return m_time;
}
