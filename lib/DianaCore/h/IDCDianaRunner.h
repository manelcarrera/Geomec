...
class IAnalysisStatusContainer; // only needed by run controller, inclueded to use the struct in both controller and
                                // runner
...
protected:
  enum eError {
    ...
    CopyMaterials,
    Running,
    PreProcessWriteInputFiles,
    PostProcessWriteInputFiles,
    EndAnalysis,
    CreateSubDir
  };
...
public:
  ...
  static void GetResultInfo(const char *workingdir, const char *filosfile, bool &bHaveResults, bool &bCalculationResult,
                            bool expectOkMessage = true);
...
public:
  ...
  public:
    double operator()() {
      return 1.0e-4; // Diana required a higher precision for a while (1E-10) but if all is well, this has been set back
                     // to 1E-4
    }
  };
...
protected:
  ...
  virtual bool UserDefinedFilosFile(const QString & /*getPathName*/, std::string & /*strFilosFile*/,
...
private:
  ...
  bool WriteDatabase(const std::string &title, IRetrieveDianaFileNames &retrieveDianaFileNames);
