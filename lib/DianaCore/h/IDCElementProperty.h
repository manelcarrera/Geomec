...
public:
  ...
  public:
    bool operator()(const IElementProperty *mat1, const IElementProperty *mat2) const { return *mat1 < *mat2; }
  };
  ...
  // Values expects a pointer within allocated memory of size stride * Size, to which the values should be written, all
  // in one call
  //   typical use: for all elements 0 <= i < #Elts, get values 0 <= j < Size, call
  //     element[i].IElementProperty.WriteFilosParamValues(..., values + Size * i, 1) // [ Elt 0's value 0 ... Size, Elt
  //     1's value 0 ... Size, etc ]
  //   or
  //     element[i].IElementProperty.WriteFilosParamValues(..., values + i, #Elts) // [ Elt 0 ... #Elts' value 0, Elt 0
  //     ... #Elts' value 1, etc ]
  ...
  //   b. some values, like matrices, are calculated all at once, and it would otherwise be cumbersome to spread it out
  //   over multiple calls c. stride == #Elts is expected to be faster for additional work in the calling routine
