...
template <class OBSERVED_TYPE, class DELEGATE_TYPE, class CHILD_TYPE,
  class CHILD_OBS_TYPE, class UNDEFINEDICON_PROVIDER, BOOL RENAME,
  enum REMOVE_TYPE REMOVE>
  class CStateBranch_Delegate : public CNodeObserver_Delegate <OBSERVED_TYPE,
    DELEGATE_TYPE, CHILD_TYPE, CHILD_OBS_TYPE, RENAME, REMOVE>
{
public:
  CStateBranch_Delegate(OBSERVED_TYPE& node,
                        CTreeNode& parent,
                        const BOOL rename,
  ...
  virtual void ToggleState()
  {
    ...
    if (AllSameState())
    {
      ...
      for (TChildren::iterator it = children.begin(); it != children.end();
        it++)
      {
        if ((*it)->StateIcon() != 0)
        {
