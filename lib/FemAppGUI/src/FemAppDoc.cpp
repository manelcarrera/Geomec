...
void CFemAppDoc::AssertValid() const
{
	CDocument::AssertValid();
}

//##ModelId=3BC55D6300B5
void CFemAppDoc::Dump(CDumpContext& dc) const
{
	CDocument::Dump(dc);
}
#endif //_DEBUG
...
void CFemAppDoc::OnSelect(CGraphNode *pNode)
{
	...
	if(pNode != pPreviousNode)
	{
		...
		if(pNode)
    {
      CGraphNode_Delegate* pNode_Delegate =
        dynamic_cast <CGraphNode_Delegate*> (pNode->getDelegate());
...
void CFemAppDoc::CurrentScene(ISceneWrapper *pScene)
{
    if ((!m_pCurrent || m_pCurrent != pScene) && !m_bStickToScene)
    {
      ...
      // we need to update views first, as Activate might visit them and otherwise they may follow the pointer we just deleted
      UpdateAllViews(0, NEW_CURRENT_SCENE, (CObject*)(m_pCurrent));
      if (m_pCurrent)
        m_pCurrent->Activate();
    }
...
bool CFemAppDoc::StickToScene() const
{
  return m_bStickToScene;
}

void CFemAppDoc::StickToScene(bool bStick)
{
  m_bStickToScene = bStick;
}
...
void CFemAppDoc::OnZoomWindow()
{
	UpdateAllViews(0,ON_ZOOMWINDOW_BUTTON);
}

CUnitNode& CFemAppDoc::UnitNode()
{
	return m_unit;
}

const CUnitNode& CFemAppDoc::UnitNode() const
{
	return m_unit;
}

/////////////////////////////////////////////////////////////////////////////
void CFemAppDoc::OnInvalidateStorage()
{
	SetModifiedFlag();
}
/*
...
CFemAppModel* CFemAppDoc::Model()
{
	return m_pModel;
}

const CFemAppModel* CFemAppDoc::Model() const
{
	return m_pModel;
}

CFemAppGUI* CFemAppDoc::GUI()
{
	return m_pGUI;
}

const CFemAppGUI* CFemAppDoc::GUI() const
{
	return m_pGUI;
}

CFemAppGUI* CFemAppDoc::retrieveMatchingGUI(CFemAppGUI* femAppGUI,
  CFemAppModel* femAppModel) const
{
...
void CFemAppDoc::OnContextMenu( ICtrlObject &ctrl_object)
{
	m_pContext = &ctrl_object;
}
...
ICtrlObject& CFemAppDoc::ContextMenuCtrlObject()
{
	return *m_pContext;
}

const ICtrlObject& CFemAppDoc::ContextMenuCtrlObject() const
{
	return *m_pContext;
}

const CGraphNode* CFemAppDoc::SelectedNode() const
{
	return m_pSelection.Ptr();
}

CGraphNode* CFemAppDoc::SelectedNode()
{
	return m_pSelection.Ptr();
}
