...
public:
  CTaskCollectValuesByGeometry(const OIDIGeometryI& g, const IValueComponentBase *pValueComponent, IQuantityDouble::UNIT unit, CScalarCollector& collector)
    : m_geometry(&g)
    , m_pValueComponent(pValueComponent)
    , m_unit(unit)
...
OIDIScalarSetI* OIDIMesh::collectValuesByGeometry(const IValueComponentBase *pValueComponent, IQuantityDouble::UNIT unit, IProgressBase & progress) const
{
...
OIDIVectorSetI* OIDIMesh::collectVectorsByGeometry(const IVectorResult::IVectorComponent* pVectorComponent, IQuantityDouble::UNIT unit, IProgressBase & progress) const
{
    std::size_t geometrySize = m_geometry.getSize();
    ...
    for (std::size_t i = 0; i < geometrySize; ++i)
    {
        ...
        if (unit == IQuantityDouble::FIELD_UNIT)
          values->add(MbVec3d(pResult->ConvertToField(v.X()), pResult->ConvertToField(v.Y()), pResult->ConvertToField(v.Z())));
        else
...
OIDITensorSetI* OIDIMesh::collectTensorsByGeometry(const IValueComponentBase* pTensorComponent, IQuantityDouble::UNIT unit, IProgressBase & progress) const
{
    std::size_t geometrySize = m_geometry.getSize();
...
OIDIScalarSetI* OIDIMeshSU::collectValuesByTopology( const IValueComponentBase *pValueComponent, IQuantityDouble::UNIT unit, IProgressBase & progress ) const
{
    if (!isFault())
        return OIDIMeshTemplate<OIDISurfaceTopologyExplicitI, OIDISurfaceCell, MiSurfaceMeshUnstructured>::collectValuesByTopology(pValueComponent, unit, progress);
    else
...
OIDIScalarSetI* OIDIMeshSU::collectValuesByGeometry( const IValueComponentBase *pValueComponent, IQuantityDouble::UNIT unit, IProgressBase & progress ) const
{
    if (!isFault())
...
//X001
OIDIScalarSetI* OIDIMeshSU::collectFaultedValuesByGeometry( const IValueComponentBase * pValueComponent, IQuantityDouble::UNIT unit, IProgressBase & progress ) const
{
...
OIDIVectorSetI* OIDIMeshSU::collectVectorsByTopology( const IVectorResult::IVectorComponent *pVectorComponent, IQuantityDouble::UNIT unit, IProgressBase & progress ) const
{
    if (!isFault())
    {
        return OIDIMeshTemplate<OIDISurfaceTopologyExplicitI, OIDISurfaceCell, MiSurfaceMeshUnstructured>::collectVectorsByTopology(pVectorComponent, unit, progress);
    }
    else
    {
...
OIDIVectorSetI* OIDIMeshSU::collectVectorsByGeometry( const IVectorResult::IVectorComponent *pVectorComponent, IQuantityDouble::UNIT unit, IProgressBase & progress ) const
{
    if (!isFault())
...
public:
  CTaskCollectFaultedVectorsByGeometry(const OIDISurfaceTopologyExplicitI& t, const IVectorResult::IVectorComponent *pVectorComponent, IQuantityDouble::UNIT unit, CInterfaceVectorCollector& collector)
    : m_topology(&t)
    , m_pVectorComponent(pVectorComponent)
    , m_unit(unit)
    , m_collector(collector)
    , m_dispatcher(0)
    , m_bSequential(false)
  ...
  void operator()(const tbb::blocked_range<size_t>& r) const
  {
    ...
    geo::IParallelInitializationCallback *callback = !m_bSequential ? const_cast<IValueComposite&>(m_pVectorComponent->Parent()).GetParallelInitializationCallback() : 0;
    ...
    for (size_t i = r.begin(); i != r.end(); ++i)
    {
      ...
      m_pVectorComponent->MapValueElementCB(*ifElt, results, CValueType::MT_NONE, m_unit, callback); // no weighting for now
    ...
    m_collector.add(record);
#ifdef DEBUG_MPKERNEL
    mp::MPOutput::instance()->write(QString("END range from %1 to %2, size is %3\n").arg(r.begin()).arg(r.end()).arg(r.size()).toStdString());
#endif
  }

};

OIDIVectorSetI* OIDIMeshSU::collectFaultedVectorsByGeometry(const IVectorResult::IVectorComponent * pVectorComponent, IQuantityDouble::UNIT unit, IProgressBase & progress) const
{
    std::size_t numTopologyCells = getTopology().getNumCells();
    std::size_t geometrySize = getGeometry().getSize();
    ...
    {
      const geo::IElementSet *pElementSet = numTopologyCells > 0 ? getTopology().getCell(0)->getParent()->IndexingElementSet() : 0;
      ...
      vectors->adjustForInterfaceElements(getTopology()); // not needed, but should lead to less allocation actions in the set
...
OIDITensorSetI* OIDIMeshSU::collectTensorsByGeometry(const IValueComponentBase* pTensorComponent, IQuantityDouble::UNIT unit, IProgressBase & progress) const
{
  return collectGeometryTensorsByTopology(pTensorComponent, unit, progress);
}

OIDIMesh::MeshType OIDIMeshSU::meshType() const
{
    return OIDIMesh::SurfaceMesh;
}
...
public:
  CTaskCollectFaultedVectorsByTopology(const OIDISurfaceTopologyExplicitI& t, const IVectorResult::IVectorComponent *pVectorComponent, IQuantityDouble::UNIT unit, CVectorCollector& collector)
    : m_topology(&t)
    , m_pVectorComponent(pVectorComponent)
    , m_unit(unit)
    , m_collector(collector)
...
OIDIVectorSetI* OIDIMeshSU::collectFaultedVectorsByTopology(const IVectorResult::IVectorComponent * pVectorComponent, IQuantityDouble::UNIT unit, IProgressBase & progress) const
{
    std::size_t numTopologyCells = getTopology().getNumCells();
    ...
    if (vm->UseLegacyCode())
    {
      ...
      for (std::size_t i = 0; i < numTopologyCells; ++i)
      {
        pVectorComponent->MapValueElement(*getTopology().getCell(i)->getInterfaceElement(), results, CValueType::MT_NONE, unit); // no weighting for now
...
OIDIScalarSetI* OIDIMeshSU::collectFaultedValuesByTopology( IProgressBase & progress, const IValueComponentBase * pValueComponent ) const
{
    std::size_t numTopologyCells = getTopology().getNumCells();
    ...
    for (std::size_t i = 0; i < numTopologyCells; ++i)
    {
        values->add(computeAverage(pValueComponent->ScalarData().ValueElement(*getTopology().getCell(i)->getInterfaceElement()), true));
        if (i % 100 == 0) progress.Step(100);
    }
...
OIDIMesh::MeshType OIDIMeshLU::meshType() const
{
    return OIDIMesh::LineMesh;
}

OIDIMesh::MeshType OIDIMeshVU::meshType() const
{
    return OIDIMesh::VolumeMesh;
}

OIDIScalarSetI* OIDIMeshVU::collectValuesByGeometry( const IValueComponentBase *pValueComponent, IQuantityDouble::UNIT unit, IProgressBase & progress ) const
{
        return collectGeometryValuesByTopology(pValueComponent, unit, progress);
}

OIDIVectorSetI* OIDIMeshVU::collectVectorsByGeometry(const IVectorResult::IVectorComponent *pVectorComponent, IQuantityDouble::UNIT unit, IProgressBase & progress ) const
{
    return collectGeometryVectorsByTopology(pVectorComponent, unit, progress);
}

OIDITensorSetI* OIDIMeshVU::collectTensorsByGeometry(const IValueComponentBase* pTensorComponent, IQuantityDouble::UNIT unit, IProgressBase & progress) const
{
  return collectGeometryTensorsByTopology(pTensorComponent, unit, progress);
}
...
bool OIDIMeshVU::computeCellMap( std::string meshName, const OIDIMeshVU * mainMesh, std::vector<int> & cellMap ) const
{
    ...
    for (size_t i = 0; i < numCells; ++i)
    {
        ...
        if (mainMeshCellId < 0) return false;
...
{
    if (!m_probe) m_probe = MiPointProbeUnstructured::getNewInstance(* this);
    return m_probe;
}

OIDIMeshVU::~OIDIMeshVU()
{
    delete m_probe;
}
...
void OIDIMeshTemplate<TopologyType, CellType, MiMeshUnstructuredType>::checkConsistency( MbVec3d & min, MbVec3d & max ) const
{
    min = DBL_MAX;
    max = -DBL_MAX;
...
template<class TopologyType, class CellType>
class CTaskCollectValuesByTopology
{
...
public:
  CTaskCollectValuesByTopology(const TopologyType& t, const IValueComponentBase *pValueComponent, IQuantityDouble::UNIT unit, CScalarCollector& collector)
    : m_topology(&t)
    , m_pValueComponent(pValueComponent)
    , m_unit(unit)
    , m_collector(collector)
  ...
  void operator()(const tbb::blocked_range<size_t>& r) const
  {
    ...
    for (size_t i = r.begin(); i != r.end(); ++i)
    {
      ...
      const IValueDataInterfaceScalar::TValueVec elementValues = m_pValueComponent->ScalarData().ValueElement(*cell->getParent(), m_unit, callback);
...
template < class TopologyType, class CellType, class MiMeshUnstructuredType >
OIDIScalarSetI* OIDIMeshTemplate<TopologyType, CellType, MiMeshUnstructuredType>::collectValuesByTopology( const IValueComponentBase *pValueComponent, IQuantityDouble::UNIT unit, IProgressBase & progress ) const
{
    ...
    if (vm->UseLegacyCode())
    {
      for (std::size_t i = 0; i < numTopologyCells; ++i)
      {
        values->add(computeAverage(pValueComponent->ScalarData().ValueElement(*m_topology.getCell(i)->getParent(), unit)));
        if (i % 100 == 0) progress.Step(100);
      }
    }
    else
    {
      const geo::IElementSet *pElementSet = numTopologyCells > 0 ? m_topology.getCell(0)->getParent()->IndexingElementSet() : 0;
...
template<class TopologyType, class CellType>
class CTaskCollectGeometryValuesByTopology
{
  const TopologyType  *m_topology;
...
public:
  CTaskCollectGeometryValuesByTopology(const TopologyType& t, const IValueComponentBase *pValueComponent, IQuantityDouble::UNIT unit, std::vector<double>& accV, std::vector<size_t>& accC, std::vector<size_t> &invVC, std::vector<tbb::spin_mutex>& locks)
    : m_topology(&t)
    , m_pValueComponent(pValueComponent)
    , m_unit(unit)
    , m_accumulatedValues(accV)
    , m_accumulationCounts(accC)
    , m_invalidValueCounts(invVC)
    , m_locks(locks)
    , m_dispatcher(0)
    , m_bSequential(false)
  {
...
template < class TopologyType, class CellType, class MiMeshUnstructuredType >
OIDIScalarSetI* OIDIMeshTemplate<TopologyType, CellType, MiMeshUnstructuredType>::collectGeometryValuesByTopology( const IValueComponentBase *pValueComponent, IQuantityDouble::UNIT unit, IProgressBase & progress ) const
{
  std::size_t numTopologyCells = m_topology.getNumCells();
  ...
  if (vm->UseLegacyCode())
  {
    ...
    {
      const CellType * cell = m_topology.getCell(i);
      const IValueDataInterfaceScalar::TValueVec elementValues = pValueComponent->ScalarData().ValueElement(*cell->getParent(), unit);
  ...
  else
  {
    ...
    CTaskCollect taskCollect(m_topology, pValueComponent, unit, accumulatedValues, accumulationCounts, invalidValueCounts, locks);
...
template < class TopologyType, class CellType, class MiMeshUnstructuredType >
OIDIVectorSetI* OIDIMeshTemplate<TopologyType, CellType, MiMeshUnstructuredType>::collectVectorsByTopology( const IVectorResult::IVectorComponent *pVectorComponent, IQuantityDouble::UNIT unit, IProgressBase & progress ) const
{
    std::size_t numTopologyCells = m_topology.getNumCells();
    ...
    for (std::size_t i = 0; i < numTopologyCells; ++i)
    {
        pVectorComponent->MapValueElement(*m_topology.getCell(i)->getParent(), results, CValueType::MT_NONE, unit); // no weighting for now
...
template<class TopologyType, class CellType>
class CTaskCollectGeometryVectorsByTopology
{
  const TopologyType  *m_topology;
...
public:
  CTaskCollectGeometryVectorsByTopology(const TopologyType& t, const IVectorResult::IVectorComponent *pVectorComponent, IQuantityDouble::UNIT unit, std::vector<geo::CVector>& accV, std::vector<size_t>& accC, std::vector<tbb::spin_mutex>& locks)
    : m_topology(&t)
    , m_pVectorComponent(pVectorComponent)
    , m_unit(unit)
    , m_accumulatedVectors(accV)
    , m_accumulationCounts(accC)
    , m_locks(locks)
    , m_dispatcher(0)
    , m_bSequential(false)
  ...
  void operator()(const tbb::blocked_range<size_t>& r) const
  {
    ...
    {
      const CellType * cell = m_topology->getCell(i);
      //const IValueDataInterfaceVector::TValueVec elementVectors = m_pVectorComponent->VectorData().ValueElement(*cell->getParent(), m_unit);
      m_pVectorComponent->MapValueElementCB(*cell->getParent(), results, CValueType::MT_NONE, m_unit, callback); // no weighting for now
...
template < class TopologyType, class CellType, class MiMeshUnstructuredType >
OIDIVectorSetI* OIDIMeshTemplate<TopologyType, CellType, MiMeshUnstructuredType>::collectGeometryVectorsByTopology( const IVectorResult::IVectorComponent *pVectorComponent, IQuantityDouble::UNIT unit, IProgressBase & progress ) const
{
    std::size_t numTopologyCells = m_topology.getNumCells();
    std::size_t geometrySize = getSize();
...
template<class TopologyType, class CellType>
class CTaskCollectTensorsByTopology
{
  const TopologyType  *m_topology;
...
public:
  CTaskCollectTensorsByTopology(const TopologyType& t, const IValueComponentBase* pTensorComponent, IQuantityDouble::UNIT unit, CTensorCollector& collector)
    : m_topology(&t)
    , m_pTensorComponent(pTensorComponent)
    , m_unit(unit)
    , m_collector(collector)
    , m_dispatcher(0)
    , m_bSequential(false)
  ...
  void operator()(const tbb::blocked_range<size_t>& r) const
  {
    ...
    for (size_t i = r.begin(); i != r.end(); ++i)
    {
      ...
      IValueDomainTensor::TValueVec tensors = m_pTensorComponent->TensorData()->ValueElement(*m_topology->getCell(i)->getParent(), m_unit, callback);
...
template < class TopologyType, class CellType, class MiMeshUnstructuredType >
OIDITensorSetI* OIDIMeshTemplate<TopologyType, CellType, MiMeshUnstructuredType>::collectTensorsByTopology(const IValueComponentBase* pTensorComponent, IQuantityDouble::UNIT unit, IProgressBase & progress) const
{
	std::size_t numTopologyCells = m_topology.getNumCells();
...
template<class TopologyType, class CellType>
class CTaskCollectGeometryTensorsByTopology
{
  const TopologyType  *m_topology;
...
public:
  CTaskCollectGeometryTensorsByTopology(const TopologyType& t, const IValueComponentBase* pTensorComponent, IQuantityDouble::UNIT unit, std::vector<CTensor>& accumulatedTensors, std::vector<size_t>& accumulationCounts, std::vector<tbb::spin_mutex>& locks)
    : m_topology(&t)
    , m_pTensorComponent(pTensorComponent)
    , m_unit(unit)
    , m_accumulatedTensors(accumulatedTensors)
    , m_accumulationCounts(accumulationCounts)
    , m_locks(locks)
    , m_dispatcher(0)
    , m_bSequential(false)
  {
  ...
  void operator()(const tbb::blocked_range<size_t>& r) const
  {
    ...
    for (size_t i = r.begin(); i != r.end(); ++i)
    {
      ...
      IValueDomainTensor::TValueVec tensors = m_pTensorComponent->TensorData()->ValueElement(*cell->getParent(), m_unit, callback);
...
template < class TopologyType, class CellType, class MiMeshUnstructuredType >
OIDITensorSetI* OIDIMeshTemplate<TopologyType, CellType, MiMeshUnstructuredType>::collectGeometryTensorsByTopology( const IValueComponentBase* pTensorComponent, IQuantityDouble::UNIT unit, IProgressBase & progress ) const
{
    std::size_t numTopologyCells = m_topology.getNumCells();
    std::size_t geometrySize = getSize();
...
bool OIDIVolumeCell::hasSameNodeIndices( const OIDIVolumeCell * cell ) const
{
    ...
    {       
        if (!cell->findNodeIndex (getNodeIndex(n))) return false;
    }
...
bool OIDIVolumeCell::findNodeIndex( size_t nodeIndex ) const
{
    for (int n = 0; n < getNumNodes(); ++n)
    {
        if (getNodeIndex(n) == nodeIndex) return true;
    }
    return false;
}
...
geo::CPtrArray<geo::IPoint> * CreateArray( const geo::IPoint &Point )
{
    ...
    // we used to need to span a surface here, drawing the point with as a triangle with some minor offsets which created ugly artifacts
    // it seems now (OIV 9.5.1) we can just use the same coordinates, creating an actual point
    // if we push less than 3 points, OIV doesn't render it -- perhaps in a future version? :/
    // (two points seems to crash OIV)
