...
template <class IElementType, class MiCellType>
class OIDICell : public MiCellType
{
protected:
...
public:
  ...
  void setParent(IElementType *parent)
  {
    m_Parent = parent;
  }
  ...
  virtual bool isPointInsideCell(const MiGeometryI &  meshGeometry, const MbVec3d &  point, std::vector< double > &  weights)  const = 0;
};
...
public:
  ...
  void setInterfaceElement(geo::IInterfaceElement *element)
  {
    m_InterfaceElt = element;
  }
...
public:
  ...
  {
    m_VectorIndex = index * NodesPerCell;
  }

  virtual size_t getNodeIndex(size_t node) const
  {
    return m_VectorIndex + node;
  }
