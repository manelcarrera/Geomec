...
// Tasks will not work on global OIDISets, but on "records". When they're finished, these records will be added to a collector
// and they can be processed afterwards to move the data to the global sets.
// While we could do this in parallel too, it should be too fast to bother with it anyway.

template<class Type>
class ICollectorRecord
{
protected:
  size_t m_offset;
...
public:
  ICollectorRecord(size_t offset, size_t size)
    : m_offset(offset)
  {
    m_values.resize(size, Type(DBL_UNDEFINED));
  }
...
template<class Type, class Container>
class CCollector : public ICollector<Type, Container>
{
public:
...
template<class Type, class Container>
class CInterfaceCollector : public ICollector<Type, Container>
{
public:
