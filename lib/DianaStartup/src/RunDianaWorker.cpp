...
RunDianaWorker::RunDianaWorker( const QString& path, int pid ) 
	: m_path( path ), m_pid( pid ), m_res( false )
{
}
...
void RunDianaWorker::process()
{
	...
	client = new GeomecBusClient( base, name ); // this_->m_qapp -> no difference passing the parent in terms of events handling -> so don't pass it
	...
	{
		//Printer::instance()->info( "run_diana_task : waiting... result available:%d", client->m_result_available ); ///something was going wrong with 'client->m_result_available' update
	...
	m_res = !client->m_heartbeat_error && 
			client->m_result_available;
