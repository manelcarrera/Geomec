...
GeomecBusClient::GeomecBusClient(const QString &base, const QString &name, QObject *parent)
    : ModGMLocalBusClient(base, name, parent), m_result_available(false), m_diana_running(false), m_status(Stopped) {
...
void GeomecBusClient::send(const QString &to, BusClientCmd cmd) {
  Printer::instance()->info("[Geomec]BC > send ['%s'->'%s','%s'] ->", m_name.toStdString().c_str(),
                            to.toStdString().c_str(), g_bus_client_cmd_s[cmd].c_str());
...
void GeomecBusClient::onReceivedMessage(const ModGMBusMessage &message) {
  ...
  Printer::instance()->info("[Geomec]BC > on_rec ['%s'->'%s','%s'] <-", from.toStdString().c_str(),
