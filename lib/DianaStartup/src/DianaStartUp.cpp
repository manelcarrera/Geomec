...
// 3) fix vars (connstants all along the program) are set at launch time via diana envars and taken fron 'Environment'
// singleton (from system ENVVARs first and then from config file )
...
static int ReportProgressHandler(const char *txt) { return 0; }
...
void CDianaStartUp::on_diana_msg(eDianaMsg type_, const QString &str, bool bStop) {
  ...
  if (m_gi) {
    m_gi->push_ipc(
        Cmd(DianaMsg,
            new std::string(
                (g_diana_msg_type[type_] + (type_ == eDianaMsg::StopExe ? bStop ? "1" : "0" : str)).toStdString())));
  }
  //
  // only non-dsa
...
CDianaStartUp::CDianaStartUp()
    : // m_pid( NO_PID ),
      m_di(nullptr), m_gi(nullptr), m_quit(false) {
  // assert(!m_pThis);
  // m_pThis=this;
...
int CDianaStartUp::RunDiana_DSA_OLD() {
  ...
  connect(worker, SIGNAL(finished()), this, SLOT(on_finished()),
          Qt::DirectConnection); // this is not blocked by the processEvents
...
CDianaStartUp::eError CDianaStartUp::checks() {
  ...
  if (DIAPATH_.isEmpty() || !QDir(DIAPATH_).exists())
    return DIAPATH_E;
...
void CDianaStartUp::var(const QString &key, const QString &val) { m_vars_m[key] = val; }
...
bool CDianaStartUp::is_var(const QString &key) { return m_vars_m.find(key) != m_vars_m.end(); }
...
int CDianaStartUp::RunDiana_ONE_PROCESS() {
  ...
  if (res != NO_E) // ERROR
  {
    QString error_s = QString("ERROR: ") + res == STP0_E ? CEnvironment::STP0
  ...
  //------------------ 3) Set CALLBACKS
  InstallDCMessageHandler(ASK_DC_HANDLE_ERRMSG, ASK_DC_TERMINATE_ERRMSG, diana::cb::DisplayDianaErrorMessage);

  InstallDCMessageHandler(ASK_DC_APPSPC_MSG, NULL, diana::cb::AppSpcMessageHandler);

  InstallDCMessageHandler(ASK_DC_REPORT_PROGRESS, ASK_DC_ENDOF_PROGRESSTEXT, diana::cb::ReportProgressHandler);

  if (is_var(CDianaStartUp::USER_LEADER) && is_var(CDianaStartUp::USER_TRAILER)) {
    InstallDCMessageHandler(var(CDianaStartUp::USER_LEADER).toStdString().c_str(),
                            var(CDianaStartUp::USER_TRAILER).toStdString().c_str(), diana::cb::UserMessageHandler);
  }
...
void CDianaStartUp::Print_DianaVars() {
  for (auto arg : m_vars_m) {
    printer->debug("[DianaV] %s: %s", arg.first.toStdString().c_str(), arg.second.toStdString().c_str());
  }
}
...
void CDianaStartUp::Print_DianaEnv() {
  ...
  auto keys = {CEnvironment::DIAPATH,    CEnvironment::DIASHARE,        CEnvironment::DIALIB, CEnvironment::DIASLIB,
               CEnvironment::DIAERRPATH, CEnvironment::APP_SPC_LOGGING, CEnvironment::STP0};

  for (auto key : keys)
    printer->debug("[Diana ] %s=%s", key.toStdString().c_str(), DiGetenv(key.toStdString().c_str()));

  for (auto key : {CDianaStartUp::FF, CDianaStartUp::FFDIR})
    printer->debug("[Diana+] %s=%s", key.toStdString().c_str(), DiGetenv(key.toStdString().c_str()));
}

const QString CDianaStartUp::GetDianaEnv(const QString &key) { return QString(DiGetenv(key.toStdString().c_str())); }

void CDianaStartUp::SetDianaEnv(const QString &key, const QString &value) {
  vDiSetenv("%s=%s", key.toStdString().c_str(), value.toStdString().c_str());
...
bool CDianaStartUp::SetDianaEnv_v2(QString varName, QString value) {
  ...
  return (vDiSetenv("%s=%s", varName.toStdString().c_str(), value.toStdString().c_str()) == 0);
#else  // (!defined(_WIN32) && !defined(_WIN64)) || defined(NDEBUG)
  return (vDiSetenv(DiStrsave(QString(varName + "=" + value).toStdString().c_str())) == 0);
...
void CDianaStartUp::Print_Env() {
  ...
  for (auto key : keys)
    printer->debug("%s=%s", key.toStdString().c_str(), env->get(key).toStdString().c_str());
}
...
void CDianaStartUp::ClearDianaEnv_All() const {
  ...
  for (auto var : {"STP0", "DIASLIB", "DIALIB", "FEMGVX", "DIAPATH", "FGVWORK", "FF", "FFDIR", "FGVSTR", "DIABIN",
                   "DIASHARE", "DIASCRIPT", "BINSEG", "DIALIB", "DIASLIB", "SEGPATH", "DIAERRPATH"})
    vDiSetenv("%s=", var);
}
...
void CDianaStartUp::ClearDianaEnv_Model() const {
  ...
  for (auto var : {CDianaStartUp::FF, CDianaStartUp::FFDIR})
