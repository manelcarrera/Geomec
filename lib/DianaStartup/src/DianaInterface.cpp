...
//
bool exe(const QString &bin, QStringList params, QString &result) {
  QProcess p1;
  p1.setProgram(bin);
...
bool running(const QString &process) {
  QProcess p;
  p.start("tasklist", QStringList() << "/NH"
                                    << "/FO" << "CSV"
...
bool running_(const qint64 &pid) {
  ...
  QProcess p;
  QStringList params = QStringList() << "/NH"
                                     << "/FO"
                                     << "CSV"
                                     << "/FI" << QString("PID eq %1").arg(pid_qs);
  ...
  printer->debug("running: tasklist %s / %s", params.join(" ").toStdString().c_str(), output.toStdString().c_str());
...
// /im: image name
//
void kill(const QString &process) { QProcess::execute("taskkill /f /t /im " + process); }
...
DI::DI()
    : IListener(CEvents::DI_EH, {HeartBeat, HeartBeat_Error}), // both emitted by HB
      IListenerIpc(IListenerIpc::Server), m_diana_app_pid(0), m_diana_status(Stopped), m_hb_timeout(NOT_DEFINED),
...
void DI::on_launch_diana() {
  qint64 pid = gm::di::launch(DIANA_APP,
                              QStringList() << QString::fromStdString(token())   // for shm naming
  ...
  if (!diana_app_pid()) {
    ...
    _e->broadcast(make_cmd(
        RunError, new CEvents::ErrorData{CEvents::DI_EH, UnableToLaunchDiana})); // joins and deletes this thread
  }
}
...
QStringList DI::run_diana_params() {
  ...
  QStringList params = QStringList()
                       // env
                       << m_dsu->GetDianaEnv(CDianaStartUp::FFDIR) << m_dsu->GetDianaEnv(CDianaStartUp::FF)
                       << STP0_filename
                       // vars
                       << m_dsu->var(CDianaStartUp::BASE) << m_dsu->var(CDianaStartUp::DISPLAY)
                       << m_dsu->var(CDianaStartUp::USER_LEADER) << m_dsu->var(CDianaStartUp::USER_TRAILER);

  auto print_ = [=]() {
    printer->debug(
        "size:%d --FFDIR-- %s --FF-- %s --STP0-- %s --BASE-- %s --DISPLAY-- %s --USER_LEADER-- %s --USER_TRAILER-- %s",
        params.join("*").toStdString().size(), params[0].toStdString().c_str(), params[1].toStdString().c_str(),
        params[2].toStdString().c_str(), params[3].toStdString().c_str(), params[4].toStdString().c_str(),
        params[5].toStdString().c_str(), params[6].toStdString().c_str());
  };
  print_();
...
void DI::on_diana_response(const Cmd &cmd) {
  ...
  case eCmd::QuitDiana_Ack:
    diana_status(Stopped);
    break;
  default:
...
void DI::on_launch_diana() {
  ...
  if (res == DianaNotRunning || res == DianaKilled) {
