...
HB::HB(CEvents::eEventsHandler eh, CEvents::eEventsHandler notify_to)
    : IListener(eh, {}), m_th(nullptr), m_b_quit_hb(false), m_b_hb(false), m_hb_timeout(HB_TIMEOUT),
...
void HB::hb() // thread
{
  ...
  for (;;) {
    auto print_ = [=]() {
      auto time_ = [](const ptime &pt) {
        return QString::fromStdString(to_simple_string(pt)).split(" ")[1].toStdString();
      };
      ...
      m_printer->debug("hb : tic ... s -> %s, r <- %s, now:%s ... quit:%s ... res:%s", t_send.c_str(), t_rec.c_str(),
                       t_now.c_str(), m_b_quit_hb ? "yes" : "no", m_time_sent > m_time_rec ? "Error" : "Ok");
    };
    print_();
