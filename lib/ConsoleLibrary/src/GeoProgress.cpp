...
CGeoProgress::CGeoProgress(CWnd * /*parent*/, const QString &sTitle, bool bCancel)
    : CProgressBase(), m_nSteps(0), m_nCurStep(0), m_strTitle(sTitle), m_bDeleteDlg(true), m_bCancel(bCancel),
      m_bBlockStatusMessage(false) {}

CGeoProgress::CGeoProgress(CWnd * /*parent*/, unsigned int uTitle, bool bCancel)
    : CProgressBase(), m_nSteps(0), m_nCurStep(0), m_bDeleteDlg(true), m_bCancel(bCancel),
      m_bBlockStatusMessage(false) {
...
void CGeoProgress::StatusMessage(const QString &sMessage) {
  if (sMessage != m_strStatusMessage) {
    _m()->outstream() << std::endl
                      << CProgressBase::getDateTimeStamp().toStdString() << sMessage.toStdString() << std::endl;
