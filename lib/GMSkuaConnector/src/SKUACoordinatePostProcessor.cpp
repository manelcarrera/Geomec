...
void SKUACoordinatePostProcessor::FixCoordinates() {
  if (std::abs(m_data->coord_unit[0] - 1) > 1E-12 || std::abs(m_data->coord_unit[1] - 1) > 1E-12 ||
      std::abs(m_data->coord_unit[2] - 1) > 1E-12)
    tbb::parallel_for_each(m_data->vertices.begin(), m_data->vertices.end(), [&](SKUAParseData::Vertex &vertex) {
      if (vertex.link < 0)
        for (int i = 0; i < 3; ++i)
