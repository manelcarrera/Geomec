...
void bodytriangle2triangle_nolink(const std::vector<SKUAParseData::Vertex> &vertices,
                                  const std::vector<SKUAParseData::Tetra> &tetras,
                                  const SKUAParseData::BodyTriangle &bdytriangle, int (&node)[3]) {
...
void bodytriangle2triangle(const std::vector<SKUAParseData::Vertex> &vertices,
                           const std::vector<SKUAParseData::Tetra> &tetras,
                           const SKUAParseData::BodyTriangle &bdytriangle, int (&node)[3]) {
  const SKUAParseData::Tetra &tetra = tetras[bdytriangle.tetra];
...
struct SortedTriangle {
  ...
  void sort() { std::sort(std::begin(sorted_node), std::end(sorted_node)); }
  ...
  SortedTriangle(const std::vector<SKUAParseData::Vertex> &vertices, const std::vector<SKUAParseData::Tetra> &tetras,
                 const SKUAParseData::BodyTriangle &bdytriangle) {
...
void SKUASolidPostProcessor::FixVolumeReferences() {
  ...
  tbb::parallel_for_each(m_data->tetras.begin(), m_data->tetras.end(),
                         [&](SKUAParseData::Tetra &tetra) { tetra.model_region = refs[tetra.model_region]; });
...
void SKUASolidPostProcessor::FixSurfaceReferences() {
  ...
  tbb::parallel_for_each(m_data->tetras.begin(), m_data->tetras.end(), [&](SKUAParseData::Tetra &tetra) {
    for (int i = 0; i < 4; ++i)
      tetra.opposite_faces[i] = refs[tetra.opposite_faces[i]];
  });
}
...
void SKUASolidPostProcessor::FixSurfaceBodyTriangles() {
  ...
  tbb::parallel_for_each(
      m_data->surface_names.begin(), m_data->surface_names.end(), [&](SKUAParseData::String &surface_name) {
        ...
        if (minus.size() == 0 &&
            plus.size() == 0) // if we didn't have any mapping info in the tetras (no CTETRA lines) then we create it
        {
          ...
          for (int i = 0; i < tetras.size(); ++i) {
            for (int j = 0; j < 4; ++j) {
              ...
              std::map<internal::SortedTriangle, int>::iterator it =
                  sorted.find(internal::SortedTriangle(vertices, tetras, tetra_triangle));
              if (it != sorted.end()) {
          ...
          if (minus.size() > 0 && plus.size() > 0 && minus.size() != plus.size()) {
            ...
            for (int i = 0; i < tmp.size(); ++i) {
              std::map<internal::SortedTriangle, int>::iterator it =
                  sorted.find(internal::SortedTriangle(vertices, tetras, tmp[i]));
              if (it != sorted.end()) {
                if (triangles[it->second].minus < 0) {
                  triangles[it->second].minus = (int)minus.size();
            ...
            if (minus.size() > 0) {
              ...
              for (int i = 0; i < size; ++i) {
                std::map<internal::SortedTriangle, int>::iterator it =
                    sorted.find(internal::SortedTriangle(vertices, tetras, minus[i]));
                if (it != sorted.end()) {
            ...
            if (plus.size() > 0) {
              ...
              for (int i = 0; i < size; ++i) {
                std::map<internal::SortedTriangle, int>::iterator it =
                    sorted.find(internal::SortedTriangle(vertices, tetras, plus[i]));
                if (it != sorted.end()) {
...
// Or not at all.
// Primary concern is to detect faults, where we will follow Earth and the old rule that a fault has the same volume on
// either side of it We also detect real boundaries.
// TODO: Top and Bottom?
...
void SKUASolidPostProcessor::FixSurfaceTopBottom() {
  ...
  std::for_each(m_data->surface_names.begin(), m_data->surface_names.end(), [&](SKUAParseData::String &surface_name) {
    ...
    if (type == SKUAParseData::SurfaceType::SurfBoundary || type == SKUAParseData::SurfaceType::SurfTop) {
      ...
      std::for_each(triangles.begin(), triangles.end(), [&](const SKUAParseData::Triangle &triangle) {
        for (int i = 0; i < 3; ++i) {
          const SKUAParseData::Vertex &vertex = vertices[triangle.node[i]].link < 0
                                                    ? vertices[triangle.node[i]]
                                                    : vertices[vertices[triangle.node[i]].link];
...
// Set volume types: they are either normal volumes or DSF volumes, which are quasi volumes just to fill up space
// between DSF surfaces
void SKUASolidPostProcessor::FixVolumeTypes() {
  ...
  tbb::parallel_for_each(volume_names.begin(), volume_names.end(), [&](const SKUAParseData::String &volume_name) {
    ...
    if (volume.size() == 0)
      ...
      if (std::all_of(volume.begin(), volume.end(), [&](int index) {
...
// Some minor issues remain, such as shared vertices on intersection lines with horizons going to edge of fault (fault
// edge should be non-split)
void SKUASolidPostProcessor::FixSharedVertices() {
  ...
  tbb::parallel_for_each(tetras.begin(), tetras.end(), [&](SKUAParseData::Tetra &tetra) {
    for (int i = 0; i < 4; ++i)
  ...
  tbb::parallel_for_each(values.begin(), values.end(), [&](std::vector<double> &old_prop) {
