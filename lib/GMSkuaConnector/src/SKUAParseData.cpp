...
bool SKUAParseData::check_orientation(const Triangle &triangle) const {
  ...
  for (int i = 0; i < 3; ++i) {
    const Vertex &vertex =
        vertices[triangle.node[i]].link >= 0 ? vertices[vertices[triangle.node[i]].link] : vertices[triangle.node[i]];

    for (int j = 0; j < 3; ++j)
      points[i][j] = vertex.coordinate[j];
  }

  double cross[3] = {points[0][1] * points[1][2] - points[1][1] * points[0][2],
                     points[1][0] * points[0][2] - points[0][0] * points[1][2],
...
bool SKUAParseData::check_orientation(const Tetra &tetra) const {
  ...
  for (int i = 0; i < 4; ++i) {
    const Vertex &vertex =
        vertices[tetra.node[i]].link >= 0 ? vertices[vertices[tetra.node[i]].link] : vertices[tetra.node[i]];
  ...
  double cross[3] = {vector_a[1] * vector_b[2] - vector_a[2] * vector_b[1],
...
bool SKUAParseData::has_double_sided_faults() const {
  return std::any_of(volume_types.begin(), volume_types.end(),
                     [](int type) { return type == VolumeType::VolDSFVolume; });
}
...
template <class T> bool compareVector(const std::vector<T> &lhs, const std::vector<T> &rhs) {
...
template <class T> void assignVector(std::vector<T> &lhs, const std::vector<T> &rhs) {
  lhs.resize(rhs.size());
  std::copy(rhs.begin(), rhs.end(), lhs.begin());
}

template <class T> void assignVectorVector(std::vector<std::vector<T>> &lhs, const std::vector<std::vector<T>> &rhs) {
  lhs.resize(rhs.size());
