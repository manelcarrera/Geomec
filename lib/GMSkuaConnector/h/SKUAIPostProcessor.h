...
public:
  ...
  SKUAIPostProcessor(SKUAParseData *data, SKUAIPostProcessor *nextPostProcessor) : m_data(data), m_next((*nextPostProcessor)(data)) {}
  virtual ~SKUAIPostProcessor() {}

  void SetNextPostProcessor(SKUAIPostProcessor *nextPostProcessor) { m_next = (*nextPostProcessor)(m_data); }
  SKUAIPostProcessor *operator()(SKUAParseData *data) { m_data = data; return this; }
