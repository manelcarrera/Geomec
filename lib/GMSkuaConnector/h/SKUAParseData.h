...
struct INTERFACE_GMSKUACONNECTOR SKUAParseData {
  struct Vertex {
    ...
    Vertex(int id = 0, int link = -1) : id(id), link(link), elements(0) {}
  };
  ...
  struct VertexRange {
    ...
    VertexRange() : start(0), size(0), offset(0) {}

    VertexRange(int start, int size, int offset) : start(start), size(size), offset(offset) {}
  };
  ...
  struct Tetra {
    ...
    Tetra(int model_region = 0) : model_region(model_region), tetra_region(0) {}
  };
  ...
  struct Triangle {
    ...
    bool collapsed() const { return node[0] == node[1] || node[0] == node[2] || node[1] == node[2]; }
  ...
  struct Face {
    ...
    Face() : id(-1), type(0), triangle(-1), size(0) { key_vertices[0] = key_vertices[1] = key_vertices[2] = -1; }
  };
  ...
  struct String {
    ...
    String() : id(-1) { data[0] = 0; }

    String(const String &rhs) : id(rhs.id) {
      std::strncpy(data, rhs.data, MAX_IDENTIFIER_SIZE - 1);
      data[MAX_IDENTIFIER_SIZE - 1] = 0;
    }

    String(const String &rhs, size_t truncate_from_end) : id(rhs.id) {
    ...
    String(int id, const char *string) : id(id) {
    ...
    bool operator<(const String &rhs) const { return std::strncmp(data, rhs.data, MAX_IDENTIFIER_SIZE - 1) < 0; }
  ...
  struct PropertyData {
    ...
    PropertyData(size_t capacity = 24) : capacity(capacity), size(0) {
  ...
  enum VolumeType : int { VolUnknown = 0, VolEmpty = 1, VolNormal = 2, VolDSFVolume = 3 };
  ...
  enum SKUAType : int { Unknown = 0, LightTSolid = 1, TSolid = 2, TSurf = 3, VSet = 4 } skua_type;
  ...
  SKUAParseData(SKUAType type, size_t vertex_capacity = 10000, size_t tetra_capacity = 10000)
      : skua_type(type), volume(-1), last_range(vertex_range.rbegin()), vertex_props(vertex_capacity),
