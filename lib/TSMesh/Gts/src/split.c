...
#define HEAP_INSERT_OBJECT(h, e) (GTS_OBJECT(e)->reserved = gts_eheap_insert(h, e))
#define HEAP_REMOVE_OBJECT(h, e) (gts_eheap_remove(h, GTS_OBJECT(e)->reserved), GTS_OBJECT(e)->reserved = NULL)

static GtsObjectClass *cface_class(void) {
  static GtsObjectClass *klass = NULL;

  if (klass == NULL) {
    GtsObjectClassInfo cface_info = {"GtsCFace",
                                     sizeof(CFace),
                                     sizeof(CFaceClass),
...
static GtsTriangle *replace_edge_collapse(GtsEdge *e, GtsEdge *with, CFace *cf, GtsEHeap *heap
#ifdef DYNAMIC_SPLIT
                                          ,
                                          GtsTriangle ***a1
#endif
...
static CFace *cface_new(GtsFace *f, GtsEdge *e, GtsVertex *v1, GtsVertex *v2, GtsSplit *vs, GtsEHeap *heap,
                        GtsEdgeClass *klass
#ifdef DYNAMIC_SPLIT
                        ,
                        GtsSplitCFace *scf
  ...
  v = GTS_SEGMENT(e1)->v1 == v1 ? GTS_SEGMENT(e1)->v2 : GTS_SEGMENT(e1)->v1;
#ifdef NEW
...
static void find_vvs(GtsVertex *vs, GtsTriangle *t, GtsVertex **v, GtsEdge **vvs, gboolean orientation) {
  ...
  if (SEGMENT_USE_VERTEX(GTS_SEGMENT(e2), vs) || !gts_segments_touch(GTS_SEGMENT(e1), GTS_SEGMENT(e2))) {
    tmp = e1;
    e1 = e2;
    e2 = e3;
    e3 = tmp;
...
static void cface_expand(CFace *cf, GtsTriangle **a1, GtsTriangle **a2, GtsEdge *e, GtsVertex *v1, GtsVertex *v2,
                         GtsVertex *vs, GtsEdgeClass *klass) {
  GtsVertex *v;
  GtsEdge *e1, *e2, *vvs;
...
static void split_class_init(GtsObjectClass *klass) { klass->destroy = split_destroy; }
...
GtsSplitClass *gts_split_class(void) {
  ...
  if (klass == NULL) {
    GtsObjectClassInfo split_info = {"GtsSplit",
                                     sizeof(GtsSplit),
                                     ...
                                     (GtsArgSetFunc)NULL,
                                     (GtsArgGetFunc)NULL};
    klass = gts_object_class_new(gts_object_class(), &split_info);
  }
...
#ifdef DEBUG
    fprintf(stderr, "cface: %p->%d t: %p->%d a1: ", cf->f, id(cf->f), CFACE(cf->f)->t, id(CFACE(cf->f)->t));
    {
      GtsTriangle *t, **a;
      a = cf->a1;
      while ((t = *(a++)))
...
static gboolean split_traverse_pre_order(GtsSplit *vs, GtsSplitTraverseFunc func, gpointer data) {
  if (func(vs, data))
    return TRUE;
  if (GTS_IS_SPLIT(vs->v1) && split_traverse_pre_order(GTS_SPLIT(vs->v1), func, data))
    return TRUE;
  if (GTS_IS_SPLIT(vs->v2) && split_traverse_pre_order(GTS_SPLIT(vs->v2), func, data))
    return TRUE;
  return FALSE;
}
...
static gboolean split_depth_traverse_pre_order(GtsSplit *vs, guint depth, GtsSplitTraverseFunc func, gpointer data) {
  ...
  if (GTS_IS_SPLIT(vs->v1) && split_depth_traverse_pre_order(GTS_SPLIT(vs->v1), depth, func, data))
    return TRUE;
  if (GTS_IS_SPLIT(vs->v2) && split_depth_traverse_pre_order(GTS_SPLIT(vs->v2), depth, func, data))
    return TRUE;
  return FALSE;
}

static gboolean split_traverse_post_order(GtsSplit *vs, GtsSplitTraverseFunc func, gpointer data) {
  if (GTS_IS_SPLIT(vs->v1) && split_traverse_post_order(GTS_SPLIT(vs->v1), func, data))
    return TRUE;
  if (GTS_IS_SPLIT(vs->v2) && split_traverse_post_order(GTS_SPLIT(vs->v2), func, data))
    return TRUE;
  if (func(vs, data))
    return TRUE;
  return FALSE;
...
static gboolean split_depth_traverse_post_order(GtsSplit *vs, guint depth, GtsSplitTraverseFunc func, gpointer data) {
  depth--;
  if (depth) {
    if (GTS_IS_SPLIT(vs->v1) && split_depth_traverse_post_order(GTS_SPLIT(vs->v1), depth, func, data))
      return TRUE;
    if (GTS_IS_SPLIT(vs->v2) && split_depth_traverse_post_order(GTS_SPLIT(vs->v2), depth, func, data))
      return TRUE;
  }
  if (func(vs, data))
    return TRUE;
...
gboolean gts_split_is_collapsable(GtsSplit *vs) {
  ...
  v1 = GTS_SPLIT_V1(vs);
  v2 = GTS_SPLIT_V2(vs);
  g_return_val_if_fail((e = GTS_EDGE(gts_vertices_are_connected(v1, v2))), FALSE);
...
#ifdef DEBUG_HEXPAND
        expand_indent(stderr);
        fprintf(stderr, "expand a1: cf->f: %p t: %p parent_split: %p\n", cf->f, t, GTS_HSPLIT(CFACE(t)->parent_split));
#endif
        gts_hsplit_force_expand(GTS_HSPLIT(CFACE(t)->parent_split), hsurface);
#ifdef DEBUG_HEXPAND
        g_assert(!IS_CFACE(t));
#endif
      }
...
static void index_object(GtsObject *o, guint *n) { o->reserved = GUINT_TO_POINTER((*n)++); }
...
void gts_psurface_write(GtsPSurface *ps, FILE *fptr) {
  ...
  while (ps->pos) {
    ...
    fprintf(fptr, "%llu %llu", GPOINTER_TO_UINT(GTS_OBJECT(vs->v)->reserved), vs->ncf);
    ...
    while (i--) {
      ...
      fprintf(fptr, "%llu %llu", GPOINTER_TO_UINT(g_hash_table_lookup(hash, cf->t)), cf->flags);
  ...
  gts_surface_foreach_vertex(ps->s, (GtsFunc)gts_object_reset_reserved, NULL);
  g_hash_table_destroy(hash);
}

static GtsSurface *surface_read(GtsSurface *surface, FILE *fptr, guint *line, GPtrArray *vertices, GPtrArray *faces) {
  GtsEdge **edges;
    ...
    if (delim == EOF || delim == '\n')
      ...
      if (delim == EOF)
        ...
        if (p1 == 0 || p2 == 0 || p1 > np || p2 > np)
          ...
          GtsEdge *new_edge = gts_edge_new(surface->edge_class, g_ptr_array_index(vertices, p1 - 1),
...
GtsSplit *gts_psurface_read_vertex(GtsPSurface *ps, FILE *fptr) {
  ...
  delim = (*GTS_OBJECT_CLASS(ps->s->vertex_class)->read)(&(vs->v1), fptr);
  if (delim == EOF)
    ok = FALSE;
  else {
    vs->v1->reserved = vs;
    ...
    delim = (*GTS_OBJECT_CLASS(ps->s->vertex_class)->read)(&(vs->v2), fptr);
    if (delim == EOF)
      ok = FALSE;
    else {
      vs->v2->reserved = vs;
        ...
        if (delim == EOF || it == 0 || it > ps->faces->len)
          ...
          if (delim != '\n') {
            if (GTS_OBJECT(f)->klass->read)
              delim = (*GTS_OBJECT(f)->klass->read)((GtsObject **)&f, fptr);
            if (delim != '\n')
              gts_get_newline(fptr);
          }
          scf->f = f;
  ...
  if (vs->v1)
    gts_object_destroy(vs->v1);
  if (vs->v2)
    gts_object_destroy(vs->v2);
