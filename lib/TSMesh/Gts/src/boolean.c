...
#ifdef DEBUG
static void write_edges_vertices(GSList *edges, GSList *vertices, FILE *fptr) {
  GSList *i = vertices;
  guint v = 1;

  fprintf(fptr, "%u %u 0\n", g_slist_length(vertices), g_slist_length(edges));
  while (i) {
    GtsPoint *p = i->data;
  ...
  while (i) {
    ...
    fprintf(fptr, "%u %u %p:%s\n", GPOINTER_TO_UINT(GTS_OBJECT(s->v1)->reserved),
            GPOINTER_TO_UINT(GTS_OBJECT(s->v2)->reserved), s, GTS_NEDGE(s)->name);
    i = i->next;
  }
  i = vertices;
  while (i) {
...
static void surface_inter_destroy(GtsObject *object) {
  ...
  (*GTS_OBJECT_CLASS(gts_surface_inter_class())->parent_class->destroy)(object);
}

static void surface_inter_class_init(GtsObjectClass *klass) { klass->destroy = surface_inter_destroy; }
...
GtsSurfaceInterClass *gts_surface_inter_class(void) {
  ...
  if (klass == NULL) {
    GtsObjectClassInfo surface_inter_info = {"GtsSurfaceInter",
 ...
 */
static GtsPoint *segment_triangle_intersection(GtsSegment *s, GtsTriangle *t, GtsPointClass *klass, GtsEdge **cut) {
  GtsPoint *A, *B, *C, *D, *E;
  ...
  gts_triangle_vertices_edges(t, NULL, (GtsVertex **)&A, (GtsVertex **)&B, (GtsVertex **)&C, &AB, &BC, &CA);
  D = GTS_POINT(s->v1);
  E = GTS_POINT(s->v2);
  *cut = NULL;
  ...
  if (pi == NULL) {
    ...
    pi = gts_point_new(klass, E->x + c * (D->x - E->x), E->y + c * (D->y - E->y), E->z + c * (D->z - E->z));
  }
  return pi;
}

#define prepend_unique(list, data) (g_slist_find(list, data) ? list : g_slist_prepend(list, data))

static GtsVertex *intersects(GtsEdge *e, GtsTriangle *t, GtsSurface *surface) {
  GtsPoint *pi;
  GtsVertex *vi = NULL;
  ...
  while (i) {
    ...
    if (cut == t || cut == t->e1 || cut == t->e2 || cut == t->e3 || v == v1 || v == v2 || v == v3)
      return v;
    i = i->next;
  }
  s = GTS_SEGMENT(e);
  /* Warning: wild cast! */
  pi = segment_triangle_intersection(s, t, GTS_POINT_CLASS(surface->vertex_class), &cut);
  if (pi) {
    vi = GTS_VERTEX(pi);
    if (cut) {
      ...
      if (GTS_NVERTEX(vi)->name[0] == '\0')
        g_snprintf(GTS_NVERTEX(vi)->name, GTS_NAME_LENGTH, "%s|%s", GTS_NEDGE(e)->name, GTS_NEDGE(cut)->name);
#endif
    } else {
      GTS_OBJECT(vi)->reserved = t;
#ifdef DEBUG_NAMED
      if (GTS_NVERTEX(vi)->name[0] == '\0')
        g_snprintf(GTS_NVERTEX(vi)->name, GTS_NAME_LENGTH, "%s|%s", GTS_NEDGE(e)->name, GTS_NFACE(t)->name);
#endif
    }
    if (vi != s->v1 && vi != s->v2)
...
static gdouble intersection_orientation(GtsTriangle *t1, GtsEdge *e, GtsTriangle *t2) {
  GtsVertex *v1, *v2, *v3;
  GtsEdge *e2, *e3;
  GtsVertex *v4, *v5, *v6;
  gdouble o1;
  ...
  o1 = gts_point_orientation_3d(GTS_POINT(v4), GTS_POINT(v5), GTS_POINT(v6), GTS_POINT(v2));
  if (o1 != 0.)
    return o1;
  return -gts_point_orientation_3d(GTS_POINT(v4), GTS_POINT(v5), GTS_POINT(v6), GTS_POINT(v1));
}
...
#ifdef DEBUG_NAMED
      g_snprintf(GTS_NEDGE(e)->name, GTS_NAME_LENGTH, "%s:%s", GTS_NVERTEX(vi1)->name, GTS_NVERTEX(vi2)->name);
#endif
    } else {
      GtsEdge *duplicate;
      e = GTS_EDGE(s);
...
static void triangulate_face(GtsTriangle *t, GtsSurface *sn) {
  ...
  GtsSurface *s = gts_surface_new(gts_surface_class(), sn->face_class, sn->edge_class, sn->vertex_class);
  ...
  fprintf(stderr, "=============== triangulation t.%d: %p:%d %p->%p->%p\n", nt, t, id(t), t->e1, t->e2, t->e3);
  sprintf(name, "t.%d", nt++);
  fptr = fopen(name, "wt");
#endif
  ...
  if (GTS_OBJECT(t->e1)->reserved) {
    vertices = g_slist_concat(g_slist_copy(GTS_OBJECT(t->e1)->reserved), vertices);
  ...
  if (GTS_OBJECT(t->e2)->reserved) {
    vertices = g_slist_concat(g_slist_copy(GTS_OBJECT(t->e2)->reserved), vertices);
    segment_detach_endpoints(GTS_SEGMENT(t->e2));
  } else
    edges = prepend_unique(edges, t->e2);
  if (GTS_OBJECT(t->e3)->reserved) {
    vertices = g_slist_concat(g_slist_copy(GTS_OBJECT(t->e3)->reserved), vertices);
  ...
  gts_surface_add_face(s, gts_face_new(gts_face_class(), enclosing->e1, enclosing->e2, enclosing->e3));
#ifdef DEBUG
  ...
  while (i) {
#ifdef DEBUG
    fprintf(stderr, "adding %p->%d: (%.20g,%.20g,%.20g)\n", i->data, id(i->data), GTS_POINT(i->data)->x,
            GTS_POINT(i->data)->y, GTS_POINT(i->data)->z);
#endif
...
#ifdef DEBUG
    GSList *inter;
    fprintf(stderr, "adding constraint %p->%d: %p:%d->%p:%d\n", i->data, id(i->data), GTS_SEGMENT(i->data)->v1,
            id(GTS_SEGMENT(i->data)->v1), GTS_SEGMENT(i->data)->v2, id(GTS_SEGMENT(i->data)->v2));
...
GtsSurfaceInter *gts_surface_inter_new(GtsSurfaceInterClass *klass, GtsSurface *s1, GtsSurface *s2, GNode *faces_tree1,
  ...
  si->s1 = gts_surface_new(gts_surface_class(), s1->face_class, s1->edge_class, s1->vertex_class);
  si->s2 = gts_surface_new(gts_surface_class(), s2->face_class, s2->edge_class, s2->vertex_class);
  gts_bb_tree_traverse_overlapping(faces_tree1, faces_tree2, (GtsBBTreeTraverseFunc)intersect_edges, si);

  s = gts_surface_new(gts_surface_class(), s1->face_class, s1->edge_class, s1->vertex_class);
  ...
  s = gts_surface_new(gts_surface_class(), s2->face_class, s2->edge_class, s2->vertex_class);
...
/* Given @e and @f returns a #GtsFace compatible with @f and belonging to
   @s1 or @s2 */
static GtsFace *next_compatible_face(GtsEdge *e, GtsFace *f, GtsSurface *s1, GtsSurface *s2, GtsSurface *ssss) {
    ...
    if (f1 != f && GTS_IS_FACE(f1)) {
      ...
      if (gts_face_has_parent_surface(f1, s2)) {
        if (f2 == NULL)
          ...
          g_assert_not_reached(); /* s2 is a non-manifold surface */
      }
    }
    i = i->next;
  }
...
static gint triangle_orientation(GtsTriangle *t, GtsEdge *e) {
  GtsSegment *s = GTS_SEGMENT(t->e1 == e ? t->e2 : t->e2 == e ? t->e3 : t->e1);
...
void gts_surface_inter_boolean(GtsSurfaceInter *si, GtsSurface *surface, GtsBooleanOperation op) {
  ...
  while (i) {
    ...
    while (j) {
      if (gts_face_has_parent_surface(j->data, s) && orient * triangle_orientation(j->data, e) > 0) {
  ...
  g_slist_foreach(si->edges, (GFunc)gts_object_reset_reserved, NULL);
  gts_surface_foreach_face(surface, (GtsFunc)gts_object_reset_reserved, NULL);
}
