...
static void vertex_encroaches(GtsVertex *v, GtsSurface *surface, GtsFifo *encroached, GtsEncroachFunc encroaches,
                              gpointer data) {
  GSList *triangles, *i;
  ...
  while (i) {
    ...
    if (GTS_IS_FACE(f) && gts_face_has_parent_surface(f, surface)) {
      ...
      if (!ALREADY_ENCROACHED(e) && GTS_IS_CONSTRAINT(e) && (*encroaches)(v, e, surface, data)) {
...
static void make_encroached_fifo(GtsEdge *e, gpointer *datas) {
  ...
  if (GTS_IS_CONSTRAINT(e) && gts_edge_is_encroached(e, s, encroaches, data))
    gts_fifo_push(fifo, e);
}

#define SQUARE_ROOT_TWO 1.41421356237309504880168872420969807856967187
#define DISTANCE_2D(v1, v2)                                                                                            \
  (sqrt((GTS_POINT(v2)->x - GTS_POINT(v1)->x) * (GTS_POINT(v2)->x - GTS_POINT(v1)->x) +                                \
        (GTS_POINT(v2)->y - GTS_POINT(v1)->y) * (GTS_POINT(v2)->y - GTS_POINT(v1)->y)))
...
static GtsVertex *split_edge(GtsEdge *e, GtsSurface *surface) {
  ...
  if (c) {
    ...
    if (GTS_SEGMENT(c)->v1 == v2 || GTS_SEGMENT(c)->v2 == v2)
      split = 1. - split;
    return gts_vertex_new(surface->vertex_class, (1. - split) * GTS_POINT(v1)->x + split * GTS_POINT(v2)->x,
                          (1. - split) * GTS_POINT(v1)->y + split * GTS_POINT(v2)->y,
                          (1. - split) * GTS_POINT(v1)->z + split * GTS_POINT(v2)->z);
  } else
    return gts_segment_midvertex(GTS_SEGMENT(e), surface->vertex_class);
}

static gint split_encroached(GtsSurface *surface, GtsFifo *encroached, gint steiner_max, GtsEncroachFunc encroaches,
                             gpointer data) {
  GtsSegment *s;
...
#else
    GtsEdge *e1 = gts_edge_new(GTS_EDGE_CLASS(GTS_OBJECT(s)->klass), s->v1, v);
    GtsEdge *e2 = gts_edge_new(GTS_EDGE_CLASS(GTS_OBJECT(s)->klass), v, s->v2);
#endif
...
guint gts_delaunay_conform(GtsSurface *surface, gint steiner_max, GtsEncroachFunc encroaches, gpointer data) {
  ...
  split_encroached(surface, encroached, steiner_max, encroaches, data);
...
static void heap_surface_add_face(GtsSurface *s, GtsFace *f) {
  ...
  if (GTS_SURFACE_CLASS(GTS_OBJECT(s)->klass->parent_class)->add_face)
    (*GTS_SURFACE_CLASS(GTS_OBJECT(s)->klass->parent_class)->add_face)(s, f);
}
...
static void heap_surface_remove_face(GtsSurface *s, GtsFace *f) {
  ...
  if (GTS_SURFACE_CLASS(GTS_OBJECT(s)->klass->parent_class)->remove_face)
    (*GTS_SURFACE_CLASS(GTS_OBJECT(s)->klass->parent_class)->remove_face)(s, f);
}
...
static GtsObjectClass *heap_surface_class_new(GtsObjectClass *parent_class) {
  ...
  heap_surface_info.class_init_func = (GtsObjectClassInitFunc)heap_surface_class_init;
  return gts_object_class_new(parent_class, &heap_surface_info);
}
...
guint gts_delaunay_refine(GtsSurface *surface, gint steiner_max, GtsEncroachFunc encroaches, gpointer encroach_data,
  ...
  while (steiner_max-- != 0 && (f = gts_eheap_remove_top(heap, NULL))) {
    GtsVertex *c =
        GTS_VERTEX(gts_triangle_circumcircle_center(GTS_TRIANGLE(f), GTS_POINT_CLASS(surface->vertex_class)));
    EHEAP_PAIR(f) = NULL;
    g_assert(c != NULL);
    ...
    if (!gts_fifo_is_empty(encroached)) {
      ...
      steiner_max = split_encroached(surface, encroached, steiner_max, encroaches, encroach_data);
    }
  }
