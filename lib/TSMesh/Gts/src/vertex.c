...
static void vertex_init(GtsVertex *vertex) { vertex->segments = NULL; }
...
GtsVertexClass *gts_vertex_class(void) {
  ...
  if (klass == NULL) {
    GtsObjectClassInfo vertex_info = {"GtsVertex",
                                      sizeof(GtsVertex),
                                      ...
                                      (GtsArgSetFunc)NULL,
                                      (GtsArgGetFunc)NULL};
    klass = gts_object_class_new(GTS_OBJECT_CLASS(gts_point_class()), &vertex_info);
  }
  ...
  while (i) {
    ...
    if (GTS_IS_EDGE(s)) {
      ...
      while (j) {
        ...
        if (GTS_IS_FACE(t) && (!surface || gts_face_has_parent_surface(GTS_FACE(t), surface)) && !g_slist_find(list, t))
...
GSList *gts_vertex_neighbors(GtsVertex *v, GSList *list, GtsSurface *surface) {
  ...
  while (i) {
    ...
    if (v1 != v && (!surface || (GTS_IS_EDGE(s) && gts_edge_has_parent_surface(GTS_EDGE(s), surface))) &&
...
gboolean gts_vertex_is_boundary(GtsVertex *v, GtsSurface *surface) {
  ...
  i = v->segments;
  while (i) {
    if (GTS_IS_EDGE(i->data) && gts_edge_is_boundary(i->data, surface))
      return TRUE;
    i = i->next;
  }
...
GList *gts_vertices_merge(GList *vertices, gdouble epsilon) {
  ...
  while (i) {
    ...
    if (!GTS_OBJECT(v)->reserved) { /* Do something only if v is active */
      ...
      /* build bounding box */
      bbox = gts_bbox_new(gts_bbox_class(), v, GTS_POINT(v)->x - epsilon, GTS_POINT(v)->y - epsilon,
                          GTS_POINT(v)->z - epsilon, GTS_POINT(v)->x + epsilon, GTS_POINT(v)->y + epsilon,
...
static GSList *edge_fan_list(GtsVertex *v, GtsSurface *surface, GtsFace *f, GtsEdge *e, GtsFace *first) {
  ...
  while (i) {
    GtsFace *f1 = i->data;
    if (GTS_IS_FACE(f1) && f1 != f && gts_face_has_parent_surface(f1, surface)) {
  ...
  g_return_val_if_fail(GTS_SEGMENT(enext)->v1 == v || GTS_SEGMENT(enext)->v2 == v, NULL);

  return g_slist_prepend(edge_fan_list(v, surface, neighbor, enext, first), next);
}
...
GSList *gts_vertex_fan_oriented(GtsVertex *v, GtsSurface *surface) {
  ...
  while (i) {
    ...
    if (GTS_IS_EDGE(e)) {
      ...
      guint degree = 0;
      while (j) {
        if (GTS_IS_FACE(j->data) && gts_face_has_parent_surface(j->data, surface)) {
          f1 = j->data;
          degree++;
        }
        j = j->next;
      ...
      if (f1 != NULL) {
        ...
        if (degree == 1) {
          gts_triangle_vertices_edges(GTS_TRIANGLE(f1), NULL, &v1, &v2, &v3, &e1, &e2, &e3);
          if (v == v2) {
            e2 = e3;
            e3 = e1;
          } else if (v == v3) {
...
static GtsEdge *replace_vertex(GtsTriangle *t, GtsEdge *e1, GtsVertex *v, GtsVertex *with) {
  GtsEdge *e = NULL;
