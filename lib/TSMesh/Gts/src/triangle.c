...
static void triangle_destroy(GtsObject *object) {
  ...
  if (!GTS_OBJECT_DESTROYED(e1) && !gts_allow_floating_edges && e1->triangles == NULL)
    gts_object_destroy(GTS_OBJECT(e1));

  e2->triangles = g_slist_remove(e2->triangles, triangle);
  if (!GTS_OBJECT_DESTROYED(e2) && !gts_allow_floating_edges && e2->triangles == NULL)
    gts_object_destroy(GTS_OBJECT(e2));

  e3->triangles = g_slist_remove(e3->triangles, triangle);
  if (!GTS_OBJECT_DESTROYED(e3) && !gts_allow_floating_edges && e3->triangles == NULL)
    gts_object_destroy(GTS_OBJECT(e3));

  (*GTS_OBJECT_CLASS(gts_triangle_class())->parent_class->destroy)(object);
}

static void triangle_class_init(GtsObjectClass *klass) { klass->destroy = triangle_destroy; }

static void triangle_init(GtsTriangle *triangle) { triangle->e1 = triangle->e2 = triangle->e3 = NULL; }
...
GtsTriangleClass *gts_triangle_class(void) {
  ...
  if (klass == NULL) {
    GtsObjectClassInfo triangle_info = {"GtsTriangle",
                                        sizeof(GtsTriangle),
...
void gts_triangle_set(GtsTriangle *triangle, GtsEdge *e1, GtsEdge *e2, GtsEdge *e3) {
  ...
  if (GTS_SEGMENT(e1)->v1 == GTS_SEGMENT(e2)->v1)
    g_return_if_fail(gts_segment_connect(GTS_SEGMENT(e3), GTS_SEGMENT(e1)->v2, GTS_SEGMENT(e2)->v2));
  else if (GTS_SEGMENT(e1)->v2 == GTS_SEGMENT(e2)->v1)
    g_return_if_fail(gts_segment_connect(GTS_SEGMENT(e3), GTS_SEGMENT(e1)->v1, GTS_SEGMENT(e2)->v2));
  else if (GTS_SEGMENT(e1)->v2 == GTS_SEGMENT(e2)->v2)
    g_return_if_fail(gts_segment_connect(GTS_SEGMENT(e3), GTS_SEGMENT(e1)->v1, GTS_SEGMENT(e2)->v1));
  else if (GTS_SEGMENT(e1)->v1 == GTS_SEGMENT(e2)->v2)
    g_return_if_fail(gts_segment_connect(GTS_SEGMENT(e3), GTS_SEGMENT(e1)->v2, GTS_SEGMENT(e2)->v1));
  else
...
gdouble gts_triangles_angle(GtsTriangle *t1, GtsTriangle *t2) {
  ...
  theta = atan2(sqrt(pvx * pvx + pvy * pvy + pvz * pvz), nx1 * nx2 + ny1 * ny2 + nz1 * nz2) - PI;
...
gboolean gts_triangles_are_compatible(GtsTriangle *t1, GtsTriangle *t2, GtsEdge *e) {
  ...
  if (t2->e1 == e)
    ...
    g_assert_not_reached();
  if (GTS_SEGMENT(e1)->v1 == GTS_SEGMENT(e2)->v1 || GTS_SEGMENT(e1)->v1 == GTS_SEGMENT(e2)->v2 ||
      GTS_SEGMENT(e1)->v2 == GTS_SEGMENT(e2)->v1 || GTS_SEGMENT(e1)->v2 == GTS_SEGMENT(e2)->v2)
    return FALSE;
  return TRUE;
}
...
gdouble gts_triangle_perimeter(GtsTriangle *t) {
  ...
  return gts_point_distance(GTS_POINT(GTS_SEGMENT(t->e1)->v1), GTS_POINT(GTS_SEGMENT(t->e1)->v2)) +
         gts_point_distance(GTS_POINT(GTS_SEGMENT(t->e1)->v1), GTS_POINT(v)) +
         gts_point_distance(GTS_POINT(GTS_SEGMENT(t->e1)->v2), GTS_POINT(v));
}
...
gdouble gts_triangle_quality(GtsTriangle *t) {
  ...
  return perimeter > 0.0 ? GOLDEN_PERIMETER * sqrt(gts_triangle_area(t)) / perimeter : 0.0;
}
 ...
 *
 * Computes the coordinates of the oriented normal of @t as the dot-product
 * of two edges. The normal is not normalized.
 */
void gts_triangle_normal(GtsTriangle *t, gdouble *x, gdouble *y, gdouble *z) {
  ...
  if (GTS_SEGMENT(t->e1)->v1 == GTS_SEGMENT(t->e2)->v1) {
    ...
    fprintf(stderr,
            "t: %p t->e1: %p t->e2: %p t->e3: %p t->e1->v1: %p t->e1->v2: %p t->e2->v1: %p t->e2->v2: %p t->e3->v1: %p "
            "t->e3->v2: %p\n",
            t, t->e1, t->e2, t->e3, GTS_SEGMENT(t->e1)->v1, GTS_SEGMENT(t->e1)->v2, GTS_SEGMENT(t->e2)->v1,
            GTS_SEGMENT(t->e2)->v2, GTS_SEGMENT(t->e3)->v1, GTS_SEGMENT(t->e3)->v2);
    g_assert_not_reached();
  }
...
gdouble gts_triangle_orientation(GtsTriangle *t) {
  ...
  if (GTS_SEGMENT(t->e1)->v1 == GTS_SEGMENT(t->e2)->v1) {
    ...
    g_assert_not_reached();
  return gts_point_orientation(GTS_POINT(v1), GTS_POINT(v2), GTS_POINT(v3));
}
...
GtsTriangle *gts_triangle_enclosing(GtsTriangleClass *klass, GSList *points, gdouble scale) {
  ...
  if (r == 0.0)
    r = scale;
  v1 = gts_vertex_new(gts_vertex_class(), xo + r * SQRT3, yo - r, 0.0);
  v2 = gts_vertex_new(gts_vertex_class(), xo, yo + 2. * r, 0.0);
  v3 = gts_vertex_new(gts_vertex_class(), xo - r * SQRT3, yo - r, 0.0);
...
GtsTriangle *gts_triangle_is_duplicate(GtsTriangle *t) {
  ...
  while (i) {
    GtsTriangle *t1 = i->data;
    if (t1 != t && (t1->e1 == e2 || t1->e2 == e2 || t1->e3 == e2) && (t1->e1 == e3 || t1->e2 == e3 || t1->e3 == e3))
      return t1;
    i = i->next;
  }
...
GtsTriangle *gts_triangle_use_edges(GtsEdge *e1, GtsEdge *e2, GtsEdge *e3) {
  ...
  while (i) {
    ...
    if ((t->e1 == e2 && (t->e2 == e3 || t->e3 == e3)) || (t->e2 == e2 && (t->e1 == e3 || t->e3 == e3)) ||
...
GtsPoint *gts_triangle_circumcircle_center(GtsTriangle *t, GtsPointClass *point_class) {
  ...
  if (det == 0.)
    return NULL;
  return gts_point_new(point_class, (yae * yad * (yd - ye) + xad * yae * xd - xae * yad * xe) / det,
                       -(xae * xad * (xd - xe) + yad * xae * yd - yae * xad * ye) / det, 0.);
}
...
static GtsVertex *triangle_use_vertices(GtsTriangle *t, GtsVertex *A, GtsVertex *B) {
  GtsVertex *v1 = GTS_SEGMENT(t->e1)->v1, *v2 = GTS_SEGMENT(t->e1)->v2, *v3 = gts_triangle_vertex(t);
...
gboolean gts_triangles_are_folded(GSList *triangles, GtsVertex *A, GtsVertex *B, gdouble max) {
  ...
  while (i) {
    ...
    while (j) {
      ...
      if (points_are_folded(GTS_POINT(A), GTS_POINT(B), GTS_POINT(C), GTS_POINT(D), max))
        return TRUE;
      j = j->next;
    }
    i = i->next;
