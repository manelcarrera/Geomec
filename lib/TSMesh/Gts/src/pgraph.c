...
static void gnode_split_destroy(GtsObject *object) {
  ...
  (*GTS_OBJECT_CLASS(gts_gnode_split_class())->parent_class->destroy)(object);
}
...
GtsGNodeSplitClass *gts_gnode_split_class(void) {
  ...
  if (klass == NULL) {
    GtsObjectClassInfo gnode_split_info = {"GtsGNodeSplit",
    ...
    if (n3 != n2) {
      ...
      while (j) {
        ...
        if (n4 == n2) { /* found triangle n1 (e13) n3 (e32) n2 */
          gts_wgedge_new(klass, ns->n, n3, gts_gedge_weight(e13) + gts_gedge_weight(e32));
          GTS_OBJECT(e13)->reserved = n3;
          GTS_OBJECT(e32)->reserved = n3;
          GTS_SLIST_CONTAINER(n3)->items = g_slist_remove(GTS_SLIST_CONTAINER(n3)->items, e32);
        }
        j = next;
      }
      if (GTS_OBJECT(e13)->reserved == n3)
        GTS_SLIST_CONTAINER(n3)->items = g_slist_remove(GTS_SLIST_CONTAINER(n3)->items, e13);
    }
    i = i->next;
  }
...
static void restore_edge(GtsGEdge *e, gpointer *data) {
  ...
  if (e->n1 == n)
    ...
    g_assert_not_reached();
  GTS_SLIST_CONTAINER(n)->items = g_slist_remove(GTS_SLIST_CONTAINER(n)->items, e);
}
...
void gts_gnode_split_expand(GtsGNodeSplit *ns, GtsGraph *g) {
  ...
  g_return_if_fail(ns != NULL);
  g_return_if_fail(g != NULL);
  g_return_if_fail(gts_containee_is_contained(GTS_CONTAINEE(ns->n), GTS_CONTAINER(g)));
...
static void pgraph_class_init(GtsPGraphClass *klass) { GTS_OBJECT_CLASS(klass)->destroy = pgraph_destroy; }
...
GtsPGraphClass *gts_pgraph_class(void) {
  ...
  if (klass == NULL) {
    GtsObjectClassInfo pgraph_info = {"GtsPGraph",
                                      sizeof(GtsPGraph),
...
static void match_neighbor(GtsGNode *n, gpointer *data) {
  if (!GTS_OBJECT(n)->reserved) {
    ...
    while (i) {
      ...
      if (!GTS_OBJECT(n1)->reserved && gts_gedge_weight(i->data) > wmax &&
...
GtsPGraph *gts_pgraph_new(GtsPGraphClass *klass, GtsGraph *g, GtsGNodeSplitClass *split_class,
  ...
  while (gts_container_size(GTS_CONTAINER(g)) > min && (matching = maximal_matching(g))) {
    GSList *i = matching;
    ...
    while (i && gts_container_size(GTS_CONTAINER(g)) > min) {
      GtsGEdge *e = i->data;
      GtsGNode *n = GTS_GNODE(gts_wgnode_new(node_class, gts_gnode_weight(e->n1) + gts_gnode_weight(e->n2)));
      GtsGNodeSplit *ns = gts_gnode_split_new(split_class, n, GTS_OBJECT(e->n1), GTS_OBJECT(e->n2));
