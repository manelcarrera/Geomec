...
static GtsFace *closest_face(GtsSurface *s, GtsPoint *p) {
  ...
  if (!nt)
    return NULL;
  ns = (guint)exp(log((gdouble)nt) / 3.);

  // TNO hash_table->size is cast to guint in order to remove a compile warning...
  for (i = 0, n = 0; i < (guint)(hash_table->size) && n < ns; i++)
    for (node = hash_table->nodes[i]; node && n < ns; node = node->next, n++) {
      GtsFace *f = node->key;

      d = gts_point_distance2(p, GTS_POINT(GTS_SEGMENT(GTS_TRIANGLE(f)->e1)->v1));
      if (d < dmin) {
        dmin = d;
        closest = f;
      }
...
static GtsFace *neighbor(GtsFace *f, GtsEdge *e, GtsSurface *surface) {
  GSList *i = e->triangles;
  GtsTriangle *t = GTS_TRIANGLE(f);

  while (i) {
    GtsTriangle *t1 = i->data;
    if (t1 != t && GTS_IS_FACE(t1) && gts_face_has_parent_surface(GTS_FACE(t1), surface))
      return GTS_FACE(t1);
    i = i->next;
  }
  return NULL;
...
static GtsEdge *triangle_next_edge(GtsTriangle *t, GtsPoint *o, GtsPoint *p, gboolean *on_summit) {
  GtsVertex *v1, *v2, *v3;
  GtsEdge *e1, *e2, *e3;
  gdouble orient = 0.0;
...
static void triangle_barycenter(GtsTriangle *t, GtsPoint *b) {
  ...
  b->x = (p->x + GTS_POINT(GTS_SEGMENT(t->e1)->v1)->x + GTS_POINT(GTS_SEGMENT(t->e1)->v2)->x) / 3.;
  b->y = (p->y + GTS_POINT(GTS_SEGMENT(t->e1)->v1)->y + GTS_POINT(GTS_SEGMENT(t->e1)->v2)->y) / 3.;
}

/* wedx 09-06-2008: implemented 'fix' suggested on
 * http://mail.flightgear.org/pipermail/terragear-devel/2003-March/000489.html */
static GtsFace *point_locate(GtsPoint *o, GtsPoint *p, GtsFace *f, GtsSurface *surface, int count) {
  GtsEdge *prev;
  gboolean on_summit;
  GtsVertex *v1, *v2, *v3;
  GtsEdge *e2, *e3;
  ...
  if (!prev) {
    ...
    if (count == 0) {
      if ((f1 = neighbor(f, GTS_TRIANGLE(f)->e1, surface)) || (f1 = neighbor(f, GTS_TRIANGLE(f)->e2, surface)) ||
          (f1 = neighbor(f, GTS_TRIANGLE(f)->e3, surface))) {
        triangle_barycenter(GTS_TRIANGLE(f1), o);
        return point_locate(o, p, f1, surface, 1);
      }
    } else if (count == 1) {
      if ((f1 = neighbor(f, GTS_TRIANGLE(f)->e2, surface)) || (f1 = neighbor(f, GTS_TRIANGLE(f)->e3, surface)) ||
          (f1 = neighbor(f, GTS_TRIANGLE(f)->e1, surface))) {
        triangle_barycenter(GTS_TRIANGLE(f1), o);
        return point_locate(o, p, f1, surface, 2);
      }
    } else if (count == 2) {
      if ((f1 = neighbor(f, GTS_TRIANGLE(f)->e3, surface)) || (f1 = neighbor(f, GTS_TRIANGLE(f)->e1, surface)) ||
          (f1 = neighbor(f, GTS_TRIANGLE(f)->e2, surface))) {
        triangle_barycenter(GTS_TRIANGLE(f1), o);
        return point_locate(o, p, f1, surface, 0);
      }
    }
  ...
  if (f)
    gts_triangle_vertices_edges(GTS_TRIANGLE(f), prev, &v1, &v2, &v3, &prev, &e2, &e3);
  ...
  while (f) {
    ...
    if (orient < 0.0) {
      ...
      if ((f1 = neighbor(f, e2, surface)) || (f1 = neighbor(f, e3, surface))) {
        triangle_barycenter(GTS_TRIANGLE(f1), o);
        return point_locate(o, p, f1, surface, count);
      }
      return NULL;
    }
    ...
    if (f) {
      if (prev == GTS_TRIANGLE(f)->e1) {
        ...
        e2 = GTS_TRIANGLE(f)->e3;
        e3 = GTS_TRIANGLE(f)->e1;
      } else {
        e2 = GTS_TRIANGLE(f)->e1;
        e3 = GTS_TRIANGLE(f)->e2;
      }
...
GtsConstraintClass *gts_constraint_class(void) {
  ...
  if (klass == NULL) {
    GtsObjectClassInfo constraint_info = {
        "GtsConstraint",         sizeof(GtsConstraint), sizeof(GtsConstraintClass), (GtsObjectClassInitFunc)NULL,
        (GtsObjectInitFunc)NULL, (GtsArgSetFunc)NULL,   (GtsArgGetFunc)NULL};
    klass = gts_object_class_new(GTS_OBJECT_CLASS(gts_edge_class()), &constraint_info);
  }
...
static void swap_if_in_circle(GtsFace *f1, GtsVertex *v1, GtsVertex *v2, GtsVertex *v3, GtsEdge *e1, GtsEdge *e2,
  ...
  if (GTS_TRIANGLE(f2)->e1 == e1) {
    e4 = GTS_TRIANGLE(f2)->e2;
    e5 = GTS_TRIANGLE(f2)->e3;
  } else if (GTS_TRIANGLE(f2)->e2 == e1) {
    e4 = GTS_TRIANGLE(f2)->e3;
    e5 = GTS_TRIANGLE(f2)->e1;
  } else {
    e4 = GTS_TRIANGLE(f2)->e1;
    e5 = GTS_TRIANGLE(f2)->e2;
  }
  if (GTS_SEGMENT(e4)->v1 == GTS_SEGMENT(e1)->v1 || GTS_SEGMENT(e4)->v1 == GTS_SEGMENT(e1)->v2)
    v4 = GTS_SEGMENT(e4)->v2;
  else
    v4 = GTS_SEGMENT(e4)->v1;

  if (gts_point_in_circle(GTS_POINT(v4), GTS_POINT(v1), GTS_POINT(v2), GTS_POINT(v3)) > 0.0) {
    GtsEdge *en;
...
GtsVertex *gts_delaunay_add_vertex_to_face(GtsSurface *surface, GtsVertex *v, GtsFace *f) {
  ...
  gts_triangle_vertices_edges(GTS_TRIANGLE(f), NULL, &v1, &v2, &v3, &e1, &e2, &e3);
  if (v == v1 || v == v2 || v == v3) /* v already in @surface */
    return NULL;
  if (GTS_POINT(v)->x == GTS_POINT(v1)->x && GTS_POINT(v)->y == GTS_POINT(v1)->y)
    return v1;
  if (GTS_POINT(v)->x == GTS_POINT(v2)->x && GTS_POINT(v)->y == GTS_POINT(v2)->y)
    return v2;
  if (GTS_POINT(v)->x == GTS_POINT(v3)->x && GTS_POINT(v)->y == GTS_POINT(v3)->y)
    return v3;
...
static gboolean polygon_in_circle(GSList *poly, GtsPoint *p1, GtsPoint *p2, GtsPoint *p3) {
  ...
  while (poly) {
    ...
    GtsVertex *v;
    v = s->v1;
    if (v != v1 && v != v2 && v != GTS_VERTEX(p1) && v != GTS_VERTEX(p2) && v != GTS_VERTEX(p3) &&
...
static void triangulate_polygon(GSList *poly, GtsSurface *surface) {
  ...
  while (i && !found) {
    ...
    if (v3 != v1 && gts_point_orientation(GTS_POINT(v1), GTS_POINT(v2), GTS_POINT(v3)) >= 0. &&
        !polygon_in_circle(poly, GTS_POINT(v1), GTS_POINT(v2), GTS_POINT(v3)))
      found = TRUE;
    else
      i = i->next;
  }
  ...
  if (!GTS_IS_EDGE(s2))
    ...
    e2 = GTS_EDGE(s2);
  gts_surface_add_face(surface, gts_face_new(surface->face_class, GTS_EDGE(s), e1, e2));
...
#define NEXT_CUT(edge, edge1, list)                                                                                    \
  {                                                                                                                    \
    ...
    *list = g_slist_prepend(*list, edge1);                                                                             \
    return g_slist_concat(constraint, remove_intersected_edge(s, edge, next, surface, left, right));                   \
  }
...
static GSList *remove_intersected_edge(GtsSegment *s, GtsEdge *e, GtsFace *f, GtsSurface *surface, GSList **left,
                                       GSList **right) {
  GtsVertex *v1, *v2, *v3;
  GtsEdge *e1, *e2;
  gdouble o1, o2;
  ...
  gts_triangle_vertices_edges(GTS_TRIANGLE(f), e, &v1, &v2, &v3, &e, &e1, &e2);

  o1 = gts_point_orientation(GTS_POINT(v2), GTS_POINT(v3), GTS_POINT(s->v2));
  o2 = gts_point_orientation(GTS_POINT(v3), GTS_POINT(v1), GTS_POINT(s->v2));
  ...
  if (o1 >= 0.) {
    ...
    gdouble o3 = gts_point_orientation(GTS_POINT(s->v1), GTS_POINT(s->v2), GTS_POINT(v3));
    if (o3 > 0.)
      NEXT_CUT(e1, e2, left)
    else
      NEXT_CUT(e2, e1, right)
...
static GSList *remove_intersected_vertex(GtsSegment *s, GtsVertex *v, GtsSurface *surface, GSList **left,
                                         GSList **right) {
  ...
  while (i) {
    ...
    if (GTS_IS_FACE(t) && gts_face_has_parent_surface(GTS_FACE(t), surface)) {
      GtsVertex *v1, *v2, *v3;
      gdouble o1, o2;
      ...
      if ((o1 = gts_point_orientation(GTS_POINT(v), GTS_POINT(v2), GTS_POINT(s->v2))) >= 0. &&
          (o2 = gts_point_orientation(GTS_POINT(v3), GTS_POINT(v), GTS_POINT(s->v2))) >= 0.) {
        gdouble o3 = gts_point_orientation(GTS_POINT(v2), GTS_POINT(v3), GTS_POINT(s->v2));
...
GSList *gts_delaunay_add_constraint(GtsSurface *surface, GtsConstraint *c) {
  ...
  constraints = remove_intersected_vertex(GTS_SEGMENT(c), v1, surface, &left, &right);
  ...
  left = g_slist_prepend(left, c);
  {
    ...
    while (i) {
      ...
      fprintf(fp2,
              ...
              s, s->v1, s->v2, GTS_POINT(s->v1)->x, GTS_POINT(s->v1)->y, GTS_POINT(s->v2)->x, GTS_POINT(s->v2)->y);
      i = i->next;
    }
    fprintf(fp2, "}\n");
    fprintf(fp1, "LIST {\n");
...
static void delaunay_check(GtsTriangle *t, gpointer *data) {
  ...
  if (*face == NULL) {
    ...
    while (i && *face == NULL) {
      GtsVertex *v = i->data;
      if (v != v1 && v != v2 && v != v3 &&
          gts_point_in_circle(GTS_POINT(v), GTS_POINT(v1), GTS_POINT(v2), GTS_POINT(v3)) > 0.)
        *face = GTS_FACE(t);
      i = i->next;
    }
    g_slist_free(list);
...
void gts_delaunay_remove_hull(GtsSurface *surface) {
  ...
  while (boundary) {
    ...
    if (!GTS_IS_CONSTRAINT(e)) {
      ...
      if (t != NULL) {
        if (t->e1 != e && !GTS_IS_CONSTRAINT(t->e1) && !gts_edge_is_boundary(t->e1, surface))
          boundary = g_slist_prepend(boundary, t->e1);
        if (t->e2 != e && !GTS_IS_CONSTRAINT(t->e2) && !gts_edge_is_boundary(t->e2, surface))
          boundary = g_slist_prepend(boundary, t->e2);
        if (t->e3 != e && !GTS_IS_CONSTRAINT(t->e3) && !gts_edge_is_boundary(t->e3, surface))
