...
static void bbox_init(GtsBBox *bbox) { bbox->bounded = NULL; }
...
GtsBBoxClass *gts_bbox_class(void) {
  ...
  if (klass == NULL) {
    GtsObjectClassInfo bbox_info = {"GtsBBox",
                                    sizeof(GtsBBox),
                                    sizeof(GtsBBoxClass),
...
GtsBBox *gts_bbox_bboxes(GtsBBoxClass *klass, GSList *bboxes) {
  ...
  bb = bboxes->data;
  bbox = gts_bbox_new(klass, bboxes, bb->x1, bb->y1, bb->z1, bb->x2, bb->y2, bb->z2);
  bboxes = bboxes->next;
  while (bboxes) {
    bb = bboxes->data;
    if (bb->x1 < bbox->x1)
...
#define bbox_volume(bb) (((bb)->x2 - (bb)->x1) * ((bb)->y2 - (bb)->y1) * ((bb)->z2 - (bb)->z1))
...
void gts_bb_tree_traverse_overlapping(GNode *tree1, GNode *tree2, GtsBBTreeTraverseFunc func, gpointer data) {
  ...
  else if (tree2->children == NULL || (tree1->children != NULL && bbox_volume(bb1) > bbox_volume(bb2))) {
...
void gts_bb_tree_triangle_distance(GNode *tree, GtsTriangle *t, GtsBBoxDistFunc distance, gdouble delta,
  ...
  gts_triangle_vertices(t, (GtsVertex **)&p1, (GtsVertex **)&p2, (GtsVertex **)&p3);
  ...
  for (i = 0; i <= n1; i++, t1 += dt1) {
    ...
    for (j = 0; j <= n2; j++, t2 += dt2) {
      ...
      gts_range_add_value(range, gts_bb_tree_point_distance(tree, p, distance, NULL));
    }
  }
 ...
 * maximum value and the standard deviation.
 */
void gts_bb_tree_segment_distance(GNode *tree, GtsSegment *s, gdouble (*distance)(GtsPoint *, gpointer), gdouble delta,
                                  GtsRange *range) {
  GtsPoint *p1, *p2, *p;
  GtsVector p1p2;
  gdouble l, t, dt;
 ...
 * sampled points used.
 */
void gts_bb_tree_surface_distance(GNode *tree, GtsSurface *s, GtsBBoxDistFunc distance, gdouble delta,
                                  GtsRange *range) {
  gpointer data[5];
  ...
  gts_surface_foreach_face(s, (GtsFunc)surface_distance_foreach_triangle, data);

  if (total_area > 0.) {
    range->stddev = sqrt((range->stddev - range->mean * range->mean / total_area) / total_area);
 ...
 */
void gts_bb_tree_surface_boundary_distance(GNode *tree, GtsSurface *s, gdouble (*distance)(GtsPoint *, gpointer),
  ...
  gts_surface_foreach_edge(s, (GtsFunc)surface_distance_foreach_boundary, data);

  if (total_length > 0.) {
    range->stddev = sqrt((range->stddev - range->mean * range->mean / total_length) / total_length);
