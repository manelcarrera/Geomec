...
static void gnode_init(GtsGNode *n) { n->level = 0; }
...
GtsGNodeClass *gts_gnode_class(void) {
  ...
  if (klass == NULL) {
    GtsObjectClassInfo gnode_info = {"GtsGNode",
                                     sizeof(GtsGNode),
                                     ...
                                     (GtsArgSetFunc)NULL,
                                     (GtsArgGetFunc)NULL};
    klass = gts_object_class_new(GTS_OBJECT_CLASS(gts_slist_container_class()), &gnode_info);
  }
...
void gts_gnode_foreach_neighbor(GtsGNode *n, GtsGraph *g, GtsFunc func, gpointer data) {
  ...
  while (i) {
    ...
    if (g == NULL || gts_containee_is_contained(GTS_CONTAINEE(n1), GTS_CONTAINER(g)))
      (*func)(n1, data);
    i = i->next;
  }
}
...
gfloat gts_gnode_move_cost(GtsGNode *n, GtsGraph *src, GtsGraph *dst) {
  ...
  g_return_val_if_fail(gts_containee_is_contained(GTS_CONTAINEE(n), GTS_CONTAINER(src)), G_MAXFLOAT);
  ...
  while (i) {
    ...
    if (gts_containee_is_contained(GTS_CONTAINEE(neighbor), GTS_CONTAINER(src)))
      cost += gts_gedge_weight(ge);
    else if (gts_containee_is_contained(GTS_CONTAINEE(neighbor), GTS_CONTAINER(dst)))
      cost -= gts_gedge_weight(ge);
    i = i->next;
  }
...
static void ngnode_init(GtsNGNode *n) { n->id = 0; }
...
GtsNGNodeClass *gts_ngnode_class(void) {
  ...
  if (klass == NULL) {
    GtsObjectClassInfo ngnode_info = {"GtsNGNode",
                                      sizeof(GtsNGNode),
                                      ...
                                      (GtsArgSetFunc)NULL,
                                      (GtsArgGetFunc)NULL};
    klass = gts_object_class_new(GTS_OBJECT_CLASS(gts_gnode_class()), &ngnode_info);
  }
...
static gfloat wgnode_weight(GtsGNode *n) { return GTS_WGNODE(n)->weight; }

static void wgnode_class_init(GtsWGNodeClass *klass) { GTS_GNODE_CLASS(klass)->weight = wgnode_weight; }

static void wgnode_init(GtsWGNode *n) { n->weight = 1.; }
...
GtsWGNodeClass *gts_wgnode_class(void) {
  ...
  if (klass == NULL) {
    GtsObjectClassInfo wgnode_info = {"GtsWGNode",
                                      sizeof(GtsWGNode),
                                      ...
                                      (GtsArgSetFunc)NULL,
                                      (GtsArgGetFunc)NULL};
    klass = gts_object_class_new(GTS_OBJECT_CLASS(gts_gnode_class()), &wgnode_info);
  }
...
static void fnode_init(GtsFNode *fn) { fn->f = NULL; }
...
GtsFNodeClass *gts_fnode_class(void) {
  ...
  if (klass == NULL) {
    GtsObjectClassInfo fnode_info = {"GtsFNode",
                                     sizeof(GtsFNode),
                                     ...
                                     (GtsArgSetFunc)NULL,
                                     (GtsArgGetFunc)NULL};
    klass = gts_object_class_new(GTS_OBJECT_CLASS(gts_gnode_class()), &fnode_info);
  }
...
static void gedge_remove_container(GtsContainee *i, GtsContainer *c) {
  ...
  if (n1 != NULL && n2 != NULL) {
    if (GTS_CONTAINER(n1) == c) {
      if (n2)
        gts_container_remove(GTS_CONTAINER(n2), i);
    } else if (GTS_CONTAINER(n2) == c) {
      if (n1)
        gts_container_remove(GTS_CONTAINER(n1), i);
    } else
      g_assert_not_reached();
    (*GTS_OBJECT_CLASS(gts_gedge_class())->parent_class->destroy)(GTS_OBJECT(i));
  }
}
...
static void gedge_init(GtsGEdge *object) { object->n1 = object->n2 = NULL; }
...
GtsGEdgeClass *gts_gedge_class(void) {
  ...
  if (klass == NULL) {
    GtsObjectClassInfo gedge_info = {"GtsGEdge",
                                     sizeof(GtsGEdge),
                                     ...
                                     (GtsArgSetFunc)NULL,
                                     (GtsArgGetFunc)NULL};
    klass = gts_object_class_new(GTS_OBJECT_CLASS(gts_containee_class()), &gedge_info);
  }
...
static gfloat wgedge_weight(GtsGEdge *e) { return GTS_WGEDGE(e)->weight; }

static void wgedge_class_init(GtsWGEdgeClass *klass) { GTS_GEDGE_CLASS(klass)->weight = wgedge_weight; }

static void wgedge_init(GtsWGEdge *e) { e->weight = 1.; }
...
GtsWGEdgeClass *gts_wgedge_class(void) {
  ...
  if (klass == NULL) {
    GtsObjectClassInfo wgedge_info = {"GtsWGEdge",
                                      sizeof(GtsWGEdge),
                                      ...
                                      (GtsArgSetFunc)NULL,
                                      (GtsArgGetFunc)NULL};
    klass = gts_object_class_new(GTS_OBJECT_CLASS(gts_gedge_class()), &wgedge_info);
  }
 ...
 */
GtsWGEdge *gts_wgedge_new(GtsWGEdgeClass *klass, GtsGNode *g1, GtsGNode *g2, gfloat weight) {
  GtsWGEdge *we;
...
static void graph_class_init(GtsGraphClass *klass) { klass->weight = NULL; }
...
GtsGraphClass *gts_graph_class(void) {
  ...
  if (klass == NULL) {
    GtsObjectClassInfo graph_info = {"GtsGraph",
                                     sizeof(GtsGraph),
                                     ...
                                     (GtsArgSetFunc)NULL,
                                     (GtsArgGetFunc)NULL};
    klass = gts_object_class_new(GTS_OBJECT_CLASS(gts_hash_container_class()), &graph_info);
  }
...
void gts_graph_print_stats(GtsGraph *g, FILE *fp) {
  ...
  fprintf(fp, "# nodes: %lld weight: %g\n", gts_container_size(GTS_CONTAINER(g)), gts_graph_weight(g));
  fprintf(fp, "#   degree: ");
  ...
  fprintf(fp, "\n");
  fprintf(fp, "#   edges cut: %lld edges cut weight: %g\n", gts_graph_edges_cut(g), gts_graph_edges_cut_weight(g));
}
...
static void reset_level(GtsGNode *n) { n->level = 0; }
...
GtsGraphTraverse *gts_graph_traverse_new(GtsGraph *g, GtsGNode *n, GtsTraverseType type, gboolean reinit) {
  ...
  g_return_val_if_fail(gts_containee_is_contained(GTS_CONTAINEE(n), GTS_CONTAINER(g)), NULL);
...
GtsGNode *gts_graph_farthest(GtsGraph *g, GSList *gnodes) {
  ...
  i = gnodes;
  while (i) {
    GTS_OBJECT(i->data)->reserved = gts_graph_traverse_new(g, i->data, GTS_BREADTH_FIRST, reinit);
    reinit = FALSE;
    i = i->next;
  }
...
gfloat gts_graph_edges_cut_weight(GtsGraph *g) {
  ...
  data[0] = &weight;
  data[1] = g;
  gts_container_foreach(GTS_CONTAINER(g), (GtsFunc)sum_edge_cuts_weight, data);
...
static gfloat wgraph_weight(GtsGraph *g) { return GTS_WGRAPH(g)->weight; }
...
static void wgraph_init(GtsWGraph *g) { g->weight = 0.; }
...
GtsWGraphClass *gts_wgraph_class(void) {
  ...
  if (klass == NULL) {
    GtsObjectClassInfo wgraph_info = {"GtsWGraph",
                                      sizeof(GtsWGraph),
                                      ...
                                      (GtsArgSetFunc)NULL,
                                      (GtsArgGetFunc)NULL};
    klass = gts_object_class_new(GTS_OBJECT_CLASS(gts_graph_class()), &wgraph_info);
  }
    ...
    if (GTS_IS_FACE(f) && gts_face_has_parent_surface(f, s)) {
      ...
      while (j) {
        ...
        if (GTS_IS_FACE(f1) && gts_face_has_parent_surface(f1, s))
          gts_gedge_new(gts_gedge_class(), GTS_OBJECT(f)->reserved, GTS_OBJECT(f1)->reserved);
        j = j->next;
      }
    }
    i = i->next;
...
GtsSurface *gts_surface_graph_surface(GtsGraph *surface_graph, GtsSurface *s) {
  ...
  s1 = gts_surface_new(GTS_SURFACE_CLASS(GTS_OBJECT(s)->klass), s->face_class, s->edge_class, s->vertex_class);
  gts_container_foreach(GTS_CONTAINER(surface_graph), (GtsFunc)add_to_surface, s1);
  return s1;
}
