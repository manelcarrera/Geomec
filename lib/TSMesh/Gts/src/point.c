...
GtsPointClass *gts_point_class(void) {
  ...
  if (klass == NULL) {
    GtsObjectClassInfo point_info = {"GtsPoint",
                                     sizeof(GtsPoint),
...
gdouble gts_point_orientation_3d(GtsPoint *p1, GtsPoint *p2, GtsPoint *p3, GtsPoint *p4) {
  g_return_val_if_fail(p1 != NULL && p2 != NULL && p3 != NULL && p4 != NULL, 0.0);
#ifdef USE_ROBUST_PREDICATES
  return orient3d((gdouble *)&p1->x, (gdouble *)&p2->x, (gdouble *)&p3->x, (gdouble *)&p4->x);
#else
  {
    gdouble adx, bdx, cdx;
    gdouble ady, bdy, cdy;
...
#ifdef USE_ROBUST_PREDICATES
  return incircle((gdouble *)&p1->x, (gdouble *)&p2->x, (gdouble *)&p3->x, (gdouble *)&p->x);
#else
  {
...
gdouble gts_point_in_circle(GtsPoint *p, GtsPoint *p1, GtsPoint *p2, GtsPoint *p3) {
  g_return_val_if_fail(p != NULL && p1 != NULL && p2 != NULL && p3 != NULL, 0.0);
...
gdouble gts_point_segment_distance2(GtsPoint *p, GtsSegment *s) {
  ...
  t = ((p2->x - p1->x) * (p->x - p1->x) + (p2->y - p1->y) * (p->y - p1->y) + (p2->z - p1->z) * (p->z - p1->z)) / ns2;
  if (t > 1.0)
...
void gts_point_segment_closest(GtsPoint *p, GtsSegment *s, GtsPoint *closest) {
  ...
  if (t > 1.0)
    ...
    gts_point_set(closest, (1. - t) * p1->x + t * p2->x, (1. - t) * p1->y + t * p2->y, (1. - t) * p1->z + t * p2->z);
}
...
gdouble gts_point_triangle_distance2(GtsPoint *p, GtsTriangle *t) {
  ...
  gts_triangle_vertices_edges(t, NULL, (GtsVertex **)&p1, (GtsVertex **)&p2, (GtsVertex **)&p3, &e1, &e2, &e3);
...
void gts_point_triangle_closest(GtsPoint *p, GtsTriangle *t, GtsPoint *closest) {
  ...
  det = B * B - E * C;
  if (det == 0.) { /* p1p2 and p1p3 are colinear */
    GtsPoint *cp = GTS_POINT(gts_object_new(GTS_OBJECT_CLASS(gts_point_class())));
  ...
  if (t1 < 0.)
    ...
    gts_point_set(closest, p1->x + t1 * p1p2[0] + t2 * p1p3[0], p1->y + t1 * p1p2[1] + t2 * p1p3[1],
...
GtsPoint *gts_segment_triangle_intersection(GtsSegment *s, GtsTriangle *t, gboolean boundary, GtsPointClass *klass) {
  ...
  gts_point_set(I, E->x + c * (D->x - E->x), E->y + c * (D->y - E->y), E->z + c * (D->z - E->z));
  return I;
}
...
gdouble gts_point_orientation(GtsPoint *p1, GtsPoint *p2, GtsPoint *p3) {
  ...
  return orient2d((gdouble *)&p1->x, (gdouble *)&p2->x, (gdouble *)&p3->x);
#else
