...
static void segment_destroy(GtsObject *object) {
  ...
  if (!GTS_OBJECT_DESTROYED(v1) && !gts_allow_floating_vertices && v1->segments == NULL)
    gts_object_destroy(GTS_OBJECT(v1));

  v2->segments = g_slist_remove(v2->segments, segment);
  if (!GTS_OBJECT_DESTROYED(v2) && !gts_allow_floating_vertices && v2->segments == NULL)
    gts_object_destroy(GTS_OBJECT(v2));

  (*GTS_OBJECT_CLASS(gts_segment_class())->parent_class->destroy)(object);
}

static void segment_class_init(GtsObjectClass *klass) { klass->destroy = segment_destroy; }

static void segment_init(GtsSegment *segment) { segment->v1 = segment->v2 = NULL; }
...
GtsSegmentClass *gts_segment_class(void) {
  ...
  if (klass == NULL) {
    GtsObjectClassInfo segment_info = {"GtsSegment",
                                       sizeof(GtsSegment),
...
GtsIntersect gts_segments_are_intersecting(GtsSegment *s1, GtsSegment *s2) {
  ...
  g_return_val_if_fail(s1 != NULL && s2 != NULL, GTS_ON);
...
GtsVertex *gts_segment_midvertex(GtsSegment *s, GtsVertexClass *klass) {
  ...
  p1 = GTS_POINT(s->v1);
  p2 = GTS_POINT(s->v2);
  return gts_vertex_new(klass, (p1->x + p2->x) / 2., (p1->y + p2->y) / 2., (p1->z + p2->z) / 2.);
}
