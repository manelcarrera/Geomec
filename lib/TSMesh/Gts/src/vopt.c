...
static void triangle_normal(GtsTriangle *t, gdouble *nx, gdouble *ny, gdouble *nz, gdouble *nt) {
  ...
  *nt = ((p1->y * p2->z - p1->z * p2->y) * p3->x + (p1->z * p2->x - p1->x * p2->z) * p3->y +
...
static gdouble edge_boundary_cost(GtsEdge *e, GtsVertex *v) {
  ...
  while (i) {
    GtsFace *f;
    if (GTS_IS_EDGE(i->data) && (f = gts_edge_is_boundary(i->data, NULL)))
  ...
  while (i) {
    GtsFace *f;
    if (i->data != e && GTS_IS_EDGE(i->data) && (f = gts_edge_is_boundary(i->data, NULL)))
      cost += boundary_cost(i->data, f, v);
    i = i->next;
  }
...
static gdouble edge_volume_cost(GtsEdge *e, GtsVertex *v) {
  ...
  while (i) {
    if (GTS_IS_FACE(i->data)) {
      ...
      a = GTS_POINT(v)->x * n1 + GTS_POINT(v)->y * n2 + GTS_POINT(v)->z * n3 - nt;
      cost += a * a;
    }
    i = i->next;
  }
...
static gdouble edge_shape_cost(GtsEdge *e, GtsVertex *v) {
  GSList *list, *i;
  GtsVertex *v1 = GTS_SEGMENT(e)->v1, *v2 = GTS_SEGMENT(e)->v2;
  gdouble cost = 0.;
...
GtsVertex *gts_volume_optimized_vertex(GtsEdge *edge, GtsVertexClass *klass, GtsVolumeOptimizedParams *params) {
  ...
  while (i) {
    ...
    GtsFace *f;
    if (GTS_IS_EDGE(edge1) && (f = gts_edge_is_boundary(edge1, NULL))) {
  ...
  while (i) {
    GtsEdge *edge1 = i->data;
    GtsFace *f;
    if (edge1 != edge && GTS_IS_EDGE(edge1) && (f = gts_edge_is_boundary(edge1, NULL))) {
      boundary_preservation(edge1, f, e1, e2, Hb, cb);
      nb++;
    }
    i = i->next;
  ...
  if (nb > 0) {
    GtsMatrix *H = gts_matrix_new(e1[2] * e1[2] + e1[1] * e1[1], -e1[0] * e1[1], -e1[0] * e1[2], -e1[0] * e1[1],
                                  e1[2] * e1[2] + e1[0] * e1[0], -e1[1] * e1[2], -e1[0] * e1[2], -e1[1] * e1[2],
                                  e1[1] * e1[1] + e1[0] * e1[0]);
    GtsVector c = {e1[1] * e2[2] - e1[2] * e2[1], e1[2] * e2[0] - e1[0] * e2[2], e1[0] * e2[1] - e1[1] * e2[0]};
  ...
  if (n < 3) {
    ...
    GtsMatrix *H = gts_matrix_new(sn11, sn12, sn13, sn12, sn22, sn23, sn13, sn23, sn33);
    GtsVector c = {-stn1, -stn2, -stn3};
    gdouble le = 9. * params->boundary_weight * gts_point_distance2(GTS_POINT(v1), GTS_POINT(v2));
    guint i, j;
  ...
  if (n < 3) {
    ...
    H = gts_matrix_new(nv, 0., 0., 0., nv, 0., 0., 0., nv);
    n = gts_matrix_quadratic_optimization(A, b, n, H, c);
    gts_matrix_destroy(H);
  }
  ...
  if (n < 3) {
    ...
    GtsMatrix *H;
    GtsVector c;
    gdouble l2 = gts_point_distance2(GTS_POINT(v1), GTS_POINT(v2));
    ...
    H = gts_matrix_new(wv * sn11 + wb * Hb[0][0] + ws * nv, wv * sn12 + wb * Hb[0][1], wv * sn13 + wb * Hb[0][2],
                       wv * sn12 + wb * Hb[1][0], wv * sn22 + wb * Hb[1][1] + ws * nv, wv * sn23 + wb * Hb[1][2],
                       wv * sn13 + wb * Hb[2][0], wv * sn23 + wb * Hb[2][1], wv * sn33 + wb * Hb[2][2] + ws * nv);
  ...
  v = gts_vertex_new(klass, Ai[0][0] * b[0] + Ai[0][1] * b[1] + Ai[0][2] * b[2],
...
gdouble gts_volume_optimized_cost(GtsEdge *e, GtsVolumeOptimizedParams *params) {
  ...
  length2 = gts_point_distance2(GTS_POINT(GTS_SEGMENT(e)->v1), GTS_POINT(GTS_SEGMENT(e)->v2));
  cost = params->volume_weight * edge_volume_cost(e, v) + params->boundary_weight * length2 * edge_boundary_cost(e, v) +
