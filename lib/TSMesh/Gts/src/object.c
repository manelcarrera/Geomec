...
#ifdef DEBUG_LEAKS
  fprintf(stderr, "destroy %s %p->%d\n", object->klass->name, object, id(object));
#endif
  id_remove(object);
#endif
  g_free(object);
...
GtsObjectClass *gts_object_class(void) {
  ...
  if (klass == NULL) {
    GtsObjectClassInfo object_info = {"GtsObject",
                                      sizeof(GtsObject),
...
gpointer gts_object_check_cast(gpointer object, gpointer klass) {
  if (!object) {
    g_warning("invalid cast from (NULL) pointer to `%s'", GTS_OBJECT_CLASS(klass)->info.name);
    return object;
  }
  if (!((GtsObject *)object)->klass) {
    g_warning("invalid unclassed pointer in cast to `%s'", GTS_OBJECT_CLASS(klass)->info.name);
    return object;
  }
  if (!gts_object_is_from_class(object, klass)) {
    g_warning("invalid cast from `%s' to `%s'", ((GtsObject *)object)->klass->info.name,
...
gpointer gts_object_class_check_cast(gpointer klass, gpointer from) {
  if (!klass) {
    g_warning("invalid cast from (NULL) pointer to `%s'", GTS_OBJECT_CLASS(from)->info.name);
    return klass;
  }
  if (!gts_object_class_is_from_class(klass, from)) {
    g_warning("invalid cast from `%s' to `%s'", GTS_OBJECT_CLASS(klass)->info.name, GTS_OBJECT_CLASS(from)->info.name);
    return klass;
  }
  return klass;
}
...
GtsObject *gts_object_new(GtsObjectClass *klass) {
  ...
  id_insert(object);
#ifdef DEBUG_LEAKS
  fprintf(stderr, "new %s %p->%d\n", objclass->info.name, object, id(object));
#endif
#endif
...
GtsObject *gts_object_clone(GtsObject *object) {
  ...
  id_insert(clone);
#ifdef DEBUG_LEAKS
  fprintf(stderr, "clone %s %p->%d\n", clone->klass->info.name, clone, id(clone));
#endif
#endif
