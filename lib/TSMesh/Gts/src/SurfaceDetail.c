...
int PointInPoly(int npol, double *xp, double *yp, double x, double y, int IncLines) {
  ...
  if (c1 != c2)
    return IncLines; // point is on a line
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// COARSENING////////////////////////////////////////////////////////////////////////////////////////////////////////////
static GtsVertex *EdgeCollapse(GtsEdge *e, GtsEHeap *heap, GtsCoarsenFunc coarsen_func, gpointer coarsen_data,
...
static void CreateHeapCoarsen(GtsEdge *e, GtsEHeap *heap) { HEAP_INSERT_EDGE(heap, e); }

// An implementation of the stop function...//////////////////////////////////
gboolean CoarsenStopFunc(gdouble cost, guint nedge, gpointer data) {
  UNUSED(nedge);
...
void RegionCoarsen(GtsSurface *surface, GtsKeyFunc cost_func, gpointer cost_data, GtsCoarsenFunc coarsen_func,
                   gpointer coarsen_data, GtsStopFunc stop_func, GSList *regions_list, gdouble minangle) {
  GtsEHeap *heap;
  GtsEdge *e;
  GtsVertex *v;
  gdouble top_cost;
  ...
  while ((e = gts_eheap_remove_top(heap, &top_cost)) && (top_cost < G_MAXDOUBLE)) {
    if ((region_temp = FindRegionCoarsen(e, heap, regions_list, surface)) &&
        !(*stop_func)(top_cost, gts_eheap_size(heap) - gts_edge_face_number(e, surface), &(region_temp->stop_data))) {
      v = EdgeCollapse(e, heap, coarsen_func, coarsen_data, surface->vertex_class, maxcosine2);
...
static void CreateHeapRefine(GtsEdge *e, GtsHeap *heap) { gts_heap_insert(heap, e); }
...
gboolean RefineStopFunc(gdouble cost, guint nedge, gpointer data) {
  UNUSED(nedge);
...
void RegionRefine(GtsSurface *surface, GCompareFunc compare_func, GtsStopFunc stop_func, GSList *regions_list) {
  GtsHeap *heap;
  GtsEdge *e;
  Region *region_temp;
  ...
  while ((e = gts_heap_remove_top(heap))) {
    if ((region_temp = FindRegionRefine(e, regions_list)) &&
        !(*stop_func)(gts_point_distance(GTS_POINT(GTS_SEGMENT(e)->v1), GTS_POINT(GTS_SEGMENT(e)->v2)),
                      gts_heap_size(heap) + gts_edge_face_number(e, surface) + 2, &(region_temp->stop_data))) {
...
GtsSurface *SurfaceDetail(GtsSurface *surface, GtsKeyFunc cost_func_coarsen, gpointer cost_data_coarsen,
                          GtsCoarsenFunc coarsen_func, gpointer coarsen_data, GtsStopFunc stop_func_coarsen,
                          gdouble minangle_coarsen, GCompareFunc compare_func_refine, GtsStopFunc stop_func_refine,
                          GSList *regions_list) {
  GtsSurface *surfacetemp;
  surfacetemp = gts_surface_new(gts_surface_class(), gts_face_class(), gts_edge_class(), gts_vertex_class());

  gts_surface_copy(surfacetemp, surface);

  RegionRefine(surfacetemp, compare_func_refine, stop_func_refine, regions_list);
  RegionCoarsen(surfacetemp, cost_func_coarsen, cost_data_coarsen, coarsen_func, coarsen_data, stop_func_coarsen,
                regions_list, minangle_coarsen);
