...
void g_warning(const char *txt, ...) { UNUSED(txt); }
...
void g_slist_free_1(GSList *list) { free(list); }
...
void g_list_free_1(GList *list) { free(list); }
...
#define g_array_elt_zero(array, pos, len) (memset(g_array_elt_pos((array), pos), 0, g_array_elt_len((array), len)))
...
static void g_array_maybe_expand(GRealArray *array, gint len) {
  guint want_alloc = g_array_elt_len(array, array->len + len + array->zero_terminated);
...
GArray *g_array_append_vals(GArray *farray, gconstpointer data, guint len) {
  ...
  memcpy(g_array_elt_pos(array, array->len), data, g_array_elt_len(array, len));
...
static guint g_direct_hash(gconstpointer v) { return GPOINTER_TO_UINT(v); }
...
static void g_hash_node_destroy(GHashNode *hash_node, GDestroyNotify key_destroy_func,
                                GDestroyNotify value_destroy_func) {
  if (key_destroy_func)
    key_destroy_func(hash_node->key);
  if (value_destroy_func)
...
static guint g_hash_table_foreach_remove_or_steal(GHashTable *hash_table, GHRFunc func, gpointer user_data,
                                                  gboolean notify) {
  GHashNode *node, *prev;
  gint i;
  guint deleted = 0;
  ...
  restart:
    ...
    for (node = hash_table->nodes[i]; node; prev = node, node = node->next) {
      if ((*func)(node->key, node->value, user_data)) {
        ...
        if (prev) {
          prev->next = node->next;
          g_hash_node_destroy(node, notify ? hash_table->key_destroy_func : NULL,
...
GHashTable *g_hash_table_new_full(GHashFunc hash_func, GEqualFunc key_equal_func, GDestroyNotify key_destroy_func,
                                  GDestroyNotify value_destroy_func) {
...
void g_hash_table_remove_all(GHashTable *hash_table) {
  ...
  for (i = 0; i < hash_table->size; i++) {
    g_hash_nodes_destroy(hash_table->nodes[i], hash_table->key_destroy_func, hash_table->value_destroy_func);
...
gboolean g_hash_table_remove(GHashTable *hash_table, gconstpointer key) {
  ...
  if (*node) {
    dest = *node;
    (*node) = dest->next;
    g_hash_node_destroy(dest, hash_table->key_destroy_func, hash_table->value_destroy_func);
    hash_table->nnodes--;
