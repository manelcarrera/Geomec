...
void gts_graph_partition_print_stats(GSList *partition, FILE *fp) {
  ...
  fprintf(fp,
          ...
          "#   weight: ",
          g_slist_length(partition), gts_graph_partition_edges_cut(partition),
...
GSList *gts_graph_partition_clone(GSList *partition) {
  ...
  while (partition) {
    cparts = g_slist_prepend(cparts, gts_object_clone(GTS_OBJECT(partition->data)));
    partition = partition->next;
  }
  return cparts;
}
...
static void partition_update(GSList *list, GtsGraph *g) {
  ...
  while (i) {
    ...
    GTS_OBJECT(seed)->reserved = gts_graph_traverse_new(g, seed, GTS_BREADTH_FIRST, reinit);
    reinit = FALSE;
    i = i->next;
  }
...
GSList *gts_graph_bubble_partition(GtsGraph *g, guint np, guint niter, GtsFunc step_info, gpointer data) {
  ...
  info[1] = g;
  info[2] = &min;
  gts_container_foreach(GTS_CONTAINER(g), (GtsFunc)find_smallest_degree, info);
  if (seed == NULL)
    return NULL;
...
static gdouble degree_cost(GtsGNode *n, GtsGraph *g) { return (gdouble)gts_gnode_degree(n, g); }

static void add_seed(GtsGNode *n, GtsEHeap *heap) { gts_eheap_insert(heap, n); }
...
static void boundary_node1(GtsGNode *n, GtsGraphBisection *bg) {
  ...
  while (i) {
    ...
    if (gts_containee_is_contained(GTS_CONTAINEE(n1), GTS_CONTAINER(bg->g2))) {
...
static void boundary_node2(GtsGNode *n, GtsGraphBisection *bg) {
  ...
  while (i) {
    GtsGNode *n1 = GTS_GNODE_NEIGHBOR(n, i->data);
    if (gts_containee_is_contained(GTS_CONTAINEE(n1), GTS_CONTAINER(bg->g1))) {
...
static void check_bg(GtsGNode *n, gpointer *data) {
  ...
  if (nn > 0)
    (*nb)++;
  if ((nn > 0 && !g_hash_table_lookup(bg, n)) || (nn == 0 && g_hash_table_lookup(bg, n))) {
...
GtsGraphBisection *gts_graph_ggg_bisection(GtsGraph *g, guint ntry) {
  ...
  while (ntry && ((seed = gts_eheap_remove_top(degree_heap, NULL)))) {
    ...
    g1 = gts_graph_new(GTS_GRAPH_CLASS(GTS_OBJECT(g)->klass), g->node_class, g->edge_class);
    g2 = gts_graph_new(GTS_GRAPH_CLASS(GTS_OBJECT(g)->klass), g->node_class, g->edge_class);
...
#ifdef DEBUG
  fprintf(stderr, "bestcost: %5g g1: %5g|%5d g2: %5g|%5d\n", bestcost, gts_graph_weight(bestg1),
          gts_container_size(GTS_CONTAINER(bestg1)), gts_graph_weight(bestg2),
...
GtsGraphBisection *gts_graph_bfgg_bisection(GtsGraph *g, guint ntry) {
  ...
  while (ntry && ((seed = gts_eheap_remove_top(degree_heap, NULL)))) {
    ...
    GtsGNode *n;
    gdouble cost;
    GtsGraphTraverse *t = gts_graph_traverse_new(g, seed, GTS_BREADTH_FIRST, TRUE);
...
static gdouble node_move_cost1(GtsGNode *n, GtsGraphBisection *bg) { return gts_gnode_move_cost(n, bg->g1, bg->g2); }

static gdouble node_move_cost2(GtsGNode *n, GtsGraphBisection *bg) { return gts_gnode_move_cost(n, bg->g2, bg->g1); }

static void build_heap(GtsGNode *n, GtsEHeap *heap) { GTS_OBJECT(n)->reserved = gts_eheap_insert(heap, n); }
...
gdouble gts_graph_bisection_kl_refine(GtsGraphBisection *bg, guint mmax) {
  ...
  do {
    ...
    if (n) {
      ...
      while (i) {
        GtsGNode *n1 = GTS_GNODE_NEIGHBOR(n, i->data);
        if (GTS_OBJECT(n1)->reserved && gts_containee_is_contained(GTS_CONTAINEE(n1), GTS_CONTAINER(bg->g))) {
          GtsEHeap *h = gts_containee_is_contained(GTS_CONTAINEE(n1), GTS_CONTAINER(bg->g1)) ? h1 : h2;
  ...
  for (i = 0; i < nm; i++) {
    GtsGNode *n = moves[i];
    GtsGraph *g1 = gts_containee_is_contained(GTS_CONTAINEE(n), GTS_CONTAINER(bg->g1)) ? bg->g1 : bg->g2;
...
static void update_neighbors(GtsGNode *n, GtsGraphBisection *bg, GtsEHeap *h1, GtsEHeap *h2) {
  ...
  while (i) {
    GtsGNode *n1 = GTS_GNODE_NEIGHBOR(n, i->data);
    if (gts_containee_is_contained(GTS_CONTAINEE(n1), GTS_CONTAINER(bg->g))) {
      GtsEHeap *h;
      GtsGraph *g2;
      GHashTable *bg1;
...
gdouble gts_graph_bisection_bkl_refine(GtsGraphBisection *bg, guint mmax) {
  ...
  gts_container_foreach(GTS_CONTAINER(bg->g), (GtsFunc)gts_object_reset_reserved, NULL);
  gts_eheap_destroy(h1);
  gts_eheap_destroy(h2);
  ...
  for (i = 0; i < nm; i++) {
    ...
    if (gts_containee_is_contained(GTS_CONTAINEE(n), GTS_CONTAINER(bg->g1))) {
      g1 = bg->g1;
      g2 = bg->g2;
      bg1 = bg->bg1;
      bg2 = bg->bg2;
...
static void bisection_children(GtsGNodeSplit *ns, GtsGraphBisection *bg) {
  ...
  if (gts_containee_is_contained(GTS_CONTAINEE(ns->n), GTS_CONTAINER(bg->g1))) {
    g = bg->g1;
    g1 = bg->g2;
    bbg = bg->bg1;
  } else {
...
GtsGraphBisection *gts_graph_bisection_new(GtsWGraph *wg, guint ntry, guint mmax, guint nmin) {
  ...
  g = GTS_GRAPH(wg);
  pg = gts_pgraph_new(gts_pgraph_class(), g, gts_gnode_split_class(), gts_wgnode_class(), gts_wgedge_class(), nmin);
  ...
  fprintf(stderr, "before size: %5d weight: %5g cuts: %5d cweight: %5g\n", gts_container_size(GTS_CONTAINER(bg->g1)),
          gts_graph_weight(bg->g1), gts_graph_edges_cut(bg->g1), gts_graph_edges_cut_weight(bg->g1));
...
static void recursive_bisection(GtsWGraph *wg, guint np, guint ntry, guint mmax, guint nmin, GSList **list) {
  if (np == 0)
