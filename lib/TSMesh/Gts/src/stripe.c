...
#define HEAP_INSERT_TRIANGLE(h, t) (GTS_OBJECT(t)->reserved = gts_eheap_insert(h, t))

static gdouble triangle_neighbors(GtsTriangle *t, GtsSurface *s) {
  return (gdouble)gts_face_neighbor_number(GTS_FACE(t), s);
}

static void create_triangle_heap(GtsTriangle *t, GtsEHeap *heap) { HEAP_INSERT_TRIANGLE(heap, t); }
...
static GtsTriangle *most_isolated_neighbor(GtsTriangle *t, GtsEHeap *heap) {
