...
GtsClusterClass *gts_cluster_class(void) {
  ...
  if (klass == NULL) {
    GtsObjectClassInfo cluster_info = {"GtsCluster",
                                       sizeof(GtsCluster),
...
static void cluster_grid_destroy(GtsObject *object) {
  ...
  g_hash_table_foreach(cluster_grid->clusters, (GHFunc)destroy_cluster, NULL);
  g_hash_table_destroy(cluster_grid->clusters);

  (*GTS_OBJECT_CLASS(gts_cluster_grid_class())->parent_class->destroy)(object);
}
...
static void cluster_grid_init(GtsClusterGrid *cluster_grid) {
  ...
  cluster_grid->clusters = g_hash_table_new(cluster_id_hash, cluster_id_equal);
}
...
GtsClusterGridClass *gts_cluster_grid_class(void) {
  ...
  if (klass == NULL) {
    GtsObjectClassInfo cluster_grid_info = {"GtsClusterGrid",
...
GtsClusterGrid *gts_cluster_grid_new(GtsClusterGridClass *klass, GtsClusterClass *cluster_class, GtsSurface *s,
  ...
  g_return_val_if_fail(size[0] <= 2. * G_MAXINT + 2. && size[1] <= 2. * G_MAXINT + 2. && size[2] <= 2. * G_MAXINT + 2.,
                       NULL);
  cluster_grid = GTS_CLUSTER_GRID(gts_object_new(GTS_OBJECT_CLASS(klass)));
...
static GtsClusterId cluster_index(GtsPoint *p, GtsBBox *bb, GtsVector n) {
  ...
  // TNO id initialized here in order to supress warning during compiling...
  id.x = 0;
  id.y = 0;
  id.z = 0;
  // ***********************************************************************
...
static GtsCluster *cluster_grid_add_point(GtsClusterGrid *cluster_grid, GtsPoint *p, gpointer data) {
  GtsClusterId id = cluster_index(p, cluster_grid->bbox, cluster_grid->size);
  GtsCluster *c = g_hash_table_lookup(cluster_grid->clusters, &id);

  if (c == NULL) {
    c = gts_cluster_new(cluster_grid->cluster_class, id, cluster_grid->surface->vertex_class);
...
void gts_cluster_grid_add_triangle(GtsClusterGrid *cluster_grid, GtsPoint *p1, GtsPoint *p2, GtsPoint *p3,
  ...
  if (c1 != c2 && c2 != c3 && c3 != c1) {
    ...
    if (new_edge || !gts_triangle_use_edges(e1, e2, e3))
      gts_surface_add_face(cluster_grid->surface, gts_face_new(cluster_grid->surface->face_class, e1, e2, e3));
  }
}
...
static void update_cluster(gint *id, GtsCluster *cluster, GtsRange *stats) {
  ...
  gts_range_add_value(stats, (gdouble)cluster->n);
}
...
GtsRange gts_cluster_grid_update(GtsClusterGrid *cluster_grid) {
  ...
  g_hash_table_foreach(cluster_grid->clusters, (GHFunc)update_cluster, &stats);
