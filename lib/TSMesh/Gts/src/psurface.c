...
static void psurface_class_init(GtsObjectClass *klass) { klass->destroy = psurface_destroy; }
...
GtsPSurfaceClass *gts_psurface_class(void) {
  ...
  if (klass == NULL) {
    GtsObjectClassInfo psurface_info = {"GtsPSurface",
                                        sizeof(GtsPSurface),
                                        ...
                                        (GtsArgSetFunc)NULL,
                                        (GtsArgGetFunc)NULL};
    klass = gts_object_class_new(gts_object_class(), &psurface_info);
  }

  return klass;
}

static GtsVertex *edge_collapse(GtsPSurface *ps, GtsEdge *e, GtsEHeap *heap, GtsCoarsenFunc coarsen_func,
                                gpointer coarsen_data, gdouble maxcosine2) {
...
static void create_heap_coarsen(GtsEdge *e, GtsEHeap *heap) { HEAP_INSERT_OBJECT(heap, e); }
...
static void check_fold(GtsTriangle *t, gdouble *maxcosine2) {
  ...
  if (gts_triangles_are_folded(e1->triangles, GTS_SEGMENT(e1)->v1, GTS_SEGMENT(e1)->v2, *maxcosine2) ||
      gts_triangles_are_folded(e2->triangles, GTS_SEGMENT(e2)->v1, GTS_SEGMENT(e2)->v2, *maxcosine2) ||
      gts_triangles_are_folded(e3->triangles, GTS_SEGMENT(e3)->v1, GTS_SEGMENT(e3)->v2, *maxcosine2)) {
...
GtsPSurface *gts_psurface_new(GtsPSurfaceClass *klass, GtsSurface *surface, GtsSplitClass *split_class,
  ...
  while ((e = gts_eheap_remove_top(heap, &top_cost)) && (top_cost < G_MAXDOUBLE) &&
    ...
    GtsVertex *v = edge_collapse(psurface, e, heap, coarsen_func, coarsen_data, maxcosine2);
    if (v != NULL) {
      ...
      if (gts_vertex_is_contact(v, FALSE) != 1) {
        ...
        fprintf(fptr, "(geometry \"sphere\" { = SPHERE 0.1 0. 0. 0. })\n"
