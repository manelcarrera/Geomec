...
public:
	...
	public:
		CTSSurfaceStat(const double &SurfaceArea,const double &FaceAreaMean, const double &EdgeLengthMean, const double &EdgeAngleMean)
		:	m_dSurfaceArea(SurfaceArea),
			m_dFaceAreaMean(FaceAreaMean),
			m_dEdgeLengthMean(EdgeLengthMean),
	...
	// use a point cloud
	CTSSurface(const geo::CPtrArray<geo::IPoint> &points, ITSProgressBase &progress, const geo::CPolygon &Polygon, bool UsePolyPoints = false);
	...
	CTSSurface(const std::vector<geo::CPoint>& vcPoint,
...
private:
	...
	void InsertExtensionPoints(const std::vector<GtsVertex*> &vcBoundary,
	...
	void ProcessEdgeRing(std::vector<GtsVertex*> &vcVertices, const TVertexMap &mpVertices,
