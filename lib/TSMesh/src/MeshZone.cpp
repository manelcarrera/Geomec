...
CMeshZone::CMeshZone() : m_pMateRegion(0), m_dEdgeSquared(0) { assert(m_vcElementPoints.size() == 0); }
...
CMeshZone::~CMeshZone() { m_vcElementPoints.clear(); }

const geo::IPoint &CMeshZone::Point(const int nIndex) const { return m_vcElementPoints[nIndex]; }
...
int CMeshZone::NrOfPoints() const { return m_vcElementPoints.size(); }
