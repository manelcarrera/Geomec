...
GtsEdge *CTSSurface::InsertEdge(TIndexToEdgeMap &mpEdge, int nFirst, int nSecond) {
  ...
  if (it == mpEdge.end()) {
    GtsEdge *pEdge = gts_edge_new(gts_edge_class(), m_vcNodes[nFirst]->Vertex(), m_vcNodes[nSecond]->Vertex());
...
void CTSSurface::ConstructUsingFaces(const std::vector<const geo::IFace *> &vcFace, ITSProgressBase &progress) {
  ...
  m_pGtsSurface = gts_surface_new(gts_surface_class(), gts_face_class(), gts_edge_class(), gts_vertex_class());
  ...
  for (size_t i = 0; i < vcFace.size(); i++) {
    ...
    for (int nPoint = 0; nPoint < 3; nPoint++) {
      ...
      if (it == m_mpPoint.end()) {
        ...
        GtsVertex *pVertex = gts_vertex_new(gts_vertex_class(), (gdouble)ptCurrent->X(), (gdouble)ptCurrent->Y(),
                                            (gdouble)ptCurrent->Z());
    ...
    GtsFace *f = gts_face_new(gts_face_class(), InsertEdge(mpEdge, vcPoint[0], vcPoint[1]),
                              InsertEdge(mpEdge, vcPoint[1], vcPoint[2]), InsertEdge(mpEdge, vcPoint[2], vcPoint[0]));
    // add face to surface
    gts_surface_add_face(m_pGtsSurface, f);

    geo::CTriangle *pFace = new geo::CTriangle(*this, m_vcElements.size(), vcPoint[0], vcPoint[1], vcPoint[2]);
...
CTSSurface::CTSSurface(const std::vector<const geo::IFace *> &vcFace, ITSProgressBase &progress)
    : m_bIsDirty(true), m_pPointCloud(0), m_pRegions(0), m_pGtsBBox(0), bPlanar(false) {
...
CTSSurface::CTSSurface(const geo::ISurface &surface, ITSProgressBase &progress)
    : m_bIsDirty(true), m_pPointCloud(0), m_pCopyPointCloud(0), m_vcElements(surface.FaceSize()),
      m_vcNodes(surface.PointSize()), m_pRegions(0), m_pGtsBBox(0), bPlanar(false) {
  ...
  for (int nPoint = 0; nPoint < surface.PointSize(); nPoint++) {
    // Create a gts-vertex and a point
    GtsVertex *pVertex = gts_vertex_new(gts_vertex_class(), (gdouble)surface.Point(nPoint).X(),
                                        (gdouble)surface.Point(nPoint).Y(), (gdouble)surface.Point(nPoint).Z());
  ...
  for (int i = 0; i < surface.FaceSize(); i++) {
    ...
    GtsFace *f = gts_face_new(gts_face_class(), InsertEdge(mpEdge, nPoint0, nPoint1),
                              InsertEdge(mpEdge, nPoint1, nPoint2), InsertEdge(mpEdge, nPoint2, nPoint0));

    //		// add face to surface
    gts_surface_add_face(m_pGtsSurface, f);

    geo::CTriangle *pFace = new geo::CTriangle(*this, m_vcElements.size(), nPoint0, nPoint1, nPoint2);
...
CTSSurface::CTSSurface(const CTSSurface &surface)
    : m_bIsDirty(true), m_pPointCloud(0), m_pRegions(0), m_pGtsBBox(0)
...
CTSSurface::CTSSurface(const std::vector<geo::CPoint> &vcPoint, const std::vector<std::vector<int>> &vcTriangle,
                       ITSProgressBase &progress)
    : m_bIsDirty(true), m_pPointCloud(0), m_pCopyPointCloud(0), m_vcElements(vcTriangle.size()),
      m_vcNodes(vcPoint.size()), m_pRegions(0), m_pGtsBBox(0), bPlanar(false) {
  ...
  for (int nPoint = 0; nPoint < vcPoint.size(); nPoint++) {
    // Create a gts-vertex and a point
    GtsVertex *pVertex = gts_vertex_new(gts_vertex_class(), (gdouble)vcPoint[nPoint].X(), (gdouble)vcPoint[nPoint].Y(),
...
CTSSurface::CTSSurface(const geo::CPtrArray<geo::IPoint> &points, ITSProgressBase &progress)
    : m_bIsDirty(true), m_pGtsSurface(0), m_pCopyPointCloud(0), m_pRegions(0), m_pGtsBBox(0), bPlanar(false) {
  ...
  for (i = 0, it = points.begin(); it != points.end(); it++, i++) {
    ...
    if (!i)
      ...
      g_slist_append(m_pPointCloud, (gpointer)v);
...
// of the surface) and a polygon describing the edge if UsePolyPoints is false else only supply the points inside the
// polygon.
CTSSurface::CTSSurface(const geo::CPtrArray<geo::IPoint> &points, ITSProgressBase &progress,
                       const geo::CPolygon &Polygon, bool UsePolyPoints /* = false */)
    : m_bIsDirty(true), m_pRegions(0), m_pGtsBBox(0), bPlanar(false) {
  GtsVertex *v;
  m_pCopyPointCloud = NULL;
  m_pPointCloud = g_slist_alloc();
  ...
  if (UsePolyPoints)
    progsize += Polygon.NrOfPoints();
  progress.Begin(progsize);

  for (i = 0, pointit = points.begin(); pointit != points.end(); pointit++, i++) {
    v = gts_vertex_new(gts_vertex_class(), (gdouble)(*pointit)->X(), (gdouble)(*pointit)->Y(),
                       (gdouble)(*pointit)->Z());
    if (!i)
      m_pPointCloud->data = (gpointer)v;
    else
...
CTSSurface::~CTSSurface() {
  ...
  if (m_pGtsSurface)
    gts_object_destroy(GTS_OBJECT(m_pGtsSurface));
  if (m_pPointCloud)
    g_slist_free(m_pPointCloud);
...
int CTSSurface::FaceSize() const { return m_vcElements.size(); }

// Returns the nr. of nodes in the surface...
int CTSSurface::PointSize() const { return m_vcNodes.size(); }
...
static gint FacesAtCallback(gpointer item, gpointer data) {
  ...
  if (gts_point_is_in_triangle((&pData->point), pTriangle) == GTS_IN ||
      gts_point_is_in_triangle((&pData->point), pTriangle) == GTS_ON) {
...
void CTSSurface::SharedPoints(std::set<geo::IPoint *> &OwnPoints, std::set<geo::IPoint *> &OtherPoints,
  ...
  for (i = 0; i < Surface.PointSize(); i++) {
    ...
    if (!ItPair.second) { // Point is shared
...
void CTSSurface::AddMeshZone(
    CMeshZone &meshzone) { // AT THE MOMENT I ASSUME THAT THE m_dEdgeSquared (Criterium()) IS LARGER THAN ZERO!!
  if (!m_pRegions)
    m_pRegions = g_slist_alloc();
...
CTSSurface *CTSSurface::ChangeSurfaceDetail(double *DetailData, double &MinimumAngle, eTypeOfDetailChange Type) {
  ...
  case ZONES: // Coarsen and/or refine in the defined zones.....
    assert(m_pRegions);
    tempsurface = (SurfaceDetail(m_pGtsSurface, NULL, NULL, NULL, NULL, CoarsenStopFunc, (gdouble)MinimumAngle, NULL,
                                 RefineStopFunc, m_pRegions));
    break;
  }
...
void CTSSurface::RemoveRegionsAndMeshZones() {
  ...
  for (it = m_stMeshZones.begin(); it != m_stMeshZones.end(); it++) {
    m_pRegions = g_slist_remove(m_pRegions, const_cast<CMeshZone &>(*it).Region());
...
// used for tests...
void CTSSurface::WriteSurface(FILE *OutFile) { gts_surface_write(m_pGtsSurface, OutFile); }
...
CTSSurface::CTSSurfaceStat CTSSurface::Statistics() const {
  ...
  return CTSSurfaceStat(stats.face_area.mean * stats.face_area.n, stats.face_area.mean, stats.edge_length.mean,
                        stats.edge_angle.mean);
}
...
bool CTSSurface::Increase(double dXNOffset, double dYNOffset, double dXMOffset, double dYMOffset,
                          double dOffset /* = 0 */) {
  // Calculate the resizing box.
  bool ret = Increase(geo::CBox(geo::CPoint(m_pGtsBBox->x1 - dXNOffset, m_pGtsBBox->y1 - dYNOffset),
                                geo::CPoint(m_pGtsBBox->x2 + dXMOffset, m_pGtsBBox->y2 + dYMOffset)),
                      dOffset);
...
void CTSSurface::InsertEdge(TVertexMap &mpVertices, GtsEdge *pEdge, GtsVertex *v) const {
  std::pair<TVertexMap::iterator, bool> prInsert =
      mpVertices.insert(TVertexMap::value_type(v, std::vector<GtsEdge *>()));
...
GtsVertex *CTSSurface::OtherVertex(GtsEdge *pEdge, GtsVertex *pVertex) const {
  ...
  if (pVertex == pEdge->segment.v1)
    return pEdge->segment.v2;
...
bool CTSSurface::NextInBBox(const GtsVertex *p, const geo::CVector &v, const geo::CBox &box) const {
  if (p->p.x + v.X() < box.Min().X() + EPS)
    return false;
  if (p->p.x + v.X() > box.Max().X() - EPS)
    return false;
  if (p->p.y + v.Y() < box.Min().Y() + EPS)
    return false;
  if (p->p.y + v.Y() > box.Max().Y() - EPS)
    return false;
...
GtsVertex *CTSSurface::CreateVertexOnBBox(const GtsVertex *p, const geo::CVector &v, const geo::CBox &box) const {
  ...
  if (p->p.x < box.Min().X() || p->p.x > box.Max().X())
    return 0;
  if (p->p.y < box.Min().Y() || p->p.y > box.Max().Y())
    return 0;
...
void CTSSurface::InsertExtensionPoints(const std::vector<GtsVertex *> &vcBoundary,
                                       const std::vector<geo::CVector> &vcVectors, const geo::CBox &box) {
...
bool CTSSurface::Increase(geo::CBox box, double dOffset /* = 0 */) {
  ...
  if ((dOffset > 0) && ((dGtsBBoxX1 - minpoint.X() > dHalfEps) || (dGtsBBoxY1 - minpoint.Y() > dHalfEps) ||
                        (dGtsBBoxX2 - maxpoint.X() < -dHalfEps) || (dGtsBBoxY2 - maxpoint.Y() < -dHalfEps))) {
    InvalidateCache();
    ...
    while (l) {
      GtsVertex *v = gts_vertex_new(gts_vertex_class(), ((GtsVertex *)l->data)->p.x, ((GtsVertex *)l->data)->p.y,
                                    ((GtsVertex *)l->data)->p.z);
...
bool CTSSurface::Decrease(double dXNOffset, double dYNOffset, double dXMOffset, double dYMOffset)
{
  ...
  if ((m_pGtsBBox->x2-m_pGtsBBox->x1 > dXNOffset+dXMOffset) && (m_pGtsBBox->y2-m_pGtsBBox->y1 > dYNOffset+dYMOffset))
  {
    ...
    while (l)
    {
      ...
      if ((((GtsVertex *)l->data)->p.x > m_pGtsBBox->x1+dXNOffset) && (((GtsVertex *)l->data)->p.x <
m_pGtsBBox->x2-dXMOffset) &&
        (((GtsVertex *)l->data)->p.y > m_pGtsBBox->y1+dYNOffset) && (((GtsVertex *)l->data)->p.y <
m_pGtsBBox->y2-dYMOffset))
      {
        v = gts_vertex_new(gts_vertex_class(),((GtsVertex *)l->data)->p.x,((GtsVertex *)l->data)->p.y,((GtsVertex
*)l->data)->p.z); m_pCopyPointCloud = g_slist_append(m_pCopyPointCloud, (gpointer) v);
      }
      l = l->next;
    }
