...
struct _GThreadFunctions {
  ...
  void (*thread_create)(GThreadFunc thread_func, gpointer arg, gulong stack_size, gboolean joinable, gboolean bound,
...
#define G_THREAD_ECF(op, fail, mutex, type)                                                                            \
  (g_thread_supported() ? ((type (*)(GMutex *, gulong, gchar *))(*g_thread_functions_for_glib_use.op))(                \
                              mutex, G_MUTEX_DEBUG_MAGIC, G_STRLOC)                                                    \
                        : (fail))
...
#define g_mutex_lock(mutex) G_THREAD_ECF(mutex_lock, (void)0, mutex, void)
...
#define g_cond_wait(cond, mutex)                                                                                       \
  (g_thread_supported() ? ((void (*)(GCond *, GMutex *, gulong, gchar *))g_thread_functions_for_glib_use.cond_wait)(   \
                              cond, mutex, G_MUTEX_DEBUG_MAGIC, G_STRLOC)                                              \
                        : (void)0)
#define g_cond_timed_wait(cond, mutex, abs_time)                                                                       \
  (g_thread_supported() ? ((gboolean (*)(GCond *, GMutex *, GTimeVal *, gulong,                                        \
                                         gchar *))g_thread_functions_for_glib_use.cond_timed_wait)(                    \
                              cond, mutex, abs_time, G_MUTEX_DEBUG_MAGIC, G_STRLOC)                                    \
                        : TRUE)
...
GThread *g_thread_create(GThreadFunc thread_func, gpointer arg, gulong stack_size, gboolean joinable, gboolean bound,
...
void g_static_private_set_for_thread(GStaticPrivate *private_key, GThread *thread, gpointer data,
...
#define G_LOCK(name)                                                                                                   \
  G_STMT_START {                                                                                                       \
    g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "file %s: line %d (%s): locking: %s ", __FILE__, __LINE__,                  \
          G_GNUC_PRETTY_FUNCTION, #name);                                                                              \
    g_static_mutex_lock(&G_LOCK_NAME(name));                                                                           \
  }                                                                                                                    \
  G_STMT_END
#define G_UNLOCK(name)                                                                                                 \
  G_STMT_START {                                                                                                       \
    g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "file %s: line %d (%s): unlocking: %s ", __FILE__, __LINE__,                \
          G_GNUC_PRETTY_FUNCTION, #name);                                                                              \
    g_static_mutex_unlock(&G_LOCK_NAME(name));                                                                         \
  }                                                                                                                    \
  G_STMT_END
#define G_TRYLOCK(name)                                                                                                \
  (g_log(G_LOG_DOMAIN, G_LOG_LEVEL_DEBUG, "file %s: line %d (%s): try locking: %s ", __FILE__, __LINE__,               \
         G_GNUC_PRETTY_FUNCTION, #name),                                                                               \
