...
#define G_HOOK_IS_UNLINKED(hook)                                                                                       \
  (((GHook *)hook)->next == NULL && ((GHook *)hook)->prev == NULL && ((GHook *)hook)->hook_id == 0 &&                  \
...
void g_hook_list_marshal_check(GHookList *hook_list, gboolean may_recurse, GHookCheckMarshaller marshaller,
                               gpointer data);
