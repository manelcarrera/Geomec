...
TEST(SanityCheck, verifyValidNames) {
  ...
  const CGraphEntryTemp<IResult> *graphEntryTemp =
      static_cast<const CGraphEntryTemp<IResult> *>(pModel->GraphEntry(MD_BASE_RESULT));
...
void verifyChildren(const IResult *node) {
  ...
  for (unsigned int mode = 0; mode < node->ModeSize(); ++mode) {
    for (unsigned int component = 0; component < node->ComponentSize(mode); ++component) {
      const IResultComponent *resultComponent =
          dynamic_cast<const IResultComponent *>(&(node->Component(component, mode)));
      ...
      if (resultComponent != 0) {
        ...
        std::pair<TStrings::iterator, bool> result = uniqueCombinations.insert(combination);
...
TEST(SanityChecks, verifyUniquePaths) {
  ...
  const TResultEntry *resultEntry = static_cast<const TResultEntry *>(modelBase->GraphEntry(MD_BASE_RESULT));
  const TResultEntry::TNodeSet nodeSet = resultEntry->EntryNodes();
  TStrings uniquePaths;

  for (TResultEntry::TNodeSet::const_iterator node = nodeSet.begin(); node != nodeSet.end(); ++node) {
...
TEST(SanityCheck, isResultsFilterComplete) {
  ...
  for (std::map<QString, bool>::const_iterator result = resultsFilter.begin(); result != resultsFilter.end();
       ++result) {
...
size_t KNOWN_VALUE_TYPES[] = {
    32957, 32958, 32959, 32960, 32961, 32963, 32964, 32965, 32967, 32979, 32981, 32987, 32989, 32991, 32993,
    32995, 33064, 33065, 33066, 33070, 33071, 33072, 33073, 33075, 33076, 40002, 40003, 40005, 40006, 40007,
    40008, 40009, 40010, 40011, 40012, 40013, 40014, 40015, 40016, 40018, 40019, 40020, 40021, 40022, 40023,
    40024, 40025, 40026, 40027, 40028, 40029, 40030, 40031, 40032, 40033, 40034, 40035, 40036, 40037, 40038,
    40100, 40101, 40109, 40110, 40111, 40112, 40113, 40114, 40115, 40116, 40117, 40118, 40119, 40120, 40121,
    40122, 40123, 40124, 40125, 40126, 40127, 40128, 40129, 40130, 40131, 40135, 40136, 40137, 40138, 40140,
    40143, 40144, 40145, 40147, 40148, 40149, 40150, 40151, 40152, 40153, 40156, 40157, 40158, 40159, 40160,
    40161, 40162, 40163, 40207, 40208, 40209, 40210, 40211, 40212, 40213, 40218, 40219};

std::vector<size_t> VALUE_TYPES(KNOWN_VALUE_TYPES,
                                KNOWN_VALUE_TYPES + (sizeof(KNOWN_VALUE_TYPES) / sizeof(KNOWN_VALUE_TYPES[0])));
...
bool isValueTypesFilterComplete() {
  ...
  int materialModel = 0;
  for (ml::TMaterialCreatorRefPtr materialCreator = f->getMatCreator(materialModel); materialCreator != 0;
    ...
    for (size_t p = 0; (libraryMaterial != 0) && (p < libraryMaterial->ParameterSize()); ++p) {
      if (std::find(VALUE_TYPES.begin(), VALUE_TYPES.end(), libraryMaterial->Parameter(p).ValueTypeID()) ==
          VALUE_TYPES.end()) {
        std::cout << VALUE_TYPE_IS_NOT_PRESENT.arg(libraryMaterial->Parameter(p).ValueTypeID()).toStdString()
                  << std::endl;
...
void listValueTypes() {
  ...
  for (ml::TMaterialCreatorRefPtr materialCreator = f->getMatCreator(materialModel); materialCreator != 0;
    ...
    for (size_t p = 0; (libraryMaterial != 0) && (p < libraryMaterial->ParameterSize()); ++p) {
      std::cout << "material model " << materialModel << ":" << libraryMaterial->Parameter(p).ValueTypeID()
                << std::endl;
    }
...
void testQStringCString() {
  ...
  EXPECT_TRUE((cFilled.CompareNoCase(noCase) == 0) && (qFilled.compare(noCase, Qt::CaseInsensitive) == 0));
}
...
void testFindAndManipulate() {
  ...
  EXPECT_TRUE((cSuffix.Left(SUFFIX_LENGTH) == SUFFIX_STRING) && (qSuffix.left(SUFFIX_LENGTH) == SUFFIX_STRING));
}
...
void testOldSpanExcluding() {
  ...
  EXPECT_TRUE(cString.SpanExcluding(EXCLUDE) ==
              qString.left((qString.indexOf(EXCLUDE) != -1 ? qString.indexOf(EXCLUDE) : qString.length())));

  cString = cString.SpanExcluding(EXCLUDE);
  qString = qString.left((qString.indexOf(EXCLUDE) != -1 ? qString.indexOf(EXCLUDE) : qString.length()));
...
const char *QDATE_TIME_FORMAT_1 = "'*** starting runner at' dd MMM yyyy',' hh':'mm '***'";

const char *CDATE_TIME_FORMAT_2 = "*** runner returned at %d %b %Y, %H:%M ***";
const char *QDATE_TIME_FORMAT_2 = "'*** runner returned at' dd MMM yyyy',' hh':'mm '***'";
...
void testTimeInterval() {
  QDateTime qBegin = QDateTime::fromString(BEGIN_DATE_TIME, DATE_TIME_FORMAT);
  QDateTime qEnd = QDateTime::fromString(END_DATE_TIME, DATE_TIME_FORMAT);
...
template <typename T> void fillValues(std::vector<T> &values) {
...
template <typename T> void testAverage(const std::vector<T> &values) {
...
void readFileRegistry(const QString &string, int integer, bool only_user = false) {
  ...
  QString stringValue = registry->getProfileString(SECTION_NAME, STRING_ENTRY_NAME);
...
TEST(SanityChecks, QtFileRegistry) {
  ...
  test_qtfileregistry::readFileRegistry(test_qtfileregistry::NEW_STRING_VALUE, test_qtfileregistry::NEW_INT_VALUE,
                                        true);
...
void testNative2Diana() {
  QString assignment = QString(ASSIGNMENT).arg(TEST_NATIVE_2_DIANA).arg(TEST_NATIVE_2_DIANA);
  ...
  assignment = QString(ASSIGNMENT).arg(TEST_NATIVE_2_DIANA).arg(EMPTY);
...
void testDiana2Native() {
  QString assignment = QString(ASSIGNMENT).arg(TEST_DIANA_2_NATIVE).arg(TEST_DIANA_2_NATIVE);
  ...
  assignment = QString(ASSIGNMENT).arg(TEST_DIANA_2_NATIVE).arg(EMPTY);
...
Simple::Simple() : CGraphNode(1) {}

Simple::~Simple() {}

unsigned int Simple::IconId() const { return 0; }

unsigned int Simple::TypeId() const { return 0; }
...
Simple_Delegate::Simple_Delegate(Simple *simple) : CGraphNode_Delegate(simple) {}

Simple_Delegate::~Simple_Delegate() {}

template <class NODE_TYPE, class DELEGATE_TYPE> DELEGATE_TYPE *testTemplate(NODE_TYPE &simple) {
  std::string name = typeid(simple).name();

  name = typeid(&simple).name();

  DELEGATE_TYPE *simpleDelegate = static_cast<DELEGATE_TYPE *>(simple.getDelegate());
...
void testDelegate() {
  Simple simple;
  Simple_Delegate *simpleDelegate = testTemplate<Simple, Simple_Delegate>(simple);
...
template <class T> class SimpleTemplate : public CGraphNode {
public:
  SimpleTemplate();
  ~SimpleTemplate();
...
template <class T> SimpleTemplate<T>::SimpleTemplate() : CGraphNode(1) {}

template <class T> SimpleTemplate<T>::~SimpleTemplate() {}

template <class T> unsigned int SimpleTemplate<T>::IconId() const { return 0; }

template <class T> unsigned int SimpleTemplate<T>::TypeId() const { return 0; }

template <class T> class SimpleTemplate_Delegate : public CGraphNode_Delegate {
public:
...
template <class T>
SimpleTemplate_Delegate<T>::SimpleTemplate_Delegate(SimpleTemplate<T> *simple) : CGraphNode_Delegate(simple) {
  ACTIVATE_TEMPLATE_DELEGATE(SimpleTemplate<T>, SimpleTemplate_Delegate<T>);
}

template <class T> SimpleTemplate_Delegate<T>::~SimpleTemplate_Delegate() {}

void testTemplateDelegate() {
  SimpleTemplate<Simple> *anotherSimple = new SimpleTemplate<Simple>();
  SimpleTemplate_Delegate<Simple> *anotherSimpleDelegate = new SimpleTemplate_Delegate<Simple>(anotherSimple);
  ...
  SimpleTemplate_Delegate<Simple> *simpleDelegate =
      testTemplate<SimpleTemplate<Simple>, SimpleTemplate_Delegate<Simple>>(simple);
  ...
  EXPECT_TRUE(
      (name == "class `anonymous namespace'::SimpleTemplate_Delegate<class `anonymous namespace'::Simple> *") ||
...
template <class T> class UnaryTemplate {
public:
  UnaryTemplate() { std::string name = typeid(T).name(); }
};
...
void testTypeID() {
  ...
  EXPECT_TRUE((name == "class `anonymous namespace'::A *") || (name == "class `anonymous namespace'::A * __ptr64"));
  name = typeid(*a).name();
  ...
  name = typeid(c).name();
  EXPECT_TRUE((name == "class `anonymous namespace'::B *") || (name == "class `anonymous namespace'::B * __ptr64"));
  name = typeid(*c).name();
  EXPECT_TRUE(name == "class `anonymous namespace'::B");
