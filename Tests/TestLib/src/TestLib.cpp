...
std::string &FileCompare::trim(std::string &s) { return ltrim(rtrim(s)); }
...
bool FileCompare::Identical(std::string file_0, std::string file_1) {
  ...
  return std::equal(std::istreambuf_iterator<char>(f0), std::istreambuf_iterator<char>(),
...
bool FileCompare::IdenticalTokens(std::string file_0, std::string file_1, double epsilon) {
  ...
  struct printTokens {
    ...
    printTokens(std::size_t count, std::string token_0, std::string token_1, double d_0, double d_1, double epsilon,
                double error, double relative_error) {
  ...
  while (!f0.eof() && !f1.eof()) {
    ...
    if (token_0 != token_1) {
      ...
      if (token_0.find('.') != std::string::npos ||
          token_1.find('.') != std::string::npos || // is one a double? (assume the other is too)
...
const char *TestLib::ModelPath() { return test_model_path; }
...
const char *TestLib::ExecPath() { return test_exec_path; }
...
std::string TestLib::getDateTime() {
  ...
  char date[20];
  sprintf(date, "%04d-%02d-%02d %02d_%02d_%02d", now->tm_year + 1900, now->tm_mon + 1, now->tm_mday, now->tm_hour,
...
std::string TestLib::getDateTimeEx() {
  ...
  char date[30];
  sprintf(date, "%04d-%02d-%02dT%02d:%02d:%02d.%06d+%02d", now->tm_year + 1900, now->tm_mon + 1, now->tm_mday,
          now->tm_hour, now->tm_min, now->tm_sec, 0, 1);
...
CModelBase *TestLib::newModel(CAnalysisLogger *pLogger) {
  CModelBase *pModel =
      IModelLifetimeFacade::NewMain(HEXA_MODEL, *pLogger, CVersionManager(test_lib::TestLib::getDefaultAppVersion()));
...
// run all
//
int TestLib::runModel(CModelBase *pModel, QString sModelFile, CAnalysisType::TAnalysisType anyType,
                      QString sAppVersion) {
  controller::Params p =
      controller::Params{*pModel, anyType, false, false, false, GetGeomecTempPathExt(CTempPath::TEMP_CALCULATION)};
...
//
int TestLib::run_model(const controller::Params &pp, const QString &url, const QString &results_path,
                       QString app_version) {
  if (!&pp.model) {
  ...
  dia::IDianaRunner::RunParams p = {url,   name.toStdString(), drc.tmp().toStdString(), &diaexec, &smc, &retr, nullptr,
                                    false, _g->dsa()};
  drc.params(p);
...
int TestLib::runModelWithSave(CModelBase *pModel, QString sModelFile, CAnalysisType::TAnalysisType anyType,
                              QString sAppVersion) {
  if (!pModel) {
  ...
  dia::IDianaRunner::RunParams p = {
      sModelFile, sModelName.toStdString(), drc.tmp().toStdString(), &diaexec, &smc, &retr, nullptr, false, _g->dsa()};
  drc.params(p);
...
int TestLib::runStepsModel(CModelBase *pModel, QString sModelFile, QString sAppVersion) {
  ...
  CDianaRunController drc(*pModel, CAnalysisType::AT_NONLIN, false, false, false,
  ...
  std::this_thread::sleep_for(
      std::chrono::milliseconds(500)); // 200ms., not enough / 300ms. is Ok // 500ms. just in case
  //
  //
  //
  return 0; // 0 is success
...
void TestLib::importResultMacroLibrary(TMacros &macros, CModelBase *pModel, QString sExportMacroLib) {
  CExportResultDataEntry *pExportResultDataEntry =
      dynamic_cast<CExportResultDataEntry *>(pModel->GraphEntry(MD_BASE_EXPORT_MACROS));
