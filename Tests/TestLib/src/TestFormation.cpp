...
void CFormation::storeReferences( TStream& stream ) const
{
  ...
  stream << index;
  index = -1;
  if ( m_pLower ) index = isReferenced( *m_pLower );
  stream << index;
  index = -1;
  if ( m_pMaterial ) index = isReferenced( *m_pMaterial );
  stream << index;
}
...
void CFormation::restoreReferences( TStream& stream, const CStreamVersion& fileVersion )
{
  ...
  else m_pUpper = (CHorizon*)&referenceAt( index );
  stream >> index;
  if ( index == -1 ) m_pLower = 0;
  else m_pLower = (CHorizon*)&referenceAt( index );
  stream >> index;
  if ( index == -1 ) m_pMaterial = 0;
  else m_pMaterial = (CMaterial*)&referenceAt( index );
}
