...
{
  return LibraryLoad_SpecialModelOperations();
}

TEST_P(ModelOperationsBatch, Scripts)
{
  execute();
}

TEST_P(ModelOperationsShell, GeomecShell)
{
  ::test_model_operations::execute_geomec_shell(GetParam());
}
...
TEST_F(ModelOperationsCORA, CORA) {
  some_grouped_tests();
}

TEST_F(ModelOperationsRGI, RGI) {
  some_grouped_tests();
}

TEST_F(ModelOperationsRGI, MonitoringPoints) {
  monitoring_points_test();
}

TEST_F(ModelOperationsRGI, MonitoringPointsProblem) {
  monitoring_points_problem_test();
}

TEST_F(ModelOperationsRGI, LogMessages) {
  log_messages_test();
}

TEST_F(ModelOperationsSkua, Import) {
  import_test();
}
