...
void ModelOperationsCORA::some_grouped_tests()
{
  ...
  if (QFile::exists(path + base + ".reference"))
  {
      ...
      cmd = "--interface cora \"" + *it + "\" getmodelinfo 2.0 \"" + outputBase + ".modelinfo_2.0\" \"" + outputBase + ".modelinfo_result_summary_2.0\"";
      //
      // geomec_shell
      //
      ::test_model_operations::execute_geomec_cora(cmd.toStdString().c_str(), &output);
      ...
      EXPECT_TRUE(CORATestGetLines(path + base + ".reference.xml") == CORATestGetLines(outputBase + ".modelinfo_2.0.xml"));
...
{
void compareProperties(const std::vector <double>& oldProperties,
  const std::vector <double>& newProperties)
{
...
void compareNode(const CGocadData::CNode& oldNode,
  const CGocadData::CNode& newNode)
{
...
void compareVertex(const CGocadData::CVertex& oldVertex,
...
void compareAtom(const CGocadData::CAtom& oldAtom,
  const CGocadData::CAtom& newAtom)
{
  compareVertex(oldAtom.Vertex(), newAtom.Vertex());
}

void compareTetra(const CGocadData::CTetra& oldTetra,
  const CGocadData::CTetra& newTetra)
{
...
void compareVolume(const CGocadData::CTVolume& oldVolume,
  const CGocadData::CTVolume& newVolume)
{
...
void compareVolumes(CGocadData::CTSolid& oldSolid,
...
void compareTriangle(const CGocadData::CTriangle& oldTriangle,
  const CGocadData::CTriangle& newTriangle)
{
...
void compareFace(const CGocadData::CTFace& oldFace,
  const CGocadData::CTFace& newFace)
{
...
void compareSurface(const CGocadData::CSurface& oldSurface,
  const CGocadData::CSurface& newSurface)
{
...
void compareSurfaces(CGocadData::CTSolid& oldSolid,
  CGocadData::CTSolid& newSolid)
{
...
void comparePropertyNames(CGocadData::CTSolid& oldSolid,
  CGocadData::CTSolid& newSolid)
{
...
void compareNoDataValues(CGocadData::CTSolid& oldSolid,
  CGocadData::CTSolid& newSolid)
{
...
void compareESizes(CGocadData::CTSolid& oldSolid,
  CGocadData::CTSolid& newSolid)
{
...
void compareTetraProperties(CGocadData::CTSolid& oldSolid,
  CGocadData::CTSolid& newSolid)
{
...
void compareTetraNoDataValues(CGocadData::CTSolid& oldSolid,
  CGocadData::CTSolid& newSolid)
{
...
void compareTetraESize(CGocadData::CTSolid& oldSolid,
  CGocadData::CTSolid& newSolid)
{
...
void compareVertexIDs(CGocadData::CTSolid& oldSolid,
  CGocadData::CTSolid& newSolid)
{
...
void compareAtomIDs(CGocadData::CTSolid& oldSolid,
  CGocadData::CTSolid& newSolid)
{
...
void compareSolids(CGocadData::CTSolid& oldSolid,
  CGocadData::CTSolid& newSolid)
{
...
ModelOperationsBatch::ModelOperationsBatch() 
  : m_ModelsPath(test_lib::TestLib::ModelPath())
  , m_OutputPath(GetGeomecTempPathExt(CTempPath::TEMP_GENERAL))
{
}
...
void ModelOperationsRGI::some_grouped_tests()
{
  ...
  QString cleanPath = QDir::current().absoluteFilePath(m_ModelsPath); // TODO: taken from geomec_shell; should be moved somewhere more global
  cleanPath = QDir::current().cleanPath(cleanPath);
  cleanPath = QDir::toNativeSeparators(cleanPath);

  printer->debug("RGI : clean path:%s ... m_OutputPath:%s", cleanPath.toStdString().c_str(), m_OutputPath.toStdString().c_str());
  ...
  else // input[testNr].type >= 1
  {
      ...
      int elts = input[testNr].type == 1 ? 1 : 1000; // larger model in 2, just to prevent any warnings in Diana (seems it can't handle 1 element models well)
    ...
    cmd = cmd.replace(".gm5", "_D0.gm5");
      ...
      rgInterface->clearCommandList();
      //rgInterface->addCommand(GMCommand(typeCommandLoadProperty, RGProperty(RGPropertyType::propElementMaterialType)));
  ...
  */
  ...
  if (pModel)
  {
    ...
    for (int i = 3; i < nvsSize; ++i)
    {
      ...
      std::set_difference(inputProps.begin(), inputProps.end(), outputProps.begin(), outputProps.end(), std::inserter(missing, missing.end()));
      std::set_difference(outputProps.begin(), outputProps.end(), inputProps.begin(), inputProps.end(), std::inserter(extra, extra.end()));
    ...
    for (std::set<std::string>::iterator it = extra.begin(); it != extra.end(); ++it)
      ...
      EXPECT_EQ(42, unusedComponent_count); // 14 tensors only have the main/diagonal components with values, the three other components are named "-": 14 * 3 = 42
...
void ModelOperationsRGI::log_messages_test()
{
  ...
  QString cmd = "--interface rgi /ROCKMECHFILE:" + rescue + PATH_DELIM + "rgi.bin " + m_ModelsPath + "TestRGILogMessages.gm5";
...
void ModelOperationsSkua::import_test()
{
  ...
  CTetraFormationEntry *tetraFormationEntry = static_cast<CTetraFormationEntry *>(pModel->GraphEntry(MD_TETRA_FORMATION));
