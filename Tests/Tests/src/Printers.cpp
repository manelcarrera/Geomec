...
mstest /publishresultsfile:test_output.trx /publish:https://sww-sede-team.shell.com/tfs/PTSSW /TeamProject:Geomec
/platform:x64 /flavor:Release /publishbuild:"Geomec Test_20131109.1"

*/

using namespace test_lib;

RTXPrinter::RTXPrinter(const std::string &fileName) : m_FileName(fileName) { m_Tests.reserve(1000); }

void RTXPrinter::OnTestProgramStart(const UnitTest & /*unit_test*/) { m_TestListID = TestLib::getGUID(); }
...
void RTXPrinter::OnTestProgramEnd(const UnitTest &unit_test) {
  ...
  output << "<TestRun id=\"" << TestLib::getGUID() << "\" name=\"" << name << "\" runUser=\"" << TestLib::getUser()
         << "\" xmlns=\"http://microsoft.com/schemas/VisualStudio/TeamTest/2010\">" << std::endl;
  ...
  output << "  <Counters total=\"" << total << "\" executed=\"" << execed << "\" passed=\"" << passed << "\" failed=\""
         << failed << "\"/>" << std::endl;
  ...
  for (std::size_t i = 0; i < m_Tests.size(); ++i) {
    ...
    output << "  <UnitTest id=\"" << test.UnitTest << "\" storage=\"geomec3\\x64\\" << TestLib::getConfiguration()
           << "\\Test.exe\" name=\"" << test.Method << "\">" << std::endl;
    output << "    <Description>" << test.Class << "." << test.Method << "</Description>" << std::endl;
    output << "    <Execution id=\"" << test.Execution << "\"/>" << std::endl;
    output << "    <TestMethod codeBase=\"geomec3\\x64\\" << TestLib::getConfiguration() << "\\Test.exe\" className=\""
           << test.Class << ", Test, Version=1.0.0.0, Culture=neutral, PublicKeyToken=null\" name=\"" << test.Method
           << "\" />" << std::endl;
  ...
  for (std::size_t i = 0; i < m_Tests.size(); ++i) {
    ...
    output << "  <TestEntry testId=\"" << test.UnitTest << "\" executionId=\"" << test.Execution << "\" testListId=\""
           << m_TestListID << "\" />" << std::endl;
  }
  output << "</TestEntries>" << std::endl;
  ...
  for (std::size_t i = 0; i < m_Tests.size(); ++i) {
    ...
    output << "  <UnitTestResult executionId=\"" << test.Execution << "\" testId=\"" << test.UnitTest
           << "\" testListId=\"" << m_TestListID << "\" outcome=\"" << (test.Result ? "Passed" : "Failed") << "\" ";
    output << "testName=\"" << test.Method << "\" testType=\"13cdc9d9-ddb5-4fa4-a97d-d965ccfc6d4b\" computerName=\""
           << TestLib::getHost() << "\" relativeResultsDirectory=\"";
    output << test.Execution << "\" duration=\"" << test.Duration << "\" startTime=\"" << test.Start << "\" endTime=\""
           << test.End << "\">" << std::endl;
    if (!test.Result) {
