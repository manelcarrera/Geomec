...
QStringList default_() {
  ...
  list << QUtil::url(QString(getenv("PROGRAMFILES")), "Geomec - SVS") << usr << QUtil::url(usr, "Desktop")
       << QUtil::url(usr, "Documents") << QUtil::url(usr, "Downloads") << appdata << QUtil::url(appdata, "Geomec")
       << getenv("LOCALAPPDATA") << getenv("TMP") << getenv("TEMP"); // C:\Users\Username\AppData\Local\Temp
...
// to conffig file
// app::paths::
void write(const QStringList &paths) { CXml::update(CONFIG_FILE, paths::KEY, paths.join(DELIM).toStdString()); }
...
void read_output() {
  ...
  printer->debug("out:%s", p1.readAllStandardOutput().constData());
...
void read_error() {
  printer->debug("err:%s", p1.readAllStandardError().constData());
...
// selector.xml
// version::
void unset() { CXml::update(app::CONFIG_FILE, version::KEY, ""); }
...
// to selector.xml
// version::
void set(const QString &version) { CXml::update(app::CONFIG_FILE, version::KEY, version.toStdString()); }
// to geomec.ini
// version::
...
std::pair<std::vector<QString>, std::vector<QString>> list_ext(const QStringList &paths) {
  ...
  return std::pair<std::vector<QString>, std::vector<QString>>(version_v, paths_v);
}
...
TPair get_() {
  ...
  QString cmd = QUtil::url(QString::fromStdString(util::GetAppPath()), "geomec_shell.exe --version");
  ...
  return TPair(version.toStdString(), build.toStdString());
}
// not used currently
//
// version::
...
void udr(std::vector<std::string> results_v) {
  ...
  for (auto result : results_v) {
    ...
    settings.setValue(section + '/' + entry, QString::fromStdString(result));
    i++;
  }
}
