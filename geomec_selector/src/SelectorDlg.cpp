...
QString version_logs_url(const QString &version) {
  QString v = version;
  QString ver = CONFIG_BASE_PATH +
                "/"
                "gm_" +
                v.replace("_build_", "_") + ".log";
  return ver;
}
...
bool url_exists(const QString &url) { return QFileInfo(url).exists() || QDir(url).exists(); }

void add_style(QWidget *widget, const QString &style) { widget->setStyleSheet(widget->styleSheet().append(style)); }
...
void CSelectorDlg::update_style() {
  for (int i = 0; i < m_versions_v.size(); i++) {
    ...
    if (item && item->widget())
      set_style(qobject_cast<QRadioButton_Custom *>(item->widget()), m_paths_v[i], m_versions_v[i]);
  }
}
...
// -Bold: installed one
// -Blue: default one
CSelectorDlg::CSelectorDlg(const std::pair<std::vector<QString>, std::vector<QString>> &versions_p) {
  ...
  for (int i = 0; i < m_versions_v.size(); i++) {
    ...
    QPushButton_Custom *btn1 =
        new QPushButton_Custom(QPushButton_Custom::Version_Config_File, i, this, b_version_ini_file);
  ...
  if (!url_exists(LOGS_CONFIG_URL))
    m_btn_logs_config.setStyleSheet(
        "background-color: rgba(255, 0, 0, 0.2); border: none"); // border: none -> needed, otherwise a padding around
                                                                 // is added
...
void CSelectorDlg::on_open(const QString &url) {
  if (url_exists(LOGS_CONFIG_URL))
    ...
    QMessageBox::warning(nullptr, "Open resource", "Resource not found !!!");
  }
}
...
void CSelectorDlg::on_logs_config() {
  if (url_exists(LOGS_CONFIG_URL))
    ...
    QMessageBox::StandardButton res =
        QMessageBox::critical(nullptr, "Create file", "Create logs config file?", QMessageBox::Yes | QMessageBox::No);

    if (res == QMessageBox::No)
      return;
    else {
      bool res_ = QFile::copy(QUtil::url(PACKAGES_PATH, "logs.xml"), LOGS_CONFIG_URL);

      QFile::setPermissions(LOGS_CONFIG_URL, QFile::ReadOther | QFile::WriteOther);

      if (res_) {
        m_btn_logs_config.setStyleSheet("background-color: rgba(255, 255, 255, 0); border: none");
        // border: none -> needed, otherwise a padding around is added
      } else {
        QMessageBox::warning(nullptr, "Create logs config file", "File not created<br>Something went wrong!");
      }
    }
  }
}
...
void CSelectorDlg::on_version_logs() {
  ...
  int id = btn->id();
  QDesktopServices::openUrl(QUrl::fromLocalFile(version_logs_url(QString(m_versions_v[id]))));
}
...
void CSelectorDlg::on_version_ini_file() {
  ...
  // create it
    ...
    QFile::setPermissions(file_url, QFile::ReadOther | QFile::WriteOther);
    ...
    if (QFile::exists(file_url)) {
      ...
      QMessageBox::warning(nullptr, "Create version config file", "Config file not created<br>Something went wrong!");
    }
  }
}
...
bool CSelectorDlg::check() { return m_cb.isChecked(); }

int CSelectorDlg::id() { return m_rb_group.checkedId(); }
...
bool CSelectorDlg::delete_config_file(int id) {
  ...
  QMessageBox::StandardButton res =
      QMessageBox::critical(nullptr, "Geomec versions", msg, QMessageBox::Yes | QMessageBox::No);
  ...
  if (res_) {
    QPushButton_Custom *btn = qobject_cast<QPushButton_Custom *>(m_grid_layout.itemAtPosition(id, COL_FILE)->widget());
    ...
    QMessageBox::warning(nullptr, "Delete version config file", "Config file not deleted<br>Something went wrong!");
  }
  return res_;
}
...
bool CSelectorDlg::delete__(int id) {
  if (!m_b_delete) {
    QMessageBox::warning(nullptr, "Remove version",
...
void QPushButton_Custom::on_custom_context_menu(const QPoint &point) { m_menu->exec(mapToGlobal(point)); }
void QPushButton_Custom::on_delete() { ((CSelectorDlg *)parentWidget())->delete_config_file(m_id); }
...
QPushButton_Custom::QPushButton_Custom(eType_ type_, const int id, QWidget *parent, bool has_context_menu)
    : m_type(type_), m_id(id), m_has_context_menu(has_context_menu), m_menu(nullptr) {
  if (has_context_menu) {
    m_menu = new QMenu();
    ...
    connect(this, SIGNAL(customContextMenuRequested(const QPoint &)), this,
            SLOT(on_custom_context_menu(const QPoint &)));

    if (type_ == Version_Config_File) {
      QAction *action = new QAction("Delete", this);
      m_menu->addAction(action);

      connect(action, SIGNAL(triggered()), this, SLOT(on_delete()));
    }
  }
}
...
void QRadioButton_Custom::on_custom_context_menu(const QPoint &point) { m_menu->exec(mapToGlobal(point)); }

void QRadioButton_Custom::on_delete() { ((CSelectorDlg *)parentWidget())->delete_(this); }

void QRadioButton_Custom::on_set_as_default() { ((CSelectorDlg *)parentWidget())->set_as_default(this); }

QRadioButton_Custom::QRadioButton_Custom(const QString &caption, QWidget *parent) : QRadioButton(caption, parent) {
  m_menu = new QMenu();
  ...
  connect(action2, SIGNAL(triggered()), this, SLOT(on_set_as_default()));
};
