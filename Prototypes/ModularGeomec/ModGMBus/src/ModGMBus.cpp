...
ModGMBusMessage::ModGMBusMessage()
{
  memset(package, 0, TOTAL_SIZE);
}

ModGMBusMessage::ModGMBusMessage(Type type, const char *from, const char *to)
  : ModGMBusMessage()
{
  fields.type = type;
...
void ModGMBusMessage::setType(Type type)
{
  fields.type = type;
}
...
void ModGMQueue::init()
{
  memset(this, 0, sizeof(ModGMQueue));
}
...
QString ModGMLocalBusHelper::getBase(quint64 pid)
{
  return QString("\\\\.\\pipe\\ModGM\\%1\\").arg(pid);
}
...
{
  m_command = command; // only one at the time; if more are needed, caller needs to check signal commandDelivered before sending again
  m_socket->connectToServer(m_base + BUS_LISTENER);
}

void ModGMConnector::handleConnect()
{
  ModGMLocalBusHelper::writeTo(m_socket, m_command);
}
...
ModGMSharedMemorySocket::ModGMSharedMemorySocket(Type type)
  : QObject()
  , m_type(type)
  , m_sem_readbuf_empty(nullptr)
  , m_sem_readbuf_full(nullptr)
  , m_sem_writebuf_empty(nullptr)
  , m_sem_writebuf_full(nullptr)
  , m_quit(false)
{
}
...
ModGMLocalBusClient::~ModGMLocalBusClient()
{
  m_connector.blockSignals(true);
}
...
void ModGMLocalBusClient::setup()
{
  connectToListener();
}
...
ModGMLocalBusServer::ModGMLocalBusServer(const QString& base, const QString& name, QObject *parent)
  ...
  , m_base(base)
  , m_name(name)
  , m_shm_socket(name, ModGMSharedMemorySocket::SERVER)
{
...
ModGMLocalBusManager::~ModGMLocalBusManager()
{
  ...
  disconnect(m_connectionManager, &IConnectionManager::registerChannel, this, &ModGMLocalBusManager::handleRegisterChannel);
