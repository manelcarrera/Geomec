...
void strip_spaces(std::string &str) { str.erase(remove_if(str.begin(), str.end(), isspace), str.end()); }
...
int main(int argc, char *argv[]) {
  ...
  if (retval != 0) {
    std::cerr << "Usage: " << argv[0]
              << " <path to msi file> <path to MsiInfo.exe> <path to release drop folder> [1 if auto-distributing]"
              << std::endl;
    std::cerr
        << "   OR: " << argv[0]
        << " <path to aip file> <path to AdvancedInstaller.com> <path to release drop folder> [1 if auto-distributing]"
        << std::endl;
    return retval;
  }
