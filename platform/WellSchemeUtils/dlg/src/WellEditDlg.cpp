...
/*virtual*/ CWellEditDlg::~CWellEditDlg()
{
  m_doc->destroy();
}
...
int CWellEditDlg::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
  ...
  if (!m_3DViewToolBar.CreateEx(m_pFrameWnd) ||
    !m_3DViewToolBar.LoadToolBar(IDR_TOOLBAR_WELL_EDIT))
  {
...
well::CWellPathBase& CWellEditDlg::WellPath() const
{
  return *m_pWellPath;
}


void CWellEditDlg::SetAzimuth(bool azimuth)
{
  m_pSpreadView->SetAzimuth(azimuth);
}


void CWellEditDlg::SetType(DEF_TYPE type)
{
  m_pSpreadView->SetDefType(type);
}
...
void CWellEditDlg::OnMenuApply() 
{
  OnOK();
}

void CWellEditDlg::OnMenuClose() 
{
  OnCancel();
