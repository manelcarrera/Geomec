...
static char *EXPORT_NAMES[] = {"General Well Information", "Detail Well Information", "Wellpath Plans"};

#define IMPORT_OBJECTS 1
static char *IMPORT_NAMES[] = {"General Well Information", "Detail Well Information"
...
public:
  ...
  CMdAziInc(double tmd, double azi, double inc) {
    m_tmd = tmd;
    m_azi = azi;
    m_inc = inc;
  }
...
CDexWellInterface::CDexWellInterface(well::CWellPathBase &well_path, QString app_name, QString version)
    : m_WellPath(well_path), m_app_name(app_name), m_version(version) {}
...
BOOL CDexWellInterface::ImportFormationSections(QString path_name, QString &strError, QList<double> &tmd_values,
                                                QStringList &names) {
#ifdef USE_DEX
    ...
    if (BusinessObj() == "Formation Tops") {
      while (DexGetNextValue(&szTable, &szField, &szType, &szUnit, &szValue, &szParent, &iRecNo)) {
        ...
        if (Table() == "FORMATION-TOPS") {
          ...
          } else if (Field() == "PROGNOSED-DEPTH" &&
                     type == CDexDepthTypeDlg::PROGNOSED) { // this will be available first, but ideally we want the
                                                            // Logged Depth...
            tmd_values.push_back(
                length.Convert(Value().toDouble(), IQuantityDouble::SI_UNIT, IQuantityDouble::FIELD_UNIT));
          } else if (Field() == "LOGGED-DEPTH" &&
                     type == CDexDepthTypeDlg::LOGGED) { // Ah! Logged depth is available! Forget the Prognosed Depth,
...
BOOL CDexWellInterface::ImportWellPathFromDEX(QString path_name, QString &strError) {
  ...
  while (DexGetNextObject(&szBusinessObject, &szAuthor, &szVersion, &szWellID, &szDate)) {
    ...
    if (BusinessObj() == "Survey" && !bWellGeometrySet) { // survey available: Great! use it...
      ImportWellGeometry(SURVEY);
      bWellGeometrySet = true;
    }
    if (BusinessObj() == "Non-Definitive Surveys" &&
        !bWellGeometrySet) { // the non-definitive survey available, use this one...
      ImportWellGeometry(SURVEY);
      bWellGeometrySet = true;
    }
    if (BusinessObj() == "Wellpath Plans" && !bWellGeometrySet) { // no surveys available? use the well plans...
...
BOOL CDexWellInterface::ExportToDEX(QString path_name, QString file_name, QString &strError) const {
  ...
  if (!CFile::GetStatus(path_name.toAscii().constData(), status) ||
      DexImpExpDialog(FALSE, sPath.toAscii().constData(), file_name.toAscii().constData(),
                      AppName().toAscii().constData(), m_WellPath.GetWellName().toAscii().constData(), szChosen,
                      &iTotal)) {
    CString sChose;
    ...
    if (CFile::GetStatus(path_name.toAscii().constData(), status) &&
        !DexOpen(path_name.toAscii().constData(), &error_msg, TRUE))
      bState = false;
...
void CDexWellInterface::ExportGeneralWellInformation() const {
  ...
  if (!DexCreateObject("General Well Information", AppName().toAscii().constData(), Version().toAscii().constData(),
                       WellRepresentation().toAscii().constData()))
    DexPutObject("General Well Information", AppName().toAscii().constData(), Version().toAscii().constData(),
                 WellRepresentation().toAscii().constData());
  ...
  case well::CWellPathBase::PLATFORM: {
    ...
    ExportValue(
        "GENERAL", "WATER-DEPTH",
        length.Convert(m_WellPath.HeightOfWaterColumn(), IQuantityDouble::FIELD_UNIT, IQuantityDouble::SI_UNIT));
    break;
  }
...
void CDexWellInterface::ExportDetailWellInformation() const {
  ...
  if (!DexCreateObject("Detail Well Information", AppName().toAscii().constData(), Version().toAscii().constData(),
                       WellRepresentation().toAscii().constData()))
    DexPutObject("Detail Well Information", AppName().toAscii().constData(), Version().toAscii().constData(),
                 WellRepresentation().toAscii().constData());
...
void CDexWellInterface::ExportWellPlans() const {
  ...
  if (!DexCreateObject("Wellpath Plans", AppName().toAscii().constData(), Version().toAscii().constData(),
                       WellRepresentation().toAscii().constData()))
    DexPutObject("Wellpath Plans", AppName().toAscii().constData(), Version().toAscii().constData(),
                 WellRepresentation().toAscii().constData());
  ...
  while (DexGetNextValue(&szTable, &szField, &szType, &szUnit, &szValue, &szParent, &iRecNo)) {
    ...
    if (Table() == "GENERAL") {
      if (Field() == "WELL-DESCRIPTION") {
        ...
        case well::CWellPathBase::PLATFORM:
        case well::CWellPathBase::SUBSEA: {
          m_WellPath.MeanGroundOrSeabedLevel().Value(
              water_depth + m_WellPath.MeanSeaLevel().Value(IQuantityDouble::FIELD_UNIT), IQuantityDouble::FIELD_UNIT);
          break;
        }
        default:
          assert(false);
...
void CDexWellInterface::SetWellPathGeometry() {
  ...
  if (m_vc_tvd.size() == 0) { // must be a vertical wellpath, use the tmd
                              //	temp_depth = m_vc_tmd;
