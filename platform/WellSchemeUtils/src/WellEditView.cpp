...
#ifdef _DEBUG
void CWellEditView::AssertValid() const { CView::AssertValid(); }

void CWellEditView::Dump(CDumpContext &dc) const { CView::Dump(dc); }
#endif //_DEBUG

void CWellEditView::OnSize(unsigned int nType, int cx, int cy) { COpenGLView::OnSize(nType, cx, cy); }
...
void CWellEditView::UpdateWellView() {
  ...
  if (m_pWellpath->Defined()) {
    ...
    well::IWellSection *section =
        new well::CWellSection(*m_pWellpath, m_pWellpath->DefPointList().First()->TMD().Value(),
                               m_pWellpath->DefPointList().Last()->TMD().Value());
...
void CWellEditView::geometryChanged() { UpdateWellView(); }
