...
void CWellPointListCtrl::ShowPosition(bool show)
{
  m_ShowPosition =show;
}

void CWellPointListCtrl::ShowDirections(bool show)
{
  m_ShowDirections =show;
}

void CWellPointListCtrl::ShowCoordinates(bool show)
{
  m_ShowCoordinates =show;
}

void CWellPointListCtrl::ShowNumbering(bool show)
{
  m_ShowNumbering = show;
}
...
void CWellPointListCtrl::OnInitList
...
void CWellPointListCtrl::RemoveSelectedPoints(const bool bNewWellPath) 
{
  ...
  else // wjrx mantis 3564
  {
      ...
      for ( std::list<CNewWellPoint>::const_iterator it= m_pNewList->begin()
          ; it != m_pNewList->end()
          ; ++it
          )
      {
...
void CWellPointListCtrl::UpdateList()
{
  ...
  if ( ( m_pList == 0 || m_pList->NrOfPoints() ==0 )
     && 
       ( m_pNewList == 0 || m_pNewList->empty() )
     )
  return;
