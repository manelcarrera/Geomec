...
CSpreadView::CSpreadView() : CFormView(CSpreadView::IDD) {
  m_Us = QU::SI_UNIT;
  m_Type = TMD_INC;
  m_Azimuth = true;
  m_CellChanged = false;
...
#ifdef _DEBUG
void CSpreadView::AssertValid() const { CFormView::AssertValid(); }

void CSpreadView::Dump(CDumpContext &dc) const { CFormView::Dump(dc); }
#endif //_DEBUG
...
ON_EVENT(CSpreadView, IDC_SPREAD1, 18 /* LeaveRow */, OnLeaveRow,
...
void CSpreadView::OnInitialUpdate() { CFormView::OnInitialUpdate(); }
...
void CSpreadView::UpdateWellPath() {
  ...
  if (count > 1) {
    if (m_Type == TMD_INC) {
      m_pWellpath->InitFromArray_tmd_azi_inc(global_northing, global_easting, global_depth, start_northing,
                                             start_easting, start_depth, tmd, azi, inc);
    } else if (m_Type == TMD_TVD) {
      m_pWellpath->InitFromArray_tmd_tvd_azi(global_northing, global_easting, global_depth, start_northing,
                                             start_easting, start_inc, tmd, tvd, azi);
...
bool CSpreadView::IsDirty() { return m_Dirty; }
...
/*virtual*/ void CSpreadView::OnLeaveRow(long Row, BOOL RowWasLast, BOOL RowChanged, BOOL AllCellsHaveData, long NewRow,
