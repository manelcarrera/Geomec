...
public:
  ...
  void migrateProfileSettings(const LPCTSTR &pszRegistryKey, const LPCTSTR &pszProfileName, WORD major, WORD minor);
...
private:
  ...
  bool migrateProfileSettings(const CString &strOldProductName, const LPCTSTR &pszRegistryKey,
                              const LPCTSTR &pszProfileName);

  // hSourceKey can be opened with KEY_READ access,
  // hTargetKey requires KEY_ALL_ACCESS.

  bool CopyRegistrySubTree(HKEY hSourceKey, const CString &strSourceSubKey, HKEY hTargetKey,
