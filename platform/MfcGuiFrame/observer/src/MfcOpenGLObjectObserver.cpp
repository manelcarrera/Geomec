...
void CMfcOpenGLObjectObserver::appendContextMenu(CContextMenuInvoker &invoker) {
  ...
  invoker.AddCommand("Delete",
                     *new TObjectCommand(observedObject(), &IModelObject::destroy, &IModelObject::canDestroy));

  invoker.AddSeparator();
  invoker.AddCommand("Properties", *new TObserverCommand(*this, &CModelObjectObserver::showProperties,
