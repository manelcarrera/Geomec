...
C2DBody::CPolygon::CPolygon() : IPolygon(false) {}

int C2DBody::CPolygon::NrOfPoints() const { return m_vcVertex.size(); }

const geo::IPoint &C2DBody::CPolygon::Point(const int nIndex) const { return m_vcVertex[nIndex]->Point(); }

const C2DVertex &C2DBody::CPolygon::Vertex(int nIndex) const { return *m_vcVertex[nIndex]; }

void C2DBody::CPolygon::Point(const int nIndex, const geo::IPoint &pt) { assert(false); }
...
void C2DBody::CPolygon::PushBack(const C2DVertex &point) { m_vcVertex.push_back(&point); }

int C2DBody::CPolygon::SegmentSize() const { return NrOfPoints(); }
...
const C2DBody::CPolygon &C2DBody::Polygon() const { return m_polygon; }
/*
bool C2DBody::MouseRelease(TFrame& frame, TButtonState state, TButtonState button, const TScreenPoint& point, const
TObjectVec& vcHit)
{
...
*/
int C2DBody::geometrySize() const { return 1; }
...
void C2DBody::AppendContextMenu(CContextMenuInvoker &invoker)
{
  ...
  {
    invoker.AddCommand( _T("&Refine formation"),*(new TFCommand(formation, &C2DFormation::OnRefine,
&C2DFormation::CanRefine))); invoker.AddCommand( _T("&Coarsen formation"),*(new TFCommand(formation,
&C2DFormation::OnCoarsen, &C2DFormation::CanCoarsen))); invoker.AddSeparator(); invoker.AddCommand( _T("&Refine
compartment"),*(new TCCommand(*this, &C2DBody::OnRefine, &C2DBody::CanRefine))); invoker.AddCommand( _T("&Coarsen
compartment"),*(new TCCommand(*this, &C2DBody::OnCoarsen, &C2DBody::CanCoarsen)));
  }
...
bool C2DBody::CanCoarsen() const { return ElementSizeEnabled() && (ElementSize() * 1.5 < m_dMaxElementSize); }

bool C2DBody::CanRefine() const { return true; }

bool C2DBody::ElementSizeEnabled() const { return m_bElementSize; }
...
void C2DBody::DisableElementSize() { m_bElementSize = false; }

const double &C2DBody::ElementSize() const { return m_dElementSize; }

const double &C2DBody::MaxElementSize() const { return m_dMaxElementSize; }
...
int C2DBody::Resemblance(const std::set<IModelObject *> &identifier) const {
  ...
  std::set_difference(m_identifier.begin(), m_identifier.end(), identifier.begin(), identifier.end(),
