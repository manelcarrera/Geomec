...
void TraceVertex(const geo::ICoordinate& coor)
{
...
unsigned char COpenGLView::m_threeto8[8] = {
  0, 0111>>1, 0222>>1, 0333>>1, 0444>>1, 0555>>1, 0666>>1, 0377
};
//##ModelId=3B653BCA01F7
unsigned char COpenGLView::m_twoto8[4] = {
   0, 0x55, 0xaa, 0xff
};
//##ModelId=3B653BCA01F6
unsigned char COpenGLView::m_oneto8[2] = {
  0, 255
};

//##ModelId=3B653BCA0206
int COpenGLView::m_defaultOverride[13] = {
...
//virtual
BYTE COpenGLView::StencilBufferDepth() const
{
  return 3;
}
...
GLint COpenGLView::GetFontID(QFont font) const
{
  ...
  if ( it == m_fontMap.end() ) return (GLuint)-1;
  return it.value();
}
...
COpenGLView::TColor COpenGLView::BackgroundColor() const
{
...
COpenGLView::TColor COpenGLView::BackgroundColor2() const
{
...
COpenGLView::TColor COpenGLView::TextColor() const
{
  return m_TextColor;
}
...
int COpenGLView::ViewPortX() const
{
  return 0;
}

int COpenGLView::ViewPortY() const
{
  return 0;
}
...
{
  while(ChildSize() > 0) Child(0).ReParent(0);
...
BOOL COpenGLView::IsCurrent() const
{
...
bool COpenGLView::DeviceContext() const
{
  return true;
}


void COpenGLView::MakeNotCurrent() const
{
  VERIFY(wglMakeCurrent(NULL,NULL));
}

GLuint COpenGLView::DefaultFontID()
{
  return m_FontID;
}
...
HDC COpenGLView::GetCurrentDC() const
{
  return m_hDC;
}

void COpenGLView::FlipBuffers()
{
  SwapBuffers(GetCurrentDC());
}

void COpenGLView::RenderText(const double& dX, const double& dY, const double& dZ, const QString& text, const QFont& fnt)
{
...
BOOL COpenGLView::OnEraseBkgnd(CDC*) 
{
  return TRUE;
}
...
// freed with delete[]
void COpenGLView::ClientAsBitmap(BITMAPINFOHEADER *pHeader, unsigned char **ppPixelData, int *pDataSize, int *pSizeX, int *pSizeY)
{
  MakeCurrent();
...
bool COpenGLView::BitmapToFile(BITMAPINFOHEADER *pBMIH, unsigned char *pPixelData, int DataSize, const CString &szPathName)
{
  ...
  unsigned int nWrittenFileHeaderSize = fwrite(&bmfh, 1, 
  ...
  unsigned int nWrittenInfoHeaderSize = fwrite(pBMIH, 
  ...
  unsigned int nWrittenDIBDataSize = 
       fwrite(pPixelData, 1, lImageSize, pFile);
...
void COpenGLView::SetDocument(CDocument* pDoc)
{
  m_pDocument = pDoc;
}
