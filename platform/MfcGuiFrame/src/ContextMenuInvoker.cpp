...
CContextMenuInvoker::CContextMenuInvoker() { m_menu.CreatePopupMenu(); }

// This causes a separator to appear in the menu
void CContextMenuInvoker::AddSeparator() { appendSeparator(); }
...
// Invoke will first try to invoke the command by its base. When this fails (ie the command can not be found) it
// searches all other invokers an tries them.
...
void CContextMenuInvoker::append(IGuiCommand *cmd) { AddCommand(cmd->caption().toStdString(), *cmd); }