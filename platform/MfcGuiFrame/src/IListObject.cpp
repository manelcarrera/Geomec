...
IListObject::IListObject(CListCtrl &ctrl, int nIndex, BOOL bSelected) : m_ctrl(ctrl) {
  if (nIndex == -1)
...
CListCtrl &IListObject::Ctrl() const { return m_ctrl; }

std::size_t IListObject::SubObjectSize() const { return m_mpSubObject.size(); }
...
void IListObject::Update() {
  ...
  if (StateIcon() != 0) {
    ...
    VERIFY(pCtrl->SetItemState(Index(), INDEXTOSTATEIMAGEMASK(pCtrl->StateIconImageList().IconToImage(StateIcon()) + 1),
                               LVIS_STATEIMAGEMASK));
  }
  pCtrl->Update(Index());
}
...
BOOL IListObject::IsSelected() const { return Ctrl().GetItemState(Index(), LVIS_SELECTED) == LVIS_SELECTED; }

void IListObject::SelectionStateChanged(bool bSelected) {}

void IListObject::OnDoubleClick() { ICtrlObject::OnDoubleClick(); }

void IListObject::OnDoubleClick(int nItemIndex) { OnDoubleClick(); }
