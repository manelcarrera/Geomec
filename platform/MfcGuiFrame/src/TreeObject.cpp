...
CTreeObject::~CTreeObject() { m_observer.m_pTreeObject = 0; }

unsigned int CTreeObject::Icon() const { return m_observer.onIconId(); }

unsigned int CTreeObject::StateIcon() const { return m_observer.onIconId(); }

QString CTreeObject::Text() const { return (LPCSTR)m_observer.onName(); }

void CTreeObject::AppendContextMenu(CContextMenuInvoker &invoker) { m_observer.appendCommands(invoker); }
...
BOOL CTreeObject::Bold() { return m_observer.bold(); }

BOOL CTreeObject::Enable() { return m_observer.enable(); }

void CTreeObject::Delete() { m_observer.destroy(); }

BOOL CTreeObject::CanDelete() const { return m_observer.canDestroy(); }
...
BOOL CTreeObject::operator<(const ICtrlObjectBase &object) const {
  ...
  for (int i = 0; i < pParent->m_observer.childSize(); i++) {
    if (&pParent->m_observer.childAt(i) == &m_observer.observedObject())
      nThis = i;
    if (&pParent->m_observer.childAt(i) == &pObject->m_observer.observedObject())
      nRhs = i;
  }
