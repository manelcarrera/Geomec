...
CTnoWinApp::CRegKey::CRegKey(HKEY hKey) : m_hKey(hKey) {}
...
HKEY CTnoWinApp::CRegKey::Key() const { return m_hKey; }
...
CTnoWinApp::CTnoWinApp() : m_bExiting(false) {}
...
HKEY CTnoWinApp::GetAppSystemRegistryKey() {
  ...
  if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, _T("SOFTWARE"), 0, KEY_READ, &hSoftKey) == ERROR_SUCCESS) {
    DWORD dw;
    if (RegCreateKeyEx(hSoftKey, m_pszRegistryKey, 0, REG_NONE, REG_OPTION_NON_VOLATILE, KEY_READ, NULL, &hCompanyKey,
                       &dw) == ERROR_SUCCESS) {
      RegCreateKeyEx(hCompanyKey, m_pszProfileName, 0, REG_NONE, REG_OPTION_NON_VOLATILE, KEY_READ, NULL, &hAppKey,
                     &dw);
    }
  }
  if (hSoftKey != NULL)
...
HKEY CTnoWinApp::GetSystemSectionKey(LPCTSTR lpszSection) {
  ...
  DWORD dw;
  RegCreateKeyEx(hAppKey, lpszSection, 0, REG_NONE, REG_OPTION_NON_VOLATILE, KEY_READ, NULL, &hSectionKey, &dw);
  RegCloseKey(hAppKey);
  return hSectionKey;
}
...
bool CTnoWinApp::IsExiting() const { return m_bExiting; }

void CTnoWinApp::SetIsExiting() { m_bExiting = true; }