...
ITreeObject::ITreeObject(CTreeCtrl &ctrl, HTREEITEM hParent, HTREEITEM hInsertAfter) : m_ctrl(ctrl) {
  m_hTreeHandle =
      Ctrl().InsertItem(TVIF_TEXT | TVIF_IMAGE | TVIF_PARAM | TVIF_SELECTEDIMAGE, LPSTR_TEXTCALLBACK, I_IMAGECALLBACK,
                        I_IMAGECALLBACK, TVIS_STATEIMAGEMASK, TVIS_STATEIMAGEMASK, (LPARAM)0, hParent, hInsertAfter);
  InitSpecialProps();
  assert(m_hTreeHandle);
}

ITreeObject::ITreeObject(ITreeObject &parent, HTREEITEM hInsertAfter) : m_ctrl(parent.Ctrl()) {
  m_hTreeHandle = Ctrl().InsertItem(TVIF_TEXT | TVIF_IMAGE | TVIF_PARAM | TVIF_SELECTEDIMAGE, LPSTR_TEXTCALLBACK,
                                    I_IMAGECALLBACK, I_IMAGECALLBACK, TVIS_STATEIMAGEMASK, TVIS_STATEIMAGEMASK,
                                    (LPARAM)0, parent.Handle(), hInsertAfter);
  InitSpecialProps();
  assert(m_hTreeHandle);
}

ITreeObject::ITreeObject(CTreeCtrl &ctrl) : m_ctrl(ctrl), m_hTreeHandle(0) { InitSpecialProps(); }

void ITreeObject::InitSpecialProps() { m_logfont.lfFaceName[0] = '\0'; }
...
const CTreeCtrl &ITreeObject::Ctrl() const { return m_ctrl; }

CTreeCtrl &ITreeObject::Ctrl() { return m_ctrl; }
...
BOOL ITreeObject::IsExpanded() const { return Ctrl().GetItemState(Handle(), TVIS_EXPANDED) & TVIS_EXPANDED; }
...
void ITreeObject::Update() {
  ...
  assert(ctrl);
  ctrl->SetItemState(Handle(), INDEXTOSTATEIMAGEMASK(ctrl->GetIconImageList(TVSIL_STATE)->IconToImage(StateIcon())),
                     TVIS_STATEIMAGEMASK);
...
BOOL ITreeObject::IsSelected() const { return Ctrl().GetSelectedItem() == Handle(); }
...
BOOL ITreeObject::Bold() { return FALSE; }

BOOL ITreeObject::Enable() { return TRUE; }

LOGFONT ITreeObject::Font() { return m_logfont; }
