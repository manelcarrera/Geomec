...
void CTreeCtrlBase::AllowPaint(bool bAllowPaint)
{
...
void CTreeCtrlBase::OnSelChanged(NMHDR* pNMHDR, LRESULT* pResult) 
{
  ...
  if(pNew) 
  {
    if(pNew->Enable())
    {
      // Added LVM_FIRST; this is the action we receive sometimes (eg expanding results) that causes the selection of Global Temperature
      if(!m_bContextOperation && pNMTreeView->action != TVC_UNKNOWN && pNMTreeView->action != LVM_FIRST) // TVC_UNKNOWN: software-generated selchange, don't respond (e.g. collapse of branch)
      {
        if(pNew->OnSelect())
        {
          OnSelect(pOld, pNew);
...
DROPEFFECT CTreeCtrlBase::OnDragOver(TCtrlObjectVec vcDragged, COleDataObject* pDataObject, DWORD dwKeyState, CPoint point)
{
...
BOOL CTreeCtrlBase::OnDrop(TCtrlObjectVec vcDragged, 
...
CWnd& CTreeCtrlBase::GetContextWnd() const 
{ 
  return (CWnd&)(*this); 
}
...
void CTreeCtrlBase::RegisterDropTarget()
{
...
void CTreeCtrlBase::UpdateChildren(HTREEITEM hParent)
{
  ...
  while (hChildItem)
  {
    ...
    if(pObject)
    {
      ...
      if (pObject->Handle()) UpdateChildren(pObject->Handle());
    }
...
CIconImageList* CTreeCtrlBase::GetIconImageList(unsigned int nImage) //Need state list before activated (state is not a callback)
{
  switch( nImage )
  {
  case TVSIL_STATE:
