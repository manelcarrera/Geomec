...
CEdit *CListCtrlBase::EditSubLabel(std::pair<int, int> location) {
  ...
  if (!EnsureVisible(location.first, TRUE))
    return NULL;
...
DROPEFFECT CListCtrlBase::OnDragOver(TCtrlObjectVec vcDragged, COleDataObject *pDataObject, DWORD dwKeyState,
                                     CPoint point) {
...
BOOL CListCtrlBase::OnDrop(TCtrlObjectVec vcDragged, COleDataObject *pDataObject, DROPEFFECT dropEffect, CPoint point) {
  BOOL bRet = FALSE;
...
CWnd &CListCtrlBase::GetContextWnd() const { return (CWnd &)(*this); }

void CListCtrlBase::RegisterDropTarget() { m_target.Register(this); }
  ...
  if (location.first != -1) {
    ...
    if (pObject != 0) {
      ...
      if (location.second > 0)
        pObject->SubObject(location.second)->OnDoubleClick(point);
    }
  }
...
IListObject *CListCtrlBase::GetObject(int iRow) { return (IListObject *)GetItemData(iRow); }
