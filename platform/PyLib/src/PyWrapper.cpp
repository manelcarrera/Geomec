...
} // namespace PythonObjectToBuild

//------------------------------------------------------------------------------------ parse : tuple

char *PyWrapper::_parse_char_ptr(PyTuple *tuple) {
  char *ignorestring;
  pyd::PyArg_ParseTuple(tuple, PythonObjectToBuild::String,
                        &ignorestring); // FIXME: what happens with the reference to the char*?
  return ignorestring;
}
...
TPyObjectPtrV PyWrapper::_parse_tuple(PyTuple *tuple) {
  ...
  if (is_tuple) {
    ...
    } else // Format: one params for cmd_id and a second param for the params list -> send_cmd( cmd_id, (param0,
           // param1,) )
    {
...
// PyObject* PyWrapper::_build_cmd( Command cmd ){				return Py_BuildValue( PythonObjectToBuild::Command, cmd.cmd(),
// cmd.param() );	}

// PyObject* PyWrapper::_build_obj( void* _obj ){				return Py_BuildValue( PythonObjectToBuild::Object,
// PyWrapper::tuple( (PyObject*)_obj ) ); }
...
PyObject *PyWrapper::_build_array(size_t rows, size_t cols, TMemoryChunkPtr memory_chunck, DataType data_type) {
  ...
  PyArrayObject *np_array = reinterpret_cast<PyArrayObject *>(
      PyArray_SimpleNewFromData(ND, dims,
...
PyObject *PyWrapper::_build_array(TData array) {
  return _build_array(array.rows, array.cols, array.data, array.data_type);
}

//------------------------------------------------------------------------------- execute
PyObject *PyWrapper::execute(TPyModule py_module, TCModuleV c_modules_v, int (*_callback)(PyTuple *)) {
  Printer::instance()->info("CPython : execute : module:'%s' funcion:'%s' param:'%s' param_py:'%p'",
                            py_module.module.c_str(), py_module.function.c_str(), py_module.param.c_str(),
                            py_module.param_py);
  ...
  for (int i = 0; i < c_modules_v.size(); i++)
    ...
    pyd::Py_InitModule4_64(c_modules_v[i].name.c_str(), c_modules_v[i].methods, (char *)NULL, (PyObject *)NULL,
                           PYTHON_API_VERSION);
    ...
    if (pFunc && pyd::PyCallable_Check(pFunc)) {
      if (py_module.param_py) {
        ...
        if (py_module.param.empty())
          ...
          pArgs =
              PyWrapper::build<TPyObjectPtrV>(TPyObjectPtrV{PyWrapper::build<const char *>(py_module.param.c_str())});
      }
