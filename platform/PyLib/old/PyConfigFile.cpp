...
PyConfigFile::Data::Data(std::string _app_data, std::string _python_root, std::string _python_dll,
                         std::string _base_path, std::string _log_file, std::string _py_files, std::string _python_path,
                         std::string _module, std::string _function) {
  app_data = _app_data;
...
const std::string PyConfigFile::Data::PYTHONPATH_ =
    "%PYTHONROOT%\\libs;%PYTHONROOT%\\DLLs;%PYTHONROOT%\\Lib;%PYTHONROOT%\\Lib\\site-packages;%PYTHONROOT%\\Lib\\lib-"
    "tk;%PY_FILES%";
...
const std::vector<std::string> PyConfigFile::Data::DEFAULT_DATA_V = {
    Data::APPDATA,  Data::PYTHONROOT,  Data::PYTHON_DLL, Data::BASE_PATH, Data::LOG_FILE,
    Data::PY_FILES, Data::PYTHONPATH_, Data::MODULE,     Data::FUNCTION};

void PyConfigFile::exitWithError(const std::string &error) { Printer::instance()->debug((char *)error.c_str()); }
...
void PyConfigFile::create_default_file(const std::string &name) {
  if (util::create_dir(m_data->base_path)) {
    ...
    Printer::instance()->debug("Config file ERROR: %s [%s]", util::GetLastErrorAsString().c_str(),
                               m_data->base_path.c_str());
  }

  // std::vector<std::string> keys_v		= {"APPDATA", "PYTHONROOT","PYTHON_DLL","BASE_PATH", "LOG_FILE", "PY_FILES",
  // "PYTHONPATH", "MODULE", "FUNCTION"};
  std::vector<std::string> keys_v = {"PYTHONROOT", "PYTHON_DLL", "BASE_PATH", "LOG_FILE",
                                     "PY_FILES",   "PYTHONPATH", "MODULE",    "FUNCTION"};
...
void PyConfigFile::replace_config_wildcards() {
  ...
  if (!file) {
    ...
    QString default_module_qs = "from Tkinter import *\n"
                                "def main():\n"
                                "	root = Tk()\n"
    ...
    outfile.close();
    Printer::instance()->debug("Default MODULE.FUNCTION()[%s.%s()] created, contect: [%s]", contents["MODULE"].c_str(),
                               contents["FUNCTION"].c_str(), default_module_qs.toStdString().c_str());
  }
}
...
PyConfigFile::PyConfigFile(const std::string &name) { ExtractKeys(); }