...
/*virtual*/ void CEditForm::OnInitialUpdate()
{
...
{
  if(	!m_NrOfNodeX.Undefined() &&
    !m_NrOfNodeY.Undefined() &&
    !m_NrOfNodeZ.Undefined())
  {
    return TRUE;
  }
...
void CEditForm::OnButtonGenSurfaces() 
{
  ...
  m_pFrame->OnCutSurface(GetBoundingBox(), int(m_NrOfNodeX), int(m_NrOfNodeY), int(m_NrOfNodeZ), (m_pBox == 0), m_BaseName);
  m_IsGenerated=true;
  UpdateData(FALSE);
}
...
void CEditForm::Init(CCutSurfaceFrame*	frame,CCutSurfaceDlg* dlg,CDoubleQuantity::UNIT unit, BOOL calculateDefaultZoomBox,const geo::CBox* pBox)
{
  m_unit=unit;
  m_pDlg=dlg;	
  m_pFrame=frame;
