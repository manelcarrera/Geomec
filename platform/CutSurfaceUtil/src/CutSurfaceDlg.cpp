...
CCutSurfaceDlg::CCutSurfaceDlg(const geo::CBox &box, CDoubleQuantity::UNIT unit /*=CDoubleQuantity::SI_UNIT*/,
                               CWnd *pParent /*=NULL*/)
...
/*virtual*/ CCutSurfaceDlg::~CCutSurfaceDlg() { delete m_OrgSurfaces; }
...
int CCutSurfaceDlg::OnCreate(LPCREATESTRUCT lpCreateStruct) {
  ...
  if (!m_3DViewToolBar.CreateEx(m_pFrameWnd) || !m_3DViewToolBar.LoadToolBar(IDR_TOOLBAR_OPENGL_VIEW)) {
...
const CString &CCutSurfaceDlg::BaseName() const { return m_BaseName; }
...
/*virtual*/ void CCutSurfaceDlg::OnOK() { CDialog::OnOK(); }

/*virtual*/ void CCutSurfaceDlg::OnCancel() { CDialog::OnCancel(); }

const CNamedSurfaces &CCutSurfaceDlg::OrgSurfaces() const { return *m_OrgSurfaces; }

const std::vector<SIDE_ID> CCutSurfaceDlg::GetSides() const { return m_pIntersec->GetSides(); }

const geo::CTriSurface *CCutSurfaceDlg::GetSideSurfaces(SIDE_ID SideID) const {
  return m_pIntersec->GetSideSurface(SideID);
}

const std::map<SIDE_ID, geo::CTriSurface *> &CCutSurfaceDlg::getSideSurfaces() const {
  return m_pIntersec->getSideSurfaces();
}

int CCutSurfaceDlg::CutSurfacesCount() const { return m_pIntersec->CutSurfacesCount(); }

const STNamedSurface &CCutSurfaceDlg::GetCutSurface(int index) const { return m_pIntersec->GetCutSurfaces(index); }

const std::vector<STNamedSurface> &CCutSurfaceDlg::getCutSurfaces() const { return m_pIntersec->getCutSurfaces(); }
