...
CCutSurfaceFrame::CCutSurfaceFrame() : m_bShowMinMaxPoints(false) {
...
void CCutSurfaceFrame::Init(CCutsurfaceTreeView *treeview, COpenGLView *view, CCutSurfaceDlg *dlg,
...
void CCutSurfaceFrame::GetSideSurfaceMap(TNamedSurfMap &surfmap) { GetSurfaceMap(HDNAME_SIDE_SURFACE, surfmap); }

void CCutSurfaceFrame::GetCutSurfaceMap(TNamedSurfMap &surfmap) { GetSurfaceMap(HDNAME_CUT_SURFACE, surfmap); }

void CCutSurfaceFrame::OnCutSurface(const geo::CBox *pBox, int nodesX, int nodesY, int nodesZ, bool bShowMinMaxPoints,
                                    const CString &basename) {
