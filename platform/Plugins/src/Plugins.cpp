...
CPlugins::CPlugins(const std::string& plugins_path)
  : m_plugins_path(plugins_path)
{
  init();
}

std::string CPlugins::manual( std::string plugin_name ){ return has_manual( plugin_name ) ? m_manuals_map[ plugin_name ] : NOT_FOUND; }
...
bool CPlugins::provide_pythonroot( std::string dir )
{
  ...
  while( !found && !exit )
  {
    QString dir_ = QFileDialog::getExistingDirectory( nullptr /*parent*/, "Select PYRHONROOT directory", "c:/", QFileDialog::ShowDirsOnly );
  ...
  if( exit )
  {
    ...
    QMessageBox::warning( nullptr, QString("PYTHONROOT"), QString::fromStdString("PYTHONROOT not provided.\n\nProvide it editing\n\n 'PYTHONROOT' var in\n\n'" + plugins_xml ) );
  }
  return !exit;
}
...
bool CPlugins::installed( std::string name )
{ 
  return QUtil::exists( QUtil::url( m_plugins_path, name ), QUtil::Dir ); 
}
    ...
    if( QFileInfo( tmp_dir_path ).isDir() )
    {
      ...
      RemoveDir(dest.c_str());
            ...
            if( retval )
              ...
              printer->error( "installation of new plugin failed (and we may have removed an old one!)" ); // If this happens (a lot), we should make this more user friendly
          }
        }
      }
...
bool CPlugins::uninstall( int pos ) // last pos = -1
{
  ...
  if( pos < nE )
  {
    ...
    if( remove_dir_r )
    {
      printer->debug( "plugins : remove : dir : '%s' -> '%s' : %s", name.c_str(), dir( name ).c_str(), remove_dir_r ? "OK" : "ERROR" );
