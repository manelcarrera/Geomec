...
// Unfortunately, the tbb templates splice our TASK functor somewhere, so we can't use abstract base classes where it
// counts (the execute and sort methods). Instead, we also pass the TASK as a template variable.
//
// For now, we define both a Parallel and Sequential kernel, mainly for comparisons and to play around with different
// kinds of kernels. In the future, the Sequential kernel should probably go; and a GPUKernel might be added.
...
// For convenience:
#define MP_DUMMY_DATA                                                                                                  \
  std::vector<size_t> &data() const {                                                                                  \
    std::vector<size_t> *dummy = 0;                                                                                    \
    return *dummy;                                                                                                     \
  }

template <class TASK> class CKernelParallel : public IKernel {
  size_t grainSize;
...
template <class TASK> class CKernelSequential : public IKernel {
public:
  CKernelSequential() {}
...
template <class KERNEL, class TASK> class CDispatchedTask : public IDispatchedTask {
  KERNEL &m_kernel;
  TASK &m_task;
...
public:
  ...
  void Step(int n = 1) { m_counter.fetch_and_add(n); }
  ...
  bool Processed() const { return m_counter != 0; }
