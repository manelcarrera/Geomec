...
C3DWellPoint::C3DWellPoint(const double &dAHD, const double &dInclination, const double &dAzimuth, const double &dX,
...
C3DWellPoint::C3DWellPoint(const C3DWellPoint &WellPoint) : geo::CPoint(WellPoint) {
  ...
  if (WellPoint.DirectVectCalculated() && !m_bDirectCalc) { // calculate the directional vector for the first time.
    CalcDirectionalVec();
  } else if (WellPoint.DirectVectCalculated() &&
             m_bDirectCalc) { // The directional vector is no longer valid and should be recalculated.
    m_bDirectCalc = false;
