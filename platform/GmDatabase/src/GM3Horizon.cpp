...
const QString &C3DHorizon::Name() const { return m_strName; }
...
void C3DHorizon::LoadHorizon(const long nIndex, C3DDatabase &db, IProgressBase &callback) {
  ...
  m_ptMin = geo::CPoint(rs.GetFieldValue(FD_HORIZON_Y_MIN).dblVal, rs.GetFieldValue(FD_HORIZON_X_MIN).dblVal,
                        rs.GetFieldValue(FD_HORIZON_Z_MIN).dblVal);

  m_ptMax = geo::CPoint(rs.GetFieldValue(FD_HORIZON_Y_MAX).dblVal, rs.GetFieldValue(FD_HORIZON_X_MAX).dblVal,
    ...
    for (int x = 0; x < 10; x++) {
      for (int y = 0; y < 10; y++) {
        m_vcPoint.PushBack(geo::CPoint(m_ptMin.X() + (m_ptMax.X() - m_ptMin.X()) * ((double)x / 9.0),
                                       m_ptMin.Y() + (m_ptMax.Y() - m_ptMin.Y()) * ((double)y / 9.0), m_ptMax.Z()));
      }
    }
  } else {
    while (!rs.IsEOF()) {
      ...
      m_vcPoint.PushBack(geo::CPoint(rs.GetFieldValue(FD_HORIZON_PT_Y).lVal, rs.GetFieldValue(FD_HORIZON_PT_X).lVal,
...
BOOL C3DHorizon::ConstantDepth() const { return m_bConstantDepth; }
const C3DHorizon::TPointVec &C3DHorizon::Vertices() const { return m_vcPoint; }

const geo::IPoint &C3DHorizon::Min() const { return m_ptMin; }

const geo::IPoint &C3DHorizon::Max() const { return m_ptMax; }

int C3DHorizon::Color() const { return m_lColor; }
