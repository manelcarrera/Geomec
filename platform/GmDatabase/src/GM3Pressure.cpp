...
C3DPressure::C3DPressure() { assert(m_vcPoint.size() == 0); }

// ##ModelId=3BC55D4C0371
C3DPressure::~C3DPressure() {}

// ##ModelId=3BC55D4C0373
void C3DPressure::ReadPressure(C3DDatabase &db, const long lFormationKey, const int nTimeStep,
  ...
  m_ptMax = geo::CPoint(rs.GetFieldValue(FD_PRESSURE_MAX_Y).dblVal, rs.GetFieldValue(FD_PRESSURE_MAX_X).dblVal,
                        rs.GetFieldValue(FD_PRESSURE_MAX_Z).dblVal);
  m_ptMin = geo::CPoint(rs.GetFieldValue(FD_PRESSURE_MIN_Y).dblVal, rs.GetFieldValue(FD_PRESSURE_MIN_X).dblVal,
  ...
  if (m_type == OWCGWC) {
    ...
    while (!rs.IsEOF()) {
      ...
      m_vcPoint.push_back(TPressurePoint(
          geo::CPoint(rs.GetFieldValue(FD_PRESSURE_PT_Y).lVal, rs.GetFieldValue(FD_PRESSURE_PT_X).lVal, 0),
