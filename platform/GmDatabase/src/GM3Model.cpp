...
C3DModel::C3DModel() : m_bTimeAnalysis(TRUE) {}

// ##ModelId=3BC55D4D00F0
C3DModel::~C3DModel() {}

// ##ModelId=3BC55D4D00D2
const std::vector<C3DHorizon> &C3DModel::Horizons() const { return m_vcHorizon; }

// ##ModelId=3BC55D4D00D4
const std::vector<C3DFormation> &C3DModel::Formations() const { return m_vcFormation; }

const std::vector<C3DCrossSection> &C3DModel::CrossSections() const { return m_vcCrossSection; }

const std::vector<C3DWellPath> &C3DModel::WellPaths() const { return m_vcWellPath; }
...
void C3DModel::ReadMesh(C3DDatabase &db, IProgressBase &callback) {
  ...
  m_min = geo::CPoint(rs.GetFieldValue(FD_MESH_MIN_Y).dblVal, rs.GetFieldValue(FD_MESH_MIN_X).dblVal);

  m_max = geo::CPoint(rs.GetFieldValue(FD_MESH_MAX_Y).dblVal, rs.GetFieldValue(FD_MESH_MAX_X).dblVal);
...
int C3DModel::TimestepSize() const { return m_vcTimestep.size(); }

const CTimestep &C3DModel::Timestep(int nIndex) const { return m_vcTimestep[nIndex]; }
...
const geo::IPoint &C3DModel::Min() const { return m_min; }

const geo::IPoint &C3DModel::Max() const { return m_max; }

const double &C3DModel::RectangleSize() const { return m_dRectangleSize; }

const std::vector<C3DMeshZone> &C3DModel::MeshZones() const { return m_vcMeshZone; }
