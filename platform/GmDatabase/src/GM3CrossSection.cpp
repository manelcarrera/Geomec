...
const QString& C3DCrossSection::Name() const
{
  return m_strName;
}

const geo::IPoint& C3DCrossSection::First() const
{
  return m_point.first;
}

const geo::IPoint& C3DCrossSection::Second() const
{
...
void C3DCrossSection::ReadCrossSection(C3DDatabase &db, const long lCrossSectionKey, IProgressBase &callback)
{
  ...
  m_point.first = geo::CPoint(rs.GetFieldValue(FD_XSECTION_FIRST_Y).dblVal,
                rs.GetFieldValue(FD_XSECTION_FIRST_X).dblVal);

  m_point.second = geo::CPoint(rs.GetFieldValue(FD_XSECTION_SECOND_Y).dblVal,
                rs.GetFieldValue(FD_XSECTION_SECOND_X).dblVal);
