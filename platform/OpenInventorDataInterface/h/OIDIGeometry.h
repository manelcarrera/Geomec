...
public:
  OIDIGeometryI(const geo::IElementSet *parent) : m_Flag(HAS_ELEMENTS), m_Parent(0), m_PointSet(0), m_Box(0), m_TimeStamp(MxTimeStamp::getTimeStamp()), m_translation(0, 0, 0)
  {
  ...
  OIDIGeometryI(const geo::IArray *parent) : m_Parent(0), m_PointSet(parent), m_Box(0), m_TimeStamp(MxTimeStamp::getTimeStamp())
  , m_Flag(parent && parent->Size() > 0 && dynamic_cast<const geo::ILine *>(&parent->ObjectBase(0)) ? HAS_LINES : (dynamic_cast<const geo::IInterfaceElement *>(&parent->ObjectBase(0)) ? HAS_ELEMENTS : HAS_POINTS)), m_translation(0, 0, 0)
  {
  if (m_Flag == HAS_ELEMENTS)
  {
    ...
    m_PointSet = 0;
      ...
      assert(m_Parent); // actually, for now, we only deal with the situation where we have one (may change in the future)
  }
  }

  OIDIGeometryI(const geo::IBox *parent) : m_Parent(0), m_PointSet(0), m_Box(parent), m_TimeStamp(MxTimeStamp::getTimeStamp())
  , m_Flag(HAS_LINES), m_translation(0, 0, 0)
  {
  }

  const geo::IElementSet *Parent() const {
  return m_Parent;
  }
  const geo::IArray *PointSet() const {
  return m_PointSet;
  }
  ...
  {
    return MbVec3d(	m_deformation.points[ i ][ 0 ] - m_translation[0], 
            m_deformation.points[ i ][ 1 ] - m_translation[1], 
            m_deformation.points[ i ][ 2 ] - m_translation[2] );
  }
  else
    return MbVec3d( point->X() - m_translation[0], 
            point->Y() - m_translation[1], 
            point->Z() - m_translation[2] );
  }
  ...
  MbVec3d getTranslation() const
  {
    return MbVec3d( m_translation[0], m_translation[1], m_translation[2] );
  }
