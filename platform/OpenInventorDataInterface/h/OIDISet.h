...
public:
  ...
  public:
    ...
    Handle getHandle(std::size_t index) { assert(index >= 0 && index < m_Handles.size()); return m_Handles[index]; }
  ...
  TMeshVector::iterator begin()
  {
    return m_MeshVector.begin();
  }

  TMeshVector::iterator end()
  {
    return m_MeshVector.end();
  }
