...
template <typename T> class OIDIDatasetI : public MiDataSetI<T> {
public:
  OIDIDatasetI(size_t dataSize, MiDataSet::DataBinding binding = PER_CELL)
      : m_binding(binding), m_timestamp(MxTimeStamp::getTimeStamp()), m_begin(0), m_min(-1), m_max(-1) {
  ...
  T get(size_t i) const {
    if (i < m_begin || i >= m_begin + m_data.size())
      return T(DBL_UNDEFINED); // return some undefined value
    else
      return m_data[i - m_begin];
  }

  const T &operator[](size_t i) const {
    static const T undefined(DBL_UNDEFINED);
    if (i < m_begin || i >= m_begin + m_data.size())
      return undefined; // return some undefined value
    else
      return m_data[i - m_begin];
  }
  ...
  void updateTimeStamp() { m_timestamp = MxTimeStamp::getTimeStamp(); }

  size_t beginIndex() const { return m_begin; }

  size_t endIndex() const { return m_begin + m_data.size(); }
   ...
   */
  DataBinding getBinding() const { return m_binding; }
...
public:
  OIDITensorSetI(std::size_t dataSize, MiDataSet::DataBinding binding = PER_CELL) : OIDIDatasetI(dataSize, binding) {}
};
...
void GenerateEigenVectorsAndValuesFromTensors(const OIDITensorSetI &tensors, unsigned short component,
                                              OIDIVectorSetI *&vectors, OIDIScalarSetI *&values);
