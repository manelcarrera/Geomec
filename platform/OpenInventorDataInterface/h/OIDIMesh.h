...
public:
  OIDIMesh(const COpenGLNode *openGLNode, const geo::IElementSet *elementSet)
      : m_openGLNode(openGLNode), m_geometry(elementSet), m_objectSet(elementSet), m_min(DBL_MAX), m_max(-DBL_MAX) {}

  OIDIMesh(const COpenGLNode *openGLNode, const geo::IArray *pointSet)
      : m_openGLNode(openGLNode), m_geometry(pointSet), m_objectSet(pointSet), m_min(DBL_MAX), m_max(-DBL_MAX) {}

  OIDIMesh(const COpenGLNode *openGLNode, const geo::IBox *boxSet)
      : m_openGLNode(openGLNode), m_geometry(boxSet), m_objectSet(boxSet), m_min(DBL_MAX), m_max(-DBL_MAX) {}

  virtual ~OIDIMesh() {}

  enum MeshType { Unknown = 0, LineMesh = 1, SurfaceMesh = 2, VolumeMesh = 3, PointSetMesh = 4, PointMesh = 5 };

  // To avoid dynamic_casts ...
  virtual MeshType meshType() const { return Unknown; }
  ...
  const geo::IObject *getObjectSet() const { return m_objectSet; }

  MbVec3d getCoord(size_t i) const { return m_geometry.getCoord(i); }

  virtual OIDIScalarSetI *collectValuesByTopology(const IValueComponentBase *pValueComponent,
                                                  IQuantityDouble::UNIT unit, IProgressBase &progress) const = 0;
  virtual OIDIScalarSetI *collectGeometryValuesByTopology(const IValueComponentBase *pValueComponent,
                                                          IQuantityDouble::UNIT unit,
                                                          IProgressBase &progress) const = 0;
  virtual OIDIVectorSetI *collectVectorsByTopology(const IVectorResult::IVectorComponent *pVectorComponent,
                                                   IQuantityDouble::UNIT unit, IProgressBase &progress) const = 0;
  virtual OIDITensorSetI *collectTensorsByTopology(const IValueComponentBase *pTensorComponent,
                                                   IQuantityDouble::UNIT unit, IProgressBase &progress) const = 0;
  virtual OIDIVectorSetI *collectGeometryVectorsByTopology(const IVectorResult::IVectorComponent *pVectorComponent,
                                                           IQuantityDouble::UNIT unit,
                                                           IProgressBase &progress) const = 0;

  // Ditto, we don't own the returned pointer
  virtual OIDIScalarSetI *collectValuesByGeometry(const IValueComponentBase *pValueComponent,
                                                  IQuantityDouble::UNIT unit, IProgressBase &progress) const;
  virtual OIDIVectorSetI *collectVectorsByGeometry(const IVectorResult::IVectorComponent *pVectorComponent,
                                                   IQuantityDouble::UNIT unit, IProgressBase &progress) const;
  virtual OIDITensorSetI *collectTensorsByGeometry(const IValueComponentBase *pTensorComponent,
                                                   IQuantityDouble::UNIT unit, IProgressBase &progress) const;
...
public:
  OIDIMeshTemplate(const COpenGLNode *openGLNode, const geo::IElementSet *elementSet,
  ...
  const OIDIGeometryI &getGeometry() const { return OIDIMesh::getGeometry(); }

  const TopologyType &getTopology() const { return m_topology; }

  virtual const CellType *getCell(size_t id) const { return m_topology.getCell(id); }

  virtual size_t getNumCells() const { return m_topology.getNumCells(); }
  ...
  virtual OIDITensorSetI *collectTensorsByTopology(const IValueComponentBase *pTensorComponent,
                                                   IQuantityDouble::UNIT unit, IProgressBase &progress) const;
  virtual OIDITensorSetI *collectGeometryTensorsByTopology(const IValueComponentBase *pTensorComponent,
                                                           IQuantityDouble::UNIT unit, IProgressBase &progress) const;
};
...
public:
  OIDIMeshVU(const COpenGLNode *openGLNode, const geo::IElementSet *elementSet,
             const std::vector<OIDIVolumeCell> &cells)
  ...
  OIDIScalarSetI *collectValuesByGeometry(const IValueComponentBase *pValueComponent, IQuantityDouble::UNIT unit,
                                          IProgressBase &progress) const;
  OIDIVectorSetI *collectVectorsByGeometry(const IVectorResult::IVectorComponent *pVectorComponent,
                                           IQuantityDouble::UNIT unit, IProgressBase &progress) const;
  virtual OIDITensorSetI *collectTensorsByGeometry(const IValueComponentBase *pTensorComponent,
                                                   IQuantityDouble::UNIT unit, IProgressBase &progress) const;
};
...
public:
  OIDIMeshSU(const COpenGLNode *openGLNode, const geo::IElementSet *elementSet,
             const std::vector<OIDISurfaceCell> &cells)
  ...
  OIDIScalarSetI *collectFaultedValuesByTopology(IProgressBase &progress,
                                                 const IValueComponentBase *pValueComponent) const;

  OIDIScalarSetI *collectValuesByGeometry(const IValueComponentBase *pValueComponent, IQuantityDouble::UNIT unit,
                                          IProgressBase &progress) const;

  OIDIScalarSetI *collectFaultedValuesByGeometry(const IValueComponentBase *pValueComponent, IQuantityDouble::UNIT unit,
                                                 IProgressBase &progress) const;
  ...
  OIDIVectorSetI *collectFaultedVectorsByTopology(const IVectorResult::IVectorComponent *pVectorComponent,
                                                  IQuantityDouble::UNIT unit, IProgressBase &progress) const;

  virtual OIDIVectorSetI *collectVectorsByGeometry(const IVectorResult::IVectorComponent *pVectorComponent,
                                                   IQuantityDouble::UNIT unit, IProgressBase &progress) const;

  OIDIVectorSetI *collectFaultedVectorsByGeometry(const IVectorResult::IVectorComponent *pVectorComponent,
                                                  IQuantityDouble::UNIT unit, IProgressBase &progress) const;
  virtual OIDITensorSetI *collectTensorsByGeometry(const IValueComponentBase *pTensorComponent,
                                                   IQuantityDouble::UNIT unit, IProgressBase &progress) const;
};
...
public:
  OIDIMeshLU(const COpenGLNode *openGLNode, const geo::IElementSet *elementSet, const std::vector<OIDILineCell> &cells)
      : OIDIMeshTemplate(openGLNode, elementSet, cells) {
    checkConsistency(m_min, m_max);
  }
  OIDIMeshLU(const COpenGLNode *openGLNode, const geo::IArray *lineSet, const std::vector<OIDILineCell> &cells)
      : OIDIMeshTemplate(openGLNode, lineSet, cells) {
    checkConsistency(m_min, m_max);
  }
  OIDIMeshLU(const COpenGLNode *openGLNode, const geo::IBox *lineSet, const std::vector<OIDILineCell> &cells)
      : OIDIMeshTemplate(openGLNode, lineSet, cells) {
    checkConsistency(m_min, m_max);
  }
  virtual OIDIMesh::MeshType meshType() const;
...
public:
  OIDIPointSetMesh(const COpenGLNode *openGLNode, const geo::IArray *pointSet)
      : OIDIMesh(openGLNode, pointSet), m_topology(pointSet) {
    checkConsistency(m_min, m_max);
  }

  virtual const MiSurfaceTopologyExplicitI &getTopology() const { return m_topology; }

  virtual size_t getNumCells() const { return m_topology.getNumCells(); }

  virtual const MiCell *getCell(size_t id) const { return m_topology.getCell(id); }

  virtual OIDIMesh::MeshType meshType() const { return OIDIMesh::PointSetMesh; }
...
public:
  ...
  virtual OIDIMesh::MeshType meshType() const { return OIDIMesh::PointMesh; }
