...
OIDITensorVectorSetI::OIDITensorVectorSetI(size_t dataSize, MiDataSet::DataBinding binding,
                                           std::shared_ptr<OIDITensorSetI> tensors)
    : OIDIVectorSetI(dataSize, binding), m_tensors(tensors) {
...
OIDITensorVectorSignSetI::OIDITensorVectorSignSetI(size_t dataSize, MiDataSet::DataBinding binding,
                                                   std::shared_ptr<OIDITensorSetI> tensors)
    : OIDIScalarSetI(dataSize, binding), m_tensors(tensors) {
  SbVec3f normal(1, 0, 0);
  setNormal(normal, TensorVectorFull);
}
...
#ifndef NDEBUG
      MbVec3d eigenValueVector(eigenValueMatrix.Value(0, 0), eigenValueMatrix.Value(1, 1),
                               eigenValueMatrix.Value(2, 2));
#endif
      double eigenValue = eigenValueMatrix.Value(component, component);

      MbVec3d eigenVector(eigenValue * eigenVectorMatrix.Value(0, component),
...
void GenerateEigenVectorsAndValuesFromTensors(const OIDITensorSetI &tensors, unsigned short component,
                                              OIDIVectorSetI *&eigenVectors, OIDIScalarSetI *&eigenValues) {
  assert(component <= 2);
  component = 2 - component; // reverse as smallest = first
...
MbVec3d product(const ITensor &tensor, const SbVec3f &vector) {
  if (tensor.XX() == DBL_UNDEFINED)
    ...
    return MbVec3d(tensor.XX() * vector[0] + tensor.XY() * vector[1] + tensor.XZ() * vector[2],
...
bool IsUndefined(const double &val) { return val == DBL_UNDEFINED; }

bool IsUndefined(const MbVec3d &val) { return val[0] == DBL_UNDEFINED; }

bool IsUndefined(const CTensor &val) { return val.XX() == DBL_UNDEFINED; }

bool operator<(const CTensor &v1, const CTensor &v2) { return v1.Norm() < v2.Norm(); }
