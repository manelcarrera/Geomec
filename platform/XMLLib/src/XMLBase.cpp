...
CXMLBase::~CXMLBase() { delete m_DomDoc; }

QDomElement CXMLBase::Root() const { return m_DomDoc->documentElement(); }
...
QString CXMLBase::RootName() const { return Root().tagName(); }

void CXMLBase::Save(QTextStream &stream) const { stream << m_DomDoc->toString(); }

QDomNode CXMLBase::CurrentNode() { return m_CurrentNode; }

const QDomNode CXMLBase::CurrentNode() const { return m_CurrentNode; }

void CXMLBase::CurrentNode(QDomNode current) const { m_CurrentNode = current; }
