...
class CNodeObserver_Delegate : public CNodeObserver<OBSERVED_TYPE, CHILD_TYPE, CHILD_OBS_TYPE, RENAME, REMOVE> {
public:
  CNodeObserver_Delegate(OBSERVED_TYPE &node, CTreeNode &parent, const BOOL rename, const enum REMOVE_TYPE remove,
  ...
  CNodeObserver_Delegate(OBSERVED_TYPE &node, CTreeCtrl &ctrl, const BOOL rename, const enum REMOVE_TYPE remove,
  ...
  const DELEGATE_TYPE &Delegate() const {
    CDelegate *delegate = const_cast<CGraphNode &>(ObservedItem()).getDelegate();
    DELEGATE_TYPE *newDelegate = static_cast<DELEGATE_TYPE *>(delegate);

    return const_cast<const DELEGATE_TYPE &>(*newDelegate);
  }

  DELEGATE_TYPE &Delegate() { return static_cast<DELEGATE_TYPE &>(*ObservedItem().getDelegate()); }
};
