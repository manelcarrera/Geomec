...
template <class T> class CGraphEntryTemp_Delegate : public CGraphEntry_Delegate {
public:
...
template <class T>
CGraphEntryTemp_Delegate<T>::CGraphEntryTemp_Delegate(CGraphEntryTemp<T> *graphEntryTemp)
    : CGraphEntry_Delegate(graphEntryTemp), m_graphEntryTemp(graphEntryTemp) {
  ACTIVATE_TEMPLATE_DELEGATE(CGraphEntryTemp<T>, CGraphEntryTemp_Delegate<T>);
}

template <class T> bool CGraphEntryTemp_Delegate<T>::CanModify() const {
  CGraphEntryTemp<T>::TNodeSet stNodes = m_graphEntryTemp->EntryNodes();

  for (CGraphEntryTemp<T>::TNodeSet::const_iterator it = stNodes.begin(); it != stNodes.end(); it++) {
    CGraphNode_Delegate *graphNode_Delegate = static_cast<CGraphNode_Delegate *>((*it)->getDelegate());
...
template <class T> bool CGraphEntryTemp_Delegate<T>::Modify() { return false; }

template <class T> bool CGraphEntryTemp_Delegate<T>::CanDestroy() const { return m_graphEntryTemp->CanDestroy(); }

template <class T> bool CGraphEntryTemp_Delegate<T>::Destroy() { return m_graphEntryTemp->Destroy(); }
