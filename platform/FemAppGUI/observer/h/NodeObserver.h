...
public:
  CDummyNode(const CString strName) : CGraphNode((LPCSTR)strName) {}
};
...
public:
  CDummyNode *m_pDummy;
  CDummyObserver(CGraphNode &node, CTreeNode &parent, const BOOL rename, const enum REMOVE_TYPE remove,
  ...
  virtual BOOL CanEditTest() const {
    assert(FALSE);
    return false;
  }
...
public:
  ...
  CNodeObserver(OBSERVED_TYPE &node, CTreeNode &parent, const BOOL rename, const enum REMOVE_TYPE remove,
                HTREEITEM hInsertAfter = TVI_LAST, BOOL bInitialUpdate = TRUE, BOOL bSortAfterInsertion = TRUE)
      : CChildEnumerator<CHILD_TYPE>(parent.Ctrl(), parent.Handle(), hInsertAfter), m_node(node), m_remove(remove),
        m_rename(rename) {
  ...
  CNodeObserver(OBSERVED_TYPE &node, CTreeCtrl &ctrl, const BOOL rename, const enum REMOVE_TYPE remove,
  ...
  virtual CTreeNode *InsertChild(CHILD_TYPE &t) { return new CHILD_OBS_TYPE(t, *this, RENAME, REMOVE); }
  ...
  virtual BOOL CanModify() const {
    ...
    CGraphNode_Delegate *graphNode_Delegate = static_cast<CGraphNode_Delegate *>(m_node.getDelegate());
  ...
  virtual BOOL CanEditText() const { return m_rename == TRUE ? true : false; }

  virtual const CGraphNode &ObservedItem() const { return m_node; }
  virtual CGraphNode &ObservedItem() { return m_node; }
