...
CGraphMainFrameProgress::CGraphMainFrameProgress()
    : CProgressBase(), m_message(), m_graphMainFrm(dynamic_cast<CGraphMainFrm *>(FemAppGetMainWnd())),
      m_messageSet(false) {
  assert(m_graphMainFrm != 0);
}

CGraphMainFrameProgress::~CGraphMainFrameProgress() { m_graphMainFrm->EndProgress(); }
