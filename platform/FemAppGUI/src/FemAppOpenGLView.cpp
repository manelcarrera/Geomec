...
CFemAppOpenGLView::CFemAppOpenGLView() : m_bStep(FALSE) {}

CFemAppOpenGLView::~CFemAppOpenGLView() {}

void CFemAppOpenGLView::OnDestroy() { COpenGLView::OnDestroy(); }
...
void CFemAppOpenGLView::AssertValid() const { CView::AssertValid(); }

void CFemAppOpenGLView::Dump(CDumpContext &dc) const { CView::Dump(dc); }
#endif //_DEBUG
...
void CFemAppOpenGLView::OnUpdateEditDelete(CCmdUI *pCmdUI) { pCmdUI->Enable(MouseListener() != 0); }
...
void CFemAppOpenGLView::CopyClipboard() { CopyBitmap(); }
...
BOOL CFemAppOpenGLView::CopyBitmap(const CString &sPathName) {
  ...
  if (sizexView == 0 || sizeyView == 0)
    return FALSE; // no view to copy (?)
  ...
  for (int i = 0; i < sizey; i++) {
    int j;
    for (j = 0; j < linesizeLegend; j++)
      *(pos++) = *(posLegend++);
    for (j = 0; j < linesizeView; j++)
      *(pos++) = *(posView++);
  }
