...
bool COpenGLSceneNode::CDefaultListener::DispatchSelection(TKeyboardModifiers state, TMouseButton button, const TScreenPoint& point, pfHandler handler)
{
  // Get Selection
  ...
  for(itNode = stOpenGLNode.begin(); itNode != stOpenGLNode.end(); itNode++)
  {
    ...
    if(vcHit.size() > 0)
    {
      COpenGLNode_Delegate* pNode = static_cast <COpenGLNode_Delegate*> (
    ((COpenGLNode*) (*itNode))->getDelegate());
...
bool COpenGLSceneNode::CDefaultListener::MousePress(TKeyboardModifiers state, TMouseButton button, const TScreenPoint& point)
{
  return DispatchSelection(state, button, point,
  &COpenGLNode_Delegate::MousePress);
}

bool COpenGLSceneNode::CDefaultListener::MouseRelease(TKeyboardModifiers state, TMouseButton button, const TScreenPoint& point)
{
  return DispatchSelection(state, button, point,
  &COpenGLNode_Delegate::MouseRelease);
}

bool COpenGLSceneNode::CDefaultListener::MouseDblClk(TKeyboardModifiers state, TMouseButton button, const TScreenPoint& point)
{
  return DispatchSelection(state, button, point,
  &COpenGLNode_Delegate::MouseDblClk);
}

bool COpenGLSceneNode::CDefaultListener::MouseMove(TKeyboardModifiers state, TMouseButton button, const TScreenPoint& point)
{
  return DispatchSelection(state, button, point,
  &COpenGLNode_Delegate::MouseMove);
}

bool COpenGLSceneNode::CDefaultListener::OnOK()
{
  return m_bDestroy;
}

bool COpenGLSceneNode::CDefaultListener::OnDelete()
{
  return m_bDestroy;
}

bool COpenGLSceneNode::CDefaultListener::OnCancel()
{
  return m_bDestroy;
}

void COpenGLSceneNode::CDefaultListener::Destroy()
{
  m_bDestroy = TRUE;
}
...
void COpenGLSceneNode::CSceneOperationReceiver::OnBeginOperation()
{
  m_scene.OnBeginOperation();
}

void COpenGLSceneNode::CSceneOperationReceiver::OnEndOperation()
{
  m_scene.OnEndOperation();
}
//////////////////////////////////////////////////////////////////////
...
: CStorageNode(rhs), COpenGLScene(rhs),
  m_bFill(rhs.m_bFill),
  m_pSelected(0),
  m_bDirty(false),
  m_bProgress(false)
...
bool COpenGLSceneNode::IsOperation() const
{
  return m_pReceiver->IsOperation();
}
...
COpenGLSceneNode::TOpenGLNodeSet COpenGLSceneNode::NodesInSceneForLegend() const
{
  return NodesInScene();
}

std::vector<COpenGLSceneNode::TColor> COpenGLSceneNode::OnColor(const COpenGLNode& node, const geo::IObject &object) const
{
...
unsigned int COpenGLSceneNode::IconId() const
{
  return 0;
}

unsigned int COpenGLSceneNode::TypeId() const
{
  return 0;
}

bool COpenGLSceneNode::Fill() const
{
  return m_bFill;
}
...
bool COpenGLSceneNode::Empty() const
{
  return true;
}
...
void COpenGLSceneNode::SaveStream(TSTREAM& stream, TPROGRESS& progress)
{
  CStorageNode::SaveStream(stream, progress);
}

COpenGLSceneNode::eSelectMode COpenGLSceneNode::SelectMode() const
{
  return SM_SELECT;
}
...
bool COpenGLSceneNode::ReParent(IOpenGLFrame* pParent)
{
  if(IsLegendFrame())
  {
    ...
    CFemAppOpenGLView* pView =
      dynamic_cast<CFemAppOpenGLView*>(Parent());
...
/*!
  ...
  \sa LegendFrame()
*/
void COpenGLSceneNode::OnUpdateLegend()
{
...
/*!
  ...
  The OnInsertNodeInLegend() call back function is called so that the client can overide this function to filter the node.
  Default this function returns true.
  The function return the number of rejected nodes by OnInsertNodeInLegend() function.
...
*/
bool COpenGLSceneNode::OnInsertNodeInLegend(const COpenGLNode& node) const
{
  return true;
}
