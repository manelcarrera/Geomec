...
CArchiveCArchive::CArchiveCArchive(const QString& filename)
  : m_archive(0)
{
...
bool CArchiveCArchive::IsLoading() const
{
  return m_archive && m_archive->IsLoading();
}

bool CArchiveCArchive::IsStoring() const
{
  return m_archive && m_archive->IsStoring();
}
...
quint64 CArchiveCArchive::GetPosition()
{
  return m_archive->GetFile()->GetPosition();
}

void CArchiveCArchive::SetPosition(quint64 position)
{
  m_archive->GetFile()->Seek(position, CFile::begin);
}

void CArchiveCArchive::Flush()
{
  m_archive->Flush();
}

void CArchiveCArchive::Close()
{
  m_archive->Close();
}

void CArchiveCArchive::Abort()
{
  m_archive->Abort();
}
...
CArchiveCArchive& CArchiveCArchive::operator >> (QDateTime& dt)
{
  ...
  *m_archive >> oleDateTime;
  ...
  QDate date(oleDateTime.GetYear(), oleDateTime.GetMonth(),
  oleDateTime.GetDay());
  QTime time(oleDateTime.GetHour(), oleDateTime.GetMinute(),
