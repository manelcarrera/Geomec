...
COpenGLValueScene::COpenGLValueScene(CFemAppModel &model)
    : COpenGLSceneNode(model), m_pUnit(0), m_pValue(0), m_pComposite(0), m_pColorScale(0), m_pColorScaleEntry(0),
      m_bShowColor(true)
...
COpenGLValueScene::COpenGLValueScene(const QString &strName, CFemAppModel &model)
    : COpenGLSceneNode(strName, model), m_pUnit(0), m_pValue(0), m_pComposite(0), m_pColorScale(0),
      m_pColorScaleEntry(0), m_bShowColor(true) {
  init(model);
}

COpenGLValueScene::COpenGLValueScene(const unsigned int uName, CFemAppModel &model)
    : COpenGLSceneNode(uName, model), m_pUnit(0), m_pValue(0), m_pComposite(0), m_pColorScale(0), m_pColorScaleEntry(0),
      m_bShowColor(true) {
  init(model);
}

COpenGLValueScene::COpenGLValueScene(const COpenGLValueScene &rhs)
    : COpenGLSceneNode(rhs), m_pUnit(rhs.m_pUnit), m_pValue(rhs.m_pValue), m_pComposite(rhs.m_pComposite),
      m_pColorScale(rhs.m_pColorScale), m_pColorScaleEntry(rhs.m_pColorScaleEntry), m_bShowColor(rhs.m_bShowColor) {}
...
BOOL COpenGLValueScene::ShowColor() const { return m_bShowColor; }

void COpenGLValueScene::ShowColor(BOOL bShowColor) { ShowValue(!bShowColor); }

BOOL COpenGLValueScene::ShowValue() const { return !m_bShowColor; }

const CUnitNode &COpenGLValueScene::UnitNode() const { return *m_pUnit; }
...
const IValueComponentBase *COpenGLValueScene::ValueComponent() const { return m_pValue; }

IValueComponentBase *COpenGLValueScene::ValueComponent() { return m_pValue; }

const COpenGLValueScene::TMinMax &COpenGLValueScene::MinMaxValue() const { return m_MinMaxValue; }
...
const COpenGLValueScene::TMinMax &COpenGLValueScene::MinMax(IProgressBase &progressBase,
                                                            const COpenGLNode &node) const {
...
void COpenGLValueScene::ColorScale(IColorScaleNode &color_scale) { m_pColorScale = &color_scale; }

const IColorScaleNode *COpenGLValueScene::ColorScale() const { return m_pColorScale; }

IColorScaleNode *COpenGLValueScene::ColorScale() { return m_pColorScale; }

bool COpenGLValueScene::operator==(const COpenGLValueScene &rhs) const {
  if (!COpenGLSceneNode::operator==(rhs))
    return false;

  return ((m_pValue == rhs.m_pValue) && (m_pColorScale == rhs.m_pColorScale) && (m_MinMaxValue == rhs.m_MinMaxValue));
}
...
std::vector<CDrawDef::TColor> COpenGLValueScene::OnColor(const COpenGLNode &node, const geo::IObject &object) const {
  ...
  if (pElement) {
    ...
    CDrawDef::TColor col = m_pColorScaleEntry->CurrentColorScale().Color(
        ValueComponent()->ScalarData().ValuePoint(*pPoint, UnitNode().Unit()), MinMaxValue());
