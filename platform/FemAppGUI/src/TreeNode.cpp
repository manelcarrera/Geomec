...
//////////////////////////////////////////////////////////////////////
IGraphTreeObject::IGraphTreeObject(CTreeCtrl &ctrl)
: ITreeObject(ctrl)
{
}
...
CTreeNode::CObserverNode::CObserverNode(CTreeNode &tree_node) 
: CGraphNode(_T("")),  m_pTreeNode(&tree_node) {};

CTreeNode::CObserverNode::~CObserverNode()
{
  if(m_pTreeNode) m_pTreeNode->m_pObserver = 0;
  delete m_pTreeNode;
}
...
void CTreeNode::CObserverNode::OnNewNeighbour(const CGraphNode &node)
{
  ...
  if(m_pTreeNode) m_pTreeNode->OnNewNeighbour(node);	
}
...
void CTreeNode::CObserverNode::OnNeighbourModified(const CGraphNode &node, enum ModifiedHint uHint)
{
  ...
  // Redirected to the node observer ...
  if(m_pTreeNode) m_pTreeNode->OnNeighbourModified(node, uHint);
}
...
void CTreeNode::CObserverNode::OnNeighbourDeleted(const CGraphNode &node)
{
  ...
  // Redirected to the node observer ...
  if(m_pTreeNode) m_pTreeNode->OnNeighbourDeleted(node);
}

CTreeNode::CTreeNode(CTreeCtrl &ctrl)
: IGraphTreeObject(ctrl), m_pObserver(0)
{
}
...
QString CTreeNode::Text() const
{
  return ObservedItem().Name();
}

BOOL CTreeNode::CanEditText() const
{
  return FALSE;
}
...
unsigned int CTreeNode::Icon() const
{
  return ObservedItem().IconId();
}
...
BOOL CTreeNode::CanDelete() const
{
