...
CGraphNode *CGraphMainFrm::Paste() { return m_clip_board.Ptr(); }

const CGraphNode *CGraphMainFrm::Paste() const { return m_clip_board.Ptr(); }
...
void CGraphMainFrm::StartProgress(const CString &sCaption, int nSteps) {
  if (m_ProgressStack.empty())
    m_sStatusText = StatusBar().GetPaneText(0);
  else
    DestroyProgress();
...
void CGraphMainFrm::EndProgress() {
  ...
  if (m_ProgressStack.empty())
    StatusBar().SetPaneText(0, m_sStatusText, TRUE);
  else
    CreateProgress(m_ProgressStack.top());
}
...
int CGraphMainFrm::CProgressStackItem::Pos() const { return m_pos; }

int CGraphMainFrm::CProgressStackItem::MaxPos() const { return m_maxpos; }

int CGraphMainFrm::CProgressStackItem::SkipPos() const { return m_skippos; }

const CString &CGraphMainFrm::CProgressStackItem::Caption() const { return m_caption; }
