...
template <class CHILD_TYPE> class CChildEnumerator : public CTreeNode {
  std::set<CTreeNode *> m_stGenerated;

public:
  typedef CHILD_TYPE child_type; // Type of the child ...
  CChildEnumerator(CTreeCtrl &ctrl, HTREEITEM hParent,
                   HTREEITEM hInsertAfter); // Constructor ...
  CChildEnumerator(IGraphTreeObject &tree_object, HTREEITEM hInsertAfter);
...
template <class CHILD_TYPE> void CChildEnumerator<CHILD_TYPE>::Update() {
  bool bInsert = false;
...
template <class CHILD_TYPE> void CChildEnumerator<CHILD_TYPE>::OnChildDeleted(CTreeNode &child) {
  m_stGenerated.erase(&child);
}

template <class CHILD_TYPE> BOOL CChildEnumerator<CHILD_TYPE>::OnFilter(const CHILD_TYPE &t) const { return TRUE; }
...
template <class CHILD_TYPE>
CChildEnumerator<CHILD_TYPE>::CChildEnumerator(CTreeCtrl &ctrl, HTREEITEM hParent, HTREEITEM hInsertAfter)
    : CTreeNode(ctrl, hParent, hInsertAfter) {}

template <class CHILD_TYPE> void CChildEnumerator<CHILD_TYPE>::OnChildModified(CTreeNode &child) { SortChildren(); }
