...
#define Py_UNBLOCK_THREADS _save =
  PyEval_SaveThread();
#define Py_END_ALLOW_THREADS    PyEval_RestoreThread(_save); \
...

  {
    ...
    TPyObjectPtrV params_v =
        cmd_v.size() > 1 ? wrapper::parse<TPyObjectPtrV>(cmd_v[IT_Data::ePyTuplePos::Params]) : TPyObjectPtrV{};

    TPyObjectPtrV data_v = cmd_v.size() > 2 ? wrapper::parse<TPyObjectPtrV>(cmd_v[IT_Data::ePyTuplePos::Data_])
                                            : TPyObjectPtrV{}; // depletion -> 2x obj, phase II -> only 1x obj

    switch (type_(cmd_id)) {
    case Cmd_:
      cmd = Command(cmd_id, nullptr);
      break;
    case Cmd_Params:
      cmd = Command(cmd_id, new IT_Data(params(params_v)));
      break;
    case Cmd_Params_Data: {
      IT_Data *data = nullptr;
      switch (cmd_id) {
      case Command::IT1_Set_Depletion:
        data = depletion(data_v, params_v);
        break; // Depletion
      // Phase II
      case Command::DB_PointSet_Create:
        data = gen_3d(data_v[0], params_v);
        break;

      case Command::DB_PointSet_AddValues:
      case Command::DB_PointSet_ReplaceValues:
        data = gen_1d(data_v[0], params_v);
        break;

      case Command::DB_Material_SetValues:
        data = material(data_v, params_v);
        break; // FIXME
      default:
        Printer::instance()->debug("GeomecPythonInterface : send_cmd : data gen TO IMPLEMENT : cmd:[%s]",
                                   cmd.name().c_str());
        break;
      }
  ...
  if (data) {
    ...
    for (int i = 0; i < tuple.size(); i++) // objects
    {
      ...
      case DataType::String:
        obj_v.push_back(wrapper::build<const char *>((const char *)obj.data));
        break;
      case DataType::Double:
        obj_v.push_back(wrapper::build<double>(*(double *)obj.data));
        break;
      case DataType::Integer:
        obj_v.push_back(wrapper::build<int>(*(int *)obj.data));
        break;
      default:
        obj_v.push_back(wrapper::build<TData>(obj));
        break;
      }
    }
