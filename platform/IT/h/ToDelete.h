#pragma once
  ...
  //TFormationBaseEntry::TNodeSet formations = static_cast< TFormationBaseEntry* >( model.GraphEntry( MD_BASE_MESH_RESULT ) )->EntryNodes();
...
void getExportResultData( CExportResultData* exportResultData )
{
  ...
  m_RGObserver = QSharedPointer <CExportResultGroupObserver> (
    new CExportResultGroupObserver(	*this, 
                    m_model.ResultTree(), 
                    m_tcValue,
                    (m_exportResultData == 0 ? TResultComponentSet() :
                                  m_exportResultData->m_RGObserverData ) ) );
  ...
  void get_formation_data( const std::string _name )
  {
    ...
    for( TFormationBaseEntry::TNodeSet::iterator it = formations.begin(); it != formations.end(); ++it )
    {
      ...
      if( name.compare( _name ) == 0 )
      {
        ...
        {
          Printer::_print_format( "%.0f\t%E\t%E\t%E", 
        ...
        for(int e = 0; e < es2.ElementSize(); e++)  //elems
        {
          Printer::_print_format( "%.0f %.0f %.0f %.0f", 
            elems[ e ][ 0 ],
            elems[ e ][ 1 ],
            elems[ e ][ 2 ],
            elems[ e ][ 3 ] );
