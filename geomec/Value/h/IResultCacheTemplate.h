...
template<class T>
class IResultCacheTemplate : public IResult
{
...
public:
  ...
  typedef CValueType::MAP_TYPE TMapType;
  const T& ValuePoint(const geo::IPoint& point, 
            const CResultRegister& result_register,
            const CDepletionStage& stage,
            bool bLinear) const;
  const TValueVec& ValueElement(const geo::IElement& element, 
                              const CResultRegister& result_register,
                      const CDepletionStage& stage,
               			  bool bLinear) const;
  const TValueVec& MapValueElement(const geo::IElement& element,
                                 TMapType map_type,
                                 const CResultRegister& result_register,
                         const CDepletionStage& stage,
                         bool bLinear) const;
...
protected:
  virtual void onSetValuePoint(const geo::IPoint& point,
                             T& value, 
                 const CResultRegister& result_register,
                 const CDepletionStage& stage,
                 bool bLinear) const = 0;
  virtual void onSetValueElement(const geo::IElement& element,
                               TValueVec& value, 
                   const CResultRegister& result_register,
                   const CDepletionStage& stage,
                   bool bLinear) const = 0;
  virtual void onSetMapValueElement(const geo::IElement& element,
                                  TValueVec& value, 
                    TMapType map_type,
                      const CResultRegister& result_register,
                      const CDepletionStage& stage,
