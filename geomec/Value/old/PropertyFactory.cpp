...
//////////////////////////////////////////////////////////////////////
CPropertyFactory::IFormat::IFormat(CPropertyFactory& factory)
: m_factory(factory)
{
}

const CPropertyFactory& CPropertyFactory::IFormat::Factory() const
{
  return m_factory;
}

CPropertyFactory& CPropertyFactory::IFormat::Factory()
{
  return m_factory;
}
...
//////////////////////////////////////////////////////////////////////
CPropertyFactory::INodalFormat::INodalFormat(CPropertyFactory& factory)
: IFormat(factory), m_nHeaderCount(0)
{
...
int CPropertyFactory::INodalFormat::RowSize() const
{
  return Factory().PointSet().PointSize();
}
...
enum CToken::TokenRet CPropertyFactory::INodalFormat::ParseHeaderTag(const CString &sHeaderTag)
{
  ...
  if(!Factory().IsPointSet() && (Factory().Format() == NODAL))
  {
    if((m_mpCoordinate.find(COORD_X) == m_mpCoordinate.end()) || (m_mpCoordinate.find(COORD_Y) == m_mpCoordinate.end()))
    {
...
: CValueTypeFactory(((IPointSet&) point_set).Model()), 
  m_state(WRITE_NONE), 
  m_format(format), 
  m_pNodalFormat(0),
  m_unit(CQuantity::SI_UNIT),
...
BOOL CPropertyFactory::IsPointSet() const
{
...
enum CPropertyFactory::FORMAT CPropertyFactory::Format() const
{
  return m_format;
}

enum CPropertyFactory::STATE CPropertyFactory::State() const
{
  return m_state;
}

void CPropertyFactory::FormatError(const CString& sErrorMessage)
{
  m_state = FORMAT_ERROR;
}
...
int CPropertyFactory::CommentSize() const
{
  return m_vcComment.size();
}
