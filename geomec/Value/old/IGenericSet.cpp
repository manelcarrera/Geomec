...
IGenericSet::IGenericSet(CGraphModel &model) : COpenGLNode(model) {}

IGenericSet::IGenericSet(const CString &strName, CGraphModel &model) : COpenGLNode(strName, model) {}

IGenericSet::IGenericSet(const CString &sName, CGraphModel &model, int nValueSetSize) : COpenGLNode(sName, model) {
  for (int i = 0; i < nValueSetSize; i++)
    AddValueSet();
  ASSERT(nValueSetSize == ValueSetSize());
}

IGenericSet::IGenericSet(const IGenericSet &rhs) : COpenGLNode(rhs), m_vcValueSet(rhs.m_vcValueSet) {
...
int IGenericSet::ValueSetSize() const { return m_vcValueSet.size(); }
...
long IGenericSet::SavedItems() const { return 1; }
