...
CDerivedResult_Delegate::CDerivedResult_Delegate(CDerivedResult *derivedResult)
    : IResult_Delegate(derivedResult), m_derivedResult(derivedResult) {}
...
bool CDerivedResult_Delegate::Attributes() {
  ...
  if (name == "class CDerivedResult::CDerivedResultComponent") {
    ...
    valueComponent_Delegate =
        static_cast<CDerivedResult_Delegate::CDerivedResultComponent_Delegate *>(anotherValueComponent.getDelegate());
  } else {
    assert(false);

    valueComponent_Delegate = static_cast<IValueComponent_Delegate *>(valueComponent.getDelegate());
  }
...
void CDerivedResult_Delegate::AppendContextMenu(CContextMenuInvoker &invoker) {
  ...
  invoker.AddCommand("&Export results",
                     *(new TCommand(*this, &CDerivedResult_Delegate::Export, &CDerivedResult_Delegate::CanExport)));
  invoker.AddSeparator();

  IValueComposite_Delegate::AppendContextMenu(invoker);
}

bool CDerivedResult_Delegate::CanExport() const { return m_derivedResult->CanExport(); }
...
bool CDerivedResult_Delegate::Destroy() { return m_derivedResult->Destroy(); }

CDerivedResult_Delegate::CDerivedResultComponent_Delegate::CDerivedResultComponent_Delegate(
    CDerivedResult::CDerivedResultComponent *derivedResultComponent)
    : IResultComponent_Delegate(derivedResultComponent), m_derivedResultComponent(derivedResultComponent) {}

bool CDerivedResult_Delegate::CDerivedResultComponent_Delegate::Attributes() {
  CDerivedResult &result = dynamic_cast<CDerivedResult &>(m_derivedResultComponent->Parent());
  ...
  if (0 > result.Index()) {
    if (!result.RegisterResult()) {
      ...
      if (result.StoreOnFile()) {
        ...
        sMessage =
            QString("The maximum of %1 registry results is reached.").arg(DERIVED_RESULT_BASE - REGISTRY_RESULT_BASE);
      }
  ...
  if (m_derivedResultComponent->Stack() == 0) {
    ...
    CModelBase &model = dynamic_cast<CModelBase &>(m_derivedResultComponent->Model());

    new CDerivedResult::CResultCoordinateProxy(IDS_RC_NORTHING, *m_derivedResultComponent->Stack(),
                                               CDerivedResult::CResultCoordinateProxy::CO_X);
    new CDerivedResult::CResultCoordinateProxy(IDS_RC_EASTING, *m_derivedResultComponent->Stack(),
                                               CDerivedResult::CResultCoordinateProxy::CO_Y);
    new CDerivedResult::CResultCoordinateProxy(IDS_RC_DEPTH, *m_derivedResultComponent->Stack(),
    ...
    if (p3DModel && result.StoreOnFile()) {
      ...
      new CRpnTopDisplacementProxy<0, IDS_MEASURED_TOP_DISPL_N>(*p3DModel, *m_derivedResultComponent->Stack());
      new CRpnTopDisplacementProxy<1, IDS_MEASURED_TOP_DISPL_E>(*p3DModel, *m_derivedResultComponent->Stack());
      new CRpnTopDisplacementProxy<2, IDS_MEASURED_TOP_DISPL_D>(*p3DModel, *m_derivedResultComponent->Stack());
    }
    ...
    if (result.StoreOnFile()) {
      ...
      const TPointSetEntry &ps_entry =
          static_cast<const TPointSetEntry &>(*m_derivedResultComponent->Model().GraphEntry(MD_BASE_POINTSET));
      ...
      for (it = stPointSetNodes.begin(); it != stPointSetNodes.end(); ++it) {
        ...
        for (i = dim; i < ps.ValueSetSize(); ++i) {
          ...
          {
            new CRpnValueSet::CValueSetProxy(ps.ValueSet(i), *m_derivedResultComponent->Stack());
          }
        }
      }
    }
...
void CDerivedResult_Delegate::CDerivedResultComponent_Delegate::AppendContextMenu(CContextMenuInvoker &invoker) {}

CDerivedResultGroup_Delegate::CDerivedResultGroup_Delegate(CDerivedResultGroup *derivedResultGroup)
    : CResultGroup_Delegate(derivedResultGroup), m_derivedResultGroup(derivedResultGroup) {}
...
void CDerivedResultGroup_Delegate::AppendContextMenu(CContextMenuInvoker &invoker) {
  ...
  invoker.AddCommand(_T("&Create result"),
                     *(new TCommand(*this, &CDerivedResultGroup_Delegate::OnCreateDerivedResult)));
}
...
void CDerivedResultGroup_Delegate::OnCreateDerivedResult() {
  ...
  CDerivedResult_Delegate *pNewResult_Delegate = new CDerivedResult_Delegate(pNewResult);
