...
void IElementSet_Delegate::Export()
{
  ...
  CTnoFileDialog dlg(FALSE, "dat", m_elementSet->Name().toStdString().c_str(),
...
: IElementSet_Delegate(elementSet)
, m_elementSet(elementSet)
{
}
...
void CElementSet_Delegate::AppendContextMenu(CContextMenuInvoker& invoker)
{
  invoker.AddCommand(_T("A&ssign to model boundary"),
    *(new CSingleCommandTemplate <CElementSet> (*m_elementSet,
      &CElementSet::AssignToBoundary, &CElementSet::CanAssignToBoundary)));
  invoker.AddCommand(_T("&Export"),
    *(new CSingleCommandTemplate <CElementSet_Delegate> (*this,
    &CElementSet_Delegate::Export)));
