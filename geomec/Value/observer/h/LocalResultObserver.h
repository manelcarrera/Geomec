...
public:
  ...
  public:
    ...
    virtual const CGraphNode &ObservedItem() const { return m_component; }

    virtual CGraphNode &ObservedItem() { return m_component; }
    ...
    virtual void ToggleState() { ValueTypeObserver().ParentObserver().Dlg().Select(&m_component); }
...
public:
  ...
  virtual const CGraphNode &ObservedItem() const { return m_value_type; }

  virtual CGraphNode &ObservedItem() { return m_value_type; }
  ...
  virtual unsigned int StateIcon() const { return ParentObserver().ResultStateIconId(m_value_type); }

  virtual void ToggleState() { ParentObserver().ResultToggleState(m_value_type); }

  virtual void AppendContextMenu(CContextMenuInvoker &invoker) {}

  virtual CTreeNode *InsertChild(CValueComponent &component) { return new CComponentObserver(component, *this); }

  virtual BOOL OnFilter(const CValueComponent &t) const { return t.Parent().ComponentSize() > 1; }
};

template <class OWNER> class CGVTResultGroupObserverTempl : public IResultGroupObserver {
  OWNER &m_dlg;
...
public:
  CGVTResultGroupObserverTempl(CResultGroup &group, IResultGroupObserver &parent, OWNER &dlg)
      : IResultGroupObserver(group, parent), m_dlg(dlg) {}
...
template <class OWNER> class CGVTResultGroupObserverSingleSelectTempl : public CGVTResultGroupObserverTempl<OWNER> {
public:
  ...
  virtual CTreeNode *InsertChild(CGraphNode &node) {
    if (dynamic_cast<const TDeltaT *>(&node) || dynamic_cast<const TDeltaV *>(&node))
      return new CValueTypeObserverTempl<CGVTResultGroupObserverSingleSelectTempl<OWNER>>(
          static_cast<CValueType &>(node), *this);
    else
      return IResultGroupObserver::InsertChild(node);
  }
...
template <class OWNER> class CLocalResultGroupObserver : public IResultGroupObserver {
  const OWNER &m_Owner;
...
public:
  ...
  virtual ~CLocalResultGroupObserver() { const_cast<OWNER &>(m_Owner).AddResults(Results()); }

  void newSelection(const TResultComponentSet &newSelection) { m_stResultComponent = newSelection; }
  ...
  virtual BOOL CanInsertResultGroup(const CResultGroup &group) const { return Valid(group); }

  virtual BOOL CanInsertResult(const IResult &result) const { return m_Owner.Valid(result); }
