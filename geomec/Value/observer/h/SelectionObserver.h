...
public:
  CSelectionLeafObserver(OBSERVED_TYPE &node, CTreeNode &parent, const BOOL rename, const enum REMOVE_TYPE remove,
                         HTREEITEM hInsertAfter = TVI_LAST, BOOL bInitialUpdate = TRUE, BOOL bSortAfterInsertion = TRUE)
      : CNodeObserver<OBSERVED_TYPE, CDummyNode, CDummyObserver, FALSE, FIXED_ITEM>(
            node, parent, rename, remove, hInsertAfter, bInitialUpdate, bSortAfterInsertion),
        m_pHolder(0), m_selected(0), m_enabled(0), m_select(0) {
    assert(false);
  } // Called by base ...

  CSelectionLeafObserver(OBSERVED_TYPE &node, CTreeNode &parent, STATE_HOLDER &state, TGetSelected isSelected,
...
public:
  ...
  CSelectionBranchObserver(OBSERVED_TYPE &node, CTreeNode &parent, const BOOL rename, const enum REMOVE_TYPE remove,
  ...
  CSelectionBranchObserver(OBSERVED_TYPE &node, CTreeCtrl &view, STATE_HOLDER &state, TGetSelected isSelected,
                           TSetSelected setSelected, TGetSelected enabled = 0, BOOL rename = FALSE,
                           enum REMOVE_TYPE remove = FIXED_ITEM)
      : CNodeObserver<OBSERVED_TYPE, CHILD_TYPE, CHILD_OBS_TYPE, RENAME, REMOVE>(node, view, rename, remove, TVI_ROOT,
                                                                                 TVI_LAST, FALSE),
        m_pHolder(&state), m_selected(isSelected), m_enabled(enabled), m_select(setSelected) {
    Update();
  }
  ...
  // Construction for root
  CSelectionBranchObserver(OBSERVED_TYPE &node, CTreeNode &parent, STATE_HOLDER &state, TGetSelected isSelected,
                           TSetSelected setSelected, TGetSelected enabled, BOOL rename, enum REMOVE_TYPE remove)
      : CNodeObserver<OBSERVED_TYPE, CHILD_TYPE, CHILD_OBS_TYPE, FALSE, FIXED_ITEM>(node, parent, rename, remove,
                                                                                    TVI_LAST, FALSE),
        m_pHolder(&state), m_selected(isSelected), m_enabled(enabled), m_select(setSelected) {
    Update();
  }
...
public:
  ...
  virtual void ToggleState() {
    ...
    for (TChildren::iterator it = children.begin(); it != children.end(); it++) {
      ...
      while (pObserver->StateIcon() != icon)
