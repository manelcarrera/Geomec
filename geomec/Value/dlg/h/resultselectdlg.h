...
public:
  ...
  void AddResults(const TResultComponentSet &results) { m_Results.insert(results.begin(), results.end()); }
  ...
  CResultSelectDlg(CModelBase &model, BOOL bGlobal,
  ...
  public:
    static struct TResultsFromDialog {
    } resultsFromDialog;

    CResultComponentSetIterator(CResultSelectDlg *resultSelectDlg);
    CResultComponentSetIterator(CResultSelectDlg *resultSelectDlg, TResultsFromDialog &resultsFromDialog);

    const IValueComponentBase *get();

  private:
    CResultComponentSetIterator(const CResultComponentSetIterator &rhs);
    CResultComponentSetIterator &operator=(const CResultComponentSetIterator &rhs);
...
private:
  ...
  void UpdateTimeStep();
  void onAnalysisType(int dialogItem, CAnalysisType::TAnalysisType analysisType);
