...
CString CComponentConvertDlg::DlgText() const
{
  ...
  CString sMessage;
  sMessage.Format("Convert component %s of %s %s to %s %s.", 
          m_value_type.Name(), 
          sSource, 
          m_value_type.PointSet().Name(),
          sTarget,
          m_target.Name());
  return sMessage;
}
...
void CComponentConvertDlg::DoDataExchange(CDataExchange* pDX)
{
  ...
  BOOL bEnable = dynamic_cast<const IElementSet*>(&m_value_type.PointSet()) &&
          dynamic_cast<const IElementSet*>(&m_target);
