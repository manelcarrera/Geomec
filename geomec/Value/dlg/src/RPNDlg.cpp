...
unsigned int CRpnDlg::CNewItemListObject::Icon() const { return IDI_NEW_ITEM; }
...
void CRpnDlg::CNewItemListObject::SelectMaterialItem() {
  ...
  rpn::CRpnStack::TValueProxyVec valueProxyVector = m_dlg.RpnStack().ValueProxies();

  for (size_t ii = 0; ii < valueProxyVector.size(); ++ii) {
    stAvailableOperands.insert(valueProxyVector[ii]->TextTag().toStdString().c_str());
  }

  CFillMaterialParameterProxies fillMaterialParameterProxies(m_dlg.Model().Mesh(), m_dlg.RpnStack());
  std::vector<IRpnMaterialParameterProxy *> matProxies = fillMaterialParameterProxies.getMaterialParameterProxies();
  ...
  if (dlg.DoModal() == IDOK) {
    ...
    for (std::vector<IRpnMaterialParameterProxy *>::const_iterator selectedProxy = selectedProxies.begin();
         selectedProxy != selectedProxies.end(); ++selectedProxy) {
      bool found = stAvailableOperands.find((*selectedProxy)->TextTag().toStdString()) != stAvailableOperands.end();

      for (int i = 0; !found && (i < m_dlg.ListCtrl().GetItemCount()); ++i) {
        CNewItemListObject *newItemListObject = (CNewItemListObject *)m_dlg.ListCtrl().GetItemData(i);

        found = found || (newItemListObject->Text() == (*selectedProxy)->TextTag());
      }
  ...
  for (size_t i = 0; i < matProxies.size(); ++i) {
    // waij TFS 101571 do not allow duplicate material items
    //
    std::set<std::string>::const_iterator nameIt = stAvailableOperands.find(matProxies[i]->TextTag().toStdString());
    ...
    if (!foundAvailable)
      for (std::vector<IRpnMaterialParameterProxy *>::const_iterator selectedProxy = selectedProxies.begin();
           !foundCreated && (selectedProxy != selectedProxies.end()); ++selectedProxy) {
...
void CRpnDlg::CNewItemListObject::SelectResultItem() {
  ...
  if (dlg.DoModal() == IDOK) {
        ...
        if (CheckUniqueName(name)) {
          CDerivedResult::CDerivedResultComponent::CValueComponentProxy *pProxy =
              new CDerivedResult::CDerivedResultComponent::CValueComponentProxy(
                  name.c_str(), const_cast<IValueComponentBase &>(*value_component), m_dlg.RpnStack(),
                  dlg.FixedLinStage());
    ...
    } else // outputs
    {
      ...
      CResultSelectDlg::CResultComponentSetIterator resultComponentSetIterator(
          &dlg, CResultSelectDlg::CResultComponentSetIterator::resultsFromDialog);
      ...
      while ((valueComponent = resultComponentSetIterator.get()) != 0) {
        ...
        if (CheckUniqueName(name)) {
          CDerivedResult::CDerivedResultComponent::CValueComponentProxy *pProxy =
              new CDerivedResult::CDerivedResultComponent::CValueComponentProxy(
                  name.c_str(), const_cast<IValueComponentBase &>(*valueComponent), m_dlg.RpnStack(),
                  dlg.FixedLinStage());
          pProxy->StoreOnFile(!m_dlg.StoreGlobal());
          new CValueProxyListObject(m_dlg, *pProxy, 1);
        }
...
QString CRpnDlg::CUnAssignedListObject::Text() const { return QString("<") + (LPCSTR)m_sDescription + ">"; }

unsigned int CRpnDlg::CUnAssignedListObject::Icon() const { return IDI_UNDEFINED; }
...
CRpnDlg::CRpnDlg(CRpnValueSet &value_set, CWnd *pParent)
    : CRpnDlgBase(value_set.RpnStack(), CRpnDlg::IDD, pParent), m_sName(value_set.Name().toStdString().c_str()),
      m_pResult(0), m_pValueSet(&value_set) {
  m_sName.TrimLeft(" ");
  if (m_sName.IsEmpty())
...
CRpnDlg::CRpnDlg(CDerivedResult::CDerivedResultComponent &result, CWnd *pParent)
    : CRpnDlgBase(result.RpnStack(), CRpnDlg::IDD, pParent), m_sName(result.Name().toStdString().c_str()),
      m_pResult(&result), m_pValueSet(0) {
  m_sName.TrimLeft(" ");
  if (m_sName.IsEmpty())
    m_bNameEqualToFormula = TRUE;
  else
...
void CRpnDlg::OnBtEditFormula() {
  ...
  const std::set<std::string> &matProxyNames = CFillMaterialParameterProxies::GetNames(Model());
  ...
  operandNames.insert(CMeshResultTree::getNames().begin(), CMeshResultTree::getNames().end());

  CRpnEditFormulaDlg dlg(Model(), RpnStack().Formula().toStdString(), operandNames, output);
  ...
  while (output.size() > 0) {
    ...
    if (word.type == Shunt::CShunt::E_OPAND) // operand
    {
      ...
      } else // named operand
      {
        ...
        if (word.data == "pi") // wjrx mantis 2558
        {
          ...
          for (size_t ii = 0; ii < valueProxyVector.size(); ++ii) {
            if (word.data == valueProxyVector[ii]->TextTag().toStdString()) {
              // Identical to
          ...
          if (foundIt == false) // attempt to create material/result item
          {
            IRpnMaterialParameterProxy *newproxy1 =
                CFillMaterialParameterProxies::CreateMaterialParameterProxy(Model(), word.data, RpnStack());
            ...
            } else // attempt to create result item
            {
              ...
              if (resultInfo.Result()) {
                const IValueComponentBase &component = resultInfo.Result()->Component(resultInfo.Component(), 0);
                CDerivedResult::CDerivedResultComponent::CValueComponentProxy *newproxy2 =
                    new CDerivedResult::CDerivedResultComponent::CValueComponentProxy(
                        word.data.c_str(), const_cast<IValueComponentBase &>(component), RpnStack(),
    ...
    } else // binary operator or function
    {
      if (word.data == "arcsin")
        OnBtArcSin();
      else if (word.data == "arccos")
        OnBtArcCos();
      else if (word.data == "arctan")
        OnBtArcTan();
      else if (word.data == "round")
        OnBtRound();
      else if (word.data == "if") {
        ;
      } // load stack and wait for 'else'
      else if (word.data == "then") {
        ;
      } // load stack and wait for 'else'
      else if (word.data == "else")
        OnBtIf();
      else if (word.data == "sign")
        OnBtSignFunction();
      else if (word.data == "sqrt")
        OnBtSqrt();
      else if (word.data == "sinh")
        OnBtSinh();
      else if (word.data == "cosh")
        OnBtCosh();
      else if (word.data == "tanh")
        OnBtTanh();
      else if (word.data == "abs")
        OnBtAbs();
      else if (word.data == "exp")
        OnBtExp();
      else if (word.data == "sin")
        OnBtSin();
      else if (word.data == "cos")
        OnBtCos();
      else if (word.data == "tan")
        OnBtTan();
      else if (word.data == "log")
        OnBtLog();
      else if (word.data == "ln")
        OnBtLn();

      else if (word.data == "ft_m")
        InsertUnitConverter(rpn::CRpnUnitConverter::FT_M);
      else if (word.data == "m_ft")
        InsertUnitConverter(rpn::CRpnUnitConverter::M_FT);
      else if (word.data == "ppg_kgm3")
        InsertUnitConverter(rpn::CRpnUnitConverter::PPG_KGPM3);
      else if (word.data == "kgm3_ppg")
        InsertUnitConverter(rpn::CRpnUnitConverter::KGPM3_PPG);
      else if (word.data == "C_K")
        InsertUnitConverter(rpn::CRpnUnitConverter::C_K);
      else if (word.data == "K_C")
        InsertUnitConverter(rpn::CRpnUnitConverter::K_C);
      else if (word.data == "F_C")
        InsertUnitConverter(rpn::CRpnUnitConverter::F_C);
      else if (word.data == "C_F")
        InsertUnitConverter(rpn::CRpnUnitConverter::C_F);
      else if (word.data == "psi_MPa")
        InsertUnitConverter(rpn::CRpnUnitConverter::PSI_MPA);
      else if (word.data == "MPa_psi")
        InsertUnitConverter(rpn::CRpnUnitConverter::MPA_PSI);

      else if (word.data == "not")
        OnBtNot();
      else if (word.data == "and")
        OnBtAnd();
      else if (word.data == "or")
        OnBtOr();
      else if (word.data == "min")
        OnBtMin();
      else if (word.data == "max")
        OnBtMax();
      else if (word.data == "pow")
        OnBtPow();
      else if (word.data == "!=")
        OnBtNotEqual();
      else if (word.data == "<=")
...
const CDerivedResult::CDerivedResultComponent *CRpnDlg::Result() const { return m_pResult; }

void CRpnDlg::UpdateSwapButton() { GetDlgItem(IDC_BT_SWAP)->EnableWindow(m_lcStack.GetSelectedCount() == 2); }

void CRpnDlg::UpdateEditFormulaButton() { GetDlgItem(IDC_BT_EDIT_FORMULA)->EnableWindow(m_lcStack.GetItemCount() < 2); }
