...
CResultExportDlg::CResultExportDlg(CModelBase &model, const TResultExportConfiguration &resultExportConfiguration,
                                   const CString &textCancel, bool buttonOK, CWnd *pParent)
    : IExportDlg(CResultExportDlg::IDD, pParent), m_pSlipHorizonSelection(0), m_pHorizonSelection(0),
      m_pFormationSelection(0), m_pFormationPlaneSelection(0), m_pRTCISelection(0), m_pNewWellPathSelection(0),
      m_pPointSetSelection(0), m_pElementSetSelection(0), m_pNonMeshedSurfaceSelection(0),
      m_resultExportConfiguration(resultExportConfiguration), m_model(model), m_nWeightingType(0), m_nUnit(-1),
      m_bLinear(TRUE), m_nTZ(0), m_nCenterPoints(0), m_nFormationNames(0), m_nWellPoints(0),
      m_bExportNewWellpaths(false), m_MatRGObserver(0), m_RGObserver(0), m_GVTRGObserver(0), m_InitiallyOn(0),
      m_nInitialIndex(-1), m_textCancel(textCancel), m_buttonOK(buttonOK), m_exportResultData(0), m_updateData(false),
...
CResultExportDlg::CResultExportDlg(CResultGroup &result_group,
    ...
    : IExportDlg(CResultExportDlg::IDD, pParent), m_pSlipHorizonSelection(0), m_pHorizonSelection(0),
      m_pFormationSelection(0), m_pFormationPlaneSelection(0), m_pRTCISelection(0), m_pNewWellPathSelection(0),
      m_pPointSetSelection(0), m_pElementSetSelection(0), m_pNonMeshedSurfaceSelection(0),
      m_resultExportConfiguration(resultExportConfiguration), m_model((CModelBase &)result_group.Model()),
      m_nWeightingType(0), m_nUnit(-1), m_bLinear(TRUE), m_nTZ(0), m_nCenterPoints(0), m_nFormationNames(0),
      m_nWellPoints(0), m_bExportNewWellpaths(false), m_MatRGObserver(0), m_RGObserver(0), m_GVTRGObserver(0),
      m_InitiallyOn(&result_group), m_nInitialIndex(-1), m_textCancel(textCancel), m_buttonOK(buttonOK),
      m_exportResultData(0), m_updateData(false), m_bBranchToggle(false) {
  Init();
}
...
CResultExportDlg::CResultExportDlg(const IValueComposite &composite, int nIndex,
    ...
    : IExportDlg(CResultExportDlg::IDD, pParent), m_pSlipHorizonSelection(0), m_pHorizonSelection(0),
      m_pFormationSelection(0), m_pFormationPlaneSelection(0), m_pRTCISelection(0), m_pNewWellPathSelection(0),
      m_pPointSetSelection(0), m_pElementSetSelection(0), m_pNonMeshedSurfaceSelection(0),
      m_resultExportConfiguration(resultExportConfiguration), m_model((CModelBase &)composite.Model()), m_nUnit(-1),
      m_nWeightingType(0), m_bLinear(TRUE), m_nTZ(0), m_nCenterPoints(0), m_nFormationNames(0), m_nWellPoints(0),
      m_bExportNewWellpaths(false), m_MatRGObserver(0), m_RGObserver(0), m_GVTRGObserver(0), m_InitiallyOn(&composite),
      m_nInitialIndex(nIndex), m_textCancel(textCancel), m_buttonOK(buttonOK), m_exportResultData(0),
      m_updateData(false), m_bBranchToggle(false) {
  Init();
}
...
static bool CheckResult(const CDepletionStage &stage, const CAnalysisType &antype, const IResultComponent &component) {
  ...
  const IResultComponent *pComponent =
      result.ResultComponent(stage, antype, component.RegisterIndex(), component.ComponentIndex());
  if (pComponent) {
...
BOOL CResultExportDlg::Valid(const IValueComponentBase &component) const {
  ...
  const CMaterialResult::CMaterialResultComponent *pMatComp =
      dynamic_cast<const CMaterialResult::CMaterialResultComponent *>(&component);
  if (pMatComp)
  ...
  if (pComponent) {
    ...
    for (TTimeStepSet::const_iterator it = m_stTimeStep.begin(); it != m_stTimeStep.end(); it++) {
      ...
      if (m_bLinear && CheckResult(stage, CAnalysisType::AT_LINEAR, *pComponent))
        return TRUE;
      if (m_bNonLinear && CheckResult(stage, CAnalysisType::AT_NONLIN, *pComponent))
        return TRUE;
      if (m_bHeat && CheckResult(stage, CAnalysisType::AT_HEAT, *pComponent))
        return TRUE;
      if (m_bMixture && CheckResult(stage, CAnalysisType::AT_MIXTURE, *pComponent))
        return TRUE;
      if (m_bMixtureContainment && CheckResult(stage, CAnalysisType::AT_MIXTURE_CONTAINMENT, *pComponent))
        return TRUE;
    }
...
void CResultExportDlg::UpdateButtons() {
  ...
  bool bNoResults = !Model().ResultRegister().Linear() && !Model().ResultRegister().NonLinear() &&
                    !Model().ResultRegister().Mixture();

  // bool bElementSet = m_vcCategory.isDerivedCategory <CElementSetCategory> ();
  bool bElementSet = ((m_pElementSetSelection != 0) && !m_pElementSetSelection->empty());

  bool bPointSet = ((m_pPointSetSelection != 0) && !m_pPointSetSelection->empty());

  bool bDeformation = ((m_pHorizonSelection != 0) && !m_pHorizonSelection->empty()) ||
                      ((m_pFormationSelection != 0) && !m_pFormationSelection->empty()) ||
                      ((m_pFormationPlaneSelection != 0) && !m_pFormationPlaneSelection->empty());

  // wjrx mantis 3322
  bool bNewWellPath = ((m_pNewWellPathSelection != 0) && !m_pNewWellPathSelection->empty());
  ...
  bool bFormations = ((m_pFormationSelection != 0) && !m_pFormationSelection->empty()) ||
                     ((m_pFormationPlaneSelection != 0) && !m_pFormationPlaneSelection->empty());

  bool bHorizons = ((m_pHorizonSelection != 0) && !m_pHorizonSelection->empty()) ||
...
// wjrx mantis 3322
void CResultExportDlg::SelectAllNewWellpaths(bool bSelect) { m_bExportNewWellpaths = bSelect; }
...
bool areInputResultsPresent(CLocalResultGroupObserver<CResultExportDlg> *exportResultGroupObserver) {
  ...
  for (TResultComponentSet::const_iterator result = results.begin();
    ...
    const CResultTree::CScalarResult *scalarResult = dynamic_cast<const CResultTree::CScalarResult *>((*result).first);
...
void CResultExportDlg::OnExport() {
  ...
  if (Model().ResultRegister().ResultsAvailable()) {
    ...
    if (!inputResultsArePresent && !m_bLinear && !m_bNonLinear && !m_bHeat && !m_bMixture && !m_bMixtureContainment) {
  ...
  if (m_RGObserver->Results().size() == 0 && m_MatRGObserver->Results().size() == 0 &&
      m_GVTRGObserver->Results().size() == 0 && !m_bExportNewWellpaths) {
  ...
  if (getPathName(pathName, filterIndex) == IDOK) {
    ...
    if ((pathName.Right(4).CompareNoCase(".las") == 0) && m_resultExportConfiguration.export2LAS) {
...
void CResultExportDlg::setExportResultData(CExportResultData *exportResultData, bool updateData) {
...
bool CResultExportDlg::OnExport(const CString &sFileName, bool bExcel, bool bOld) {
  ...
  bool retval = exportResults.onExport((LPCSTR)sFileName, bExcel, bOld, m_resultExportConfiguration.export2LAS);
...
bool CResultExportDlg::OnExportGoCad(const CString &sFileName) { return true; }
...
INT_PTR CResultExportDlg::getPathName(CString &pathName, DWORD &filterIndex) {
  ...
  // CString	sFilter = "ASCII Files (*.dat)|*.dat|Excel Files (*.xls;*.xls*)|*.xls;*.xls*|Eclipse Files
  // (*.grid)|*.grid|";
...
void CResultExportDlg::OnImportExportMacro() {
  ...
  CExportResultDataEntry *exportResultDataEntry = (CExportResultDataEntry *)m_model.GraphEntry(MD_BASE_EXPORT_MACROS);

  if ((exportResultDataEntry != 0) && (exportResultDataEntry->EntryNodes().size() > 0)) {
    CExportResultData *exportResultData;
    CSelectExportMacroDialog selectExportMacroDialog(&exportResultData, exportResultDataEntry->GraphEntryNodes());
...
void CResultExportDlg::OnSaveExportMacro() {
  ...
  const CGraphEntry::TEntryNodeSet &entryNodeSet = exportResultDataEntry->GraphEntryNodes();
  CString macroName;
  ...
  if (exportResultDataEntry != 0) {
    for (CGraphEntry::TEntryNodeSet::const_iterator iterator = entryNodeSet.begin();
         !duplicateName && (iterator != entryNodeSet.end()); ++iterator) {
  ...
  if (!duplicateName) {
    CExportResultData *exportResultData = new CExportResultData((LPCTSTR)macroName, m_model);
    CExportResultData_Delegate *exportResultData_Delegate = new CExportResultData_Delegate(exportResultData);

    getExportResultData(exportResultData);
  } else {
    if (m_exportResultData == 0) {
      for (CGraphEntry::TEntryNodeSet::const_iterator iterator = entryNodeSet.begin(); iterator != entryNodeSet.end();
           ++iterator) {
...
void CResultExportDlg::OnMacroName() {
  GetDlgItem(IDC_SAVE_EXPORT_MACRO)->EnableWindow(m_macroName.GetWindowTextLength() > 0);
}
...
void CResultExportDlg::OnEnableExport() {
  ...
  bool nothingSelected = ((m_pSlipHorizonSelection != 0) && m_pSlipHorizonSelection->empty()) &&
  ...
  bool onlyHorizons = ((m_pSlipHorizonSelection != 0) && !m_pSlipHorizonSelection->empty()) &&
                      ((m_pHorizonSelection != 0) && !m_pHorizonSelection->empty()) &&
                      ((m_pFormationSelection != 0) && m_pFormationSelection->empty()) &&
                      ((m_pFormationPlaneSelection != 0) && m_pFormationPlaneSelection->empty()) &&
                      ((m_pRTCISelection != 0) && m_pRTCISelection->empty()) &&
                      ...
                      ((m_pElementSetSelection != 0) && m_pElementSetSelection->empty()) &&
                      ((m_pNonMeshedSurfaceSelection != 0) && m_pNonMeshedSurfaceSelection->empty());
  bool noResultsSelected =
      (m_exportResultData && m_exportResultData->ReservoirMacro()) ||
      (m_MatRGObserver->Results().empty() && m_RGObserver->Results().empty() && m_GVTRGObserver->Results().empty());
  bool onlyMaterialParameters = (!m_exportResultData || !m_exportResultData->ReservoirMacro()) &&
                                !m_MatRGObserver->Results().empty() && m_RGObserver->Results().empty() &&
                                m_GVTRGObserver->Results().empty();
...
void CResultExportDlg::toggleState() const { const_cast<CResultExportDlg *>(this)->OnEnableExport(); }
...
void CResultExportDlg::initializeDialog(bool firstTime) {
  ...
  if (firstTime) {
    if (m_resultExportConfiguration.enableSlipHorizonSelection) {
      ...
      re::CreateExportResultTreeBranch<re::CSlipHorizonEntryObserver, THorizonBaseEntry, MD_BASE_HORIZON,
                                       THorizonBaseSelection>(m_model, *m_pSlipHorizonSelection, m_tcObject);
    }
    ...
    if (m_resultExportConfiguration.enableHorizonSelection) {
      ...
      re::CreateExportResultTreeBranch<re::CHorizonEntryObserver, THorizonBaseEntry, MD_BASE_HORIZON,
                                       THorizonBaseSelection>(m_model, *m_pHorizonSelection, m_tcObject);
    }

    if (m_resultExportConfiguration.enableFormationSelection) {
      m_pFormationSelection = new TFormationSelection(Selection());
      re::CreateExportResultTreeBranch<re::CFormationBaseEntryObserver, TFormationBaseEntry, MD_BASE_FORMATION,
                                       TFormationSelection>(m_model, *m_pFormationSelection, m_tcObject);
    }
    ...
    if (m_resultExportConfiguration.enableFormationPlaneSelection) {
      ...
      re::CreateExportResultTreeBranch<re::CFormationPlaneEntryObserver, CHexaFormationEntry, MD_HEXA_FORMATION,
                                       TFormationPlaneSelection>(m_model, *m_pFormationPlaneSelection, m_tcObject);
    }

    if (m_resultExportConfiguration.enableRTCISelection) {
      m_pRTCISelection = new TRTCISelection(Selection());
      re::CreateExportResultTreeBranch<re::CRTCIEntryObserver, CWellCasingRTCIEntry, MD_WELLCASING_RTCI,
                                       TRTCISelection>(m_model, *m_pRTCISelection, m_tcObject);
    }

    if (m_resultExportConfiguration.enableNewWellPathSelection) {
      m_pNewWellPathSelection = new TNewWellPathSelection(Selection());
      re::CreateExportResultTreeBranch<re::CNewWellPathEntryObserver, CNewWellPathEntry, MD_NEW_WELLPATH,
                                       TNewWellPathSelection>(m_model, *m_pNewWellPathSelection, m_tcObject);
    }

    if (m_resultExportConfiguration.enablePointSetSelection) {
      m_pPointSetSelection = new TPointSetSelection(Selection());
      re::CreateExportResultTreeBranch<re::CPointSetEntryObserver, TPointSetEntry, MD_BASE_POINTSET,
                                       TPointSetSelection>(m_model, *m_pPointSetSelection, m_tcObject);
    }
    ...
    if (m_resultExportConfiguration.enableElementSetSelection) {
      ...
      re::CreateExportResultTreeBranch<re::CElementSetEntryObserver, TPointSetEntry, MD_BASE_POINTSET,
                                       TElementSetSelection>(m_model, *m_pElementSetSelection, m_tcObject);
    }
    ...
    if (m_resultExportConfiguration.enableNonMeshedSurfaceSelection) {
      ...
      re::CreateExportResultTreeBranch<re::CNonMeshedSurfaceEntryObserver, CNonMeshedSurfaceEntry,
                                       MD_BASE_NONMESHEDSURFACE, TNonMeshedSurfaceSelection>(
          m_model, *m_pNonMeshedSurfaceSelection, m_tcObject);
    }
  }
  ...
  if (!m_exportResultData || !m_exportResultData->ReservoirMacro()) {
    ...
    m_MatRGObserver->newSelection(m_exportResultData == 0 ? TResultComponentSet()
                                                          : m_exportResultData->m_MatRGObserverData);
    m_RGObserver->newSelection(m_exportResultData == 0 ? TResultComponentSet() : m_exportResultData->m_RGObserverData);
    m_GVTRGObserver->newSelection(m_exportResultData == 0 ? TResultComponentSet()
                                                          : m_exportResultData->m_GVTRGObserverData);
    ...
    typedef CSelectionLeafObserver_Delegate<IValueComposite, IValueComposite_Delegate, CResultExportDlg, FALSE>
        TCompositeObs;
    typedef CSelectionBranchObserver_Delegate<CGraphEntry, CGraphEntry_Delegate, IValueComposite, TCompositeObs,
                                              IValueComposite, CResultExportDlg, FALSE, FIXED_ITEM>
        TCompositeEntryObs;

    // Insert resulttree
    if (firstTime) {
      new TCompositeEntryObs(*Model().GraphEntry(MD_BASE_MESH_RESULT), m_tcValue, *this,
                             &CResultExportDlg::ResultComposite, &CResultExportDlg::MeshResultComposite);
    }
