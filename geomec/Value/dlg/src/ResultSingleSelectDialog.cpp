...
/////////////////////////////////////////////////////////////////////////////
CResultSingleSelectDialog::CResultGroupObserver::CResultGroupObserver(CResultGroup& result_group, CResultSingleSelectDialog& dlg)
: IResultGroupObserver(result_group, dlg.m_lbValue, TVI_ROOT, TVI_LAST),
  m_dlg(dlg)
{
  Update();
}

unsigned int CResultSingleSelectDialog::CResultGroupObserver::ResultComponentStateIconId(const IValueComposite& composite, int nComponent) const
{
...
void CResultSingleSelectDialog::CResultGroupObserver::ResultComponentToggleState(IValueComposite& composite, int nComponent)
{
  const IResult& result = static_cast<const IResult&>(composite);
  ...
  if(&m_dlg.TimeStep() == &m_dlg.Model().InitialDepletionStage())
  {
    assert(!m_dlg.Model().InitialDepletionStage().Last());
    if(result.Mode(m_dlg.AnalysisType(), m_dlg.Model().InitialDepletionStage().Next().Index()) > -1)
      m_dlg.Select(&result.Component(nComponent, result.Mode(m_dlg.AnalysisType(), m_dlg.Model().InitialDepletionStage().Next().Index())));
  }
...
void CResultSingleSelectDialog::CResultGroupObserver::AppendResultComponentContextMenu(IResult_Delegate& result, int nComponent, CContextMenuInvoker &invoker)
{
...
CTreeNode* CResultSingleSelectDialog::CResultGroupObserver::InsertChild(CGraphNode& node)
{
  ...
  if (pGVTGroup)
  return new CGVTResultGroupObserverSingleSelectTempl<CResultSingleSelectDialog>(*pGVTGroup, *this, const_cast<CResultSingleSelectDialog&>(m_dlg));
...
const CGraphNode& CResultSingleSelectDialog::CPointSetObserver::ObservedItem() const
{
  return m_point_set;
}

CGraphNode& CResultSingleSelectDialog::CPointSetObserver::ObservedItem()
{
  return m_point_set;
}
...
CResultSingleSelectDialog::CResultSingleSelectDialog(CModelBase& model, BOOL bGlobal, const CDerivedResult::CDerivedResultComponent* pResultBeingCreated, CWnd* pParent /*=NULL*/)
: CDialog(CResultSingleSelectDialog::IDD, pParent), 
  ...
  m_pTimeStep(&model.InitialDepletionStage()),
...
CResultSingleSelectDialog::CResultSingleSelectDialog(CDerivedResult::CDerivedResultComponent::CValueComponentProxy& proxy, BOOL bGlobal, CWnd* pParent)
: CDialog(CResultSingleSelectDialog::IDD, pParent),
 m_bGlobal(bGlobal),
 m_model(dynamic_cast<CModelBase&>(proxy.ValueComponent().Model())),
 m_antype(CAnalysisType::AT_LINEAR), 
 m_pTimeStep(&dynamic_cast<CModelBase&>(proxy.ValueComponent().Model()).InitialDepletionStage()),
 m_sName(proxy.TextTag().toStdString().c_str()),
 m_pSelection(&proxy.ValueComponent()),
 m_pResultBeingCreated(0)
{
  m_bInput = !(dynamic_cast<CDerivedResult*>(&proxy.ValueComponent().Parent()) || dynamic_cast<IResult*>(&proxy.ValueComponent().Parent()) || dynamic_cast<const TDeltaT *>(&proxy.ValueComponent().Parent()) || dynamic_cast<const TDeltaV *>(&proxy.ValueComponent().Parent()));
...
{ 
  if(m_pSelection) return &composite == &m_pSelection->Parent(); 
  return false;
}

void CResultSingleSelectDialog::Composite(const IValueComposite& composite, bool bEnable) 
{ 
  if(bEnable)	Select( &composite.Component() ); 
}
...
void CResultSingleSelectDialog::BuildTree()
{
  ...
  { 
    typedef CSelectionLeafObserver_Delegate<IValueComposite, IValueComposite_Delegate, CResultSingleSelectDialog, TRUE> TCompositeObs;
    typedef CSelectionBranchObserver_Delegate<CGraphEntry, CGraphEntry_Delegate, IValueComposite, TCompositeObs, IValueComposite, CResultSingleSelectDialog, FALSE, FIXED_ITEM> TCompositeEntryObs;
...
void CResultSingleSelectDialog::UpdateSelection()
{
  ...
  const IResultComponent* pScalComp =
    dynamic_cast<const IResultComponent*>(m_pSelection);
  ...
  btMixtureContainment.EnableWindow(!m_bInput && m_bSetLinStage && result.Mode(CAnalysisType::AT_MIXTURE_CONTAINMENT, *m_pTimeStep) > -1);
...
void CResultSingleSelectDialog::OnNameChange() 
{
...
void CResultSingleSelectDialog::UpdateName()
{
  ...
  if(m_pSelection)
  {
      ...
      if ( m_bSetLinStage ||
           ! (pDResult= dynamic_cast<const IResult *>(&m_pSelection->Parent()) )
         )
...
BOOL CResultSingleSelectDialog::Global() const
{
  return m_bGlobal;
}
...
bool CResultSingleSelectDialog::Valid(const IValueComponentBase& component) const
{
  return true;
}
