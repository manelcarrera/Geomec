...
CRpnEditFormulaDlg::CRpnEditFormulaDlg(const CModelBase &model, const std::string &input,
                                       const std::set<std::string> &operandNames,
                                       std::queue<Shunt::CShunt::Op_t> &output, CWnd *pParent /*=NULL*/
                                       )
    : CDialog(CRpnEditFormulaDlg::IDD, pParent), m_model(model), m_sInfixFormula(input), m_qsRPNFormula(output),
      m_bValidatedOk(false), m_bCaughtException(false) {
  ...
  try {
    for (std::set<std::string>::const_iterator it = operandNames.begin(); it != operandNames.end(); ++it) {
      addOperandName(*it);
    }
...
void CRpnEditFormulaDlg::addOperandName(const std::string &operandName) { m_namedOperands.insert(operandName); }

bool CRpnEditFormulaDlg::checkOperandNames(const std::list<Shunt::CRpnToken *> &tokens) {
  ...
  for (it = tokens.begin(); it != tokens.end(); ++it) {
    ...
    if (!opand)
      continue;
    if (Shunt::CShunt::validFloatingPoint(opand->getValue()))
      continue;
    ...
    if (SimpleSplitTextTag(opand->getValue(), baseTag, timeStep) && timeStep != -1) {
      if (CFillMaterialParameterProxies::GetNames(m_model).find(baseTag) ==
        ...
        std::string msg = "\"" + baseTag + "\" operand does not accept a depletion stage index";
      ...
      if (m_model.FindDepletionStage(timeStep)) {
        ...
        std::string msg = "\"" + opand->getValue() + "\" material item does not contain a valid depletion stage index";
        m_shunt.setMessage(tokens, pos, msg);
        return false;
      }
      ...
      std::string msg =
          "\"" + opand->getValue() +
          "\" is not a numerical value and not an existing variable.\nPlease only use existing variable names.";
      m_shunt.setMessage(tokens, pos, msg);
      return false;
    }
  } // for tokens
...
void CRpnEditFormulaDlg::OnOK() {
  ...
  try {
    ...
    if (checkOperandNames(tokens) == true && m_shunt.convertInfixToPostfix(tokens, m_qsRPNFormula) == true) {
      m_bValidatedOk = true;
    } else {
      m_bValidatedOk = false;
      GetDlgItem(IDC_RPN_ERRORMSG)->SetWindowText(m_shunt.getMessage().c_str());
    }
...
void CRpnEditFormulaDlg::DoDataExchange(CDataExchange *pDX) {
  ...
  try {
    ...
    if (!pDX->m_bSaveAndValidate) {
      ...
      instance->createTokens(m_sInfixFormula, tokens);
      instance->deleteTokens(tokens);
      GetDlgItem(IDC_EB_RPNFORMULA)->SetWindowText(m_sInfixFormula.c_str());
    }
  } catch (...) {
    m_bCaughtException = true;
  }
