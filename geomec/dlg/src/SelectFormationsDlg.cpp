...
CSelectFormationsDlg::CSelectFormationsDlg(CStorageNode &container, const IFormationFilter *formationFilter /*= 0*/)
    : CDialog(IDD_UPSCALING_SELECT_FORMATIONS_DLG), m_formationFilter(formationFilter), m_container(container) {}
...
BOOL CSelectFormationsDlg::OnInitDialog() {
  ...
  const TFormationBaseEntry *pEntry =
      static_cast<const TFormationBaseEntry *>(m_container.Model().GraphEntry(MD_BASE_FORMATION));
  assert(pEntry);
...
//// CFormationObserver
CSelectFormationsDlg::CFormationObserver::CFormationObserver(const CFormationBase &formation, CSelectFormationsDlg &dlg,
                                                             CListCtrl &ctrl)
    : IListObject(ctrl), m_formation(formation), m_dlg(dlg) {
  ctrl.SetItemData(ctrl.GetItemCount() - 1, (DWORD_PTR)this);
  Update();
}

unsigned int CSelectFormationsDlg::CFormationObserver::Icon() const { return m_formation.IconId(); }

QString CSelectFormationsDlg::CFormationObserver::Text() const { return m_formation.Name(); }
...
CSelectFormationsSetNode::CSelectFormationsSetNode(TFormationsSet &stFormations, CFemAppModel &model)
    : CStorageNode(model), m_stFormations(stFormations) {
...
unsigned int CSelectFormationsSetNode::IconId() const { return 0; }

unsigned int CSelectFormationsSetNode::TypeId() const { return 0; }

bool CSelectFormationsSetNode::Empty() const { return true; }

long CSelectFormationsSetNode::SavedItems() const { return 0; }
