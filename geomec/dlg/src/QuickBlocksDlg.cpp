...
CQuickBlocksDlg::CQuickBlocksDlg(CQBSettings &settings, CWnd *pParent /*=NULL*/)
    : CDialog(CQuickBlocksDlg::IDD, pParent), m_dNorth(0), m_dEast(0), m_dLength(0), m_dWidth(0), m_dTrack(0),
      m_dBin(0), m_Source(settings), m_lbPressures(settings.Model()), m_lbTemperatures(settings.Model()) {}
...
BOOL CQuickBlocksDlg::OnInitDialog() {
  ...
  for (size_t i = 0; i < m_Source.DeltaPressures().size(); ++i) {
    new CTimeLapseListObject<CQBSettings>(m_Source.Model(), m_lbPressures, m_Source.DeltaPressures()[i].first,
                                          m_Source.DeltaPressures()[i].second, &m_Source);
  }
  ...
  for (size_t i = 0; i < m_Source.DeltaTemperatures().size(); ++i) {
    new CTimeLapseListObject<CQBSettings, Temperature>(m_Source.Model(), m_lbTemperatures,
                                                       m_Source.DeltaTemperatures()[i].first,
                                                       m_Source.DeltaTemperatures()[i].second, &m_Source);
  }
...
void CQuickBlocksDlg::OnBnClickedButtonQbStart() { m_Source.StartQB(); }
...
void CQuickBlocksDlg::OnBnClickedButtonQbWriteDat() {
  ...
  CTnoFileDialog dlg(FALSE, "dat", NULL, OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT,
                     "QB dat file (*dat)|*.dat|All Files (*.*)|*.*||");
