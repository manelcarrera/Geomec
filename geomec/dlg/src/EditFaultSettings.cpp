...
CEditFaultSettings::CEditFaultSettings(CFemAppModel *femAppModel, CWnd *pParent)
    : CDialog(CEditFaultSettings::IDD, pParent), m_femAppModel(femAppModel), m_faultFractureList(m_femAppModel, Unit()),
      m_faultFractureComboBox(), m_handleFaultFractureBehaviour(this), m_applyBehaviourToAllFaults(),
      m_applyInitD0ToAllFaults(), m_initialSelection(0) {}

CEditFaultSettings::CEditFaultSettings(CFemAppModel *femAppModel, const CHorizonBase *horizonBase, CWnd *pParent)
    : CDialog(CEditFaultSettings::IDD, pParent), m_femAppModel(femAppModel), m_faultFractureList(m_femAppModel, Unit()),
      m_faultFractureComboBox(), m_handleFaultFractureBehaviour(this), m_applyBehaviourToAllFaults(),
      m_applyInitD0ToAllFaults(), m_initialSelection(determineInitialSelection(m_faultFractureList, horizonBase)) {}
...
BOOL CEditFaultSettings::OnInitDialog() {
  ...
  for (size_t index = 0; index < m_faultFractureList.size(); ++index) {
    m_faultFractureComboBox.AddString(m_faultFractureList[index].name().toStdString().c_str());
  }
...
void CEditFaultSettings::OnCbnSelchangeFaultFractureComboBox() { UpdateData(FALSE); }

void CEditFaultSettings::OnBnClickedFaultInitializationMethod() {
  CFaultInitSettingsDlg faultInitSettingsDlg(static_cast<CModelBase &>(*m_femAppModel));

  faultInitSettingsDlg.DoModal();
}

void CEditFaultSettings::OnBnClickedGlobalFaultBehaviour() {
  CSetGlobalFaultBehaviourDialog setGlobalFaultBehaviourDialog(m_femAppModel, this);

  setGlobalFaultBehaviourDialog.DoModal();
}

void CEditFaultSettings::OnEnChangeDefaultFaultParameters() { UpdateData(TRUE); }
...
void CEditFaultSettings::handleFaultFractureBehaviour(CDataExchange *pDX, int &currentSelection) {
  ...
  m_handleFaultFractureBehaviour.enableSlipButtons(m_faultFractureList[currentSelection].Slip() &&
                                                   !m_faultFractureList[currentSelection].IsBranch());
...
void CEditFaultSettings::handleDefaultFaultParameters(CDataExchange *pDX, int &currentSelection) {
  if (m_applyInitD0ToAllFaults.GetCheck() == BST_CHECKED) {
...
int CEditFaultSettings::determineInitialSelection(const CFaultFractureList &faultFractureList,
