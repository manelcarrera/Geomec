...
CExecuteDianaDialog::CExecuteDianaDialog(CWnd* pParent /*=NULL*/)
	: CDialog(CExecuteDianaDialog::IDD, pParent)
  , m_exe_diana_signals(CDianaStartUp::instance(), this)
{
...
//
// run logic: 'AddMessage' and 'AddWarning' use 'ProcessStatus' imnplemented in the base class 'IExecuteDianaDialog' (DGlue lib)
//
...
//		Return value:	If a message is available, the return value is nonzero.
//
...
void CExecuteDianaDialog::ExecuteDianaCleanup()
{
  CDianaStartUp::instance()->RunDianaCleanup();
}
...
void CExecuteDianaDialog::StepProgressBar()
{
	...
	if( !( ProgressBar = (CProgressCtrl *)GetDlgItem( IDC_PROGRESS ) ) ) return;
	if( !( ProgressBarText = GetDlgItem( IDC_TXT_PROGRESS ) ) ) return;
