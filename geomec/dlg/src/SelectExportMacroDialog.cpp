...
CSelectExportMacroDialog::CSelectExportMacroDialog(CExportResultData **exportResultData,
                                                   const CGraphEntry::TEntryNodeSet &entryNodeSet,
                                                   CWnd *pParent /*=NULL*/)
    : CDialog(CSelectExportMacroDialog::IDD, pParent), m_exportResultData(exportResultData),
...
BOOL CSelectExportMacroDialog::OnInitDialog() {
  ...
  for (CGraphEntry::TEntryNodeSet::const_iterator iterator = m_entryNodeSet.begin(); iterator != m_entryNodeSet.end();
       ++iterator) {
    int index = m_selectExportMacrosList.AddString((*iterator)->Name().toStdString().c_str());
