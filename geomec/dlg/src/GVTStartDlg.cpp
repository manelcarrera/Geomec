...
CGVTSettingsDlg::CGVTSettingsDlg(CGVTSettings &settings, CWnd *pParent /*=NULL*/)
    : CDialog(CGVTSettingsDlg::IDD, pParent), m_SourceSettings(settings), m_GVTSettings(settings),
      m_strVelocityModel(settings.VelocityModelFile().toStdString().c_str()),
      m_strOutputPrefix(settings.OutputPrefix().toStdString().c_str()),
      m_strOutputDirectory(settings.OutputDirectory().toStdString().c_str()), m_ndvp(1), m_ndt(1), m_nImportBack(1),
      m_lbTimeLapse(settings.Model()), m_velocitySource(settings.velocitySource()) {
...
void CGVTSettingsDlg::SetupTimeLapseListCtrl() {
  ...
  for (size_t i = 0; i < m_GVTSettings.TimeLapses().size(); ++i) {
    new CTimeLapseListObject<CGVTSettings>(m_GVTSettings.Model(), m_lbTimeLapse, m_GVTSettings.TimeLapses()[i].first,
                                           m_GVTSettings.TimeLapses()[i].second, &m_GVTSettings);
  }
...
ON_BN_CLICKED(IDC_GVT_VELOCITY_FROM_FILE, &CGVTSettingsDlg::OnBnClickedGvtVelocityFromFile)
ON_BN_CLICKED(IDC_GVT_VELOCITY_FROM_MATERIAL, &CGVTSettingsDlg::OnBnClickedGvtVelocityFromMaterial)
...
void CGVTSettingsDlg::OnBnClickedGvtBrowseVelocity() {
  ...
  CTnoFileDialog dlg(TRUE, NULL, NULL, NULL, sFilter);
  if (dlg.DoModal() == IDOK) {
...
void CGVTSettingsDlg::OnBnClickedGvtCheckbox() { UpdateData(TRUE); }

// ******************************************************************
// CTimeLapseListObject implementation
// ******************************************************************
/*CGVTSettingsDlg::CTimeLapseListObject::CTimeLapseListObject(CListCtrl &ctrl, CDepletionStage *t1, CDepletionStage *t2,
CGVTSettings *settings) :IListObject(ctrl), m_t1(t1), m_t2(t2), m_GVTSettings(settings)
{
...
CGVTSettingsDlg::CParametersListObject::CParametersListObject(CListCtrl &ctrl, CGVTVelocityModel *velmod, CString param)
    : IListObject(ctrl), m_VelMod(velmod), m_Parameter(param) {
  Ctrl().SetItemData(Ctrl().GetItemCount() - 1, (DWORD_PTR)this);
  new CParamSubObject(*this);
}

BOOL CGVTSettingsDlg::CParametersListObject::CanEditText() const { return FALSE; }

QString CGVTSettingsDlg::CParametersListObject::Text() const { return (LPCSTR)m_Parameter; }

unsigned int CGVTSettingsDlg::CParametersListObject::Icon() const { return 0; }

CGVTVelocityModel *CGVTSettingsDlg::CParametersListObject::VelocityModel() { return m_VelMod; }

const CString &CGVTSettingsDlg::CParametersListObject::Parameter() const { return m_Parameter; }
...
BOOL CGVTSettingsDlg::CParamSubObject::CanEditText() const { return TRUE; }
...
CGVTSettingsDlg::CFormationListObject::CFormationListObject(CListCtrl &ctrl, CFormationBase *formation,
    ...
    : IListObject(ctrl), m_Formation(formation), m_VelModel(velmodel), m_ParentDlg(parentDlg) {
  Ctrl().SetItemData(Ctrl().GetItemCount() - 1, (DWORD_PTR)this);
  new CVelModelSubObject(*this);
}
QString CGVTSettingsDlg::CFormationListObject::Text() const { return m_Formation->Name(); }

BOOL CGVTSettingsDlg::CFormationListObject::CanEditText() const { return FALSE; }
unsigned int CGVTSettingsDlg::CFormationListObject::Icon() const { return m_Formation->IconId(); }

CGVTVelocityModel *CGVTSettingsDlg::CFormationListObject::VelocityModel() { return m_VelModel; }

const CGVTVelocityModel *CGVTSettingsDlg::CFormationListObject::VelocityModel() const { return m_VelModel; }
...
BOOL CGVTSettingsDlg::CVelModelSubObject::CanEditText() const { return TRUE; }
...
bool CGVTSettingsDlg::allMaterialModelsHaveVelocityP() const {
  const TFormationBaseEntry *formationBaseEntry =
      dynamic_cast<const TFormationBaseEntry *>(m_GVTSettings.Model().GraphEntry(MD_BASE_FORMATION));
  ...
  for (TFormationBaseEntry::TNodeSet::const_iterator formation = formationBaseEntry->EntryNodes().begin();
    ...
    const CDepletionStageEntry *depletionStageEntry =
        dynamic_cast<const CDepletionStageEntry *>(m_GVTSettings.Model().GraphEntry(MD_BASE_DEPLETION_STAGE));

    for (CDepletionStageEntry::const_iterator stage = depletionStageEntry->begin(); stage != depletionStageEntry->end();
         ++stage) {
      if ((*formation)->ConnectedMaterial((*stage).InitialStage()) != 0) {
        ...
        if (materialRock != 0) {
          ...
          for (size_t i = 0; i < libraryMaterial.ParameterSize(); ++i) {
            if (libraryMaterial.Parameter(i).ValueTypeID() == IDT_VALUETYPE_VELOCITYP) {
              foundVelocityP = true;
            }
          }
        }

        allMaterialModelsHaveVelocityP = allMaterialModelsHaveVelocityP && foundVelocityP;
      } else {
...
void CGVTSettingsDlg::setupVelocityP() {
  GetDlgItem(IDC_GVT_VELMODEL_EDIT)->EnableWindow(m_GVTSettings.velocitySource() == GVT::VELOCITY_FROM_FILE);
  GetDlgItem(IDC_GVT_BROWSE_VELOCITY)->EnableWindow(m_GVTSettings.velocitySource() == GVT::VELOCITY_FROM_FILE);
  GetDlgItem(IDC_GVT_VELOCITY_FROM_MATERIAL)->EnableWindow(allMaterialModelsHaveVelocityP());
}
