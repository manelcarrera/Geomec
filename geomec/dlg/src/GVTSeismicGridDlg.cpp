...
CGVTSeismicGridDlg::CGVTSeismicGridDlg(CGVTSeismicGridDefinition &griddefinition, const CModelBase &model,
                                       CWnd *pParent /*=NULL*/)
    : CDialog(CGVTSeismicGridDlg::IDD, pParent), m_Model(model), m_SeismicGridDefinition(griddefinition),
      m_Name(griddefinition.Name().toStdString().c_str()), m_FilePath(griddefinition.KeyFile().toStdString().c_str()),
      m_Northing(griddefinition.CornerNorthing()), m_Easting(griddefinition.CornerEasting()),
      m_Depth(griddefinition.Depth() ? griddefinition.Depth() : model.Mesh().Max().Z()),
      m_Azimuth(griddefinition.Azimuth()), m_TrackSpacing(griddefinition.TrackSpacing()),
      m_BinSpacing(griddefinition.BinSpacing()), m_DeltaBinNum(griddefinition.DeltaBin()),
      m_DeltaTrackNum(griddefinition.DeltaTrack()), m_FirstBin(griddefinition.FirstBin()),
      m_FirstTrack(griddefinition.FirstTrack()), m_NumBin(griddefinition.NumBin()),
