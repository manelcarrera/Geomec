...
// static const QString QS_STYLE		= QString("border-bottom-width: 1px; border-top-width: 1px; border-left-width: 1px;
// border-right-width: 1px; border-bottom-style: solid; border-radius: 0px;");
...
// CMeshRefinementBoxDlg::CMeshRefinementBoxDlg( CMeshRefinementBox* box, QWidget *parent ) : QDialog( parent ), m_box(
// box )
CMeshRefinementBoxDlg::CMeshRefinementBoxDlg(QWidget *parent) : QDialog(parent) {
  ...
  for (int i = 0; i <= Pd; i++)
    step(i, STEP_POSITION);
  for (int i = Rx; i <= Rz; i++)
    step(i, STEP_ROTATION);
  for (int i = Sx; i <= Sz; i++)
    step(i, STEP_SCALE);
