...
const std::string ChangelogDlg::GEOMEC_CONFIG_FILE	= QUtil::native_sepatators( std::string( getenv("APPDATA") ) + "/Geomec/geomec.xml");
const std::string ChangelogDlg::CHANGELOG_PACKAGE	= QUtil::native_sepatators( GetAppPath().toStdString() + "/packages/changelog.zip");
...
#else //QTextBrowser
	browser.document()->setMetaInformation( QTextDocument::DocumentUrl, QString::fromStdString( "file:" + img_path + "/" ) ); // Attention: don't play with this last slash: '/'
#endif
