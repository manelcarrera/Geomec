...
CExportGocadDlg::CObjectSelection::CObjectSelection(CExportGocadDlg &dlg) : m_dlg(dlg) {}
...
void CExportGocadDlg::CObjectSelection::populateTree() {
  { // formations
    ...
    typedef CSelectionBranchObserver_Delegate<TFormationBaseEntry, TFormationBaseEntry_Delegate, CFormationBase,
                                              TLeafObs, CFormationBase, CObjectSelection, FALSE, FIXED_ITEM>
        TEntryObs;
    new TEntryObs(entry, m_dlg.m_tcObjects, *this, &CObjectSelection::formation_selected,
                  &CObjectSelection::select_formation);
  }
  ...
  {                                                        // faults
    if (m_dlg.Model().GraphEntry(MD_TETRA_SUB_FAULT) != 0) // must be a tetra model
    {
      ...
      typedef CSelectionLeafObserver_Delegate<CTetraSubHorizon, CTetraSubHorizon_Delegate, CObjectSelection, FALSE>
          TLeafObs;
      typedef CSelectionBranchObserver_Delegate<CTetraSubHorizonEntry, CTetraSubHorizonEntry_Delegate, CTetraSubHorizon,
                                                TLeafObs, CTetraSubHorizon, CObjectSelection, FALSE, FIXED_ITEM>
          TEntryObs;

      new TEntryObs(entry, m_dlg.m_tcObjects, *this, &CObjectSelection::fault_selected,
                    &CObjectSelection::select_fault);
    }
  }
  ...
  { // pointsets
    ...
    typedef CSelectionBranchObserver_Delegate<TPointSetEntry, TPointSetEntry_Delegate, CPointSet, TLeafObs, CPointSet,
                                              CObjectSelection, FALSE, FIXED_ITEM>
        TEntryObs;
    new TEntryObs(entry, m_dlg.m_tcObjects, *this, &CObjectSelection::pointset_selected,
                  &CObjectSelection::select_pointset);
  }
}
...
CExportGocadDlg::CExportGocadDlg(CGocadExport &gocadexport, const CResultGroup *pResultGroup, CWnd *pParent)
    : IExportDlg(IDD_EXPORT_GOCAD, pParent), m_source(gocadexport), m_pCopy(new CGocadExport(gocadexport)),
      m_current(0), m_MatRGObserver(0), m_RGObserver(0), m_GVTRGObserver(0), m_InitiallyOn(pResultGroup) {
...
BOOL CExportGocadDlg::OnInitDialog() {
  ...
  m_MatRGObserver =
      new CLocalResultGroupObserver<CExportGocadDlg>(*this, m_source.Model().MaterialResultTree(), m_tcResults);
  m_GVTRGObserver =
      new CLocalResultGroupObserver<CExportGocadDlg>(*this, m_source.Model().GVTResultTree(), m_tcResults);
  m_RGObserver = new CLocalResultGroupObserver<CExportGocadDlg>(*this, m_source.Model().ResultTree(), m_tcResults);

  typedef CSelectionLeafObserver_Delegate<IValueComposite, IValueComposite_Delegate, CExportGocadDlg, FALSE>
      TCompositeObs;
  typedef CSelectionBranchObserver_Delegate<CGraphEntry, CGraphEntry_Delegate, IValueComposite, TCompositeObs,
                                            IValueComposite, CExportGocadDlg, FALSE, FIXED_ITEM>
      TCompositeEntryObs;
...
CModelBase &CExportGocadDlg::Model() { return m_source.Model(); }

const CModelBase &CExportGocadDlg::Model() const { return m_source.Model(); }

bool CExportGocadDlg::TimeStep(const CDepletionStage &stage) const { return m_pCopy->DepletionStage(stage); }
...
bool CExportGocadDlg::Formation(const CFormationBase &formation) const { return m_pCopy->Formation(formation); }
...
bool CExportGocadDlg::Fault(const CTetraSubHorizon &fault) const { return m_pCopy->Horizon(fault); }
...
bool CExportGocadDlg::PointSet(const CPointSet &ptSet) const { return m_pCopy->PointSet(ptSet); }
