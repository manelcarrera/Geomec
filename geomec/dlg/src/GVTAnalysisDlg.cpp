...
CGVTAnalysisDlg::CGVTAnalysisDlg(CGVTSettings &settings, CWnd *pParent /*=NULL*/)
    : CDialog(CGVTAnalysisDlg::IDD, pParent), m_GVTSettings(settings),
...
void CGVTAnalysisDlg::SetStartState() {
  if (!m_GVTSettings.AnalysisIsGo()) {
    GetDlgItem(IDC_GVTSART_STATIC)
        ->SetWindowTextA("Unable to start the GVT analysis, make sure the settings are set up correctly. ");
