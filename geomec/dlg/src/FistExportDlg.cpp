...
CFistExportDlg::CFistExportDlg(CWnd *pParent /*=NULL*/) : CDialog(CFistExportDlg::IDD, pParent) {
  //{{AFX_DATA_INIT(CFistExportDlg)
  // NOTE: the ClassWizard will add member initialization here
  //}}AFX_DATA_INIT
}

CFistExportDlg::CFistExportDlg(const CNewWellPath *pWellPath, const CModelBase *pModel, CWnd *pParent /*=NULL*/)
    : CDialog(CFistExportDlg::IDD, pParent), m_pNewWellPath(pWellPath), m_pModel(pModel),
      m_bNewWellPaths(false) // don't care
{                            // export all wells for which fist export points are selected
  // Construct
}

CFistExportDlg::CFistExportDlg(bool bNewWellPaths, const CModelBase *pModel, CWnd *pParent /*=NULL*/)
    : CDialog(CFistExportDlg::IDD, pParent), m_pNewWellPath(0), m_pModel(pModel),
      m_bNewWellPaths(bNewWellPaths) { // export all wells for which fist export points are selected
                                       // Construct
}
...
void CFistExportDlg::DoDataExchange(CDataExchange *pDX) {
  ...
  if (!pDX->m_bSaveAndValidate) {
    UpdateDateList();
    if ((m_pNewWellPath != 0 && m_NewDateList.size() == 0))
...
void CFistExportDlg::OnBtnAddDate() {
  ...
  first.setDate(m_pModel->InitialDepletionStage().Time().GetYear(), m_pModel->InitialDepletionStage().Time().GetMonth(),
                1);
  ...
  last.setDate(pLastStage->Time().GetYear(), pLastStage->Time().GetMonth(), 1);
  ...
  if (dlg.DoModal() == IDOK) {
    ...
    if (*date < first || *date > last) {
      _m()->msg(QString("New dates cannot be inserted before the initial depletion stage, " +
                        first.toString("dd MM yyyy") + ",\n" + " or after the last calculated depletion stage, " +
...
void CFistExportDlg::OnBtnIncStages() {
  ...
  while (pStage) {
    CAnalysisType antype =
        (m_pModel->ResultRegister().NonLinear() ? CAnalysisType::AT_NONLIN : CAnalysisType::AT_LINEAR);
...
void CFistExportDlg::OnBtnRmvDate() {
  ...
  while (pos) {
    int nItem = m_ListDateCtrl.GetNextSelectedItem(pos);
    {
      ...
      for (int jj = 0; jj < m_NewDateList.size() && it != m_NewDateList.end(); ++jj) {
        if (nItem == jj) {
...

  {
    ...
    for (it = m_DatesToRemove.begin(); it != m_DatesToRemove.end(); ++it) {
...
void CFistExportDlg::UpdateDateList() {
  ...
  m_ListDateCtrl.DeleteAllItems();
  {
    ...
    for (it = m_NewDateList.begin(); it != m_NewDateList.end(); ++it) {
...
void CFistExportDlg::AssignDatesToWellPath() const {
  if (!(m_bNewWellPaths == false && m_pNewWellPath == 0)) {
    ...
    for (it = m_NewWellList.begin(); it != m_NewWellList.end(); ++it) {
      ...
      for (it2 = m_NewDateList.begin(); it2 != m_NewDateList.end(); ++it2) {
...
void CFistExportDlg::OnExport() {
  ...
  // CTnoFileDialog dlg(FALSE, "fxi", NULL, OFN_HIDEREADONLY|OFN_OVERWRITEPROMPT|OFN_ENABLETEMPLATE/*|OFN_EXPLORER *//*|
  // OFN_ENABLETEMPLATEHANDLE*/, (LPCSTR)(file_types), FemAppGetMainWnd());
  CTnoFileDialog dlg(FALSE, "fxi", NULL, OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, (LPCSTR)(file_types),
                     FemAppGetMainWnd());

  dlg.m_ofn.lpstrTitle = "Export to FIST";
  /*dlg.m_ofn.lpTemplateName =
  MAKEINTRESOURCE(ID_EXPORT_DLG);//MAKEINTRESOURCE("EXPORT");//MAKEINTRESOURCE("NEWFILEOPENORD"); dlg.m_ofn.hInstance =
