...
CGraphLimitsDlg::CGraphLimitsDlg(OIV2DRangeInterface &rangeInterface, CWnd *pParent /*=NULL*/)
    : CDialog(CGraphLimitsDlg::IDD, pParent), m_rangeInterface(rangeInterface), m_iAutoLeft(true), m_iAutoTop(true),
      m_iAutoBottom(true) {
...
void CGraphLimitsDlg::DoDataExchange(CDataExchange *pDX) {
  ...
  if (pDX->m_bSaveAndValidate) {
    ...
    if (m_rangeInterface.HaveAxis(OIV2DRangeInterface::AXIS_BOTTOM)) {
      ...
      } else if (!m_iAutoBottom &&
                 !m_rangeInterface.RangeLargeEnough(OIV2DRangeInterface::AXIS_BOTTOM, dMinBottom, dMaxBottom)) {
        if (!error.isEmpty())
          error += "\r\n";
