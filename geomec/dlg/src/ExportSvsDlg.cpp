...
CExportSvsDlg::CExportSvsDlg(CModelBase &model, CWnd *pParent /*=NULL*/)
    : IExportDlg(CExportSvsDlg::IDD, pParent), m_Model(model), m_bFormationNames(FALSE), m_pCurrentSelection(0) {
...
BOOL CExportSvsDlg::OnInitDialog() {
  ...
  GetDlgItem(IDC_CHECK_SVS_MIXTC)
      ->EnableWindow(m_Model.ResultRegister().ResultsAvailable(CAnalysisType::AT_MIXTURE_CONTAINMENT));
  ...
  re::CreateExportResultTreeBranch<re::CFormationBaseEntryObserver, TFormationBaseEntry, MD_BASE_FORMATION,
                                   TFormationSelection>(m_Model, *m_pFormationSelection, m_tcCategories);
  m_vcSelection.push_back(m_pFormationSelection);

  m_pPointSetSelection = new TPointSetSelection(Selection());
  re::CreateExportResultTreeBranch<re::CPointSetEntryObserver, TPointSetEntry, MD_BASE_POINTSET, TPointSetSelection>(
      m_Model, *m_pPointSetSelection, m_tcCategories);
...
void CExportSvsDlg::TimeStep(const CDepletionStage &stage, bool bEnable) {
  if (!bEnable)
    m_stDeplStages.erase(&stage);
  if (bEnable)
    m_stDeplStages.insert(&stage);
}
...
void CExportSvsDlg::OnExport() {
  ...
  if (dlg.DoModal() == IDOK) {
    ...
    if (dlg.GetPathName().Right(4).CompareNoCase(".xls") == 0 ||
        dlg.GetPathName().Right(5).Left(4).CompareNoCase(".xls") == 0)
...
bool CExportSvsDlg::OnExport(const CString &sFileName, bool bExcel, bool bOld) {
  ...
  if (pExporter) {
    if (bExcel)
      pExporter->ExportExcel((LPCSTR)sFileName, data, arg);
    else
      pExporter->ExportASCII((LPCSTR)sFileName, data, arg);
  }
