...
CSetGlobalFaultBehaviourDialog::CSetGlobalFaultBehaviourDialog(CFemAppModel *femAppModel, CWnd *pParent)
    : CDialogEx(CSetGlobalFaultBehaviourDialog::IDD, pParent), m_setGlobalFaultBehaviour(femAppModel),
      m_editReferenceDepth(0), m_editReferencePressure(0), m_editGradient(0), m_firstTime(true),
...
void CSetGlobalFaultBehaviourDialog::OnBnClickedRadioButton() { enableControls(); }
...
ON_BN_CLICKED(IDC_RADIO_MAXIMUM, &CSetGlobalFaultBehaviourDialog::OnBnClickedRadioButton)
ON_BN_CLICKED(IDC_RADIO_MINIMUM, &CSetGlobalFaultBehaviourDialog::OnBnClickedRadioButton)
ON_BN_CLICKED(IDC_RADIO_GRADIENT, &CSetGlobalFaultBehaviourDialog::OnBnClickedRadioButton)
ON_BN_CLICKED(IDC_RADIO_MAXGRADIENT, &CSetGlobalFaultBehaviourDialog::OnBnClickedRadioButton)
END_MESSAGE_MAP()
...
void CSetGlobalFaultBehaviourDialog::OnOK() {
  ...
  if ((m_maximum.GetCheck() == BST_CHECKED) || (m_minimum.GetCheck() == BST_CHECKED) ||
      (m_gradient.GetCheck() == BST_CHECKED) || (m_maximumGradient.GetCheck() == BST_CHECKED)) {
    CGlobalUnitNode globalUnitNode;
    CConvertModeType convertModeType;
    CFaultPressure::TModeType modeType = convertModeType(m_valueGlobalFaultBehaviour);
    ...
    m_setGlobalFaultBehaviour.setGlobalFaultBehaviour(values, globalUnitNode.unitNode(), modeType);
  }
...
void CSetGlobalFaultBehaviourDialog::enableControls() {
  ...
  GetDlgItem(IDC_LBL_DEPTH)->EnableWindow(canModifyPressure && isGradientChecked);
  GetDlgItem(IDC_ED_DEPTH)->EnableWindow(canModifyPressure && isGradientChecked);
  GetDlgItem(IDC_UN_DEPTH)->EnableWindow(canModifyPressure && isGradientChecked);

  GetDlgItem(IDC_LBL_PRESSURE)->EnableWindow(canModifyPressure && isGradientChecked);
  GetDlgItem(IDC_ED_PRESSURE)->EnableWindow(canModifyPressure && isGradientChecked);
  GetDlgItem(IDC_UN_PRESSURE)->EnableWindow(canModifyPressure && isGradientChecked);

  GetDlgItem(IDC_LBL_GRADIENT)->EnableWindow(canModifyPressure && isGradientChecked);
  GetDlgItem(IDC_ED_GRADIENT)->EnableWindow(canModifyPressure && isGradientChecked);
  GetDlgItem(IDC_UN_GRADIENT)->EnableWindow(canModifyPressure && isGradientChecked);
}

namespace {

CString QString2CString(const QString &q) { return q.toStdString().c_str(); }

} // anonymous namespace

void CSetGlobalFaultBehaviourDialog::setUnitNames(CDataExchange *pDX) const {
  CGlobalUnitNode globalUnitNode;
  std::vector<QString> unitNames = m_setGlobalFaultBehaviour.getUnitNames(globalUnitNode.unitNode());
  std::vector<CString> newUnitNames;

  newUnitNames.resize(unitNames.size());

  std::transform(unitNames.begin(), unitNames.end(), newUnitNames.begin(), QString2CString);

  DDX_Text(pDX, IDC_UN_DEPTH, newUnitNames[CSetGlobalFaultBehaviour::DEPTH]);
  DDX_Text(pDX, IDC_UN_PRESSURE, newUnitNames[CSetGlobalFaultBehaviour::PRESSURE]);
  DDX_Text(pDX, IDC_UN_GRADIENT, newUnitNames[CSetGlobalFaultBehaviour::GRADIENT]);
}
