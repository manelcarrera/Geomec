...
void QAboutDlg::add_texts() {
  std::vector<eText> texts_v = {Working_Dir,     Gm_Default_Version, App_Last_Model_Version, Config_File,
                                Diapath_Current, Diapath_Env,        Diapath_Config_File,    Diapath_Force_Local};
  ...
  for (const auto &id : texts_v) {
    ...
    if (id == Diapath_Current || id == Diapath_Env) {
      QString path = id == Diapath_Current ? CEnvironment::instance()->get(CEnvironment::DIAPATH)
    ...
    if (id == Diapath_Env || id == Diapath_Config_File || id == Diapath_Force_Local) {
...
void QAboutDlg::add_pdfs() {
  QDir dir(QString::fromStdString(util::GetAppPath()), "*.pdf", QDir::Name,
...
void QAboutDlg::on_open_pdf(int i) { QDesktopServices::openUrl(QUrl::fromLocalFile(m_pdfs.at(i))); }
