...
std::string CMesherDlg::name( eLogLevel level )
{
  return g_log_level_s[ level ];
}
...
: CDialog(CMesherDlg::IDD, pParent),
  m_msg_v( std::vector <std::string>{} )
{
...
{ 
  CTnoFileDialog tnoFileDialog(FALSE, _T(".txt"), NULL, OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, _T("Text Files (*.txt)|*.txt||"));
