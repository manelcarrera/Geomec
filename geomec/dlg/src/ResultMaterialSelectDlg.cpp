...
CResultMaterialSelectDlg::CResultMaterialSelectDlg(CModelBase &model,
                                                   const std::vector<IRpnMaterialParameterProxy *> &materialProxies,
                                                   CWnd *pParent /*=NULL*/)
    : CDialog(CResultMaterialSelectDlg::IDD, pParent), m_Model(model), m_vcMaterialProxies(materialProxies),
      m_pTimeStep(0) {}
...
void CResultMaterialSelectDlg::OnBnClickedOk() {
  ...
  if ((m_chSetDepletion.GetCheck() == BST_CHECKED)) {
    for (std::vector<IRpnMaterialParameterProxy *>::const_iterator selectedProxy = m_pSelectedProxies.begin();
