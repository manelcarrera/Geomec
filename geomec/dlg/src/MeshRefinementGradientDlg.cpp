		...
		m_edit_dlg( nullptr )
{
	...
	m_values->gradient( 
		exists  ? 
			boxes_handler->data_m()[ box_name ].Profile() : 
			std::vector< RelativeSize >{} );

	m_type = exists  ? 
		boxes_handler->data_m()[ box_name ].GradientType() : 
		RefinementBox::PlanarZ;

	std::vector< 
		std::pair< QPushButton*, const QString& > > 
			btns_v = 	{
				{ m_delete_btn,	DELETE_IMG_URL },
				{ m_warning_btn,OK_IMG_URL },
				{ m_modified,	DONE_IMG_URL },
				{ m_zoom,		ZOOM_IMG_URL },
				{ m_enlarge,	ENLARGE_IMG_URL },
				{ m_decrease,	DECREASE_IMG_URL },
				{ m_edit_btn,	EDIT_IMG_URL },
				{ m_type_btn,	m_type == RefinementBox::PlanarZ ? PLANAR_IMG_URL : CENTER_IMG_URL },
	...
	for( auto btn : btns_v )
		set_btn_img( 
			btn.first, 
			btn.second, 
			btn.first != m_warning_btn );

	std::vector< QWidget* > wgt_v = { 
		m_checkbox, 
		m_delete_btn, 
		m_warning_btn, 
		m_modified, 
	...
	std::vector< 
		std::pair< eActions, const QString& > > 
			actions_v = {
				{ New,		TEXT_NEW },
				{ Delete,	TEXT_DELETE },
				{ Go,		TEXT_GO },
				{ Show_Hide,TEXT_SHOW_HIDE } };
	...
	for( auto a : actions_v )
	{
		...
		int a_ = a.first;
		connect(	m_actions[ a_ ],	&QAction::triggered,	
					this,				[ this, a_ ]{ action( (eActions)a_ ); });
	}
	...
	connect( this, SIGNAL( checkbox_change( const QString&, int ) ),	parent, SLOT( on_box_checkbox_change( const QString&, int ) ) );
	connect( this, SIGNAL( zoom_to_box_request( const QString& ) ),		parent, SLOT( on_zoom_to_box_request( const QString& ) ) );
	connect( this, SIGNAL( resize( const QString&, int ) ),				parent, SLOT( on_resize( const QString&, int ) ) ); // name ... up / down
...
void CGradientRow::on_apply()
{
	...
	if( pos_0 != pos )
		m_boxes_handler->pos( m_name, 
			RBox::rw_to_oiv( pos, RBox::Pos ) );

	if( size_0 != size )
		m_boxes_handler->size( m_name, 
			RBox::rw_to_oiv( size, RBox::Size ) );
}
...
void CGradientRow::on_cancel()
{
	...
	if( pos_0 != pos )
		m_boxes_handler->pos( m_name, 
			RBox::rw_to_oiv( pos_0, RBox::Pos ) );

	if( size_0 != size )
		m_boxes_handler->size( m_name, 
			RBox::rw_to_oiv( size_0, RBox::Size ) );
...
void CGradientRow::on_rotate()
{ 
	m_boxes_handler->rotate( m_name ); 
}
...
void CGradientRow::on_custom_context_menu( const QPoint &point )
{ 
...
void CGradientRow::update_warning( bool val )
{
	set_btn_img( 
		m_warning_btn, 
		val ? 
...
TGradientV CGradientRow::gradient()
{ 
	return m_values->gradient();  
}

bool CGradientRow::is_values_visible()
{ 
	return m_checkbox->isVisible(); 
};

void CGradientRow::show_values( bool val )
{ 
	on_checkbox( val ? Qt::Checked : Qt::Unchecked );
};
...
void CGradientRow::action( eActions action_ )
{
	switch( action_ )
	{
		case New:			on_new();			break;
		case Delete:		on_delete();		break;
		case Go:			on_zoom_to_box();	break;
...
CGradientValues::CGradientValues( QWidget *parent ) : QFrame( parent )
{
	...
	for( int i=0; i < DIVISIONS; i++ )
	{
		...
		if( i == 0 )				edit = new QLineEdit_Center( this );
		else if( i == DIVISIONS-1 )	edit = new QLineEdit();
		else						edit = new QLineEdit_Others( this );
		...
		edit->editingFinished();  // in case is a new one and not set_gradient is called later on : bg colors need to be set and warnig icon shown
	}
...
T3da CBoxEdit::pos()
{
	return T3da{ m_px->text().toDouble(), m_py->text().toDouble(), m_pz->text().toDouble() };
}

//rw coordinates
T3da CBoxEdit::size()
{
	return T3da{ m_sx->text().toDouble(), m_sy->text().toDouble(), m_sz->text().toDouble() };
}

//rw coordinates
T3da CBoxEdit::pos_ini()
{
	return m_pos_ini;
}

//rw coordinates
T3da CBoxEdit::size_ini()
{
	return m_size_ini;
}

void CBoxEdit::closeEvent( QCloseEvent *event )
{
	emit close();
}

void CBoxEdit::update(	T3da pos_,
						T3da size_ )
{
