...
QPushButton *add_btn(const QString &name, const QString &img, QHBoxLayout *layout) {
  QPushButton *btn = new QPushButton(name);
  btn->setIcon(QIcon(QPixmap(img)));
...
// pop-up's
namespace {
void show_warnings(const QString &name, const std::vector<QString> &over_v, bool res1, bool res2) {
  QString msg2;
  for (auto box : over_v)
  ...
  QString msg;
  if (!res1 && !res2)
    msg = QString::asprintf("<p align='left'>Box out of scope.<br><br>Box overlaps:<br>%s</p>",
                            msg2.toStdString().c_str());
  if (!res1 && res2)
  ...
  QMessageBox::warning(nullptr, QString::asprintf("Boxe geometry checks: %s", name.toStdString().c_str()), msg);
}
...
void show_overlap(std::vector<QString> &over_v) {
  ...
  for (auto name : over_v) {
    ...
    msg += QString::asprintf("-'%s'-'%s'<br>", list[0].toStdString().c_str(), list[1].toStdString().c_str());
  }
  msg = msg.left(msg.size() - 4);

  QMessageBox::warning(
      nullptr, "Boxes geometry checks",
      QString::asprintf("<p align='left'>Box(es) overlap (%dx):<br>%s</p>", over_v.size(), msg.toStdString().c_str()));
}
...
void show_out_of_scope(std::vector<QString> &out_v) {
  ...
  QMessageBox::warning(nullptr, "Boxes geometry checks",
                       QString::asprintf("<p align='left'>Box(es) out of scope (%dx):<br>%s</p>", out_v.size(),
...
MeshRefinementDlg::MeshRefinementDlg(CMeshRefinementBoxesHandler *boxes, QWidget *parent)
...
std::vector<RelativeSize> MeshRefinementDlg::gradient(const QString &name) { return m_rows_m[name]->gradient(); }

bool MeshRefinementDlg::visible(const QString &name) { return m_rows_m[name]->is_values_visible(); }

RefinementBox::GradientType_ MeshRefinementDlg::type(const QString &name) { return m_rows_m[name]->type(); }
...
void MeshRefinementDlg::on_pdf() { AfxGetApp()->GetMainWnd()->PostMessage(WM_COMMAND, ID_MR_MANUAL, 0); }
...
void MeshRefinementDlg::on_save() {
  if (is_changes()) // boxes + gradient
  {
    ...
    QMessageBox::warning(nullptr, TEXT_CONFIRM_TITLE, QString("No changes made so nothing to be saved"));
  }
}
...
void MeshRefinementDlg::showEvent(QShowEvent *) { m_boxes->show(); }
  ...
  if (gui_data_m.size() != data_m.size()) {
    ...
    for (auto name : gui_data_m.keys()) {
      if (data_m.contains(name)) {
        ...
        if (d1.Profile() != d2.Profile() || d1.IsVisible() != d2.IsVisible()) {
          equal = false;
          break;
        }
      } else {
...
// GUI boxes before being trespassed to data
// one box
bool MeshRefinementDlg::check_overlap(const QString &name, std::vector<QString> &over_v) {
...
// one box
bool MeshRefinementDlg::check(const QString &name, std::vector<QString> &over_v, bool &res1, bool &res2) {
  // scope
