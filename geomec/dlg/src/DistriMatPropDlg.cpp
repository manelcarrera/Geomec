...
CDistriMatPropDlg::CDistriMatPropDlg(const std::vector<const CValueType *> &vcValueTypes,
                                     std::set<const CValueType *> &stExtrapolatingValueTypes, CWnd *pParent)
    : CDialog(IDD_CONFIG_DISTRI_MAT, pParent), m_vcValueTypes(vcValueTypes), m_stSource(stExtrapolatingValueTypes),
      m_stCopy(stExtrapolatingValueTypes), m_pCurrent(0) {}
...
CDistriMatPropDlg::CMatPropObserver::CMatPropObserver(const CValueType &valuetype, CTreeCtrl &ctrl,
                                                      CDistriMatPropDlg &dlg)
    : ITreeObject(ctrl, TVI_ROOT, TVI_LAST), m_valuetype(valuetype), m_dlg(dlg) {
  ctrl.SetItemData(Handle(), (DWORD_PTR)this);
}

BOOL CDistriMatPropDlg::CMatPropObserver::CanDelete() { return FALSE; }
...
QString CDistriMatPropDlg::CMatPropObserver::Text() const { return m_valuetype.Name(); }

unsigned int CDistriMatPropDlg::CMatPropObserver::Icon() const { return m_valuetype.IconId(); }
