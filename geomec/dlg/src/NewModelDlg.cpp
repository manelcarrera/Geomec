...
CNewModelDlg::~CNewModelDlg() { delete m_pImageList; }
...
BOOL CNewModelDlg::OnInitDialog() {
  ...
  for (size_t i = 0; i < m_vcModel.size(); i++) {
    int nItem = m_lbModel.InsertItem(m_lbModel.GetItemCount(), (const char *)m_vcModel[i].sType.toStdString().c_str(),
...
int CNewModelDlg::SelectedModel() const { return m_nSelectedModel; }
