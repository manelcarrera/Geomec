...
CPointSetCreateDlg::CPointSetCreateDlg(CModelBase &model, CWnd *pParent /*=NULL*/)
    : CDialog(CPointSetCreateDlg::IDD, pParent), m_model(model), m_pointSetCoordinates(m_model, UnitNode(), false),
      m_bValidateDensityOnly(FALSE) {
  QString name = model.GraphEntry(MD_BASE_POINTSET)->createUniqueName(DEFAULT_POINT_SET_NAME, DEFAULT_INITIAL_NUMBER);
...
void CPointSetCreateDlg::CreatePointSet() {
  ...
  CPointSet *pPointSet = createPointSet.create(m_model, UnitNode(), (LPCTSTR)m_sName);
}
