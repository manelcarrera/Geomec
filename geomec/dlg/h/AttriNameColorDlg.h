...
template <class T> class CAttriNameColorDlg : public CAttributesTemplate<T> {
public:
...
template <class T> void CAttriNameColorDlg<T>::DoDataExchange(CDataExchange *pDX) {
...
template <class T> void CAttriNameColorDlg<T>::OnStColor() {
  Copy().Color(SelectColor(Copy().Color()));
  Invalidate();
}

template <class T> void CAttriNameColorDlg<T>::OnPaint() {
  CPaintDC dc(this);
