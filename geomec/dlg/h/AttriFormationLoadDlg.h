...
template <class LOAD_TYPE> class CAttriFormationLoadDlg : public CAttributesTemplate<LOAD_TYPE> {
public:
...
CAttriFormationLoadDlg<LOAD_TYPE>::CAttriFormationLoadDlg(LOAD_TYPE &load, unsigned int uDialogTemplate,
                                                          CWnd *pParent /*=NULL*/)
    : CAttributesTemplate<LOAD_TYPE>(uDialogTemplate, load, pParent) {}

// ##ModelId=3BC55D650153
template <class LOAD_TYPE> void CAttriFormationLoadDlg<LOAD_TYPE>::DoDataExchange(CDataExchange *pDX) {
  ...
  if (Copy().Type() == LOAD_TYPE::CONSTANT_MODE && (!Copy().DistributedOnly() || Copy().DistributedSize() == 0)) {
    ...
    DDV_MinMaxDouble(pDX, dConstantValue, Copy().Constant().ReferenceValue().MinValue(UnitNode().Unit()),
                     Copy().Constant().ReferenceValue().MaxValue(UnitNode().Unit()));
    DDV_MinMaxDouble(pDX, dConstantDepth, Copy().Constant().ReferenceDepth().MinValue(UnitNode().Unit()),
                     Copy().Constant().ReferenceDepth().MaxValue(UnitNode().Unit()));
    DDV_MinMaxDouble(pDX, dGradient, Copy().Constant().Gradient().MinValue(UnitNode().Unit()),
...
template <class LOAD_TYPE> BOOL CAttriFormationLoadDlg<LOAD_TYPE>::OnInitDialog() {
...
template <class LOAD_TYPE> CString CAttriFormationLoadDlg<LOAD_TYPE>::previousUndefinedLabel() const {
  return _T("Same as previous depletion stage");
}

template <class LOAD_TYPE> void CAttriFormationLoadDlg<LOAD_TYPE>::OnPreDefined() {
  UpdateData(TRUE);
  Copy().DistributedOnly(false);
  UpdateControls();
}

template <class LOAD_TYPE> void CAttriFormationLoadDlg<LOAD_TYPE>::OnExtraPolate() {
  UpdateData(TRUE);
  Copy().DistributedOnly(true);
  UpdateControls();
}

template <class LOAD_TYPE> void CAttriFormationLoadDlg<LOAD_TYPE>::OnUndefined() {
  UpdateData(TRUE);
  Copy().Type(LOAD_TYPE::REPEAT_MODE);
  UpdateControls();
}

template <class LOAD_TYPE> void CAttriFormationLoadDlg<LOAD_TYPE>::OnConstant() {
  UpdateData(TRUE);
  Copy().Type(LOAD_TYPE::CONSTANT_MODE);
  UpdateControls();
}

template <class LOAD_TYPE> void CAttriFormationLoadDlg<LOAD_TYPE>::UpdateControls() {
  ...
  if (GetDlgItem(IDC_RD_UNDEFINED))
    GetDlgItem(IDC_RD_UNDEFINED)
        ->EnableWindow(!bDisableAll && (!Copy().DistributedOnly() || Copy().DistributedSize() == 0));
  if (GetDlgItem(IDC_RB_CONSTANT))
    GetDlgItem(IDC_RB_CONSTANT)
        ->EnableWindow(!bDisableAll && (!Copy().DistributedOnly() || Copy().DistributedSize() == 0));

  if (GetDlgItem(IDC_ED_CONSTANT_PRESSURE)) {
    GetDlgItem(IDC_ED_CONSTANT_PRESSURE)
        ->EnableWindow((Copy().Type() == LOAD_TYPE::CONSTANT_MODE) && !bDisableAll &&
                       (!Copy().DistributedOnly() || Copy().DistributedSize() == 0));
    GetDlgItem(IDC_LBL_CONSTANT_PRESSURE)
        ->EnableWindow((Copy().Type() == LOAD_TYPE::CONSTANT_MODE) && !bDisableAll &&
                       (!Copy().DistributedOnly() || Copy().DistributedSize() == 0));
    if (GetDlgItem(IDC_PRESSURE_UNIT_1))
      GetDlgItem(IDC_PRESSURE_UNIT_1)
          ->EnableWindow((Copy().Type() == LOAD_TYPE::CONSTANT_MODE) && !bDisableAll &&
                         (!Copy().DistributedOnly() || Copy().DistributedSize() == 0));
  }

  if (GetDlgItem(IDC_ED_CONSTANT_DEPTH)) {
    GetDlgItem(IDC_ED_CONSTANT_DEPTH)
        ->EnableWindow((Copy().Type() == LOAD_TYPE::CONSTANT_MODE) && !bDisableAll &&
                       (!Copy().DistributedOnly() || Copy().DistributedSize() == 0));
    GetDlgItem(IDC_LBL_CONSTANT_DEPTH)
        ->EnableWindow((Copy().Type() == LOAD_TYPE::CONSTANT_MODE) && !bDisableAll &&
                       (!Copy().DistributedOnly() || Copy().DistributedSize() == 0));
    GetDlgItem(IDC_ST_DEPTH_UNIT_1)
        ->EnableWindow((Copy().Type() == LOAD_TYPE::CONSTANT_MODE) && !bDisableAll &&
                       (!Copy().DistributedOnly() || Copy().DistributedSize() == 0));
  }

  if (GetDlgItem(IDC_ED_CONSTANT_GRADIENT)) {
    GetDlgItem(IDC_ED_CONSTANT_GRADIENT)
        ->EnableWindow((Copy().Type() == LOAD_TYPE::CONSTANT_MODE) && !bDisableAll &&
                       (!Copy().DistributedOnly() || Copy().DistributedSize() == 0));
    GetDlgItem(IDC_LBL_CONSTANT_GRADIENT)
        ->EnableWindow((Copy().Type() == LOAD_TYPE::CONSTANT_MODE) && !bDisableAll &&
                       (!Copy().DistributedOnly() || Copy().DistributedSize() == 0));
    GetDlgItem(IDC_ST_GRADIENT_UNIT_1)
        ->EnableWindow((Copy().Type() == LOAD_TYPE::CONSTANT_MODE) && !bDisableAll &&
                       (!Copy().DistributedOnly() || Copy().DistributedSize() == 0));
  }
