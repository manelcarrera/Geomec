...
template <class LOAD_TYPE> class CAttriGlobalFormationLoadDlg : public CAttributesTemplate<LOAD_TYPE> {
public:
...
CAttriGlobalFormationLoadDlg<LOAD_TYPE>::CAttriGlobalFormationLoadDlg(unsigned int uDialogTemplate, LOAD_TYPE &load,
                                                                      CWnd *pParent)
    : CAttributesTemplate<LOAD_TYPE>(uDialogTemplate, load, pParent) {}

template <class LOAD_TYPE> void CAttriGlobalFormationLoadDlg<LOAD_TYPE>::DoDataExchange(CDataExchange *pDX) {
...
template <class LOAD_TYPE> BOOL CAttriGlobalFormationLoadDlg<LOAD_TYPE>::OnInitDialog() {
...
template <class LOAD_TYPE> void CAttriGlobalFormationLoadDlg<LOAD_TYPE>::OnPreDefined() {
  UpdateData(TRUE);
  Copy().DistributedOnly(false);
  EnableControls();
}

template <class LOAD_TYPE> void CAttriGlobalFormationLoadDlg<LOAD_TYPE>::OnExtraPolate() {
  UpdateData(TRUE);
  Copy().DistributedOnly(true);
  EnableControls();
}

template <class LOAD_TYPE> void CAttriGlobalFormationLoadDlg<LOAD_TYPE>::EnableControls() {
  ...
  GetDlgItem(IDC_LBL_CONSTANT_PRESSURE)
      ->EnableWindow(!Copy().ReadOnly() && (!Copy().DistributedOnly() || Copy().DistributedSize() == 0));
  GetDlgItem(IDC_LBL_CONSTANT_DEPTH)
      ->EnableWindow(!Copy().ReadOnly() && (!Copy().DistributedOnly() || Copy().DistributedSize() == 0));
  GetDlgItem(IDC_LBL_CONSTANT_GRADIENT)
      ->EnableWindow(!Copy().ReadOnly() && (!Copy().DistributedOnly() || Copy().DistributedSize() == 0));

  GetDlgItem(IDC_ED_CONSTANT_VALUE)
      ->EnableWindow(!Copy().ReadOnly() && (!Copy().DistributedOnly() || Copy().DistributedSize() == 0));
  ...
  GetDlgItem(IDC_LOAD_UNIT)
      ->EnableWindow(!Copy().ReadOnly() && (!Copy().DistributedOnly() || Copy().DistributedSize() == 0));
  GetDlgItem(IDC_ST_DEPTH_UNIT)
      ->EnableWindow(!Copy().ReadOnly() && (!Copy().DistributedOnly() || Copy().DistributedSize() == 0));
  GetDlgItem(IDC_ST_GRADIENT_UNIT)
      ->EnableWindow(!Copy().ReadOnly() && (!Copy().DistributedOnly() || Copy().DistributedSize() == 0));
