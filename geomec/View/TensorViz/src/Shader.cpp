...
const char* vertexShaderSrc[] = 
{
  ...
  "uniform vec2 colorScaleRange;\n",           // The minimum (x) and maximum (y) property values to map onto the colorscale
  "uniform mat4 mvMatrix;\n",                  // The model-view matrix, transforms coordinates from object to view space
  "uniform mat4 mvpMatrix;\n",                 // The model-view-projection matrix, same as mvMatrix, but does projection as well
  ...
  "void main()\n",
  "{\n",
  "  vec4 vertex = vec4(center + scale * (rotation * position), 1.0);\n", // Transform the vertex to model space, using the instanced parameters
  "  gl_ClipDistance[0] = dot(clipPlane, mvMatrix * vertex);\n", // Necessary for the GPU to do proper clipping of our beachballs, if enabled
  "  gl_Position = mvpMatrix * vertex;\n", // Transform the vertex position by the model-view-projection matrix
  "#ifdef PRIMARY_COLORMAP_ENABLED\n",
  "  float texcoord1 = (propertyValue1 - colorScaleRange.x) / (colorScaleRange.y - colorScaleRange.x);\n", // Normalize property value
  "  vec3 primaryColor = texture(colorScale, texcoord1).xyz;\n", // Lookup color in colormap using normalized property value as texture coordinate
  "#endif\n",
  "#ifdef SECONDARY_COLORMAP_ENABLED\n",
  "  float texcoord2 = (propertyValue2 - colorScaleRange.x) / (colorScaleRange.y - colorScaleRange.x);\n",
...
Shader::Shader()
  : program(0)
  , colorScaleLocation(-1)
  , colorScaleRangeLocation(-1)
  , mvMatrixLocation(-1)
  , mvpMatrixLocation(-1)
