...
GLuint generateTextureFromColorMap(const MiColorMapping<double, SbColorRGBA> *colorMap) {
  ...
  for (int i = 0; i < width; ++i) {
    ...
    uint32_t colorABGR = ((colorRGBA & 0x000000ff) << 24) | ((colorRGBA & 0x0000ff00) << 8) |
...
// to render only a single beachball on each node, even when it's shared between several cells. The cell filter must
// also be taken into account so the "decimate" slider works for beachballs as well
void getActiveNodeIndices(const MiTopologyExplicitI &topology, const MiCellFilterI *cellFilter,
...
SbMatrix3 getRotationMatrix(const CTensor &tensor) {
  ...
  SbMatrix3 rotationMatrix = SbMatrix3((float)pdirs.Value(0, 0), (float)pdirs.Value(1, 0), (float)pdirs.Value(2, 0),
 ...
 * \param mesh       The MeshViz unstructured mesh interface giving us the geometry and topology on which to map the
 * tensors
 ...
 */
VBOMesh generateInstancedTensorsMesh(const MiMeshUnstructuredI *mesh, const MiCellFilterI *cellFilter,
                                     const MiTensorSetI &tensors, const MiScalardSetI *scalars1,
                                     const MiScalardSetI *scalars2, const Mesh &tensorMesh) {
  glBindVertexArray(0);
...
void MoMeshTensor::GLRender(SoGLRenderAction *action) {
  ...
  int shaderBuildFlags = ((primaryScalarSet != 0) ? Shader::PRIMARY_COLORMAP_ENABLED : 0) |
  ...
  if (meshType == VOLUME_MESH_UNSTRUCTURED || meshType == SURFACE_MESH_UNSTRUCTURED) {
    ...
    if ((currentTopologyTimestamp != m_topologyTimestamp) || (currentGeometryTimestamp != m_geometryTimestamp) ||
        (currentFilterTimestamp != m_cellFilterTimestamp) || (currentScalarSet1Timestamp != m_scalarSet1Timestamp) ||
        (currentScalarSet2Timestamp != m_scalarSet2Timestamp) || (currentTensorTimestamp != m_tensorsTimestamp)) {
      ...
      m_vboMesh = generateInstancedTensorsMesh(meshUnstructured, cellFilterI, *tensors, primaryScalarSet,
                                               secondaryScalarSet, m_mesh);
    }
    ...
    glUniform4f(shader.clipPlaneLocation, (float)clipPlane[0], (float)clipPlane[1], (float)clipPlane[2],
                (float)clipPlane[3]);
    GLGETERROR();
...
void MoMeshTensor::getBoundingBox(SoGetBoundingBoxAction *action) {
  ...
  if (meshType == VOLUME_MESH_UNSTRUCTURED || meshType == SURFACE_MESH_UNSTRUCTURED) {
    ...
    if (m_bboxTopologyTimestamp != topology.getTimeStamp() || m_bboxGeometryTimestamp != geometry.getTimeStamp()) {
    ...
    box.setBounds((float)m_minVec[0] - delta, (float)m_minVec[1] - delta, (float)m_minVec[2] - delta,
...
/*static*/ void MoMeshTensor::exitClass() { SO__NODE_EXIT_CLASS(MoMeshTensor); }

MoMeshTensor::MoMeshTensor()
    : m_topologyTimestamp(0), m_geometryTimestamp(0), m_tensorsTimestamp(0), m_cellFilterTimestamp(0),
      m_scalarSet1Timestamp(0), m_scalarSet2Timestamp(0), m_colorMapTimestamp(0), m_bboxTopologyTimestamp(0),
      m_bboxGeometryTimestamp(0), m_colorMapRangeMin(0.0f), m_colorMapRangeMax(1.0f),
      m_minVec(MbVec3d::numeric_limit_max()), m_maxVec(-MbVec3d::numeric_limit_max()), m_positionsVBO(0),
      m_positionsCount(0), m_colorMapTexture(0) {
