...
typedef CNodeObserver_Delegate<TPointSetEntry, TPointSetEntry_Delegate, CDummyNode, CDummyObserver, FALSE, FIXED_ITEM>
...
class CPointSetEntryObserver : public TPointSetEntryObserver {
  typedef CNodeObserver_Delegate<IPointSet, IPointSet_Delegate, CDummyNode, CDummyObserver, TRUE, DELETE_ITEM>
      TPointSetObserver;
  ...
  public:
    CPointSetSubBranch(CPointSetEntryObserver &parent_observer, const CPointSet::DIMENSION dim, const CString &strName,
                       const unsigned int uIcon, HTREEITEM hInsertAfter = TVI_LAST);
    virtual BOOL OnFilter(const child_type &t) const;
...
public:
  CPointSetEntryObserver(TPointSetEntry &node, CModelBase &model, CTreeCtrl &view, HTREEITEM hParent = TVI_ROOT,
                         HTREEITEM hInsertAfter = TVI_LAST);
  ...
  virtual CTreeNode *OnInsertPointSetObserver(CPointSetSubBranch &branch, IPointSet &point_set);
