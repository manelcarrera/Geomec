...
BOOL CPointSetEntryObserver::OnFilterPointSet(const IPointSet &point_set) const { return TRUE; }
...
CTreeNode *CPointSetEntryObserver::OnInsertPointSetObserver(CPointSetSubBranch &branch, IPointSet &point_set) {
  CTreeNode *pTreeNode =
      branch.CEnumerationBranch<IPointSet, TPointSetObserver, TRUE, DELETE_ITEM>::InsertChild(point_set);
...
CPointSetEntryObserver::CPointSetSubBranch::CPointSetSubBranch(CPointSetEntryObserver &parent_observer,
                                                               const CPointSet::DIMENSION dim, const CString &strName,
                                                               const unsigned int uIcon, HTREEITEM hInsertAfter)
...
CPointSetEntryObserver::CPointSetEntryObserver(TPointSetEntry &node, CModelBase &model, CTreeCtrl &ctrl,
                                               HTREEITEM hParent, HTREEITEM hInsertAfter)
    : CNodeObserver_Delegate<TPointSetEntry, TPointSetEntry_Delegate, CDummyNode, CDummyObserver, FALSE, FIXED_ITEM>(
          node, ctrl, FALSE, FIXED_ITEM, hParent, hInsertAfter) {
  CPointSetSubBranch *p = new CPointSetSubBranch(*this, CPointSet::DIM_2D, _T("2D"), IDI_POINTSET);
  p->StateIcon();
  new CPointSetSubBranch(*this, CPointSet::DIM_3D, _T("3D"), IDI_POINTSET);
}
...
void CPointSetEntryObserver::AppendContextMenu(CContextMenuInvoker &invoker) {
  ...
  invoker.AddCommand(_T("&Create point set ..."),
                     *(new TPointSetBranchCommand(*this, &CPointSetEntryObserver::CreatePointSet)));
