...
COpenGLSceneBase::CNodeLocationListener::CNodeLocationListener(COpenGLSceneBase &scene)
    : COpenGLValueScene::CDefaultListener(scene), m_pHighlighter(new CNodeLocationHighlighter(scene)) {}

COpenGLSceneBase::CNodeLocationListener::~CNodeLocationListener() { delete m_pHighlighter; }

bool COpenGLSceneBase::CNodeLocationListener::MouseMove(TKeyboardModifiers state, TMouseButton button,
...
COpenGLSceneBase::COpenGLSceneBase(CFemAppModel &model) : COpenGLValueScene(model) { Initialize(); }

// ##ModelId=3BE7AEBD0342
COpenGLSceneBase::COpenGLSceneBase(const QString &strName, CFemAppModel &model) : COpenGLValueScene(strName, model) {
  Initialize();
}

// ##ModelId=3BE7AEBD03AE
COpenGLSceneBase::COpenGLSceneBase(const unsigned int uName, CFemAppModel &model) : COpenGLValueScene(uName, model) {
  Initialize();
}
...
COpenGLSceneBase::~COpenGLSceneBase() {
  if (m_pBoundaryObserver)
    delete m_pBoundaryObserver;
...
void COpenGLSceneBase::OnNewNeighbour(const CGraphNode &node) {
  if (dynamic_cast<const COpenGLNode *>(&node) || dynamic_cast<const IValueComponentBase *>(&node) ||
      dynamic_cast<const IValueComposite *>(&node) || dynamic_cast<const IColorScaleNode *>(&node)) {
...
bool COpenGLSceneBase::CanConnectItem(const CGraphNode &item) const {
  ...
  if ((!doc.CurrentScene() || doc.CurrentScene()->OpenGLScene() != this) && doc.StickToScene()) {
    ...
    pFrame->StatusBar().SetWindowText("GEOMEC is currently sticking to the current view. Unset this option to allow "
                                      "switching views and display this item");
    return false;
  }
...
void COpenGLSceneBase::OnUpdateValueLegend() {
  CColorScaleEntry *pEntry =
      dynamic_cast<CColorScaleEntry *>(((CFemAppModel &)Model()).GraphEntry(MD_BASE_COLOR_SCALE));
  assert(pEntry);
...
unsigned int COpenGLSceneBase::IconId() const { return IDI_VIEW; }

// ##ModelId=3BC55D600296
unsigned int COpenGLSceneBase::TypeId() const { return 0; }
...
BOOL COpenGLSceneBase::ShowBoundary() const { return DisplayBoundingBox(); }
...
bool COpenGLSceneBase::Empty() const { return false; }
...
int COpenGLSceneBase::ShowEntry(unsigned int uEntry, enum eShowMode mode) {
  ...
  for (CGraphEntry::TEntryNodeSet::const_iterator it = stTemp.begin(); it != stTemp.end(); it++) {
    if ((mode == SHOW_ALL) || ((mode == SHOW_SELECTED) && IsLinkedTo(**it)) ||
