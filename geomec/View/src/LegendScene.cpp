...
LegendScene::LegendScene()
    : m_font(0)
    , m_material(0)
    , m_valuesTextSwitch(0)
    , m_valuesLegendSwitch(0)
    , m_valuesAdornments(0)
    , m_valuesAdornmentCount(0)
    , m_valuesLegendOffsetX(10)
    , m_valuesLegendOffsetY(25)
    , m_valuesLegendWidth(20)
    , m_valuesLegendHeight(400)
    , m_valuesText(0)
    , m_textSize(12)
    , m_textSpacing(1.5f)
    , m_valuesTextOffset(3)
    , m_valuesTextTranslation(0)
    , m_valuesCount(11)
    , m_geologySwitch(0)
    , m_geologyHeadersText(0)
...
void LegendScene::updateValuesLegend(float widthScreen, float heightScreen, double minimum, double maximum)
{
    ...
    m_valuesLegend->topRight = SbVec2f(left + offsetX, top - offsetY - height); // topRight is actually coordinates of bottomLeft on screen
    m_valuesLegend->bottomLeft = SbVec2f(left + offsetX + width, top - offsetY); // bottomLeft is actually coordinates of topRight on screen

    m_valuesTextTranslation->translation = SbVec3f (m_valuesLegendOffsetX + m_valuesLegendWidth + m_valuesTextOffset, -(m_valuesLegendOffsetY + (m_valuesAdornmentCount - 1) * m_textSize * m_textSpacing), 0);
...
void LegendScene::SetTextColor( float * rgb )
{
...
void LegendScene::updateGeologyLegend( )
{
    ...
    size_t itemIndex;
    m_listOfGeologyLegendItemLists.listLegendItems (itemIndex, m_geologyHeadersText->string, m_geologyItemsText->string, m_geologyMarkersCoordinates->vertex, m_geologyMarkersMaterial->diffuseColor);
...
bool LegendScene::isDisplayingValueLegend()
{
    return m_valuesLegendSwitch->whichChild.getValue() == SO_SWITCH_ALL;
}
...
const std::string LegendScene::getTitle() const
{
    return m_valuesLegend->title.getValue().getString();
}
...
void LegendScene::initLegend()
{
    ...
    m_valuesLegend->displayValues = false; // Not using the legend values because we cannot set font or display side of these values
...
SbBox2i32 LegendScene::getValuesLegendBoundingBox()
{
    ...
    SbBox2i32 bbox (m_valuesLegendOffsetX - extension, getFullValuesLegendOffsetY() - extension, m_valuesLegendOffsetX + m_valuesLegendWidth + extension, getFullValuesLegendOffsetY() + m_valuesLegendHeight + extension);
...
GeologyLegendItemList * ListOfGeologyLegendItemLists::getGeologyLegendItemList( const std::string & listName )
{
    ...
    for (auto itemList : m_listOfGeologyLegendItemLists)
    {
        ...
        if (itemList->name() > listName) break;
...
void ListOfGeologyLegendItemLists::listLegendItems( size_t & itemIndex, SoMFString &geologyHeaders, SoMFString &geologyItems, SoMFVec3f &point, SoMFColor &diffuseColor )
{
    itemIndex = 0;
    for (auto itemList : m_listOfGeologyLegendItemLists)
    {
...
void GeologyLegendItemList::listItems(size_t & itemIndex,  SoMFString & geologyHeaders, SoMFString & geologyItems, SoMFVec3f & point, SoMFColor & diffuseColor )
{
    for (auto item : m_geologyLegendItemList)
    {
        geologyHeaders.set1Value (itemIndex, "");
...
bool GeologyLegendItemList::empty()
{
    return m_geologyLegendItemList.empty();
}

size_t GeologyLegendItemList::size()
{
    return m_geologyLegendItemList.size();
}
