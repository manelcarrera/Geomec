...
typedef CNodeObserver_Delegate<CSupportDepletion, CSupportDepletion_Delegate, CValueType, TValueTypeObserver, FALSE, UNLINK_ITEM> TDepletionObserver;
typedef CNodeObserver_Delegate<CBaseSupportNode, CBaseSupportNode_Delegate, CSupportDepletion, TDepletionObserver, FALSE, FIXED_ITEM> TBaseSupportObserver;
typedef COpenGLNodeObserver_Delegate<CBoundaryBase, CBoundaryBase_Delegate, CBaseSupportNode, TBaseSupportObserver, FALSE, FIXED_ITEM> TBoundaryObserver;
typedef COpenGLNodeObserver_Delegate<CHexaMeshRegionBase, CHexaMeshRegionBase_Delegate, CDummyNode, CDummyObserver, TRUE, UNLINK_ITEM> TMeshRegionObserver;
//##ModelId=3E3E5F020199
typedef CStateBranch_Delegate<CHexaMeshRegionEntry, CHexaMeshRegionEntry_Delegate, CHexaMeshRegionBase, TMeshRegionObserver, CUndefinedIconProvider, TRUE, DELETE_ITEM> TMeshRegionEntryObserver;
typedef CStateBranch_Delegate<CCrossSectionEntry, CCrossSectionEntry_Delegate, CCrossSection, TCrossSectionObserver, CUndefinedIconProvider, TRUE, DELETE_ITEM> TXSectionEntryObserver;
//##ModelId=3E3E5F0201A7
typedef CStateBranch_Delegate<CNewWellPathEntry, CNewWellPathEntry_Delegate, CNewWellPath, CNewWellPathObserver, CUndefinedIconProvider, TRUE, DELETE_ITEM> TNewWellPathEntryObserver; //wjrx mantis 3185

typedef CSelectionLeafObserver_Delegate<IValueComposite, IValueComposite_Delegate, CGeomecDoc, TRUE> TCompositeObs;
typedef CSelectionBranchObserver_Delegate<CGraphEntry, CGraphEntry_Delegate, IValueComposite, TCompositeObs, IValueComposite, CGeomecDoc, FALSE, FIXED_ITEM> TMeshEntryObs;
typedef CSelectionBranchObserver_Delegate<CGraphEntry, CGraphEntry_Delegate, IValueComposite, TCompositeObs, IValueComposite, CGeomecDoc, TRUE , DELETE_ITEM> TDerivedEntryObs;

typedef CNodeObserver_Delegate <CExportResultData, CExportResultData_Delegate,
  CDummyNode, CDummyObserver, FALSE, FIXED_ITEM> TExportResultDataObserver;
typedef CNodeObserver_Delegate <CExportResultDataEntry,
  CExportResultDataEntry_Delegate, CExportResultData, TExportResultDataObserver,
  TRUE, DELETE_ITEM> TExportResultDataEntryObserver;
...
public:
	...
	CNewWellPathEntryObserver(CNewWellPathEntry& node,
						   CTreeNode &parent,
						   const BOOL rename,
	...
	CNewWellPathEntryObserver(CNewWellPathEntry& node,
						   ...
						   const BOOL bInitialUpdate = TRUE,
						   BOOL bSortAfterInsertion = TRUE)
	: TNewWellPathEntryObserver(node, ctrl, rename, remove, hParent, hInsertAfter, bInitialUpdate, bSortAfterInsertion)
	{ }
  ...
  virtual BOOL OnFilter(const CNewWellPath& node) const
  {
    return node.Links<CGroupNode>().empty();
  }
};
...
CModelTreeView::CModelTreeObject::CModelTreeObject(CFemAppDoc* pDoc,
												   CTreeCtrl& ctrl,
												   HTREEITEM hParent,
...
/* NOT OBSOLETE */ void CModelTreeView::CModelTreeObject::AppendContextMenu(CContextMenuInvoker &invoker)
{
	...
	if(pModel)
	{
		...
		if(pHexa)
		{
          ...
          {
					  invoker.AddCommand(_T("Convert to tetramodel"), *(new CSingleCommandTemplate <CHexaGUI> (*pHexaGUI, &CHexaGUI::ConvertToTetraModel, &CHexaGUI::CanConvertToTetra)));
    ...
    if(pModel->parentModel())
    {
      invoker.AddCommand(_T("Switch back to parent model"), *new CSingleCommandTemplate<CGUIBase>(*pGUI, &CGUIBase::SwitchToParent));
      invoker.AddCommand(_T("Reinitialize depletion stages"), *new CSingleCommandTemplate<CModelBase>(*pModel, &CModelBase::MenuInvokedBuildDepletionStages));
      invoker.AddCommand(_T("Set parent model results type"), *new CSingleCommandTemplate <CGUIBase> (*pGUI, &CGUIBase::SetParentModelResults));
    }
    ...
    if (pWellModel && pWellGUI)
    {
      invoker.appendSeparator();
		...
		invoker.AddCommand(_T("Mesh refinement"),*(new CSingleCommandTemplate<CGUIBase>(*pGUI,&CGUIBase::MeshRefinement,&CHexaGUI::Can_MeshRefinement)));
	}
}


/////
CModelTreeView::CViewButton::CViewButton(CModelTreeView& view, TClicked fnClicked, const CString& text, const CRect& rect, COLORREF color)
: m_view(view),
  m_fnClicked(fnClicked),
  m_text(text),
  m_rect(rect),
  m_color(color),
  m_bEnabled(true),
...
void CModelTreeView::CViewButton::OnDraw(CDC* pDC)
{
  if(m_bVisible)
  {
    ...
    pDC->TextOut(m_rect.left + m_rect.Width() / 2 - textSize.cx / 2, m_rect.top + m_rect.Height() / 2 - textSize.cy / 2, m_text);
...
bool CModelTreeView::CViewButton::isEnabled() const
{
  return m_bEnabled;
}
...
bool CModelTreeView::CViewButton::isVisible() const
{
  return m_bVisible;
}
...
void CModelTreeView::CViewButton::Invalidate()
{
  m_view.InvalidateRect(m_rect);
}

void CModelTreeView::CViewButton::Clicked()
{
  (m_view.*m_fnClicked)();
}
...
void CModelTreeView::OnInitialUpdate() 
{
    ...
    {
	    typedef CNodeObserver_Delegate<CGlobalInitialStressNode, CGlobalInitialStressNode_Delegate, IValueComposite, CValueCompositeObserver, TRUE, UNLINK_ITEM> TInitStressObs;
	    AddRoot(*(new TInitStressObs(pModel->GlobalInitialStress(), GetTreeCtrl(), FALSE, FIXED_ITEM, hModel, TVI_LAST, TRUE, FALSE)));
    }

	  typedef CNodeObserver_Delegate<CGlobalPressure, CGlobalPressure_Delegate, IValueComposite, CValueCompositeObserver, TRUE, UNLINK_ITEM> TInitPressureObs;
	  AddRoot(*(new TInitPressureObs(pModel->GlobalPressure(), GetTreeCtrl(), FALSE, FIXED_ITEM, hModel, TVI_LAST, TRUE, FALSE)));

	  typedef CNodeObserver_Delegate<CGlobalTemperature, CGlobalTemperature_Delegate, IValueComposite, CValueCompositeObserver, TRUE, UNLINK_ITEM> TInitTemperatureObs;
	  AddRoot(*(new TInitTemperatureObs(pModel->InitialTemperature(), GetTreeCtrl(), FALSE, FIXED_ITEM, hModel, TVI_LAST, TRUE, FALSE)));

    if(pCasingModel)
    {
      AddRoot(*new CWellCasingNodeObserver(pCasingModel->CasingNode(), GetTreeCtrl(), FALSE, FIXED_ITEM, hModel, TVI_LAST, FALSE, FALSE));
      AddRoot(*new CWellCasingCementObserver(pCasingModel->CasingCement(), GetTreeCtrl(), FALSE, FIXED_ITEM, hModel, TVI_LAST, FALSE, FALSE));
    }

		if(Document()->Model()->GraphEntry(MD_HEXA_HORIZON))
//			AddRoot(*(new THorizonEntryObserver((*Document()->Model()->GraphEntry(MD_HEXA_HORIZON)), GetTreeCtrl(), FALSE, DELETE_ITEM, hModel, TVI_LAST, TRUE, FALSE)));
			AddRoot(*(new C3DHorizonEntryObserver<true>((*Document()->Model()->GraphEntry(MD_HEXA_HORIZON)), GetTreeCtrl(), FALSE, DELETE_ITEM, hModel, TVI_LAST)));
    ...
    typedef C3DHorizonEntryObserver<false> TTetraHorizonEntryObserver;
		//typedef CStateBranch_Delegate<CTetraSuperHorizonEntry, CTetraSuperHorizonEntry_Delegate, CHorizonBase, CHorizonObserver<FALSE>, CUndefinedIconProvider, TRUE, DELETE_ITEM> TTetraHorizonEntryObserver;
		if(Document()->Model()->GraphEntry(MD_TETRA_SUPER_HORIZON))
		{
			...
			{
#endif
//				AddRoot(*(new TTetraHorizonEntryObserver((CTetraSuperHorizonEntry&)(*Document()->Model()->GraphEntry(MD_TETRA_SUPER_HORIZON)), GetTreeCtrl(), FALSE, DELETE_ITEM, hModel, TVI_LAST, TRUE, FALSE)));
				AddRoot(*(new TTetraHorizonEntryObserver((CTetraSuperHorizonEntry&)(*Document()->Model()->GraphEntry(MD_TETRA_SUPER_HORIZON)), GetTreeCtrl(), FALSE, DELETE_ITEM, hModel, TVI_LAST)));
#ifdef TETRA_SUPERMODEL_KEYFILE
			}
#endif
		}
    ...
    if(Document()->Model()->GraphEntry(MD_BASE_NONMESHEDSURFACE))
      AddRoot(*(new TNonMeshedSurfaceEntryObserver((CNonMeshedSurfaceEntry&)(*Document()->Model()->GraphEntry(MD_BASE_NONMESHEDSURFACE)), GetTreeCtrl(), FALSE, FIXED_ITEM, hModel, TVI_LAST)));

		if(Document()->Model()->GraphEntry(MD_HEXA_FORMATION))
		{
			AddRoot(*(new CHexaFormationEntryObserver((CHexaFormationEntry&)(*Document()->Model()->GraphEntry(MD_HEXA_FORMATION)), GetTreeCtrl(), hModel, TVI_LAST)));
		}
		else if(Document()->Model()->GraphEntry(MD_TETRA_FORMATION))
		{
			AddRoot(*(new CTetraFormationEntryObserver((CTetraFormationEntry&)(*Document()->Model()->GraphEntry(MD_TETRA_FORMATION)), GetTreeCtrl(), hModel, TVI_LAST)));
		}
    else if(Document()->Model()->GraphEntry(MD_WELLMODEL_FORMATION))
    {
      AddRoot(*(new CWellFormationEntryObserver((CWellFormationEntry&)(*Document()->Model()->GraphEntry(MD_WELLMODEL_FORMATION)), GetTreeCtrl(), hModel, TVI_LAST)));
    }

    if(pWZIModel)
    {
      typedef COpenGLNodeObserver_Delegate<CWellZoomInMesh, CWellZoomInMesh_Delegate, CDummyNode, CDummyObserver, FALSE, FIXED_ITEM> TZoomInMeshObs;
      new TZoomInMeshObs(pWZIModel->Mesh(), GetTreeCtrl(), FALSE, FIXED_ITEM, hModel, TVI_LAST, TRUE, FALSE);
    }

    if(pCasingModel)
    {
      typedef COpenGLNodeObserver_Delegate<CWellCasingMesh, CWellCasingMesh_Delegate, CDummyNode, CDummyObserver, FALSE, FIXED_ITEM> TCasingMeshObs;
    ...
    if(pWZIModel)
    {
      typedef CNodeObserver_Delegate<CWellCasingModel, CWellCasingModel_Delegate, CDummyNode, CDummyObserver, FALSE, FIXED_ITEM> TCasingObs;
      new TCasingObs(pWZIModel->CasingModel(), GetTreeCtrl(), FALSE, FIXED_ITEM, hModel, TVI_LAST, TRUE, FALSE);
    }

    if(pCasingModel)
    {
      AddRoot(*new TWellCasingRTCIEntryObserver((CWellCasingRTCIEntry&)*pCasingModel->GraphEntry(MD_WELLCASING_RTCI), GetTreeCtrl(), FALSE, FIXED_ITEM, hModel, TVI_LAST));
    }
    ...
    if(Document()->Model()->GraphEntry(MD_BASE_UPSCALING))
      AddRoot(*new CUpscalingBranch(*pObject, (CModelBase&)*Document()->Model()));
        ...
        {
            AddRoot(*(new TMeshRegionEntryObserver((CHexaMeshRegionEntry&)(*Document()->Model()->GraphEntry(MD_HEXA_MESH_ZONE)), GetTreeCtrl(), FALSE, DELETE_ITEM, hModel, TVI_LAST, TRUE, FALSE)));
        }

		if(Document()->Model()->GraphEntry(MD_BASE_ANALYSIS_POINT))
			AddRoot(*(new TAnalysisPointEntryObserver(*Document()->Model()->GraphEntry(MD_BASE_ANALYSIS_POINT), GetTreeCtrl(), FALSE, DELETE_ITEM, hModel, TVI_LAST, TRUE, FALSE)));
	
		if(Document()->Model()->GraphEntry(MD_BASE_XSECTION))
		{
			AddRoot(*(new TXSectionEntryObserver((CCrossSectionEntry&)(*Document()->Model()->GraphEntry(MD_BASE_XSECTION)), GetTreeCtrl(), FALSE, FIXED_ITEM, hModel, TVI_LAST, TRUE, FALSE)));
		}

    if (Document()->Model()->GraphEntry(MD_NEW_WELLPATH)) //wjrx mantis 3185
    {
      CNewWellPathEntry *wellPathEntry = static_cast<CNewWellPathEntry *>(Document()->Model()->GraphEntry(MD_NEW_WELLPATH));
      CGroupNodeEntry *groupNodeEntry = static_cast<CGroupNodeEntry *>(Document()->Model()->GraphEntry(MD_BASE_GROUPNODE));

      auto observer = new CNewWellPathEntryObserver(*wellPathEntry, GetTreeCtrl(), FALSE, FIXED_ITEM, hModel, TVI_LAST, TRUE, FALSE);
      AddRoot(*observer);
      ...
      if (groupNodeEntry)
      {
        ...
        if(Document()->Model()->GraphEntry(MD_BASE_ZOOMIN_MODEL))
            AddRoot(*(new TZoominModelEntryObserver((CZoominModelEntry&)(*Document()->Model()->GraphEntry(MD_BASE_ZOOMIN_MODEL)), GetTreeCtrl(), TRUE, FIXED_ITEM, hModel)));

        if(Document()->Model()->GraphEntry(MD_BASE_SCENE_NODE))
        {
          AddRoot(*new TOIVSceneEntryObserver((TOIVSceneBaseEntry&)(*Document()->Model()->GraphEntry(MD_BASE_SCENE_NODE)), GetTreeCtrl(), FALSE, FIXED_ITEM, TVI_ROOT, TVI_LAST));
        }
        ...
        AddRoot(* (new TMeshEntryObs(*Document()->Model()->GraphEntry(MD_BASE_MESH_RESULT), GetTreeCtrl(), doc,
            &CGeomecDoc::SelectComposite, &CGeomecDoc::SelectComposite, &CGeomecDoc::CanSelectComposite )));
        ...
        if (Document()->Model()->GraphEntry(MD_BASE_EXPORT_MACROS))
        {
            AddRoot(*(new TExportResultDataEntryObserver(
                (CExportResultDataEntry&)
                (*Document()->Model()->GraphEntry(MD_BASE_EXPORT_MACROS)),
                GetTreeCtrl(), FALSE, FIXED_ITEM, TVI_ROOT, TVI_LAST, TRUE, FALSE)));
        }
