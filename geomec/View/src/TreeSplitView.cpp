...
void CTreeSplitView::GetRowInfo(int row, int &cyCur, int &cyMin) const { m_wndSplitter.GetRowInfo(row, cyCur, cyMin); }
...
void CTreeSplitView::AssertValid() const { CView::AssertValid(); }

// ##ModelId=3B497D7A00BD
void CTreeSplitView::Dump(CDumpContext &dc) const { CView::Dump(dc); }
#endif //_DEBUG
...
int CTreeSplitView::OnCreate(LPCREATESTRUCT lpCreateStruct) {
  ...
  if (!m_wndSplitter.CreateStatic(this, 2, 1, WS_CHILD | WS_VISIBLE, AFX_IDW_PANE_FIRST + 8) ||
      !m_wndSplitter.CreateView(1, 0, RUNTIME_CLASS(CDataTreeView), CSize(0, 0), pContext) ||
      !m_wndSplitter.CreateView(0, 0, RUNTIME_CLASS(CModelTreeView), CSize(0, 0), pContext)) {
...
void CTreeSplitView::OnInitialUpdate() {
  ...
  int iTreeSplitViewSplitter = ISettings::instance()->getProfileInt(
      CMainFrame::s_profileHeading, CMainFrame::s_profileTreeSplitViewSplitter, rect.Height() * 0.75);
