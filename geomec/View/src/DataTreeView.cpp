...
QString CDataTreeView::CDataBranch::Text() const { return getStringTableEntry(IDS_TREE_DATA_STORAGE); }

unsigned int CDataTreeView::CDataBranch::Icon() const { return IDI_DATA_STORE; }
...
CDataTreeView::CPointSetBranch::CWellPathPointSetBranch::CWellPathPointSetBranch(CPointSetEntry &entry, CTreeCtrl &ctrl,
...
QString CDataTreeView::CPointSetBranch::CWellPathPointSetBranch::Text() const { return QObject::tr("Wells"); }
...
void CDataTreeView::CPointSetBranch::CWellPathPointSetBranch::AppendContextMenu(CContextMenuInvoker &invoker) {
  ...
  invoker.AddCommand(
      _T("Import deviated wellpath 3d &point set"),
      *(new CSingleCommandTemplate<CWellPathPointSetBranch>(
          *this, &CWellPathPointSetBranch::InsertDeviatedWellPathByPointSet, &CWellPathPointSetBranch::CanInsertNew)));
  invoker.AddCommand(COPY_MESSAGE_MAIN.toStdString(), *(new CSingleCommandTemplate<CWellPathPointSetBranch>(
                                                          *this, &CWellPathPointSetBranch::CopyAllWellPaths,
                                                          &CWellPathPointSetBranch::CanCopyAllWellPaths)));
  invoker.AddCommand(COPY_MESSAGE_ZOOM.toStdString(), *(new CSingleCommandTemplate<CWellPathPointSetBranch>(
                                                          *this, &CWellPathPointSetBranch::CopyRelevantWellPaths,
                                                          &CWellPathPointSetBranch::CanCopyRelevantWellPaths)));
}

CDataTreeView::CPointSetBranch::CPointSetSubBranch::CPointSetSubBranch(CPointSetBranch &parent_observer,
...
BOOL CDataTreeView::CPointSetBranch::CPointSetSubBranch::OnFilter(const child_type &ps) const {
  ...
  if (cast_ps && m_psType == IPointSet::POINTSET && cast_ps->pointSetType() == IPointSet::TIME_DEPTH &&
      m_dim == ps.Dimension())
    return TRUE;
...
CTreeNode *CDataTreeView::CPointSetBranch::CPointSetSubBranch::InsertChild(child_type &child) {
  ...
  if (&child != &model.Mesh()) {
    ...
    typedef CNodeObserver_Delegate<CRpnValueSet, CRpnValueSet_Delegate, CDummyNode, CDummyObserver, FALSE, FIXED_ITEM>
        TRpnValueSetObs;
    typedef CEnumerationBranch<CRpnValueSet, TRpnValueSetObs, TRUE, DELETE_ITEM> TFormulaEnumerator;
    new CPropertyEnumerator(*pObserver, _T("Properties"), IDI_PROPERTY);

    new TFormulaEnumerator(*pObserver, _T("Formulas"), IDI_RPN_FORMULAS);
  }
  return pTreeNode;
}
...
CDataTreeView::CPointSetBranch::CPointSetBranch(TPointSetEntry &node, CModelBase &model, CTreeCtrl &ctrl,
                                                HTREEITEM hParent, HTREEITEM hInsertAfter)
    : TPointSetBranchBase(node, ctrl, FALSE, FIXED_ITEM, hParent, hInsertAfter), m_model(model) {
  new CPointSetSubBranch(*this, CPointSet::DIM_3D, CPointSet::WELLPATH, _T("Wellpath"), IDI_POINTSET);

  new CPointSetSubBranch(*this, CPointSet::DIM_2D, CPointSet::POINTSET, _T("2D"), IDI_POINTSET);

  new CPointSetSubBranch(*this, CPointSet::DIM_3D, CPointSet::POINTSET, _T("3D"), IDI_POINTSET);
}
...
void CreateHullCallback() { GetGeomecDoc()->GetDataTreeView()->GetTreeCtrl().UpdateTree(); }

/* NOT OBSOLETE */ void CDataTreeView::CPointSetBranch::AppendContextMenu(CContextMenuInvoker &invoker) {
  static_cast<CPointSetEntry *>(GetGeomecDoc()->Model()->GraphEntry(MD_BASE_POINTSET))
      ->SetCreateHullCallback(CreateHullCallback);

  typedef CSingleCommandTemplate<CPointSetBranch> TPointSetBranchCommand;
  invoker.AddCommand(_T("&Import..."), *(new CWMCommandCommand(FemAppGetMainWnd()->m_hWnd, ID_FILE_IMPORT)));
  invoker.AddSeparator();
  invoker.AddCommand(_T("&Create point set ..."),
                     *(new TPointSetBranchCommand(*this, &CPointSetBranch::CreatePointSet)));
  typedef CSingleCommandTemplate<CPointSetEntry> TPointSetEntryCommand;
  invoker.AddCommand(
      _T("Generate all convex &hulls"),
      *new TPointSetEntryCommand(m_node, &CPointSetEntry::CreateAllHulls, &CPointSetEntry::CanCreateAllHulls));
  invoker.AddCommand(_T("Delete all unused pointsets"),
                     *new TPointSetEntryCommand(m_node, &CPointSetEntry::DeleteAllUnusedPointsets,
                                                &CPointSetEntry::CanDeleteAllUnusedPointsets));
}
...
void CDataTreeView::AssertValid() const { CTreeViewBase::AssertValid(); }

// ##ModelId=3B653D8E03DB
void CDataTreeView::Dump(CDumpContext &dc) const { CTreeViewBase::Dump(dc); }
#endif //_DEBUG
...
void CDataTreeView::OnInitialUpdate() {
  ...
  if (pModel) {
    ...
    if (pModel->GraphEntry(MD_BASE_POINTSET))
      AddRoot(*(new CPointSetBranch::CWellPathPointSetBranch((CPointSetEntry &)(*pModel->GraphEntry(MD_BASE_POINTSET)),
                                                             GetTreeCtrl(), pBranch->Handle())));

    // Insert surface root ...
    if (pModel->GraphEntry(MD_BASE_SURFACE))
      AddRoot(*(new TSurfaceEntryObserver((CSurfaceEntry &)(*pModel->GraphEntry(MD_BASE_SURFACE)), GetTreeCtrl(), TRUE,
                                          FIXED_ITEM, pBranch->Handle())));

    // Insert 2D / 3D root ...
    if (pModel->GraphEntry(MD_BASE_POINTSET))
      AddRoot(*(new CPointSetBranch((TPointSetEntry &)(*pModel->GraphEntry(MD_BASE_POINTSET)), *pModel, GetTreeCtrl(),
                                    pBranch->Handle())));

    // Insert materials
    if (pModel->GraphEntry(MD_ROCK_MATERIAL))
      AddRoot(*(new TMaterialEntryObserver((CMaterialEntry &)(*pModel->GraphEntry(MD_ROCK_MATERIAL)), GetTreeCtrl(),
                                           TRUE, FIXED_ITEM, pBranch->Handle())));

    if (pModel->GraphEntry(MD_BASE_INTERFACEMATERIAL))
      AddRoot(*(new TInterfaceMaterialEntryObserver(
          (CInterfaceMaterialEntry &)(*pModel->GraphEntry(MD_BASE_INTERFACEMATERIAL)), GetTreeCtrl(), TRUE, FIXED_ITEM,
          pBranch->Handle())));

    if (pModel->GraphEntry(MD_WELLCASING_STEELMATERIAL_ENTRY))
      AddRoot(*(new TWellCasingSteelMaterialEntryObserver(
          (CWellCasingSteelMaterialEntry &)(*pModel->GraphEntry(MD_WELLCASING_STEELMATERIAL_ENTRY)), GetTreeCtrl(),
          TRUE, FIXED_ITEM, pBranch->Handle())));
    /*
        ...
        if(pApp->KeyFile().DCasintUnlocked())
        {
          if(pModel->GraphEntry(MD_CASING_GRADE))
            AddRoot(*(new TMaterialEntryObserver((CMaterialEntry&)(*pModel->GraphEntry(MD_CASING_GRADE)), GetTreeCtrl(),
       TRUE, FIXED_ITEM, pBranch->Handle())));
        }
    */
  }
