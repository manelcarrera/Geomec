...
COpenGLSplitView::COpenGLSplitView() : m_bRecursionLock(false), m_originalCx(-1) {}
...
void COpenGLSplitView::AssertValid() const { CView::AssertValid(); }

// ##ModelId=3B6549E102E6
void COpenGLSplitView::Dump(CDumpContext &dc) const { CView::Dump(dc); }
#endif //_DEBUG
...
int COpenGLSplitView::OnCreate(LPCREATESTRUCT lpCreateStruct) {
  ...
  if (!m_wndSplitter.CreateStatic(this, 1, 2, WS_CHILD | WS_VISIBLE, AFX_IDW_PANE_FIRST + 8) ||
      !m_wndSplitter.CreateView(0, 1, RUNTIME_CLASS(CModelView), CSize(0, 0), pContext) ||
      !m_wndSplitter.CreateView(0, 0, RUNTIME_CLASS(CLegendView), CSize(0, 0), pContext)) {
...
void COpenGLSplitView::OnInitialUpdate() {
  ...
  int iOpenGLSplitViewSplitter = ISettings::instance()->getProfileInt(
      CMainFrame::s_profileHeading, CMainFrame::s_profileOpenGLSplitViewSplitter, rect.Width() * 0.2);

  m_wndSplitter.SetColumnInfo(0, iOpenGLSplitViewSplitter, 0);
  m_wndSplitter.RecalcLayout();
}

BOOL COpenGLSplitView::OnEraseBkgnd(CDC *) { return TRUE; }
