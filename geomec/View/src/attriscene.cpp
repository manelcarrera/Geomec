...
{
  CString strName;
  BOOL bDisplayGnomon, bDisplayBoundary, bDisplayPerspective, bAllowDynamicRotate, bFill, bTrackElement;//, bDeformation;
  double dPointSize;
  ...
  if(!pDX->m_bSaveAndValidate) // (1) SHOW
  {
    ...
    std::vector<int> control_id_v = { IDC_CK_AXIS, IDC_CK_BOUNDARY, IDC_CK_PERSPECTIVE, IDC_CK_ROTATE, IDC_CK_SOLID_FILL, IDC_ED_POINTSIZE, IDC_ED_DRAGGERTIMEOUT };
    ...
    {
      std::vector<int> control_id_v2 = { IDC_DEF_CHECK, IDC_DEF_COMBO, IDC_DEF_SCALE_GEN_LABEL, IDC_DEF_SCALE_GEN, IDC_DEF_SCALE_DEF_LABEL, IDC_DEF_SCALE_DEF };
  ...
  //if( is_model_in_scope )
  {
    ...
    if(!pDX->m_bSaveAndValidate)
      fill_stages_combo( m_combo_deformation, deformation ); // This must be performed at the end, after linking control and var
  }
