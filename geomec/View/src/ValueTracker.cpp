...
ValueTracker::ValueTracker(OIDIMeshNodeManager * meshNodeManager, CrossSectionManipulator * dragger) :
  ...
  m_draggerSwitch(0),
  m_firstTime(true),
  m_cellMarkerSwitch(0),
  m_validCellMarkerSwitch(0),
  m_numCellNodes((size_t) -1),
  m_vertexProperty(0),
  m_cellMarkerMaterial(0),
  m_cellMarkerSet(0),
  m_cellFacetSetCoordinates(0),
  m_cellFacetSet(0),
  m_volumeCellFacetSetInitialized(0),
  m_surfaceCellFacetSetInitialized(0)
{
...
bool ValueTracker::IsDisplayed()
{
  return m_switch && m_switch->whichChild.getValue() == SO_SWITCH_ALL;
}

void ValueTracker::motionCallback(GMoMeshPointProbe * probe, const MiVolumeMeshUnstructured * mesh, const std::string & meshName, size_t cellId, const MeXScalardSetI& scalars, const MeXVec3dSetI& vectors)
{
  if (!probe) return;
  if (m_switch->whichChild.getValue() == SO_SWITCH_NONE) return;
...
double ValueTracker::computeDistanceClosestNode( const MiMeshUnstructuredI * mesh, const MbVec3d & coordinates, size_t cellId, size_t & cniClosest )
{
  double distanceClosest = DBL_MAX;
  const MiCell * cell = mesh->getTopology().getCell(cellId);
  if (!cell) return distanceClosest;
...
void ValueTracker::invalidateCellMarkers()
{
  m_validCellMarkerSwitch->whichChild = SO_SWITCH_NONE;
}

void ValueTracker::computeCellMarkerSet( const MiMeshUnstructuredI * mesh, const MbVec3d & coordinates, size_t cellId, size_t &meshNodeIndexClosest, MbVec3d &meshNodeCoordinatesClosest )
{
  m_validCellMarkerSwitch->whichChild = SO_SWITCH_NONE;
...
void ValueTracker::updateValueTracker( const MiMeshUnstructuredI * mesh, const std::string & meshName, const MbVec3d & coordinates, size_t cellId, int scalarSetId, int vectorSetId, int tensorSetId )
{
  ...
  if (cellId != (size_t) -1 && ((scalarDataSet && scalarDataSet->getBinding() == MiDataSet::PER_NODE) || (vectorDataSet && vectorDataSet->getBinding() == MiDataSet::PER_NODE)))
  {
    const MiCell * cell = mesh->getTopology().getCell(cellId);
    ...
    if (scalarDataSet) m_draggerTextScene->setCellPropertyValue (scalarValue);
    if (vectorDataSet) m_draggerTextScene->setCellPropertyVector (vectorValue);
  }
}
...
void ValueTracker::computeCellFacetSet( const MiVolumeMeshUnstructured * mesh, size_t cellId )
{
  ...
  for (int n = 0; n < facetNodeIndices.size(); ++n)
  {
    ...
    m_cellFacetSetCoordinates->point.set1Value(n, meshNodeCoordinates[0], meshNodeCoordinates[1], meshNodeCoordinates[2]);
  }
}
...
void ValueTracker::SetTextColor( float * rgb )
{
  m_draggerTextScene->SetTextColor(rgb);
}

void ValueTracker::GetTextColor( float * rgb )
{
  m_draggerTextScene->GetTextColor(rgb);
}

void ValueTracker::updatePropertyName()
{
  m_draggerTextScene->setPropertyName("Value");
}
...
void ValueTracker::TrackElement(bool track)
{
  m_cellMarkerSwitch->whichChild = track ? SO_SWITCH_ALL : SO_SWITCH_NONE;
}

bool ValueTracker::TrackElement(void)
{
  return m_cellMarkerSwitch->whichChild.getValue() == SO_SWITCH_ALL;
}



void ValueTracker::showDragger(bool show)
{
  m_draggerSwitch->whichChild = show ? SO_SWITCH_ALL : SO_SWITCH_NONE;
}

CrossSectionManipulator * ValueTracker::getDragger()
{
  return m_dragger;
}

void ValueTracker::setTextSceneValues( size_t nodeIndex, int scalarSetId, int vectorSetId, int tensorSetId, size_t cellId )
{
  const MiDataSetI<double> * scalarDataset = m_meshNodeManager->getScalarDataSet(scalarSetId);
...
void ValueTracker::GetCellNodeWeights( const MiCell * cell, const MiMeshUnstructuredI * mesh, const MbVec3d & coordinates, std::vector<double> & weights )
{
