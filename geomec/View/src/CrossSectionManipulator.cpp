...
bool intersect(const SbBox3f &box, const SbPlane &plane) {
  ...
  SbVec3f corners[] = {
      SbVec3f(minVec[0], minVec[1], minVec[2]), SbVec3f(minVec[0], minVec[1], maxVec[2]),
      SbVec3f(minVec[0], maxVec[1], minVec[2]), SbVec3f(minVec[0], maxVec[1], maxVec[2]),

      SbVec3f(maxVec[0], minVec[1], minVec[2]), SbVec3f(maxVec[0], minVec[1], maxVec[2]),
      SbVec3f(maxVec[0], maxVec[1], minVec[2]), SbVec3f(maxVec[0], maxVec[1], maxVec[2]),
  };
...
CrossSectionManipulator::CrossSectionManipulator()
    : m_rotator(0), m_separator(0), m_confined(false), m_orientationConstraint(CCrossSection::ORIENTATION_ANY),
      m_meshNodeManager(0) {
...
void CrossSectionManipulator::initClass() { SO_NODE_INIT_CLASS(CrossSectionManipulator, SoJackDragger, "JackDragger"); }

SbVec3f CrossSectionManipulator::getDefaultBasePoint() { return SbVec3f(0.0f, 0.0f, 0.0f); }

SbVec3f CrossSectionManipulator::getDefaultNormal() { return SbVec3f(0.0f, 1.0f, 0.0f); }

SbPlane CrossSectionManipulator::getDefaultPlane() { return SbPlane(getDefaultNormal(), getDefaultBasePoint()); }

void CrossSectionManipulator::limitRotation(CCrossSection::OrientationType orientation) {
  m_orientationConstraint = orientation;
  if (orientation == CCrossSection::ORIENTATION_DEPTH || orientation == CCrossSection::ORIENTATION_CURRENT ||
      orientation == CCrossSection::ORIENTATION_EASTING || orientation == CCrossSection::ORIENTATION_NORTHING) {
...
void CrossSectionManipulator::removeConstraints() { limitRotation(CCrossSection::ORIENTATION_ANY); }

void CrossSectionManipulator::setTranslation(const SbVec3f &position) { translation = m_savedTranslation = position; }
...
void CrossSectionManipulator::confineTranslation() {
  ...
  if (isConfined()) {
    if (!m_meshNodeManager->DisplayedVolumesContains(translation.getValue()) &&
        !m_meshNodeManager->DisplayedVolumesContains(m_savedTranslation)) {
...
bool CrossSectionManipulator::isConfined() const { return m_confined; }
...
void CrossSectionManipulator::updateDraggerPosition() { onDraggerMotion(this); }
