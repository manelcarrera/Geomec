...
public:
  ...
  virtual void motionCallback(GMoMeshPointProbe *probe, const MiVolumeMeshUnstructured *mesh,
  ...
  double computeDistanceClosestNode(const MiMeshUnstructuredI *mesh, const MbVec3d &coordinates, size_t cellId,
                                    size_t &cellNodeId);
  void invalidateCellMarkers();
  void computeCellMarkerSet(const MiMeshUnstructuredI *mesh, const MbVec3d &coordinates, size_t cellId,
                            size_t &meshNodeIndexClosest, MbVec3d &meshNodeCoordinatesClosest);
  void updateValueTracker(const MiMeshUnstructuredI *mesh, const std::string &meshName, const MbVec3d &coordinates,
                          size_t cellId, int scalarSetId, int vectorSetId, int tensorSetId);

  void GetCellNodeWeights(const MiCell *cell, const MiMeshUnstructuredI *mesh, const MbVec3d &coordinates,
