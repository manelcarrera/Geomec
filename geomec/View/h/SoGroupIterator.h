...
public:
  ...
  SoGroupIterator(const SoGroupIterator &other) : m_group(other.m_group), m_index(other.m_index) {}
  ...
  auto operator->() -> value_type { return operator*(); }
  ...
  bool operator==(const SoGroupIterator &that) {
    return m_group == that.m_group &&
           (m_index == that.m_index || (m_index >= size(m_group) && that.m_index >= size(m_group)));
  }
  ...
  bool operator!=(const SoGroupIterator &that) { return !(this->operator==(that)); }
  ...
  SoGroupIterator operator-(difference_type n) const { return operator+(-n); }

  auto operator-(const SoGroupIterator &other) -> difference_type const { return m_index - other.m_index; }

  bool operator<(const SoGroupIterator &other) const { return m_index < other.m_index; }
  ...
  SoGroupIterator &operator-=(difference_type n) { return operator+=(-n); }
  ...
  auto operator[](difference_type n) -> value_type { return *operator+(n); }
...
inline SoGroupIterator begin(SoGroup &group) { return SoGroupIterator(group, 0); }

inline SoGroupIterator end(SoGroup &group) { return SoGroupIterator(group, INT_MAX); }
...
inline bool empty(SoGroup *group) { return begin(group) == end(group); }
...
inline int size(const SoGroup &group) { return group.getNumChildren(); }
