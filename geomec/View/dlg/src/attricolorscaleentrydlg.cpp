...
{
  CColorScaleEntry_Delegate *m_entry_Delegate =
    dynamic_cast <CColorScaleEntry_Delegate*> (m_entry.getDelegate());
...
{
  // wjrx mantis 2575
  if (  m_entry.ColorScaleType() == CColorScaleEntry::LOCAL
     && m_entry.LocalColorGradient() != NULL
     )
  {
  CColorGradient_Delegate* colorGradient_Delegate =
      new CColorGradient_Delegate(m_entry.LocalColorGradient());
...
void CAttriColorScaleEntryDlg::OnCopyGlobalScale()
{
  if (m_cbGlobal.GetCurSel() >= 0)
  {
  CColorGradient* globalColorScale = reinterpret_cast <CColorGradient*> (
      m_cbGlobal.GetItemData(m_cbGlobal.GetCurSel()));
  CColorGradient* globalColorScaleCopy =
      new CColorGradient(IDS_NEW_COLOR_GRADIENT,  // need a stringtable entry
    globalColorScale->Model(), globalColorScale->MinMaxValue().first,
    globalColorScale->MinMaxValue().second, globalColorScale->MinColor(),
    globalColorScale->MaxColor(), true);
  ...
  TColorGradient_Delegate globalColorScaleCopy_Delegate =
      TColorGradient_Delegate(
    new CColorGradient_Delegate(globalColorScaleCopy));
