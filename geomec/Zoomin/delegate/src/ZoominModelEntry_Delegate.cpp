...
CZoominModelEntry_Delegate::CZoominModelEntry_Delegate(CZoominModelEntry *zoominModelEntry)
    : CGraphEntryTemp_Delegate<CZoominModelPlaceHolder>(zoominModelEntry), m_zoominModelEntry(zoominModelEntry) {}
...
void CZoominModelEntry_Delegate::Import() {
  ...
  if (dlg.DoModal() == IDOK) {
    ...
    while (pos) {
      ...
      CModelBase *pChildModel = IModelLifetimeFacade::LoadChild(CModelBase::CModelLoadSaveDefault(&model),
                                                                LPCSTR(strFileName), model.Logger());
      ...
      if (pChildModel) {
        ...
        for (std::set<unsigned int>::const_iterator it = CModelBase::DataStorageEntryTypes().begin();
             it != CModelBase::DataStorageEntryTypes().end(); ++it) {
