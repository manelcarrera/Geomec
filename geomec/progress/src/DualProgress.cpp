...
CDualProgress::CDualProgress(int jobs, const QString &primaryTitle, bool cancel, CWnd *parent)
    : CProgressBase(), m_progressDlg(jobs, primaryTitle.toStdString().c_str(), cancel, parent), m_secondaryTitle(),
      m_totalSteps(0), m_currentSteps(0) {
  m_progressDlg.StartDialog();
}

CDualProgress::~CDualProgress() { m_progressDlg.DestroyWindow(); }
