...
CGeoProgress::CGeoProgress(CWnd *parent, const QString &sTitle, bool bCancel)
    : CProgressBase(), m_bCancel(bCancel), m_bDeleteDlg(true), m_nCurStep(0), m_nSteps(0), m_bBlockStatusMessage(false),
      m_pDlg(0), m_strTitle(sTitle), m_parent(parent) {}

CGeoProgress::CGeoProgress(CWnd *parent, unsigned int uTitle, bool bCancel)
    : CProgressBase(), m_bCancel(bCancel), m_bDeleteDlg(true), m_nCurStep(0), m_bBlockStatusMessage(false), m_nSteps(0),
      m_pDlg(0), m_parent(parent) {
  m_strTitle = getStringTableEntry(uTitle);
}

CGeoProgress::CGeoProgress(CProgressDlg_MFC &dlg, const QString &sTitle)
    : CProgressBase(), m_bCancel(false), // Not used
      m_bDeleteDlg(false), m_nCurStep(0), m_nSteps(0), m_bBlockStatusMessage(false), m_pDlg(&dlg), m_strTitle(sTitle),
      m_parent(FemAppGetMainWnd()) {}

CGeoProgress::CGeoProgress(CProgressDlg_MFC &dlg, unsigned int uTitle)
    : CProgressBase(), m_bCancel(false), // Not used
      m_bDeleteDlg(false), m_nCurStep(0), m_nSteps(0), m_bBlockStatusMessage(false), m_pDlg(&dlg),
      m_parent(FemAppGetMainWnd()) {
  m_strTitle = getStringTableEntry(uTitle);
}
...
void CGeoProgress::BlockStatusMessage(bool bBlockStatusMessage) { m_bBlockStatusMessage = bBlockStatusMessage; }
...
void CGeoProgress::Enable(bool bEnable) { m_pDlg->ShowWindow(SW_HIDE); }