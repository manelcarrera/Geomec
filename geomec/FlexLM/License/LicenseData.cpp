...
LicenseData::LicenseData(const char *p_csAppName, const char *p_csAppVersion)
    : m_bInitialized(false), m_eLicenseType(NO_LICENSE), m_csAppVersion(_strdup(p_csAppVersion)),
      m_csAppName(_strdup(p_csAppName)), m_nLicenseCounter(0) {
...
void LicenseData::SetIntialized(bool p_bInitialized) { m_bInitialized = p_bInitialized; }

bool LicenseData::IsInitialized() { return m_bInitialized; }
...
void LicenseData::BorrowLicense(__time64_t p_ctEndDate) {
  ...
  if (!(lv_nRes == 0 && CheckOut())) {
    ...
    sprintf(lv_csMsg, "License was not borrowed. Check that the selected period does not exceed %d days.",
            LICENSE_MAXIMUM_BORROW_DAYS);
    ...
    if (CheckOut((char *)GetFullLicenseName(), lv_sVersionNumber)) {
      ...
      if (CheckOut((char *)GetDemoLicenseName(), lv_sVersionNumber)) {
        ...
        } else // if (!p_bLogginOnly)
        {
          ...
          //
          lv_bChecking =
              0 != m_pReporter->ShowRetryBox(
                       "No available full or demo licenses found for this application. Please contact local support.",
                       m_csAppName);
        }
      }
    }
