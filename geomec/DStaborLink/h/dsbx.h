...
enum dsbHardeningType { dsb_Hardening = 0, dsb_Softening = 1, dsb_Parabolic = 2, dsb_Exponential = 3, dsb_None = 4 };

enum dsbInSituStressDefinition { dsb_VerticalStress = 0, dsb_GeneralStress = 1 };

enum dsbIterationScheme { dsb_Constant = 0, dsb_Linear = 1, dsb_Regular = 2, dsb_Modified = 3 };

enum dsbTypeCriteria { dsb_Energy = 0, dsb_Force = 1, dsb_Displacement = 2 };

enum dsbAnalysisType { dsb_MudWeight = 0, dsb_Openhole = 1 };
...
public:
  CLSID const &GetClsid() {
    static CLSID const clsid = {0x4e2c43ce, 0x38f8, 0x11d3, {0xb3, 0x8e, 0x0, 0xa0, 0xc9, 0x41, 0xc6, 0xb}};
    return clsid;
  }
...
public:
  ...
  BOOL RadialResults(long Component, long Step, double Angle, long NrOfPoints, VARIANT *XCoord, VARIANT *YCoord,
                     VARIANT *Results);
  BOOL TangentialResults(long Component, long Step, double Radius, long NrOfPoints, VARIANT *XCoord, VARIANT *YCoord,
                         VARIANT *Results);
  BOOL InquireTauRatio(double *FailureOnset, BOOL *FailureOnsetFound, double *FailureOffset, BOOL *FailureOffsetFound,
                       double *Extreme, BOOL *ExtremeFound, double *TensileFailure, BOOL *TensileFailureFound,
                       double *p1, double *pn, short *Inquirecase);
