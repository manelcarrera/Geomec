...
template<class T, TTimeLapseType TLT = Pressure>
...
public:
  CTimeLapseListObject(const CModelBase &model, CListCtrl& ctrl, 
    CDepletionStage *t1, CDepletionStage *t2, T *parent)
  :IListObject(ctrl),
   m_Model(model),
   m_t1(t1),
   m_t2(t2),
...
template<class T, TTimeLapseType TLT>
