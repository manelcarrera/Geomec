...
public:
  bool appendMetresClause(std::string &newMessage, const std::vector<std::string> &message) {
    QRegExp matchCoordinate(REGULAR_EXPRESSION_DOUBLE);
    bool coordinateMatched = false;

    for (std::vector<std::string>::const_iterator line = message.begin(); !coordinateMatched && (line != message.end());
         ++line) {
      coordinateMatched = coordinateMatched || (matchCoordinate.indexIn((*line).c_str()) != -1);
    }
  ...
  std::vector<QString> reorientateCoordinatesNED2END(std::string &message) {
    ...
    while ((coordinateOffset = matchCoordinate.indexIn(message.c_str(), coordinateOffset)) != -1) {
      QString coordinate = message.substr(coordinateOffset, matchCoordinate.matchedLength()).c_str();
      QRegExp matchDouble(REGULAR_EXPRESSION_DOUBLE);
      QString NED[DIMENSIONS];
      size_t dimension = NORTHING;
      int doubleOffset = 0;

      while ((dimension != DIMENSIONS) && ((doubleOffset = matchDouble.indexIn(coordinate, doubleOffset)) != -1)) {
        NED[dimension++] = coordinate.mid(doubleOffset, matchDouble.matchedLength());
        doubleOffset += matchDouble.matchedLength();
      }

      assert(dimension == DIMENSIONS);

      newCoordinates.push_back(COORDINATE_PREFIX + NED[EASTING] + COORDINATE_INFIX + NED[NORTHING] + COORDINATE_INFIX +
                               NED[DEPTH] + COORDINATE_POSTFIX);

      assert(newCoordinates.back().length() == matchCoordinate.matchedLength());

      message.replace(coordinateOffset, matchCoordinate.matchedLength(), newCoordinates.back().toStdString());
