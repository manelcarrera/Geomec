...
public:
  ...
  virtual double Value(const geo::IPoint &pt, const int nComponent = 0, const UNIT unit = SI_UNIT) const;
...
public:
  CImportTemp(CPointSet &point_set) : CDistributedQuantity(point_set, COMPONENTS) {
  ...
  virtual const IQuantityEntry &QuantityEntry() const { return *m_pEntry; }

  virtual UINT IconId() const { return ICON_ID; }

  virtual const COpenGLNode *ProjectOn() const { return &PointSet(); }
...
typedef CImportTemp<MD_QUANTITY_PRESSURE,    // Identifier to register globally the variable
                    ...
                    1.000000000>
    TPressure; // Factor to calculate to SI to FIELD
...
typedef CImportTemp<MD_QUANTITY_STRESS,         // Identifier to register globally the variable
                    ...
                    1.000000000>
    TStressTensor; // Factor to calculate to SI to FIELD
...
typedef CImportTemp<MD_QUANTITY_STRESS,                    // Identifier to register globally the variable
                    ...
                    IDS_UNIT_FIELD_STRESS,                 // Resource identifier of the field unit name
                    1.000000000>
    TMinimalPrincipalStress; // Factor to calculate to SI to FIELD
...
typedef CImportTemp<MD_QUANTITY_STRESS,                    // Identifier to register globally the variable
                    ...
                    IDS_UNIT_FIELD_STRESS,                 // Resource identifier of the field unit name
                    1.000000000>
    TMaximalPrincipalStress; // Factor to calculate to SI to FIELD
...
typedef CImportTemp<MD_QUANTITY_STRESS,           // Identifier to register globally the variable
                    ...
                    IDS_UNIT_FIELD_STRESS,        // Resource identifier of the field unit name
                    1.000000000>
    TVerticalStress; // Factor to calculate to SI to FIELD
...
typedef CImportTemp<MD_QUANTITY_YOUNGS_MODULUS,  // Identifier to register globally the variable
                    ...
                    1.000000000>
    TYoungsModulus; // Factor to calculate to SI to FIELD
...
typedef CImportTemp<MD_QUANTITY_POISSONS_RATIO,  // Identifier to register globally the variable
                    ...
                    IDS_UNIT_DIMENSIONLESS,      // Resource identifier of the field unit name
                    1.000000000>
    TPoissonsRatio; // Factor to calculate to SI to FIELD
...
typedef CImportTemp<MD_QUANTITY_COHESION,    // Identifier to register globally the variable
                    ...
                    1.000000000>
    TCohesion; // Factor to calculate to SI to FIELD
...
typedef CImportTemp<MD_QUANTITY_FRICTION_ANGLE,  // Identifier to register globally the variable
                    ...
                    1.000000000>
    TFrictionAngle; // Factor to calculate to SI to FIELD
...
typedef CImportTemp<MD_QUANTITY_POROSITY,   // Identifier to register globally the variable
                    ...
                    IDS_UNIT_DIMENSIONLESS, // Resource identifier of the field unit name
                    1.000000000>
    TPorosity; // Factor to calculate to SI to FIELD
...
typedef CImportTemp<MD_QUANTITY_BULK_DENSITY,        // Identifier to register globally the variable
                    ...
                    1.000000000>
    TBulkDensity; // Factor to calculate to SI to FIELD
...
typedef CImportTemp<MD_QUANTITY_COMPRESSIONAL_VELOCITY,  // Identifier to register globally the variable
                    ...
                    1.000000000>
    TCompressionalVelocity; // Factor to calculate to SI to FIELD
...
typedef CImportTemp<MD_QUANTITY_SHEAR_VELOCITY,  // Identifier to register globally the variable
                    ...
                    IDS_UNIT_FIELD_VELOCITY,     // Resource identifier of the field unit name
                    1.000000000>
    TShearVelocity; // Factor to calculate to SI to FIELD
...
typedef CImportTemp<MD_QUANTITY_PERMEABILITY,           // Identifier to register globally the variable
                    ...
                    IDS_UNIT_DIMENSIONLESS,             // Resource identifier of the field unit name
                    1.000000000>
    TVerticalPermeability; // Factor to calculate to SI to FIELD
...
typedef CImportTemp<MD_QUANTITY_PERMEABILITY,             // Identifier to register globally the variable
                    ...
                    IDS_UNIT_DIMENSIONLESS,               // Resource identifier of the field unit name
                    1.000000000>
    THorizontalPermeability; // Factor to calculate to SI to FIELD
...
template <class T> class CQuantityBuilderTemp : public CQuantityBuilder {
public:
