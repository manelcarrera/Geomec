...
void print_installations() {
  ...
  for (int i = 0; i < list.size(); ++i)
    printer->debug("dir: %s", list[i].toStdString().c_str());
}
...
namespace settings_ {
void init() { CSettingsIni::instance()->init(); }
} // namespace settings_
...
CGeomecApp::CGeomecApp() : m_pQapp(0), m_pKeyFile(0) {
  ...
  //
  // NOTE: 2020-07-22 mcr: why not just make url static instead of per instance? was it thought to use a file epr
  // module?
  //
...
BOOL CGeomecApp::InitInstance() {
  // No need to do these twice: either here or later. But --version for Geomec.exe to console is SLOOOOOW, so probably
  // don't use it
  ...
  pDocTemplate = new CSingleDocTemplate(IDR_MAINFRAME, RUNTIME_CLASS(CGeomecDoc),
...
// results validity.");
  ...
  if (!test.enabled) // FIXME
  {
    ...
    pDoc->changelog_dlg(gm::helper::changelog((LPCTSTR)strProductName));
  }

  GetDoc().set_dsa_icon(!_g->dsa_available() ? DSA_Icon::Empty : _g->dsa() ? DSA_Icon::_2P : DSA_Icon::_1P);
...
CGeomecApp *GetGeomecApp() { return (CGeomecApp *)AfxGetApp(); }
...
CTempPath *CGeomecApp::TempPath() { return m_TempPath; }

const CKeyFile &CGeomecApp::KeyFile() const { return *m_pKeyFile; }
