...
CNonlinParams::CNonlinParams(CModelBase &model, CWnd *pParent /*=NULL*/)
    : CDialog(CNonlinParams::IDD, pParent), m_model(model), m_bApplyAll(FALSE) {
...
void CNonlinParams::DoDataExchange(CDataExchange *pDX) {
  ...
  if (pDX->m_bSaveAndValidate) {
    ...
    // wjrx mantis 2557 end
    {
      ...
      CDepletionStage *pStage = m_vcDepletionStage[m_iPreviousSelection].second;
      pStage->SetNonLinParams((nManualStepSizes == 0), dMaxAutoStepSize, nNrOfSteps, dStepScalingFactor,
                              dConvergenceNormFactor, nMaxNumIter, ct, is);
    }
  }
}
  ...
  if (m_bApplyAll) {
    ...
    for (size_t i = 0; i < m_vcDepletionStage.size(); i++) {
      ...
      /*      if(pStage->Initial())
          ...
          pStage->SetNonLinParams(bAutoStepSizes, dMaxAutoStepSize, nSteps, dIniStepScalingFactor,
         dConvergenceNormFactor, nMaxNumIter, ict, iis);
            }
            else
            {*/
      pStage->SetNonLinParams(bAutoStepSizes, dMaxAutoStepSize, nNrOfSteps, dStepScalingFactor, dConvergenceNormFactor,
                              nMaxNumIter, ct, is);
      //      }
    }
  }
...
void CNonlinParams::OnAllowLinearCalc() { UpdateData(TRUE); }
