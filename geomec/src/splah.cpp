...
BOOL CSplashWnd::Create(unsigned int nIDRes, unsigned int duration, WORD flags) {
  ...
  m_duration = duration;
  m_flags = flags;
  return CreateEx(0, AfxRegisterWndClass(0, AfxGetApp()->LoadStandardCursor(IDC_ARROW)), NULL, WS_POPUP | WS_VISIBLE, 0,
                  0, sz.cx, sz.cy,
                  NULL, // parent wnd
                  NULL);
}
...
//
void CSplashWnd::OnClose() { DestroyWindow(); }
...
//
void CSplashWnd::PostNcDestroy() { delete this; }
...
void CSplashWnd::OnPaint() {
  ...
  CFont font;
  CFont *oldfont;
  font.CreateFont(-20, 0, 0, 0, FW_DONTCARE, 0, 0, 0, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS,
  ...
  font.DeleteObject();
  font.CreateFont(-14, 0, 0, 0, FW_DONTCARE, 0, 0, 0, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS,
                  DEFAULT_QUALITY, DEFAULT_PITCH | FF_ROMAN, "Arial");
  oldfont = dc.SelectObject(&font);
  ...
  dc.SelectObject(oldfont);
  font.DeleteObject();
  font.CreateFont(-10, 0, 0, 0, FW_DONTCARE, 0, 0, 0, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS,
                  DEFAULT_QUALITY, DEFAULT_PITCH | FF_ROMAN, "Arial");
  oldfont = dc.SelectObject(&font);
...
BOOL CSplashWnd::PreTranslateMessage(MSG *pMsg) {
  if (m_flags & CSplash::KillOnClick) {
    ...
    if (msg == WM_KEYDOWN || msg == WM_SYSKEYDOWN || msg == WM_LBUTTONDOWN || msg == WM_RBUTTONDOWN ||
        msg == WM_MBUTTONDOWN) {
      CWinApp *pApp = AfxGetApp();
      CWnd *pMainWnd = pApp->m_pMainWnd;
      if (IsWindow(pMainWnd->GetSafeHwnd())) {
