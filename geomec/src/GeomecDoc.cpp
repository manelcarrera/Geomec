...
{
	/*
	//_m()->msg("The heat flow analysis will invalidate the results from other analysis types. Are you sure you want to continue?", MB_YESNO) == IDNO)
	void globalMessageW_special( char const * msg, int code, char const * caller_name, int line )
	{
...
*/
	...
	static const QString TEXT_OLD_MESHER_DLG_TEXT	= QString(	"Upgrading to the new CM2 mesher can be performed now.<br><br> \
	...
	{ 
		...
		//btn_h->disconnect();
		//connect( btn_h, &QAbstractButton::clicked, MeshConverterTreeView::instance(), [=](){ AfxGetApp()->GetMainWnd()->PostMessage( WM_COMMAND, ID_HELP_FINDER, 0 ); } );
...
QString version_APP()
{
	return version( QString::fromStdString( util::GetAppFileName() ) );
}

QString version_APP_OLD()
{
	return version( (LPCTSTR)executable() );
}

QString version( eVersion type_ )
{
	return type_ == Folder ? 
		version_FOLDER() :
		version_APP();
...
, m_bStartUp(true)
, m_app_version()
, m_versionManager(m_app_version)
//
...
, m_pDataTreeView(0)
, m_pModelView(0)
, m_pLegendModelView(0)
, m_bChangeMode(false)
, m_nLastCGStatus(-1)
, m_bRunning( false )
, m_dsa_icon_id( DSA_Icon::Empty )
, m_changelog_dlg(nullptr)
{
...
void CGeomecDoc::AssertValid() const
{
	CDocument::AssertValid();
}

//##ModelId=3BC55D6300B5
void CGeomecDoc::Dump(CDumpContext& dc) const
{
	CDocument::Dump(dc);
}
#endif //_DEBUG
...
BOOL CGeomecDoc::OnImportFile(const CString& strFileName, IProgressBase& dlg, CGraphNode::TNodeVec& created_objects)
{
	...
	if( (strFileName.Right(4).CompareNoCase(".txt") == 0) ||
		(strFileName.Right(4).CompareNoCase(".dat") == 0) ||
  ...
  if(strFileName.Right(3).CompareNoCase(".so") == 0 ||
     strFileName.Right(3).CompareNoCase(".ts") == 0 ||
     ...
     strFileName.Right(3).CompareNoCase(".vs") == 0) //wjrx mantis 2896
  {
    ...
    if (NrOfJobs > 0)
    {
      std::auto_ptr <IProgressBase> progdlg((_g->prog()->create( eProgress::Dual, "Processing Gocad import", false, NrOfJobs )));
...
BOOL CGeomecDoc::OnOpenDocument(LPCTSTR lpszPathName)
{
  ...
  MeshConverterDlg::instance()->hide();
	...
	{
#ifdef _WIN64
    _m()->msg("The 64 bits version of GEOMEC cannot import .gm3 files anymore. Use the 32 bits version to open this file.");
    return FALSE;
#else
...
// disable the popup, as it's irritating; TODO: put in changelog dialog that this functionality exists
  ...
  {
    CGoCadModel *pModel = static_cast<CGoCadModel *>(IModelLifetimeFacade::NewMain(GOCAD_MODEL, m_Logger, m_versionManager));
...
BOOL CGeomecDoc::SaveModel(LPCTSTR lpszPathName, CGUIBase* pGui, bool bSwitch)
{
  ...
  IModelLifetimeFacade::SaveMain(CGUIBase::CMainModelLoadSave(m_app_version, pGui), model, (const char *)pathName, bSwitch);
...
/// \brief import file
void CGeomecDoc::_OnImport
(unsigned int filterStringId ///< identifier for string containing file extensions .xyz
) 
{
	CString sFilter;
    ...
    try { // we catch cancel at some places in OnImportFile, but we also need to catch it here in case the user cancels outside those try-catch blocks
...
void CGeomecDoc::OnImport() 
{
	_OnImport( IDS_IMPORT_FILTER);
}

/// \brief Import file with preference for GoCad files
void CGeomecDoc::OnImportSurface() 
{
	_OnImport( IDS_IMPORT_GOCAD_FILTER);
}

//##ModelId=3BC55D6300F4
void CGeomecDoc::OnUpdateImport(CCmdUI* pCmdUI) 
{
	pCmdUI->Enable(Model() != 0);
}
...
void CGeomecDoc::OnItemAttributes() 
{
  CGraphNode_Delegate* graphNode_Delegate =
    new CGraphNode_Delegate(&ContextMenuNode());
...
{
	bool res = false;
	COpenInventorSceneNode* scene_node = CurrentScene()->OpenInventorScene(); // -> Main scene : CStorageNode -> CGraphNode 
	if( scene_node )
	{
...
enum CGeomecDoc::FILE_STATUS CGeomecDoc::OnImportASCII(const CString &sPath, IProgressBase &dlg, CGraphNode::TNodeVec& created_objects)
{
	assert(Model());
	CModelBase& model = ((CModelBase&)*Model()).RootModel();
...
enum CGeomecDoc::FILE_STATUS CGeomecDoc::OnImportExcel(const CString &sPath, IProgressBase &dlg, CGraphNode::TNodeVec &created_objects)
{
	assert(Model());
	CModelBase& model = ((CModelBase&)*Model()).RootModel();

	CElementSetExcelFile excelfile(model);
	if(!excelfile.ReadSheets((LPCSTR) sPath, dlg, created_objects)) return FORMAT_ERROR;
...
int convert_dsa_icon_id( DSA_Icon icon_id )
{
	switch( icon_id )
	{
		case DSA_Icon::_1P:		return IDI_DIANA_1P;
		case DSA_Icon::_2P:		return IDI_DIANA_2P;
		case DSA_Icon::Running: return IDI_DIANA_RUNNING;
		case DSA_Icon::Crash:   return IDI_DIANA_CRASH;
		case DSA_Icon::Empty:   return IDI_DIANA_EMPTY;
		default:				return IDI_DIANA_EMPTY; // ERROR
	}
}
...
bool CGeomecDoc::RunAnalysis(	const CAnalysisType::TAnalysisType antype, 
...
void CGeomecDoc::OnAnalysisLinearDepletion() 
{
  RunAnalysis(CAnalysisType::AT_LINEAR, false, false);
}

void CGeomecDoc::OnAnalysisNonlinearCalculation() 
{
...
void CGeomecDoc::OnAnalysisHeatFlow()
{
  ...
  {
    if(_m()->msg("The heat flow analysis will invalidate the results from other analysis types. Are you sure you want to continue?", MB_YESNO) == IDNO)
      return;
  }
...
void CGeomecDoc::OnAnalysisMixture()
{
  RunAnalysis(CAnalysisType::AT_MIXTURE, false, false);
}
 
void CGeomecDoc::OnAnalysisMixtureContainment()
{
  RunAnalysis(CAnalysisType::AT_MIXTURE_CONTAINMENT, false, false);
}
...
void CGeomecDoc::OnUpdateAnalysisExportLinear(CCmdUI* pCmdUI)
{
  OnUpdateAnalysisLinearDepletion(pCmdUI);
}
...
void CGeomecDoc::OnAnalysisExportLinear()
{
  RunAnalysis(CAnalysisType::AT_LINEAR, true, false);
}

void CGeomecDoc::OnUpdateAnalysisExportNonlin(CCmdUI* pCmdUI)
{
  OnUpdateAnalysisNonlinearCalculation(pCmdUI);
}

void CGeomecDoc::OnAnalysisExportQuadDat()
{
  RunAnalysis(CAnalysisType::AT_LINEAR, false, true);
}
...
void CGeomecDoc::OnAnalysisExportNonlin()
{
  RunAnalysis(CAnalysisType::AT_NONLIN, true, false);
}

void CGeomecDoc::OnAnalysisExportHeatFlow()
{
  RunAnalysis(CAnalysisType::AT_HEAT, true, false);
}

void CGeomecDoc::OnAnalysisExportMixture()
{
  RunAnalysis(CAnalysisType::AT_MIXTURE, true, false);
}
...
void CGeomecDoc::OnUpdateAnalysisExportHeatFlow(CCmdUI* pCmdUI)
{
  OnUpdateAnalysisHeatFlow(pCmdUI);
}

void CGeomecDoc::OnUpdateAnalysisExportMixture(CCmdUI* pCmdUI)
{
  OnUpdateAnalysisMixture(pCmdUI);
}
...
void CGeomecDoc::OnUpdateDianaRunning(CCmdUI* pCmdUI)
{
	pCmdUI->Enable( _g->dsa_available() );
}
...
void CGeomecDoc::OnUpdateConsistencyGuard(CCmdUI* pCmdUI)
{
  if (Model())
  {
    ...
    if (CGStatus != m_nLastCGStatus)
    {
      ...
      switch (CGStatus)
      {
      case CConsistencyGuard::OK:			img_id = IDI_CG_OK; break;
      case CConsistencyGuard::UNKNOWN:		img_id = IDI_CG_UNKNOWN; break;
      case CConsistencyGuard::NEED_CLEANUP:	img_id = IDI_CG_MESSY; break;
      case CConsistencyGuard::INVALID:		img_id = IDI_CG_INVALID; break;
      case CConsistencyGuard::BROKEN:		img_id = IDI_CG_BROKEN; break;
      }
...
void CGeomecDoc::OnFileOpen()
{
  ...
  CTnoFileDialog dlg(TRUE, GEOMEC_DEFAULT_EXTENSION, NULL, OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, sFilter, FemAppGetMainWnd());
  ...
  if (database.isEmpty())
  {
    ...
    if (!strPath.IsEmpty())
    {
      ...
      if (strExtension == ".gm4")
	  {
		  ...
		  QString file_name = QFileDialog::getSaveFileName( nullptr /*parent*/, "Save File", (LPCTSTR)strPath, "Geomec Files (*.gm5)" );
...
void CGeomecDoc::OnFileSaveAs()
{
  ...
  CFileDialog dlg(FALSE, GEOMEC_DEFAULT_EXTENSION, strPath, OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, getStringTableEntry(IDS_FILE_SAVE_FILTER));
  if (dlg.DoModal() == IDOK)
  {
...
void CGeomecDoc::OnUpdateEditCopy(CCmdUI* pCmdUI) 
{
	pCmdUI->Enable(SelectedNode() != 0);
...
void CGeomecDoc::OnUpdateAnalysisNonlinearCalcParams(CCmdUI* pCmdUI) 
{
	pCmdUI->Enable(Model() != 0);
}
...
void CGeomecDoc::OnUpdateAnalysisLinearSettings(CCmdUI* pCmdUI)
{
	pCmdUI->Enable(Model() != 0);
}
...
{
  bool bEnable= Model()&& 
                static_cast< CModelBase *>(Model())->Mesh().IsMesh();
...
{
  bool bEnable= Model() &&
                static_cast< CModelBase *>(Model())->GraphEntry(MD_BASE_SURFACE);
  pCmdUI->Enable(bEnable);
}
...
{
  CModelBase *pModel= 
    static_cast< CModelBase *>(Model());
  assert(pModel);
  CSurfaceEntry *pSurfaceEntry= static_cast<CSurfaceEntry *>((pModel->GraphEntry(MD_BASE_SURFACE)));
  dynamic_cast <CSurfaceEntry_Delegate*> (
    pSurfaceEntry->getDelegate())->ExportSurfaces();
}
		...
		if( node )
		{
			...
			if( dlg )
			{
				if( dlg->save() != MeshRefinementDlg::NoError ) // error
				{
					QMessageBox::warning( 
						nullptr, 
						QString("Meshing with mesh refinement boxes"), 
...
void CGeomecDoc::OnAttachModel(CFemAppModel& model, CFemAppGUI& gui)
{
  ...
  while( 
	  !DirExists( 
		GetGeomecApp()->TempPath()->Path( 
		  CTempPath::TEMP_GENERAL ) ) )
  {
    if(_m()->msg("The path for temporary files is not valid. Please correct the settings", MB_OKCANCEL) == IDCANCEL)
    {
      ...
      else
      {
        ...
        if (strTempPath == strTempPathCalculation)
          ...
          msg = QString("GEOMEC will use '%1' as path for general temporary files for this model, and '%2' as path for temporary calculation files").arg(strTempPath).arg(strTempPathCalculation);
        _m()->msg(msg);
      }
...
bool CGeomecDoc::CanSelectComposite( const IValueComposite& composite ) const
{
        if (CurrentScene())
        {
            ...
            if (const_cast<CGeomecDoc *>(this)->CurrentScene()->ValueComponent() == &composite.Component() || const_cast<CGeomecDoc *>(this)->CurrentScene()->CanConnectItem(composite.Component()))
                return true;
	}
	return false;
}
...
bool CGeomecDoc::ChangeMode() const
{
  return m_bChangeMode;
}
...
bool CGeomecDoc::HasChangeSelection() const
{
  return !m_stChangeSelection.empty();
}
...
void CGeomecDoc::ChangeModeDeleteSelectedItems()
{
  ...
  public:
    CChangeSelectionNode(std::set<const CTreeNode*>& stSelection)
    : CGraphNode(""), m_stSelection(stSelection)
    {
...
void CGeomecDoc::OnUpdateFileClose(CCmdUI* pCmdUI)
{
	pCmdUI->Enable(Model() != 0);
}
...
{
  if ( m_pDataTreeView ) return m_pDataTreeView;
...
{
  if ( m_pModelTreeView ) return m_pModelTreeView;
...
{
  if (m_pModelView) return m_pModelView;
...
{
  if (m_pLegendModelView) return m_pLegendModelView;
...
{
  if(_m()->msg("Invalidating the heat flow results will also invalidate all dependent results.\nAre you sure you want to invalidate the heat flow results?", MB_ICONQUESTION | MB_YESNO) == IDYES)
  {
    CModelBase* pModel = static_cast<CModelBase*>(Model());
    if(pModel)
...
{
  if(_m()->msg("Are you sure you want to invalidate the mixture with containment results?", MB_ICONQUESTION | MB_YESNO) == IDYES)
  {
    CModelBase* pModel = static_cast<CModelBase*>(Model());
    if(pModel)
...
{
  pCmdUI->Enable(
    (Model() != 0) && CEditFaultSettings::hasFaultFractures(Model()));
}
...
void CGeomecDoc::OnUpdateOptimizationSettings(CCmdUI* pCmdUI)
{
  pCmdUI->Enable(Model() != 0);
}
...
void CGeomecDoc::OnUpdateFileSaveModelWithAveragedResultsOnly(CCmdUI* pCmdUI)
{
  OnUpdateFileSaveAs(pCmdUI);
}
...
void CGeomecDoc::OnViewStickToView()
{
  StickToScene(!StickToScene());
}
...
void CGeomecDoc::OnExecuteSVS()
{
  ...
  ShellExecute(GetDesktopWindow(), "open",
    QDir::toNativeSeparators(SVS_PATH).toStdString().c_str(), NULL, NULL,
    SW_SHOWNORMAL);
}

void CGeomecDoc::OnUpdateExecuteSVS(CCmdUI* pCmdUI)
{
  pCmdUI->Enable(QFile::exists(SVS_PATH));
}

void CGeomecDoc::OnExportLineData()
{
  OIV2DWellPathGraphScene* oiv2DWellPathGraphScene =
    dynamic_cast <OIV2DWellPathGraphScene*> (SelectedNode());
  QString filter = "ASCII Files (*.dat)|*.dat|";
  CFileDialog fileDialog(FALSE, "dat",
    oiv2DWellPathGraphScene->Name().toStdString().c_str(),
...
void CGeomecDoc::OnUpdateExportLineData(CCmdUI* pCmdUI)
{
  ...
  pCmdUI->Enable((oiv2DWellPathGraphScene != 0) &&
...
void CGeomecDoc::OnRunModel() { m_bRunning = true; RunAnalysis(CAnalysisType::AT_NONLIN, false, false); m_bRunning = false; };
// OnRunStep works, except that it doesn't show the Diana window (we turned that off somewhere for the commandline version)
...
void CGeomecDoc::OnWrongPythonroot()
{
	...
	if (res)
		QMessageBox::warning(nullptr, QString("PYTHONROOT"), QString("Now PYTHONROOT is correct.\n\nLaunch again the plugin please."));
};

void Launch_Py_Task_MFC(std::string name,
	...
	IPyInterface* py_iface)
{
	...
	public:
		PyTask_MFC(std::string name_,
			IGmInterface* gm_iface_,
			IPyInterface* py_iface_) : PyTask(name_, gm_iface_, py_iface_)
		{
		};
...
void CGeomecDoc::OnPlugins_Add()
{
	...
	CFileDialog dlg( TRUE, PLUGIN_INSTALLER_EXT, NULL, OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, PLUGIN_FILE_OPEN_FILTER, FemAppGetMainWnd() );
...
{ 
	...
	//printer->debug("-> %s | nID: %d | idx: %d (%s) | enable: %d ", __FUNCTION__, pCmdUI->m_nID, idx, CPlugins::instance()->get()[ idx ].c_str(), enable );
