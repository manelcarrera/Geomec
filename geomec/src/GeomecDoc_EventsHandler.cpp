...
template <class C> void delete_(C *&c) {
  if (c) {
    delete c;
    c = nullptr;
  }
...
CGeomecDoc_EventsHandler::CGeomecDoc_EventsHandler()
    : m_ra(nullptr), m_dlg(nullptr), m_prg(nullptr), m_th(nullptr), m_test(nullptr), m_ti(nullptr) {
...
void CGeomecDoc_EventsHandler::start(const CAnalysisType::TAnalysisType antype, bool bWriteInputFiles,
                                     bool bWriteQuadDat) {
  m_doc = GetGeomecDoc();

  if (_g->status() != CGlobal::Idle) {
    QString title = "Not possible to launch a new analysis";
    QString msg = _g->status() == CGlobal::Cancelling ? "Analysis being cancelled, hold on please..."
...
void CGeomecDoc_EventsHandler::on_prg(const Cmd &cmd) {
  ...
  case CProgressData::AddSteps_:
    m_prg->AddSteps(data.m_number);
    break;
  case CProgressData::Step_:
    m_prg->Step(data.m_number);
    break;
  case CProgressData::Status_:
    m_prg->StatusMessage(data.m_msg);
    break;
  case CProgressData::NextJob_:
    m_prg->NextJob(data.m_msg);
    break;
  case CProgressData::Enable_:
    m_prg->Enable(data.m_yes_no);
    break;
    //
    // FIXME:
    //
...
void CGeomecDoc_EventsHandler::on_diana_msg(const Cmd &cmd) {
  ...
  auto mfc_emit = [=]() {
    switch (type) {
    case Gen:
      emit m_dsu->OnMessage(msg_qs);
      break;
    case AppSpc:
      emit m_dsu->OnAppSpcMessage(msg_qs);
      break;
    case Error:
      emit m_dsu->OnError(msg_qs);
      break;
    case Warning:
      emit m_dsu->OnWarning(msg_qs);
      break;
    case StopExe:
      emit m_dsu->OnStopExecuting(b_stop);
      break;
    case Finished: {
      //
    ...
    default:
      printer->error("on diana msg : msg type undefined");
      break;
    }
  };
  ...
  auto mfc_handle = [=]() {
    ...
    switch (type) {
    case Gen:
      dlg->OnMessage(msg_qs);
      break;
    case AppSpc:
      dlg->OnAppSpcMessage(msg_qs);
      break;
    case Error:
      dlg->OnError(msg_qs);
      break;
    case Warning:
      dlg->OnWarning(msg_qs);
      break;
    case StopExe:
      dlg->OnStopExecuting(b_stop);
      log = false;
      break;
    case Finished: {
      log = false;
  ...
  auto qt_ = [=]() {
    ...
    default:
      printer->error("diana msg : evt not emitted");
      break;
    }
  };
...
void CGeomecDoc_EventsHandler::on_checks_error(const Cmd &cmd) {
  ...
  if (error_id != CRunAnalysis::HasResults && error_id != CRunAnalysis::Input) {
    QMessageBox::critical(nullptr, QString("Run analysis"),
                          QString::asprintf("An error occurred during the preliminary checks regarding: %s",
                                            CRunAnalysis::m_checks_cp[error_id]));
  }
...
void CGeomecDoc_EventsHandler::on_question(const Cmd &cmd) {
  ...
  case CQuestionThreadWrapper::DatFileDlg: {
    ...
    auto remove_extension = [](const QString &path_) { return path_.split(".", QString::SkipEmptyParts).at(0); };

    //
    // test: it doesn't work as EndDialog() is not closing CDatFileDialog
    //
    auto implementation_01 = [=](CDatFileDialog::Data &d, int &response, std::string &response_s) {
      if (m_test)
        d.timeout = 1000;
    ...
    //
    auto implementation_02 = [=](CDatFileDialog::Data &d, int &response, std::string &response_s) {
      if (m_test) {
        response = IDOK;
        response_s = remove_extension(ra()->context()->more.path).toStdString();
      } else {
  ...
  printer->info("question: %s ... response:%d response_s:%s", CQuestionThreadWrapper::m_question_cp[question_id],
                response, response_s.c_str());

  _e->broadcast(make_cmd(Response, new CQuestionThreadWrapper::DataRes{response, response_s}));
...
void CGeomecDoc_EventsHandler::on_hb() {
  ...
  auto implementation_03 = [=]() { gm::send::status(elapsed_time(), gm::send::eStatusType::ElapsedTime); };
  //
  // all the bar splashes from time to time: not nice ... refresh maybe less often
  //
  auto implementation_04 = [=]() { m_doc->set_dsa_icon(Running); };
...
void CGeomecDoc_EventsHandler::on_test(Cmd &cmd) {
  ...
  } else // 1a) create
  {
    ...
    if (data_) {
      ...
      std::this_thread::sleep_for(std::chrono::milliseconds(
          100)); // wait for shm to be instantiated before be accessed by Receiver -> is it needed in the case ?
      //
  ...
  case End: {
    ...
    std::this_thread::sleep_for(
        std::chrono::milliseconds(100)); // wait for the ipc mesg to be sent first by the Receiver
    //
    ...
    if (unittest()) {
      ...
      //
      // FIXME: close file if any open to avoid the save/no save pop-up? (I don't know how to skip it other than this
      // way)
      //
...
void CGeomecDoc_EventsHandler::handle(const Cmd &cmd) {
  ...
  if (!m_ra &&
      //
      cmd_id != eCmd::Test && cmd_id != eCmd::Quit_RA_Done && cmd_id != eCmd::Question)
  //
  {
    auto needs_ra = [=]() {
      int cmd_id = cmd.first;
  ...
  case eCmd::RunEnd:
    m_ra->on_run_end();
    break; // end of whole analysis (not only an iteration)
  ////////////////////////////////////////
  //
  ...
  case eCmd::QuitDiana_Done:
    break; // when diana finishes execution forcely
  case eCmd::CancelRun: {
    //
    // this arrives too late, filter impelemented in doc handle methos instead
    //
    //_e->remove( CEvents::GuiApp_EH, {Progress,DianaMsg} ); // many many (2600x) progress events has been enqueued
    // before this
    delete_(m_dlg);
    delete_(m_prg);
    break;
