...
private:
  ...
  void ProcessEndCell(TContents& contents, CExcelCell& cell, int last_number_line, bool& reading_numbers, bool& reading_dstages);
...
: m_model(model),
  ...
  m_pNodalExportFormat(0),
  m_bNewWellPath(bNewWellPath),
  m_pNewWellPathInput(0)
{
}

CElementSetExcelFileImpl::CElementSetExcelFileImpl(const IElementSetExportFormat& format)
: m_model(const_cast<CModelBase&>(format.Model())), 
  m_pElementSetExportFormat(&format),
  m_pNodalExportFormat(&format.NodalExportFormat()),
  m_bNewWellPath(false),
  m_pNewWellPathInput(0)
{
}
...
bool CElementSetExcelFileImpl::GetSheets(CExcelAppGuard& ExcelApp, const QString &sFileName, TSheetsVector &vcValidSheets)
{
  ...
  LPDISPATCH lpDisp = wbs.Open(sFileName.toStdString().c_str(), vOpt, vOpt, vOpt, vOpt, vOpt, vOpt, vOpt, vOpt, vOpt, vOpt, vOpt, vOpt);
  if(!lpDisp)
  {
    CString msg;
  ...
  {
      QString msg = QString("Ignored empty worksheet '%1'").
    arg(contents.SheetName);
...
*/
void CElementSetExcelFileImpl::ReadMatrixType(TContents& contents, IProgressBase &dlg, CGraphNode::TNodeVec &created_objects)
{
    ...
    if( row_cell.isDouble() ) {
      ...
      while( column_cell.nextColumn() ) {
        if( column_cell.isDouble() ) {
          ...
          if( !column_cell.isEmpty() ) column_cell.throwMessage("Unexpected data in matrix format.");
          break;
        }
      }
    } else {
      if( !row_cell.isEmpty() ) row_cell.throwMessage("Unexpected data in matrix format.");
      break;
    }
  }
}
...
void CElementSetExcelFileImpl::ReadContents(TContents &contents)
{
  ...
  if((upperleft.isEmpty() || upperleft.isString()) && 
    uppersecond.isDouble() && 
    secondleft.isDouble() && 
  ...
  do {
    if(cell.isEmpty()) {
      // We're register it...
      if(!iFirstEmpty) iFirstEmpty = cell.row();
    continue;
    }
    ...
    if(cell.isString())
    {
      ...
      if(str.CompareNoCase("POINTSET") == 0) {
        ...
        // Dimension keyword
        if(reading_numbers) cell.ThrowMessageID(IDS_KW_IN_NUMBERS);
        if(contents.dimrow) cell.ThrowMessageID(IDS_SECOND_DIM);
        if(contents.type == ST_POINTSET) cell.ThrowMessageID(IDS_DIMKW_IN_ELEMENTSET);
        contents.dimrow = cell.row();
        reading_dstages = false;
      } else if(str.CompareNoCase("UNIT") == 0) {
        // Unit keyword
        if(reading_numbers) cell.ThrowMessageID(IDS_KW_IN_NUMBERS);
        if(contents.unitrow) cell.ThrowMessageID(IDS_SECOND_UNIT);
        ...
        // COOR keyword
        if(reading_numbers) cell.ThrowMessageID(IDS_KW_IN_NUMBERS);
        if(contents.coorrow) cell.ThrowMessageID(IDS_SECOND_UNIT);
        if((contents.type == ST_ELEMENTSET || contents.type == ST_SURFACE) && contents.pointsrow) 
          cell.ThrowMessageID(IDS_UNITKW_AFTER_POINTSKW);
        ...
        if(reading_numbers) cell.ThrowMessageID(IDS_KW_IN_NUMBERS);
        if(contents.dstagerow) cell.ThrowMessageID(IDS_SECOND_DSTAGE);
        contents.dstagerow = cell.row();
        reading_dstages = true;
      } else if(str.CompareNoCase("POINTS") == 0) {
        // Point keyword
        assert(!reading_numbers);
        if(contents.pointsrow) cell.ThrowMessageID(IDS_SECOND_POINTSKW);
        if(contents.type != ST_ELEMENTSET &&
           contents.type != ST_SURFACE ) cell.ThrowMessageID(IDS_POINTSKW_IN_POINTSET);
        contents.pointsrow = cell.row();
        reading_dstages = false;
      }
      else if(str.CompareNoCase("ELEMENTS") == 0)
      {
        if(contents.elementsrow) cell.ThrowMessageID(IDS_SECOND_ELEMENTSKW);
        if(contents.type != ST_ELEMENTSET &&
           contents.type != ST_SURFACE) cell.ThrowMessageID(IDS_ELEMENTSKW_IN_POINTSET);
        if(!contents.pointsrow) cell.ThrowMessageID(IDS_ELEMENTSKW_BEFORE_POINTS);
        if(!contents.pointhdstart) cell.ThrowMessageID(IDS_ELEMENTSKW_BEFORE_POINTHEADERS);
        if(!contents.pointsstart) cell.ThrowMessageID(IDS_ELEMENTSKW_BEFORE_POINTSSTART);
      ...
      {
        if(contents.elmtyperow) cell.ThrowMessageID(IDS_SECOND_ELEMENTTYPE);
        if(!contents.elementsrow) cell.ThrowMessageID(IDS_ELEMENTTYPE_BEFORE_ELEMENTSKW);
    ...
    encounteredEND = true;
        ...
        if(contents.type == ST_ELEMENTSET || contents.type == ST_SURFACE)
        {
          ...
          {
            if(!contents.pointsrow) cell.ThrowMessageID(IDS_UNEXPECTED_VALUE);
            if(contents.pointhdstart) cell.ThrowMessageID(IDS_UNEXPECTED_VALUE);
            contents.pointhdstart = cell.row();
          }
          else
          {
            if(!contents.elmtyperow) cell.ThrowMessageID(IDS_UNEXPECTED_VALUE);
            if(contents.elmhdstart) cell.ThrowMessageID(IDS_UNEXPECTED_VALUE);
        ...
        do {
          if( !cell.isEmpty()   ) {
            if( !cell.isString() ) cell.throwMessage("After PROPERTY a header is expected");
            if( cell.getString().toUpper() != "PROPERTY" ) {
              if(!contents.pointhdend) contents.pointhdend = cell.row() + 1;
              else contents.elmhdend = cell.row() + 1;
              break;
            }
          }
        } while( cell.nextRow() );
...
void CElementSetExcelFileImpl::ProcessEndCell(TContents& contents, CExcelCell& cell, int last_number_line, bool& reading_numbers, bool& reading_dstages)
{
...
*/
void CElementSetExcelFileImpl::ReadElementSet(TContents &contents, IProgressBase &dlg, CGraphNode::TNodeVec &created_objects)
{
...
void CElementSetExcelFileImpl::ReadUntagged(TContents& contents, IProgressBase &dlg, CGraphNode::TNodeVec &created_objects)
{
...
void CElementSetExcelFileImpl::ReadPointSet(TContents &contents, IProgressBase &dlg, CGraphNode::TNodeVec &created_objects)
{
...
*/
void CElementSetExcelFileImpl::ReadData(IImportFormat& format,
                  CExcelCell& cell, 
                  IProgressBase &dlg, 
                  int dlgstart, 
                  int dlgend)
...
bool CElementSetExcelFileImpl::Write(const CString& sFileName, IProgressBase &callback, bool append)
{
  ...
  try
  {
    ...
    if ( existing && append ) {
      ...
      LPDISPATCH lpDisp = wbs.Open(sFileName, vNotPassed, vNotPassed, vNotPassed, vNotPassed, vNotPassed, vNotPassed, vNotPassed, vNotPassed, vNotPassed, vNotPassed, vNotPassed, vNotPassed);
      if(!lpDisp)
      {
        CString msg;
        msg.Format(IDP_CANNOTOPENFILE, sFileName);
  ...
  cell.WriteToSheet(ws);
    ...
    if ( !append )
      wb.SaveAs(vFileName, vNotPassed, vNotPassed, vNotPassed, vNotPassed, vNotPassed, 0, vNotPassed, vNotPassed, vNotPassed, vNotPassed);
    else
      wb.Save();
...
void CElementSetExcelFileImpl::WriteTable(const IExportFormat& format, CExcelCell& cell, IProgressBase &callback)
{
  for(size_t i = 0; i < format.ValueTypeSize(); i++) {
    ...
    cell.nextRow();
    if(format.ValueTypeSize() == i + 1) cell.nextRow();
  }
  ...
  for (size_t s = 0; s < format.DataValueSize(); ++s)
  {
    ...
    try
    {
      ...
      for (size_t t = 0; t < size; t++)
      {
        ...
        (stage == static_cast <const IResultComponent&> (
                  format.DataValue(s + t)).Stage().Index()) &&
        (&valueComposite == &format.DataValue(s + t).Parent()))
              {
        cell.nextColumn();
        cell.WriteString(
                  format.StringAt(0, format.getDataOffset() + s + t).
          toStdString().c_str());
        processed[s + t] = true;
              }
      }
          ...
          {
      cell.nextColumn();
      cell.WriteString(format.StringAt(0, format.getDataOffset() + s).
              toStdString().c_str());
          }
...
CElementSetExcelFile::CElementSetExcelFile(
  const CNodalExportFormat& export_format)
: m_elementSetExcelFileImpl(new CElementSetExcelFileImpl(export_format))
{
}

CElementSetExcelFile::CElementSetExcelFile(
  const IElementSetExportFormat& export_format)
: m_elementSetExcelFileImpl(new CElementSetExcelFileImpl(export_format))
{
}
...
CElementSetExcelFile::~CElementSetExcelFile()
{
  delete m_elementSetExcelFileImpl;
}
...
bool CElementSetExcelFile::ReadSheets(const QString& sFileName,
  IProgressBase& dlg, CGraphNode::TNodeVec& created_objects)
{
  return m_elementSetExcelFileImpl->ReadSheets(sFileName, dlg, created_objects);
}

bool CElementSetExcelFile::Write(const QString& sFileName,
  IProgressBase& callback, bool append)
{
  return m_elementSetExcelFileImpl->Write(sFileName.toStdString().c_str(), callback, append);
}

const CModelBase& CElementSetExcelFile::Model() const
{
  return m_elementSetExcelFileImpl->Model();
}

CModelBase& CElementSetExcelFile::Model()
{
  return m_elementSetExcelFileImpl->Model();
}
