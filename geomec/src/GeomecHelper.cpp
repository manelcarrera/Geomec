...
namespace version{
  ...
  void unset()
  {
    CXml::update( CONFIG_FILE, version::KEY, "" );
  }
  ...
  {
    CXml::update( CONFIG_FILE, version::KEY, version.toStdString() );
  }
...
void plugins()
{
  ...
  // (4) Manual
  CItem::refresh( {"&Help"}, CPlugins::instance()->get(), CItem::Manual ); //FIXME: '&Help' -> Make it independent from special characters
...
void add_default_version_menu_item()
{
  ...
  std::string caption = v == gm::helper::version::read() ? 
    "Mark as NOT default version" : 
    ...
    if( QFileInfo( dir_qs ).isDir() )
    {
      ...
      if( QFileInfo( tmp_dir_path ).isDir() )
      {
        ...
        if( res_u )
        {
          ...
          if( res )
          {
            //res = CPlugins::copy( tmp_dir_path_s + "/changelog.css", CPlugins::usr_dir( CPlugins::eUsrDir::Changelog ) + "/changelog.css" );
            res  = CSystem::unpack(ChangelogDlg::CHANGELOG_PACKAGE, QUtil::usr_dir( QUtil::eUsrDir::Changelog ), ChangelogDlg::CSS_FILE );
            if( res )
              retval = true;
          }
        }
