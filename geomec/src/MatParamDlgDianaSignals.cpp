...
CMatParamDlgDianaSignals::CMatParamDlgDianaSignals(const QObject* sender,
  CMatParamDlg* matParamDlg)
: CDianaSignals(sender)
, m_matParamDlg(matParamDlg)
{
}
...
void CMatParamDlgDianaSignals::onError(QString error)
{
  m_matParamDlg->OnDianaXDiaError(error.toStdString().c_str());
}
...
void CMatParamDlgDianaSignals::onFinished()
{
  m_matParamDlg->OnDianaXCloseDialog();
}
