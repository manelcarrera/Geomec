...
CRetrieveDianaFileNamesGUI::CRetrieveDianaFileNamesGUI(CAnalysisType::TAnalysisType analysisType, bool writeInputFiles,
                                                       bool writeQuadDat, const CModelBase &model)
    : IRetrieveDianaFileNames(), m_analysisType(analysisType), m_writeInputFiles(writeInputFiles),
      m_writeQuadDat(writeQuadDat), m_model(model) {}

CRetrieveDianaFileNamesGUI::~CRetrieveDianaFileNamesGUI() {}

bool CRetrieveDianaFileNamesGUI::retrieveDianaFileNames(QString &newTitle, const std::string &title) const {
  ...
  CDatFileDialog::Data *d =
      new CDatFileDialog::Data{m_analysisType, m_writeInputFiles, m_writeQuadDat, m_model,
                               0, // timeout
                               //
                               // CTnoFileDialog
                               //
                               FALSE, "", newTitle.toStdString().c_str(), OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT,
                               "DIANA command/data files (*.dcf;*.dat)|*.dcf;*.dat|"
                               "All Files (*.*)|*.*||",
                               FemAppGetMainWnd()};

  if (_g->dsa()) {
    CQuestionThreadWrapper th(CQuestionThreadWrapper::DatFileDlg,
                              d); // event send to doc / wait for the resposne / blocking
    th.join();
...
QString CRetrieveDianaFileNamesGUI::stripExtension(const QString &string, const QString &extension) const {
