...
const char* CTest::ana_type_cp[]={"Linear","Non-linear","Heat","Mix","Mix-containment"}; // for run and export commands
...
bool CTest::execute( int idx )
{
  return execute( m_cmd_v[ idx ] );
}
  ...
  switch(cmd.id)
  {
    ...
    case Command:
    {
      ...
      /*if(command==ID_FILE_SAVE_AS)
        //gm::send::key_stroke_ctrl('S'); // save model -> it works ... but it crashes no quit save_as dlg ( hWnd=nullptr, why? )
        gm::send::key_stroke( std::vector<WORD>{VK_DOWN,VK_DOWN} );*/
