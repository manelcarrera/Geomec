...
public:
  QStandardItem_Custom(const QString &text, CTetraMesh *mesh = nullptr)
      : QStandardItem(text), m_mesh(mesh), m_model_type(Unknown_M) {};
  QStandardItem_Custom(const QIcon &icon, const QString &text, CTetraMesh *mesh = nullptr)
      : QStandardItem(icon, text), m_mesh(mesh), m_model_type(Unknown_M) {};
  QStandardItem_Custom(const QIcon &icon, const QString &text, eModelType model_type)
      : QStandardItem(icon, text), m_mesh(nullptr), m_model_type(model_type) {};
...
eMesherType mesher_type(CModelBase *model) {
  ...
  if (model) {
    ...
    type_ = mesh ? mesh->CurrentMesherType() == 0 ? Old : New : Unknown;
  }
...
eMesherType mesher_type(QStandardItem_Custom *item) {
  ...
  eMesherType type_ = mesh ? mesh->CurrentMesherType() == 0 ? Old : New : Unknown;
  return type_;
}
...
bool is_mesher_type(CTetraMesh *mesh, eMesherType _type) { return mesher_type(mesh) == _type; }
...
void MeshConverterTreeView::init() {
  ...
  Printer::instance()->debug("ICON_TETRA: '%s' exist: %d / app_path: %s res: %d", ICON_TETRA.toStdString().c_str(), res,
  s.toStdString().c_str(), res2 );*/
...
QStandardItem *MeshConverterTreeView::add_model(CModelBase *model_b, QStandardItem *parent, const QString &name) {
  ...
  QStandardItem_Custom *item2 =
      new QStandardItem_Custom(model_icon, QString("Name: ") + model_b->Name(), !item->parent() ? mt : Unknown_M);
  ...
  // Printer::instance()->debug("%s - %s - %s", name.toStdString().c_str(), model_b->Name().toStdString().c_str(),
  // mesher_name.toStdString().c_str() );
...
bool MeshConverterTreeView::any_old_mesher() { return any_mesher(m_items, Old); }
