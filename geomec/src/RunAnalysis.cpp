...
std::pair<bool, QString> not_converged_msg(const dia::IAnalysisStatusContainer &statuses) {
  ...
  if (showWarning) {
    ...
    if (firstPhaseNumber != lastPhaseNumber)
      warning += QString(" Several phases are affected (first phase %1 and last phase %2).")
                     .arg(firstPhaseNumber)
                     .arg(lastPhaseNumber);
    else
...
std::vector<std::pair<std::string, bool>> *log(CRunAnalysis::eRunLogs step, DSA_Context *c) {
  ...
  case CRunAnalysis::Step_4: {
    ...
    *list << str;
    ...
    str = QString("*** Analysis took %1 days, %2 hours, %3 minutes, %4 seconds ***")
              .arg(timespan.days)
              .arg(timespan.hours)
  ...
  case CRunAnalysis::Step_6: {
    ...
    TTimeInterval timespan = calculateTimeInterval(c->time.save.start, c->time.save.end);

    str = QString("*** Automatic save took %1 days, %1 hours, %3 minutes, %4 seconds ***")
              .arg(timespan.days)
              .arg(timespan.hours)
              .arg(timespan.minutes)
              .arg(timespan.seconds);
...
CRunAnalysis::CRunAnalysis() : IRunAnalysis(RunAnalysis_ListenerDefinition), m_dsa_context(nullptr) {
  m_printer->info("new");
...
void CRunAnalysis::on_params(const Cmd &cmd) {
  ...
  m_printer->info("--params-- ana:%s, write input files:%s, write quad dat:%s", ana_type_cp[data.ana_type],
                  data.is_write_input_files ? "yes" : "no", data.is_write_quad_dat ? "yes" : "no");
...
void CRunAnalysis::create__(DSA_Context::eType type) {
  ...
  case DSA_Context::LicenseCheck: {
    ...
    c->drc = new CDianaRunController(*model, CAnalysisType::AT_LINEAR, false, false, false,
  ...
  case DSA_Context::Regular: {
    ...
    c->retrieve = new CRetrieveDianaFileNames(new CRetrieveDianaFileNamesGUI(
        c->input.ana_type, c->input.is_write_input_files, c->input.is_write_quad_dat, *m_model));
    c->drc =
        new CDianaRunController(*m_model, c->input.ana_type, c->input.is_write_input_files, c->input.is_write_quad_dat,
                                m_model->WriteOutputStreamFile(), GetGeomecTempPathExt(CTempPath::TEMP_CALCULATION));
    break;
  }
  default:
    break;
...
bool CRunAnalysis::run_controller() {
  ...
  //
  dia::IDianaRunner::RunParams p = {c->more.path,     c->more.title,  c->drc->tmp().toStdString(),
                                    c->dxw,           c->save,        c->retrieve,
                                    &c->out.statuses, c->more.silent, _g->dsa()};
  ...
  if (_g->dsa()) {
    _e->broadcast(
        make_cmd(RunController,
                 new dia::IDianaRunner::RunParams(p))); // no need to pass the params but it can be useful in the future

    res = true;
  } else {
    res =
        c->drc->run(); // in dsa cmd is queued and method exists immediatelly ... to be implemented: cmd 'RunController'
  }
...
bool CRunAnalysis::checks(eChecks type) {
  ...
  case HasResults: {
    if (!c->input.is_write_input_files && !c->input.is_write_quad_dat) {
      //
      // FIXME: sometimes crashes here !!!
      //
      if (m_model->ResultRegister().ResultsAvailable(c->input.ana_type) &&
          (c->input.ana_type != CAnalysisType::AT_NONLIN || !m_model->BranchState().IsBranch())) {
  ...
  //
  case Input: {
    res = m_model->CheckForAnalysis(c->input.ana_type, c->input.is_write_input_files, c->input.is_write_quad_dat);
    break;
  }
  case License: {
...
void CRunAnalysis::on_run(const Cmd &cmd) {
  ...
  // catch (CProgressCancel* e)
    ...
    _e->broadcast({UpdateViews, // TODO: verify wether this is needed
                   Quit_RA});
  }
  //
  // result ?
...
void CRunAnalysis::on_run_end() {
  ...
  //
    ...
    //
    // EndRun: no need, in gui run is always stagged, so never step-by-step one, data is retrieved per stage in the
    // controller
    //
    push_({Save});
  }
...
void CRunAnalysis::handle(Cmd cmd) {
  ...
  case eCmd::RunEnd:
    on_run_end();
    break; //= end analysis (all iterations Ok)
  case eCmd::CancelRun: {
    //
    ...
    //
    _e->notify({UpdateViews, // needs to be done before deleting ra as it depends on it
