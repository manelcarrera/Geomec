...
CZoomInCreator::CZoomInCreator(CHomogenizationBox &box)
: m_box(&box)
{
}
...
*/
void CZoomInCreator::MapResults(const QString& sResultName,
...
*/
void CZoomInCreator::ExtractResults(CElementSet& element_set, const CDepletionStage& stage, const CAnalysisType& antype)
{
	QString sAnalysis;
...
void CZoomInCreator::Create()
{
  ...
  CTnoFileDialog dlg(FALSE, GEOMEC_DEFAULT_EXTENSION, NULL, OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, sFilter.toStdString().c_str(), FemAppGetMainWnd());
	...
	if(dlg.DoModal() == IDOK) {
      ...
      if(model->model()->ResultRegister().Linear()) ExtractResults(*pResultSet, CAnalysisType::AT_LINEAR);
      if(model->model()->ResultRegister().NonLinear()) ExtractResults(*pResultSet, CAnalysisType::AT_NONLIN);
      ...
      if (GetGeomecDoc()->CurrentScene())
      {
			...
			tb.InterfaceDefinition().KtanBottom(tb.InterfaceDefinition().Ktan(CDoubleQuantity::SI_UNIT),CDoubleQuantity::SI_UNIT);
			tb.InterfaceDefinition().KtanSides(tb.InterfaceDefinition().Ktan(CDoubleQuantity::SI_UNIT),CDoubleQuantity::SI_UNIT);
			tb.InterfaceDefinition().KradTop(tb.InterfaceDefinition().Krad(CDoubleQuantity::SI_UNIT),CDoubleQuantity::SI_UNIT);
			tb.InterfaceDefinition().KradBottom(tb.InterfaceDefinition().Krad(CDoubleQuantity::SI_UNIT),CDoubleQuantity::SI_UNIT);
			tb.InterfaceDefinition().KradSides(tb.InterfaceDefinition().Krad(CDoubleQuantity::SI_UNIT),CDoubleQuantity::SI_UNIT);
			...
			if (m_box != 0)
			{
				CHomogenizationBox_Delegate* m_box_Delegate =
					dynamic_cast <CHomogenizationBox_Delegate*> (m_box->getDelegate());
