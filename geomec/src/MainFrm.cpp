...
static unsigned int indicators[] = {
    ...
    ID_INDICATOR_COOR, ID_INDICATOR_UNIT,    ID_INDICATOR_MESH, ID_INDICATOR_LIN, ID_INDICATOR_NON_LIN,
    ID_INDICATOR_HEAT, ID_INDICATOR_MIXTURE, ID_INDICATOR_CAPS, ID_INDICATOR_NUM, ID_INDICATOR_SCRL,
};
...
void F5() {
  ...
  // FIXME: move to gm
  //
  IProgressFactory *f = _g->mfc(CGlobal::Progress) ? new CProgressFactory_MFC : new CProgressFactory;
  _g->prog(f);
...
CToolBar &CMainFrame::GetConsistencyToolbar() { return m_wndCalcBar; }
...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct) {
  ...
  if (!m_wndToolBar.CreateEx(this, TBSTYLE_FLAT,
      ...
      !m_wndViewBar.CreateEx(this, TBSTYLE_FLAT,
                             WS_CHILD | WS_VISIBLE | CBRS_TOP | CBRS_GRIPPER | CBRS_TOOLTIPS | CBRS_FLYBY |
                                 CBRS_SIZE_DYNAMIC,
                             CRect(0, 0, 0, 0), ID_VIEW_TOOLBAR_VIEW) ||
      !m_wndViewBar.LoadToolBar(IDR_VIEW_BAR) ||
      !m_wndTopViewBar.CreateEx(this, TBSTYLE_FLAT,
                                WS_CHILD | WS_VISIBLE | CBRS_TOP | CBRS_GRIPPER | CBRS_TOOLTIPS | CBRS_FLYBY |
                                    CBRS_SIZE_DYNAMIC,
                                CRect(0, 0, 0, 0), ID_VIEW_TOOLBAR_TOPVIEW) ||
      // !m_wndTopViewBar.LoadToolBar(IDR_TOPVIEW_BAR) ||
      !m_wndCalcBar.CreateEx(this, TBSTYLE_FLAT,
                             WS_CHILD | WS_VISIBLE | CBRS_TOP | CBRS_GRIPPER | CBRS_TOOLTIPS | CBRS_FLYBY |
                                 CBRS_SIZE_DYNAMIC,
                             CRect(0, 0, 0, 0), ID_VIEW_TOOLBAR_CALC) ||
      !m_wndCalcBar.LoadToolBar(IDR_CALC_BAR) ||
      !m_wndTensorBar.CreateEx(this, TBSTYLE_FLAT,
                               WS_CHILD | WS_VISIBLE | CBRS_TOP | CBRS_GRIPPER | CBRS_TOOLTIPS | CBRS_FLYBY |
                                   CBRS_SIZE_DYNAMIC,
                               CRect(0, 0, 0, 0), ID_TOOLBAR_TENSOR) ||
  ...
  if (!m_wndStatusBar.Create(this) ||
      !m_wndStatusBar.SetIndicators(indicators, sizeof(indicators) / sizeof(unsigned int))) {
    TRACE0("Failed to create status bar\n");
    return -1; // fail to create
  }

  if (!m_vectorDialogBar.Create(this, IDD_VECTOR_DIALOGBAR,
                                WS_CHILD | WS_VISIBLE | CBRS_TOP | CBRS_GRIPPER | CBRS_TOOLTIPS | CBRS_FLYBY |
                                    CBRS_SIZE_DYNAMIC,
                                IDD_VECTOR_DIALOGBAR)) {
...
BOOL CMainFrame::OnCreateClient(LPCREATESTRUCT lpcs, CCreateContext *pContext) {
  int iMainFrameSplitter =
      ISettings::instance()->getProfileInt(s_profileHeading, s_profileMainFrameSplitter, SPLITTER_BAR_X);

  if (!m_wndSplitter.CreateStatic(this, 1, 2) ||
      !m_wndSplitter.CreateView(0, 1, RUNTIME_CLASS(COpenGLSplitView), CSize(0, 0), pContext) ||
      !m_wndSplitter.CreateView(0, 0, RUNTIME_CLASS(CTreeSplitView), CSize(iMainFrameSplitter, 0), pContext)) {
    return FALSE;
  } else
    return TRUE;
}
...
void CMainFrame::OnViewToolbarGeneral() { ShowControlBar(&m_wndToolBar, !m_wndToolBar.IsWindowVisible(), FALSE); }

void CMainFrame::OnUpdateViewToolbarGeneral(CCmdUI *pCmdUI) { pCmdUI->SetCheck(m_wndToolBar.IsWindowVisible()); }

void CMainFrame::OnViewToolbarView() { ShowControlBar(&m_wndViewBar, !m_wndViewBar.IsWindowVisible(), FALSE); }

void CMainFrame::OnUpdateViewToolbarView(CCmdUI *pCmdUI) { pCmdUI->SetCheck(m_wndViewBar.IsWindowVisible()); }

void CMainFrame::OnViewToolbarCalc() { ShowControlBar(&m_wndCalcBar, !m_wndCalcBar.IsWindowVisible(), FALSE); }

void CMainFrame::OnUpdateViewToolbarCalc(CCmdUI *pCmdUI) { pCmdUI->SetCheck(m_wndCalcBar.IsWindowVisible()); }

void CMainFrame::OnViewTensorToolBar() { ShowControlBar(&m_wndTensorBar, !m_wndTensorBar.IsWindowVisible(), FALSE); }

void CMainFrame::OnUpdateViewTensorToolBar(CCmdUI *pCmdUI) { pCmdUI->SetCheck(m_wndTensorBar.IsWindowVisible()); }
...
void CMainFrame::GetMessageString(UINT nID, CString &rMessage) const { rMessage.Empty(); }

void CMainFrame::OnDestroy() {
  if (!m_bDoSaveParameters)
    return;
  CString strText;
  BOOL bIconic, bMaximized;
  ...
  strText.Format("%04d %04d %04d %04d", wndpl.rcNormalPosition.left, wndpl.rcNormalPosition.top,
...
void CMainFrame::OnOIVTutorial() { OpenPdfDocument("Geomec OIV-5.0.pdf", ExePath().c_str()); }

void CMainFrame::OnMeshRefinementManual() { OpenPdfDocument("Mesh refinement guide.pdf", ExePath().c_str()); }

void CMainFrame::OnLicenseBorrowingHelp() {
  OpenPdfDocument("License borrowing and troubleshooting.pdf", ExePath().c_str());
}

void CMainFrame::OnWellZoominHelp() { OpenPdfDocument("zoominwell.pdf", ExePath().c_str()); }

void CMainFrame::OnGVTQuickTourHelp() { OpenPdfDocument("GVT Quick Tour.pdf", ExePath().c_str()); }
...
void CMainFrame::OnContextMenu(CWnd *pWnd, CPoint point) {
  ...
  // p0.x,p0.y,this);
  DWORD SelectionMade =
      pPopup->TrackPopupMenu(TPM_LEFTALIGN | TPM_LEFTBUTTON | TPM_NONOTIFY | TPM_RETURNCMD, p0.x, p0.y, wnd);
#endif
  //---------------------
