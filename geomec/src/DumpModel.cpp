...
CDumpModel::CDumpModel(const CModelBase &model)
    : m_model(model), m_lineTypes(), m_lines(), m_vcStages(), m_vcFormations(), m_vcFaults() {
  ...
  if (pEntryF) {
    ...
    for (CTetraSubHorizonEntry::TNodeSet::const_iterator itF = stFaults.begin(); itF != stFaults.end(); ++itF) {
...
std::vector<CDumpModel::TLineType> CDumpModel::getLineTypes() const { return m_lineTypes; }

std::vector<QString> CDumpModel::getLines() const { return m_lines; }

// private

void CDumpModel::generalModelInfo(const QString &modelName, const QString &fileLocation) {
  ...
  addLine(text,
          QString("%1").arg(((TFormationBaseEntry *)(m_model.GraphEntry(MD_BASE_FORMATION)))->EntryNodes().size()));
  addLine(newLine, "");
  addLine(boldText, "Number of faults: ");
  int nFaults = 0;
  THorizonBaseEntry *pEntry = (THorizonBaseEntry *)m_model.GraphEntry(MD_BASE_HORIZON);
  for (THorizonBaseEntry::TNodeSet::const_iterator it = pEntry->EntryNodes().begin(); it != pEntry->EntryNodes().end();
       ++it) {
    if ((*it)->Slip())
      ++nFaults;
  }
...
void CDumpModel::nonLinCalcSettingsInfo() {
  ...
  addLine(newLine, "");
  addLine(boldText,
          "Automatic steps\tMax relative step size\tSteps\tScale Factor\tConverg. Norm Value\tMaxIters\tIter. "
  ...
  for (size_t i = 0; i < m_vcStages.size(); ++i) {
    ...
    addLine(text, temp);
    temp = QString("\t\t%1").arg(pStage->IterationSchemeTag(pStage->NLIterationScheme()).c_str());
    addLine(text, temp);
    temp = QString("\t\t%1").arg(pStage->ConvergenceTypeTag(pStage->NLConvergenceType()).c_str());
    addLine(text, temp);
    addLine(newLine, "");
  }
...
void CDumpModel::pressureDataInfo() {
  ...
  for (size_t i = 0; i < m_vcFormations.size(); ++i) {
    ...
    for (size_t j = 0; j < m_vcStages.size(); ++j) {
      const CPressure *pPressure = &m_vcFormations[i]->Pressure(*m_vcStages[j]);
      ...
      if (pPressure->DistributedSize() == 0 || (pPressure->DistributedSize() > 0 && !pPressure->DistributedOnly())) {
...
void CDumpModel::faultPressureInfo() {
  ...
  for (size_t i = 0; i < m_vcFaults.size(); ++i) {
    ...
    enum { e_unknown = -1, e_distr = 1, e_max, e_min, e_maxgrad, e_grad } ePrevState = e_unknown;
    ...
    for (size_t j = 0; j < m_vcStages.size(); ++j) {
      ...
      if (faultPressure.DistributedOnly()) {
        ...
        case CFaultPressure::MT_GRADIENT:
          ...
          if (ePrevState != e_grad || dRefDepth != dPrevRefDepth || dRefPressure != dPrevRefPressure ||
...
void CDumpModel::temperatureDataInfo() {
  ...
  for (size_t i = 0; i < m_vcFormations.size(); ++i) {
    ...
    for (size_t j = 0; j < m_vcStages.size(); ++j) {
      const CTemperature *pTemperature = &m_vcFormations[i]->UserTemperature(*m_vcStages[j]);
...
void CDumpModel::strainDataInfo() {
  ...
  for (size_t i = 0; i < m_vcFormations.size(); ++i) {
    ...
    for (size_t j = 0; j < m_vcStages.size(); ++j) {
      ...
      const CStrainLoad *pStrainLoad = &m_vcFormations[i]->Strain(*m_vcStages[j]);
...
void CDumpModel::materialInfo(const CMaterialServer *pMatServer) {
  ...
  if (pMatServer->LibraryMaterial()) {
    QString temp = QString("%1 (%2): ")
                       .arg(pMatServer->MaterialName())
                       .arg(MaterialModelName((mlMatModel)(pMatServer->MaterialModel())));
    addLine(text, temp);
    CGlobalUnitNode globalUnitNode;
    ...
    for (size_t i = 0; i < pMatServer->LibraryMaterial()->ParameterSize(); ++i) {
      ...
      temp =
          QString("   %1 %2 %3").arg(param.Name()).arg(param.ValueToUserUnit(nUnitDef)).arg(param.UnitName(nUnitDef));
      addLine(text, temp);
      ...
      for (int j = 0; j < vcValueTypePairs.size(); ++j) {
        if (vcValueTypePairs[j].second == param.ValueTypeID()) {
          ...
          IValueDomainScalar::TMinMax mm =
              vt.Component().ScalarData().MinMax(progress1, globalUnitNode.unitNode().Unit());
          temp = QString(", pointset \"%1\", min = %2 %3, max = %4 %5, average = %6 %7")
                     .arg(vt.PointSet().Name())
                     .arg(mm.first.Value())
...
void CDumpModel::boundaryInfo() {
  ...
  if (pBoundary) {
    ...
    for (int i = 0; i < vcSurfaces.size(); ++i) {
      const CPressureSupportNode::CFaceSupportDef &fsd =
          psn.FaceSupportDef(CPressureSupportNode::CSurfaceDef(*vcSurfaces[i].first));
    ...
    for (int i = 0; i < vcCorners.size(); ++i) {
      ...
      for (CDepletionStageEntry::const_iterator it = m_model.DepletionStageEntry().begin();
           it != m_model.DepletionStageEntry().end(); ++it) {
        ...
        if (!ds.Initial()) {
          ...
          if (!vec.Empty()) {
            addLine(text, QString("%1\t%2\t%3\t%4\t%5\t%6\t%7")
                              .arg(ds.Index())
                              .arg(pt.X())
                              .arg(pt.Y())
                              .arg(pt.Z())
...
void CDumpModel::upscalingInfo() {
  ...
  for (CUpscalingEntry::TNodeSet::const_iterator it = stNodes.begin(); it != stNodes.end(); ++it) {
    ...
    if (!stFormations.empty()) {
      ...
      addLine(newLine, "");
      for (std::set<const CFormationBase *>::const_iterator it2 = stFormations.begin(); it2 != stFormations.end();
           ++it2) {
...
void CDumpModel::FaultsInfo() {
  ...
  for (size_t i = 0; i < m_vcFaults.size(); ++i) {
    ...
    QString temp;
    const CDepletionStage &initialDepletionStage = m_model.InitialDepletionStage();

    if (pFault->Slip() && pFault->hasFaultParameters(initialDepletionStage)) {
      CGlobalUnitNode globalUnitNode;

      const CFaultParametersNode &faultParametersNode = pFault->FaultParameters(initialDepletionStage);

      if (faultParametersNode.DistributedCohesion()) {
        addLine(text, ", cohesion: pointset ");
      } else {
        QString cohesionUnit = faultParametersNode.Cohesion().UnitName(globalUnitNode.unitNode().Unit()).c_str();

        double cohesion = faultParametersNode.Cohesion().Value(globalUnitNode.unitNode().Unit());
      ...
      if (faultParametersNode.DistributedFriction()) {
        ...
        double frictionAngle = faultParametersNode.FrictionAngle().Value(globalUnitNode.unitNode().Unit());
        QString frictionAngleUnit =
            faultParametersNode.FrictionAngle().UnitName(globalUnitNode.unitNode().Unit()).c_str();
