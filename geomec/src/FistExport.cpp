...
CFistExport::CLayer::CLayer(CFistExport &parent, QDomElement xmlparent, const well::CWellPoint &wellpoint,
                            double dStartAHD, double dEndAHD, const CModelBase &model,
                            TFormationNameMap &mpFormationNames)
    : m_parent(parent), m_wellpoint(wellpoint), m_pFormation(0), m_pElement(0), m_dStartAHD(dStartAHD),
      m_dEndAHD(dEndAHD), m_model(model), m_mpFormationNames(mpFormationNames) {
  FindFormationAndElement();
  CreateLayerDirectory(xmlparent);
}

const well::CWellPoint &CFistExport::CLayer::WellPoint() const { return m_wellpoint; }
...
double CFistExport::CLayer::StartAHD() const { return m_dStartAHD; }

double CFistExport::CLayer::EndAHD() const { return m_dEndAHD; }

const QString &CFistExport::CLayer::Name() const { return m_strName; }
...
//--- CNewLayer ---
//
CFistExport::CNewLayer::CNewLayer(CFistExport &parent, QDomElement xmlparent, const CNewWellPoint &wellpoint,
                                  double dStartAHD, double dEndAHD, const CModelBase &model,
                                  TFormationNameMap &mpFormationNames)
    : m_parent(parent), m_wellpoint(wellpoint), m_pFormation(0), m_pElement(0), m_dStartAHD(dStartAHD),
      m_dEndAHD(dEndAHD), m_model(model), m_mpFormationNames(mpFormationNames) {
...
const CNewWellPoint &CFistExport::CNewLayer::WellPoint() const { return m_wellpoint; }
...
double CFistExport::CNewLayer::StartAHD() const { return m_dStartAHD; }

double CFistExport::CNewLayer::EndAHD() const { return m_dEndAHD; }

const QString &CFistExport::CNewLayer::Name() const { return m_strName; }
...
void CFistExport::CNewLayer::CreateLayerDirectory(QDomElement xmlparent) {
  ...
  for (it = wp.FistDates().begin(); it != wp.FistDates().end(); ++it) {
    ...
    double dPressure =
        result_tree.PorePressure()
            .ResultAtTime(m_model.ResultRegister().NonLinear() ? CAnalysisType::AT_NONLIN : CAnalysisType::AT_LINEAR,
                          CGeomecTime(pDate->year(), pDate->month()), m_wellpoint)
            .Value();
...
CFistExport::CFistExport(const CModelBase *pModel, const CNewWellPath *pNewWellPath, bool bNewWellPaths)
    : CXMLBase("CASE_TREE"), m_pModel(pModel), m_pNewWellPath(pNewWellPath), m_bNewWellPaths(bNewWellPaths) {
...
void CFistExport::InitForNewWellPath() {
  ...
  for (itl = m_NewWellList.begin(); itl != m_NewWellList.end(); ++itl) {
    ...
    for (it = lstPoints.begin(); it != lstPoints.end(); ++it) {
      ...
      vcWellPoints.push_back(CNewGeoWellPoint(*const_cast<CModelBase *>(m_pModel), wellpoint));
    }
    ...
    for (size_t i = 0; i < vcFormationPoints.size(); ++i) {
      ...
      for (size_t j = 0; j < vcWellPoints.size(); ++j) {
        ...
        m_mpNewLayers.insert(
            TNewLayerMap::value_type(vcWellPoints[j], CNewLayer(*this, Root(), vcWellPoints[j], dLayerStartAHD,
                                                                dLayerEndAHD, *m_pModel, mpFormationNames)));
      }
    }
...
void CFistExport::CreateWellDirectory(const CNewWellPath *pPath) {
  ...
  for (it_pt = point_list.begin(); it_pt != point_list.end(); ++it_pt) {
    ...
    if (formation.Depleting() && (formtype == CFormationBase::FT_SAND || formtype == CFormationBase::FT_SANDSTONE ||
  ...
  for (i = 0, it_pt = point_list.begin(); it_pt != point_list.end(); ++it_pt, ++i) {
    ...
    for (it_date = pPath->FistDates().begin(); it_date != pPath->FistDates().end(); ++it_date) {
...
// wjrx mantis 3564
QPair<double, double> CFistExport::FindProducingInterval(int nIndex, const CNewWellPath *pPath) const {
  const std::list<CNewWellPoint> &point_list = pPath->FistExportPoints();
  ...
  std::list<CNewWellPoint>::const_iterator it;
  int ii;
  for (ii = 0, it = point_list.begin(); it != point_list.end(); ++ii, ++it) {
    if (ii == nIndex) {
      const CNewWellPoint &wellpoint = *it;
...
CStressTensor CFistExport::TotalStress(const geo::IPoint *pPoint, const QDate &date) const {
  ...
  double xx = result_tree.TotalStress()
                  .Components()
                  .ResultAtTime(antype, CGeomecTime(date.year(), date.month()), *pPoint, CDoubleQuantity::SI_UNIT,
                                ITensorGroup::CComponentComposite::TC_XX)
                  .Value();
  double yy = result_tree.TotalStress()
                  .Components()
                  .ResultAtTime(antype, CGeomecTime(date.year(), date.month()), *pPoint, CDoubleQuantity::SI_UNIT,
                                ITensorGroup::CComponentComposite::TC_YY)
                  .Value();
  double zz = result_tree.TotalStress()
                  .Components()
                  .ResultAtTime(antype, CGeomecTime(date.year(), date.month()), *pPoint, CDoubleQuantity::SI_UNIT,
                                ITensorGroup::CComponentComposite::TC_ZZ)
                  .Value();
  double xy = result_tree.TotalStress()
                  .Components()
                  .ResultAtTime(antype, CGeomecTime(date.year(), date.month()), *pPoint, CDoubleQuantity::SI_UNIT,
                                ITensorGroup::CComponentComposite::TC_XY)
                  .Value();
  double yz = result_tree.TotalStress()
                  .Components()
                  .ResultAtTime(antype, CGeomecTime(date.year(), date.month()), *pPoint, CDoubleQuantity::SI_UNIT,
                                ITensorGroup::CComponentComposite::TC_YZ)
                  .Value();
  double zx = result_tree.TotalStress()
                  .Components()
                  .ResultAtTime(antype, CGeomecTime(date.year(), date.month()), *pPoint, CDoubleQuantity::SI_UNIT,
