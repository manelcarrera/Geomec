...
SbPlane MeshCrossSection::getDefaultPlane() { return SbPlane(SbVec3f(1.0f, 0.0f, 0.0f), 0.0f); }

SbVec3f MeshCrossSection::getDefaultBasePoint() { return SbVec3f(0.0f, 0.0f, 0.0f); }

SbVec3f MeshCrossSection::getDefaultNormal() { return SbVec3f(0.0f, 1.0f, 0.0f); }

void MeshCrossSection::basepointChanged() { planeChanged(); }
...
void MeshCrossSection::planeChanged() {
  if (!m_updatingFromParent) {
    ...
    m_parent->SetScreen(geo::CPoint(p[0] + t[0], p[1] + t[1], p[2] + t[2]), geo::CVector(n[0], n[1], n[2]), false);
  }
}

void MeshCrossSection::initClass() { SO_NODE_INIT_CLASS(MeshCrossSection, SoGroup, "Group"); }

void MeshCrossSection::exitClass() { SO__NODE_EXIT_CLASS(MeshCrossSection); }
...
MeshCrossSection *MeshCrossSection::build(OIDIMeshNodeManager &mgr,
                                          CCrossSection *parent) // TODO: is it redone when swapingof model ?
{
  ...
  for (size_t i = 0; i < n; ++i) {
    ...
    if (volumeMesh != 0) // && mesh != mgr.getMainVolumeMeshNode()->getMesh())
    {
      ...
      const CWellCasingModel *casing_model =
          dynamic_cast<const CWellCasingModel *>(&volumeMesh->getOpenGLNode()->Model());
...
MeshCrossSection::MeshCrossSection()
    : m_manager(0), m_parent(0), m_formationCrossSections(new SoGroup), m_clipPlane(new SoClipPlane),
      m_updatingFromParent(false), m_tensorVectorMode(TensorVectorFull) {
...
const CCrossSection *MeshCrossSection::getParent() const { return m_parent; }
...
CCrossSection::OrientationType MeshCrossSection::getOrientation() const { return m_parent->OrientationScreen(); }
