...
CHexaGUI::CHexaGUI(CFemAppGUI *parentGUI, CHexaModel *hexaModel)
    : C3DGUI(parentGUI, hexaModel), m_hexaModel(hexaModel) {}

bool CHexaGUI::CanConvertToTetra() const { return m_hexaModel->Mesh().CanCreateMesh() || m_hexaModel->Mesh().IsMesh(); }

void CHexaGUI::ConvertToTetraModel() { ConvertToTetra(); }
...
CGUIBase *CHexaGUI::ConvertToTetra() {
  ...
  // We treat it as a child model here, even though it may be a root model; but whatever decorators were used for the
  // current model should still hold in the new one
  CGoCadGUI *pTetra =
      new CGoCadGUI(parentGUI(), static_cast<CGoCadModel *>(IModelLifetimeFacade::NewChild(
                                     GOCAD_MODEL, m_hexaModel->Logger(), m_hexaModel->getVersionManager())));
...
void CHexaGUI::OnCloseModel() { C3DGUI::OnCloseModel(); }

CHexaModel *CHexaGUI::model() const { return m_hexaModel; }
...
bool CHexaGUI::LoadPre381Stream(CStorageNode::TSTREAM &stream, CStreamVersion &version, CStorageNode::TPROGRESS &prog) {
  ...
  if ((CStreamVersion(3, 0, 48) < version) && (version < CStreamVersion(3, 8, 1))) {
    LoadScenes(stream, version, prog);
  }
