...
int CFemAppGUI::childGUISize() const
{
  return m_childGUI.size();
}

CFemAppGUI& CFemAppGUI::childGUI(int index)
{
  return *m_childGUI[index];
}

CFemAppGUI* CFemAppGUI::parentGUI()
{
  return m_parentGUI;
}
...
void CFemAppGUI::OnCreateSceneEntry()
{
  new TSceneNodeEntry(MD_BASE_SCENE_NODE, 0, 0, *m_femAppModel);
}
...
CFemAppModel* CFemAppGUI::model() const
{
  return m_femAppModel;
}

// CFemAppGUI::Observer

CFemAppGUI::Observer::Observer(CFemAppGUI& femAppGUI,
  const QString& strInstanceName)
: CGraphNode(strInstanceName)
, m_femAppGUI(femAppGUI)
{
}

CFemAppGUI::Observer::Observer(CFemAppGUI& femAppGUI,
  unsigned int uInstanceName)
...
void CFemAppGUI::Observer::OnNeighbourModified(const CGraphNode& node,
...
unsigned int CFemAppGUI::Observer::IconId() const
{
  return 0;
}

unsigned int CFemAppGUI::Observer::TypeId() const
{
  return 0;
}
