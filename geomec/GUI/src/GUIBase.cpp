...
CGUIBase::CGUIBase(CFemAppGUI* parentGUI, CModelBase* modelBase)
  : CFemAppGUI(parentGUI, modelBase)
...
{
  CPropertyPage* pPage =
  new CModelGeneralPg((CGeomecDoc&) (*GetGeomecDoc()));
...
void CGUIBase::MeshRefinement()
{ 
  m_pOpenInventorScene->show_refinement_box_dlg();
}
...
bool insertOneDeviatedWellPath(CModelBase* modelBase,
  QSharedPointer <IProgressBase>& progress, const QString& pathName,
...
bool insertOneDeviatedWellPathFromPetrel(CModelBase* modelBase,
  QSharedPointer <IProgressBase>& progress, const QString& pathName)
{
  QString sPath, sFile;
  ...
  CQuantity::UNIT defaultLateralUnit = CQuantity::SI_UNIT,
  defaultDepthUnit = CQuantity::SI_UNIT;

  {
  IProgressBase progress2;
  CImportPetrel importPetrel(sFile.toStdString().c_str(), progress2,
      defaultLateralUnit, defaultDepthUnit);

  if (importPetrel.Import())
  {
      defaultLateralUnit =
    importPetrel.getLateralUnit() == CImportPetrel::e_si_unit ?
    CQuantity::SI_UNIT : CQuantity::FIELD_UNIT;
      defaultDepthUnit =
    importPetrel.getDepthUnit() == CImportPetrel::e_si_unit ?
    CQuantity::SI_UNIT : CQuantity::FIELD_UNIT;
  }
  }

  CPetrelFileUnitDialog petrelFileUnitDialog(sFile.toStdString().c_str(),
  defaultLateralUnit, defaultDepthUnit);

  petrelFileUnitDialog.DoModal();

  if (!CNewWellPathEntry::ImportPetrelFile(pathName, *progress,
  *modelBase, petrelFileUnitDialog.getLateralUnit(),
...
void CGUIBase::InsertDeviatedWellPath(bool bInsertByPointSet/*=false*/)
{
  ...
  for (size_t i = 0; i < vcPathName.size(); i++)
  {
      ...
      {
    if (!insertOneDeviatedWellPathFromPetrel(m_modelBase, progress,
          vcPathName[i]))
    {
          break;
    }
      ...
      {
    if (!insertOneDeviatedWellPath(m_modelBase, progress, vcPathName[i],
          bInsertByPointSet))
    {
          break;
    }
...
void CGUIBase::ImportResults()
{
  ...
  OFN_ALLOWMULTISELECT | OFN_HIDEREADONLY, "All Result Files (*.str;*.bin;*.ff)|"
...
{
  if (m_modelBase->RedrawingDisabled())
  { // hajx: not sure why this is done; there seems to be no "return to automatic redrawing"
...
CModelBase* CGUIBase::model() const
{
  return m_modelBase;
}

bool CGUIBase::Load(CStorageNode::TSTREAM &stream, CStreamVersion &version, CStorageNode::TPROGRESS &prog, bool bLoadHeader)
{
  bool bRet;
...
void CGUIBase::SaveFileHeader(CStorageNode::TSTREAM &stream,
...
void CGUIBase::LoadScenes(CStorageNode::TSTREAM &stream, CStreamVersion &version, CStorageNode::TPROGRESS& progress, bool bIgnore)
{
  if (m_b_GUI_disabled)
    return;
...
bool CGUIBase::CanSwitchTo() const
{
  return m_modelBase->CanSwitchTo();
}
...
COpenInventorSceneNode& CGUIBase::OpenInventorScene()
{
     return * getDefaultOpenInventorSceneNode();
}

///// CGUIBase::CChildModelLoadSave
CGUIBase::CChildModelLoadSave::CChildModelLoadSave(CGUIBase& gui)
: m_gui(gui)
{
}

bool CGUIBase::CChildModelLoadSave::LoadModel(CStorageNode::TSTREAM& stream, CStreamVersion& version, CStorageNode::TPROGRESS& prog, CModelBase& childModel)
{
...
bool CGUIBase::CChildModelLoadSave::SaveModel(CStorageNode::TSTREAM &stream, CStorageNode::TPROGRESS& prog, CModelBase& childModel)
{
  CGUIBase* pChildGUI = getChildGUI(childModel);
...
bool CGUIBase::CChildModelLoadSave::SupportGMP() const
{
  return true;
}
...
CGUIBase::CMainModelLoadSave::CMainModelLoadSave(CGUIBase *pGUI)
  : m_gui(pGUI)
  , m_pVersionManager(0)
{
}
...
bool CGUIBase::CMainModelLoadSave::LoadModel(CStorageNode::TSTREAM& stream, CStreamVersion& version, CStorageNode::TPROGRESS& prog, CModelBase& model)
{
  assert(!m_gui);
...
bool CGUIBase::CMainModelLoadSave::SaveModel(CStorageNode::TSTREAM &stream, CStorageNode::TPROGRESS& prog, CModelBase& model)
{
  assert(m_gui);
...
bool CGUIBase::CMainModelLoadSave::SupportGMP() const
{
  return true;
}
...
CGUIBase *CGUIBase::CMainModelLoadSave::GetGUI() const
{
  return m_gui;
}
...
COpenInventorSceneNode * CGUIBase::getDefaultOpenInventorSceneNode() const
{
  return m_pOpenInventorScene;
}
