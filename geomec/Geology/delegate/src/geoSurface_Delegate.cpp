...
CSurfaceBase_Delegate::CSurfaceBase_Delegate(CSurfaceBase *surfaceBase)
    : IElementSet_Delegate(surfaceBase), m_surfaceBase(surfaceBase) {}
...
void CSurfaceBase_Delegate::AppendContextMenu(CContextMenuInvoker &invoker) {
  ...
  invoker.AddCommand(_T("&Export"), *(new TSurfaceCommand(*this, &CSurfaceBase_Delegate::ExportSurface)));
  invoker.AddSeparator();

  IElementSet_Delegate::AppendContextMenu(invoker);
}

bool CSurfaceBase_Delegate::CanDestroy() const { return m_surfaceBase->CanDestroy(); }

void CSurfaceBase_Delegate::ExportSurface() {
  QString sFileName = m_surfaceBase->Name();
  CTnoFileDialog dlg(FALSE, "ts", sFileName.toStdString().c_str(), OFN_OVERWRITEPROMPT,
                     "GoCad surface (*.ts)|*.ts|ASCII Files (*.dat;*.txt)|"
  ...
  if (dlg.DoModal() == IDOK) {
    ...
    m_surfaceBase->ExportSurface(sExt, doc.UnitNode(), (LPCSTR)dlg.GetPathName());
  }
}

CSurfaceEntry_Delegate::CSurfaceEntry_Delegate(CSurfaceEntry *surfaceEntry)
    : CStorageNodeEntry_Delegate<CSurfaceBase>(surfaceEntry), m_surfaceEntry(surfaceEntry) {}
...
void CSurfaceEntry_Delegate::AppendContextMenu(CContextMenuInvoker &invoker) {
  ...
  typedef CSingleCommandTemplate<CSurfaceEntry_Delegate> TEntry_DelegateCommand;

  invoker.AddCommand(_T("&Import..."), *(new TEntry_DelegateCommand(*this, &CSurfaceEntry_Delegate::Import,
                                                                    &CSurfaceEntry_Delegate::CanImport)));
  invoker.AddSeparator();
  invoker.AddCommand(_T("Export surfaces"), *(new TEntry_DelegateCommand(*this, &CSurfaceEntry_Delegate::ExportSurfaces,
                                                                         &CSurfaceEntry_Delegate::GetSurfaceCount)));
  invoker.AddCommand(_T("Delete unused surfaces"), *(new TEntryCommand(*m_surfaceEntry, &CSurfaceEntry::RemoveUnused,
                                                                       &CSurfaceEntry::NonUsedSurface)));
  invoker.AddCommand(_T("Unassign surfaces"),
                     *(new TEntry_DelegateCommand(*this, &CSurfaceEntry_Delegate::DoSelectAndUnassignSurfaces,
                                                  &CSurfaceEntry_Delegate::CanSelectAndUnassignSurfaces)));
  invoker.AddCommand(_T("Delete surfaces"),
                     *(new TEntry_DelegateCommand(*this, &CSurfaceEntry_Delegate::DoSelectAndDeleteSurfaces,
                                                  &CSurfaceEntry_Delegate::CanSelectAndDeleteSurfaces)));
  invoker.AddSeparator();
  invoker.AddCommand(_T("Create horizons"),
                     *(new TEntry_DelegateCommand(*this, &CSurfaceEntry_Delegate::DoSelectAndCreateHorizons,
                                                  &CSurfaceEntry_Delegate::NonUsedSurface)));
  invoker.AddCommand(_T("Create boundary sides"),
                     *(new TEntry_DelegateCommand(*this, &CSurfaceEntry_Delegate::DoSelectAndCreateSubBoundarySides,
                                                  &CSurfaceEntry_Delegate::CanSelectAndCreateSubBoundarySides)));
  invoker.AddCommand(_T("Auto-assign boundary top and bottom"),
                     *(new TEntry_DelegateCommand(*this, &CSurfaceEntry_Delegate::DoCreateSubBoundaryTopAndBottom,
                                                  &CSurfaceEntry_Delegate::CanCreateSubBoundaryTopAndBottom)));
  invoker.AddCommand(_T("Create faults"),
                     *(new TEntry_DelegateCommand(*this, &CSurfaceEntry_Delegate::DoSelectAndCreateFaults,
                                                  &CSurfaceEntry_Delegate::CanSelectAndCreateFaults)));
  invoker.AddSeparator();
  invoker.AddCommand(_T("Merge surfaces in faults"),
                     *(new TEntry_DelegateCommand(*this, &CSurfaceEntry_Delegate::DoMergeFaultSurfaces,
                                                  &CSurfaceEntry_Delegate::CanMergeFaultSurfaces)));
  invoker.AddCommand(_T("Merge surfaces"), *(new TEntry_DelegateCommand(*this, &CSurfaceEntry_Delegate::MergeSurfaces,
                                                                        &CSurfaceEntry_Delegate::GetSurfaceCount)));
...
#endif
    invoker.AddSeparator();
    invoker.AddCommand(_T("Cut surfaces"), *(new TEntry_DelegateCommand(*this, &CSurfaceEntry_Delegate::CutSurfaces,
                                                                        &CSurfaceEntry_Delegate::NonUsedSurface)));
#ifdef ZOOMIN_KEYFILE
  }
#endif
...
bool CSurfaceEntry_Delegate::CanImport() const { return GetGeomecDoc()->Model() != 0; }

/// \brief Import (surface) files
void CSurfaceEntry_Delegate::Import() { GetGeomecDoc()->OnImportSurface(); }

bool CSurfaceEntry_Delegate::GetSurfaceCount() const { return m_surfaceEntry->GetSurfaceCount(); }
...
bool CSurfaceEntry_Delegate::NonUsedSurface() const { return m_surfaceEntry->NonUsedSurface(); }
...
bool CSurfaceEntry_Delegate::CanSelectAndDeleteSurfaces() const { return m_surfaceEntry->CanSelectAndDeleteSurfaces(); }

void CSurfaceEntry_Delegate::DoSelectAndDeleteSurfaces() {
  CNamedSurfaces selected_surfaces;

  SelectSurfaces(selected_surfaces, NOT_USED_IN_MODEL,
                 "Select unassigned surfaces for deletion ***WARNING: deletion will be permanent***");
...
void CSurfaceEntry_Delegate::CutSurfaces() { DoCutSurfaces(); }
...
void CSurfaceEntry_Delegate::CutSurfacesBB(const geo::CBox &box) { DoCutSurfaces(&box); }
...
void CSurfaceEntry_Delegate::DoCutSurfaces(const geo::CBox *pbox) {
  ...
  CModelBase *pModel = static_cast<CModelBase *>(pDoc->Model());
  assert(pModel);
  m_surfaceEntry->DoCutSurfaces(*pModel, pDlg->CutSurfacesCount(), pDlg->getCutSurfaces(), pDlg->getSideSurfaces(),
                                (LPCSTR)pDlg->BaseName(), pDlg->GetSides(), pbox);
...
void CSurfaceEntry_Delegate::SelectSurfaces(CNamedSurfaces &ret, eCheckUsed check, const char *caption) {
  ...
  for (CSurfaceEntry::TNodeSet::const_iterator it = stSurfaces.begin(); it != stSurfaces.end(); it++) {
    switch (check) {
    case USED_IN_MODEL:
      if ((*it)->CanDestroy()) {
        continue;
    ...
    const geo::ISurface *surf = dynamic_cast<const geo::ISurface *>(&((*it)->Surface()));
