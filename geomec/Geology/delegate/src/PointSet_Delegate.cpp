...
CPointSet_Delegate::CPointSet_Delegate(CPointSet *pointSet) : IPointSet_Delegate(pointSet), m_pointSet(pointSet) {}
...
void CPointSet_Delegate::AppendContextMenu(CContextMenuInvoker &invoker) {
  ...
  invoker.AddCommand(_T("&Export"), *(new TPointSet_DelegateCommand(*this, &CPointSet_Delegate::Export)));

  if (m_pointSet->Dimension() == 2 || m_pointSet->Dimension() == 3) {
    // wjrx mantis 3186

    invoker.AddCommand(_T("&Sample"), *(new TPointSet_DelegateCommand(*this, &CPointSet_Delegate::Sample)));

#if _DEBUG
    invoker.AddCommand(_T("Delete Convex &Hull"),
                       *(new TPointSet_DelegateCommand(*this, &CPointSet_Delegate::DestroyConvexHull,
                                                       &CPointSet_Delegate::CanDestroyConvexHull)));
#endif
  }
...
bool CPointSet_Delegate::CanDestroy() const { return m_pointSet->CanDestroy(); }
...
void CPointSet_Delegate::Sample() {
  ...
  if (sampleDlg.DoModal() == IDOK) {
    m_pointSet->Sample((LPCSTR)sampleDlg.getName(), sampleDlg.getSizeNorth(), sampleDlg.getSizeEast(),
                       sampleDlg.getSizeDepth());
  }
}

bool CPointSet_Delegate::CanDestroyConvexHull() const { return m_pointSet->HasHull(); }

void CPointSet_Delegate::DestroyConvexHull() { m_pointSet->DestroyHull(); }

CPointSet_Delegate::CCoordinateType_Delegate::CCoordinateType_Delegate(CPointSet::CCoordinateType *coordinateType)
    : CValueType_Delegate(coordinateType), m_coordinateType(coordinateType) {}

bool CPointSet_Delegate::CCoordinateType_Delegate::CanDestroy() const { return m_coordinateType->CanDestroy(); }
