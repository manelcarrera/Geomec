...
CNewWellPath_Delegate::CNewWellPath_Delegate(CNewWellPath *newWellPath)
    : CColorNode_Delegate(newWellPath), m_pNewWellPath(newWellPath) {}

void CNewWellPath_Delegate::AppendContextMenu(CContextMenuInvoker &invoker) {
  typedef CSingleCommandTemplate<CNewWellPath_Delegate> TNewWellPath_DelegateCommand;
  invoker.AddCommand(_T("Show wellpath information"),
                     *(new CSingleCommandTemplate<CNewWellPath>(*m_pNewWellPath, &CNewWellPath::ToggleDrawWellInfo, 0,
                                                                &CNewWellPath::DrawWellInfo)));
  ...
  invoker.AddCommand(_T("Generate IDM input"),
                     *(new TNewWellPath_DelegateCommand(*this, &CNewWellPath_Delegate::GenerateIDMInput)));

  invoker.AddSeparator();
  // wjrx mantis 3564
  //
  invoker.AddCommand(_T("Select points for &FIST export"),
                     *(new TNewWellPath_DelegateCommand(*this, &CNewWellPath_Delegate::DefineFistPoints,
                                                        &CNewWellPath_Delegate::CanDefineFistPoints)));
  invoker.AddCommand(_T("&Export to FIST"),
                     *(new TNewWellPath_DelegateCommand(*this, &CNewWellPath_Delegate::ExportToFist,
                                                        &CNewWellPath_Delegate::CanExportToFist)));

  invoker.AddCommand(_T("Create zoom-in model"),
                     *new TNewWellPath_DelegateCommand(*this, &CNewWellPath_Delegate::CreateZoomInModel,
...
bool CNewWellPath_Delegate::Edit() {
  CNewWellPathEntry *pEntry = (CNewWellPathEntry *)(m_pNewWellPath->Model().GraphEntry(MD_NEW_WELLPATH));
  CNewWellPathEntry_Delegate *pEntry_Delegate = static_cast<CNewWellPathEntry_Delegate *>(pEntry->getDelegate());
...
bool CNewWellPath_Delegate::CanZoomIn() const { return m_pNewWellPath->CanZoomIn(); }

/// \brief Create Zoom-in model
void CNewWellPath_Delegate::CreateZoomInModel() { m_pNewWellPath->CreateZoomInModel(); }
...
bool CNewWellPath_Delegate::CanExportLAS() const { return true; }

bool CNewWellPath_Delegate::CanImportLAS() const { return m_pNewWellPath->CanImportLAS(); }

void CNewWellPath_Delegate::ImportLAS() {
  QString sFilter = "Log ASCII Standard files (*.las)|*.las|All Files (*.*)|*.*||";
  CTnoFileDialog dlg(true, "las", 0, OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, sFilter.toStdString().c_str(),
                     FemAppGetMainWnd());
...
CNewWellPathEntry_Delegate::CNewWellPathEntry_Delegate(CNewWellPathEntry *newWellPathEntry)
    : CStorageNodeEntry_Delegate<CNewWellPath>(newWellPathEntry), m_newWellPathEntry(newWellPathEntry) {}
...
bool CNewWellPath_Delegate::CanDefineFistPoints() const {
  ...
  if (pApp->KeyFile().Unlocked() == false)
    return false;
  return true;
#else
  return true;
...
void CNewWellPath_Delegate::DefineFistPoints() {
  ...
  CFistPointPickDlg dlg(&m_pNewWellPath->FistExportPoints(), *m_pNewWellPath, Sections, unitNode.Unit());
...
void CNewWellPath_Delegate::ExportToFist() {
  CFistExportDlg dlg(m_pNewWellPath, (const CModelBase *)(&m_pNewWellPath->Model()));
...
void CNewWellPath_Delegate::GenerateIDMInput() {
  ...
  CDsbPickPointsDlg dlg(&m_pNewWellPath->DStaborPoints(), *m_pNewWellPath, Sections, unitNode.Unit());

  dlg.WellPointListCtrl().ShowTMDOnly();
  if (dlg.DoModal() == IDOK && m_pNewWellPath->DStaborPoints().empty() == false) {
    // wjrx mantis 3680
    CStartIDMStaborDlg StartDlg(&m_pNewWellPath->DStaborPoints(), *pModel);
    StartDlg.DoModal();
  }
...
void CNewWellPathEntry_Delegate::AppendContextMenu(CContextMenuInvoker &invoker) {
  invoker.AddCommand(_T("Show wellpath information"), *(new CSingleCommandTemplate<CNewWellPathEntry>(
                                                          *m_newWellPathEntry, &CNewWellPathEntry::ToggleDrawWellInfo,
                                                          0, &CNewWellPathEntry::DrawWellInfo)));
  invoker.AddSeparator();

  invoker.AddCommand(_T("Import deviated wellpath 3d &point set"),
                     *(new CSingleCommandTemplate<CNewWellPathEntry_Delegate>(
                         *this, &CNewWellPathEntry_Delegate::InsertDeviatedWellPathByPointSet,
  ...
  pSubMenu->AddCommand(_T("&Graphically"), *(new CSingleCommandTemplate<CNewWellPathEntry_Delegate>(
                                               *this, &CNewWellPathEntry_Delegate::InsertVerticalWellPathGraphical,
                                               &CNewWellPathEntry_Delegate::CanInsertNew)));
  pSubMenu->AddCommand(_T("&By value"), *(new CSingleCommandTemplate<CNewWellPathEntry_Delegate>(
                                            *this, &CNewWellPathEntry_Delegate::InsertVerticalWellPathByValue,
                                            &CNewWellPathEntry_Delegate::CanInsertNew)));
  ...
  invoker.AddCommand(_T("&Export"),
                     *new CSingleCommandTemplate<CNewWellPathEntry_Delegate>(*this, &CNewWellPathEntry_Delegate::Export,
                                                                             &CNewWellPathEntry_Delegate::CanExport));

  invoker.AddCommand(_T("Export to &FIST"), *(new CSingleCommandTemplate<CNewWellPathEntry_Delegate>(
                                                *this, &CNewWellPathEntry_Delegate::ExportToFist,
                                                &CNewWellPathEntry_Delegate::CanExportToFist)));

  invoker.AddSeparator();

  invoker.AddCommand(_T("Add group"), *(new CSingleCommandTemplate<CNewWellPathEntry_Delegate>(
                                          *this, &CNewWellPathEntry_Delegate::InsertGroup)));
}
...
bool CNewWellPathEntry_Delegate::CanExportToFist() const { return m_newWellPathEntry->CanExportToFist(); }

void CNewWellPathEntry_Delegate::ExportToFist() {
  CFistExportDlg dlg(true, (const CModelBase *)(&m_newWellPathEntry->Model()));

  dlg.DoModal();
}

bool CNewWellPathEntry_Delegate::CanInsertNew() const { return m_newWellPathEntry->CanInsertNew(); }
...
void CNewWellPathEntry_Delegate::InsertVerticalWellPathByValue() {
  ...
  geo::CPoint point((model.Boundary().Max().X() + model.Boundary().Min().X()) / 2,
                    (model.Boundary().Max().Y() + model.Boundary().Min().Y()) / 2);

  // Create unique name

  QString sName = m_newWellPathEntry->createUniqueName(CNewWellPathEntry::DEFAULT_NEWWELL_NAME,
...
bool CNewWellPathEntry_Delegate::CanExport() const { return m_newWellPathEntry->CanExport(); }
