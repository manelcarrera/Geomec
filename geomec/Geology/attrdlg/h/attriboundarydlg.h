...
template <class T> class CAttriBoundaryDlg : public CGeomecAttriTemp<T> {
public:
  // Construction
...
template <class T> void CAttriBoundaryDlg<T>::DoDataExchange(CDataExchange *pDX) {
  ...
  if (pDX->m_bSaveAndValidate) {
    if (Copy().State() == CBoundaryBase::USER_DEFINED) {
      ...
      CBoundaryBase::TRotated rot =
          Copy().SnapToGrid(CBoundaryBase::TRotated(ptMid, dAzimuth, dLength, dWidth, dMinDepth, dMaxDepth));
      Copy().Set(rot.ptMid, rot.dAzimuth, rot.dLength, rot.dWidth, rot.dMinDepth, rot.dMaxDepth);
    }
  }
}

template <class T> BOOL CAttriBoundaryDlg<T>::OnCommand(WPARAM wParam, LPARAM lParam) {
  ...
  if (nNotification == EN_KILLFOCUS) {
    if ((nControlID == IDC_ED_MIN_EAST) || (nControlID == IDC_ED_MAX_EAST) || (nControlID == IDC_ED_MIN_NORTH) ||
...
template <class T> void CAttriBoundaryDlg<T>::OnBestFit() {
...
template <class T> void CAttriBoundaryDlg<T>::OnUserDefined() {
  if (Copy().State() != CBoundaryBase::USER_DEFINED) {
...
template <class T> void CAttriBoundaryDlg<T>::OnRotated() {
...
template <class T> void CAttriBoundaryDlg<T>::OnChangeColor() {
  Copy().Color(SelectColor(Copy().Color()));
  Invalidate();
}

template <class T> void CAttriBoundaryDlg<T>::OnPaint() {
  CPaintDC dc(this); // device context for painting

  FillFrame(IDC_ST_COLOR, Copy().Color(), dc);
}

template <class T> void CAttriBoundaryDlg<T>::OnKillFocusCoordinate() {
  UpdateData(TRUE);
  UpdateData(FALSE);
}

template <class T> void CAttriBoundaryDlg<T>::OnOK() {
  UpdateData(TRUE);
