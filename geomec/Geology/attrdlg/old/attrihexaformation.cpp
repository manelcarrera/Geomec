...
void CAttriHexaFormation::DoDataExchange(CDataExchange* pDX)
{
  ...
  if(!pDX->m_bSaveAndValidate)
  {
    ...
    strBottomName = Copy().LowerHorizon().Name();
    strTopName = Copy().UpperHorizon().Name();
  }
  ...
  DDV_MinMaxDouble(pDX, dMinThickness, Copy().MinThickness().MinValue( UnitNode().Unit()) , Copy().MinThickness().MaxValue( UnitNode().Unit()));
