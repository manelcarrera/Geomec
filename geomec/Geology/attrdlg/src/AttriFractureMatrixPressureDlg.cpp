...
CAttriFractureMatrixPressureDlg::CAttriFractureMatrixPressureDlg(CFractureMatrixPressure &prs, CWnd *pParent /*=NULL*/)
    : CAttriFormationLoadDlg<CFractureMatrixPressure>(prs, IDD_ATTRI_PRESSURE, pParent), m_bOk(false) {}
...
void CAttriFractureMatrixPressureDlg::DoDataExchange(CDataExchange *pDX) {
  ...
  bool bGetGWCValues = (Copy().Type() == CFractureMatrixPressure::GWC_MODE &&
...
CString CAttriFractureMatrixPressureDlg::UndefinedLabel() const { return _T("Equal to Pressure"); }

CString CAttriFractureMatrixPressureDlg::previousUndefinedLabel() const { return _T("Same as (fracture) pressure"); }
...
void CAttriFractureMatrixPressureDlg::UpdateControls() {
  ...
  GetDlgItem(IDC_ED_CONTACT_PRESSURE)
      ->EnableWindow((Copy().Type() == CFractureMatrixPressure::GWC_MODE) && !bDisableAll &&
                     (!Copy().DistributedOnly() || Copy().DistributedSize() == 0));
  ...
  GetDlgItem(IDC_ED_LOWER_GRADIENT)
      ->EnableWindow(Copy().Type() == CFractureMatrixPressure::GWC_MODE && Copy().GWC().IsFirstGWC() && !bDisableAll &&
                     (!Copy().DistributedOnly() || Copy().DistributedSize() == 0));
  ...
  GetDlgItem(IDC_FRAME_TRANS)
      ->EnableWindow((Copy().Type() == CFractureMatrixPressure::GWC_MODE) && (!Copy().GWC().IsFirstGWC()) &&
                     !bDisableAll && (!Copy().DistributedOnly() || Copy().DistributedSize() == 0));
