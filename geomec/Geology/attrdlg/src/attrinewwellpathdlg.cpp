...
: CDialog(IDD_ATTRI_NEWWELLPATH, pParent), 
m_well(well),
m_color(well.Color())
{
...
void CAttriNewWellPathDlg::DoDataExchange(CDataExchange* pDX)
{
  ...
  DDV_Quantity(pDX, IDC_ED_NEWWELLPATH_WH_NORTHING
      , &m_dNorthing, globalUnitNode.unitNode().Unit(),"Northing");
  DDV_Quantity(pDX, IDC_ED_NEWWELLPATH_WH_EASTING
      , &m_dEasting, globalUnitNode.unitNode().Unit(),"Easting");
  DDV_Quantity(pDX, IDC_ED_NEWWELLPATH_DER_ELEV
      , &m_dDerElev, globalUnitNode.unitNode().Unit(),"Derrick elevation");
...
void CAttriNewWellPathDlg::OnKillfocusEdName() 
{
  UpdateData(TRUE);
}
