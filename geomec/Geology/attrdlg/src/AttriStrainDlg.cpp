...
: CAttriFormationLoadDlg<CStrainLoad>(strain, IDD_ATTRI_STRAIN_DLG, pParent), m_changingMode(false), m_dVolume(0), m_dNormal(0), m_dLateral(0), m_dDepth(0), m_dGradient(0)
{
}
...
void CAttriStrainDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	if(pDX->m_bSaveAndValidate)
  {
    ...
    if (m_changingMode && Copy().Type() == CStrainLoad::CONSTANT_MODE)
    { // we're going to REPEAT_MODE, so set the checkboxes to false
        Copy().Constant().UseVolumetric(false);
        Copy().Constant().UseInPlane(false);
    }
...
CString CAttriStrainDlg::UndefinedLabel() const
{
  return _T("Unused (0)");
}
...
void CAttriStrainDlg::OnPreDefinedConstant() 
{
	...
	Copy().Type(CStrainLoad::CONSTANT_MODE); // We're doing this in DoDataExchange now, to always allow default values when DistributedOnly is false
	UpdateControls();
}
