...
QString  CValueTypeTreeObject::Text() const
{
  return ValueType().Name();
}

BOOL  CValueTypeTreeObject::CanEditText() const
{
  return TRUE;
}
...
/* NOT OBSOLETE */ void CValueTypeTreeObject::AppendContextMenu(CContextMenuInvoker &invoker)
{
  ...
  invoker.AddCommand(_T("&Remove"),*(new TCommandTemp(*this,&CValueTypeTreeObject::Delete, &CValueTypeTreeObject::CanRemove)));
}
...
const CValueType&  CValueTypeTreeObject::ValueType() const
{
  return m_value_type;
}

CValueType&  CValueTypeTreeObject::ValueType()
{
  return m_value_type;
}

const CTreeCtrl& CValueTypeTreeObject::TreeCtrl() const
{
  return m_tree;
}

CTreeCtrl& CValueTypeTreeObject::TreeCtrl()
{
  return m_tree;
}

const CListCtrl& CValueTypeTreeObject::ListCtrl() const
{
  return m_list;
}

CListCtrl& CValueTypeTreeObject::ListCtrl()
{
  return m_list;
}

//##ModelId=3C74E3FC004E
unsigned int  CValueTypeTreeObject::Icon() const
{
  return ValueType().IconId();
}
...
BOOL  CValueTypeTreeObject::OnBeginDrag()
{
  return FALSE;
}

//##ModelId=3C74E3FC0041
BOOL  CValueTypeTreeObject::CanDelete() const
{
  return ValueType().CanDestroy();
}
...
: IListObject(ctrl), m_value_set(value_set)
...
QString  CValueSetListObject::Text() const
{
  return ValueSet().Name();
}

//##ModelId=3C74E3FC01BA
unsigned int  CValueSetListObject::Icon() const
{
  return ValueSet().IconId();
}
 
//##ModelId=3C74E3FC01C5
BOOL  CValueSetListObject::CanEditText() const
{
  return TRUE;
}

BOOL  CValueSetListObject::CanDelete() const
{
  return TRUE;
}
...
/* NOT OBSOLETE */ void CValueSetListObject::AppendContextMenu(CContextMenuInvoker &invoker)
{
  ...
  invoker.AddCommand(_T("&Remove"),*(new TCommandTemp(*this,&CValueSetListObject::Delete, &CValueSetListObject::CanRemove)));
  ...
  invoker.AddCommand(_T("&Attributes"),*(new TValueSetTemp_Delegate(*valueSet_Delegate, &IValueSet_Delegate::Attributes)));
}

 CComponentTreeObject::CComponentTreeObject( CValueTypeTreeObject &root, int nIndex)
: ITreeObject(root, TVI_LAST), m_nIndex(nIndex)
, m_valueSet_Delegate(0)
{
  Ctrl().SetItemData(Handle(), (DWORD_PTR)(this));
  Update();
...
BOOL  CComponentTreeObject::CanEditText() const
{
...
int CComponentTreeObject::Index() const
{
  return m_nIndex;
}
...
unsigned int  CComponentTreeObject::Icon() const
{
  return Component().IconId();
}

//##ModelId=3C74E3FC011C
BOOL  CComponentTreeObject::OnBeginDrag()
{
  return Component().Defined();
}
...
DROPEFFECT  CComponentTreeObject::CanDrop(TCtrlObjectVec &vcDragged, BOOL bMove) const
{
  ...
  {
    if((pTreeSet != this) &&
       (pTreeSet->Component().Defined()) && 
       (!ValueTypeTreeObject().IsCoordinate() || (dynamic_cast<const CRpnValueSet*>(&pTreeSet->Component().ValueSet()) == 0)))
      return DROPEFFECT_MOVE;
  }

  CValueSetListObject *pListSet = dynamic_cast<CValueSetListObject*>(vcDragged[0]);

  if(pListSet && (!ValueTypeTreeObject().IsCoordinate() || (dynamic_cast<const CRpnValueSet*>(&pListSet->ValueSet()) == 0)))
    return DROPEFFECT_MOVE;
...
BOOL  CComponentTreeObject::CanDelete() const
{
  return Component().Defined();
}
...
bool CComponentTreeObject::IsInRange(const IValueSet& value_set) const
{
  ...
  if(!Component().IsInRange(value_set))
  {
    ...
    if(Component().RangeMax(doc.UnitNode().Unit()).Valid() && Component().RangeMin(doc.UnitNode().Unit()).Valid())
      sMessage.Format(IDS_COMPONENT_NOT_IN_RANGE_MIN_MAX, Component().RangeMin().Value(), Component().RangeMax().Value());
...
/* NOT OBSOLETE */ void CComponentTreeObject::AppendContextMenu(CContextMenuInvoker &invoker)
{
  ...
  typedef CSingleCommandTemplate<IValueSet> TValueSetTemp;
  typedef CSingleCommandTemplate <IValueSet_Delegate> TValueSetTemp_Delegate;
  invoker.AddCommand(_T("&Remove"),*(new TCommandTemp(*this,&CComponentTreeObject::Delete, &CComponentTreeObject::CanRemove)));
  if(Component().Defined())
  {
    invoker.AddSeparator();
  ...
  IValueSet_Delegate* valueSet_Delegate =
      static_cast <IValueSet_Delegate*> (Component().ValueSet().getDelegate());
