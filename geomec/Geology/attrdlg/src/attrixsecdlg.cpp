...
CAttriXSecDlg::CAttriXSecDlg(CCrossSection &Xsec, CWnd *pParent /*=NULL*/)
    : CDialog(CAttriXSecDlg::IDD, pParent), m_xsec(Xsec),
      m_basePointBookmarked(Xsec.IntersectPlaneBookmarked().BasePoint()), m_normalBookmarked(Xsec.NormalBookmarked()),
      m_normalBookmarkedSaved(Xsec.NormalBookmarked()), m_basePointScreen(Xsec.IntersectPlaneScreen().BasePoint()),
      m_normalScreen(Xsec.NormalScreen()), m_normalScreenSaved(Xsec.NormalScreen()),
      m_constraintBookmarked(Xsec.OrientationBookmarked()), m_constraintScreen(Xsec.OrientationScreen()),
      m_strName(Xsec.Name().toStdString().c_str()), m_bOk(false), m_ptFirst(), m_ptSecond(),
      m_dDepth(Xsec.IntersectPlaneScreen().BasePoint().Z()), m_crossSectionType(Xsec.crossSectionType()) {
  CCrossSection::calculateTwoArbitraryPointsInPlane(m_ptFirst, m_ptSecond, Xsec.IntersectPlaneScreen(),
...
void CAttriXSecDlg::DoDataExchange(CDataExchange *pDX) {
  ...
  if (!pDX->m_bSaveAndValidate) {
    ...
    dBasePointNorthBookmarked =
        qnLength.Convert(m_basePointBookmarked.X(), globalUnitNode.unitNode().Unit(), CQuantity::SI_UNIT);
    LimitDecimals(dBasePointNorthBookmarked, numDecimals);
    dBasePointEastBookmarked =
        qnLength.Convert(m_basePointBookmarked.Y(), globalUnitNode.unitNode().Unit(), CQuantity::SI_UNIT);
    LimitDecimals(dBasePointEastBookmarked, numDecimals);
    dBasePointDepthBookmarked =
        qnLength.Convert(m_basePointBookmarked.Z(), globalUnitNode.unitNode().Unit(), CQuantity::SI_UNIT);
    LimitDecimals(dBasePointDepthBookmarked, numDecimals);

    dNormalNorthBookmarked =
        qnLength.Convert(m_normalBookmarked.X(), globalUnitNode.unitNode().Unit(), CQuantity::SI_UNIT);
    LimitDecimals(dNormalNorthBookmarked, numDecimals);
    dNormalEastBookmarked =
        qnLength.Convert(m_normalBookmarked.Y(), globalUnitNode.unitNode().Unit(), CQuantity::SI_UNIT);
    LimitDecimals(dNormalEastBookmarked, numDecimals);
    dNormalDepthBookmarked =
        qnLength.Convert(m_normalBookmarked.Z(), globalUnitNode.unitNode().Unit(), CQuantity::SI_UNIT);
    LimitDecimals(dNormalDepthBookmarked, numDecimals);

    dBasePointNorthScreen =
        qnLength.Convert(m_basePointScreen.X(), globalUnitNode.unitNode().Unit(), CQuantity::SI_UNIT);
    LimitDecimals(dBasePointNorthScreen, numDecimals);
    dBasePointEastScreen =
        qnLength.Convert(m_basePointScreen.Y(), globalUnitNode.unitNode().Unit(), CQuantity::SI_UNIT);
    LimitDecimals(dBasePointEastScreen, numDecimals);
    dBasePointDepthScreen =
        qnLength.Convert(m_basePointScreen.Z(), globalUnitNode.unitNode().Unit(), CQuantity::SI_UNIT);
  ...
  dBasePointNorthBookmarked =
      qnLength.Convert(dBasePointNorthBookmarked, CQuantity::SI_UNIT, globalUnitNode.unitNode().Unit());
  dBasePointEastBookmarked =
      qnLength.Convert(dBasePointEastBookmarked, CQuantity::SI_UNIT, globalUnitNode.unitNode().Unit());
  dBasePointDepthBookmarked =
      qnLength.Convert(dBasePointDepthBookmarked, CQuantity::SI_UNIT, globalUnitNode.unitNode().Unit());

  dNormalNorthBookmarked =
      qnLength.Convert(dNormalNorthBookmarked, CQuantity::SI_UNIT, globalUnitNode.unitNode().Unit());
  dNormalEastBookmarked = qnLength.Convert(dNormalEastBookmarked, CQuantity::SI_UNIT, globalUnitNode.unitNode().Unit());
  dNormalDepthBookmarked =
      qnLength.Convert(dNormalDepthBookmarked, CQuantity::SI_UNIT, globalUnitNode.unitNode().Unit());
  ...
  if (pDX->m_bSaveAndValidate) {
    ...
    if (m_crossSectionType == 0) {
      ...
      bool bFirstNorth =
          (((0 <= dFirstNorth - dBoundMinX) && (dFirstNorth - dBoundMinX < EPS)) || (dFirstNorth > dBoundMinX)) &&
          (((0 <= dFirstNorth - dBoundMaxX) && (dFirstNorth - dBoundMaxX < EPS)) || (dFirstNorth < dBoundMaxX));
      bool bFirstEast =
          (((0 <= dFirstEast - dBoundMinY) && (dFirstEast - dBoundMinY < EPS)) || (dFirstEast > dBoundMinY)) &&
          (((0 <= dFirstEast - dBoundMaxY) && (dFirstEast - dBoundMaxY < EPS)) || (dFirstEast < dBoundMaxY));
      bool bSecondNorth =
          (((0 <= dSecondNorth - dBoundMinX) && (dSecondNorth - dBoundMinX < EPS)) || (dSecondNorth > dBoundMinX)) &&
          (((0 <= dSecondNorth - dBoundMaxX) && (dSecondNorth - dBoundMaxX < EPS)) || (dSecondNorth < dBoundMaxX));
      bool bSecondEast =
          (((0 <= dSecondEast - dBoundMinY) && (dSecondEast - dBoundMinY < EPS)) || (dSecondEast > dBoundMinY)) &&
...
bool CAttriXSecDlg::TryApply() {
  ...
  m_xsec.crossSectionType(CCrossSection::TCrossSectionType(m_crossSectionType));
  ...
  case 2:
    ...
    m_xsec.SetBookmarked(m_basePointBookmarked, m_normalBookmarked, m_constraintBookmarked);
    m_xsec.Clear();
    break;
  default:
    assert(false);
...
void CAttriXSecDlg::UpdateControls() {
  ...
  GetDlgItem(IDC_EDIT_SCREEN_NORMAL_NORTHING)
      ->EnableWindow((m_crossSectionType == 2) &&
                     OrientationIsSet(m_constraintScreen, 4, CCrossSection::ORIENTATION_ANY,
                                      CCrossSection::ORIENTATION_CURRENT, CCrossSection::ORIENTATION_NE,
                                      CCrossSection::ORIENTATION_NORTHING));
  GetDlgItem(IDC_EDIT_SCREEN_NORMAL_EASTING)
      ->EnableWindow((m_crossSectionType == 2) &&
                     OrientationIsSet(m_constraintScreen, 4, CCrossSection::ORIENTATION_ANY,
                                      CCrossSection::ORIENTATION_CURRENT, CCrossSection::ORIENTATION_NE,
                                      CCrossSection::ORIENTATION_EASTING));
  GetDlgItem(IDC_EDIT_SCREEN_NORMAL_DEPTH)
      ->EnableWindow((m_crossSectionType == 2) &&
                     OrientationIsSet(m_constraintScreen, 3, CCrossSection::ORIENTATION_ANY,
                                      CCrossSection::ORIENTATION_CURRENT, CCrossSection::ORIENTATION_DEPTH));

  GetDlgItem(IDC_EDIT_BM_NORMAL_NORTHING)
      ->EnableWindow((m_crossSectionType == 2) &&
                     OrientationIsSet(m_constraintBookmarked, 4, CCrossSection::ORIENTATION_ANY,
                                      CCrossSection::ORIENTATION_CURRENT, CCrossSection::ORIENTATION_NE,
                                      CCrossSection::ORIENTATION_NORTHING));
  GetDlgItem(IDC_EDIT_BM_NORMAL_EASTING)
      ->EnableWindow((m_crossSectionType == 2) &&
                     OrientationIsSet(m_constraintBookmarked, 4, CCrossSection::ORIENTATION_ANY,
                                      CCrossSection::ORIENTATION_CURRENT, CCrossSection::ORIENTATION_NE,
                                      CCrossSection::ORIENTATION_EASTING));
  GetDlgItem(IDC_EDIT_BM_NORMAL_DEPTH)
      ->EnableWindow((m_crossSectionType == 2) &&
                     OrientationIsSet(m_constraintBookmarked, 3, CCrossSection::ORIENTATION_ANY,
                                      CCrossSection::ORIENTATION_CURRENT, CCrossSection::ORIENTATION_DEPTH));
...
void KeepInBounds(const char *basePoint, const char *coordinate, double &value, double min, double max) {
  ...
  if (value < min || value > max) {
    ...
    {
      char message[256];
      sprintf(message, "Coordinate %s of Base Point %s is out of model bounds, moving it in bounds again", coordinate,
              basePoint);
      _m()->msg(message);
    }
