...
CAttriColorGradientDlg::CAttriColorGradientDlg(CColorGradient &color_gradient, CWnd *pParent /*=NULL*/)
    : CAttributesTemplate<CColorGradient>(CAttriColorGradientDlg::IDD, color_gradient, pParent), m_bInRect(FALSE),
      m_pToolTipCtrl(0) {
...
void CAttriColorGradientDlg::DoDataExchange(CDataExchange *pDX) {
  ...
  if (!pDX->m_bSaveAndValidate) {
    ...
    if (Copy().IsGlobal()) {
      ...
      SetWindowPos(0, rect.left, rect.top, rect.Width(), 4 * (rect.Height() / 5),
                   0); // Adjust size of dialog in case of local color scale...
    }
...
CRect CAttriColorGradientDlg::GetRect(CColorGradient::color_iterator it) {
  ...
  retrect.SetRect(leftpoint, rect.top, leftpoint + SMALLRECTWIDTH,
                  rect.bottom); // set the dimensions of the rectangle...
...
void CAttriColorGradientDlg::UpdateToolTip() {
  ...
  for (rectit = m_mpRectMap.begin(); rectit != m_mpRectMap.end(); rectit++) {
    ...
    {
      dValue = Copy().MinMaxValue().first +
               (Copy().MinMaxValue().second - Copy().MinMaxValue().first) *
                   (((rectit->first).left + HALFRECTWIDTH - rect.left) / ((rect.right - rect.left) * 1.0));
...
void CAttriColorGradientDlg::OnInsertColorInterval() {
  ...
  if (!Copy().AddValue(m_ptClicked.x - rcGDI.left, rcGDI.Width(),
                       qRgb(0, 255, 0))) // add interval and check if there are not too many already...
  {
    EnterPopup();
...
void CAttriColorGradientDlg::OnMouseMove(unsigned int nFlags, CPoint point) {
  if (m_bInRect) {
    ...
    if (m_rcTempRec.left + nOffset <=
        rcGDI.left + HALFRECTWIDTH) // check if new position of rectangle is too much to the left....
    {
      m_rcTempRec.SetRect(rcGDI.left + HALFRECTWIDTH, rcGDI.top, rcGDI.left + SMALLRECTWIDTH,
                          rcGDI.bottom);        // set explicitly...
      m_rcTempRec.OffsetRect(HALFRECTWIDTH, 0); // a little room bewtween the two rectangles..
    } else if (m_rcTempRec.right + nOffset >=
               rcGDI.right - 2) // check if new position of rectangle is too much to the right...
    {
      m_rcTempRec.SetRect(rcGDI.right - (SMALLRECTWIDTH + HALFRECTWIDTH), rcGDI.top, rcGDI.right - HALFRECTWIDTH,
                          rcGDI.bottom);         // set explicitly...
      m_rcTempRec.OffsetRect(-HALFRECTWIDTH, 0); // a little room bewtween the two rectangles..
    } else
...
void CAttriColorGradientDlg::OnLButtonUp(unsigned int nFlags, CPoint point) {
  if (m_bInRect) {
    ...
    double dValue = Copy().MinMaxValue().first +
                    (Copy().MinMaxValue().second - Copy().MinMaxValue().first) *
                        ((m_rcTempRec.left + HALFRECTWIDTH - rcGDI.left) / ((rcGDI.right - rcGDI.left) * 1.0));
