...
void CAttriFaultParameters::DoDataExchange(CDataExchange *pDX) {
  ...
  if (Copy().Fault().SlipType() != CHorizonBase::SLIP &&
      (!Copy().DistributedCohesion() || !Copy().CohesionExtrapolated()) && !Copy().CohesionSameAsPrevious()) {
    DDX_Text(pDX, IDC_ED_COHESION, dCohesion);
    DDV_MinMaxDouble(pDX, dCohesion, Copy().Cohesion().MinValue(UnitNode().Unit()),
  ...
  if (Copy().Fault().SlipType() != CHorizonBase::SLIP &&
      (!Copy().DistributedFriction() || !Copy().FrictionExtrapolated()) && !Copy().FrictionSameAsPrevious()) {
    DDX_Text(pDX, IDC_ED_FRICTION, dFriction);
    DDV_MinMaxDouble(pDX, dFriction, Copy().FrictionAngle().MinValue(UnitNode().Unit()),
...
void CAttriFaultParameters::EnableControls() {
  GetDlgItem(IDC_RADIO_COHESION_EXTRAPOLATED)
      ->EnableWindow(Copy().Fault().SlipType() != CHorizonBase::SLIP && Copy().DistributedCohesion() && !Locked());
  GetDlgItem(IDC_RADIO_COHESION_USE)
      ->EnableWindow(Copy().Fault().SlipType() != CHorizonBase::SLIP && Copy().DistributedCohesion() && !Locked());
  GetDlgItem(IDC_RADIO_COHESION_SAME_AS_PREV)
      ->EnableWindow(Copy().Fault().SlipType() != CHorizonBase::SLIP && !Copy().Stage().Initial() &&
                     (!Copy().DistributedCohesion() || !Copy().CohesionExtrapolated()) && !Locked());
  GetDlgItem(IDC_RADIO_COHESION_VALUE)
      ->EnableWindow(Copy().Fault().SlipType() != CHorizonBase::SLIP &&
                     (!Copy().DistributedCohesion() || !Copy().CohesionExtrapolated()) && !Locked());
  GetDlgItem(IDC_LBL_COHESION)
      ->EnableWindow(Copy().Fault().SlipType() != CHorizonBase::SLIP &&
                     (!Copy().DistributedCohesion() || !Copy().CohesionExtrapolated()) &&
                     !Copy().CohesionSameAsPrevious() && !Locked());
  GetDlgItem(IDC_ED_COHESION)
      ->EnableWindow(Copy().Fault().SlipType() != CHorizonBase::SLIP &&
                     (!Copy().DistributedCohesion() || !Copy().CohesionExtrapolated()) &&
                     !Copy().CohesionSameAsPrevious() && !Locked());
  GetDlgItem(IDC_UN_COHESION)
      ->EnableWindow(Copy().Fault().SlipType() != CHorizonBase::SLIP &&
                     (!Copy().DistributedCohesion() || !Copy().CohesionExtrapolated()) &&
                     !Copy().CohesionSameAsPrevious() && !Locked());

  GetDlgItem(IDC_RADIO_FRICTION_EXTRAPOLATED)
      ->EnableWindow(Copy().Fault().SlipType() != CHorizonBase::SLIP && Copy().DistributedFriction() && !Locked());
  GetDlgItem(IDC_RADIO_FRICTION_USE)
      ->EnableWindow(Copy().Fault().SlipType() != CHorizonBase::SLIP && Copy().DistributedFriction() && !Locked());
  GetDlgItem(IDC_RADIO_FRICTION_SAME_AS_PREV)
      ->EnableWindow(Copy().Fault().SlipType() != CHorizonBase::SLIP && !Copy().Stage().Initial() &&
                     (!Copy().DistributedFriction() || !Copy().FrictionExtrapolated()) && !Locked());
  GetDlgItem(IDC_RADIO_FRICTION_VALUE)
      ->EnableWindow(Copy().Fault().SlipType() != CHorizonBase::SLIP &&
                     (!Copy().DistributedFriction() || !Copy().FrictionExtrapolated()) && !Locked());
  GetDlgItem(IDC_LBL_FRICTION)
      ->EnableWindow(Copy().Fault().SlipType() != CHorizonBase::SLIP &&
                     (!Copy().DistributedFriction() || !Copy().FrictionExtrapolated()) &&
                     !Copy().FrictionSameAsPrevious() && !Locked());
  GetDlgItem(IDC_ED_FRICTION)
      ->EnableWindow(Copy().Fault().SlipType() != CHorizonBase::SLIP &&
                     (!Copy().DistributedFriction() || !Copy().FrictionExtrapolated()) &&
                     !Copy().FrictionSameAsPrevious() && !Locked());
  GetDlgItem(IDC_UN_FRICTION)
      ->EnableWindow(Copy().Fault().SlipType() != CHorizonBase::SLIP &&
                     (!Copy().DistributedFriction() || !Copy().FrictionExtrapolated()) &&
                     !Copy().FrictionSameAsPrevious() && !Locked());

  bool bEnableDStiff =
      (Copy().Fault().SlipType() == CHorizonBase::USER || Copy().Fault().SlipType() == CHorizonBase::FRACTURE) &&
      !Locked();
