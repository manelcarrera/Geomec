...
CModelBase_Delegate::CModelBase_Delegate(CModelBase *modelBase)
    : CFemAppModel_Delegate(modelBase), m_modelBase(modelBase) {}

bool CModelBase_Delegate::IsMesh() const { return m_modelBase->IsMesh(); }
...
void CModelBase_Delegate::AppendSwitchTo(CContextMenuInvoker &invoker) {
  ...
  CGUIBase *GUI = static_cast<CGUIBase *>(GetGeomecDoc()->retrieveMatchingGUI(GetGeomecDoc()->GUI(), m_modelBase));

  invoker.AddCommand(_T("Switch to this model"),
                     *new TGUIBaseCommand(*GUI, &CGUIBase::SwitchTo, &CGUIBase::CanSwitchTo));
}
