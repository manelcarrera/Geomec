...
: CGraphEntryTemp_Delegate <IMaterial> (materialEntry)
...
void CMaterialEntry_Delegate::AppendContextMenu(CContextMenuInvoker &invoker)
{
  ...
  invoker.AddCommand(_T("&Import from Material Library"), *(new TCommand(*this, &CMaterialEntry_Delegate::ImportFromMatLib)));
...
void CMaterialEntry_Delegate::SelectMatLib()
{
  ...
  CTnoFileDialog dlg(TRUE, "mlx", NULL, OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, "XML Material Libraries (*.mlx)|*.mlx||");
...
void CMaterialEntry_Delegate::ExportToMatLib(
  std::vector <ml::CMaterial*> selection)
{
  if (m_materialEntry->GetExternalMatLibFileName().isEmpty())
  {
    SelectMatLib();
