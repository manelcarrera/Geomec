...
CAnalysisPoint_Delegate::CAnalysisPoint_Delegate(CAnalysisPoint *analysisPoint)
    : CColorNode_Delegate(analysisPoint), m_analysisPoint(analysisPoint) {}
...
CAnalysisPointEntry_Delegate::CAnalysisPointEntry_Delegate(CAnalysisPointEntry *analysisPointEntry)
    : CStorageNodeEntry_Delegate<CAnalysisPoint>(analysisPointEntry), m_analysisPointEntry(analysisPointEntry) {}

void CAnalysisPointEntry_Delegate::AppendContextMenu(CContextMenuInvoker &invoker) {
  typedef CSingleCommandTemplate<CAnalysisPointEntry_Delegate> TMyCommand;

  invoker.AddCommand("Create &Analysis point",
                     *(new TMyCommand(*this, &CAnalysisPointEntry_Delegate::CreateAnalysisPoint)));
}
...
void CAnalysisPointEntry_Delegate::CreateAnalysisPoint() {
  QString sName = m_analysisPointEntry->createUniqueName(DEFAULT_ANALYSIS_POINT_NAME, DEFAULT_INITIAL_NUMBER);
  CModelBase &model = (CModelBase &)m_analysisPointEntry->Model();
  geo::CPoint point = model.Boundary().Min() + (model.Boundary().Max() - model.Boundary().Min()) / 2;
