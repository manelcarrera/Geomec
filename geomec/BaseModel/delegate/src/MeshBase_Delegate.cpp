...
CMeshBase_Delegate::CMeshBase_Delegate(CMeshBase *meshBase) : IElementSet_Delegate(meshBase), m_meshBase(meshBase) {}
...
bool CMeshBase_Delegate::IsMesh() const { return m_meshBase->IsMesh(); }
...
void CMeshBase_Delegate::AppendContextMenu(CContextMenuInvoker &invoker) {
  ...
  invoker.AddCommand(_T("Create pointset"),
                     *(new TCommand(*this, &CMeshBase_Delegate::CreatePointSet, &CMeshBase_Delegate::IsMesh)));
...
void CMeshBase_Delegate::CreatePointSet() {
  ...
  pMainFrame->StartProgress("Creating pointset", m_meshBase->Mesh().PointSize());
  ...
  CPointSet *pPointSet = new CPointSet(m_meshBase->Name().toStdString().c_str(), m_meshBase->Model(), 0, dim);
...
bool CMeshBase_Delegate::CanDestroy() const { return m_meshBase->CanDestroy(); }
