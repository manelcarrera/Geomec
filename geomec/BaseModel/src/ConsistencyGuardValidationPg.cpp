...
CConsistencyGuardValidationPg::CConsistencyGuardValidationPg(CModelBase *pModel)
    : CPropertyPage(CConsistencyGuardValidationPg::IDD), m_pModel(pModel), m_ValidateModel(m_pModel), m_bMesh(false),
      m_bReservoir(false), m_bDepletion(false), m_bMaterial(false), m_bValues(false), m_bBoundCond(false),
      m_bEmptForm(false), m_bEmptFault(false) {
...
void CConsistencyGuardValidationPg::DrawCheck(CPaintDC &dc, CStatic &IconCheck, bool bCheck) {
  ...
  DrawIconEx(dc, rect.left, rect.top, LoadIcon(AfxGetApp()->m_hInstance, MAKEINTRESOURCE(CheckID)), 16, 16, 0, 0,
             DI_NORMAL);
}
