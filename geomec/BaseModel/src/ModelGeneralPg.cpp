...
void CModelGeneralPg::DoDataExchange(CDataExchange *pDX) {
  ...
  if (!pDX->m_bSaveAndValidate) {
    ...
    if (GetFileAttributesEx(strPath, GetFileExInfoStandard, &Data) != -1) {
      if (Data.nFileSizeLow != -1) {
        if ((Data.nFileSizeLow / 1000000000.0) > 1.0)
          ...
          strSize.Format(_T("%d byte"), Data.nFileSizeLow);
      } else {
        strSize = _T("0 kb");
      }
    } else {
    ...
    strFileVersion.Format("%d.%d.%d", modelBase.documentVersion().majorNr(), modelBase.documentVersion().minorNr(),
                          modelBase.documentVersion().revisionNr());
...
void CModelGeneralPg::OnChangeProjectInfo() { SetModified(true); }
