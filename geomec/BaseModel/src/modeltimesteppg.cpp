...
static BOOL IsInteger(const CString& sString)
{
	...
	for(int i = 0; i < sString.GetLength(); i++)
	{
		if(i == (sString.GetLength() - 1))
			if(!((sString[i] == '-') || isdigit(sString[i]))) return FALSE;
		else
...
/////////////////////////////////////////////////////////////////////////////
CModelTimestepPg::CTimeStepListObject::CTimeStepListObject(CModelTimestepPg& dlg, CListCtrl& ctrl, CDepletionStage& stage)
: IListObject(ctrl),
  m_dlg(dlg),
  m_pStage(&stage),
  m_sName(stage.Name().toStdString().c_str()),
  m_Time(stage.Time()),
  m_OutputType(stage.OutputType()),
  m_bInserted(false)
{
...
: IListObject(list_object.Ctrl()),
  m_dlg(list_object.m_dlg),
  m_pStage(0),
  m_bInserted(false)
{
...
QString CModelTimestepPg::CTimeStepListObject::Text() const
{
...
const CGeomecTime &CModelTimestepPg::CTimeStepListObject::Time() const
{
	return m_Time;
}

CGeomecTime &CModelTimestepPg::CTimeStepListObject::Time()
{
	return m_Time;
}
...
void CModelTimestepPg::CTimeStepListObject::Time(const CGeomecTime &time)
{
	m_Time = time;
}

CDepletionStage::eOutputType CModelTimestepPg::CTimeStepListObject::OutputType() const
{
	return m_OutputType;
}
...
BOOL CModelTimestepPg::CYearSubObject::CanEditText() const
{
	return m_parent.CanEditText();
}
...
BOOL CModelTimestepPg::CMonthSubObject::CanEditText() const
{
	return m_parent.CanEditText();
}
...
BOOL CModelTimestepPg::CMonthSubObject::EditText(const QString& strText)
{
	...
	if((atoi(strText.toStdString().c_str()) < 1) ||
		(atoi(strText.toStdString().c_str()) > 12))
	{
		_m()->msg("Month is not in range 1-12");
		return FALSE;
	}
	
	m_parent.Time(CGeomecTime(m_parent.Time().GetYear(),
		atoi(strText.toStdString().c_str())));
	return TRUE;
}
...
CDepletionStage::eOutputType CModelTimestepPg::COutputSubObject::OutputType() const
{
	return m_parent.OutputType();
}

BOOL CModelTimestepPg::COutputSubObject::CanEditText() const
{
	return m_parent.CanEditText();
}
...
void CModelTimestepPg::OnChangeTimesteps() 
{
	SetModified(true);
}
...
void CModelTimestepPg::CheckOutputTypeLastStage()
{
  ...
  if(pOutputSubObject->OutputType() == CDepletionStage::NONE || pOutputSubObject->OutputType() == CDepletionStage::PHASE)
...
void CModelTimestepPg::Increase(const int n)
{
	...
	// Get latest stage
	CTimeStepListObject *pStageObject = (CTimeStepListObject*)(m_lbTimestep.GetItemData(m_lbTimestep.GetItemCount() - 1));
...
bool CModelTimestepPg::IsNumerique(const CString& s)
{
	return true;
}
