...
CElementOrderSubDialog::CElementOrderSubDialog(CDialog *dialog, CFormationBase &formationBase)
    : m_dialog(dialog), m_formationBase(formationBase),
      m_inheritFromModel(formationBase.inheritElementOrderFromModel() ? TRUE : FALSE),
...
QString createFormationID(const CFormationBase &formationBase) {
  return FORMATION_ID_FORMAT.arg(formationBase.Name()).arg(formationBase.Index());
}
...
void CElementOrderSubDialog::onBnClickedInheritFromModel() {
  ...
  if ((elementOrder = previousElementOrder.find(formationID)) == previousElementOrder.end()) {
  ...
  if (m_inheritFromModel) {
    ...
    m_elementOrder =
        elementOrder2RadioButtonIndex(dynamic_cast<const CModelBase &>(m_formationBase.Model()).ElementOrder());
  } else {
...
void CElementOrderSubDialog::onOK() {
  ...
  m_formationBase.inheritElementOrderFromModel(m_inheritFromModel ? true : false);
...
int CElementOrderSubDialog::elementOrder2RadioButtonIndex(CElementOrder::TElementOrder elementOrder) {
