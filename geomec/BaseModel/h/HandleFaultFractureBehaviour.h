...
template <typename C>
  void CHandleFaultFractureBehaviour <C> ::onSlip(C& copy)
{
  copy.SlipType(CHorizonBase::SLIP);
}

template <typename C>
  void CHandleFaultFractureBehaviour <C> ::onUserDefined(C& copy)
{
  copy.SlipType(CHorizonBase::USER);
}

template <typename C>
  void CHandleFaultFractureBehaviour <C> ::onFracture(C& copy)
{
  copy.SlipType(CHorizonBase::FRACTURE);
}

template <typename C>
  void CHandleFaultFractureBehaviour <C> ::enableSlipButtons(bool enable)
{
...
template <typename C>
  int CHandleFaultFractureBehaviour <C> ::getSlipType(const C& copy) const
{
  int slipType = 0;
