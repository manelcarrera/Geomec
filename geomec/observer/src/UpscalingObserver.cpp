...
CUpscalingObserver::CUpscalingObserver(IUpscalingNodeBase &node, CTreeNode &parent, const BOOL rename,
                                       const enum REMOVE_TYPE remove, HTREEITEM hInsertAfter, BOOL bInitialUpdate,
...
CTreeNode *CUpscalingObserver::InsertChild(CGraphNode &t) {
  ...
  if (pFormationsBranch) {
    typedef CNodeObserver_Delegate<CFormationBase, CFormationBase_Delegate, CDummyNode, CDummyObserver, FALSE,
                                   UNLINK_ITEM>
        TFormationObserver;
    typedef CNodeObserver_Delegate<CUpscalingFormationsBranch, CUpscalingFormationsBranch_Delegate, CFormationBase,
                                   TFormationObserver, FALSE, UNLINK_ITEM>
        TFormationBranchObserver;
    return new TFormationBranchObserver(*pFormationsBranch, *this, FALSE, FIXED_ITEM, (HTREEITEM)-1, true, false);
  }
  ...
  if (pPointsetRangesBranch) {
    typedef CNodeObserver_Delegate<CPointSet, CPointSet_Delegate, CDummyNode, CDummyObserver, FALSE, UNLINK_ITEM>
        TPointsetObserver;
    typedef CNodeObserver_Delegate<CUpscalingPointsetRangesBranch, CUpscalingPointsetRangesBranch_Delegate, CPointSet,
                                   TPointsetObserver, FALSE, UNLINK_ITEM>
        TUpscalingPointsetBranchObs;
    return new TUpscalingPointsetBranchObs(*pPointsetRangesBranch, *this, FALSE, FIXED_ITEM, (HTREEITEM)-1, true,
                                           false);
  }
  ...
  if (pTargetPointsetBranch) {
    typedef CNodeObserver_Delegate<CPillarMap, CPillarMap_Delegate, CDummyNode, CDummyObserver, FALSE, UNLINK_ITEM>
        TPillarMapObserver;
    typedef CNodeObserver_Delegate<CUpscalingTargetPointsetBranch, CUpscalingTargetPointsetBranch_Delegate, CPillarMap,
                                   TPillarMapObserver, FALSE, DELETE_ITEM>
        TUpscalingTargetPointsetBranchObs;
    return new TUpscalingTargetPointsetBranchObs(*pTargetPointsetBranch, *this, FALSE, FIXED_ITEM, (HTREEITEM)-1, true,
                                                 false);
  }
...
CUpscalingBranch::CUpscalingBranch(ITreeObject &parent, CModelBase &model)
    : TUpscalingBranchBase((CUpscalingEntry &)*model.GraphEntry(MD_BASE_UPSCALING), parent.Ctrl(), FALSE, FIXED_ITEM,
...
CTreeNode *CUpscalingBranch::InsertChild(CGraphNode &t) {
  ...
  if (pThinLayerUpscalingNode || pCamClayUpscalingNode) {
    ...
    return new CUpscalingObserver(*node, *this, FALSE, DELETE_ITEM, (HTREEITEM)-1, false, false);
  }
