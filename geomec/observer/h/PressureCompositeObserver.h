...
public:
  ...
  CPressureCompositeObserver(CPressure &node, CTreeNode &parent, const BOOL rename, const enum REMOVE_TYPE remove,
                             HTREEITEM hInsertAfter = TVI_LAST, BOOL bInitialUpdate = FALSE,
                             BOOL bSortAfterInsertion = TRUE);

  CPressureCompositeObserver(CPressure &node, CTreeCtrl &ctrl, const BOOL rename, const enum REMOVE_TYPE remove,
