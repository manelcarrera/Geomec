...
CColorGradient_Delegate::CColorGradient_Delegate(CColorGradient *colorGradient)
    : IColorScaleNode_Delegate(colorGradient), m_colorGradient(colorGradient) {}
...
bool CColorGradient_Delegate::CanFindExtents() const {
  ...
  if (doc.CurrentScene() && m_colorGradient->IsGlobal()) {
    ...
    return pScene->ValueComponent() && pScene->MinMaxValue().first.Valid() && pScene->MinMaxValue().second.Valid();
  }
...
bool CColorGradient_Delegate::FindExtents() {
  ...
  return m_colorGradient->FindExtents(pScene->MinMaxValue().first.Value(), pScene->MinMaxValue().second.Value());
}
