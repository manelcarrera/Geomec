...
CHomogenizationBox_Delegate::CHomogenizationBox_Delegate(CHomogenizationBox *homogenizationBox)
    : IScalingBox_Delegate(homogenizationBox), m_homogenizationBox(homogenizationBox) {}
...
void CHomogenizationBox_Delegate::AppendContextMenu(CContextMenuInvoker &invoker) {
  typedef CSingleCommandTemplate<CHomogenizationBox> THomoBoxCommand;
  typedef CSingleCommandTemplate<CHomogenizationBox_Delegate> THomoBoxCommand_Delegate;

  invoker.AddCommand(_T("Capture element set"),
                     *(new THomoBoxCommand_Delegate(*this, &CHomogenizationBox_Delegate::CaptureElSet,
  ...
  if (pApp->KeyFile().Unlocked()) {
    ...
    invoker.AddCommand(_T("Create zoom-in model"),
                       *(new THomoBoxCommand_Delegate(*this, &CHomogenizationBox_Delegate::CreateZoomIn)));
    invoker.AddCommand(_T("Cut surfaces with this box"),
                       *(new THomoBoxCommand_Delegate(*this, &CHomogenizationBox_Delegate::CutSurafceInDatastoreWithBox,
  ...
  invoker.AddCommand(_T("&Delete"), *(new THomoBoxCommand(*m_homogenizationBox, &CHomogenizationBox::Destroy)));

  CColorNode_Delegate::AppendContextMenu(invoker);
}

void CHomogenizationBox_Delegate::CaptureElSet() {
  CModelBase &model = dynamic_cast<CModelBase &>(m_homogenizationBox->Model());
  CElementSet *pElementSet = m_homogenizationBox->CaptureElementSet(model);
  CElementSet_Delegate *pElementSet_Delegate = new CElementSet_Delegate(pElementSet);
...
bool CHomogenizationBox_Delegate::Destroy() { return m_homogenizationBox->Destroy(); }
...
void CHomogenizationBox_Delegate::CutSurafceInDatastoreWithBox() {
  const CModelBase &model = dynamic_cast<const CModelBase &>(m_homogenizationBox->Model());
  CSurfaceEntry *entry = ((CSurfaceEntry *)model.GraphEntry(MD_BASE_SURFACE));
  CSurfaceEntry_Delegate *entry_Delegate = dynamic_cast<CSurfaceEntry_Delegate *>(entry->getDelegate());
...
CHomoBoxEntry_Delegate::CHomoBoxEntry_Delegate(CHomoBoxEntry *homoBoxEntry)
    : CStorageNodeEntry_Delegate<CHomogenizationBox>(homoBoxEntry), m_homoBoxEntry(homoBoxEntry) {}

void CHomoBoxEntry_Delegate::AppendContextMenu(CContextMenuInvoker &invoker) {
  typedef CSingleCommandTemplate<CHomoBoxEntry_Delegate> THomoBoxEntry_DelegateCommand;

  invoker.AddCommand(_T("New Scaling Box"),
                     *(new THomoBoxEntry_DelegateCommand(*this, &CHomoBoxEntry_Delegate::NewBox)));
}
...
void CHomoBoxEntry_Delegate::NewBox() {
  QString name =
      m_homoBoxEntry->createUniqueName(DEFAULT_SCALING_BOX_NAME, m_homoBoxEntry->GraphEntryNodes().size() + 1);
  CHomogenizationBox *pBox = new CHomogenizationBox(m_homoBoxEntry->Model(), name);
