...
bool CItem::disable_MENU_NOT_ITEM() // doesn't work : several tries
{
  ...
  // int res_i = EnableMenuItem( parent->GetSafeHmenu(), pos, MF_DISABLED | MF_GRAYED | MF_BYPOSITION ); //MF_ENABLED,
  // MF_DISABLED
...
bool CItem::add(TPathV items_path_v, TItemsNameV items_v, eEvtType type_) {
  ...
  if (menu) {
    ...
    for (int i = 0; i < items_v.size(); i++) {
      ...
      bool res = menu->AppendMenu(MF_BYPOSITION | MF_ENABLED | MF_STRING, WM_USER + type_ + i,
                                  name.c_str()); // FIXME: 'WM_USER' HC
      if (res) {
        // Printer::instance()->debug("CItem::add : %s : %s : evt: %d", type_ == Launch ? "Launch" : type_ == Manual ?
        // "Manual" : "Remove" , name.c_str(), type_ + i );
      } else
        return false;
    }
    return true;
