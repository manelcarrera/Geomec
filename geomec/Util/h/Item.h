...
public:
  ...
  enum eEvtType { Launch = 100, Manual = Launch + 10, Remove = Manual + 10, Max = Remove + 10 };
...
public:
  ...
  static bool remove_all(TPathV items_path_v,
                         int excluded = 0); // remove all items from a sub-menu giving the names path
