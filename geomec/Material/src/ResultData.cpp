...
CStressStrainArray const *CResultData::GetStressStrainSteps() const { return m_paStressStrainStep; }
  ...
  if (m_paStressStrainStep != 0) {
    for (int i = 0; i < nSelected; ++i) {
      if (piSelected[i] < (m_nExperiments - 1)) {
        std::swap(m_paStressStrainStep[piSelected[i]], m_paStressStrainStep[m_nExperiments - 1]);
      }
