...
CExperimentDataLimits::CExperimentDataLimits(CExperimentArray &experimentData, CTabExperiment *tabExperiment,
                                             CWnd *pParent /*=NULL*/
                                             )
    : CDialog(CExperimentDataLimits::IDD, pParent), m_iLowerLimit(0), m_iUpperLimit(100), m_iLowerRange(0),
      m_iUpperRange(100) // from 0-100%
      ,
      m_vcExperimentData(experimentData), m_iExperimentDataIndex(LB_ERR) // no selection
      ,
      m_pTabExperiment(tabExperiment), m_iInitialLowerLimit(experimentData.size()) {}

CExperimentDataLimits::~CExperimentDataLimits() {}

void CExperimentDataLimits::DoDataExchange(CDataExchange *pDX) { CDialog::DoDataExchange(pDX); }
...
BOOL CExperimentDataLimits::OnInitDialog() {
  ...
  CString strText;
  strText.Format("%d", m_iLowerRange);
  e1->SetWindowText(strText);
  strText.Format("%d", m_iUpperRange);
  e2->SetWindowText(strText);
  ...
  if (m_vcExperimentData.size() == 1) {
    ...
    //
    for (int ii = 0, iSelected = -1; ii < m_vcExperimentData.size() && iSelected < 0; ++ii) {
...
void CExperimentDataLimits::OnSliderReleased(NMHDR *pNMHDR, LRESULT *pResult) {
  for (size_t s = 0; s < m_vcExperimentData.size(); ++s) {
    m_iInitialLowerLimit[s] = std::min(100 * m_vcExperimentData[s].getLowerLimitFraction(),
  ...
  if (m_iExperimentDataIndex != LB_ERR) {
    m_vcExperimentData[m_iExperimentDataIndex].setLowerLimitFraction(m_iLowerLimit / 100.0);
    m_vcExperimentData[m_iExperimentDataIndex].setUpperLimitFraction(m_iUpperLimit / 100.0);
  }

  CString strText;
  strText.Format("%d", s1->GetPos());
  e1->SetWindowText(strText);
  strText.Format("%d", s2->GetPos());
  e2->SetWindowText(strText);
  ...
  if (m_pTabExperiment) {
    m_pTabExperiment->setInitialLowerLimit(true, m_iInitialLowerLimit);
    m_pTabExperiment->OnFileListUpdated();
    m_pTabExperiment->setInitialLowerLimit(false, m_iInitialLowerLimit);
  }
}
...
void CExperimentDataLimits::OnHScroll(unsigned int nSBCode, unsigned int nPos, CScrollBar *pScrollBar) {
  ...
  } else if ((CSliderCtrl *)GetDlgItem(IDC_SLIDER_LIMIT_2) == pSlide) {
...
void CExperimentDataLimits::OnSelchangeExperiments() {
  ...
  if (m_iExperimentDataIndex != LB_ERR) {
    ...
    if (s1->GetPos() <= s2->GetPos()) {
      s1->SetPos((int)(100 * m_vcExperimentData[m_iExperimentDataIndex].getLowerLimitFraction()));
      s2->SetPos((int)(100 * m_vcExperimentData[m_iExperimentDataIndex].getUpperLimitFraction()));
    } else {
      s1->SetPos((int)(100 * m_vcExperimentData[m_iExperimentDataIndex].getUpperLimitFraction()));
      s2->SetPos((int)(100 * m_vcExperimentData[m_iExperimentDataIndex].getLowerLimitFraction()));
    }
    OnSliderReleased(NULL, NULL);
