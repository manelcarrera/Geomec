...
BOOL CGammaView::PlotData(const CExperimentArray &aData, const CResultData *pResData, BOOL bIncludeYieldPoint,
                          BOOL bIsTopGraph, mlUnitDef ud, bool useInitialLowerLimit,
  ...
  for (int iExperiment = 0; iExperiment < aData.size(); ++iExperiment) {
    ...
    CCrossPlotCurve *crv =
        new CCrossPlotCurve(LPCTSTR(aData[iExperiment].GetLabel() + strLabelAddExperiment), GetQtColor(iExperiment));
    ...
    CStressStrainArray const &aStrainSteps = GetStrainSteps(aData[iExperiment]);
    if (crvLower) {
    ...
    crv->setData(new QwtPointArrayData(xx, yy, aStrainSteps.size() - lowerSize - upperSize));
    delete[] xx;
    delete[] yy;
  ...
  if (pResData)
    for (int iResult = 0; iResult < pResData->GetNrExperiments(); ++iResult) {
      ...
      CLinePlotCurve *crv =
          new CLinePlotCurve(LPCTSTR(aData[iResult].GetLabel() + strLabelAddCalculated), GetQtColor(iResult));

      CStressStrainArray const &aStrainSteps = GetStrainSteps(pResData, iResult);
      ...
      for (size_t iPoint = 0; iPoint < aStrainSteps.size(); ++iPoint) {
        ...
        if (bIsTopGraph) {
          yy[iPoint] = GetStressOrStrain(aStrainSteps[iPoint]) +
                       (useInitialLowerLimit ? aData[iResult].getAxialStrainOffset(initialLowerLimit[iResult] / 100.0)
...
BOOL CGammaView::PlotPQ(const CExperimentArray &aData, const CResultData *pResData, BOOL bIsTopGraph,
  ...
  for (int iExperiment = 0; iExperiment < aData.size(); ++iExperiment) {
    ...
    crv->setData(new QwtPointArrayData(xx1, yy1, aStrainSteps.size() - lowerSize - upperSize));
    delete[] xx1;
    delete[] yy1;
  ...
  if (pMat && bInitialized) {
    ...
    if (pMat->GetPrimaryYieldCurve(dPMin, dPMax, YieldCurveArray)) {
      if (!YieldCurveArray.empty()) {
        ...
        CLinePlotCurve *crv1 = new CLinePlotCurve(LPCTSTR(sYieldCurve1), Qt::black);
        ...
        if (pMat->GetSecondaryYieldCurve(dPMin, dPMax, YieldCurveArray)) {
          if (!YieldCurveArray.empty()) {
            ...
            CDashedPlotCurve *crv2 = new CDashedPlotCurve(LPCTSTR(sYieldCurve2), Qt::black);
...
void CGammaView::OnInitialUpdate() { CView::OnInitialUpdate(); }
...
#ifdef _DEBUG
void CGammaView::AssertValid() const { CView::AssertValid(); }

void CGammaView::Dump(CDumpContext &dc) const { CView::Dump(dc); }
#endif //_DEBUG
...
void CGammaView::SendMessageToDialog(UINT uMsg, WPARAM wParam /* = 0 */, LPARAM lParam /* = 0 */) {
  CWnd *pParent = GetParent();
  while (pParent && !pParent->SendMessage(uMsg, wParam, lParam))
    pParent = pParent->GetParent();
}
