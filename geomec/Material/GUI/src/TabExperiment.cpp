...
CTabExperiment::CTabExperiment(CMatParamDlg &dlg, CWnd *pParent)
    : CDialog(IDD, pParent), m_iShowInPicture(0), m_Dlg(dlg), m_iCurrentSel(-1), m_bUseInitialLowerLimit(false),
      m_iInitialLowerLimit(0) {}

unsigned int CTabExperiment::ID() { return IDD; }

CMatParamDlg &CTabExperiment::Dlg() { return m_Dlg; }

bool CTabExperiment::ShowAllExperiments() const { return (m_iShowInPicture == 0); }

CExperimentArray &CTabExperiment::ExperimentData() { return m_vcExperimentData; }

const CExperimentArray &CTabExperiment::ExperimentData() const { return m_vcExperimentData; }
...
void CTabExperiment::OnInsert() {
  ...
  if (dlg1.DoModal() == IDOK) {
    ...
    while (pos) {
      ...
      wb.AttachDispatch(wbs.Open(PathName, vOptional, vOptional, vOptional, vOptional, vOptional, vOptional, vOptional,
      ...
      if (nWssCount > 1) {
        ...
        for (i = 0; i < nSelectedIndices; i++)
          pSelectedIndices[i] = pSelected[i];
      } else {
      ...
      for (int i = 0; i < nSelectedIndices; i++) {
        ...
        if (ExpData.ReadFromFile(ws, FileNameFromPathName(PathName) + ", " + ws.GetName(), &m_Dlg)) {
          ...
          CString msg;
          msg.Format("The file %s is not a valid experiment file. The file can not be used.",
...
void CTabExperiment::OnFileListUpdated() {
  ...
  m_Dlg.OnExperimentFileListUpdated(nItems, nSelected, piSelected, m_bUseInitialLowerLimit, m_iInitialLowerLimit);

  delete[] piSelected;
}

void CTabExperiment::setInitialLowerLimit(bool useInitialLowerLimit, const std::vector<int> &initialLowerLimit) {
  m_bUseInitialLowerLimit = useInitialLowerLimit;
  m_iInitialLowerLimit = initialLowerLimit;
}

void CTabExperiment::getSelectedItems(int &total, int &selected, int *items[]) const {
...
void CTabExperiment::UpdatePictureRadios() {
  ...
  if (m_iShowInPicture == -1 && nItems > 0)
    m_iShowInPicture = 0;
  if (m_iShowInPicture == 1 && nSelected == 0 && nItems > 0)
    m_iShowInPicture = 0;
  if ((m_iShowInPicture == 0 || m_iShowInPicture == 1) && nItems == 0)
    m_iShowInPicture = -1;
...
void CTabExperiment::UpdateWeightFactor() {
  ...
  if (nSelected == 1) {
    ...
    char number[100];
    sprintf_s(number, "%f", m_vcExperimentData[piSelected[0]].WeightFactor());
