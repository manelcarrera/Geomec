...
/// < a
/// href="http://www.codeproject.com/Articles/11483/Let-Your-ListCtrl-Adjust-its-Column-Widths-Automat">codeproject.com
/// example</a>
void AdjustColumnWidth(CWnd *wnd //!< Cwnd that contains the CListCtrl
                       ,
...
//
int GetColumnCount(const CListCtrl *pListCtrl //!< CListCtrl to count
) {
...
CMatLibListCtrl::CMatLibListCtrl(const CTabModel *tabModel) : m_tabModel(tabModel), m_toolTipA(0), m_toolTipB(0) {}
  ...
  if ((index = HitTestEx(point, &colnum)) != -1) {
    ...
    if ((index != -1) && (colnum != -1) && (colnum == CTabModel::VALUE_COLUMN)) {
      if (!(m_tabModel->getMaterialParameterError())[index].isEmpty()) {
        _m()->msg((m_tabModel->getMaterialParameterError())[index], MB_OK | MB_ICONEXCLAMATION);
      }
    }
  }
}
