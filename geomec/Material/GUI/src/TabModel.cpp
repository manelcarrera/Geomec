...
std::vector<QString> determineMaterialParameterError(const CMatParamDlg &materialParameterDialog,
  ...
  for (CTabModel::TItemParameterMap::const_iterator itemParameter = itemParameterMap.begin();
       itemParameter != itemParameterMap.end(); ++itemParameter, ++i) {
    QString error;
    double value = itemParameter->second->ValueToUserUnit(materialParameterDialog.UnitDef());
    bool valid = itemParameter->second->CheckValueFromUserUnit(value, materialParameterDialog.UnitDef(), error);
...
CTabModel::CTabModel(CMatParamDlg &dlg, CWnd *pParent)
    : CDialog(CTabModel::IDD, pParent), m_Dlg(dlg), m_nParamTotal(0), m_ListCtrl(this), m_materialParameterError() {
  m_iType = -1;
  m_iModel = -1;

  if (EnableMatModel(MLFO_LINEAR))
    m_vcListedModels.push_back(MM_LINEAR);
  if (EnableMatModel(MLFO_CAMCLAY))
    m_vcListedModels.push_back(MM_CAMCLAY);
  if (EnableMatModel(MLFO_MOHRCOULOMB))
    m_vcListedModels.push_back(MM_MOHRCOULOMB);
  if (EnableMatModel(MLFO_MC_COHESION_HARD1))
    m_vcListedModels.push_back(MM_MC_COHESION_HARD1);
  if (EnableMatModel(MLFO_MC_COHESION_HARD2))
    m_vcListedModels.push_back(MM_MC_COHESION_HARD2);
  if (EnableMatModel(MLFO_MC_COHESION_HARD3))
    m_vcListedModels.push_back(MM_MC_COHESION_HARD3);
  if (EnableMatModel(MLFO_MC_FRICTION_HARD1))
    m_vcListedModels.push_back(MM_MC_FRICTION_HARD1);
  if (EnableMatModel(MLFO_MC_FRICTION_HARD2))
    m_vcListedModels.push_back(MM_MC_FRICTION_HARD2);
  if (EnableMatModel(MLFO_MC_FRICTION_HARD3))
    m_vcListedModels.push_back(MM_MC_FRICTION_HARD3);
  if (EnableMatModel(MLFO_MODIFIEDMOHRCOULOMB))
    m_vcListedModels.push_back(MM_MODIFIEDMOHRCOULOMB);
  if (EnableMatModel(MLFO_CREEP))
    m_vcListedModels.push_back(MM_CREEP);
  if (EnableMatModel(MLFO_UNDRAINED))
    m_vcListedModels.push_back(MM_UNDRAINED);
  if (EnableMatModel(MLFO_RIGIDITY))
    m_vcListedModels.push_back(MM_RIGIDITY);
  if (EnableMatModel(MLFO_DUALCAP_LINELA))
    m_vcListedModels.push_back(MM_DUALCAP_LINELA);
  if (EnableMatModel(MLFO_FRACTURE_ANISOTROPY))
    m_vcListedModels.push_back(MM_FRACTURE_ANISOTROPY);
  if (EnableMatModel(MLFO_UPSCALED_ANISOTROPY))
    m_vcListedModels.push_back(MM_UPSCALED_ANISOTROPY);
  if (EnableMatModel(MLFO_FRACTURE_APERTURE))
    m_vcListedModels.push_back(MM_FRACTURE_APERTURE);
  if (EnableMatModel(MLFO_ANISOTROPIC_CAMCLAY))
    m_vcListedModels.push_back(MM_ANISOTROPIC_CAMCLAY);
  if (EnableMatModel(MLFO_FRACTURE_APERTURE2))
    m_vcListedModels.push_back(MM_FRACTURE_APERTURE2);
  if (EnableMatModel(MLFO_CAMCLAY_CREEP))
    m_vcListedModels.push_back(MM_CAMCLAY_CREEP);
...
unsigned int CTabModel::ID() { return IDD; }

void CTabModel::SelectedModel(mlMatModel nModel) {
  m_iModel = -1;

  for (int i = 0; i < (int)m_vcListedModels.size(); i++)
    if (m_vcListedModels[i] == nModel)
      m_iModel = i;

  assert(m_iModel >= 0);
}

mlMatModel CTabModel::SelectedModel() { return m_vcListedModels[m_iModel]; }

CMatParamDlg &CTabModel::Dlg() { return m_Dlg; }

void CTabModel::Type(int iType) { m_iType = iType; }

int CTabModel::Type() { return m_iType; }
...
BOOL CTabModel::OnInitDialog() {
  ...
  setMaterialParameterError(determineMaterialParameterError(m_Dlg, m_mpItemParameter, getMaterialParameterError()));
...
static void ChangeItem(CWnd &parent //!< the parent of 'win'
                       ,
...
void CTabModel::VerticalResize(const int vdiff //!< the amount of pixels to resize in vertical direction
) {
...
void CTabModel::setMaterialParameterError(const std::vector<QString> &materialParameterError) {
...
bool CTabModel::hasMaterialParameterErrors() const {
  for (std::vector<QString>::const_iterator materialParameterError = m_materialParameterError.begin();
       materialParameterError != m_materialParameterError.end(); ++materialParameterError) {
...
void CTabModel::OnCancel() { m_Dlg.OnCancel(); }
...
void CTabModel::OnClickParamList(NMHDR *pNMHDR, LRESULT *pResult) {
  ...
  UpdateResetButton();
  m_ListCtrl.SetItem(phdn->iItem, 0, LVIF_IMAGE | LVIF_TEXT, label.c_str(), nImage, LVIS_FOCUSED, (unsigned int)-1, 0);
...
void CTabModel::OnEndlabeleditParamList(NMHDR *pNMHDR, LRESULT *pResult) {
  ...
  QString error;
  if (hasMaterialParameterErrors() || !pMatParam->ValueFromUserUnit(m_dCurrent, m_Dlg.UnitDef(), error)) {
...
LRESULT CTabModel::OnUpdateUnits(WPARAM wParam, LPARAM lParam) { return 0L; }
...
CLibraryMaterial *CTabModel::GetMaterial() { return m_Dlg.Material(); }

const CLibraryMaterial *CTabModel::GetOriginalMaterial() const { return m_Dlg.OriginalMaterial(); }
...
BOOL CTabModel::EnableMatModel(mlFilterOption nFilter) { return (m_Dlg.MaterialModelFilter() & nFilter); }
...
void CTabModel::InitializeModel(mlMatModel nModel) { m_Dlg.InitializeModel(nModel); }
...
void CTabModel::FillParameterListCtrl() {
  ...
  ::SendMessage(m_ListCtrl.m_hWnd, LVM_SETEXTENDEDLISTVIEWSTYLE, LVS_EX_SUBITEMIMAGES,
  ...
  for (int i = 0; i < m_nParamTotal; ++i) {
    ...
    item.mask = LVIF_IMAGE;
    item.iImage = (getMaterialParameterError()[nIndex].isEmpty() ? m_nParamTotal + 5 : m_nParamTotal + 4);
    m_ListCtrl.SetItem(&item);

    m_ListCtrl.SetItemText(nIndex, iColom++, param.UnitName(Dlg().UnitDef()));
  }
...
void CTabModel::UpdateResetButton() { GetDlgItem(IDC_RESET)->EnableWindow(*GetMaterial() != *GetOriginalMaterial()); }
