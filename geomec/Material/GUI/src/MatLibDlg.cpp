...
CMatLibDlg::CMatLibDlg(ml::CMaterialLibrary &matlib, int modelfilter, CAnalysisLogger &logger, bool bImportMode,
                       UINT nIDTemplate, CWnd *pParent)
    : CDialog(nIDTemplate, pParent), m_matlib(matlib), m_nModelFilter(modelfilter), m_logger(logger),
...
std::vector<ml::CMaterial *> CMatLibDlg::SelectedMaterials() { return m_vcSelection; }
...
int CMatLibDlg::CompareFuncMatList(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort) {
  ...
  if (bUseAscending) {
    if (icmp < 0) {
      ...
      if (m_bMatListAscending)
        return -1; // 1st should precede 2nd
      else
        return 1; // 1st should follow 2nd
    } else {
