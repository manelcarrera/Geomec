...
CGoCadModel_Delegate::CGoCadModel_Delegate(CGoCadModel *aGoCadModel)
    : CTetraModel_Delegate(aGoCadModel), m_GoCadModel(aGoCadModel) {}

/////

CGoCadSubModelEntry_Delegate::CGoCadSubModelEntry_Delegate(CGoCadSubModelEntry *goCadSubModelEntry)
    : TSubModelEntry_Delegate(goCadSubModelEntry), m_goCadSubModelEntry(goCadSubModelEntry) {}

void CGoCadSubModelEntry_Delegate::AppendContextMenu(CContextMenuInvoker &invoker) {
  invoker.AddCommand(_T("Import &Mesh"),
                     *(new CGoCadSubModelEntry_Delegate::TEntryCommand(*this, &CGoCadSubModelEntry_Delegate::ImportMesh,
                                                                       &CGoCadSubModelEntry_Delegate::CanImportMesh)));
  invoker.AddSeparator();
  invoker.AddCommand(_T("Insert &Horizon"), *(new CGoCadSubModelEntry_Delegate::TEntryCommand(
                                                *this, &CGoCadSubModelEntry_Delegate::InsertHorizon,
  ...
  if (keyfile.Unlocked()) {
#endif
    invoker.AddCommand(_T("Insert &Fault"), *(new CGoCadSubModelEntry_Delegate::TEntryCommand(
                                                *this, &CGoCadSubModelEntry_Delegate::InsertFault,
                                                &CGoCadSubModelEntry_Delegate::CanInsertSurfaceContainer)));
#ifdef TETRA_FAULTS_KEYFILE
  }
#endif
...
bool CGoCadSubModelEntry_Delegate::CanImportMesh() const {
  return dynamic_cast<const CGoCadModel *>(&m_goCadSubModelEntry->Model())->CanImportMesh();
}
...
void CGoCadSubModelEntry_Delegate::ImportMesh() {
  ...
  if (dlg.DoModal() == IDOK) {
    static_cast<CGoCadModel *>(&m_goCadSubModelEntry->Model())
        ->ImportMesh((LPCSTR)dlg.GetPathName(), selectGocadSolidDlg);
  }
}
...
void CGoCadSubModelEntry_Delegate::InsertFault() { InsertHorizon_Generic(m_goCadSubModelEntry->Model(), TRUE); }

void CGoCadSubModelEntry_Delegate::InsertHorizon() { InsertHorizon_Generic(m_goCadSubModelEntry->Model(), FALSE); }
