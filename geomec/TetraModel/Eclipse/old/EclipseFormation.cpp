...
CEclipseFormation::CEclipseFormation(CEclipseModel &model, const CEclipseReservoir &reservoir,
                                     CEclipseFormation *pUpper)
...
UINT CEclipseFormation::TypeNameId() const { return 0; }

CEclipseFormation::~CEclipseFormation() {}

const geo::IArray &CEclipseFormation::DisplayList() const { return m_arView; }

const CEclipseReservoir &CEclipseFormation::Reservoir() const { return m_reservoir; }
...
int CEclipseFormation::ElementSize() const { return m_arView.Size(); }

const geo::IBody &CEclipseFormation::Element(int index) const { return m_arView.Object(index); }

CEclipseFormationEntryObserver::CEclipseFormationEntryObserver(TEclipseFormationEntry &entry, CGraphTreeView &view,
                                                               HTREEITEM hParent, HTREEITEM hInsertAfter)
    : CStateBranch<TEclipseFormationEntry, CEclipseFormation, TEclipseFormationObserver, TRUE, FIXED_ITEM>(
          entry, view, IDI_TRI_CHECKED, FALSE, FIXED_ITEM, hParent, hInsertAfter, FALSE, FALSE) {
  OnUpdate();
}
...
CTreeNode *CEclipseFormationEntryObserver::InsertChild(CEclipseFormation &t) {
  ...
  CTreeNode *pTreeObserver =
      CStateBranch<TEclipseFormationEntry, CEclipseFormation, TEclipseFormationObserver, TRUE, FIXED_ITEM>::InsertChild(
          t);

  new TPressureEnumerator(*pTreeObserver, _T("Pressure"), IDI_PRESSURES);
  new TMaterialObserver(t.Material(), *pTreeObserver, TRUE, DELETE_ITEM, TVI_LAST, TRUE, FALSE);
