...
CIntFaceAttrDlg::CIntFaceAttrDlg(CBoundaryInterfaceDef &def, CDoubleQuantity::UNIT unit, CWnd *pParent /*=NULL*/)
    : CDialog(CIntFaceAttrDlg::IDD, pParent), m_def(def), m_unit(unit), m_Shear(def.Shear()), m_KtanTop(def.KtanTop()),
      m_KtanBottom(def.KtanBottom()), m_KtanSides(def.KtanSides()), m_KradTop(def.KradTop()),
      m_KradBottom(def.KradBottom()), m_KradSides(def.KradSides()), m_Lambda(def.Lambda()), m_nRadio(-1) {
