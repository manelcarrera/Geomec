...
CGocadSolidListCtrl::CGocadSolidListCtrl(CSelectGocadSolidDlg &dlg) : m_dlg(dlg) {}

void CGocadSolidListCtrl::OnDoubleClick(int nIndex) { m_dlg.OnDoubleClick(nIndex); }

void CGocadSolidListCtrl::OnItemChanged(NMHDR *pNMHDR, LRESULT *pResult) { m_dlg.SelectionChanged(); }
...
#ifdef SKUA_NEW
CSelectGocadSolidDlg::CSelectGocadSolidDlg(const std::vector<const gm_skua::SKUAParseData *> &solids, CWnd *pParent)
#else
CSelectGocadSolidDlg::CSelectGocadSolidDlg(const std::vector<QSharedPointer<CGocadData::CTSolid>> &solids,
                                           CWnd *pParent)
#endif
    : CDialog(IDD_SELECT_GOCAD_SOLID, pParent), m_solids(solids) {
  m_plcList = new CGocadSolidListCtrl(*this);
}

CSelectGocadSolidDlg::~CSelectGocadSolidDlg() { delete m_plcList; }

int CSelectGocadSolidDlg::SelectedSize() const { return int(m_vcSelected.size()); }

int CSelectGocadSolidDlg::SelectedEntry(int nIndex) const { return m_vcSelected[nIndex]; }

void CSelectGocadSolidDlg::SelectionChanged() { GetDlgItem(IDOK)->EnableWindow(m_plcList->GetSelectedCount() != 0); }
...
CSelectGocadSolidDlg::CGocadSolidListObject::CGocadSolidListObject(CListCtrlBase &ctrl, const CString &sName,
                                                                   int nVolumes, int nTetras)
    : IListObject(ctrl), m_sName(sName) {
...
QString CSelectGocadSolidDlg::CGocadSolidListObject::Text() const { return (LPCSTR)m_sName; }

BOOL CSelectGocadSolidDlg::CGocadSolidListObject::CanEditText() const { return FALSE; }

BOOL CSelectGocadSolidDlg::CGocadSolidListObject::EditText(const QString &strText) { return FALSE; }

unsigned int CSelectGocadSolidDlg::CGocadSolidListObject::Icon() const { return 0; }
...
CSelectGocadSolidDlg::CIntSubListObject::CIntSubListObject(CGocadSolidListObject &parent, int nColumn, int nValue)
    : ISubListObject(parent, nColumn), m_nValue(nValue) {}
...
BOOL CSelectGocadSolidDlg::CIntSubListObject::CanEditText() const { return FALSE; }

BOOL CSelectGocadSolidDlg::CIntSubListObject::EditText(const QString &strText) { return FALSE; }
