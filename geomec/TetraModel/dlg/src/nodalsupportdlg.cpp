...
CNodalSupportDlg::CNodalSupportDlg(CDisplacementSupportNode::CNodalSupportDef &supdef, const CDisplacementSupportNode &node, CWnd* pParent /*=NULL*/)
	: CSupportDlgBase(node, CNodalSupportDlg::IDD, pParent),
	m_SupportDef(supdef)
{
...
void CNodalSupportDlg::FillUnitLabels()
{
	...
	if(Unit() == IQuantityDouble::FIELD_UNIT) strUnitName = _T("ft");
...
{
	double dRet = 1;
	if(Unit() == IQuantityDouble::FIELD_UNIT) dRet = FF_FACTOR_LENGTH;
...
void CNodalSupportDlg::OnLocal() 
{
	OnRadioButton();
}

void CNodalSupportDlg::OnGlobal() 
{
	OnRadioButton();
}

void CNodalSupportDlg::OnNone() 
{
	OnRadioButton();
}
...
void CNodalSupportDlg::OnOK() 
{
	CSupportDlgBase::OnOK(IDP_SUPPORT_NODE_APPLYTOALL);
}
