...
CGlobalTensorDlg::CGlobalTensorDlg(C3DSupportNode &supnode, CWnd *pParent /*=NULL*/)
    : CAttributesTemplate<C3DSupportNode>(IDD_GLOBAL_TENSOR, supnode, pParent), m_SourceNode(supnode), m_nDeplStage(0),
...
void CGlobalTensorDlg::DoDataExchange(CDataExchange *pDX) {
  ...
  C3DSupportNode &tetraSupportNode = dynamic_cast<C3DSupportNode &>(Copy().Source());
  ...
  if (!pDX->m_bSaveAndValidate) {
    ...
    if (nGlobalTensorInput == C3DSupportNode::GTI_GENERAL) {
      ...
      if (fabs(tensor.XX()) > EPS || fabs(tensor.YY()) > EPS || fabs(tensor.ZZ()) > EPS || fabs(tensor.XY()) > EPS) {
...
void CGlobalTensorDlg::OnOK() {
  for (size_t i = 0; i < m_vcDepletionStages.size(); i++) {
    ...
    if (m_SourceNode.GlobalTensorDefined(*pStage) && !Copy().GlobalTensorDefined(*pStage) &&
      ...
      if (_m()->msg(IDP_SUPPORT_RESETGLOBALTENSOR, MB_YESNO | MB_DEFBUTTON2) == IDNO)
        return;
