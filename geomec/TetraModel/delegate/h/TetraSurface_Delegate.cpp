...
CTetraSurface_Delegate::CTetraSurface_Delegate(CTetraSurface *tetraSurface)
    : CSurfaceBase_Delegate(tetraSurface), m_tetraSurface(tetraSurface) {}
  ...
  if (!m_tetraSurface->Used()) {
    ...
    if (!bIsHexa) {
      invoker.AddCommand(_T("Insert sidesurface in sub model"),
                         *(new TTetraSurfaceDelegateCommand(*this, &CTetraSurface_Delegate::CreateSideSurface)));
      invoker.AddSeparator();
    }
    invoker.AddCommand(_T("Insert horizon in sub model"),
                       *(new TTetraSurfaceDelegateCommand(*this, &CTetraSurface_Delegate::CreateSubHorizon)));

#ifdef TETRA_SUPERMODEL_KEYFILE
    if (keyfile.Unlocked()) {
#endif
      invoker.AddCommand(
          _T("Insert horizon in super model"),
          *(new CTetraSurface::TTetraSurfaceCommand(*m_tetraSurface, &CTetraSurface::CreateSuperHorizon)));
#ifdef TETRA_SUPERMODEL_KEYFILE
    }
#endif
    ...
    if (keyfile.Unlocked()) {
      ...
      if (!bIsHexa) {
        ...
        invoker.AddCommand(_T("Insert fault in sub model"),
                           *(new TTetraSurfaceDelegateCommand(*this, &CTetraSurface_Delegate::CreateSubFault)));
      }
#ifdef TETRA_FAULTS_KEYFILE
    }
#endif
