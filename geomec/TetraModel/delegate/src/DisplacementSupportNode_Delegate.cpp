...
CDisplacementSupportNode_Delegate::CDisplacementSupportNode_Delegate(CDisplacementSupportNode *displacementSupportNode)
    : C3DSupportNode_Delegate(displacementSupportNode), m_displacementSupportNode(displacementSupportNode) {}

bool CDisplacementSupportNode_Delegate::MouseRelease(TFrame &frame, TKeyboardModifiers state, TMouseButton button,
                                                     const TScreenPoint &point, const TObjectVec &vcHit) {
        ...
        for (size_t j = 0; j < m_displacementSupportNode->getCornerNodes().size(); j++) {
          if (pPoint == m_displacementSupportNode->getCornerNodes()[j]) {
            ...
            if (button == Qt::RightButton) {
              ...
              menuInvoker.AddCommand("Attributes", *new CSingleCommandTemplate<CDisplacementSupportNode_Delegate>(
...
void CDisplacementSupportNode_Delegate::AppendContextMenu(CContextMenuInvoker &invoker) {
  invoker.AddCommand(_T("Global strain tensor"), *(new CSingleCommandTemplate<CDisplacementSupportNode_Delegate>(
...
void CDisplacementSupportNode_Delegate::OnShowDialog() {
  ...
  const CModelBase *pModel = dynamic_cast<const CModelBase *>(&m_displacementSupportNode->Model());
  ...
  CDisplacementSupportNode::TStageSupportMap::iterator its =
      m_displacementSupportNode->getSupports().find(*m_displacementSupportNode->getSelectedPoint());

  assert(its != m_displacementSupportNode->getSupports().end());

  CDisplacementSupportNode::CNodalSupportDef &def = its->second;
  CDisplacementSupportNode::CNodalSupportDef *copy = new CDisplacementSupportNode::CNodalSupportDef(def);
  ...
  if (dlg.DoModal() == IDOK) {
    if (dlg.ApplyToAll()) {
      for (CDisplacementSupportNode::TStageSupportMap::iterator it = m_displacementSupportNode->getSupports().begin();
    ...
    if (bModified) {
      CModelBase &model = static_cast<CModelBase &>(m_displacementSupportNode->Model());
...
QString CDisplacementSupportNode_Delegate::UnitName() const { return "-"; }
