...
CTetraBoundary_Delegate::CTetraBoundary_Delegate(CTetraBoundary *tetraBoundary)
    : CBoundaryBase_Delegate(tetraBoundary), m_tetraBoundary(tetraBoundary) {}
...
void CTetraBoundary_Delegate::AppendContextMenu(CContextMenuInvoker &invoker) {
  ...
  typedef CSingleCommandTemplate<CTetraBoundary_Delegate> TBoundaryCommand_Delegate;

  invoker.AddCommand(_T("&Export generated surfaces"),
                     *(new TBoundaryCommand_Delegate(*this, &CTetraBoundary_Delegate::ExportSurfaces,
  ...
  if (!m_tetraBoundary->Model().parentModel()) {
    invoker.AddCommand(_T("&Apply boundary interfaces"),
                       *(new TBoundaryCommand(*m_tetraBoundary, &CTetraBoundary::ToggleInterfaces, 0,
                                              &CTetraBoundary::CreateInterfaces)));
  }

  invoker.AddCommand(_T("&Interface attributes"),
                     *(new TBoundaryCommand_Delegate(*this, &CTetraBoundary_Delegate::InterfaceAttributes,
...
bool CTetraBoundary_Delegate::CanModify() const {
  return m_tetraBoundary->IsSuperModel() &&
         !(static_cast<const CModelBase &>(m_tetraBoundary->Model())).BranchState().IsBranch();
}
...
void CTetraBoundary_Delegate::InterfaceAttributes() {
  assert(typeid(m_tetraBoundary->InterfaceDefinition()).name() == typeid(CBoundaryInterfaceDef).name());

  CBoundaryInterfaceDef_Delegate *boundaryInterfaceDef_Delegate =
      static_cast<CBoundaryInterfaceDef_Delegate *>(m_tetraBoundary->InterfaceDefinition().getDelegate());

  boundaryInterfaceDef_Delegate->Attributes();
}

bool CTetraBoundary_Delegate::CanExportSurface() const { return m_tetraBoundary->CanExportSurface(); }
...
CAttriTetraBoundaryDlg::CAttriTetraBoundaryDlg(CTetraBoundary &boundary, CWnd *pParent)
    : CAttriBoundaryDlg<CTetraBoundary>(boundary, IDD_ATTRI_TETRA_BOUNDARY, pParent) {}
...
void CAttriTetraBoundaryDlg::DoDataExchange(CDataExchange *pDX) {
  ...
  if (!pDX->m_bSaveAndValidate) {
    ...
    sUnit = CString(Copy().DistanceToSubBoundary().UnitName(UnitNode().Unit()).c_str());
  }

  DDX_Text(pDX, IDC_ED_DISTANCE, dDistance);
  DDX_Text(pDX, IDC_ST_DISTANCE_UNIT, sUnit);
  DDV_MinMaxDouble(pDX, dDistance, Copy().DistanceToSubBoundary().MinValue(UnitNode().Unit()),
  ...
  GetDlgItem(IDC_RD_USER_DEFINED)->EnableWindow(Copy().IsSuperModel() && !bBranch);
  ...
  GetDlgItem(IDC_ST_DISTANCE_UNIT)->EnableWindow(Copy().IsSuperModel() && !bBranch);
}
