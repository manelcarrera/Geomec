...
CTetraSubHorizon_Delegate::CTetraSubHorizon_Delegate(CTetraSubHorizon *tetraSubHorizon)
    : CTetraHorizonBase_Delegate(tetraSubHorizon), m_tetraSubHorizon(tetraSubHorizon) {}
...
bool CTetraSubHorizon_Delegate::Attributes() {
  ...
  if (m_tetraSubHorizon->Slip()) {
    ...
    if (dlg.DoModal() == IDOK) {
      if (!m_tetraSubHorizon->IsLinkedTo(*m_tetraSubHorizon->Model().GraphEntry(MD_TETRA_SUB_ALL))) {
        m_tetraSubHorizon->AttachToEntry();
        assert(m_tetraSubHorizon->Model().GraphEntry(MD_BASE_HORIZON));
        m_tetraSubHorizon->LinkTo(*m_tetraSubHorizon->Model().GraphEntry(MD_BASE_HORIZON));

        CModelBase &model = dynamic_cast<CModelBase &>(m_tetraSubHorizon->Model());
...
void CTetraSubHorizon_Delegate::AppendContextMenu(CContextMenuInvoker &invoker) {
  ...
  if (m_tetraSubHorizon->keyfile.Unlocked()) {
    ...
    if (m_tetraSubHorizon->Slip()) {
      invoker.AddCommand("Change to &horizon", *(new THorCommand(*m_tetraSubHorizon, &CTetraSubHorizon::ToggleSlip,
                                                                 &CTetraSubHorizon::CanToggleSlip)));
    } else {
      invoker.AddCommand(
          "Change to &fault/fracture",
          *(new THorCommand(*m_tetraSubHorizon, &CTetraSubHorizon::ToggleSlip, &CTetraSubHorizon::CanToggleSlip)));
    }
...
CTetraSubHorizonEntry_Delegate::CTetraSubHorizonEntry_Delegate(CTetraSubHorizonEntry *tetraSubHorizonEntry)
    : CStorageNodeEntry_Delegate<CTetraSubHorizon>(tetraSubHorizonEntry), m_tetraSubHorizonEntry(tetraSubHorizonEntry) {
}
