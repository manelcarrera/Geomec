...
CPressureSupportNode_Delegate::CPressureSupportNode_Delegate(CPressureSupportNode *pressureSupportNode)
    : C3DSupportNode_Delegate(pressureSupportNode), m_pressureSupportNode(pressureSupportNode) {}

bool CPressureSupportNode_Delegate::MouseRelease(TFrame &frame, TKeyboardModifiers state, TMouseButton button,
      ...
      for (size_t j = 0; j < m_pressureSupportNode->getSurfaces().size(); j++) {
        if (vcHit[i] == m_pressureSupportNode->getSurfaces()[j].first) {
          ...
          if (button == Qt::RightButton) {
            ...
            menuInvoker.AddCommand("Attributes", *new CSingleCommandTemplate<CPressureSupportNode_Delegate>(
...
void CPressureSupportNode_Delegate::AppendContextMenu(CContextMenuInvoker &invoker) {
  invoker.AddCommand(_T("Global stress tensor"), *(new CSingleCommandTemplate<CPressureSupportNode_Delegate>(
                                                     *this, &CPressureSupportNode_Delegate::EditGlobalTensor,
...
void CPressureSupportNode_Delegate::OnShowDialog() {
  ...
  const CModelBase *pModel = dynamic_cast<const CModelBase *>(&m_pressureSupportNode->Model());
  ...
  CPressureSupportNode::TStageSupportMap::iterator its = m_pressureSupportNode->getSupports().find(
      CPressureSupportNode::CSurfaceDef(*m_pressureSupportNode->getSelectedSurface()));

  assert(its != m_pressureSupportNode->getSupports().end());

  CPressureSupportNode::CFaceSupportDef &def = its->second;
  CPressureSupportNode::CFaceSupportDef *copy = new CPressureSupportNode::CFaceSupportDef(def);
  ...
  if (dlg.DoModal() == IDOK) {
    if (dlg.ApplyToAll()) {
      for (CPressureSupportNode::TStageSupportMap::iterator it = m_pressureSupportNode->getSupports().begin();
    ...
    if (bModified) {
      CModelBase &model = static_cast<CModelBase &>(m_pressureSupportNode->Model());
