...
C3DSupportNode_Delegate::C3DSupportNode_Delegate(C3DSupportNode *tetraSupportNode)
    : CBaseSupportNode_Delegate(tetraSupportNode), m_tetraSupportNode(tetraSupportNode) {}
...
bool C3DSupportNode_Delegate::CanEditTensor() const { return m_tetraSupportNode->CanEditTensor(); }

void C3DSupportNode_Delegate::EditGlobalTensor() {
  // check whether there are multiple depletion stages

  const CModelBase *pModel = dynamic_cast<const CModelBase *>(&m_tetraSupportNode->Model());
