...
void CAttriTetraSubHorizon::DoDataExchange(CDataExchange *pDX) {
  ...
  if (pDX->m_bSaveAndValidate) {
    ...
    if (Copy().Slip()) {
      ...
      DDV_MinMaxDouble(pDX, dFrictionAngle, Copy().FrictionAngle().MinValue(UnitNode().Unit()),
                       Copy().FrictionAngle().MaxValue(UnitNode().Unit()));
      // Prevent the cohesion and friction angle from by zero
      if (fabs(dCohesion) < EPS) {
        CString sMessage;
        sMessage.Format("Enter a number between %f and %f", Copy().Cohesion().MinValue(UnitNode().Unit()),
                        Copy().Cohesion().MaxValue(UnitNode().Unit()));
      ...
      if (fabs(dFrictionAngle) < EPS) {
        CString sMessage;
        sMessage.Format("Enter a number between %f and %f", Copy().FrictionAngle().MinValue(UnitNode().Unit()),
                        Copy().FrictionAngle().MaxValue(UnitNode().Unit()));
        AfxMessageBox(sMessage);
        pDX->Fail();
      }
