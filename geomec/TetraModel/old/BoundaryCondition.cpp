...
CBoundaryCondition::CBoundaryCondition(const geo::CTetSurface &tetsurf, const CTetraModel &model,
...
void CBoundaryCondition::InsertSupport(TSupportMap &supportmap, const geo::IPoint &pt, CSupport &support,
  ...
  while (pStage) {
    ...
    case CDisplacementSupportNode::CNodalSupportDef::CSupportDef::ST_GLOBAL:
      support.Displacement(node.GlobalTensor(*pStage), vecReference, node.GlobalTensorReferenceAzimuth(*pStage),
                           nStage);
      break;
    default:
      ASSERT(FALSE);
...
void CBoundaryCondition::InterpolateAndInsertEdgeNodes(TSupportMap &supportmap, TSupportMap &localmap, bool bFull) {
  ...
  for (int i = 0; i < pointsz; i++) {
    ...
    if (localmap.find(geo::CPoint(pt)) != localmap.end())
    ...
    if (!bFull)
      supportmap.insert(std::make_pair(geo::CPoint(pt),
                                       CSupport(*pSupportPrev, dLenPrev, *pSupportNext, dLenNext, bFull, vecNormal)));
    else
      supportmap.insert(std::make_pair(geo::CPoint(pt),
                                       CSupport(*pSupportPrev, dLenPrev, *pSupportNext, dLenNext, bFull, vecNormal)));
  }
...
geo::CVector CBoundaryCondition::InterpolateVector(const geo::IPoint &pt, const geo::IVector &vec1,
                                                   const geo::IPoint &pt1, const geo::IVector &vec2,
                                                   const geo::IPoint &pt2) {
  double dDist1 = pt.Distance(pt1);
  double dDist2 = pt.Distance(pt2);

  return geo::CVector(Interpolate(vec1.X(), dDist1, vec2.X(), dDist2), Interpolate(vec1.Y(), dDist1, vec2.Y(), dDist2),
                      Interpolate(vec1.Z(), dDist1, vec2.Z(), dDist2));
}

CBoundaryCondition::TLinePointPairPair
CBoundaryCondition::GetBestEdgePoints(const geo::IPoint &pt, const std::vector<TLinePointPair> &vcEdgePoints) {
...
void CBoundaryCondition::InterpolateSurfaceNode(const geo::IPoint &pt, const std::vector<TLinePointPair> &vcEdgePoints,
  ...
  while (pStage) {
    ...
    geo::CVector vecfirst = InterpolateVector(prEdgePoints.first.second, itfirst->second.Displacement(nStage),
                                              prEdgePoints.first.first.First(), itsecond->second.Displacement(nStage),
                                              prEdgePoints.first.first.Second());
    ...
    geo::CVector vecsecond = InterpolateVector(prEdgePoints.second.second, itfirst->second.Displacement(nStage),
                                               prEdgePoints.second.first.First(), itsecond->second.Displacement(nStage),
                                               prEdgePoints.second.first.Second());

    // the vector interpolated from the the two vectors
    vcDisplacements.push_back(
        InterpolateVector(pt, vecfirst, prEdgePoints.first.second, vecsecond, prEdgePoints.second.second));
...
void CBoundaryCondition::InterpolateSurfaceNodes(TSupportMap &supportmap, TSupportMap &localmap, bool bFull) {
  ...
  if (vecNormal.IsParallel(*pOtherVec))
    pOtherVec = &geo::CVector::Xaxis;
  ...
  for (int i = 0; i < m_Surface.PointSize(); i++) {
    ...
    if (localmap.find(pt) == localmap.end()) {
      ...
      for (int j = 0; j < edgepointsz - 1; j++) {
        ...
        if (!pt.Empty())
          vcPoints.push_back(std::make_pair(l, pt));
      }
      ...
      if (!ptint.Empty())
        vcPoints.push_back(std::make_pair(l, ptint));
...
void CBoundaryCondition::ProcessNormal(TSupportMap &supportmap) {
  if (m_SortedEdge.Empty())
    m_SortedEdge = m_Surface.SortedEdge();
  ...
  for (int i = 0; i < sz; i++) {
    ...
    while (pStage) {
      ...
      case CPressureSupportNode::CFaceSupportDef::CSupportDef::PT_GLOBAL:
        prInsert.first->second.Pressure(Boundary().PressureSupportNode().GlobalTensor(*pStage), pFace->Normal(),
                                        iStage);
        break;
...
//// CSupport
CBoundaryCondition::CSupport::CSupport(int nStages) : m_bFull(true), m_vcDisplacements(nStages) {}

CBoundaryCondition::CSupport::CSupport(const geo::IVector &vecDir) : m_bFull(false) {
...
CBoundaryCondition::CSupport::CSupport(const CSupport &first, int distfirst, const CSupport &second, int distsecond,
    ...
    : m_bFull(bFull), m_vcDisplacements(first.m_vcDisplacements.size()) {
...
CBoundaryCondition::CSupport::CSupport() : m_bFull(false) {}

// derive displacement vector from global strain tensor and reference vector
void CBoundaryCondition::CSupport::Displacement(const ITensor &GlobalTensor, const geo::IVector &vecReference,
                                                const double &dRefAzimuth, int nStage) {
  ASSERT(m_bFull);
  ...
  m_vcDisplacements[nStage] =
      geo::CVector(rotatedtensor.XX() * rotatedvector.X() + rotatedtensor.EngineerZX() * rotatedvector.Z() +
                       rotatedtensor.EngineerXY() * rotatedvector.Y(),
...
// get vector
const geo::IVector &CBoundaryCondition::CSupport::Displacement(int nStage) const { return m_vcDisplacements[nStage]; }
...
CBoundaryCondition::CSupport CBoundaryCondition::CSupport::operator+(const CSupport &rhs) {
  ...
  if (m_bFull)
    Sum.m_vcDisplacements = m_vcDisplacements;
  else if (rhs.m_bFull)
    Sum.m_vcDisplacements = rhs.m_vcDisplacements;
  ...
  if (!Sum.m_bFull) // none of input supports is already full
  {
    ...
    if (vectors.Size() == 3)
      ...
      for (i = 0; i < vectors.Size(); i++)
        Sum.m_vcDirections.push_back(geo::CVector(vectors.Vector(i)));
    }
  }
...
void CBoundaryCondition::CSupport::ConformDisplacementsToDirections() {
  ...
  if (SupportDirSize() == 0 || SupportDirSize() == 3)
    return;
...
bool CBoundaryCondition::CSupport::CIndependentVectorSet::Insert(const geo::IVector &vector) {
  ...
  if (Size() == 1 && Vector(0).IsParallel(vector))
    return false;

  if (Size() == 2) {
    geo::CVector vecCross = Vector(0).CrossProduct(Vector(1));
    if (vecCross.DotProduct(vector) < EPS)
      return false; // in same plane
  }
...
int CBoundaryCondition::CSupport::CIndependentVectorSet::Size() { return m_vcVectors.size(); }
...
//// CPressure
CBoundaryCondition::CPressure::CPressure(int nStages) : m_vcPressures(nStages) {}
