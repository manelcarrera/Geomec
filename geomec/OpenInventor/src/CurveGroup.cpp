...
std::size_t CurveGroup::numCurves() const { return m_curves->getNumChildren(); }

Curve *CurveGroup::getCurve(std::size_t i) const { return static_cast<Curve *>(m_curves->getChild(i)); }
...
bool CurveGroup::containsCurve(Curve *curve) const { return m_curves->findChild(curve) >= 0; }
...
std::string CurveGroup::getCommonDenominatorName() const {
  ...
  for (int depth = 1; !lastDepth; ++depth) // how far we look into the firstCurveName
  {
    ...
    std::string denominator =
        firstCurveName.substr(0, lastDepth ? pos : pos - 2); // found a substring with specified depth
...
void CurveGroup::removeAllCurves() { m_curves->removeAllChildren(); }
...
void CurveGroup::getMinMax(float &globalMin, float &globalMax) const {
  ...
  int valueCoordinate =
      (m_axisPosition == AxisPosition::AXIS_LEFT || m_axisPosition == AxisPosition::AXIS_RIGHT) ? 1 : 0;
