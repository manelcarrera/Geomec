...
void OIDISurfaceMeshNode::buildSurfaceAppearanceNode() {
  ...
  if (isPointSetMeshNode())
    m_surfaceAppearance->addChild(m_soSurfaceMaterial);
...
void OIDISurfaceMeshNode::buildNodes() { buildSurfaceMeshNode(); }

/**
 * Constructor
 */
OIDISurfaceMeshNode::OIDISurfaceMeshNode(const OIDIMesh *meshData, OIDIMeshNodeManager *meshNodeManager)
    : OIDIMeshNode(meshData, meshNodeManager), m_surfaceSwitch(0), m_surfaceSeparator(0), m_surfaceAppearance(0),
      m_surfaceDrawStyle(0), m_surfaceMaterial(0), m_surface(0), m_isoline(0), m_isolineSwitch(0) {
  buildNodes();
}
...
void OIDISurfaceMeshNode::setIsoGap(double gap) { m_isoline->gap = gap; }
...
void OIDISurfaceMeshNode::UpdateDisplaySettings(const MeshNodeSettings &meshNodeSettings, bool showColor) {
  ...
  if (!isVisible()) {
    ...
    if (meshNodeSettings.meshFilled) {
      m_surfaceSwitch->whichChild = SO_SWITCH_ALL;
      m_surfaceDrawStyle->displayEdges =
          meshNodeSettings.meshLinesDisplayed && !showIsoLines(meshNodeSettings); // draw the edges of the triangles
      m_surfaceDrawStyle->displayFaces = true;
      m_surfaceDrawStyle->displayPoints = false;
      m_surfaceMaterial->lineColoring = MoMaterial::COLOR;
...
void OIDISurfaceMeshNode::updateColorScalarSet() { m_surface->colorScalarSetId = m_colorScalarSetId; }

void OIDISurfaceMeshNode::updateIsoScalarSet() { m_isoline->isoScalarSetId = m_isoScalarSetId; }
...
bool OIDISurfaceMeshNode::isPointSetMeshNode() { return (dynamic_cast<const OIDIPointSetMesh *>(getMesh()) != 0); }
