...
void patch_01(CNewWellPath::TPointValueVector &pointvalue_v, OIV2DWellPathGraph::DepthAxis depth_axis) {
  struct myclass {
    ...
    bool operator()(CNewWellPath::TPointValue p1,
                    CNewWellPath::TPointValue p2) // typedef std::pair<CNewWellPoint, geo::CValue> TPointValue;
    {
      switch (param) {
      case OIV2DWellPathGraph::DEPTH_AXIS_AHD:
        return (p1.first.TMD() < p2.first.TMD());
      case OIV2DWellPathGraph::DEPTH_AXIS_TVD:
        return (p1.first.TVD() < p2.first.TVD());
      case OIV2DWellPathGraph::DEPTH_AXIS_Z:
        return (p1.first.Z() < p2.first.Z());
      default:
        return (p1.first.TMD() < p2.first.TMD()); // ERROR, FIXME
      }
    }
...
void NewWellPathCurve::updatePoints() {
  ...
  for (CNewWellPath::TPointValueVector::iterator it = m_wellPointValueVector.begin();
       it != m_wellPointValueVector.end(); ++it) {
...
void NewWellPathCurve::onUnitChanged() { updatePoints(); }

NewWellPathCurve::NewWellPathCurve(const CNewWellPath *wellPath, const IValueComponentBase *valueComponent,
                                   CUnitNode::TUnitType unit, int colorIndex)
    : WellPathCurveBase(valueComponent, unit, colorIndex), m_wellPath(wellPath) {
  updatePoints();
}
...
QString NewWellPathCurve::getTextRepresentation() const {
  ...
  if (m_wellPath != 0) {
    for (CNewWellPath::TPointValueVector::const_iterator it = m_wellPointValueVector.begin();
         it != m_wellPointValueVector.end(); ++it) {
      const CNewWellPoint &p = it->first;
      const geo::CValue &value = it->second;
      if (value.Valid()) {
        QString line = lineFormat.arg(p.X() * unitFactor, 0, 'f')
...
QString NewWellPathCurve::getTextRepresentation(std::vector<const NewWellPathCurve *> &curves) {
  ...
  for (size_t j = 0; j < NrOfPoints; ++j) {
    ...
    QString line = lineBeginningFormat.arg(p.Y() * unitFactor, 0, 'f')
...
void NewWellPathCurve::refresh() { updatePoints(); }

QString NewWellPathCurve::getWellPathName() const { return m_wellPath->Name(); }

const CNewWellPath *NewWellPathCurve::getWellPath() const { return m_wellPath; }
