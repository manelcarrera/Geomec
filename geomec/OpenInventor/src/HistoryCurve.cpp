...
HistoryCurve::HistoryCurve(std::vector<DepletionStageValuePair> &depletionStageValueMap,
                           const IValueComponentBase *valueComponent, const CAnalysisPoint *analysisPoint,
                           CUnitNode::TUnitType unit, int colorIndex)
    : Curve(valueComponent, unit, colorIndex), m_analysisPoint(analysisPoint) {
...
QString HistoryCurve::getTextRepresentation() const { return QString(""); }

const CAnalysisPoint *HistoryCurve::analysisPoint() const { return m_analysisPoint; }
