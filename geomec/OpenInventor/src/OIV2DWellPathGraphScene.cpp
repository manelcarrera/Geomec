...
bool OIV2DWellPathGraphScene::operator==(const OIV2DWellPathGraphScene& node) const
{
  return false;
}
...
bool OIV2DWellPathGraphScene::CanConnectValueComponent(const IValueComponentBase* component) const
{
  ...
  if(!(
    (!m_newWellPaths.empty() && component->CanMap(*m_newWellPaths[0]))))
  {
    return false;
  }
      ...
      for(auto p : m_valueComponents)
      {
        ...
        if(resultComponent != 0)
        {
          ...
          sameComponent = sameComponent && (
            (&resultComponent->Parent() == &newResultComponent->Parent()) && 
            (resultComponent->ComponentIndex() == newResultComponent->ComponentIndex()));
        }
      }

      return sameComponent || (sameDepStage && static_cast<OIV2DWellPathGraph *> (m_graph)->canAddValueComponent(component));
    }
    else
    {
...
OIV2DWellPathGraphScene::OIV2DWellPathGraphScene(CFemAppModel& model)
  : OIV2DGraphValueColorScene(model)
{
...
const CUnitNode& OIV2DWellPathGraphScene::UnitNode() const
{
  return *m_unit;
}
...
bool OIV2DWellPathGraphScene::formationLinesVisible() const
{
  return m_formationLinesVisible;
}
...
{
  return ((!m_newWellPaths.empty()) &&
      !m_valueComponents.empty());
}
...
unsigned int OIV2DWellPathGraphScene::IconId() const
{
  return 0;
}

unsigned int OIV2DWellPathGraphScene::TypeId() const
{
  return 0;
}
...
bool OIV2DWellPathGraphScene::CanConnectItem(const CGraphNode& item) const
{
  ...
  if(newWellPath != 0)
  {
    return const_cast<CNewWellPath*>(newWellPath)->GraphPoints().size() != 0;  // these points are recalculated every time if ....size() == 0, sloppy!!!
  }

  const CDepletionStage * depletionStage = dynamic_cast<const CDepletionStage*>(&item);
  if (depletionStage != 0)
  {
//         if ((m_valueComponents.size() > 1 && m_valueComponents.size() > countStages()) ||  m_newWellPaths.size() > 1)
//             return false;
//         else
      return true;
  }
...
const QString NO_VALUES_FOUND =
  QObject::tr("No values found for '%1', ignored");

} // anonymous namespace

bool OIV2DWellPathGraphScene::ConnectItem(const CGraphNode& node)
{
  if(!CanConnectItem(node)) return false;
  if (IsLinkedTo(node)) 
    return false;
  ...
  if(valueComponent != 0)
  {
    ...
    {
      QString message = NO_VALUES_FOUND.arg(
        vcbfunctions::getResultPath(*m_rejectedComponent, 0));
      _m()->msg(message);
...
void OIV2DWellPathGraphScene::OnNeighbourDeleted(const CGraphNode& node)
{
  ...
  auto iter = std::find(m_valueComponents.begin(), m_valueComponents.end(), static_cast<const IValueComponentBase*>(&node));
...
bool OIV2DWellPathGraphScene::Empty() const
{
  return true;
}

long OIV2DWellPathGraphScene::SavedItems() const
{
  return 0;
}

int OIV2DWellPathGraphScene::SelectMode() const
{
  return 2;
}
...
void OIV2DWellPathGraphScene::showContextMenu(const TScreenPoint& point)
{
  ...
  TSceneCommand* markerCmd = new TSceneCommand(
    *this, 
    &OIV2DWellPathGraphScene::toggleMarkers,
    0,
    &OIV2DWellPathGraphScene::getMarkersVisible);

  TSceneCommand* formationLinesCmd = new TSceneCommand(
    *this,
    &OIV2DWellPathGraphScene::toggleFormationLines,
    &OIV2DWellPathGraphScene::canToggleFormationLines,
    &OIV2DWellPathGraphScene::formationLinesVisible);

  TSceneCommand* rangeCmd = new TSceneCommand(
      *this,
      &OIV2DWellPathGraphScene::SetGraphRanges);

  TSceneCommand* clipboardCmd = new TSceneCommand(
    *this,
    &OIV2DWellPathGraphScene::copyDataToClipboard,
    &OIV2DWellPathGraphScene::canCopyDataToClipboard);

  TSceneCommand* clearBottomCmd = new TSceneCommand(
    *this, 
    &OIV2DWellPathGraphScene::clearBottomAxis,
    &OIV2DWellPathGraphScene::bottomAxisFilled,
    0);

  TSceneCommand* clearTopCmd = new TSceneCommand(
    *this, 
    &OIV2DWellPathGraphScene::clearTopAxis,
    &OIV2DWellPathGraphScene::topAxisFilled,
    0);

  TSceneCommand* toggleDepthAxisCmd= new TSceneCommand(
    *this,
    &OIV2DWellPathGraphScene::toggleDepthAxis,
    &OIV2DWellPathGraphScene::canToggleDepthAxis,
    0);
  TSceneCommand* tryTopAxisCmd = new TSceneCommand(
    *this, 
    &OIV2DWellPathGraphScene::preferTopOrLeftAxis,
    &OIV2DWellPathGraphScene::canPreferTopOrLeftAxis,
    &OIV2DWellPathGraphScene::topOrLeftAxisPreferred);
  TSceneCommand* tryBottomAxisCmd = new TSceneCommand(
    *this, 
    &OIV2DWellPathGraphScene::preferBottomOrRightAxis,
...
void OIV2DWellPathGraphScene::clearBottomAxis()
{   
  clearAxis(1);
}
...
void OIV2DWellPathGraphScene::clearTopAxis()
{
  clearAxis(0);
}
...
{
  return std::count_if(m_valueComponents.begin(), m_valueComponents.end(), [](const IValueComponentBase* valueComponent)->bool
  {
    return dynamic_cast<const IResultComponent*>(valueComponent) != 0;
  });
}

size_t OIV2DWellPathGraphScene::NumberOfValueComponents() const
{
  return m_valueComponents.size();
}
...
void OIV2DWellPathGraphScene::UnLinkAllResultComponents()
{
  ...
  while ((componentIter = std::find_if(m_valueComponents.begin(), m_valueComponents.end(), isResultComponent)) != m_valueComponents.end())
  {
