...
COpenInventorEventsHandler::COpenInventorEventsHandler(COpenInventorSceneNode &sceneNode)
    : m_sceneNode(sceneNode), m_oivInitialized(false), m_cameraIsMoving(false)

      ,
      m_draggerTimeOut(5), m_secondsToTimeOut(m_draggerTimeOut), m_firstRender(true), m_updateLegend(false),
      m_mouseMoved(false), m_dynamicRotate(true), m_phongLighting(true), m_displayPerspective(true), m_viewerAlgos(0),
      m_cameraSwitch(0), m_perspectiveCamera(0), m_orthoCamera(0), m_currentCamera(0), m_rootSceneGraph(0),
      m_meshNodeManager(0), m_gnomonSwitch(0), m_bboxSwitch(0), m_bboxScale(0), m_bboxTranslation(0),
      m_coordinateAxis(0), m_background(0), m_lockedSpinDirection(-1), m_worldAction(0), m_screenDrawerGroup(0),
      m_screenDrawer(0), m_pointSize(18), m_meshInfoScreenPoint(-1, -1), m_oglContext(0), m_refinement_box_dlg(nullptr),
      m_boxes(nullptr) {
  initOIV();
}
...
bool COpenInventorEventsHandler::MouseRelease(TKeyboardModifiers state, TMouseButton button,
...
void COpenInventorEventsHandler::OnValueComponentChanged(const IValueComponentBase *pValueComponent,
                                                         CDoubleQuantity::UNIT unit, bool settingsChanged) {
...
void COpenInventorEventsHandler::ActivatePolygonScreenDrawer(CWorldAction *action,
                                                             GeomecPolygonScreenDrawer::PolygonMode mode,
                                                             std::vector<geo::CPoint> *points) {
  GeomecPolygonScreenDrawer *drawer = getNewScreenDrawer<GeomecPolygonScreenDrawer>();
  assert(drawer);
...
bool COpenInventorEventsHandler::CanZoomWindow() const { return m_screenDrawer && m_screenDrawer->getUserData() != 0; }

void COpenInventorEventsHandler::SwitchToTopView() {}

void COpenInventorEventsHandler::CheckCamera() { SetCamera(DisplayPerspective()); }

void COpenInventorEventsHandler::SetCamera(bool bPerspective) {
  if (bPerspective == (m_currentCamera == m_perspectiveCamera))
...
void COpenInventorEventsHandler::OnDeformation(Deformation::Data::Input deformation) {
  m_meshNodeManager->OnDeformation(deformation);
}
Deformation::Data::Input COpenInventorEventsHandler::Deformation() { return m_meshNodeManager->Deformation_(); }

bool COpenInventorEventsHandler::DisplayPerspective() const { return m_displayPerspective && !ShowTopView(); }

bool COpenInventorEventsHandler::CanDisplayPerspective() const { return !ShowTopView(); }
...
void COpenInventorEventsHandler::TrackElement(bool bTrack) { m_meshNodeManager->TrackElement(bTrack); }

bool COpenInventorEventsHandler::TrackElement() const { return m_meshNodeManager->TrackElement(); }
...
void COpenInventorEventsHandler::AllowDynamicRotate(bool bAllow) { m_dynamicRotate = bAllow; }

bool COpenInventorEventsHandler::AllowDynamicRotate() const { return (m_dynamicRotate && !ShowTopView()); }

bool COpenInventorEventsHandler::CanAllowDynamicRotate() const { return !ShowTopView(); }
...
bool COpenInventorEventsHandler::CanDisplayGnomon() const { return true; }

bool COpenInventorEventsHandler::HandMode() const { return false; }

bool COpenInventorEventsHandler::CanSelectHandMode() const { return false; }
...
void COpenInventorEventsHandler::SelectClipPlaneMode(bool yesOrNo) { m_meshNodeManager->SelectClipPlaneMode(yesOrNo); }
...
void COpenInventorEventsHandler::SelectMouseTracker(bool yesOrNo) { m_meshNodeManager->SelectMouseTracker(yesOrNo); }
...
void COpenInventorEventsHandler::SetDraggerConfined(bool yesOrNo) { m_meshNodeManager->SetDraggerConfined(yesOrNo); }
...
void COpenInventorEventsHandler::HideLegend(bool yesOrNo) { m_meshNodeManager->HideLegend(yesOrNo); }
...
void COpenInventorEventsHandler::Fill(bool bFill) { m_meshNodeManager->Fill(bFill); }
...
void COpenInventorEventsHandler::ViewTensorVector(TensorVectorMode mode) {
  if (m_meshNodeManager)
    m_meshNodeManager->ViewTensorVector(mode);
}
...
bool COpenInventorEventsHandler::CanRefreshVectorBaseScale() {
  if (m_meshNodeManager)
    return m_meshNodeManager->CanRefreshVectorBaseScale();
  return false;
}

void COpenInventorEventsHandler::RefreshVectorBaseScale() {
  if (m_meshNodeManager)
    m_meshNodeManager->RefreshVectorBaseScale();
}
...
void COpenInventorEventsHandler::GoHome() { m_viewerAlgos->resetToHomePosition(); }

void COpenInventorEventsHandler::SaveHome() { m_viewerAlgos->saveHomePosition(); }
...
void COpenInventorEventsHandler::ViewProportional(bool bProportional) {
  if (m_meshNodeManager)
    m_meshNodeManager->ViewProportional(bProportional);
}
...
void COpenInventorEventsHandler::CheckLighting() const {
  if (m_meshNodeManager)
    m_meshNodeManager->usePhongLightModel(Lighting());
}
...
bool COpenInventorEventsHandler::CanEnableLighting() const { return !PointSetSelected(); }

void COpenInventorEventsHandler::ShowValuesPerElement(bool perElement) {
  if (m_meshNodeManager)
    m_meshNodeManager->ShowValuesPerElement(perElement);
}
...
void COpenInventorEventsHandler::ViewBeachBalls(bool view) {
  if (m_meshNodeManager)
    m_meshNodeManager->ViewBeachBalls(view);
}
...
void COpenInventorEventsHandler::UpdateTranslation() { m_meshNodeManager->SetTranslation(true); }

// viewport size
int COpenInventorEventsHandler::Width() const { return m_sceneNode.Width(); }

int COpenInventorEventsHandler::Height() const { return m_sceneNode.Height(); }

// value/coloring information

const IColorScaleNode *COpenInventorEventsHandler::ColorScale() const { return m_sceneNode.ColorScale(); }

// display color or property values
bool COpenInventorEventsHandler::ShowColor() const {
  if (m_meshNodeManager)
    return m_meshNodeManager->ShowColor();
  else
    return true;
}
...
bool COpenInventorEventsHandler::CanShowValue() { return m_meshNodeManager && m_meshNodeManager->CanShowValue(); }
...
struct Pickability {
  Pickability(SoPickStyle::Style style, OIDIMeshNodeManager *meshNodeManager) : m_meshNodeManager(meshNodeManager) {
    m_savedStyle = m_meshNodeManager->GetPickStyle();
    m_meshNodeManager->SetPickStyle(style);
  }

  ~Pickability() { m_meshNodeManager->SetPickStyle(m_savedStyle); }
...
/* x <-> N, y <-> E */
void COpenInventorEventsHandler::GetScreenToWorldConversion(SbVec3f &screenMin, SbVec3f &worldMin,
  ...
  for (int i = 0; i < 2; ++i) {
    scaleScreenToWorld[i] =
        (worldMax[i] - worldMin[i]) / (screenMax[WorldToScreenIndex(i)] - screenMin[WorldToScreenIndex(i)]);
  }
}
...
SbVec2f COpenInventorEventsHandler::GetWorldCoordinates(const SbVec2f &screenNormalized /* x, y */) {
  ...
  for (int i = 0; i < 2; ++i) {
    world[i] = worldMin[i] +
               scaleScreenToWorld[i] * (screenNormalized[WorldToScreenIndex(i)] - screenMin[WorldToScreenIndex(i)]);
  }
...
SbVec2f COpenInventorEventsHandler::GetScreenNormalizedCoordinates(const SbVec2f &world /* x, y */) {
  ...
  for (int i = 0; i < 2; ++i) {
    screenNormalized[WorldToScreenIndex(i)] =
        screenMin[WorldToScreenIndex(i)] + (world[i] - worldMin[i]) / scaleScreenToWorld[i];
  }
...
void COpenInventorEventsHandler::TraceMeshInfo(SbVec2s &point) {
  ...
  for (size_t i = 0; (pickedPoint = pickAction.getPickedPoint(i)) != 0; ++i) {
    ...
    if (!detail || !detail->isOfType(MoFaceDetailI::getClassTypeId()))
      continue;
    ...
    if (dynamic_cast<const OIDIPointSetMesh *>(pickedMesh) != 0)
      continue;
    ;
    ...
    double distance = m_meshNodeManager->computeDistanceClosestNode(
        dynamic_cast<const MiMeshUnstructuredI *>(pickedMesh), mvCoordinates, cellId, meshNodeIndexClosest);
  ...
  m_meshNodeManager->computeValueTrackerCellMarkerSet(dynamic_cast<const MiMeshUnstructuredI *>(pickedMesh),
                                                      mvCoordinates, cellId, meshNodeIndexClosest,
                                                      meshNodeCoordinatesClosest);
  ...
  m_meshNodeManager->updateValueTracker(dynamic_cast<const MiMeshUnstructuredI *>(pickedMesh),
                                        dynamic_cast<const OIDIMesh *>(pickedMesh), mvCoordinates, cellId);
}
...
void COpenInventorEventsHandler::EnterView() { m_meshNodeManager->EnterView(); }

void COpenInventorEventsHandler::LeaveView() { m_meshNodeManager->LeaveView(); }
...
void COpenInventorEventsHandler::SelectMeshMode(MeshMode mode) { m_meshNodeManager->SelectMeshMode(mode); }
...
void COpenInventorEventsHandler::OnUpdateValueLegend() {
  CColorScaleEntry *pEntry =
      dynamic_cast<CColorScaleEntry *>(((CFemAppModel &)m_sceneNode.Model()).GraphEntry(MD_BASE_COLOR_SCALE));
  assert(pEntry);
  CGeomecDoc &doc = *GetGeomecDoc();
  ...
  if (pResultComponent) {
    ...
    // Other value object
    QString sValueName;
    if (m_sceneNode.ValueComponent()->Parent().TypeId() &&
        !dynamic_cast<CMeshResult *>(&m_sceneNode.ValueComponent()->Parent()))
  ...
  new CLegendFrame::legend_separator(LegendFrame(), 3);
  new CLegendFrame::legend_label(LegendFrame(),
                                 QString("Unit : ") + m_sceneNode.ValueComponent()->UnitName(doc.UnitNode().Unit()));
  new CLegendFrame::legend_separator(LegendFrame(), 15);

  if (InsertLinkedNodesInLegend() == 0)
    return;

  CColorGradient *pGradient = pEntry->ColorScaleType() == CColorScaleEntry::LOCAL ? pEntry->LocalColorGradient()
                                                                                  : pEntry->GlobalColorGradient();
  assert(pGradient);
  switch (pEntry->ColorScaleType()) {
  case CColorScaleEntry::LOCAL:
#if 0
  ...
  case CColorScaleEntry::HOTSPOT: {
    ...
    if (pHotSpot->CenterIsTransparent()) {
      ...
      if (pHotSpot->MinMaxValue().first != pHotSpot->MinMaxValue().second) {
        CLegendFrame::legend_scale::TColorMap mpColor;
        mpColor.insert(
            CLegendFrame::legend_scale::TColorMap::value_type(pHotSpot->MinMaxValue().first, pHotSpot->MinColor()));
        mpColor.insert(
            CLegendFrame::legend_scale::TColorMap::value_type(pHotSpot->MinMaxValue().second, pHotSpot->MaxColor()));
        new CLegendFrame::legend_scale(LegendFrame(), mpColor, 10, 15, 300);
        new CLegendFrame::legend_separator(LegendFrame(), 3);
      }
...
COpenInventorEventsHandler::TOpenGLNodeSet COpenInventorEventsHandler::NodesInScene() const {
  ...
  for (COpenInventorSceneNode::TLinkedGraphNodeSet::const_iterator it = stGraphNodes.begin(); it != stGraphNodes.end();
       ++it) {
    ...
    if (pCS) {
      ...
      for (int i = 0; i < vcCSObjects.size(); ++i) {
        const CCrossSection::CCSFormation *pCSFormation =
            dynamic_cast<const CCrossSection::CCSFormation *>(vcCSObjects[i]);
        if (pCSFormation) {
...
void COpenInventorEventsHandler::ShowGeologyLegend() { m_meshNodeManager->ShowGeologyLegend(); }

void COpenInventorEventsHandler::SetTextColor(float *rgb) { m_meshNodeManager->SetTextColor(rgb); }

void COpenInventorEventsHandler::GetTextColor(float *rgb) const { m_meshNodeManager->GetTextColor(rgb); }

void COpenInventorEventsHandler::SetCoordinateAxisColor(float *rgb) { m_coordinateAxis->SetAxisColor(rgb); }

void COpenInventorEventsHandler::GetCoordinateAxisColor(float *rgb) const { m_coordinateAxis->GetAxisColor(rgb); }

void COpenInventorEventsHandler::SetBackgroundColor(float *rgb) { m_background->color0.setValue(rgb); }
...
void COpenInventorEventsHandler::SetBackgroundColor2(float *rgb) { m_background->color1.setValue(rgb); }
...
const double &COpenInventorEventsHandler::PointSize() const { return m_pointSize; }

int COpenInventorEventsHandler::DraggerTimeOut() const { return m_draggerTimeOut; }

void COpenInventorEventsHandler::DraggerTimeOut(int timeOut) { m_draggerTimeOut = timeOut; }

int COpenInventorEventsHandler::Transparency() const { return m_meshNodeManager->Transparency(); }

void COpenInventorEventsHandler::Transparency(int transparency) { m_meshNodeManager->Transparency(transparency); }

int COpenInventorEventsHandler::IsoCount() const { return m_meshNodeManager->IsoCount(); }

void COpenInventorEventsHandler::IsoCount(int isoCount) { m_meshNodeManager->IsoCount(isoCount); }

double COpenInventorEventsHandler::IsoGap() const { return m_meshNodeManager->IsoGap(); }

void COpenInventorEventsHandler::IsoGap(double gap) { m_meshNodeManager->IsoGap(gap); }

void COpenInventorEventsHandler::removeDataSets() { m_meshNodeManager->removeDataSets(); }

void COpenInventorEventsHandler::unlinkValueComponent() {
  const_cast<COpenInventorSceneNode *>(&m_sceneNode)->unlinkValueComponent();
}

void COpenInventorEventsHandler::OnIsoValueComponentChanged(IValueComponentBase *pComponent,
...
void COpenInventorEventsHandler::RemoveAllMeshes() {
  if (m_meshNodeManager)
    m_meshNodeManager->RemoveAllMeshes();
}

void COpenInventorEventsHandler::ResetRenderAction() { m_viewerAlgos->resetRenderAction(); }
...
SoSwitch *const &COpenInventorEventsHandler::gnomonSwitch() const { return m_gnomonSwitch; }
...
bool COpenInventorEventsHandler::ShowTopView() const { return m_meshNodeManager->ShowTopView(); }
...
bool COpenInventorEventsHandler::PointSetSelected() const { return m_meshNodeManager->PointSetSelected(); }

int COpenInventorEventsHandler::LockedSpinDirection() { return m_lockedSpinDirection; }

template <class ScreenDrawerClass> ScreenDrawerClass *COpenInventorEventsHandler::getNewScreenDrawer() {
...
int COpenInventorEventsHandler::NumberOfOpenGLNodesLinked() { return m_sceneNode.NumberOfOpenGLNodesLinked(); }

void COpenInventorEventsHandler::dehighlightMesh() { m_meshNodeManager->dehighlightMesh(); }

void COpenInventorEventsHandler::clearValueTrackerCellFacetSet() { m_meshNodeManager->clearValueTrackerCellFacetSet(); }
...
void COpenInventorEventsHandler::UpdateValueTrackerDisplay() { m_meshNodeManager->UpdateValueTrackerDisplay(); }
...
CLegendFrame &COpenInventorEventsHandler::LegendFrame() { return m_sceneNode.LegendFrame(); }
