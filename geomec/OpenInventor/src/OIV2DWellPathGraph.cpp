...
namespace
{
  const char* getDepthAxisName(OIV2DWellPathGraph::DepthAxis depthAxis)
  {
    switch(depthAxis)
    {
    case OIV2DWellPathGraph::DEPTH_AXIS_AHD: return "AHD";
    case OIV2DWellPathGraph::DEPTH_AXIS_TVD: return "TVD";
    case OIV2DWellPathGraph::DEPTH_AXIS_Z:   return "Depth";
  ...
  {
    auto predicate = [value ](const CNewWellPoint & p)->bool
    {
      return p.TVD() >= value;
    };
...
PoGenAxis* OIV2DWellPathGraph::createVerticalAxis(const char* name, bool gridVisible)
{
  ...
  axis->gridVisibility = gridVisible 
    ? PoAxis::VISIBILITY_ON
    : PoAxis::VISIBILITY_OFF;
...
PoLinearAxis* OIV2DWellPathGraph::createHorizontalAxis(CurveGroup::AxisPosition pos, bool gridVisible)
{
  ...
  float y = (pos == CurveGroup::AxisPosition::AXIS_TOP) 
    ? m_ranges.m_top 
    : m_ranges.m_bottom;
  ...
  axis->gradPosition = (pos == CurveGroup::AxisPosition::AXIS_BOTTOM) 
    ? PoAxis::GRAD_BELOW 
    : PoAxis::GRAD_ABOVE;
...
{
  int visibility = (getDepthAxis() == DEPTH_AXIS_Z)
    ? SO_SWITCH_NONE
    : SO_SWITCH_ALL;
...
void OIV2DWellPathGraph::setupFormationSections(const std::vector<FormationSectionParams>& params)
{
  ...
  for(auto section : params)
  {
    ...
    char colorStr[128];
    sprintf(colorStr, "shininess 0.9 ambientColor %f %f %f diffuseColor %f %f %f", section.color.redF(), section.color.greenF(), section.color.blueF(), section.color.redF(), section.color.greenF(), section.color.blueF());
...
CurveGroup* OIV2DWellPathGraph::createCurveGroup(CurveGroup::AxisPosition axisPosition, int linePattern)
{
  ...
  group->m_domain->setValues(
      SbVec2f(xmin, m_ranges.m_bottom),
      SbVec2f(xmax / m_aspect, m_ranges.m_top));
...
bool OIV2DWellPathGraph::setupCurve(Curve* newCurve)
{
  ...
  newCurve->markerFilterType = m_markersVisible
    ? PoCurve::ALL_POINTS
    : PoCurve::INACTIVE;
...
void OIV2DWellPathGraph::updateVerticalAxes()
{
  ...
  for (int i = 0; i < iGraduations; ++i)
  {
    ...
    if(getDepthAxis() == DEPTH_AXIS_TVD) // then also show AHD
    {
      ...
      CLengthQuantity tmp(depthValue, m_unit);
      if (!m_newWellPaths.empty())
        tmd = tmp.Convert(::TVD2TMD (* m_newWellPaths[0], tmp.Value()), m_unit, CDoubleQuantity::SI_UNIT); // poor man's TVD2TMD
...
{
  m_verticalDomain->setValues(
      SbVec2f(0.0f, m_ranges.m_bottom),
      SbVec2f(1.0f, m_ranges.m_top));
...
{
  OIV2DRangeInterface::TAxis axis = (group->m_axisPosition == CurveGroup::AxisPosition::AXIS_TOP) ? OIV2DRangeInterface::AXIS_TOP : OIV2DRangeInterface::AXIS_BOTTOM;
  ...
  if (m_ranges.m_auto[axis])
  {
    ...
    group->getMinMax(rangeMinimum, rangeMaximum);
      ...
      GetAxisBounds(rangeMinimum, rangeMaximum, m_ranges.m_tick[axis], m_ranges.m_subtick[axis] / 2, m_ranges.m_horizontalMin[axis - 1], m_ranges.m_horizontalMax[axis - 1], m_ranges.m_horizontalStp[axis - 1]);
  }
  else
  {
      m_ranges.m_horizontalStp[axis - 1] = (m_ranges.m_horizontalMax[axis - 1] - m_ranges.m_horizontalMin[axis - 1]) / m_ranges.m_tick[axis];

      float y = (group->m_axisPosition == CurveGroup::AxisPosition::AXIS_TOP)
    ? m_ranges.m_top
    : m_ranges.m_bottom;
  ...
  float domainGlobalMax = m_ranges.m_horizontalMin[axis - 1] + (m_ranges.m_horizontalMax[axis - 1] - m_ranges.m_horizontalMin[axis - 1]) / m_aspect;
...
OIV2DWellPathGraph::OIV2DWellPathGraph(CUnitNode::TUnitType unit, float aspect, int preferredGroup)
  : OIV2DGraph (aspect, preferredGroup)
  , m_unit(unit)
  , m_depthAxis(DEPTH_AXIS_AHD)
  , m_formationLinesVisible(true)
  , m_markersVisible(true)
  , m_lineWidth(1)
  , m_ranges(*this, 0, -1000)
{
  createGraph();
  setupTopAndBottom();
}

OIV2DRangeInterface& OIV2DWellPathGraph::RangeInterface()
{
  return m_ranges;
}
  ...
  if(std::find(m_newWellPaths.begin(), m_newWellPaths.end(), wellPath) == m_newWellPaths.end())
  {
    ...
    for(auto vc : m_valueComponents)
    {
      ...
      if( is_first_vc )
      {
        ...
        newCurve->setColor( color_, color_ ); // mcr 2018-12-02: don't do in 'NewWellPathCurve' as all the classes chain expects 'int' instead of 'unsigned in'
        is_first_vc = false;
      }
      setupCurve(newCurve);
    }
...
{
  m_valueComponents.erase(
    std::remove(m_valueComponents.begin(), m_valueComponents.end(), component), 
    m_valueComponents.end());

  removeCurvesByPredicate([component](const Curve * curve) -> bool
    { 
      return curve->valueComponent() == component; 
    });
...
const NewWellPathCurve* OIV2DWellPathGraph::getCurve(const CNewWellPath* wellPath, const IValueComponentBase* component) const
{
  for (auto g : m_curveGroups)
  {
    CurveGroup* group = static_cast<CurveGroup*>(g);
...
void OIV2DWellPathGraph::showFormationLines(bool visible)
{
  ...
  m_formationLinesSwitch->whichChild = visible
    ? SO_SWITCH_ALL
    : SO_SWITCH_NONE;
}
...
void OIV2DWellPathGraph::showMarkers(bool visible)
{
  ...
  for (auto g : m_curveGroups)
  {
    ...
    {
      Curve* curve = static_cast<Curve*>(c);
      curve->markerFilterType = visible
        ? PoCurve::ALL_POINTS
        : PoCurve::INACTIVE;
    }
  }
...
{
  setDepthAxis((m_depthAxis == OIV2DWellPathGraph::DEPTH_AXIS_AHD) ? OIV2DWellPathGraph::DEPTH_AXIS_TVD : OIV2DWellPathGraph::DEPTH_AXIS_AHD);
}
...
void OIV2DWellPathGraph::RangeInterface::ResetMinMax(TAxis axis)
{
  ...
  if (CurveGroup *group = m_graph.getCurveGroup(axis))
  {
    ...
    group->getMinMax(min, max);
      ...
      GetAxisBounds(min, max, m_tick[axis], m_subtick[axis], m_horizontalMin[axis - 1], m_horizontalMax[axis - 1], m_horizontalStp[axis - 1]);
  }
  break;
  }
}

int OIV2DWellPathGraph::RangeInterface::Tick(TAxis axis) const
{
  return m_tick[axis];
}

void OIV2DWellPathGraph::RangeInterface::Tick(TAxis axis, int value)
{
  m_tick[axis] = value;
}

int OIV2DWellPathGraph::RangeInterface::SubTick(TAxis axis) const
{
  return m_subtick[axis];
}

void OIV2DWellPathGraph::RangeInterface::SubTick(TAxis axis, int value)
{
  m_subtick[axis] = value;
}

bool OIV2DWellPathGraph::RangeInterface::Auto(TAxis axis) const
{
  return m_auto[axis];
}

void OIV2DWellPathGraph::RangeInterface::Auto(TAxis axis, bool value)
{
  m_auto[axis] = value;
}

void OIV2DWellPathGraph::RangeInterface::UpdateGraph()
{
  m_graph.refreshVisualization();
}
