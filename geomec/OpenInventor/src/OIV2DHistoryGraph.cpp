...
void OIV2DHistoryGraph::PopulateDepletionStageValueMap(
    ...
    while(pStage)
    {
        ...
        if(nMode >= 0)
        {
            ...
            if(result.Component(pComponent->ComponentIndex(), nMode).Defined())
            {
                ...
                {
                    IResultComponent * resultComponent = dynamic_cast<IResultComponent *> (&result.Component(pComponent->ComponentIndex(), nMode));
                    value =	resultComponent->ValuePoint(analysisPoint->Point(), unit);  
                }
                else if (bIsVectorComponent)
                {
                    auto pVectorComponent = static_cast<const IVectorResult::CVectorComponent *>(&result.Component(pComponent->ComponentIndex(), nMode));
                    ...
                    {                       
                        ...
                        auto direction = (unsigned short) static_cast<const ITensorGroup::CVectorComposite *>(&result)->Direction(); // min, med, or max
                    ...
                    else
                    {
                        ...
                        if (unit == IQuantityDouble::FIELD_UNIT)
                            vector = MbVec3d(pResult->ConvertToField(v.X()), pResult->ConvertToField(v.Y()), pResult->ConvertToField(v.Z()));
                        else
                ...
                {
                    auto pTensorComponent = static_cast<const ITensorGroup::CComponentComposite::CFullTensorComponent *>(&result.Component(pComponent->ComponentIndex(), nMode));
...
int OIV2DHistoryGraph::getNumCurves(CurveGroup * curveGroup) const
{
    return curveGroup->numCurves();
}
...
Curve* OIV2DHistoryGraph::getCurve(CurveGroup * curveGroup, int index) const
{
    return curveGroup->getCurve(index);
}
...
void OIV2DHistoryGraph::updateDomain(CurveGroup * curveGroup)
{
    ...
    curveGroup->m_axis->gradPosition = curveGroup->m_axisPosition == CurveGroup::AxisPosition::AXIS_RIGHT ? PoAxis::GRAD_BELOW : PoAxis::GRAD_ABOVE;
    curveGroup->m_axis->start = SbVec3f(curveGroup->m_axisPosition == CurveGroup::AxisPosition::AXIS_RIGHT ? xAxisMaximum : xAxisMinimum, yAxisMinimum,  0.0f);
    ...
    if (m_label) m_label->setup(m_domain, m_aspect);
}
...
bool OIV2DHistoryGraph::addCurve( const IValueComponentBase* valueComponent, const CAnalysisPoint * analysisPoint, CUnitNode * unitNode )
{
    // Create curve
    std::vector<DepletionStageValuePair> depletionStageValueMap;
...
void OIV2DHistoryGraph::setUnit( CUnitNode::TUnitType unit )
{
...
{
    if (group->numCurves() == 0) return "empty";
