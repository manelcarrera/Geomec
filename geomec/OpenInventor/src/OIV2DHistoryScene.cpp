...
OIV2DHistoryScene::OIV2DHistoryScene(CFemAppModel& model)
...
void OIV2DHistoryScene::updateLegend()
{
  ...
  if ((m_valueComponents.size() == 1 && itemsHeader != defaultHeader) || m_analysisPoints.empty())
  {
    ...
    {
      std::string name = vcbfunctions::getResultPath(* valueComponent, 0).toStdString() + " [" + valueComponent->UnitName(m_unit->Unit()).toStdString() + "]";
      m_legend->addLeader(0, name);
    }
  }
  else
  ...
  if (m_graph && m_graph->getNumCurves() == 0) return;
...
bool OIV2DHistoryScene::operator==(const OIV2DHistoryScene& node) const
{
  return false;
}

const CUnitNode& OIV2DHistoryScene::UnitNode() const
{
  return *m_unit;
}
...
{
  if (m_analysisPoints.size() > 1) // only left axis and only one valuecomponent is used in this case, so we just unlink all analysis points
  {
    auto analysisPoints = m_analysisPoints;
    for (auto analysisPoint : analysisPoints)
    {
...
bool OIV2DHistoryScene::leftAxisFilled() const
{
     return m_graph->getNumCurves(0) != 0;
}

void OIV2DHistoryScene::clearRightAxis()
{
   clearAxis(1);
}

bool OIV2DHistoryScene::rightAxisFilled() const
{
  return m_graph->getNumCurves(1) != 0;
}
...
namespace
{
  ...
  {
    auto iter = std::find_if(valueMap.begin(), valueMap.end(), [stage](const DepletionStageValuePair& pair) 
      { 
...
void OIV2DHistoryScene::copyDataToClipboard()
{
  ...
  for(auto p : m_analysisPoints)
  {
    ...
    {
      clip += QString("\t%1 [%2]")
        .arg(vcbfunctions::getResultPath(*vc))
        .arg(vc->UnitName(m_unit->Unit()));
    }
    clip += "\r\n";
...
unsigned int OIV2DHistoryScene::IconId() const
{
  return 0;
}

unsigned int OIV2DHistoryScene::TypeId() const
{
  return 0;
}

int OIV2DHistoryScene::SelectMode() const
{
  return 2;
}
...
bool OIV2DHistoryScene::CanConnectItem(const CGraphNode& node) const
{
  ...
  if (static_cast<const CModelBase&>(Model()).Mesh().IsMesh())
  {
      ...
      if (pResultComponent)
    return pResultComponent->Defined() && CanDisplayResultComponent(*pResultComponent); // keep order, as CanDisplay... can be very expensive when we have no results and many depletion stages
  }
...
bool OIV2DHistoryScene::ConnectItem(const CGraphNode& node)
{
  ...
  if (linked)
  {
    ...
    {
      char message[128];
      sprintf (message, "No values found for '%s', ignored", vcbfunctions::getResultPath(* m_rejectedComponent, 0).toStdString().c_str());
      _m()->msg(message);
...
bool OIV2DHistoryScene::CanDisplayResultComponent(const IResultComponent &result_component) const
{
  ...
  {
    assert (stage);
    pRes = pResult->ResultComponent(* stage,
      currentResultComponent->AnalysisType(),
      result_component.RegisterIndex(),
...
bool OIV2DHistoryScene::Empty() const
{
  return true;
}

long OIV2DHistoryScene::SavedItems() const
{
  return 0;
}
...
void OIV2DHistoryScene::showContextMenu(const TScreenPoint& point)
{
  ...
  TSceneCommand* markerCmd = new TSceneCommand(
    *this, 
    &OIV2DHistoryScene::toggleMarkers,
    0,
    &OIV2DHistoryScene::getMarkersVisible);
  TSceneCommand* clipboardCmd = new TSceneCommand(
    *this,
    &OIV2DHistoryScene::copyDataToClipboard);
   
  TSceneCommand* clearLeftCmd = new TSceneCommand(
    *this, 
    &OIV2DHistoryScene::clearLeftAxis,
    &OIV2DHistoryScene::leftAxisFilled,
    0);
  TSceneCommand* clearRightCmd = new TSceneCommand(
    *this, 
    &OIV2DHistoryScene::clearRightAxis,
    &OIV2DHistoryScene::rightAxisFilled,
    0);
  TSceneCommand* tryLeftAxisCmd = new TSceneCommand(
    *this, 
    &OIV2DHistoryScene::preferTopOrLeftAxis,
    &OIV2DHistoryScene::canPreferTopOrLeftAxis,
    &OIV2DHistoryScene::topOrLeftAxisPreferred);
  TSceneCommand* tryRightAxisCmd = new TSceneCommand(
    *this, 
    &OIV2DHistoryScene::preferBottomOrRightAxis,
    &OIV2DHistoryScene::canPreferBottomOrRightAxis,
    &OIV2DHistoryScene::bottomOrRightAxisPreferred);
...
size_t OIV2DHistoryScene::NumberOfValueComponents() const
{
...
bool OIV2DHistoryScene::MultipleValueComponentsAllowed() const
{
  return NumberOfAnalysisPoints() <= 1;
}

size_t OIV2DHistoryScene::NumberOfAnalysisPoints() const
{
  return m_analysisPoints.size();
}
...
{
  return (leftAxisUnit() == "empty" || leftAxisUnit() == unitName || rightAxisUnit() == "empty" || rightAxisUnit() == unitName);
...
//             return true;
//         if (unitNames.size() < 2 && std::find (unitNames.begin(), unitNames.end(), currentResultComponent->UnitName()) == unitNames.end())
...
bool OIV2DHistoryScene::MultipleStagesAllowed() const
{
  return false;
}
