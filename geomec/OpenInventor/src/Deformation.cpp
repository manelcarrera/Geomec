...
//FIXME
const OIDIGeometryI::TDeformation OIDIGeometryI::NO_DEFORMATION = OIDIGeometryI::TDeformation{false,std::vector<MbVec3d>(),std::make_pair(MbVec3d(0.0,0.0,0.0),MbVec3d(0.0,0.0,0.0))};
...
const Deformation::Data::Result::Geo Deformation::INVALID_GEO = 
              Data::Result::Geo{
                std::vector<MbVec3d>(),								// points
                Deformation::Data::Result::Geo::TMinMax{	MbVec3d(0.0,0.0,0.0),
                                      MbVec3d(0.0,0.0,0.0) } };		// min_max

const Deformation::Data::Result Deformation::INVALID_RESULT = 
              Data::Result{
                INVALID_INFO,
                INVALID_GEO };

const Deformation::Data::Input Deformation::INVALID_INPUT = 
  Deformation::Data::Input{ -1,											// stage
...
bool Deformation::is( OIDIMesh *mesh, eFormationType _type ){ return Deformation::is( (COpenGLNode*)mesh->getOpenGLNode(), _type ); }

bool Deformation::is( COpenGLNode* node, eFormationType _type ){ return formation_type( node ) == _type; }

Deformation::eFormationType Deformation::formation_type( OIDIMesh *mesh ){ return formation_type( (COpenGLNode*)mesh->getOpenGLNode() ); }
...
const IVectorResult::CFullVectorComponent* Deformation::displacement_component( int stage_idx )
{
  ...
  if( pStage )
  {
    const IVectorResult::CFullVectorComponent* p_full_vector_component =  gm::res::composite::component::get_FULL( gm::eComposite::C_Displacement, pStage );
...
void Deformation::add_result( OIDIMesh *mesh, const IVectorResult::CFullVectorComponent* component, Data::Input::TScale scale )
{
  Data::Result r = INVALID_RESULT;

  r.info.deformation = component && component->Defined() ? get(mesh,component) : std::vector<MbVec3d>( mesh->getSize(), MbVec3d(0.0,0.0,0.0) );
  ...
  IT_Printer::instance()->debug( "deformation : add : openGL_node: '%s' : nP: %d", r.info.name.c_str(), r.geo.points.size() );
}
...
bool Deformation::apply()
{
  ...
  else
  {
    for( int i=0; i< d.results_v.size(); i++ )
    {
      ...
      if( d.input.active )
      {
        IT_Printer::instance()->debug( "deformation : refresh : openGL_node: '%s' : nP: %d", r.info.name.c_str(), r.geo.points.size() );
...
{
  bool bRefresh = refresh || ( input != m_data.input && ( m_data.input.active || input.active ) ); // != -> active, stage, scale 
  ...
  if( bRefresh )
  {
    ...
    for (std::size_t i = 0; i < meshes.NrOfMesh(); ++i)
    {
      ...
      if(mesh)
      {
        ...
        //Printer::instance()->debug("get : mesh : %d : %s : VU:%d", i, mesh->name().c_str(), dynamic_cast<OIDIMeshVU*>( mesh ) );
      }
...
std::vector<MbVec3d> Deformation_TEST::convert_data( OIDIVectorSetI* vectors, MbVec3d scale_def_3d )
{
  ...
  for( int i = 0; i < vectors->dataSize(); i++ )
  {
    ...
    MbVec3d v_2 = MbVec3d(	v[0] == myNaN ? 0.0 : scale*v[0],
                v[1] == myNaN ? 0.0 : scale*v[1],
...
void Deformation_TEST::explore_geometry_TEST( const OIDIGeometryI& g, double scale_gen )
{
  ...
  for( int i=0; i < size; i++ )
  {
    ...
    for( int j=0; j < nP; j++ )
    {
      ...
      IT_Printer::instance()->debug( "E:%d P:%d : %d\t%d\t%0.f", i, j, Algebra::fractional(p.X()), Algebra::fractional(p.Y()), p.Z() );
    }
