...
void print(const SoTabBoxDragger *box, const TGradientV gradient, const RefinementBox::GradientType_ &type_) {
...
0 };	//rotation : TODO
  ...
  printer->debug(
    "handler : print : transform : c: [%.2f,%.2f, %.2f], s: [%.2f,%.2f,
%.2f]", 
    c[0], c[1], c[2], 
    s[0], s[1], s[2] );
}*/
...
void print(const SbVec3f &v, std::string text = "") {
  printer->debug("handler : print : %s : t: [%.2f,%.2f, %.2f]", text, v[0], v[1], v[2]);
}
...
bool is_box_changed(const RefinementBox &box, const SoTabBoxDragger *gui_box) {
...
RefinementBox refinement_box_gui_to_data(const SoTabBoxDragger *gui_box, const TGradientV gradient, const QString &name,
                                         const bool &visible, const RefinementBox::GradientType_ &type_) {
  const SbVec3f &sf = gui_box->scaleFactor.getValue();
  const SbVec3f &t = gui_box->translation.getValue();
...
CMeshRefinementBoxesHandler::CMeshRefinementBoxesHandler(CMeshRefinementModel *boxes_model)
    : m_boxes_model(boxes_model), m_data_m(boxes_model->data()) {
  m_sw = new SoSwitch;
  m_b = new SoCube;
...
void CMeshRefinementBoxesHandler::gui_to_data(const TGradientM &g_m, const TTypeM &type_m) {
...
TBoxMap CMeshRefinementBoxesHandler::gui_data_m(const TGradientM &g_m, const TTypeM &type_m) {
  // in
  QMap<QString, SoTabBoxDragger *> &b = m_boxes_m;
  // out
  TBoxMap d;
...
void CMeshRefinementBoxesHandler::add(const QString &name) { create_box(name); }

// restore box position and dimensions
void CMeshRefinementBoxesHandler::undo(const QString &name) { data_to_gui_box(name); }
...
void CMeshRefinementBoxesHandler::show(bool val) { show_(m_sw, val); }
