...
void OIDIVolumeMeshNode::buildNodes() { buildVolumeMeshNode(); }

/**
 * Constructor
 */
OIDIVolumeMeshNode::OIDIVolumeMeshNode(const OIDIMesh *meshData, OIDIMeshNodeManager *meshNodeManager)
    : OIDIMeshNode(meshData, meshNodeManager), m_skinSwitch(0), m_skinSeparator(0), m_skinAppearance(0),
      m_skinDrawStyle(0), m_skinMaterial(0), m_skin(0), m_cellShapeSwitch(0), m_cellShapeSeparator(0),
      m_cellShapeAppearance(0), m_cellShapeDrawStyle(0), m_cellShapeMaterial(0), m_cellShape(0) {
  buildNodes();
  ...
  if (!isVisible()) {
    ...
    if (meshNodeSettings.meshFilled) {
      ...
      case SkinMesh:
        ...
        m_cellShapeSwitch->whichChild = SO_SWITCH_NONE;
        m_skinDrawStyle->displayFaces = true;
        m_skinDrawStyle->displayEdges = meshNodeSettings.meshLinesDisplayed &&
                                        !showIsoLines(meshNodeSettings); // do not draw edges if drawing isolines
        m_skinMaterial->lineColoring = MoMaterial::COLOR;
...
void OIDIVolumeMeshNode::setIsoGap(double gap) { m_skinIsoline->gap = gap; }
...
void OIDIVolumeMeshNode::updateColorScalarSet() {
  ...
  MoMaterial::ColoringType colorType = m_colorScalarSetId == -1 ? MoMaterial::COLOR : MoMaterial::CONTOURING;

  m_skinMaterial->faceColoring = colorType;
  m_cellShapeMaterial->faceColoring = colorType;
}

void OIDIVolumeMeshNode::updateIsoScalarSet() { m_skinIsoline->isoScalarSetId = m_isoScalarSetId; }
