...
InfoLabel::InfoLabel() : m_textOffsetX(0.0f), m_textOffsetY(0.0f) { build(); }

void InfoLabel::setLabelColor(float r, float g, float b) { m_labelMaterial->diffuseColor = SbColor(r, g, b); }
...
void InfoLabel::show(bool visible) { whichChild = visible ? SO_SWITCH_ALL : SO_SWITCH_NONE; }

void InfoLabel::clearText() { m_text->string.deleteValues(0, -1); }

void InfoLabel::setCrossHairColor(float color[3]) { m_crossHairMaterial->diffuseColor = SbColor(color); }
