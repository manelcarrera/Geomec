...
OIV2DGraph::OIV2DGraph(float aspect, int preferredGroup)
    : m_aspect(aspect), m_preferredCurveGroup(preferredGroup), m_curveGroups(0), m_label(0), m_clipPlaneRight(0) {
...
void OIV2DGraph::GetAxisBounds(float minimum, float maximum, int ticks, int subticks, float &lowerBound,
                               float &upperBound, float &tickSize) {
  ...
  lowerBound -=
      1e-7 * std::abs(lowerBound); // to make sure lowerBound < dbl_lowerBound, to get the right ticks in meshviz
...
void OIV2DGraph::showLabel(bool visible) {
  if (m_label)
    m_label->show(visible);
}

void OIV2DGraph::setNormalLabelColor() {
  if (m_label)
    m_label->setLabelColor(m_labelTextColor[0], m_labelTextColor[1], m_labelTextColor[2]);
}

void OIV2DGraph::setHighlightLabelColor() {
  if (m_label)
    m_label->setLabelColor(m_labelTextColor[0], m_labelTextColor[1], 1.0f - m_labelTextColor[2]);
}
...
void OIV2DGraph::updateTitle(const CurveGroup *group) { group->updateTitle(); }

ColorManager &OIV2DGraph::colorManager() { return m_colorManager; }
...
void OIV2DGraph::removeCurvesByPredicate(std::function<bool(const Curve *)> predicate) {
  for (auto g : m_curveGroups) {
    ...
    // std::find_if expects a SoNode * -> bool predicate, which the predicate supplied essentially is as a Curve is a
    // SoNode and group only contains Curves
    while ((iter = std::find_if(iter, endCurves,
                                *reinterpret_cast<std::function<bool(const SoNode *)> *>(&predicate))) != endCurves) {
...
CurveGroup *OIV2DGraph::selectCurveGroup(int preferredCurveGroupIndex,
  ...
  if (preferredCurveGroup->numCurves() == 0 ||
      preferredCurveGroup->getCurve(0)->valueComponent()->UnitName() == valueComponent->UnitName()) {
...
CurveGroup *OIV2DGraph::getCurveGroup(OIV2DRangeInterface::TAxis axis) const {
  CurveGroup::AxisPosition curveAxis = axis == OIV2DRangeInterface::AXIS_TOP ? CurveGroup::AxisPosition::AXIS_TOP
                                                                             : CurveGroup::AxisPosition::AXIS_BOTTOM;
...
void OIV2DGraph::setPreferredGroup(int preferredGroup) { m_preferredCurveGroup = preferredGroup; }
