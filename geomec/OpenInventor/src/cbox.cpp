...
{
  return std::vector< int >{	degrees( m_rx->angle.getValue() ), 
                degrees( m_ry->angle.getValue() ), 
...
void CBox::translate( int coordinate, int step, int plus_or_minus )
{
  ...
  for( int i=0; i < STEPS; i++ ) // MCR: This doesn't render the intermedite steps
  {
    ...
    if(			coordinate == CBoxDlg::Pe )	Pe += inc2;
    else if(	coordinate == CBoxDlg::Pn )	Pn += inc2;
    else if(	coordinate == CBoxDlg::Pd )	Pd += inc2;
