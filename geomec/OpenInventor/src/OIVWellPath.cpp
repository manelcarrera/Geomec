...
SoNode* OIVWellPath::createWellPathNode(const CNewWellPath& wellPath)
{
  ...
  coords.push_back (SbVec3f(wellPath.GlobalNorthing() - m_translation[0], wellPath.GlobalEasting() - m_translation[1], wellPath.DerElev() - m_translation[2])); // derrick floor
...
void OIVWellPath::showInfo(bool enable)
{
  m_infoSwitch->whichChild = enable ? SO_SWITCH_ALL : SO_SWITCH_NONE;
}
