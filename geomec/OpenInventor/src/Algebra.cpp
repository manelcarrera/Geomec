...
void Algebra::Data::_print() {
  IT_Printer::instance()->debug("a:%f b:%f d1:%f d:%f intersection: %f\t%f\t%f", alfa, beta, d1, distance,
...
MbVec3d Algebra::intersection_method_A(const geo::IElement &e, eSurface _type) {
  return _type == Bottom ? intersection_method_A(convert(e.Point(0)), convert(e.Point(2)), convert(e.Point(3)))
...
std::vector<MbVec3d> Algebra::points_mod_DEPRECATED(const OIDIGeometryI &g, OIDIVectorSetI *vectors, double scale_gen,
                                                    double scale_def) {
...
std::vector<MbVec3d> Algebra::points_mod(const OIDIGeometryI &g, std::vector<MbVec3d> deformation_v, double scale_gen,
                                         double scale_def) {
  ...
  IT_Printer::instance()->debug("SIZES : Def: %d - Geo: %d - Elems: %d - Orphans: %d - IFace: %d ", nD, g.getSize(),
                                axis_m.size(), orphans, iface_c);
  ...
  for (std::size_t i = 0; i < nP; ++i) {
    ...
    if (equal(intersections_v[i], IFACE) || equal(intersections_v[i], NO_ELEMENT)) {
...
std::vector<MbVec3d> Algebra::points_mod_SAME(const OIDIGeometryI &g, std::vector<MbVec3d> deformation_v,
...
std::vector<MbVec3d> Algebra::convert(OIDIVectorSetI *vectors) {
  ...
  for (int i = 0; i < vectors->dataSize(); i++) {
    ...
    MbVec3d v_2 = MbVec3d(v[0] == NaN ? 0.0 : v[0], v[1] == NaN ? 0.0 : v[1], v[2] == NaN ? 0.0 : v[2]);
...
bool Algebra::regular(const geo::IPoint *p) { return (!orphan(p) && !iface(p)); }
...
void Algebra::print_point(const geo::IPoint &p) {
  IT_Printer::instance()->debug("P : %f\t%f\t%f", p.X(), p.Y(), p.Z());
}
...
double Algebra::scale(const geo::IElement &e, eSurface _type) {
  return _type == Bottom ? scale(e.Point(0), e.Point(1), e.Point(2), e.Point(3))
