...
static QRgb GetQColor(const float *rgb) { return qRgb((int)(rgb[0] * 255), (int)(rgb[1] * 255), (int)(rgb[2] * 255)); }
...
OIV2DScene::OIV2DScene(CFemAppModel &model)
    : COIVSceneBase(model), m_background(0), m_graph(0), m_legend(0), m_lineThickness(1), m_markersVisible(true),
      m_storedLegendFrameWidth(0) {
  init();
}

OIV2DScene::OIV2DScene(const QString &name, CFemAppModel &model)
    : COIVSceneBase(name, model), m_background(0), m_graph(0), m_legend(0), m_lineThickness(1),
      m_storedLegendFrameWidth(0) {
  init();
}
...
void OIV2DScene::Activate() {
  ...
  pView->SetColumnInfo(0, 1, 0); // Setting it to 0 leads to the legend frame no longer showing fonts after we restore
                                 // the width in Deactivate (TBI)
}
...
OIV2DScene::OIV2DScene(unsigned int name, CFemAppModel &model) : COIVSceneBase(name, model) { init(); }
...
int OIV2DScene::getWidth() const { return m_width; }

int OIV2DScene::getHeight() const { return m_height; }
...
SoGroup *OIV2DScene::getSceneGraph() const { return m_sceneGraph; }
...
void OIV2DScene::zoomIn() { m_camera->scaleHeight(0.8f); }

void OIV2DScene::zoomOut() { m_camera->scaleHeight(1.25f); }
...
bool OIV2DScene::canPreferBottomOrRightAxis() const { return m_preferredCurveGroup != 1; }

bool OIV2DScene::bottomOrRightAxisPreferred() const { return m_preferredCurveGroup == 1; }
...
bool OIV2DScene::canPreferTopOrLeftAxis() const { return m_preferredCurveGroup != 0; }

bool OIV2DScene::topOrLeftAxisPreferred() const { return m_preferredCurveGroup == 0; }
...
QRgb OIV2DScene::ForegroundColor() const { return GetQColor(m_foregroundColor); }
...
void OIV2DScene::ForegroundColor(QRgb color) {
  ...
  if (m_legend)
    m_legend->setForegroundColor(m_foregroundColor);
}
...
void OIV2DScene::setLineThickness(int t) {
  if (t == m_lineThickness)
    return;
  m_lineThickness = t;

  if (m_graph)
    m_graph->setLineThickness(t);
}

int OIV2DScene::getLineThickness() const { return m_lineThickness; }

void OIV2DScene::toggleMarkers() { setMarkersVisible(!getMarkersVisible()); }

bool OIV2DScene::getMarkersVisible() const { return m_markersVisible; }
...
bool OIV2DScene::MouseDblClk(TKeyboardModifiers state, TMouseButton button, const TScreenPoint &point) { return false; }
...
bool OIV2DScene::KeyPress(TKeyboardModifiers state, int nChar, TKey key, int nRepCount) { return false; }

bool OIV2DScene::KeyRelease(TKeyboardModifiers state, int nChar, TKey key, int nRepCount) { return false; }
