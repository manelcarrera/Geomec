...
COpenInventorSceneNode::COpenInventorSceneNode(CFemAppModel &model)
    : COIVSceneBase(model), m_pValue(0), m_pValueInterrupted(0), m_pValueBlocked(0), m_pIsoValueBlocked(0),
      m_forceSettings(false), m_pComposite(0), m_pColorScale(0), m_pColorScaleEntry(0), m_pUnit(0), m_unitCurrent(),
      m_pHandler(new COpenInventorEventsHandler(*this)), m_meshNodeAssistant(m_pHandler, this),
      m_formationNodeAssistant(m_pHandler), m_allowExclusiveDisplay(true), m_adjustView(true), m_redrawing(false),
      m_steelFormation(0) {
  init(model);
}

COpenInventorSceneNode::COpenInventorSceneNode(const QString &strName, CFemAppModel &model)
    : COIVSceneBase(strName, model), m_pValue(0), m_pValueInterrupted(0), m_pValueBlocked(0), m_pIsoValueBlocked(0),
      m_forceSettings(false), m_pComposite(0), m_pColorScale(0), m_pColorScaleEntry(0), m_pUnit(0), m_unitCurrent(),
      m_pHandler(new COpenInventorEventsHandler(*this)), m_meshNodeAssistant(m_pHandler, this),
      m_formationNodeAssistant(m_pHandler), m_allowExclusiveDisplay(true), m_adjustView(true), m_redrawing(false),
      m_steelFormation(0) {
...
COpenInventorSceneNode::COpenInventorSceneNode(unsigned int uName, CFemAppModel &model)
    : COIVSceneBase(uName, model), m_pValue(0), m_pValueInterrupted(0), m_pValueBlocked(0), m_pIsoValueBlocked(0),
      m_forceSettings(false), m_pComposite(0), m_pColorScale(0), m_pColorScaleEntry(0), m_pUnit(0), m_unitCurrent(),
      m_pHandler(new COpenInventorEventsHandler(*this)), m_meshNodeAssistant(m_pHandler, this),
      m_formationNodeAssistant(m_pHandler), m_allowExclusiveDisplay(true), m_adjustView(true), m_redrawing(false),
      m_steelFormation(0) {
...
void COpenInventorSceneNode::Activate() { m_pHandler->UpdateLegend(); }

int COpenInventorSceneNode::Width() const { return m_sizeX <= 0 ? 800 : m_sizeX; }

int COpenInventorSceneNode::Height() const { return m_sizeY <= 0 ? 800 : m_sizeY; }
...
bool COpenInventorSceneNode::Empty() const { return true; }

long COpenInventorSceneNode::SavedItems() const { return 0; }
...
void COpenInventorSceneNode::LoadStream(TSTREAM &stream, CStreamVersion &version, TPROGRESS &progress) {
  ...
  if (version > CStreamVersion(4, 1, 19)) {
    ...
    RestoreColorFromStream(static_cast<void (COpenInventorSceneNode::*)(QRgb)>(&COpenInventorSceneNode::AxisColor),
                           stream);
    RestoreColorFromStream(static_cast<void (COpenInventorSceneNode::*)(QRgb)>(&COpenInventorSceneNode::TextColor),
                           stream);
    RestoreColorFromStream(
        static_cast<void (COpenInventorSceneNode::*)(QRgb)>(&COpenInventorSceneNode::BackgroundColor), stream);
    RestoreColorFromStream(
        static_cast<void (COpenInventorSceneNode::*)(QRgb)>(&COpenInventorSceneNode::BackgroundColor2), stream);
...
bool COpenInventorSceneNode::NodeCanBeDisplayed(const COpenGLNode *oglNode) const {
  ...
  if (dynamic_cast<const CNewWellPath *>(oglNode) != 0)
    return m_meshNodeAssistant.canBeDisplayed();
  if (dynamic_cast<const CPointSet *>(oglNode) != 0)
    return static_cast<const CPointSet *>(oglNode)->isDrawable();
  if (dynamic_cast<const CSurfaceBase *>(oglNode) != 0)
    return true;
  if (dynamic_cast<const CHorizonBase *>(oglNode) != 0)
    return true;
  if (dynamic_cast<const CZoominModelPlaceHolder *>(oglNode) != 0)
    return true;
  if (dynamic_cast<const CFormationBase *>(oglNode) != 0)
    return true;
...
bool COpenInventorSceneNode::CanConnectToMultipleItems(const CGraphNode &node) const { return true; }
...
bool COpenInventorSceneNode::CanConnectItem(const CGraphNode &item) const {
  ...
  if (pComponent) {
    ...
    if (!m_stOpenGLNodes.empty()) {
      auto predicate = [&pComponent](const COpenGLNode *node) -> bool { return pComponent->CanMap(*node); };
      canMap = std::find_if(m_stOpenGLNodes.begin(), m_stOpenGLNodes.end(), predicate) != m_stOpenGLNodes.end();
    }

    if (!canMap) {
      auto predicate = [](const CGraphNode *node) -> bool { return dynamic_cast<const CCrossSection *>(node); };
...
void COpenInventorSceneNode::SetRedrawing(bool redrawing) { m_redrawing = redrawing; }
...
bool COpenInventorSceneNode::KeyRelease(TKeyboardModifiers state, int nChar, TKey key, int nRepCount) {
  ...
  case 'T':
    ...
    break;
  case VK_OEM_PLUS /* + */:
    if (m_keyPressed[VK_CONTROL])
      ZoomIn();
    if (m_keyPressed[VK_SHIFT])
      MouseWheel(0, 1, TScreenPoint(100, 100));
    break;
  case VK_OEM_MINUS /* - */:
    if (m_keyPressed[VK_CONTROL])
      ZoomOut();
...
void COpenInventorSceneNode::EnterView() { m_pHandler->EnterView(); }

void COpenInventorSceneNode::LeaveView() { m_pHandler->LeaveView(); }

void COpenInventorSceneNode::OnZoomWindow() { m_pHandler->OnZoomRectangle(); }

bool COpenInventorSceneNode::CanZoomWindow() const { return m_pHandler->CanZoomWindow(); }

void COpenInventorSceneNode::SwitchToTopView() { m_pHandler->SwitchToTopView(); }

void COpenInventorSceneNode::DisplayPerspective(bool bPerspective) { m_pHandler->DisplayPerspective(bPerspective); }

void COpenInventorSceneNode::OnDeformation(Deformation::Data::Input deformation) {
  m_pHandler->OnDeformation(deformation);
}
Deformation::Data::Input COpenInventorSceneNode::Deformation() { return m_pHandler->Deformation(); }

bool COpenInventorSceneNode::DisplayPerspective() const { return m_pHandler->DisplayPerspective(); }

bool COpenInventorSceneNode::CanDisplayPerspective() const { return m_pHandler->CanDisplayPerspective(); }

void COpenInventorSceneNode::ShowBoundary(bool bShow) { m_pHandler->ShowBoundary(bShow); }

bool COpenInventorSceneNode::ShowBoundary() const { return m_pHandler->ShowBoundary(); }

bool COpenInventorSceneNode::CanShowBoundary() const { return m_pHandler->CanShowBoundary(); }

void COpenInventorSceneNode::TrackElement(bool bShow) { m_pHandler->TrackElement(bShow); }

bool COpenInventorSceneNode::TrackElement() const { return m_pHandler->TrackElement(); }

bool COpenInventorSceneNode::CanTrackElement() const { return m_pHandler->CanTrackElement(); }

void COpenInventorSceneNode::AllowDynamicRotate(bool bAllow) { m_pHandler->AllowDynamicRotate(bAllow); }

bool COpenInventorSceneNode::AllowDynamicRotate() const { return m_pHandler->AllowDynamicRotate(); }

bool COpenInventorSceneNode::CanAllowDynamicRotate() const { return m_pHandler->CanAllowDynamicRotate(); }

void COpenInventorSceneNode::DisplayGnomon(bool bDisplay) { m_pHandler->DisplayGnomon(bDisplay); }

bool COpenInventorSceneNode::DisplayGnomon() const { return m_pHandler->DisplayGnomon(); }

bool COpenInventorSceneNode::CanDisplayGnomon() const { return m_pHandler->CanDisplayGnomon(); }
...
IOpenGLFrame::DIMENSION COpenInventorSceneNode::Dimension() const { return IOpenGLFrame::DIM_3D; }

void COpenInventorSceneNode::Dimension(IOpenGLFrame::DIMENSION dim) {}

const double &COpenInventorSceneNode::PointSize() const { return m_pHandler->PointSize(); }

void COpenInventorSceneNode::PointSize(double size) { m_pHandler->PointSize(size); }

int COpenInventorSceneNode::Transparency() const { return m_pHandler->Transparency(); }
...
int COpenInventorSceneNode::DraggerTimeOut() const { return m_pHandler->DraggerTimeOut(); }

void COpenInventorSceneNode::DraggerTimeOut(int timeOut) { m_pHandler->DraggerTimeOut(timeOut); }

int COpenInventorSceneNode::IsoCount() const { return m_pHandler->IsoCount(); }
...
double COpenInventorSceneNode::IsoGap() const { return m_pHandler->IsoGap(); }

void COpenInventorSceneNode::IsoGap(double gap) {
  m_pHandler->IsoGap(gap);
  UpdateModelView();
}
void COpenInventorSceneNode::DisplayCoordinateAxis(bool bDisplay) { m_pHandler->DisplayCoordinateAxis(bDisplay); }

bool COpenInventorSceneNode::DisplayCoordinateAxis() const { return m_pHandler->DisplayCoordinateAxis(); }

bool COpenInventorSceneNode::CanDisplayCoordinateAxis() const { return m_pHandler->CanDisplayCoordinateAxis(); }

void COpenInventorSceneNode::SelectMeshMode(MeshMode mode) { m_pHandler->SelectMeshMode(mode); }

MeshMode COpenInventorSceneNode::MeshModeSelected() const { return m_pHandler->MeshModeSelected(); }

bool COpenInventorSceneNode::CanSelectMeshMode() const { return m_pHandler->CanSelectMeshMode(); }

void COpenInventorSceneNode::SelectDraggerTracker(bool yesOrNo) { m_pHandler->SelectDraggerTracker(yesOrNo); }

bool COpenInventorSceneNode::DraggerTrackerSelected() const { return m_pHandler->DraggerTrackerSelected(); }

bool COpenInventorSceneNode::CanSelectDraggerTracker() const { return m_meshNodeAssistant.canBeDisplayed(); }

void COpenInventorSceneNode::SelectMouseTracker(bool yesOrNo) { m_pHandler->SelectMouseTracker(yesOrNo); }

bool COpenInventorSceneNode::MouseTrackerSelected() const { return m_pHandler->MouseTrackerSelected(); }

bool COpenInventorSceneNode::CanSelectMouseTracker() const { return m_meshNodeAssistant.canBeDisplayed(); }
...
void COpenInventorSceneNode::SetDraggerConfined(bool yesOrNo) { m_pHandler->SetDraggerConfined(yesOrNo); }

bool COpenInventorSceneNode::LegendShown() const { return !LegendHidden(); }

bool COpenInventorSceneNode::LegendHidden() const { return m_pHandler->LegendHidden(); }

bool COpenInventorSceneNode::CanSelectHideLegend() const { return m_pHandler->CanSelectHideLegend(); }

void COpenInventorSceneNode::ShowLegend(bool yesOrNo) { HideLegend(!yesOrNo); }
...
void COpenInventorSceneNode::UpdateLegend() { m_pHandler->UpdateLegend(false, true); }

void COpenInventorSceneNode::SelectHandMode(bool yesOrNo) { m_pHandler->SelectHandMode(yesOrNo); }

bool COpenInventorSceneNode::HandMode() const { return m_pHandler->HandMode(); }

bool COpenInventorSceneNode::CanSelectHandMode() const { return m_pHandler->CanSelectHandMode(); }

bool COpenInventorSceneNode::ClipPlaneModeSelected() const { return m_pHandler->ClipPlaneModeSelected(); }

bool COpenInventorSceneNode::CanSelectClipPlaneMode() const { return m_pHandler->CanSelectClipPlaneMode(); }

void COpenInventorSceneNode::SelectClipPlaneMode(bool yesOrNo) { m_pHandler->SelectClipPlaneMode(yesOrNo); }

bool COpenInventorSceneNode::ContourLineModeSelected() const { return m_pHandler->ContourLineModeSelected(); }

bool COpenInventorSceneNode::CanSelectContourLineMode() const { return m_pHandler->CanSelectContourLineMode(); }
...
bool COpenInventorSceneNode::MeshLinesDisplayed() const { return m_pHandler->MeshLinesDisplayed(); }

bool COpenInventorSceneNode::CanSelectMeshLineDisplayMode() const { return m_pHandler->CanSelectMeshLineDisplayMode(); }
...
bool COpenInventorSceneNode::FormationOutlinesDisplayed() const { return m_pHandler->FormationOutlinesDisplayed(); }
...
void COpenInventorSceneNode::Fill(bool bFill) { m_pHandler->Fill(bFill); }

bool COpenInventorSceneNode::Fill() const { return m_pHandler->Fill(); }

bool COpenInventorSceneNode::CanFill() const { return m_pHandler->CanFill(); }
...
void COpenInventorSceneNode::LockRotationVector(const geo::IVector &vector) { m_pHandler->LockRotationVector(vector); }

const geo::IVector &COpenInventorSceneNode::LockRotationVector() const { return m_pHandler->LockRotationVector(); }

bool COpenInventorSceneNode::CanLockRotationVector() const { return m_pHandler->CanLockRotationVector(); }
...
TensorVectorMode COpenInventorSceneNode::ViewTensorVector() { return m_pHandler->ViewTensorVector(); }

void COpenInventorSceneNode::ViewEDirection() { m_pHandler->ViewEDirection(); }

void COpenInventorSceneNode::ViewNDirection() { m_pHandler->ViewNDirection(); }

void COpenInventorSceneNode::ViewZDirection() { m_pHandler->ViewZDirection(); }

void COpenInventorSceneNode::SaveHome() { m_pHandler->SaveHome(); }
void COpenInventorSceneNode::GoHome() { m_pHandler->GoHome(); }

void COpenInventorSceneNode::ViewBirdEyeDirection() { m_pHandler->ViewBirdEyeDirection(); }

void COpenInventorSceneNode::ViewProportional(bool bProportional) { m_pHandler->ViewProportional(bProportional); }

bool COpenInventorSceneNode::ViewProportional() const { return m_pHandler->ViewProportional(); }

void COpenInventorSceneNode::Lighting(bool bLighting) { m_pHandler->Lighting(bLighting); }

bool COpenInventorSceneNode::Lighting() const { return m_pHandler->Lighting(); }

bool COpenInventorSceneNode::CanEnableLighting() const { return m_pHandler->CanEnableLighting(); }
...
bool COpenInventorSceneNode::ShowValuesPerElement() const { return m_pHandler->ShowValuesPerElement(); }

bool COpenInventorSceneNode::CanShowValuesPerElement() { return m_pHandler->CanShowValuesPerElement(); }

void COpenInventorSceneNode::ViewBeachBalls(bool view) { m_pHandler->ViewBeachBalls(view); }

bool COpenInventorSceneNode::BeachBallsOnView() const { return m_pHandler->BeachBallsOnView(); }

bool COpenInventorSceneNode::CanViewBeachBalls() const { return m_pHandler->CanViewBeachBalls(); }

void COpenInventorSceneNode::Render() { m_pHandler->Render(); }

void COpenInventorSceneNode::Clear() {}

bool COpenInventorSceneNode::ShowColor() const { return m_pHandler->ShowColor(); }

void COpenInventorSceneNode::ShowColor(bool bShowColor) { m_pHandler->ShowColor(bShowColor); }

bool COpenInventorSceneNode::ShowValue() const { return !ShowColor(); }

void COpenInventorSceneNode::ShowValue(bool bShowValue) { ShowColor(!bShowValue); }

bool COpenInventorSceneNode::CanShowValue() { return m_pHandler->CanShowValue(); }

const IValueComponentBase *COpenInventorSceneNode::ValueComponent() const {
  return m_pValue ? m_pValue : m_pValueBlocked;
}

IValueComponentBase *COpenInventorSceneNode::ValueComponent() { return m_pValue ? m_pValue : m_pValueBlocked; }

const IValueComposite *COpenInventorSceneNode::ValueComposite() const { return m_pComposite; }

IValueComposite *COpenInventorSceneNode::ValueComposite() { return m_pComposite; }

const IColorScaleNode *COpenInventorSceneNode::ColorScale() const { return m_pColorScale; }

const CUnitNode *COpenInventorSceneNode::UnitNode() const { return m_pUnit; }

unsigned int COpenInventorSceneNode::IconId() const { return 0; }

unsigned int COpenInventorSceneNode::TypeId() const { return 0; }
...
void COpenInventorSceneNode::SetCursor(Qt::CursorShape shape) { m_pHandler->SetCursor(shape); }
...
void COpenInventorSceneNode::SetVectorScale(float scale) { m_pHandler->SetVectorScale(scale); }

void COpenInventorSceneNode::SetVectorVisibleFraction(float factor) { m_pHandler->SetVectorVisibleFraction(factor); }
...
const double COpenInventorSceneNode::MinValue() { return m_pHandler->MinValue(); }

const double COpenInventorSceneNode::MaxValue() { return m_pHandler->MaxValue(); }
...
void COpenInventorSceneNode::ShowGeologyLegend() { m_pHandler->ShowGeologyLegend(); }
...
void COpenInventorSceneNode::OnIsoValueComponentChanged(IValueComponentBase *pComponent, CDoubleQuantity::UNIT unit) {
  if (m_pHandler)
    m_pHandler->OnIsoValueComponentChanged(pComponent, unit);
}

void COpenInventorSceneNode::RemoveAllMeshes() {
  if (m_pHandler)
    m_pHandler->RemoveAllMeshes();
}

void COpenInventorSceneNode::ResetRenderAction() {
  if (m_pHandler)
    m_pHandler->ResetRenderAction();
}

bool COpenInventorSceneNode::CanRefreshVectorBaseScale() {
  if (m_pHandler)
    return m_pHandler->CanRefreshVectorBaseScale();
  return false;
}

void COpenInventorSceneNode::RefreshVectorBaseScale() {
  if (m_pHandler)
    m_pHandler->RefreshVectorBaseScale();
}

void COpenInventorSceneNode::AllowExclusiveDisplay(bool allow) { m_allowExclusiveDisplay = allow; }

bool COpenInventorSceneNode::AllowExclusiveDisplay() { return m_allowExclusiveDisplay; }
...
#else
  else if (dynamic_cast<CPressureSupportNode *>(pOpenGLNode) != 0 ||
           dynamic_cast<CDisplacementSupportNode *>(pOpenGLNode) != 0) {
    return EXCLUSIVE;
  }
#endif
...
void COpenInventorSceneNode::TimerCalled() { m_pHandler->TimerCalled(); }
...
void COpenInventorSceneNode::CreateHexaMeshRegion(bool rectangular) {
  ...
  COpenInventorSceneNodeAction *action =
      new COpenInventorSceneNodeAction(this, &COpenInventorSceneNode::CreatePolygonHexaMeshRegion, 0);
  m_pHandler->ActivatePolygonScreenDrawer(
      action, rectangular ? GeomecPolygonScreenDrawer::Rectangular
                          : GeomecPolygonScreenDrawer::Polygon); // action to be executed (and deleted) upon completion
                                                                 // of a rectanglescreendrawer
}
...
void COpenInventorSceneNode::EditHexaMainMeshRegion(CHexaMainMeshRegion &hexaMainMeshRegion) {
  ...
  COpenInventorSceneNodeAction *action = new COpenInventorSceneNodeAction(
      this, &COpenInventorSceneNode::ModifyRectangleHexaMainMeshRegion, &hexaMainMeshRegion);
  ...
  m_pHandler->ActivatePolygonScreenDrawer(
      action, GeomecPolygonScreenDrawer::Rectangular,
      &points); // action to be executed (and deleted) upon completion of a polygonscreendrawer
}
...
void COpenInventorSceneNode::EditVerticalNewWellPath(CNewWellPath *pWellPath) {
  ...
  COpenInventorSceneNodeAction *action =
      new COpenInventorSceneNodeAction(this, &COpenInventorSceneNode::EditVerticalNewWellPath, pWellPath);
...
void COpenInventorSceneNode::EditVerticalNewWellPath(void *user_data, const std::vector<geo::CPoint> &points) {
  ...
  if (wellPathPoint.X() < min.X() || wellPathPoint.X() > max.X() || wellPathPoint.Y() < min.Y() ||
      wellPathPoint.Y() > max.Y()) {
    ...
    if (user_data) {
      ...
      QString sName =
          dynamic_cast<const CNewWellPathEntry *>(model.GraphEntry(MD_NEW_WELLPATH))
              ->createUniqueName(CNewWellPathEntry::DEFAULT_NEWWELL_NAME, CNewWellPathEntry::DEFAULT_INITIAL_NUMBER);
  ...
  if (wellPath) {
    if (CanConnectItem(*wellPath))
      ConnectItem(*wellPath);
...
void COpenInventorSceneNode::EditHexaSubMeshRegion(CHexaSubMeshRegion &hexaSubMeshRegion) {
  ...
  COpenInventorSceneNodeAction *action = new COpenInventorSceneNodeAction(
      this, &COpenInventorSceneNode::ModifyPolygonHexaSubMeshRegion, &hexaSubMeshRegion);
  ...
  m_pHandler->ActivatePolygonScreenDrawer(
      action, GeomecPolygonScreenDrawer::Polygon,
      &points); // action to be executed (and deleted) upon completion of a polygonscreendrawer
}
...
void COpenInventorSceneNode::ModifyRectangleHexaMainMeshRegion(void *user_data,
                                                               const std::vector<geo::CPoint> &points) {
  assert(user_data);
  ...
  if (!geo::CRectangle::ValidRectangleData(points[0], points[2]))
    return;
  ...
  if (!((rectangle.Min().X() == meshRegionBoundary.Min().X()) &&
    ...
    if (vc_ptrs.size() > 0) {
      ...
      message.Format("One or more items like vertical wells, analysis points and/or cross sections\nare outside the "
                     "new boundary and will be deleted.\nContinue?");
...
void COpenInventorSceneNode::ShowHexaFormationTopMesh() {
  if (!Model().GraphEntry(MD_HEXA_FORMATION))
    return;
  // if(FormationPlane() == 0)
  {
...
void COpenInventorSceneNode::dehighlightMesh() { m_pHandler->dehighlightMesh(); }

void COpenInventorSceneNode::clearValueTrackerCellFacetSet() { m_pHandler->clearValueTrackerCellFacetSet(); }
...
void COpenInventorSceneNode::ScheduleConnectItem(COpenGLNodeBase &node) { m_scheduledItems.push_back(&node); }
...
const COpenInventorSceneNode::TOpenGLNodeSet &COpenInventorSceneNode::NodesInScene() const { return m_stOpenGLNodes; }
