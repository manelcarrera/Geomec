...
public:
  ...
  enum AddedOrDeleted { Added, Deleted };
...
public:
  ...
  void computeValueTrackerCellMarkerSet(const MiMeshUnstructuredI *mesh, const MbVec3d &coordinates, size_t cellId,
  ...
  void updateValueTracker(const MiMeshUnstructuredI *mesh, const OIDIMesh *oidiMesh, const MbVec3d &coordinates,
                          size_t cellId);
...
private:
  ...
  enum ComponentType { ColorComponent, IsoComponent };
  ...
  void ActUponValueComponentChanged(const IValueComponentBase *pValueComponent, CDoubleQuantity::UNIT unit,
                                    ComponentType why, bool removeDataSets = false);
