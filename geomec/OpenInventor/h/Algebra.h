...
public:
	...
	public:
		Data()
			: alfa(0.0),beta(0.0),d1(0.0),distance(0.0), intersection(MbVec3d(0.0,0.0,0.0))
		{}
		...
		{}
		Data( const Data& d )
			: alfa(d.alfa),beta(d.beta),d1(d.d1),distance(d.distance), intersection(d.intersection)
		{}
		void _print();
	};
	...
	static MbVec3d vector( MbVec3d p_ini, MbVec3d p_end ){ return MbVec3d( p_end[0]-p_ini[0], p_end[1]-p_ini[1], p_end[2]-p_ini[2] ); }
	static double alfa( MbVec3d v1, MbVec3d v2 ){ return acos( cos_a( v1, v2 ) ); }
	static double beta( double alfa ){ return M_PI - 2*alfa; }
	static double distance_to_intersection( double distance1, double alfa, double beta ){ return distance1*(sin(alfa)/cos(beta)); }
	...
	static std::vector<MbVec3d> points_mod_DEPRECATED( const OIDIGeometryI& g, OIDIVectorSetI* vectors, double scale_gen, double scale_def );
	static std::vector<MbVec3d> points_mod( const OIDIGeometryI& g, std::vector<MbVec3d> deformation_v, double scale_gen, double scale_def );
	static std::vector<MbVec3d> points_mod_SAME( const OIDIGeometryI& g, std::vector<MbVec3d> deformation_v, double scale_gen, double scale_def );
