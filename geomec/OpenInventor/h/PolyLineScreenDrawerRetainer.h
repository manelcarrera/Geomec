...
/// As the real world coordinates are stored in the polylinedrawer we can only effectively handle one polylinedrawer at
/// a time and only one retainer per polylinedrawer at a time. So, there will always be at most one PolyLineDrawer that
/// "CanWork()". Requires to be in TopView mode in order to "CanWork()" as functions used work only for TopView mode.
