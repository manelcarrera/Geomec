...
CWellCasingCementObserver::CWellCasingCementObserver(CWellCasingCement &node, CTreeCtrl &ctrl, const BOOL rename,
                                                     const enum REMOVE_TYPE remove, HTREEITEM hParent,
                                                     HTREEITEM hInsertAfter, BOOL bInitialUpdate,
                                                     BOOL bSortAfterInsertion)
    : TWellCasingCementObserver(node, ctrl, rename, remove, hParent, hInsertAfter, bInitialUpdate,
                                bSortAfterInsertion) {
  ...
  new TPressureEnumerator(*this, _T("Pressure"), IDI_PRESSURES);

  new TTemperatureEnumerator(*this, _T("Temperature"), IDI_TEMPERATURES);

  new TCalcTemperEnumerator(*this, _T("Calculated temperatures"), IDI_CALC_TEMPERS);

  new TStrainEnumerator(*this, _T("Strains"), IDI_VOLUMETRIC_STRAINS);

  new TMaterialEnumerator(*this, _T("Materials"), IDI_ROCK_MATERIALS);
}