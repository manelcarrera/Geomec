...
CAttriWellZoomInModelDlg::CAttriWellZoomInModelDlg(IWellModel &model, const CString &strCaption, CWnd *pParent)
    : CDialog(IDD_ATTRI_WELL_ZOOMIN_MODEL, pParent), m_model(model), m_strCaption(strCaption) {}
...
void CAttriWellZoomInModelDlg::DoDataExchange(CDataExchange *pDX) {
  ...
  if (!pDX->m_bSaveAndValidate) {
    ...
    if (!bInsideModel)
      GetDlgItem(IDC_INVALID_AHD_WARNING)
          ->SetWindowText("Both the start AHD and the end AHD must be inside the parent model");
    else if (!bStartToEnd)
...
void CAttriWellZoomInModelDlg::OnChangeStartAHD() {
  ...
  double d;
  if ((_sntscanf_s(sText, sText.GetLength(), _T("%lf"), &d) == 1) && (d != previousValue)) {
    previousValue = d;
    UpdateData(TRUE);
    UpdateData(FALSE);
  }
