...
template <class LOAD, class GRADIENT_QTY> class CAttriWellCasingInternalLoadDlg : public CAttributesTemplate<LOAD> {
public:
...
template <class LOAD, class GRADIENT_QTY> BOOL CAttriWellCasingInternalLoadDlg<LOAD, GRADIENT_QTY>::OnInitDialog() {
...
void CAttriWellCasingInternalLoadDlg<LOAD, GRADIENT_QTY>::DoDataExchange(CDataExchange *pDX) {
  ...
  if (!pDX->m_bSaveAndValidate) {
    ...
    strReferenceDepth.Format("%g",
                             qnLength.Convert(Copy().Gradient().ReferenceDepth(), unit, CDoubleQuantity::SI_UNIT));
  ...
  if (pDX->m_bSaveAndValidate) {
    if ((Copy().DistributedSize() == 0 || !Copy().DistributedOnly()) && Copy().LoadingMode() == LOAD::LM_GRADIENT) {
      Copy().Gradient().ReferenceValue(ControlValue(pDX, strReferenceValue, IDC_ED_CONSTANT_PRESSURE) /
                                       Copy().Gradient().UnitFactor(unit));
      Copy().Gradient().ReferenceDepth(qnLength.Convert(ControlValue(pDX, strReferenceDepth, IDC_ED_CONSTANT_DEPTH),
                                                        CDoubleQuantity::SI_UNIT, unit));
      Copy().Gradient().Gradient(
          qnGradient.Convert(ControlValue(pDX, strGradient, IDC_ED_CONSTANT_GRADIENT), CDoubleQuantity::SI_UNIT, unit));
    }
...
template <class LOAD, class GRADIENT_QTY>
double CAttriWellCasingInternalLoadDlg<LOAD, GRADIENT_QTY>::ControlValue(CDataExchange *pDX, const CString &strValue,
                                                                         unsigned int idc) const {
...
template <class LOAD, class GRADIENT_QTY> void CAttriWellCasingInternalLoadDlg<LOAD, GRADIENT_QTY>::EnableControls() {
...
template <class LOAD, class GRADIENT_QTY> void CAttriWellCasingInternalLoadDlg<LOAD, GRADIENT_QTY>::OnRadio() {
  UpdateData(TRUE);
  EnableControls();
  UpdateData(FALSE);
}
