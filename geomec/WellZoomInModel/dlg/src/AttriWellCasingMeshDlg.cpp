...
static TCasingType casingtypes[] = {
    {"7\"", 0.1778}, {"10\"", 0.254}, {0, 0} // terminator
};
...
CAttriWellCasingMeshDlg::CAttriWellCasingMeshDlg(CWellCasingMesh &mesh, CWnd *pParent /*=NULL*/)
    : CAttributesTemplate<CWellCasingMesh>(IDD_ATTRI_CASINGMESH, mesh, pParent),
      ...
      m_numberOfCementElements(Source().numberOfCementElements()), m_spinNumberOfSteelElements(),
      m_spinNumberOfCementElements(), m_outerDiameterRock(Source().outerDiameterRock()) {}
...
BOOL CAttriWellCasingMeshDlg::OnInitDialog() {
  ...
  m_spinNumberOfSteelElements.SetRange(MINIMUM_NUMBER_OF_STEEL_ELEMENTS, MAXIMUM_NUMBER_OF_STEEL_ELEMENTS);
  m_spinNumberOfCementElements.SetRange(MINIMUM_NUMBER_OF_CEMENT_ELEMENTS, MAXIMUM_NUMBER_OF_CEMENT_ELEMENTS);
...
const QString OUTER_DIAMETER_ROCK_ERROR =
    QObject::tr("The outer diameter of "
...
void CAttriWellCasingMeshDlg::DoDataExchange(CDataExchange *pDX) {
  ...
  DDX_Control(pDX, IDC_SPIN_NUMBER_OF_STEEL_ELEMENTS, m_spinNumberOfSteelElements);
  DDX_Control(pDX, IDC_SPIN_NUMBER_OF_CEMENT_ELEMENTS, m_spinNumberOfCementElements);
  ...
  if (pDX->m_bSaveAndValidate) {
    ...
    if (!tangentialelementtypes[nIdxNumTangential].supported) {
      QString str = QString("Warning: %1 tangential elements is a legacy value and will probably lead to divergence in "
                            "the calculation")
                        .arg(tangentialelementtypes[nIdxNumTangential].numelements);
      _m()->msg(str);
    }
    ...
    if ((Copy().HoleDiameter() > outerDiameterRock) ||
        (outerDiameterRock > (2 * (dynamic_cast<CWellCasingModel &>(Copy().Model()).Radius())))) {
      QString errorMessage = QString(OUTER_DIAMETER_ROCK_ERROR)
                                 .arg(CLengthQuantity(2 * (dynamic_cast<CWellCasingModel &>(Copy().Model()).Radius()))
                                          .Value(GetGeomecDoc()->UnitNode().Unit()))
                                 .arg(lengthQuantity.UnitName(GetGeomecDoc()->UnitNode().Unit()).c_str());
