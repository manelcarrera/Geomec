...
namespace IT {
namespace reservoir {
Reservoir &get(int reservoir_id, int surface_id,
               TReservoirPropertyV properties_v = {Thickness, PoissonsRatio, YoungsModulus, PorePressure},
               int depletion_id = 0);
namespace properties {
...
void add(int reservoir_id, const IT_Data::TPointsV &points_v, const IT_Data::TValuesV &pressures_v,
         eDepletionType _type = Pore_Pressure, int depletion_id = LAST_DEPLETION_STAGE, bool is_branch = true);
void add(
    int reservoir_id,                     // Formation to wich add the provided pressure
...
get_ONE(eComposite composite_, CDepletionStage *pStage,
...
// [ SURFACE ]
namespace surface {
IT_Data::T3Values value(const geo::IFace &face, TDisplacementComponentV &component_v, CDepletionStage *pStage,
...
namespace IT {
namespace displacement {
T_Elem_And_Val_And_Z BD_elem_and_val_and_z(const geo::IPoint &point, geo::CBodyGroup::CSideSurface &surface,
                                           IValueComposite *composite, TDisplacementComponentV &component_v);
T_Elem_And_Val_And_Z_V BD_elems_and_vals_and_z(int reservoir_id, int composite_idx, int surface_id,
                                               TDisplacementComponentV component_v, int volume_idx,
...
namespace pointset {
int add(const IT_Data::TPointsV &points_v, eDepletionType _type, int depletion_id);
