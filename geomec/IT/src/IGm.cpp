...
CStringV *list(eList type_, int param) {
  switch (type_) {
  case L_Surface:
    return model::formation::surface::list(param);
  case L_Formation:
    return model::formation::list(false);
  case L_Reservoir:
    return model::formation::list(true);
  case L_Depletion:
    return model::depletion::list();
  case L_BD_Composite:
    return res::composite::names_list((eComposite)param);
  default:
    return new CStringV();
  }
}
...
CFormationBase *get(int idx, bool bReservoir) {
  if (model::get()) {
    ...
    TFormationBaseEntry::TNodeSet formations =
        static_cast<TFormationBaseEntry *>(model::get()->GraphEntry(MD_BASE_FORMATION))->EntryNodes();
...
std::pair<TCompositeV, TPointSetV> list(eComposite composite_) {
  ...
  if (model::get()) {
    TPointSetEntry::TNodeSet point_set_set =
        static_cast<TPointSetEntry *>(model::get()->GraphEntry(MD_BASE_POINTSET))->EntryNodes();
...
const IResult *result(eComposite composite_, CDepletionStage *pStage) {
  ...
  if (model::get()) {
    ...
    case C_TotalStress:
      result = &tree.TotalStress().Components();
      break;
    case C_TotalStressChange:
      result = &tree.TotalStressChange().Components();
      break;
    case C_TotalStrain:
      result = &tree.TotalStrain().Components();
      break;
    case C_PlasticStrain:
      result = &tree.PlasticStrain().Components();
      break;
    case C_EffectiveStress:
      result = &tree.EffectiveStress().Components();
      ;
      break;
    case C_EffectiveStressChange:
      result = &tree.EffectiveStressChange().Components();
      ;
      break;
...
namespace component {
const IResultComponent *get(eComposite composite_id, CDepletionStage *pStage,
  ...
  if (result) {
    CAnalysisType::TAnalysisType anType = model::get()->ResultRegister().ResultsAvailable(CAnalysisType::AT_NONLIN)
...
const IVectorResult::CVectorComponent *get_ONE(eComposite composite_, CDepletionStage *pStage,
                                               IVectorResult::VECTOR_COMPONENT _component) {
...
const IVectorResult::CFullVectorComponent *get_FULL(eComposite composite_, CDepletionStage *pStage) {
  const IVectorResult::CFullVectorComponent *component = dynamic_cast<const IVectorResult::CFullVectorComponent *>(
      get(composite_, pStage, IVectorResult::VECTOR_COMPONENT::VC_FULLVECTOR));
  return component;
}
...
IT_Data::T3ValuesV values(eComposite composite_id, int composite_idx, int depletion_id,
...
IT_Data::T3Values value(const geo::CPoint &point, TDisplacementComponentV &component_v, CDepletionStage *pStage,
...
double value(const geo::CPoint &point, eComponent component_id, CDepletionStage *pStage, eComposite composite_id) {
  ...
  const IVectorResult::CVectorComponent *component = dynamic_cast<const IVectorResult::CVectorComponent *>(
      res::composite::component::get(composite_id, pStage, type_));
...
double value(const geo::IPoint &point, IValueComposite *composite, eComponent component_) {
  return composite->Component(component_).ScalarData().ValuePoint(point).Value();
}
...
namespace pointset {
bool create(int id, const IT_Data::TPointsV &points_v) { return PointSetManager::instance()->create(id, points_v); }
...
bool remove(int id) { return PointSetManager::instance()->remove(id); }
...
bool link(int ps_id, int idx, int reservoir_id, int depletion_id, bool link_) {
  ...
  if (pStage && pFormation) {
    gm::draw(Disable);
    PointSetManager *ps_mgr = PointSetManager::instance();
    res = link_ ? ps_mgr->link(ps_id, idx, pFormation, pStage) : ps_mgr->unlink(ps_id, idx, pFormation, pStage);
    gm::draw(Enable);
  }
  return res;
}
...
int id(eReservoirPropMat material) {
  switch (material) {
  case P2_PoissonsRatio:
    return IDT_VALUETYPE_POISSONS_RATIO;
  case P2_YoungsModulus:
    return IDT_VALUETYPE_YOUNGS_MODULUS;
  case P2_Cm:
    return IDT_VALUETYPE_ELASCOMPRES;
