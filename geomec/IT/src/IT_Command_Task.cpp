...
namespace{
	...
	IT_Data* data_v1( Command* cmd )
	{
		...
		switch( cmd->cmd() )
		{
			case Command::IT1_List_Surface:{				data = gm::list( gm::eList::L_Surface, p[0] ); break; } // [ res_idx ]
			case Command::IT1_List_Reservoir:{				data = gm::list( gm::eList::L_Reservoir );	break; }  
			case Command::IT1_List_Measured_Displacement:{	data = gm::list( gm::eList::L_BD_Composite, gm::eComposite::C_Displacement ); break; }
			case Command::IT1_List_Depletion:{				data = gm::list( gm::eList::L_Depletion );	break; }

			case Command::IT1_Get_Reservoir:{				data = gm::model::formation::IT::reservoir::get(	p[0], p[1],				properties_v2() ); break; } // [ res_idx, sur_idx ]
			case Command::IT1_Get_Displacement:{			data = gm::res::composite::IT::displacement::get(	p[0], p[1], p[2], p[3], components_v2() ); break; } // [ res_idx, sur_idx, depletion_id, dis_idx, meas_dis_idx ] 
			case Command::IT1_Get_Measured_Displacement:{	data = gm::bd::composite::IT::displacement::get(	p[0], p[1], p[2],		components_v2() ); break; } // [ res_idx, meas_dis_idx, sur_idx ]
			/* NOT USED */
			case Command::IT1_Get_Material_Parameter:{		data = gm::bd::material::surface::param( p[0], p[1], (gm::eReservoirPropMat)p[2], p[3] ); break; } // [ res_idx, 0, mat_param_id, sur_idx ]
			default:
	...
	IT_Data* data_v2( Command* cmd )
	{
		...
		switch( cmd->cmd() )
		{
			case Command::Model_Elems:{		data = new GenData_3i( gm::model::formation::IT::elems(					p[0], p[1] ) ); break; } // [ res, sur ]
			case Command::Model_Points:{	data = new GenData_3d( gm::model::formation::surface::points(			p[0], p[1] ) ); break; } // [ res, sur ]
			case Command::Model_Prop_Form:{	data = new GenData_1d( gm::model::formation::surface::properties::get(	p[0], p[1], (gm::eReservoirPropForm)p[2] ) ); break; } // [ res, sur, prop ]
			case Command::Model_Prop_Mat:{	data = new GenData_1d( gm::model::formation::surface::properties::get(	p[0], p[1], p[2], (gm::eReservoirPropMat)p[3] ) ); break; } // [ res, sur, dep, prop ]
			case Command::Model_Prop_Dep:{	data = new GenData_1d( gm::model::formation::surface::properties::get(	p[0], p[1], p[2], (gm::eReservoirPropDep)p[3] ) ); break; } // [ res, sur, dep, prop ]
			
			//case Command::Res_Values:{		data = new GenData_1d( gm::res::composite::component::IT::values( p[0], p[1], p[2], p[3], (gm::eComposite)p[4], (gm::eComponent)p[5] ) ); break; } // [ res, sur, dep, bd_composite_idx, composite, component ]
			// points from PS not mapped to the surface
			case Command::Res_Available:{	data = new GenData_1i( IT_Data::TIntV{ gm::res::available() ? 1:0 } ); break; } 
			case Command::Res_Values:{		data = new GenData_1d( gm::res::composite::component::values( (gm::eComposite)p[0], (gm::eComponent)p[1], p[2], p[3] ) ); break; } // [ composite, component, bd_composite_idx, dep ]

			case Command::DB_Points:{		data = new GenData_3d( gm::bd::composite::points(		(gm::eComposite)p[0], p[1] ) ); break; } // [ composite, composite_idx ] 
			case Command::DB_Values:{		data = new GenData_1d( gm::bd::composite::component::values(		(gm::eComposite)p[0], p[1], (gm::eComponent)p[2] ) ); break; } // [ composite, composite_idx, component ]
			case Command::DB_Elems:{		data = new GenData_1i( gm::bd::composite::IT::elems(	p[0], p[1], (gm::eComposite)p[2], p[3] ) ); break; } // elems_id -> [ res, sur, composite, composite_idx ] 

			case Command::DB_PointSet_Values:{		data = new GenData_1d( gm::bd::pointset::values(		p[0], p[1] ) ); break; } // [ ps_id, values_id ]
			case Command::DB_PointSet_ValuesList:{	data = new GenData_1i( gm::bd::pointset::values_list(	p[0] ) ); break; } // [ ps_id ]

			case Command::DB_Material_List:{		data = gm::bd::material::list(); break; }
			case Command::DB_Material_GetValues:{	data = new GenData_1d( gm::bd::material::values( p[0] ) ); break; } // [ idx ]

			case Command::Gen_List:{		data = gm::list( (gm::eList)p[0], p[1] ); break; } // [ type, idx ] 
			default:
	...
	void exe( Command& cmd )
	{
		...
		{
			//v1
			case Command::IT1_Set_Depletion:{			gm::model::depletion::IT::pressure::add( p[0], (gm::eDepletionType)p[1], p[2], p[3] == 1, static_cast< Depletion* >( cmd.data() ) ); break; }
			case Command::IT1_Exe_Run_Geomec:			gm::model::run(); break;
			case Command::IT1_Exe_Run_Geomec_Step:		gm::model::run_step(); break;
			case Command::IT1_Exe_Wait_End_Run_Geomec:	gm::model::wait_end_run(); break;
			case Command::IT1_Exe_Save_Displacement: break;
			case Command::IT1_Exe_Remove_Depletion:{	gm::model::depletion::remove(); break; /*LAST*/ }
			...
			case Command::DB_PointSet_Remove:{ gm::bd::pointset::remove( p[0] ); break; } 						
			case Command::DB_PointSet_AddValues:
			{ 
			...
			case Command::DB_PointSet_RemoveValues:{ gm::bd::pointset::remove_vals( p[0], p[1] ); break;	} // (ps_id, values_idx)
			case Command::DB_PointSet_ReplaceValues:
			{ 
				GenData_1d* d = static_cast< GenData_1d* >( cmd.data() );
			...
			case Command::DB_PointSet_Link:
			case Command::DB_PointSet_Unlink:{	gm::bd::pointset::link( p[0], -1, p[1], p[2], cmd.cmd() == Command::DB_PointSet_Link ); break; } //-1: values_id
			case Command::Model_Dep_Add:{		gm::model::depletion::add( p[1] == 1 );	break; } // ( output )
			case Command::Model_Dep_Remove:{	gm::model::depletion::remove( p[1] ); break; } // ( id )
			case Command::Model_Dep_Output:{	gm::model::depletion::output( p[1], p[1] == 1 ); break; } // ( id, output )

			case Command::DB_Material_List:{		gm::bd::material::list(); break; }
			case Command::DB_Material_Create:{		gm::bd::material::create( p[0] ); break; }
			case Command::DB_Material_Remove:{		gm::bd::material::remove( p[0] ); break; } 						
			case Command::DB_Material_SetValues:{	gm::bd::material::values( p[0], static_cast< IT::Material* >( cmd.data() ) ); break; }
			case Command::DB_Material_Link:{		gm::bd::material::link( p[0], p[1] ); break; } // ( idx, res_id )
			case Command::DB_Material_Unlink:{		gm::bd::material::unlink( p[0], p[1] ); break; } // (idx, res_id)
...
IT_Command_Task::IT_Command_Task( GeomecPythonInterface* gm_iface_, Command _cmd, HWND hWnd_ ) : gm_iface( gm_iface_ ), cmd( _cmd ), hWnd( hWnd_ ){}
...
tbb::task* IT_Command_Task::execute() 
{
	if( gm::model::get() )
	{
		...
		Printer::instance()->debug("Req: %s | Treated: %.2fs.", cmd.name().c_str(), std::chrono::duration_cast< std::chrono::milliseconds >( end - begin ).count() / 1000.0 );
