...
void element(const geo::IElement &element) {
  Printer::instance()->debug("%d %d %d %d", element.PointIndex(0), element.PointIndex(1), element.PointIndex(2),
                             element.PointIndex(3));
}
...
void point(const int point_index, const geo::IPoint &point) {
  Printer::instance()->debug("%d\t%E\t%E\t%E", point_index, point.X(), point.Y(), point.Z());
}
...
void point_value(const TPointValueV point_value_v) {
  for (TPointValueV::const_iterator it = point_value_v.begin(); it != point_value_v.end(); ++it) {
    ...
    Printer::instance()->debug("%E\t%E\t%E\t\t%E", point.X(), point.Y(), point.Z(), value.Value());
  }
}
...
void elem_values(const TElemValuesV elem_values_v) {
  ...
  for (TElemValuesV::const_iterator it = elem_values_v.begin(); it != elem_values_v.end(); ++it) {
    ...
    Printer::instance()->debug("%d\t%E\t%E\t%E\t%E", elem.Index(), values[0].Value(), values[1].Value(),
                               values[2].Value(), values[3].Value()); // FIXME: hard-coded for tetras
  }
}
}; // namespace print
...
void get_v1() {
  ...
  // MD_BASE_FORMATION ) )->EntryNodes(); CExportResultData exportResultData( model );
  ...
  // CResultTree::CDisplacementResult& displacement = dynamic_cast< CResultTree::CDisplacementResult& >( output.Child( 0
  // ) ); //displacements

  // b)
  CResultTree::CDisplacementResult &displacement =
      (CResultTree::CDisplacementResult &)r.Displacement(); // displacement it's a composite

  // IValueComponenBase& comp = displacement.Component();

  // enum VECTOR_COMPONENT { VC_X = 0, VC_Y, VC_Z, VC_LENGTH, VC_FULLVECTOR };
  IVectorResult::CVectorComponent &component_x =
      dynamic_cast<IVectorResult::CVectorComponent &>(displacement.childAt(1)); // FIXME: run through the array.
  ...
  TFormationBaseEntry::TNodeSet formations =
      static_cast<TFormationBaseEntry *>(base::model().GraphEntry(MD_BASE_FORMATION))->EntryNodes();
  for (TFormationBaseEntry::TNodeSet::iterator it = formations.begin(); it != formations.end(); ++it) {
    ...
    if (name.compare(_name) == 0) {
      for (int e = 0; e < es2.ElementSize(); e++) // elems
      {
        ...
        std::vector<geo::CValue> _vector = component_x.ValueElement(
...
TPointValueV get(CFormationBase &formation, IVectorResult::CVectorComponent &component) {
  ...
  for (int e = 0; e < es2.ElementSize(); e++) // elems
  {
    ...
    for (std::vector<geo::CValue>::iterator it = _vector.begin(); it != _vector.end(); ++it) {
      ...
      displacements_v.push_back(TPointValueP(point, _value));
...
void do_something() {
  ...
  //-------------------------------------------------------------- Displacement: depending on the (1)depletion and the
  selected (2)component CDepletionStage& pStage = model().InitialDepletionStage().Next();
  IVectorResult::CVectorComponent& component = foo.get_displacement_component( &pStage,
  ...
  //-------------------------------------------------------------- Pressures: depending on the (1)depletion and the
  // selected (2)component
...
/*void IT_Geomec_Interface::print_formation_v1( CFormationBase& formation )
{
    ...
    long double (*points)[	POINTS_COLS ]		= wrapper::_create_array< long double (*)[ POINTS_COLS ],		_NUM_POINTS,
POINTS_COLS >(); long double (*elems)[	POINTS_PER_ELEM ]	= wrapper::_create_array< long double (*)[ POINTS_PER_ELEM ],
_NUM_ELEMS,		POINTS_PER_ELEM >();
