...
MaterialManager::MaterialManager(CModelBase &model)
    : m_model(model) //, m_nodes( nullptr )
{
...
std::vector<std::string> MaterialManager::list() {
  ...
  // Printer::instance()->debug("mat mgr : materials_list : [%d] [%s]", i, mat_entry->Material( i
...
void MaterialManager::connect_materials_to_formations(const std::vector<IMaterial *> materials) {
  ...
  for (TFormationBaseEntry::TNodeSet::const_iterator formation = formationBaseEntry->EntryNodes().begin();
       formation != formationBaseEntry->EntryNodes().end(); ++formation, ++material_idx) {
...
void MaterialManager::copy_diff_type_mat_lib(IMaterial *src, IMaterial *dst) {
  ...
  copy_material_library_value(src_mat_lib, dst_mat_lib, IDT_VALUETYPE_YOUNGS_MODULUS, IDT_VALUETYPE_YOUNGMODULUS_NORM,
                              IDT_VALUETYPE_YOUNGMODULUS_TRANS);

  bool res =
      copy_material_library_value(src_mat_lib, dst_mat_lib, IDT_VALUETYPE_YOUNG_DECOMP,
                                  IDT_VALUETYPE_YOUNGMODULUS_NORM_DECOMP, IDT_VALUETYPE_YOUNGMODULUS_TRANS_DECOMP);
  if (!res)
    copy_material_library_value(src_mat_lib, dst_mat_lib, IDT_VALUETYPE_YOUNGS_MODULUS,
                                IDT_VALUETYPE_YOUNGMODULUS_NORM_DECOMP, IDT_VALUETYPE_YOUNGMODULUS_TRANS_DECOMP);

  copy_material_library_value(src_mat_lib, dst_mat_lib, IDT_VALUETYPE_POISSONS_RATIO, IDT_VALUETYPE_POISSONRATIO_NORM,
                              IDT_VALUETYPE_POISSONRATIO_TRANS);

  res = copy_material_library_value(src_mat_lib, dst_mat_lib, IDT_VALUETYPE_POISSON_DECOMP,
                                    IDT_VALUETYPE_POISSONRATIO_NORM_DECOMP, IDT_VALUETYPE_POISSONRATIO_TRANS_DECOMP);
  if (!res)
    copy_material_library_value(src_mat_lib, dst_mat_lib, IDT_VALUETYPE_POISSONS_RATIO,
                                IDT_VALUETYPE_POISSONRATIO_NORM_DECOMP, IDT_VALUETYPE_POISSONRATIO_TRANS_DECOMP);

  copy_material_library_value(src_mat_lib, dst_mat_lib, IDT_VALUETYPE_SHEARMODULUS,
...
bool MaterialManager::copy_material_library_value(CLibraryMaterial *old_mat_lib, CLibraryMaterial *new_mat_lib,

                                                  int from_type,

                                                  int to_type_1, int to_type_2) {
  ...
  for (int i = mlMatModel::MM_LINEAR; i <= mlMatModel::MM_CAMCLAY_CREEP; i++) {
    ...
    if (lib_mat) {
      ...
      for (size_t j = 0; j < size; j++) {
        ...
        // Printer::instance()->debug("{ %d, %d, %d, %s, %f }", i, j, param2.ValueTypeID(),
        // param.Name().toStdString().c_str(), param.Value() ); Printer::instance()->debug("{ %d, %d, %d, %s }\n", i, j,
        // param2.ValueTypeID(), param.Name().toStdString().c_str() );
        ...
        if (int(param2.Value()) != param2.Value()) // is decimal
        {
          ...
          dump += QString::asprintf("\t[%d, %d, \"%s\", %s],\n", j, param2.ValueTypeID(),
                                    param2.Name().toStdString().c_str(), str.c_str());
          vals_v.push_back(str);
        } else // is int
        {
          dump += QString::asprintf("\t[%d, %d, \"%s\", %d],\n", j, param2.ValueTypeID(),
                                    param2.Name().toStdString().c_str(), (int)param2.Value());
          vals_v.push_back(std::to_string((int)param2.Value()));
        }
        // dump += QString::asprintf("\t[%d, %d, \"%s\", %d],\n", j, param2.ValueTypeID(),
        // param2.Name().toStdString().c_str(), param2.Value() );
      }
      dump.chop(2); // remove "\n,"
...
std::vector<double> MaterialManager::values(int idx) {
  ...
  for (size_t i = 0; i < size; i++) {
    ...
    Printer::instance()->debug("mat mgr : values : ids:[%d] : name:[%s] : id:[%d] : val:[%f]", idx,
                               param.Name().toStdString().c_str(), param2.ValueTypeID(), param.Value());
  }
  return vals;
}