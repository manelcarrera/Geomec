...
bool PointSetManager::create( int id, const IT_Data::TPointsV& points_v, IPointSet::DIMENSION dim, std::string name ) // int > IT_Geomec_Interface::eDepletionType
{
	...
	if( m_model )
	{
		if( !exist( id ) )
		{
			if( points_v.size() )
			{
				CPointSet* ps = new CPointSet( QString::fromStdString( name.empty() ? DEFAULT_POINT_SET_NAME : name ), *m_model, 0 /*size*/, dim ); // name = "Inversion Tool Pressure"
...
bool PointSetManager::add_values( int id, int _type, const IT_Data::TValuesV& values_v )
{
	...
	if( ps )
	{
		...
		CValueType* composite = m_factory->BuildValueType( *ps, composite_info.id, QString::fromStdString( composite_info.name ) );
...
PointSetManager::PS_Composite PointSetManager::info( int _type ) //IT_Geomec_Interface::eDepletionType
{
	...
	{
		case gm::eDepletionType::D_Strain:			comp = PS_Composite{ IDT_VALUETYPE_VOLUMETRICSTRAIN,	DEFAULT_STRAIN_NAME };		break;
		case gm::eDepletionType::D_Compressibility:	comp = PS_Composite{ IDT_VALUETYPE_ELASCOMPRES,			DEFAULT_MATERIAL_NAME };	break;
		case gm::eDepletionType::D_Pore_Pressure:	comp = PS_Composite{ IDT_VALUETYPE_PRESSURE,			DEFAULT_PRESSURE_NAME };	break;
		case gm::eDepletionType::D_Temperature:		comp = PS_Composite{ IDT_VALUETYPE_TEMPERATURE,			DEFAULT_TEMPERATURE_NAME };	break;
	}
	return comp;
}
...
bool PointSetManager::link_dep_to_ps_composite( int ps_id, int values_id, CFormationBase* pFormation, CDepletionStage* pStage )
{
	bool res = false;
	...
	if( pStage && pFormation )
	{
		...
		if( info )
		{
			...
			if( exist_val( ps_id, values_id ) ) //Take care with this
			{
				...
				{
					case gm::eDepletionType::D_Strain:			res=pFormation->Strain(			*pStage).ConnectItem( *values.composite );	break;
					case gm::eDepletionType::D_Compressibility:	res=pFormation->Material(		*pStage).ConnectItem( *values.composite );	break;
					case gm::eDepletionType::D_Pore_Pressure:	res=pFormation->Pressure(		*pStage).ConnectItem( *values.composite );	break;
					case gm::eDepletionType::D_Temperature:		res=pFormation->UserTemperature(*pStage).ConnectItem( *values.composite );	break;
				}
			}
		}
	}
...
{
	CGraphEntry& point_set_entry = *m_model->GraphEntry( MD_BASE_POINTSET ); // this is also to retrieve point set, loop and compare with the name
	CPointSet* ps = get( ps_id );
	ps->LinkTo( point_set_entry );	
	Printer::instance()->debug("[PS] link_ps_to_model : ps_id: %d ", ps_id );
}

bool PointSetManager::unlink( int ps_id, int values_id, CFormationBase* pFormation, CDepletionStage* pStage ) // Jeroen ? (1) unlink (2) wich order?
{
	bool res = unlink_dep_from_ps_composite( ps_id, values_id, pFormation, pStage );
	return res;
}

bool PointSetManager::unlink_dep_from_ps_composite( int ps_id, int values_id, CFormationBase* pFormation, CDepletionStage* pStage ) // Jeroen ? (1) unlink (2) wich order?
{
	bool res = false;
	...
	if( pStage && pFormation )
	{
		...
		if( info )
		{
			...
			if( exist_val( ps_id, values_id ) ) //Take care with this
			{
				...
				switch( (gm::eDepletionType)values.composite_id_usr )
				{
					case gm::eDepletionType::D_Strain:			pFormation->Strain(			*pStage).UnLink( *values.composite );	break;
					case gm::eDepletionType::D_Compressibility:	pFormation->Material(		*pStage).UnLink( *values.composite );	break;
					case gm::eDepletionType::D_Pore_Pressure:	pFormation->Pressure(		*pStage).UnLink( *values.composite );	break;
					case gm::eDepletionType::D_Temperature:		pFormation->UserTemperature(*pStage).UnLink( *values.composite );	break;
				}
				res = true;
			}
		}
