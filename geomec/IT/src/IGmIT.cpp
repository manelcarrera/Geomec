...
/*
  ...
  3.1
    PP,Cm,strain:	IT_Data::TValuesV formation::surface::properties::values( int reservoir_id, int surface_id, int
  depletion_id, eReservoirProperty prop )

    Left to do: Cm

  3.2
    Jeroen: If stage removed do we need to 'unlink' ps from depletion or is it automatically done ? -> NOT NEDDEDeven if
  orphan info (UNUSED but present)
  ...
  4. Results: + ->
      ...
      IT_Data::TValuesV gm::res::composite::component::surface::values( int reservoir_id, int surface_id, int
  depletion_id, int bd_composite_id, eComposite composite_, eComponent component_ );
...
IT_Data::T4Values values(TReservoirPropertyV &properties_v, const geo::IPoint &point, CFormationBase *pFormation,
...
namespace IT {
namespace pressure {
void add(int reservoir_id, const IT_Data::TPointsV &points_v, const IT_Data::TValuesV &pressures_v,
         eDepletionType _type, int depletion_id, bool is_branch) {
  if (points_v.size() == 0 || (points_v.size() != pressures_v.size())) {
    ...
    if (pStage && pFormation) {
      ...
      bool r2 = depletion_id == LAST_DEPLETION_STAGE ? gm::bd::pointset::replace_values(ps_id, pressures_v, -1)
                                                     : // FIXME: '-1' hard-coded
...
void add(int reservoir_id, eDepletionType _type, int depletion_id, bool is_branch, Depletion *depletion) {
...
namespace component {
namespace IT {
IT_Data::T3ValuesV values(int reservoir_id, int surface_id, int depletion_id, int bd_composite_id,
                          eComposite composite_, TDisplacementComponentV component_v) {
  ...
  if (pStage && pFormation) {
    ...
    for (int i = 0; i < num_vol; i++) {
      geo::CBodyGroup::CSideSurface *surface = model::formation::surface::get(reservoir_id, i, surface_id);
      if (surface) {
        std::vector<int> elems = bd::composite::IT::elems(reservoir_id, i, surface_id, composite_,
                                                          bd_composite_id); // gathered only points in the convex hull
        for (int j = 0; j < elems.size(); j++)
          values_v.push_back(
              res::composite::component::surface::value(surface->Face(elems[j]), component_v, pStage, composite_));
      }
    }
  }
  return values_v;
}
IT_Data::TValuesV values(int reservoir_id, int surface_id, int depletion_id, int bd_composite_id, eComposite composite_,
  ...
  if (pStage && pFormation) {
    ...
    for (int i = 0; i < num_vol; i++) {
      ...
      if (surface) {
        ...
        for (int j = 0; j < elems.size(); j++)
          values_v.push_back(
              res::composite::component::surface::value(surface->Face(elems[j]), component, pStage, composite_));
      }
    }
  }
  return values_v;
...
Displacement *get(int reservoir_id, int surface_id, int depletion_id, int bd_composite_id,
  ...
  // displacement->subsidence(	component::IT::values( reservoir_id, surface_id, depletion_id, bd_composite_id,
  // composite_id, component_v ) );
...
namespace component {
namespace IT {
IT_Data::T3ValuesV values(eComposite composite_id, int composite_idx, TDisplacementComponentV component_v,
...
Displacement *get(int reservoir_id, int displacement_id, int surface_id, TDisplacementComponentV component_v) {
  ...
  Printer::instance()->debug("elems: %d subs: %d z: %d", displacement->elems().size(),
                             displacement->subsidence().size(), displacement->z().size());
