  ...
  for (int i = 0; i < num_volumes; i++) {
    ...
    for (int j = 0; j < num_surfaces; j++) {
      if (count == surface_idx) {
        ...
        // Printer::instance()->debug("gm::model::formation::surface::get( %d ) : %s", surface_idx,
        // tet_surface.SurfaceDesc().Name().c_str() );
        return &surface;
      } else
        count++;
    }
...
double value(const geo::IFace &face, eComponent component_, CDepletionStage *pStage, eComposite composite_) {
  ...
  const IVectorResult::CVectorComponent *component =
      dynamic_cast<const IVectorResult::CVectorComponent *>(res::composite::component::get(composite_, pStage, type_));
