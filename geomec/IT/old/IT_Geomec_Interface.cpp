...
namespace 
{
  ...
  bool copyMaterialLibraryValue(	CLibraryMaterial *oldMatLib, 
                  CLibraryMaterial *newMatLib, 
                  int fromValueType, 
                  int toValueType_1, 
                  int toValueType_2 = -1)
  {
...
CMaterialEntry* IT_Geomec_Interface::createMaterial( CModelBase& modelBase, const QString& materialName, int materialModel )
{
...
{
  TFormationBaseEntry* formationBaseEntry = dynamic_cast <TFormationBaseEntry*> ( modelBase.GraphEntry( MD_BASE_FORMATION ) );
...
void IT_Geomec_Interface::connectMaterial2Formation( CModelBase& modelBase, IMaterial* material, CFormationBase* formationBase )
{
  if( formationBase ) // formation may be 0 when we were handling interface elements
  {
    CDepletionStageEntry* depletionStageEntry = dynamic_cast < CDepletionStageEntry* > ( modelBase.GraphEntry( MD_BASE_DEPLETION_STAGE ) );

    for( CDepletionStageEntry::iterator stage = depletionStageEntry->begin(); stage != depletionStageEntry->end(); ++stage )
    {
      ...
      if( materialServer != 0 )
      {
        if( materialServer->LibraryMaterial() != 0 )
        {
          ...
          formationBase->ConnectedMaterial( *stage )->UnLink( *(formationBase->ConnectedMaterial(*stage)->LibraryMaterial() ) );
        }
...
TSurfaceL& IT_Geomec_Interface::surfaces_list( int reservoir_id ) //FIXME: need of 'TSurfaceL' and 'TStringL' -> verify this
{
...
CStringV& IT_Geomec_Interface::list( eList type_, int param )
{
  switch( type_ )
  {
    case L_Surface:			return surfaces_list( param );
    case L_Formation:		return formations_list( false );
    case L_Reservoir:		return formations_list( true );
    case L_Depletion:		return depletions_list();
    case L_BD_Composite:	return 	G_BD_composite_names_list( (eComposite)param );
...
int IT_Geomec_Interface::num_depletion_stages(){ return model() ? model()->DepletionStageEntry().EntryNodes().size() : 0; }
//--------------------------------------------------------------------------------------------------------------------------------------
...
void IT_Geomec_Interface::branch( int depletion_id, bool is_branch )
{
  ...
  if( pStage )
    IT_Geomec_Interface::depletion_stage( depletion_id )->setOutputType( is_branch ? CDepletionStage::BRANCH : CDepletionStage::RESULTS );
}
//--------------------------------------------------------------------------------------------------------------------------------------
...
void IT_Geomec_Interface::add_pressure( int reservoir_id, 
...
CFormationBase* IT_Geomec_Interface::formation( int idx, bool bReservoir ) //FIXME: 'formation' means 'reservoir' so do call it 'reservoir'
{
  if( model() )
  {
    int i=0;
...
int IT_Geomec_Interface::material_id( eMaterialParameter material )
{
  switch( material )
  {
    case PoissonsRatio_:		return IDT_VALUETYPE_POISSONS_RATIO;
    case YoungsModulus_:		return IDT_VALUETYPE_YOUNGS_MODULUS;
    default:					break;
  }
  return NOT_DEFINED;
}
...
IT_Geomec_Interface::eMaterialParameter IT_Geomec_Interface::convert( eReservoirProperty material )
{
  switch( material )
  {
    case PoissonsRatio:		return PoissonsRatio_;
    case YoungsModulus:		return YoungsModulus_;
    default:				break;
  }
  return NotDefined_;
}
...
IT_Data::T3ValuesV IT_Geomec_Interface::BD_values( eComposite composite_, int composite_idx, TDisplacementComponentV component_v )
{
...
IT_Data::T3Values IT_Geomec_Interface::BD_value( const geo::IPoint& point, IValueComposite* composite, TDisplacementComponentV& component_v )
{
...
std::pair<TCompositeV,TPointSetV> IT_Geomec_Interface::G_BD_composite_list( eComposite composite_ )
{
  ...
  if( model() )
  {
    TPointSetEntry::TNodeSet point_set_set = static_cast< TPointSetEntry* >( model()->GraphEntry( MD_BASE_POINTSET ) )->EntryNodes();
...
const IResultComponent* IT_Geomec_Interface::G_RES_component( eComposite composite_, CDepletionStage *pStage, IVectorResult::VECTOR_COMPONENT _component )
{
...
const IVectorResult::CVectorComponent* IT_Geomec_Interface::G_RES_component_ONE( eComposite composite_, CDepletionStage *pStage, IVectorResult::VECTOR_COMPONENT _component )
{
...
const IVectorResult::CFullVectorComponent* IT_Geomec_Interface::G_RES_component_FULL( eComposite composite_, CDepletionStage *pStage )
{
  const IVectorResult::CFullVectorComponent* component = dynamic_cast< const IVectorResult::CFullVectorComponent* >(  G_RES_component( composite_, pStage, IVectorResult::VECTOR_COMPONENT::VC_FULLVECTOR) );
  return component;
}
...
const geo::IElementSet& IT_Geomec_Interface::elements( const CFormationBase& formation ){	return formation.ElementSet(0).ElementSet(); }
const IPointSet& IT_Geomec_Interface::points( const CFormationBase& formation ){			return formation.ElementSet(0); }
