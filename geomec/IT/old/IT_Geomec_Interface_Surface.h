...
struct T_Elem_And_Val_And_Z 
{
	...
	T_Elem_And_Val_And_Z(	int& elem_id_, 
							IT_Data::T3Values& val_, 
							IT_Data::TCoordinate& z_ ) : elem_id( elem_id_ ), val( val_ ), z( z_ ) {}
...
public:
	...
	*/
	static Reservoir& reservoir( int reservoir_id, int surface_id, TReservoirPropertyV properties_v={ Thickness, PoissonsRatio, YoungsModulus, PorePressure}, int depletion_id = 0 );
	...
	IT_Data::TTriV reservoir_E( int reservoir_id, int surface_id );
		...
		-------
			Vals:			parameter's values per each surface element
							1-D doubles vector
	*/
	static MaterialParameter& material_parameter( int reservoir_id, int depletion_id, eMaterialParameter _material_id, int surface_id );

	/**/
	static double material_parameter_value( CFormationBase* pFormation, CDepletionStage* pStage, int material_id_, const geo::IFace& body );
	...
	*/
	static Displacement& BD_displacement( int reservoir_id, int displacement_id, int surface_id, base::TDisplacementComponentV component_v );
	...
	*/
	static Displacement& RES_displacement( int reservoir_id, int surface_id, int depletion_id, int bd_composite_id, base::TDisplacementComponentV component_v );
...
private:
	...
	static T_Elem_And_Val_And_Z BD_elem_and_val_and_z( const geo::IPoint& point, geo::CBodyGroup::CSideSurface& surface, IValueComposite* composite, base::TDisplacementComponentV& component_v );

	/* Point (some) properties values */
	static IT_Data::T4Values formation_properties_values( TReservoirPropertyV& properties_v, const geo::IPoint& point, CFormationBase* pFormation, CDepletionStage* pStage = nullptr );

	/* 
		Point property value 
		Used by 'reservoir()'
	*/
	static double formation_property_value( eReservoirProperty prop, const geo::IPoint& point, CFormationBase* pFormation, CDepletionStage* pStage = nullptr );

	/* Used by 'measured_displacement()' */
	static T_Elem_And_Val_And_Z_V BD_elems_and_vals_and_z(  int reservoir_id, int composite_idx, int surface_id, base::TDisplacementComponentV component_v, int volume_idx, eComposite composite_id  );

	/* Used by 'displacement_from_point_set()' */
	static IT_Data::TIntV BD_elems( int reservoir_id, int volume_idx, int surface_id, eComposite composite_id, int composite_idx );
	...
	/**/
	static IT_Data::T3Values G_element_component_value( const geo::IFace& face, base::TDisplacementComponentV& component_v, CDepletionStage* pStage, eComposite composant_ );
...
private: /* NOT USED */
	...
	/* NOT USED */
	//static IT_Data::T3ValuesV measured_subsidence(  int reservoir_id, int displacement_id,  base::TDisplacementComponentV component_v, int volume_idx );

	/* NOT USED */
	//static TPointsAndSubsidencesP measured_subsidence_and_points(  int reservoir_id, int displacement_id,  base::TDisplacementComponentV component_v, int volume_idx  );
