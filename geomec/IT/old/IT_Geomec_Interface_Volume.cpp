...
MaterialParameter& IT_Geomec_Interface_Volume::material_parameter( int reservoir_id, int depletion_id, eMaterialParameter _material_id )
{
	...
	if( pFormation )
	{
		...
		for( int i = 0; i < elements.ElementSize(); i++ ) // elems loop
		{
			...
			for( int j = 0; j < e.NrOfPoints(); j++ ) //points per elem
			{
				...
				double _value = pFormation->ThicknessAt( p, point_top, point_bottom, nullptr ).Value(); //FIXME: paralalize / it takes 5s. first time, then 0.05s.
...
Displacement& IT_Geomec_Interface_Volume::measured_displacement_DEPRECATED( int reservoir_id, int displacement_id,  base::TDisplacementComponentV component_v )
{
	IT_Data::TPointsV points_v;
...
IT_Data::T3ValuesV IT_Geomec_Interface_Volume::measured_subsidence(  int reservoir_id, int displacement_id,  base::TDisplacementComponentV component_v  )
{
	...
	if( pFormation )
	{
		...
		for(int e = 0; e < es.ElementSize(); e++)  //elems
		{
			...
			for( base::TDisplacementComponentV::iterator it = component_v.begin(); it != component_v.end(); ++it )
			{
				...
				std::vector< geo::CValue > elem_values_v = composite->Component( _type ).ScalarData().ValueElement( element ); //values per each point in the element
...
IT_Data::T3ValuesV IT_Geomec_Interface_Volume::subsidence( int reservoir_id, int depletion_id,  base::TDisplacementComponentV component_v )
{
	IT_Data::T3ValuesV subsidence_v;
...
Displacement& IT_Geomec_Interface_Volume::displacement_DEPRECATED( int reservoir_id, int depletion_id,  base::TDisplacementComponentV component_v )
{
	IT_Data::TPointsV points_v;
