...
geo::CBodyGroup::CSideSurface* IT_Geomec_Interface_Surface::G_surface(  int reservoir_id, int volume_idx, int surface_id  )
{
...
IT_Data::T3Values IT_Geomec_Interface_Surface::G_element_component_value( const geo::IFace& face, base::TDisplacementComponentV& component_v, CDepletionStage* pStage, eComposite composite_ )
{
	...
	for( int j = 0; j < nComponents; j++ )
	{
		...
		const IVectorResult::CVectorComponent* component = dynamic_cast< const IVectorResult::CVectorComponent* >( base::G_RES_component( composite_, pStage, type_ ) );
...
T_Elem_And_Val_And_Z IT_Geomec_Interface_Surface::BD_elem_and_val_and_z( const geo::IPoint& point, geo::CBodyGroup::CSideSurface& surface, IValueComposite* composite, base::TDisplacementComponentV& component_v )
{
...
T_Elem_And_Val_And_Z_V IT_Geomec_Interface_Surface::BD_elems_and_vals_and_z( int reservoir_id, int composite_idx, int surface_id, base::TDisplacementComponentV component_v, int volume_idx, eComposite composite_ )
{
...
IT_Data::TIntV IT_Geomec_Interface_Surface::BD_elems( int reservoir_id, int surface_id, eComposite composite_, int composite_idx )
{
	IT_Data::TIntV elems_v;
...
IT_Data::TIntV IT_Geomec_Interface_Surface::BD_elems( int reservoir_id, int volume_idx, int surface_id, eComposite composite_, int composite_idx )
{
	IT_Data::TIntV elems_v;
...
double IT_Geomec_Interface_Surface::material_parameter_value( CFormationBase* pFormation, CDepletionStage* pStage, int material_id_, const geo::IFace& body )
{
	double val = NaN;
...
MaterialParameter& IT_Geomec_Interface_Surface::material_parameter( int reservoir_id, int depletion_id, eMaterialParameter _material_id, int surface_id )
{
...
IT_Data::T4Values IT_Geomec_Interface_Surface::formation_properties_values( TReservoirPropertyV& properties_v, const geo::IPoint& point, CFormationBase* pFormation, CDepletionStage* pStage )
{
...
double IT_Geomec_Interface_Surface::formation_property_value( eReservoirProperty prop, const geo::IPoint& point, CFormationBase* pFormation, CDepletionStage* pStage )
{
	double val = NaN;
	if( pFormation )
	{
...
//--------------------------------------------------------------------------------------------------------------------------------------
Reservoir& IT_Geomec_Interface_Surface::reservoir( int reservoir_id, int surface_id, TReservoirPropertyV properties_v, int depletion_id )
{
	//------------------------------------- elems / points
	...
	Printer::instance()->debug("get : res : surf : res_id:%d -> num: points:%d elems:%d params:%d header:%s", 
		reservoir_id, points_v.size(), elems_v.size(), params_v.size(), base::reservoir_header( properties_v ).c_str() );
...
IT_Data::TValuesV IT_Geomec_Interface_Surface::reservoir_property( int reservoir_id, int surface_id, eReservoirProperty property_, int depletion_id )
{
	IT_Data::TValuesV values_v;
...
Displacement& IT_Geomec_Interface_Surface::BD_displacement( int reservoir_id, int displacement_id, int surface_id, base::TDisplacementComponentV component_v )
{
	IT_Data::TIntV elems_v;
	...
	if( pFormation )
	{
		...
		for( int i = 0; i < num_volumes; i++ )
		{
			T_Elem_And_Val_And_Z_V e_and_s_and_z_v = BD_elems_and_vals_and_z( reservoir_id, displacement_id, surface_id, component_v, i, C_Displacement );
 	...
 	Printer::instance()->debug("get : disp : surf : depth : res_id:%d -> num: elems:%d subs:%d depth:%d header:%s wdat:%.2f", 
		reservoir_id, elems_v.size(), subsidence_v.size(), depth_v.size(), base::displacement_header( component_v ).c_str(), dummy::WDAT );
...
Displacement& IT_Geomec_Interface_Surface::RES_displacement( int reservoir_id, int surface_id, int depletion_id, int bd_composite_id, base::TDisplacementComponentV component_v )
{
	IT_Data::T3ValuesV subsidence_v;
	IT_Data::TIntV elems_v;
	...
	if( pStage && pFormation )
	{
		...
		for( int i = 0; i < num_volumes; i++ )
		{
			...
			std::vector<int> elems_vol_v = BD_elems( reservoir_id, i, surface_id, C_Displacement, bd_composite_id ); // gathered only points in the convex hull 
			elems_v.insert( elems_v.end(), elems_vol_v.begin(), elems_vol_v.end()); 

			for( int j = 0; j < elems_vol_v.size(); j++ )
				subsidence_v.push_back( G_element_component_value( surface.Face( elems_v[ j ] ), component_v, pStage, C_Displacement ) );
		}
	}
	Printer::instance()->debug("get : disp : surf : res_id:%d -> num: points:%d subs:%d header:%s wdat:%.2f", 
		reservoir_id, elems_v.size(), subsidence_v.size(), base::displacement_header( component_v ).c_str(), dummy::WDAT );
...
//--------------------------------------------------------------------------------------------------------------------------------
/*
IT_Data::T3ValuesV IT_Geomec_Interface_Surface::measured_subsidence(  int reservoir_id, int displacement_id,  base::TDisplacementComponentV component_v, int volume_idx  )
{
	IT_Data::T3ValuesV subsidence_v;
	...
	if( surface )
	{
		...
		for( int i=0; i< nFaces; i++ )
		{
			...
			for( base::TDisplacementComponentV::iterator it = component_v.begin(); it != component_v.end(); ++it )
			{
				...
				std::vector< geo::CValue > elem_values_v = composite->Component( _type ).ScalarData().ValueElement( face ); //values per each point in the element

				int p=0;
				for( std::vector<geo::CValue>::iterator points_it = elem_values_v.begin(); points_it != elem_values_v.end(); ++points_it )
				{
...
TPointsAndSubsidencesP IT_Geomec_Interface_Surface::measured_subsidence_and_points(  int reservoir_id, int displacement_id,  base::TDisplacementComponentV component_v, int volume_idx  )
{
	IT_Data::TPointsV points_v;
	IT_Data::T3ValuesV subsidence_v;
	...
	{
		//subsidence
		std::pair<IValueComposite*,CPointSet*> composite_p = base::G_BD_composite( IDT_VALUETYPE_DISPLACEMENT, displacement_id );
		IValueComposite* composite = composite_p.first;
		CPointSet* point_set = composite_p.second;
...
geo::CPoint IT_Geomec_Interface_Surface::translate_point_to_face( const geo::CPoint& point, const geo::IFace& face  )
{
	...
	geo::CPoint trans(point.X() - signedDist * normal.X(), point.Y() - signedDist * normal.Y(), point.Z() - signedDist * normal.Z());
...
Displacement& IT_Geomec_Interface_Surface::displacement_DEPRECATED( int reservoir_id, int depletion_id, base::TDisplacementComponentV component_v )
{
