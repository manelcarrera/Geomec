...
template<class T>
array_temp<T>::array_temp()
{
}

template<class T>
array_temp<T>::array_temp(int x, int y, int z)
{
  resize(x, y, z);
}

template<class T>
array_temp<T>::array_temp(const array_temp<T> &rhs)
: m_data(rhs.m_data)
{
}

template<class T>
void array_temp<T>::resize(int x, int y, int z)
{
  assert((x > 0) && (y > 0) && (z > 0));
...
template<class T>
T& array_temp<T>::get_at(int x, int y, int z)
{
  return m_data.at(x).at(y).at(z);
}

template<class T>
const T& array_temp<T>::get_at(int x, int y, int z) const
{
  return m_data[x][y][z];
}

template<class T>
void array_temp<T>::set_at(const int x, const int y, const int z, const T& type)
{
  m_data.at(x).at(y).at(z) = type;
}

template<class T>
int array_temp<T>::size_x() const
{
  return (int)m_data.size();
}

template<class T>
int array_temp<T>::size_y() const
{
  return (int)m_data[0].size();
}

template<class T>
int array_temp<T>::size_z() const
{
  return (int)m_data[0][0].size();
}
