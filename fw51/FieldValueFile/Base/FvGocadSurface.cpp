...
CSurfaceHeader::CSurfaceHeader(const QString &sName) : CGoCadHeader(sName) {}
...
geo::CValue CGoCadSurface::CTriGoCadFace::InterpolateValue(const geo::IPoint & /*point*/,
...
size_t CGoCadSurface::CTriGoCadFace::NrOfNodes() const { return 3; }
...
const geo::IElementSet *CGoCadSurface::CTriGoCadFace::IndexingElementSet() const { return &m_surface.Mesh(); }

/*virtual*/ std::string CGoCadSurface::CTriGoCadFace::Type() const { return "T6EPS"; }

CGoCadSurface::CGoCadSurface(geo::IMesh &mesh, const geo::ISurface &surface, const QString &sName) : CFaceGroup(mesh) {
  ...
  for (i = 0; i < surface.FaceSize(); i++) {
    ...
    // Insert triangle
    InsertTriangle(mpPoint.find(surface.Face(i).Point(0))->second, mpPoint.find(surface.Face(i).Point(1))->second,
...
CGoCadSurface::CGoCadSurface(geo::IMesh &mesh) : CFaceGroup(mesh) {}

CGoCadSurface::~CGoCadSurface() { delete m_pHeader; }
...
*/
bool CGoCadSurface::isPoint(int nIndex) const { return m_mpPoint.find(nIndex) != m_mpPoint.end(); }
...
void CGoCadSurface::Header(const QString &sName) { m_pHeader = new CSurfaceHeader(sName); }
    ...
    for (j = 0; j < 3; j++) {
      ...
      for (k = 0; k < PointSize(); k++) {
        ...
        if (pt1.Distance(pt2) < EPS) {
          point_present = true;
          vcPtIndices.push_back(Mesh().NodeIndex(
              pt2)[0]); // since points can only be present once, just take the first index in the vector.
          break;
        }
      }
