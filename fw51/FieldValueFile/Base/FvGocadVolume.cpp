...
CGoCadVolume::CGoCadVolume(geo::IMesh &mesh) : CBodyGroup(mesh), m_pHeader(0) {}

CGoCadVolume::~CGoCadVolume() {}

void CGoCadVolume::Header(const QString &sName) { m_pHeader = new CGoCadHeader(sName); }
...
bool CGoCadVolume::isPoint(int nIndex) const { return m_mpPtInd.find(nIndex) != m_mpPtInd.end(); }
...
int CGoCadVolume::InsertTetrahedron(const int nIndex1, const int nIndex2, const int nIndex3, const int nIndex4) {
  ...
  if (dVol > 0)
    Tet = new geo::CTetrahedron(Mesh(), p1, p2, p3, p4);
  else
    Tet = new geo::CTetrahedron(Mesh(), p2, p1, p3, p4);
...
int CGoCadVolume::PointSize() const { return m_nodes.size(); }
