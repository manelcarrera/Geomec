...
CGoCadFile::CGoCadFile(geo::IMesh &mesh) : m_type(GOCAD_IDLE), m_state(GOCAD_CLOSED), m_mesh(mesh) {
  // Comment ...
  DefineComment("#");
  DefineComment("*");
...
CGoCadFile::~CGoCadFile() { RemoveAll(); }
...
CGoCadSurface *CGoCadFile::LatestSurf() { return m_vcSurface[m_vcSurface.size() - 1]; }

CGoCadVolume *CGoCadFile::LatestVol() { return m_vcVolume[m_vcVolume.size() - 1]; }
...
bool CGoCadFile::ReadTFace(TInputStream & /*stream*/, const QString & /*sToken*/) { return true; }

size_t CGoCadFile::SurfaceSize() const { return m_vcSurface.size(); }

size_t CGoCadFile::VolumeSize() const { return m_vcVolume.size(); }
...
bool CGoCadFile::WriteSurface(TOutputStream &stream, const CGoCadSurface &surface) {
  stream << "GOCAD TSurf" << '\n';
