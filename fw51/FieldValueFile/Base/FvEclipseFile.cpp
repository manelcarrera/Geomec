...
bool CEclipseFile::ReadCoordSys(TInputStream &stream, const QString &sToken) {
  for (size_t nReservoir = 0; nReservoir < m_vcReservoir.size(); nReservoir++) {
    ...
    if (stream.peek() != '/') {
      ...
      m_vcReservoir[nReservoir]->ReservoirBoundary(*m_vcReservoir[nLowerIndex], *m_vcReservoir[nUpperIndex]);
    } else
      m_vcReservoir[nReservoir]->ReservoirBoundary(*m_vcReservoir[nReservoir], *m_vcReservoir[nReservoir]);
...
int CEclipseFile::HorizonSize() const { return (int)m_vcEclipseHorizon.size(); }
...
CEclipseCell CEclipseFile::Cell(int x, int y, int z) const { return Reservoir(z).Cell(x, y, z); }

size_t CEclipseFile::PropertySize() const { return m_vcProperty.size(); }
...
long CEclipseFile::SavedItems() const {
  return (long)((SizeX() + 1) * (SizeY() + 1) * 2 * m_vcReservoir.size() +
                SizeX() * SizeY() * SizeZ() * (8 + PropertySize()));
}
