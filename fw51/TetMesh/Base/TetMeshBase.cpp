...
CTetMeshBase::CTetMeshBase() : m_nBodySize(-1), m_nContactSize(0), m_nInterfaceSize(0), m_dMeshTolerance(1E-5) {}

CTetMeshBase::~CTetMeshBase() { Clear(); }

/*!
Returns the number of surfaces in the mesh
*/
int CTetMeshBase::NrOfSurfaces() const { return (int)m_vcSurfaceDesc.size(); }

/*!
Returns a surface descriptor based on an index
*/
const CSurfaceDesc &CTetMeshBase::SurfaceDesc(int nIndex) const { return *m_vcSurfaceDesc[nIndex]; }

/*!
Returns a surface descriptor based on an index
*/
CSurfaceDesc &CTetMeshBase::SurfaceDesc(int nIndex) { return *m_vcSurfaceDesc[nIndex]; }
...
*/
int CTetMeshBase::AddSurface(const ISurface &surface, const std::string &sName, bool bFault) {
...
*/
int CTetMeshBase::RegisterNodeInMainMesh(IPoint &point, int nElements) { return RegisterNode(point, false, nElements); }

/*!
Adds a hexahedron element in the main mesh and returns the element index
*/
int CTetMeshBase::RegisterTetrahedronInMainMesh(int nNode0, int nNode1, int nNode2, int nNode3) {
  CTetrahedron *pTetrahedron = new CTetrahedron(*this, nNode0, nNode1, nNode2, nNode3);
...
*/
bool CTetMeshBase::RegisterTetSurface(CSurfaceDesc &desc, CBodyGroup &body_group, const std::vector<int> &vcNode) {
...
*/
bool CTetMeshBase::AddSurfaceToMesh(void *pMesh, const ISurface &surface, int nSurfaceId, const std::string &sName,
                                    bool bSlip) {
  // Is the mesh OK?
  if (MESH_ERROR(pMesh))
    return false;
...
void CTetMeshBase::AddSurfacePointsToMesh(void *pMesh, const ISurface &surface) {
  for (int i = 0; i < surface.PointSize(); i++) {
    MeshPointAdd(pMesh, surface.Point(i).X(), surface.Point(i).Y(), surface.Point(i).Z());
  }
}
...
const geo::INode &CTetMeshBase::NearestNode(const geo::IPoint &point, const geo::IFace *face) {
...
*/
CTetMeshBase::TRet CTetMeshBase::CreateMesh(const std::string &sDianaPath, const std::string &sDiaSharePath,
                                            const std::string &sDianaLibPath, const std::string &sDianaSLibPath,
                                            const std::string &sFgvExeName, CTetMeshProgress &progress) {
  ...
  for (i = 0; i < m_vcSurfaceDesc.size(); i++) {
    ...
    if (!AddSurfaceToMesh(pMesh, surf_desc.Surface(), i, surf_desc.Name(), surf_desc.Slip()))
      return OnMeshError(pMesh);
  }
  ...
  MeshGenerate(sDianaPath.c_str(), sDiaSharePath.c_str(), sDianaLibPath.c_str(), sDianaSLibPath.c_str(),
               sFgvExeName.c_str(), pMesh);
  ...
  for (TInterfaceMap::iterator it = interface_map.begin(); it != interface_map.end(); it++) {
...
*/
int CTetMeshBase::NrOfVolumes() const { return (int)m_vcVolume.size(); }

/*!
Returns number of generated contact elements
*/
int CTetMeshBase::ContactElementSize() const { return m_nContactSize; }

/*!
Returns the number of generated interfaces
*/
int CTetMeshBase::InterfaceElementSize() const { return m_nInterfaceSize; }
...
bool CTetMeshBase::Option(int /*option*/) const { return false; }
...
const IInterfaceElement *CTetMeshBase::AddInterfaceElement(CSurfaceDesc &desc, const geo::IFace *front,
  ...
  CInterfaceElement *pElement = new CInterfaceElement(*this, front, back, vcNode);
