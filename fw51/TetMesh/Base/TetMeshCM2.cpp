...
CTetMeshCM2::CTetMeshCM2() : CTetMeshBase(), m_bNoStrict(false), m_bNoCrash(false) {}
...
void CTetMeshCM2::RegisterTetrahedrons(const MesherOutput &output) {
  _tet_lookup.clear();
  for (vector<vector<size_t>>::const_iterator iter = output.Tetrahedrons().begin(); iter != output.Tetrahedrons().end();
       ++iter) {
...
void CTetMeshCM2::RegisterOuterSurfaces(const MesherOutput &output) {
  for (size_t i = 0; i < output.SideSurfaces().size(); ++i) {
    ...
    for (vector<MesherOutput::face_t>::const_iterator iter = side_surface.tet_surface.begin();
         iter != side_surface.tet_surface.end(); ++iter) {
...
bool CTetMeshCM2::TriangleListHasDuplicates(const vector<vector<size_t>> &triangle_list, bool verbose = false) const {
  ...
  for (vector<vector<size_t>>::const_iterator tl_iter = triangle_list.begin(); tl_iter != triangle_list.end();
       ++tl_iter) {
  ...
  if (verbose && duplicates > 0) {
    ...
    for (vector<vector<size_t>>::const_iterator iter = sorted_triangles.begin(); iter != sorted_triangles.end();
         ++iter) {
...
void CTetMeshCM2::VerifyOutput(const MesherOutput &output, const MesherInput &input, bool verbose = false) const {
  ...
  if (verbose)
    cout << "Verifying " << connect.size() << " terahedrons...";
  for (vector<vector<size_t>>::const_iterator tet_iter = connect.begin(); tet_iter != connect.end(); ++tet_iter) {
    ...
    assert(*(max_element(tet.begin(), tet.end())) < pos.size() &&
           "Each node in the tetrahedron must refer to a node in the pos matrix.");
    // todo: possibly add some verifcation that the coordinates of each tet node actually differ (note that the pos
    // matrix may have double entries, due to the splitting of the fault nodes.
  }
  if (verbose)
    cout << "OK!" << endl;
  const vector<MesherOutput::interface_t> &interfaces = output.Interfaces();
  if (verbose)
    cout << "Verifying " << interfaces.size() << " interfaces." << endl;
  for (vector<MesherOutput::interface_t>::const_iterator int_iter = interfaces.begin(); int_iter != interfaces.end();
       ++int_iter) {
    if (verbose)
      cout << "Interface " << distance(interfaces.begin(), int_iter);
    vector<vector<size_t>> triangles_front, triangles_rear;
    const MesherOutput::interface_t &intrfc = *int_iter;
    for (vector<Surface>::const_iterator fault_iter = input.Faults().begin(); fault_iter != input.Faults().end();
         ++fault_iter) {
      const Surface &fault_input_surface = *fault_iter;
      if (fault_input_surface.Id() != intrfc.surface_id)
        continue;
      assert(intrfc.elements.size() >= fault_input_surface.Connect().size() &&
             "Interface needs to have at least the number of elements as the original input surface for the fault.");
      break;
    }
    for (vector<MesherOutput::interface_element_t>::const_iterator elm_iter = intrfc.elements.begin();
         elm_iter != intrfc.elements.end(); ++elm_iter) {
      ...
      assert(face_front.tet_id < connect.size() &&
             "Interface element front tet must refer to an actual tet in the model.");
      assert(face_rear.tet_id < connect.size() &&
             "Interface element rear tet must refere to an actual tet in the model.");
    ...
    if (verbose)
      cout << " " << triangles_front.size() << " front triangles, " << triangles_rear.size() << " rear triangles...";
    assert(!TriangleListHasDuplicates(triangles_front) &&
           "Front face of the fault needs to consist of unique triangles.");
    assert(!TriangleListHasDuplicates(triangles_rear) &&
           "Rear face of the fault needs to consist of unique triangles.");
    if (verbose)
      cout << " OK!" << endl;
  }
  const vector<MesherOutput::tet_surface_t> &side_surfaces = output.SideSurfaces();
  if (verbose)
    cout << "Verifying " << side_surfaces.size() << " side surfaces." << endl;
  for (vector<MesherOutput::tet_surface_t>::const_iterator srf_iter = side_surfaces.begin();
       srf_iter != side_surfaces.end(); ++srf_iter) {
    if (verbose)
      cout << "Side surface " << distance(side_surfaces.begin(), srf_iter);
    vector<vector<size_t>> triangles;
    const MesherOutput::tet_surface_t &tet_surf = *srf_iter;
    for (vector<MesherOutput::face_t>::const_iterator f_iter = tet_surf.tet_surface.begin();
         f_iter != tet_surf.tet_surface.end(); ++f_iter) {
      const MesherOutput::face_t &face = *f_iter;
      triangles.push_back(GetTetFace(connect[face.tet_id], face.face_id));
    }
    if (verbose)
      cout << " " << triangles.size() << " triangles...";
  ...
  if (verbose)
    cout << "Verifying " << volumes.size() << " volumes." << endl;
  for (vector<vector<size_t>>::const_iterator vol_iter = volumes.begin(); vol_iter != volumes.end(); ++vol_iter) {
    ...
    if (verbose)
      cout << "Volume " << distance(volumes.begin(), vol_iter) << " " << volume.size() << " tets";
    ...
    assert(*(max_element(volume.begin(), volume.end())) < connect.size() &&
           "Each tet in the volume must refer to a tet that actually exists.");
    if (verbose)
      cout << " OK!" << endl;
  }
...
CTetMeshBase::TRet CTetMeshCM2::CreateMesh(const std::string &sDianaPath, const std::string &sDiaSharePath,
