...
static int EdgeNewList(Edge_t *edge, Surface_t *surface, int triangleId, int edgeNum) {
...
extern int EdgeKeyOffset(const Edge_t *edge, const EdgeKey_t *key) { return RBTreeFind(edge->edgeKeyTree, key); }
...
extern EdgeKey_t *EdgeKeyFirst(Edge_t *edge, Iterator_t *iter) { return RBTreeFirst(edge->edgeKeyTree, iter); }

extern EdgeKey_t *EdgeKeyNext(Edge_t *edge, Iterator_t *iter) { return RBTreeNext(edge->edgeKeyTree, iter); }
...
extern EdgeList_t *EdgeListFirst(Edge_t *edge, const EdgeKey_t *key) { return EDGE_LIST(edge, key->list); }

extern EdgeList_t *EdgeListNext(Edge_t *edge, const EdgeList_t *list) { return EDGE_LIST(edge, list->next); }
...
extern void EdgeListPrint(const EdgeList_t *list, FILE *fp) {
  ...
  fprintf(fp, " surface=%s triangleId=%d edge=%d\n", SurfaceUserInfo(buf, surf), EdgeListTriangleId(list), list->edge);
}
...
extern bool_t EdgeInternal(Edge_t *edge, const EdgeKey_t *key) {
  ...
  while (list && isInternal) {
    ...
    const Surface_t **prevSurf =
        (type == surfNormal ? &normal
                            : ((type == surfFrontFault ? &front : ((type == surfBackFault ? &back : (NULL))))));
...
extern bool_t EdgeFaultTouchesOuter(Edge_t *edge, const EdgeKey_t *key) {
...
extern BoundBox_t *EdgeKeyBoundBox(const EdgeKey_t *key, PointSet_t *pointSet, BoundBox_t *bb) {
  int point1, point2;
...
extern void EdgeKeyMeshMessageAdd(void *mesh, Edge_t *edge, EdgeKey_t *key, PointSet_t *pointSet) {
