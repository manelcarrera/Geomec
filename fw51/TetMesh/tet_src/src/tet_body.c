...
extern BodySurface_t *BodyFirst(Body_t *body, Iterator_t *iter) { return SetFirst(body->bodySurface, iter); }

extern BodySurface_t *BodyNext(Body_t *body, Iterator_t *iter) { return SetNext(body->bodySurface, iter); }
...
extern void BodyPrint(Body_t *body, FILE *fp) {
  ...
  if (body->outer)
    fprintf(fp, ", subbody of ID=%d ", BodyPrintIdGet(body->outer));
  fprintf(fp, ")\n");
  while (surf) {
...
extern int BodySize(const Body_t *body) { return SetSize(body->bodySurface); }
...
extern bool_t BodySurfacePosDir(const BodySurface_t *bs) { return (bs->dir != 0); }

extern Surface_t *BodySurfaceGet(BodySurface_t *bs) { return bs->surface; }
...
static bool_t BodyGroupIsDefined(const Body_t *body) { return (body->group >= 0); }
...
extern void BodyGroupSet(Body_t *body, int group) { body->group = group; }

extern int BodyPrintIdGet(const Body_t *body) { return body->printId; }

extern void BodyPrintIdSet(Body_t *body, int printId) { body->printId = printId; }

extern bool_t BodyContains(Body_t *body, const Surface_t *surface, bool_t posDir) {
