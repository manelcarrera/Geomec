...
static SurfaceSet_t *SurfaceSetInit(SurfaceSet_t *surfaceSet) {
  ...
  surfaceSet->surface = RBTreeCreate(sizeof(SurfaceData_t), SurfaceSetCompareSurface);
  return surfaceSet;
}

extern bool_t SurfaceSetTest(SurfaceSet_t *surfaceSet, Surface_t *surface) {
  SurfaceData_t data;
  UTIL_ZERO(&data);
...
extern SurfaceSet_t *SurfaceSetCopy(SurfaceSet_t *orgSurfSet) {
...
extern int SurfaceSetSize(const SurfaceSet_t *surfaceSet) { return (int)RBTreeSize(surfaceSet->surface); }

/* Return number of triangles in all surfaces in set */
extern int SurfaceSetTriangleSize(SurfaceSet_t *surfaceSet) {
  int size = 0;
  Iterator_t iter;
...
static int *GroupTriangles(SurfaceSet_t *surfaceSet, int *numGroups) {
...
static SurfaceSet_t *SynchronizeFault(SurfaceSet_t *faultSet) {
  SurfaceSet_t *result = SurfaceSetCreate();
  ...
  while (front) {
    ...
    assert(found);
    newBack = SurfaceCreate(SurfaceID(back), SurfacePartGet(front), /* This is what we are doing */
                            SurfaceType(back), SurfaceGetFrontDir(back), SurfaceUserName(back),
...
static SurfaceSet_t *SynchronizeFrontBack(SurfaceSet_t *surfaceSet) {
...
extern SurfaceSet_t *SurfaceSetSplitIntoSimpleSurface(void *mesh, SurfaceSet_t *surfaceSet,
    ...
    while (surface) {
      ...
      while (triangle) {
        ...
        if (!newSurface) {
          ...
          idPartFound->part += 1;
          newSurf[groupId[triangleId]] =
              SurfaceCreate(SurfaceID(surface), idPartFound->part, SurfaceType(surface), SurfaceGetFrontDir(surface),
...
static bool_t IsActiveSurf(const Surface_t *surf, const bool_t posDir) {
...
static bool_t JointDataFill(JointData_t *jd, bool_t useBothSidesFault, SurfaceSet_t *surfaceSet,
                            const EdgeList_t *edge1, const bool_t posDir1, const EdgeList_t *edge2,
                            const bool_t posDir2, const double angle) {
  Surface_t *surf1 = EdgeListSurface(edge1);
  Surface_t *surf2 = EdgeListSurface(edge2);
  const bool_t isActive = (useBothSidesFault || (IsActiveSurf(surf1, posDir1) && IsActiveSurf(surf2, posDir2)));
...
static void AnalyzeSinglePair(const EdgeList_t *edge1, const EdgeList_t *edge2, bool_t useBothSidesFault,
                              SurfaceSet_t *surfaceSet, PointSet_t *pointSet, Set_t *jointData) {
  ...
  bool_t orientOK = SurfaceCheckEdgeOrientation(surface1, EdgeListTriangleId(edge1), edge1->edge, FALSE, surface2,
                                                EdgeListTriangleId(edge2), edge2->edge, FALSE);
  ...
  double cosAngle = orientOK ? VecInprod(normal1, normal2, 3) : -VecInprod(normal1, normal2, 3);
  ...
  /* Add reverse pairs */
  isActive[2] = JointDataFill(jd + 2, useBothSidesFault, surfaceSet, jd[0].edge2, jd[0].posDir2, jd[0].edge1,
                              jd[0].posDir1, jd[0].angle);
  isActive[3] = JointDataFill(jd + 3, useBothSidesFault, surfaceSet, jd[1].edge2, jd[1].posDir2, jd[1].edge1,
                              jd[1].posDir1, jd[1].angle);
...
static void AnalyzeSingleJoint(void *mesh, bool_t useBothSidesFault, Edge_t *edge, const EdgeKey_t *key,
                               const EdgeList_t *edges, const int nEdges, PointSet_t *pointSet, Set_t *jointWork,
                               SurfaceSet_t *surfaceSet, Set_t *pairs) {
  int n;
  int nFault;
...
static void ProcessPairs(const Set_t *pairs, TetGraph_t *graph) {
...
static void SurfaceSetFrontDirSet(void *mesh, SurfaceSet_t *surfaceSet, const int ID, const FrontDir_t fdir) {
...
static void SurfaceSetAnalyzeJoints(void *mesh, PointSet_t *pointSet, SurfaceSet_t *surfaceSet, TetGraph_t *graph) {
  Edge_t *edge = SurfaceSetEdge(surfaceSet);
  Iterator_t iter;
  EdgeKey_t *key = EdgeKeyFirst(edge, &iter);
  ...
  while (key && !MESH_ERROR(mesh)) {
    if (!EdgeInternal(edge, key)) {
      ...
      EdgeListGet(edge, key, edges, NULL);
      if (!MESH_ERROR(mesh)) {
        AnalyzeSingleJoint(mesh, TRUE, edge, key, edges, listLen, pointSet, jointWork, surfaceSet, pairs);
...
static void SetIndistinguishableFault(void *mesh, SurfaceSet_t *surfaceSet) {
  Iterator_t iter;
  Surface_t *surface = SurfaceSetFirst(surfaceSet, &iter);
  while (surface) {
...
static bool_t SurfaceSetDegenerate(SurfaceSet_t *surfaceSet) {
...
static int GetOppositeId(const int id) {
  int oppositeId;
  if (id % 2) {
    oppositeId = id - 1; /* Odd */
  } else {
...
static void SetOuterBound(void *mesh, SurfaceSet_t *surfaceSet, bool_t *outerBound, int id, int *numChange) {
...
static int ProcessOuterBoundPairs(void *mesh, SurfaceSet_t *surfaceSet, Set_t *pairs, bool_t faultsOpen,
                                  bool_t *outerBound) {
  int numChange = 0;
  const int nPairs = SetSize(pairs);
  int n;
...
static int MarkOuterBoundaryLoop(void *mesh, bool_t faultsOpen, SurfaceSet_t *surfaceSet, PointSet_t *pointSet,
                                 bool_t *outerBound) {
  int numChange = 0;
  Edge_t *edge = SurfaceSetEdge(surfaceSet);
  Iterator_t iter;
  ...
  while (key && !MESH_ERROR(mesh)) {
    if (!EdgeInternal(edge, key)) {
      ...
      if (!MESH_ERROR(mesh)) {
        ...
        AnalyzeSingleJoint(mesh, useBothSidesFault, edge, key, edges, listLen, pointSet, jointDataSet, surfaceSet,
                           pairs);
...
static void MarkExtremalSurfaces(const double smallLength, SurfaceSet_t *surfaceSet, PointSet_t *pointSet,
                                 bool_t *outerBound) {
  Iterator_t surfSetIter;
  Surface_t *surf = SurfaceSetFirst(surfaceSet, &surfSetIter);
...
static int ExtendBoundaryFaults(void *mesh, SurfaceSet_t *surfaceSet, bool_t *outerBound) {
  int numChange = 0;
...
static bool_t *SurfaceSetOuterBound(void *mesh, bool_t faultsOpen, SurfaceSet_t *surfaceSet, PointSet_t *pointSet) {
...
static void SearchDistinguishableFault(void *mesh, Edge_t *edge, EdgeKey_t *key, SurfaceSet_t *surfaceSet,
                                       PointSet_t *pointSet, Set_t *idSetFront, Set_t *idSetBack, Set_t *edgeSet,
                                       Set_t *jointWork, Set_t *pairs) {
  EdgeList_t *list = EdgeListFirst(edge, key);
...
static void SetDistinguishableFault(void *mesh, SurfaceSet_t *surfaceSet, PointSet_t *pointSet) {
  ...
  if (faultDirUnknown) {
    ...
    while (key && !MESH_ERROR(mesh)) {
      ...
      if (nFault && (nTriangle - nFault) && !EdgeInternal(edge, key)) {
        ...
        SearchDistinguishableFault(mesh, edge, key, surfaceSet, pointSet, SetClear(idSetFront), SetClear(idSetBack),
                                   SetClear(edgeListWork), SetClear(jointWork), SetClear(pairs));
      }
      key = EdgeKeyNext(edge, &iter);
    }
  }
...
extern void SurfaceSetFaultDirection(void *mesh, SurfaceSet_t *surfaceSet, PointSet_t *pointSet) {
...
static bool_t BodyCorrect(SurfaceSet_t *surfaceSet, PointSet_t *pointSet, double smallLength, Body_t *body) {
...
static void GenerateBodies(void *mesh, TetGraph_t *graph, SurfaceSet_t *surfaceSet, PointSet_t *pointSet,
                           BodySet_t *bodySet) {
...
static void SurfaceSetFillSingleBodySet(void *mesh, SurfaceSet_t *surfaceSet, PointSet_t *pointSet,
                                        BodySet_t *bodySet) {
...
extern void SurfaceSetFillBodySet(void *mesh, SurfaceSet_t *surfaceSet, PointSet_t *pointSet, BodySet_t *bodySet) {
  int n;
...
static int LinkFaultPoints(XPointSet_t *xpointSet, const EdgeKey_t *key, const Set_t *pairSet, int numFault,
  ...
  for (n = 0; n < SetSize(pairSet); n++) {
    ...
    if (!skipOuter2Outer ||
        !(SurfaceIsOuterBoundary(surf1, joint->posDir1) && SurfaceIsOuterBoundary(surf2, joint->posDir2))) {
...
extern void SurfaceSetLinkFaultPoints(void *mesh, PointSet_t *pointSet, XPointSet_t *xpointSet,
                                      SurfaceSet_t *surfaceSet, bool_t skipOuter2Outer) {
  ...
  while (!MESH_ERROR(mesh) && numChange) {
    ...
    while (key && !MESH_ERROR(mesh)) {
      ...
      EdgeKeyPoints(key, &p1, &p2);
      if ((numFault || XPointSetPointInAnyFault(xpointSet, p1) || XPointSetPointInAnyFault(xpointSet, p2)) &&
          !MESH_ERROR(mesh)) {
...
extern void SurfaceSetMarkOuterBoundary(void *mesh, bool_t faultsOpen, PointSet_t *pointSet, SurfaceSet_t *surfaceSet) {
...
static void GetFaultPair(Edge_t *edge, EdgeKey_t *key, const Surface_t **front, const Surface_t **back) {
  EdgeList_t *list = EdgeListFirst(edge, key);
  *front = NULL;
  *back = NULL;
  while (list) {
 ...
 */
extern bool_t SurfaceSetClosedFault(void *mesh, SurfaceSet_t *surfaceSet, Edge_t *edge, EdgeKey_t *key) {
...
extern void SurfaceSetAddBody(SurfaceSet_t *surfaceSet, Body_t *body) {
  Iterator_t iter;
  BodySurface_t *bs = BodyFirst(body, &iter);
  while (bs) {
...
extern void SurfaceSetAddSurfaceSet(SurfaceSet_t *to, SurfaceSet_t *from, bool_t deepCopy) {
  Iterator_t iter;
...
extern void SurfaceSetToGocad(SurfaceSet_t *surfaceSet, PointSet_t *pointSet)
...
extern void SurfaceSetPrintConnectivity(SurfaceSet_t *surfaceSet, FILE *file) {
  ...
  /* Output connectivity */
  {
    ...
    while (surfPair) {
      fprintf(file, "Connection between %s and %s\n", SurfaceUserName(surfPair->surf1),
              SurfaceUserName(surfPair->surf2));
...
extern const Surface_t *SurfaceSetFind(SurfaceSet_t *surfaceSet, int userID, int part, SurfaceType_t type) {
  Iterator_t iter;
  const Surface_t *surface = SurfaceSetFirst(surfaceSet, &iter);
  while (surface) {
...
extern SurfaceSet_t *SurfaceSetSameType(SurfaceSet_t *surfaceSet, SurfaceType_t surfaceType) {
  SurfaceSet_t *result = SurfaceSetCreate();
  Iterator_t iter;
  Surface_t *surface = SurfaceSetFirst(surfaceSet, &iter);
  /* Get id's of surfaces */
...
extern void SurfaceSetMeshMessageAdd(void *meshV, SurfaceSet_t *surfaceSet) {
  Iterator_t iter;
  const Surface_t *surface = SurfaceSetFirst(surfaceSet, &iter);
  while (surface) {
