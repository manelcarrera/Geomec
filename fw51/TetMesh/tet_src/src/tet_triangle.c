...
enum { posUndefined = -1, posLeft, posMiddle, posRight, posLast };

static double Norm3D(const double *p) { return sqrt(p[0] * p[0] + p[1] * p[1] + p[2] * p[2]); }
...
extern int TriangleInterfaceIdx(const Triangle_t *triangle) { return triangle->interfaceIdx; }
...
extern Triangle_t *TriangleCreateFromPoints(int p0, int p1, int p2, int interfaceIdx) {
  int list[3];
  list[0] = p0;
  list[1] = p1;
  list[2] = p2;
...
extern const int *TrianglePoints(const Triangle_t *triangle) { return triangle->vertex; }
...
extern const char *TriangleSPrint(char *buf, const Triangle_t *triangle, PointSet_t *pointSet) {
  ...
  sprintf(buf, "%s\n%s\n%s\n", PointSetSPrint(pointBuf0, pointSet, pointId[0]),
          PointSetSPrint(pointBuf1, pointSet, pointId[1]), PointSetSPrint(pointBuf2, pointSet, pointId[2]));
  return buf;
}
...
extern void TriangleCenter(const Triangle_t *triangle, PointSet_t *pointSet, double *center) {
  const int *pointId = TrianglePoints(triangle);
...
extern int TrianglePointPosition(const Triangle_t *triangle, PointSet_t *pointSet, const double eps,
                                 const double *point) {
...
extern BoundBox_t *TriangleBoundBox(const Triangle_t *triangle, PointSet_t *pointSet, BoundBox_t *boundBox) {
...
extern double TriangleArea(const Triangle_t *triangle, PointSet_t *pointSet) {
  double normal[3];
 ...
 */
static double PointDistancePlane2D(const double *refPoint, const double *normal, const double *point) {
  return normal[0] * (point[0] - refPoint[0]) + normal[1] * (point[1] - refPoint[1]);
}
 ...
 */
static double PointDistancePlane3D(const double *refPoint, const double *normal, const double *point) {
  return normal[0] * (point[0] - refPoint[0]) + normal[1] * (point[1] - refPoint[1]) +
...
static int LineIntersectPlane(const double *planePoint, const double *unitNormal, const double *linePoint1,
                              const double *linePoint2, const double eps, double *pointInPlane) {
...
static void OrthoNormalAxes(const double *normal0, double *normal1, double *normal2) {
  int d;
  int dMin = -1;
  double inpMin = 1.;
  double inp;
...
static void ProjectPointOnPlane2D(const double *refPoint, const double *normal1, const double *normal2,
                                  const double *point3D, double *point2D) {
  double p[3];
  /* Shift */
 ...
 */
static bool_t LineNormal2D(const double *p0, const double *p1, double *normal, const double smallLength) {
  double rvec[2], dist;
  bool_t isDegenerate;
 ...
 * this is not tested.
 */
static bool_t PointOnLine2D(const double *line0, const double *line1, const double *point) {
 ...
 */
static int PointPositionLine2D(const double *line0, const double *line1, const double *refPoint, const double *point,
...
static bool_t PointInTriangle2D(const double *t0, const double *t1, const double *t2, const double *point,
                                const double smallLength) {
  int pos;
 ...
 */
static bool_t LineIntersectLine2D(const double *p0, const double *p1, const double *q0, const double *q1,
                                  const double smallLength) {
  bool_t intersect = FALSE;
  double normal[2];
  bool_t isDegenerate = LineNormal2D(p0, p1, normal, smallLength);
  const double dist0 = PointDistancePlane2D(p0, normal, q0), dist1 = PointDistancePlane2D(p0, normal, q1);
  const int pos0 = DistToPos(dist0, smallLength), pos1 = DistToPos(dist1, smallLength);
 ...
 */
extern bool_t TriangleIntersectLine(const Triangle_t *triangle, PointSet_t *pointSet, const double *p1,
                                    const double *p2, const double smallLength) {
  ...
  if (!TriangleUnitNormal(triangle, pointSet, unitNormal)) {
    ...
    if (nInPlane) {
      double pb[3], pc[3], xAxis2D[3], yAxis2D[3], pa2D[2], pb2D[2], pc2D[2];
...
extern double TriangleAspectRatio(const Triangle_t *triangle, PointSet_t *pointSet) {
  int side;
...
static double TriangleAngle(const Triangle_t *triangle, PointSet_t *pointSet, int corner) {
  ...
  len1 = Norm3D(d1);
  len1 = MAX(len1, DBL_MIN);
  len2 = Norm3D(d2);
  len2 = MAX(len2, DBL_MIN);
  return ArcCosinus(ABS(inprod / len1 / len2));
}

/* Return smallest angle in radians */
extern double TriangleSmallestAngle(const Triangle_t *triangle, PointSet_t *pointSet) {
...
extern bool_t TriangleCommonEdge(const Triangle_t *triangle1, const Triangle_t *triangle2, int *side1, int *side2) {
  bool_t found = FALSE;
  for (*side1 = 0; *side1 < 3; (*side1)++) {
    ...
    for (*side2 = 0; *side2 < 3; (*side2)++) {
      ...
      found = MIN(p1_1, p1_2) == MIN(p2_1, p2_2) && MAX(p1_1, p1_2) == MAX(p2_1, p2_2);
      if (found)
        break;
    }
    if (found)
...
extern bool_t TriangleCommonPoint(const Triangle_t *triangle1, const Triangle_t *triangle2, int *commonP) {
  int common = -1;
...
extern double TriangleAngleBetween(const Triangle_t *triangle1, const Triangle_t *triangle2, PointSet_t *pointSet) {
  int side1, side2;
  int p1, p2, other1, other2;
  double xAxis2D[3], yAxis2D[3], point1[3], point2[3], unitEdge[3], refPoint[3], corOther1_3D[3], corOther1_2D[2],
      corOther2_3D[3], corOther2_2D[2];
  bool_t isDegenerate;
  ...
  isDegenerate = UtilNormalize(corOther1_2D, NULL, 2);
  assert(!isDegenerate);
  ...
  isDegenerate = UtilNormalize(corOther2_2D, NULL, 2);
  assert(!isDegenerate);

  return ArcCosinus(VecInprod(corOther1_2D, corOther2_2D, 2));
}

extern void TriangleLocate2(const Triangle_t *triangle, int p0, int p1, int *other, bool_t *orient) {
  const int *points = TrianglePoints(triangle);
  int n;
...
static bool_t TriangleIntersect12(const Triangle_t *triangle1, const Triangle_t *triangle2, PointSet_t *pointSet) {
  bool_t intersect = FALSE;
  int side2;
  for (side2 = 0; side2 < 3; side2++) {
    int p1, p2;
    TriangleSide(triangle2, side2, &p1, &p2);
    if (TriangleIntersectLine(triangle1, pointSet, PointSetGet(pointSet, p1), PointSetGet(pointSet, p2),
...
extern bool_t TriangleIntersect(const Triangle_t *triangle1, const Triangle_t *triangle2, PointSet_t *pointSet) {
  bool_t intersect = TriangleIntersect12(triangle1, triangle2, pointSet);
  if (!intersect)
    intersect = TriangleIntersect12(triangle2, triangle1, pointSet);
  return intersect;
}

extern bool_t TriangleContains(const Triangle_t *triangle, int point) {
  return (triangle->vertex[0] == point || triangle->vertex[1] == point || triangle->vertex[2] == point);
}

extern bool_t TriangleDebug(const Triangle_t *triangle) {
  bool_t found = FALSE;
...
static void IntersectAddTriangle(Surface_t *triangleSet, int p0, int p1, int p2, int interfaceIdx) {
  if (p0 != p1 && p0 != p2 && p1 != p2) {
    SurfaceAdd(triangleSet, TriangleCreateFromPoints(p0, p1, p2, interfaceIdx));
  }
}

extern void TriangleIntersectPlane(Surface_t *triangleSet, const Triangle_t *triangle, PointSet_t *pointSet,
                                   const double *planePoint, const double *unitNormal) {
  int n;
  const int *points = TrianglePoints(triangle);
  const double *cor[3];
  int pos[3];
  ...
  if (posCount[posMiddle] == 3) {
    ...
    IntersectAddTriangle(triangleSet, points[pointRight[0]], points[pointMiddle[0]], i0,
                         TriangleInterfaceIdx(triangle));
...
extern void TriangleMeshMessageAdd(void *meshV, PointSet_t *pointSet, const Triangle_t *triangle) {
  char buf[BUFSIZ];
