...
extern void BodySetAdd(BodySet_t *bodySet, Body_t *body) { SetAdd(bodySet->body, &body, 1); }
...
extern int BodySetSize(const BodySet_t *bodyset) { return SetSize(bodyset->body); }

extern BoundBox_t *BodySetBoundBox(BodySet_t *bodySet, PointSet_t *pointSet, BoundBox_t *boundBox) {
  Iterator_t iter;
  Body_t *body = BodySetFirst(bodySet, &iter);
  while (body) {
...
static bool_t BodyContainsFullTest(Body_t *bodyOuter, Body_t *bodyInner, PointSet_t *pointSet,
...
static Set_t *BodyContainBodies(const int bodyId, BoundBoxTree_t *bbTree, BodySet_t *bodySet, PointSet_t *pointSet,
                                const double smallLength) {
...
static void GetHierarchy(void *mesh, BodySet_t *bodySet, const int bodyId, const int outerBodyId, PointSet_t *pointSet,
                         BoundBoxTree_t *bbTree, void *bodyDone) {
...
extern void BodySetHierarchy(void *mesh, BodySet_t *bodySet, PointSet_t *pointSet) {
