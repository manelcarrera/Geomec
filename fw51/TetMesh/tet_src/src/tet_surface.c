...
extern Surface_t *SurfaceCreate( int surfaceID, int part, SurfaceType_t surfaceType, 
                                 FrontDir_t frontDir, 
...
{
   Surface_t *result = SurfaceCreate( SurfaceID(surface), 
                                      SurfacePartGet(surface), 
                                      SurfaceType(surface), 
...
extern void  SurfaceAddSurface( 
                  Surface_t                  *surfaceTo, 
                  Surface_t                  *surfaceFrom )
{
   Iterator_t  iter;
...
extern int   SurfaceSize( const Surface_t *surface )
{
   return SetSize( surface->triangle );
}

extern const char *SurfaceUserName( const Surface_t *surface )
{
   return surface->userName;
}

extern int SurfaceID( const Surface_t *surface )
{
   return surface->surfaceID;
}

extern SurfaceType_t SurfaceType( const Surface_t *surface )
{
   return surface->surfaceType;
}

extern FrontDir_t SurfaceGetFrontDir( const Surface_t *surface )
{
   return surface->frontDir;
}

extern void SurfaceSetFrontDir( Surface_t *surface, FrontDir_t frontDir )
{
   assert( surface->frontDir == frontUnknownDir || 
           surface->frontDir == frontDir );
...
extern const char *SurfaceUserInfo( char *buf, const Surface_t *surface )
{
   ...
   assert( surface != NULL );
   sprintf( buf, " NAME=%s\tID=%d\tPART=%d\tTYPE=%s%s ", 
                     SurfaceUserName( surface ), 
                     SurfaceID( surface ),
                     SurfacePartGet( surface ),
                     SurfaceTypeString ( surface, surfType ),
...
{
   char buf[ BUFSIZ ];
   fprintf( fp, "SURFACE %s (number of triangles=%d) ", 
...
extern bool_t SurfaceCheckEdgeOrientation( 
                  Surface_t  *surface1, int poly1, int edge1, bool_t flip1,
                  Surface_t  *surface2, int poly2, int edge2, bool_t flip2  )
{
...
static void SurfaceGetNeighbours( 
                     Surface_t              *surface,
                     int                     triangleId,
                     Set_t                  *neighbours )
{
...
static bool_t SurfaceOrientateTriangle( Surface_t *surface, int curPolyId, bool_t curFlip,
                                     int ngbpolyId )
{
   bool_t     ngbFlip = FALSE;
   int        curEdge, ngbEdge;
   bool_t     found = SurfaceCommonEdge( surface, curPolyId, ngbpolyId, 
                                         &curEdge, &ngbEdge  );
   assert( found );
   ngbFlip = !SurfaceCheckEdgeOrientation( surface, curPolyId, curEdge, curFlip, 
                                           surface, ngbpolyId, ngbEdge, FALSE );
   return ngbFlip;
}
...
extern void SurfaceOrientate(  Surface_t *surface )
{
   ...
   enum { surfOrientDoneFlip = -2, surfOrientStackFlip,
          surfOrientFresh = 0, 
          surfOrientStack, surfOrientDone };
...
extern BoundBox_t *SurfaceBoundBox( 
                     Surface_t              *surface,
                     PointSet_t             *pointSet,
                     BoundBox_t             *boundBox )
{
...
extern bool_t SurfaceFrontBackOfSameFault(
...
extern SurfOuterBoundary_t SurfaceOuterBoundaryGet( const Surface_t *surf )
{
  return surf->outerBoundarY;
}
...
extern bool_t SurfaceGetOpenFault( const Surface_t *surf )
{
  return surf->openFault;
}

extern void SurfaceSetOpenFault( Surface_t *surf, bool_t isOuter )
{
  surf->openFault = isOuter;
}
...
extern int SurfacePartGet( const Surface_t *surf )
{
  return surf->part;
}

static char *SurfaceGocadFileName(
...
extern Surface_t *SurfaceIntersectPlane( 
...
extern bool_t SurfaceFrontBackIdenticalTest( 
                  Surface_t                  *front, 
                  Surface_t                  *back )
{
...
extern bool_t SurfaceIsFault( 
                  const Surface_t            *surf )
{
  return ( SurfaceType( surf ) != surfNormal );
}

extern int SurfacePairCompare(
                  const void                 *surfPair1V,
                  const void                 *surfPair2V )
{
...
extern SurfacePair_t *SurfacePairInit( 
...
extern void SurfaceMeshMessageAdd( 
                  void                       *meshV, 
