...
static XPoint_t *XPointInit(int orgId, const Surface_t *surf, int triangleId, XPoint_t *xp) {
  UTIL_ZERO(xp);
  xp->orgId = orgId;
  xp->surf = surf;
  assert(triangleId >= 0);
...
extern int XPointSetSize(const XPointSet_t *xPointSet) { return (int)RBTreeSize(xPointSet->extraPoints); }

static XFaultPoint_t *XFaultPointInit(XFaultPoint_t *xfp, const double *shift) {
  UTIL_ZERO(xfp);
...
extern void XPointSetAddFault(XPointSet_t *xpointSet, int orgId, const Surface_t *fault, int triangleId,
                              const double *shift) {
...
extern int XPointSetUnshift(XPointSet_t *xpointSet, int orgId, const Surface_t *surf, int triangleId) {
  int numChange = 0;
...
extern XPoint_t *XPointSetFirst(XPointSet_t *xpointSet, Iterator_t *iter) {
  return RBTreeFirst(xpointSet->extraPoints, iter);
}

/* Return pointer to pointDataId */
extern XPoint_t *XPointSetNext(XPointSet_t *xpointSet, Iterator_t *iter) {
  return RBTreeNext(xpointSet->extraPoints, iter);
}

extern XPoint_t *XPointSetFind(const XPointSet_t *xpointSet, int orgId, const Surface_t *surf, int triangleId) {
  XPoint_t xp;
  return RBTreeFindGet(xpointSet->extraPoints, XPointInit(orgId, surf, triangleId, &xp));
}

extern XPointType_t XPointType(const XPoint_t *xp) { return xp->type; }

extern int XPointOrgId(const XPoint_t *xp) { return xp->orgId; }

extern const XPoint_t *XPointSetFollowLink(const XPointSet_t *xpointSet, const XPoint_t *xp) {
...
extern int XPointSetPointId(const XPointSet_t *xpointSet, const XPoint_t *xp) {
  int pointId = -1;
...
static int LinkFaultPoint(XPointSet_t *xpointSet, XPoint_t *xp1, XPoint_t *xp2) {
  int numChange = 0;
...
extern int XPointSetLinkFault(XPointSet_t *xpointSet, int orgId, const Surface_t *fault1, int triangleId1,
                              const Surface_t *fault2, int triangleId2) {
  int numChange = 0;
...
extern int XPointSetLinkNormal(XPointSet_t *xpointSet, int orgId, const Surface_t *normal, int triangleId,
...
fprintf( stderr, "\n\n" );
    ...
    if (xp1Link != xp2Link) {
      ...
      if ((type1 == xpointFault) && (type2 == xpointFault)) {
        ...
        numChange += XPointSetLinkFault(xpointSet, xp1Link->orgId, xp1Link->surf, xp1Link->triangleId, xp2Link->surf,
                                        xp2Link->triangleId);
...
static int LinkNormalToLinkedNormal(int orgId, const Surface_t *normal, int triangleId, const XPoint_t *xp,
...
extern int XPointSetLinkNormal2(XPointSet_t *xpointSet, int orgId, const Surface_t *normal1, int triangleId1,
                                const Surface_t *normal2, int triangleId2) {
  int numChange = 0;
...
static void XPointSetPrintFaultPoint(const XFaultPoint_t *fault, FILE *file) {
  fprintf(file, " pointId=%d   shift=(%f, %f, %f)", fault->pointId, fault->shift[0], fault->shift[1], fault->shift[2]);
}

static void XPointSetPrintNormalPoint(const XNormal2Fault_t *normal, FILE *file) {
  UNUSED(normal);
  UNUSED(file);
}

extern void XPointSetPrintPoint(const XPoint_t *xp, FILE *file) {
  char buf[BUFSIZ];
  fprintf(file, "ExtraPoint orgId=%d offset=%d type=%s surf=%s triangleId=%d", xp->orgId, xp->offset,
          XPointTypeToString[xp->type], SurfaceUserInfo(buf, xp->surf), xp->triangleId);
...
extern void XPointSetPrint(XPointSet_t *xpointSet, FILE *file) {
  Iterator_t iter;
...
extern bool_t XPointSetPointInAnyFault(const XPointSet_t *xpointSet, int orgId) {
  return (XPointSetFind(xpointSet, orgId, NULL, 0) != NULL);
}

static bool_t XPointSetTestShift(const double *shift, PointSet_t *pointSet, XPointSet_t *xpointSet, Set_t *xpoints) {
  bool_t isOK = TRUE;
  Iterator_t iter;
...
static void XPointGetCleverShift(double *shift, PointSet_t *pointSet, XPointSet_t *xpointSet, Set_t *xpoints) {
 ...
 */
extern double *XPointGetShift(PointSet_t *pointSet, XPointSet_t *xpointSet, const XPoint_t *xp, double shiftScaleFactor,
                              double *shift) {
