...
static ShiftPointSet_t *ShiftPointSetCreate(void) { return RBTreeCreate(sizeof(ShiftPoint_t), ShiftPointComp); }

static ShiftPointSet_t *ShiftPointSetDelete(ShiftPointSet_t *spSet) { return RBTreeDelete(spSet); }

static const ShiftPoint_t *ShiftPointSetFind(ShiftPointSet_t *spSet, int pointId) {
  ShiftPoint_t sp;
  sp.pointId = pointId;
...
static int ShiftPointGetOrgId(const ShiftPoint_t *sp) { return sp->orgId; }

static int ShiftPointSetGetOrgId(ShiftPointSet_t *spSet, int pointId) {
...
static const ShiftPoint_t *ShiftPointSetAdd(ShiftPointSet_t *spSet, int pointId, int orgId) {
  ShiftPoint_t sp;
...
static Set_t *ClusterInnerBodies(void *mesh, Set_t *innerBodySet) {
...
static Body_t *CavityShell(void *mesh, BodySet_t *bodySet) {
  RBTree_t *surfBodyTree = RBTreeCreate(sizeof(Surf2Bodies_t), Surf2BodiesComp);
...
static RBTree_t *BodyInnerCreate(void *mesh, BodySet_t *bodySet) {
...
static RBTree_t *BodyInnerDelete(RBTree_t *bodyInnertree) {
  if (bodyInnertree) {
    Iterator_t iter;
...
static bool_t SurfIdTypeExists(void *surfIdTypeTree, int ID, SurfaceType_t type) {
  SurfIdType_t sit;
  bool_t result = FALSE;
  sit.ID = ID;
  sit.type = type;
  if (RBTreeFind(surfIdTypeTree, &sit) != RBTREE_INVALID_ID)
    result = TRUE;
  return result;
}

static void SurfIdTypeAdd(void *surfIdTypeTree, const char *name, int ID, SurfaceType_t type) {
  SurfIdType_t sit;
  sit.ID = ID;
  sit.type = type;
...
static MeshErrorData_t *MeshErrorDataCopy(MeshErrorData_t *from) {
 ...
 *
 * Clearly these examples do not exhaust all possible situations. Now we go over * to the to the assemble of bodies from
 * the simple surfaces (normal and shifted front/back).
 *
 ...
 *     *-N-N-N-N-N-N-N-N-N-*
 *
 * After meshing in FemGvMeshBody(), the fault points are unshifted and added to * the full tetrahedron set in
 * MeshAddTetrahedra(). Finally the mesh points and tetrahedra can be retrieved (MeshBodyTetrahedronAt(),
 * MeshBodyCount(), etc.).
 */
...
static MeshDerivedData_t *MeshDerivedDataDelete(MeshDerivedData_t *derivedData) {
  if (derivedData) {
...
extern void MeshSurfaceOpen(void *meshv, int surfaceID, const char *userName, bool_t bFault) {
  Mesh_t *mesh = meshv;
  MeshModelData_t *modelData = mesh->modelData;
  MeshReadBuffer_t *readBuffer = mesh->readBuffer;
  bool_t normalExists = SurfIdTypeExists(modelData->surfIdTypeTree, surfaceID, surfNormal);
  bool_t frontExists = SurfIdTypeExists(modelData->surfIdTypeTree, surfaceID, surfFrontFault);
  bool_t backExists = SurfIdTypeExists(modelData->surfIdTypeTree, surfaceID, surfBackFault);
  assert(readBuffer->frontSurface == NULL && readBuffer->backSurface == NULL);
  ...
  if (!bFault) {
    if (normalExists || frontExists || backExists) {
      ...
      sprintf(buf,
              "Failure to create normal surface USERNAME=%s;"
              " ID=%d exists already ",
              userName, surfaceID);
    ...
    if (normalExists || frontExists || backExists) {
      ...
      MeshMessageOpen(mesh, MeshStatusSurfaceIdExists);
      sprintf(buf,
              "Failure to create front of fault USERNAME=%s;"
              " ID=%d exists already ",
              userName, surfaceID);
      MeshMessageAdd(mesh, buf);
      MeshMessageClose(mesh);
  ...
  if (bFault) {
    SurfIdTypeAdd(modelData->surfIdTypeTree, userName, surfaceID, surfFrontFault);
    SurfIdTypeAdd(modelData->surfIdTypeTree, userName, surfaceID, surfBackFault);
    readBuffer->frontSurface = SurfaceCreate(surfaceID, 0, surfFrontFault, frontUnknownDir, userName, surfOuterUnknown);
    readBuffer->backSurface = SurfaceCreate(surfaceID, 0, surfBackFault, frontUnknownDir, userName, surfOuterUnknown);
  } else {
    SurfIdTypeAdd(modelData->surfIdTypeTree, userName, surfaceID, surfNormal);
    readBuffer->frontSurface = SurfaceCreate(surfaceID, 0, surfNormal, frontUnknownDir, userName, surfOuterUnknown);
  }
}
  ...
  if (SurfaceSize(surface)) {
    ...
    if (!MESH_ERROR(mesh) && SurfaceIsFault(surface)) {
      ...
      if (SurfaceSetSize(simpleSurfSet) > 1) {
        ...
        MeshMessageAdd(
            mesh,
            "Fault surface consists of more than 1 part. Give different labels to different parts of the fault:\n");
        SurfaceMeshMessageAdd(mesh, surface);
        MeshMessageClose(mesh);
      }
    }
...
extern void MeshPointAdd(void *meshV, const double p0, const double p1, const double p2) {
  Mesh_t *mesh = meshV;
  BoundBox_t bb;
  MeshReadBuffer_t *readBuffer = mesh->readBuffer;
...
extern void MeshSurfaceAdd(void *meshv, const double *p1, const double *p2, const double *p3) {
  Mesh_t *mesh = meshv;
  int corners[4];
  MeshReadBuffer_t *readBuffer = mesh->readBuffer;
...
static void OrientFault(Mesh_t *mesh, PointSet_t *pointSet, Surface_t *front, Surface_t *back) {
  bool_t ready = FALSE;
...
static void MeshOrientFaults(Mesh_t *mesh, PointSet_t *pointSet, SurfaceSet_t *surfaceSet) {
  int n;
...
static bool_t UnexpectedIntersection(const Triangle_t *triangle1, const Triangle_t *triangle2, PointSet_t *pointSet) {
 ...
 */
static void MeshCheckSurfaceIntersection(Mesh_t *mesh, const bool_t *surfTypeActive, SurfaceSet_t *surfaceSet,
                                         PointSet_t *pointSet, ShiftPointSet_t *spSet) {
...
static void MeshIsolatedEdges(Mesh_t *mesh, SurfaceSet_t *surfaceSet, PointSet_t *pointSet) {
...
static void MeshCheckTriangleAngles(Mesh_t *mesh, PointSet_t *pointSet, SurfaceSet_t *surfaceSet) {
...
static void MeshCheckTriangleAngleBetween(Mesh_t *mesh, PointSet_t *pointSet, SurfaceSet_t *surfaceSet) {
  Edge_t *edge = SurfaceSetEdge(surfaceSet);
  Iterator_t keyIter;
  EdgeKey_t *key = EdgeKeyFirst(edge, &keyIter);
    ...
    for (t1 = 0; t1 < edgeSize; t1++) {
      ...
      for (t2 = t1 + 1; t2 < edgeSize; t2++) {
        ...
        if (!SurfaceFrontBackOfSameFault(edgeSurface1, edgeSurface2)) {
          ...
          const double angle = TriangleAngleBetween(SurfaceGet(edgeSurface1, triangleId1),
                                                    SurfaceGet(edgeSurface2, triangleId2), pointSet);
          assert(angle >= 0);
          if (angle < minAngle) {
            minAngle = angle;
            surf1 = edgeSurface1;
...
static void InvalidateIsolatedFaultEdgeXPoints(Mesh_t *mesh, SurfaceSet_t *surfaceSet, XPointSet_t *xpointSet) {
  Edge_t *edge = SurfaceSetEdge(surfaceSet);
  Iterator_t keyIter;
  EdgeKey_t *key = EdgeKeyFirst(edge, &keyIter);
...
static void SetFaultPointShift(PointSet_t *pointSet, const Surface_t *surf, const Triangle_t *triangle, double *shift) {
...
static void DuplicateFaultXPoints(PointSet_t *pointSet, SurfaceSet_t *surfaceSet, XPointSet_t *xpointSet) {
  Iterator_t surfaceSetIter;
  Surface_t *surf = SurfaceSetFirst(surfaceSet, &surfaceSetIter);
...
static int ShiftNewPoint(Mesh_t *mesh, int orgId, const double *baseShift, PointSet_t *pointSet) {
  int newPointId = -1;
  double org[3];
  bool_t ready = FALSE;
...
static void AddExtraFaultPoints(Mesh_t *mesh, PointSet_t *pointSet, XPointSet_t *xpointSet, ShiftPointSet_t *spSet) {
  Iterator_t iter;
  XPoint_t *xp = XPointSetFirst(xpointSet, &iter);
  while (xp && !MESH_ERROR(mesh)) {
    if (xp->type == xpointFault) {
      double baseShift[3];
      const int pointId = ShiftNewPoint(
          mesh, xp->orgId, XPointGetShift(pointSet, xpointSet, xp, MeshShiftScaleFactor(mesh), baseShift), pointSet);
      if (!MESH_ERROR(mesh)) {
...
static void ChangeTriangles(SurfaceSet_t *surfaceSet, XPointSet_t *xpointSet) {
  Iterator_t surfaceSetIter;
  Surface_t *surf = SurfaceSetFirst(surfaceSet, &surfaceSetIter);
...
static bool_t FaultCornerSet(RBTree_t *edges, EdgeKey_t *key, const EdgeList_t **list, Set_t *edgeList) {
...
static void AnalyzeFaultTriangle(const EdgeKey_t *key, const EdgeList_t *list, int *other, bool_t *orient) {
  const Surface_t *fault = EdgeListSurface(list);
...
static void SplitTriangle(Surface_t *surf, int triangleId, int interfaceIdx, int point0, int point1, int pointX) {
...
static bool_t SameFaultSide(const EdgeKey_t *key, const EdgeList_t *list1, const EdgeList_t *list2) {
  bool_t sameSide = !SurfaceFrontBackOfSameFault(EdgeListSurface(list1), EdgeListSurface(list2));
  if (sameSide) {
    int other1, other2;
...
*/
const EdgeList_t *FindBackTriangle(Edge_t *edge, EdgeKey_t *key, int nInterfaceIdx) {
...
*/
static bool_t SortEdgeList(Edge_t *edge, EdgeKey_t *key, const EdgeList_t **ret) {
  int i;
  const EdgeList_t *list = EdgeListFirst(edge, key);
  ...
  if (ret[0] && ret[1] && ret[2] && ret[3] && SameFaultSide(key, ret[0], ret[2]) && SameFaultSide(key, ret[1], ret[3]))
    return TRUE;
 ...
 *        x   o    x
 */
static bool_t AddExtraMidPoints(Mesh_t *mesh, PointSet_t *pointSet, SurfaceSet_t *surfaceSet) {
  Edge_t *edge = SurfaceSetEdge(surfaceSet);
      ...
      if (!EdgeIsolatedFault(edge, key)) {
        ...
        if (RBTreeFindGet(edgePoints, &p0) && RBTreeFindGet(edgePoints, &p1)) {
          ...
          if (SortEdgeList(edge, key, list)) {
            ...
            if (!FaultCornerSet(edges, key, list, edgeList))
              ready = FALSE;
          }
        }
      }
...
static void MeshExtraFaultPoints(Mesh_t *mesh, ShiftPointSet_t *spSet, PointSet_t *pointSet, SurfaceSet_t *surfaceSet) {
...
static int MeshOutputNode2Offset(const MeshOutput_t *meshOutput, int nodeId) {
  int n2p[2];
  n2p[0] = nodeId;
  return RBTreeFind(meshOutput->node2Point, n2p);
}

static int Node2PointAdd(RBTree_t *node2Point, ShiftPointSet_t *spSet, int nodeId) {
  int nodePoint[2], *np;
  nodePoint[0] = nodeId;
  nodePoint[1] = -1;
...
static void MeshAddTetrahedra(Mesh_t *mesh, Body_t *body, SurfaceSet_t *activeSurfaces, ShiftPointSet_t *spSet,
                              TetrahedronSet_t *tetSet, MeshOutput_t *meshOutput) {
  Iterator_t iter;
  BodyMesh_t bm;
...
static SurfaceSet_t *GetActiveSurfaces(Body_t *body, BodySet_t *cavities) {
...
static MeshTriangleSet_t *BuildMeshTriangleSet(TetrahedronSet_t *tetSet) {
...
static bool_t MeshTriangleSurfaces(MeshTriangleSet_t *mtSet, PointSet_t *pointSet, SurfaceSet_t *surfaceSet) {
...
static bool_t MeshBodyIsCorrect(TetrahedronSet_t *tetSet, PointSet_t *pointSet, SurfaceSet_t *surfaceSet) {
  /* Any tetrahedra ? */
...
static void SplitTriangleList(Mesh_t *mesh, Edge_t *edge, EdgeKey_t *key, int p0, int p1, int midPoint) {
  const EdgeList_t *list = EdgeListFirst(edge, key);
  ...
  while (list) {
    ...
    if (SurfaceType(surface) == surfNormal) {
      ...
      if (SurfaceType(surface) == surfFrontFault) {
        ...
        int nInterfaceIdx = TriangleInterfaceIdx(SurfaceGet(EdgeListSurface(list), EdgeListTriangleId(list)));
        const EdgeList_t *back = FindBackTriangle(edge, key, nInterfaceIdx);
        if (back) {
          SplitTriangle(EdgeListSurface(list), EdgeListTriangleId(list), newIdx, p0, p1, midPoint);
          SplitTriangle(EdgeListSurface(back), EdgeListTriangleId(back), newIdx, p0, p1, midPoint);
        }
      }
    }
    list = EdgeListNext(edge, list);
...
static bool_t SplitSingleEdgeSeam(Mesh_t *mesh, SurfaceSet_t *surfaceSet, PointSet_t *pointSet) {
  bool_t ready = TRUE;
...
static void ConstructBodies(Mesh_t *mesh, ShiftPointSet_t *spSet, PointSet_t *pointSet, SurfaceSet_t *surfaceSet,
                            BodySet_t *bodySet) {
...
static bool_t UnshiftPoints(ShiftPointSet_t *spSet, int nPoints, const int *oldPoints, int *newPoints) {
  bool_t changed = FALSE;
  int n;
  for (n = 0; n < nPoints; n++) {
    newPoints[n] = ShiftPointSetGetOrgId(spSet, oldPoints[n]);
    /* This point is shifted? */
    if (oldPoints[n] != newPoints[n])
      changed = TRUE;
  }
  return changed;
}

static bool_t UnshiftTriangle(ShiftPointSet_t *spSet, Triangle_t *triangle) {
  bool_t changed = FALSE;
  int newPoints[3];
...
static void UnshiftSurfaces(ShiftPointSet_t *spSet, SurfaceSet_t *surfaceSet) {
  bool_t changed = FALSE;
  Iterator_t surfaceSetIter;
  Surface_t *surf = SurfaceSetFirst(surfaceSet, &surfaceSetIter);
  while (surf) {
...
static TetrahedronSet_t *UnshiftTetrahedra(ShiftPointSet_t *spSet, TetrahedronSet_t *tetSet) {
...
static TetrahedronSet_t *UnshiftSurfacesAndMesh(ShiftPointSet_t *spSet, SurfaceSet_t *surfaceSet,
                                                TetrahedronSet_t *tetSet) {
  UnshiftSurfaces(spSet, surfaceSet);
  return UnshiftTetrahedra(spSet, tetSet);
}

static void MeshBodyFailure(Mesh_t *mesh, SurfaceSet_t *activeSurfaces) {
...
static SurfaceSet_t *MarkTrueOuterBoundary(Mesh_t *mesh, ShiftPointSet_t *spSet, PointSet_t *pointSet,
                                           SurfaceSet_t *surfaceSet) {
...
static void SetPointShift(PointSet_t *pointSet, const Surface_t *surf, const Triangle_t *triangle, double *shift) {
  /* Inward shift */
...
static void ShiftIntersectingOuterBoundaryPoint(Surface_t *fault, PointSet_t *pointSet, XPointSet_t *xpointSet,
                                                Edge_t *edge, EdgeKey_t *key, int point) {
  EdgeList_t *list = EdgeListFirst(edge, key);
  while (list) {
    const Surface_t *surf = EdgeListSurface(list);
...
static void OpenSelfIntersectingOuterBoundary(Mesh_t *mesh, ShiftPointSet_t *spSet, PointSet_t *pointSet,
                                              SurfaceSet_t *surfaceSet) {
  Edge_t *edge = SurfaceSetEdge(surfaceSet);
  Iterator_t iter;
  EdgeKey_t *key = EdgeKeyFirst(edge, &iter);
...
static TetrahedronSet_t *MeshTamperedBody(Mesh_t *mesh, ShiftPointSet_t *oldSpSet, PointSet_t *pointSet,
                                          SurfaceSet_t *surfaceSet, Body_t *body, BodySet_t *cavities,
    ...
    for (surfCount = 0; surfCount < nSurface; surfCount++) {
      ...
      if (SurfaceAtOuterBoundary(outerSurf)) {
        ...
        if (!SurfaceSetFind(newSurfaceSet, ID, part, surfNormal)) {
          newSurf = SurfaceCreate(ID, part, surfNormal, frontUnknownDir, SurfaceUserName(surf),
                                  SurfaceOuterBoundaryGet(outerSurf));
        }
      } else {
        /* This is not a copy: the outer boundary info is taken from outerSurf !! */
        newSurf = SurfaceCreate(ID, part, SurfaceType(surf), SurfaceGetFrontDir(surf), SurfaceUserName(surf),
                                SurfaceOuterBoundaryGet(outerSurf));
      }
      if (newSurf) {
        /* Copy surface */
...
static void MeshCheckSingleSideHorizonAddBody(Mesh_t *mesh, Body_t *bodyAll, Body_t *body) {
  Iterator_t iter;
  BodySurface_t *bs = BodyFirst(body, &iter);
...
static void MeshCheckSingleSideHorizon(Mesh_t *mesh, Body_t *body, BodySet_t *cavities) {
...
static TetrahedronSet_t *MeshSingleBody(Mesh_t *mesh, ShiftPointSet_t *spSet, PointSet_t *pointSet,
                                        SurfaceSet_t *surfaceSet, Body_t *body, BodySet_t *cavities,
                                        const FemGvPath_t *FemGvPath) {
  TetrahedronSet_t *result = NULL;
  Edge_t *edge = SurfaceSetEdge(surfaceSet);
...
static void MeshAddSingleBody(Mesh_t *mesh, ShiftPointSet_t *spSet, Body_t *body, BodySet_t *cavities,
                              PointSet_t *pointSet, MeshOutput_t *meshOutput, const FemGvPath_t *FemGvPath) {
  if (!MESH_ERROR(mesh)) {
...
static void MeshIsCorrect(Mesh_t *mesh, TetrahedronSet_t *tetSet, PointSet_t *pointSet, SurfaceSet_t *surfaceSet) {
  MeshTriangleSet_t *mtSet = BuildMeshTriangleSet(tetSet);
...
static void MeshSinglePointSeamIntersection(Mesh_t *mesh, SurfaceType_t faultType, SurfaceSet_t *surfaceSet,
                                            PointSet_t *pointSet) {
  int nPoints = PointSetSize(pointSet);
    ...
    for (n = 0; n < nPoints; n++) {
      ...
      if (!pointOK[n]) {
        ...
        if (!isOK) {
          ...
          MeshMessageAdd(mesh, "intersect in a single point with coordinates\n");
...
static void MeshCheckIntersection(Mesh_t *mesh, PointSet_t *pointSet, SurfaceSet_t *surfaceSet) {
...
static void MeshPrepareModel(Mesh_t *mesh) {
  ...
  if (!MESH_ERROR(mesh) && PointSetSize(pointSet) > 1) {
    int id1, id2;
    double measure;
    const double dist = PointSetMinimumDistance(pointSet, &id1, &id2, &measure);
    const double crit = 1.e-3;
    if (dist / measure < crit) {
      char buf[BUFSIZ];
      MeshMessageOpen(mesh, MeshStatusClosePoints);
      sprintf(buf, "Smallest distance between two points in the model: %f\n", dist);
      MeshMessageAdd(mesh, buf);
...
extern void MeshGenerate(const char *sDianaPath, const char *sDiaSharePath, const char *sDianaLibPath,
                         const char *sDianaSLibPath, const char *sFgvExeName, void *meshV) {
  Mesh_t *mesh = meshV;
...
/* Mesh bodies */
extern int MeshBodyCount(const void *meshV) {
  const Mesh_t *mesh = meshV;
  return (mesh->meshOutput ? SetSize(mesh->meshOutput->bodyMesh) : 0);
}

extern int MeshBodyTetrahedronCount(int nBodyIndex, const void *meshV) {
  const Mesh_t *mesh = meshV;
  const BodyMesh_t *bm = SetElmGet(mesh->meshOutput->bodyMesh, nBodyIndex);
  return (int)bm->tetrahedronSize;
}

extern int MeshBodyTetrahedronAt(int nBodyIndex, int nElementIndex, const void *meshV) {
  const Mesh_t *mesh = meshV;
  const BodyMesh_t *bm = SetElmGet(mesh->meshOutput->bodyMesh, nBodyIndex);
...
extern int MeshBodySideSurfaceCount(int nBodyIndex, const void *meshV) {
  const Mesh_t *mesh = meshV;
  const BodyMesh_t *bm = SetElmGet(mesh->meshOutput->bodyMesh, nBodyIndex);
  return SurfaceSetSize(bm->sideSurfaces);
}

static Surface_t *MeshBodySideSurfaceGet(int nBodyIndex, int nSideSurfaceIndex, const void *meshV) {
  const Mesh_t *mesh = meshV;
  const BodyMesh_t *bm = SetElmGet(mesh->meshOutput->bodyMesh, nBodyIndex);
  return SurfaceSetGet(bm->sideSurfaces, nSideSurfaceIndex);
}

extern int MeshBodySideSurfaceParentIndex(int nBodyIndex, int nSideSurfaceIndex, const void *meshV) {
  const Surface_t *surf = MeshBodySideSurfaceGet(nBodyIndex, nSideSurfaceIndex, meshV);
  return SurfaceID(surf);
}

extern int MeshBodySideSurfaceParentType(int nBodyIndex, int nSideSurfaceIndex, const void *meshV) {
  const Surface_t *surf = MeshBodySideSurfaceGet(nBodyIndex, nSideSurfaceIndex, meshV);
  return SurfaceType(surf);
}

extern int MeshBodySideSurfaceTriangleCount(int nBodyIndex, int nSideSurfaceIndex, const void *meshV) {
  const Surface_t *surf = MeshBodySideSurfaceGet(nBodyIndex, nSideSurfaceIndex, meshV);
  return SurfaceSize(surf);
}

extern int MeshBodySideSurfaceTriangleInterfaceIdx(int nBodyIndex, int nSideSurfaceIndex, int nTriangleIndex,
                                                   const void *meshV) {
  const Mesh_t *mesh = meshV;
...
extern int MeshBodySideSurfaceTriangleNode(int nBodyIndex, int nSideSurfaceIndex, int nTriangleIndex,
                                           int nLocalNodeIndex, const void *meshV) {
  const Mesh_t *mesh = meshV;
  const Surface_t *surf = MeshBodySideSurfaceGet(nBodyIndex, nSideSurfaceIndex, mesh);
  const Triangle_t *triangle = SurfaceGet(surf, nTriangleIndex);
  const int *points = TrianglePoints(triangle);
