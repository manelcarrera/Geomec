...
struct MODELFRAME_EXPORT TFilePosition {
  ...
  TFilePosition(int _pos) {
    lineNr = -1;
    pos = _pos;
    field = -1;
    line = (const char *)0;
  }
  TFilePosition(int _lineNr, int _pos, int _field, const QString &_line)
      : lineNr(_lineNr), pos(_pos), field(_field), line(_line) {}
...
struct MODELFRAME_EXPORT TFileMessage : public TModelMessage, public TFilePosition {
  ...
  TFileMessage(const QString &msg, int lineNr, int pos, int field, const QString &line)
      : TModelMessage(msg), TFilePosition(lineNr, pos, field, line) {}
};
...
struct MODELFRAME_EXPORT TFileError : public TModelError, public TFilePosition {
  ...
  TFileError(const QString &msg, int errorNo, int lineNr, int pos, int field, const QString &line)
      : TModelError(msg, errorNo), TFilePosition(lineNr, pos, field, line) {}
};
