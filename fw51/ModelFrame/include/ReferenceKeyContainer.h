...
template <class TITEM, class TKEY> class CReferenceKeyContainer : public CReferenceContainer<TITEM> {
public:
...
protected:
  // overrides
  virtual void referenceInserted(const IModelObject &origin, const IModelObject &reference);
...
CReferenceKeyContainer<TITEM, TKEY>::CReferenceKeyContainer(const QString &text) : CReferenceContainer<TITEM>(text) {}

/*!
  Returns the contained TITEM object having the requested key.
  If no such object is found, a zero is returned.
*/
template <class TITEM, class TKEY> const TITEM *CReferenceKeyContainer<TITEM, TKEY>::findKey(TKEY key) const {
...
*/
template <class TITEM, class TKEY>
void CReferenceKeyContainer<TITEM, TKEY>::referenceInserted(const IModelObject &origin, const IModelObject &reference) {
  if (&origin == this) {
...
*/
template <class TITEM, class TKEY>
void CReferenceKeyContainer<TITEM, TKEY>::referenceRemoved(const IModelObject &origin, const IModelObject &reference) {
  if (&origin == this) {
    const TITEM *item = dynamic_cast<const TITEM *>(&reference);
    if (item) {
      TKEY key = getKey(*item);
