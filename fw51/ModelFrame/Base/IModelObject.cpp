...
public:
  bool operator()(const IModelObject *A, const IModelObject *B) const { return A->less(*B); }
};
...
IModelObject::IModelObject()
    : m_color(qRgb(122, 122, 122)), m_text(QString::null), m_created(false), m_destroyed(false) {}
...
IModelObject::IModelObject(const QString &text)
    : QObject(0), m_color(qRgb(122, 122, 122)), m_text(text), m_created(false), m_destroyed(false) {
  setObjectName(text);
}
...
CUndoOperation *IModelObject::constructCreateUndoOperation(IModelObject &object, const QString &description) {
  return new CCreateUndo(object, description);
}

CUndoOperation *IModelObject::constructDestroyUndoOperation(IModelObject &location, IModelObject &object,
...
/*!
  ...
  like undo/redo.
*/
void IModelObject::create(IModelObject *newParent /* = 0 */, const IModelObject *before /* = 0 */) {
  ...
  if (newParent) {
    ...
    if (operation.stackEnabled())
      operation.push(constructCreateUndoOperation(*this, tr("Create %1").arg(text())));
...
void IModelObject::addToParent(IModelObject *newParent, const IModelObject *before /* = 0 */) {
  DIA_ASSERT(m_created);
...
void IModelObject::setText(const QString &new_text) {
  if (new_text != m_text) {
    ...
    if (operation.stackEnabled())
      operation.push(new CTextUndo(*this, m_text, tr("Rename \"%1\"").arg(m_text)));
    m_text = new_text;
    displayChanged();
  }
}
...
*/
QString IModelObject::text() const { return m_text; }

void IModelObject::stateChanged() {
  CModelOperation operation(*this);

  stateChanged(*this);
  for (QList<IModelObject *>::const_iterator it = m_links.begin(); it != m_links.end(); ++it) {
...
void IModelObject::propertyChanged(IModelObject &origin) {
  ...
  IModelObject *pParent = (IModelObject *)parent();
  if (pParent)
    pParent->propertyChanged(origin);
...
void IModelObject::geometryChanged(IModelObject &origin) {
  ...
  IModelObject *pParent = (IModelObject *)parent();
  if (pParent)
    pParent->geometryChanged(origin);
...
void IModelObject::parameterChanged(IModelObject &origin) {
  ...
  IModelObject *pParent = (IModelObject *)parent();
  if (pParent)
    pParent->parameterChanged(origin);
...
void IModelObject::childInserted(IModelObject &origin, IModelObject &child) {
  IModelObject *pParent = (IModelObject *)parent();
  if (pParent)
    pParent->childInserted(origin, child);
...
void IModelObject::childRemoved(IModelObject &origin, IModelObject &child) {
  ...
  IModelObject *pParent = (IModelObject *)parent();
  if (pParent)
    pParent->childRemoved(origin, child);
...
void IModelObject::referenceInserted(const IModelObject &ref) { referenceInserted(*this, ref); }

/*!
  A reference is inserted and the storage is invalidated.
*/
void IModelObject::referenceInserted(const IModelObject &origin, const IModelObject &ref) {
  DIA_ASSERT(m_created);
  COperation operation(*this);
  IModelObject *pParent = (IModelObject *)parent();
  if (pParent)
    pParent->referenceInserted(origin, ref);
  emit onReferenceInserted();
}

void IModelObject::referenceRemoved(const IModelObject &ref) { referenceRemoved(*this, ref); }

/*!
  A reference is removed and the storage is invalidated.
*/
void IModelObject::referenceRemoved(const IModelObject &origin, const IModelObject &ref) {
  DIA_ASSERT(m_created);
  IModelObject *pParent = (IModelObject *)parent();
  if (pParent)
    pParent->referenceRemoved(origin, ref);
  emit onReferenceRemoved();
}

void IModelObject::error(const QString &description) { error(*this, description); }
...
void IModelObject::error(IModelObject &origin, const QString &description) {
  ...
  IModelObject *pParent = (IModelObject *)parent();
  if (pParent)
    pParent->error(origin, description);
  emit onError(description);
}

void IModelObject::warning(const QString &description) { warning(*this, description); }
...
void IModelObject::storeReferences(TStream &stream) const {
  ...
  for (i = 0; i < childSize(); ++i)
    childAt(i).storeReferences(stream);
}
...
/*!
  ...
  Existence of the document is compulsory.
*/
void IModelObject::restoreReferences(TStream &stream, const CStreamVersion &fileVersion) {
  DIA_ASSERT(document());
  ...
  for (i = 0; i < size; ++i) {
    ...
    if (before != object)
      insertReference(*object, before);
  }
  while (referenceSize() > size)
    removeReference(referenceSize() - 1);

  for (i = 0; i < childSize(); ++i)
    childAt(i).restoreReferences(stream, fileVersion);
}

void IModelObject::reParent(IModelObject *new_parent, const IModelObject *before) {
  DIA_ASSERT(m_created);
  IModelObject *pParent = dynamic_cast<IModelObject *>(parent());
  IModelObject *operationLocation = new_parent;
  if (!operationLocation)
    operationLocation = pParent;
  if (!operationLocation)
    return; // No current and not new parent, weird
  ...
  if (pParent) {
    ...
    if (operation.stackEnabled())
      operation.push(constructDestroyUndoOperation(*pParent, *this, "Delete old parent"));
...
*/
int IModelObject::referenceSize() const { return m_references.count(); }
...
*/
int IModelObject::isReferenced(const IModelObject &object) const { return m_references.indexOf(&object); }
...
QRgb IModelObject::color() const { return m_color; }

void IModelObject::color(QRgb c) {
  if (m_color != c) {
    CModelOperation operation(*this);
    if (operation.stackEnabled())
      operation.push(new CColorUndo(*this, m_color, tr("Color \"%1\"").arg(m_text)));
    m_color = c;
    displayChanged();
  }
}
...
void IModelObject::destroy() {
  DIA_ASSERT(m_created);
  if (m_destroyed)
    return; // No sense in handling a second call
  ...
  { // !!! New scope to end operation before delete this
    ...
    while (!m_links.isEmpty())
      m_links.first()->removeReference(*this);
  ...
  while ( sz > 0 ) childAt( --sz ).destroy();
    ...
    if (parent()) {
      ...
      pParent->childRemoved(*this);
      if (operation.stackEnabled())
        operation.push(constructDestroyUndoOperation(*pParent, *this, tr("Delete %1").arg(text())));
      pParent->m_children.removeOne(this);
      setParent(0);
#if 0
...
void IModelObject::Connect(const TModelObjectVec & /*object*/) { DIA_ASSERT(m_created); }
...
void IModelObject::restore(TStream &stream, const CStreamVersion &file_version, IProgressBase &indicator) {
  ...
  if (m_text != previousText || m_color != previousColor)
    displayChanged();
  ...
  if (version >= CStreamVersion(0, 0, 2)) {
    ...
    if (restoreChildren) {
      ...
      if (m_children.count() != nSize)
        qDebug() << "IModelObject::restore " << metaObject()->className() << " " << m_text
                 << " child count = " << m_children.count() << " restored    = " << nSize;
    }
  }
...
void IModelObject::store(TStream &stream, IProgressBase &indicator, bool includeChildren) const {
  // Local version for local changes
...
bool IModelObject::isChildInTree(const IModelObject &object) const {
  ...
  for (i = 0; i < childSize(); i++) {
    if (&childAt(i) == &object)
      return true;
    if (childAt(i).isChildInTree(object))
      return true;
  }
  return false;
}

void IModelObject::moveChildren(const QList<IModelObject *> &children, int destinationIndex) {
...
/*!
  ...
  object.
*/
IModelObject::TModelObjectIterator IModelObject::referenceOrigins() const { return TModelObjectIterator(m_links); }
