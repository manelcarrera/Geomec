...
CDocumentBase::CDocumentBase(const QString &name) : IModelObject(name), m_bOpen(false) {
  m_operationStack = new CModelOperationStack(*this);
  m_operationStack->setEnabled(false);
  connect(m_operationStack, SIGNAL(operationStarted()), this, SIGNAL(onBeginOperation()));
  connect(m_operationStack, SIGNAL(operationFinished()), this, SIGNAL(onEndOperation()));
}
...
const QString &CDocumentBase::pathName() const { return m_path_name; }

CModelOperationStack &CDocumentBase::operationStack() { return *m_operationStack; }
...
void CDocumentBase::stateChanged(IModelObject & /*origin*/) { emit onStateChanged(); }

void CDocumentBase::displayChanged(IModelObject &origin) { emit onDisplayChanged(origin); }

void CDocumentBase::propertyChanged(IModelObject &origin) { emit onPropertyChanged(origin); }

void CDocumentBase::geometryChanged(IModelObject & /*origin*/) { emit onGeometryChanged(); }

void CDocumentBase::parameterChanged(IModelObject & /*origin*/) { emit onParameterChanged(); }

void CDocumentBase::childInserted(IModelObject &origin, IModelObject &child) { emit onChildInserted(origin, child); }

void CDocumentBase::childRemoved(IModelObject &origin, IModelObject &child) { emit onChildRemoved(origin, child); }

void CDocumentBase::referenceInserted(const IModelObject & /*origin*/, const IModelObject & /*ref*/) {
  emit onReferenceInserted();
}

void CDocumentBase::referenceRemoved(const IModelObject & /*origin*/, const IModelObject & /*ref*/) {
  emit onReferenceRemoved();
}

void CDocumentBase::error(IModelObject & /*origin*/, const QString &description) { emit onError(description); }

void CDocumentBase::warning(IModelObject & /*origin*/, const QString &description) { emit onWarning(description); }

void CDocumentBase::open() { m_bOpen = true; }

/*!
  Returns true if the user can create and use a new, empty document.
*/
bool CDocumentBase::canCreateEmptyDocument() const { return true; }
...
bool CDocumentBase::canReadVersion(const CStreamVersion &version) const {
  return (version >= lastReadableVersion()) && (version <= documentVersion());
}
...
bool CDocumentBase::isOpen() { return m_bOpen; }

const QString &CDocumentBase::documentDescription() const { return m_document_description; }

void CDocumentBase::documentDescription(const QString &sDescription) { m_document_description = sDescription; }
