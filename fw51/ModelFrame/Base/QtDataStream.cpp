...
CQtDataStream::CQtDataStream(QDataStream& stream)
: m_stream(stream)
{
}
...
IModelStream& CQtDataStream::operator<< ( long i )
{
  if ( sizeof( long ) == 8 ) m_stream << qint64(i);
  else if ( sizeof( long ) == 4 ) m_stream << qint32(i);
  else qFatal( "can't cope with this sizeof(long)" );
  return *this;
}

IModelStream& CQtDataStream::operator<< ( ulong i )
{
  if ( sizeof( long ) == 8 ) m_stream << quint64(i);
  else if ( sizeof( long ) == 4 ) m_stream << quint32(i);
  else qFatal( "can't cope with this sizeof(ulong)" );
  return *this;
}
