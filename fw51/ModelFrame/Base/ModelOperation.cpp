...
*/
bool CModelOperation::stackEnabled() const
{
  return m_stack != 0 && m_stack->isEnabled();
}

/*!
  Returns a description about the state change(s) being gathered.
*/
const QString& CModelOperation::text() const
{
  return m_text;
}
...
*/
bool CModelOperation::isEmpty() const
{
  return m_localStack.size() == 0;
}
