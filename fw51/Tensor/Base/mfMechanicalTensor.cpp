...
CMechanicalTensor::CMechanicalTensor(const ITensor &rhs) : CTensor(rhs) {}

CMechanicalTensor::CMechanicalTensor(const double &xx, const double &yy, const double &zz, const double &xy,
                                     const double &yz, const double &xz)
    : CTensor(xx, yy, zz, xy, yz, xz) {}

double CMechanicalTensor::InvarI1() const { return XX() + YY() + ZZ(); }
...
double CMechanicalTensor::AzimuthMaxHorizontal() const {
  ...
  two_d_mat.Value(0, 0, XX());
  two_d_mat.Value(0, 1, XY());
  two_d_mat.Value(1, 0, XY());
  two_d_mat.Value(1, 1, YY());
