...
CTensor ITensor::operator*(const double &rhs) const {
  assert(!Empty());
  return CTensor(XX() * rhs, YY() * rhs, ZZ() * rhs, XY() * rhs, YZ() * rhs, XZ() * rhs);
}

ITensor &ITensor::operator*=(const double &rhs) {
  assert(!Empty());
  Set(XX() * rhs, YY() * rhs, ZZ() * rhs, XY() * rhs, YZ() * rhs, XZ() * rhs);
...
CTensor ITensor::operator-(const ITensor &rhs) const {
  ...
  return CTensor(XX() - rhs.XX(), YY() - rhs.YY(), ZZ() - rhs.ZZ(), XY() - rhs.XY(), YZ() - rhs.YZ(), XZ() - rhs.XZ());
}
...
CTensor ITensor::operator+=(const ITensor &rhs) {
  ...
  Set(XX() + rhs.XX(), YY() + rhs.YY(), ZZ() + rhs.ZZ(), XY() + rhs.XY(), YZ() + rhs.YZ(), XZ() + rhs.XZ());
  return *this;
}
...
bool ITensor::operator==(const ITensor &rhs) const {
  ...
  return (fabs(XX() - rhs.XX()) < EPS && fabs(YY() - rhs.YY()) < EPS && fabs(ZZ() - rhs.ZZ()) < EPS &&
          fabs(XY() - rhs.XY()) < EPS && fabs(YZ() - rhs.YZ()) < EPS && fabs(XZ() - rhs.XZ()) < EPS);
}
...
geo::CMatrixPair ITensor::GetEigenPairs() const { return Matrix().GetEigenPairs(false); }
