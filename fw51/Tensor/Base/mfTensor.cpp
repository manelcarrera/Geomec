...
CTensor::CTensor() : m_bEmpty(true) {}

CTensor::CTensor(const double &dInitVal) : m_bEmpty(false) {
  for (int i = 0; i < 6; i++)
    m_pData[i] = dInitVal;
}

// ##ModelId=3BC6995B02C6
CTensor::CTensor(const geo::IMatrix &InitMat) : m_bEmpty(false) {
...
CTensor::CTensor(const double &xx, const double &yy, const double &zz, const double &xy, const double &yz,
                 const double &xz)
    : m_bEmpty(false) {
  m_pData[0] = xx;
  m_pData[1] = yy;
...
CTensor::CTensor(const double &dPrincVal1, const double &dPrincVal2, const double &dPrincVal3,
                 const geo::IVector &xaxis, const geo::IVector &yaxis, const geo::IVector &zaxis)
    : m_bEmpty(true) {
  ...
  Set(rotated.Value(0, 0), rotated.Value(1, 1), rotated.Value(2, 2), rotated.Value(1, 0), rotated.Value(1, 2),
      rotated.Value(0, 2));
}
// ##ModelId=3BC6995B02D3
CTensor::~CTensor() {}

CTensor::CTensor(const CTensor &rhs) : ITensor(rhs), m_bEmpty(rhs.Empty()) {
  if (!rhs.Empty()) {
    for (int i = 0; i < 6; i++)
...
CTensor::CTensor(const ITensor &rhs) : m_bEmpty(rhs.Empty()) {
  if (!rhs.Empty()) {
    m_pData[0] = rhs.XX();
    m_pData[1] = rhs.YY();
    m_pData[2] = rhs.ZZ();
...
bool CTensor::Empty() const { return m_bEmpty; }

void CTensor::Invalidate() { m_bEmpty = true; }

// The and only set function
void CTensor::Set(const double &xx, const double &yy, const double &zz, const double &xy, const double &yz,
                  const double &xz) {
  m_bEmpty = false;
  m_pData[0] = xx;
  m_pData[1] = yy;
