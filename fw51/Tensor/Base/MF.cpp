...
// Shear capacity *******************************************************************************
geo::CValue MF::ShearCapacity(const CStressTensor &effective_stress, const double &friction_angle, const double &cohesion, ANGLE_TYPE angle_type)
{
  ...
  if(fabs(Qyield) < EPS) return geo::CValue();
...
geo::CValue MF::FaultNormalStress(const geo::IInterfaceElement &fault_elm, const geo::IVector &fault_stress)
{
  ...
  if(pFace) return geo::CValue(pFace->Normal().DotProduct(fault_stress));
...
geo::CValue MF::FaultShearCapacity(const geo::IVector &shear_stress, const double &effective_normal_stress, const double &friction_angle, const double &cohesion, ANGLE_TYPE angle_type)
{
  double friction = friction_angle;
...
double MF::CalcFluidStressGradient(const double &Depth, const double &OverPressure,
                   const double &FluidCompressibility, const double &FluidPressure)
{
