...
public:
  ...
  CTensor(const double &xx, const double &yy, const double &zz, const double &xy, const double &yz, const double &xz);
  ...
  CTensor(const double &dPrincVal1, const double &dPrincVal2, const double &dPrincVal3, const geo::IVector &xaxis,
          const geo::IVector &yaxis, const geo::IVector &zaxis);
...
inline std::ostream &operator<<(std::ostream &out, const CTensor &tensor) {
  out << '[' << tensor.XX() << ", " << tensor.YY() << ", " << tensor.ZZ() << ", " << tensor.XY() << ", " << tensor.YZ()
      << ", " << tensor.XZ() << ']';
  return out;
}
