...
class CGuiCommand : public IGuiCommand
{
public:
  ...
  CGuiCommand( const QString& caption,
               TARGET_TYPE& target,
               TExecMember exec,
               TCanExecMember canExec = 0,
               TIsChkMember isChecked = 0 );
  CGuiCommand( const QString& caption,
               TARGET_TYPE& target,
               TBoolExecMember exec,
               TCanExecMember canExec = 0,
               TIsChkMember isChecked = 0 );
...
template<class TARGET_TYPE>
CGuiCommand<TARGET_TYPE>::CGuiCommand( const QString& caption,
                                       TARGET_TYPE& target,
                                       TExecMember exec,
                                       TCanExecMember canExec,
                                       TIsChkMember isChecked )
: m_caption( caption ),
  m_target( &target ),
  m_execMember( exec ),
  m_boolExecMember( 0 ),
  m_canExecMember( canExec ),
...
template<class TARGET_TYPE>
QString CGuiCommand<TARGET_TYPE>::caption()
{
  return m_caption;
}

template<class TARGET_TYPE>
void CGuiCommand<TARGET_TYPE>::execute()
{
  if ( m_execMember ) (m_target->*m_execMember)();
  else if ( m_boolExecMember ) (m_target->*m_boolExecMember)();
}

template<class TARGET_TYPE>
bool CGuiCommand<TARGET_TYPE>::canExecute()
{
  if ( m_canExecMember ) return (m_target->*m_canExecMember)();
  return true;
}

template<class TARGET_TYPE>
bool CGuiCommand<TARGET_TYPE>::isChecked()
{
  if ( m_isCheckedMember ) return (m_target->*m_isCheckedMember)();
  return false;
}
