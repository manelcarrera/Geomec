...
CModelFileObs::CModelFileObs(IModelFile &modelFile, bool suppressStatus)
    : m_outStream(NULL), m_weOwnOutStream(false), m_errStream(NULL), m_weOwnErrStream(false),
...
*/
CModelFileObs::CModelFileObs(IModelFile &modelFile, QTextStream *outStream, QTextStream *errStream, bool suppressStatus)
    : m_outStream(outStream), m_weOwnOutStream(false), m_errStream(errStream), m_weOwnErrStream(false),
      m_suppressStatus(suppressStatus) {
  init(modelFile);
}
...
void CModelFileObs::init(IModelFile &modelFile) {
  ...
  connect(&modelFile, SIGNAL(statusMessage(const TFileMessage &)), this, SLOT(onStatusMessage(const TFileMessage &)));
  connect(&modelFile, SIGNAL(warningMessage(const TFileError &)), this, SLOT(onWarningMessage(const TFileError &)));
  connect(&modelFile, SIGNAL(fatalMessage(const TFileError &)), this, SLOT(onFatalMessage(const TFileError &)));
  connect(&modelFile, SIGNAL(abortMessage(const TFileError &)), this, SLOT(onAbortMessage(const TFileError &)));
}
...
void CModelFileObs::putMessage(QTextStream &os, const QString &severity, const TModelError &msg) {
  ...
  if (!msg.url.isEmpty())
    os << tr("Reference : ") << msg.url << endl;
  if (!msg.text.isEmpty())
    os << tr("Message   : ") << msg.text << endl;
}
