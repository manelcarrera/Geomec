...
COpenGLObjectObserver::CObserverDrawDef::CObserverDrawDef(COpenGLObjectObserver &observer) : m_observer(observer) {}

const COpenGLObjectObserver &COpenGLObjectObserver::CObserverDrawDef::Observer() const { return m_observer; }

COpenGLObjectObserver &COpenGLObjectObserver::CObserverDrawDef::Observer() { return m_observer; }

std::vector<COpenGLObjectObserver::CObserverDrawDef::TColor>
COpenGLObjectObserver::CObserverDrawDef::Color(const geo::IObject &object) const {
  // Pass function call
  return m_observer.onColor(object);
}

bool COpenGLObjectObserver::CObserverDrawDef::Visible() const { return TRUE; }

bool COpenGLObjectObserver::CObserverDrawDef::PolyFillFront() const { return m_observer.scene().fill(); }

bool COpenGLObjectObserver::CObserverDrawDef::PolyFillBack() const { return m_observer.scene().fill(); }

bool COpenGLObjectObserver::CObserverDrawDef::CurvedDraw() const { return m_observer.scene().curvedDraw(); }

bool COpenGLObjectObserver::CObserverDrawDef::LineStipple() const { return false; }
...
bool COpenGLObjectObserver::CObserverDrawDef::PolyDrawFront() const { return TRUE; }

bool COpenGLObjectObserver::CObserverDrawDef::PolyDrawBack() const { return TRUE; }

int COpenGLObjectObserver::CObserverDrawDef::DepthFunc() const { return GL_LESS; }

bool COpenGLObjectObserver::CObserverDrawDef::Value() const { return TRUE; }
...
const CModelObjectScene &COpenGLObjectObserver::scene() const { return m_scene; }

CModelObjectScene &COpenGLObjectObserver::scene() { return m_scene; }
...
int COpenGLObjectObserver::displayListSize() const { return observedObject().geometrySize(); }

const geo::IObject &COpenGLObjectObserver::displayList(int nIndex) const { return observedObject().geometry(nIndex); }

const IDrawDef &COpenGLObjectObserver::drawDef(int /*nIndex*/) const { return *m_pDrawDef; }
...
COpenGLObjectObserver::TObjectVec COpenGLObjectObserver::filterSelection(const COpenGLObjectObserver &observer,
...
bool COpenGLObjectObserver::mousePress(TKeyboardModifiers modifiers, TMouseButton button, const TScreenPoint &point,
...
bool COpenGLObjectObserver::mouseRelease(TKeyboardModifiers modifiers, TMouseButton button, const TScreenPoint &point,
                                         const TObjectVec &vcHit) {
...
bool COpenGLObjectObserver::mouseDblClk(TKeyboardModifiers modifiers, TMouseButton button, const TScreenPoint &point,
                                        const TObjectVec &vcHit) {
  if (vcHit.size()) {
    for (QObjectList::const_iterator it = children().begin(); it != children().end(); it++) {
      COpenGLObjectObserver *pObserver = dynamic_cast<COpenGLObjectObserver *>(*it);
...
bool COpenGLObjectObserver::mouseMove(TKeyboardModifiers modifiers, TMouseButton button, const TScreenPoint &point,
                                      const TObjectVec &vcHit) {
  if (vcHit.size()) {
    for (QObjectList::const_iterator it = children().begin(); it != children().end(); it++) {
      COpenGLObjectObserver *pObserver = dynamic_cast<COpenGLObjectObserver *>(*it);
