...
{
	while(m_observers.size() > 0) delete m_observers[0];
}
...
bool CModelObjectScene::curvedDraw() const
{
	return m_curvedDraw;
}
...
bool CModelObjectScene::fill() const
{
	return m_fill;
}

CModelObjectScene::TObjectVec CModelObjectScene::filterSelection(const COpenGLObjectObserver& observer, const TObjectVec& vcSelection) const
{
	TObjectVec vcHit;
...
std::vector<CModelObjectScene::TColor> CModelObjectScene::onColor(const COpenGLObjectObserver& /*node*/, const geo::IObject &/*object*/) const
{
