...
bool IMatrix::Empty() const { return ((ColumnSize() == 0) || (RowSize() == 0)); }
...
is of order three and bCreateRightHandSystem = true, a check is carried out if the vectors still span a right hand side
system. If not, the third vector has its sign switched.*/
{
  int i;
  int j;
  int k;
  ...
  for (iitr = 0; iitr < imitr; iitr++) // maximum of mitr iterations.
  {
    ...
    if (delmax < EPS2)
      break; // largest element of off-diagonal terms is small enough.
  ...
  if (RowSize() == 3 && bCreateRightHandSystem) // Check if eigenvector matrix still spans right hand side system,
                                                // change sign of third vector if not.
  {
    CVector v1, v2;
...
CMatrix IMatrix::EigenValues() const { return GetEigenPairs(false).first; }
...
CMatrix IMatrix::RotateMatStiffnessDeg(const IVector &rot_vector,
                                       const double &dAngle) { // Will return a Rotated 6x6 matrix. Any 6x6 matrix is
                                                               // accepted. Make sure you pass the right one.
...
CMatrix IMatrix::RotateMatStiffnessRad(const IVector &rot_vector,
                                       const double &dAngle) { // Will return a Rotated 6x6 matrix. Any 6x6 matrix is
                                                               // accepted. Make sure you pass the right one.
  assert(RowSize() == 6 && ColumnSize() == 6);
...
bool IMatrix::operator!=(const IMatrix &rhs) const { return !(this == &rhs); }
