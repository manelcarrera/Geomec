...
CMatrix::CMatrix() : m_rows(0), m_columns(0), m_pData(0) {}

// ##ModelId=3BC6995D0045
CMatrix::CMatrix(int rows, int columns) : m_rows(0), m_columns(0), m_pData(0) { ReSize(rows, columns); }

CMatrix::CMatrix(const IVector &v1, const IVector &v2, const IVector &v3) : m_rows(0), m_columns(0), m_pData(0) {
  ReAllocate(3, 3);
  for (int r = 0; r < 3; ++r) {
    Value(r, 0, v1[r]);
    Value(r, 1, v2[r]);
...
CMatrix::CMatrix(const IMatrix &rhs) : m_rows(0), m_columns(0), m_pData(0) {
...
CMatrix::CMatrix(const CMatrix &rhs) : IMatrix(rhs), m_rows(0), m_columns(0), m_pData(0) {
  ReAllocate(rhs.RowSize(), rhs.ColumnSize());
...
int CMatrix::RowSize() const { return m_rows; }

int CMatrix::ColumnSize() const { return m_columns; }
...
CMatrixPair::CMatrixPair(const CMatrixPair &rhs) : std::pair<CMatrix, CMatrix>(rhs) {}
