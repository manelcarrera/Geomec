...
//////////////////////////////////////////////////////////////////////
CBodyGroup::CSideSurface::CSideSurface(CBodyGroup &body_group) : m_body_group(body_group) {
...
const CBodyGroup &CBodyGroup::CSideSurface::BodyGroup() const { return m_body_group; }
...
int CBodyGroup::CSideSurface::PointSize() const { return (int)m_vcPoint.size(); }
...
int CBodyGroup::CSideSurface::FaceSize() const { return (int)m_vcFace.size(); }

void CBodyGroup::CSideSurface::ReplaceNode(const INode &oldNode, const INode &newNode) {
...
CBodyGroup::CBodyGroup(IMesh &mesh) : m_mesh(mesh) { mesh.AddElementGroup(*this); }
...
void CBodyGroup::reserveBodySize(size_t size) { m_vcElement.reserve(size); }
...
const IMesh &CBodyGroup::Mesh() const { return m_mesh; }

IMesh &CBodyGroup::Mesh() { return m_mesh; }
...
int CBodyGroup::BodySize() const { return (int)m_vcElement.size(); }

const IPoint &CBodyGroup::Point(int nIndex) const { return m_mesh.Point(m_vcNode[nIndex]); }

int CBodyGroup::PointSize() const { return (int)m_vcNode.size(); }
...
std::vector<int> CBodyGroup::Nodes(const IElement & /*element*/) const { return std::vector<int>(); }
...
int CBodyGroup::SideSurfaceSize() const { return (int)m_vcSurface.size(); }

const CBodyGroup::CSideSurface &CBodyGroup::SideSurface(int nIndex) const { return *m_vcSurface[nIndex]; }

CBodyGroup::CSideSurface &CBodyGroup::SideSurface(int nIndex) { return *m_vcSurface[nIndex]; }
  ...
  if (it != m_mpPoint.end()) {
    ...
    if (itv != vcBody.end()) {
      ...
      if (vcBody.empty()) {
        ...
        m_mpPoint.erase(it);
        std::vector<int>::iterator itvn = std::find(m_vcNode.begin(), m_vcNode.end(), oldNode.Index());
        if (itvn != m_vcNode.end())
          m_vcNode.erase(itvn);
      }
    }

    std::pair<TPointMap::iterator, bool> prInsert = m_mpPoint.insert(TPointMap::value_type(&newNode, TBodyVec()));
