...
CMeshNodeGroup::CMeshNodeGroup(IMesh &mesh) : m_mesh(mesh), m_nIndex(m_mesh.NodeGroupSize()) {
  m_mesh.AddNodeGroup(*this);
}

CMeshNodeGroup::~CMeshNodeGroup() { m_mesh.RemoveNodeGroup(*this); }

int CMeshNodeGroup::Index() const { return m_nIndex; }
...
int CMeshNodeGroup::NodeSize() const { return (int)m_vcNode.size(); }
...
bool CMeshNodeGroup::Empty() const { return (NodeSize() == 0); }
