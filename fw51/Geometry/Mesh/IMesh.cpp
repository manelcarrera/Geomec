...
int IMesh::PointSize() const
{
  return NodeSize();
}

const IPoint& IMesh::Point(int nIndex) const
{
  return Node(nIndex);
}

// Tyings in Diana
int IMesh::TyingSize() const
{
  return (int)m_vcTying.size();
}

const CTying& IMesh::Tying(int nTyingIndex) const
{
  return *m_vcTying[nTyingIndex];
}

// Supports in Diana
int IMesh::SupportSize() const
{
  return (int)m_vcSupport.size();
}
...
bool IMesh::HasTranslationSupport(int nNodeNr) const // see if the specified node index is supported
{
  ...
  if(it == m_SupportMap.end()) return false;
...
{
  CTying *pTying = new CTying(*this, nSlave, vecSlaveDir, nSlaveType,
                nMaster0, vecMaster0Dir, nMaster0Type, dFactor0,
...
{
  for(int i = 0; i < m_vcTying.size(); i++) delete m_vcTying[i];
  m_vcTying.clear();
}
...
int IMesh::ElementGroupSize() const
{
  return (int)m_vcElementGroup.size();
}

const IElementSet& IMesh::ElementGroup(int nElementGroupIndex) const
{
  return *m_vcElementGroup[nElementGroupIndex];
}

  // Node grouping in Diana.
int IMesh::NodeGroupSize() const
{ 
  return (int)m_vcMeshNodeGroup.size();
}
...
void IMesh::AddElementGroup(IElementSet &elementset)
{
  m_vcElementGroup.push_back(&elementset);
}
...
void IMesh::AddNodeGroup(CMeshNodeGroup &nodegroup)
{
  m_vcMeshNodeGroup.push_back(&nodegroup);
}
...
void IMesh::GetElementTypeRanges(std::vector<std::pair<int, int> >& bodies, std::vector<std::pair<int, int> >& faults, int *pMaxBodyNodes, int *pMaxFaultNodes) const
{
  if (ElementSize() == 0)
  return;
