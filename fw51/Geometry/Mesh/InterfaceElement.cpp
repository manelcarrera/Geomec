...
CInterfaceElement::CInterfaceElement(IMesh &mesh, const IFace *front, const IFace *back,
                                     const std::vector<int> &vcPoint)
    : m_front(front), m_back(back), m_nFrontStartIndex(-1), m_nBackStartIndex(-1), m_vcNode(vcPoint), m_mesh(mesh) {
...
const IElementSet *CInterfaceElement::IndexingElementSet() const { return &m_mesh; }
...
const IFace *CInterfaceElement::FrontFace() const { return m_front; }

const IFace *CInterfaceElement::BackFace() const { return m_back; }

int CInterfaceElement::NrOfPoints() const { return NrOfNodes(); }
...
void CInterfaceElement::Point(int /*nIndex*/, const IPoint & /*pt*/) { assert(false); }

size_t CInterfaceElement::NrOfNodes() const { return (int)m_vcNode.size(); }
...
void CInterfaceElement::Node(int /*nIndex*/, const IPoint & /*pt*/) { assert(false); }

int CInterfaceElement::Index() const { return m_nIndex; }
...
int CInterfaceElement::FrontStartIndex() const { return m_nFrontStartIndex; }

int CInterfaceElement::BackStartIndex() const { return m_nBackStartIndex; }
...
int CInterfaceElement::BodyNode(int nIndex) const {
  ...
  if (pQuad)
    return pQuad->BodyNode(nFaceNode);
  if (pTri)
    return pTri->BodyNode(nFaceNode);
