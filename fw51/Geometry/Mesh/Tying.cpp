...
CTying::CTying(IMesh &mesh, int nSlave, const IVector &vecSlaveDir, TType nSlaveType, int nMaster,
...
CTying::CTying(IMesh &mesh, int nSlave, const IVector &vecSlaveDir, TType nSlaveType, int nMaster0,
               const IVector &vecMaster0Dir, TType nMaster0Type, double dFactor0, int nMaster1,
               const IVector &vecMaster1Dir, TType nMaster1Type, double dFactor1)
    : m_mesh(mesh), m_nSlaveNode(nSlave), m_pSlaveDirection(0), m_SlaveType(nSlaveType) {
  m_pSlaveDirection = new CVector(vecSlaveDir);
...
CTying::~CTying() {
  for (int i = 0; i < m_vcMaster.size(); i++)
    delete m_vcMaster[i].first;
}

const IMesh &CTying::Mesh() const { return m_mesh; }

int CTying::SlaveIndex() const { return m_nSlaveNode; }
...
int CTying::MasterSize() const { return (int)m_vcMaster.size(); }
...
CTying::TType CTying::SlaveType() const { return m_SlaveType; }

std::string CTying::SlaveTypeString() const { return CNodeDef::Type2TypeName(m_SlaveType); }
