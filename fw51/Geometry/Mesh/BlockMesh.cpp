...
// Point creation ************************************************************************
  ...
  for(i = 0; i < m_height_division + 1; i++)
  {
    for(j = 0; j < m_width_division + 1; j++)
    {
      ...
      {
        nPtIndex = i * NrOfPointsX * NrOfPointsY +
               j * NrOfPointsX +
               k;

        CPoint *pPoint = new CPoint(m_dXbase + (double)k * m_spacing_x,
                      m_dYbase + (double)j * m_spacing_y,
...
void CBlockMesh::SetMesh(const IPoint &minpoint,
               const double &length, 
               const double &width, 
               const double &height, 
               int length_division,
...
{
  if(fabs(m_dXbase - rhs.XBase()) > compareTolerance()) return false;
  if(fabs(m_dYbase - rhs.YBase()) > compareTolerance()) return false;
  if(fabs(m_dZbase - rhs.ZBase()) > compareTolerance()) return false;

  if(fabs(m_spacing_x - rhs.XSpace()) > compareTolerance()) return false;
  if(fabs(m_spacing_y - rhs.YSpace()) > compareTolerance()) return false;
  if(fabs(m_spacing_z - rhs.ZSpace()) > compareTolerance()) return false;
