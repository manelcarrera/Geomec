...
double vect_len2(Vector v) { return sqrt(v[0] * v[0] + v[1] * v[1]); }
...
double vect_det2(Vector v1, Vector v2) { return ((v1[0] * v2[1]) - (v2[0] * v1[1])); }
...
static void RecursiveIsect(Vector p1, Vector p2, Vector q1, Vector q2, Vector ret) {
  ...
  while (!vect_equal(m, s1) && !vect_equal(m, s2)) {
    /* assert(vect_det2(q, s1) <= 0); */
    /* assert(vect_det2(q, s2) >= 0); */

    if (vect_det2(q, m) <= 0)
      Vcpy2(s1, m) else Vcpy2(s2, m)
...
int vect_inters2(Vector p1, Vector p2, Vector q1, Vector q2, Vector S1, Vector S2) {
  ...
  if (max(p1[0], p2[0]) < min(q1[0], q2[0]) || max(q1[0], q2[0]) < min(p1[0], p2[0]))
    return 0;
  ...
  if (fabs(deel) < EPSILON) /* parallel */
  {
    ...
    if (Vequ2(q1, p1) || Vequ2(q1, p2))
      Vcpy2(S1, q1) else if (Vequ2(q2, p1) || Vequ2(q2, p2)) Vcpy2(S1, q2) else {
    ...
    if (fabs(inp3s) < EPSILON)
      inp3s = 0;

    if (fabs(inp4s) < EPSILON)
      inp4s = 0;
