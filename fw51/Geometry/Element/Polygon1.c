...
node_add
 ...
 (C) 1997 Alexey Nikitin, Michael Leonov
*/
static VNODE *node_add(VNODE *dest, Vector po, int *new_point) {
  ...
  for (p = (last = dest)->next; d > vect_dist2(dest->point, p->point); p = (last = p)->next)
    ;
...
static int node_add_point(VNODE *a, VNODE *b, Vector p) {
  int res = 0;
...
static int intersect(PLINE *a, PLINE *b) {
  VNODE *a_iter, *b_iter;
  ...
  do {
    ...
    do {
      cnt =
          vect_inters2(a_iter->point, a_iter->next->point, b_iter->point, b_iter->next->point, intersect1, intersect2);
      res += cnt;
      if (cnt > 0) {
...
static void M_POLYAREA_intersect(jmp_buf *e, POLYAREA *afst, POLYAREA *bfst) {
...
static int cntr_in_M_POLYAREA(PLINE *poly, POLYAREA *outfst) {
  PLINE *curc;
...
static unsigned int node_label(Vector p2, VNODE *pn) {
...
static void label_contour(PLINE *a) {
...
static void cntr_label_POLYAREA(PLINE *poly, POLYAREA *ppl) {
...
static void M_POLYAREA_label(POLYAREA *afst, POLYAREA *b) {
  POLYAREA *a = afst;
  PLINE *curc;
...
static void PutContour(jmp_buf *e, PLINE *cntr, POLYAREA **contours, PLINE **holes) {
  PLINE *cur;
    ...
    *src = curh->next;
    ...
    do {
      ...
      if (poly_ContourInContour(curc->contours, curh) &&
          (container == NULL || poly_ContourInContour(container->contours, curc->contours)))
        container = curc;
...
typedef enum { FORW, BACKW } DIRECTION;
...
static int UniteS_Rule(VNODE *cur, DIRECTION *initdir) {
  *initdir = FORW;
  return (PLF_ESTATE(cur) == OUTSIDE) || (PLF_ESTATE(cur) == SHARED && cur->next->t.link == cur->t.link->next);
}

static int IsectS_Rule(VNODE *cur, DIRECTION *initdir) {
  *initdir = FORW;
  return (PLF_ESTATE(cur) == INSIDE) || (PLF_ESTATE(cur) == SHARED && cur->next->t.link == cur->t.link->next);
}

static int SubS_Rule(VNODE *cur, DIRECTION *initdir) {
  *initdir = FORW;
  return (PLF_ESTATE(cur) == OUTSIDE) || (PLF_ESTATE(cur) == SHARED && cur->next->t.link == cur->t.link->prev);
}

static int SubSlaveS_Rule(VNODE *cur, DIRECTION *initdir) {
  *initdir = BACKW;
  return (PLF_ESTATE(cur) == INSIDE);
}

static int IsectJ_Rule(VNODE *cur, DIRECTION *cdir) {
  assert(*cdir == FORW);
  return (PLF_ESTATE(cur->t.link) == INSIDE);
}

static int UniteJ_Rule(VNODE *cur, DIRECTION *cdir) {
  assert(*cdir == FORW);
  return (PLF_ESTATE(cur) != OUTSIDE && PLF_ESTATE(cur->t.link) == OUTSIDE);
}

static int SubJ_Rule(VNODE *cur, DIRECTION *cdir) {
  if (*cdir == FORW) {
    if (PLF_ESTATE(cur->t.link->prev) == INSIDE) {
      *cdir = BACKW;
      return TRUE;
    }
  } else if (PLF_ESTATE(cur->t.link) == OUTSIDE || PLF_ESTATE(cur->t.link) == SHARED) {
    *cdir = FORW;
    return TRUE;
  }
  return FALSE;
...
static int Gather(VNODE *start, PLINE **result, J_Rule j_rule, DIRECTION initdir) {
...
static void Collect(jmp_buf *e, PLINE *a, POLYAREA **contours, PLINE **holes, S_Rule c_rule, J_Rule j_rule) {
  VNODE *cur;
  PLINE *p = NULL;
  int errc = err_ok;
  DIRECTION dir;
...
static void cntr_Collect(jmp_buf *e, PLINE *A, POLYAREA **contours, PLINE **holes, int action) {
  PLINE *tmprev;
...
static void M_POLYAREA_Collect(jmp_buf *e, POLYAREA *afst, POLYAREA **contours, PLINE **holes, int action) {
  POLYAREA *a = afst;
  PLINE *cur;
...
static void M_InitPolygon(POLYAREA *afst) {
  PLINE *curc;
  VNODE *curn;
  POLYAREA *a = afst;
