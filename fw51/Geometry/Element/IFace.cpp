...
// Lock for IFace's line cache. We lock on (this >> 6 & 0xf) to spread the mutexes a bit, without giving a mutex to each
// IFace. The latter is not only to reduce space, but also because mutexes can't be copied, and we have many derived
// classes, which would need to call the currently non-existent IFace copy constructor... more trouble than it's worth.
...
IFace::CFaceLine::CFaceLine(const IFace &parent, int nIndex) : m_nIndex(nIndex), m_parent(parent) {}

size_t IFace::CFaceLine::NrOfNodes() const { return m_parent.NrOfLineNodes(); }
...
int IFace::CFaceLine::NrOfPoints() const { return m_parent.NrOfLinePoints(); }
...
size_t IFace::CFaceLine::Order() const { return m_parent.Order(); }
...
const IElementSet *IFace::CFaceLine::IndexingElementSet() const { return 0; }
...
int IFace::NrOfLines() const { return NrOfPoints(); }
...
size_t IFace::NrOfLinePoints() const { return Order() + 1; }
...
CVector IFace::NormalInPoint(const geo::IPoint & /*pt*/) const { return Normal(); }

void IFace::AssertValid() const { assert(NrOfPoints() > 2); }
...
double IFace::GetPointArrInPlane(const IPlane &plane, CArray<CPoint> &arr, IMatrix *pTM /*=NULL */,
...
bool IFace::dblCmpLt(const double &par1, const double &par2) const { return ((par1 - par2) > EPS); }
...
bool IFace::dblCmpGt(const double &par1, const double &par2) const { return ((par2 - par1) > EPS); }
...
bool IFace::Contains(const IPoint &point, bool bIncludeEdge) const {
  if (fabs(SignedDistance(point, true)) > EPS)
    return false;
  ...
  for (i = 0; i < NrOfPoints(); i++) {
    ...
    if (normal.isNullVector())
      continue; // No contribution to angle

    if (faceNormal.Empty())
      faceNormal = normal; // To compare if one plane
...
void IFace::InvalidateCache() { DestroyLineCache(); }
