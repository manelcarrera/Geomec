...
{
	if (Point1.Empty()) return false;
	if (Point2.Empty()) return false;
	...
	//directions should be perpendicular
	if (! dir1.IsPerpendicular(dir2)) return false;
	return true;
}
...
const IPoint& CRectangle::Refpoint() const
{
	return m_RefPoint;
}

const IVector& CRectangle::Direction1() const
{
	return m_vcDirection1;
}

const IVector& CRectangle::Direction2() const
{
	return m_vcDirection2;
}
	
const IPoint &CRectangle::Point(int nIndex) const
{
	return Node(nIndex);
}

void CRectangle::Point(int nIndex, const IPoint& point)
{
	Node(nIndex, point);
}

size_t CRectangle::Order() const
{
	return 1;
}
...
size_t CRectangle::NrOfNodes() const
{
	return 4;
}
...
void CRectangle::Node(int /*nIndex*/, const IPoint& /*point*/)
{
	assert(false);
}
