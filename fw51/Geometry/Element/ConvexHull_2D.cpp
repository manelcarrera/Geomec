...
*/
tPointStructure::tPointStructure()
: vnum(-1), bDelete(false), m_coor_disabled(0)
{
}
...
{
  if( m_coor_disabled == 0 ) return p.Z();
  return p.X();
}

const double &tPointStructure::Y() const
{
  if( m_coor_disabled == 1 ) return p.Z();
  return p.Y();
}
...
long tPointStructure::SaveProgressSize() const
{
  return 0;
}
...
CConvexHull_2D::CConvexHull_2D(const CPtrArray<IPoint> &points, int coor_disabled)
: m_size(points.Size())
{
  m_aPoint = new tPointStructure[m_size];
  for(int i = 0; i < m_size; i++) {
...
long CConvexHull_2D::SaveProgressSize() const
{
  return 1;
}
...
void CConvexHull_2D::BuildConvexHull()
{
  ...
  qsort(
     &m_aPoint[1],             /* pointer to 1st elem */
