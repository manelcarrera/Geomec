...
CPolygon::CPolygon(const CPtrArray<CPoint> &points, bool bIsHole) : IPolygon(bIsHole) {
...
CPolygon::CPolygon(const CArray<CPoint> &points, bool bIsHole) : IPolygon(bIsHole) {
...
CPolygon::CPolygon(const IFace &face, bool bIsHole) : IPolygon(bIsHole) {
  CPoint p;
...
CPolygon::CPolygon(bool bIsHole) : IPolygon(bIsHole) {}
...
CPolygon::CPolygon(const CPolygon &rhs) : IPolygon(rhs.IsHole()), m_points(rhs.m_points) {}

CPolygon::CPolygon(const IPolyLine &poly_line, bool bIsHole) : IPolygon(bIsHole) {
  CPoint p;
...
size_t CPolygon::Order() const { return 1; }

void CPolygon::Mirror(const geo::IPlane &plane) { m_points.Mirror(plane); }
