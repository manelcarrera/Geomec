...
b. We also removed the hull increment code from VolumeSign, as it is not used during hull creation (we calculate normals
afterwards) c. We create the vertex data structures all at once in the beginning, and keep track of vertices processed,
on hull, etc, in situ
...
3. We first process 16 points as the original did, to accommodate regular shapes (for example, cube, as in the
ConvexHull3D unit test) Walk over the boxes, and process those points Do a parallel yes/no check to determine the points
still outside the box Process the remaining points
...
IConvexHullImpl *createConvexHullImpl(const CConvexHull &parent) { return new CConvexHullImpl(parent); }
...
tVertexStructure::tVertexStructure()
    : pDuplicate(0), bOnhull(!ONHULL), bProcessed(false), bDeleted(false), nNext(-1), nPrev(-1) {}
...
bool tVertexStructure::operator<(const tVertexStructure &rhs) const { return dWeight > rhs.dWeight; }
...
CConvexHullImpl::CConvexHullImpl(const CConvexHull &parent)
    : IConvexHullImpl(parent), m_tEdges(0), m_tFaces(0), nVertexRoot(-1), nVertexCount(0), m_parent(parent) {}

CConvexHullImpl::~CConvexHullImpl() { ClearAll(); }
...
void CConvexHullImpl::SetPoints(IProgressBase *progress, const std::vector<const IPoint *> &points) {
  ...
  CPoint centre((xmin + xmax) / 2., (ymin + ymax) / 2., (zmin + zmax) / 2.);
  ...
  m_maxDist = absXmin;
  if (m_maxDist < absYmin)
    m_maxDist = absYmin;
  if (m_maxDist < absZmin)
    m_maxDist = absZmin;
  if (m_maxDist < absXmax)
    m_maxDist = absXmax;
  if (m_maxDist < absYmax)
    m_maxDist = absYmax;
  if (m_maxDist < absZmax)
    m_maxDist = absZmax;
  ...
  if (m_vcVertices.size() <= 500) {
    ...
    if (progress)
      progress->AddSteps(m_box.m_vcBoxes.size());
    ...
    for (int i = 0; i < m_vcVertices.size(); ++i) {
      ...
      if (i > 0 &&
          m_vcVertices[i].Point == m_vcVertices[i - 1].Point) // skip (exact) duplicates; TBD if we can use epsilon
        m_vcVertices[i].bProcessed = true;
 ...
 */
void CConvexHullImpl::SetValidDigits(double digits) { m_validDigits = digits; }

double CConvexHullImpl::GetValidDigits() const { return m_validDigits; }

CVector CConvexHullImpl::MidPoint() const { return m_centrePoint; }

void CConvexHullImpl::GetPointsAndFaces(std::vector<CPoint> &points,
                                        std::vector<CConvexHull::CConvexHullFace> &faces) const {
  ...
  i = 0;
  for (tFace f = m_tFaces; f != NULL; f = f->pNext) {
    faces[i].Set(m_parent, FindPointIndex(points, f->vertex[0]->Point), FindPointIndex(points, f->vertex[1]->Point),
                 FindPointIndex(points, f->vertex[2]->Point), f->normal.X(), f->normal.Y(), f->normal.Z());
    ++i;
  }
}
...
bool CConvexHullImpl::CheckValidDigits(const double &testValue, double &digits) {
  ...
  double step = 1;
  double diff = 0;
  const double eps = pow(10.0, (int)log10(testValue / pow(2.0, 52))); // IEEE-754 -> 52
  double directPow3, indirectPow3;
  double curValue;

  do {
    // sample values around the testValue
    for (curValue = 0.9 * testValue; curValue <= 1.1 * testValue; curValue += 0.01 * testValue) {
...
public:
  CTaskFilterVertices(const CConvexHullImpl &parent, std::vector<tsVertex> &data, const tFace faces)
      : m_parent(parent), m_data(data), m_tFaces(faces), m_dispatcher(0) {}
...
void CConvexHullImpl::ConstructHull(IProgressBase *progress) {
  ...
  if (size > 500) {
    ...
    if (progress) {
      ...
      mp::IDispatchedTask *disTaskFilterVertices =
          NEW_DISPATCH_TASK(mp::CKernelParallel, CTaskFilterVertices)(parKernel, taskFilterVertices);
...
tFace CConvexHullImpl::MakeConeFace(tEdge e, tVertex p) throw(const char *) {
  ...
  for (i = 0; i < 2; ++i) {
    ...
    if (!new_edge[i]->adjface[0])
      new_edge[i]->adjface[0] = new_face;
    else if (!new_edge[i]->adjface[1])
      new_edge[i]->adjface[1] = new_face;
    else
      throw((const char *)("Convex Hull Failed (MakeConeFace)"));
  }
...
//---------------------------------------------------------------------
bool CConvexHullImpl::Collinear(const IPoint &a, const IPoint &b, const IPoint &c) {
