...
bool IBody::Intersects(const IPlane &plane, bool bIncludeEdge /* = true */) const {
  ...
  for (i = 0; i < NrOfFaces(); i++) {
    for (j = 0; j < Face(i).NrOfPoints(); j++) {
      ...
      if (fabs(distance) < EPS) { // point is on plane, plane intersects..
        return bIncludeEdge;
      }

      if (i == 0 && j == 0) { // first point, set the side...
        side = (distance > 0);
      } else { // set the bool with which side will be compared...
        bool comp = (distance > 0);

        if (comp != side) { // point is on other side of plane, plane intersects..
          return true;
        }
      }
    }
...
CPolygon IBody::IntersectionConv(const IPlane &plane) const {
  ...
  if (Intersects(plane, true)) {
    ...
    if (line.Valid()) {
      CPolygon ret;
      double cosalpha = plane.Normal().UnitVector().DotProduct(line.Normal().UnitVector());
      if (cosalpha > 0) {
...
void IBody::Intersection(const IPlane &plane, std::set<CLine, ILine::CLineLess> &LineSet,
                         std::set<CPoint> *PointSet /* = 0 */) const {
  if (Intersects(plane, true)) {
    for (int i = 0; i < NrOfFaces(); i++) {
      IFace::TLineVec vcLine = Face(i).Intersection(plane);
      for (int j = 0; j < vcLine.size(); j++) {
...
bool IBody::Contains(const IPoint &point, bool bIncludeEdge) const {
  ...
  for (int i = 0; i < NrOfFaces(); i++) {
    if (Face(i).Contains(point, true))
      return bIncludeEdge;
    if (Face(i).SignedDistance(point, false) > 0)
      return false;
  }
  return true;
}
...
static polylib::Polyhedron *GetPolyhedron(const IBody &body, const geo::IPoint &ptOffset) {
  ...
  int i;
  for (i = 0; i < body.NrOfPoints(); i++)
    vcPoints.push_back(&body.Point(i));
...
IBody::TIntersection IBody::Intersection(const IBody &rhs) const {
  ...
  if (rhs.Min().X() > Max().X() || rhs.Min().Y() > Max().Y() || rhs.Min().Z() > Max().Z() ||
      rhs.Max().X() < Min().X() || rhs.Max().Y() < Min().Y() || rhs.Max().Z() < Min().Z()) {
    // not intersecting
    ret.first = 0.0;
    return ret;
  }
...
const char *IBody::SideName(int nIndex) const { return FaceName(nIndex); }

bool IBody::IsBody() const { return true; }
