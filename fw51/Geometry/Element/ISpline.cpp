...
*/
ISpline::CSplineFace::CSplineFace(const ISpline& parent, int nIndex)
...
*/
const ISpline& ISpline::CSplineFace::Spline() const
{
  return m_parent;
}
...
*/
int ISpline::CSplineFace::Index() const
{
  return m_nIndex;
}

size_t ISpline::CSplineFace::NrOfNodes() const
{
  return 4;
}

const INode &ISpline::CSplineFace::Node(int nIndex) const
{
  return Spline().Node(ParentNodeIndex(nIndex));
}
...
int ISpline::CSplineFace::ParentNodeIndex(int nIndex) const
{
  ...
  // bottom side
  if(nIndex < 2) return Index() + nIndex;
...
*/
void ISpline::CSplineFace::Node(int /*nIndex*/, const IPoint &/*pt*/)
{
  assert(false);
}

size_t ISpline::CSplineFace::Order() const
{
  return 1;
}
...
*/
ISpline::CSplineLine::CSplineLine(const ISpline& parent, int nFirst, int nSecond)
: m_parent(parent)
{
  m_Nodes[0] = nFirst;
  m_Nodes[1] = nSecond;
...
/*!
  Returns the parent ISpline object.
*/
const ISpline& ISpline::CSplineLine::localParent() const
{
  return m_parent;
}

size_t ISpline::CSplineLine::NrOfNodes() const
{
  return 2;
}

const INode& ISpline::CSplineLine::Node(int nIndex) const
{
  return localParent().Node(ParentNodeIndex(nIndex));
}

/*!
  Modification of nodes is not allowed, this should be done on the parent ISpline object. This function asserts false.
*/
void ISpline::CSplineLine::Node(int /*nIdex*/, const IPoint& /*pt*/)
{
  assert(false);
}

size_t ISpline::CSplineLine::Order() const
{
  return 1;
}
...
/*!
  ...
  The ISpline class is the general base for all spline based elements in DIANA. The elements are planar and rectangular, and can contain a arbitrary number of faces along its length (i.e. in the element's x direction).

  Spline elements have no unique concept of isoparametric coordinates. Each rectangular field has its own isoparametric axis system. Therefore, the functions based on isoparametric coordinates can not be used. All these functions implemented in IElement are overridden in ISpline.
*/
ISpline::ISpline()
{
}
...
{
  int i;
  for(i = 0; i < m_vcFaces.size(); i++) delete m_vcFaces[i];
  m_vcFaces.clear();
}
...
{
  int i;
  for(i = 0; i < m_vcLines.size(); i++) delete m_vcLines[i];
  m_vcLines.clear();
}

int ISpline::NrOfPoints() const
{
  return NrOfNodes();
}
...
void ISpline::BuildFaceCache() const
{
  ...
  int i;
  int nf = NrOfFaces();
  for(i = 0; i < nf; i++) m_vcFaces.push_back(new CSplineFace(*this, i));
}

const IQuadrilateral& ISpline::Face(int nIndex) const
{
  assert(nIndex >= 0 && nIndex < NrOfFaces());
  if(m_vcFaces.empty()) BuildFaceCache();
...
int ISpline::NrOfLines() const
{
  return 3 * NrOfFaces() + 1;
}
...
const ILine &ISpline::Line(int nIndex) const
{
  ...
  if(m_vcLines.empty()) BuildLineCache();
...
CValue ISpline::InterpolateValue(const IPoint& /*point*/, const std::vector<CValue>& /*values*/, int /*iField*/) const
{
...
double ISpline::RepresentativeLength() const
{
  return Node(0).Distance(Node(NrOfNodes() - 1));
}
