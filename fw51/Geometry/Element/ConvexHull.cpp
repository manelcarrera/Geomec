...
CConvexHull::CConvexHull(const std::vector<const IPoint *> &points)
    : m_dHullIncrement(0.0), m_centrePoint(0.0, 0.0, 0.0), m_validDigits(0.0), // undetermined
      m_invalid(false), m_vcInputPoints(points) {}

CConvexHull::CConvexHull(CPtrArray<IPoint> &points)
    : m_dHullIncrement(0.0), m_centrePoint(0.0, 0.0, 0.0), m_validDigits(0.0), // undetermined
      m_invalid(false), m_vcInputPoints(points.Size()) {
 ...
 */
void CConvexHull::SetValidDigits(const double &digits /*=-1*/) { m_validDigits = digits; }

CPoint CConvexHull::GlobalPoint(int nIndex) const {
  return m_vcHullPoints[nIndex] + m_centrePoint; // undo translation
}

CElementPoint CConvexHull::MidPoint() const { return m_centrePoint; }

long CConvexHull::SaveProgressSize() const { return m_vcHullPoints.size() + m_vcHullFaces.size(); }
...
int CConvexHull::NrOfPoints() const { return (int)m_vcHullPoints.size(); }
...
int CConvexHull::NrOfFaces() const { return (int)m_vcHullFaces.size(); }

const CConvexHull::TIndexVec &CConvexHull::FacePointIndices(int nIndex) const throw(const char *) {
  const CConvexHullFace *pFace = dynamic_cast<const CConvexHullFace *>(&Face(nIndex));
  if (!pFace)
    throw((const char *)("Convex Hull Failed (FacePointIndices)"));
  return pFace->PointIndices();
}

const CConvexHull::TIndexVec &CConvexHull::FaceNodeIndices(int nIndex) const throw(const char *) {
  if (!(Order() == 1))
    throw((const char *)("Convex Hull Failed (FaceNodeIndices)"));
...
size_t CConvexHull::Order() const { return 1; }
...
bool CConvexHull::Contains(const IPoint &point, bool bIncludeEdge) const {
  ...
  if (ret && NrOfZeroVolume > 0 &&
      NrOfZeroVolume ==
          m_vcHullFaces
              .size()) // See Bug 124671; the pointsets there have #0vol == #faces, but we may fail to catch all errors
  {
    if (m_min.Empty()) {
      ...
      if (m_dHullIncrement > 0) {
        ...
        // let's not quibble about precision errors and extend the CH with 1% in each direction (due to the nature of
        // the bug, we expect that this doesn't really matter)
    ...
    if (p.X() < m_min.X() || p.Y() < m_min.Y() || p.Z() < m_min.Z() || p.X() > m_max.X() || p.Y() > m_max.Y() ||
        p.Z() > m_max.Z()) {
      if (!m_invalid) {
        m_invalid = true;
        throw((const char *)("Convex Hull is not consistent and unsuitable to determine containment of points. Using "
...
// #############################################################################
CConvexHull::CConvexHullFace::CConvexHullFace() : m_vcPoint(3), m_parent(0) {}
...
void CConvexHull::CConvexHullFace::Set(const CConvexHull &Parent, int n1, int n2, int n3, double x, double y,
                                       double z) {
  m_parent = &Parent;
  m_vcPoint[0] = n1;
  m_vcPoint[1] = n2;
...
CVector CConvexHull::CConvexHullFace::Normal() const { return m_normal; }

const CConvexHull::TIndexVec &CConvexHull::CConvexHullFace::PointIndices() const { return m_vcPoint; }
...
void CConvexHull::CConvexHullFace::Point(int /*nIndex*/, const IPoint & /*pt*/) { assert(false); }

int CConvexHull::CConvexHullFace::NrOfPoints() const { return 3; }

size_t CConvexHull::CConvexHullFace::Order() const { return 1; }
