...
void IQuadrilateral::BuildIntegrationPoints(IElement::TIntPtVec &vec, int numint) {
  int ixi, ieta;
  double xxi, xeta;
  double wxi, weta;
...
const IElement::TIntPtVec &IQuadrilateral::IntegrationPoints() const { return IntegrationPoints(Order()); }
...
double IQuadrilateral::GetDepthAt(const double &x, const double &y) const {
  ...
  if ((Order() == 1) && (Point(0).X() == Point(3).X() || Point(0).X() == Point(1).X()) &&
    ...
    if (Point(0).X() == Point(3).X()) {
      ...
      isocoords[0] = 2.0 * (x - Point(0).X()) / (Point(1).X() - Point(0).X()) - 1.0;
      isocoords[1] = 2.0 * (y - Point(0).Y()) / (Point(3).Y() - Point(0).Y()) - 1.0;
    } else // Point(0).X() == Point(1).X()
    {
      assert(x >= Point(1).X() && x <= Point(2).X());
      isocoords[0] = 2.0 * (y - Point(0).Y()) / (Point(1).Y() - Point(0).Y()) - 1.0;
      isocoords[1] = 2.0 * (x - Point(1).X()) / (Point(2).X() - Point(1).X()) - 1.0;
    }
  } else {
...
int IQuadrilateral::IntegrationPointSize() const { return (int)IntegrationPoints().size(); }
