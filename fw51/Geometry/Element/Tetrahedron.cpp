...
CTetrahedron::CTetrahedron(IMesh &mesh, const std::vector<const IPoint *> &points)
  ...
  assert(Contains(CPoint((Point(0).X() + Point(1).X() + Point(2).X() + Point(3).X()) / 4,
                         ...
                         (Point(0).Z() + Point(1).Z() + Point(2).Z() + Point(3).Z()) / 4),
                  true));
}

CTetrahedron::CTetrahedron(IMesh &mesh, const IPoint &p0, const IPoint &p1, const IPoint &p2, const IPoint &p3,
                           bool bUnique)
...
CTetrahedron::CTetrahedron(IMesh &mesh, const std::vector<int> &points) : m_vcNode(points), m_mesh(mesh) {
  // Register element
  m_nIndex = m_mesh.RegisterElement(*this);
...
CTetrahedron::CTetrahedron(IMesh &mesh, int p0, int p1, int p2, int p3) : m_vcNode(4), m_mesh(mesh) {
  m_vcNode[0] = p0;
  m_vcNode[1] = p1;
  m_vcNode[2] = p2;
  m_vcNode[3] = p3;
...
const IElementSet *CTetrahedron::IndexingElementSet() const { return &m_mesh; }

int CTetrahedron::Index() const { return m_nIndex; }

int CTetrahedron::PointIndex(int nIndex) const { return m_vcNode[nIndex]; }

const geo::IPoint &CTetrahedron::Point(int nIndex) const { return m_mesh.Point(m_vcNode[nIndex]); }
...
size_t CTetrahedron::NrOfNodes() const { return 4; }
...
size_t CTetrahedron::Order() const { return 1; }
...
std::string CTetrahedron::Type() const { return std::string("TE12L"); }

const IMesh &CTetrahedron::Mesh() const { return m_mesh; }

IMesh &CTetrahedron::Mesh() { return m_mesh; }
