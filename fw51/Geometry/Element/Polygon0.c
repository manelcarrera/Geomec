...
void poly_PreContour(PLINE *C, BOOL optimize) {
  ...
  if (optimize) {
    for (c = (p = &C->head)->next; c != &C->head; c = (p = c)->next) {
      if (IsZero(p->point[0], c->point[0]) && IsZero(p->point[1], c->point[1])) {
        poly_ExclVertex(c);
        free(c);
        c = p;
      }
...
BOOL poly_M_InclContour(POLYAREA **p, PLINE *c) {
  ...
  if (GETBITS(c, PLF_ORIENT) == PLF_DIR) {
    ...
    do {
      ...
      if (poly_ContourInContour(a->contours, c) && (t == NULL || poly_ContourInContour(t->contours, a->contours)))
        t = a;
    } while ((a = a->f) != *p);
    if (t == NULL)
      return FALSE;
...
BOOL poly_ExclContour(POLYAREA *p, PLINE *c) {
  ...
  if (GETBITS(c, PLF_ORIENT) == PLF_DIR) {
    ...
    for (tmp = p->contours; (tmp->next != c) && (tmp->next != NULL); tmp = tmp->next) {
    }
    if (tmp->next == NULL)
      return FALSE;
    tmp->next = c->next;
...
int poly_InsideContour(PLINE *c, Vector p) {
  ...
  do {
    if ((((cur->point[1] <= p[1]) && (p[1] < cur->prev->point[1])) ||
        ...
        (p[0] < (cur->prev->point[0] - cur->point[0]) * (p[1] - cur->point[1]) / (cur->prev->point[1] - cur->point[1]) +
                    cur->point[0]))
      f = !f;
...
static BOOL inside_sector(VNODE *pn, Vector p2) {
  ...
  do {
    ...
    do {
      if (!node_neighbours(a1, a2) &&
          (icnt = vect_inters2(a1->point, a1->next->point, a2->point, a2->next->point, i1, i2)) > 0) {
        if (icnt > 1)
          return TRUE;
        ...
        if (inside_sector(hit1, hit2->prev->point) || inside_sector(hit1, hit2->next->point) ||
            inside_sector(hit2, hit1->prev->point) || inside_sector(hit2, hit1->next->point))
          return TRUE;
      }
