...
CHexahedron::CHexahedron(IMesh &mesh, const std::vector<const IPoint *> &points)
...
CHexahedron::CHexahedron(IMesh &mesh, const std::vector<int> &points) : m_vcNode(points), m_mesh(mesh) {
  assert(points.size() == 8);
...
const IElementSet *CHexahedron::IndexingElementSet() const { return &m_mesh; }

int CHexahedron::Index() const { return m_nIndex; }

int CHexahedron::PointIndex(int nIndex) const { return m_vcNode[nIndex]; }

const geo::IPoint &CHexahedron::Point(int nIndex) const { return m_mesh.Point(m_vcNode[nIndex]); }
...
size_t CHexahedron::NrOfNodes() const { return 8; }
...
size_t CHexahedron::Order() const { return 1; }
...
IElement::TDoubleVec CHexahedron::WorldToIso(const geo::IPoint &point) const {
  ...
  bool bCanUseSimpleImplementation =
      ...
      (v1.DotProduct(v2) < EPS) && (v2.DotProduct(v3) < EPS) && (v3.DotProduct(v4) < EPS) &&
  ...
  Zt = Point(4).Z() * 0.25 * (1 - Ep) * (1 - Np) + Point(5).Z() * 0.25 * (1 + Ep) * (1 - Np) +
       Point(6).Z() * 0.25 * (1 + Ep) * (1 + Np) + Point(7).Z() * 0.25 * (1 - Ep) * (1 + Np);

  // the z bottom coordinate
  Zb = Point(0).Z() * 0.25 * (1 - Ep) * (1 - Np) + Point(1).Z() * 0.25 * (1 + Ep) * (1 - Np) +
       Point(2).Z() * 0.25 * (1 + Ep) * (1 + Np) + Point(3).Z() * 0.25 * (1 - Ep) * (1 + Np);
...
bool CHexahedron::Contains(const geo::IPoint &point, bool bIncludeEdge) const {
  ...
  if ((point.X() < min.X() - compareTolerance() || point.Y() < min.Y() - compareTolerance() ||
       point.Z() < min.Z() - compareTolerance()) ||
      (point.X() > max.X() + compareTolerance() || point.Y() > max.Y() + compareTolerance() ||
...
std::string CHexahedron::Type() const { return std::string("HX24L"); }
...
const IMesh &CHexahedron::Mesh() const { return m_mesh; }

IMesh &CHexahedron::Mesh() { return m_mesh; }
