...
CCylindricHull::Vector::Vector(const geo::IPoint& point)
  : x(point.X())
  , y(point.Y())
  , z(point.Z())
{
...
static inline double length2(double x, double y, double z)
{
  return x * x + y * y + z * z;
}
...
double CCylindricHull::Radius() const
{
  return m_radius;
}
...
#define checkDistance(l) if (l <= m_radiusSquared) { if (includeEdge || l < m_radiusSquared) return true; }
