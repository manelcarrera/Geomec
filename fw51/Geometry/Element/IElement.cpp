...
const IElementSet *IElement::IndexingElementSet() const { return 0; }
...
int IElement::PointIndex(int /*nIndex*/) const { return -1; }
...
const char *IElement::SideName(int /*nIndex*/) const { return 0; }
...
bool IElement::operator<(const IElement &rhs) const {
  ...
  return std::lexicographical_compare(vec1.begin(), vec1.end(), vec2.begin(), vec2.end(), less);
}

std::string IElement::Type() const { return std::string(); }

void IElement::AssertValid() const { assert(NrOfPoints() > 0); }

bool IElement::Empty() const { return !(NrOfPoints() > 0); }
...
IElement::TDoubleVec IElement::WorldToIso(const geo::IPoint &point) const {
  ...
  for (int iter = 0; iter < maxiter; ++iter) {
    ...
    if (matJacobi.RowSize() != matJacobi.ColumnSize()) {
      assert(matJacobi.RowSize() >
             matJacobi.ColumnSize()); // less isoparametric than world coordinates (e.g. face in 3D space)
...
std::vector<CValue> IElement::MapValuesFinal(const CElementCacheObject &cached, const std::vector<CValue> &SourceValues,
  ...
  case WT_REUSS: {
    ...
    for (i = 0; i < ret.size(); i++) {
      if (fabs(ret[i]) > EPS)
        retvals.push_back(CValue(1.0 / ret[i]));
      else
        retvals.push_back(CValue());
    }
  ...
  for (int i = 0; i < ret.size(); i++)
    retvals.push_back(CValue(ret[i]));
  return retvals;
}
...
std::vector<double> IElement::MapValuesVoightFinal(const CElementCacheObject &cached,
  ...
  for (int i = 0; i < nrop; i++)
    ret[i] = uvec.Value(i, 0);

  return ret;
}

std::vector<double> IElement::MapValuesVoight(TSourceValueVector &SourceValues,
...
double IElement::ReussDerived(TSourceValueVector &SourceValues, const std::vector<double> &WeightFactors,
  ...
  if (WeightFactors.size() > 0) {
    // normalize to 1
    double dSum = 0;
    for (i = 0; i < WeightFactors.size(); i++)
      dSum += WeightFactors[i];
    assert(dSum != 0);
    for (i = 0; i < WeightFactors.size(); i++)
      Weight[i] = WeightFactors[i] / dSum;
  } else {
...
std::vector<double> IElement::MapValuesReuss(TSourceValueVector &SourceValues,
                                             const std::vector<double> &WeightFactors) const {
  int nrop = NrOfNodes();
  CMatrix u(nrop, 1);
  CMatrix J(nrop, nrop);
...
std::vector<CValue> IElement::MapValues(TSourceValueVector &SourceValues, TWeightingType nWeightingType,
                                        const std::vector<double> &WeightFactors) const {
...
void IElement::InsertCorners(const geo::IElement &elem, geo::CPtrArray<geo::IPoint> &result) const {
  ...
  for (j = 0; j < NrOfPoints(); j++)
    if (elem.Contains(Point(j), true))
      result.PushBack(*(new geo::CPoint(Point(j))));

  for (j = 0; j < elem.NrOfPoints(); j++)
    if (Contains(elem.Point(j), true))
      result.PushBack(*(new geo::CPoint(elem.Point(j))));
}
...
double IElement::JacobianDeterminant(const IMatrix &J) const {
  ...
  case 2:
    return CVector(J.Value(0, 0), J.Value(1, 0), J.Value(2, 0))
        .CrossProduct(CVector(J.Value(0, 1), J.Value(1, 1), J.Value(2, 1)))
        .Length();
  }
...
*/
int IElement::IntegrationPointSize() const { return 0; }
...
bool IElement::IsElement() const { return true; }
