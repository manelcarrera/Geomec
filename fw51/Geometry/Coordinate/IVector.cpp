...
CVector IVector::operator*(const double &d) const { return CVector(X() * d, Y() * d, Z() * d); }

CVector IVector::operator-() const { return Flipped(); }

/*!
  Returns the 2-norm (euclidean vector norm) of the vector.
*/
double IVector::Length() const { return sqrt(X() * X() + Y() * Y() + Z() * Z()); }

CVector IVector::CrossProduct(const IVector &vector) const {
  return CVector(Y() * vector.Z() - Z() * vector.Y(), Z() * vector.X() - X() * vector.Z(),
...
double IVector::AngleDeg(const IVector &vector) const { return AngleRad(vector) * RAD_TO_DEG; }
...
double IVector::SquareLength() { return X() * X() + Y() * Y() + Z() * Z(); }
