...
CPoint IPoint::Min() const { return ICoordinate::Min(); }

CPoint IPoint::Max() const { return ICoordinate::Max(); }
...
double IPoint::Distance(const IPoint &pt) const { return sqrt(SquareDistance(pt)); }

// ##ModelId=3A90DFD301F0
CPoint IPoint::operator*(const double &val) const {
  AssertValid();
  return CPoint(X() * val, Y() * val, Z() * val);
}
...
CPoint IPoint::Max(const IPoint &rhs) const {
  ...
  return CPoint(std::max(X(), rhs.X()), std::max(Y(), rhs.Y()), std::max(Z(), rhs.Z()));
}
...
CPoint IPoint::Min(const IPoint &rhs) const {
  ...
  // construct without factory
  return CPoint(std::min(X(), rhs.X()), std::min(Y(), rhs.Y()), std::min(Z(), rhs.Z()));
}
...
CPoint operator*(const double &val, const IPoint &rhs) { return rhs * val; }
