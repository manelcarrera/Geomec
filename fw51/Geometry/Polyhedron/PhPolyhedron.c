 ...
 */
static void RaySort(Matrix *Ray, SatMatrix *Sat, int NbBid, int NbRay, int *equal_bound, int *sup_bound,
                    unsigned RowSize1, unsigned RowSize2, unsigned bx, unsigned jx) {
 ...
 */
static int Chernikova(Matrix *Mat, Matrix *Ray, SatMatrix *Sat, unsigned NbBid, unsigned NbMaxRays,
                      unsigned FirstConstraint, unsigned dual) {
  ...
  TRY {
    ...
    for (k = FirstConstraint; k < NbConstraints; k++) {
      ...
      if (index_non_zero < NbBid) {
        ...
        RaySort(Ray, Sat, NbBid, NbRay, &equal_bound, &sup_bound, RowSize1, RowSize2, bx, jx);
        ...
        for (i = equal_bound; i < sup_bound; i++) /* for all pairs of R- and R+ */
          for (j = sup_bound; j < bound; j++) {
            ...
            for (m = MSB; m != bx; m >>= 1)
              if (!(aux & m))
                nbcommonconstraints++;
            rayonly = (value_zero_p(Ray->p[i][Dimension]) && value_zero_p(Ray->p[j][Dimension]) && (dual == 0));
            if (rayonly)
...
static Polyhedron *Remove_Redundants(Matrix *Mat, Matrix *Ray, SatMatrix *Sat, unsigned *Filter) {
  ...
  int i, j, k;
  unsigned Dimension, sat_nbcolumns, NbRay, NbConstraints, RowSize1, RowSize2, *Trace = NULL, *bx = NULL, *jx = NULL,
                                                                               Dim_RaySpace, b;
  ...
  TRY {
    ...
    for (j = 0; j < NbConstraints; j++) {
      ...
      if (i == (int)Dimension) {
        ...
        if ((value_eq(Mat->p[j][0], tmp1)) && (value_notzero_p(Mat->p[j][Dimension]))) {
    ...
    for (i = 0; i < NbEq; i++) {
      ...
      if (value_ne(Mat->p[i][0], tmp1)) {
        ...
        for (k = i + 1; value_ne(Mat->p[k][0], tmp1) && k < NbConstraints; k++)
          ;
        if (k == (int)NbConstraints) /* If none found then error */
          break;
 ...
 */
void PolyPrint(Polyhedron *Pol) { Polyhedron_Print(stderr, "%4d", Pol); } /* PolyPrint */
...
Polyhedron *DomainIntersection(Polyhedron *Pol1, Polyhedron *Pol2, unsigned NbMaxRays) {
  ...
  if (Pol1->Dimension != Pol2->Dimension) {
    errormsg1("DomainIntersection", "diffdim", "operation on different dimensions");
    return (Polyhedron *)0;
  }
...
Polyhedron *Polyhedron_Copy(Polyhedron *Pol) {
  ...
  Vector_Copy(Pol->Constraint[0], Pol1->Constraint[0], Pol->NbConstraints * (Pol->Dimension + 2));
 ...
 *     }
 */
static void addToFilter(int k, unsigned *Filter, SatMatrix *Sat, Value *tmpR, Value *tmpC, int NbRays,
                        int NbConstraints) {
...
static void FindSimple(Polyhedron *P1, Polyhedron *P2, unsigned *Filter, unsigned NbMaxRays) {
  ...
  Value *p1, *p2, p3;
  unsigned NbRays = 0;
  unsigned Dimension, NbConstraints, bx, nc;
      ...
      for (k = 0; k < NbConstraints; k++) {
        if (Filter[jx] & bx)
          ...
          for (i = 0; i < NbRays; i++) {
            ...
            if (value_zero_p(p3) || (value_pos_p(p3) && value_notzero_p(P1->Constraint[k][0]))) {
            ...
            if (value_eq(tmp, NbConstraintsLeft)) {
              ...
              for (k = 0; k < NbConstraints; k++) {
                if (value_posz_p(tmpC[k]) && ((Sat->p[i][jx] & bx) == 0)) {
                  addToFilter(k, Filter, Sat, tmpR, tmpC, NbRays, NbConstraints);
                  Vector_Copy(P1->Constraint[k], Mat->p[Mat->NbRows], Dimension + 1);
                  Mat->NbRows++;
  ...
  for (p1 = Pol1; p1; p1 = p1->next) {
    ...
    if (!empty) {
      ...
      for (k = 0, jx = 0, bx = MSB; k < NbConstraints; k++) {
        ...
        if (Filter[jx] & bx) {
          Vector_Copy(p1->Constraint[k], Constraints->p[Constraints->NbRows], Dimension);
          Constraints->NbRows++;
        }
        NEXT(jx, bx);
      }
...
Polyhedron *Stras_DomainSimplify(Polyhedron *Pol1, Polyhedron *Pol2, unsigned NbMaxRays) {
  ...
  Polyhedron *p1, *p2 = NULL, *p3 = NULL, *d = NULL;
  unsigned k, jx, bx, nbentries, NbConstraints, Dimension, NbCon, empty;
...
Polyhedron *DomainDifference(Polyhedron *Pol1, Polyhedron *Pol2, unsigned NbMaxRays) {
  ...
  if (!Pol1 || !Pol2)
    return (Polyhedron *)0;
  if (Pol1->Dimension != Pol2->Dimension) {
    errormsg1("DomainDifference", "diffdim", "operation on different dimensions");
    return (Polyhedron *)0;
  }
  d = (Polyhedron *)0;
  for (p2 = Pol2; p2; p2 = p2->next) {
 ...
 */
Polyhedron *align_context(Polyhedron *Pol, unsigned int align_dimension, unsigned int NbMaxRays) {
  ...
  TRY {
    ...
    for (i = 0; i < NbRay; i++) {
      ...
      if (I->MaxI == -1 || value_gt(tmp1, tmp2) ||
          (value_eq(tmp1, tmp2) && value_eq(d, tmp3) && value_ne(I->MaxD, tmp3))) {
      ...
      if (I->MinI == -1 || value_lt(tmp1, tmp2) ||
          (value_eq(tmp1, tmp2) && value_eq(d, tmp3) && value_ne(I->MinD, tmp3))) {
...
Polyhedron *DomainAddConstraints(Polyhedron *Pol, Matrix *Mat, unsigned NbMaxRays) {
  ...
  if (Pol->Dimension != Mat->NbColumns - 2) {
    errormsg1("DomainAddConstraints", "diffdim", "operation on different dimensions");
    return (Polyhedron *)0;
  }
