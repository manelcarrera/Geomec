...
Polyhedron *PDomainIntersection(Polyhedron *Pol1, Polyhedron *Pol2, unsigned NbMaxRays) {
  ...
  if ((Pol1->Dimension != Pol2->Dimension) || (Pol1->NbEq != Pol2->NbEq)) {
    fprintf(stderr, "? PDomainIntersection: operation on different dimensions\n");
    return (Polyhedron *)0;
  }
...
static Polyhedron *Add_CEqualities(Polyhedron *D) {
  ...
  if (!CEqualities)
    ...
    for (d = D->next; d; d = d->next) {
      tmp->next = AddConstraints(d->Constraint[0], d->NbConstraints, CEqualities, ws);
      tmp = tmp->next;
    }
    Domain_Free(D);
    return (r);
...
static int count_sat(int *mf) {
  ...
  for (i = 0; i < nr; i++) {
    tmp = mf[i];
    cnt =
        cnt + cntbit[tmp & 0xff] + cntbit[(tmp >> 8) & 0xff] + cntbit[(tmp >> 16) & 0xff] + cntbit[(tmp >> 24) & 0xff];
  }
  return cnt;
} /* count_sat */
...
static SatMatrix *Poly2Sat(Polyhedron *Pol, int **L) {
  ...
  /* Build the localSat matrix */
  nr = (NbRay - 1) / (sizeof(int) * 8) + 1; /* Set globally */
  localSat = SMAlloc(NbCon, nr);
  Temp = (int *)malloc(nr * sizeof(int));
  memset(localSat->p_init, 0, nr * NbCon * sizeof(int));
...
Param_Polyhedron *GenParamPolyhedron(Polyhedron *Pol) {
  ...
  for (i = 0; i < nbRows; i++)
    if (value_notone_p(rays->p[i][0]) || value_zero_p(rays->p[i][nbColumns - 1]))
      return NULL;
 ...
 */
Param_Polyhedron *Find_m_faces(Polyhedron **Di, Polyhedron *C, int keep_dom, int working_space, Polyhedron **CEq,
                               Matrix **CT) {
...
#endif
#ifdef DEBUGPP3
  fprintf(stderr, "Target: find faces that saturate %d constraints and %d rays/lines\n", D1->Dimension - m_dim,
          m_dim + 1);
#endif
 ...
 *
 */
Param_Polyhedron *Polyhedron2Param_SimplifiedDomain(Polyhedron **Din, Polyhedron *Cin, int working_space,
