  ...
  for(int i = 0; i < ElementSize(); i++)
  {
    ...
    if(pBody)
    {
      ...
      for(int j = 0; j < pBody->NrOfFaces(); j++)
      {
        ...
        std::pair<TFaceMap::iterator, bool> ret = mpFace.insert(TFaceMap::value_type(face_index, std::pair<const IFace*, int>(&face, 1))); 
        if(!ret.second)
...
void IVolume::IntersectionWithEdgeFaces(const ILine &Line, std::set<CPoint> &PointSet) const
{
  ...
  for(TFaceMap::iterator i = mpFace.begin(); i != mpFace.end(); i++)
  {
    if(i->second.second == 1) {
      ...
      if(!point.Empty()) PointSet.insert(point);
    }
  }
}
...
void IVolume::Intersection(const IPlane &Plane, CArray<CPolygon> &PolArr, std::set<CPoint> *PointSet /* = 0 */, IProgressBase* pProgress /* = 0 */) const
{
  ...
  {	
    ...
    if(TmpLineSet.size() < 2)
    { // plane crosses an 'edge' of a body or doesn't intersect at all.
      TmpLineSet.clear();
      continue;
    }
  ...
  for(it = PolyLineSet.begin(); it != PolyLineSet.end(); it++)
  { // create an array from the set because I need random acces in ExtractPolyLine()....
...
CArray<CPolyLine> IVolume::ExtractPolyLine(CArray<CLine> &LineArr) const
{
  ...
  while(LineArr.Size() > 0)
  {
    ...
    {
      bool ptPresent;
      ptPresent = (bUseLastPoint) ? PLine.PushBack(LineArr.Object(i).Second()) : PLine.PushFront(LineArr.Object(i).Second());
      if(!ptPresent)
      {// apparently the second point is also already present in the polyline -> the line must be closed, we found a polygon!
    ...
    {
      bool ptPresent;
      ptPresent = (bUseLastPoint) ? PLine.PushBack(LineArr.Object(i).First()) : PLine.PushFront(LineArr.Object(i).First());
      if(!ptPresent)
      {// apparently the first point is also already present in the polyline -> the line must be closed, we found a polygon!
        PLineArr.PushBack(PLine);
        PLine.RemoveAll();
        j = 0;
      }
  ...
  while(TmpLineArr.Size() > 0)
  {
    ...
    if(TmpLineArr.Object(i).LastPoint() == PLine.LastPoint())
    {
      ...
      for(k = NrPoints - 2; k >= 0; k--) // don't use the last point, is already present...
      {
        ...
        { // closed a polyline with another
          assert(k == 0);
          PLineArr.PushBack(PLine);
          PLine.RemoveAll();
          j = 0;
...
const IElement &IVolume::Element(int nIndex) const
{
  return Body(nIndex);
}

int IVolume::ElementSize() const
{
  return BodySize();
}
