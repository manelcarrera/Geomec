...
template <class VALUE_TYPE> class IValueSetTemplate {
public:
...
VALUE_TYPE IValueSetTemplate<VALUE_TYPE>::InterpolateValue(const IPoint &point, int nElement,
...
template <class VALUE_TYPE> void IValueSetTemplate<VALUE_TYPE>::FilterElements(std::vector<int> & /*indices*/) const {}
...
template <class VALUE_TYPE>
void IValueSetTemplate<VALUE_TYPE>::ElementValue(const IElement &element, TValueVec &element_values,
                                                 IParallelInitializationCallback *cb) const {
  ...
  const CBodyTriangle *pBodyTriangle = dynamic_cast<const CBodyTriangle *>(&element);
  const CBodyQuadrilateral *pBodyQuadrilateral = dynamic_cast<const CBodyQuadrilateral *>(&element);
  const CEmbeddedTriangle *pEmbeddedTriangle = dynamic_cast<const CEmbeddedTriangle *>(&element);
    ...
    for (int i = 0; i < 3; ++i) {
      ...
      if (ifElt) {
        ...
        for (size_t j = 0; j < 3; j++) {
          if (!IsValid(values[j + offset]))
            return;
        }

        element_values[i] = values[offset] * vcShape[offset] + values[offset + 1] * vcShape[offset + 1] +
                            values[offset + 2] * vcShape[offset + 2];
      }
    }
    return;
  }
  ...
  if (pBodyTriangle || pBodyQuadrilateral) {
    ...
    if (pBodyTriangle)
      ...
      pBody = pBodyQuadrilateral->Parent();
...
template <class VALUE_TYPE> int IValueSetTemplate<VALUE_TYPE>::ValueSize(int nElement) const {
  const geo::IElement &element = ElementSet().Element(nElement);
  if (!element.NrOfNodes())
    return element.NrOfPoints();
...
bool IValueSetTemplate<VALUE_TYPE>::Contains(const geo::IElement &target, int nSourceIndex) const {
  ...
  for (int i = 0; i < target.NrOfPoints(); i++) {
    if (!source_element.Contains(target.Point(i), true))
      return false;
  }
  return true;
}
