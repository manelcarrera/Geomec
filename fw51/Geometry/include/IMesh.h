...
public:
  ...
  void GetElementTypeRanges(std::vector<std::pair<int, int>> &bodies, std::vector<std::pair<int, int>> &faults,
                            int *pMaxBodyNodes = 0, int *pMaxFaultNodes = 0) const;
  ...
  // single master
  const CTying &CreateTying(int nSlave, const IVector &vecSlaveDir, CTying::TType nSlaveType, int nMaster,
                            const IVector &vecMasterDir, CTying::TType nMasterType, double dFactor);

  // two masters
  const CTying &CreateTying(int nSlave, const IVector &vecSlaveDir, CTying::TType nSlaveType, int nMaster0,
                            const IVector &vecMaster0Dir, CTying::TType nMaster0Type, double dFactor0, int nMaster1,
                            const IVector &vecMaster1Dir, CTying::TType nMaster1Type, double dFactor1);
