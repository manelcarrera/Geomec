...
*/
template <class KEY, class VALUE, class EPSILON = CEpsilon>
class COctree
{
private:
  template <class N_KEY, class N_VALUE, class N_EPSILON>
  class CNode
  {
  public:
...
public:
  template <class CI_KEY, class CI_VALUE, class CI_EPSILON>
  ...
  public:
    ...
    CConstIterator(TNode* node, TContainer& cont)
    : m_pNode(node),
      m_pCont(&cont)
    {
    }

    CConstIterator(const CConstIterator& it)
    : m_pNode(it.m_pNode),
      m_pCont(it.m_pCont)
    {
    }
    ...
    {
      return (container() == itRight.container() &&
              Node() == itRight.Node());
    }

    bool operator!=(const CConstIterator& itRight)
    {
      return !operator==(itRight);
    }
  ...
  public:
    ...
    CIterator(TNode* node, TContainer& cont)
    : TBase(node, cont)
    {
    }
  ...
  template <class CRI_KEY, class CRI_VALUE, class CRI_EPSILON>
  ...
  public:
    ...
    CConstReverseIterator(TNode* node, TContainer& cont)
    : m_pNode(node),
      m_pCont(&cont)
    {
    }

    CConstReverseIterator(const CConstReverseIterator& it)
    : m_pNode(it.m_pNode),
      m_pCont(it.m_pCont)
    {
    }

    CConstReverseIterator(const CConstIterator<CRI_KEY, CRI_VALUE, CRI_EPSILON>& it)
    : m_pCont(it.container())
    {
      if(it.Node())
    ...
    bool operator==(const CConstReverseIterator& it)
    {
      return (m_pCont == it.m_pCont &&
              m_pNode == it.m_pNode);
    }
  ...
  public:
    ...
    CReverseIterator(TNode* node, TContainer& cont)
    : TBase(node, cont)
    {
    }

    CReverseIterator(const CIterator<RI_KEY, RI_VALUE, RI_EPSILON>& it)
    : TBase(it)
    {
    }
...
public:
  ...
  {
    return (const_cast<COctree*>(this))->find(key);
  }
  ...
  {
    return (const_cast<COctree*>(this))->begin();
  }

  const_iterator end() const
  {
    return (const_cast<COctree*>(this))->end();
  }
  ...
  {
    return (const_cast<COctree*>(this))->rbegin();
  }
