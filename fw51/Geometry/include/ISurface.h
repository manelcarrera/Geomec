...
private:
  ...
  void InsertPointsIntoMapAndSet(TPointPairSet &stPointPairs, TPointMap &mpFaces, const IPoint &pt1, const IPoint &pt2,
                                 const IFace &face) const;
  void CreateConnectivityPointMapAndSet(TPointPairSet &stPointPairs, TPointMap &mpFaces) const;
  std::multimap<double, const IPoint *> NearestXYPoints(int size_x, int size_y, const double &dX,
  ...
  bool LocateRingIndex(const IPoint &point, std::set<const IPoint *> &stEncountered,
...
public:
  ...
  virtual std::vector<std::vector<const IPoint *>> SortedEdgePoints(bool bRetry = false,
                                                                    double dEpsilon = 0.0001) const;
  ...
  void FilterElementsInOrthoBox(const geo::IPoint &ptMin, const geo::IPoint &ptMax, TElementSet &elements_on_the_side,
                                TElementSet &elements_strict_inside, TElementSet &elements_strict_outside) const;
