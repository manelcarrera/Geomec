...
template <class BASE> class CInterfaceElementPart : public BASE {
...
public:
  ...
  */
  CInterfaceElementPart(const IElement &parent, int side) : m_parent(parent), m_side(side) {}
  ...
  */
  virtual const INode &Node(int n) const {
    ...
    } else if (this->NrOfLines() > 2 && 2 * this->NrOfLines() * m_parent.Order() != m_parent.NrOfNodes()) {
      if (m_side == 0) // the Front
      {
        return m_parent.Node(n);
      } else // the Back
  ...
  */
  virtual size_t NrOfNodes() const {
    ...
    if ((this->NrOfLines() > 2) && (2 * this->NrOfLines() * m_parent.Order() != m_parent.NrOfNodes())) {
      if (m_side == 0) // the Front
      {
  ...
  */
  virtual size_t Order() const {
    ...
    if ((this->NrOfLines() <= 2 && m_parent.NrOfNodes() % 2 == 1) ||
        (this->NrOfLines() > 2 && 2 * this->NrOfLines() * m_parent.Order() != m_parent.NrOfNodes())) {
  ...
  */
  virtual int Index() const { return m_parent.Index(); }
};
