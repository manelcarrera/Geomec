...
/*!
  ...
  \brief this class only used by InterfaceElement.h. It has been created for handle a InterfaceElement in drawing
  classes
  \author : Antoine
  \date : 21 april 2006
*/
...
public:
  ...
  */
  virtual int NrOfPoints() const { return (int)NrOfNodes(); }

  /*!
  Returns the i-th point.
  */
  virtual const IPoint &Point(int nIndex) const { return Node(nIndex); }

  /*!
    Sets the i-th point to pt.
  */
  virtual void Point(int nIndex, const IPoint &pt) { Node(nIndex, pt); }

  /*!
  Returns the total number of lines of the interface element.
  */
  virtual int NrOfLines() const { return Front().NrOfLines() + Back().NrOfLines(); }
  ...
  */
  ...
  used.
  */
  virtual double RepresentativeLength() const { return 0; }

  virtual double Size() const { return Front().Size(); }

  virtual bool Visit(IVisitor &visitor) { return visitor.HandleInterface(*this); }

  virtual std::vector<TDoubleVec> IsoCoordinates() const { return Front().IsoCoordinates(); }
  ...
  virtual CPoint IsoToWorld(const TDoubleVec &isocoords) const { return Front().IsoToWorld(isocoords); }
  ...
  virtual bool IsInterfaceElement() const { return true; }
};
