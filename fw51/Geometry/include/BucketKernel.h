...
*/
template<class T>
class CSetExpandCallBack : public CExpandCallBack<T>
{
public:
  typedef typename std::set<T> TBucket;
private:
...
template<class T>
CBucketKernelTempl<T>::CBucketKernelTempl()
: m_dEdgeSize(-1) 
{
  m_size[0] = 0;
  m_size[1] = 0;
...
template<class T>
CBucketKernelTempl<T>::CBucketKernelTempl(const IPoint& orgin, int size_x, int size_y, int size_z, const double& dEdgeSize)
{
  Resize(orgin, size_x, size_y, size_z, dEdgeSize);
}

template<class T>
CBucketKernelTempl<T>::CBucketKernelTempl(const IPoint& min, const IPoint& max, int nr_of_buckets)
{
...
template<class T>
void CBucketKernelTempl<T>::Resize(const IPoint& min, const IPoint& max, int nr_of_buckets)
{
  m_min = min;
...
//	assert(nDim > 0);
  ...
  m_max = CPoint(min.X() + m_size[0] * m_dEdgeSize, min.Y() + m_size[1] * m_dEdgeSize, min.Z() + m_size[2] * m_dEdgeSize);
}
...
template<class T>
typename CBucketKernelTempl<T>::TBucket CBucketKernelTempl<T>::Buckets(const ILine& line) const
{
  return Buckets(line.Min(), line.Max());
}

template<class T>
bool CBucketKernelTempl<T>::Insert(int size_x, int size_y, int size_z, T& value)
{
  assert(size_x >= 0);
  assert(size_y >= 0);
  assert(size_z >= 0);
...
template<class T>
bool CBucketKernelTempl<T>::Insert(const IPoint& point, T& value)
{
  return Insert(BucketX(point.X()), BucketY(point.Y()), BucketZ(point.Z()), value);
}

template<class T>
bool CBucketKernelTempl<T>::Insert(const IPoint& min, const IPoint& max, T& value)
{
  CPoint range = max - min;
  assert(range.X() >= 0);
  assert(range.Y() >= 0);
  ...
  for(int x = min_x; x <= max_x; x++)
    for(int y = min_y; y <= max_y; y++)
      for(int z = min_z; z <= max_z; z++)
        if(Insert(x, y, z, value)) bRet = true;

  return bRet;
}

template<class T>
bool CBucketKernelTempl<T>::Remove(int size_x, int size_y, int size_z, T& value)
{
  assert(size_x >= 0);
  assert(size_y >= 0);
  assert(size_z >= 0);
...
template<class T>
bool CBucketKernelTempl<T>::Remove(const IPoint& point, T& value)
{
  return Remove(BucketX(point.X()), BucketY(point.Y()), BucketZ(point.Z()), value);
}

template<class T>
bool CBucketKernelTempl<T>::Remove(const IPoint& min, const IPoint& max, T& value)
{
  CPoint range = max - min;
  assert(range.X() >= 0);
  assert(range.Y() >= 0);
  ...
  for(int x = min_x; x <= max_x; x++)
    for(int y = min_y; y <= max_y; y++)
      for(int z = min_z; z <= max_z; z++)
        if(Remove(x, y, z, value)) bRet = true;

  return bRet;
}

template<class T>
int CBucketKernelTempl<T>::BucketX(const double& dX) const
{
...
template<class T>
int CBucketKernelTempl<T>::BucketY(const double& dY) const
{
...
template<class T>
int CBucketKernelTempl<T>::BucketZ(const double& dZ) const
{
...
template<class T>
int CBucketKernelTempl<T>::Limit(int nMin, int nValue, int nMax) const
{
  if(nValue < nMin)
    return nMin;
  if(nValue > nMax)
...
template<class T>
void CBucketKernelTempl<T>::Expand(TCallBack& ec, const IPoint& point, int nExpand) const
{
  Expand(ec, BucketX(point.X()), BucketY(point.Y()), BucketZ(point.Z()), nExpand);
}

template<class T>
void CBucketKernelTempl<T>::ExpandXY(TCallBack& ec, const IPoint& point, int nExpand) const
{
