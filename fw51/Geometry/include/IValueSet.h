...
public:
  ...
  // get the value at the given point using the given weighting (not smoothed, but averaged when in two or more
  // elements)
  virtual CValue PointValue(const IPoint &point, TWeightingType nWeightingType,
  ...
  virtual CValue PointValue(const IPoint &point, TWeightingType nWeightingType, const IElement &Overlap,
                            IParallelInitializationCallback *cb) const;

  // get the smoothed value at the given point, where nodal smoothing is performed using the given weighting type
  virtual CValue PointValueSmoothed(const IPoint &point, TWeightingType nWeightingType,
                                    IParallelInitializationCallback *cb) const;
  ...
  virtual bool MapValues(const IBody &body, std::vector<CValue> &values, TWeightingType nWeightingType,
                         IParallelInitializationCallback *cb) const;

  // get nodal values for the element from smoothed values in the set
  virtual bool GetSmoothedNodalValues(const IElement &element, std::vector<CValue> &values,
                                      IParallelInitializationCallback *cb) const;

  // map values from this valueset (and its elementset) to the given valueset (and its elementset)
  virtual void MapValues(IValueSet &TargetValues, TWeightingType nWeightingType, IParallelInitializationCallback *cb,
...
private:
  ...
  CValue GetWeightedAverage(const std::vector<CValue> &values, const std::vector<double> &weightfactors,
                            TWeightingType nWeightingType) const;
  CValue PointValue(const IPoint &point, std::vector<int> &vecElements, TWeightingType nWeightingType,
                    IParallelInitializationCallback *cb) const;
  bool MapValuesWithCache(CElementCacheObject &cached, const IBody &body, std::vector<CValue> &values,
