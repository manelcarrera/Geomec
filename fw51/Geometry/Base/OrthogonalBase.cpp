...
COrthogonalBase::COrthogonalBase(const IVector &vector) { VERIFY(AddVector(vector)); }
...
COrthogonalBase::~COrthogonalBase() {
  for (size_t i = 0; i < m_vcDirections.size(); i++)
    delete m_vcDirections[i];
}
...
size_t COrthogonalBase::Size() const { return m_vcDirections.size(); }
    ...
    *m_vcDirections[0] = vector.UnitVector();
    ...
    if (Direction(0).IsParallel(geo::CVector::Xaxis))
      ...
      *m_vcDirections[1] = Direction(0).CrossProduct(CVector::Xaxis).UnitVector();
      ...
      if (vec1.SquareLength() > vec2.SquareLength())
        *m_vcDirections[1] = vec1.UnitVector();
      else
        *m_vcDirections[1] = vec2.UnitVector();
    }
