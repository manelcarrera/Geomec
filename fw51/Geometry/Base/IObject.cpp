...
*/
ftn_double_t IObject::compareTolerance()
{
  return m_compareTolerance;
}
...
bool IObject::BoundingBoxIntersect(const geo::IObject &rhs, bool bIncludeEdges) const
{
	...
	bool bXOverlap = (ptMinThis.X() + compareTolerance() > ptMinRhs.X() &&
                    ptMinThis.X() < ptMaxRhs.X()) ||
					         (ptMaxThis.X() > ptMinRhs.X() &&
                    ptMaxThis.X() < ptMaxRhs.X() + compareTolerance());
	bool bYOverlap = (ptMinThis.Y() + compareTolerance() > ptMinRhs.Y() &&
                    ptMinThis.Y() < ptMaxRhs.Y()) ||
					         (ptMaxThis.Y() > ptMinRhs.Y() &&
                    ptMaxThis.Y() < ptMaxRhs.Y() + compareTolerance());
	bool bZOverlap = (ptMinThis.Z() + compareTolerance() > ptMinRhs.Z() &&
                    ptMinThis.Z() < ptMaxRhs.Z()) ||
					         (ptMaxThis.Z() > ptMinRhs.Z() &&
                    ptMaxThis.Z() < ptMaxRhs.Z() + compareTolerance());
...
bool IObject::PointInsideBoundingBox(const geo::IPoint &point) const
{
	...
	if ( point.X() > ptMinThis.X()+compareTolerance() &&
       point.X() + compareTolerance() < ptMaxThis.X() &&
       point.Y() > ptMinThis.Y()+compareTolerance() &&
       point.Y() + compareTolerance() < ptMaxThis.Y() &&
       point.Z() > ptMinThis.Z()+compareTolerance() &&
       point.Z() + compareTolerance() < ptMaxThis.Z() )
    return true;
...
bool IObject::BBRhsInside(const geo::IObject &rhs) const
{
	...
	if(!(ptMinRhs.X() > ptMinThis.X() +compareTolerance() &&
       ptMaxRhs.X() +compareTolerance() < ptMaxThis.X()  ))
		return false;

	if(!(ptMinRhs.Y() > ptMinThis.Y() +compareTolerance() &&
       ptMaxRhs.Y() +compareTolerance() < ptMaxThis.Y()  ))
		return false;

	if(!(ptMinRhs.Z() > ptMinThis.Z() +compareTolerance() &&
       ptMaxRhs.Z() +compareTolerance() < ptMaxThis.Z()  ))
		return false;
...
bool IObject::IsElement() const
{
  return false;
}

bool IObject::IsInterfaceElement() const
{
  return false;
}

bool IObject::IsBody() const
{
  return false;
}
