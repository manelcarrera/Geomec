...
std::vector<double> IValueSet::WeightFactors(const std::vector<int> &vecElements) const {
  double Total = 0;
  for (std::vector<int>::const_iterator it = vecElements.begin(); it != vecElements.end(); it++)
    Total += ElementSet().Element(*it).Size();

  std::vector<double> ret(vecElements.size());
  for (size_t i = 0; i < vecElements.size(); i++)
    ret[i] = ElementSet().Element(vecElements[i]).Size() / Total;

  return ret;
}

CValue IValueSet::GetWeightedAverage(const std::vector<CValue> &values, const std::vector<double> &weightfactors,
...
bool IValueSet::IsValid(const CValue &value) const { return value.Valid(); }

CValue IValueSet::PointValue(const IPoint &point, std::vector<int> &vecElements, TWeightingType nWeightingType,
                             IParallelInitializationCallback *cb) const {
  CValue retval = 0;
...
CValue IValueSet::PointValue(const IPoint &point, TWeightingType nWeightingType,
                             IParallelInitializationCallback *cb) const {
...
// get the value at the given point using the given weighting, but only if (partially) overlapping with the given
// element (not smoothed, but averaged when in two or more elements)
CValue IValueSet::PointValue(const IPoint &point, TWeightingType nWeightingType, const IElement &Overlap,
                             IParallelInitializationCallback *cb) const {
  std::vector<int> vecElements = ElementSet().ElementsAt(point);
  assert(vecElements.size() > 0); // must give point inside elementset
  ...
  for (size_t i = 0; i < vecElements.size(); i++)
    elements[i] = &ElementSet().Element(vecElements[i]);

  std::vector<int> vecOverlapping = Overlap.OverlappingElementIndices(elements);
  std::vector<int> newvec(vecOverlapping.size());
  for (size_t i = 0; i < vecOverlapping.size(); i++)
    newvec[i] = vecElements[vecOverlapping[i]];
...
CValue IValueSet::PointValueSmoothed(const IPoint &point, TWeightingType nWeightingType,
                                     IParallelInitializationCallback *cb) const {
  std::vector<int> vecElements = ElementSet().ElementsAt(point);
  assert(vecElements.size() > 0); // must give point inside elementset
  ...
  for (size_t i = 0; i < nodes.size(); i++)
    values[i] = NodeValue(nodes[i], nWeightingType, cb);
...
bool IValueSet::MapValues(const IBody &body, std::vector<CValue> &values, TWeightingType nWeightingType,
                          IParallelInitializationCallback *cb) const {
  ...
  for (size_t i = 0; i < vcSourceValues.size(); i++)
    delete vcSourceValues[i].first;
...
bool IValueSet::MapValuesWithCache(CElementCacheObject &cached, const IBody &body, std::vector<CValue> &values,
                                   TWeightingType nWeightingType, IParallelInitializationCallback *cb) const {
  ...
  if (!cached.Valid() || cached.Type() != nWeightingType) {
    std::set<int> stIndex = ElementSet().Candidates(body.Min(), body.Max());
    if (stIndex.empty())
      return false; // since Candidates is cached, we're not caching this
...
bool IValueSet::GetSmoothedNodalValues(const IElement &element, std::vector<CValue> &values,
                                       IParallelInitializationCallback *cb) const {
  // If we "know" the element we return the stored values
...
void IValueSet::MapValues(IValueSet &TargetValues, TWeightingType nWeightingType, IParallelInitializationCallback *cb,
                          IProgressBase *pProgress /* = 0 */) const {
  const IElementSet &stTarget = TargetValues.ElementSet();

  if (pProgress)
    pProgress->AddSteps(stTarget.ElementSize());
