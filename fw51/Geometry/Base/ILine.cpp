...
const IElement::TIntPtVec &ILine::IntegrationPoints() const { return IntegrationPoints(Order()); }
...
double ILine::SquareLength() const { return First().SquareDistance(Second()); }

double ILine::Size() const {
  if (NrOfNodes())
    return IElement::Size(); // a real mesh element
...
CPoint ILine::Max() const { return First().Max(Second()); }

// ##ModelId=3A90DFD401D0
CPoint ILine::Min() const { return First().Min(Second()); }
...
CVector ILine::Dir() const { return CVector(Second() - First()); }
...
bool ILine::Empty() const { return First().Empty() || Second().Empty(); }

const IPoint &ILine::First() const { return Point(0); }

void ILine::First(const IPoint &point) { Point(0, point); }

const IPoint &ILine::Second() const { return Point(NrOfPoints() - 1); }

void ILine::Second(const IPoint &point) { Point(NrOfPoints() - 1, point); }
...
const IPoint &ILine::Point(int nIndex) const { return Node(nIndex); }

void ILine::Point(int nIndex, const IPoint &pt) { Node(nIndex, pt); }
...
double ILine::RepresentativeLength() const { return Length(); }

int ILine::NrOfLines() const { return 1; }

const ILine &ILine::Line(int /*nIndex*/) const { return *this; }
...
int ILine::IntegrationPointSize() const { return (int)IntegrationPoints().size(); }
