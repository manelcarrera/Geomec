...
CDirectedSegment::CDirectedSegment(const CRingFactory &factory, int nFirst, const TSegment &segment)
  ...
  assert((nFirst < m_factory.PointSize()) && (segment.first < m_factory.PointSize()) &&
         (segment.second < m_factory.PointSize()));
...
CDirectedSegment::CDirectedSegment(const CDirectedSegment &rhs) : m_segment(rhs.m_segment), m_factory(rhs.m_factory) {}

int CDirectedSegment::First() const { return m_segment.first; }

int CDirectedSegment::Second() const { return m_segment.second; }

std::vector<CDirectedSegment> CDirectedSegment::NextSegments() const { return AdjacentSegments(Second()); }

std::vector<CDirectedSegment> CDirectedSegment::PrevSegments() const { return AdjacentSegments(First()); }
...
bool CDirectedSegment::operator==(const CDirectedSegment &rhs) const { return m_segment == rhs.m_segment; }
...
double CDirectedSegment::Angle(const CDirectedSegment &rhs) const {
  assert((First() == rhs.First()) || (First() == rhs.Second()) || (Second() == rhs.First()) ||
         (Second() == rhs.Second()));
