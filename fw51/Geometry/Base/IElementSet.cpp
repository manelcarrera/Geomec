...
: m_pKernel(0), m_cache_interface(&CCacheInterfaceDefault::Instance())
{
}
...
void IElementSet::XDir(const IPlane &Plane, std::set<int>& stElementIndex) const
{
  ...
  for(y = 0; y <= BucketKernel().SizeY(); y++)
  {
    ...
    for(int z = 0; z <= BucketKernel().SizeZ(); z++)
    {
      ...
      CPoint pt = Plane.Intersection(geo::CLine( geo::CPoint(BucketKernel().Min().X(), dY, dZ),
                             geo::CPoint(BucketKernel().Max().X(), dY, dZ)));
...
void IElementSet::YDir(const IPlane &Plane, std::set<int>& stElementIndex) const
{
  ...
  for(x = 0; x <= BucketKernel().SizeX(); x++)
  {
    ...
    for(int z = 0; z <= BucketKernel().SizeZ(); z++)
    {
      ...
      double dZ = BucketKernel().Min().Z() + z * dEdgeSize;
      CPoint pt = Plane.Intersection(geo::CLine( geo::CPoint(dX,  BucketKernel().Min().Y(), dZ),
                             geo::CPoint(dX, BucketKernel().Max().Y(), dZ)));
...
void IElementSet::ZDir(const IPlane &Plane, std::set<int>& stElementIndex) const
{
  ...
  for(x = 0; x <= BucketKernel().SizeX(); x++)
  {
    ...
    for(int y = 0; y <= BucketKernel().SizeY(); y++)
    {
      double dX = BucketKernel().Min().X() + x * dEdgeSize;
      double dY = BucketKernel().Min().Y() + y * dEdgeSize;
      CPoint pt = Plane.Intersection(geo::CLine( geo::CPoint(dX, dY, BucketKernel().Min().Z()),
                            geo::CPoint(dX, dY, BucketKernel().Max().Z())));
...
ICacheInterface& IElementSet::CacheInterface()
{
  return *m_cache_interface;
}

void IElementSet::CacheInterface(ICacheInterface &cache_interface)
{
  m_cache_interface = &cache_interface;
}
...
bool IElementSet::KnownPoint(const IPoint &point) const
{
  ...
  for(size_t i = 0; i < vcElements.size(); i++)
  {
    ...
    {
      if(&element.Point(j) == &point) return true;
    }
  }
