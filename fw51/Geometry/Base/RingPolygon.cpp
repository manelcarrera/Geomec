...
const IPoint &CRingPolygon::Point(int nIndex) const
{
  return m_factory.Point(m_ring[nIndex]);
}
void CRingPolygon::Point(int /*nIndex*/, const IPoint &/*pt*/)
{
  assert(false);
}

int CRingPolygon::NrOfPoints() const
{
  return (int)m_ring.size();
}

size_t CRingPolygon::Order() const
{
  return 1;
}
