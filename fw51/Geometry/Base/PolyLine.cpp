...
CPolyLine::CSegment::CSegment(IPoint& first, IPoint& second, int firstidx, int secondidx)
: m_line(&first, &second)
{
...
const IPoint& CPolyLine::CSegment::Point(int nIndex) const
{
  ...
  if(nIndex == 0) return *m_line.first;

  return *m_line.second;
}

std::vector<int> CPolyLine::CSegment::Nodes() const
{
  return m_NodeIndices;
}

size_t CPolyLine::CSegment::Order() const
{
  return 1;
}
...
void CPolyLine::AssertValid() const
{
...
IPoint &CPolyLine::PointAt(int nIndex)
{
  return *m_vcPoint[nIndex];
}
