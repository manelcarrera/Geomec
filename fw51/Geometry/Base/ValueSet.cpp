...
CValueSet::CValueSet(const IElementSet &elementset) : m_ElementSet(elementset) {}

CValueSet::CValueSet(const IElementSet &elementset, const std::vector<TValueVec> &values) : m_ElementSet(elementset) {
...
CValueSet::CValueSet(const CValueSet &rhs) : m_ElementSet(rhs.m_ElementSet), m_Values(rhs.m_Values) {}

CValueSet::~CValueSet() {}

// get the elementset this valueset belongs to (get the size there as well)
const IElementSet &CValueSet::ElementSet() const { return m_ElementSet; }
...
void CValueSet::ElementValues(TValueVec &values, int iElementIndex, IParallelInitializationCallback * /*cb*/) const {
  assert(m_Values.size() <= ElementSet().ElementSize()); // we should actually check that the difference equals number
                                                         // of interface elements, in case the elementset is the mesh
...
const std::vector<CValueSet::TValueVec> &CValueSet::valueSetValues() const { return m_Values; }

void CValueSet::valueSetValues(const std::vector<TValueVec> &values) { m_Values = values; }

// empty the set
void CValueSet::Empty() { m_Values.clear(); }

// check for emptyness
bool CValueSet::IsEmpty() const { return m_Values.empty(); }
...
void CValueSet::reserveSize(size_t size) { m_Values.reserve(size); }
