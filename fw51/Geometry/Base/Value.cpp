...
namespace geo {
static inline bool _isvalid(const double &v) { return !(isnan(v) || isinf(v)); }

//////////////////////////////////////////////////////////////////////
// Construction/Destruction
//////////////////////////////////////////////////////////////////////

CValue::CValue() : m_dValue(std::numeric_limits<double>::quiet_NaN()) { M_BVALID(false); }

CValue::CValue(const double &dValue) : m_dValue(std::numeric_limits<double>::quiet_NaN()) {
  M_BVALID(_isvalid(dValue));
  if (_isvalid(dValue))
    m_dValue = dValue;
}

CValue::CValue(const IValue &value) : m_dValue(std::numeric_limits<double>::quiet_NaN()) {
  M_BVALID(value.Valid());
  if (value.Valid())
    m_dValue = value.Value();
}

bool CValue::Valid() const { return _isvalid(m_dValue); }
