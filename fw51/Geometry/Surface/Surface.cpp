...
int CSurface::CFace::NrOfPoints() const
{
...
const IElementSet* CSurface::CFace::IndexingElementSet() const
{
	return &m_surface;
}
...
size_t CSurface::CFace::Order() const
{
	return 1;
}
...
CSurface::CSurface(const std::vector<geo::CPoint>& vcPoint, const std::vector<std::vector<int> >& vcTriangle)
{
	...
	for(int j = 0; j < vcTriangle.size(); j++)
	{
		...
		{
		case 3:
			AddTriangle(vcPoint[vcTriangle[j][0]], 
						vcPoint[vcTriangle[j][1]],
						vcPoint[vcTriangle[j][2]]);
			break;
		case 4:
			AddRectangle(vcPoint[vcTriangle[j][0]], 
						 vcPoint[vcTriangle[j][1]],
						 vcPoint[vcTriangle[j][2]],
		...
		default:
			m_vcFace.push_back(
                       new CFace(*this, (int)m_vcFace.size(), vcTriangle[j]) );
...
void CSurface::reserveFaceSize(size_t size)
{
...
int CSurface::FaceSize() const
{
	return (int)m_vcFace.size();
}

const IPoint& CSurface::Point(int nIndex) const
{
	return *m_vcPoint[nIndex];
}
