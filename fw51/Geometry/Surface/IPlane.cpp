...
double IPlane::Distance(const IPoint &vertex) const
{
	return vertex.Distance(Project(vertex));
}


double IPlane::SquareDistance(const IPoint &vertex) const
{
	return vertex.SquareDistance(Project(vertex));
}
...
bool IPlane::IsParallel(const ILine &line) const
{
	return IsParallel(CVector(line.Second() - line.First()));
}
...
CPoint IPlane::RealIntersection(const ILine &line, bool bIncP1, bool bIncP2) const
{
	...
	if(ptRet.Empty()) return ptRet; // parallel

	if(ptRet == line.First())
	{
		if(bIncP1) return ptRet;
		return CPoint();
	}
	if(ptRet == line.Second())
	{
		if(bIncP2) return ptRet;
		return CPoint();
	}

	if(line.Contains(ptRet, false)) return ptRet;
	return CPoint();
}
