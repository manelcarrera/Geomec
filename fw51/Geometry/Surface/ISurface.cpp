...
const IElement &ISurface::Element(int nIndex) const
{
  return Face(nIndex);
}

int ISurface::ElementSize() const
{
  return FaceSize();
}
...
void ISurface::AssertValid() const
{
  assert(Empty() || PointSize() != 0);
}

void ISurface::InsertPointsIntoMapAndSet(TPointPairSet &stPointPairs,
                                         TPointMap &mpFaces,
                                         const IPoint &pt1,
                                         const IPoint &pt2,
                                         const IFace &face) const
{
  assert(!pt1.Empty());
  assert(!pt2.Empty());
...
CPolyLine ISurface::SortedEdge() const
{
  ...
  for(size_t i = 1; i < arLines.Size(); i++) stLines.insert(&arLines.Object(i));
...
std::vector<std::vector<const IPoint *> > ISurface::SortedEdgePoints(bool bRetry, double dEpsilon) const
{
  ...
  if(vcFaces.size() == 1)
  {
      ...
      TEdgePointMap::iterator it = mpEdgePoints.insert(std::make_pair(&pPair->First(), std::vector<const IPoint *>())).first;
...
bool ISurface::LocateRingIndex(const IPoint& point, std::set<const IPoint*>& stEncountered, std::map<const IPoint*, int>& ringmap) const
{
  // are we there?
...
const IPoint &ISurface::CPointPair::First() const
{
  return *m_p1;
}

const IPoint &ISurface::CPointPair::Second() const
{
  return *m_p2;
}
...
{
  int i;
  for(i = 0; i < m_vcEdgeNearestNeighbour.size(); i++) delete m_vcEdgeNearestNeighbour[i];
...
std::multimap<double, const IPoint*> ISurface::NearestXYPoints(int size_x, int size_y, const double& dX, const double& dY) const
{
...
void ISurface::FilterElementsInOrthoBox( const geo::IPoint& ptMin,
