...
CTriSurface::CTriSurface(const ISurface &rhs) : m_vcPoint(rhs.PointSize()), m_vcFace(rhs.FaceSize()) {
  ...
  for (int j = 0; j < rhs.FaceSize(); j++) {
    if (bIndexed) {
      ...
      m_vcFace[j] =
          new CTriangle(*this, j, rhs.Face(j).PointIndex(0), rhs.Face(j).PointIndex(1), rhs.Face(j).PointIndex(2));
...
CTriSurface::~CTriSurface() { Clear(); }

void CTriSurface::InsertFaceIntoPointMap(CTriangle &newface) {
  TPointToFacesMap::iterator it;
  it = m_mpPointToFaces.insert(TPointToFacesMap::value_type(&m_vcPoint[newface.PointIndex(0)], CPtrArray<IFace>()))
           .first;
  it->second.PushBack(newface);
  it = m_mpPointToFaces.insert(TPointToFacesMap::value_type(&m_vcPoint[newface.PointIndex(1)], CPtrArray<IFace>()))
           .first;
  it->second.PushBack(newface);
  it = m_mpPointToFaces.insert(TPointToFacesMap::value_type(&m_vcPoint[newface.PointIndex(2)], CPtrArray<IFace>()))
           .first;
  it->second.PushBack(newface);
}
...
int CTriSurface::FaceSize() const { return (int)m_vcFace.size(); }

const IPoint &CTriSurface::Point(int nIndex) const { return m_vcPoint[nIndex]; }
...
int CTriSurface::PointSize() const { return (int)m_vcPoint.size(); }
