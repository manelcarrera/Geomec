...
// Rectangle dragger
//////////////////////////////////////////////////////////////////////
CRectangleDragger::CRectangleDragger(TFrame& frame, const geo::IPlane& plane)
:CDragger(frame, plane)
{
}
...
void CRectangleDragger::DrawScene()
{
	if(IsDragging())
	{
		if((BeginScreenPoint().x() == CurrentScreenPoint().x()) || 
		   (BeginScreenPoint().y() == CurrentScreenPoint().y()))
			return;
...
geo::CRectangle CRectangleDragger::Rectangle() const
{
	...
	if(!Plane().Empty())
	{
		geo::CLine l1(Frame().UnProject(BeginScreenPoint(),0),
					 Frame().UnProject(BeginScreenPoint(),1));
...
bool CRectangleModifier::Dragging() const
{
	return !m_DragPoint.Empty();
}
...
bool CRectangleModifier::OnOK()
{
	return true;
}

geo::CRectangle CRectangleModifier::GetModifiedRectangle()
{
	return m_Rectangle;
}
...
bool CRectangleModifier::MouseDblClk(TKeyboardModifiers /*state*/, TMouseButton /*button*/, const TScreenPoint& /*point*/)
{
	return true;
}
...
bool CRectangleModifier::MouseRelease(TKeyboardModifiers /*state*/, TMouseButton button, const TScreenPoint& /*point*/)
{
	if(button == Qt::LeftButton)
	{
		if(Dragging())
