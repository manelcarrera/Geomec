...
COpenGLDraw::COpenGLDraw(IOpenGLFrame &frame) : m_DrawState(CLOSED), m_frame(frame) {}
...
void COpenGLDraw::SetColor(TColor color) {
  glColor4f(qRed(color) / 256.0, qGreen(color) / 256.0, qBlue(color) / 256.0, qAlpha(color) / 256.0);
}

void COpenGLDraw::DrawVertex(const geo::IPoint &point) { glVertex3f(point.X(), point.Y(), point.Z()); }
...
void COpenGLDraw::DrawBodyGroup(const geo::CBodyGroup &BodyGroup, const IDrawDef &drawdef, bool bDrawSteps) const {
  ...
  if (bDrawSurfaces) {
    ...
    for (int i = 0; i < BodyGroup.SideSurfaceSize(); i++)
      DrawSurface(BodyGroup.SideSurface(i), drawdef, bDrawSteps);
  } else {
    // we need to draw all the bodies
    for (int i = 0; i < BodyGroup.BodySize(); i++)
      DrawBody(BodyGroup.Body(i), drawdef, bDrawSteps);
  }
}
...
void COpenGLDraw::DrawPoint(const geo::IPointElement &element, const IDrawDef &drawdef, bool bDrawSteps) const {
  ...
  glPointSize(6.0);
  if (!bPointSmooth)
    glEnable(GL_POINT_SMOOTH); // Draw small filled circles instead of squares
...
void COpenGLDraw::DrawLine(const geo::ILine &line, const IDrawDef &drawdef, bool bDrawSteps) const {
  ...
  if (!drawdef.CurvedDraw()) {
    ...
    default:; //! \todo support lines with 5 nodes?
    }
    ...
    case 2: {
      ...
      if (drawdef.PolyFillBack() || drawdef.PolyFillFront()) {
        ...
        for (int i = 1; i <= CURVE_SEGMENTATION; i++) {
          OpenDrawState(TRIANGLES);
          glEvalCoord2d((double)i / (double)CURVE_SEGMENTATION,
                        (double)(CURVE_SEGMENTATION - i) / (double)CURVE_SEGMENTATION);
          glEvalCoord2d((double)(i - 1) / (double)CURVE_SEGMENTATION,
                        (double)(CURVE_SEGMENTATION - i) / (double)CURVE_SEGMENTATION);
          glEvalCoord2d((double)(i - 1) / (double)CURVE_SEGMENTATION,
                        (double)(CURVE_SEGMENTATION - i + 1) / (double)CURVE_SEGMENTATION);
          CloseDrawState();
        }
        ...
        OpenDrawState(LINES);
        for (int i = 0; i <= CURVE_SEGMENTATION; i++) {
          glEvalCoord2d((double)(CURVE_SEGMENTATION - i) / (double)CURVE_SEGMENTATION,
                        (double)i / (double)CURVE_SEGMENTATION);
        }
        CloseDrawState();
      }
...
void COpenGLDraw::DrawBody(const geo::IBody &body, const IDrawDef &drawdef, bool bDrawSteps) const {
  ...
  for (int i = 0; i < body.NrOfFaces(); i++) {
    ...
    if (body.Face(i).Order() == 1)
      SetNormal(body.Face(i).Normal()); // wjrx mantis 2405
...
//		else
//			m_pView->glPrintRotate(lbp.X() - offsetX, lbp.Y() - offsetY, lbp.Z(), lbp.GetRotate(), drawdef.Font(),
// lbp.Label()); 		break; 	case CLabelPoint::LA_BOTTOM_RIGHT://LA_BOTTOM_RIGHT 		if (lbp.GetRotate() == 0)
...
//		else
//			m_pView->glPrintRotate(lbp.X() - .5*offsetX, lbp.Y() - 0.5*offsetY, lbp.Z(), lbp.GetRotate(), drawdef.Font(),
// lbp.Label()); 		break;
//
//	}
//
...
void COpenGLDraw::SetOpenGLAtrib(const IDrawDef &drawdef) const {
  GLbitfield mask = GL_DEPTH_BUFFER_BIT | GL_ENABLE_BIT | GL_LINE_BIT | GL_POLYGON_BIT | GL_TRANSFORM_BIT |
                    GL_STENCIL_BUFFER_BIT | GL_VIEWPORT_BIT;
...
void COpenGLDraw::DrawSurfaceFace(const geo::IFace &face, const IDrawDef &drawdef, TPointNormalMap &mpPointNormal,
                                  bool bNeedNormals, bool bPlanar, geo::IVector &Normal) const {
...
// In this function the right colors are selected to draw the surface with and the mean normal in each point is
// calculated for shading effects.
...
*/
void COpenGLDraw::DrawInterface(const geo::IInterfaceElement &interfaceElement, const IDrawDef &drawdef,
                                bool bDrawSteps) {
...
void COpenGLDraw::DrawPolygonArr(const geo::CPolygonPtrArray &poly_arr, const IDrawDef &drawdef,
                                 bool bDrawSteps) const {
...
void COpenGLDraw::DrawSymbol(const ISymbol &symbol, const IDrawDef &drawdef, bool bDrawSteps) {
  ...
  GLbitfield mask =
      GL_DEPTH_BUFFER_BIT | GL_ENABLE_BIT | GL_LINE_BIT | GL_POLYGON_BIT | GL_TRANSFORM_BIT | GL_VIEWPORT_BIT;
  ...
  for (i = 0; i < symbol.NrOfPositions(); i++) {
    ...
    gluUnProject(root_point.x(), root_point.y(), vcPoint[i].second, ModelMatrix, ProjectionMatrix, ViewPort, &dX, &dY,
                 &dZ);
