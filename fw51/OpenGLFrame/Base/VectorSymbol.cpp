...
CVectorSymbolBase::CVectorSymbolBase(const geo::IPoint &location, const geo::IVector &dir, const double &length,
...
CVectorSymbolBase::~CVectorSymbolBase() { m_geometry.ClearAndDelete(); }

int CVectorSymbolBase::NrOfPositions() const { return 1; }
...
const geo::IArray &CVectorSymbolBase::Geometry() const { return m_geometry; }

const geo::IPoint &CVectorSymbolBase::PivotPoint() const { return m_pivot; }
...
/*virtual*/
bool CVectorSymbolBase::Empty() const { return (m_pivot.Empty() || m_geometry.Empty() || m_location.Empty()); }
...
/*virtual*/ void CVectorSymbolBase::Move(const geo::IVector &vec) { m_location.Move(vec); }

/*virtual*/ void CVectorSymbolBase::Transform(const geo::IMatrix &matrix) { m_location.Transform(matrix); }

void CVectorSymbol::CreateGeometry(const geo::IVector &vector) {
  geo::CLine *line = new geo::CLine(geo::CPoint::NullPoint, vector);
  geo::CConus *conus =
      new geo::CConus(line->Point(1) - vector.UnitVector() * CONEHEIGHT, vector, CONESIZE, CONEHEIGHT, 0);
...
CVectorSymbol::CVectorSymbol(const geo::IPoint &location, const geo::IVector &dir, const double &length,
                             bool TailIsPivot)
