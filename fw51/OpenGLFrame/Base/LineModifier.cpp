  ...
  m_Plane(plane)
{
	...
	if(!m_OriginalLine.Empty()) m_pModifiedLine = new CModifierLine(linepoint1, linepoint2);
...
{
	if(m_pModifiedLine) delete m_pModifiedLine;
}

bool CLineModifier::Dragging() const
{
	return (m_pDragPoint != 0);
}

bool CLineModifier::OnOK()
{
	return true;
}
...
//// CModifierLine
CLineModifier::CModifierLine::CModifierLine(const geo::IPoint& pt1, const geo::IPoint& pt2)
: m_pt1(pt1), m_pt2(pt2)
{
}
...
bool CLineModifier::CModifierLine::Empty() const
{
	return (m_pt1.Empty() || m_pt2.Empty());
}
