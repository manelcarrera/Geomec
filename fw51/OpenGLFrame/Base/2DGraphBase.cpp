...
//////////////////////////////////////////////////////////////////////
C2DGraphBase::CDisplayPair::CDisplayPair(C2DGraphBase& parent)
:m_parent(parent)
{
}
...
geo::CPtrArray<geo::IObject>& C2DGraphBase::CDisplayPair::Array() 
{
  return m_array;
}

CDrawDef& C2DGraphBase::CDisplayPair::DrawDef()
{
  return m_drawdef;
}

void C2DGraphBase::CDisplayPair::AddObject(geo::IObject & Object)
{
...
: m_dMinX(DBL_MAX),
  ...
  m_bAutoX(true),
  m_bAutoY(true),
  m_DragLineColor(qRgb(255,255,255)),
  m_bMarkerEnable(true), 
  m_InvalidSectionColor(qRgb(90,90,90)),
  m_bInvalidSectionEnable(true),
...
{
  RemoveAllObjects();
  while ( !m_vcGraphLine.isEmpty() ) delete m_vcGraphLine.takeLast();
}
...
void C2DGraphBase::InsertPolygon(int nMinIndex,
                 int nMaxIndex,
                 TColor color,
                 const double& dEps,
                 bool bCutOnEps)
...
int C2DGraphBase::LineSize() const
{
  return m_vcGraphLine.count();
}

const C2DGraphLineBase &C2DGraphBase::Line(int nIndex) const
{
  return *m_vcGraphLine.at(nIndex);
}

C2DGraphLineBase &C2DGraphBase::Line(int nIndex)
{
  return *m_vcGraphLine.at(nIndex);
}
...
void C2DGraphBase::UpdateGraph()
{
  ...
  {
    FillPolygon(Line(m_vcPolygonLine[i].first.first),
          Line(m_vcPolygonLine[i].first.second),
          m_vcPolygonLine[i].second.first,
          m_vcPolygonLine[i].second.second.first,
          m_vcPolygonLine[i].second.second.second);
  }
}

C2DGraphBase::TColor C2DGraphBase::DragLineColor() const
{
  return m_DragLineColor;
}

void C2DGraphBase::DragLineColor(const TColor & Color)
{
  m_DragLineColor = Color;
}
...
bool C2DGraphBase::MarkersEnabled() const
{
  return m_bMarkerEnable;
}
...
double C2DGraphBase::MaxX() const
{
  return m_dMaxX;
}

double C2DGraphBase::MinX() const
{
  return m_dMinX;
}

double C2DGraphBase::GridX() const
{
  return m_dGridX;
}

double C2DGraphBase::GridY() const
{
  return m_dGridY;
}
...
void C2DGraphBase::SetupModelMatrix() const
{
  IOpenGLFrame::SetupModelMatrix();


}

//wjrx mantis 2559
bool C2DGraphBase::AutoX() const
{
  return m_bAutoX;
}

//wjrx mantis 2559
bool C2DGraphBase::AutoY() const
{
  return m_bAutoY;
}

//wjrx mantis 2559
void C2DGraphBase::AutoX(bool bAutoX )
{
  m_bAutoX = bAutoX;
}

//wjrx mantis 2559
void C2DGraphBase::AutoY(bool bAutoY )
{
  m_bAutoY = bAutoY;
}
...
geo::CValue C2DGraphBase::MaxY() const
{
  return m_dMaxY;
}

geo::CValue C2DGraphBase::MinY() const
{
  return m_dMinY;
}
...
void C2DGraphBase::FillPolygon(const C2DGraphLineBase::TSerie& first_serie, 
                 const C2DGraphLineBase::TSerie& second_serie,
                 TColor color,
                 const double& dEps,
                 bool bCutOnEps)
{
  ...
  for(cur_it = mpPoint.begin(); cur_it != mpPoint.end(); cur_it++)
  {
    ...
    if(cur_it->second.first.size() == 0)
    {
      ...
      while((next_it != mpPoint.end()) && (next_it->second.first.size() == 0)) next_it++;
    ...
    if(cur_it->second.second.size() == 0)
    {
      // Search for next value
      TPointMap::iterator next_it = cur_it;
      while((next_it != mpPoint.end()) && (next_it->second.second.size() == 0)) next_it++;
...
void C2DGraphBase::FillPolygon(const C2DGraphLineBase& min_line, 
                 const C2DGraphLineBase& max_line, TColor color, 
                 const double& dEps,
                 bool bCutOnEps)
{
  ...
  for(size_t nFirst = 0; nFirst < first_serie.size(); nFirst++)
  {
    ...
    {
      FillPolygon(first_serie[nFirst],
            second_serie[nSecond],
            color,
            dEps,
            bCutOnEps);
    }
...
void C2DGraphBase::EnableInvalidSections(bool bEnable)
{
  m_bInvalidSectionEnable = bEnable;
}

bool C2DGraphBase::InvalidateSections() const
{
  return m_bInvalidSectionEnable;
}

C2DGraphBase::TColor C2DGraphBase::InvalidSectionColor() const
{
  return m_InvalidSectionColor;
}

void C2DGraphBase::InvalidSectionColor(TColor color)
{
  m_InvalidSectionColor = color;
}

void C2DGraphBase::UpdateGraphLayout(bool /*bEditLimits*/)
{
  // wjrx mantis 3383
  if ( m_nDivisionX< 2 ) m_nDivisionX= 2;
  ...
  */
  ...
  if ( m_bAutoY ) // wjrx mantis 2559
  {
    ...
    if ( ! m_dMinY.Valid() ) m_dMinY.Value(0);
    if ( ! m_dMaxY.Valid() ) m_dMaxY.Value(0);
  ...
  // wjrx mantis 3383
  if ( m_nDivisionY< 2 ) m_nDivisionY= 2;
...
bool C2DGraphBase::MouseMove(TKeyboardModifiers /*state*/, TMouseButton button, const TScreenPoint& point)
{
  ...
  if(prHit.first > -1)
  {
    ...
    //
    int deltaX = point.x()
               + QFontMetrics( QFont() ).size( 0, pMarker->Label() ).width()
         - ViewPortWidth()
         + 5; //pixels
    int deltaY = point.y()
               + QFontMetrics( QFont() ).size( 0, pMarker->Label() ).height()
         - ViewPortHeight()
               + 5; //pixels
  ...
  if(MarkersEnabled() && MaxY().Valid() && MinY().Valid())
  {
    ...
    for(int iLine = 0; iLine < LineSize(); iLine++)
    {
      for(int iPoint = 0; iPoint < Line(iLine).PointSize(); iPoint++)
      {
        ...
        if(value.Valid())
        {
          if((fabs(dY - value.Value()) < dEPSY) &&
             (fabs(dX - Line(iLine).X(iPoint)) < dEPSX))
          {
...
void C2DGraphBase::DefineAxis(double *Min, double *Max, double *Inc,int MinTicks)
{
  ...
  //
  myInc= 1;
  if ( normInc > myInc ) myInc= 2;
  if ( normInc > myInc ) myInc= 3;
  if ( normInc > myInc ) myInc= 4;
  if ( normInc > myInc ) myInc= 5;
  if ( normInc > myInc ) myInc= 6;
  if ( normInc > myInc ) myInc= 7;
  if ( normInc > myInc ) myInc= 8;
  if ( normInc > myInc ) myInc= 9;
  if ( normInc > myInc ) myInc= 10;
...
C2DGraphBase::TColor C2DGraphBase::GetGraphBorderColor() const
{
  return qRgb(0,250,0);
}

C2DGraphBase::TColor C2DGraphBase::GetGraphGridColor() const
{
  return qRgb(150,150,150);
}

C2DGraphBase::TColor C2DGraphBase::GetGraphTextColor() const
{
