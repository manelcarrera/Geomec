...
C2DGraphLineBase::C2DGraphLineBase(C2DGraphBase &parent, unsigned long color, bool bLineStipple)
    : m_parent(parent), m_Color(color), m_LineWidth(1), m_bLineStipple(bLineStipple) {
  parent.m_vcGraphLine.append(this);
}

C2DGraphLineBase::C2DGraphLineBase(C2DGraphBase &parent, const QString &description, unsigned long color,
                                   bool bLineStipple)
    : m_parent(parent), m_Color(color), m_LineWidth(1), m_Description(description), m_bLineStipple(bLineStipple) {
  parent.m_vcGraphLine.append(this);
}
  ...
  if (m_parent.InvalidateSections()) {
    if (vcSerie.size() > 0) {
      ...
      if (MinX() < vcSerie[0][0].first.X()) {
        if (geo::CRectangle::ValidRectangleData(geo::CPoint(MinX(), m_parent.MinY().Value()),
                                                geo::CPoint(vcSerie[0][0].first.X(), m_parent.MaxY().Value()))) {
          if (!pSectionPair)
            pSectionPair = m_parent.CreateNewPair();
          pSectionPair->AddObject(*new geo::CRectangle(geo::CPoint(MinX(), m_parent.MinY().Value()),
                                                       geo::CPoint(vcSerie[0][0].first.X(), m_parent.MaxY().Value())));
        }
      }
      ...
      for (size_t i = 0; i < (vcSerie.size() - 1); i++) {
        ...
        if (fabs(dXStart - dXEnd) > EPS) {
          if (geo::CRectangle::ValidRectangleData(geo::CPoint(dXStart, m_parent.MinY().Value()),
                                                  geo::CPoint(dXEnd, m_parent.MaxY().Value()))) {
            if (!pSectionPair)
              pSectionPair = m_parent.CreateNewPair();
            pSectionPair->AddObject(*new geo::CRectangle(geo::CPoint(dXStart, m_parent.MinY().Value()),
                                                         geo::CPoint(dXEnd, m_parent.MaxY().Value())));
          }
        }
      }
      ...
      if (dLastX < MaxX()) {
        ...
        if (geo::CRectangle::ValidRectangleData(geo::CPoint(dLastX, m_parent.MinY().Value()),
                                                geo::CPoint(MaxX(), m_parent.MaxY().Value()))) {
          if (!pSectionPair)
            pSectionPair = m_parent.CreateNewPair();
          pSectionPair->AddObject(*new geo::CRectangle(geo::CPoint(dLastX, m_parent.MinY().Value()),
                                                       geo::CPoint(MaxX(), m_parent.MaxY().Value())));
        }
      }
    }
...
double C2DGraphLineBase::MaxX() const { return Parent().MaxX(); }

double C2DGraphLineBase::MinX() const { return Parent().MinX(); }
...
enum C2DGraphLineBase::ePointStatus C2DGraphLineBase::Status(const double &dMin, const double &dMax,
                                                             const geo::CValue &value) const {
...
unsigned long C2DGraphLineBase::Color() const { return m_Color; }

//////////////////////////////////////////////////////////////////////

void C2DGraphLineBase::Color(unsigned long color) { m_Color = color; }
//////////////////////////////////////////////////////////////////////

unsigned long C2DGraphLineBase::LineWidth() const { return m_LineWidth; }
//////////////////////////////////////////////////////////////////////

void C2DGraphLineBase::LineWidth(unsigned long linewidth) { m_LineWidth = linewidth; }

//////////////////////////////////////////////////////////////////////

bool C2DGraphLineBase::LineStipple() const { return m_bLineStipple; }

void C2DGraphLineBase::LineStipple(bool bLinestipple) { m_bLineStipple = bLinestipple; }
...
C2DGraphBase &C2DGraphLineBase::Parent() { return m_parent; }

const C2DGraphBase &C2DGraphLineBase::Parent() const { return m_parent; }

const QString &C2DGraphLineBase::Description() const { return m_Description; }

void C2DGraphLineBase::Description(const QString &description) { m_Description = description; }
