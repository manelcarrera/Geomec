...
IOpenGLFrame::IOpenGLFrame() : m_pParent(0), m_ViewPort(0), m_bLightingEnabled(false), m_updateCount(0) {
...
IOpenGLFrame *IOpenGLFrame::Parent() { return m_pParent; }
...
/*!
  ...
  \sa Root(), ReParent()
*/
const IOpenGLFrame *IOpenGLFrame::Parent() const { return m_pParent; }

/*!
  Returns the root parent, the parent with Parent() == 0, of the current frame. Note the frame itself could also be the
  root.
...
/*!
  ...
  The displaylist is deleted with DeleteList(). Note that by detaching the frame with device context FlushDisplayList()
  is called and existing lists should be deleted. This function can be used in the DrawScene() override.
...
/*!
  DeleteList() deletes the displaylist. Note that by detaching the frame with device context FlushDisplayList() is
  called and existing lists should be deleted. The displaylist can be generated with the CompileList() function.
  DrawList() draws an existing displaylist.
...
void IOpenGLFrame::RenderText(const double &dX, const double &dY, const double &dZ, const QString &text,
                              const QFont &fnt) {
  assert(Parent());
...
*/
int IOpenGLFrame::ChildSize() const { return (int)m_vcChild.size(); }

/*!
  Returns the child by index. The index is zero based.
*/
const IOpenGLFrame &IOpenGLFrame::Child(int nIndex) const { return *m_vcChild[nIndex]; }

/*!
  Returns the child by index. The index is zero based.
*/
IOpenGLFrame &IOpenGLFrame::Child(int nIndex) { return *m_vcChild[nIndex]; }
...
geo::CPoint IOpenGLFrame::UnProject(const TScreenPoint &screen_point, const double &dDepth) const {
  ...
  gluUnProject(root_point.x(), root_point.y(), dDepth, m_ModelMatrix, m_ProjectionMatrix, m_ViewPortMatrix, &dX, &dY,
               &dZ);
...
IOpenGLFrame::TScreenPointDepth IOpenGLFrame::Project(const geo::IPoint &world_point) const {
  ...
  gluProject(world_point.X(), world_point.Y(), world_point.Z(), m_ModelMatrix, m_ProjectionMatrix, m_ViewPortMatrix,
             &dX, &dY, &dZ);
...
/*!
  Function returns when frame align to the parent size. When frame has a fixed size align to parent function return
  false.

  \sa SetViewPort(), RestoreAlignToParent()
*/
bool IOpenGLFrame::AlignToParent() const { return m_ViewPort == 0; }
...
/*!
  Sets the viewport of the frame. AlignToParent() will return false. Automatic alignment can be restored by calling
  RestoreAlignToParent().
...
void IOpenGLFrame::OnBeginDraw(int nSteps) {
  if (Parent())
    Parent()->OnBeginDraw(nSteps);
}

void IOpenGLFrame::OnDrawStep(int nSteps) {
  if (Parent())
    Parent()->OnDrawStep(nSteps);
}
...
*/
void IOpenGLFrame::DrawStep(int nSteps) const { const_cast<IOpenGLFrame &>(*this).OnDrawStep(nSteps); }
