...
///////////////////////////////////////////////////////////////////////
bool CInnerConstraint::ValidatePoint(const geo::IPoint &pt) const { return !m_constraint.Contains(pt, true); }

bool COuterConstraint::ValidatePoint(const geo::IPoint &pt) const { return m_constraint.Contains(pt, true); }
...
CMouseListener::CMouseListener(TFrame &frame) : m_frame(frame), m_bDeleting(false) {
...
int CMouseListener::ConstraintSize() const { return (int)m_vcConstraints.size(); }
...
geo::CPoint CMouseListener::OnSetDragPoint(const geo::IPoint &ptDrag) { return ptDrag; }

void CMouseListener::PushBack(CConstraint *ptr) { m_vcConstraints.push_back(ptr); }
...
bool CMouseListener::OnCancel() { return true; }

// Pre	: OK() is called
// Post	: Return value
//			TRUE	Mouselistener is deleted
//			FALSE	Mouselistener is not deleted OK is not procedeed
bool CMouseListener::OnOK() { return true; }

// Pre	: Delete() is called
// Post	: Return value
//			TRUE	Mouselistener is deleted
//			FALSE	Mouselistener is not deleted operation Delete is not procedeed
bool CMouseListener::OnDelete() { return true; }

const CMouseListener::TFrame &CMouseListener::Frame() const { return m_frame; }

CMouseListener::TFrame &CMouseListener::Frame() { return m_frame; }

bool CMouseListener::MousePress(TKeyboardModifiers /*state*/, TMouseButton /*button*/, const TScreenPoint & /*point*/) {
  return false;
}

bool CMouseListener::MouseRelease(TKeyboardModifiers /*state*/, TMouseButton /*button*/,
                                  const TScreenPoint & /*point*/) {
  return false;
}

bool CMouseListener::MouseDblClk(TKeyboardModifiers /*state*/, TMouseButton /*button*/,
                                 const TScreenPoint & /*point*/) {
  return false;
}
