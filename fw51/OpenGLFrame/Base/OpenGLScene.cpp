...
: m_dPointSize(5.0),
  ...
  m_bDisplayPerspective(FALSE),
  m_bAllowDynamicRotate(TRUE),
  m_bAllowDynamicZoom(TRUE),
  m_bAllowDynamicPan(TRUE),
  m_bProportional(true),
  m_bUseFixedBoundingBox(false),
  m_dimension(DIM_3D),
  m_ScaleVector(geo::CVector(1,1,1)),
  m_DisplacementVector(geo::CVector::NullVector),
  m_ViewVector(geo::CVector::Zaxis),
  m_UpVector(geo::CVector::Yaxis),
  m_RotateLockVector( geo::CVector::NullVector),
  m_ModelScaleVector(geo::CVector(1,1,1)),
  m_pAxis(0),
...
void COpenGLScene::InvalidateMaxMin()
{
  m_bRecalcMaxMin = true;
}
...
void COpenGLScene::ViewVector(const geo::IVector &vec)
{
  m_ViewVector=vec;
}

const geo::CVector &COpenGLScene::ViewVector() const
{
  return m_ViewVector;
}

void COpenGLScene::UpVector(const geo::CVector &vec)
{
  m_UpVector=vec;
}

const geo::CVector &COpenGLScene::UpVector() const
{
  return m_UpVector;
}

bool COpenGLScene::DisplayBoundingBox() const
{
  return m_bDisplayBoundingBox;
}
void COpenGLScene::DisplayBoundingBox(bool newval)
{
  m_bDisplayBoundingBox=newval;

}


bool COpenGLScene::DisplayAxis() const
{
  return Axis().Visible();
}

void COpenGLScene::DisplayAxis(bool newval)
{
  Axis().Visible(newval);
}
...
COpenGLScene::TColor COpenGLScene::BackgroundColor() const
{
  return m_BackgroundColor;
}

/*virtual*/ void COpenGLScene::BackgroundColor(TColor color)
{
...
COpenGLScene::TColor COpenGLScene::AxisColor() const
{
  return Axis().AxisColor(geo::CVector::Xaxis);
}
...
COpenGLScene::DIMENSION COpenGLScene::Dimension() const
{
  return m_dimension;
}

bool COpenGLScene::DisplayPerspective() const
{
  return m_bDisplayPerspective;
}

void COpenGLScene::DisplayPerspective(bool newval)
{
  m_bDisplayPerspective=newval;
}

const geo::CVector &COpenGLScene::ModelScaleVector() const
{
...
const geo::CVector &COpenGLScene::ScaleVector() const
{
  return m_ScaleVector;
}

void COpenGLScene::ScaleVector(const geo::CVector &newval)
{
  m_ScaleVector=newval;

}


void COpenGLScene::PointSize(const double &dNewSize)
{
  m_dPointSize=dNewSize;
}

const double &COpenGLScene::PointSize() const
{
  return m_dPointSize;
}

void COpenGLScene::AllowDynamicZoom(bool newval)
{
  m_bAllowDynamicZoom=newval;
}

bool COpenGLScene::AllowDynamicZoom() const
{

  return m_bAllowDynamicZoom;
}

const geo::CVector &COpenGLScene::DisplacementVector() const
{
  return m_DisplacementVector;
}

void COpenGLScene::DisplacementVector(const geo::CVector &newval)
{
  m_DisplacementVector=newval;
}

bool COpenGLScene::AllowDynamicPan() const
{
  return m_bAllowDynamicPan;
}

void COpenGLScene::AllowDynamicPan(bool newval)
{
  m_bAllowDynamicPan=newval;
}
...
void COpenGLScene::ResetScale()
{
  m_ScaleVector = geo::CVector(1,1,1);
}

void COpenGLScene::ResetPan()
{
  m_DisplacementVector=geo::CVector::NullVector;
...
void COpenGLScene::RotateLockVector(const geo::CVector& vec)
{
  m_RotateLockVector=vec;
}


const geo::CVector &COpenGLScene::RotateLockVector() const
{
  return m_RotateLockVector;
}
...
: QObject(),  // QObject(const QObject&) is private,
  ...
  m_BackgroundColor(rhs.BackgroundColor()),
...
{
  return ((LightingEnabled() == rhs.LightingEnabled()) &&
      (PointSize() == rhs.PointSize()) &&
      (AxisColor() == rhs.AxisColor()) &&
      (BackgroundColor() == rhs.BackgroundColor()) &&
      (DisplayAxis() == rhs.DisplayAxis()) &&
      (DisplayBoundingBox() == rhs.DisplayBoundingBox()) &&
      (DisplayPerspective() == rhs.DisplayPerspective()) &&
      (AllowDynamicRotate() == rhs.AllowDynamicRotate()) &&
      (AllowDynamicZoom() == rhs.AllowDynamicZoom()) &&
      (AllowDynamicPan() == rhs.AllowDynamicPan()) &&
      (ScaleVector() == rhs.ScaleVector()) &&
      (DisplacementVector() == rhs.DisplacementVector()) &&
      (ViewVector() == rhs.ViewVector()) &&
      (UpVector() == rhs.UpVector()) &&
      (RotateLockVector() == rhs.RotateLockVector()) &&
      (Dimension() == rhs.Dimension()));// &&
...
void COpenGLScene::GeometryObjectRemoved(const geo::IObject * pObject)
{
  erase(*pObject);
}
...
bool COpenGLScene::DynamicZoomMouseState( TKeyboardModifiers  modifiers,
                                          TMouseButton        button,
