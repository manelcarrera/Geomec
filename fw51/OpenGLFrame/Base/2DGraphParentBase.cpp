...
void C2DGraphParentBase::Clear() { Graph().Clear(); }

double C2DGraphParentBase::TextOffset() const { return 20; }

double C2DGraphParentBase::Offset() const { return 10; }

void C2DGraphParentBase::UpdateGraph() { Graph().UpdateGraph(); }
...
QString C2DGraphParentBase::XLabel() const { return QString(); }

QString C2DGraphParentBase::XUnit() const { return QString(); }

QString C2DGraphParentBase::YLabel() const { return QString(); }

QString C2DGraphParentBase::YUnit() const { return QString(); }

void C2DGraphParentBase::TickX(const unsigned int t) { Graph().TickX(t); }

void C2DGraphParentBase::TickY(const unsigned int t) { Graph().TickY(t); }

void C2DGraphParentBase::SetMinMaxX(const double &dMinX, const double &dMax) { Graph().SetMinMaxX(dMinX, dMax); }
...
void C2DGraphParentBase::DrawXLabels() {
  if ((Graph().LineSize() > 0) && (Graph().MinX() < Graph().MaxX()) && (Graph().MaxY().Valid()) &&
      (Graph().MinY().Valid())) {
...
void C2DGraphParentBase::DrawAxisLabel(const QString sLabel, TAxis axis, QFont font) {
  //
  double x, y;
  double dRotate = 0;
  switch (axis) {
...
void C2DGraphParentBase::DrawLabel(const QString &sLabel, const double &dFraction, TDirection direction, TAxis axis) {
  double dRotate = 0;
  ...
  case AxisTop:
    ...
    y = (int)(Graph().ViewPortY() + Graph().ViewPortHeight() + 0.75 * TextOffset());
    break;
  case AxisBottom:
    assert((direction == LeftRight) || (direction == RightLeft));
    align = CLabelPoint::LA_CENTER;
...
int C2DGraphParentBase::ScaleFactor(const double &Maximum, const double &Minimum) {
  ...
  if ((Maximum != 0) && (range / fabs(Maximum) < 1e-4))
    return 0;
  if ((Minimum != 0) && (range / fabs(Minimum) < 1e-4))
    return 0;
...
void C2DGraphParentBase::OnResizeParent() {
  ...
  Graph().SetViewPort(int(dLeftOffset), int(dBottomOffset), int(ViewPortWidth() - dRightOffset - dLeftOffset),
                      int(ViewPortHeight() - dTopOffset - dBottomOffset));
}

void C2DGraphParentBase::AdaptOffset(TAxis axis, double &dLeftOffset, double &dRightOffset, double &dTopOffset,
...
void C2DGraphParentBase::SetupProjectionMatrix() const { glOrtho(0, ViewPortWidth(), 0, ViewPortHeight(), 0, 1); }
