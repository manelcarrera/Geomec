...
: legend_square(frame),
  m_label(0, 0, 0, text, QFont(), CLabelPoint::LA_TOP_LEFT),
  m_size_x(size_x),
  m_size_y(size_y),
  m_color(color)
{
...
: CLegendFrame::legend_square(frame), 
  m_marker(marker), 
...
int CLegendFrame::ScrollY() const
{
  return m_nScrollY;
}

void CLegendFrame::setScrollX(int sx)
{
  m_nScrollX = sx;
}

void CLegendFrame::setScrollY(int sy)
{
  m_nScrollY = sy;
}
