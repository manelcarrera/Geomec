...
#endif
#ifdef TRILIBRARY
/* wed: renamed header file, for name clash with geometry library */
...
struct mesh;
struct behavior;
typedef enum insertvertexresult (*vertexfunction)(struct mesh *, struct behavior *, vertex, struct otri *,
                                                  struct osub *, int, int, REAL);

struct flipstacker {
  triangle flippedtri; /* A recently flipped triangle. */
#ifdef _WIN32
  // this is a quick but dirty way to get rid of warning C4054; the problem is that
  // prevflip (data pointer) is also used to store a function pointer, and that mix is not allowed.
  // needs to be tested on linux too; for now, there are guards in place to only use this on windows
  union {
    struct flipstacker *data;
...
//
// Note: 'unsigned long' is 32 bits and not enough to contain a pointer
//       so we changed it to 'size_t'
//       This has been done ONLY for pointers; counters are still 32 bits,
//       hence the surfaces are limited in size (which is expected to be
//       sufficient)
//
/*                                                                           */
...
#define encode(otri) (triangle)((size_t)(otri).tri | (size_t)(otri).orient)
...
#define infect(otri) (otri).tri[6] = (triangle)((size_t)(otri).tri[6] | (size_t)2l)

#define uninfect(otri) (otri).tri[6] = (triangle)((size_t)(otri).tri[6] & ~(size_t)2l)

/* Test a triangle for viral infection.                                      */

#define infected(otri) (((size_t)(otri).tri[6] & (size_t)2l) != 0l)
...
#define sdecode(sptr, osub)                                                                                            \
  ...
  (osub).ss = (subseg *)((size_t)(sptr) & ~(size_t)3l)

/* sencode() compresses an oriented subsegment into a single pointer.  It    */
/*   relies on the assumption that all subsegments are aligned to two-byte   */
/*   boundaries, so the least significant bit of (osub).ss is zero.          */

#define sencode(osub) (subseg)((size_t)(osub).ss | (size_t)(osub).ssorient)
...

{
  ...
  printf("triangle x%llx with orientation %d:\n", (size_t)t->tri, t->orient);
  decode(t->tri[0], printtri);
  if (printtri.tri == m->dummytri) {
    printf("    [0] = Outer space\n");
  } else {
    printf("    [0] = x%llx  %d\n", (size_t)printtri.tri, printtri.orient);
  }
  ...
  if (printvertex == (vertex)NULL)
    ...
    printf("    Origin[%d] = x%llx  (%.12g, %.12g)\n", (t->orient + 1) % 3 + 3, (size_t)printvertex, printvertex[0],
           printvertex[1]);
  dest(*t, printvertex);
  if (printvertex == (vertex)NULL)
    printf("    Dest  [%d] = NULL\n", (t->orient + 2) % 3 + 3);
  else
    printf("    Dest  [%d] = x%llx  (%.12g, %.12g)\n", (t->orient + 2) % 3 + 3, (size_t)printvertex, printvertex[0],
           printvertex[1]);
  apex(*t, printvertex);
  if (printvertex == (vertex)NULL)
    printf("    Apex  [%d] = NULL\n", t->orient + 3);
  else
    printf("    Apex  [%d] = x%llx  (%.12g, %.12g)\n", t->orient + 3, (size_t)printvertex, printvertex[0],
           printvertex[1]);

  if (b->usesegments) {
    sdecode(t->tri[6], printsh);
    if (printsh.ss != m->dummysub) {
      printf("    [6] = x%llx  %d\n", (size_t)printsh.ss, printsh.ssorient);
    }
...

{
  ...
  if (printvertex == (vertex)NULL)
    ...
    printf("    Origin[%d] = x%llx  (%.12g, %.12g)\n", 2 + s->ssorient, (size_t)printvertex, printvertex[0],
           printvertex[1]);
  sdest(*s, printvertex);
  if (printvertex == (vertex)NULL)
    printf("    Dest  [%d] = NULL\n", 3 - s->ssorient);
  else
    printf("    Dest  [%d] = x%llx  (%.12g, %.12g)\n", 3 - s->ssorient, (size_t)printvertex, printvertex[0],
           printvertex[1]);
...

{
  ...
  pool->nextitem = (VOID *)(alignptr + (size_t)pool->alignbytes - (alignptr % (size_t)pool->alignbytes));
...
#else  /* not ANSI_DECLARATORS */
void poolinit(pool, bytecount, itemcount, wtype, alignment) struct memorypool *pool;
int bytecount;
int itemcount;
enum wordtype wtype;
int alignment;
...

{
  ...
  pool->itemwords = ((bytecount + pool->alignbytes - 1) / pool->alignbytes) * (pool->alignbytes / wordsize);
  ...
  pool->firstblock = (VOID **)trimalloc(pool->itemsperblock * pool->itembytes + sizeof(VOID *) + pool->alignbytes);
  ...
  if (pool->deaditemstack != (VOID *)NULL) {
    ...
    if (pool->unallocateditems == 0) {
      ...
      if (*(pool->nowblock) == (VOID *)NULL) {
        ...
        newblock = (VOID **)trimalloc(pool->itemsperblock * pool->itembytes + sizeof(VOID *) + pool->alignbytes);
...

{
  ...
  pool->pathitem = (VOID *)(alignptr + (size_t)pool->alignbytes - (alignptr % (size_t)pool->alignbytes));
...
#else  /* not ANSI_DECLARATORS */
void dummyinit(m, b, trianglewords, subsegwords) struct mesh *m;
struct behavior *b;
int trianglewords;
int subsegwords;
#endif /* not ANSI_DECLARATORS */

{
  ...
  m->dummytribase = (triangle *)trimalloc(trianglewords * sizeof(triangle) + m->triangles.alignbytes);
  /* Align `dummytri' on a `triangles.alignbytes'-byte boundary. */
  alignptr = (size_t)m->dummytribase;
  m->dummytri = (triangle *)(alignptr + (size_t)m->triangles.alignbytes - (alignptr % (size_t)m->triangles.alignbytes));
  ...
  if (b->usesegments) {
    ...
    /*   subsegment.                                                   */
    m->dummysubbase = (subseg *)trimalloc(subsegwords * sizeof(subseg) + m->subsegs.alignbytes);
    /* Align `dummysub' on a `subsegs.alignbytes'-byte boundary. */
    alignptr = (size_t)m->dummysubbase;
    m->dummysub = (subseg *)(alignptr + (size_t)m->subsegs.alignbytes - (alignptr % (size_t)m->subsegs.alignbytes));
...

{
  ...
  poolinit(&m->vertices, vertexsize, VERTEXPERBLOCK, (sizeof(REAL) >= sizeof(triangle)) ? FLOATINGPOINT : POINTER, 0);
}
...

{
  ...
  if (b->usesegments) {
    ...
    poolinit(&m->subsegs, 6 * sizeof(triangle) + sizeof(int), SUBSEGPERBLOCK, POINTER, 4);
...

{
  ...
  foundvertex = (vertex)(alignptr + (size_t)m->vertices.alignbytes - (alignptr % (size_t)m->vertices.alignbytes));
...

{
  ...
  /* Purify */
  memset(abdet, 0, sizeof abdet);
  memset(adet, 0, sizeof adet);
  memset(axxbc, 0, sizeof axxbc);
  memset(ayybc, 0, sizeof ayybc);
  memset(bdet, 0, sizeof bdet);
  memset(bxxca, 0, sizeof bxxca);
  memset(byyca, 0, sizeof byyca);
  memset(cdet, 0, sizeof cdet);
  memset(cxxab, 0, sizeof cxxab);
  memset(cyyab, 0, sizeof cyyab);
...
#else  /* not ANSI_DECLARATORS */
static REAL incircle(m, b, pa, pb, pc, pd)
struct mesh *m;
struct behavior *b;
vertex pa;
vertex pb;
...
#ifdef ANSI_DECLARATORS
static REAL orient3dadapt(vertex pa, vertex pb, vertex pc, vertex pd, REAL aheight, REAL bheight, REAL cheight,
                          REAL dheight, REAL permanent)
#else  /* not ANSI_DECLARATORS */
static REAL orient3dadapt(pa, pb, pc, pd, aheight, bheight, cheight, dheight, permanent)
vertex pa;
vertex pb;
vertex pc;
vertex pd;
...

{
  ...
  if ((adxtail == 0.0) && (bdxtail == 0.0) && (cdxtail == 0.0) && (adytail == 0.0) && (bdytail == 0.0) &&
      (cdytail == 0.0) && (adheighttail == 0.0) && (bdheighttail == 0.0) && (cdheighttail == 0.0)) {
    return det;
  }
...
#ifdef ANSI_DECLARATORS
static REAL orient3d(struct mesh *m, struct behavior *b, vertex pa, vertex pb, vertex pc, vertex pd, REAL aheight,
                     REAL bheight, REAL cheight, REAL dheight)
#else  /* not ANSI_DECLARATORS */
static REAL orient3d(m, b, pa, pb, pc, pd, aheight, bheight, cheight, dheight)
struct mesh *m;
struct behavior *b;
vertex pa;
vertex pb;
...

{
  if (b->weighted == 0) {
    ...
    return orient3d(m, b, pa, pb, pc, pd, pa[0] * pa[0] + pa[1] * pa[1] - pa[2], pb[0] * pb[0] + pb[1] * pb[1] - pb[2],
                    pc[0] * pc[0] + pc[1] * pc[1] - pc[2], pd[0] * pd[0] + pd[1] * pd[1] - pd[2]);
  } else {
...
#ifdef ANSI_DECLARATORS
void findcircumcenter(struct mesh *m, struct behavior *b, vertex torg, vertex tdest, vertex tapex, vertex circumcenter,
                      REAL *xi, REAL *eta, REAL *minedge)
#else  /* not ANSI_DECLARATORS */
void findcircumcenter(m, b, torg, tdest, tapex, circumcenter, xi, eta, minedge) struct mesh *m;
struct behavior *b;
vertex torg;
vertex tdest;
vertex tapex;
...

{
  ...
  *eta = (xdo * dy - dx * ydo) * (2.0 * denominator);
  ...
  /*   the input PSLG.                                                */
  *minedge = ((dodist < aodist) && (dodist < dadist)) ? dodist : (aodist < dadist) ? aodist : dadist;
}
...
{
  m->vertices.maxitems = m->triangles.maxitems = m->subsegs.maxitems = m->viri.maxitems = m->badsubsegs.maxitems =
      m->badtriangles.maxitems = m->flipstackers.maxitems = m->splaynodes.maxitems = 0l;
  m->vertices.itembytes = m->triangles.itembytes = m->subsegs.itembytes = m->viri.itembytes = m->badsubsegs.itembytes =
      m->badtriangles.itembytes = m->flipstackers.itembytes = m->splaynodes.itembytes = 0;
...

{
  ...
  while (triangleloop.tri != (triangle *)NULL) {
    ...
    for (triangleloop.orient = 0; triangleloop.orient < 3; triangleloop.orient++) {
      ...
      shouldbedelaunay = (oppotri.tri != m->dummytri) && !deadtri(oppotri.tri) && (triangleloop.tri < oppotri.tri) &&
                         (triorg != m->infvertex1) && (triorg != m->infvertex2) && (triorg != m->infvertex3) &&
                         (tridest != m->infvertex1) && (tridest != m->infvertex2) && (tridest != m->infvertex3) &&
                         (triapex != m->infvertex1) && (triapex != m->infvertex2) && (triapex != m->infvertex3) &&
                         (oppoapex != m->infvertex1) && (oppoapex != m->infvertex2) && (oppoapex != m->infvertex3);
...
#ifdef ANSI_DECLARATORS
void enqueuebadtri(struct mesh *m, struct behavior *b, struct otri *enqtri, REAL angle, vertex enqapex, vertex enqorg,
                   vertex enqdest)
#else  /* not ANSI_DECLARATORS */
void enqueuebadtri(m, b, enqtri, angle, enqapex, enqorg, enqdest) struct mesh *m;
struct behavior *b;
struct otri *enqtri;
REAL angle;
vertex enqapex;
...
int under60degrees(struct osub *sub1, struct osub *sub2) {
  ...
  sdest(*sub1, v1);
  sdest(*sub2, v2);
  dotprod = (v2[0] - segmentapex[0]) * (v1[0] - segmentapex[0]) + (v2[1] - segmentapex[1]) * (v1[1] - segmentapex[1]);
  return (dotprod > 0.0) && (4.0 * dotprod * dotprod > ((v1[0] - segmentapex[0]) * (v1[0] - segmentapex[0]) +
...
int splitpermitted(struct mesh *m, struct osub *testsubseg, REAL iradius) {
  struct osub cwsubseg, ccwsubseg = {0, 0}, cwsubseg2 = {0, 0}, ccwsubseg2 = {0, 0};
  struct osub testsym;
  ...
  seglength = (dest1[0] - suborg[0]) * (dest1[0] - suborg[0]) + (dest1[1] - suborg[1]) * (dest1[1] - suborg[1]);
  ...
  if ((nearestpoweroffour > 1.001 * seglength) || (nearestpoweroffour < 0.999 * seglength)) {
    return 1;
  }
  ...
  if (cwsmall) {
    ...
    do {
      ...
      seglength = (dest2[0] - suborg[0]) * (dest2[0] - suborg[0]) + (dest2[1] - suborg[1]) * (dest2[1] - suborg[1]);
      ...
      edgelength =
          0.5 * nearestpoweroffour *
          (1 - (((dest1[0] - suborg[0]) * (dest2[0] - suborg[0]) + (dest1[1] - suborg[1]) * (dest2[1] - suborg[1])) /
    ...
    if (dotproduct < 0.0) {
      if (b->nolenses ||
          (dotproduct * dotproduct >=
           0.25 * ((eorg[0] - eapex[0]) * (eorg[0] - eapex[0]) + (eorg[1] - eapex[1]) * (eorg[1] - eapex[1])) *
               ((edest[0] - eapex[0]) * (edest[0] - eapex[0]) + (edest[1] - eapex[1]) * (edest[1] - eapex[1])))) {
        encroached = 1;
      }
    }
  }
...

{
  ...
  if (b->vararea || b->fixedarea || b->usertest) {
    ...
    if ((b->vararea) && (area > areabound(*testtri)) && (areabound(*testtri) > 0.0)) {
      /* Add this triangle to the list of bad triangles. */
      enqueuebadtri(m, b, testtri, angle, tapex, torg, tdest);
      return;
    }
...
#ifdef ANSI_DECLARATORS
enum locateresult preciselocate(struct mesh *m, struct behavior *b, vertex searchpoint, struct otri *searchtri,
  ...
  while (1) {
    ...
    if (destorient > 0.0) {
      if (orgorient > 0.0) {
        ...
        moveleft =
            (fapex[0] - searchpoint[0]) * (fdest[0] - forg[0]) + (fapex[1] - searchpoint[1]) * (fdest[1] - forg[1]) >
            0.0;
      } else {
        moveleft = 1;
      }
...

{
  ...
  org(*searchtri, torg);
  searchdist =
      (searchpoint[0] - torg[0]) * (searchpoint[0] - torg[0]) + (searchpoint[1] - torg[1]) * (searchpoint[1] - torg[1]);
  if (b->verbose > 2) {
  ...
  for (i = 0; i < sampleblocks; i++) {
    ...
    firsttri = (triangle *)(alignptr + (size_t)m->triangles.alignbytes - (alignptr % (size_t)m->triangles.alignbytes));
    for (j = 0; j < samplesperblock; j++) {
      if (i == triblocks - 1) {
        samplenum = randomnation((int)(m->triangles.maxitems - (i * TRIPERBLOCK)));
      } else {
        samplenum = randomnation(TRIPERBLOCK);
      }
      sampletri.tri = (triangle *)(firsttri + (samplenum * m->triangles.itemwords));
...
#ifdef ANSI_DECLARATORS
enum insertvertexresult insertvertex(struct mesh *m, struct behavior *b, vertex newvertex, struct otri *searchtri,
                                     struct osub *splitseg, int segmentflaws, int triflaws, REAL iradius)
#else  /* not ANSI_DECLARATORS */
enum insertvertexresult insertvertex(m, b, newvertex, searchtri, splitseg, segmentflaws, triflaws, iradius)
struct mesh *m;
struct behavior *b;
vertex newvertex;
struct otri *searchtri;
...

{
  ...
  struct otri newtopright = {0, 0};
  struct otri botlcasing, botrcasing;
  struct otri toplcasing, toprcasing = {0, 0};
  struct otri testtri;
  struct osub botlsubseg, botrsubseg;
  struct osub toplsubseg, toprsubseg;
  ...
  if ((intersect == ONEDGE) || (intersect == OUTSIDE)) {
          ...
          if (enq) {
            ...
            if (b->verbose > 2) {
              printf("  Queueing encroached subsegment (%.12g, %.12g) (%.12g, %.12g).\n", encroached->subsegorg[0],
                     encroached->subsegorg[1], encroached->subsegdest[0], encroached->subsegdest[1]);
            }
          }
        }
...
#ifdef _WIN32
      m->lastflip->prevflip_union.function = &insertvertex;
#else
    ...
    if (m->checkquality) {
      ...
      m->lastflip->flippedtri = encode(horiz);
#ifdef _WIN32
      m->lastflip->prevflip_union.data = NULL;
#else
...
#ifdef _WIN32
            newflip->prevflip_union.data = m->lastflip;
#else
...
#ifdef SELF_CHECK
          if (newvertex != (vertex)NULL) {
            if (counterclockwise(m, b, leftvertex, newvertex, rightvertex) < 0.0) {
              printf("Internal error in insertvertex():\n");
            ...
            */
            ...
            if (counterclockwise(m, b, newvertex, rightvertex, farvertex) < 0.0) {
              printf("Internal error in insertvertex():\n");
...
#ifdef ANSI_DECLARATORS
void triangulatepolygon(struct mesh *m, struct behavior *b, struct otri *firstedge, struct otri *lastedge,
...

{
  ...
  if (b->verbose > 2) {
    ...
    printf("    (%.12g, %.12g) (%.12g, %.12g)\n", leftbasevertex[0], leftbasevertex[1], rightbasevertex[0],
           rightbasevertex[1]);
  }
...
#ifdef _WIN32
    if (m->lastflip->prevflip_union.data == NULL) {
#else
...
#ifdef _WIN32
    } else if (m->lastflip->prevflip_union.function == &insertvertex) {
#else
...
#ifdef _WIN32
    m->lastflip = m->lastflip->prevflip_union.data;
#else
  ...
  while (left < right) {
    ...
    do {
      left++;
             ...
             ((sortarray[left][0] < pivotx) || ((sortarray[left][0] == pivotx) && (sortarray[left][1] < pivoty))));
    /* Search for a vertex whose x-coordinate is too small for the right. */
    do {
      right--;
    } while ((left <= right) &&
             ((sortarray[right][0] > pivotx) || ((sortarray[right][0] == pivotx) && (sortarray[right][1] > pivoty))));
    if (left < right) {
...
#ifdef ANSI_DECLARATORS
void divconqrecurse(struct mesh *m, struct behavior *b, vertex *sortarray, int vertices, int axis, struct otri *farleft,
                    struct otri *farright)
#else  /* not ANSI_DECLARATORS */
...

{
  ...
  while (vertexloop != (vertex)NULL) {
    ...
    if (insertvertex(m, b, vertexloop, &starttri, (struct osub *)NULL, 0, 0, 0.0) == DUPLICATEVERTEX) {
      if (!b->quiet) {
        printf("Warning:  A duplicate vertex at (%.12g, %.12g) appeared and was ignored.\n", vertexloop[0],
               vertexloop[1]);
      }
  ...
  while (notdone) {
    ...
    if (rightchild < heapsize) {
      if ((heap[rightchild]->ykey < heap[smallest]->ykey) ||
          ((heap[rightchild]->ykey == heap[smallest]->ykey) && (heap[rightchild]->xkey < heap[smallest]->xkey))) {
        smallest = rightchild;
      }
    }
...

{
  ...
  return pc[1] + (xac * bclen2 - xbc * aclen2 + sqrt(aclen2 * bclen2 * ablen2)) / (2.0 * ccwabc);
}
...
#ifdef ANSI_DECLARATORS
struct splaynode *circletopinsert(struct mesh *m, struct behavior *b, struct splaynode *splayroot, struct otri *newkey,
...
#ifdef ANSI_DECLARATORS
struct splaynode *frontlocate(struct mesh *m, struct splaynode *splayroot, struct otri *bottommost, vertex searchvertex,
                              struct otri *searchtri, int *farright)
#else  /* not ANSI_DECLARATORS */
struct splaynode *frontlocate(m, splayroot, bottommost, searchvertex, searchtri, farright)
struct mesh *m;
struct splaynode *splayroot;
...

{
  ...
  struct otri searchtri;
  struct otri fliptri;
  struct otri lefttri, righttri, farlefttri = {0, 0}, farrighttri = {0, 0};
  ...
  poolinit(&m->splaynodes, sizeof(struct splaynode), SPLAYNODEPERBLOCK, POINTER, 0);
  ...
  while (heapsize > 0) {
    ...
    if (nextevent->xkey < m->xmin) {
      ...
      if (randomnation(SAMPLERATE) == 0) {
        ...
        splayroot = circletopinsert(m, b, splayroot, &lefttri, leftvertex, midvertex, rightvertex, nextevent->ykey);
      }
    } else {
    ...
    if (check4events) {
      ...
      if (lefttest > 0.0) {
        ...
        newevent->ykey = circletop(m, leftvertex, midvertex, rightvertex, lefttest);
      ...
      if (righttest > 0.0) {
        ...
        freeevents = (struct event *)freeevents->eventptr;
        newevent->xkey = m->xminextreme;
        newevent->ykey = circletop(m, leftvertex, midvertex, rightvertex, righttest);
...
#ifdef ANSI_DECLARATORS
int reconstruct(struct mesh *m, struct behavior *b, int *trianglelist, REAL *triangleattriblist, REAL *trianglearealist,
                int elements, int corners, int attribs, int *segmentlist, int *segmentmarkerlist, int numberofsegments)
#else  /* not ANSI_DECLARATORS */
int reconstruct(m, b, trianglelist, triangleattriblist, trianglearealist, elements, corners, attribs, segmentlist,
                segmentmarkerlist, numberofsegments)
struct mesh *m;
struct behavior *b;
int *trianglelist;
...
#ifdef ANSI_DECLARATORS
long reconstruct(struct mesh *m, struct behavior *b, char *elefilename, char *areafilename, char *polyfilename,
                 FILE *polyfile)
#else  /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
void segmentintersection(struct mesh *m, struct behavior *b, struct otri *splittri, struct osub *splitsubseg,
                         vertex endpoint2)
#else  /* not ANSI_DECLARATORS */
...

{
  ...
  success = insertvertex(m, b, newvertex, &searchtri1, (struct osub *)NULL, 0, 0, 0.0);
  if (success == DUPLICATEVERTEX) {
    ...
    if (success == VIOLATINGVERTEX) {
      ...
      success = insertvertex(m, b, newvertex, &searchtri1, &brokensubseg, 0, 0, 0.0);
      if (success != SUCCESSFULVERTEX) {
...
{
  struct otri searchtri1 = {0, 0}, searchtri2 = {0, 0};
  triangle encodedtri;
  vertex checkvertex;
  triangle ptr; /* Temporary variable used by sym(). */
...
#else  /* not ANSI_DECLARATORS */
  void formskeleton(m, b, segmentlist, segmentmarkerlist, numberofsegments) struct mesh *m;
  struct behavior *b;
  int *segmentlist;
  int *segmentmarkerlist;
  int numberofsegments;
...

{
  ...
  while (virusloop != (triangle **)NULL) {
    ...
    if (b->verbose > 2) {
      ...
      printf("    Checking (%.12g, %.12g) (%.12g, %.12g) (%.12g, %.12g)\n", deadorg[0], deadorg[1], deaddest[0],
             deaddest[1], deadapex[0], deadapex[1]);
    }
      ...
      if ((neighbor.tri != m->dummytri) && !infected(neighbor) && (neighborsubseg.ss == m->dummysub)) {
        if (b->verbose > 2) {
          ...
          printf("    Marking (%.12g, %.12g) (%.12g, %.12g) (%.12g, %.12g)\n", regionorg[0], regionorg[1],
                 regiondest[0], regiondest[1], regionapex[0], regionapex[1]);
        }
        /* Infect the neighbor. */
        infect(neighbor);
        /* Ensure that the neighbor's neighbors will be infected. */
...

{
  ...
  while (subsegloop.ss != (subseg *)NULL) {
    ...
    checkseg4encroach(m, b, &subsegloop, 0.0);
    subsegloop.ss = subsegtraverse(m);
  }
}
      ...
      if (!deadsubseg(currentenc.ss) && (eorg == encloop->subsegorg) && (edest == encloop->subsegdest)) {
        ...
        if (!b->nolenses && !acuteorg && !acutedest) {
          ...
          while ((vertextype(eapex) == FREEVERTEX) &&
                 ((eorg[0] - eapex[0]) * (edest[0] - eapex[0]) + (eorg[1] - eapex[1]) * (edest[1] - eapex[1]) < 0.0)) {
        ...
        if (acuteorg || acutedest) {
          segmentlength =
              sqrt((edest[0] - eorg[0]) * (edest[0] - eorg[0]) + (edest[1] - eorg[1]) * (edest[1] - eorg[1]));
        ...
        if (b->verbose > 1) {
          printf("  Splitting subsegment (%.12g, %.12g) (%.12g, %.12g) at (%.12g, %.12g).\n", eorg[0], eorg[1],
                 edest[0], edest[1], newvertex[0], newvertex[1]);
        }
        ...
        success = insertvertex(m, b, newvertex, &enctri, &currentenc, 1, triflaws, 0.0);
        ...
        checkseg4encroach(m, b, &currentenc, 0.0);
...

{
  ...
  if (!deadtri(badotri.tri) && (borg == badtri->triangorg) && (bdest == badtri->triangdest) &&
      (bapex == badtri->triangapex)) {
    if (b->verbose > 1) {
    ...
    if (((newvertex[0] == borg[0]) && (newvertex[1] == borg[1])) ||
      ...
      if (!b->quiet) {
        printf("Warning:  New vertex (%.12g, %.12g) falls on existing vertex.\n", newvertex[0], newvertex[1]);
        errorflag = 1;
      }
      vertexdealloc(m, newvertex);
    } else {
      ...
      success = insertvertex(m, b, newvertex, &badotri, (struct osub *)NULL, 1, 1, minedge);
      if (success == SUCCESSFULVERTEX) {
        if (m->steinerleft > 0) {
          m->steinerleft--;
        }
...

{
  ...
  poolinit(&m->badsubsegs, sizeof(struct badsubseg), BADSUBSEGPERBLOCK, POINTER, 0);
  if (b->verbose) {
  ...
  if ((b->minangle > 0.0) || b->vararea || b->fixedarea || b->usertest) {
    ...
    poolinit(&m->badtriangles, sizeof(struct badtriang), BADTRIPERBLOCK, POINTER, 0);
    ...
    poolinit(&m->flipstackers, sizeof(struct flipstacker), FLIPSTACKERPERBLOCK, POINTER, 0);
    m->checkquality = 1;
    if (b->verbose) {
...

{
  ...
  do {
    ...
    while ((*result != '\0') && (*result != '#') && (*result != '.') && (*result != '+') && (*result != '-') &&
           ((*result < '0') || (*result > '9'))) {
      result++;
    }
...
#ifdef ANSI_DECLARATORS
void transfernodes(struct mesh *m, struct behavior *b, REAL *pointlist, REAL *pointattriblist, int *pointmarkerlist,
                   int numberofpoints, int numberofpointattribs)
#else  /* not ANSI_DECLARATORS */
  void transfernodes(m, b, pointlist, pointattriblist, pointmarkerlist, numberofpoints,
                     numberofpointattribs) struct mesh *m;
  struct behavior *b;
  REAL *pointlist;
...
#ifdef ANSI_DECLARATORS
void readholes(struct mesh *m, struct behavior *b, FILE *polyfile, char *polyfilename, REAL **hlist, int *holes,
...
#else  /* not ANSI_DECLARATORS */
  void writenodes(m, b, pointlist, pointattriblist, pointmarkerlist) struct mesh *m;
  struct behavior *b;
  REAL **pointlist;
  REAL **pointattriblist;
  int **pointmarkerlist;
...

{
  ...
  if ((m->nextras > 0) && (*pointattriblist == (REAL *)NULL)) {
    *pointattriblist = (REAL *)trimalloc(outvertices * m->nextras * sizeof(REAL));
  }
...

{
  ...
  if (*trianglelist == (int *)NULL) {
    *trianglelist = (int *)trimalloc(m->triangles.items * ((b->order + 1) * (b->order + 2) / 2) * sizeof(int));
  }
  /* Allocate memory for output triangle attributes if necessary. */
  if ((m->eextras > 0) && (*triangleattriblist == (REAL *)NULL)) {
    *triangleattriblist = (REAL *)trimalloc(m->triangles.items * m->eextras * sizeof(REAL));
  }
  tlist = *trianglelist;
  ...
  while (triangleloop.tri != (triangle *)NULL) {
    ...
    if (b->order == 1) {
      ...
      fprintf(outfile, "%4ld    %4d  %4d  %4d  %4d  %4d  %4d", elementnumber, vertexmark(p1), vertexmark(p2),
              vertexmark(p3), vertexmark(mid1), vertexmark(mid2), vertexmark(mid3));
#endif /* not TRILIBRARY */
    }
...
#ifdef ANSI_DECLARATORS
void writepoly(struct mesh *m, struct behavior *b, char *polyfilename, REAL *holelist, int holes, REAL *regionlist,
               int regions, int argc, char **argv)
#else  /* not ANSI_DECLARATORS */
...

{
  ...
  if (holes > 0) {
    for (holenumber = 0; holenumber < holes; holenumber++) {
      ...
      fprintf(outfile, "%4ld   %.17g  %.17g\n", b->firstnumber + holenumber, holelist[2 * holenumber],
  ...
  if (regions > 0) {
    ...
    for (regionnumber = 0; regionnumber < regions; regionnumber++) {
      ...
      fprintf(outfile, "%4ld   %.17g  %.17g  %.17g  %.17g\n", b->firstnumber + regionnumber,
              regionlist[4 * regionnumber], regionlist[4 * regionnumber + 1], regionlist[4 * regionnumber + 2],
...
#else  /* not ANSI_DECLARATORS */
  void writevoronoi(m, b, vpointlist, vpointattriblist, vpointmarkerlist, vedgelist, vedgemarkerlist,
                    vnormlist) struct mesh *m;
  struct behavior *b;
  REAL **vpointlist;
...

{
  ...
  if (*vpointattriblist == (REAL *)NULL) {
    *vpointattriblist = (REAL *)trimalloc(m->triangles.items * m->nextras * sizeof(REAL));
  }
    ...
    for (triangleloop.orient = 0; triangleloop.orient < 3; triangleloop.orient++) {
      ...
      if ((triangleloop.tri < trisym.tri) || (trisym.tri == m->dummytri)) {
        ...
        if (trisym.tri == m->dummytri) {
          ...
          fprintf(outfile, "%4ld   %d  %d   %.17g  %.17g\n", vedgenumber, p1, -1, tdest[1] - torg[1],
                  torg[0] - tdest[0]);
#endif /* not TRILIBRARY */
        } else {
...

{
  ...
  while (triangleloop.tri != (triangle *)NULL) {
    ...
    fprintf(outfile, " 3   %4d  %4d  %4d\n", vertexmark(p1) - 1, vertexmark(p2) - 1, vertexmark(p3) - 1);
    triangleloop.tri = triangletraverse(m);
  }
  finishfile(outfile, argc, argv);
}
...

{
  ...
  printf("  1.1547 - %-6.6g    :  %8d    | %6.6g - %-6.6g     :  %8d\n", ratiotable[0], aspecttable[0], ratiotable[7],
         ratiotable[8], aspecttable[8]);
  for (i = 1; i < 7; i++) {
    printf("  %6.6g - %-6.6g    :  %8d    | %6.6g - %-6.6g     :  %8d\n", ratiotable[i - 1], ratiotable[i],
           aspecttable[i], ratiotable[i + 7], ratiotable[i + 8], aspecttable[i + 8]);
  }
  printf("  %6.6g - %-6.6g    :  %8d    | %6.6g -            :  %8d\n", ratiotable[6], ratiotable[7], aspecttable[7],
         ratiotable[14], aspecttable[15]);
...

{
...
#ifdef TRILIBRARY
  transfernodes(&m, &b, in->pointlist, in->pointattributelist, in->pointmarkerlist, in->numberofpoints,
...
#ifdef TRILIBRARY
    m.hullsize = reconstruct(&m, &b, in->trianglelist, in->triangleattributelist, in->trianglearealist,
                             in->numberoftriangles, in->numberofcorners, in->numberoftriangleattributes,
                             in->segmentlist, in->segmentmarkerlist, in->numberofsegments);
#else  /* not TRILIBRARY */
    m.hullsize = reconstruct(&m, &b, b.inelefilename, b.areafilename, b.inpolyfilename, polyfile);
#endif /* not TRILIBRARY */
  } else {
    m.hullsize = delaunay(&m, &b); /* Triangulate the vertices. */
  }
...
#else  /* not TRILIBRARY */
    readholes(&m, &b, polyfile, b.inpolyfilename, &holearray, &m.holes, &regionarray, &m.regions);
#endif /* not TRILIBRARY */
    if (!b.refine) {
...
#else  /* not TRILIBRARY */
      writepoly(&m, &b, b.outpolyfilename, holearray, m.holes, regionarray, m.regions, argc, argv);
#endif /* not TRILIBRARY */
    }
  }
#ifndef TRILIBRARY
  ...
  if (b.voronoi) {
#ifdef TRILIBRARY
    writevoronoi(&m, &b, &vorout->pointlist, &vorout->pointattributelist, &vorout->pointmarkerlist, &vorout->edgelist,
