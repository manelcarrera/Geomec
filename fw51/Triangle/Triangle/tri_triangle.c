...
struct mesh;
struct behavior;
typedef enum insertvertexresult (*vertexfunction)(struct mesh *, struct behavior *, vertex, struct otri *, struct osub *, int, int, REAL);
...

{
  ...
  foundvertex = (vertex) (alignptr + (size_t) m->vertices.alignbytes -
...
static REAL orient3dadapt(vertex pa, vertex pb, vertex pc, vertex pd,
                   ...
                   REAL permanent)
#else /* not ANSI_DECLARATORS */
static REAL orient3dadapt(pa, pb, pc, pd,
...

{
  ...
  for (i = 0; i < sampleblocks; i++) {
    ...
    firsttri = (triangle *) (alignptr + (size_t) m->triangles.alignbytes
                      - (alignptr % (size_t) m->triangles.alignbytes));
