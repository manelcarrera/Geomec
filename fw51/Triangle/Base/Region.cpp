...
CRegion::CRegion(const CTriangleSurface &surface)
: m_surface(surface), m_maxarea(-1), m_bAreaConstraint(false)
{
}
...
const CTriangleSurface &CRegion::Surface() const
{
  return m_surface;
}

size_t CRegion::TriangleSize() const
{
  return m_vcTriangleIndices.size();
}
...
size_t CRegion::SegmentSize() const
{
...
bool CRegion::AreaConstraint() const
{
  return m_bAreaConstraint;
}
...
void CRegion::BuildNodeCache() const
{
  ...
  for(it = m_stNodes.begin(); it != m_stNodes.end(); it++) m_vcNodes.push_back(*it);
}
...
const geo::IPoint &CRegion::Point(int nIndex) const
{
  ...
  if(m_vcNodes.empty()) BuildNodeCache();
...
int CRegion::PointSize() const
{
  return (int)m_stNodes.size();
}

const geo::IElement &CRegion::Element(int nIndex) const
{
  return Triangle(nIndex);
}

int CRegion::ElementSize() const
{
  return TriangleSize();
}
...
std::vector<int> CRegion::Nodes(const geo::IElement &element) const
{
  ...
  for(i = 0; i < tri->NrOfPoints(); i++) ret.push_back(tri->PointIndex(i));
