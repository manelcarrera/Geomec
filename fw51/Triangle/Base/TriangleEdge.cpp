...
CTriangleEdge::CTriangleEdge(const CTriangleSurface &surface, int nIndex)
    : m_surface(surface), m_nIndex(nIndex), m_prTriangle((const geo::ITriangle *)0, (const geo::ITriangle *)0) {}
...
const geo::IPoint &CTriangleEdge::First() const { return m_surface.Point(FirstIndex()); }

void CTriangleEdge::First(const geo::IPoint & /*point*/) { assert(false); }

void CTriangleEdge::Second(const geo::IPoint & /*point*/) { assert(false); }

const geo::IPoint &CTriangleEdge::Second() const { return m_surface.Point(SecondIndex()); }

int CTriangleEdge::SecondIndex() const { return m_surface.m_mesh->edgelist[2 * m_nIndex + 1]; }

int CTriangleEdge::FirstIndex() const { return m_surface.m_mesh->edgelist[2 * m_nIndex]; }
...
int CTriangleEdge::SegmentIndex() const { return m_surface.m_mesh->edgemarkerlist[m_nIndex] - 1; }
