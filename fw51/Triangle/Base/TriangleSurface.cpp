...
CTriangleSurface::CTriangleSurface(const geo::ISurface &rhs)
 : m_bAllowSteinerPointsOnBoundary(true)
{
	m_input = new triangulateio;
	memset(m_input, 0, sizeof (triangulateio));
...
CTriangleSurface::CTriangleSurface(
                    const std::vector<geo::CPoint>       &vcPoint,
...
CTriangleSurface::CTriangleSurface(
                     const std::vector<const geo::IPoint *> &vcPoint,
                     const std::vector<std::vector<int> >   &vcTriangle)
 : m_bAllowSteinerPointsOnBoundary(true)
{
	m_input = new triangulateio;
...
void CTriangleSurface::SetSurface(const geo::ISurface &rhs)
{
	...
	for(i = 0; i < rhs.FaceSize(); i++)
	{
		...
		int j;
		for(j = 0; j < 3; j++) m_mesh->trianglelist[3 * i + j] = face.PointIndex(j);
	}
}

void CTriangleSurface::SetSurface(const std::vector<geo::CPoint> &vcPoint, const std::vector<std::vector<int> > &vcTriangle)
{
	assert(PointSize() == 0);
	assert(ElementSize() == 0);
	...
	for(size_t i = 0; i < vcTriangle.size(); i++)
	{
		...
		for(size_t j = 0; j < tri.size(); j++) m_mesh->trianglelist[3 * i + j] = tri[j];
	}
}

void CTriangleSurface::SetSurface(const std::vector<const geo::IPoint *> &vcPoint, const std::vector<std::vector<int> > &vcTriangle)
{
	assert(PointSize() == 0);
	assert(ElementSize() == 0);
	m_mesh->numberofpoints = (int)vcPoint.size();
...
void CTriangleSurface::ClearTriangleMesh()
{
	ClearTriangleStruct(m_mesh);
	ClearTriangleStruct(m_input);
	for(size_t i = 0; i < m_vcRegions.size(); i++) delete m_vcRegions[i];
	m_vcRegions.clear();	
}

void CTriangleSurface::InvalidateCache()
{
	for(size_t i = 0; i < m_vcNode.size(); i++) delete m_vcNode[i];
	m_vcNode.clear();

	for(size_t i = 0; i < m_vcElement.size(); i++) delete m_vcElement[i];
	m_vcElement.clear();

	for(size_t i = 0; i < m_vcEdge.size(); i++) delete m_vcEdge[i];
	m_vcEdge.clear();

	for(int i = 0; i < RegionSize(); i++) Region(i).Clear();
...
const geo::IPoint& CTriangleSurface::Point(int nIndex) const
{
	...
	if(m_vcNode.empty()) BuildNodeInterface();
	return *m_vcNode[nIndex];
}

int CTriangleSurface::PointSize() const
{
	return m_mesh->numberofpoints;
}
...
bool CTriangleSurface::IsMesh() const
{
	return (FaceSize() > 0);
}
...
bool CTriangleSurface::CreateMesh(double minangle /* = 20 */, double max_area /* = 0 */)
{
  ...
  if (max_area > 0)
	...
	// set segment markers
	if(m_input->segmentmarkerlist) free(m_input->segmentmarkerlist);
	...
	// set regions
	if(m_input->regionlist) free(m_input->regionlist);
	...
	for(int i = 0; i < RegionSize(); i++)
	{
		...
		double maxarea;
		if(region.AreaConstraint()) maxarea = region.MaximumArea();
		else                        maxarea = DBL_MAX;
	...
	if ( !m_bAllowSteinerPointsOnBoundary ) strcat(cmdline, "YY");
	ClearTriangleStruct(m_mesh);
...
int CTriangleSurface::FaceSize() const
{
	return m_mesh->numberoftriangles;
}

const geo::IFace &CTriangleSurface::Face(int nIndex) const
{
	assert(nIndex >= 0 && nIndex < FaceSize());
	if(m_vcElement.empty()) BuildElementInterface();
...
int CTriangleSurface::EdgeSize() const
{
	return m_mesh->numberofedges;
}
...
const CTriangleEdge &CTriangleSurface::Edge(int nIndex) const
{
	...
	if(m_vcEdge.empty()) BuildEdgeInterface();
...
std::vector<int> CTriangleSurface::Nodes(const geo::IElement &element) const
{
	...
	int i;
	for(i = 0; i < 3; i++) ret[i] = pTriangle->PointIndex(i);
...
geo::CPoint CTriangleSurface::RegionInternalPoint(const CRegion &region) const
{
  ...
  if(signedarea > 0) vecNormal.Flip();
...
int CTriangleSurface::RegionSize() const
{
	return (int)m_vcRegions.size();
}
...
int CTriangleSurface::SegmentSize() const
{
	return m_input->numberofsegments;
}
...
void CTriangleSurface::AllowSteinerPointsOnBoundary(bool allow)
{
	m_bAllowSteinerPointsOnBoundary=allow;
}

int CTriangleSurface::InternalInputPointSize() const
{
  return m_input ? m_input->numberofpoints : 0;
}
const double *CTriangleSurface::InternalInputPoint(int index) const
{
  return m_input->pointlist + 2 * index;
}
