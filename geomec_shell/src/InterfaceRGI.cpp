...
const QString APP_VERSION = "RockMechProcessor " xstr(GM_MAJOR) "." xstr(GM_MINOR) "." xstr(GM_BUILD) "." xstr(
    GM_REVISION) " " GM_BUILD_PREFIX " " xstr(GM_BUILD_STRING);
...
bool ModelIsEmpty(CModelBase *model, std::vector<QString> &warnings) {
  ...
  if (stage->OutputType() != CDepletionStage::RESULTS && stage->OutputType() != CDepletionStage::PHASE &&
      stage->OutputType() != CDepletionStage::BRANCH) {
  ...
  for (TFormationBaseEntry::TNodeSet::iterator it = stFormations.begin(); it != stFormations.end(); ++it) {
    ...
    if (pressure->DistributedSize() > 0 || pressure->Type() != CPressure::REPEAT_MODE) {
      warnings.push_back(
          QObject::tr("Second depletion stage of formation '%1' has pressure loads defined").arg((*it)->Name()));
      retval = false;
    }
    ...
    if (temperature->DistributedSize() > 0 || temperature->Type() != CTemperature::REPEAT_MODE) {
      warnings.push_back(
          QObject::tr("Second depletion stage of formation '%1' has temperature loads defined").arg((*it)->Name()));
      retval = false;
    }
    ...
    if (strain->DistributedSize() > 0 || strain->Type() != CStrainLoad::REPEAT_MODE) {
      warnings.push_back(
          QObject::tr("Second depletion stage of formation '%1' has strain loads defined").arg((*it)->Name()));
      retval = false;
    }
  }
  ...
  if (pEntryF) {
    ...
    for (CTetraSubHorizonEntry::TNodeSet::const_iterator it = stFaults.begin(); it != stFaults.end(); ++it) {
      ...
      if (faultPressure.DistributedSize() > 0 || faultPressure.Mode() != prevPressure.Mode()) {
        warnings.push_back(
            QObject::tr("Second depletion stage of fault '%1' has pressures defined").arg((*it)->Name()));
        retval = false;
      }
    }
  }
...
CRGIGlobalMessageObserver::CRGIGlobalMessageObserver(GeomecRGI::CRockMechProcessor *RMP)
    : m_pRMP(RMP), m_pRgi(nullptr) {
  _m()->AttachObserver(this);
}

CRGIGlobalMessageObserver::~CRGIGlobalMessageObserver() { _m()->DetachObserver(this); }

void CRGIGlobalMessageObserver::setRGInterface(RGInterface *pRgi) { m_pRgi = pRgi; }
...
class CChannelRGICORA : public CChannelCORA {
  ...
  RGFailure::RGFailureMechamismObject oType(int type) {
    switch (type) {
    case 1:
      return RGFailure::Fault;
    case 2:
      return RGFailure::Formation;
    case 5:
      return RGFailure::TopSurface;
    case 6:
      return RGFailure::Well;
    default:
      return RGFailure::UndefinedObject;
    }
  }
...
public:
  CChannelRGICORA(IInterfaceBase *master) : CChannelCORA(master) {}
  ...
  std::vector<RGFailureMechanism> &FailureMechanisms() { return m_failureMechanisms; }
};
...
public:
  CRGIFilosFatalStrategy(GeomecRGI::CRockMechProcessor &rockMechProcessor, RGInterface &rgInterface)
      : m_rockMechProcessor(rockMechProcessor), m_rgInterface(rgInterface) {}
...
CInterfaceRGI::CInterfaceRGI()
    : m_pModelContext(0), m_pRockMechProcessor(0), m_pSync(0), m_pRgi(0), m_pBuildTensor(0), m_bSkipShutdown(false),
      m_bSaveModel(false), m_bQuit(false), m_bHibernate(false), m_bHibernating(false), m_nCommands(0),
      m_nCurrentCommand(-1), m_bHaveCalculation(false) {}

const char *CInterfaceRGI::ShortDescription() const { return "RockMechProcessor"; }
...
bool CInterfaceRGI::AcceptParameters(CArgumentParser &argParser, bool bForced) {
  ...
  struct {
    const char *key;
    QString *value;
  } params[] = {{"rockmechfile", &parsedData.sRockMechFile},
...
void CInterfaceRGI::PrintUsage() const {
  ...
  std::cerr << "Usage: " << progName
            << " /ROCKMECHFILE:<rockmechfile> [<modelfile>] [/OUTPUTDIR:<dir>] [/LOG:<filename>] [/TEMPDIR:<dir>]"
            << std::endl
            << "       " << progName << " --version" << std::endl
            << std::endl;
}
...
void CInterfaceRGI::Startup() {
  ...
  if (!m_initialized) {
    ...
    m_pRockMechProcessor =
        new GeomecRGI::CRockMechProcessor(parsedData.sRockMechFile, parsedData.sOutputDir, parsedData.sTempDir);
    ...
    if (!parsedData.sLog.isEmpty() && !_m()->openlogfile(parsedData.sLog.toStdString())) {
      m_pRockMechProcessor->AddLogLine(QString("Unable to open log file '%1'").arg(parsedData.sLog), nullptr, false,
                                       true);
    ...
    if (!m_pModelContext->Model()) {
      m_pRockMechProcessor->AddLogLine(QString("Failed to load model '%1'").arg(parsedData.sModelFile), nullptr, false,
                                       true);
    ...
    m_pRockMechProcessor->AddLogLine(
        QObject::tr("Starting RockMechProcessor with filename '%1'").arg(parsedData.sRockMechFile));
    ...
    if (m_pModelContext->Model()) {
      if (!m_pModelContext->Created()) {
        if (!m_pModelContext->Validate(true)) {
          m_pRockMechProcessor->AddLogLine(QObject::tr("Invalid Geomec model, not all calculation criteria are met!"),
                                           nullptr, false, true);
      ...
      if (!ModelIsEmpty(m_pModelContext->Model(), warnings)) {
        m_pRockMechProcessor->AddLogLine(
            QObject::tr("Geomec model hasn't been cleaned up; please resolve the following issues:"), nullptr, false,
            true);
...
void CInterfaceRGI::LogCommand(GMCommand &cmd, int command) {
  ...
  case RGI_SHUTDOWN:
    ...
    break;
  default:
    msg = QString("Received GMCommand[%1]: %2 (%3)")
              .arg(m_nCurrentCommand)
              .arg(cmd.toStringCmd().c_str())
              .arg(cmd.toStringPrms().c_str());
...
void CInterfaceRGI::HandleLastCommand() {
  ...
  if (m_bSaveModel != bOldSaveModel) {
    QString msg = QString("Flag to save model has changed from '%1' to '%2'")
                      .arg(bOldSaveModel ? "true" : "false")
                      .arg(m_bSaveModel ? "true" : "false");
    m_pRockMechProcessor->AddLogLine(msg, m_pRgi, false, false);
  }
...
CInterfaceModelContext *CInterfaceRGI::GetModelContext() { return m_pModelContext; }
...
void CInterfaceRGI::RGISetup() {
  if (!m_bQuit) {
    ...
    */
    ...
    if (m_nCommands == 0) {
      QString l = QString("The RGI project '%1' does not contain any commands ('%2' commands)")
                      .arg(parsedData.sRockMechFile)
                      .arg(m_nCommands);
    ...
    if (!commandHibernateOrQuitEncountered) {
      QString l =
          QString("The RGI project '%1' does not contain either the 'typeCommandHibernate' or the 'typeCommandQuit' "
                  "command, the 'typeCommandQuit' command is used as the default last command")
              .arg(parsedData.sRockMechFile);
      m_pRockMechProcessor->AddLogLine(l, m_pRgi, false, true);
    }
...
void CInterfaceRGI::RGIShutdown() {
  ...
  // in hibernation mode we need to do saving on quit here, as it seems that dynamo kills our thread after the lock is
  // released
  if (m_bQuit) {
    if (m_bSaveModel) {
      bool retVal = m_error == 0;
      QString fileName = m_pRockMechProcessor->getSaveModelName(parsedData.sModelFile, m_pModelContext->Stage(), retVal,
                                                                !m_pModelContext->Created());
      ...
      if (retVal || m_pModelContext->Stage() == 0) {
        ...
        QString msg = QString("Quitting(save model flag is 'true'), but NOT saving '%1' -- error received (%2)")
                          .arg(fileName)
                          .arg(m_error);
        m_pRockMechProcessor->AddLogLine(msg, m_pRgi, false, false);
      }
