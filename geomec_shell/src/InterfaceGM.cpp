...
bool CInterfaceGM::Expose() const
{
  return true;
}

const char *CInterfaceGM::ShortDescription() const
{
  return "Geomec";
}
...
bool CInterfaceGM::AcceptParameters(CArgumentParser& argParser, bool bForced)
{
  if (bForced)
  {
    ...
    for (size_t i = 0; i < argParser.ArgumentSize(); ++i)
    {
      ...
      else if (value == "linear" || value == "nonlin" || value == "heat" || value == "mixture" || value == "containment" || value == "FF") //FIXME
      {
        m_type = value.c_str();
      }
...
void CInterfaceGM::PrintUsage() const
{
  std::cerr << std::endl
    ...
    << "                           run [linear|nonlin|heat|mixture|containment] <model> [<child index> [<child index> ...]]" << std::endl
    << "                           <skua file> <output file>" << std::endl
    << "  only gm5 models are supported" << std::endl
    << "  this feature is EXPERIMENTAL" << std::endl << std::endl;
}
...
{
  CNewWellPathEntry::TNodeSet wps = static_cast<const CNewWellPathEntry *>(model.GraphEntry(MD_NEW_WELLPATH))->EntryNodes();
  for (CNewWellPathEntry::TNodeSet::iterator it = wps.begin(); it != wps.end(); ++it)
  {
    if (!(*it)->CanZoomIn())
    {
      ...
      {
        std::cout << indent << child.childModel(0).Index() << "\tcasing\t" << child.childModel(0).Name().toStdString() << std::endl;
      }
    }
  }
}
  ...
  case GM_RUN:
    {
      for (size_t i = 0; i < m_indices.size(); ++i)
      {
        ...
        if (index > 0)
        {
          CNewWellPathEntry::TNodeSet wps = static_cast<const CNewWellPathEntry *>(m_pModelContext->Model()->GraphEntry(MD_NEW_WELLPATH))->EntryNodes();
          for (CNewWellPathEntry::TNodeSet::iterator it = wps.begin(); it != wps.end(); ++it)
          {
            if (!(*it)->CanZoomIn())
            {
...
CInterfaceModelContext *CInterfaceGM::GetModelContext()
{
