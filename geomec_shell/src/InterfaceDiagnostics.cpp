...
CArchiveQTextStream::CArchiveQTextStream(QIODevice* device)
  : archive(device), compressed(false)
{
}
...
bool CArchiveQTextStream::IsLoading() const
{
  return archive.device()->isReadable();
}

bool CArchiveQTextStream::IsStoring() const
{
  return archive.device()->isWritable();
}

bool CArchiveQTextStream::Exists() const
{
  return true;
}

quint64 CArchiveQTextStream::GetPosition()
{
  return archive.device()->pos();
}

void CArchiveQTextStream::SetPosition(quint64 position)
{
...
void CArchiveQTextStream::Close()
{
  archive.device()->close();
}

void CArchiveQTextStream::Abort()
{
  archive.device()->close();
}
...
CArchiveQTextStream& CArchiveQTextStream::operator << (
  const QDateTime& /*dt*/)
{
  //archive << dt;
...
void CArchiveQTextStream::DestroyCompressedStream()
{
  compressed = false;
}

bool CArchiveQTextStream::IsCompressed() const
{
  return compressed;
}
...
void CModelDiagnosticsHookResultRegister::start(CModelBase* /*model*/)
{
  ++m_order;
}
...
void CModelDiagnostics::Setting(const QString& name, bool value)
{
  m_onOffSettings[name] = value;
}

void CModelDiagnostics::Hook(const QString& name, IModelDiagnosticsHook& hook)
{
  m_hooks[name] = &hook;
}
...
bool CInterfaceDiagnostics::Expose() const
{
  return false;
}

const char *CInterfaceDiagnostics::ShortDescription() const
{
  return "Geomec Diagnostics";
}
...
{
  std::cout << "load [main]" << std::endl << "\tload model; option to stop before loading zoomin models" << std::endl
  << "ls depletions|analyses|zoomin" << std::endl << "\tlist" << std::endl
  << "cd ..|<zoomin model>" << std::endl << "\tswitch to parent or child model" << std::endl
  << "ignore [main|zoomin|wellzoomin] results" << std::endl << "\tdon't store read results in memory" << std::endl
  << "save <file>" << std::endl << "\tsave model" << std::endl
  << "flush results" << std::endl << "\tflush result cache" << std::endl
  << "save results [txt] <file>" << std::endl << "\tsave results of current model (unencrypted)" << std::endl
  << "sethook resultregister [exit] # <file>" << std::endl << "\tsave #th register that is loaded to file (and exit thereafter)" << std::endl
  << "sethook analysislog [exit] <file>" << std::endl << "\tsave the (main) analysis log to file (and exit)" << std::endl
  << "tmp <dir>" << std::endl << "\tuse <dir> as temporary directory" << std::endl
  << "quit" << std::endl << "\tquit the program" << std::endl << std::endl
  << "use ignore and sethook before loading; don't use quotes or extra spaces" << std::endl << std::endl;
}
...
void CInterfaceDiagnostics::Startup()
{
  ...
  std::cout << "using interface Geomec Diagnostics for model " << m_model.toStdString() << std::endl << std::endl
      ...
      << "use help to view current commands" << std::endl << std::endl
      << "$ ";
...
int CInterfaceDiagnostics::ParseAndEvaluateList(QString& line)
{
  ...
  while (pStage)
  {
      ...
      std::cout << (m_pModelContext->Model()->ResultRegister().ResultsAvailable(CAnalysisType::AT_LINEAR, *pStage) ? "l" : ".");
      std::cout << (m_pModelContext->Model()->ResultRegister().ResultsAvailable(CAnalysisType::AT_NONLIN, *pStage) ? "n" : ".");
      std::cout << (m_pModelContext->Model()->ResultRegister().ResultsAvailable(CAnalysisType::AT_HEAT, *pStage) ? "h" : ".");
      std::cout << (m_pModelContext->Model()->ResultRegister().ResultsAvailable(CAnalysisType::AT_MIXTURE, *pStage) ? "m" : ".");
      std::cout << (m_pModelContext->Model()->ResultRegister().ResultsAvailable(CAnalysisType::AT_MIXTURE_CONTAINMENT, *pStage) ? "c" : ".") << "\t";
  ...
  {
  CZoominModelEntry* pZIEntry = static_cast<CZoominModelEntry*>(m_pModelContext->Model()->GraphEntry(MD_BASE_ZOOMIN_MODEL));
