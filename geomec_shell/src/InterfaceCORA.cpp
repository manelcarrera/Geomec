...
bool validateModel(cora::CSummaryResultFile &summaryResultFile, CModelData &modelData) {
  if (modelData() == 0) {
...
int getModelInfo(int argc, char *argv[]) {
  ...
  if (!validateModel(getModelInfoData.summaryResultFile(), getModelInfoData.modelData())) {
    return 1;
  }
...
int runModel(int argc, char *argv[]) {
  ...
  if (!validateModel(runModelData.summaryResultFile(), runModelData.modelData())) {
    return 1;
  }
...
CChannelCORA::CChannelCORA(IInterfaceBase *master) : IChannel(master, "cora") {}

const char *CInterfaceCORA::ShortDescription() const { return "CORA"; }
...
void CInterfaceCORA::PrintUsage() const {
  std::cerr << std::endl
            << "geomec_shell <modeldata> " << GETMODELINFO.toStdString() << " [version number] <modelinfofile>"
            << std::endl
...
void handleObjects(CChannelCORA &channel, const cora::TObjects &objects, cora::TFailureModes &failureModes) {
  ...
  for (cora::TObjects::const_iterator object = objects.begin(); object != objects.end(); ++object) {
    for (cora::TFailureModes::iterator failureMode = failureModes.begin(); failureMode != failureModes.end();
         ++failureMode) {
      if ((*failureMode)->getObject() == *object) {
        for (int i = 0; i < 7; ++i)
          channel.AddFailureMechanism((*object)->name().toStdString(), (*object)->getObjectType(),
                                      (*failureMode)->getFailureModeType(), i, values);
      }
    }
    // const cora::TParameters parameters = (*object)->getParameters();
    // for (cora::TParameters::const_iterator parameter = parameters.begin(); parameter != parameters.end();
    // ++parameter)
    //{
